...
```
`--read-only` sessiya yaratmaydi va token ro'yxatga olmaydi. Prod'ga chiqarish = `bash scripts/sync-dars-api.sh main` (build o'zi ketadi) + GitLab'da
`deploy` job'ini qo'lda bosish (biz Maintainer'miz; yoki Kristina) — faqat staging'da §13 sinovi o'tgach. Prod oldidan: sirlar rotatsiyasi (kid v2 + yangi `sapi_`),
