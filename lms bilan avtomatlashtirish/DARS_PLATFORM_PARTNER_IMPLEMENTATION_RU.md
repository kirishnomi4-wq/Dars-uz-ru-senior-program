...
В обоих случаях партнёр получает одинаковый `role = mentor`. Отдельный `role_id` в JWT не передаётся и партнёру не требуется: `sub` всегда содержит `teacher_list.ID` сотрудника, который фактически открыл урок. Куратор, администратор или сотрудник, не назначенный этой группе, mentor JWT не получает.
...
8. Неназначенный сотрудник и куратор не получают mentor JWT.
9. Сессия хранит проверенные `gid`, `teacher_id`, `lesson_id` и `jti`.
10. Ученик своей группы входит автоматически.
11. Ученик другой группы не входит в чужую сессию.
12. F5 возвращает пользователя в ту же сессию и не создаёт дубликат.
13. Любой JSX-материал открывается независимо от названия темы; старые JSX без `liveToken` не ломаются.
14. Если автоматический вход недоступен, старый PIN fallback работает.
15. Новый результат получает `201`.
16. Идентичный повтор получает `200` и `duplicate: true`.
17. Изменённый повтор получает `409`.
18. Частично неверная группа учеников сохраняет корректных и возвращает отклонённых.
19. `GET` по `event_id` подтверждает сохранённое событие.
20. В frontend, URL и логах отсутствуют `sapi_...` токены, JWT secret и исходные JWT.
