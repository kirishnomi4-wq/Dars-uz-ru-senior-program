...
1. Ученик, уже авторизованный в Coddy Camp LMS, входит в Dars-platforma без повторного ввода имени.
2. После урока backend Dars-platforma отправляет в School API результаты учеников для аналитики.
...
```text
Авторизованный пользователь LMS
        |
        | открывает JSX-урок
        v
...
| JWT shared secret | `CODDYCAMP_LIVE_JWT_SECRET` | Проверка подписи входного JWT на backend партнёра |
...
- Повторный вход после F5 с тем же `jti` в ту же сессию разрешён.
...
На текущем LMS student-flow уже реализован: backend Coddy Camp выпускает ученический JWT, а frontend передаёт `liveToken` в JSX.

Для полностью автоматического группового входа без PIN дополнительно нужен mentor-flow, который создаёт на стороне партнёра сессию, привязанную к проверенному `gid`. Пока mentor-flow не подключён, ручной PIN должен остаться fallback. Автоматически убрать только имя уже можно: использовать проверенный `name` из student JWT.
...
7. Сессия хранит проверенные `gid`, `teacher_id`, `lesson_id` и `jti`.
8. Ученик своей группы входит автоматически.
9. Ученик другой группы не входит в чужую сессию.
10. F5 возвращает пользователя в ту же сессию и не создаёт дубликат.
11. Если автоматический вход недоступен, старый PIN fallback работает.
12. Новый результат получает `201`.
13. Идентичный повтор получает `200` и `duplicate: true`.
14. Изменённый повтор получает `409`.
15. Частично неверная группа учеников сохраняет корректных и возвращает отклонённых.
16. `GET` по `event_id` подтверждает сохранённое событие.
17. В frontend, URL и логах отсутствуют `sapi_...` токены и JWT secret.
