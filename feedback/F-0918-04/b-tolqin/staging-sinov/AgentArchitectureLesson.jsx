...
//  Manba:  src/6-Modull/AgentArchitectureLesson.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/6-Modull/AgentArchitectureLesson.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/6-Modull/AgentArchitectureLesson.jsx
...
var ou = (node) => node && typeof node === "object" && !React3.isValidElement(node) ? node.uz ?? node.ru ?? "" : node;
...
var LESSON_META = { lessonId: "agent-arch-06-04-v18", lessonTitle: { uz: "AI-agent nima — detektiv metaforasi", ru: "Что такое AI-агент" } };
var HW_TOKENS = [
...
var INLINE_KEYS = { s4: 1, s8: 2, s11: 0, s14: 3, s15: 0, practice: -1 };
var RECAPS = {
  4: {
...
function RecapOverlay({ screenIdx, onClose }) {
  ...
  return <div className="rc-overlay">
      <div className="rc-head">
        <span className="rc-tag">{tr2({ uz: "📖 Qayta tushuntirish", ru: "📖 Разбираем заново" })}</span>
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr2({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и сколько ✅/❌ — скрыто. По кнопке «Открыть результат» всё откроется разом и у вас, и на экранах учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlash tavsiya etiladi.</>, ru: <>⚠️ Верно только <b>{pct}%</b> — тема осталась непонятной для класса. Перед продолжением стоит коротко повторить.</> })}</p>
              {onOpenRecap && <button className="rc-open" onClick={onOpenRecap}>{tr2({ uz: "📖 Qayta tushuntirish —", ru: "📖 Разбираем заново —" })} {tr2(RECAPS[screenIdx]?.title)}</button>}
            </>}
            {level === "maybe" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>🟡 <b>{pct}%</b> to'g'ri — yomon emas. Xohlasangiz, davom etishdan oldin qisqa takrorlab oling.</>, ru: <>🟡 Верно <b>{pct}%</b> — неплохо. При желании коротко повторите перед продолжением.</> })}</p>
              {onOpenRecap && <button className="rc-open soft" onClick={onOpenRecap}>{tr2({ uz: "📖 Qisqa takrorlash", ru: "📖 Короткое повторение" })}</button>}
            </>}
            {level === "good" && <p className="mstats-verdict-t">{tr2({ uz: <>✅ <b>{pct}%</b> to'g'ri — sinf mavzuni o'zlashtirdi. Bemalol davom eting!</>, ru: <>✅ Верно <b>{pct}%</b> — класс усвоил тему. Спокойно продолжайте!</> })}</p>}
            {level === "few" && <p className="mstats-verdict-t">{tr2({ uz: `Javob berganlar kam (${answered} ta) — foiz bo'yicha xulosa chiqarish qiyin. O'zingiz baholang.`, ru: `Ответивших мало (${answered}) — по проценту вывод сделать трудно. Оцените сами.` })}</p>}
          </div>;
  })()}
      ...
      {reveal && struggling && <p className="mstats-warn">{tr2({ uz: "⚠️ Ko'pchilik xato qildi — bu mavzu tushunarsiz bo'lgan ko'rinadi. Qayta tushuntirish tavsiya etiladi.", ru: "⚠️ Большинство ошиблось — похоже, тема осталась непонятной. Рекомендуем объяснить заново." })}</p>}
...
var QuestionScreen = ({ screen, idx, scope, eyebrow, question, questionText, options, correctIdx, explainCorrect, explainWrong, audioText, audioOk, audioWrong, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const pick = (i) => {
    ...
    if (live && live.recordAttempt && !fpPractice) live.recordAttempt(screen, SCREEN_META[screen]?.id || `s${screen}`, i, Date.now() - mountTs.current, { question: questionText, options: uzOpts, picked: uzOpts[i], correct: uzOpts[correctIdx], lang: typeof __lang !== "undefined" && __lang === "ru" ? "ru" : "uz" });
  };
  ...
  return <div className="dd fade-up">
      <div className="dd-slots">
        {slots.map((sid, i) => <div key={i} ref={(el) => slotRefs.current[i] = el} className={`dd-slot ${sid ? "filled" : ""} ${solved && sid ? "ok" : ""} ${wrong && sid && sid !== order[i] ? "bad" : ""}`}>
            ...
            {sid ? <button key={sid} className="dd-chip in" onPointerDown={(e) => down(e, sid, i)}>{tr2(byId[sid].label)}</button> : <span className="dd-hint">{hints ? tr2(hints[i]) : tr2({ uz: "bu yerga joylang", ru: "поместите сюда" })}</span>}
          </div>)}
      </div>
      ...
      {solved && <div className="dd-done">✓ {tr2(doneText || { uz: "To'g'ri tartib!", ru: "Порядок верный!" })}</div>}
...
var FLOW_ITEMS = FLOW.map((f) => ({ id: f.id, label: { uz: `${f.ico} ${f.label.uz}`, ru: `${f.ico} ${f.label.ru}` } }));
var FLOW_ORDER = FLOW.map((f) => f.id);
var FLOW_HINTS = FLOW.map((f) => ({ uz: `${f.label.uz} bosqichi`, ru: `Этап «${f.label.ru}»` }));
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={{ uz: "Dars · kirish", ru: "Урок · вступление" }} screen={screen} scrollSignal={sc} navContent={<NavNext disabled={picked === null} label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>Shaharga kirdingiz. Bitta iltimos: «do'stimga sovg'a top, 200 minggacha, tez yetkazilsin». Ikki xil <span className="italic" style={{ color: T.accent }}>xizmat</span>.</>, ru: <>Вы приехали в город. Одна просьба: «найди подарок другу, до 200 тысяч, доставить быстро». Две разные <span className="italic" style={{ color: T.accent }}>службы</span>.</> })}</h1>
        ...
        <Zoomable><Split>
          <Col>
            <div className="sk-info" style={{ borderLeft: `4px solid ${T.ink3}` }}>
              <p className="note-h" style={{ color: T.ink2 }}>{tr2({ uz: "💬 Ma'lumot byurosi (faqat javob)", ru: "💬 Справочное бюро (только ответ)" })}</p>
              {tried ? <p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "«Mana mos variantlar: quloqchin (180k), powerbank (150k). O'zingiz tanlab buyurtma bering.»", ru: "«Вот подходящие варианты: наушники (180 тыс.), повербанк (150 тыс.). Выберите и закажите сами.»" })}</p> : <p className="small" style={{ margin: 0, color: T.ink3, fontStyle: "italic" }}>…</p>}
            </div>
            <div className="agent-card">
              <span className="agent-lbl">{tr2({ uz: "🕵️ Detektiv-agent (ishni bajaradi)", ru: "🕵️ Детектив-агент (делает дело)" })}</span>
              {tried ? <p className="agent-msg">{tr2({ uz: "«Topdim ✓ band qildim ✓ tez yetkazishni rasmiylashtirdim ✓ — 35 daqiqada yetkaziladi.»", ru: "«Нашёл ✓ забронировал ✓ оформил быструю доставку ✓ — привезут за 35 минут.»" })}</p> : <p className="agent-msg" style={{ color: T.ink3, fontStyle: "italic" }}>…</p>}
            </div>
            <button className="btn-soft" style={{ alignSelf: "flex-start" }} onClick={poke} disabled={tried}>{tried ? tr2({ uz: "✓ Solishtirildi", ru: "✓ Сравнили" }) : tr2({ uz: "▶ Ikki javobni ko'rish", ru: "▶ Посмотреть два ответа" })}</button>
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Asosiy farq nimada?", ru: "В чём главная разница?" })}</p>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const Preview = <Col>
      <p className="flow-label">{tr2({ uz: "dars oxirida — detektivni arxitekturada joylaysiz", ru: "к концу урока вы разместите детектива в архитектуре" })}</p>
      <div className="clients-map">
        <div className="cm-clients"><div className="cm-client on"><span>📁</span><span className="cm-lbl">{tr2({ uz: "Arxiv", ru: "Архив" })}</span></div><div className="cm-client on"><span>🔬</span><span className="cm-lbl">{tr2({ uz: "Ekspert", ru: "Эксперт" })}</span></div><div className="cm-client on"><span>📡</span><span className="cm-lbl">{tr2({ uz: "Aloqa", ru: "Связь" })}</span></div></div>
        <span className="cm-arrow">←</span>
        <div className="cm-core"><div className="cm-core-node">🕵️<span>{tr2({ uz: "Detektiv", ru: "Детектив" })}</span></div></div>
      </div>
      <div className="sk-info"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Detektiv-agent — backend ichidagi aqlli komponent. Ruxsatnomalar orqali tizimning boshqa qismlariga «qo'l» cho'zadi.", ru: "Детектив-агент — умный компонент внутри бэкенда. По пропускам он «протягивает руку» к другим частям системы." })}</p></div>
    </Col>;
  const StepsB = <Col>
  ...
  return <Stage eyebrow={{ uz: "Reja", ru: "План" }} screen={screen} mentorStatic scrollSignal={showSteps} navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Boshlaymiz →", ru: "Начинаем →" }} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Detektiv-agent — tizimning <span className="italic" style={{ color: T.accent }}>aqlli komponenti</span>.</>, ru: <>Детектив-агент — <span className="italic" style={{ color: T.accent }}>умный компонент</span> системы.</> })}</h2></div>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState3(storedAnswer ? new Set(VS_ROWS.map((r) => r.id)) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={{ uz: "Tushuncha · farq", ru: "Понятие · разница" }} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `4 farqni ko'ring (${seen.size}/4)`, ru: `Посмотрите 4 отличия (${seen.size}/4)` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <><span className="italic" style={{ color: T.accent }}>Ma'lumot byurosi</span> — gapiradi. <span className="italic" style={{ color: T.accent }}>Detektiv</span> — bajaradi.</>, ru: <><span className="italic" style={{ color: T.accent }}>Справочное бюро</span> — говорит. <span className="italic" style={{ color: T.accent }}>Детектив</span> — делает.</> })}</h2></div>
        ...
        <Zoomable><div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {VS_ROWS.map((r) => <button key={r.id} className="gchip" onClick={() => tap(r.id)} style={seen.has(r.id) ? { boxShadow: `inset 0 0 0 1.5px ${T.success}`, color: T.success } : void 0}>{seen.has(r.id) ? "✓ " : ""}{tr2(r.k)}</button>)}
            </div>
          ...
          <Col>
            {cur ? <div className="fade-step" key={active} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <p className="flow-label">{tr2(cur.k)}</p>
                  <div className="frame" style={{ borderLeft: `4px solid ${T.ink3}` }}><p className="note-h" style={{ color: T.ink2 }}>{tr2({ uz: "💬 Ma'lumot byurosi", ru: "💬 Справочное бюро" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2(cur.ai)}</p></div>
                  <div className="agent-card"><span className="agent-lbl">{tr2({ uz: "🕵️ Detektiv", ru: "🕵️ Детектив" })}</span><p className="agent-msg">{tr2(cur.agent)}</p></div>
                </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Jihatni bosing ←", ru: "Нажмите на пункт ←" })}</p></div>}
          </Col>
        </div></Zoomable>
      </div>
    </Stage>;
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={{ uz: "Animatsiya · bir vazifa, ikki yo'l", ru: "Анимация · одна задача, два пути" }} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Farqni ko'ring", ru: "Посмотрите разницу" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bitta vazifa — byuro <span className="italic" style={{ color: T.accent }}>1 qadam</span>, detektiv <span className="italic" style={{ color: T.accent }}>ko'p qadam</span>.</>, ru: <>Одна задача — бюро <span className="italic" style={{ color: T.accent }}>1 шаг</span>, детектив <span className="italic" style={{ color: T.accent }}>много шагов</span>.</> })}</h2></div>
        ...
        <button className="btn" style={{ alignSelf: "flex-start" }} disabled={run} onClick={() => {
    setRun(true);
    setSc((n) => n + 1);
  }}>{run ? tr2({ uz: "✓ Ko'rsatildi", ru: "✓ Показано" }) : tr2({ uz: "▶ Ikki yondashuvni ishga tushir", ru: "▶ Запустить оба подхода" })}</button>
        <Zoomable><div className="split">
          <Col>
            <div className="sk-info" style={{ borderLeft: `4px solid ${T.ink3}` }}>
              <p className="note-h" style={{ color: T.ink2 }}>{tr2({ uz: "💬 Byuro — bir qadam", ru: "💬 Бюро — один шаг" })}</p>
              {run ? <div className="fade-step"><p className="body" style={{ margin: "0 0 8px", color: T.ink }}>{tr2({ uz: "«Mana variantlar: …» → tugadi.", ru: "«Вот варианты: …» → всё." })}</p><span className="loop-badge" style={{ background: T.bg, color: T.ink2 }}>{tr2({ uz: "1 qadam · faqat matn", ru: "1 шаг · только текст" })}</span></div> : <p className="small" style={{ margin: 0, color: T.ink3, fontStyle: "italic" }}>{tr2({ uz: "Tugmani bosing →", ru: "Нажмите кнопку →" })}</p>}
            </div>
          </Col>
          <Col>
            <p className="flow-label" style={{ color: T.accent }}>{tr2({ uz: "🕵️ Detektiv — sikl + ruxsatnomalar", ru: "🕵️ Детектив — цикл + пропуска" })} {run && <span className="loop-badge">↻ loop</span>}</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              {run && AG.map((s, i) => <div key={i} className={`agent-step fade-step ${s.ico === "✅" ? "done" : ""}`} style={{ animationDelay: `${i * 0.45}s` }}>
                  <span className="as-phase">{s.ico} {i < AG.length - 1 ? tr2({ uz: `amal ${i + 1}`, ru: `действие ${i + 1}` }) : tr2({ uz: "tayyor", ru: "готово" })}</span>
                  <span className="as-txt">{tr2(s.txt)}</span>
                </div>)}
              {!run && <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Tugmani bosing — detektiv ishga tushadi", ru: "Нажмите кнопку — детектив возьмётся за дело" })}</p></div>}
            </div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Detektiv 3 ta amal qildi va tizimga ta'sir o'tkazdi. Byuro esa faqat gapirdi. Mana arxitektura farqi.", ru: "Детектив сделал 3 действия и повлиял на систему. А бюро только поговорило. Вот она, архитектурная разница." })}</p></div>}
          </Col>
        </div></Zoomable>
      </div>
    </Stage>;
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [step, setStep] = useState3(storedAnswer ? ENGINE.length : 0);
  const [sc, setSc] = useState3(0);
  ...
  return <Stage eyebrow={{ uz: "Ichki dvigatel", ru: "Внутренний двигатель" }} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `Dvigatelni ko'ring (${step}/${ENGINE.length})`, ru: `Посмотрите двигатель (${step}/${ENGINE.length})` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Detektivning ichida <span className="italic" style={{ color: T.accent }}>dvigatel</span>: kuzat → xulosa → harakat.</>, ru: <>Внутри детектива — <span className="italic" style={{ color: T.accent }}>двигатель</span>: наблюдай → вывод → действие.</> })}</h2></div>
        ...
        <div className="fade-up"><div className="flow-row" style={{ justifyContent: "center" }}>
          {ENGINE.map((p, i) => <React3.Fragment key={p.id}>
              {i > 0 && <span className="fl-track" />}
              <div className={`fl-node ${step > i ? "done" : ""}`}><span className="fl-node-ico">{p.ico}</span><span className="fl-node-lbl">{tr2(p.label)}</span></div>
            </React3.Fragment>)}
          <span className="archloop" style={{ marginLeft: 6 }}>{tr2({ uz: "↺ qayta", ru: "↺ заново" })}</span>
        </div></div>
        <Zoomable><div className="split">
          <Col>
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={done} onClick={advance}>{done ? tr2({ uz: "✓ Tugadi", ru: "✓ Готово" }) : step === 0 ? tr2({ uz: "▶ Dvigatelni yoqish", ru: "▶ Завести двигатель" }) : tr2({ uz: "Keyingi qadam →", ru: "Следующий шаг →" })}</button>
            {step > 0 && <div className="sk-info fade-step" key={step}><p className="body" style={{ margin: 0, color: T.ink }}>{tr2(NOTES[step - 1])}</p></div>}
          </Col>
          <Col>
            <div className="sk-info"><p className="note-h">{tr2({ uz: "🔁 Nega sikl?", ru: "🔁 Почему цикл?" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Har harakatdan keyin detektiv natijani ko'radi va keyingi qadamni tanlaydi — maqsad bajarilguncha. Mana shu sikl agentni avtonom qiladi.", ru: "После каждого действия детектив смотрит результат и выбирает следующий шаг — пока цель не выполнена. Именно этот цикл делает агента самостоятельным." })}</p></div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Bu dvigatel — detektiv komponentining ichida. Tashqaridan siz unga maqsad va ruxsatnomalar berasiz, qolganini o'zi qiladi.", ru: "Этот двигатель — внутри компонента-детектива. Снаружи вы даёте ему цель и пропуска, остальное он делает сам." })}</p></div>}
          </Col>
        </div></Zoomable>
      </div>
    </Stage>;
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={{ uz: "Ulanish · ruxsatnomalar", ru: "Подключение · пропуска" }} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Ruxsatnoma nima?", ru: "Что такое пропуск?" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Detektiv shaharga <span className="italic" style={{ color: T.accent }}>ruxsatnomalar</span> orqali ta'sir qiladi.</>, ru: <>Детектив влияет на город через <span className="italic" style={{ color: T.accent }}>пропуска</span>.</> })}</h2></div>
        ...
        <Zoomable><div className="split">
          <Col>
            <div className="frame" style={{ borderLeft: `4px solid ${T.accent}` }}><p className="note-h" style={{ color: T.accent }}>{tr2({ uz: "🗝️ Ruxsatnoma nima?", ru: "🗝️ Что такое пропуск?" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Ruxsatnoma (tool) — detektiv chaqira oladigan funksiya. U orqali agent tizimning boshqa komponentlariga (idoralarga) ta'sir qiladi.", ru: "Пропуск (tool) — функция, которую детектив может вызвать. Через него агент влияет на другие компоненты системы (ведомства)." })}</p></div>
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={show} onClick={() => {
    setShow(true);
    setSc((n) => n + 1);
  }}>{show ? tr2({ uz: "✓ Ko'rdingiz", ru: "✓ Посмотрели" }) : tr2({ uz: "Ruxsatnomalar tizimga qanday ulanadi?", ru: "Как пропуска подключаются к системе?" })}</button>
          </Col>
          <Col>
            {show ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {TOOLS.map((t) => <div key={t.id} className="sk-info"><p className="body" style={{ margin: 0, color: T.ink }}>{t.ico} <b>{tr2({ uz: <>{tr2(t.label)} ruxsatnomasi:</>, ru: <>Пропуск «{tr2(t.label)}»:</> })}</b> {tr2(t.desc)}</p></div>)}
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Demak detektiv yangi tizim emas — u mavjud idoralaringizni (arxiv/ekspert/aloqa) ruxsatnoma orqali ishlatadi. U — aqlli muvofiqlashtiruvchi.", ru: "Значит детектив — не новая система: он через пропуска пользуется вашими уже существующими ведомствами (архив/эксперт/связь). Он — умный координатор." })}</p></div>}
          </Col>
        </div></Zoomable>
      </div>
    </Stage>;
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState3(storedAnswer ? new Set(TOOLS.map((t) => t.id)) : /* @__PURE__ */ new Set());
  const [active, setActive] = useState3(null);
  const [sc, setSc] = useState3(0);
  ...
  return <Stage eyebrow={{ uz: "Arxitektura · agent o'rni", ru: "Архитектура · место агента" }} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `3 ruxsatnomani oching (${seen.size}/3)`, ru: `Откройте 3 пропуска (${seen.size}/3)` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Detektiv — markazda, <span className="italic" style={{ color: T.accent }}>qo'llari</span> shahar idoralariga cho'ziladi.</>, ru: <>Детектив — в центре, его <span className="italic" style={{ color: T.accent }}>руки</span> тянутся к городским ведомствам.</> })}</h2></div>
        ...
        <div className="fade-up"><div className="clients-map">
          <div className="cm-core"><div className="cm-core-node" style={{ background: T.accent }}>🕵️<span>{tr2({ uz: "Detektiv", ru: "Детектив" })}</span></div></div>
          <span className="cm-arrow">→</span>
          <div className="cm-clients">
            {TOOLS.map((t) => <div key={t.id} className={`cm-client ${seen.has(t.id) ? "on" : ""} ${active === t.id ? "sel" : ""}`}><span>{t.ico}</span><span className="cm-lbl">{tr2(t.label)}</span></div>)}
          </div>
        </div></div>
        <Zoomable><div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {TOOLS.map((t) => <button key={t.id} className="gchip" onClick={() => tap(t.id)} style={seen.has(t.id) ? { boxShadow: `inset 0 0 0 1.5px ${T.success}`, color: T.success } : void 0}>{seen.has(t.id) ? "✓ " : ""}{t.ico} {tr2(t.label)}</button>)}
            </div>
          </Col>
          <Col>
            {cur ? <div className="sk-info fade-step" key={active}><p className="note-h">{cur.ico} {tr2({ uz: <>{tr2(cur.label)} ruxsatnomasi</>, ru: <>Пропуск «{tr2(cur.label)}»</> })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2(cur.desc)}</p></div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Ruxsatnomani bosing ←", ru: "Нажмите на пропуск ←" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Detektiv — bitta komponent, lekin uchta ruxsatnoma orqali butun tizim bilan ishlaydi. Qancha ruxsatnoma bersangiz — shuncha ish qila oladi (ehtiyot bo'lib).", ru: "Детектив — один компонент, но через три пропуска работает со всей системой. Сколько пропусков дадите — столько он и сможет сделать (будьте осторожны)." })}</p></div>}
          </Col>
        </div></Zoomable>
      </div>
    </Stage>;
...
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={{ uz: "Qaror · qachon detektiv", ru: "Решение · когда детектив" }} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Qoidani ko'ring", ru: "Посмотрите правило" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Qachon detektiv, qachon <span className="italic" style={{ color: T.accent }}>byuro</span>?</>, ru: <>Когда детектив, а когда <span className="italic" style={{ color: T.accent }}>бюро</span>?</> })}</h2></div>
        ...
        <Zoomable><div className="split">
          <Col>
            <div className="sk-info" style={{ borderLeft: `4px solid ${T.ink3}` }}><p className="note-h" style={{ color: T.ink2 }}>{tr2({ uz: "💬 Byuro yetadi — qachon?", ru: "💬 Когда хватит бюро?" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Bir martalik, aniq ish: tarjima, matn yozish, manzil, savolga javob. Tizim bilan ko'p qadamli ishlash shart emas.", ru: "Разовое, чёткое дело: перевод, написать текст, адрес, ответ на вопрос. Многошаговая работа с системой не нужна." })}</p></div>
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={show} onClick={() => {
    setShow(true);
    setSc((n) => n + 1);
  }}>{show ? tr2({ uz: "✓ Ko'rdingiz", ru: "✓ Посмотрели" }) : tr2({ uz: "Detektiv qachon kerak?", ru: "Когда нужен детектив?" })}</button>
          </Col>
          <Col>
            {show ? <div className="agent-card fade-step"><span className="agent-lbl">{tr2({ uz: "🕵️ DETEKTIV — QACHON", ru: "🕵️ ДЕТЕКТИВ — КОГДА" })}</span><p className="agent-msg">{tr2({ uz: "Ko'p qadamli, maqsadga yo'naltirilgan, tizim bilan ishlaydigan vazifa: shikoyatni to'liq hal qil, ma'lumot yig'ib qaror qil, o'zi bir necha amal bajar.", ru: "Многошаговая задача с целью и работой по системе: полностью решить жалобу, собрать данные и принять решение, самому выполнить несколько действий." })}</p></div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Tugmani bosing ←", ru: "Нажмите кнопку ←" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Qoida: <b>bir qadam → byuro; ko'p qadamli maqsad → detektiv.</b> Keraksiz joyda detektiv chaqirish — over-engineering.</>, ru: <>Правило: <b>один шаг → бюро; многошаговая цель → детектив.</b> Звать детектива без нужды — over-engineering.</> })}</p></div>}
          </Col>
        </div></Zoomable>
      </div>
    </Stage>;
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [idx, setIdx] = useState3(storedAnswer ? TASKS.length : 0);
  ...
  const choose = (ans) => {
    ...
    if (ans === cur.ans) {
      ...
      setWrong(ans);
      setTimeout(() => setWrong((w) => w === ans ? null : w), 450);
    }
  };
  return <Stage eyebrow={{ uz: "Mashq · qaysi birini", ru: "Практика · кого выбрать" }} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `Tanlang (${idx}/${TASKS.length})`, ru: `Выберите (${idx}/${TASKS.length})` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Har vazifaga: <span className="italic" style={{ color: T.accent }}>byuro</span> yoki <span className="italic" style={{ color: T.accent }}>detektiv</span>?</>, ru: <>На каждую задачу: <span className="italic" style={{ color: T.accent }}>бюро</span> или <span className="italic" style={{ color: T.accent }}>детектив</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: "Endi o'zingiz qaror qiling. Har vazifani o'qing: u bir martalik ishmi (byuro) yoki ko'p qadamli, tizim bilan ishlaydigan maqsadmi (detektiv)?", ru: "Теперь решайте сами. Читайте каждую задачу: это разовое дело (бюро) или многошаговая цель с работой по системе (детектив)?" })}</Mentor>
        <Zoomable><div className="split">
          <Col>
            {cur ? <div className="frame" key={cur.id} style={{ borderLeft: `4px solid ${T.accent}` }}><p className="note-h" style={{ color: T.accent }}>{tr2({ uz: "🧩 Vazifa", ru: "🧩 Задача" })} {idx + 1}/{TASKS.length}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2(cur.task)}</p></div> : <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Hammasi to'g'ri! Endi vazifaga qarab to'g'ri vositani tanlay olasiz — bu arxitektorning muhim qarori.", ru: "Всё верно! Теперь вы умеете подбирать инструмент под задачу — это важное решение архитектора." })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "qaysi birini chaqirasiz?", ru: "кого позовёте?" })}</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <button className={`pick-row ${wrong === "ai" ? "shake" : ""}`} disabled={done} onClick={() => choose("ai")}><span style={{ marginRight: 6 }}>💬</span><span style={{ flex: 1 }}>{tr2({ uz: <>Ma'lumot byurosi <span style={{ color: T.ink3, fontWeight: 500 }}>· bir martalik</span></>, ru: <>Справочное бюро <span style={{ color: T.ink3, fontWeight: 500 }}>· разово</span></> })}</span><span className="pick-plus">+</span></button>
              <button className={`pick-row ${wrong === "agent" ? "shake" : ""}`} disabled={done} onClick={() => choose("agent")}><span style={{ marginRight: 6 }}>🕵️</span><span style={{ flex: 1 }}>{tr2({ uz: <>Detektiv <span style={{ color: T.ink3, fontWeight: 500 }}>· ko'p qadamli maqsad</span></>, ru: <>Детектив <span style={{ color: T.ink3, fontWeight: 500 }}>· многошаговая цель</span></> })}</span><span className="pick-plus">+</span></button>
            </div>
            {wrong && !done && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Qaytadan o'ylang: bu bir martalik ishmi yoki ko'p qadamli maqsadmi?", ru: "Подумайте ещё раз: это разовое дело или многошаговая цель?" })}</p></div>}
          </Col>
        </div></Zoomable>
      </div>
    </Stage>;
...
var Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [shown, setShown] = useState3(storedAnswer ? CASE_STEPS.length : 0);
  const [sc, setSc] = useState3(0);
  ...
  return <Stage eyebrow={{ uz: "Hayotiy · shahar detektivi", ru: "Из жизни · городской детектив" }} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `Detektivni kuzating (${shown}/${CASE_STEPS.length})`, ru: `Понаблюдайте за детективом (${shown}/${CASE_STEPS.length})` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Shahar detektivi — vazifadan <span className="italic" style={{ color: T.accent }}>natijagacha</span> o'zi.</>, ru: <>Городской детектив — от задачи <span className="italic" style={{ color: T.accent }}>до результата</span> сам.</> })}</h2></div>
        ...
        <Zoomable><div className="split">
          <Col>
            <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              {CASE_STEPS.slice(0, shown).map((s, i) => <div key={i} className={`agent-step fade-step ${s.ico === "✅" ? "done" : ""}`}>
                  <span className="as-phase">{s.ico} {s.ico === "🎯" ? tr2({ uz: "vazifa", ru: "задача" }) : s.ico === "👁️" ? tr2({ uz: "kuzat", ru: "наблюдай" }) : s.ico === "🧠" ? tr2({ uz: "xulosa", ru: "вывод" }) : s.ico === "🚶" ? tr2({ uz: "harakat (ruxsatnoma)", ru: "действие (пропуск)" }) : tr2({ uz: "tayyor", ru: "готово" })}</span>
                  <span className="as-txt">{tr2(s.txt)}</span>
                </div>)}
              {shown === 0 && <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Tugmani bosing — detektiv ishga tushadi.", ru: "Нажмите кнопку — детектив возьмётся за дело." })}</p></div>}
            </div>
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={done} onClick={advance}>{done ? tr2({ uz: "✓ Vazifaga yetildi", ru: "✓ Цель достигнута" }) : shown === 0 ? tr2({ uz: "▶ Detektivga vazifa berish", ru: "▶ Дать детективу задачу" }) : tr2({ uz: "Keyingi qadam →", ru: "Следующий шаг →" })}</button>
          </Col>
          <Col>
            <div className="sk-info"><p className="note-h">{tr2({ uz: "🗝️ Ishlatilgan ruxsatnomalar", ru: "🗝️ Использованные пропуска" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{usedTools === 0 ? tr2({ uz: "hali yo'q", ru: "пока нет" }) : tr2({ uz: `${usedTools} ta ruxsatnoma ishlatildi (arxiv, aloqa) — har biri tizimga ta'sir qildi.`, ru: `Использовано пропусков: ${usedTools} (архив, связь) — каждый повлиял на систему.` })}</p></div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Siz faqat vazifa berdingiz. Detektiv kuzat→xulosa→harakat sikli bilan ruxsatnomalarni ishlatib, ishni bajardi. Mana detektivning arxitekturadagi kuchi.", ru: "Вы только дали задачу. Детектив по циклу наблюдай→вывод→действие использовал пропуска и довёл дело до конца. Вот в чём сила детектива в архитектуре." })}</p></div>}
          </Col>
        </div></Zoomable>
      </div>
    </Stage>;
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={{ uz: "Ehtiyot · vakolat chegarasi", ru: "Осторожно · рамки полномочий" }} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Nega chegara?", ru: "Зачем рамки?" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Detektiv <span className="italic" style={{ color: T.accent }}>amal qiladi</span> — demak vakolat chegarasi kerak.</>, ru: <>Детектив <span className="italic" style={{ color: T.accent }}>действует</span> — значит нужны рамки полномочий.</> })}</h2></div>
        ...
        <Zoomable><div className="split">
          <Col>
            <div className="frame" style={{ borderLeft: `4px solid ${T.accent}` }}><p className="note-h" style={{ color: T.accent }}>{tr2({ uz: "🕵️ Detektiv backend ichida", ru: "🕵️ Детектив внутри бэкенда" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Arxitekturada detektiv — backend komponenti. U faqat siz bergan ruxsatnomalarga ega; bermagan ishingizni qila olmaydi.", ru: "В архитектуре детектив — компонент бэкенда. У него есть только выданные вами пропуска; то, чего вы не дали, он сделать не может." })}</p></div>
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={show} onClick={() => {
    setShow(true);
    setSc((n) => n + 1);
  }}>{show ? tr2({ uz: "✓ Tushundim", ru: "✓ Понятно" }) : tr2({ uz: "Qanday chegara?", ru: "Какие рамки?" })}</button>
          </Col>
          <Col>
            {show ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <div className="sk-info"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>🧾 <b>Cheklangan ruxsatnomalar:</b> faqat kerakli ruxsatnomalarni bering (o'chirish/to'lovni — yo'q).</>, ru: <>🧾 <b>Ограниченные пропуска:</b> выдавайте только нужные пропуска (удаление/оплату — нет).</> })}</p></div>
                  <div className="sk-info"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>✋ <b>Tasdiq:</b> xavfli amaldan oldin odam tasdig'ini so'rasin (order kerak).</>, ru: <>✋ <b>Подтверждение:</b> перед опасным действием пусть спросит разрешение человека (нужен ордер).</> })}</p></div>
                </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Tugmani bosing ←", ru: "Нажмите кнопку ←" })}</p></div>}
            {done && <div className="agent-card fade-step"><span className="agent-lbl">{tr2({ uz: "📍 KEYINGI DARS", ru: "📍 СЛЕДУЮЩИЙ УРОК" })}</span><p className="agent-msg">{tr2({ uz: <>Agent va AI xulqini qanday <b>aniq shakllantirish</b> mumkin? Buni <b>Claude Skills</b> bilan qilamiz — keyingi darsda.</>, ru: <>Как <b>чётко задать</b> поведение агента и ИИ? Сделаем это через <b>Claude Skills</b> — на следующем уроке.</> })}</p></div>}
          </Col>
        </div></Zoomable>
      </div>
    </Stage>;
...
var Screen14 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={{ uz: "Mashq · 4-savol", ru: "Практика · вопрос 4" }}
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={{ uz: "Yakuniy · amaliy", ru: "Итог · практика" }} screen={screen} scrollSignal={solved ? 1 : 0} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!solved} label={solved ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Oqimni yig'ing", ru: "Соберите поток" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Oxirgi qadam: detektiv ish oqimini <span className="italic" style={{ color: T.accent }}>to'g'ri tartibda</span> yig'ing.</>, ru: <>Последний шаг: соберите рабочий поток детектива <span className="italic" style={{ color: T.accent }}>в правильном порядке</span>.</> })}</h2></div>
        ...
        <Zoomable><div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "detektiv oqimi (siz yig'yapsiz)", ru: "поток детектива (собираете вы)" })}</p>
...
var ACH_TRIGGERS = { s4: "theAgent", s8: "accessPass", s14: "withWarrant", s15: "missionLoop" };
  ...
  return <div className="qz-arena">
    ...
    return <div key={i} className={`qz-tile rv ${win ? "win" : "lose"} ${pickedThis ? "picked" : ""}`} style={{ background: QUIZ_COLORS[i] }}>
          ...
          {!isMentor && <div className={`qz-res ${my?.correct ? "good" : "bad"}`}>
              {my?.correct ? <><span className="qz-res-pts">+{myPtsFor(qi)}</span><span className="qz-res-t">{tr2({ uz: "ball", ru: "баллов" })}{streakUpTo(qi) >= 2 ? ` · 🔥 x${streakUpTo(qi)} streak` : ""}</span></> : <span className="qz-res-t">{my ? tr2({ uz: "Adashdingiz — 0 ball. Keyingisida olasiz! 💪", ru: "Ошибка — 0 баллов. В следующий раз получится! 💪" }) : tr2({ uz: "Vaqt tugadi — 0 ball. Tezroq bo'ling! ⏱", ru: "Время вышло — 0 баллов. Будьте быстрее! ⏱" })}</span>}
      ...
      {phase === "done" && <div className="qz-view fade-step">
          ...
          {solo ? <div className="qz-solo-res">
              ...
              <p className="qz-sub">{tr2({ uz: `ball · ${soloScore.ok}/${QUIZ_BANK.length} to'g'ri`, ru: `баллов · ${soloScore.ok}/${QUIZ_BANK.length} верно` })}{soloScore.maxStreak >= 2 ? tr2({ uz: ` · eng uzun streak 🔥x${soloScore.maxStreak}`, ru: ` · самая длинная серия 🔥x${soloScore.maxStreak}` }) : ""}</p>
...
function ScreenLivePractice({ title, task, checklist, screen, storedAnswer, onAnswer, onNext, onPrev, live, eyebrow, place }) {
  ...
  return <Stage eyebrow={eyebrow || { uz: "Amaliyot", ru: "Практика" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Avval bajaring", ru: "Сначала выполните" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2(title)}</h2></div>
        <Mentor>{tr2({ uz: <>Bu topshiriqni <b style={{ color: T.ink }}>o'z {tr2(PLACE)}</b> bajaring. Har bosqichni bajarib, belgilab boring. Tugagach <b style={{ color: T.ink }}>«Bajardim»</b> tugmasini bosing — ustoz kuzatib turadi.</>, ru: <>Выполните это задание <b style={{ color: T.ink }}>{tr2(PLACE)}</b>. Делайте шаг за шагом и отмечайте их. Закончив, нажмите <b style={{ color: T.ink }}>«Сделал»</b> — наставник следит.</> })}</Mentor>
        <div className="split">
          <Col>
    ...
    return <button key={i} className={`lp-step ${on ? "on" : ""}`} onClick={() => toggle(i)}>
            ...
            <button className={`lp-done-btn ${done ? "is-done" : ""}`} disabled={done} onClick={complete}>
              {done ? tr2({ uz: "✓ Bajarildi — ustozni kuting", ru: "✓ Готово — ждите наставника" }) : tr2({ uz: "✅ Bajardim", ru: "✅ Сделал" })}
            </button>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Zo'r! Vazifani bajardingiz. Ustoz tekshirib, keyingi qadamga o'tkazadi.", ru: "Отлично! Задание выполнено. Наставник проверит и переведёт на следующий шаг." })}</p></div>}
          </Col>
        </div>
      </div>
    </Stage>;
...
var ScreenAgentPractice = (props) => <ScreenLivePractice
  {...props}
  eyebrow={{ uz: "Amaliyot · dizayn", ru: "Практика · проектирование" }}
  place={{ uz: "daftaringizda", ru: "у себя в тетради" }}
...
var SummaryScreen = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={{ uz: "Tayyor", ru: "Готово" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "Agentning o'rnini tushundingiz", ru: "Вы поняли место агента" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Detektiv-agent — tizimning <span className="italic" style={{ color: T.accent }}>aqlli komponenti</span>.</>, ru: <>Детектив-агент — <span className="italic" style={{ color: T.accent }}>умный компонент</span> системы.</> })}</h2>{
    /* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */
        ...
        <div className={`qz-cta cs-cta fade-up d2 ${studentLive ? "ready" : ""}`}>
          <CsWordmark stats={false} liveOn={studentLive} disabled={studentWait} onClick={studentWait ? void 0 : openArena} hint={studentWait ? { uz: "⏳ Mentorni kuting", ru: "⏳ Подождите ментора" } : void 0} />
        </div>
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>{tr2({ uz: "📝 Uyga vazifa", ru: "📝 Домашнее задание" })}</div><ul>{HOMEWORK.map((h, i) => <li key={i}><b>{tr2(h.b)}</b> <span className="t">{tr2(h.t)}</span></li>)}</ul><p className="hw-note">{tr2({ uz: "🚀 Keyingi dars — Claude Skills: AI va agent xulqini aniq shakllantirish.", ru: "🚀 Следующий урок — Claude Skills: как чётко задать поведение ИИ и агента." })}</p></div>}
...
function AgentArchitectureLesson({ lang: langProp, onFinished, liveToken }) {
...
/* PRODUCTION: shu @import OLIB TASHLANADI — shriftlarni LMS yuklaydi (platform_contract). */
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={{ uz: "AI-agent darsi", ru: "Урок про AI-агента" }} /> : <>
...
export {
  AgentArchitectureLesson as default
};
