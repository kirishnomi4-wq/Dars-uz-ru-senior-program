...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up" style={{ maxWidth: 760 }}>{tr3({ uz: <>Turniket <span className="italic" style={{ color: T.accent }}>qachon</span> ochiladi?</>, ru: <><span className="italic" style={{ color: T.accent }}>Когда</span> откроется турникет?</> })}</h1>
        ...
        <Split>
          <Col>
            <p className="flow-label">{tr3({ uz: "Yoshingizni tanlang", ru: "Выберите свой возраст" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8 }}>
              {[8, 12, 15].map((a) => <button key={a} className={`chip ${age === a ? "chip-on" : ""}`} onClick={() => setAge(a)}>{a} {tr3({ uz: "yosh", ru: "лет" })}</button>)}
            </div>
            <div style={{ background: allowed ? T.successSoft : T.accentSoft, borderRadius: 16, padding: "22px 16px", textAlign: "center", boxShadow: `0 8px 20px -6px rgba(${T.shadowBase},0.16)`, transition: "background 0.35s ease" }}>
              ...
              <p className="demo-swap" key={age} style={{ fontFamily: "Georgia, serif", fontWeight: 700, color: allowed ? T.success : T.accent, margin: "16px 0 2px", fontSize: "clamp(16px,2.4vw,20px)" }}>{allowed ? tr3({ uz: "✅ Turniket ochildi!", ru: "✅ Турникет открылся!" }) : tr3({ uz: "⛔ Turniket yopiq", ru: "⛔ Турникет закрыт" })}</p>
              <p className="mono small" style={{ color: T.ink2, margin: 0 }}>yosh = {age} · {age} {">="} 12 → {String(allowed)}</p>
            </div>
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr3({ uz: "Dastur turniketni qanday hal qiladi?", ru: "Как программа решает вопрос турникета?" })}</p>
    ...
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null} onClick={() => pick(o.id)}>
            ...
            {picked !== null && <p className="hook-ack fade-step">{tr3({ uz: <>To'g'ri yo'nalish! Dastur <b>shartni</b> tekshiradi. Buni <span className="mono">if</span> bilan yozamiz — bugun shuni o'rganamiz.</>, ru: <>Верное направление! Программа проверяет <b>условие</b>. Мы запишем это через <span className="mono">if</span> — этому сегодня и научимся.</> })}</p>}
          </Col>
        </Split>
        </Zoomable>
      </div>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const PreviewBlock = <Col>
      ...
      <pre className="code-box fade-up" style={{ fontSize: "clamp(12.5px,1.9vw,14.5px)" }}><Kw>if</Kw> (<Vr>yosh</Vr> <Op>{">="}</Op> <Nm>12</Nm>) {"{"}{"\n"}{"  "}<Vr>console</Vr>.<Vr>log</Vr>(<St>{tr3({ uz: '"Kiring!"', ru: '"Входите!"' })}</St>){"\n"}{"}"} <Kw>else</Kw> {"{"}{"\n"}{"  "}<Vr>console</Vr>.<Vr>log</Vr>(<St>{tr3({ uz: `"Ruxsat yo'q"`, ru: '"Нет доступа"' })}</St>){"\n"}{"}"}</pre>
      ...
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            ...
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8 }}>
              {[10, 15].map((a) => <button key={a} className={`chip ${age === a ? "chip-on" : ""}`} onClick={() => setA(a)}>{a} {tr3({ uz: "yosh", ru: "лет" })}</button>)}
            </div>
            <pre className="code-box fade-up delay-2" style={{ fontSize: "clamp(13px,2vw,15px)" }}>
              ...
              <span style={{ background: cond ? "rgba(31,122,77,0.25)" : "transparent", borderRadius: 4, opacity: cond ? 1 : 0.4 }}>{"  "}<Vr>console</Vr>.<Vr>log</Vr>(<St>{tr3({ uz: '"Kiring!"', ru: '"Входите!"' })}</St>)</span>{"\n"}
              {"}"}
            </pre>
          </Col>
          <Col>
            ...
            <div className="demo-swap" key={age + "r"} style={{ background: T.paper, borderRadius: 14, padding: "18px", textAlign: "center", boxShadow: `0 8px 20px -6px rgba(${T.shadowBase},0.14)` }}>
              <BoolPill value={cond} />
              <p className="body" style={{ margin: "12px 0 0", color: T.ink }}>{cond ? tr3({ uz: "✅ Shart rost → kod ishladi: «Kiring!»", ru: "✅ Условие истинно → код сработал: «Входите!»" }) : tr3({ uz: "⛔ Shart yolg'on → kod o'tkazib yuborildi", ru: "⛔ Условие ложно → код пропущен" })}</p>
            </div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>✓ <b>if</b> — bu darvoza: shart <b>true</b> bo'lsagina ichidagi kodga yo'l ochiladi.</>, ru: <>✓ <b>if</b> — это ворота: путь к коду внутри открывается, только если условие <b>true</b>.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [age, setAge] = useState4(10);
  const [seen, setSeen] = useState4(/* @__PURE__ */ new Set([10]));
  const allowed = age >= 12;
        ...
        <div className="split">
          <Col>
            ...
            <pre className="code-box fade-up delay-2" style={{ fontSize: "clamp(12.5px,1.9vw,14.5px)" }}>
              ...
              <span style={{ background: allowed ? "rgba(31,122,77,0.25)" : "transparent", borderRadius: 4, opacity: allowed ? 1 : 0.4, transition: "background 0.35s, opacity 0.35s" }}>{"  "}<Vr>console</Vr>.<Vr>log</Vr>(<St>{tr3({ uz: '"Kiring!"', ru: '"Входите!"' })}</St>)</span>{"\n"}
              {"}"} <Kw>else</Kw> {"{"}{"\n"}
              <span style={{ background: !allowed ? "rgba(255,79,40,0.22)" : "transparent", borderRadius: 4, opacity: !allowed ? 1 : 0.4, transition: "background 0.35s, opacity 0.35s" }}>{"  "}<Vr>console</Vr>.<Vr>log</Vr>(<St>{tr3({ uz: `"Ruxsat yo'q"`, ru: '"Нет доступа"' })}</St>)</span>{"\n"}
              {"}"}
            </pre>
          </Col>
          <Col>
            ...
            <div className="demo-swap" key={age} style={{ background: allowed ? T.successSoft : T.accentSoft, borderRadius: 14, padding: "20px", textAlign: "center", boxShadow: `0 8px 20px -6px rgba(${T.shadowBase},0.14)`, transition: "background 0.35s" }}>
              ...
              <p className="mono" style={{ margin: "8px 0 0", fontWeight: 700, color: allowed ? T.success : T.accent }}>{allowed ? tr3({ uz: '"Kiring!"', ru: '"Входите!"' }) : tr3({ uz: `"Ruxsat yo'q"`, ru: '"Нет доступа"' })}</p>
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>✓ <b>if / else</b> — ikki yo'lli ayri: rost bo'lsa biri, yolg'on bo'lsa ikkinchisi. Hech qachon ikkalasi birga emas.</>, ru: <>✓ <b>if / else</b> — развилка на два пути: истина — один, ложь — другой. Никогда оба сразу.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
        ...
        <div className="split">
          <Col>
            <button onClick={() => tap("assign")} className="fade-up delay-1" style={{ textAlign: "left", cursor: "pointer", border: "none", width: "100%", borderRadius: 14, padding: "15px 17px", background: T.paper, boxShadow: active === "assign" ? `inset 0 0 0 2px ${T.accent}, 0 8px 20px -6px rgba(255,79,40,0.22)` : `0 6px 16px -6px rgba(${T.shadowBase},0.14)`, transition: "all 0.18s", marginBottom: 10 }}>
              ...
              <p className="small" style={{ margin: 0, color: T.ink2 }}>📥 {tr3({ uz: `O'zlashtirish — "10 ni ball qutisiga sol"`, ru: "Присваивание — «положи 10 в коробку ball»" })} {seen.has("assign") && "✓"}</p>
            </button>
            <button onClick={() => tap("compare")} className="fade-up delay-1" style={{ textAlign: "left", cursor: "pointer", border: "none", width: "100%", borderRadius: 14, padding: "15px 17px", background: T.paper, boxShadow: active === "compare" ? `inset 0 0 0 2px ${T.accent}, 0 8px 20px -6px rgba(255,79,40,0.22)` : `0 6px 16px -6px rgba(${T.shadowBase},0.14)`, transition: "all 0.18s" }}>
              <p className="mono" style={{ margin: "0 0 6px", fontSize: 16, color: T.ink }}><Vr>ball</Vr> <Op>===</Op> <Nm>10</Nm></p>
              <p className="small" style={{ margin: 0, color: T.ink2 }}>❓ {tr3({ uz: 'Taqqoslash — "ball 10 ga tengmi?" → true/false', ru: "Сравнение — «ball равно 10?» → true/false" })} {seen.has("compare") && "✓"}</p>
            </button>
          </Col>
          <Col>
            {active ? <div className="sk-info fade-step" key={active}>
                {active === "assign" ? <><span className="sk-tagbig"><span className="sk-wordbadge mono">=</span><span style={{ fontWeight: 600, color: T.ink }}>{tr3({ uz: "o'zlashtirish", ru: "присваивание" })}</span></span><p className="body" style={{ color: T.ink, margin: "10px 0 0" }}>{tr3({ uz: "Quti yaratganda ishlatamiz (1-darsdan). Hech narsa tekshirmaydi — shunchaki qiymat soladi.", ru: "Используем при создании коробки (с 1-го урока). Ничего не проверяет — просто кладёт значение." })}</p><div className="pop-in" style={{ marginTop: 12, display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}><span className="mono" style={{ fontWeight: 700, color: T.ink }}>10</span><span className="flow-x" style={{ color: T.accent, fontWeight: 700, fontSize: 18 }}>→</span><span className="var-box" style={{ minWidth: 92 }}><span className="var-name">ball</span><span className="var-val" style={{ fontSize: 18, color: T.ink }}>10</span></span></div></> : <><span className="sk-tagbig"><span className="sk-wordbadge mono">===</span><span style={{ fontWeight: 600, color: T.ink }}>{tr3({ uz: "taqqoslash", ru: "сравнение" })}</span></span><p className="body" style={{ color: T.ink, margin: "10px 0 4px" }}>{tr3({ uz: <>Ikki qiymatni solishtiradi va <b>true</b> yoki <b>false</b> qaytaradi.</>, ru: <>Сравнивает два значения и возвращает <b>true</b> или <b>false</b>.</> })}</p><div className="pop-in" style={{ margin: "10px 0", display: "flex", alignItems: "center", justifyContent: "center", gap: 11, flexWrap: "wrap" }}><span className="mono" style={{ color: T.ink2 }}>10 === 10</span><span className="flow-x" style={{ color: T.ink3, fontSize: 18 }}>→</span><BoolPill value={true} pulse /></div><p className="body" style={{ margin: 0, color: T.ink }}><b style={{ color: T.accent }}>{tr3({ uz: "Shartlarda (if) doim shu — ===.", ru: "В условиях (if) всегда он — ===." })}</b></p></>}
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <><b>Qoida:</b> qiymat berish — <span className="mono">=</span>, tekshirish — <span className="mono">===</span>. Aralashtirmang!</>, ru: <><b>Правило:</b> положить значение — <span className="mono">=</span>, проверить — <span className="mono">===</span>. Не путайте!</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
        ...
        <div className="split">
          ...
          <Col>
            ...
            <div className="demo-swap" key={age} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "6px 0" }}>
              ...
              <div className={`var-box ${val ? "ring-green" : "ring-red"}`} style={{ minWidth: 150 }}><div className="var-name">{tr3({ uz: "o'zgaruvchi: katta", ru: "переменная: katta" })}</div><div className="var-val pop-num" style={{ color: val ? T.success : T.accent }}>{String(val)}</div></div>
            </div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>✓ Ko'rdingizmi — <span className="mono">yosh {">="} 18</span> shunchaki <b>true/false</b>. Shart va boolean — bir narsa!</>, ru: <>✓ Видите — <span className="mono">yosh {">="} 18</span> это просто <b>true/false</b>. Условие и boolean — одно и то же!</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [age, setAge] = useState4(10);
  const [ticket, setTicket] = useState4(false);
  const [touch, setTouch] = useState4(0);
  const ageOk = age >= 12;
  const done = touch >= 2;
  const bump = (fn) => {
    fn();
  ...
  return <Stage eyebrow={tr3({ uz: "Ichma-ich shart", ru: "Вложенное условие" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Sozlamalarni sinang", ru: "Попробуйте настройки" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Bir shart ichida <span className="italic" style={{ color: T.accent }}>yana bir shart</span> bo'lsa-chi?</>, ru: <>А если внутри условия — <span className="italic" style={{ color: T.accent }}>ещё одно условие</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Attraksionga kirish uchun: avval <b style={{ color: T.ink }}>yosh</b> yetarli, keyin <b style={{ color: T.ink }}>chipta</b> ham kerak. Demak <span className="mono">if</span> ichiga yana <span className="mono">if</span> yozamiz — <b style={{ color: T.ink }}>ichma-ich shart</b>. Sozlamalarni o'zgartiring.</>, ru: <>Чтобы попасть на аттракцион: сначала подходит <b style={{ color: T.ink }}>возраст</b>, потом нужен ещё и <b style={{ color: T.ink }}>билет</b>. Значит внутрь <span className="mono">if</span> пишем ещё один <span className="mono">if</span> — <b style={{ color: T.ink }}>вложенное условие</b>. Меняйте настройки.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}><span className="flow-label" style={{ minWidth: 54 }}>{tr3({ uz: "Yosh", ru: "Возраст" })}</span>{[10, 15].map((a) => <button key={a} className={`chip ${age === a ? "chip-on" : ""}`} onClick={() => bump(() => setAge(a))}>{a}</button>)}</div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}><span className="flow-label" style={{ minWidth: 54 }}>{tr3({ uz: "Chipta", ru: "Билет" })}</span><button className={`chip ${ticket ? "chip-on" : ""}`} onClick={() => bump(() => setTicket((t) => !t))}>{ticket ? tr3({ uz: "🎫 Bor", ru: "🎫 Есть" }) : tr3({ uz: "✖ Yo'q", ru: "✖ Нет" })}</button></div>
            </div>
            <pre className="code-box fade-up delay-2" style={{ fontSize: "clamp(11px,1.7vw,13px)" }}>
              ...
              <span style={{ display: "block", opacity: ageOk ? 1 : 0.4, transition: "opacity 0.35s" }}>{"  "}<Kw>if</Kw> (<Vr>chiptaBor</Vr>) {"{"} <Cm>{tr3({ uz: "kir", ru: "проходи" })}</Cm> {"}"}</span>
              <span style={{ display: "block", opacity: ageOk ? 1 : 0.4, transition: "opacity 0.35s" }}>{"  "}<Kw>else</Kw> {"{"} <Cm>{tr3({ uz: "chipta ol", ru: "купи билет" })}</Cm> {"}"}</span>
              <span style={{ display: "block" }}>{"}"} <Kw>else</Kw> {"{"} <Cm>{tr3({ uz: "ruxsat yo'q", ru: "нет доступа" })}</Cm> {"}"}</span>
            </pre>
          </Col>
          <Col>
            <p className="flow-label">{tr3({ uz: "Ikkala shart ham kerak", ru: "Нужны оба условия" })}</p>
            <div className="fade-up" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <span className="mono" style={{ fontWeight: 700, fontSize: 13, padding: "6px 12px", borderRadius: 99, background: ageOk ? T.successSoft : T.accentSoft, color: ageOk ? T.success : T.accent, transition: "all 0.3s" }}>{ageOk ? "✓" : "✗"} {tr3({ uz: "yosh", ru: "возраст" })} ≥ 12</span>
              <span className="mono" style={{ fontWeight: 700, fontSize: 13, padding: "6px 12px", borderRadius: 99, background: ticket ? T.successSoft : T.accentSoft, color: ticket ? T.success : T.accent, transition: "all 0.3s" }}>{ticket ? "✓" : "✗"} {tr3({ uz: "chipta", ru: "билет" })}</span>
            </div>
            <div className="demo-swap" key={`${age}-${ticket}`} style={{ background: T.paper, borderRadius: 14, padding: "20px", textAlign: "center", boxShadow: `0 8px 20px -6px rgba(${T.shadowBase},0.14)` }}>
              ...
              <p style={{ fontFamily: "Georgia, serif", fontWeight: 700, margin: "8px 0 0", color: result.c, fontSize: "clamp(15px,2.2vw,18px)" }}>{tr3(result.t)}</p>
            </div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>✓ <b>Ichma-ich shart</b>: tashqi if rost bo'lsagina, ichki if tekshiriladi. Bosqichma-bosqich qaror.</>, ru: <>✓ <b>Вложенное условие</b>: внутренний if проверяется, только если внешний истинен. Решение шаг за шагом.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen12 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr3({ uz: "if ichiga yana bitta if yozilsa, bu nima deyiladi?", ru: "Как называется if, записанный внутри другого if?" })}</h2></>}
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Amaliyot · shart tuz", ru: "Практика · соберите условие" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Shart tuzing", ru: "Соберите условие" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Turniket shartini <span className="italic" style={{ color: T.accent }}>o'zingiz tuzing</span></>, ru: <>Соберите условие турникета <span className="italic" style={{ color: T.accent }}>сами</span></> })}</h2></div>
        <Mentor>{tr3({ uz: <>Endi o'zingiz shart tuzasiz. <b style={{ color: T.ink }}>Operator</b> va <b style={{ color: T.ink }}>son</b>ni tanlang — turniket shartini quring. So'ng yoshlar bilan sinab ko'ring.</>, ru: <>Теперь вы сами соберёте условие. Выберите <b style={{ color: T.ink }}>оператор</b> и <b style={{ color: T.ink }}>число</b> — постройте условие турникета. Потом проверьте его на разных возрастах.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            ...
            <p className="flow-label">{tr3({ uz: "Son (yosh chegarasi)", ru: "Число (порог возраста)" })}</p>
          ...
          <Col>
            ...
            {ready ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <div style={{ display: "flex", gap: 7 }}>{[8, 12, 16].map((t) => <button key={t} className={`chip ${test === t ? "chip-on" : ""}`} onClick={() => setTest(t)}>{t} {tr3({ uz: "yosh", ru: "лет" })}</button>)}</div>
                {test !== null && <div className={`demo-swap ${evalCond(test) ? "ring-green" : "ring-red"}`} key={test} style={{ background: evalCond(test) ? T.successSoft : T.accentSoft, borderRadius: 14, padding: "16px", textAlign: "center", boxShadow: `0 8px 20px -6px rgba(${T.shadowBase},0.14)` }}>
                    ...
                    <p className="pop-num" style={{ fontWeight: 700, margin: 0, color: evalCond(test) ? T.success : T.accent }}>{evalCond(test) ? tr3({ uz: "✅ Turniket ochildi", ru: "✅ Турникет открылся" }) : tr3({ uz: "⛔ Yopiq", ru: "⛔ Закрыт" })}</p>
                  </div>}
                <div className="frame-success"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: "✓ Shartingiz tayyor! Yoshlarni sinab, qanday ishlashini ko'ring.", ru: "✓ Ваше условие готово! Проверьте разные возрасты и посмотрите, как оно работает." })}</p></div>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  />
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>📝 {tr3({ uz: "Uyga vazifa", ru: "Домашнее задание" })}</div><p className="body" style={{ margin: "0 0 10px", color: T.ink }}>{tr3({ uz: "Quyidagi shartlarni yozib ko'ring:", ru: "Попробуйте написать эти условия:" })}</p><ul>{HOMEWORK.map((h, i) => <li key={i}><b className="mono">{h.b}</b> <span className="t">{tr3(h.t)}</span></li>)}</ul><p className="hw-note">{tr3({ uz: "Keyingi darsda kompyuterni ko'p marta takrorlatamiz — sikllar! 🔁", ru: "На следующем уроке заставим компьютер повторять много раз — циклы! 🔁" })}</p></div>}
