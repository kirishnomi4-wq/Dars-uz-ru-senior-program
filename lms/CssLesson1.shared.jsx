...
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx src/1-Modull/CssLesson1.jsx
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>Bir xil matn — nega biri <span className="italic" style={{ color: T.accent }}>chiroyli</span>?</>, ru: <>Одинаковый текст — почему один <span className="italic" style={{ color: T.accent }}>красивый</span>?</> })}</h1>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [clicked, setClicked] = useState3(() => storedAnswer ? /* @__PURE__ */ new Set(["sel", "prop", "val"]) : /* @__PURE__ */ new Set());
...
var Screen3b = ({ screen, onNext, onPrev }) => {
  ...
  const [active, setActive] = useState3("inline");
  const [seen, setSeen] = useState3(() => /* @__PURE__ */ new Set(["inline"]));
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [tc, setTc] = useState3(storedAnswer?.tc || "#0E0E10");
  const [bg, setBg] = useState3(storedAnswer?.bg || "#FFFFFF");
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [sel, setSel] = useState3(() => storedAnswer ? Number.isInteger(storedAnswer.sel) && COLORS[storedAnswer.sel] ? storedAnswer.sel : 1 : null);
  const done = sel !== null;
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [font, setFont] = useState3(() => FONTS.some((f) => f.k === storedAnswer?.font) ? storedAnswer.font : "sans");
  const [size, setSize] = useState3(() => Number.isFinite(storedAnswer?.size) ? storedAnswer.size : 24);
  const [touched, setTouched] = useState3(!!storedAnswer);
  const done = touched;
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [bold, setBold] = useState3(!!storedAnswer?.bold);
  const [align, setAlign] = useState3(storedAnswer?.align || "left");
  const [bt, setBt] = useState3(!!storedAnswer);
  const [at, setAt] = useState3(!!storedAnswer);
  const done = bt && at;
  useEffect4(() => {
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [pad, setPad] = useState3(storedAnswer ? storedAnswer.pad ?? 26 : 12);
  const [touched, setTouched] = useState3(!!storedAnswer);
  const done = touched;
  const set = (v) => {
    setPad(v);
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [mar, setMar] = useState3(storedAnswer ? storedAnswer.mar ?? 24 : 10);
  const [touched, setTouched] = useState3(!!storedAnswer);
  const done = touched;
  const set = (v) => {
    setMar(v);
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [ap, setAp] = useState3(storedAnswer?.ap || {});
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [wrongMsg, setWrongMsg] = useState3(false);
  const [flashLine, setFlashLine] = useState3(-1);
  const pickWrong = (i) => {
    if (found) return;
    setWrongMsg(true);
    setFlashLine(i);
        ...
        <div className="split">
          <div className="col">
            <div className="ai-card fade-up delay-2">
              ...
              {!found && <p className="ai-prompt">{wrongMsg ? tr2({ uz: "Bu qatorda xato yo'q — yana qarang.", ru: "В этой строке ошибки нет — посмотрите ещё раз." }) : tr2({ uz: "Qaysi qatorda xato? Bosing.", ru: "В какой строке ошибка? Нажмите." })}</p>}
