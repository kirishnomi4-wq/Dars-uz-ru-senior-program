...
var NavNext = ({ disabled, label, onClick, optionalLive }) => {
  ...
  const lbl = label === void 0 ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr(label);
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
    ...
    return <div key={i} className={`mstats-row ${reveal && !isC ? "dimmed" : ""}`}>
              ...
              <span className="mono mstats-count" style={isC ? { color: T.success, fontWeight: 800 } : void 0}>{n > 0 ? `${n} ${tr({ uz: "o'quvchi", ru: "учен." })} · ${pct}%` : "—"}</span>
            </div>;
  })}
      </div>}
      {reveal && answered > 0 && (() => {
    ...
    return <button key={i} className={cls} disabled={solved || isMentorLive} onClick={() => pick(i)} style={{ padding: "clamp(13px,1.9vw,17px) clamp(15px,2.2vw,20px)", fontSize: "clamp(15px,1.85vw,17px)", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <FeedbackBlock show={isMentorLive ? mReveal : picked !== null} isCorrect={isMentorLive ? true : solved && !wrongLocked} neutral={waiting}>
          <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: waiting ? T.blue : isMentorLive || solved && !wrongLocked ? T.success : T.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>
            {isMentorLive ? <>{tr({ uz: "✓ To'g'ri javob:", ru: "✓ Правильный ответ:" })} {String.fromCharCode(65 + correctIdx)} — {fmtCode(options[correctIdx])}</> : waiting ? tr({ uz: "📨 Javobingiz qabul qilindi", ru: "📨 Ваш ответ принят" }) : wrongLocked ? <>{tr({ uz: "To'g'ri javob:", ru: "Правильный ответ:" })} {String.fromCharCode(65 + correctIdx)} — {fmtCode(options[correctIdx])}</> : solved ? tr({ uz: "To'g'ri", ru: "Верно" }) : tr({ uz: "Qaytadan urinib ko'ring", ru: "Попробуйте ещё раз" })}
          </p>
          <p className="body" style={{ margin: 0 }}>
            {isMentorLive ? fmtCode(explainCorrect) : waiting ? tr({ uz: "Hozir to'g'ri javobni bilib olasiz.", ru: "Сейчас вы узнаете правильный ответ." }) : wrongLocked ? fmtCode(explainWrong[picked] ?? explainWrong.default) : solved ? fmtCode(explainCorrect) : fmtCode(explainWrong[picked] ?? explainWrong.default)}
          </p>
          {
...
var RoCard = ({ name, emoji, likes, onLike, starred, onStar }) => {
  ...
  return <div className="rocard el-in">
      <div className="rothumb" style={{ background: bg }}>
        ...
        {onStar ? <button className={`rostar-btn ${starred ? "on" : ""}`} onClick={onStar} title={tr({ uz: "Sevimlilarga qo'shish", ru: "Добавить в избранное" })} aria-label={tr({ uz: "Sevimlilarga qo'shish", ru: "Добавить в избранное" })}>{starred ? "⭐" : "☆"}</button> : starred && <span className="rostar el-in">⭐</span>}
      ...
      <div className="robody">
        ...
        <div className="rostats">
          {onLike ? <button className="rolike-btn" onClick={onLike} title={tr({ uz: "Like bosib ko'ring", ru: "Попробуйте поставить лайк" })}><span key={likes} className="hpop">👍</span> <span className="rolike-num">{likes}</span></button> : <span className="rolike-static">👍 {pct}%</span>}
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  const [deadClicks, setDeadClicks] = useState(storedAnswer ? 3 : 0);
  ...
  return <Stage eyebrow={tr({ uz: "Kirish", ru: "Введение" })} screen={screen} navContent={<NavNext disabled={picked === null} label={tr({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up" style={{ maxWidth: 780 }}>{tr({ uz: <>Bir xil ikki tugma — nega <span className="italic" style={{ color: T.accent }}>biri ishlamaydi</span>?</>, ru: <>Две одинаковые кнопки — почему <span className="italic" style={{ color: T.accent }}>одна не работает</span>?</> })}</h1>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Reja", ru: "План" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head">
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Muammo", ru: "Проблема" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${tr({ uz: "👍 ni 3 marta bosing", ru: "Нажмите 👍 3 раза" })} (${clicks}/3)`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Oddiy o'zgaruvchi nega <span className="italic" style={{ color: T.accent }}>ekranga yetib bormaydi</span>?</>, ru: <>Почему обычная переменная <span className="italic" style={{ color: T.accent }}>не доходит до экрана</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <pre className="code-box fade-up delay-1">
              <Jx>{"let"}</Jx>{" likes = "}<St>0</St>{";  "}<Cm>{tr({ uz: "// oddiy o'zgaruvchi", ru: "// обычная переменная" })}</Cm>{"\n\n"}
              <Jx>{"<button"}</Jx> <At>onClick</At>{"={() => {"}{"\n"}
              {"  likes = likes + "}<St>1</St>{";"}{"\n"}
              {"  console.log(likes);  "}<Cm>{tr({ uz: "// o'syapti!", ru: "// растёт!" })}</Cm>{"\n"}
            ...
            <div className="code-box" style={{ padding: "9px 13px", minHeight: 64 }}>
              {clicks === 0 ? <TLine out={<span style={{ color: CODE.comment }}>{tr({ uz: "hozircha bo'sh — 👍 ni bosing", ru: "пока пусто — нажмите 👍" })}</span>} /> : Array.from({ length: clicks }, (_, i) => <TLine key={i} out={<span style={{ color: CODE.str }}>likes = {i + 1} ✓ {tr({ uz: "(xotirada o'sdi)", ru: "(выросло в памяти)" })}</span>} />)}
            </div>
          </Col>
          <Col>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(storedAnswer ? /* @__PURE__ */ new Set(["val", "set", "init"]) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow="useState" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${seen.size}/3 ${tr({ uz: "qism o'rganildi", ru: "части изучено" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            ...
            {active ? <div className="sk-info" key={active} style={{ boxShadow: `0 8px 20px -6px rgba(${T.shadowBase},0.16), inset 3px 0 0 ${COL[active]}` }}>
              ...
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink2, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Koddan bir qismni bosing", ru: "Нажмите на часть кода" })}</p></div>}
...
var Screen4 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "Faqat boshlang'ich 0 sonini", ru: "Только начальное число 0" }), tr({ uz: "Juftlik: qiymat va yangilovchi funksiya", ru: "Пару: значение и функцию-обновитель" }), tr({ uz: "Butunlay yangi komponent", ru: "Совершенно новый компонент" }), tr({ uz: "CSS uslublar klassini", ru: "Класс CSS-стилей" })]}
  correctIdx={1}
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const GOAL = 5;
  const memRef = useRef(storedAnswer ? GOAL : 0);
  const [memory, setMemory] = useState(storedAnswer ? GOAL : 0);
  const [screenLikes, setScreenLikes] = useState(storedAnswer ? GOAL : 0);
  ...
  return <Stage eyebrow={tr({ uz: "Like mashinasi", ru: "Машина лайков" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: `🎯 Ekrandagi 👍 ni ${GOAL} ga yetkazing (${screenLikes}/${GOAL})`, ru: `🎯 Доведите 👍 на экране до ${GOAL} (${screenLikes}/${GOAL})` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Xotirani o'zgartirish <span className="italic" style={{ color: T.accent }}>ekranni yangilamaydi</span> — qaysi tugma ekranga yetadi?</>, ru: <>Изменение памяти <span className="italic" style={{ color: T.accent }}>не обновляет экран</span> — какая кнопка дойдёт до экрана?</> })}</h2></div>
    ...
    /* 📦 Xotira qutisi → truba-kanal → 🖥 ekran-karta */
            ...
            <div className="lm-flow fade-up delay-1">
              ...
              <div key={"lm" + shake} className={`lm-screen render-wrap ${flash ? "flash" : ""} ${shake ? "lm-frozen" : ""}`} style={{ maxWidth: 172 }}>
                ...
                {flash && <span className="render-badge fade-step">{tr({ uz: "🔄 qayta chizildi!", ru: "🔄 перерисовано!" })}</span>}
              </div>
            </div>
          ...
          <Col>
            ...
            <div className="lm-levers fade-up delay-2">
              <button className="lm-lever dead" disabled={done} onClick={deadPull}>
                ...
                <span className="lm-lever-sub">{tr({ uz: "xotirani o'zgartiradi", ru: "меняет память" })}</span>
              </button>
...
var Screen5b = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "likes = likes + 1 deb to'g'ridan-to'g'ri yozamiz", ru: "Пишем напрямую: likes = likes + 1" }), tr({ uz: "Sahifani F5 tugmasi bilan yangilaymiz", ru: "Обновляем страницу клавишей F5" }), tr({ uz: "setLikes(likes + 1) chaqiramiz", ru: "Вызываем setLikes(likes + 1)" }), tr({ uz: "Brauzerni butunlay qayta o'rnatamiz", ru: "Полностью переустанавливаем браузер" })]}
  correctIdx={2}
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Ikkinchi xotira", ru: "Вторая память" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Ikkalasini ham sinang", ru: "Попробуйте обе" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            <pre className="code-box fade-up delay-1">
              <Cm>{tr({ uz: "// ikkita mustaqil xotira:", ru: "// две независимые ячейки памяти:" })}</Cm>{"\n"}
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="useEffect" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Kartochkani qo'shing", ru: "Добавьте карточку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            <pre className="code-box fade-up delay-1">
              <Jx>{"useEffect"}</Jx>{"(() => {"}{"\n"}
              {"  console.log("}<St>{tr({ uz: '"Salom! Men ekranga chiqdim"', ru: '"Привет! Я появился на экране"' })}</St>{");"}{"\n"}
              {"}, "}<At>[]</At>{");  "}<Cm>{tr({ uz: "// [] = faqat bir marta", ru: "// [] = только один раз" })}</Cm>
            </pre>
            {!mounted && <button className="btn fade-up delay-2" style={{ alignSelf: "flex-start" }} onClick={() => setMounted(true)}>{tr({ uz: "+ Kartochkani sahifaga qo'shish", ru: "+ Добавить карточку на страницу" })}</button>}
            <p className="flow-label" style={{ margin: 0 }}>{tr({ uz: "Konsol", ru: "Консоль" })}</p>
            <div className="code-box" style={{ padding: "9px 13px", minHeight: 46 }}>
              {mounted ? <TLine out={<span style={{ color: CODE.str }}>{tr({ uz: "✓ Salom! Men ekranga chiqdim", ru: "✓ Привет! Я появился на экране" })}</span>} /> : <TLine out={<span style={{ color: CODE.comment }}>{tr({ uz: "kutilmoqda…", ru: "ожидание…" })}</span>} />}
            </div>
          </Col>
          <Col>
            <p className="flow-label">localhost:5173</p>
            <Win title="robo-games — localhost:5173" minH={110}>
              {mounted ? <div style={{ maxWidth: 165 }}><RoCard name="Tower of Hell" /></div> : <p style={{ color: T.ink2, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13 }}>{tr({ uz: "Sahifa bo'sh — kartochka hali qo'shilmagan…", ru: "Страница пуста — карточка ещё не добавлена…" })}</p>}
            </Win>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="Effect + State" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${tr({ uz: "👍 ni 3 marta bosing", ru: "Нажмите 👍 3 раза" })} (${likes}/3)`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen9 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "Har safar 👍 bosilganda", ru: "При каждом нажатии 👍" }), tr({ uz: "Hech qachon ishlamaydi", ru: "Никогда не срабатывает" }), tr({ uz: "Komponent ekranga chiqqanda bir marta", ru: "Один раз, когда компонент появляется на экране" }), tr({ uz: "Faqat komponent o'chirilganda", ru: "Только когда компонент удаляется" })]}
  correctIdx={2}
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [stage, setStage] = useState(storedAnswer ? 3 : 0);
  const [likes, setLikes] = useState(storedAnswer ? 1 : 0);
  const done = stage >= 3;
  useEffect(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  ...
  return <Stage eyebrow={tr({ uz: "Hayot yo'li", ru: "Жизненный путь" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "3 bosqichni o'tkazing", ru: "Пройдите 3 этапа" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            ...
            <div className="code-box" style={{ padding: "9px 13px", minHeight: 72 }}>
              {stage === 0 ? <TLine out={<span style={{ color: CODE.comment }}>{tr({ uz: "kutilmoqda — o'yinchini kiriting…", ru: "ожидание — впустите игрока…" })}</span>} /> : LOGS.filter((l) => stage >= l.at).map((l, i) => <TLine key={i} out={<span style={{ color: l.color }}>✓ {tr(l.text)}</span>} />)}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr({ uz: "Server (sahifa)", ru: "Сервер (страница)" })}</p>
            <Win title="robo-games — localhost:5173" minH={110}>
  ...
  }} /></div> : <p style={{ color: T.ink2, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13 }}>{stage >= 3 ? tr({ uz: "Kartochka ketdi — sahifa yana bo'sh. Hayot yo'li yakunlandi.", ru: "Карточка ушла — страница снова пуста. Жизненный путь завершён." }) : tr({ uz: "Sahifa bo'sh…", ru: "Страница пуста…" })}</p>}
            </Win>
            <div className="fade-up delay-2" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Keyingi qadam · AI", ru: "Следующий шаг · AI" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Agent bilan ishlab ko'ring", ru: "Поработайте с агентом" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Jonli tugmani <span className="italic" style={{ color: T.accent }}>AI'ga yozdirsak</span>-chi?</>, ru: <>А что если <span className="italic" style={{ color: T.accent }}>поручить AI</span> написать живую кнопку?</> })}</h2></div>
            ...
            <Win title="robo-games — localhost:5173" minH={130}>
              {done && cur ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {cur.id === "t2" && <div className="welcome-toast el-in">
                      ...
                      <div className="wt-text"><b>{tr({ uz: "Xush kelibsiz!", ru: "Добро пожаловать!" })}</b><span>{tr({ uz: "robo-games'ga qaytib keldingiz", ru: "вы вернулись в robo-games" })}</span></div>
                      <span className="wt-badge">{tr({ uz: "[] · 1 marta", ru: "[] · 1 раз" })}</span>
                    </div>}
                ...
                </div> : <p style={{ color: T.ink2, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13 }}>{tr({ uz: "Buyruq bering va rejani tasdiqlang…", ru: "Дайте команду и утвердите план…" })}</p>}
            </Win>
            {done ? <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: <>Tugmani <b>o'zingiz bosib sinadingizmi</b>? Kod ham o'qildi: useState bor, set chaqirilgan. Agent to'g'ri qilganini endi <b>isbotlay olasiz</b>.</>, ru: <>Вы <b>сами проверили кнопку нажатием</b>? И код прочитан: useState есть, set вызван. Теперь вы можете <b>доказать</b>, что агент всё сделал правильно.</> })}</p></div> : <p className="body" style={{ margin: 0, color: T.ink2, fontSize: 13 }}>{tr({ uz: "Natija shu yerda paydo bo'ladi — keyin uni o'zingiz sinaysiz.", ru: "Результат появится здесь — потом вы сами его проверите." })}</p>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen12 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "Farqi yo'q — ikkalasi bir narsa", ru: "Ничем — это одно и то же" }), tr({ uz: "State — o'z xotirasi; props — tashqaridan keladi", ru: "State — своя память; props приходит снаружи" }), tr({ uz: "State faqat sonlarni saqlay oladi", ru: "State может хранить только числа" }), tr({ uz: "Props ancha tezroq ishlaydi", ru: "Props работает гораздо быстрее" })]}
  correctIdx={1}
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [initTried, setInitTried] = useState(storedAnswer ? /* @__PURE__ */ new Set([0, 10]) : /* @__PURE__ */ new Set([0]));
  const [stepTried, setStepTried] = useState(storedAnswer ? /* @__PURE__ */ new Set([1, 5]) : /* @__PURE__ */ new Set([1]));
  ...
  return <Stage eyebrow={tr({ uz: "Amaliyot · sozlash", ru: "Практика · настройка" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Sozlang va bosib sinang", ru: "Настройте и проверьте нажатием" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>O'z <span className="italic" style={{ color: T.accent }}>like hisoblagichingizni</span> sozlay olasizmi?</>, ru: <>Сможете настроить <span className="italic" style={{ color: T.accent }}>свой счётчик лайков</span>?</> })}</h2></div>
        ...
        <div className="split">
          ...
          <Col>
            ...
            <span className="tagpill fade-step" style={{ color: done ? T.success : T.ink }}>{initTried.size >= 2 ? "✓" : "○"} {tr({ uz: "boshlang'ich", ru: "начальное" })} · {stepTried.size >= 2 ? "✓" : "○"} {tr({ uz: "qadam", ru: "шаг" })} · {clicks >= 1 ? "✓" : "○"} {tr({ uz: "sinash", ru: "проверка" })}</span>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: <>Siz hozir state'ni <b>to'liq boshqardingiz</b>: boshlang'ich qiymat, o'zgarish qadami va qayta chizish — hammasi qo'lingizda.</>, ru: <>Вы только что <b>полностью управляли state</b>: начальное значение, шаг изменения и перерисовка — всё в ваших руках.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Debugging", ru: "Отладка" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : found ? tr({ uz: "Endi tuzating", ru: "Теперь исправьте" }) : tr({ uz: "Xatoni toping", ru: "Найдите ошибку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            <div className="ai-card fade-up delay-2">
              <div className="ai-row"><span className="ai-badge">AI</span><span className="ai-bubble">{tr({ uz: "Like tugmasini yozdim:", ru: "Я написал кнопку лайка:" })}</span></div>
    ...
    if (!found) setPicked("btn");
                ...
                {!fixed ? <div className={`ai-line ${found ? "bad" : ""}`} onClick={pickMut}>{"  likes = likes + "}<St>1</St>{";  "}<Cm>{tr({ uz: "// yangilash", ru: "// обновление" })}</Cm></div> : <div className="ai-line ok el-in">{"  setLikes(likes + "}<St>1</St>{");  "}<Cm>{tr({ uz: "// yangilash", ru: "// обновление" })}</Cm></div>}
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!passed} label={passed ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Xotira qatorini yozing", ru: "Напишите строку памяти" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            ...
            {passed && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: "✓ Mukammal! Xotira yaratildi — tugma jonlandi. Endi siz state'ni noldan yoza olasiz.", ru: "✓ Отлично! Память создана — кнопка ожила. Теперь вы можете написать state с нуля." })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr({ uz: "natija — bosib sinang", ru: "результат — проверьте нажатием" })}</p>
            <Win title="robo-games — localhost:5173" minH={130}>
...
var MentorPracticeStats = ({ live, screen }) => {
  ...
  return <div className="lp-mstats fade-up">
      ...
      {data.players === null ? <p className="small" style={{ color: T.ink2, margin: 0, fontStyle: "italic" }}>{tr({ uz: "Yuklanmoqda…", ru: "Загрузка…" })}</p> : players.length === 0 ? <p className="small" style={{ color: T.ink2, margin: 0, fontStyle: "italic" }}>{tr({ uz: "Hali hech kim qo'shilmagan.", ru: "Пока никто не присоединился." })}</p> : <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
...
function ScreenLivePractice({ title, task, checklist, screen, storedAnswer, onAnswer, onNext, onPrev, live }) {
  ...
  return <Stage eyebrow={tr({ uz: "Amaliyot · VS Code", ru: "Практика · VS Code" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Avval bajaring", ru: "Сначала выполните" })} onClick={onNext} /></>}>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Kartochkangiz <span className="italic" style={{ color: T.accent }}>jonlandi</span>.</>, ru: <>Ваша карточка <span className="italic" style={{ color: T.accent }}>ожила</span>.</> })}</h2>{
    /* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */
        ...
        <div className={`qz-cta cs-cta fade-up d2 ${studentLive ? "ready" : ""}`}>
          <CsWordmark stats={false} liveOn={studentLive} disabled={studentWait} onClick={studentWait ? void 0 : openArena} hint={studentWait ? tr({ uz: "⏳ Mentorni kuting", ru: "⏳ Дождитесь ментора" }) : void 0} />
        </div>
...
function ReactStateEffectLesson({ lang: langProp, onFinished }) {
