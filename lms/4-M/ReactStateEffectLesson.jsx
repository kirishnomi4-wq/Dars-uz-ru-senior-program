...
//  Manba:  src/3-Modull/ReactStateEffectLesson.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/3-Modull/ReactStateEffectLesson.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/3-Modull/ReactStateEffectLesson.jsx
...
var LESSON_META = { lessonId: "react-state-effect-03-v18", lessonTitle: { uz: "State va Effect: useState + useEffect", ru: "State и Effect: useState + useEffect" } };
var HW_TOKENS = [
...
var NavNext = ({ disabled, label, onClick, optionalLive }) => {
  ...
  const lbl = label === void 0 ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2(label);
...
var INLINE_KEYS = { s4: 1, s5b: 2, s9: 2, s12: 1, s15: -1 };
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <p className="mstats-verdict-t">{tr2({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верных — тема осталась для класса непонятной. Перед тем как идти дальше, рекомендуем коротко повторить.</> })}</p>}
            {level === "maybe" && <p className="mstats-verdict-t">{tr2({ uz: <>🟡 <b>{pct}%</b> to'g'ri — yomon emas. Xohlasangiz, davom etishdan oldin qisqa takrorlab oling.</>, ru: <>🟡 <b>{pct}%</b> верных — неплохо. Если хотите, коротко повторите перед продолжением.</> })}</p>}
      ...
      {reveal && struggling && <p className="mstats-warn">{tr2({ uz: "⚠️ Ko'pchilik xato qildi — bu mavzu tushunarsiz bo'lgan ko'rinadi. Qayta tushuntiring.", ru: "⚠️ Большинство ошиблось — похоже, тема осталась непонятной. Объясните ещё раз." })}</p>}
    ...
    return <button key={i} className={cls} disabled={solved || isMentorLive} onClick={() => pick(i)} style={{ padding: "clamp(13px,1.9vw,17px) clamp(15px,2.2vw,20px)", fontSize: "clamp(15px,1.85vw,17px)", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <FeedbackBlock show={isMentorLive ? mReveal : picked !== null} isCorrect={isMentorLive ? true : solved && !wrongLocked} neutral={waiting}>
          ...
          <p className="body" style={{ margin: 0 }}>
            {isMentorLive ? fmtCode(explainCorrect) : waiting ? tr2({ uz: "Hozir to'g'ri javobni bilib olasiz.", ru: "Сейчас вы узнаете правильный ответ." }) : wrongLocked ? fmtCode(explainWrong[picked] ?? explainWrong.default) : solved ? fmtCode(explainCorrect) : fmtCode(explainWrong[picked] ?? explainWrong.default)}
          </p>
          {
...
var RoCard = ({ name, emoji, likes, onLike, starred, onStar }) => {
  ...
  return <div className="rocard el-in">
      <div className="rothumb" style={{ background: bg }}>
        <span className="rothumb-icon">{em}</span>
        {onStar ? <button className={`rostar-btn ${starred ? "on" : ""}`} onClick={onStar} title={tr2({ uz: "Sevimlilarga qo'shish", ru: "Добавить в избранное" })} aria-label={tr2({ uz: "Sevimlilarga qo'shish", ru: "Добавить в избранное" })}>{starred ? "⭐" : "☆"}</button> : starred && <span className="rostar el-in">⭐</span>}
      ...
      <div className="robody">
        ...
        <div className="rostats">
          {onLike ? <button className="rolike-btn" onClick={onLike} title={tr2({ uz: "Like bosib ko'ring", ru: "Попробуйте поставить лайк" })}><span key={likes} className="hpop">👍</span> <span className="rolike-num">{likes}</span></button> : <span className="rolike-static">👍 {pct}%</span>}
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  const [deadClicks, setDeadClicks] = useState3(storedAnswer ? 3 : 0);
  const [liveLikes, setLiveLikes] = useState3(storedAnswer ? 1 : 0);
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} navContent={<NavNext disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up" style={{ maxWidth: 780 }}>{tr2({ uz: <>Bir xil ikki tugma — nega <span className="italic" style={{ color: T.accent }}>biri ishlamaydi</span>?</>, ru: <>Две одинаковые кнопки — почему <span className="italic" style={{ color: T.accent }}>одна не работает</span>?</> })}</h1>
        ...
        <Split>
          <Col>
            <div className="fade-up delay-1" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div>
                <p className="flow-label" style={{ marginBottom: 7 }}>{tr2({ uz: "1-versiya", ru: "1-я версия" })}</p>
                <RoCard name="Adopt Me!" likes={0} onLike={() => setDeadClicks((c) => c + 1)} />
                <p className="mono small" style={{ margin: "7px 0 0", color: deadClicks > 0 ? T.accent : T.ink3 }}>{deadClicks > 0 ? `${tr2({ uz: "xotirada", ru: "в памяти" })}: likes = ${deadClicks}` : tr2({ uz: "bosib ko'ring ↑", ru: "нажмите ↑" })}</p>
                {deadClicks > 0 && <p className="mono small" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: "ekranda esa: 👍 0 — qotib qoldi!", ru: "а на экране: 👍 0 — застыло!" })}</p>}
              </div>
              <div>
                <p className="flow-label" style={{ marginBottom: 7 }}>{tr2({ uz: "2-versiya", ru: "2-я версия" })}</p>
                <RoCard name="Adopt Me!" likes={liveLikes} onLike={() => setLiveLikes((c) => c + 1)} />
                <p className="mono small" style={{ margin: "7px 0 0", color: liveLikes > 0 ? T.success : T.ink3 }}>{liveLikes > 0 ? tr2({ uz: "✓ ekran darhol yangilandi!", ru: "✓ экран обновился сразу!" }) : tr2({ uz: "bosib ko'ring ↑", ru: "нажмите ↑" })}</p>
              </div>
            </div>
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Sizningcha, chapdagisiga nima bo'ldi?", ru: "Как вы думаете, что случилось с левой?" })}</p>
    ...
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null || !tried} style={{ opacity: !tried ? 0.55 : 1 }} onClick={() => pick(o.id)}>
            ...
            {!tried && <p className="small" style={{ color: T.ink2, fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Avval ikkala kartochkada 👍 bosing ←", ru: "Сначала нажмите 👍 на обеих карточках ←" })}</p>}
            {picked !== null && <p className="hook-ack fade-step">{tr2({ uz: <>Topdingiz! Kod to'g'ri — son <b>xotirada o'sdi</b>, lekin React buni <b>sezmadi</b>. 2-versiyada maxsus xotira bor: <b>state</b>.</>, ru: <>Вы угадали! Код верный — число <b>выросло в памяти</b>, но React этого <b>не заметил</b>. Во 2-й версии есть особая память: <b>state</b>.</> })}</p>}
          </Col>
        </Split>
        </Zoomable>
      </div>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const [likes, setLikes] = useState3(0);
  const PreviewBlock = <Col>
      <p className="flow-label">{tr2({ uz: "Dars oxirida — sizning natijangiz", ru: "В конце урока — ваш результат" })}</p>
      ...
      <p className="mono small" style={{ color: T.accent, margin: 0 }}>{tr2({ uz: "→ mana shu qatorni dars oxirida to'liq o'zingiz yozasiz", ru: "→ в конце урока вы сами полностью напишете эту строку" })}</p>
    </Col>;
  ...
  return <Stage eyebrow={tr2({ uz: "Reja", ru: "План" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head">
          <h2 className="title h-title fade-up">{tr2({ uz: <>Kartochkadagi <span className="italic" style={{ color: T.accent }}>like qanday jonlanadi</span>?</>, ru: <>Как <span className="italic" style={{ color: T.accent }}>оживает лайк</span> на карточке?</> })}</h2>
        </div>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [clicks, setClicks] = useState3(storedAnswer ? 3 : 0);
  const done = clicks >= 3;
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  return <Stage eyebrow={tr2({ uz: "Muammo", ru: "Проблема" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : `${tr2({ uz: "👍 ni 3 marta bosing", ru: "Нажмите 👍 3 раза" })} (${clicks}/3)`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Oddiy o'zgaruvchi nega <span className="italic" style={{ color: T.accent }}>ekranga yetib bormaydi</span>?</>, ru: <>Почему обычная переменная <span className="italic" style={{ color: T.accent }}>не доходит до экрана</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Mana 1-versiyaning kodi: oddiy <span className="mono">let likes</span>. Tugmani bosing va <b style={{ color: T.ink }}>konsolga qarang</b> — son rostdan o'syapti! Lekin React'ga hech kim <b style={{ color: T.ink }}>"qayta chiz"</b> demayapti. Shuning uchun ekran eski holatda qotib qolgan.</>, ru: <>Вот код 1-й версии: обычная <span className="mono">let likes</span>. Нажмите кнопку и <b style={{ color: T.ink }}>посмотрите в консоль</b> — число действительно растёт! Но никто не говорит React <b style={{ color: T.ink }}>«перерисуй»</b>. Поэтому экран застыл в старом состоянии.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <pre className="code-box fade-up delay-1">
              <Jx>{"let"}</Jx>{" likes = "}<St>0</St>{";  "}<Cm>{tr2({ uz: "// oddiy o'zgaruvchi", ru: "// обычная переменная" })}</Cm>{"\n\n"}
              <Jx>{"<button"}</Jx> <At>onClick</At>{"={() => {"}{"\n"}
              {"  likes = likes + "}<St>1</St>{";"}{"\n"}
              {"  console.log(likes);  "}<Cm>{tr2({ uz: "// o'syapti!", ru: "// растёт!" })}</Cm>{"\n"}
            ...
            <div className="code-box" style={{ padding: "9px 13px", minHeight: 64 }}>
              {clicks === 0 ? <TLine out={<span style={{ color: CODE.comment }}>{tr2({ uz: "hozircha bo'sh — 👍 ni bosing", ru: "пока пусто — нажмите 👍" })}</span>} /> : Array.from({ length: clicks }, (_, i) => <TLine key={i} out={<span style={{ color: CODE.str }}>likes = {i + 1} ✓ {tr2({ uz: "(xotirada o'sdi)", ru: "(выросло в памяти)" })}</span>} />)}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Ekranda — 👍 ni bosing", ru: "На экране — нажмите 👍" })}</p>
            ...
            <div className="gap-viz fade-up delay-1">
              <div className="gap-box mem">
                <span className="gap-lbl">{tr2({ uz: "📦 Xotira (let likes)", ru: "📦 Память (let likes)" })}</span>
                <span key={clicks} className={clicks > 0 ? "gap-num pop" : "gap-num"}>{clicks}</span>
              </div>
              <span className="gap-vs">{tr2({ uz: "teng emas", ru: "не равно" })}</span>
              <div className="gap-box scr">
                <span className="gap-lbl">{tr2({ uz: "🖥 Ekranda", ru: "🖥 На экране" })}</span>
                <span className="gap-num frozen">0 <span className="gap-lock">🔒</span></span>
              </div>
            </div>
            {clicks > 0 && !done && <p className="mono small" style={{ margin: 0, color: T.accent }}>{tr2({ uz: "↑ xotira o'syapti, ekran esa qotgan…", ru: "↑ память растёт, а экран застыл…" })}</p>}
            {done && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Ko'rdingizmi? Xotirada <b>likes = {clicks}</b>, ekranda esa <b>0</b>. Oddiy o'zgaruvchi o'zgarganini React <b>sezmaydi</b> — unga maxsus xotira kerak. Keyingi ekranda tanishamiz!</>, ru: <>Видели? В памяти <b>likes = {clicks}</b>, а на экране <b>0</b>. React <b>не замечает</b>, что обычная переменная изменилась — ему нужна особая память. Познакомимся с ней на следующем экране!</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const PARTS = {
    val: { word: { uz: "likes — joriy qiymat", ru: "likes — текущее значение" }, info: { uz: <>Xotiradagi hozirgi qiymat. Uni JSX ichida o'qiysiz: <span className="mono">{"👍 {likes}"}</span>. Faqat <b>o'qish</b> uchun — to'g'ridan-to'g'ri o'zgartirib bo'lmaydi!</>, ru: <>Нынешнее значение в памяти. Вы читаете его внутри JSX: <span className="mono">{"👍 {likes}"}</span>. Только для <b>чтения</b> — напрямую менять нельзя!</> } },
    set: { word: { uz: "setLikes — yangilovchi", ru: "setLikes — обновитель" }, info: { uz: <>Qiymatni o'zgartiradigan <b>yagona to'g'ri yo'l</b>. <span className="mono">setLikes(5)</span> deysiz — React xotirani yangilaydi VA ekranni qayta chizadi. Mana 1-versiyada yetishmagan narsa!</>, ru: <><b>Единственный правильный способ</b> изменить значение. Говорите <span className="mono">setLikes(5)</span> — React обновляет память И перерисовывает экран. Вот чего не хватало 1-й версии!</> } },
    init: { word: { uz: "useState(0) — boshlang'ich", ru: "useState(0) — начальное" }, info: { uz: <>Qavs ichidagi qiymat — xotiraning <b>boshlang'ich holati</b>. <span className="mono">useState(0)</span> → birinchi ko'rinishda 👍 0. <span className="mono">useState(100)</span> yozsangiz — 100 dan boshlanadi.</>, ru: <>Значение в скобках — <b>начальное состояние</b> памяти. <span className="mono">useState(0)</span> → при первом показе 👍 0. Напишете <span className="mono">useState(100)</span> — начнётся со 100.</> } }
  };
  const [active, setActive] = useState3(null);
  const [seen, setSeen] = useState3(storedAnswer ? /* @__PURE__ */ new Set(["val", "set", "init"]) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow="useState" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : `${seen.size}/3 ${tr2({ uz: "qism o'rganildi", ru: "части изучено" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bu xotira qatori <span className="italic" style={{ color: T.accent }}>nimalardan iborat</span>?</>, ru: <>Из чего состоит <span className="italic" style={{ color: T.accent }}>эта строка памяти</span>?</> })}</h2></div>
        ...
        <div className="split">
          ...
          <Col>
            ...
            {active ? <div className="sk-info" key={active} style={{ boxShadow: `0 8px 20px -6px rgba(${T.shadowBase},0.16), inset 3px 0 0 ${COL[active]}` }}>
                <span className="sk-tagbig"><span className="sk-wordbadge" style={{ color: COL[active], background: COL[active] + "22" }}>{tr2(PARTS[active].word)}</span></span>
                <p className="body" style={{ color: T.ink, margin: "11px 0 0" }}>{tr2(PARTS[active].info)}</p>
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink2, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Koddan bir qismni bosing", ru: "Нажмите на часть кода" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Formula: <span className="mono">const [qiymat, setQiymat] = useState(boshlang'ich)</span>. Juftlik nomi erkin — <span className="mono">[son, setSon]</span> ham bo'laveradi.</>, ru: <>Формула: <span className="mono">const [значение, setЗначение] = useState(начальное)</span>. Имена пары свободные — <span className="mono">[son, setSon]</span> тоже подойдёт.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen4 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <><span className="mono" style={{ color: T.accent }}>useState(0)</span> nima beradi?</>, ru: <>Что даёт <span className="mono" style={{ color: T.accent }}>useState(0)</span>?</> })}</h2></>}
  options={[tr2({ uz: "Faqat boshlang'ich 0 sonini", ru: "Только начальное число 0" }), tr2({ uz: "Juftlik: qiymat va yangilovchi funksiya", ru: "Пару: значение и функцию-обновитель" }), tr2({ uz: "Butunlay yangi komponent", ru: "Совершенно новый компонент" }), tr2({ uz: "CSS uslublar klassini", ru: "Класс CSS-стилей" })]}
  correctIdx={1}
  explainCorrect={tr2({ uz: "To'g'ri! useState juftlik qaytaradi: [likes, setLikes] — joriy qiymat va uni yangilovchi funksiya. 0 esa — boshlang'ich qiymat.", ru: "Верно! useState возвращает пару: [likes, setLikes] — текущее значение и функцию для его обновления. А 0 — начальное значение." })}
  explainWrong={{
    0: tr2({ uz: "Yo'q — 0 faqat boshlang'ich qiymat. useState juftlik beradi: qiymat va yangilovchi funksiya.", ru: "Нет — 0 это лишь начальное значение. useState даёт пару: значение и функцию-обновитель." }),
    2: tr2({ uz: "Yo'q — komponent yaratmaydi. useState komponent ICHIDA xotira yaratadi.", ru: "Нет — компонент он не создаёт. useState создаёт память ВНУТРИ компонента." }),
    3: tr2({ uz: "Yo'q — CSS'ga aloqasi yo'q. useState — xotira: [qiymat, yangilovchi].", ru: "Нет — к CSS он не относится. useState — память: [значение, обновитель]." }),
    default: tr2({ uz: "useState — juftlik: [joriy qiymat, yangilovchi funksiya].", ru: "useState — пара: [текущее значение, функция-обновитель]." })
  }}
/>;
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const GOAL = 5;
  const memRef = useRef3(storedAnswer ? GOAL : 0);
  const [memory, setMemory] = useState3(storedAnswer ? GOAL : 0);
  const [screenLikes, setScreenLikes] = useState3(storedAnswer ? GOAL : 0);
  const [wasted, setWasted] = useState3(0);
  const [pulse, setPulse] = useState3(0);
  const [flash, setFlash] = useState3(false);
  const [shake, setShake] = useState3(0);
  const [toast, setToast] = useState3(false);
  const done = screenLikes >= GOAL;
  const tRef = useRef3(null);
  const fRef = useRef3(null);
  useEffect4(() => () => {
  ...
  const deadPull = () => {
    if (done) return;
    memRef.current += 1;
    setMemory(memRef.current);
    setWasted((w) => w + 1);
    setShake((s) => s + 1);
    setToast(true);
    clearTimeout(tRef.current);
    tRef.current = setTimeout(() => setToast(false), 1500);
  };
  const livePull = () => {
    if (done) return;
    memRef.current += 1;
    setMemory(memRef.current);
    setScreenLikes((s) => s + 1);
    setPulse((p) => p + 1);
    setFlash(true);
    clearTimeout(fRef.current);
    fRef.current = setTimeout(() => setFlash(false), 650);
  };
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { stage: "like-machine", screenIdx: screen, correct: true, picked: true, wasted });
  }, [done]);
  return <Stage eyebrow={tr2({ uz: "Like mashinasi", ru: "Машина лайков" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `🎯 Ekrandagi 👍 ni ${GOAL} ga yetkazing (${screenLikes}/${GOAL})`, ru: `🎯 Доведите 👍 на экране до ${GOAL} (${screenLikes}/${GOAL})` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Xotirani o'zgartirish <span className="italic" style={{ color: T.accent }}>ekranni yangilamaydi</span> — qaysi tugma ekranga yetadi?</>, ru: <>Изменение памяти <span className="italic" style={{ color: T.accent }}>не обновляет экран</span> — какая кнопка дойдёт до экрана?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Ikki tugma bor. <b style={{ color: T.ink }}>✏️ likes = likes + 1</b> — xotirani o'zgartiradi, lekin React bexabar qoladi (ekran qotadi). <b style={{ color: T.ink }}>⚡ setLikes(likes + 1)</b> — xotirani o'zgartiradi VA React'ga "qayta chiz" deydi. Maqsad: <b style={{ color: T.ink }}>ekrandagi 👍 ni {GOAL} ga yetkazing</b>. Qaysi tugma haqiqatan ishlaydi?</>, ru: <>Есть две кнопки. <b style={{ color: T.ink }}>✏️ likes = likes + 1</b> — меняет память, но React остаётся не в курсе (экран застывает). <b style={{ color: T.ink }}>⚡ setLikes(likes + 1)</b> — меняет память И говорит React «перерисуй». Цель: <b style={{ color: T.ink }}>доведите 👍 на экране до {GOAL}</b>. Какая кнопка работает по-настоящему?</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            {
    /* 📦 Xotira qutisi → truba-kanal → 🖥 ekran-karta */
            ...
            <div className="lm-flow fade-up delay-1">
              <div className="lm-mem">
                <span className="gap-lbl">{tr2({ uz: "📦 Xotira (likes)", ru: "📦 Память (likes)" })}</span>
                <span key={memory} className={memory > 0 ? "gap-num pop" : "gap-num"}>{memory}</span>
              </div>
              <div className={`lm-channel ${flash ? "lit" : ""}`}>
                {pulse > 0 && <span key={pulse} className="lm-impulse" aria-hidden="true" />}
                <span className="lm-channel-lbl">setLikes</span>
              </div>
              <div key={"lm" + shake} className={`lm-screen render-wrap ${flash ? "flash" : ""} ${shake ? "lm-frozen" : ""}`} style={{ maxWidth: 172 }}>
                <RoCard key={screenLikes} name="Blox Fruits" likes={screenLikes} onLike={livePull} />
                {flash && <span className="render-badge fade-step">{tr2({ uz: "🔄 qayta chizildi!", ru: "🔄 перерисовано!" })}</span>}
              </div>
            </div>
            <div className="lm-gap fade-up delay-2">
              <span className="lm-gap-i">{tr2({ uz: "xotira:", ru: "память:" })} <b style={{ color: T.accent }}>{memory}</b></span>
              <span className="lm-gap-vs">{tr2({ uz: "teng emas", ru: "не равно" })}</span>
              <span className="lm-gap-i">{tr2({ uz: "ekran:", ru: "экран:" })} <b style={{ color: T.success }}>{screenLikes}</b></span>
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: <>🎯 Maqsad — ekrandagi 👍 ni {GOAL} ga yetkazing</>, ru: <>🎯 Цель — доведите 👍 на экране до {GOAL}</> })}</p>
            ...
            <div className="lm-levers fade-up delay-2">
              <button className="lm-lever dead" disabled={done} onClick={deadPull}>
                ...
                <span className="lm-lever-sub">{tr2({ uz: "xotirani o'zgartiradi", ru: "меняет память" })}</span>
              </button>
              <button className="lm-lever live" disabled={done} onClick={livePull}>
                <span className="lm-lever-code">⚡ setLikes(likes + 1)</span>
                <span className="lm-lever-sub">{tr2({ uz: "xotira + ekran", ru: "память + экран" })}</span>
              </button>
            </div>
            {wasted > 0 && !done && <p key={wasted} className="mono small lm-wasted" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: <>behuda ✏️ bosishlar: <b style={{ color: T.accent }}>{wasted}</b> — ekranga yetmadi</>, ru: <>впустую нажато ✏️: <b style={{ color: T.accent }}>{wasted}</b> — до экрана не дошло</> })}</p>}
            {toast && <div className="lm-toast fade-step">{tr2({ uz: "🔒 React sezmadi — xotira o'zgardi, lekin ekran qotib qoldi", ru: "🔒 React не заметил — память изменилась, но экран застыл" })}</div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>🎉 G'alaba! Ekrandagi 👍 {GOAL} ga yetdi — faqat <b>setLikes</b> orqali. ✏️ oddiy o'zgartirish {wasted} marta xotirani o'zgartirdi, lekin ekranga yetmadi. <b>Xotira ekranga teng emas</b>: React'ni faqat set… uyg'otadi.</>, ru: <>🎉 Победа! 👍 на экране дошёл до {GOAL} — только через <b>setLikes</b>. ✏️ обычное изменение {wasted} раз меняло память, но до экрана не дошло. <b>Память не равна экрану</b>: React будит только set…</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {done && <Confetti />}
...
var Screen5b = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "Mustahkamlash", ru: "Закрепление" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Ekranda yangi qiymat ko'rinishi uchun <span className="italic" style={{ color: T.accent }}>nima qilamiz</span>?</>, ru: <><span className="italic" style={{ color: T.accent }}>Что мы делаем</span>, чтобы новое значение появилось на экране?</> })}</h2></>}
  options={[tr2({ uz: "likes = likes + 1 deb to'g'ridan-to'g'ri yozamiz", ru: "Пишем напрямую: likes = likes + 1" }), tr2({ uz: "Sahifani F5 tugmasi bilan yangilaymiz", ru: "Обновляем страницу клавишей F5" }), tr2({ uz: "setLikes(likes + 1) chaqiramiz", ru: "Вызываем setLikes(likes + 1)" }), tr2({ uz: "Brauzerni butunlay qayta o'rnatamiz", ru: "Полностью переустанавливаем браузер" })]}
  correctIdx={2}
  explainCorrect={tr2({ uz: "To'g'ri! Faqat setLikes orqali — shunda React xotirani yangilaydi VA komponentni qayta chizadi. To'g'ridan-to'g'ri o'zgartirish 1-versiyadagi 'qotib qolgan' tugma edi.", ru: "Верно! Только через setLikes — тогда React обновляет память И перерисовывает компонент. Прямое изменение — это та самая «застывшая» кнопка из 1-й версии." })}
  explainWrong={{
    0: tr2({ uz: "Esingizdami 1-versiya? Xotirada o'sdi, ekran qotib qoldi. To'g'ridan-to'g'ri o'zgartirishni React sezmaydi.", ru: "Помните 1-ю версию? В памяти выросло, экран застыл. Прямое изменение React не замечает." }),
    1: tr2({ uz: "F5 butun sahifani qayta yuklaydi — eski saytlar usuli. React'da setLikes yetadi: faqat kerakli joy yangilanadi.", ru: "F5 перезагружает всю страницу — так делали старые сайты. В React достаточно setLikes: обновится только нужное место." }),
    3: tr2({ uz: "Yo'q — brauzerda muammo yo'q. setLikes chaqirilsa hammasi ishlaydi.", ru: "Нет — с браузером всё в порядке. Вызовите setLikes — и всё заработает." }),
    default: tr2({ uz: "Faqat setLikes(yangi qiymat) — React qayta chizishni o'zi bajaradi.", ru: "Только setLikes(новое значение) — перерисовку React выполнит сам." })
  }}
/>;
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [likes, setLikes] = useState3(storedAnswer ? 1 : 0);
  const [starred, setStarred] = useState3(!!storedAnswer);
  const [starTouched, setStarTouched] = useState3(!!storedAnswer);
  const done = likes >= 1 && starTouched;
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  return <Stage eyebrow={tr2({ uz: "Ikkinchi xotira", ru: "Вторая память" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Ikkalasini ham sinang", ru: "Попробуйте обе" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bitta komponentda <span className="italic" style={{ color: T.accent }}>nechta xotira</span> bo'la oladi?</>, ru: <>Сколько <span className="italic" style={{ color: T.accent }}>ячеек памяти</span> может быть в одном компоненте?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <pre className="code-box fade-up delay-1">
              <Cm>{tr2({ uz: "// ikkita mustaqil xotira:", ru: "// две независимые ячейки памяти:" })}</Cm>{"\n"}
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "👍 like va ⭐ yulduzni bosing", ru: "Нажмите 👍 лайк и ⭐ звёздочку" })}</p>
            <div style={{ maxWidth: 188 }}><RoCard name="Brookhaven" likes={likes} onLike={() => setLikes((l) => l + 1)} starred={starred} onStar={() => {
    setStarred((s) => !s);
    setStarTouched(true);
  }} /></div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>👍 bosganda ⭐ o'zgarmadi — har state <b>mustaqil quti</b>. <span className="mono">false/true</span> ham xotira bo'la oladi: son, matn, belgi — hammasi.</>, ru: <>Нажали 👍 — ⭐ не изменилась: каждый state — <b>независимая коробка</b>. <span className="mono">false/true</span> тоже может быть памятью: число, текст, флажок — что угодно.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [mounted, setMounted] = useState3(!!storedAnswer);
  const done = mounted;
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  return <Stage eyebrow="useEffect" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Kartochkani qo'shing", ru: "Добавьте карточку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Kartochka ekranga chiqishi bilan <span className="italic" style={{ color: T.accent }}>"Salom!" deyishi</span> mumkinmi?</>, ru: <>Может ли карточка <span className="italic" style={{ color: T.accent }}>сказать «Привет!»</span>, как только появится на экране?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <pre className="code-box fade-up delay-1">
              <Jx>{"useEffect"}</Jx>{"(() => {"}{"\n"}
              {"  console.log("}<St>{tr2({ uz: '"Salom! Men ekranga chiqdim"', ru: '"Привет! Я появился на экране"' })}</St>{");"}{"\n"}
              {"}, "}<At>[]</At>{");  "}<Cm>{tr2({ uz: "// [] = faqat bir marta", ru: "// [] = только один раз" })}</Cm>
            </pre>
            {!mounted && <button className="btn fade-up delay-2" style={{ alignSelf: "flex-start" }} onClick={() => setMounted(true)}>{tr2({ uz: "+ Kartochkani sahifaga qo'shish", ru: "+ Добавить карточку на страницу" })}</button>}
            <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "Konsol", ru: "Консоль" })}</p>
            <div className="code-box" style={{ padding: "9px 13px", minHeight: 46 }}>
              {mounted ? <TLine out={<span style={{ color: CODE.str }}>{tr2({ uz: "✓ Salom! Men ekranga chiqdim", ru: "✓ Привет! Я появился на экране" })}</span>} /> : <TLine out={<span style={{ color: CODE.comment }}>{tr2({ uz: "kutilmoqda…", ru: "ожидание…" })}</span>} />}
            </div>
          </Col>
          <Col>
            <p className="flow-label">localhost:5173</p>
            <Win title="robo-games — localhost:5173" minH={110}>
              {mounted ? <div style={{ maxWidth: 165 }}><RoCard name="Tower of Hell" /></div> : <p style={{ color: T.ink2, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13 }}>{tr2({ uz: "Sahifa bo'sh — kartochka hali qo'shilmagan…", ru: "Страница пуста — карточка ещё не добавлена…" })}</p>}
            </Win>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Kartochka ekranga chiqdi (Mount) → effect <b>bir marta</b> ishladi va salom yozdi. E'tibor bering: 👍 bosilsa effect <b>qayta ishlamaydi</b> — <span className="mono">[]</span> shuni anglatadi.</>, ru: <>Карточка появилась на экране (родилась) → эффект сработал <b>один раз</b> и написал привет. Обратите внимание: при нажатии 👍 эффект <b>не срабатывает снова</b> — вот что означает <span className="mono">[]</span>.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [likes, setLikes] = useState3(storedAnswer ? 3 : 0);
  const done = likes >= 3;
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  return <Stage eyebrow="Effect + State" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : `${tr2({ uz: "👍 ni 3 marta bosing", ru: "Нажмите 👍 3 раза" })} (${likes}/3)`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Like soni <span className="italic" style={{ color: T.accent }}>tab sarlavhasiga</span> chiqsinmi?</>, ru: <>Показать число лайков <span className="italic" style={{ color: T.accent }}>в заголовке вкладки</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <pre className="code-box fade-up delay-1">
              ...
              {"}, "}<At>[likes]</At>{");  "}<Cm>{tr2({ uz: "// likes o'zgarsa — qayta ishla", ru: "// изменился likes — сработай снова" })}</Cm>
            </pre>
            <div className="hint fade-up delay-2"><p className="small" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: <><span className="mono">[]</span> — faqat ekranga chiqqanda · <span className="mono">[likes]</span> — likes har o'zgarganda · ikkalasi ham useEffect'ning "qachon ishlash" sozlamasi.</>, ru: <><span className="mono">[]</span> — только при появлении на экране · <span className="mono">[likes]</span> — при каждом изменении likes · и то и другое — настройка useEffect «когда срабатывать».</> })}</p></div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "📑 Brauzer tab sarlavhasi", ru: "📑 Заголовок вкладки браузера" })}</p>
            <div className="tab-preview fade-up delay-1">
              ...
              {likes > 0 && <span className="tab-cue fade-step">{tr2({ uz: "← useEffect yangiladi", ru: "← обновил useEffect" })}</span>}
            </div>
...
var Screen9 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <><span className="mono" style={{ color: T.accent }}>useEffect(…, [])</span> qachon ishlaydi?</>, ru: <>Когда срабатывает <span className="mono" style={{ color: T.accent }}>useEffect(…, [])</span>?</> })}</h2></>}
  options={[tr2({ uz: "Har safar 👍 bosilganda", ru: "При каждом нажатии 👍" }), tr2({ uz: "Hech qachon ishlamaydi", ru: "Никогда не срабатывает" }), tr2({ uz: "Komponent ekranga chiqqanda bir marta", ru: "Один раз, когда компонент появляется на экране" }), tr2({ uz: "Faqat komponent o'chirilganda", ru: "Только когда компонент удаляется" })]}
  correctIdx={2}
  explainCorrect={tr2({ uz: "To'g'ri! [] bo'sh massiv = kuzatadigan hech narsa yo'q — effect faqat komponent ekranga chiqqanda (Mount) bir marta ishlaydi.", ru: "Верно! Пустой массив [] = следить не за чем — эффект срабатывает только один раз, при появлении компонента на экране (Mount)." })}
  explainWrong={{
    0: tr2({ uz: "Bu [likes] bo'lganda shunday bo'lardi. [] bo'sh — faqat ekranga chiqqanda bir marta.", ru: "Так было бы с [likes]. Пустой [] — только один раз при появлении на экране." }),
    1: tr2({ uz: "Ishlaydi — lekin faqat bir marta, komponent ekranga birinchi chiqqanda.", ru: "Срабатывает — но только один раз, когда компонент впервые появляется на экране." }),
    3: tr2({ uz: "Yo'q — ekrandan ketish Unmount. [] bilan effect ekranga chiqqanda (Mount) ishlaydi.", ru: "Нет — уход с экрана это Unmount. С [] эффект срабатывает при появлении на экране (Mount)." }),
    default: tr2({ uz: "[] = faqat birinchi chizilganda, bir marta.", ru: "[] = только при первой отрисовке, один раз." })
  }}
/>;
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Hayot yo'li", ru: "Жизненный путь" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "3 bosqichni o'tkazing", ru: "Пройдите 3 этапа" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Komponent ham o'yinchiday <span className="italic" style={{ color: T.accent }}>yashaydimi</span>?</>, ru: <>Компонент тоже <span className="italic" style={{ color: T.accent }}>живёт</span>, как игрок?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              {PHASES.map((p, i) => <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 13px", borderRadius: 11, background: stage > i ? T.successSoft : T.bg, opacity: stage > i ? 1 : 0.55, transition: "all 0.4s" }}>
                  ...
                  <span style={{ display: "flex", flexDirection: "column" }}>
                    ...
                    <span style={{ fontFamily: "'Manrope',sans-serif", fontSize: 11.5, color: T.ink2 }}>{tr2(p.d)}</span>
                  </span>
                </div>)}
            </div>
            <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "Konsol", ru: "Консоль" })}</p>
            <div className="code-box" style={{ padding: "9px 13px", minHeight: 72 }}>
              {stage === 0 ? <TLine out={<span style={{ color: CODE.comment }}>{tr2({ uz: "kutilmoqda — o'yinchini kiriting…", ru: "ожидание — впустите игрока…" })}</span>} /> : LOGS.filter((l) => stage >= l.at).map((l, i) => <TLine key={i} out={<span style={{ color: l.color }}>✓ {tr2(l.text)}</span>} />)}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Server (sahifa)", ru: "Сервер (страница)" })}</p>
            <Win title="robo-games — localhost:5173" minH={110}>
              {stage >= 1 && stage < 3 ? <div style={{ maxWidth: 165 }}><RoCard name="Blox Fruits" likes={likes} onLike={() => {
    if (stage >= 1 && stage < 3) {
      setLikes((l) => l + 1);
      setStage((s) => Math.max(s, 2));
    }
  }} /></div> : <p style={{ color: T.ink2, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13 }}>{stage >= 3 ? tr2({ uz: "Kartochka ketdi — sahifa yana bo'sh. Hayot yo'li yakunlandi.", ru: "Карточка ушла — страница снова пуста. Жизненный путь завершён." }) : tr2({ uz: "Sahifa bo'sh…", ru: "Страница пуста…" })}</p>}
            </Win>
            <div className="fade-up delay-2" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {stage === 0 && <button className="btn" onClick={() => setStage(1)}>{tr2({ uz: "▶ O'yinchi kirdi — kartochkani qo'shish", ru: "▶ Игрок зашёл — добавить карточку" })}</button>}
              {stage === 1 && <p className="small" style={{ color: T.ink2, margin: 0, fontStyle: "italic" }}>{tr2({ uz: "↑ endi 👍 ni bosing — Update bosqichi", ru: "↑ теперь нажмите 👍 — этап Update" })}</p>}
              {stage === 2 && <button className="btn" onClick={() => setStage(3)}>{tr2({ uz: "✕ O'yinchi chiqdi — olib tashlash", ru: "✕ Игрок вышел — убрать" })}</button>}
            </div>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [likes, setLikes] = useState3(0);
  const [starred, setStarred] = useState3(false);
  ...
  return <Stage eyebrow={tr2({ uz: "Keyingi qadam · AI", ru: "Следующий шаг · AI" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Agent bilan ishlab ko'ring", ru: "Поработайте с агентом" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Jonli tugmani <span className="italic" style={{ color: T.accent }}>AI'ga yozdirsak</span>-chi?</>, ru: <>А что если <span className="italic" style={{ color: T.accent }}>поручить AI</span> написать живую кнопку?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Endi siz <b style={{ color: T.ink }}>state kodini o'qiy olasiz</b>! Buyruq bering, agent rejasini <b style={{ color: T.ink }}>tasdiqlang</b>, keyin kodini tekshiring: <span className="mono">useState</span> bormi, <span className="mono">set…</span> chaqirilganmi, <span className="mono">[]</span> to'g'rimi. Oxirida natijani <b style={{ color: T.ink }}>o'zingiz bosib sinang</b> — boshliq siz.</>, ru: <>Теперь вы <b style={{ color: T.ink }}>умеете читать код state</b>! Дайте команду, <b style={{ color: T.ink }}>утвердите</b> план агента, потом проверьте его код: есть ли <span className="mono">useState</span>, вызван ли <span className="mono">set…</span>, верный ли <span className="mono">[]</span>. В конце <b style={{ color: T.ink }}>сами проверьте результат нажатием</b> — начальник тут вы.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "1. Agentga so'z bilan ayting", ru: "1. Скажите агенту словами" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {TASKS.map((t) => <button key={t.id} className={`chip ${task === t.id ? "chip-on" : ""}`} onClick={() => choose(t.id)} style={{ justifyContent: "flex-start", textAlign: "left" }}>"{tr2(t.label)}"</button>)}
            </div>
            {!cur && <div className="frame-dash"><p className="small" style={{ color: T.ink2, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Yuqoridan bitta buyruqni tanlang", ru: "Выберите одну команду выше" })}</p></div>}
            {cur && <div className="ai-card fade-step" key={task || "stored"}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {cur.plan.map((p, i) => <div key={i} style={{ display: "flex", gap: 8, alignItems: "center", fontSize: 13 }}><span style={{ color: phase === "planned" ? T.ink3 : T.success }}>{phase === "planned" ? "○" : "✓"}</span><span style={{ color: T.ink }}>{tr2(p)}</span></div>)}
                </div>
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "2. Natija — bosib sinab ko'ring", ru: "2. Результат — проверьте нажатием" })}</p>
            <Win title="robo-games — localhost:5173" minH={130}>
              {done && cur ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {cur.id === "t2" && <div className="welcome-toast el-in">
                      <span className="wt-emoji">👋</span>
                      <div className="wt-text"><b>{tr2({ uz: "Xush kelibsiz!", ru: "Добро пожаловать!" })}</b><span>{tr2({ uz: "robo-games'ga qaytib keldingiz", ru: "вы вернулись в robo-games" })}</span></div>
                      <span className="wt-badge">{tr2({ uz: "[] · 1 marta", ru: "[] · 1 раз" })}</span>
                    </div>}
                ...
                </div> : <p style={{ color: T.ink2, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13 }}>{tr2({ uz: "Buyruq bering va rejani tasdiqlang…", ru: "Дайте команду и утвердите план…" })}</p>}
            </Win>
            {done ? <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Tugmani <b>o'zingiz bosib sinadingizmi</b>? Kod ham o'qildi: useState bor, set chaqirilgan. Agent to'g'ri qilganini endi <b>isbotlay olasiz</b>.</>, ru: <>Вы <b>сами проверили кнопку нажатием</b>? И код прочитан: useState есть, set вызван. Теперь вы можете <b>доказать</b>, что агент всё сделал правильно.</> })}</p></div> : <p className="body" style={{ margin: 0, color: T.ink2, fontSize: 13 }}>{tr2({ uz: "Natija shu yerda paydo bo'ladi — keyin uni o'zingiz sinaysiz.", ru: "Результат появится здесь — потом вы сами его проверите." })}</p>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen12 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <><span className="italic" style={{ color: T.accent }}>State</span> props'dan nimasi bilan farq qiladi?</>, ru: <>Чем <span className="italic" style={{ color: T.accent }}>state</span> отличается от props?</> })}</h2></>}
  options={[tr2({ uz: "Farqi yo'q — ikkalasi bir narsa", ru: "Ничем — это одно и то же" }), tr2({ uz: "State — o'z xotirasi; props — tashqaridan keladi", ru: "State — своя память; props приходит снаружи" }), tr2({ uz: "State faqat sonlarni saqlay oladi", ru: "State может хранить только числа" }), tr2({ uz: "Props ancha tezroq ishlaydi", ru: "Props работает гораздо быстрее" })]}
  correctIdx={1}
  explainCorrect={tr2({ uz: "To'g'ri! Props — tashqaridan keladigan ma'lumot (<GameCard name='…' />), state — komponentning ichki xotirasi (likes), uni komponent o'zi setLikes bilan o'zgartiradi.", ru: "Верно! Props — данные, приходящие снаружи (<GameCard name='…' />), state — внутренняя память компонента (likes), которую он сам меняет через setLikes." })}
  explainWrong={{
    0: tr2({ uz: "Farqi katta: props tashqaridan KELADI, state esa komponentning O'Z ichki xotirasi.", ru: "Разница большая: props ПРИХОДИТ снаружи, а state — СОБСТВЕННАЯ внутренняя память компонента." }),
    2: tr2({ uz: "Yo'q — state istalgan narsani saqlaydi: son, matn, true/false (starred esingizdami?).", ru: "Нет — state хранит что угодно: число, текст, true/false (помните starred?)." }),
    3: tr2({ uz: "Yo'q — tezlik bir xil. Farq manbada: props tashqaridan, state ichkaridan.", ru: "Нет — скорость одинаковая. Разница в источнике: props снаружи, state изнутри." }),
    default: tr2({ uz: "State — ichki xotira (o'zi o'zgartiradi), props — tashqaridan kelgan ma'lumot.", ru: "State — внутренняя память (меняет сам), props — данные, переданные снаружи." })
  }}
/>;
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const INITS = [0, 10, 100];
  const STEPS = [1, 5, 10];
  const [init, setInit] = useState3(0);
  const [step, setStep] = useState3(1);
  const [clicks, setClicks] = useState3(0);
  const [initTried, setInitTried] = useState3(storedAnswer ? /* @__PURE__ */ new Set([0, 10]) : /* @__PURE__ */ new Set([0]));
  const [stepTried, setStepTried] = useState3(storedAnswer ? /* @__PURE__ */ new Set([1, 5]) : /* @__PURE__ */ new Set([1]));
  ...
  return <Stage eyebrow={tr2({ uz: "Amaliyot · sozlash", ru: "Практика · настройка" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Sozlang va bosib sinang", ru: "Настройте и проверьте нажатием" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>O'z <span className="italic" style={{ color: T.accent }}>like hisoblagichingizni</span> sozlay olasizmi?</>, ru: <>Сможете настроить <span className="italic" style={{ color: T.accent }}>свой счётчик лайков</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Endi o'zingiz boshqaring! <b style={{ color: T.ink }}>Boshlang'ich qiymat</b>ni (useState ichidagi son) va <b style={{ color: T.ink }}>qadam</b>ni tanlang, keyin 👍 bosib sinang. Kod qanday o'zgarishini kuzating.</>, ru: <>Теперь управляйте сами! Выберите <b style={{ color: T.ink }}>начальное значение</b> (число внутри useState) и <b style={{ color: T.ink }}>шаг</b>, затем проверьте нажатием 👍. Следите, как меняется код.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Boshlang'ich qiymat — useState(?)", ru: "Начальное значение — useState(?)" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {INITS.map((v) => <button key={v} className="gchip" style={init === v ? { background: T.accent, color: "#fff" } : void 0} onClick={() => pickInit(v)}>{v} {initTried.has(v) ? "✓" : ""}</button>)}
            </div>
            <p className="flow-label" style={{ marginTop: 2 }}>{tr2({ uz: "Qadam — setLikes(likes + ?)", ru: "Шаг — setLikes(likes + ?)" })}</p>
            <div className="fade-up delay-2" style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {STEPS.map((v) => <button key={v} className="gchip" style={step === v ? { background: T.accent, color: "#fff" } : void 0} onClick={() => pickStep(v)}>+{v} {stepTried.has(v) ? "✓" : ""}</button>)}
            </div>
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "Hisoblagichingiz — bosib sinang", ru: "Ваш счётчик — проверьте нажатием" })}</p>
            <Win title="robo-games — localhost:5173" minH={100}>
              <div style={{ maxWidth: 185 }}><RoCard key={`${init}-${step}`} name="Tower of Hell" likes={likes} onLike={() => setClicks((c) => c + 1)} /></div>
            </Win>
            <span className="tagpill fade-step" style={{ color: done ? T.success : T.ink }}>{initTried.size >= 2 ? "✓" : "○"} {tr2({ uz: "boshlang'ich", ru: "начальное" })} · {stepTried.size >= 2 ? "✓" : "○"} {tr2({ uz: "qadam", ru: "шаг" })} · {clicks >= 1 ? "✓" : "○"} {tr2({ uz: "sinash", ru: "проверка" })}</span>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Siz hozir state'ni <b>to'liq boshqardingiz</b>: boshlang'ich qiymat, o'zgarish qadami va qayta chizish — hammasi qo'lingizda.</>, ru: <>Вы только что <b>полностью управляли state</b>: начальное значение, шаг изменения и перерисовка — всё в ваших руках.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [picked, setPicked] = useState3(storedAnswer ? "mut" : null);
  ...
  return <Stage eyebrow={tr2({ uz: "Debugging", ru: "Отладка" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : found ? tr2({ uz: "Endi tuzating", ru: "Теперь исправьте" }) : tr2({ uz: "Xatoni toping", ru: "Найдите ошибку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            <div className="ai-card fade-up delay-2">
              <div className="ai-row"><span className="ai-badge">AI</span><span className="ai-bubble">{tr2({ uz: "Like tugmasini yozdim:", ru: "Я написал кнопку лайка:" })}</span></div>
              <div className="ai-code">
                <div className={`ai-line ${picked === "st" ? "ok" : ""}`} onClick={() => {
    if (!found) setPicked("st");
  }}><Jx>{"const"}</Jx>{" ["}<At>likes</At>{", "}<At>setLikes</At>{"] = "}<Jx>{"useState"}</Jx>{"("}<St>0</St>{");"}</div>
                <div className={`ai-line ${picked === "btn" ? "ok" : ""}`} onClick={() => {
    if (!found) setPicked("btn");
  }}><Jx>{"<button"}</Jx> <At>onClick</At>{"={() => {"}</div>
                {!fixed ? <div className={`ai-line ${found ? "bad" : ""}`} onClick={pickMut}>{"  likes = likes + "}<St>1</St>{";  "}<Cm>{tr2({ uz: "// yangilash", ru: "// обновление" })}</Cm></div> : <div className="ai-line ok el-in">{"  setLikes(likes + "}<St>1</St>{");  "}<Cm>{tr2({ uz: "// yangilash", ru: "// обновление" })}</Cm></div>}
                <div className={`ai-line ${picked === "jsx" ? "ok" : ""}`} onClick={() => {
    if (!found) setPicked("jsx");
  }}>{"}}"}<Jx>{">"}</Jx>{"👍 {likes}"}<Jx>{"</button>"}</Jx></div>
              </div>
              {!found && <p className="ai-prompt">{tr2({ uz: `Qaysi qator ekranni "qotirib" qo'ydi? Bosing.`, ru: "Какая строка «заморозила» экран? Нажмите на неё." })}</p>}
              {found && !fixed && <button className="btn fade-step" style={{ alignSelf: "flex-start" }} onClick={fix}>{tr2({ uz: "🔧 setLikes'ga almashtirish", ru: "🔧 Заменить на setLikes" })}</button>}
              {fixed && <p className="ai-prompt" style={{ color: T.success, fontStyle: "normal", fontWeight: 600 }}>{tr2({ uz: "✓ Tuzatildi — endi React xabardor!", ru: "✓ Исправлено — теперь React в курсе!" })}</p>}
            </div>
          </Col>
          <Col>
            {!found && (picked === "st" || picked === "btn" || picked === "jsx" ? <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Bu qator to'g'ri{picked === "st" ? " — useState joyida" : picked === "jsx" ? <> — <span className="mono">{"{likes}"}</span> to'g'ri o'qilyapti</> : ""}. Yana qarang: qiymat <b>qanday o'zgartirilyapti</b>?</>, ru: <>Эта строка верная{picked === "st" ? " — useState на месте" : picked === "jsx" ? <> — <span className="mono">{"{likes}"}</span> читается правильно</> : ""}. Посмотрите ещё раз: <b>как меняется</b> значение?</> })}</p></div> : <div className="hint"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: <>Eslang, 1-versiyadagi qotib qolgan tugmani: <b style={{ color: T.ink }}>to'g'ridan-to'g'ri o'zgartirishni React sezmaydi</b>. Qaysi qator xuddi shunday qilyapti?</>, ru: <>Вспомните застывшую кнопку из 1-й версии: <b style={{ color: T.ink }}>прямое изменение React не замечает</b>. Какая строка делает то же самое?</> })}</p></div>)}
            {found && !fixed && <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.accent }}>{tr2({ uz: "✓ Topdingiz!", ru: "✓ Нашли!" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <><span className="mono">likes = likes + 1</span> — xotirada o'zgaradi, lekin React'ga <b>xabar bermaydi</b> — ekran qotib qoladi. To'g'risi: <span className="mono">setLikes(likes + 1)</span>. Chapdagi tugma bilan tuzating →</>, ru: <><span className="mono">likes = likes + 1</span> — меняет память, но <b>не сообщает</b> React — экран застывает. Правильно так: <span className="mono">setLikes(likes + 1)</span>. Исправьте кнопкой слева →</> })}</p></div>}
            {fixed && <>
                <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "Endi ishlaydi — bosib sinang", ru: "Теперь работает — проверьте нажатием" })}</p>
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!passed} label={passed ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Xotira qatorini yozing", ru: "Напишите строку памяти" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Oxirgi qadam: xotirani <span className="italic" style={{ color: T.accent }}>o'zingiz</span> yarating.</>, ru: <>Последний шаг: создайте память <span className="italic" style={{ color: T.accent }}>сами</span>.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            <div className="fade-up delay-2" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <span className="tagpill" style={{ opacity: hasPair ? 1 : 0.4 }}>{hasPair ? "✓" : "1"} {tr2({ uz: "[likes, setLikes] juftlik", ru: "пара [likes, setLikes]" })}</span>
              <span className="tagpill" style={{ opacity: hasHook ? 1 : 0.4 }}>{hasHook ? "✓" : "2"} = useState(…)</span>
              <span className="tagpill" style={{ opacity: hasInit ? 1 : 0.4 }}>{hasInit ? "✓" : "3"} {tr2({ uz: "boshlang'ich (0)", ru: "начальное (0)" })}</span>
            </div>
            {lowerHook && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Deyarli! <span className="mono">useState</span> — <b>S katta harf</b> bilan yoziladi.</>, ru: <>Почти! <span className="mono">useState</span> пишется с <b>большой буквой S</b>.</> })}</p></div>}
            {passed && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "✓ Mukammal! Xotira yaratildi — tugma jonlandi. Endi siz state'ni noldan yoza olasiz.", ru: "✓ Отлично! Память создана — кнопка ожила. Теперь вы можете написать state с нуля." })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "natija — bosib sinang", ru: "результат — проверьте нажатием" })}</p>
            <Win title="robo-games — localhost:5173" minH={130}>
              {valid ? <div className="fade-step" style={{ maxWidth: 165 }}><RoCard name="Blox Fruits" likes={likes} onLike={() => setLikes((l) => l + 1)} /></div> : <p style={{ fontFamily: "Georgia, serif", color: T.ink2, fontStyle: "italic", margin: 0, textAlign: "center", lineHeight: 1.5 }}>{tr2({ uz: <>2-qator yozilmaguncha tugma ishlamaydi: <span className="mono" style={{ fontStyle: "normal" }}>likes</span> degan xotira yo'q…</>, ru: <>Пока 2-я строка не написана, кнопка не работает: памяти по имени <span className="mono" style={{ fontStyle: "normal" }}>likes</span> нет…</> })}</p>}
            </Win>
          </Col>
        </div>
        </Zoomable>
  ...
  return <Stage eyebrow={tr2({ uz: "Amaliyot · VS Code", ru: "Практика · VS Code" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Avval bajaring", ru: "Сначала выполните" })} onClick={onNext} /></>}>
    ...
    return <button key={i} className={`lp-step ${on ? "on" : ""}`} onClick={() => toggle(i)}>
            ...
            {!isMentor && <button className={`lp-done-btn ${done ? "is-done" : ""}`} disabled={done} onClick={complete}>
              {done ? tr2({ uz: "✓ Bajarildi — ustozni kuting", ru: "✓ Выполнено — дождитесь наставника" }) : tr2({ uz: "✅ Bajardim", ru: "✅ Выполнил" })}
            </button>}
...
var ScreenPractice1 = (props) => <ScreenLivePractice
  {...props}
  title={{ uz: "Like tugmasini jonlantiring", ru: "Оживите кнопку лайка" }}
  task={{ uz: "robo-games loyihangizda GameCard komponentiga useState bilan ishlaydigan 👍 like tugmasini qo'shing — bosilganda son o'zi o'ssin.", ru: "В вашем проекте robo-games добавьте компоненту GameCard работающую кнопку 👍 лайка на useState — при нажатии число должно расти само." }}
  checklist={[
    { uz: "App faylida yuqoriga `import { useState } from 'react'` yozing", ru: "В начале файла App напишите `import { useState } from 'react'`" },
    { uz: "Komponent ichida `const [likes, setLikes] = useState(0)` e'lon qiling", ru: "Внутри компонента объявите `const [likes, setLikes] = useState(0)`" },
    { uz: "Tugmaga `onClick={() => setLikes(likes + 1)}` bog'lang", ru: "Привяжите к кнопке `onClick={() => setLikes(likes + 1)}`" },
    { uz: "👍 {likes} chiqaring — bosganingizda son o'sishini tekshiring", ru: "Выведите 👍 {likes} — проверьте, что при нажатии число растёт" }
  ]}
/>;
var ScreenPractice2 = (props) => <ScreenLivePractice
  {...props}
  title={{ uz: "⭐ Sevimli toggle qo'shing", ru: "⭐ Добавьте переключатель избранного" }}
  task={{ uz: "GameCard'ga ikkinchi state qo'shing: ⭐ sevimlilar tugmasi. Bosilganda true/false o'zgarsin va yulduzcha ko'rinishi almashsin.", ru: "Добавьте GameCard второй state: кнопку ⭐ избранного. При нажатии true/false должно переключаться, а вид звёздочки — меняться." }}
  checklist={[
    { uz: "`const [starred, setStarred] = useState(false)` e'lon qiling", ru: "Объявите `const [starred, setStarred] = useState(false)`" },
    { uz: "Tugmaga `onClick={() => setStarred(!starred)}` bog'lang", ru: "Привяжите к кнопке `onClick={() => setStarred(!starred)}`" },
    { uz: "Shartli ko'rsating: `{starred ? '⭐' : '☆'}`", ru: "Покажите по условию: `{starred ? '⭐' : '☆'}`" },
    { uz: "Bosib sinang — like va yulduz bir-biriga xalaqit bermasin", ru: "Проверьте нажатием — лайк и звёздочка не должны мешать друг другу" }
  ]}
/>;
var ACHIEVEMENTS = {
  reactive: { icon: "⚡", name: "Reactive!", desc: { uz: "State siklini o'zlashtirdingiz", ru: "Вы освоили цикл state" } },
  effectPro: { icon: "🎯", name: "Effect Pro!", desc: { uz: "useEffect qachon ishlashini bildingiz", ru: "Вы узнали, когда срабатывает useEffect" } },
  builder: { icon: "🧱", name: "State Builder!", desc: { uz: "useState qatorini o'zingiz yozdingiz", ru: "Вы сами написали строку useState" } },
  graduate: { icon: "🏆", name: "Level Up!", desc: { uz: "State va Effect darsini to'liq yakunladingiz", ru: "Вы полностью прошли урок State и Effect" } }
};
var ACH_TRIGGERS = { s9: "effectPro", s12: "reactive", s15: "builder" };
...
var Q_LABELS = { 4: "1 — useState", 6: "2 — setLikes", 11: "3 — useEffect []", 15: "4 — State vs props", 18: { uz: "5 — Yakuniy amaliy", ru: "5 — Финальная практика" } };
var QUIZ_MS = 15e3;
var QZ_BG_SHAPES = [
  { ch: "useState", l: 5, t: 10, s: 30, d: 19, dl: 0 },
  { ch: "setLikes", l: 84, t: 7, s: 30, d: 23, dl: 1.5 },
  { ch: "👍", l: 8, t: 72, s: 30, d: 27, dl: 0.8 },
  { ch: "useEffect", l: 78, t: 68, s: 34, d: 21, dl: 2.2 },
  { ch: "render", l: 44, t: 86, s: 30, d: 25, dl: 1.1 },
  { ch: "state", l: 66, t: 26, s: 30, d: 17, dl: 0.4 },
  { ch: "[likes]", l: 26, t: 34, s: 26, d: 20, dl: 1.9 },
  { ch: "mount", l: 55, t: 5, s: 26, d: 22, dl: 0.6 },
  { ch: "⚛", l: 91, t: 42, s: 36, d: 24, dl: 1.3 },
  { ch: "Unmount", l: 2, t: 45, s: 22, d: 26, dl: 2.6 }
];
var QUIZ_BANK = [
  { q: { uz: "useState nimani qaytaradi?", ru: "Что возвращает useState?" }, opts: [{ uz: "Qiymat va yangilovchi funksiya juftligini", ru: "Пару: значение и функцию-обновитель" }, { uz: "Faqat boshlang'ich raqamli qiymatni", ru: "Только начальное числовое значение" }, { uz: "Butunlay yangi HTML sahifani", ru: "Совершенно новую HTML-страницу" }, { uz: "To'liq CSS uslublar to'plamini", ru: "Полный набор CSS-стилей" }], correct: 0 },
  { q: { uz: "Oddiy `let likes` o'zgarsa, React nima qiladi?", ru: "Что делает React, если обычная `let likes` изменилась?" }, opts: [{ uz: "Ekranni darhol yangilaydi", ru: "Сразу обновляет экран" }, { uz: "Hech narsa — sezmay qoladi", ru: "Ничего — не замечает" }, { uz: "Darhol xatolik chiqaradi", ru: "Сразу выдаёт ошибку" }, { uz: "Sahifani qayta yuklaydi", ru: "Перезагружает страницу" }], correct: 1 },
  ...
  { q: { uz: "Xotirada son o'sdi, ekranda esa o'smadi — nega?", ru: "Число выросло в памяти, но не на экране — почему?" }, opts: [{ uz: "Internet juda sekin edi", ru: "Интернет был очень медленным" }, { uz: "set yo'q — React chizmadi", ru: "Нет set — React не перерисовал" }, { uz: "Kod butunlay buzuq edi", ru: "Код был полностью сломан" }, { uz: "Brauzer juda eskirgan", ru: "Браузер слишком старый" }], correct: 1 },
  { q: { uz: "useEffect ichidagi `return () => {...}` nima uchun?", ru: "Зачем нужен `return () => {...}` внутри useEffect?" }, opts: [{ uz: "Yangi state yaratish uchun", ru: "Чтобы создать новый state" }, { uz: "Qiymat qaytarish uchun", ru: "Чтобы вернуть значение" }, { uz: "Tozalash uchun, unmount'da", ru: "Для уборки, на unmount" }, { uz: "Element rang berish uchun", ru: "Чтобы раскрасить элемент" }], correct: 2 },
  { q: { uz: "Bitta komponentda nechta useState ishlatsa bo'ladi?", ru: "Сколько useState можно использовать в одном компоненте?" }, opts: [{ uz: "Faqat bitta xotira", ru: "Только одну ячейку памяти" }, { uz: "Ko'pi bilan ikkita", ru: "Максимум две" }, { uz: "Faqat funksiyadan tashqarida", ru: "Только вне функции" }, { uz: "Istagancha ko'p, alohida xotira", ru: "Сколько угодно, каждая — отдельная память" }], correct: 3 }
];
...
function QzFX() {
  ...
  useEffect4(() => {
    ...
    const TOK = ["useState", "setLikes", "useEffect", "render", "👍", "[likes]", "state", "mount"];
    const em = [], toks = [];
...
function QuizArena({ live, onClose, startSolo }) {
  ...
  return <div className="qz-arena">
      ...
      {phase === "lobby" && <div className="qz-view fade-step">
          <CsWordmark />
          <p className="qz-sub" style={{ marginTop: -4 }}>{tr2({ uz: "Tezroq to'g'ri bossangiz — ko'proq ball. Ketma-ket to'g'ri javoblar 🔥 bonus beradi!", ru: "Быстрее нажали верно — больше баллов. Верные ответы подряд дают 🔥 бонус!" })}</p>
    ...
    return <button key={i} className={`qz-tile ${my ? pickedThis ? "picked" : "faded" : ""}`} style={{ background: QUIZ_COLORS[i] }} disabled={isMentor || !!my} onClick={() => answer(i)}>
          ...
          {my && !isMentor && !solo && <p className="qz-waitmsg">{tr2({ uz: "✔ Javob qabul qilindi — natijani kuting…", ru: "✔ Ответ принят — дождитесь результата…" })}</p>}
    ...
    return <div key={i} className={`qz-tile rv ${win ? "win" : "lose"} ${pickedThis ? "picked" : ""}`} style={{ background: QUIZ_COLORS[i] }}>
      ...
      {phase === "done" && <div className="qz-view fade-step">
          ...
          {solo ? <div className="qz-solo-res">
              ...
              <p className="qz-sub">{tr2({ uz: "ball", ru: "баллов" })} · {soloScore.ok}/{QUIZ_BANK.length} {tr2({ uz: "to'g'ri", ru: "верно" })}{soloScore.maxStreak >= 2 ? ` · ${tr2({ uz: "eng uzun streak", ru: "самый длинный streak" })} 🔥x${soloScore.maxStreak}` : ""}</p>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  const HOMEWORK = [
    { b: { uz: "Jonli like", ru: "Живой лайк" }, t: { uz: "— robo-games loyihangizda GameCard'ga useState bilan ishlaydigan 👍 qo'shing", ru: "— в вашем проекте robo-games добавьте GameCard работающий 👍 на useState" } },
    { b: { uz: "Ikkinchi xotira", ru: "Вторая память" }, t: { uz: "— ⭐ sevimlilar tugmasi: bosilganda yulduzcha yonsin/o'chsin (true/false)", ru: "— кнопка ⭐ избранного: при нажатии звёздочка загорается/гаснет (true/false)" } },
    { b: { uz: "Tab kuzatuvchisi", ru: "Наблюдатель вкладки" }, t: { uz: "— useEffect bilan like soni brauzer tab sarlavhasiga chiqsin", ru: "— через useEffect выведите число лайков в заголовок вкладки браузера" } }
  ];
  ...
  return <Stage eyebrow={tr2({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Kartochkangiz <span className="italic" style={{ color: T.accent }}>jonlandi</span>.</>, ru: <>Ваша карточка <span className="italic" style={{ color: T.accent }}>ожила</span>.</> })}</h2>{
    /* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>{tr2({ uz: "📝 Uyga vazifa", ru: "📝 Домашнее задание" })}</div><p className="body" style={{ margin: "0 0 10px", color: T.ink }}>{tr2({ uz: "Antigravity bilan o'z loyihangizda sinang:", ru: "Попробуйте в своём проекте вместе с Antigravity:" })}</p><ul>{HOMEWORK.map((h, i) => <li key={i}><b>{tr2(h.b)}</b> <span className="t">{tr2(h.t)}</span></li>)}</ul><p className="hw-note">{tr2({ uz: "Keyingi darsda komponentlar jamoa bo'lib ishlaydi — ma'lumot bittasidan ikkinchisiga oqadi! 🚀", ru: "На следующем уроке компоненты заработают командой — данные потекут от одного к другому! 🚀" })}</p></div>}
...
function ReactStateEffectLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  const screens = [Screen0, Screen1, Screen2, Screen3, Screen4, Screen5, Screen5b, Screen6, ScreenPractice1, Screen7, Screen8, Screen9, Screen10, ScreenPractice2, Screen11, Screen12, Screen13, Screen14, Screen15, ScreenPodium, ScreenFlashcards, Screen16];
  ...
  return <LangContext.Provider value={lang}>
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={{ uz: "State va Effect darsi", ru: "Урок State и Effect" }} /> : <>
...
export {
  ReactStateEffectLesson as default
};
