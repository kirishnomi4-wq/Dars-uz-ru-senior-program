...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и число ✅/❌ скрыты — при нажатии «Открыть результат» всё откроется сразу и у вас, и на экранах учеников." })}</p>}
...
function CardFactory({
  ...
  const renderSlot = (c, i) => {
    ...
    return <div
      ...
      title={mode === "oneway" ? tr({ uz: "Teshikka qaytarib sudrab ko'ring", ru: "Попробуйте перетащить обратно в щель" }) : void 0}
    >
  ...
  return <div className={`cf cf--${mode} ${humming ? "cf-hum" : ""} ${jammed ? "cf-jam" : ""}`}>
      ...
      <div className="cf-body">
        ...
        {mode === "feeder" && <button type="button" className={`cf-lever ${leverPulled ? "pulled" : ""}`} onClick={onLever} aria-label={tr({ uz: "Richagni torting", ru: "Потяните рычаг" })}>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Uzatish", ru: "Передача" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Varaqani yuboring", ru: "Отправьте посылку" })} onClick={onNext} /></>}>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Bir nechta props", ru: "Несколько props" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "3 ta props'ni to'ldiring", ru: "Заполните 3 props" })} onClick={onNext} /></>}>
...
var Screen4 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: 'Atribut orqali: name="Doors"', ru: 'Через атрибут: name="Doors"' }), tr({ uz: "Internet orqali yuboradi", ru: "Отправляет через интернет" }), tr({ uz: "Fayl orqali saqlab beradi", ru: "Сохраняет в файл" }), tr({ uz: "GameCard o'zi App'dan olib ketadi", ru: "GameCard сам забирает из App" })]}
  correctIdx={0}
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [flow, setFlow] = useState(storedAnswer ? 2 : 0);
  const [running, setRunning] = useState(false);
  ...
  />
          ...
          <Col>
            ...
            <div className="fade-up delay-2" style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              ...
              <button className={reversed ? "btn-soft btn-did" : flow >= 2 ? "btn btn-turn" : "btn-soft"} style={{ alignSelf: "flex-start" }} onClick={sendUp} disabled={flow < 2} title={flow < 2 ? tr({ uz: "Avval pastga yuboring", ru: "Сначала отправьте вниз" }) : void 0}>{tr({ uz: "⬆ Boladan otaga yuborib ko'rish", ru: "⬆ Попробовать от ребёнка родителю" })} {reversed ? "✓" : ""}</button>
            </div>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(storedAnswer ? new Set(LIST.map((g) => g.name)) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={tr({ uz: "Ma'lumotlar ro'yxati", ru: "Список данных" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: `${seen.size}/3 qatorni ko'ring`, ru: `Посмотрите строки: ${seen.size}/3` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            ...
            <pre className="code-box fade-up delay-1" style={{ lineHeight: 1.9 }}>
              <Jx>{"const"}</Jx>{" games = ["}{"\n"}
              {LIST.map((g, i) => <React.Fragment key={g.name}>
          ...
          <Col>
            ...
            {active ? <div className="fade-step" key={active} style={{ maxWidth: 160 }}>
              ...
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Chapdan qatorni bosing", ru: "Нажмите строку слева" })}</p></div>}
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const LIST = GAMES.slice(0, 3);
  const [shown, setShown] = useState(storedAnswer ? 3 : 0);
  const [running, setRunning] = useState(false);
  const timer = useRef(null);
  const done = shown >= 3;
  ...
  return <Stage eyebrow={tr({ uz: "Katalog", ru: "Каталог" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Katalogni chizing", ru: "Нарисуйте каталог" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen9 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "Katalog kodini ham qayta yozish kerak", ru: "Придётся переписать и код каталога" }), tr({ uz: "Hech narsa — sayt eski holatda qoladi", ru: "Ничего — сайт останется прежним" }), tr({ uz: "Butun katalog buzilib qoladi", ru: "Весь каталог сломается" }), tr({ uz: "React yangi kartochkani o'zi chizadi", ru: "React сам нарисует новую карточку" })]}
  correctIdx={3}
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Yangi o'yin", ru: "Новая игра" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: `Kamida 2 ta qo'shing (${added.length}/2)`, ru: `Добавьте минимум 2 (${added.length}/2)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen12 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "Ha — xohlagancha o'zgartiradi", ru: "Да — меняет сколько угодно" }), tr({ uz: "Yo'q — props faqat o'qiladi", ru: "Нет — props только читаются" }), tr({ uz: "Faqat kichik harf bilan yozsa bo'ladi", ru: "Можно, только строчными буквами" }), tr({ uz: "Faqat kechasi o'zgartiradi", ru: "Меняет только по ночам" })]}
  correctIdx={1}
        ...
        <div className="split">
          ...
          <Col>
            ...
            <Win title="mening-katalogim — localhost:5173" minH={120}>
              {mine.length === 0 ? <p style={{ color: T.ink3, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13 }}>{tr({ uz: "Bo'sh — ro'yxatga o'yin qo'shing…", ru: "Пусто — добавьте игры в список…" })}</p> : <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Debugging", ru: "Дебаггинг" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Xatoni toping", ru: "Найдите ошибку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!passed} label={passed ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Konveyer qatorini yozing", ru: "Напишите строку конвейера" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
function DebugChallenge({ lines, fixed, explain, onSolved }) {
  ...
  return <div className="dbg fade-up">
      ...
      {!solved ? <p className="dbg-hint">{tr({ uz: "👆 Xato bor qatorni toping va bosing", ru: "👆 Найдите строку с ошибкой и нажмите её" })}</p> : <div className="dbg-ok">{tr({ uz: "✓ Topdingiz!", ru: "✓ Нашли!" })} {explain}</div>}
    </div>;
}
...
function Flashcards({ cards }) {
  ...
  return <div className="fc fade-up">
      ...
      {flipped ? <div className="fc-actions"><button className="fc-btn again" disabled={!!exiting} onClick={again}>{tr({ uz: "✗ Takrorlash", ru: "✗ Повторить" })}</button><button className="fc-btn knew" disabled={!!exiting} onClick={knew}>{tr({ uz: "✓ Bildim", ru: "✓ Знаю" })}</button></div> : <p className="fc-hint">{tr({ uz: "👆 Kartani bosing — javobni ko'rasiz", ru: "👆 Нажмите карту — увидите ответ" })}</p>}
    </div>;
}
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tayyor", ru: "Готово" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Katalogingiz <span className="italic" style={{ color: T.accent }}>tayyor</span>.</>, ru: <>Ваш каталог <span className="italic" style={{ color: T.accent }}>готов</span>.</> })}</h2>{
...
function ReactPropsReuseLesson({ lang: langProp, onFinished }) {
  ...
  return <LangContext.Provider value={lang}>
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={tr({ uz: "React darsi", ru: "Урок React" })} /> : <>
