...
//  Manba:  src/3-Modull/ReactPropsReuseLesson.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/3-Modull/ReactPropsReuseLesson.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/3-Modull/ReactPropsReuseLesson.jsx
...
var LESSON_META = { lessonId: "react-props-reuse-04-v18", lessonTitle: { uz: "Props va qayta ishlatish", ru: "Props и переиспользование" } };
var HW_TOKENS = [
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlab oling.</>, ru: <>⚠️ Только <b>{pct}%</b> верных — тема осталась классу непонятной. Перед продолжением рекомендуем короткое повторение.</> })}</p>
...
var gameByName = (nm) => [...GAMES, ...EXTRA].find((g) => g.name.toLowerCase() === String(nm).toLowerCase());
var RoCard = ({ name, emoji, players, top, xray, onClick }) => {
  ...
  return <div className={`rocard el-in ${onClick ? "tappable" : ""}`} onClick={onClick} style={{ position: "relative" }}>
...
function CardFactory({
  ...
  const renderSlot = (c, i) => {
    ...
    return <div
      ...
      key={`${mode}-${i}-${nm}`}
      draggable={mode === "oneway" ? true : void 0}
      onDragStart={mode === "oneway" ? (e) => {
        e.preventDefault();
        onJam && onJam();
      } : void 0}
      onClick={mode === "oneway" && onJam ? onJam : void 0}
      title={mode === "oneway" ? tr2({ uz: "Teshikka qaytarib sudrab ko'ring", ru: "Попробуйте перетащить обратно в щель" }) : void 0}
    >
  ...
  return <div className={`cf cf--${mode} ${humming ? "cf-hum" : ""} ${jammed ? "cf-jam" : ""}`}>
      <div className="cf-meter">
        <span className="cf-meter-i">{tr2({ uz: "Mashina", ru: "Машина" })} <b>1</b></span><span className="cf-dot">·</span>
        <span className="cf-meter-i">{tr2({ uz: "Ishlatildi", ru: "Запусков" })} <b>{usedN}</b></span><span className="cf-dot">·</span>
        <span className="cf-meter-i">{tr2({ uz: "Kartochka", ru: "Карточек" })} <b>{count}</b></span>
      </div>
      <div className="cf-top">
        {mode === "feeder" && hopper.length > 0 && <div className="cf-hopper">{hopper.map((h, i) => <span className="cf-hopper-slip" key={i} style={{ zIndex: hopper.length - i }}>{h}</span>)}</div>}
        <span className="cf-hole" />
        {slip && <div key={`slip-${feed}`} className={`cf-slip ${slipIn ? "cf-slip-in" : ""} ${jammed ? "cf-slip-jam" : ""}`}>{slip}</div>}
        {jammed && <span className="cf-jam-note el-in">{tr2({ uz: "⛔ faqat yuqoridan pastga", ru: "⛔ только сверху вниз" })}</span>}
      </div>
      <div className="cf-body">
        <div className="cf-plate">{"<GameCard />"}</div>
        {mode === "feeder" && <button type="button" className={`cf-lever ${leverPulled ? "pulled" : ""}`} onClick={onLever} aria-label={tr2({ uz: "Richagni torting", ru: "Потяните рычаг" })}>
            <span className="cf-lever-arm"><span className="cf-lever-knob" /></span>
            <span className="cf-lever-lbl">{leverPulled ? "…" : tr2({ uz: "TORT", ru: "ТЯНИ" })}</span>
          </button>}
      </div>
      <div className="cf-chute">
        <span className="cf-lip" />
        <div className="cf-tray">
          {count === 0 ? <span className="cf-tray-empty">{tr2({ uz: "javon bo'sh…", ru: "полка пуста…" })}</span> : trayCards.map(renderSlot)}
        </div>
      </div>
    </div>;
}
var ScanIcon = () => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const [xrayUsed, setXrayUsed] = useState3(!!storedAnswer);
  ...
  const pick = (v) => {
    if (picked !== null || !xrayUsed) return;
    setPicked(v);
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up" style={{ maxWidth: 780 }}>{tr2({ uz: <>6 ta har xil kartochka — <span className="italic" style={{ color: T.accent }}>nechta kod</span>?</>, ru: <>6 разных карточек — <span className="italic" style={{ color: T.accent }}>сколько кода</span>?</> })}</h1>
        ...
        <Split>
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <button className={`chip ${xray ? "chip-on" : ""}`} onClick={toggle}><ScanIcon /> {xray ? tr2({ uz: "Rentgen yoqilgan", ru: "Рентген включён" }) : tr2({ uz: "Rentgen rejimi", ru: "Режим Рентгена" })}</button>
              {xray && <span className="mono small fade-step" style={{ color: T.accent }}>{tr2({ uz: "ichkarida — hammasi BIR XIL!", ru: "внутри — всё ОДИНАКОВОЕ!" })}</span>}
            </div>
          ...
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Sizningcha, bunga nechta komponent kodi yozilgan?", ru: "Как думаете, сколько кода компонентов за этим написано?" })}</p>
            <div className="fade-up delay-3" style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {OPTS.map((o) => {
    const on = picked === o.id;
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null || !xrayUsed} style={{ opacity: !xrayUsed ? 0.55 : 1 }} onClick={() => pick(o.id)}>
                    <span className="radio">{on && <span className="radio-dot" />}</span>
                    <span>{tr2(o.label)}</span>
                  </button>;
  })}
            </div>
            {!xrayUsed && <p className="small" style={{ color: T.ink3, fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Avval Rentgenni yoqib ko'ring ←", ru: "Сначала включите Рентген ←" })}</p>}
            {picked !== null && <p className="hook-ack fade-step">{tr2({ uz: <>Aynan! Qolip — <b>bitta <span className="mono">{"<GameCard />"}</span></b>, farq esa faqat <b>ma'lumotda</b> (props). Bugun shu ma'lumotni komponentlarga <b>uzatish san'atini</b> o'rganamiz — va butun katalogni quramiz.</>, ru: <>Именно! Шаблон — <b>один <span className="mono">{"<GameCard />"}</span></b>, а разница только в <b>данных</b> (props). Сегодня освоим <b>искусство передачи</b> этих данных компонентам — и соберём весь каталог.</> })}</p>}
          </Col>
        </Split>
        </Zoomable>
      </div>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const PreviewBlock = <Col>
      ...
      <p className="mono small" style={{ color: T.accent, margin: 0 }}>{tr2({ uz: "→ butun katalog — bitta qator kod bilan", ru: "→ весь каталог — одной строкой кода" })}</p>
    </Col>;
  ...
  return <Stage eyebrow={tr2({ uz: "Reja", ru: "План" })} screen={screen} audioState={audio} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head">
          <h2 className="title h-title fade-up">{tr2({ uz: <>Butun katalogni <span className="italic" style={{ color: T.accent }}>bitta qator kod</span> bilan chiza olamizmi?</>, ru: <>Сможем нарисовать весь каталог <span className="italic" style={{ color: T.accent }}>одной строкой кода</span>?</> })}</h2>
        </div>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Uzatish", ru: "Передача" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Varaqani yuboring", ru: "Отправьте посылку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>App ma'lumotni GameCard'ga <span className="italic" style={{ color: T.accent }}>qanday yetkazadi</span>?</>, ru: <>Как App <span className="italic" style={{ color: T.accent }}>доставляет данные</span> в GameCard?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "App — jo'natuvchi", ru: "App — отправитель" })}</p>
            <pre className="code-box fade-up delay-1" style={{ lineHeight: 2 }}>
              <Cm>{tr2({ uz: "// App ichida:", ru: "// внутри App:" })}</Cm>{"\n"}
            ...
            <button className="btn fade-up delay-2" style={{ alignSelf: "flex-start" }} onClick={send} disabled={running}>{running ? tr2({ uz: "Yo'lda…", ru: "В пути…" }) : done ? tr2({ uz: "📄 Yana yuborish", ru: "📄 Отправить ещё раз" }) : tr2({ uz: "📄 Varaqani yuborish", ru: "📄 Отправить слип" })}</button>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Yo'l: <b>atribut yozildi → props'ga tushdi → kartochka chizildi</b>. Atribut nomi = props ichidagi nom: <span className="mono">name</span> → <span className="mono">props.name</span>.</>, ru: <>Путь: <b>написан атрибут → попал в props → нарисована карточка</b>. Имя атрибута = имя внутри props: <span className="mono">name</span> → <span className="mono">props.name</span>.</> })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "GameCard — qabul qiluvchi", ru: "GameCard — получатель" })}</p>
            ...
            <p className="flow-label" style={{ marginTop: 2 }}>{tr2({ uz: "Fabrika — GameCard-3000", ru: "Фабрика — GameCard-3000" })}</p>
            <CardFactory
    mode="debut"
    humming={running}
    slipIn={phase === 1}
    slip={phase < 3 ? <><span className="cf-slip-k">name</span>="Doors" · <span className="cf-slip-k">players</span>="310K"</> : null}
    trayCards={phase >= 3 ? [{ name: "Doors", players: "310K" }] : []}
    used={phase >= 3 ? 1 : 0}
    feed={phase}
  />
          </Col>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [name, setName] = useState3(storedAnswer ? "Bee Swarm" : null);
  const [players, setPlayers] = useState3(storedAnswer ? "260K" : null);
  ...
  return <Stage eyebrow={tr2({ uz: "Bir nechta props", ru: "Несколько props" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "3 ta props'ni to'ldiring", ru: "Заполните 3 props" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bitta kartochkaga <span className="italic" style={{ color: T.accent }}>nechta ma'lumot</span> uzatsa bo'ladi?</>, ru: <>Сколько <span className="italic" style={{ color: T.accent }}>данных</span> можно передать одной карточке?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">name <span className="propchip-tag">{tr2({ uz: "matn props", ru: "текстовый props" })}</span></p>
            ...
            <p className="flow-label" style={{ marginTop: 2 }}>players <span className="propchip-tag">{tr2({ uz: "son props", ru: "числовой props" })}</span></p>
            ...
            <p className="flow-label" style={{ marginTop: 2 }}>emoji <span className="propchip-tag">{tr2({ uz: "belgi props", ru: "props-значок" })}</span></p>
            <div className="fade-up delay-2" style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {EMOJIS.map((v) => <button key={v} className={`propchip propchip-emoji ${emoji === v ? "sel" : ""}`} onClick={() => setEmoji(v)}>{v}</button>)}
            </div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>3 ta props — 3 ta atribut. Komponent ularni <span className="mono">props.name</span>, <span className="mono">props.players</span>, <span className="mono">props.emoji</span> deb o'qiydi.</>, ru: <>3 props — 3 атрибута. Компонент читает их как <span className="mono">props.name</span>, <span className="mono">props.players</span>, <span className="mono">props.emoji</span>.</> })}</p></div>}
          </Col>
          <Col>
            ...
            <p className="flow-label">{tr2({ uz: "Fabrika — slip → kartochka", ru: "Фабрика — слип → карточка" })}</p>
            <CardFactory
    mode="fields"
    slipIn={done}
    feed={done ? 1 : 0}
...
var Screen4 = (props) => <QuestionScreen
  ...
  questionText={tr2({ uz: "App komponenti GameCard'ga ma'lumotni qanday uzatadi?", ru: "Как компонент App передаёт данные в GameCard?" })}
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>App ma'lumotni GameCard'ga <span className="italic" style={{ color: T.accent }}>qanday uzatadi</span>?</>, ru: <>Как App <span className="italic" style={{ color: T.accent }}>передаёт данные</span> в GameCard?</> })}</h2></>}
  options={[tr2({ uz: 'Atribut orqali: name="Doors"', ru: 'Через атрибут: name="Doors"' }), tr2({ uz: "Internet orqali yuboradi", ru: "Отправляет через интернет" }), tr2({ uz: "Fayl orqali saqlab beradi", ru: "Сохраняет в файл" }), tr2({ uz: "GameCard o'zi App'dan olib ketadi", ru: "GameCard сам забирает из App" })]}
  correctIdx={0}
  explainCorrect={tr2({ uz: "To'g'ri! Chaqiruvda atribut yoziladi, komponent ichida esa o'sha ma'lumot props bo'lib keladi: name → props.name.", ru: "Верно! При вызове пишется атрибут, а внутри компонента эти данные приходят как props: name → props.name." })}
  explainWrong={{
    1: tr2({ uz: "Yo'q — internet kerak emas. Bu kod ichidagi uzatish: atribut → props.", ru: "Нет — интернет не нужен. Это передача внутри кода: атрибут → props." }),
    2: tr2({ uz: `Yo'q — fayl ham kerak emas. Atributning o'zi yetadi: <GameCard name="…" />.`, ru: 'Нет — и файл не нужен. Достаточно атрибута: <GameCard name="…" />.' }),
    3: tr2({ uz: "Yo'q — komponent o'zi 'olib keta olmaydi'. Ota (App) jo'natadi, bola (GameCard) qabul qiladi.", ru: "Нет — компонент не может «забрать сам». Родитель (App) отправляет, ребёнок (GameCard) принимает." }),
    default: tr2({ uz: 'Atribut orqali: <GameCard name="…" /> → props.name.', ru: 'Через атрибут: <GameCard name="…" /> → props.name.' })
  }}
/>;
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const KIDS = [GAMES[0], EXTRA[0], EXTRA[2]];
  const [flow, setFlow] = useState3(storedAnswer ? 2 : 0);
  const [running, setRunning] = useState3(false);
  const [reversed, setReversed] = useState3(!!storedAnswer);
  const [shaking, setShaking] = useState3(false);
  const [upTick, setUpTick] = useState3(0);
  const timer = useRef3(null);
  ...
  return <Stage eyebrow={tr2({ uz: "Bir tomonlama konveyer", ru: "Односторонний конвейер" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : flow < 2 ? tr2({ uz: "Avval pastga yuboring", ru: "Сначала отправьте вниз" }) : tr2({ uz: "Endi tepaga sinab ko'ring", ru: "Теперь попробуйте наверх" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bola komponent otasiga ma'lumot <span className="italic" style={{ color: T.accent }}>yubora oladimi</span>?</>, ru: <>Может ли дочерний компонент <span className="italic" style={{ color: T.accent }}>отправить данные</span> родителю?</> })}</h2></div>
    ...
    jammed={reversed}
    onJam={sendUp}
    slip={flow < 2 ? tr2({ uz: <><span className="cf-slip-k">App</span> → props (⬇ pastga)</>, ru: <><span className="cf-slip-k">App</span> → props (⬇ вниз)</> }) : null}
    trayCards={flow >= 2 ? KIDS.map((g) => g.name) : []}
    used={flow >= 2 ? KIDS.length : 0}
  />
              {upTick > 0 && <span key={upTick} className="tree-upball">{tr2({ uz: "🚫 tepaga yo'l yo'q", ru: "🚫 наверх пути нет" })}</span>}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Ma'lumotni yuborib ko'ring", ru: "Попробуйте отправить данные" })}</p>
            <div className="fade-up delay-2" style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              <button className={flow >= 2 ? "btn-soft btn-did" : "btn btn-turn"} style={{ alignSelf: "flex-start" }} onClick={sendDown} disabled={running}>{tr2({ uz: "⬇ App'dan bolalarga yuborish", ru: "⬇ Отправить от App детям" })} {flow >= 2 ? "✓" : ""}</button>
              <button className={reversed ? "btn-soft btn-did" : flow >= 2 ? "btn btn-turn" : "btn-soft"} style={{ alignSelf: "flex-start" }} onClick={sendUp} disabled={flow < 2} title={flow < 2 ? tr2({ uz: "Avval pastga yuboring", ru: "Сначала отправьте вниз" }) : void 0}>{tr2({ uz: "⬆ Boladan otaga yuborib ko'rish", ru: "⬆ Попробовать от ребёнка родителю" })} {reversed ? "✓" : ""}</button>
            </div>
            {flow >= 2 && !reversed && <div className="hint fade-step"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: <>Varaqa pastga tushdi: App → 3 ta bola, har biri o'z props'ini oldi. Endi kartochkani <b style={{ color: T.ink }}>teshikka qaytarib</b> sudrang ↑</>, ru: <>Слип упал вниз: App → 3 ребёнка, каждый получил свои props. Теперь затащите карточку <b style={{ color: T.ink }}>обратно в щель</b> ↑</> })}</p></div>}
            {reversed && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>❌ Konveyer orqaga aylanmaydi. Bola otaga props uzata <b>olmaydi</b> — React'da ma'lumot <b>faqat yuqoridan pastga</b>.</>, ru: <>❌ Конвейер не крутится назад. Ребёнок <b>не может</b> передать props родителю — в React данные идут <b>только сверху вниз</b>.</> })}</p></div>}
...
var Screen5b = (props) => <QuestionScreen
  ...
  questionText={tr2({ uz: "Props qaysi yo'nalishda oqadi?", ru: "В каком направлении текут props?" })}
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "Mustahkamlash", ru: "Закрепление" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Props qaysi <span className="italic" style={{ color: T.accent }}>yo'nalishda</span> oqadi?</>, ru: <>В каком <span className="italic" style={{ color: T.accent }}>направлении</span> текут props?</> })}</h2></>}
  options={[tr2({ uz: "Pastdan yuqoriga — boladan otaga", ru: "Снизу вверх — от ребёнка к родителю" }), tr2({ uz: "Istalgan tomonga — yuqoriga ham, pastga ham", ru: "В любую сторону — и вверх, и вниз" }), tr2({ uz: "Faqat yuqoridan pastga — otadan bolaga", ru: "Только сверху вниз — от родителя к ребёнку" }), tr2({ uz: "Props umuman oqmaydi — bir joyda turadi", ru: "Props вообще не текут — стоят на месте" })]}
  correctIdx={2}
  explainCorrect={tr2({ uz: "To'g'ri! Daryo kabi — faqat pastga: App → GameCard → LikeButton. Bola otaga props uzata olmaydi.", ru: "Верно! Как река — только вниз: App → GameCard → LikeButton. Ребёнок не может передать props родителю." })}
  explainWrong={{
    0: tr2({ uz: "Teskari yuborishda nima bo'lgandi? ❌ Yo'l yo'q — bola otaga uzata olmaydi.", ru: "Помните, что было при отправке наверх? ❌ Пути нет — ребёнок не может передать родителю." }),
    1: tr2({ uz: "Yo'q — React'da qat'iy tartib bor: faqat yuqoridan pastga. Shu tufayli kod tushunarli qoladi.", ru: "Нет — в React строгий порядок: только сверху вниз. Благодаря этому код остаётся понятным." }),
    3: tr2({ uz: "Oqadi — atribut yozilgan zahoti pastga: name → props.name.", ru: "Текут — как только написан атрибут, данные идут вниз: name → props.name." }),
    default: tr2({ uz: "Faqat yuqoridan pastga: otadan bolaga.", ru: "Только сверху вниз: от родителя к ребёнку." })
  }}
/>;
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [triedMutate, setTriedMutate] = useState3(!!storedAnswer);
  const [shaking, setShaking] = useState3(false);
  const [parentName, setParentName] = useState3("Adopt Me!");
  const [parentSent, setParentSent] = useState3(!!storedAnswer);
  const [dropping, setDropping] = useState3(false);
  const [dropTick, setDropTick] = useState3(0);
  const timer = useRef3(null);
  const dropTimer = useRef3(null);
  ...
  const fromParent = () => {
    ...
    dropTimer.current = setTimeout(() => {
      setParentName((n) => n === "Adopt Me!" ? "Bee Swarm" : "Adopt Me!");
      setParentSent(true);
      setDropping(false);
    }, 650);
  };
  ...
  return <Stage eyebrow={tr2({ uz: "Faqat o'qish", ru: "Только чтение" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : !triedMutate ? tr2({ uz: "1-usulni sinang", ru: "Попробуйте способ 1" }) : tr2({ uz: "2-usulni sinang", ru: "Попробуйте способ 2" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Komponent o'ziga kelgan props'ni <span className="italic" style={{ color: T.accent }}>o'zgartira oladimi</span>?</>, ru: <>Может ли компонент <span className="italic" style={{ color: T.accent }}>изменить</span> пришедшие props?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              <button className={triedMutate ? "btn-soft btn-did" : "btn btn-turn"} style={{ alignSelf: "flex-start" }} onClick={mutate}>{tr2({ uz: '1 · Ichkaridan: props.name = "Yangi"', ru: '1 · Изнутри: props.name = "Новое"' })} {triedMutate ? "✓" : ""}</button>
              <button className={parentSent ? "btn-soft btn-did" : triedMutate ? "btn btn-turn" : "btn-soft"} style={{ alignSelf: "flex-start" }} onClick={fromParent}>{tr2({ uz: "2 · App'dan yangi name yuborish", ru: "2 · Отправить новый name из App" })} {parentSent ? "✓" : ""}</button>
            </div>
            ...
            <div className="code-box" style={{ padding: "9px 13px", minHeight: 64 }}>
              {!triedMutate && !parentSent && <TLine out={<span style={{ color: CODE.comment }}>{tr2({ uz: "tugmalarni sinab ko'ring…", ru: "попробуйте кнопки…" })}</span>} />}
              {triedMutate && <TLine out={<span style={{ color: CODE.tag }}>{tr2({ uz: "❌ TypeError: props faqat o'qish uchun!", ru: "❌ TypeError: props только для чтения!" })}</span>} />}
              {parentSent && <TLine out={<span style={{ color: CODE.str }}>{tr2({ uz: "✓ App yangi props yubordi → kartochka qayta chizildi", ru: "✓ App отправил новые props → карточка перерисована" })}</span>} />}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Fabrika — muhrlangan kartochka", ru: "Фабрика — запечатанная карточка" })}</p>
            <CardFactory
    mode="sealed"
    sealed
    shaking={shaking}
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState3(storedAnswer ? new Set(LIST.map((g) => g.name)) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={tr2({ uz: "Ma'lumotlar ro'yxati", ru: "Список данных" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `${seen.size}/3 qatorni ko'ring`, ru: `Посмотрите строки: ${seen.size}/3` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Roblox'dagi barcha o'yinlar ma'lumoti <span className="italic" style={{ color: T.accent }}>qayerda saqlanadi</span>?</>, ru: <>Где <span className="italic" style={{ color: T.accent }}>хранятся</span> данные всех игр Roblox?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "games — ma'lumotlar massivi", ru: "games — массив данных" })}</p>
            <pre className="code-box fade-up delay-1" style={{ lineHeight: 1.9 }}>
              <Jx>{"const"}</Jx>{" games = ["}{"\n"}
              {LIST.map((g, i) => <React3.Fragment key={g.name}>
          ...
          <Col>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
              <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "Bu qator qanday kartochka bo'ladi?", ru: "Какой карточкой станет эта строка?" })}</p>
              <span className="small mono" style={{ color: done ? T.success : T.ink3 }}>{seen.size} / 3</span>
            </div>
            {active ? <div className="fade-step" key={active} style={{ maxWidth: 160 }}>
                <RoCard name={active} players={gameByName(active)?.players} />
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Chapdan qatorni bosing", ru: "Нажмите строку слева" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Har qator — bitta kartochkaning <b>xom ashyosi</b>. Endi savol: 3 ta qatorni 3 ta kartochkaga <b>kim aylantiradi</b>? Keyingi ekranda!</>, ru: <>Каждая строка — <b>сырьё</b> для одной карточки. Теперь вопрос: <b>кто превратит</b> 3 строки в 3 карточки? На следующем экране!</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const LIST = GAMES.slice(0, 3);
  const [shown, setShown] = useState3(storedAnswer ? 3 : 0);
  const [running, setRunning] = useState3(false);
  const timer = useRef3(null);
  const done = shown >= 3;
  ...
  return <Stage eyebrow={tr2({ uz: "Katalog", ru: "Каталог" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Katalogni chizing", ru: "Нарисуйте каталог" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Ro'yxatdan katalogga — <span className="italic" style={{ color: T.accent }}>bitta map qatori</span> yetadimi?</>, ru: <>От списка к каталогу — хватит <span className="italic" style={{ color: T.accent }}>одной строки map</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <button className="btn fade-up delay-1" style={{ alignSelf: "flex-start" }} onClick={run} disabled={running}>{running ? tr2({ uz: "Chizilmoqda…", ru: "Рисуем…" }) : done ? tr2({ uz: "↻ Yana chizish", ru: "↻ Нарисовать ещё раз" }) : tr2({ uz: "▶ Katalogni chizish", ru: "▶ Нарисовать каталог" })}</button>
            <pre className="code-box fade-up delay-2" style={{ lineHeight: 1.9 }}>
              ...
              {"\n"}
              <Cm>{tr2({ uz: "// konveyer — har qatorga bitta kartochka:", ru: "// конвейер — по карточке на каждую строку:" })}</Cm>{"\n"}
              {"{games.map(g => "}<Jx>{"<GameCard "}</Jx><At>name</At>{"={g.name}"}<Jx>{" />"}</Jx>{")}"}
            </pre>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Fabrika — avto-feeder (map)", ru: "Фабрика — авто-фидер (map)" })}</p>
            <CardFactory
    mode="feeder"
    humming={running}
    leverPulled={running}
    ...
    trayCards={LIST.slice(0, shown).map((g) => g.name)}
    used={shown}
  />
            {done && <span className="tagpill fade-step" style={{ color: T.success }}>{tr2({ uz: "✓ 3 qator ma'lumot → 3 kartochka · kod: 1 qator", ru: "✓ 3 строки данных → 3 карточки · код: 1 строка" })}</span>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>E'tibor bering: <span className="mono">name={"{g.name}"}</span> — <b>jingalak qavs</b>! Chunki qiymat qo'shtirnoqdagi matn emas, <b>ro'yxatdan kelyapti</b>.</>, ru: <>Обратите внимание: <span className="mono">name={"{g.name}"}</span> — <b>фигурные скобки</b>! Потому что значение не текст в кавычках, а <b>приходит из списка</b>.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen9 = (props) => <QuestionScreen
  ...
  questionText={tr2({ uz: "Ro'yxatga yangi o'yin qo'shilsa nima bo'ladi?", ru: "Что будет, если добавить в список новую игру?" })}
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>games ro'yxatiga <span className="italic" style={{ color: T.accent }}>yangi o'yin qo'shilsa</span> nima bo'ladi?</>, ru: <>Что будет, если в список games <span className="italic" style={{ color: T.accent }}>добавить новую игру</span>?</> })}</h2></>}
  options={[tr2({ uz: "Katalog kodini ham qayta yozish kerak", ru: "Придётся переписать и код каталога" }), tr2({ uz: "Hech narsa — sayt eski holatda qoladi", ru: "Ничего — сайт останется прежним" }), tr2({ uz: "Butun katalog buzilib qoladi", ru: "Весь каталог сломается" }), tr2({ uz: "React yangi kartochkani o'zi chizadi", ru: "React сам нарисует новую карточку" })]}
  correctIdx={3}
  explainCorrect={tr2({ uz: "To'g'ri! map ro'yxatdagi HAR BIR element uchun ishlaydi — qator qo'shildi, kartochka o'zi paydo bo'ladi. Kod bir qator bo'lib qolaveradi.", ru: "Верно! map работает для КАЖДОГО элемента списка — строка добавилась, карточка появится сама. Код так и останется одной строкой." })}
  explainWrong={{
    0: tr2({ uz: "Yo'q — bu eski usul edi. map bilan kod o'zgarmaydi: u ro'yxatning hammasiga ishlaydi.", ru: "Нет — так было раньше. С map код не меняется: он работает со всем списком." }),
    1: tr2({ uz: "Aksincha — map yangi qatorni ham ko'radi va kartochkasini chizadi.", ru: "Наоборот — map увидит и новую строку и нарисует её карточку." }),
    2: tr2({ uz: "Yo'q — buzilmaydi. map nechta qator bo'lsa, shuncha kartochka yasayveradi.", ru: "Нет — не сломается. Сколько строк, столько карточек map и сделает." }),
    default: tr2({ uz: "map ro'yxatga qarab ishlaydi: yangi qator → yangi kartochka, kod o'zgarmaydi.", ru: "map работает по списку: новая строка → новая карточка, код не меняется." })
  }}
/>;
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const BASE = GAMES.slice(0, 3);
  const [added, setAdded] = useState3(storedAnswer ? ["Doors", "Piggy"] : []);
  ...
  return <Stage eyebrow={tr2({ uz: "Yangi o'yin", ru: "Новая игра" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Kamida 2 ta qo'shing (${added.length}/2)`, ru: `Добавьте минимум 2 (${added.length}/2)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Katalogga yangi o'yin qo'shish uchun <span className="italic" style={{ color: T.accent }}>kod kerakmi</span>?</>, ru: <>Нужен ли <span className="italic" style={{ color: T.accent }}>код</span>, чтобы добавить в каталог новую игру?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            <pre className="code-box fade-up delay-2" style={{ lineHeight: 1.8 }}>
              <Jx>{"const"}</Jx>{" games = ["}{"\n"}
              {BASE.map((g) => <React3.Fragment key={g.name}>{"  { name: "}<St>"{g.name}"</St>{" },"}{"\n"}</React3.Fragment>)}
              {added.map((nm) => <span key={nm} className="el-in" style={{ display: "inline-block", background: "rgba(31,122,77,0.13)", borderRadius: 6, padding: "1px 5px" }}>{"  { name: "}<St>"{nm}"</St>{" },  "}<Cm>{tr2({ uz: "// + yangi", ru: "// + новая" })}</Cm>{"\n"}</span>)}
              {"];"}{"\n\n"}
              <Cm>{tr2({ uz: "// katalog kodi O'ZGARMADI:", ru: "// код каталога НЕ ИЗМЕНИЛСЯ:" })}</Cm>{"\n"}
              {"{games.map(g => "}<Jx>{"<GameCard "}</Jx><At>name</At>{"={g.name}"}<Jx>{" />"}</Jx>{")}"}
            </pre>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Ko'rdingizmi? <b>Faqat ma'lumot o'zgardi</b> — kartochkalar o'zi paydo bo'ldi. Buning nomi <b>data-driven sayt</b>: sayt ro'yxatga qarab o'zini chizadi. Roblox ham shunday ishlaydi!</>, ru: <>Видели? <b>Изменились только данные</b> — карточки появились сами. Это называется <b>data-driven сайт</b>: сайт рисует себя по списку. Roblox работает так же!</> })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Fabrika — javon o'sadi", ru: "Фабрика — полка растёт" })}</p>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Keyingi qadam · AI", ru: "Следующий шаг · AI" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Agent bilan ishlab ko'ring", ru: "Поработайте с агентом" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Katalogni <span className="italic" style={{ color: T.accent }}>AI bilan</span> boyitsak-chi?</>, ru: <>Обогатим каталог <span className="italic" style={{ color: T.accent }}>с помощью AI</span>?</> })}</h2></div>
          ...
          <Col>
            ...
            <Win title="robo-games — localhost:5173" minH={130}>
              {done && cur ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {cur.id === "t3" && <p style={{ fontFamily: "'Manrope',sans-serif", fontWeight: 800, fontSize: 13.5, color: T.ink, margin: 0 }}>{tr2({ uz: "🎮 Robo Games — 3 ta o'yin", ru: "🎮 Robo Games — 3 игры" })} <span className="mono small" style={{ color: T.success, fontWeight: 700 }}>+ count</span></p>}
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                    <RoCard name="Adopt Me!" players={cur.id === "t1" ? "402K" : void 0} />
                    <RoCard name="Blox Fruits" players={cur.id === "t1" ? "750K" : void 0} top={cur.id === "t2"} />
                    <RoCard name="Doors" players={cur.id === "t1" ? "310K" : void 0} />
                  </div>
            ...
            {done ? <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Kodni o'qing: ma'lumot <b>ro'yxatdan</b> kelyapti, props <b>pastga</b> oqyapti, jingalak qavs joyida. Agent ishini <b>isbot bilan</b> qabul qildingiz.</>, ru: <>Прочитайте код: данные приходят <b>из списка</b>, props текут <b>вниз</b>, фигурные скобки на месте. Вы приняли работу агента <b>с доказательствами</b>.</> })}</p></div> : <p className="body" style={{ margin: 0, color: T.ink3, fontSize: 13 }}>{tr2({ uz: "Natija shu yerda paydo bo'ladi — keyin uni o'zingiz tekshirasiz.", ru: "Результат появится здесь — потом вы сами его проверите." })}</p>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen12 = (props) => <QuestionScreen
  ...
  questionText={tr2({ uz: "GameCard o'ziga kelgan props.name'ni o'zgartira oladimi?", ru: "Может ли GameCard изменить пришедший ему props.name?" })}
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>GameCard o'ziga kelgan <span className="mono" style={{ color: T.accent }}>props.name</span>'ni o'zgartira oladimi?</>, ru: <>Может ли GameCard изменить пришедший ему <span className="mono" style={{ color: T.accent }}>props.name</span>?</> })}</h2></>}
  options={[tr2({ uz: "Ha — xohlagancha o'zgartiradi", ru: "Да — меняет сколько угодно" }), tr2({ uz: "Yo'q — props faqat o'qiladi", ru: "Нет — props только читаются" }), tr2({ uz: "Faqat kichik harf bilan yozsa bo'ladi", ru: "Можно, только строчными буквами" }), tr2({ uz: "Faqat kechasi o'zgartiradi", ru: "Меняет только по ночам" })]}
  correctIdx={1}
  explainCorrect={tr2({ uz: "To'g'ri! Props — otadan kelgan sovg'a: faqat o'qiladi. O'zgartirishni ota qiladi (yangi props yuboradi), komponentning o'z o'zgaruvchan narsasi esa state'da yashaydi.", ru: "Верно! Props — подарок от родителя: только читается. Меняет родитель (отправляя новые props), а собственное изменяемое компонента живёт в state." })}
  explainWrong={{
    0: tr2({ uz: "Esingizdami konsol xatosi? ❌ TypeError — props faqat o'qish uchun.", ru: "Помните ошибку в консоли? ❌ TypeError — props только для чтения." }),
    2: tr2({ uz: "Yo'q — harfga bog'liq emas. Props har qanday holatda read-only.", ru: "Нет — дело не в буквах. Props в любом случае read-only." }),
    3: tr2({ uz: "Yo'q — vaqtga ham bog'liq emas. Props hech qachon ichkaridan o'zgarmaydi.", ru: "Нет — и время ни при чём. Props никогда не меняются изнутри." }),
    default: tr2({ uz: "Props — read-only. O'zgartirish kerakmi? Ota yangi props yuboradi yoki state ishlatiladi.", ru: "Props — read-only. Нужно изменить? Родитель отправляет новые props или используется state." })
  }}
/>;
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const POOL = [...GAMES.slice(0, 3), ...EXTRA];
  const [mine, setMine] = useState3(storedAnswer ? ["Adopt Me!", "Doors", "Piggy"] : []);
  const [topName, setTopName] = useState3(storedAnswer ? "Doors" : null);
  const done = mine.length >= 3 && !!topName;
  const toggle = (nm) => {
    setMine((prev) => {
      if (prev.includes(nm)) {
        if (topName === nm) setTopName(null);
        return prev.filter((x) => x !== nm);
      }
  ...
  return <Stage eyebrow={tr2({ uz: "Amaliyot · katalog", ru: "Практика · каталог" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : mine.length < 3 ? tr2({ uz: `O'yin tanlang (${mine.length}/3)`, ru: `Выберите игры (${mine.length}/3)` }) : tr2({ uz: "TOP belgilang", ru: "Отметьте TOP" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>O'z katalogingizni <span className="italic" style={{ color: T.accent }}>yig'a olasizmi</span>?</>, ru: <>Соберёте <span className="italic" style={{ color: T.accent }}>свой каталог</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Ro'yxatga qo'shing", ru: "Добавьте в список" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {POOL.map((g) => <button key={g.name} className={`gchip ${mine.includes(g.name) ? "" : ""}`} style={mine.includes(g.name) ? { background: T.accent, color: "#fff" } : void 0} onClick={() => toggle(g.name)}>{g.emoji} {g.name}</button>)}
            </div>
            <pre className="code-box fade-up delay-2" style={{ lineHeight: 1.8 }}>
              <Jx>{"const"}</Jx>{" mening = ["}{"\n"}
              {mine.length === 0 ? <Cm>{tr2({ uz: "  // o'yinlarni tanlang…", ru: "  // выберите игры…" })}{"\n"}</Cm> : mine.map((nm) => <React3.Fragment key={nm}>{"  { name: "}<St>"{nm}"</St>{topName === nm ? <>, top: <At>true</At></> : ""}{" },"}{"\n"}</React3.Fragment>)}
              {"];"}{"\n\n"}
              {"{mening.map(g => "}<Jx>{"<GameCard "}</Jx><At>name</At>{"={g.name} "}<At>top</At>{"={g.top}"}<Jx>{" />"}</Jx>{")}"}
            </pre>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Katalogingiz", ru: "Ваш каталог" })} {mine.length >= 3 && !topName ? tr2({ uz: "— kartochkani bosib TOP belgilang", ru: "— нажмите карточку и отметьте TOP" }) : ""}</p>
            <Win title="mening-katalogim — localhost:5173" minH={120}>
              {mine.length === 0 ? <p style={{ color: T.ink3, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13 }}>{tr2({ uz: "Bo'sh — ro'yxatga o'yin qo'shing…", ru: "Пусто — добавьте игры в список…" })}</p> : <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    {mine.map((nm) => <RoCard key={nm} name={nm} top={topName === nm} onClick={() => setTopName(nm)} />)}
                  </div>}
            </Win>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Katalogingiz tayyor: ro'yxat + map + props (<span className="mono">name</span>, <span className="mono">top</span>). Siz endi <b>ma'lumot bilan boshqaradigan</b> dasturchisiz.</>, ru: <>Ваш каталог готов: список + map + props (<span className="mono">name</span>, <span className="mono">top</span>). Теперь вы разработчик, который <b>управляет данными</b>.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Debugging", ru: "Дебаггинг" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Xatoni toping", ru: "Найдите ошибку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            <div className="ai-row"><span className="ai-badge">AI</span><span className="ai-bubble">{tr2({ uz: "Katalog kodini yozdim:", ru: "Я написал код каталога:" })}</span></div>
            <DebugChallenge
    lines={LINES}
    fixed={"  <GameCard name={g} />"}
    explain={tr2({ uz: "Konveyer aylandi, lekin <GameCard /> varaqasiz chaqirilgan — g qo'lda turibdi, lekin uzatilmagan. name={g} qo'shilsa, ma'lumot kartochkalarga oqadi.", ru: "Конвейер крутился, но <GameCard /> вызван без слипа — g в руках, а не передан. Добавьте name={g} — и данные потекут в карточки." })}
    onSolved={solve}
  />
          </Col>
          <Col>
            {!done ? <div className="hint"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: <>Kartochkalar nomsiz va kulrang — demak ularga <b style={{ color: T.ink }}>props yetib bormagan</b>. Varaqa qaysi qatorda jo'natilmay qolgan?</>, ru: <>Карточки без имён и серые — значит, <b style={{ color: T.ink }}>props до них не дошли</b>. В какой строке слип так и не был отправлен?</> })}</p></div> : <>
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  useEffect4(() => {
    if (valid && !passed) {
      setPassed(true);
      onAnswer(screen, { stage: "final", screenIdx: screen, question: tr2({ uz: "map ichida <GameCard name={g} /> ni yozing", ru: "Напишите <GameCard name={g} /> внутри map" }), studentAnswer: value, correct: true, firstAttemptCorrect: true, solved: true, picked: value });
    }
  }, [valid]);
  const Ln = ({ n, children }) => <div className="vsc-line"><span className="vsc-ln">{n}</span><span style={{ whiteSpace: "pre" }}>{children}</span></div>;
  return <Stage eyebrow={tr2({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!passed} label={passed ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Konveyer qatorini yozing", ru: "Напишите строку конвейера" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Oxirgi qadam: katalogni <span className="italic" style={{ color: T.accent }}>o'zingiz</span> ishga tushiring.</>, ru: <>Последний шаг: запустите каталог <span className="italic" style={{ color: T.accent }}>сами</span>.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            <div className="fade-up delay-2" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <span className="tagpill" style={{ opacity: hasComp ? 1 : 0.4 }}>{hasComp ? "✓" : "1"} {"<GameCard"} {tr2({ uz: "— Katta harf", ru: "— с Большой буквы" })}</span>
              <span className="tagpill" style={{ opacity: hasDyn ? 1 : 0.4 }}>{hasDyn ? "✓" : "2"} name={"{g}"} {tr2({ uz: "— jingalak qavs", ru: "— фигурные скобки" })}</span>
              <span className="tagpill" style={{ opacity: hasClose ? 1 : 0.4 }}>{hasClose ? "✓" : "3"} {"/>"} {tr2({ uz: "yopilishi", ru: "— закрытие" })}</span>
            </div>
            {hasLowerComp && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Esingizdami? Komponent nomi <b>Katta harf</b> bilan: <span className="mono">GameCard</span>.</>, ru: <>Помните? Имя компонента — <b>с Большой буквы</b>: <span className="mono">GameCard</span>.</> })}</p></div>}
            {quoted && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Deyarli! <span className="mono">name="g"</span> — bu shunchaki "g" degan matn bo'lib qoladi. Ma'lumot <b>ro'yxatdan kelishi</b> uchun jingalak qavs kerak: <span className="mono">name={"{g}"}</span></>, ru: <>Почти! <span className="mono">name="g"</span> — это просто текст «g». Чтобы данные <b>пришли из списка</b>, нужны фигурные скобки: <span className="mono">name={"{g}"}</span></> })}</p></div>}
            {passed && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "✓ Mukammal! Konveyer ishladi: har o'yinga bitta kartochka. Data-driven katalog — sizning qo'lingizda.", ru: "✓ Отлично! Конвейер заработал: по карточке на каждую игру. Data-driven каталог — в ваших руках." })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "natija — localhost:5173", ru: "результат — localhost:5173" })}</p>
            <Win title="robo-games — localhost:5173" minH={130}>
              {valid ? <div className="fade-step" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 9 }}>
                    <RoCard name="Adopt Me!" />
                    <RoCard name="Doors" />
                  </div> : <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center", lineHeight: 1.5 }}>{tr2({ uz: <>6-qator yozilmaguncha katalog bo'sh: <span className="mono" style={{ fontStyle: "normal" }}>{"<GameCard"}</span> + <span className="mono" style={{ fontStyle: "normal" }}>name={"{g}"}</span> + <span className="mono" style={{ fontStyle: "normal" }}>{"/>"}</span></>, ru: <>Пока строка 6 не написана, каталог пуст: <span className="mono" style={{ fontStyle: "normal" }}>{"<GameCard"}</span> + <span className="mono" style={{ fontStyle: "normal" }}>name={"{g}"}</span> + <span className="mono" style={{ fontStyle: "normal" }}>{"/>"}</span></> })}</p>}
            </Win>
          </Col>
        </div>
        </Zoomable>
...
function DebugChallenge({ lines, fixed, explain, onSolved }) {
  ...
  return <div className="dbg fade-up">
      ...
      {!solved ? <p className="dbg-hint">{tr2({ uz: "👆 Xato bor qatorni toping va bosing", ru: "👆 Найдите строку с ошибкой и нажмите её" })}</p> : <div className="dbg-ok">{tr2({ uz: "✓ Topdingiz!", ru: "✓ Нашли!" })} {explain}</div>}
    </div>;
}
...
function Flashcards({ cards }) {
  ...
  return <div className="fc fade-up">
      ...
      {flipped ? <div className="fc-actions"><button className="fc-btn again" disabled={!!exiting} onClick={again}>{tr2({ uz: "✗ Takrorlash", ru: "✗ Повторить" })}</button><button className="fc-btn knew" disabled={!!exiting} onClick={knew}>{tr2({ uz: "✓ Bildim", ru: "✓ Знаю" })}</button></div> : <p className="fc-hint">{tr2({ uz: "👆 Kartani bosing — javobni ko'rasiz", ru: "👆 Нажмите карту — увидите ответ" })}</p>}
    </div>;
}
...
function ScreenLivePractice({ title, task, checklist, screen, storedAnswer, onAnswer, onNext, onPrev, live }) {
  ...
  return <Stage eyebrow={tr2({ uz: "Amaliyot · VS Code", ru: "Практика · VS Code" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Avval bajaring", ru: "Сначала выполните" })} onClick={onNext} /></>}>
    ...
    return <button key={i} className={`lp-step ${on ? "on" : ""}`} onClick={() => toggle(i)}>
            ...
            {done && !isMentor && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Juda yaxshi! Vazifani bajardingiz. Ustoz tekshirib, keyingi qadamga o'tkazadi.", ru: "Отлично! Задание выполнено. Наставник проверит и переведёт к следующему шагу." })}</p></div>}
          </Col>
        </div>
      </div>
    </Stage>;
...
var REACT_FLASHCARDS = [
  { front: { uz: "Komponentga uzatiladigan ma'lumot qanday ataladi?", ru: "Как называются данные, которые передают компоненту?" }, back: "Props", note: '<GameCard name="Doors" />' },
  { front: { uz: 'Chaqiruvdagi name="Doors" yozuvi nima deb ataladi?', ru: 'Как называется запись name="Doors" при вызове?' }, back: { uz: "Atribut", ru: "Атрибут" }, note: { uz: "komponent ichida props bo'lib ko'rinadi", ru: "внутри компонента виден уже как props" } },
  { front: { uz: "Props'ni qaysi komponent uzatadi?", ru: "Какой компонент передаёт props?" }, back: "App", note: { uz: "chaqirayotgan — ota komponent", ru: "тот, кто вызывает — родительский компонент" } },
  { front: { uz: "Bitta kartochkaga nechta props uzatish mumkin?", ru: "Сколько props можно передать одной карточке?" }, back: { uz: "Qancha kerak bo'lsa", ru: "Сколько нужно" }, note: "name · players · emoji" },
  { front: { uz: "Props qaysi yo'nalishda oqadi?", ru: "В каком направлении текут props?" }, back: { uz: "Faqat pastga", ru: "Только вниз" }, note: { uz: "otadan bolaga: App → GameCard", ru: "от родителя к ребёнку: App → GameCard" } },
  { front: { uz: "GameCard o'ziga kelgan props'ni App'ga qaytara oladimi?", ru: "Может ли GameCard вернуть пришедшие props обратно в App?" }, back: { uz: "Yo'q", ru: "Нет" }, note: { uz: "oqim bir tomonlama — orqaga ko'tarilmaydi", ru: "поток односторонний — обратно не поднимается" } },
  { front: { uz: "Komponent o'ziga kelgan props'ni o'zgartira oladimi?", ru: "Может ли компонент менять пришедшие props?" }, back: { uz: "Yo'q, faqat o'qiydi", ru: "Нет, только читает" }, note: { uz: "read-only — muhrlangan xat kabi", ru: "read-only — как запечатанное письмо" } },
  { front: { uz: "O'zgarib turadigan qiymat uchun nima ishlatiladi?", ru: "Что используют для значения, которое меняется?" }, back: "State", note: { uz: "props tashqaridan keladi, state ichida o'zgaradi", ru: "props приходят снаружи, state меняется внутри" } },
  { front: { uz: "O'yinlar ma'lumoti qayerda saqlanadi?", ru: "Где хранятся данные об играх?" }, back: { uz: "Ro'yxatda (massivda)", ru: "В списке (массиве)" }, note: "const games = [...]" },
  { front: { uz: "Ro'yxatdagi har element uchun kartochka yasaydigan buyruq qaysi?", ru: "Какая команда делает карточку для каждого элемента списка?" }, back: "map", note: { uz: "games.map(…) — konveyer kabi ishlaydi", ru: "games.map(…) — работает как конвейер" } },
  { front: { uz: "Props qiymati o'zgaruvchidan kelsa, qanday qavs qo'yiladi?", ru: "Какие скобки ставят, если значение props берётся из переменной?" }, back: { uz: "Jingalak qavs { }", ru: "Фигурные скобки { }" }, note: "name={g.name}" },
  { front: { uz: "Ro'yxatga yangi o'yin qo'shsangiz, kartochkasi qanday paydo bo'ladi?", ru: "Если добавить в список новую игру — как появится её карточка?" }, back: { uz: "O'zi chiqadi", ru: "Появится сама" }, note: { uz: "map qatoriga tegmaysiz — sayt ma'lumotga qarab chiziladi", ru: "строку с map не трогаете — сайт рисуется по данным" } }
];
...
var ACHIEVEMENTS = {
  ...
  builder: { icon: "🧱", name: "Catalog Builder!", desc: { uz: "Konveyer qatorini yozib katalogni ochdingiz", ru: "Вы написали строку конвейера и открыли каталог" } },
  graduate: { icon: "🏆", name: "Level Up!", desc: { uz: "Props darsini yakunladingiz", ru: "Вы завершили урок про Props" } }
};
var ACH_TRIGGERS = { s12: "sharp", s14: "debugger", s15: "builder" };
...
var Q_LABELS = { 4: { uz: "1 — Uzatish usuli", ru: "1 — Способ передачи" }, 6: { uz: "2 — Oqim yo'nalishi", ru: "2 — Направление потока" }, 10: { uz: "3 — Ro'yxat + map", ru: "3 — Список + map" }, 13: "4 — Read-only", 17: { uz: "5 — Yakuniy amaliy", ru: "5 — Финальное практическое" } };
var INLINE_KEYS = { s4: 0, s5b: 2, s9: 3, s12: 1, s15: -1 };
var QUIZ_MS = 15e3;
var QZ_BG_SHAPES = [
    ...
    /* ===== YAKUN — PODIUM ===== */
      ...
      {phase === "done" && <div className="qz-view fade-step">
          ...
          {solo ? <div className="qz-solo-res">
              ...
              <p className="qz-sub">{tr2({ uz: `ball · ${soloScore.ok}/${QUIZ_BANK.length} to'g'ri${soloScore.maxStreak >= 2 ? ` · eng uzun streak 🔥x${soloScore.maxStreak}` : ""}`, ru: `баллов · ${soloScore.ok}/${QUIZ_BANK.length} верных${soloScore.maxStreak >= 2 ? ` · самый длинный стрик 🔥x${soloScore.maxStreak}` : ""}` })}</p>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Tayyor", ru: "Готово" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Katalogingiz <span className="italic" style={{ color: T.accent }}>tayyor</span>.</>, ru: <>Ваш каталог <span className="italic" style={{ color: T.accent }}>готов</span>.</> })}</h2>{
    /* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>{tr2({ uz: "📝 Uyga vazifa", ru: "📝 Домашнее задание" })}</div><p className="body" style={{ margin: "0 0 10px", color: T.ink }}>{tr2({ uz: "Antigravity bilan o'z loyihangizda sinang:", ru: "Попробуйте в своём проекте с Antigravity:" })}</p><ul>{HOMEWORK.map((h, i) => <li key={i}><b>{tr2(h.b)}</b> <span className="t">{tr2(h.t)}</span></li>)}</ul><p className="hw-note">{tr2({ uz: "Keyingi darsda yangi usulni o'rganamiz: o'yinlar ro'yxati kod ichidan chiqib ketadi — uni internetdagi serverdan olamiz! 🚀", ru: "На следующем уроке освоим новый способ: список игр покинет код — мы получим его с сервера в интернете! 🚀" })}</p></div>}
...
function ReactPropsReuseLesson({ lang: langProp, onFinished, liveToken }) {
...
export {
  ReactPropsReuseLesson as default
};
