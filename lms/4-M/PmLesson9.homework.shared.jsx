...
//  Manba:  src/3-Modull/PmLesson9.homework.jsx
//  Kompilyator: TASHQI MODUL — https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx src/3-Modull/PmLesson9.homework.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/3-Modull/PmLesson9.homework.jsx
...
var EDGE_SOZ = new RegExp(`bo${APO}sh|ikki|qayta|xato|noto${APO}g${APO}ri|adash|nol|\\b0\\b|oshsa|katta|minus|пуст|дважд|два раза|ошиб|невер|повтор|снова|ноль|больше|минус`, "i");
var ishOk = (s) => (s || "").trim().length >= ISH_MIN;
var shartLen = (s) => (s || "").trim().length >= SHART_MIN;
var checkOk = (s) => (s || "").trim().length >= CHECK_MIN;
var hasNum = (s) => /\d/.test(s || "");
...
var StageChecks = ({ data, setData, work }) => {
...
var checksDone = (d) => [0, 1, 2].every((i) => checkOk(((d || {}).answers || [])[i]));
var StageEdge = ({ data, setData }) => {
  const v = data.text || "";
...
var edgeDone = (d) => shartLen((d || {}).text) && EDGE_SOZ.test((d || {}).text || "");
var QZ_HOLD_MS = 1500;
var QZ_OUT_MS = 380;
var numOk = (data) => {
  const w = (data || {}).work || {};
...
var StageSum = ({ data, setData, full, setWork }) => {
  const d = data || {};
  ...
  return <div className="col">
      ...
      <div className="frame fade-up d1">
        {nOk ? <div className="frame-success"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: "✓ Shartlaringizda aniq son bor — bunday shartni tekshirish oson.", ru: "✓ В ваших условиях есть точное число — такое условие легко проверить." })}</p></div> : <>
            ...
            {shartlar.map((s, i) => <div key={i} className="wrow-f" style={{ marginBottom: 8 }}>
...
var sumDone = (d, full) => numOk(full) && QUIZ.every((q) => ((d || {}).ans || {})[q.id] === q.correct);
var STAGES = [
...
function PmLesson9Homework({ lang: langProp, onFinished }) {
  ...
  return <div className="hw-root">
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult ? <StageResult data={data} goStage={setStage} onFinishClick={finish} finished={finished} onCelebrated={() => setDataRaw((prev) => prev.celebrated ? prev : { ...prev, celebrated: true })} /> : cur.key === "work" ? <StageWork work={data.work} setWork={setStageData("work")} /> : cur.key === "checks" ? <StageChecks data={data.checks || {}} setData={setStageData("checks")} work={data.work} /> : cur.key === "edge" ? <StageEdge data={data.edge || {}} setData={setStageData("edge")} /> : <StageSum data={data.sum || {}} setData={setStageData("sum")} full={data} setWork={setStageData("work")} />}
        </div>
      </div>
