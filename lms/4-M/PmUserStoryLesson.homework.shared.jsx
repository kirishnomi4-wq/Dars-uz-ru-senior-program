...
//  Manba:  src/pm/PmUserStoryLesson.homework.jsx
//  Kompilyator: TASHQI MODUL — https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx src/pm/PmUserStoryLesson.homework.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/pm/PmUserStoryLesson.homework.jsx
...
var S_MIN = 2;
var DESC_MIN = 8;
var sOk = (s) => (s || "").trim().length >= S_MIN;
var low = (s) => (s || "").trim().toLowerCase();
var storyFull = (c) => !!c && sOk(c.kim) && sOk(c.nima) && sOk(c.natija);
var natijaTakror = (c) => storyFull(c) && low(c.natija) === low(c.nima);
var storyOk = (c) => storyFull(c) && !natijaTakror(c);
var STORIES_KEY = "pm-m3d2-stories";
var lessonStoriesRead = () => {
  try {
...
var TARGET_KEY = "pm-m3d2-hw-target";
var lessonTargetRead = () => {
  try {
    return (localStorage.getItem(TARGET_KEY) || "").trim();
  } catch {
    return "";
  }
};
...
var StoryBlock = ({ c, setC, n, kimTakrorMsg, pulseFirst }) => {
  const firstBad = STORY_FIELDS.findIndex((f) => !sOk(c[f.k]));
  return <div className="wrow">
...
var StageStories = ({ stories, setStories }) => {
  ...
  return <div className="col">
      ...
      <div className="frame fade-up d1" style={{ padding: "clamp(6px,1.2vw,10px) clamp(14px,2.2vw,20px)" }}>
        {list.map((c, i) => <div key={i} style={{ borderBottom: i < 2 ? `1px solid ${T.line}` : "none" }}>
            <StoryBlock
    c={c}
    setC={(nc) => setStories(list.map((x, j) => j === i ? nc : x))}
    n={i + 1}
  ...
  return <div className="col">
      ...
      <div className="frame fade-up d1">
        ...
        <div className="chips" role="radiogroup">
          {HW_TARGETS.map((x) => <button key={x.id} type="button" role="radio" aria-checked={pick === x.id} className={`chip ${pick === x.id ? "on" : ""}`} onClick={() => setData({ ...data, pick: x.id })}>
        ...
        {customMode && <div style={{ marginTop: 10 }}>
...
var targetDone = (d) => !!targetLabel(d) && ((d || {}).desc || "").trim().length >= DESC_MIN;
var nimaTakrorNews = (a, b) => storyFull(a) && storyFull(b) && low(a.nima) === low(b.nima);
var StageNews = ({ data, setData, target }) => {
  const seededRef = useRef(false);
  useEffect(() => {
    if (seededRef.current) return;
    seededRef.current = true;
    const a2 = data.a || {}, b2 = data.b || {};
    if (target && !(a2.kim || "").trim() && !(b2.kim || "").trim()) {
      setData({ ...data, a: { ...a2, kim: target }, b: { ...b2, kim: target } });
    }
  }, []);
...
var newsDone = (d) => storyOk((d || {}).a || {}) && storyOk((d || {}).b || {}) && !nimaTakrorNews((d || {}).a || {}, (d || {}).b || {});
var QZ_HOLD_MS = 1500;
var QZ_OUT_MS = 380;
var allStoriesOf = (data) => {
  const lesson = [0, 1, 2].map((i) => ({ ...((data || {}).stories || [])[i] || {}, isNew: false }));
  const n = (data || {}).news || {};
  return [...lesson, { ...n.a || {}, isNew: true }, { ...n.b || {}, isNew: true }];
};
var StageSum = ({ data, setData, all }) => {
  ...
  return <div className="col">
    ...
    return <button key={i} type="button" className={`option star-opt ${on ? "q-ok" : ""}`} disabled={!full} onClick={() => toggleStar(i)}>
...
var sumDone = (d) => Array.isArray((d || {}).stars) && d.stars.length === 3 && QUIZ.every((q) => ((d || {}).ans || {})[q.id] === q.correct);
var STAGES = [
...
function PmUserStoryHomework({ lang: langProp, onFinished }) {
  ...
  return <div className="hw-root">
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult ? <StageResult data={data} goStage={setStage} onFinishClick={finish} finished={finished} onCelebrated={() => setDataRaw((prev) => prev.celebrated ? prev : { ...prev, celebrated: true })} /> : cur.key === "stories" ? <StageStories stories={data.stories} setStories={setStories} /> : cur.key === "target" ? <StageTarget data={data.target || {}} setData={setStageData("target")} /> : cur.key === "news" ? <StageNews data={data.news || {}} setData={setStageData("news")} target={targetLabel(data.target || {})} /> : <StageSum data={data.sum || {}} setData={setStageData("sum")} all={allStoriesOf(data)} />}
        </div>
      </div>
...
export {
  HOMEWORK,
  PmUserStoryHomework as default
};
