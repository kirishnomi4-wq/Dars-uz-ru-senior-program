...
import React3, { useState as useState4, useEffect as useEffect5, useRef as useRef4, useMemo as useMemo2, createContext as createContext2, useContext as useContext2, useCallback as useCallback3, useState, useEffect, useLayoutEffect, useRef, useMemo, useCallback, isValidElement } from "react";
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <p className="mstats-verdict-t">{tr3({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Верно только <b>{pct}%</b> — тему класс не понял. Перед тем как идти дальше, стоит коротко повторить.</> })}</p>}
      ...
      {reveal && struggling && <p className="mstats-warn">{tr3({ uz: "⚠️ Ko'pchilik xato qildi — bu mavzu tushunarsiz bo'lgan ko'rinadi. Yana bir bor tushuntiring.", ru: "⚠️ Большинство ошиблось — похоже, тема осталась непонятной. Стоит объяснить заново." })}</p>}
...
var QuestionScreen = ({ screen, idx, scope, eyebrow, question, questionText, options, correctIdx, explainCorrect, explainWrong, renderMode, ctaLabel, revealPrefix = tr3({ uz: "To'g'ri javob", ru: "Верный ответ" }), hsFx, storedAnswer, onAnswer, onNext, onPrev }) => {
  const gate = useContext2(LiveGateCtx) || {};
...
function StoryBoard() {
  const [slots, setSlots] = useState4(boardSlots);
  const [open, setOpen] = useState4(_sboardOpen);
  useEffect5(() => {
  ...
  return <button
    ...
    onClick={toggle}
    title={open ? tr3({ uz: "Daftarni yig'ish", ru: "Свернуть блокнот" }) : tr3({ uz: "Daftarni ochish", ru: "Открыть блокнот" })}
    aria-label={tr3({ uz: `Hikoya-daftar: ${n}/3 tayyor`, ru: `Блокнот историй: готово ${n}/3` })}
  >
      <span className="sboard-ic">📒</span>
      {open && <span className="sboard-lbl">{tr3({ uz: "Hikoyalarim", ru: "Мои истории" })}</span>}
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const [seen, setSeen] = useState4(() => /* @__PURE__ */ new Set([0]));
  ...
  return <Stage eyebrow={tr3({ uz: "Kirish · ikki so'rov", ru: "Введение · две просьбы" })} screen={screen} navContent={<NavNext optionalLive disabled={(picked === null || !seenBoth) && !isMentor} label={isMentor ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : !seenBoth ? tr3({ uz: "Ikkala xabarni o'qing", ru: "Прочитайте оба сообщения" }) : picked === null ? tr3({ uz: "Fikringizni belgilang", ru: "Отметьте своё мнение" }) : tr3({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Ikki mijoz yordam so'radi — qaysi biri <span className="italic" style={{ color: T.accent }}>tushunarli</span>?</>, ru: <>Два клиента попросили о помощи — какая просьба <span className="italic" style={{ color: T.accent }}>понятнее</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Sizga ikki mijozdan xabar keldi. <b style={{ color: T.ink }}>«1-mijoz»</b> va <b style={{ color: T.ink }}>«2-mijoz»</b> tugmalarini birma-bir bosib o'qing — so'ng o'ng tomonda fikringizni belgilang.</>, ru: <>Вам пришли сообщения от двух клиентов. Нажмите по очереди кнопки <b style={{ color: T.ink }}>«1-й клиент»</b> и <b style={{ color: T.ink }}>«2-й клиент»</b> и прочитайте — потом справа отметьте своё мнение.</> })}</Mentor>
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8 }}>
              {HOOK_REQS.map((q, i) => <button key={i} className={`hk-chip ${view === i ? "on" : ""}${turnCls(litMsg, i, pendMsg.length > 1)}`} onClick={() => show(i)}>{tr3(q.tag)}{seen.has(i) && view !== i ? " ✓" : ""}</button>)}
            </div>
            <div key={view} className={`hreq-card ${r.cls} hk-view fade-step`}>
              <span className="hreq-tag">💬 {tr3({ uz: <>{tr3(r.tag)}dan xabar</>, ru: <>Сообщение от {tr3(r.tag)}</> })}</span>
              <p className="hreq-txt">{tr3(r.text)}</p>
            </div>
            <MentorNote>{tr3({ uz: "O'quvchilar fikr belgilaydi, diagramma o'sib boradi — natijani sinf bilan muhokama qiling.", ru: "Ученики отмечают мнение, диаграмма растёт — обсудите результат с классом." })}</MentorNote>
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr3({ uz: "Sizningcha, qaysi xabar tushunarli?", ru: "Как по-вашему, какое сообщение понятнее?" })}</p>
...
var Screen1 = ({ screen, onNext, onPrev }) => <Stage eyebrow={tr3({ uz: "Maqsad", ru: "Цель" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr3({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
    <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
      <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Dars oxirida siz <span className="italic" style={{ color: T.accent }}>nimalarga</span> erishasiz?</>, ru: <>Чего вы <span className="italic" style={{ color: T.accent }}>добьётесь</span> к концу урока?</> })}</h2></div>
      <Mentor>{tr3({ uz: <>Ikkinchi mijozning gapi — tayyor <b style={{ color: T.ink }}>User Story</b> edi; bugun o'zingiz ham 3 ta shunday hikoya yozasiz. Pastdagi kartalarni kuzating — YouTube uchun yozilgan hikoyalar o'z-o'zidan to'lib boradi.</>, ru: <>Фраза второго клиента — это и была готовая <b style={{ color: T.ink }}>User Story</b>; сегодня вы и сами напишете три такие истории. Смотрите на карточки внизу — истории, написанные для YouTube, заполняются сами собой.</> })}</Mentor>
      <div className="demo-src fade-up delay-1"><span className="demo-src-ic">▶️</span> {tr3({ uz: "YouTube jamoasi shunday yozadi", ru: "Вот так пишет команда YouTube" })}</div>
        ...
        {DEMO_STORIES.map((s, i) => <div key={i} className="story-silo demo-card" style={{ "--cd": `${0.2 + i * 0.18}s` }}>
            ...
            <div className="story-silo-slots">
              {[["kim", { uz: "KIM", ru: "КТО" }], ["nima", { uz: "NIMA", ru: "ЧТО" }], ["natija", { uz: "NATIJA", ru: "РЕЗУЛЬТАТ" }]].map(([k, lbl], j) => <span key={k} className={`silo-slot ${k} demo-slot`} style={{ "--fd": `${1.1 + (i * 3 + j) * 0.42}s` }}>
                  <span className="silo-lbl">{tr3(lbl)}</span>
                  <span className="silo-fill">{tr3(s[k])}</span>
                </span>)}
            </div>
            <span className="silo-done" style={{ "--fd": `${1.1 + (i * 3 + 2) * 0.42 + 0.3}s` }}>✓</span>
          </div>)}
      </div>
      <div className="takeaway fade-up delay-2"><span className="ta-bulb">🎯</span><p className="ta-h">{tr3({ uz: "Dars oxirida sizning 3 kartangiz ham xuddi shunday yozilgan bo'ladi.", ru: "К концу урока три ваши карточки будут написаны точно так же." })}</p></div>
    </div>
  </Stage>;
...
var Screen2 = ({ screen, onNext, onPrev }) => {
  const [st, setSt] = useState4({ picks: {}, hint: -1 });
  const choose = (i, cat) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Muhokama · savol", ru: "Обсуждение · вопрос" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!allSorted} label={tr3({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Hikoya <span className="italic" style={{ color: T.accent }}>nimadan</span> yasaladi?</>, ru: <><span className="italic" style={{ color: T.accent }}>Из чего</span> состоит история?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Hikoya uch bo'lakdan yasaladi: <b style={{ color: T.ink }}>KIM</b>, <b style={{ color: T.ink }}>harakat</b> va <b style={{ color: T.ink }}>sabab</b>. Avval harakat bilan sababni ajratishni o'rganamiz — pastdagi gaplarni belgilab ko'ring. Barchasini to'g'ri belgilagach, to'liq hikoya quramiz.</>, ru: <>История состоит из трёх частей: <b style={{ color: T.ink }}>КТО</b>, <b style={{ color: T.ink }}>действие</b> и <b style={{ color: T.ink }}>причина</b>. Сначала научимся отличать действие от причины — отметьте фразы внизу. Когда всё будет отмечено верно, соберём полную историю.</> })}</Mentor>
        <MentorNote>{tr3({ uz: "Sinf bilan og'zaki muhokama: «Qaysi ilovani nima maqsadda ishlatasiz?» — ilova nomi emas, NIMA UCHUN ishlatishi muhim. 2-3 javob oling, so'ng mashqqa o'ting.", ru: "Устное обсуждение с классом: «Каким приложением и ради чего вы пользуетесь?» — важно не название приложения, а ЗАЧЕМ им пользуются. Возьмите 2-3 ответа и переходите к упражнению." })}</MentorNote>
        <div className={`s2sort fade-up delay-1${allSorted ? " dim" : ""}`}>
          <span className="flow-label">{tr3({ uz: "Bo'laklarni ajrating: harakatmi yoki sabab?", ru: "Разделите кусочки: это действие или причина?" })}</span>
          {S2_FRAGS.map((f, i) => {
    const pick = st.picks[i];
    return <div key={i} className={`s2row ${pick ? "done" : ""}${turnCls(litRow, String(i), pendRows.length > 1)}`}>
                <span className="s2txt">{pick === "harakat" ? "🏃 " : pick === "sabab" ? "💡 " : ""}«{tr3(f.txt)}»</span>
                {pick ? <span className={`s2tag ${pick}`}>{pick === "harakat" ? tr3({ uz: "harakat", ru: "действие" }) : tr3({ uz: "sabab", ru: "причина" })} ✓</span> : <span className="s2btns"><button className="s2btn" onClick={() => choose(i, "harakat")}>🏃 {tr3({ uz: "harakat", ru: "действие" })}</button><button className="s2btn" onClick={() => choose(i, "sabab")}>💡 {tr3({ uz: "sabab", ru: "причина" })}</button></span>}
                {st.hint === i && <span className="s2hint">{tr3({ uz: "Yana o'ylab ko'ring: «...uchun» bilan tugasa — bu sabab. 🙂", ru: "Подумайте ещё: если заканчивается на «чтобы...» — это причина. 🙂" })}</span>}
              </div>;
  })}
        </div>
        {
       ...
       keyingi ekranda keladi: induktiv tartib). Ko'prik-gap keyingi ekranga ishorat qiladi. */
        ...
        {allSorted && <Zoomable>
            <div className="grow-card fade-step" ref={doneRef}>
              <span className="ex-lbl">{tr3({ uz: "🌱 Mana sizning to'liq hikoyangiz", ru: "🌱 Вот ваша полная история" })}</span>
              <div className="grow-from">
                <span className={`gf-chip harakat gf-l${gs >= 1 ? " in" : ""}`}>🏃 «{tr3({ uz: "YouTube'ni ochdim", ru: "Открыл YouTube" })}»</span>
                <span className={`gf-plus gf-f${gs >= 1 ? " in" : ""}`}>+</span>
                <span className={`gf-chip sabab gf-r${gs >= 1 ? " in" : ""}`}>💡 «{tr3({ uz: "matematikani tushunish uchun", ru: "чтобы разобраться в математике" })}»</span>
              </div>
              {gs >= 2 && <span className="grow-arrow ga-pulse" aria-hidden="true">↓</span>}
              {gs >= 2 && <p className="ex-body">{tr3({
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "1 hikoya — 3 bo'lak", ru: "1 история — 3 части" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Uch bo'lakni joylang", ru: "Разложите три части" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Hikoyani 3 bo'lakdan o'zingiz <span className="italic" style={{ color: T.accent }}>tuza olasizmi</span>?</>, ru: <>Сможете <span className="italic" style={{ color: T.accent }}>собрать</span> историю из 3 частей сами?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>User Story ichida 3 bo'lak bor: <b style={{ color: T.ink }}>«Men [KIM] sifatida, [NIMA]ni xohlayman, [NATIJA] uchun»</b> — ikkinchi mijoz gapi aynan shunday tuzilgan edi. Pastdagi bo'laklarni birma-bir bosib, hikoyani o'zingiz yig'ing.</>, ru: <>Внутри User Story три части: <b style={{ color: T.ink }}>«Я как [КТО], хочу [ЧТО], чтобы [РЕЗУЛЬТАТ]»</b> — фраза второго клиента была построена именно так. Нажимайте на части внизу по одной и соберите историю сами.</> })}</Mentor>
        <div className="formula-line fade-up delay-1">
          <span className="fw">{tr3(FORMULA_WORDS.pre)}</span>
          {SLOT_META.map((s, i) => <React3.Fragment key={s.key}>
              <button
    className={`fslot ${s.key} ${st.placed[i] ? "filled" : ""} ${st.shake === i ? "shake" : ""} ${selActive && st.placed[i] === null ? "targetable" : ""}`}
    disabled={st.placed[i] !== null}
    onClick={() => trySlot(i)}
  >{st.placed[i] === null || st.placed[i] === void 0 ? tr3(s.label) : typeof st.placed[i] === "number" ? tr3(FRAG_POOL[st.placed[i]].txt) : st.placed[i]}</button>
              <span className="fw">{tr3(FORMULA_WORDS.after[i])}</span>
            </React3.Fragment>)}
        </div>
    ...
    return <button key={idx} className={`frag-chip ${st.sel === idx ? "sel" : ""}${turnCls(litFrag, String(idx), pendFrags.length > 1)}`} onClick={() => pickChip(idx)}>{tr3(f.txt)}</button>;
  })}
        </div>}
        {done && <div className="fade-step" style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <div className="done-mini">{tr3({ uz: "✅ Hikoya to'liq!", ru: "✅ История собрана!" })} <span className="dm-sub">{tr3({ uz: "E'tibor bering: video ko'rish — harakat, «masalani mustaqil yechish» — undan keladigan foyda. NATIJA doim foydani aytadi", ru: "Обратите внимание: посмотреть видео — это действие, а «решить задачу самостоятельно» — польза от него. РЕЗУЛЬТАТ всегда называет пользу" })}</span></div>
            <button className="btn-soft" onClick={reset}>{tr3({ uz: "↻ Qaytadan", ru: "↻ Заново" })}</button>
          </div>}
      </div>
    </Stage>;
};
...
var Screen4 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Biznesdan real misol 🥤", ru: "Реальный пример из бизнеса 🥤" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={betPending && !isMentorK} label={betPending && !isMentorK ? tr3({ uz: "Avval taxminingizni tanlang", ru: "Сначала выберите свою догадку" }) : last ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: `Keyingi bosqich (${i + 1}/${K11_SLIDES.length})`, ru: `Следующий шаг (${i + 1}/${K11_SLIDES.length})` })} onClick={last ? onNext : () => setI(i + 1)} /></>}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <><span className="italic" style={{ color: T.accent }}>Milkshake</span> — biznesdagi mashhur misol</>, ru: <><span className="italic" style={{ color: T.accent }}>Молочный коктейль</span> — известный пример из бизнеса</> })}</h2></div>
        {c.predict ? <>
            <div className="kp-bet fade-step" key={`b${i}`}>
              <span className="k-slide-eyebrow">{tr3({ uz: "🎲 Taxmin o'yini · ball yo'q", ru: "🎲 Игра в догадки · без баллов" })}</span>
    ...
    return <button key={k} className={cls} disabled={locked} onClick={() => setBets((p) => ({ ...p, [i]: k }))}>
              ...
              {bet === void 0 ? <p className="kp-sub">{tr3({ uz: "Birini tanlang — javobi ochiladi.", ru: "Выберите один — ответ откроется." })}</p> : bet !== void 0 && !isMentorK && <p className={`kp-res ${bet === c.predict.ans ? "hit" : "miss"}`}>{bet === c.predict.ans ? tr3({ uz: "🎯 Topdingiz!", ru: "🎯 Угадали!" }) : tr3({ uz: <>Adashdingiz — asl javob «{tr3(c.predict.chips[c.predict.ans].t)}», «{tr3(c.predict.chips[bet].t)}» emas.</>, ru: <>Не угадали — верный ответ «{tr3(c.predict.chips[c.predict.ans].t)}», а не «{tr3(c.predict.chips[bet].t)}».</> })}</p>}
            </div>
            ...
            <span className="k-slide-eyebrow">📊 {tr3({ uz: "Milkshake misoli", ru: "Пример с коктейлем" })} · {i + 1} / {K11_SLIDES.length}</span>
            <div className="k-slide-ic">{c.ic}</div>
            <h3 className="k-slide-h">{tr3(c.h)}</h3>
            <p className="k-slide-body">{tr3(c.body)}</p>
          </div>}
        <div className="k-dots">{K11_SLIDES.map((_, k) => <button key={k} className={`k-dot ${k === i ? "cur" : k < i ? "fill" : ""}`} onClick={() => setI(k)} aria-label={tr3({ uz: `${k + 1}-bosqich`, ru: `Шаг ${k + 1}` })} />)}</div>
        {last && !betPending && <div className="frame-soft fade-step">
          <p className="body" style={{ margin: 0, color: T.accent, fontWeight: 600 }}>
            {tr3({ uz: "Dars boshidagi ikki mijozni eslang — bu misol ham xuddi shuni aytadi: odam nima sotib olayotgani emas, NEGA olayotgani muhim.", ru: "Вспомните двух клиентов в начале урока — этот пример говорит о том же: важно не что человек покупает, а ЗАЧЕМ он это покупает." })}
          </p>
        </div>}
      </div>
    </Stage>;
...
var ScreenStoryWorkshop = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const saveHint = !v.full ? null : natijaTakror ? tr3({ uz: "NATIJA harakatning takrori bo'lib qoldi. Harakatdan keyin hayotda nima o'zgaradi — shuni yozing.", ru: "РЕЗУЛЬТАТ повторил действие. Напишите, что изменится в жизни после этого действия." }) : kimTakror ? tr3({ uz: "Bu KIM allaqachon daftarda bor — boshqa foydalanuvchi turini o'ylang.", ru: "Такой КТО в блокноте уже есть — придумайте другой тип пользователя." }) : null;
  ...
  const navLabel = done || isMentorW ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: `✍️ ${saved.length}/3 — hikoyani yozib saqlang`, ru: `✍️ ${saved.length}/3 — напишите историю и сохраните` });
  const [fieldFocus, setFieldFocus] = useState4(false);
  ...
  return <Stage eyebrow={tr3({ uz: "Amaliyot · hikoya-ustaxona ✍️", ru: "Практика · мастерская историй ✍️" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={turnBusyW} disabled={!done && !isMentorW} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>O'z loyihangiz uchun <span className="italic" style={{ color: T.accent }}>3 ta hikoya</span> yozing</>, ru: <>Напишите <span className="italic" style={{ color: T.accent }}>3 истории</span> для своего проекта</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Endi navbat sizda — o'z loyihangiz uchun yozasiz. Kartani to'ldirib <b style={{ color: T.ink }}>«Saqlash»</b>ni bosing: tepadagi qadam-belgisi ✓ bo'ladi va yangi karta keladi.</>, ru: <>Теперь ваша очередь — пишете для своего проекта. Заполните карточку и нажмите <b style={{ color: T.ink }}>«Сохранить»</b>: шаг наверху станет ✓ и придёт новая карточка.</> })}</Mentor>
        {
    ...
    /* Qadam-indikatori navbat maydonlarda yurayotganda tinchlanadi — lahzada bitta puls (88-qonun (a)) */
        ...
        <div className={`jw-steps fade-up ${turnQuietW ? "turn-quiet" : ""}`} aria-label={tr3({ uz: `${saved.length}/3 hikoya yozildi`, ru: `Написано историй: ${saved.length}/3` })}>
          {[0, 1, 2].map((i) => <React3.Fragment key={i}>
              ...
              <span className={`jws ${saved[i] ? "on" : i === saved.length && showEditor ? "cur" : ""}`}>
                ...
                <em className="jws-t">{saved[i] ? saved[i].kim : tr3({ uz: `${i + 1}-hikoya`, ru: `История ${i + 1}` })}</em>
              </span>
            </React3.Fragment>)}
        </div>
        {showEditor ? <div className="swed fade-up" key={editing ? `e${editIdx}` : `n${saved.length}`}>
            {editing && <span className="swed-tag">✎ {tr3({ uz: `${editIdx + 1}-hikoyani tahrirlash`, ru: `Правка истории ${editIdx + 1}` })}</span>}
            <p className="swed-sent">{tr3({
    ...
    ru: <>Я как <b className={`ss-slot kim ${draft.kim ? "on" : ""}`}>{draft.kim || "кто"}</b>, хочу <b className={`ss-slot nima ${draft.nima ? "on" : ""}`}>{draft.nima || "что"}</b>, чтобы <b className={`ss-slot natija ${draft.natija ? "on" : ""}`}>{draft.natija || "результат"}</b>.</>
            ...
            <div className="swcard-fields">
              <label className={`smini-f kim ${v.kimOk ? "on" : ""}${turnCls(litField, "kim", pendFields.length > 1)}`}><span>{tr3({ uz: "KIM", ru: "КТО" })}</span><input value={draft.kim} onChange={(e) => setD({ kim: e.target.value })} onFocus={() => setFieldFocus(true)} onBlur={() => setFieldFocus(false)} placeholder={tr3({ uz: "foydalanuvchi turi", ru: "тип пользователя" })} /></label>
              <label className={`smini-f nima ${v.nimaOk ? "on" : ""}${turnCls(litField, "nima", pendFields.length > 1)}`}><span>{tr3({ uz: "NIMA", ru: "ЧТО" })}</span><input value={draft.nima} onChange={(e) => setD({ nima: e.target.value })} onFocus={() => setFieldFocus(true)} onBlur={() => setFieldFocus(false)} placeholder={tr3({ uz: "harakat", ru: "действие" })} /></label>
              <label className={`smini-f natija ${v.natijaOk ? "on" : ""}${turnCls(litField, "natija", pendFields.length > 1)}`}><span>{tr3({ uz: "NATIJA", ru: "РЕЗУЛЬТАТ" })}</span><input value={draft.natija} onChange={(e) => setD({ natija: e.target.value })} onFocus={() => setFieldFocus(true)} onBlur={() => setFieldFocus(false)} placeholder={tr3({ uz: "natija", ru: "результат" })} /></label>
            </div>
            {saveHint && <p className="swed-hint">💡 {saveHint}</p>}
            <div className="swed-btns">
              {editing && <button className="btn-ghost" onClick={() => setSt((prev) => ({ ...prev, draft: emptyCard(), editIdx: -1 }))}>{tr3({ uz: "Bekor qilish", ru: "Отмена" })}</button>}
              {!v.full && <span className="swed-cnt">{[draft.kim, draft.nima, draft.natija].filter((x) => (x || "").trim().length >= 2).length}/3 {tr3({ uz: "maydon to'ldi", ru: "полей заполнено" })}</span>}
              <button className={`swed-save${saveTurn ? " turn-ring" : ""}`} disabled={!canSave} onClick={saveDraft}>✓ {tr3({ uz: "Saqlash", ru: "Сохранить" })}</button>
            </div>
          </div> : <>
            <div className="done-mini fade-step">{tr3({ uz: "✅ Uchta hikoya tayyor", ru: "✅ Три истории готовы" })} <span className="dm-sub">{tr3({ uz: "— tahrirlash uchun ✎ belgisidan foydalaning", ru: "— для правки нажмите значок ✎" })}</span></div>
    ...
    return <div key={i} className={`svd-card ${editIdx === i ? "editing" : ""}`}>
                    <div className="svd-top">
                      ...
                      <span className="svd-stars">{[1, 2, 3, 4, 5].map((st2) => <button key={st2} className={`star ${c.star >= st2 ? "on" : ""}`} onClick={() => setStar(i, st2)} aria-label={tr3({ uz: `Muhimligi: ${st2} yulduz`, ru: `Важность: ${st2} звёзд` })} title={tr3({ uz: `Muhimligi: ${st2} yulduz`, ru: `Важность: ${st2} звёзд` })}>★</button>)}</span>
                      <button className="svd-edit" onClick={() => editCard(i)} aria-label={tr3({ uz: `${i + 1}-hikoyani tahrirlash`, ru: `Править историю ${i + 1}` })}>✎ {tr3({ uz: "Tahrirlash", ru: "Править" })}</button>
                    </div>
                    <p className="svd-sent">{tr3({
...
var peerLine = (card, v) => {
  const why = tr3(card.why);
  if (card.ok) return v.ok ? { good: true, t: tr3({ uz: `Ha, to'g'ri: ${why}`, ru: `Да, верно: ${why}` }) } : { good: false, t: tr3({ uz: `Bu hikoya aslida to'g'ri: ${why}`, ru: `На самом деле эта история верная: ${why}` }) };
  if (v.ok) return { good: false, t: tr3({ uz: `Bu hikoya noto'g'ri: ${why}`, ru: `Эта история неверная: ${why}` }) };
  if (v.reason === card.flaw) return { good: true, t: tr3({ uz: `To'g'ri: ${why}`, ru: `Верно: ${why}` }) };
  return { good: false, t: tr3({ uz: `Kamchilik boshqa joyda: ${why}`, ru: `Недостаток в другом месте: ${why}` }) };
};
var ScreenPeer = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Tekshiruvchi stoli · 🔍", ru: "Стол проверяющего · 🔍" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentorP} label={done || isMentorP ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: `Yana ${left} kartani baholang`, ru: `Оцените ещё: ${left}` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="peer-top">
          <h2 className="title h-title fade-up" style={{ margin: 0 }}>{tr3({ uz: <>Uch kartani <span className="italic" style={{ color: T.accent }}>tekshiring</span>.</>, ru: <><span className="italic" style={{ color: T.accent }}>Проверьте</span> три карточки.</> })}</h2>
        ...
        <Mentor>{done ? tr3({ uz: <>Mana, uchala javobingiz yonma-yon.</>, ru: <>Вот все три ваших ответа рядом.</> }) : tr3({ uz: <>Har kartani o'qing va <b style={{ color: T.ink }}>✓ To'g'ri</b> yoki <b style={{ color: T.ink }}>✕ Noto'g'ri</b>ga ajrating.</>, ru: <>Прочитайте каждую карточку и отнесите её к <b style={{ color: T.ink }}>✓ Верно</b> или <b style={{ color: T.ink }}>✕ Неверно</b>.</> })}</Mentor>
        {card && <>
            <div key={idx} className="peer-big fade-step">
              <p className="peer-sent">{tr3({
    uz: <>Men <b style={{ color: T.blue }}>{tr3(card.kim)}</b> sifatida, <b style={{ color: "#B77A16" }}>{tr3(card.nima)}</b>ni xohlayman, <b style={{ color: T.success }}>{tr3(card.natija)}</b> uchun.</>,
    ru: <>Я как <b style={{ color: T.blue }}>{tr3(card.kim)}</b>, хочу <b style={{ color: "#B77A16" }}>{tr3(card.nima)}</b>, чтобы <b style={{ color: T.success }}>{tr3(card.natija)}</b>.</>
  })}</p>
            </div>
            <div className="peer-acts">
              <button type="button" className={`peer-vbtn yes ${cur && cur.ok ? "on" : ""}${waveCls(verdictHint, 0, 2)}`} onClick={() => choose(true)} disabled={!!cur}>✓ {tr3({ uz: "To'g'ri", ru: "Верно" })}</button>
              <button type="button" className={`peer-vbtn no ${cur && !cur.ok ? "on" : ""} ${asking ? "armed" : ""}${waveCls(verdictHint, 1, 2)}`} onClick={() => choose(false)} disabled={!!cur}>✕ {tr3({ uz: "Noto'g'ri", ru: "Неверно" })}</button>
            </div>
            {asking && !cur && <div className="peer-why fade-step">
                <span className="peer-why-l">{tr3({ uz: "Kamchiligi nimada?", ru: "В чём недостаток?" })}</span>
                {PEER_REASONS.map((r, ri) => <button key={r.key} type="button" className={`peer-chip${waveCls(reasonHint, ri, PEER_REASONS.length)}`} onClick={() => commit({ ok: false, reason: r.key })}>{tr3(r.t)}</button>)}
              </div>}
            {cur && fb && <div className="peer-fb fade-step">
                <p className={`peer-fb-t ${fb.good ? "good" : ""}`}>{fb.good ? "✅" : "💡"} {fb.t}</p>
                <button type="button" className={`btn-soft peer-go${goHint ? " turn-ring" : ""}`} onClick={goNext}>{idx + 1 < PEER_CARDS.length ? tr3({ uz: "Keyingisi ▸", ru: "Следующая ▸" }) : tr3({ uz: "Xulosa ▸", ru: "Итог ▸" })}</button>
              </div>}
          </>}
    ...
    return <div key={i} className="peer-srow">
                  <span className="peer-snom">{tr3({ uz: `${i + 1}-karta`, ru: `Карточка ${i + 1}` })}</span>
                  <span className="peer-sv">{v.ok ? tr3({ uz: "✓ to'g'ri", ru: "✓ верно" }) : tr3({ uz: `✕ noto'g'ri — ${rr ? tr3(rr.t) : "tuzatish kerak"}`, ru: `✕ неверно — ${rr ? tr3(rr.t) : "нужно поправить"}` })}</span>
                  <span className="peer-sm" title={exact ? tr3({ uz: "Aniq topdingiz", ru: "Попали точно" }) : tr3({ uz: "Izohini ko'rdingiz", ru: "Вы увидели пояснение" })}>{exact ? "✅" : "💡"}</span>
                </div>;
  })}
            {
    /* F-0725-02: «shu ko'z bilan qarang» ko'chma ma'nosi olib tashlandi — harakat-tili (42-qonun): nima qilish + nimani qidirish. */
  }
            <p className="peer-close">{tr3({ uz: "Endi o'z hikoyangizda ham shu kamchiliklarni qidiring.", ru: "Теперь поищите эти же недостатки в своей истории." })}</p>
          </div>}
        <StudentPracticePulse live={live} screen={screen} />
        <MentorPracticeStats live={live} screen={screen} label={tr3({ uz: "🔍 Uch kartani baholaganlar", ru: "🔍 Кто оценил три карточки" })} />
        <MentorNote>{tr3({ uz: "Har javobdan keyin bitta savol bering: «Nega shunday ajratdingiz?» — sabab javobning o'zidan qimmatroq. To'liq karta — o'rtadagi 2-karta (qidiruv qatori hikoyasi), qolgan ikkitasida kamchilik bor.", ru: "После каждого ответа задайте один вопрос: «Почему вы так решили?» — причина ценнее самого ответа. Полная карточка — вторая, в середине (история про строку поиска), в двух других есть недостатки." })}</MentorNote>
      </div>
    </Stage>;
};
var BT_REQS = [
...
var ScreenClinic = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "So'rov-sinovi 🛠", ru: "Испытание просьб 🛠" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentorB} label={done || isMentorB ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Quruq va to'liq so'rovni sinab ko'ring", ru: "Испытайте сухую и полную просьбу" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Qaysi hikoyadan <span className="italic" style={{ color: T.accent }}>yaxshi natija</span> chiqadi?</>, ru: <>Из какой истории выйдет <span className="italic" style={{ color: T.accent }}>хороший результат</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>So'rov qanday yozilgani natijani o'zgartiradi. Mana shu uchta so'rov bitta narsa haqida, lekin turlicha yozilgan — birini tanlab <b style={{ color: T.ink }}>«Sinab ko'rish»</b>ni bosing va farqni o'zingiz ko'ring.</>, ru: <>От того, как написана просьба, зависит результат. Три просьбы ниже — об одном и том же, но написаны по-разному: выберите одну, нажмите <b style={{ color: T.ink }}>«Испытать»</b> и увидите разницу сами.</> })}</Mentor>
        <div className="split">
          <Col>
            <p className="flow-label">{tr3({ uz: "So'rovlar — birini tanlang", ru: "Просьбы — выберите одну" })}</p>
    ...
    return <button key={r.id} className={`bt-req ${on ? "on" : ""}${turnCls(lit, r.id, pend.length > 1)}`} disabled={reading} onClick={() => pick(r.id)}>
                    <span className="bt-req-tag">{tr3(r.tag)}{was && <span className="bt-req-done">✓ {tr3({ uz: "sinaldi", ru: "испытано" })}</span>}</span>
                    <span className="bt-req-txt">{tr3(r.txt)}</span>
                  </button>;
  })}
            </div>
            <button className={`bt-send${sendHint ? " turn-ring go" : ""}`} onClick={send} disabled={!sel || reading} style={{ alignSelf: "flex-start" }}>
              {reading ? tr3({ uz: "⏳ O'qilmoqda…", ru: "⏳ Читает…" }) : tr3({ uz: "▶️ Sinab ko'rish", ru: "▶️ Испытать" })}
            </button>
          </Col>
          <Col>
            <p className="flow-label">{tr3({ uz: "So'rov o'qilmoqda", ru: "Просьбу читают" })}</p>
            <div className="bt-desk fade-up delay-2">
              <div className="bt-desk-top"><span className="bt-dev" aria-hidden="true">🧑‍💻</span><span className="bt-dev-name">{tr3({ uz: "Ish stoli", ru: "Рабочий стол" })}</span><span className="bt-desk-t">{!req ? tr3({ uz: "so'rov kutilmoqda…", ru: "ждём просьбу…" }) : reading ? tr3({ uz: "o'qilmoqda…", ru: "читает…" }) : phase === "out" ? tr3({ uz: "ish tugadi", ru: "работа готова" }) : tr3({ uz: "so'rov keldi — «Sinab ko'rish»ni bosing", ru: "просьба пришла — нажмите «Испытать»" })}</span></div>
    ...
    return <div key={k} className={`bt-part p${i} ${bad ? "bad" : okP ? "ok" : ""} ${cur ? "cur" : ""}`}>
                      <span className="bt-part-lbl">{tr3(lbl)}</span>
                      <span className="bt-part-val">{seen ? tr3(val) || tr3({ uz: "— yo'q —", ru: "— нет —" }) : "?"}</span>
                    </div>;
  })}
              </div>
              {phase === "out" && req && !req.ok && <p className="bt-guess">💭 {tr3(req.guess)}</p>}
              {phase === "out" && req && req.ok && <Confetti />}
              {phase === "out" && req && <div className={req.ok ? "frame-success fade-step" : "frame-warn fade-step"}>
                  <p className="body" style={{ margin: 0, color: T.ink }}>{tr3(req.out)}</p>
                </div>}
            </div>
            {done && <div className="done-mini fade-step">{tr3({ uz: "✅ Farqni ko'rdingiz!", ru: "✅ Вы увидели разницу!" })} <span className="dm-sub">{tr3({ uz: "— to'liq hikoya aniq yo'nalishni ko'rsatadi, taxmin qilishga o'rin qolmaydi", ru: "— полная история задаёт точное направление, гадать больше не нужно" })}</span></div>}
          </Col>
        </div>
      </div>
    </Stage>;
...
var ScreenPriority = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [stories] = useState4(() => {
    const s = readFullStories();
    return s.length ? s : DEMO_STORIES;
  });
  const [isDemo] = useState4(() => readFullStories().length === 0);
  const [st, setSt] = useState4(() => {
  ...
  return <Stage eyebrow={tr3({ uz: "Muhimlik darajalari · 🔥", ru: "Уровни важности · 🔥" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentorPr} label={done || isMentorPr ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "🔥 3 hikoyani ajrating", ru: "🔥 Разложите 3 истории" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Qaysi hikoya <span className="italic" style={{ color: T.accent }}>eng muhim</span>? Darajasini belgilang</>, ru: <>Какая история <span className="italic" style={{ color: T.accent }}>самая важная</span>? Отметьте её уровень</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Siz 3 ta hikoya yozdingiz — lekin uchalasini birdaniga qurib bo'lmaydi, shuning uchun eng muhimi birinchi qilinadi; buni <b style={{ color: T.ink }}>navbat belgilash</b> (prioritet) deyiladi. Hikoyani bosib tanlang, keyin unga mos darajani bosing.</>, ru: <>Вы написали 3 истории — но все три сразу не построишь, поэтому первой делают самую важную; это называют <b style={{ color: T.ink }}>расстановкой очереди</b> (приоритет). Нажмите на историю, а потом на подходящий ей уровень.</> })}</Mentor>
        {isDemo && <p className="small fade-up" style={{ margin: 0, color: T.ink3, fontStyle: "italic" }}>{tr3({ uz: "Namuna hikoyalar — ustaxonada yozilganlar shu yerda chiqadi.", ru: "Это истории-образцы — здесь появятся те, что вы напишете в мастерской." })}</p>}
        {
    /* F-0727-08: kartalar mentor-gapga yopishib turmasin — belgi-yorliqli LAGANCHA: qizil puls-border
       «bularni joylashtirish kerak» signalini beradi; birinchi karta joylashgach tinchlanadi. */
        ...
        {stories.some((_, i) => !assign[i]) && <div className={`pd-tray ${trayTurn ? "" : "calm"}`}>
            <span className="pd-tray-lbl">{tr3({ uz: "✋ Bu 3 hikoyangizni pastdagi darajalarga ajrating", ru: "✋ Разложите эти 3 истории по уровням внизу" })} <span className="pd-tray-arrow">↓</span></span>
            <div className="pd-pool">
              {stories.map((c, i) => assign[i] ? null : <button key={i} className={`pd-card ${sel === i ? "sel" : ""}`} onClick={() => pickStory(i)}>
                  ...
                  <span className="pd-card-txt"><b style={{ color: T.blue }}>{tr3(c.kim)}</b> — {tr3(c.nima)}</span>
                </button>)}
            </div>
          </div>}
    ...
    return <div key={col.k} className={`ms-row ${col.k} ${shakeCol === col.k ? "shake" : ""} ${sel >= 0 && !full ? "targetable" : ""}${waveCls(colTurn && !full, di, 3)}`} onClick={() => tryCol(col.k)} role="button" tabIndex={0} onKeyDown={(e) => {
      if (e.key === "Enter") tryCol(col.k);
                ...
                <div className="ms-h"><span className="ms-t">{tr3(col.t)}</span><span className="ms-sub">{tr3(col.sub)}</span></div>
                {items.length ? items.map((i) => <button key={i} className={`pd-card placed ${sel === i ? "sel" : ""}`} onClick={(e) => {
      e.stopPropagation();
      pickStory(i);
    }}>
                    <span className="pd-card-n">{i + 1}</span>
                    <span className="pd-card-txt"><b style={{ color: T.blue }}>{tr3(stories[i].kim)}</b> — {tr3(stories[i].nima)}</span>
                  </button>) : <div className="ms-empty">{col.k === "p1" ? tr3({ uz: "eng muhim hikoya shu yerga", ru: "сюда — самую важную историю" }) : col.k === "p2" ? tr3({ uz: "keyingisi shu yerga", ru: "сюда — следующую" }) : tr3({ uz: "keyinroq qilinadigani shu yerga", ru: "сюда — то, что можно позже" })}</div>}
              </div>;
  })}
        </div>
        {allPlaced && topIdx !== void 0 && <div className="done-mini fade-step">{tr3({ uz: "✅ Ajratildi!", ru: "✅ Разложено!" })} <span className="dm-sub">{tr3({ uz: "— eng muhim vazifangiz:", ru: "— ваша самая важная задача:" })} «{tr3(stories[topIdx].nima)}» 🔥</span></div>}
        <StudentPracticePulse live={live} screen={screen} />
        <MentorPracticeStats live={live} screen={screen} label={tr3({ uz: "🔥 Ajratib bo'lganlar", ru: "🔥 Кто разложил" })} />
        <MentorNote>{tr3({ uz: "Joylashgan kartani bosib boshqa darajaga ko'chirsa bo'ladi. «Eng muhim» talashib qolganlarga savol: qaysi hikoya foydalanuvchiga ENG katta foydani beradi? Javob — o'sha birinchi.", ru: "Уже разложенную карточку можно нажать и перенести на другой уровень. Тем, кто спорит про «самое важное», задайте вопрос: какая история даёт пользователю САМУЮ большую пользу? Она и идёт первой." })}</MentorNote>
      </div>
    </Stage>;
};
...
var Screen7 = (props) => <QuestionScreen
  {...props}
  eyebrow={tr3({ uz: "Tekshiruv · hikoya 1", ru: "Проверка · история 1" })}
  scope="module-mikro"
  ctaLabel={tr3({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr3({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ
    ask={tr3({ uz: "Bu hikoyada qaysi bo'lak yetishmayapti? Tanlang.", ru: "Какой части не хватает в этой истории? Выберите." })}
    story={tr3({ uz: "Men foydalanuvchi sifatida, qizil tugma xohlayman.", ru: "Я как пользователь, хочу красную кнопку." })}
  />}
  questionText={tr3({ uz: "Hikoya 1: qizil tugma", ru: "История 1: красная кнопка" })}
  options={[tr3({ uz: "KIM bo'lagi — foydalanuvchi turi", ru: "Часть КТО — тип пользователя" }), tr3({ uz: "NIMA bo'lagi — harakat", ru: "Часть ЧТО — действие" }), tr3({ uz: "NATIJA bo'lagi — real foyda", ru: "Часть РЕЗУЛЬТАТ — реальная польза" })]}
  correctIdx={2}
  explainCorrect={tr3({ uz: "To'g'ri — NATIJA umuman yo'q: «qizil tugma» harakat, lekin foydalanuvchi undan qanday foyda olishi aytilmagan. Yana bir kamchilik: KIM ham aniq emas («foydalanuvchi» — qaysi turi?).", ru: "Верно — РЕЗУЛЬТАТА нет совсем: «красная кнопка» — это действие, но не сказано, какую пользу пользователь из него получит. Есть и второй недостаток: КТО тоже неконкретен («пользователь» — какой именно?)." })}
  explainWrong={{ 0: tr3({ uz: "KIM bor — «foydalanuvchi» (aniq bo'lmasa ham yozilgan). Yetishmayotgani — NATIJA: harakatdan keyin nima foyda?", ru: "КТО есть — «пользователь» (пусть и неконкретно, но написан). Не хватает РЕЗУЛЬТАТА: какая польза после действия?" }), 1: tr3({ uz: "NIMA bor — «qizil tugma xohlayman». Yetishmayotgani — NATIJA: undan keyin nima foyda?", ru: "ЧТО есть — «хочу красную кнопку». Не хватает РЕЗУЛЬТАТА: какая от этого польза?" }), default: tr3({ uz: "NATIJA (real foyda) yetishmaydi — oxirgi variantni tanlang.", ru: "Не хватает РЕЗУЛЬТАТА (реальной пользы) — выберите последний вариант." }) }}
/>;
var Screen8 = (props) => <QuestionScreen
  {...props}
  eyebrow={tr3({ uz: "Tekshiruv · hikoya 2", ru: "Проверка · история 2" })}
  scope="module-mikro"
  ctaLabel={tr3({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr3({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ
    ask={tr3({ uz: "Bu hikoyada qaysi bo'lak oldingisini takrorlaydi? Tanlang.", ru: "Какая часть этой истории повторяет предыдущую? Выберите." })}
    story={tr3({ uz: "Men mehmon sifatida, saytga kirishni xohlayman, saytga kirish uchun.", ru: "Я как гость, хочу войти на сайт, чтобы войти на сайт." })}
  />}
  questionText={tr3({ uz: "Hikoya 2: saytga kirish", ru: "История 2: вход на сайт" })}
  options={[tr3({ uz: "Men mehmon sifatida", ru: "Я как гость" }), tr3({ uz: "saytga kirishni xohlayman", ru: "хочу войти на сайт" }), tr3({ uz: "saytga kirish uchun", ru: "чтобы войти на сайт" })]}
  correctIdx={2}
  explainCorrect={tr3({ uz: "To'g'ri — «saytga kirish uchun» harakatning takrori, foyda aytilmagan. To'g'ri varianti, masalan: «buyurtmamni tez topish uchun».", ru: "Верно — «чтобы войти на сайт» повторяет действие, польза не названа. Правильно было бы, например: «чтобы быстро найти свой заказ»." })}
  explainWrong={{ 0: tr3({ uz: "KIM aniq («mehmon») — bu joyi to'g'ri. Xato NATIJA'da: u harakatni takrorlaydi. Oxirgi variantni tanlang.", ru: "КТО конкретен («гость») — здесь всё верно. Ошибка в РЕЗУЛЬТАТЕ: он повторяет действие. Выберите последний вариант." }), 1: tr3({ uz: "NIMA to'g'ri yozilgan. Muammo — NATIJA harakatni takrorlaydi. Oxirgi variantni tanlang.", ru: "ЧТО написано верно. Проблема в том, что РЕЗУЛЬТАТ повторяет действие. Выберите последний вариант." }), default: tr3({ uz: "NATIJA harakatni takrorlaydi (foyda yo'q) — oxirgi variantni tanlang.", ru: "РЕЗУЛЬТАТ повторяет действие (пользы нет) — выберите последний вариант." }) }}
/>;
var Screen9 = (props) => <QuestionScreen
  {...props}
  eyebrow={tr3({ uz: "Tekshiruv · hikoya 3", ru: "Проверка · история 3" })}
  scope="module-mikro"
  ctaLabel={tr3({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr3({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ
    ask={tr3({ uz: "Bu gap User Story'mi? Javobni tanlang.", ru: "Эта фраза — User Story? Выберите ответ." })}
    story={tr3({ uz: "Saytda dark mode (qorong'i rejim) bo'lsin.", ru: "Пусть на сайте будет dark mode (тёмная тема)." })}
  />}
  questionText={tr3({ uz: "Dark mode gapi: baho", ru: "Фраза про dark mode: оценка" })}
  options={[tr3({ uz: "Tayyor User Story — o'zgartirish shart emas", ru: "Готовая User Story — менять не нужно" }), tr3({ uz: "Imkoniyat-so'rovi (feature request) — hali hikoya emas", ru: "Заявка на возможность (feature request) — ещё не история" }), tr3({ uz: "JTBD misoli — milkshake'dagi kabi «natija»", ru: "Пример JTBD — «результат», как в истории с коктейлем" })]}
  correctIdx={1}
  explainCorrect={tr3({ uz: "To'g'ri — bu hali hikoya emas, shunchaki so'rov: unda KIM ham, NATIJA ham yo'q, faqat NIMA aytilgan.", ru: "Верно — это ещё не история, а просто просьба: в ней нет ни КТО, ни РЕЗУЛЬТАТА, названо только ЧТО." })}
  explainWrong={{ 0: tr3({ uz: "Hali tayyor emas: gapda KIM ham, NATIJA ham yo'q — faqat NIMA kerakligi aytilgan. Bunday gapni imkoniyat-so'rovi deymiz.", ru: "Ещё не готова: во фразе нет ни КТО, ни РЕЗУЛЬТАТА — сказано только, ЧТО нужно. Такую фразу называют заявкой на возможность." }), 2: tr3({ uz: "JTBD — odam mahsulotdan kutadigan foydali natija haqidagi g'oya. Bu gap esa oddiy so'rov: unda na KIM bor, na NATIJA.", ru: "JTBD — это идея о полезном результате, которого человек ждёт от продукта. А здесь обычная просьба: ни КТО, ни РЕЗУЛЬТАТА." }), default: tr3({ uz: "Bu — imkoniyat-so'rovi: faqat NIMA kerakligi aytilgan, KIM ham, NATIJA ham yo'q. Hali hikoya emas.", ru: "Это заявка на возможность: сказано только ЧТО нужно, нет ни КТО, ни РЕЗУЛЬТАТА. Ещё не история." }) }}
/>;
var KODING_STARTER = {
...
var ScreenCoding = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Koding · 🛠 kompilyator", ru: "Кодинг · 🛠 компилятор" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Avval kompilyatorda bajaring", ru: "Сначала выполните в компиляторе" })} onClick={onNext} /></>}>
        ...
        <div className="kdx fade-up delay-1">
          ...
          <div className="kdx-out">
            {previewStories.map((s, i) => <div key={i} className="kdx-card" style={{ "--kd": `${0.5 + i * 0.3}s` }}>
                ...
                {tr3({ uz: <>Men <b>{tr3(s.kim)}</b> sifatida, <b>{tr3(s.nima)}</b>ni xohlayman, <b>{tr3(s.natija)}</b> uchun.</>, ru: <>Я как <b>{tr3(s.kim)}</b>, хочу <b>{tr3(s.nima)}</b>, чтобы <b>{tr3(s.natija)}</b>.</> })}
              </div>)}
          </div>
        </div>
    ...
    writeKodingOpen(true);
          ...
          {done && <span className="kdx-cta-sub">{tr3({ uz: "Bajarildi — xohlasangiz kodni yana yaxshilang", ru: "Выполнено — при желании доработайте код" })}</span>}
          {
    /* 🔓 TAKRORLASH-YO'LI (89-qonun) — FAQAT erkin rejimda va FAQAT bajarilmagan holatda.
       ...
       Puls ham olmaydi — navbat «Kompilyatorni ochish» tugmasida (88-qonun (a)). */
          ...
          {!done && isSelf && <button className="kdx-skip" onClick={onNext}>{tr3({ uz: "✓ Bu mashqni sinfda bajarganman — davom etish →", ru: "✓ Это задание я делал(а) в классе — продолжить →" })}</button>}
        </div>
        <div className="takeaway fade-up delay-2"><span className="ta-bulb">📌</span><p className="ta-h">{tr3({ uz: "User Story kod yozishdan OLDIN yoziladi — avval «kimga nima kerak»ni bilamiz, keyin quramiz.", ru: "User Story пишут ДО кода — сначала узнаём, «кому что нужно», потом строим." })}</p></div>
        {done && <div className="done-mini fade-step" style={{ alignSelf: "center" }}>{tr3({ uz: "✅ Ishladi!", ru: "✅ Заработало!" })} <span className="dm-sub">{tr3({ uz: "— 3 hikoyangiz koddan karta bo'lib chiqdi", ru: "— ваши 3 истории вышли из кода карточками" })}</span></div>}
        <StudentPracticePulse live={live} screen={screen} />
...
function PairTimer({ onStage, muted }) {
  ...
  return <div className="pair-timer">
      {st.running ? <div className="pair-live">
        ...
        </div> : <p className="pair-now" style={{ margin: 0 }}>{st.done ? tr3({ uz: "✓ Vaqt tugadi — ikkalangiz ham aytib bo'ldingiz. Barakalla!", ru: "✓ Время вышло — рассказали оба. Молодцы!" }) : tr3({ uz: "Har biringizga 30 soniyadan — avval A, keyin B.", ru: "По 30 секунд каждому — сначала A, потом B." })}</p>}
      <div className="pair-timer-btns">
        {!st.running && <button className={st.done ? "btn-soft" : `pair-start${startTurn ? "" : " calm"}`} onClick={() => setSt({ running: true, left: 60, done: false })}>{st.done ? tr3({ uz: "↻ Yana 1 daqiqa", ru: "↻ Ещё 1 минута" }) : tr3({ uz: "▶ 1 daqiqani boshlash", ru: "▶ Запустить минуту" })}</button>}
...
var Screen11 = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Mustahkamlash · 2 qadam", ru: "Закрепление · 2 шага" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext turnBusy={!written} label={tr3({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Eng muhim hikoyangizni <span className="italic" style={{ color: T.accent }}>yoddan</span> aytib bera olasizmi?</>, ru: <>Сможете рассказать свою главную историю <span className="italic" style={{ color: T.accent }}>по памяти</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Dars deyarli tugadi — endi eng muhim hikoyangizni yoddan aytib berasiz: <b style={{ color: T.ink }}>KIM</b> uchun yozdingiz va u qanday <b style={{ color: T.ink }}>foyda</b> oladi? <span style={{ whiteSpace: "nowrap" }}>«▶ 1 daqiqani boshlash»</span> tugmasini bosing — avval siz sherigingizga aytasiz, keyin u sizga.</>, ru: <>Урок почти закончен — теперь расскажите свою главную историю по памяти: для <b style={{ color: T.ink }}>КОГО</b> вы её написали и какую <b style={{ color: T.ink }}>пользу</b> он получит? Нажмите <span style={{ whiteSpace: "nowrap" }}>«▶ Запустить минуту»</span> — сначала рассказываете вы напарнику, потом он вам.</> })}</Mentor>
        <div className="rcp-flow">
          <div className="rcp-step fade-up delay-1">
            <div className="rcp-step-h"><span className="rcp-n">1</span><div><span className="rcp-t">{tr3({ uz: "🗣 Sherigingizga ayting: kim uchun, qanday foyda uchun", ru: "🗣 Расскажите напарнику: для кого и ради какой пользы" })}</span></div></div>
          ...
          <div className="rcp-step fade-up delay-2">
            ...
            <span className={`turn-wrap${inputTurn ? " turn-ring" : ""}`}>
              <input className="reflect-input" value={text} onChange={(e) => save(e.target.value)} onFocus={() => setReflFocus(true)} onBlur={() => setReflFocus(false)} placeholder={tr3({ uz: "Men ... uchun hikoya yozdim, chunki unga ... kerak edi", ru: "Я написал(а) историю для ..., потому что ему нужно было ..." })} maxLength={160} />
            </span>
...
var HwTaskCard = () => {
  ...
  const chosenT = customMode ? (custom || "").trim() : hwById(target) ? tr3(hwById(target).t) : (target || "").trim();
  const chosen = chosenT;
  const pickTurn = useTurnHint(!chosen);
  return <div className="card hw fade-up d4">
      <div className="card-lbl" style={{ color: T.accent }}>📝 {tr3({ uz: <>Uyda <b>kim</b> uchun hikoya yozasiz?</>, ru: <>Для <b>кого</b> напишете историю дома?</> })}</div>
      <p className="body" style={{ margin: "0 0 10px", color: T.ink }}>{tr3({ uz: <>Uyda endi <b style={{ color: T.ink }}>boshqa foydalanuvchi</b> uchun hikoya yozasiz — ro'yxatdan bittasini tanlang yoki «✍️ o'z variantimni yozaman»ni bosing.</>, ru: <>Дома вы напишете историю уже для <b style={{ color: T.ink }}>другого пользователя</b> — выберите одного из списка или нажмите «✍️ напишу свой вариант».</> })}</p>
      <div className="hw-chips">
        {HW_TARGETS.map((x, ti) => <button key={x.id} className={`hw-chip ${target === x.id && !customMode ? "on" : ""}${waveCls(pickTurn, ti, HW_TARGETS.length + 1)}`} onClick={() => pick(x.id)}>{tr3(x.t)}</button>)}
        <button className={`hw-chip add ${customMode ? "on" : ""}${waveCls(pickTurn, HW_TARGETS.length, HW_TARGETS.length + 1)}`} onClick={openCustom}>✍️ {tr3({ uz: "o'z variantimni yozaman", ru: "напишу свой вариант" })}</button>
      </div>
      {customMode && <input className="reflect-input fade-step" value={custom} onChange={(e) => setCustom(e.target.value)} placeholder={tr3({ uz: "masalan: o'qituvchi, ota-ona, ilk mijoz…", ru: "например: учитель, родитель, первый клиент…" })} maxLength={40} autoFocus />}
      {chosen ? <div className="pmtask fade-step">
          <div className="pmtask-head"><span className="pmtask-tag">🗂 {tr3({ uz: "Topshiriq kartasi", ru: "Карточка задания" })}</span><span className="pmtask-id">US-UY</span></div>
          <div className="pmtask-rows">
            <div className="pmtask-row"><span className="pmtask-k">{tr3({ uz: "Kim uchun", ru: "Для кого" })}</span><span className="pmtask-v"><b className="pmtask-val" key={chosen} style={{ color: T.accent }}>{chosen}</b></span></div>
            <div className="pmtask-row"><span className="pmtask-k">{tr3({ uz: "Nechta hikoya", ru: "Сколько историй" })}</span><span className="pmtask-v"><b>{tr3({ uz: "2 ta yangi", ru: "2 новые" })}</b> <span className="pmtask-sub">{tr3({ uz: "(sinfda 3 ta, uyda 2 ta — jami 5 ta)", ru: "(3 в классе, 2 дома — всего 5)" })}</span></span></div>
            </div>
          <div className="pmtask-steps">
            <span className="pmtask-step"><i>1</i>{tr3({ uz: <><b>{chosen}</b> qanday odam ekanini bir gapda yozing</>, ru: <>Одной фразой опишите, что за человек <b>{chosen}</b></> })}</span>
            <span className="pmtask-step"><i>2</i>{tr3({ uz: "unga 2 ta to'liq hikoya yozing", ru: "напишите для него 2 полные истории" })}</span>
            <span className="pmtask-step"><i>3</i>{tr3({ uz: "5 hikoyadan eng muhim 3 tasini belgilang", ru: "из 5 историй отметьте 3 самые важные" })}</span>
          </div>
        </div> : <div className="frame-soft fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: "👆 Avval kim uchun yozishingizni tanlang — topshiriq-karta shunga moslashadi.", ru: "👆 Сначала выберите, для кого будете писать — карточка задания подстроится." })}</p></div>}
    </div>;
};
var ACHIEVEMENTS = {
...
function QuizArena({ live, onClose, startSolo }) {
  ...
  return <div className="qz-arena">
      <div className="qz-bg" aria-hidden="true">
        {QZ_BG_SHAPES.map((s, i) => <span key={i} className="qz-shp" style={{ left: `${s.l}%`, top: `${s.t}%`, fontSize: s.s, color: s.c, animationDuration: `${s.d}s`, animationDelay: `${s.dl}s` }}>{tr3(s.ch)}</span>)}
      </div>
      <QzFX />
...
var ScreenPodium = ({ screen, answers, achievements, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr3({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
    ...
    return <span key={id} className={`pod-solo-b ${got ? "got" : ""}`} title={a.name}>{got ? a.icon : "🔒"}</span>;
              ...
              <div className="pod-solo-sec">
                <span className="pod-solo-lbl">📒 {tr3({ uz: "Hikoya-daftar", ru: "Блокнот историй" })}</span>
                <div className="pod-solo-badges">
                  {soloSlots.map((ok, i) => <span key={i} className={`sboard-slot big ${ok ? "ok" : ""}`}>{ok ? "✓" : i + 1}</span>)}
                </div>
              </div>
            </div>
            <div className="frame-soft" style={{ maxWidth: 480 }}><p className="body" style={{ margin: 0 }}>{tr3({ uz: "Bu — shaxsiy natijangiz. Jonli darsda shu yerda butun guruh reytingi — 🥇🥈🥉 podium chiqadi.", ru: "Это ваш личный результат. На живом уроке здесь появится рейтинг всей группы — подиум 🥇🥈🥉." })}</p></div>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr3({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr3({ uz: <>Birinchi 3 <span className="italic" style={{ color: T.accent }}>hikoyangiz</span> tayyor.</>, ru: <>Ваши первые 3 <span className="italic" style={{ color: T.accent }}>истории</span> готовы.</> })}</h2>{
...
function PmUserStoryLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  return <LangContext.Provider value={lang}>
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={tr3({ uz: "User Story darsi", ru: "Урок про User Story" })} /> : <>
