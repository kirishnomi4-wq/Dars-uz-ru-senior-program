...
var TaomCard = ({ taom, outline, label, hidePrice, btnInteractive, btnActive, btnSeen, onBtnClick }) => <div style={{ position: "relative", background: "#fff", borderRadius: 12, padding: "9px 10px", boxShadow: "0 3px 12px -5px rgba(0,0,0,0.14)", border: outline ? `1.5px dashed ${T.accent}` : "1px solid rgba(0,0,0,0.05)" }}>
    ...
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 5 }}>
      {hidePrice ? <span style={{ fontSize: 11, fontWeight: 700, color: T.danger, fontStyle: "italic" }}>{tr({ uz: "narx?", ru: "цена?" })}</span> : <span className="price-rise" style={{ fontSize: 11.5, fontWeight: 800, color: T.ink }}>{taom.narx} {tr({ uz: "so'm", ru: "сум" })}</span>}
      <span
...
var Hero = ({ outline }) => <div style={{ position: "relative", borderRadius: 11, padding: "13px 14px", background: "linear-gradient(135deg,#FDE3D2,#F7CBB0)", color: T.ink, border: outline ? `1.5px dashed ${T.accent}` : "none" }}>
    ...
    <p style={{ fontSize: 11.5, color: T.ink2, margin: "0 0 8px" }}>{tr({ uz: "Issiq taomlar — bepul yetkazib berish", ru: "Горячая еда — бесплатная доставка" })}</p>
    <span style={{ display: "inline-block", fontSize: 11, fontWeight: 800, background: T.accent, color: "#fff", borderRadius: 7, padding: "5px 11px" }}>{tr({ uz: "Buyurtma berish", ru: "Заказать" })}</span>
  </div>;
...
var SiteFooter = ({ outline }) => <div style={{ position: "relative", borderRadius: 9, padding: "9px 12px", background: T.bg, color: T.ink2, boxShadow: `inset 0 0 0 1px ${T.line}`, border: outline ? `1.5px dashed ${T.accent}` : "none" }}>
    ...
    <span style={{ fontSize: 10.5 }}>{tr({ uz: "© Yetkaz · Aloqa · Ijtimoiy tarmoqlar", ru: "© Yetkaz · Контакты · Соцсети" })}</span>
  </div>;
...
var AgentBuild = ({ base, parts, planSteps, code, onDone, storedDone }) => {
  const [sel, setSel] = useState(storedDone ? new Set(parts.map((p) => p.id)) : /* @__PURE__ */ new Set());
  ...
  return <>
      ...
      <div className="ai-card">
        {phase === "compose" && <div className="ai-row"><QurarBot size={44} mood="idle" /><span className="ai-badge">{tr({ uz: "Qurar-bot", ru: "Строй-бот" })}</span><span className="ai-bubble">{tr({ uz: "Aniq buyruq bering — men aynan shuni quraman.", ru: "Дайте точную команду — я построю именно это." })}</span></div>}
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и число ✅/❌ скрыто — по кнопке «Открыть результат» всё появится сразу и у вас, и на экранах учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верных — класс не понял эту тему. Перед тем как идти дальше, рекомендуем короткое повторение.</> })}</p>
...
var ScreenPodium = ({ screen, answers, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
    ...
    return <div key={q} className="qstat-row">
              ...
              {live.mode === "mentor" && <p className="small" style={{ margin: "10px 0 0", color: T.ink2 }}>{tr({ uz: "⚠️ belgili savollar — sinf qiynalgan mavzular. Qayta tushuntiring.", ru: "Вопросы с ⚠️ — темы, где класс споткнулся. Рекомендуем объяснить ещё раз." })}</p>}
            </div>
          </>}
      </div>
    </Stage>;
...
var MentorPracticeStats = ({ live, screen, label }) => {
  ...
  return <div className="lp-mstats fade-up">
      <div className="card-lbl" style={{ color: T.blue }}>{tr(label || { uz: "👀 Kim bajardi", ru: "👀 Кто выполнил" })} — <b>{doers.length}</b>{total ? ` / ${total}` : ""}</div>
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Kirish · bitiruv", ru: "Вступление · выпуск" })} screen={screen} audioState={audio} scrollSignal={picked !== null} navContent={<NavNext optionalLive disabled={picked === null} label={tr({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Reja", ru: "План" })} screen={screen} mentorStatic audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive label={tr({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head">
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [chosen, setChosen] = useState(storedAnswer ? /* @__PURE__ */ new Set(["Bosh", "Taom", "Savat"]) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={tr({ uz: "1-qadam · sahifalar", ru: "Шаг 1 · страницы" })} screen={screen} audioState={audio} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "3 ta sahifani tanlang", ru: "Выберите 3 страницы" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(storedAnswer ? new Set(KEYS) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={tr({ uz: "2-qadam · bo'limlar", ru: "Шаг 2 · секции" })} screen={screen} audioState={audio} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: `${seen.size}/4 bo'limni toping`, ru: `Найдите секции: ${seen.size}/4` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen4 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr({ uz: "Mashq · 1-savol", ru: "Тренировка · вопрос 1" })}
  ...
  options={[tr({ uz: 'Bitta jumlada "hammasini qur" deyman', ru: "Скажу одной фразой «построй всё»" }), tr({ uz: "Bo'limlarga bo'lib, har biriga aniq prompt yozaman", ru: "Разобью на секции и напишу точный промпт для каждой" }), tr({ uz: "Hamma kodni o'zim qo'lda yozaman", ru: "Напишу весь код вручную" }), tr({ uz: "Avval eng chiroyli rangni tanlayman", ru: "Сначала выберу самый красивый цвет" })]}
  correctIdx={1}
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "3-qadam · takror → komponent", ru: "Шаг 3 · повтор → компонент" })} screen={screen} audioState={audio} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Javobni tanlang", ru: "Выберите ответ" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [sel, setSel] = useState(storedAnswer ? new Set(SPECS.map((s) => s.id)) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={tr({ uz: "5-qadam · aniq prompt", ru: "Шаг 5 · точный промпт" })} screen={screen} audioState={audio} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "3 ta aniqlik qo'shing", ru: "Добавьте 3 уточнения" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Qurish · agent", ru: "Сборка · агент" })} screen={screen} audioState={audio} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Agent bilan quring", ru: "Соберите с агентом" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
  ...
  />
          ...
          <Col>
            ...
            <Win title="Yetkaz — localhost:5173" minH={150}>
              {done ? <div className="fade-step"><HomePreview build /></div> : <p style={{ color: T.ink3, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13, textAlign: "center" }}>{tr({ uz: "Promptni yig'ib, agentga yuboring…", ru: "Соберите промпт и отправьте агенту…" })}</p>}
            </Win>
...
var Screen9 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr({ uz: "Mashq · 3-savol", ru: "Тренировка · вопрос 3" })}
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Debugging · tuzatish", ru: "Дебаггинг · исправление" })} screen={screen} audioState={audio} scrollSignal={fixed} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Xatoni tuzating", ru: "Исправьте ошибку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            <div className="ai-card fade-up delay-1">
              <div className="ai-row"><QurarBot size={44} mood={fixed ? "happy" : "confused"} shake={wrong === "redo"} /><span className="ai-badge">{tr({ uz: "Qurar-bot", ru: "Строй-бот" })}</span><span className="ai-bubble">{fixed ? tr({ uz: "Narxni qo'shdim — endi to'g'ri!", ru: "Добавил цену — теперь правильно!" }) : wrong === "redo" ? tr({ uz: "Hammasini qaytadan?! Faqat bittasi yetishmayapti…", ru: "Всё заново?! Не хватает же только одного…" }) : tr({ uz: "Bosh sahifani qurib bo'ldim!", ru: "Я закончил главную страницу!" })}</span></div>
              <div style={{ background: T.bg, borderRadius: 10, padding: 10 }}>
                <TaomGrid hidePrice={!fixed} />
              </div>
              {!fixed ? <p className="ai-prompt" style={{ color: T.danger, fontStyle: "normal", fontWeight: 600 }}>{tr({ uz: "⚠ Kartochkalarda narx ko'rinmayapti!", ru: "⚠ На карточках не видно цены!" })}</p> : <p className="ai-prompt" style={{ color: T.success, fontStyle: "normal", fontWeight: 600 }}>{tr({ uz: "✓ Narxlar qo'shildi — endi to'g'ri!", ru: "✓ Цены добавлены — теперь правильно!" })}</p>}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr({ uz: "Aniqlashtiruvchi promptni tanlang", ru: "Выберите уточняющий промпт" })}</p>
  ...
  return <Stage eyebrow={tr({ uz: "O'z saytingiz", ru: "Ваш сайт" })} screen={screen} audioState={audio} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : idea ? tr({ uz: "Bo'laklashni tugating", ru: "Завершите декомпозицию" }) : tr({ uz: "Saytingizni tanlang", ru: "Выберите свой сайт" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            {!idea ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Chapdan g'oya tanlang", ru: "Выберите идею слева" })}</p></div> : <div className="fade-step">
...
var Screen12 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr({ uz: "Mashq · 4-savol", ru: "Тренировка · вопрос 4" })}
  ...
  options={[tr({ uz: "Shundayligicha qoldiraman — ishlayapti", ru: "Оставлю как есть — работает же" }), tr({ uz: "Hammasini o'chiraman", ru: "Удалю всё" }), tr({ uz: "Bo'limlarga ajratishni so'rayman (Navbar, Hero, Footer...)", ru: "Попрошу разбить на секции (Navbar, Hero, Footer...)" }), tr({ uz: "Yana ko'proq kod qo'shaman", ru: "Добавлю ещё больше кода" })]}
  correctIdx={2}
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(storedAnswer ? new Set(JOURNEY.map((j) => j.id)) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={tr({ uz: "Bitiruv · to'liq yo'l", ru: "Выпуск · весь путь" })} screen={screen} audioState={audio} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Yo'lingizni ko'ring", ru: "Посмотрите свой путь" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} audioState={audio} scrollSignal={passed} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!passed} label={passed ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Komponentni e'lon qiling", ru: "Объявите компонент" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen15 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Modul tugadi 🎓", ru: "Модуль завершён 🎓" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: "React moduli tugadi", ru: "Модуль React завершён" })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Siz endi <span className="italic" style={{ color: T.accent }}>React dasturchisiz</span>.</>, ru: <>Теперь вы — <span className="italic" style={{ color: T.accent }}>React-разработчик</span>.</> })}</h2>{
...
function ReactBuildSiteLesson({ lang: langProp, onFinished }) {
