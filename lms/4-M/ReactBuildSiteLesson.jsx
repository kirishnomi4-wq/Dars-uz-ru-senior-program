...
//  Manba:  src/3-Modull/ReactBuildSiteLesson.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/3-Modull/ReactBuildSiteLesson.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/3-Modull/ReactBuildSiteLesson.jsx
...
var LESSON_META = { lessonId: "react-build-site-final-p4-v18", lessonTitle: { uz: "Praktika: Istalgan saytni qurish — bo'laklash + aniq prompt", ru: "Практика: Сборка любого сайта — декомпозиция + точный промпт" } };
var HW_TOKENS = [
...
var Stage = ({ children, eyebrow, screen, totalScreens = TOTAL_SCREENS, navContent, narrow, mentorStatic, scrollSignal, audioState }) => {
...
var TAOMS = [
  ...
  { id: 3, nom: { uz: "Lavash", ru: "Лаваш" }, emoji: "🌯", narx: "28 000", cat: "Fast food", color: "linear-gradient(135deg,#CFD8B2,#B0BC90)" },
  { id: 4, nom: { uz: "Coca-Cola 0.5", ru: "Coca-Cola 0.5" }, emoji: "🥤", narx: "12 000", cat: "Ichimlik", color: "linear-gradient(135deg,#BAC4EC,#98A6DC)" }
];
var CATS = [
  { uz: "🍕 Pitsa", ru: "🍕 Пицца" },
...
var OutlineLabel = ({ children }) => <span style={{ position: "absolute", top: -9, left: 8, background: T.accent, color: "#fff", fontSize: 9, fontWeight: 800, padding: "2px 7px", borderRadius: 5, fontFamily: "'JetBrains Mono',monospace", whiteSpace: "nowrap", zIndex: 2 }}>{children}</span>;
var TaomCard = ({ taom, outline, label, hidePrice, btnInteractive, btnActive, btnSeen, onBtnClick }) => <div style={{ position: "relative", background: "#fff", borderRadius: 12, padding: "9px 10px", boxShadow: "0 3px 12px -5px rgba(0,0,0,0.14)", border: outline ? `1.5px dashed ${T.accent}` : "1px solid rgba(0,0,0,0.05)" }}>
    ...
    <p style={{ fontFamily: "'Manrope',sans-serif", fontWeight: 800, fontSize: 12, color: T.ink, margin: "0 0 4px", lineHeight: 1.2 }}>{tr2(taom.nom)}</p>
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 5 }}>
      {hidePrice ? <span style={{ fontSize: 11, fontWeight: 700, color: T.danger, fontStyle: "italic" }}>{tr2({ uz: "narx?", ru: "цена?" })}</span> : <span className="price-rise" style={{ fontSize: 11.5, fontWeight: 800, color: T.ink }}>{taom.narx} {tr2({ uz: "so'm", ru: "сум" })}</span>}
      <span
  onClick={btnInteractive ? (e) => {
    e.stopPropagation();
    onBtnClick && onBtnClick();
  } : void 0}
  style={{ fontSize: 10, fontWeight: 700, color: T.accent, background: T.paper, boxShadow: `inset 0 0 0 1.5px ${T.accent}`, borderRadius: 7, padding: "4px 8px", ...btnInteractive ? { cursor: "pointer", ...btnActive ? { background: T.accent, color: "#fff" } : btnSeen ? { color: T.success, boxShadow: `inset 0 0 0 1.5px ${T.success}` } : {} } : {} }}
>{tr2({ uz: "Savatga", ru: "В корзину" })}</span>
    </div>
  </div>;
var SiteNavbar = ({ outline }) => <div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", borderRadius: 9, background: T.paper, color: T.ink, boxShadow: `inset 0 0 0 1px ${T.line}`, border: outline ? `1.5px dashed ${T.accent}` : "none" }}>
...
var Hero = ({ outline }) => <div style={{ position: "relative", borderRadius: 11, padding: "13px 14px", background: "linear-gradient(135deg,#FDE3D2,#F7CBB0)", color: T.ink, border: outline ? `1.5px dashed ${T.accent}` : "none" }}>
    {outline && <OutlineLabel>{"<Hero />"}</OutlineLabel>}
    <p style={{ fontFamily: "'Source Serif 4',serif", fontWeight: 600, fontSize: 15, margin: "0 0 3px" }}>{tr2({ uz: "30 daqiqada yetkazamiz 🚀", ru: "Доставим за 30 минут 🚀" })}</p>
    <p style={{ fontSize: 11.5, color: T.ink2, margin: "0 0 8px" }}>{tr2({ uz: "Issiq taomlar — bepul yetkazib berish", ru: "Горячая еда — бесплатная доставка" })}</p>
    <span style={{ display: "inline-block", fontSize: 11, fontWeight: 800, background: T.accent, color: "#fff", borderRadius: 7, padding: "5px 11px" }}>{tr2({ uz: "Buyurtma berish", ru: "Заказать" })}</span>
  </div>;
var CategoryRow = ({ outline }) => <div style={{ position: "relative", border: outline ? `1.5px dashed ${T.accent}` : "none", borderRadius: 9, padding: outline ? "9px 7px" : 0 }}>
    {outline && <OutlineLabel>{"<Categories />"}</OutlineLabel>}
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
      {CATS.map((c, i) => <span key={i} style={{ fontSize: 11, fontWeight: 700, color: T.ink, background: T.paper, borderRadius: 99, padding: "5px 10px", boxShadow: "0 3px 10px -5px rgba(0,0,0,0.14)" }}>{tr2(c)}</span>)}
    </div>
  </div>;
var SiteFooter = ({ outline }) => <div style={{ position: "relative", borderRadius: 9, padding: "9px 12px", background: T.bg, color: T.ink2, boxShadow: `inset 0 0 0 1px ${T.line}`, border: outline ? `1.5px dashed ${T.accent}` : "none" }}>
    {outline && <OutlineLabel>{"<Footer />"}</OutlineLabel>}
    <span style={{ fontSize: 10.5 }}>{tr2({ uz: "© Yetkaz · Aloqa · Ijtimoiy tarmoqlar", ru: "© Yetkaz · Контакты · Соцсети" })}</span>
  </div>;
var TaomGrid = ({ outline, hidePrice }) => <div style={{ position: "relative", border: outline ? `1.5px dashed ${T.accent}` : "none", borderRadius: 11, padding: outline ? "11px 8px 8px" : 0 }}>
    {outline && <OutlineLabel>{"<TaomList />"}</OutlineLabel>}
    <div className={outline ? "card-spread" : void 0} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}>
      {TAOMS.map((t) => <TaomCard key={t.id} taom={t} outline={outline} label={outline ? "<TaomCard />" : null} hidePrice={hidePrice} />)}
    </div>
  </div>;
var HomePreview = ({ outline, hidePrice, build }) => <div className={build ? "blk-build" : void 0} style={{ display: "flex", flexDirection: "column", gap: outline ? 12 : 8 }}>
...
var QurarBot = ({ mood = "idle", size = 46, shake = false }) => {
  const glow = mood === "happy" ? "#2BD97C" : mood === "confused" ? T.blue : mood === "work" ? "#FFC94D" : "#FFB59E";
  return <svg width={size} height={size} viewBox="0 0 64 64" aria-hidden="true" className={`qbot qbot-${mood}${shake ? " qbot-shake" : ""}`} style={{ flexShrink: 0 }}>
      {
    /* antenna */
  }
    ...
    /* mouth */
      ...
      {mood === "happy" ? <path d="M26 39 q6 5 12 0" fill="none" stroke={glow} strokeWidth="2.4" strokeLinecap="round" /> : mood === "confused" ? <path d="M26 40 q3 -3.5 6 0 t6 0" fill="none" stroke={glow} strokeWidth="2.2" strokeLinecap="round" /> : <rect x="27" y="38.5" width="10" height="2.4" rx="1.2" fill={glow} />}
      {
    /* status badge */
  }
...
var AgentBuild = ({ base, parts, planSteps, code, onDone, storedDone }) => {
  const [sel, setSel] = useState3(storedDone ? new Set(parts.map((p) => p.id)) : /* @__PURE__ */ new Set());
  const [phase, setPhase] = useState3(storedDone ? "done" : "compose");
  ...
  return <>
      <p className="flow-label">{tr2({ uz: "1. Promptni yig'ing — aniqlik qo'shing", ru: "1. Соберите промпт — добавьте точности" })}</p>
      <div className="ai-card">
        {phase === "compose" && <div className="ai-row"><QurarBot size={44} mood="idle" /><span className="ai-badge">{tr2({ uz: "Qurar-bot", ru: "Строй-бот" })}</span><span className="ai-bubble">{tr2({ uz: "Aniq buyruq bering — men aynan shuni quraman.", ru: "Дайте точную команду — я построю именно это." })}</span></div>}
        <div className="prompt-box">
          <span className="prompt-q">"</span>{base}{chosen.length ? <> — {chosen.map((p, i) => <span key={p.id}><span style={{ color: T.success, fontWeight: 700 }}>{p.label}</span>{i < chosen.length - 1 ? ", " : ""}</span>)}</> : <span style={{ color: T.ink3, fontStyle: "italic" }}> {tr2({ uz: "…aniqlik qo'shing", ru: "…добавьте точности" })}</span>}<span className="prompt-q">"</span>
        </div>
        {phase === "compose" && <>
            ...
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={!ready} onClick={send}>{ready ? tr2({ uz: "Agentga yuborish →", ru: "Отправить агенту →" }) : tr2({ uz: `Yana ${parts.length - sel.size} ta aniqlik qo'shing`, ru: `Добавьте ещё уточнений: ${parts.length - sel.size}` })}</button>
          </>}
        {phase !== "compose" && <>
            <div className="ai-row"><QurarBot size={44} mood={phase === "done" ? "happy" : "work"} /><span className="ai-badge">{tr2({ uz: "Qurar-bot", ru: "Строй-бот" })}</span><span className="ai-bubble">{phase === "planned" ? tr2({ uz: "Mana rejam — tasdiqlaysizmi?", ru: "Вот мой план — подтверждаете?" }) : phase === "building" ? tr2({ uz: "Yozyapman…", ru: "Пишу…" }) : tr2({ uz: "Bajardim — kodni tekshiring", ru: "Готово — проверьте код" })}</span></div>
            ...
            {phase === "planned" && <button className="btn fade-step" style={{ alignSelf: "flex-start" }} onClick={approve}>{tr2({ uz: "Rejani tasdiqlash", ru: "Подтвердить план" })}</button>}
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr2({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и число ✅/❌ скрыто — по кнопке «Открыть результат» всё появится сразу и у вас, и на экранах учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верных — класс не понял эту тему. Перед тем как идти дальше, рекомендуем короткое повторение.</> })}</p>
            ...
            {level === "few" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>Javob berganlar kam ({answered} ta) — foiz bo'yicha xulosa chiqarish qiyin. O'zingiz baholang:</>, ru: <>Ответивших мало ({answered}) — делать выводы по процентам трудно. Оцените сами:</> })}</p>
      ...
      {reveal && struggling && <p className="mstats-warn">{tr2({ uz: "⚠️ Ko'pchilik xato qildi — bu mavzu tushunarsiz bo'lgan ko'rinadi. Qayta tushuntiring.", ru: "⚠️ Большинство ошиблось — похоже, тема осталась непонятной. Рекомендуем объяснить ещё раз." })}</p>}
...
var BUILD_FLASHCARDS = [
  ...
  { front: { uz: "AI ishni chala qilsa, nima yozasiz?", ru: "Что вы пишете, если ИИ сделал работу сыро?" }, back: "Aniqlashtiruvchi prompt (follow-up)", note: { uz: "faqat kerakli joyni tuzatishni so'raysiz", ru: "просите починить только нужное место" } },
...
var ACHIEVEMENTS = {
  ...
  debugger: { icon: "🐞", name: "Nice Catch!", desc: { uz: "AI xatosini aniqlashtiruvchi prompt bilan tuzatdingiz", ru: "Вы исправили ошибку ИИ уточняющим промптом" } },
...
var ACH_TRIGGERS = { s9: "sniper", s10: "debugger", s14: "builder" };
...
var INLINE_KEYS = { s4: 1, s5b: 3, s9: 0, s12: 2, s14: -1, practice: -1 };
var ScreenPodium = ({ screen, answers, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
    ...
    return <div key={q} className="qstat-row">
              ...
              {live.mode === "mentor" && <p className="small" style={{ margin: "10px 0 0", color: T.ink2 }}>{tr2({ uz: "⚠️ belgili savollar — sinf qiynalgan mavzular. Qayta tushuntiring.", ru: "Вопросы с ⚠️ — темы, где класс споткнулся. Рекомендуем объяснить ещё раз." })}</p>}
            </div>
          </>}
      </div>
    </Stage>;
...
var QUIZ_BANK = [
  ...
  { q: { uz: "AI narxni unutdi — nima qilasiz?", ru: "ИИ забыл цену — что делаете?" }, opts: [{ uz: "Qoldiraman", ru: "Оставлю" }, { uz: "Hammasini qayta yozdiraman", ru: "Заставлю переписать всё" }, { uz: "Rangni o'zgartiraman", ru: "Поменяю цвет" }, { uz: "Aniq prompt: narxni qo'sh", ru: "Точный промпт: добавь цену" }], correct: 3 },
...
function QuizArena({ live, onClose, startSolo }) {
  ...
  return <div className="qz-arena">
      ...
      {phase === "lobby" && <div className="qz-view fade-step">
          <CsWordmark />
          <p className="qz-sub" style={{ marginTop: -4 }}>{tr2({ uz: "Tezroq to'g'ri bossangiz — ko'proq ball. Ketma-ket to'g'ri javoblar 🔥 bonus beradi!", ru: "Чем быстрее правильный ответ — тем больше баллов. Серия верных ответов даёт 🔥 бонус!" })}</p>
    ...
    return <div key={i} className={`qz-tile rv ${win ? "win" : "lose"} ${pickedThis ? "picked" : ""}`} style={{ background: QUIZ_COLORS[i] }}>
      ...
      {phase === "done" && <div className="qz-view fade-step">
          ...
          {solo ? <div className="qz-solo-res">
              ...
              <p className="qz-sub">{tr2({ uz: `ball · ${soloScore.ok}/${QUIZ_BANK.length} to'g'ri${soloScore.maxStreak >= 2 ? ` · eng uzun streak 🔥x${soloScore.maxStreak}` : ""}`, ru: `баллов · ${soloScore.ok}/${QUIZ_BANK.length} верно${soloScore.maxStreak >= 2 ? ` · лучшая серия 🔥x${soloScore.maxStreak}` : ""}` })}</p>
    ...
    return <div key={rank} className={`qz-pod-col p${rank + 1} ${b && b.id === live.playerId ? "me" : ""}`}>
              ...
              {isStudent && <button className="qz-btn" onClick={startPractice}>{tr2({ uz: "↻ Testni qayta ishlash — mashq (jadvalga yozilmaydi)", ru: "↻ Пройти тест ещё раз — тренировка (в таблицу не идёт)" })}</button>}
            </>}
...
var MentorPracticeStats = ({ live, screen, label }) => {
  ...
  useEffect4(() => {
    ...
    const tick = async () => {
      ...
      if (alive) t = setTimeout(tick, 3e3);
    };
    tick();
    return () => {
      alive = false;
  ...
  if (!on) return null;
  if (!data || data.players === null || data.players.length === 0) return null;
  ...
  return <div className="lp-mstats fade-up">
      <div className="card-lbl" style={{ color: T.blue }}>{tr2(label || { uz: "👀 Kim bajardi", ru: "👀 Кто выполнил" })} — <b>{doers.length}</b>{total ? ` / ${total}` : ""}</div>
...
var StudentPracticePulse = ({ live, screen }) => {
  ...
  useEffect4(() => {
    ...
    const tick = async () => {
      try {
        const [players, rows] = await Promise.all([livePlayers(live.pin), liveAnswers(live.pin, PRACTICE_BASE + screen)]);
        if (alive) setData({ total: players.length, done: new Set(rows.map((r) => r.player_id)).size });
      } catch {
      }
      if (alive) t = setTimeout(tick, 3e3);
    };
...
var ScreenBuildPractice = (props) => <ScreenLivePractice
  {...props}
  title={{ uz: "O'z saytingizni qurishni boshlang", ru: "Начните собирать свой сайт" }}
  statsLabel={{ uz: "🏗 Bo'lim qurganlar", ru: "🏗 Собрали секцию" }}
  task={{ uz: "Ekranda Qurar-bot bilan mashq qildingiz — endi haqiqiysi bilan: Antigravity o'sha ishni sizning loyihangizda bajaradi. Vite bilan yaratgan loyihangizni Antigravity'da oching va tanlagan saytingiz Bosh sahifasini bo'lak-bo'lak quring. Har bo'limga aniq prompt yozing, natijani tekshirib, aniqlashtiruvchi prompt bilan tuzating.", ru: "На экране вы тренировались со Строй-ботом — теперь с настоящим: Antigravity сделает ту же работу в вашем проекте. Откройте в Antigravity проект, созданный через Vite, и соберите главную страницу выбранного сайта блок за блоком. К каждой секции напишите точный промпт, проверьте результат и почините уточняющим промптом." }}
  checklist={[
    ...
    { uz: "Agentga yuboring, natijani tekshiring, aniqlashtiruvchi prompt bilan tuzating", ru: "Отправьте агенту, проверьте результат, почините уточняющим промптом" }
  ]}
/>;
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const [split, setSplit] = useState3(false);
  const [used, setUsed] = useState3(!!storedAnswer);
  ...
  const OPTS = [
    { id: "a", label: tr2({ uz: 'Bitta jumla: "menga ovqat sayti qur"', ru: "Одна фраза: «построй мне сайт с едой»" }) },
    { id: "b", label: tr2({ uz: "Bo'laklab, har qismni aniq aytib", ru: "Разбив на блоки и точно описав каждый" }) },
    { id: "c", label: tr2({ uz: "Hammasini o'zim qo'lda yozaman", ru: "Напишу всё вручную сам" }) }
  ];
  const pick = (v) => {
    if (picked !== null || !used) return;
    setPicked(v);
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish · bitiruv", ru: "Вступление · выпуск" })} screen={screen} audioState={audio} scrollSignal={picked !== null} navContent={<NavNext optionalLive disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        ...
        <Split>
          ...
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Bunday saytni AI bilan qanday quramiz?", ru: "Как построить такой сайт с ИИ?" })}</p>
            <div className="fade-up delay-3" style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {OPTS.map((o) => {
    const on = picked === o.id;
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null || !used} style={{ opacity: !used ? 0.55 : 1 }} onClick={() => pick(o.id)}>
                    <span className="radio">{on && <span className="radio-dot" />}</span>
                    <span>{o.label}</span>
                  </button>;
  })}
            </div>
            {!used && <p className="small" style={{ color: T.ink3, fontStyle: "italic", margin: 0 }}>{tr2({ uz: `Avval "Bo'laklarga ajratish"ni bosing ←`, ru: "Сначала нажмите «Разбить на блоки» ←" })}</p>}
            {picked !== null && <p className="hook-ack fade-step">{tr2({ uz: <>Aynan! Katta sayt — <b>kichik bloklar</b> yig'indisi: <span className="mono">{"<Navbar />"}</span>, <span className="mono">{"<Hero />"}</span>, <span className="mono">{"<TaomCard />"}</span>… Bugun ikkitasini o'rganamiz: <b>bo'laklash</b> va har bo'lakka <b>aniq prompt</b>.</>, ru: <>Именно! Большой сайт — сумма <b>маленьких блоков</b>: <span className="mono">{"<Navbar />"}</span>, <span className="mono">{"<Hero />"}</span>, <span className="mono">{"<TaomCard />"}</span>… Сегодня освоим два: <b>разбиение</b> и <b>точный промпт</b> к каждому блоку.</> })}</p>}
          </Col>
        </Split>
        </Zoomable>
      </div>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const STEPS = [
    { text: tr2({ uz: "Sahifalarga bo'lish", ru: "Разбить на страницы" }), tag: "Router" },
    { text: tr2({ uz: "Har sahifani bo'limlarga", ru: "Каждую страницу — на секции" }), tag: tr2({ uz: "komponentlar", ru: "компоненты" }) },
    { text: tr2({ uz: "Har bo'lakka aniq prompt", ru: "Точный промпт для каждого блока" }), tag: tr2({ uz: "Nima + Mazmun + Ma'lumot", ru: "Что + Содержание + Данные" }) },
    { text: tr2({ uz: "Qur → tekshir → tuzat", ru: "Построй → проверь → почини" }), tag: tr2({ uz: "AI bilan", ru: "вместе с ИИ" }) },
    { text: tr2({ uz: "O'z saytingizni boshlash", ru: "Начать свой сайт" }), tag: tr2({ uz: "bitiruv", ru: "выпускной проект" }) }
  ];
  const isNarrow = useIsMobile(768);
  const [showSteps, setShowSteps] = useState3(false);
  const PreviewBlock = <Col>
      <p className="flow-label">{tr2({ uz: "Dars oxirida — sizning saytingiz boshlanadi", ru: "В конце урока — стартует ваш сайт" })}</p>
      <div className="frame" style={{ padding: "16px 18px" }}>
        <p style={{ fontFamily: "'Source Serif 4',serif", fontWeight: 600, fontSize: 16, color: T.ink, margin: "0 0 10px" }}>{tr2({ uz: "Har sayt = bir xil bosqichlar:", ru: "Каждый сайт = одни и те же шаги:" })}</p>
        <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
          {[["📄", tr2({ uz: "Sahifalar", ru: "Страницы" }), tr2({ uz: "Bosh · detal · savat", ru: "Главная · деталь · корзина" })], ["🧩", tr2({ uz: "Bo'limlar", ru: "Секции" }), tr2({ uz: "Hero, grid, footer...", ru: "Hero, сетка, футер..." })], ["✍️", tr2({ uz: "Aniq prompt", ru: "Точный промпт" }), tr2({ uz: "AI aynan kerakligini quradi", ru: "ИИ строит именно то, что нужно" })]].map(([e, a, b]) => <div key={a} style={{ display: "flex", alignItems: "center", gap: 10 }}><span style={{ fontSize: 16 }}>{e}</span><span style={{ fontWeight: 700, fontSize: 13, color: T.ink, minWidth: 92 }}>{a}</span><span style={{ fontSize: 12, color: T.ink2 }}>{b}</span></div>)}
        </div>
      </div>
      <p className="mono small" style={{ color: T.accent, margin: 0 }}>{tr2({ uz: "→ bu usul bilan istalgan saytni qurasiz", ru: "→ этим способом вы соберёте любой сайт" })}</p>
    </Col>;
  ...
  return <Stage eyebrow={tr2({ uz: "Reja", ru: "План" })} screen={screen} mentorStatic audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive label={tr2({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head">
          <h2 className="title h-title fade-up">{tr2({ uz: <>Oxirgi mahorat: saytni <span className="italic" style={{ color: T.accent }}>bo'laklab</span>, aniq prompt bilan qurish.</>, ru: <>Последний навык: <span className="italic" style={{ color: T.accent }}>разбить</span> сайт и собрать его точным промптом.</> })}</h2>
        </div>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const PAGES = [
    { path: "/", page: "Bosh", icon: "🏠", desc: tr2({ uz: "barcha taomlar (katalog)", ru: "все блюда (каталог)" }) },
    { path: "/taom/:id", page: "Taom", icon: "🍽️", desc: tr2({ uz: 'bitta taom + "Savatga"', ru: "одно блюдо + «В корзину»" }) },
    { path: "/savat", page: "Savat", icon: "🛒", desc: tr2({ uz: "tanlangan taomlar + jami", ru: "выбранные блюда + итог" }) }
  ];
  const SCREENS = [
    { v: "Bosh", t: tr2({ uz: "Bosh", ru: "Главная" }) },
    { v: "Taom", t: tr2({ uz: "Taom", ru: "Блюдо" }) },
    { v: "Savat", t: tr2({ uz: "Savat", ru: "Корзина" }) },
    { v: "Sozlamalar", t: tr2({ uz: "Sozlamalar", ru: "Настройки" }) }
  ];
  const [chosen, setChosen] = useState3(storedAnswer ? /* @__PURE__ */ new Set(["Bosh", "Taom", "Savat"]) : /* @__PURE__ */ new Set());
  const [shake, setShake] = useState3(null);
  const timer = useRef3(null);
  const valid = new Set(PAGES.map((p) => p.page));
  const done = chosen.size >= 3 && [...chosen].every((c) => valid.has(c));
  useEffect4(() => () => clearTimeout(timer.current), []);
  const tap = (s) => {
    if (valid.has(s)) setChosen((prev) => {
      ...
      setShake(s);
      timer.current = setTimeout(() => setShake(null), 450);
    }
  };
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  return <Stage eyebrow={tr2({ uz: "1-qadam · sahifalar", ru: "Шаг 1 · страницы" })} screen={screen} audioState={audio} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "3 ta sahifani tanlang", ru: "Выберите 3 страницы" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Birinchi bo'lish: sayt <span className="italic" style={{ color: T.accent }}>nechta sahifa</span>?</>, ru: <>Первое деление: <span className="italic" style={{ color: T.accent }}>сколько страниц</span> у сайта?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Qaysi sahifalar kerak?", ru: "Какие страницы нужны?" })}</p>
            ...
            {shake && <p className="small fade-step" style={{ color: T.danger, fontStyle: "italic", margin: 0 }}>{tr2({ uz: `"Sozlamalar" hozir shart emas — avval ishlaydigan sayt, keyin qo'shimchalar.`, ru: "«Настройки» пока не нужны — сначала работающий сайт, потом дополнения." })}</p>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Sahifalar (Router) xaritasi", ru: "Карта страниц (Router)" })}</p>
            <div className="code-box fade-up delay-2" style={{ display: "flex", flexDirection: "column", gap: 6, padding: "13px 15px" }}>
              {PAGES.map((p) => <div key={p.path} style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: "clamp(11.5px,1.4vw,13px)", lineHeight: 1.6, opacity: chosen.has(p.page) ? 1 : 0.4, transition: "opacity 0.3s" }}>{chosen.has(p.page) ? "✓" : "○"} <span style={{ color: CODE.attr }}>{p.path}</span> <span style={{ color: CODE.comment }}>→ {p.desc}</span></div>)}
            </div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>3 ta sahifa — har biri bitta <span className="mono">{"<Route>"}</span>. Birinchi katta bo'lak tayyor. Endi bitta sahifani (Bosh) ichkari bo'laklaymiz.</>, ru: <>3 страницы — каждая по одному <span className="mono">{"<Route>"}</span>. Первый крупный блок готов. Теперь разберём одну страницу (Главную) изнутри.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const COMPS = {
    hero: { lbl: "<Hero />", desc: tr2({ uz: "Tepa banner — aksiya va asosiy tugma.", ru: "Верхний баннер — акция и главная кнопка." }) },
    cats: { lbl: "<Categories />", desc: tr2({ uz: "Kategoriyalar qatori — Pitsa, Burger...", ru: "Ряд категорий — Пицца, Бургеры..." }) },
    list: { lbl: "<TaomList />", desc: tr2({ uz: "Taomlar grid — ichida TaomCard takrorlanadi.", ru: "Сетка блюд — внутри повторяется TaomCard." }) },
    footer: { lbl: "<Footer />", desc: tr2({ uz: "Pastki qism — aloqa, havolalar.", ru: "Нижняя часть — контакты, ссылки." }) }
  };
  ...
  return <Stage eyebrow={tr2({ uz: "2-qadam · bo'limlar", ru: "Шаг 2 · секции" })} screen={screen} audioState={audio} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `${seen.size}/4 bo'limni toping`, ru: `Найдите секции: ${seen.size}/4` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bosh sahifa — <span className="italic" style={{ color: T.accent }}>qaysi bo'limlardan</span> tuzilgan?</>, ru: <>Из <span className="italic" style={{ color: T.accent }}>каких секций</span> состоит главная страница?</> })}</h2></div>
          ...
          <Col>
            <Win title="Yetkaz · Bosh — localhost:5173" minH={150}>
              <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                ...
                <div onClick={() => tap("cats")} className={!seen.has("cats") ? "tap-hint" : void 0} style={{ cursor: "pointer", borderRadius: 9, outline: outline("cats") }}><CategoryRow /></div>
                <div onClick={() => tap("list")} className={!seen.has("list") ? "tap-hint" : void 0} style={{ cursor: "pointer", borderRadius: 11, outline: outline("list") }}><TaomGrid /></div>
                <div onClick={() => tap("footer")} className={!seen.has("footer") ? "tap-hint" : void 0} style={{ cursor: "pointer", borderRadius: 9, outline: outline("footer") }}><SiteFooter /></div>
              </div>
            </Win>
          </Col>
          <Col>
            ...
            <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "Komponent daraxti", ru: "Дерево компонентов" })}</p>
...
var Screen4 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr2({ uz: "Mashq · 1-savol", ru: "Тренировка · вопрос 1" })}
  questionText={tr2({ uz: "Katta saytni AI bilan qurishning to'g'ri yo'li qaysi?", ru: "Каков правильный путь сборки большого сайта с ИИ?" })}
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Katta saytni AI bilan <span className="italic" style={{ color: T.accent }}>qanday</span> quramiz?</>, ru: <><span className="italic" style={{ color: T.accent }}>Как</span> мы строим большой сайт с ИИ?</> })}</h2></>}
  options={[tr2({ uz: 'Bitta jumlada "hammasini qur" deyman', ru: "Скажу одной фразой «построй всё»" }), tr2({ uz: "Bo'limlarga bo'lib, har biriga aniq prompt yozaman", ru: "Разобью на секции и напишу точный промпт для каждой" }), tr2({ uz: "Hamma kodni o'zim qo'lda yozaman", ru: "Напишу весь код вручную" }), tr2({ uz: "Avval eng chiroyli rangni tanlayman", ru: "Сначала выберу самый красивый цвет" })]}
  correctIdx={1}
  audioText="Katta saytni AI bilan qanday quramiz? To'g'ri javobni tanlang."
  explainCorrect={tr2({ uz: "To'g'ri! Avval bo'laklash (sahifalar + bo'limlar), keyin har bo'lakka aniq prompt. AI shunda taxmin qilmaydi — aynan kerakligini quradi.", ru: "Верно! Сначала декомпозиция (страницы + секции), потом точный промпт для каждого блока. Тогда ИИ не гадает — строит именно то, что нужно." })}
  explainWrong={{
    0: tr2({ uz: `Yo'q — "hammasini qur" juda noaniq. AI taxmin qiladi va chalkash natija beradi. Bo'laklab, aniq so'rang.`, ru: "Нет — «построй всё» слишком расплывчато. ИИ будет гадать и выдаст сумбурный результат. Разбейте и попросите точно." }),
    2: tr2({ uz: "Hammasini qo'lda yozish — sekin. AI tez yozadi; sizning ishingiz — bo'laklash, aniq prompt, tekshirish.", ru: "Писать всё вручную — медленно. ИИ пишет быстро; ваша работа — декомпозиция, точный промпт, проверка." }),
    3: tr2({ uz: "Rang — bezak. Avval struktura: sahifalar va bo'limlarga bo'lish kerak.", ru: "Цвет — украшение. Сначала структура: нужно разбить на страницы и секции." }),
    default: tr2({ uz: "To'g'ri yo'l: bo'laklash + har bo'lakka aniq prompt.", ru: "Правильный путь: декомпозиция + точный промпт для каждого блока." })
  }}
/>;
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [xray, setXray] = useState3(false);
  const [used, setUsed] = useState3(!!storedAnswer);
  const [picked, setPicked] = useState3(storedAnswer?.picked ?? null);
  const toggle = () => {
    setXray((v) => !v);
  ...
  const OPTS = [{ id: "a", label: tr2({ uz: "4 ta — har taomga alohida komponent", ru: "4 — отдельный компонент на каждое блюдо" }) }, { id: "b", label: tr2({ uz: "1 ta — qolgani faqat har xil ma'lumot (props)", ru: "1 — остальное лишь разные данные (props)" }) }];
  const pick = (v) => {
    if (picked !== null || !used) return;
    setPicked(v);
    onAnswer(screen, { stage: null, screenIdx: screen, picked: v, correct: true });
  };
  const done = picked !== null;
  return <Stage eyebrow={tr2({ uz: "3-qadam · takror → komponent", ru: "Шаг 3 · повтор → компонент" })} screen={screen} audioState={audio} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Javobni tanlang", ru: "Выберите ответ" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Gridda 4 ta taom — lekin <span className="italic" style={{ color: T.accent }}>nechta komponent</span>?</>, ru: <>В сетке 4 блюда — но <span className="italic" style={{ color: T.accent }}>сколько компонентов</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <button className={`chip ${xray ? "chip-on" : ""}`} onClick={toggle}>{tr2({ uz: "🔍 Rentgen", ru: "🔍 Рентген" })} {xray ? tr2({ uz: "yoqilgan", ru: "включён" }) : ""}</button>
              {xray && <span className="mono small fade-step" style={{ color: T.accent }}>{tr2({ uz: "hammasi bir xil!", ru: "все одинаковые!" })}</span>}
            </div>
          ...
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Sizningcha, nechta TaomCard komponenti yozilgan?", ru: "Как думаете, сколько компонентов TaomCard написано?" })}</p>
            <div className="fade-up delay-3" style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {OPTS.map((o) => {
    const on = picked === o.id;
...
var Screen5b = (props) => <QuestionScreen
  ...
  questionText={tr2({ uz: "Gridda bir xil karta 12 marta takrorlansa, nima qilasiz?", ru: "Одинаковая карточка повторяется в сетке 12 раз — что делаете?" })}
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "Mustahkamlash", ru: "Закрепление" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Bir xil karta <span className="italic" style={{ color: T.accent }}>12 marta takrorlansa</span>?</>, ru: <>Если карточка <span className="italic" style={{ color: T.accent }}>повторяется 12 раз</span>?</> })}</h2></>}
  options={[tr2({ uz: "12 marta bir xil kod ko'chiraman", ru: "Скопирую один и тот же код 12 раз" }), tr2({ uz: "12 ta alohida fayl yarataman", ru: "Создам 12 отдельных файлов" }), tr2({ uz: "Hech narsa — takror normal", ru: "Ничего — повтор это нормально" }), tr2({ uz: "Bitta komponent yasab, props + map bilan chizaman", ru: "Сделаю один компонент и нарисую через props + map" })]}
  correctIdx={3}
  audioText="Bir xil karta 12 marta takrorlansa nima qilasiz? To'g'ri javobni tanlang."
  explainCorrect={tr2({ uz: "To'g'ri! Takror = bitta komponent + props. map ro'yxatdagi har element uchun chizadi: {taomlar.map(t => <TaomCard taom={t} />)}. Bitta kod — 12 ta karta.", ru: "Верно! Повтор = один компонент + props. map рисует по элементу списка: {taomlar.map(t => <TaomCard taom={t} />)}. Один код — 12 карточек." })}
  explainWrong={{
    0: tr2({ uz: "Yo'q — ko'chirish yomon: narxni o'zgartirsangiz 12 joyni tuzatasiz. Bitta komponent + props.", ru: "Нет — копировать плохо: поменяете цену — придётся чинить 12 мест. Один компонент + props." }),
    1: tr2({ uz: "Yo'q — 12 ta fayl shart emas. Bitta komponent yetadi.", ru: "Нет — 12 файлов не нужно. Хватит одного компонента." }),
    2: tr2({ uz: "Takror — belgi: bu yerda komponent kerak. Bitta yasab, props bilan ishlating.", ru: "Повтор — сигнал: здесь нужен компонент. Сделайте один и используйте с props." }),
    default: tr2({ uz: "Takror → bitta komponent + props + map.", ru: "Повтор → один компонент + props + map." })
  }}
/>;
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const ITEMS = [
    { q: tr2({ uz: "Taomlar ro'yxati", ru: "Список блюд" }), srcId: "api", hint: tr2({ uz: "serverdan keladi", ru: "приходит с сервера" }) },
    { q: tr2({ uz: "Taom nomi va narxi", ru: "Название и цена блюда" }), srcId: "props", hint: tr2({ uz: "sahifadan TaomCard'ga", ru: "со страницы в TaomCard" }) },
    { q: tr2({ uz: "Savatdagi soni (bosilganda ortadi)", ru: "Счётчик корзины (растёт по клику)" }), srcId: "state", hint: tr2({ uz: "komponent xotirasi", ru: "память компонента" }) }
  ];
  const SRC = [{ id: "api", label: "API (fetch)" }, { id: "props", label: "Props" }, { id: "state", label: "State" }];
  const [taskIdx, setTaskIdx] = useState3(storedAnswer ? ITEMS.length : 0);
  const [shake, setShake] = useState3(null);
  const timer = useRef3(null);
  ...
  const tap = (id) => {
    if (done) return;
    if (id === cur.srcId) setTaskIdx((t) => t + 1);
    else {
      clearTimeout(timer.current);
      setShake(id);
      timer.current = setTimeout(() => setShake(null), 450);
  ...
  return <Stage eyebrow={tr2({ uz: "4-qadam · ma'lumot", ru: "Шаг 4 · данные" })} screen={screen} audioState={audio} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Ma'lumotni joylang (${Math.min(taskIdx, ITEMS.length)}/3)`, ru: `Разместите данные (${Math.min(taskIdx, ITEMS.length)}/3)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Har ma'lumot <span className="italic" style={{ color: T.accent }}>qayerda</span> yashaydi?</>, ru: <><span className="italic" style={{ color: T.accent }}>Где</span> живут какие данные?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Ma'lumotlar", ru: "Данные" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {ITEMS.map((it, i) => {
    const matched = i < taskIdx;
    const activeRow = !done && i === taskIdx;
    return <div key={it.q} className="routerow" style={{ boxShadow: activeRow ? `inset 0 0 0 1.5px ${T.accent}` : matched ? `inset 0 0 0 1.5px ${T.success}` : `0 4px 12px -6px rgba(${T.shadowBase},0.14)`, background: matched ? T.successSoft : T.paper }}>
          ...
          <Col>
            {!done ? <>
                <div className="sk-info" key={taskIdx}><p className="body" style={{ margin: 0, color: T.ink }}><b style={{ color: T.accent }}>{cur.q}</b> {tr2({ uz: "— qaysi uyda yashaydi?", ru: "— в каком доме живёт?" })} <span style={{ color: T.ink3 }}>({cur.hint})</span></p></div>
                <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "Uyni tanlang", ru: "Выберите дом" })}</p>
                <div className="fade-up delay-1" style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {SRC.map((s) => <button key={s.id} className={`gchip ${shake === s.id ? "shake" : ""}`} onClick={() => tap(s.id)} style={{ padding: "11px 15px" }}>{s.label}</button>)}
                </div>
              </> : <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Tayyor! <b>API</b> — serverdagi taomlar ro'yxati · <b>props</b> — TaomCard'ga uzatiladigan nom/narx · <b>state</b> — savatdagi soni (bosilganda o'zgaradi). Endi promptni aniq yozishga tayyorsiz.</>, ru: <>Готово! <b>API</b> — список блюд на сервере · <b>props</b> — название/цена, передаваемые в TaomCard · <b>state</b> — счётчик корзины (меняется по клику). Теперь вы готовы писать точный промпт.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const SPECS = [
    { id: "sec", tag: tr2({ uz: "① Qaysi bo'limlar", ru: "① Какие секции" }), add: tr2({ uz: "Hero (aksiya banner), Kategoriyalar, Mashhur taomlar grid, Footer", ru: "Hero (баннер с акцией), Категории, сетка популярных блюд, Footer" }) },
    { id: "what", tag: tr2({ uz: "② Har bo'lim nima ko'rsatadi", ru: "② Что показывает каждая" }), add: tr2({ uz: `grid'da 8 ta TaomCard — rasm, nom, narx, "Savatga" tugma`, ru: "в сетке 8 TaomCard — картинка, название, цена, кнопка «В корзину»" }) },
    { id: "data", tag: tr2({ uz: "③ Ma'lumot / uslub", ru: "③ Данные / стиль" }), add: tr2({ uz: "taomlar API'dan kelsin, issiq apelsin-qizil uslub", ru: "блюда пусть приходят из API, тёплый оранжево-красный стиль" }) }
  ];
  const [sel, setSel] = useState3(storedAnswer ? new Set(SPECS.map((s) => s.id)) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={tr2({ uz: "5-qadam · aniq prompt", ru: "Шаг 5 · точный промпт" })} screen={screen} audioState={audio} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "3 ta aniqlik qo'shing", ru: "Добавьте 3 уточнения" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>"Bosh sahifa qur" yetarli emas — <span className="italic" style={{ color: T.accent }}>aniq</span> prompt yozamiz.</>, ru: <>«Построй главную» — мало. Пишем <span className="italic" style={{ color: T.accent }}>точный</span> промпт.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Promptni aniqlang — qo'shimchalarni bosing", ru: "Уточните промпт — нажимайте дополнения" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <div className="prompt-box fade-up delay-2">
              <span className="prompt-q">"</span>{tr2({ uz: "Bosh sahifani qur", ru: "Построй главную страницу" })}{chosen.length ? <>: {chosen.map((s, i) => <span key={s.id}><span style={{ color: T.success, fontWeight: 700 }}>{s.add}</span>{i < chosen.length - 1 ? "; " : ""}</span>)}</> : <span style={{ color: T.ink3, fontStyle: "italic" }}> {tr2({ uz: "…aniqlik qo'shing", ru: "…добавьте точности" })}</span>}<span className="prompt-q">"</span>
            </div>
          </Col>
          <Col>
            <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "Prompt aniqligi", ru: "Точность промпта" })}</p>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Qurish · agent", ru: "Сборка · агент" })} screen={screen} audioState={audio} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Agent bilan quring", ru: "Соберите с агентом" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Aniq prompt tayyor — endi <span className="italic" style={{ color: T.accent }}>agentga</span> yuboramiz.</>, ru: <>Точный промпт готов — теперь отправим его <span className="italic" style={{ color: T.accent }}>агенту</span>.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <AgentBuild
    base={tr2({ uz: "Bosh sahifani qur", ru: "Построй главную страницу" })}
    parts={[{ id: "sec", label: tr2({ uz: "Hero, Kategoriyalar, Taomlar grid, Footer bo'limlari bilan", ru: "с секциями Hero, Категории, сетка блюд, Footer" }) }, { id: "card", label: tr2({ uz: 'gridda takrorlanuvchi TaomCard (nom, narx, "Savatga")', ru: "в сетке повторяющийся TaomCard (название, цена, «В корзину»)" }) }, { id: "data", label: tr2({ uz: "taomlar API'dan, map bilan chizilsin", ru: "блюда из API, отрисовать через map" }) }]}
    planSteps={[tr2({ uz: "Bo'limlar: Hero, Categories, TaomList, Footer", ru: "Секции: Hero, Categories, TaomList, Footer" }), tr2({ uz: "TaomCard komponenti: function TaomCard(props)", ru: "Компонент TaomCard: function TaomCard(props)" }), tr2({ uz: "Taomlarni API'dan olib, map bilan chizaman", ru: "Возьму блюда из API и нарисую через map" })]}
  ...
  />
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "Natija — bosh sahifa", ru: "Результат — главная страница" })}</p>
            <Win title="Yetkaz — localhost:5173" minH={150}>
              {done ? <div className="fade-step"><HomePreview build /></div> : <p style={{ color: T.ink3, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13, textAlign: "center" }}>{tr2({ uz: "Promptni yig'ib, agentga yuboring…", ru: "Соберите промпт и отправьте агенту…" })}</p>}
            </Win>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Bosh sahifa qurildi! Hero + Kategoriyalar + TaomCard'li grid + Footer — aniq prompt aynan kerakligini berdi. Endi tekshiramiz: hammasi joyidami?", ru: "Главная построена! Hero + Категории + сетка с TaomCard + Footer — точный промпт дал именно то, что нужно. Теперь проверим: всё ли на месте?" })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen9 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr2({ uz: "Mashq · 3-savol", ru: "Тренировка · вопрос 3" })}
  questionText={tr2({ uz: "Qaysi prompt AI'ga eng yaxshi natija beradi?", ru: "Какой промпт даст ИИ лучший результат?" })}
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Qaysi <span className="italic" style={{ color: T.accent }}>prompt</span> eng yaxshi natija beradi?</>, ru: <>Какой <span className="italic" style={{ color: T.accent }}>промпт</span> даст лучший результат?</> })}</h2></>}
  options={[tr2({ uz: `"Bosh sahifa: Hero (aksiya), Kategoriyalar, 8 ta TaomCard'li grid (nom+narx+Savatga), Footer"`, ru: "«Главная: Hero (акция), Категории, сетка из 8 TaomCard (название+цена+В корзину), Footer»" }), tr2({ uz: '"Chiroyli ovqat sayti qur"', ru: "«Построй красивый сайт с едой»" }), tr2({ uz: '"Sayt qur"', ru: "«Построй сайт»" }), tr2({ uz: '"Menga kod yoz"', ru: "«Напиши мне код»" })]}
  correctIdx={0}
  audioText="Qaysi prompt AI'ga eng yaxshi natija beradi? To'g'ri javobni tanlang."
  explainCorrect={tr2({ uz: "To'g'ri! Aniq prompt = qaysi bo'limlar + har bo'lim nima ko'rsatadi. AI taxmin qilmaydi — aynan shuni quradi.", ru: "Верно! Точный промпт = какие секции + что показывает каждая. ИИ не гадает — строит именно это." })}
  explainWrong={{
    1: tr2({ uz: `"Chiroyli" — noaniq. Qaysi bo'limlar? Nima ko'rsatadi? AI taxmin qiladi. Aniq ayting.`, ru: "«Красивый» — расплывчато. Какие секции? Что показывают? ИИ будет гадать. Скажите точно." }),
    2: tr2({ uz: "Juda umumiy — AI nima qurishni bilmaydi. Bo'limlar va mazmunni sanang.", ru: "Слишком общо — ИИ не знает, что строить. Перечислите секции и содержание." }),
    3: tr2({ uz: "Qanday kod? Nima haqida? AI uchun aniq bo'lim va mazmun kerak.", ru: "Какой код? О чём? ИИ нужны точные секции и содержание." }),
    default: tr2({ uz: "Eng yaxshi prompt: bo'limlar + har bo'lim mazmuni aniq sanalgan.", ru: "Лучший промпт: точно перечислены секции и содержание каждой." })
  }}
/>;
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const audio = useAudio([{ id: "s10", text: `Agent bosh sahifani qurdi. Lekin diqqat bilan qarang — bir narsa yetishmayapti! AI har doim ham mukammal qilmaydi. Siz nazoratchisiz: xatoni topib, aniqlashtiruvchi prompt berasiz.`, trigger: "on_mount", waits_for: null }]);
  const FIXES = [
    { id: "price", label: tr2({ uz: `Har TaomCard'ga narxni qo'sh (props.narx + " so'm")`, ru: 'Добавь цену в каждый TaomCard (props.narx + " сум")' }), ok: true },
    { id: "redo", label: tr2({ uz: "Hammasini o'chirib, qaytadan yozdir", ru: "Удали всё и напиши заново" }), ok: false },
    { id: "color", label: tr2({ uz: "Kartochka rangini o'zgartir", ru: "Поменяй цвет карточки" }), ok: false }
  ];
  ...
  const choose = (f) => {
    ...
    if (f.ok) setFixed(true);
      ...
      clearTimeout(timer.current);
      setWrong(f.id);
      timer.current = setTimeout(() => setWrong(null), 2600);
    }
  };
  return <Stage eyebrow={tr2({ uz: "Debugging · tuzatish", ru: "Дебаггинг · исправление" })} screen={screen} audioState={audio} scrollSignal={fixed} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Xatoni tuzating", ru: "Исправьте ошибку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>AI qurdi — siz <span className="italic" style={{ color: T.accent }}>tekshirasiz</span> va tuzatasiz.</>, ru: <>ИИ построил — вы <span className="italic" style={{ color: T.accent }}>проверяете</span> и чините.</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Agent bosh sahifani qurdi. Lekin diqqat bilan qarang — <b style={{ color: T.ink }}>bir narsa yetishmayapti</b>! AI har doim ham mukammal qilmaydi. Siz — NAZORATCHI: xatoni topib, <b style={{ color: T.ink }}>aniqlashtiruvchi prompt (follow-up)</b> berasiz.</>, ru: <>Агент построил главную. Но приглядитесь — <b style={{ color: T.ink }}>кое-чего не хватает</b>! ИИ не всегда идеален. Вы — КОНТРОЛЁР: найдите ошибку и дайте <b style={{ color: T.ink }}>исправляющий (follow-up) промпт</b>.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <div className="ai-card fade-up delay-1">
              <div className="ai-row"><QurarBot size={44} mood={fixed ? "happy" : "confused"} shake={wrong === "redo"} /><span className="ai-badge">{tr2({ uz: "Qurar-bot", ru: "Строй-бот" })}</span><span className="ai-bubble">{fixed ? tr2({ uz: "Narxni qo'shdim — endi to'g'ri!", ru: "Добавил цену — теперь правильно!" }) : wrong === "redo" ? tr2({ uz: "Hammasini qaytadan?! Faqat bittasi yetishmayapti…", ru: "Всё заново?! Не хватает же только одного…" }) : tr2({ uz: "Bosh sahifani qurib bo'ldim!", ru: "Я закончил главную страницу!" })}</span></div>
              <div style={{ background: T.bg, borderRadius: 10, padding: 10 }}>
                <TaomGrid hidePrice={!fixed} />
              </div>
              {!fixed ? <p className="ai-prompt" style={{ color: T.danger, fontStyle: "normal", fontWeight: 600 }}>{tr2({ uz: "⚠ Kartochkalarda narx ko'rinmayapti!", ru: "⚠ На карточках не видно цены!" })}</p> : <p className="ai-prompt" style={{ color: T.success, fontStyle: "normal", fontWeight: 600 }}>{tr2({ uz: "✓ Narxlar qo'shildi — endi to'g'ri!", ru: "✓ Цены добавлены — теперь правильно!" })}</p>}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Aniqlashtiruvchi promptni tanlang", ru: "Выберите уточняющий промпт" })}</p>
            <div className="fade-up delay-2" style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {FIXES.map((f) => <button key={f.id} className={`hook-option ${fixed && f.ok ? "on" : ""} ${wrong === f.id ? "shake" : ""}`} disabled={fixed} onClick={() => choose(f)}>
                  <span style={{ fontSize: 15 }}>{fixed && f.ok ? "✓" : "✍️"}</span><span>{f.label}</span>
                </button>)}
            </div>
            {wrong && <p className="small fade-step" style={{ color: T.danger, fontStyle: "italic", margin: 0 }}>{wrong === "redo" ? tr2({ uz: "Hammasini qaytadan yozish shart emas — faqat bitta narsa (narx) yetishmayapti. Aniq, kichik prompt bering.", ru: "Переписывать всё не нужно — не хватает лишь одного (цены). Дайте точный, маленький промпт." }) : tr2({ uz: "Rang muammo emas — narx yetishmayapti. Aniq nimani tuzatishni ayting.", ru: "Цвет не проблема — не хватает цены. Скажите точно, что чинить." })}</p>}
            {fixed && <div className="takeaway fade-step"><div className="ta-bulb">🛠️</div><p className="ta-h">{tr2({ uz: "Topdingiz va aniq prompt bilan tuzatdingiz!", ru: "Вы нашли ошибку и починили точным промптом!" })}</p><p className="ta-sub">{tr2({ uz: "AI tez yozadi, siz tekshirib aniq tuzatasiz — yaxshi jamoa", ru: "ИИ быстро пишет, вы проверяете и точно чините — отличная команда" })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const IDEAS = [
    { id: "recipe", e: "🍳", name: tr2({ uz: "Retseptlar", ru: "Рецепты" }), card: "RetseptCard", pages: tr2({ uz: "Bosh · retsept · sevimlilar", ru: "Главная · рецепт · избранное" }), first: tr2({ uz: "Hero + retseptlar grid (RetseptCard) + Footer", ru: "Hero + сетка рецептов (RetseptCard) + Footer" }) },
    { id: "game", e: "🎮", name: tr2({ uz: "O'yinlar", ru: "Игры" }), card: "OyinCard", pages: tr2({ uz: "Bosh · o'yin · top ro'yxat", ru: "Главная · игра · топ-список" }), first: tr2({ uz: "Hero + o'yinlar grid (OyinCard) + Footer", ru: "Hero + сетка игр (OyinCard) + Footer" }) },
    { id: "book", e: "📚", name: tr2({ uz: "Kutubxona", ru: "Библиотека" }), card: "KitobCard", pages: tr2({ uz: "Bosh · kitob · o'qiganlarim", ru: "Главная · книга · прочитанное" }), first: tr2({ uz: "Hero + kitoblar grid (KitobCard) + Footer", ru: "Hero + сетка книг (KitobCard) + Footer" }) },
    { id: "club", e: "⚽", name: tr2({ uz: "Sport klub", ru: "Спортклуб" }), card: "MashgulotCard", pages: tr2({ uz: "Bosh · mashg'ulot · a'zolik", ru: "Главная · тренировка · членство" }), first: tr2({ uz: "Hero + mashg'ulotlar grid (MashgulotCard) + Footer", ru: "Hero + сетка тренировок (MashgulotCard) + Footer" }) }
  ];
  const [idea, setIdea] = useState3(storedAnswer ? IDEAS[0] : null);
  const [step, setStep] = useState3(storedAnswer ? 3 : 0);
  const done = step >= 3;
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  ...
  return <Stage eyebrow={tr2({ uz: "O'z saytingiz", ru: "Ваш сайт" })} screen={screen} audioState={audio} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : idea ? tr2({ uz: "Bo'laklashni tugating", ru: "Завершите декомпозицию" }) : tr2({ uz: "Saytingizni tanlang", ru: "Выберите свой сайт" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>o'z saytingizni</span> tanlab, bo'laklang.</>, ru: <>Теперь выберите <span className="italic" style={{ color: T.accent }}>свой сайт</span> и разбейте его.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Bitta g'oyani tanlang", ru: "Выберите одну идею" })}</p>
          ...
          <Col>
            {!idea ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Chapdan g'oya tanlang", ru: "Выберите идею слева" })}</p></div> : <div className="fade-step">
                <p className="flow-label" style={{ marginBottom: 8 }}>{tr2({ uz: `"${idea.name}" bo'laklari`, ru: `Блоки сайта «${idea.name}»` })}</p>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <div className="dstep" style={{ opacity: step >= 1 ? 1 : 0.4 }}>
                    <span className="dstep-n">📄</span><div><b style={{ fontSize: 12.5 }}>{tr2({ uz: "Sahifalar", ru: "Страницы" })}</b><p style={{ fontSize: 11.5, color: T.ink2, margin: 0 }}>{idea.pages}</p></div>
                  </div>
                  {step >= 2 && <div className="dstep el-in"><span className="dstep-n">🧩</span><div><b style={{ fontSize: 12.5 }}>{tr2({ uz: "Takrorlanuvchi komponent", ru: "Повторяющийся компонент" })}</b><p style={{ fontSize: 11.5, color: T.ink2, margin: 0 }}><span className="mono">{`<${idea.card} />`}</span> {tr2({ uz: "— gridda takrorlanadi", ru: "— повторяется в сетке" })}</p></div></div>}
                  {step >= 3 && <div className="dstep el-in" style={{ boxShadow: `inset 0 0 0 1.5px ${T.success}` }}><span className="dstep-n">✍️</span><div><b style={{ fontSize: 12.5, color: T.success }}>{tr2({ uz: "Birinchi aniq prompt", ru: "Первый точный промпт" })}</b><p style={{ fontSize: 11.5, color: T.ink2, margin: 0 }}>{tr2({ uz: `"Bosh sahifa: ${idea.first}"`, ru: `«Главная страница: ${idea.first}»` })}</p></div></div>}
                  {step === 1 && <button className="btn-soft" style={{ alignSelf: "flex-start" }} onClick={() => setStep(2)}>{tr2({ uz: "Keyingi: takror komponent →", ru: "Дальше: повторяющийся компонент →" })}</button>}
                  {step === 2 && <button className="btn-soft" style={{ alignSelf: "flex-start" }} onClick={() => setStep(3)}>{tr2({ uz: "Keyingi: birinchi prompt →", ru: "Дальше: первый промпт →" })}</button>}
                </div>
                {done && <div className="frame-success fade-step" style={{ marginTop: 10 }}><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Ko'rdingizmi? "{idea.name}" ham xuddi "Yetkaz" kabi bo'laklandi: sahifalar + <span className="mono">{idea.card}</span> + birinchi aniq prompt. <b>Usul o'sha</b> — istalgan saytga ishlaydi.</>, ru: <>Видите? «{idea.name}» разобрался так же, как «Yetkaz»: страницы + <span className="mono">{idea.card}</span> + первый точный промпт. <b>Способ тот же</b> — работает для любого сайта.</> })}</p></div>}
              </div>}
          </Col>
        </div>
        </Zoomable>
...
var Screen12 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr2({ uz: "Mashq · 4-savol", ru: "Тренировка · вопрос 4" })}
  questionText={tr2({ uz: "AI hamma kodni bitta ulkan komponentga yozdi. Nima qilasiz?", ru: "ИИ написал весь код в один огромный компонент. Что делаете?" })}
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>AI hamma kodni <span className="italic" style={{ color: T.accent }}>bitta ulkan komponentga</span> yozdi — nima qilasiz?</>, ru: <>ИИ написал весь код в <span className="italic" style={{ color: T.accent }}>один огромный компонент</span> — что делаете?</> })}</h2></>}
  options={[tr2({ uz: "Shundayligicha qoldiraman — ishlayapti", ru: "Оставлю как есть — работает же" }), tr2({ uz: "Hammasini o'chiraman", ru: "Удалю всё" }), tr2({ uz: "Bo'limlarga ajratishni so'rayman (Navbar, Hero, Footer...)", ru: "Попрошу разбить на секции (Navbar, Hero, Footer...)" }), tr2({ uz: "Yana ko'proq kod qo'shaman", ru: "Добавлю ещё больше кода" })]}
  correctIdx={2}
  audioText="AI hamma kodni bitta ulkan komponentga yozdi — nima qilasiz? To'g'ri javobni tanlang."
  explainCorrect={tr2({ uz: "To'g'ri! Bitta ulkan komponent (monolit) — yomon: o'zgartirish, qayta ishlatish qiyin. AI'dan uni kichik komponentlarga bo'lishni so'raysiz — har biri alohida, oson.", ru: "Верно! Один огромный компонент (монолит) — плохо: трудно менять и переиспользовать. Попросите ИИ разбить его на маленькие компоненты — каждый отдельно, легко." })}
  explainWrong={{
    0: tr2({ uz: "Ishlasa ham — monolitni keyin o'zgartirish azob. Bo'laklashni so'rang.", ru: "Пусть работает — но менять монолит потом мучение. Попросите разбить." }),
    1: tr2({ uz: "Yo'q — o'chirmaymiz, bo'laklaymiz: bitta katta → bir nechta kichik komponent.", ru: "Нет — не удаляем, а разбиваем: один большой → несколько маленьких компонентов." }),
    3: tr2({ uz: "Aksincha — kod ko'paymaydi, lekin tartibli bo'laklarga ajraladi.", ru: "Наоборот — кода больше не станет, но он разложится на аккуратные блоки." }),
    default: tr2({ uz: "Monolitni kichik komponentlarga bo'lishni so'rash — to'g'ri yo'l.", ru: "Попросить разбить монолит на маленькие компоненты — правильный путь." })
  }}
/>;
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const JOURNEY = [
    { id: "comp", e: "🧩", t: tr2({ uz: "Komponent + props", ru: "Компонент + props" }), d: tr2({ uz: "qayta ishlatiladigan bloklar", ru: "переиспользуемые блоки" }) },
    { id: "state", e: "💾", t: "State + Effect", d: tr2({ uz: "jonli, o'zgaradigan ilova", ru: "живое, изменяющееся приложение" }) },
    { id: "api", e: "🌐", t: "API — CRUD", d: tr2({ uz: "serverdan olish va saqlash", ru: "получать и сохранять на сервере" }) },
    { id: "router", e: "🧭", t: "Router", d: tr2({ uz: "ko'p sahifali sayt", ru: "многостраничный сайт" }) },
    { id: "prompt", e: "✍️", t: tr2({ uz: "Aniq prompt", ru: "Точный промпт" }), d: tr2({ uz: "bo'limlar + mazmun + ma'lumot", ru: "секции + содержание + данные" }) },
    { id: "debug", e: "🔧", t: tr2({ uz: "Tekshir → tuzat", ru: "Проверь → почини" }), d: tr2({ uz: "aniqlashtiruvchi prompt bilan tuzatish", ru: "починка уточняющим промптом" }) }
  ];
  const [seen, setSeen] = useState3(storedAnswer ? new Set(JOURNEY.map((j) => j.id)) : /* @__PURE__ */ new Set());
  const done = seen.size >= JOURNEY.length;
  const tap = (t) => setSeen((prev) => {
    const s = new Set(prev);
    s.add(t);
    return s;
  });
  ...
  return <Stage eyebrow={tr2({ uz: "Bitiruv · to'liq yo'l", ru: "Выпуск · весь путь" })} screen={screen} audioState={audio} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Yo'lingizni ko'ring", ru: "Посмотрите свой путь" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Qarang — siz <span className="italic" style={{ color: T.accent }}>qancha yo'l</span> bosib o'tdingiz.</>, ru: <>Посмотрите, <span className="italic" style={{ color: T.accent }}>какой путь</span> вы прошли.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {JOURNEY.map((j) => <button key={j.id} className={`vcard${seen.has(j.id) ? "" : " tap-hint"}`} onClick={() => tap(j.id)} style={{ boxShadow: seen.has(j.id) ? `inset 0 0 0 1.5px ${T.success}` : void 0 }}>
          ...
          <Col>
            <div className="frame" data-dark-ok="ma'no-cho'qqisi" style={{ background: T.ink, color: "#fff", padding: "20px 22px" }}>
              <p style={{ fontFamily: "'Source Serif 4',serif", fontWeight: 600, fontSize: "clamp(18px,2.5vw,22px)", margin: "0 0 10px", color: "#fff" }}>{tr2({ uz: "Sizning formulangiz:", ru: "Ваша формула:" })}</p>
              <p style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 13.5, lineHeight: 1.9, color: "#fff", margin: 0 }}>
                {tr2({ uz: <>g'oya → <span style={{ color: "#FFD380" }}>bo'laklash</span> → <span style={{ color: "#7DD181" }}>aniq prompt</span> → agent quradi → <span style={{ color: "#FF7755" }}>tekshir</span> → tuzat</>, ru: <>идея → <span style={{ color: "#FFD380" }}>декомпозиция</span> → <span style={{ color: "#7DD181" }}>точный промпт</span> → агент строит → <span style={{ color: "#FF7755" }}>проверь</span> → почини</> })}
              </p>
              {done && <p className="fade-step" style={{ fontFamily: "'Manrope',sans-serif", fontSize: 13.5, margin: "14px 0 0", color: "#fff", opacity: 0.92 }}>{tr2({ uz: <>Bu formula bilan siz <b style={{ color: "#FFD380" }}>istalgan saytni</b> qurasiz — ovqat, do'kon, o'yin, kutubxona... mavzu muhim emas. Usul sizniki.</>, ru: <>С этой формулой вы построите <b style={{ color: "#FFD380" }}>любой сайт</b> — еда, магазин, игры, библиотека... тема не важна. Способ — ваш.</> })}</p>}
            </div>
          </Col>
        </div>
        </Zoomable>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} audioState={audio} scrollSignal={passed} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!passed} label={passed ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Komponentni e'lon qiling", ru: "Объявите компонент" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Oxirgi qadam: takrorlanuvchi <span className="italic" style={{ color: T.accent }}>komponentni</span> o'zingiz yarating.</>, ru: <>Последний шаг: создайте повторяющийся <span className="italic" style={{ color: T.accent }}>компонент</span> сами.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            <div className="fade-up delay-2" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              <span className="tagpill" style={{ opacity: hasCap ? 1 : 0.4 }}>{hasCap ? "✓" : "2"} {tr2({ uz: "Katta harf nom", ru: "Имя с Заглавной" })}</span>
              <span className="tagpill" style={{ opacity: hasProps ? 1 : 0.4 }}>{hasProps ? "✓" : "3"} (props)</span>
            </div>
            {lowerName && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Komponent nomi <b>Katta harf</b> bilan: <span className="mono">TaomCard</span> (React shundan komponentligini biladi).</>, ru: <>Имя компонента — с <b>Заглавной буквы</b>: <span className="mono">TaomCard</span> (по ней React понимает, что это компонент).</> })}</p></div>}
            {passed && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "✓ Mukammal! Siz komponent yaratdingiz — bo'laklashning eng asosiy amali. Endi istalgan UI'ni kichik, qayta ishlatiladigan bloklarga ajrata olasiz.", ru: "✓ Отлично! Вы создали компонент — главное действие декомпозиции. Теперь вы можете разложить любой UI на маленькие переиспользуемые блоки." })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "natija — komponent tayyor", ru: "результат — компонент готов" })}</p>
            <Win title="Yetkaz — localhost:5173" minH={130}>
              {valid ? <div className="fade-step"><TaomGrid /><p className="small" style={{ color: T.success, fontWeight: 700, margin: "7px 0 0" }}>{tr2({ uz: "✓ Bitta TaomCard — to'rtta taom (props bilan)", ru: "✓ Один TaomCard — четыре блюда (через props)" })}</p></div> : <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center", lineHeight: 1.5 }}>{tr2({ uz: "Komponent e'lon qilinmaguncha bo'sh: ", ru: "Пусто, пока компонент не объявлен: " })}<span className="mono" style={{ fontStyle: "normal" }}>function TaomCard(props) {"{"}</span></p>}
            </Win>
          </Col>
        </div>
        </Zoomable>
...
var Screen15 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  const audio = useAudio([{ id: "s15", text: `Tabriklaymiz — endi siz React dasturchisiz! Esda saqlang: katta sayt kichik bloklarning yig'indisi. Uni sahifalar va komponentlarga bo'laklaysiz, har bo'lakka aniq prompt yozasiz, AI bilan qurib, tekshirib, aniqlashtiruvchi prompt bilan tuzatasiz. Bu formula istalgan saytga ishlaydi. Endi o'z g'oyangizni boshlang!`, trigger: "on_mount", waits_for: null }]);
  const _gate = useContext2(LiveGateCtx) || {};
  ...
  const RECAP = [
    tr2({ uz: "Saytni bo'laklash: sahifalar (Router) + bo'limlar (komponentlar)", ru: "Декомпозиция сайта: страницы (Router) + секции (компоненты)" }),
    tr2({ uz: "Takrorlanuvchi → bitta komponent + props + map", ru: "Повторяющееся → один компонент + props + map" }),
    tr2({ uz: "Aniq prompt: qaysi bo'limlar + har bo'lim mazmuni + ma'lumot", ru: "Точный промпт: какие секции + содержание каждой + данные" }),
    tr2({ uz: "Qur → tekshir → aniqlashtiruvchi prompt bilan tuzat", ru: "Построй → проверь → почини уточняющим промптом" }),
    tr2({ uz: "g'oya → bo'laklash → prompt → qurish → tekshirish", ru: "идея → декомпозиция → промпт → сборка → проверка" })
  ];
  const HOMEWORK = [
    { b: tr2({ uz: "O'z saytingiz", ru: "Ваш сайт" }), t: tr2({ uz: "— g'oyani qog'ozga bo'laklang: sahifalar, komponentlar, birinchi bo'lim prompti", ru: "— разбейте идею на бумаге: страницы, компоненты, промпт первой секции" }) },
    { b: tr2({ uz: "Qurib boshlang", ru: "Начните сборку" }), t: tr2({ uz: "— Antigravity bilan bosh sahifadan boshlab, bo'lim-bo'lim quring", ru: "— начните с главной в Antigravity и собирайте секцию за секцией" }) },
    { b: tr2({ uz: "Portfolio", ru: "Портфолио" }), t: tr2({ uz: "— tayyor saytni deploy qilib, do'stlaringizga ko'rsating", ru: "— задеплойте готовый сайт и покажите друзьям" }) }
  ];
  const correct = SCORED_IDX.filter((i) => answers[i]?.correct).length;
  const total = SCORED_IDX.length;
  const PASSED = (total ? correct / total : 0) >= 0.6;
  return <Stage eyebrow={tr2({ uz: "Modul tugadi 🎓", ru: "Модуль завершён 🎓" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "React moduli tugadi", ru: "Модуль React завершён" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Siz endi <span className="italic" style={{ color: T.accent }}>React dasturchisiz</span>.</>, ru: <>Теперь вы — <span className="italic" style={{ color: T.accent }}>React-разработчик</span>.</> })}</h2>{
    /* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>{tr2({ uz: "🎓 Bitiruv vazifasi", ru: "🎓 Выпускное задание" })}</div><p className="body" style={{ margin: "0 0 10px", color: T.ink }}>{tr2({ uz: "Endi o'z saytingizni quring:", ru: "Теперь соберите свой сайт:" })}</p><ul>{HOMEWORK.map((h, i) => <li key={i}><b>{h.b}</b> <span className="t">{h.t}</span></li>)}</ul><p className="hw-note">{tr2({ uz: "Modul tugadi — lekin sizning yo'lingiz endi boshlanyapti. G'oyangiz bormi? Bugun boshlang! 🚀", ru: "Модуль завершён — но ваш путь только начинается. Есть идея? Начните сегодня! 🚀" })}</p></div>}
...
function ReactBuildSiteLesson({ lang: langProp, onFinished, liveToken }) {
...
export {
  ReactBuildSiteLesson as default
};
