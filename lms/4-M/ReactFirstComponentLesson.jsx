...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и сколько ✅/❌ — скрыто. По кнопке «Открыть результат» всё откроется одновременно и у вас, и на экранах учеников." })}</p>}
  ...
  return <div className="rocard el-in">
      ...
      <div className="robody">
        ...
        <div className="rostats">
          {likeable ? <button className={`rolike ${liked ? "on" : ""}`} onClick={() => setLiked((v) => !v)} title={tr({ uz: "Like bosib ko'ring", ru: "Попробуйте поставить лайк" })}>
...
function CardMachine({ mode = "anatomy", active = null, seen = null, n = 0, cards = [], phase = 0, running = false, onPull }) {
  ...
  return <div className={`cm cm--${mode} ${shake ? "cm-chunk" : ""}`}>
    ...
    /* RICHAG — old tomonda (S7) */
      ...
      {mode === "lever" && <button type="button" className={`cm-lever ${shake ? "pulled" : ""}`} onClick={pull} disabled={n >= 4} aria-label={tr({ uz: "Richagni torting", ru: "Потяните рычаг" })}>
    ...
    /* CHIQISH LATOGI (S6 ret + barcha modelar chiqishi) */
      ...
      <div className={`cm-chute ${glow("ret")}`}>
        ...
        <div className="cm-tray">
          {count === 0 ? <span className="cm-tray-empty">{tr({ uz: "latok bo'sh…", ru: "лоток пуст…" })}</span> : trayCards.map((nm, i) => <div className="cm-card-slot" key={`${mode}-${i}`}><RoCard name={nm} /></div>)}
        </div>
      </div>
    </div>;
}
        ...
        <Split>
          <Col>
            ...
            <div className="frame fade-up delay-2" style={{ padding: "11px 14px", minHeight: 138 }}>
              {phase < 2 ? <p style={{ color: T.ink3, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13, textAlign: "center", paddingTop: 38 }}>{phase === 0 ? tr({ uz: "Hozircha bo'sh — buyruqni ishga tushiring…", ru: "Пока пусто — запустите команду…" }) : tr({ uz: "yaratilmoqda…", ru: "создаётся…" })}</p> : <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "O'rnatish", ru: "Установка" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${tr({ uz: "Qadamlar", ru: "Шаги" })}: ${step}/4`} onClick={onNext} /></>}>
  ...
  >
          ...
          <Col>
            ...
            {done ? <Win title="localhost:5173" minH={120}>
              ...
              </Win> : <div className="frame-dash" style={{ minHeight: 120, display: "flex", alignItems: "center", justifyContent: "center" }}><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "4 qadamni bajaring — sayt shu yerda ochiladi", ru: "Выполните 4 шага — сайт откроется здесь" })}</p></div>}
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Loyiha tuzilishi", ru: "Структура проекта" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${seen.size}/4 ${tr({ uz: "fayl ko'rildi", ru: "файла просмотрено" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            ...
            {active ? <div className="sk-info" key={active}>
              ...
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Chapdan faylni bosing", ru: "Нажмите на файл слева" })}</p></div>}
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="JSX" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${seen.size}/3 ${tr({ uz: "qoida o'rganildi", ru: "правила изучено" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {KEYS.map((k) => <button key={k} className={`chip ${active === k ? "chip-on" : ""}`} onClick={() => tap(k)}>{tr(RULES[k].chip)} {seen.has(k) ? "✓" : ""}</button>)}
            </div>
          ...
          <Col>
            ...
            {active ? <div className="sk-info" key={active}>
              ...
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Chapdan qoidani tanlang", ru: "Выберите правило слева" })}</p></div>}
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [active, setActive] = useState(null);
  const [seen, setSeen] = useState(storedAnswer ? /* @__PURE__ */ new Set(["name", "ret", "jsx"]) : /* @__PURE__ */ new Set());
  const done = seen.size >= 3;
  const tap = (k) => {
    setActive(k);
    setSeen((prev) => {
  ...
  return <Stage eyebrow={tr({ uz: "Komponent", ru: "Компонент" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${seen.size}/3 ${tr({ uz: "qism topildi", ru: "части найдено" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            ...
            {active ? <div className="sk-info" key={active}>
              ...
              </div> : <div className="frame-dash" style={{ padding: "10px 14px" }}><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Koddan bir qismni bosing", ru: "Нажмите на часть кода" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: <>Komponent = <b>Katta harfli funksiya + return JSX</b>. Tamom — butun formula shu!</>, ru: <>Компонент = <b>функция с Заглавной буквы + return JSX</b>. Всё — вот и вся формула!</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Chaqirish", ru: "Вызов" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${tr({ uz: "Kamida 2 marta chaqiring", ru: "Вызовите минимум 2 раза" })} (${n}/2)`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [tried, setTried] = useState(storedAnswer ? /* @__PURE__ */ new Set(["Adopt Me!", "Blox Fruits"]) : /* @__PURE__ */ new Set(["Adopt Me!"]));
  ...
  return <Stage eyebrow="Props" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${tr({ uz: "3 xil nom sinang", ru: "Попробуйте 3 разных имени" })} (${tried.size}/3)`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
    ...
    setCards(["Adopt Me!"]);
  }}>{tr({ uz: "↺ Tozalash", ru: "↺ Очистить" })}</button>}
            </div>
            <pre className="code-box fade-up delay-2">
...
var Screen9 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "Komponentga uzatiladigan ma'lumot", ru: "Данные, которые передают компоненту" }), tr({ uz: "Komponentning tashqi rangi", ru: "Внешний цвет компонента" }), tr({ uz: "React'ni tezlashtiruvchi sozlama", ru: "Настройка, ускоряющая React" }), tr({ uz: "Brauzerga qo'shiladigan qo'shimcha dastur", ru: "Дополнение для браузера" })]}
  correctIdx={0}
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Ma'lumot yo'li", ru: "Путь данных" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Yo'lni kuzating", ru: "Проследите путь" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Keyingi qadam · AI", ru: "Следующий шаг · AI" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Agent bilan ishlab ko'ring", ru: "Поработайте с агентом" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [nmTried, setNmTried] = useState(storedAnswer ? /* @__PURE__ */ new Set(["Adopt Me!", "Blox Fruits"]) : /* @__PURE__ */ new Set(["Adopt Me!"]));
  const [emTried, setEmTried] = useState(storedAnswer ? /* @__PURE__ */ new Set(["🐾", "🍇"]) : /* @__PURE__ */ new Set(["🐾"]));
  ...
  return <Stage eyebrow={tr({ uz: "Amaliyot · props", ru: "Практика · props" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Ikkala props'ni almashtiring", ru: "Поменяйте оба props" })} onClick={onNext} /></>}>
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!passed} label={passed ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Chaqiruvni yozing", ru: "Напишите вызов" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tayyor", ru: "Готово" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: "Dars tugadi", ru: "Урок окончен" })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Birinchi komponentingiz <span className="italic" style={{ color: T.accent }}>tayyor</span>.</>, ru: <>Ваш первый компонент <span className="italic" style={{ color: T.accent }}>готов</span>.</> })}</h2>{
...
function ReactFirstComponentLesson({ lang: langProp, onFinished }) {
