...
var NavNext = ({ disabled, label, onClick, optionalLive }) => {
  ...
  const lbl = label !== void 0 ? tr(label) : tr({ uz: "Davom etish", ru: "Продолжить" });
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и число ✅/❌ скрыто — после «Открыть результат» всё появится сразу и у Вас, и на экранах учеников." })}</p>}
    ...
    return <button key={i} className={cls} disabled={solved || isMentorLive} onClick={() => pick(i)} style={{ padding: "clamp(13px,1.9vw,17px) clamp(15px,2.2vw,20px)", fontSize: "clamp(15px,1.85vw,17px)", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <FeedbackBlock show={isMentorLive ? mReveal : picked !== null} isCorrect={isMentorLive ? true : solved && !wrongLocked} neutral={waiting}>
          <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: waiting ? T.blue : isMentorLive || solved && !wrongLocked ? T.success : T.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>
            {isMentorLive ? <>{tr({ uz: "✓ To'g'ri javob:", ru: "✓ Верный ответ:" })} {String.fromCharCode(65 + correctIdx)} — {fmtCode(tr(options[correctIdx]))}</> : waiting ? tr({ uz: "📨 Javobingiz qabul qilindi", ru: "📨 Ваш ответ принят" }) : wrongLocked ? <>{tr({ uz: "To'g'ri javob:", ru: "Верный ответ:" })} {String.fromCharCode(65 + correctIdx)} — {fmtCode(tr(options[correctIdx]))}</> : solved ? tr({ uz: "To'g'ri", ru: "Верно" }) : tr({ uz: "Qaytadan urinib ko'ring", ru: "Попробуйте ещё раз" })}
          </p>
...
function DebugChallenge({ lines, fixed, explain, onSolved }) {
  ...
  return <div className="dbg fade-up">
      ...
      {!solved ? <p className="dbg-hint">{tr({ uz: "👆 Xato bor qatorni toping va bosing", ru: "👆 Найдите строку с ошибкой и нажмите на неё" })}</p> : <div className="dbg-ok">{tr({ uz: "✓ Topdingiz!", ru: "✓ Нашли!" })} {tr(explain)}</div>}
    </div>;
}
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(storedAnswer ? new Set(VKEYS) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={tr({ uz: "Fe'llar", ru: "Глаголы" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `${seen.size}/4 fe'lni sinang`, ru: `Попробуйте глаголы: ${seen.size}/4` }} onClick={onNext} /></>}>
    ...
    return <button key={k} className={`vcard ${!seen.has(k) && !on ? "hint" : ""}`} onClick={() => tap(k)} style={{ boxShadow: on ? `inset 0 0 0 1.5px ${v.col}, 0 8px 20px -6px rgba(${T.shadowBase},0.2)` : void 0 }}>
          ...
          <Col>
            ...
            <div className="code-box" style={{ padding: "9px 13px", minHeight: 40 }}>
              {active ? <TLine out={<span className="el-in" style={{ display: "inline-block" }}><span style={{ color: VERBS[active].col, fontWeight: 700 }}>{active}</span> /games{active === "PUT" || active === "DELETE" ? <span style={{ color: CODE.attr }}>/{active === "PUT" ? 2 : 3}</span> : ""} → {CONSOLE[active]}</span>} /> : <TLine out={<span style={{ color: CODE.comment, fontStyle: "italic" }}>{tr({ uz: "chapdan fe'lni tanlang…", ru: "выберите глагол слева…" })}</span>} />}
            </div>
...
var Screen4 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "GET — u hamma ishni qiladi", ru: "GET — он делает всю работу" }), tr({ uz: "POST — yangi yozuv yaratadi", ru: "POST — создаёт новую запись" }), tr({ uz: "Sahifani yangilash kifoya", ru: "Достаточно обновить страницу" }), tr({ uz: "DELETE — u yozuvni o'chiradi", ru: "DELETE — он удаляет запись" })]}
  correctIdx={1}
        ...
        <div className="split">
          ...
          <Col>
            ...
            <div className="code-box" style={{ padding: "10px 13px", minHeight: 52 }}>
              {done ? <TLine out={<span className="el-in" style={{ display: "inline-block", color: CODE.str }}>{`'{"name":"${name}","emoji":"${emoji}"}'`}</span>} /> : <TLine out={<span style={{ color: CODE.comment, fontStyle: "italic" }}>{tr({ uz: "name va emoji tanlang…", ru: "выберите name и emoji…" })}</span>} />}
            </div>
...
var Screen5b = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "O'yinni darhol katalogga chizadi", ru: "Сразу рисует игру в каталоге" }), tr({ uz: "Obyektda xato bor-yo'qligini tekshiradi", ru: "Проверяет объект на ошибки" }), tr({ uz: "Obyektni JSON matnga aylantiradi", ru: "Превращает объект в JSON-текст" }), tr({ uz: "Serverdan javob olib keladi", ru: "Приносит ответ с сервера" })]}
  correctIdx={2}
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Parvoz", ru: "Полёт" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Posilkani kuzating", ru: "Проследите за посылкой" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            ...
            <div className="code-box" style={{ padding: "9px 13px", minHeight: 38 }}>
              {step >= 3 ? <TLine out={<span className="el-in" style={{ display: "inline-block" }}><span style={{ color: VERBS.POST.col, fontWeight: 700 }}>POST</span> /games → <span style={{ color: CODE.str }}>201 Created ✓</span></span>} /> : <TLine out={<span style={{ color: CODE.comment, fontStyle: "italic" }}>{step >= 2 ? tr({ uz: "posilka yo'lda…", ru: "посылка в пути…" }) : tr({ uz: "konsol kutmoqda…", ru: "консоль ждёт…" })}</span>} />}
            </div>
...
var Screen9 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "7 ta o'yin olib keladi", ru: "Принесёт 7 игр" }), tr({ uz: "Katalogning 7-sahifasini ochadi", ru: "Откроет 7-ю страницу каталога" }), tr({ uz: "7 soniya kutib turadi", ru: "Подождёт 7 секунд" }), tr({ uz: "ID raqami 7 bo'lgan bitta o'yin", ru: "Одна игра с ID номер 7" })]}
  correctIdx={3}
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="DELETE" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Eski o'yinni o'chiring", ru: "Удалите старую игру" }} onClick={onNext} /></>}>
    ...
    setAsking(true);
    setCancelled(false);
  }}>{tr({ uz: "DELETE yuborish", ru: "Отправить DELETE" })}</button>}
          </Col>
          <Col>
    ...
    setAsking(false);
    setDeleted(true);
  }}>{tr({ uz: "Ha, o'chirilsin", ru: "Да, удалить" })}</button>
                </div>
              </div>}
...
var Screen12 = (props) => <QuestionScreen
  ...
  options={["DELETE /games/3", tr({ uz: "DELETE /games — hammasiga yuboramiz", ru: "DELETE /games — отправим всем" }), "POST /games/3", "GET /games/3"]}
  correctIdx={0}
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Amaliyot · pult", ru: "Практика · пульт" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `Posilka ${Math.min(stage + 1, 3)}/3`, ru: `Посылка ${Math.min(stage + 1, 3)}/3` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        ...
        <div className="split">
          <Col>
            {!done ? <div className="sk-info" key={stage}><p className="flow-label" style={{ marginBottom: 5 }}>{tr({ uz: "Posilka", ru: "Посылка" })} {stage + 1}/3</p><p className="body" style={{ margin: 0, color: T.ink, fontWeight: 600 }}>{STAGE_LBL[stage]}</p></div> : <div className="takeaway fade-step"><div className="ta-bulb">🎛️</div><p className="ta-h">{tr({ uz: "Pult sizniki!", ru: "Пульт Ваш!" })}</p><p className="ta-sub">Create · Read · Update · Delete — CRUD</p></div>}
            {stage === 0 && !done && <>
          ...
          <Col>
            ...
            <div className="code-box" style={{ padding: "9px 13px", minHeight: 38 }}>
              {done ? <TLine out={<span style={{ color: CODE.str }}>{tr({ uz: "✓ 3/3 posilka jo'natildi — CRUD to'liq", ru: "✓ 3/3 посылки отправлены — CRUD полный" })}</span>} /> : postFly && stage <= 1 ? <TLine out={<span className="el-in" style={{ display: "inline-block" }}><span style={{ color: VERBS.POST.col, fontWeight: 700 }}>POST</span> /games → <span style={{ color: CODE.str }}>201 Created ✓</span></span>} /> : <TLine out={<span style={{ color: CODE.comment, fontStyle: "italic" }}>{tr({ uz: `posilka ${stage + 1}/3 kutilmoqda…`, ru: `ждём посылку ${stage + 1}/3…` })}</span>} />}
            </div>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Keyingi qadam · AI", ru: "Следующий шаг · AI" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Agent bilan ishlab ko'ring", ru: "Поработайте с агентом" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
  ...
  return <Stage eyebrow={tr({ uz: "Debugging", ru: "Дебаггинг" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : found ? { uz: "Endi tuzating", ru: "Теперь исправьте" } : { uz: "Yetishmayotganini toping", ru: "Найдите, чего не хватает" }} onClick={onNext} /></>}>
    ...
    if (!found) setPicked("end");
          ...
          <Col>
            ...
            <div className="code-box" style={{ padding: "9px 13px" }}>
              {!fixed ? <TLine out={<span><span style={{ color: VERBS.GET.col, fontWeight: 700 }}>GET</span> /games → <span style={{ color: CODE.str }}>200 OK</span> <span style={{ color: CODE.comment }}>{tr({ uz: "…lekin hech narsa qo'shilmadi", ru: "…но ничего не добавилось" })}</span></span>} /> : <TLine out={<span className="el-in" style={{ display: "inline-block" }}><span style={{ color: VERBS.POST.col, fontWeight: 700 }}>POST</span> /games → <span style={{ color: CODE.str }}>201 Created ✓</span></span>} />}
            </div>
            <div className="code-box" style={{ padding: "11px 13px", display: "flex", flexDirection: "column", gap: 5 }}>
              <SrvRow id={1} name="Adopt Me!" />
              <SrvRow id={2} name="Doors" />
            ...
            {fixed && <div className="takeaway fade-step"><div className="ta-bulb">🛠️</div><p className="ta-h">{tr({ uz: "Ko'rinmas xatoni ham topdingiz!", ru: "Вы нашли даже невидимую ошибку!" })}</p><p className="ta-sub">{tr({ uz: "Konsoldagi fe'lni o'qish — dasturchilar odati", ru: "Читать глагол в консоли — привычка профессионала" })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen16 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!passed} label={passed ? { uz: "Davom etish", ru: "Продолжить" } : nameOk ? { uz: "Fe'lni yozing", ru: "Напишите глагол" } : { uz: "O'yiningizga nom bering", ru: "Дайте игре название" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
  ...
  />
          ...
          <Col>
            ...
            <Win title="robo-games — localhost:5173" minH={130}>
              ...
              {passed && !loaded && <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                  <SkelCard /><SkelCard /><SkelCard />
                </div>}
              {passed && loaded && <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 9 }}>
                  <RoCard name="Adopt Me!" />
                  <RoCard name="Doors" />
...
function AchCelebrate({ ach, onDone }) {
  ...
  return <div className="acu-overlay" onClick={onDone} role="status" aria-label={`${tr({ uz: "Yangi nishon:", ru: "Новая награда:" })} ${tr(ach.name)}`}>
...
function ReactApiPostLesson({ lang: langProp, onFinished }) {
