...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up" style={{ maxWidth: 800 }}>{tr2({ uz: <>Siz o'yin yaratdingiz. Nega u katalogga <span className="italic" style={{ color: T.accent }}>chiqmayapti</span>?</>, ru: <>Вы создали игру. Почему она <span className="italic" style={{ color: T.accent }}>не появляется</span> в каталоге?</> })}</h1>
...
var Screen16 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const EMOJIS = ["🤖", "🏎️", "🐉", "⚽", "🎯"];
  const [gname, setGname] = useState3(storedAnswer?.gameName || "");
