...
//  Manba:  src/3-Modull/ReactApiPostLesson.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/3-Modull/ReactApiPostLesson.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/3-Modull/ReactApiPostLesson.jsx
...
var LESSON_META = { lessonId: "react-api-post-06-v18", lessonTitle: { uz: "API: POST/PUT/DELETE — serverga ma'lumot yuborish", ru: "Работа с API — POST/PUT/DELETE" } };
var HW_TOKENS = [
...
var NavNext = ({ disabled, label, onClick, optionalLive }) => {
  ...
  const lbl = label !== void 0 ? tr2(label) : tr2({ uz: "Davom etish", ru: "Продолжить" });
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr2({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и число ✅/❌ скрыто — после «Открыть результат» всё появится сразу и у Вас, и на экранах учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верных — класс не понял эту тему. Перед продолжением советуем коротко повторить.</> })}</p>
      ...
      {reveal && struggling && <p className="mstats-warn">{tr2({ uz: "⚠️ Ko'pchilik xato qildi — bu mavzu tushunarsiz bo'lgan ko'rinadi. Qayta tushuntiring.", ru: "⚠️ Большинство ошиблось — похоже, тема осталась непонятной. Советуем объяснить ещё раз." })}</p>}
...
function DragDropOrder({ items, hints, onSolved }) {
  ...
  return <div className="dd fade-up">
      ...
      <div className="dd-pool">
        {pool.length === 0 && !solved && <span className="dd-pool-empty">{tr2({ uz: "Tartib xato — bo'lakni bosib qaytaring va qayta joylang", ru: "Порядок неверный — нажмите на блок, верните его и разместите заново" })}</span>}
...
var gameByName = (nm) => GAMES.find((g) => g.name.toLowerCase() === String(nm).toLowerCase());
var VERBS = {
...
var VKEYS = ["GET", "POST", "PUT", "DELETE"];
var SrvRow = ({ id, name, state, extra }) => <div className={`srow ${state ? `srow-${state}` : ""}`}>
    <span className="srid">{id}</span>
    <span className="srname">{name}</span>
    {extra && <span className="srextra">{extra}</span>}
  </div>;
var RoCard = ({ name, emoji, players, top, likes, onLike }) => {
  ...
  return <div className="rocard el-in" style={{ position: "relative" }}>
      ...
      <div className="robody">
        ...
        <div className="rostats">
          <span key={pct} className={onLike ? "hpop" : void 0} onClick={onLike} style={onLike ? { cursor: "pointer", fontWeight: 700, color: T.ink } : void 0}>👍 {pct}%</span>
          {players && <span>👥 {players}</span>}
        </div>
      </div>
    </div>;
};
var SkelCard = () => <div className="rocard">
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  const [fetched, setFetched] = useState3(!!storedAnswer);
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>Siz o'yin yaratdingiz. Nega u katalogga <span className="italic" style={{ color: T.accent }}>chiqmayapti</span>?</>, ru: <>Вы создали игру. Почему она <span className="italic" style={{ color: T.accent }}>не появляется</span> в каталоге?</> })}</h1>
          ...
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ maxWidth: 150, flexShrink: 0 }}>
                <p className="flow-label" style={{ marginBottom: 6 }}>{tr2({ uz: "Sizning kompyuteringizda", ru: "На Вашем компьютере" })}</p>
                <div className="frame-dash" style={{ padding: 8 }}><RoCard name="Robo Race" emoji="🤖" likes={100} /></div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <button className="btn" onClick={doGet} disabled={loading}>{loading ? tr2({ uz: "So'rov yo'lda…", ru: "Запрос в пути…" }) : fetched ? tr2({ uz: "↻ Yana GET yuborish", ru: "↻ Отправить GET ещё раз" }) : tr2({ uz: "GET bilan olish", ru: "Получить через GET" })}</button>
                {fetched && <span className="mono small fade-step" style={{ color: T.ink2 }}>GET /games → 200 OK</span>}
              </div>
            </div>
            <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "robo-games — katalog", ru: "robo-games — каталог" })}</p>
            <Win title="robo-games — localhost:5173" minH={96}>
              ...
              {!loading && <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                  <RoCard name="Adopt Me!" />
                  <RoCard name="Doors" />
                  <RoCard name="Piggy" />
                </div>}
            </Win>
            {fetched && <p className="small fade-step" style={{ color: T.accent, fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Katalog keldi… lekin Robo Race ICHIDA YO'Q!", ru: "Каталог пришёл… но Robo Race в нём НЕТ!" })}</p>}
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Sizningcha, nega chiqmadi?", ru: "Как Вы думаете, почему не появилась?" })}</p>
            <div className="fade-up delay-3" style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {OPTS.map((o) => {
    const on = picked === o.id;
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null || !fetched} style={{ opacity: !fetched ? 0.55 : 1 }} onClick={() => pick(o.id)}>
                    <span className="radio">{on && <span className="radio-dot" />}</span>
                    <span>{tr2(o.label)}</span>
                  </button>;
  })}
            </div>
            {!fetched && <p className="small" style={{ color: T.ink3, fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Avval GET yuborib ko'ring ←", ru: "Сначала отправьте GET ←" })}</p>}
            {picked !== null && <p className="hook-ack fade-step">{tr2({ uz: <>Aynan! GET faqat <b>o'qiydi</b> — bor narsani olib keladi. O'yiningiz hali serverda yo'q — bugun <b>yuborishni</b> o'rganamiz: POST, PUT, DELETE.</>, ru: <>Именно! GET только <b>читает</b> — приносит то, что уже есть. Вашей игры на сервере ещё нет — сегодня научимся <b>отправлять</b>: POST, PUT, DELETE.</> })}</p>}
          </Col>
        </Split>
        </Zoomable>
      </div>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  const STEPS = [
    { text: tr2({ uz: "So'rov fe'llari — 4 buyruq", ru: "Глаголы запросов — 4 команды" }), tag: "GET · POST · PUT · DELETE" },
    { text: tr2({ uz: "POST — yangi qo'shish", ru: "POST — добавить новое" }), tag: "method: 'POST'" },
    { text: tr2({ uz: "Posilka yuki — body", ru: "Груз посылки — body" }), tag: "JSON.stringify(yangi)" },
    { text: tr2({ uz: "PUT — almashtirish", ru: "PUT — заменить" }), tag: "/games/2 + ID" },
    { text: tr2({ uz: "DELETE — o'chirish", ru: "DELETE — удалить" }), tag: tr2({ uz: "tasdiqlash bilan", ru: "с подтверждением" }) }
  ];
  const isNarrow = useIsMobile(768);
  const [showSteps, setShowSteps] = useState3(false);
  const PreviewBlock = <Col>
      <p className="flow-label">{tr2({ uz: "Dars oxirida — sizning o'yiningiz serverda", ru: "В конце урока — Ваша игра на сервере" })}</p>
      ...
      <p className="mono small" style={{ color: T.accent, margin: 0 }}>{tr2({ uz: "→ endi siz serverga buyruq berasiz", ru: "→ теперь Вы отдаёте команды серверу" })}</p>
    </Col>;
  ...
  return <Stage eyebrow={tr2({ uz: "Reja", ru: "План" })} screen={screen} audioState={audio} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Boshlaymiz →", ru: "Начинаем →" }} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head">
          <h2 className="title h-title fade-up">{tr2({ uz: <>Bugun siz o'yinchi emas — <span className="italic" style={{ color: T.accent }}>dasturchi</span> bo'lasiz.</>, ru: <>Сегодня Вы не игрок — Вы <span className="italic" style={{ color: T.accent }}>разработчик</span>.</> })}</h2>
        </div>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState3(storedAnswer ? new Set(VKEYS) : /* @__PURE__ */ new Set());
  ...
  const CONSOLE = {
    GET: <span style={{ color: CODE.str }}>{tr2({ uz: "200 OK — 3 qator o'qildi", ru: "200 OK — прочитано 3 строки" })}</span>,
    POST: <span style={{ color: CODE.str }}>{tr2({ uz: "201 Created — yangi qator qo'shildi", ru: "201 Created — добавлена новая строка" })}</span>,
    PUT: <span style={{ color: CODE.str }}>{tr2({ uz: "200 OK — 2-qator almashtirildi", ru: "200 OK — строка 2 заменена" })}</span>,
    DELETE: <span style={{ color: CODE.str }}>{tr2({ uz: "200 OK — 3-qator o'chirildi", ru: "200 OK — строка 3 удалена" })}</span>
  };
  ...
  return <Stage eyebrow={tr2({ uz: "Fe'llar", ru: "Глаголы" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `${seen.size}/4 fe'lni sinang`, ru: `Попробуйте глаголы: ${seen.size}/4` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Serverga necha xil <span className="italic" style={{ color: T.accent }}>buyruq</span> berish mumkin?</>, ru: <>Сколько разных <span className="italic" style={{ color: T.accent }}>команд</span> можно отдать серверу?</> })}</h2></div>
    ...
    const v = VERBS[k];
    const on = active === k;
    return <button key={k} className={`vcard ${!seen.has(k) && !on ? "hint" : ""}`} onClick={() => tap(k)} style={{ boxShadow: on ? `inset 0 0 0 1.5px ${v.col}, 0 8px 20px -6px rgba(${T.shadowBase},0.2)` : void 0 }}>
            ...
            {active && <div className="sk-info" key={active}><p className="body" style={{ margin: 0, color: T.ink }}><b style={{ color: VERBS[active].col }}>{active}</b> — {tr2(VERBS[active].desc)}.</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "robo-api.uz — server jadvali", ru: "robo-api.uz — таблица сервера" })}</p>
            <div className="code-box fade-up delay-2" style={{ padding: "11px 13px", display: "flex", flexDirection: "column", gap: 5 }}>
              ...
              <SrvRow id={2} name={active === "PUT" ? "Doors 2.0" : "Doors"} state={active === "GET" ? "read" : active === "PUT" ? "changed" : null} extra={active === "PUT" ? tr2({ uz: "yangilandi", ru: "обновлено" }) : null} />
              <SrvRow id={3} name="Piggy" state={active === "GET" ? "read" : active === "DELETE" ? "del" : null} extra={active === "DELETE" ? tr2({ uz: "o'chirildi", ru: "удалено" }) : null} />
              {active === "POST" && <SrvRow id={4} name="Robo Race" state="new" extra={tr2({ uz: "+ yangi", ru: "+ новое" })} />}
            </div>
            <div className="code-box" style={{ padding: "9px 13px", minHeight: 40 }}>
              {active ? <TLine out={<span className="el-in" style={{ display: "inline-block" }}><span style={{ color: VERBS[active].col, fontWeight: 700 }}>{active}</span> /games{active === "PUT" || active === "DELETE" ? <span style={{ color: CODE.attr }}>/{active === "PUT" ? 2 : 3}</span> : ""} → {CONSOLE[active]}</span>} /> : <TLine out={<span style={{ color: CODE.comment, fontStyle: "italic" }}>{tr2({ uz: "chapdan fe'lni tanlang…", ru: "выберите глагол слева…" })}</span>} />}
            </div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Bular — <b>HTTP fe'llari</b>. Manzil bitta (<span className="mono">/games</span>), fe'l har xil — server har safar boshqa ish qiladi. Endi har birini alohida o'rganamiz.</>, ru: <>Это — <b>HTTP-глаголы</b>. Адрес один (<span className="mono">/games</span>), глаголы разные — и сервер каждый раз делает разную работу. Теперь разберём каждый по отдельности.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="POST" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Posilkani yuboring", ru: "Отправьте посылку" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Ma'lumot posilkasi endi <span className="italic" style={{ color: T.accent }}>qaysi tomonga</span> uchadi?</>, ru: <>В какую сторону теперь <span className="italic" style={{ color: T.accent }}>летит</span> посылка с данными?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Taqqoslang", ru: "Сравните" })}</p>
            <pre className="code-box fade-up delay-1" style={{ lineHeight: 2 }}>
              <Cm>{tr2({ uz: "// o'tgan dars — olish:", ru: "// прошлый урок — получить:" })}</Cm>{"\n"}
              {"fetch(url)"}{"\n\n"}
              <Cm>{tr2({ uz: "// bugun — yuborish:", ru: "// сегодня — отправить:" })}</Cm>{"\n"}
            ...
            <button className="btn fade-up delay-2" style={{ alignSelf: "flex-start" }} onClick={send} disabled={running}>{running ? tr2({ uz: "Yo'lda…", ru: "В пути…" }) : done ? tr2({ uz: "📦 Yana yuborish", ru: "📦 Отправить ещё раз" }) : tr2({ uz: "📦 POST yuborish", ru: "📦 Отправить POST" })}</button>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Ikkinchi qism — <b>sozlamalar qutisi</b> <span className="mono">{"{ }"}</span>: fetch'ga "qanday borish"ni aytadi. <span className="mono">method: 'POST'</span> = "olib kelma — <b>olib bor</b>".</>, ru: <>Вторая часть — <b>коробка настроек</b> <span className="mono">{"{ }"}</span>: она говорит fetch, «как идти». <span className="mono">method: 'POST'</span> = «не приноси — <b>отнеси</b>».</> })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Posilka yo'li", ru: "Путь посылки" })}</p>
            <div className="fade-up delay-2" style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ flex: 1, textAlign: "center", padding: "10px 8px", borderRadius: 12, background: T.paper, boxShadow: `0 4px 12px -6px rgba(${T.shadowBase},0.14)` }}>
                <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "SIZ", ru: "ВЫ" })}</p>
              ...
              <div style={{ flex: 1, textAlign: "center", padding: "10px 8px", borderRadius: 12, background: phase >= 2 ? T.successSoft : T.paper, boxShadow: phase >= 2 ? `inset 0 0 0 1.5px ${T.success}` : `0 4px 12px -6px rgba(${T.shadowBase},0.14)`, transition: "all 0.35s" }}>
                <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "SERVER", ru: "СЕРВЕР" })}</p>
                <p className="mono small" style={{ margin: "4px 0 0", color: phase >= 2 ? T.success : T.ink3 }}>{phase >= 2 ? tr2({ uz: "qabul qildi ✓", ru: "принял ✓" }) : tr2({ uz: "kutmoqda…", ru: "ждёт…" })}</p>
              </div>
            </div>
            <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "robo-api.uz — jadval", ru: "robo-api.uz — таблица" })}</p>
            <div className="code-box" style={{ padding: "11px 13px", display: "flex", flexDirection: "column", gap: 5 }}>
              ...
              {phase >= 2 && <SrvRow id={4} name="Robo Race" state="new" extra={tr2({ uz: "+ yangi", ru: "+ новое" })} />}
            </div>
            {done && <span className="tagpill fade-step" style={{ color: T.success }}>{tr2({ uz: "✓ POST — serverda YANGI qator paydo qildi", ru: "✓ POST — на сервере появилась НОВАЯ строка" })}</span>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen4 = (props) => <QuestionScreen
  ...
  questionText={tr2({ uz: "Serverga YANGI ma'lumot qo'shish uchun qaysi so'rov ishlatiladi?", ru: "Какой запрос добавляет на сервер НОВЫЕ данные?" })}
  audioText="Serverga yangi ma'lumot qo'shish uchun qaysi so'rov ishlatiladi? To'g'ri javobni tanlang."
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Serverga <span className="italic" style={{ color: T.accent }}>yangi ma'lumot qo'shish</span> uchun qaysi so'rov?</>, ru: <>Какой запрос <span className="italic" style={{ color: T.accent }}>добавляет новые данные</span> на сервер?</> })}</h2></>}
  options={[tr2({ uz: "GET — u hamma ishni qiladi", ru: "GET — он делает всю работу" }), tr2({ uz: "POST — yangi yozuv yaratadi", ru: "POST — создаёт новую запись" }), tr2({ uz: "Sahifani yangilash kifoya", ru: "Достаточно обновить страницу" }), tr2({ uz: "DELETE — u yozuvni o'chiradi", ru: "DELETE — он удаляет запись" })]}
  correctIdx={1}
  explainCorrect={tr2({ uz: "To'g'ri! POST = qo'shish. fetch(url, { method: 'POST', … }) — ikkinchi qismdagi fe'l serverga 'yangi yozuv yarat' deydi.", ru: "Верно! POST = добавить. fetch(url, { method: 'POST', … }) — глагол во второй части говорит серверу: «создай новую запись»." })}
  explainWrong={{
    0: tr2({ uz: "Hook'ni eslang: GET yubordingiz — o'yiningiz katalogga chiqmadi. GET faqat O'QIYDI.", ru: "Вспомните начало урока: Вы отправили GET — игра в каталоге не появилась. GET только ЧИТАЕТ." }),
    2: tr2({ uz: "Yangilash faqat bor narsani qayta ko'rsatadi. Serverda yo'q narsa paydo bo'lmaydi.", ru: "Обновление лишь заново показывает то, что есть. То, чего нет на сервере, не появится." }),
    3: tr2({ uz: "DELETE — aksincha, o'chiradi! Qo'shish uchun POST.", ru: "DELETE — наоборот, удаляет! Для добавления — POST." }),
    default: tr2({ uz: "Qo'shish = POST. Fe'l sozlamalar qutisida yoziladi: { method: 'POST' }.", ru: "Добавить = POST. Глагол пишется в коробке настроек: { method: 'POST' }." })
  }}
/>;
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const NAMES = ["Robo Race", "Super Kart", "Pixel Quest"];
  const EMOJIS = ["🤖", "🏎️", "🐉"];
  const [name, setName] = useState3(storedAnswer ? "Robo Race" : null);
  const [emoji, setEmoji] = useState3(storedAnswer ? "🤖" : null);
  ...
  return <Stage eyebrow="Body" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Posilkani yig'ing", ru: "Соберите посылку" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>O'yiningiz posilkaning <span className="italic" style={{ color: T.accent }}>qaysi qismiga</span> joylanadi?</>, ru: <>В <span className="italic" style={{ color: T.accent }}>какую часть</span> посылки кладётся Ваша игра?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            <div className="fade-up delay-1" style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {NAMES.map((v) => <button key={v} className="gchip" style={name === v ? { background: T.accent, color: "#fff" } : void 0} onClick={() => setName(v)}>{v}</button>)}
            </div>
            <p className="flow-label" style={{ marginTop: 2 }}>emoji</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {EMOJIS.map((v) => <button key={v} className="gchip" style={emoji === v ? { background: T.accent } : void 0} onClick={() => setEmoji(v)}>{v}</button>)}
            </div>
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "Tarjimon ikki tomonga ishlaydi", ru: "Переводчик работает в обе стороны" })}</p>
            <div className="fade-up delay-2" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 11, padding: "10px 14px", borderRadius: 12, background: T.paper, boxShadow: `0 4px 12px -6px rgba(${T.shadowBase},0.14)` }}>
                <span className="mono" style={{ fontSize: 11.5, fontWeight: 700, color: T.blue, minWidth: 72 }}>{tr2({ uz: "KELISHDA", ru: "НА ВХОДЕ" })}</span>
                <span style={{ fontSize: 12.5, color: T.ink }}><span className="mono">res.json()</span> — {tr2({ uz: "JSON matn → massiv", ru: "JSON-текст → массив" })}</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 11, padding: "10px 14px", borderRadius: 12, background: T.paper, boxShadow: `0 4px 12px -6px rgba(${T.shadowBase},0.14)` }}>
                <span className="mono" style={{ fontSize: 11.5, fontWeight: 700, color: T.success, minWidth: 72 }}>{tr2({ uz: "KETISHDA", ru: "НА ВЫХОДЕ" })}</span>
                <span style={{ fontSize: 12.5, color: T.ink }}><span className="mono">JSON.stringify()</span> — {tr2({ uz: "obyekt → JSON matn", ru: "объект → JSON-текст" })}</span>
              </div>
            </div>
            <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "Posilka ichi", ru: "Внутри посылки" })} {done ? tr2({ uz: "— tayyor", ru: "— готово" }) : ""}</p>
            <div className="code-box" style={{ padding: "10px 13px", minHeight: 52 }}>
              {done ? <TLine out={<span className="el-in" style={{ display: "inline-block", color: CODE.str }}>{`'{"name":"${name}","emoji":"${emoji}"}'`}</span>} /> : <TLine out={<span style={{ color: CODE.comment, fontStyle: "italic" }}>{tr2({ uz: "name va emoji tanlang…", ru: "выберите name и emoji…" })}</span>} />}
            </div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Tanish ko'rinishmi? O'tgan darsda javob <b>shunday matn</b> bo'lib kelardi! Endi siz ham serverga <b>o'sha tilda</b> yozyapsiz. <span className="mono">stringify</span> = "matnga aylantir".</>, ru: <>Знакомо выглядит? На прошлом уроке ответ приходил <b>именно таким текстом</b>! Теперь и Вы пишете серверу <b>на том же языке</b>. <span className="mono">stringify</span> = «преврати в текст».</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen5b = (props) => <QuestionScreen
  ...
  questionText={tr2({ uz: "JSON.stringify(yangi) nima qiladi?", ru: "Что делает JSON.stringify(yangi)?" })}
  audioText="JSON.stringify(yangi) nima qiladi? To'g'ri javobni tanlang."
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "Mustahkamlash", ru: "Закрепление" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <><span className="mono" style={{ color: T.accent }}>JSON.stringify(yangi)</span> nima qiladi?</>, ru: <>Что делает <span className="mono" style={{ color: T.accent }}>JSON.stringify(yangi)</span>?</> })}</h2></>}
  options={[tr2({ uz: "O'yinni darhol katalogga chizadi", ru: "Сразу рисует игру в каталоге" }), tr2({ uz: "Obyektda xato bor-yo'qligini tekshiradi", ru: "Проверяет объект на ошибки" }), tr2({ uz: "Obyektni JSON matnga aylantiradi", ru: "Превращает объект в JSON-текст" }), tr2({ uz: "Serverdan javob olib keladi", ru: "Приносит ответ с сервера" })]}
  correctIdx={2}
  explainCorrect={tr2({ uz: "To'g'ri! Server obyektni emas, JSON matnni tushunadi. stringify — 'matnga aylantir': obyektdan JSON matn yasaydi. Bu .json()ning teskarisi.", ru: "Верно! Сервер понимает не объект, а JSON-текст. stringify — «преврати в текст»: делает из объекта JSON-текст. Это операция, обратная .json()." })}
  explainWrong={{
    0: tr2({ uz: "Yo'q — chizish React'ning ishi. stringify faqat tarjima qiladi: obyekt → matn.", ru: "Нет — рисовать это работа React. stringify только переводит: объект → текст." }),
    1: tr2({ uz: "Yo'q — u tekshirmaydi, aylantiradi. Obyektdan JSON matn yasaydi.", ru: "Нет — он не проверяет, а превращает. Делает из объекта JSON-текст." }),
    3: tr2({ uz: "Bu .json()ning ishi — KELGAN javobni o'girish. stringify esa KETAYOTGAN yukni o'giradi.", ru: "Это работа .json() — переводить ПРИШЕДШИЙ ответ. А stringify переводит УХОДЯЩИЙ груз." }),
    default: tr2({ uz: "stringify = obyektni JSON matnga aylantirish, body shu matn bilan uchadi.", ru: "stringify = превратить объект в JSON-текст; body летит именно с этим текстом." })
  }}
/>;
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const STEPS = [
    { z: 1, t: tr2({ uz: "Yuk tayyorlandi — stringify matnga o'girdi", ru: "Груз готов — stringify перевёл его в текст" }) },
    { z: 2, t: tr2({ uz: "POST uchdi — posilka serverga yo'lda", ru: "POST полетел — посылка в пути к серверу" }) },
    { z: 3, t: tr2({ uz: "Server qabul qildi — 201 Created", ru: "Сервер принял — 201 Created" }) },
    { z: 4, t: tr2({ uz: "Katalog yangilandi — kartochka paydo", ru: "Каталог обновился — карточка появилась" }) }
  ];
  ...
  return <Stage eyebrow={tr2({ uz: "Parvoz", ru: "Полёт" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Posilkani kuzating", ru: "Проследите за посылкой" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Posilka serverga yetib borganini <span className="italic" style={{ color: T.accent }}>qayerdan bilamiz</span>?</>, ru: <>Как <span className="italic" style={{ color: T.accent }}>узнать</span>, что посылка долетела до сервера?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <button className="btn fade-up delay-1" style={{ alignSelf: "flex-start" }} onClick={run} disabled={running}>{running ? tr2({ uz: "Parvozda…", ru: "В полёте…" }) : done ? tr2({ uz: "↻ Yana kuzatish", ru: "↻ Посмотреть ещё раз" }) : tr2({ uz: "▶ Posilkani uchirish", ru: "▶ Запустить посылку" })}</button>
            <pre className="code-box fade-up delay-2" style={{ lineHeight: 1.95 }}>
          ...
          <Col>
            <div className="fade-up delay-2" style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {STEPS.map((s) => <div key={s.z} style={{ display: "flex", alignItems: "center", gap: 11, padding: "8px 12px", borderRadius: 11, background: step >= s.z ? T.successSoft : T.paper, boxShadow: running && step === s.z ? `inset 0 0 0 1.5px ${T.accent}` : step >= s.z ? `inset 0 0 0 1.5px ${T.success}` : `0 4px 12px -6px rgba(${T.shadowBase},0.14)`, transition: "all 0.35s" }}>
            ...
            <div className="code-box" style={{ padding: "9px 13px", minHeight: 38 }}>
              {step >= 3 ? <TLine out={<span className="el-in" style={{ display: "inline-block" }}><span style={{ color: VERBS.POST.col, fontWeight: 700 }}>POST</span> /games → <span style={{ color: CODE.str }}>201 Created ✓</span></span>} /> : <TLine out={<span style={{ color: CODE.comment, fontStyle: "italic" }}>{step >= 2 ? tr2({ uz: "posilka yo'lda…", ru: "посылка в пути…" }) : tr2({ uz: "konsol kutmoqda…", ru: "консоль ждёт…" })}</span>} />}
            </div>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [taskIdx, setTaskIdx] = useState3(storedAnswer ? 2 : 0);
  const [picked, setPicked] = useState3(null);
  const [shakeId, setShakeId] = useState3(null);
  const shakeTimer = useRef3(null);
  const done = taskIdx >= TASKS.length;
  useEffect4(() => () => clearTimeout(shakeTimer.current), []);
  ...
  return <Stage eyebrow="ID" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `Manzilni toping (${taskIdx}/2)`, ru: `Найдите адрес (${taskIdx}/2)` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Server minglab o'yindan <span className="italic" style={{ color: T.accent }}>keraklisini</span> qanday topadi?</>, ru: <>Как сервер находит <span className="italic" style={{ color: T.accent }}>нужную</span> игру среди тысяч?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "robo-api.uz — jadval", ru: "robo-api.uz — таблица" })}</p>
            <div className="code-box fade-up delay-1" style={{ padding: "11px 13px", display: "flex", flexDirection: "column", gap: 5 }}>
              {ROWS.map((r) => <SrvRow key={r.id} id={r.id} name={r.name} state={!done && cur.target === r.name ? "read" : null} extra={!done && cur.target === r.name ? tr2({ uz: "← shu kerak", ru: "← нужна эта" }) : null} />)}
            </div>
            {!done ? <div className="sk-info" key={taskIdx}><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Topshiriq {taskIdx + 1}/2: <b>{cur.target}</b> ustida ishlamoqchimiz. Qaysi manzilga so'rov yuboramiz?</>, ru: <>Задание {taskIdx + 1}/2: работаем с <b>{cur.target}</b>. На какой адрес отправим запрос?</> })}</p></div> : <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>ID — yozuvning aniq manzili. <span className="mono">/games</span> = hammasi, <span className="mono">/games/2</span> = faqat 2-yozuv. PUT va DELETE <b>doim ID bilan</b> ishlaydi.</>, ru: <>ID — точный адрес записи. <span className="mono">/games</span> = все, <span className="mono">/games/2</span> = только запись 2. PUT и DELETE работают <b>всегда с ID</b>.</> })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Manzilni tanlang", ru: "Выберите адрес" })}</p>
            <div className="fade-up delay-2" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {ROWS.map((r) => <button key={r.id} className={`gchip ${shakeId === r.id ? "shake" : ""}`} onClick={() => tap(r.id)} style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 13, padding: "10px 14px", justifyContent: "flex-start", background: picked === r.id ? T.success : void 0, color: picked === r.id ? "#fff" : void 0 }}>
                  robo-api.uz/games/<b>{r.id}</b>
                </button>)}
            </div>
            {done && <span className="tagpill fade-step" style={{ color: T.success }}>{tr2({ uz: "✓ 2/2 — manzil + ID = aniq nishon", ru: "✓ 2/2 — адрес + ID = точная цель" })}</span>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="PUT" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : found ? { uz: "PUT yuboring", ru: "Отправьте PUT" } : { uz: "Xato yozuvni toping", ru: "Найдите запись с ошибкой" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Serverdagi xatoni qanday <span className="italic" style={{ color: T.accent }}>tuzatamiz</span>?</>, ru: <>Как <span className="italic" style={{ color: T.accent }}>исправить</span> ошибку на сервере?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "robo-api.uz — jadval", ru: "robo-api.uz — таблица" })} {found ? "" : tr2({ uz: "(xatoni bosing)", ru: "(нажмите на ошибку)" })}</p>
            <div className="code-box fade-up delay-1" style={{ padding: "11px 13px", display: "flex", flexDirection: "column", gap: 5 }}>
              {ROWS.map((r) => <div key={r.id} className={shakeId === r.id ? "shake" : ""} onClick={() => tapRow(r.id)} style={{ cursor: found ? "default" : "pointer" }}>
                  <SrvRow id={r.id} name={r.id === 4 && fixed ? "Robo Race" : r.name} state={r.id === 4 ? fixed ? "changed" : found ? "read" : null : null} extra={r.id === 4 ? fixed ? tr2({ uz: "almashtirildi", ru: "заменено" }) : found ? tr2({ uz: "← xato!", ru: "← ошибка!" }) : null : null} />
                </div>)}
            </div>
            {found && !fixed && <pre className="code-box fade-step" style={{ lineHeight: 1.9 }}>
            ...
            {found && !fixed && <button className="btn fade-step" style={{ alignSelf: "flex-start" }} onClick={() => setFixed(true)}>{tr2({ uz: "PUT yuborish", ru: "Отправить PUT" })}</button>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Katalogda", ru: "В каталоге" })}</p>
            <div style={{ maxWidth: 160 }}>
              <RoCard key={fixed ? "ok" : "bad"} name={fixed ? "Robo Race" : "Robo Rase"} emoji="🤖" likes={100} />
            </div>
            {!found && <div className="hint"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: <>Jadvalda bitta nom <b style={{ color: T.ink }}>xato yozilgan</b> — diqqat bilan o'qing va o'sha qatorni bosing.</>, ru: <>В таблице одно название <b style={{ color: T.ink }}>написано с ошибкой</b> — внимательно прочитайте и нажмите на эту строку.</> })}</p></div>}
            {found && !fixed && <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.accent }}>{tr2({ uz: '✓ Topdingiz: "Robo Rase"', ru: "✓ Нашли: «Robo Rase»" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>E'tibor bering: manzilda <b>ID bor</b> — <span className="mono">/games/4</span>. PUT aynan shu yozuvni body'dagi yangisiga almashtiradi.</>, ru: <>Обратите внимание: в адресе <b>есть ID</b> — <span className="mono">/games/4</span>. PUT заменит именно эту запись на новую из body.</> })}</p></div>}
            {fixed && <>
                <div className="code-box" style={{ padding: "9px 13px" }}>
                  <TLine out={<span className="el-in" style={{ display: "inline-block" }}><span style={{ color: VERBS.PUT.col, fontWeight: 700 }}>PUT</span> /games/4 → <span style={{ color: CODE.str }}>{tr2({ uz: "200 OK — almashtirildi ✓", ru: "200 OK — заменено ✓" })}</span></span>} />
                </div>
                <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>PUT formulasi: <b>manzil + ID</b> (qaysi yozuv) + <b>body</b> (yangi varianti). Server eskisini olib tashlab, yangisini qo'yadi.</>, ru: <>Формула PUT: <b>адрес + ID</b> (какая запись) + <b>body</b> (новый вариант). Сервер убирает старое и ставит новое.</> })}</p></div>
              </>}
          </Col>
        </div>
        </Zoomable>
...
var Screen9 = (props) => <QuestionScreen
  ...
  questionText={tr2({ uz: "robo-api.uz/games/7 — bu manzil nima degani?", ru: "robo-api.uz/games/7 — что означает этот адрес?" })}
  audioText="robo-api.uz slesh games slesh yetti — bu manzil nima degani? To'g'ri javobni tanlang."
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <><span className="mono" style={{ color: T.accent }}>robo-api.uz/games/7</span> — bu manzil nima degani?</>, ru: <><span className="mono" style={{ color: T.accent }}>robo-api.uz/games/7</span> — что означает этот адрес?</> })}</h2></>}
  options={[tr2({ uz: "7 ta o'yin olib keladi", ru: "Принесёт 7 игр" }), tr2({ uz: "Katalogning 7-sahifasini ochadi", ru: "Откроет 7-ю страницу каталога" }), tr2({ uz: "7 soniya kutib turadi", ru: "Подождёт 7 секунд" }), tr2({ uz: "ID raqami 7 bo'lgan bitta o'yin", ru: "Одна игра с ID номер 7" })]}
  correctIdx={3}
  explainCorrect={tr2({ uz: "To'g'ri! Manzil oxiridagi raqam — ID, yozuvning pasport raqami. PUT/DELETE shu aniq yozuvga qaratiladi.", ru: "Верно! Число в конце адреса — ID, «номер паспорта» записи. PUT/DELETE нацеливаются именно на эту запись." })}
  explainWrong={{
    0: tr2({ uz: "Yo'q — soni emas, manzili. /games hammasi bo'lardi, /games/7 esa faqat bittasi.", ru: "Нет — это не количество, а адрес. /games — все записи, /games/7 — только одна." }),
    1: tr2({ uz: "Yo'q — bu sahifa emas, serverdagi yozuvning raqami.", ru: "Нет — это не страница, а номер записи на сервере." }),
    2: tr2({ uz: "Yo'q — vaqtga aloqasi yo'q. 7 — yozuvning ID raqami.", ru: "Нет — время тут ни при чём. 7 — это ID записи." }),
    default: tr2({ uz: "/games/7 = ID'si 7 bo'lgan bitta yozuv. PUT va DELETE shunga ishlaydi.", ru: "/games/7 = одна запись с ID 7. PUT и DELETE работают именно так." })
  }}
/>;
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [likes, setLikes] = useState3(storedAnswer ? 93 : 92);
  const [sent, setSent] = useState3(!!storedAnswer);
  const [refreshing, setRefreshing] = useState3(false);
  const [refreshed, setRefreshed] = useState3(!!storedAnswer);
  const timer = useRef3(null);
  ...
  return <Stage eyebrow={tr2({ uz: "Serverda saqlanadi", ru: "Хранится на сервере" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : sent ? { uz: "Endi sahifani yangilang", ru: "Теперь обновите страницу" } : { uz: "Like bosing", ru: "Поставьте лайк" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Like bosganingiz nega <span className="italic" style={{ color: T.accent }}>yo'qolib qolmaydi</span>?</>, ru: <>Почему Ваш лайк <span className="italic" style={{ color: T.accent }}>не исчезает</span>?</> })}</h2></div>
          ...
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
              <div style={{ maxWidth: 160, flexShrink: 0 }}>
                {refreshing ? <SkelCard /> : <RoCard name="Adopt Me!" likes={likes} onLike={like} />}
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <button className="btn-soft" onClick={refresh} disabled={!sent || refreshing}>{tr2({ uz: "⟳ Sahifani yangilash", ru: "⟳ Обновить страницу" })} {refreshed ? "✓" : ""}</button>
                {!sent && <p className="small" style={{ color: T.ink3, fontStyle: "italic", margin: 0 }}>{tr2({ uz: "← avval 👍 ni bosing", ru: "← сначала нажмите 👍" })}</p>}
              </div>
            </div>
            <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "Konsol", ru: "Консоль" })}</p>
            <div className="code-box" style={{ padding: "10px 13px", minHeight: 64 }}>
              {!sent && <TLine out={<span style={{ color: CODE.comment, fontStyle: "italic" }}>{tr2({ uz: "like kutilmoqda…", ru: "ждём лайк…" })}</span>} />}
              ...
              {refreshing && <TLine out={<span><span style={{ color: VERBS.GET.col, fontWeight: 700 }}>GET</span> /games → <span style={{ color: CODE.comment }}>{tr2({ uz: "yuklanmoqda…", ru: "загружается…" })}</span></span>} />}
              {refreshed && sent && <TLine out={<span className="el-in" style={{ display: "inline-block", color: CODE.str }}>{tr2({ uz: "✓ serverdan keldi: likes = 93 — saqlanib qolgan!", ru: "✓ пришло с сервера: likes = 93 — сохранилось!" })}</span>} />}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "robo-api.uz — jadval", ru: "robo-api.uz — таблица" })}</p>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [deleted, setDeleted] = useState3(!!storedAnswer);
  ...
  return <Stage eyebrow="DELETE" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Eski o'yinni o'chiring", ru: "Удалите старую игру" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Keraksiz yozuvni qanday <span className="italic" style={{ color: T.accent }}>olib tashlaymiz</span>?</>, ru: <>Как <span className="italic" style={{ color: T.accent }}>убрать</span> ненужную запись?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            <div className="code-box fade-up delay-1" style={{ padding: "11px 13px", display: "flex", flexDirection: "column", gap: 5 }}>
              ...
              {!deleted ? <SrvRow id={5} name="test test" state={asking ? "read" : null} extra={tr2({ uz: "← eski sinov", ru: "← старый тест" })} /> : <div className="el-in" style={{ opacity: 0.45 }}><SrvRow id={5} name="test test" state="del" extra={tr2({ uz: "o'chirildi", ru: "удалено" })} /></div>}
            </div>
            <pre className="code-box fade-up delay-2" style={{ lineHeight: 1.9, padding: "10px 14px" }}>
              ...
              {"});  "}<Cm>{tr2({ uz: "// body yo'q — faqat nishon", ru: "// body нет — только цель" })}</Cm>
            </pre>
            {!deleted && !asking && <button className="btn fade-up delay-2" style={{ alignSelf: "flex-start" }} onClick={() => {
    setAsking(true);
    setCancelled(false);
  }}>{tr2({ uz: "DELETE yuborish", ru: "Отправить DELETE" })}</button>}
          </Col>
          <Col>
            {asking && !deleted && <div className="frame fade-step" style={{ boxShadow: `inset 0 0 0 1.5px ${VERBS.DELETE.col}, 0 8px 22px -6px rgba(${T.shadowBase},0.2)` }}>
                <p className="note-h" style={{ color: VERBS.DELETE.col }}>{tr2({ uz: "Rostdan o'chirilsinmi?", ru: "Точно удалить?" })}</p>
                <p className="body" style={{ margin: "0 0 12px", color: T.ink }}>{tr2({ uz: <>"test test" (ID 5) butunlay o'chadi. Bu amalni <b>qaytarib bo'lmaydi</b>.</>, ru: <>«test test» (ID 5) исчезнет полностью. Это действие <b>нельзя отменить</b>.</> })}</p>
                <div style={{ display: "flex", gap: 9 }}>
                  <button className="btn-soft" onClick={() => {
    setAsking(false);
    setCancelled(true);
  }}>{tr2({ uz: "Bekor qilish", ru: "Отмена" })}</button>
                  <button className="btn" style={{ background: VERBS.DELETE.col }} onClick={() => {
    setAsking(false);
    setDeleted(true);
  }}>{tr2({ uz: "Ha, o'chirilsin", ru: "Да, удалить" })}</button>
                </div>
              </div>}
...
var Screen12 = (props) => <QuestionScreen
  ...
  questionText={tr2({ uz: "Piggy (ID 3) ni o'chirish uchun to'g'ri so'rov qaysi?", ru: "Какой запрос правильно удалит Piggy (ID 3)?" })}
  audioText="Piggy — ID uch — ni o'chirish uchun to'g'ri so'rov qaysi? To'g'ri javobni tanlang."
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Piggy (ID 3) ni <span className="italic" style={{ color: T.accent }}>o'chirish</span> uchun to'g'ri so'rov qaysi?</>, ru: <>Какой запрос правильно <span className="italic" style={{ color: T.accent }}>удалит</span> Piggy (ID 3)?</> })}</h2></>}
  options={["DELETE /games/3", tr2({ uz: "DELETE /games — hammasiga yuboramiz", ru: "DELETE /games — отправим всем" }), "POST /games/3", "GET /games/3"]}
  correctIdx={0}
  explainCorrect={tr2({ uz: "To'g'ri! Fe'l (DELETE) + aniq nishon (/games/3 — ID). Body kerak emas.", ru: "Верно! Глагол (DELETE) + точная цель (/games/3 — ID). Body не нужен." })}
  explainWrong={{
    1: tr2({ uz: "Xavfli! ID'siz DELETE — qaysi birini? Butun ro'yxatdan ayrilish mumkin. Doim aniq ID bilan.", ru: "Опасно! DELETE без ID — какую именно? Можно лишиться всего списка. Всегда с точным ID." }),
    2: tr2({ uz: "POST — qo'shadi, o'chirmaydi. Fe'l noto'g'ri.", ru: "POST — добавляет, а не удаляет. Глагол не тот." }),
    3: tr2({ uz: "GET — faqat o'qiydi: Piggy ma'lumotini olib keladi, lekin o'chirmaydi.", ru: "GET — только читает: принесёт данные Piggy, но не удалит." }),
    default: tr2({ uz: "O'chirish = DELETE + ID: DELETE /games/3.", ru: "Удалить = DELETE + ID: DELETE /games/3." })
  }}
/>;
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [stage, setStage] = useState3(storedAnswer ? 3 : 0);
  const [postFly, setPostFly] = useState3(!!storedAnswer);
  const [putShake, setPutShake] = useState3(false);
  const [delAsking, setDelAsking] = useState3(false);
  const [delDone, setDelDone] = useState3(!!storedAnswer);
  const shakeTimer = useRef3(null);
  const flyTimer = useRef3(null);
  const done = stage >= 3;
  useEffect4(() => () => {
    clearTimeout(shakeTimer.current);
  ...
  const postSolved = () => {
    setPostFly(true);
    flyTimer.current = setTimeout(() => setStage(1), 950);
  };
  const pickAddr = (ok) => {
    if (ok) setStage(2);
    else {
      clearTimeout(shakeTimer.current);
      setPutShake(true);
      shakeTimer.current = setTimeout(() => setPutShake(false), 450);
    }
  };
  const DEL_LINES = [
  ...
  const delFixed = tr2({ uz: "  method: 'DELETE'   // fe'l DELETE — body ko'tarmaydi", ru: "  method: 'DELETE'   // глагол DELETE — body не несёт" });
  const confirmDelete = () => {
    setDelAsking(false);
    setDelDone(true);
    setStage(3);
  ...
  const rows = [
    ...
    ...stage >= 1 ? [{ id: 6, name: tr2({ uz: "Yangi o'yin", ru: "Новая игра" }), state: stage === 1 ? "new" : null }] : [],
    ...!delDone ? [{ id: 5, name: "test test", state: stage === 2 ? "read" : null }] : [{ id: 5, name: "test test", state: "del", extra: tr2({ uz: "o'chirildi", ru: "удалено" }) }]
  ];
  const STAGE_LBL = [tr2({ uz: "📦 POST — posilkani yig'ing", ru: "📦 POST — соберите посылку" }), tr2({ uz: "✏️ PUT — manzilni tanlang", ru: "✏️ PUT — выберите адрес" }), tr2({ uz: "🗑️ DELETE — yorliqni tuzating", ru: "🗑️ DELETE — исправьте ярлык" })];
  ...
  return <Stage eyebrow={tr2({ uz: "Amaliyot · pult", ru: "Практика · пульт" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `Posilka ${Math.min(stage + 1, 3)}/3`, ru: `Посылка ${Math.min(stage + 1, 3)}/3` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Yuk jo'natish <span className="italic" style={{ color: T.accent }}>pulti</span> — 3 posilkani jo'nating.</>, ru: <><span className="italic" style={{ color: T.accent }}>Пульт</span> отправки грузов — отправьте 3 посылки.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            {!done ? <div className="sk-info" key={stage}><p className="flow-label" style={{ marginBottom: 5 }}>{tr2({ uz: "Posilka", ru: "Посылка" })} {stage + 1}/3</p><p className="body" style={{ margin: 0, color: T.ink, fontWeight: 600 }}>{STAGE_LBL[stage]}</p></div> : <div className="takeaway fade-step"><div className="ta-bulb">🎛️</div><p className="ta-h">{tr2({ uz: "Pult sizniki!", ru: "Пульт Ваш!" })}</p><p className="ta-sub">Create · Read · Update · Delete — CRUD</p></div>}
            {stage === 0 && !done && <>
                <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "Chiplarni to'g'ri tartibda yig'ing", ru: "Соберите чипы в правильном порядке" })}</p>
            ...
            {stage === 1 && <>
                <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: '"Doors" → "Doors 2" — qaysi manzilga PUT?', ru: "«Doors» → «Doors 2» — на какой адрес PUT?" })}</p>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                {putShake && <p className="small" style={{ color: T.accent, fontStyle: "italic", margin: 0 }}>{tr2({ uz: "ID'siz — qaysi birini almashtiray? Aniq manzil kerak.", ru: "Без ID — какую именно заменить? Нужен точный адрес." })}</p>}
              </>}
            {stage === 2 && !delAsking && <>
                <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "Buzuq yorliqni toping va tuzating", ru: "Найдите и исправьте сломанный ярлык" })}</p>
            ...
            {delAsking && <div className="frame fade-step" style={{ boxShadow: `inset 0 0 0 1.5px ${VERBS.DELETE.col}` }}>
                <p className="note-h" style={{ color: VERBS.DELETE.col }}>{tr2({ uz: "Rostdan o'chirilsinmi?", ru: "Точно удалить?" })}</p>
                <p className="body" style={{ margin: "0 0 12px", color: T.ink }}>{tr2({ uz: `"test test" (ID 5) butunlay o'chadi. Qaytarib bo'lmaydi.`, ru: "«test test» (ID 5) исчезнет полностью. Отменить нельзя." })}</p>
                <div style={{ display: "flex", gap: 9 }}>
                  <button className="btn-soft" onClick={() => setDelAsking(false)}>{tr2({ uz: "Bekor qilish", ru: "Отмена" })}</button>
                  <button className="btn" style={{ background: VERBS.DELETE.col }} onClick={confirmDelete}>{tr2({ uz: "Ha, o'chirilsin", ru: "Да, удалить" })}</button>
                </div>
              </div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "robo-api.uz — jadval (jonli)", ru: "robo-api.uz — таблица (живая)" })}</p>
            ...
            <div className="code-box" style={{ padding: "9px 13px", minHeight: 38 }}>
              {done ? <TLine out={<span style={{ color: CODE.str }}>{tr2({ uz: "✓ 3/3 posilka jo'natildi — CRUD to'liq", ru: "✓ 3/3 посылки отправлены — CRUD полный" })}</span>} /> : postFly && stage <= 1 ? <TLine out={<span className="el-in" style={{ display: "inline-block" }}><span style={{ color: VERBS.POST.col, fontWeight: 700 }}>POST</span> /games → <span style={{ color: CODE.str }}>201 Created ✓</span></span>} /> : <TLine out={<span style={{ color: CODE.comment, fontStyle: "italic" }}>{tr2({ uz: `posilka ${stage + 1}/3 kutilmoqda…`, ru: `ждём посылку ${stage + 1}/3…` })}</span>} />}
            </div>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const TASKS = [
    { id: "t1", label: tr2({ uz: "O'yin qo'shish formasi yasa — tugma bosilganda POST ketsin", ru: "Сделай форму добавления игры — по кнопке пусть уходит POST" }), plan: [tr2({ uz: "Forma: name + emoji uchun input va state", ru: "Форма: input и state для name + emoji" }), tr2({ uz: "Tugmada: fetch POST + body: JSON.stringify(yangi)", ru: "В кнопке: fetch POST + body: JSON.stringify(yangi)" })], code: <>{"fetch(url, { "}<At>method</At>{": "}<St>'POST'</St>{", "}<At>body</At>{": JSON.stringify(yangi) })"}</> },
    { id: "t2", label: tr2({ uz: "Like tugmasi serverda saqlanadigan bo'lsin", ru: "Пусть лайк сохраняется на сервере" }), plan: [tr2({ uz: "onLike ichida PUT yuboraman: /games/ID", ru: "Внутри onLike отправлю PUT: /games/ID" }), tr2({ uz: "body'da yangi likes soni ketadi", ru: "В body уйдёт новое число likes" })], code: <>{"fetch(url + "}<St>'/'</St>{" + g.id, { "}<At>method</At>{": "}<St>'PUT'</St>{", "}<At>body</At>{": … })"}</> },
    { id: "t3", label: tr2({ uz: "O'chirish tugmasi qo'sh — lekin avval tasdiqlash so'rasin", ru: "Добавь кнопку удаления — но сначала пусть спросит подтверждение" }), plan: [tr2({ uz: "Avval confirm: 'Rostdan o'chirilsinmi?'", ru: "Сначала confirm: «Точно удалить?»" }), tr2({ uz: "Ha bo'lsa: fetch DELETE /games/ID — body'siz", ru: "Если да: fetch DELETE /games/ID — без body" })], code: <>{"if (tasdiq) fetch(url + "}<St>'/'</St>{" + g.id, { "}<At>method</At>{": "}<St>'DELETE'</St>{" })"}</> }
  ];
  ...
  return <Stage eyebrow={tr2({ uz: "Keyingi qadam · AI", ru: "Следующий шаг · AI" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Agent bilan ishlab ko'ring", ru: "Поработайте с агентом" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>To'liq boshqaruvni <span className="italic" style={{ color: T.accent }}>AI bilan</span> saytga qo'shamizmi?</>, ru: <>Добавим на сайт полное управление <span className="italic" style={{ color: T.accent }}>вместе с AI</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            {!cur && <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Yuqoridan bitta buyruqni tanlang", ru: "Выберите одну команду сверху" })}</p></div>}
            {cur && <div className="ai-card fade-step" key={task || "stored"}>
                <div className="ai-row"><span className="ai-badge">{tr2({ uz: "Agent", ru: "Агент" })}</span><span className="ai-bubble">{phase === "planned" ? tr2({ uz: "Mana rejam — tasdiqlaysizmi?", ru: "Вот мой план — утверждаете?" }) : phase === "building" ? tr2({ uz: "Yozyapman…", ru: "Пишу…" }) : tr2({ uz: "Bajardim — kodni tekshiring", ru: "Готово — проверьте код" })}</span></div>
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "2. Natija — localhost:5173", ru: "2. Результат — localhost:5173" })}</p>
            <Win title="robo-games — localhost:5173" minH={130}>
              {done && cur ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {cur.id === "t1" && <>
                      <div style={{ display: "flex", gap: 7, alignItems: "center" }}>
                        ...
                        <span style={{ fontFamily: "'Manrope',sans-serif", fontWeight: 700, fontSize: 11, background: T.accent, color: "#fff", borderRadius: 7, padding: "6px 10px" }}>{tr2({ uz: "+ Qo'shish", ru: "+ Добавить" })}</span>
                      </div>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                  ...
                  {cur.id === "t2" && <>
                      <p style={{ fontFamily: "'Manrope',sans-serif", fontWeight: 800, fontSize: 13, color: T.ink, margin: 0 }}>{tr2({ uz: "👍 bosildi", ru: "👍 нажат" })} <span className="mono small" style={{ color: T.success, fontWeight: 700 }}>{tr2({ uz: "→ PUT → serverda", ru: "→ PUT → на сервере" })}</span></p>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                        <RoCard name="Adopt Me!" likes={93} /><RoCard name="Doors" /><RoCard name="Piggy" />
                      </div>
                    </>}
                  {cur.id === "t3" && <>
                      <div style={{ borderRadius: 9, padding: "8px 11px", background: VERBS.DELETE.soft, fontFamily: "'Manrope',sans-serif", fontSize: 11.5, color: T.ink }}>{tr2({ uz: "Rostdan o'chirilsinmi?", ru: "Точно удалить?" })} <b>{tr2({ uz: "[Bekor]", ru: "[Отмена]" })}</b> <b style={{ color: VERBS.DELETE.col }}>{tr2({ uz: "[Ha]", ru: "[Да]" })}</b></div>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                        <RoCard name="Adopt Me!" /><RoCard name="Doors" /><div style={{ borderRadius: 12, border: `1.5px dashed ${T.ink3}`, display: "flex", alignItems: "center", justifyContent: "center", minHeight: 86 }}><span className="small" style={{ color: T.ink3, fontStyle: "italic" }}>{tr2({ uz: "o'chdi", ru: "удалено" })}</span></div>
                      </div>
                    </>}
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [picked, setPicked] = useState3(storedAnswer ? "opts" : null);
  const [fixed, setFixed] = useState3(!!storedAnswer);
  ...
  return <Stage eyebrow={tr2({ uz: "Debugging", ru: "Дебаггинг" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : found ? { uz: "Endi tuzating", ru: "Теперь исправьте" } : { uz: "Yetishmayotganini toping", ru: "Найдите, чего не хватает" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>AI yordam beradi — siz esa <span className="italic" style={{ color: T.accent }}>tekshirasiz</span>.</>, ru: <>AI помогает — а Вы <span className="italic" style={{ color: T.accent }}>проверяете</span>.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="ai-card fade-up delay-2">
              <div className="ai-row"><span className="ai-badge">AI</span><span className="ai-bubble">{tr2({ uz: "Qo'shish kodini yozdim:", ru: "Написал код добавления:" })}</span></div>
              <div className="ai-code">
                <div className={`ai-line ${picked === "obj" ? "ok" : ""}`} onClick={() => {
    if (!found) setPicked("obj");
  }}><Jx>{"const"}</Jx>{" yangi = { name: "}<St>'Super Kart'</St>{" };"}</div>
                {!fixed ? <div className={`ai-line ${found ? "bad" : ""}`} onClick={() => {
    if (!found) setPicked("opts");
  }}>{"fetch(url, {"}</div> : <div className="ai-line ok el-in">{"fetch(url, {"}</div>}
                {fixed && <div className="ai-line ok el-in">{"  "}<At>method</At>{": "}<St>'POST'</St>{",  "}<Cm>{tr2({ uz: "// yetishmagan fe'l!", ru: "// недостающий глагол!" })}</Cm></div>}
                <div className={`ai-line ${picked === "body" ? "ok" : ""}`} onClick={() => {
    if (!found) setPicked("body");
  }}>{"  "}<At>body</At>{": JSON.stringify(yangi)"}</div>
                <div className={`ai-line ${picked === "end" ? "ok" : ""}`} onClick={() => {
    if (!found) setPicked("end");
  }}>{"});"}</div>
              </div>
              {!found && <p className="ai-prompt">{tr2({ uz: "Fe'l qayerda yozilishi kerak edi? O'sha qatorni bosing.", ru: "Где должен был стоять глагол? Нажмите на ту строку." })}</p>}
              {found && !fixed && <button className="btn fade-step" style={{ alignSelf: "flex-start" }} onClick={() => setFixed(true)}>{tr2({ uz: "🔧 method: 'POST' qo'shish", ru: "🔧 Добавить method: 'POST'" })}</button>}
              {fixed && <p className="ai-prompt" style={{ color: T.success, fontStyle: "normal", fontWeight: 600 }}>{tr2({ uz: "✓ Tuzatildi — endi server fe'lni tushunadi!", ru: "✓ Исправлено — теперь сервер понимает глагол!" })}</p>}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Konsol va jadval", ru: "Консоль и таблица" })}</p>
            <div className="code-box" style={{ padding: "9px 13px" }}>
              {!fixed ? <TLine out={<span><span style={{ color: VERBS.GET.col, fontWeight: 700 }}>GET</span> /games → <span style={{ color: CODE.str }}>200 OK</span> <span style={{ color: CODE.comment }}>{tr2({ uz: "…lekin hech narsa qo'shilmadi", ru: "…но ничего не добавилось" })}</span></span>} /> : <TLine out={<span className="el-in" style={{ display: "inline-block" }}><span style={{ color: VERBS.POST.col, fontWeight: 700 }}>POST</span> /games → <span style={{ color: CODE.str }}>201 Created ✓</span></span>} />}
            </div>
            <div className="code-box" style={{ padding: "11px 13px", display: "flex", flexDirection: "column", gap: 5 }}>
              <SrvRow id={1} name="Adopt Me!" />
              <SrvRow id={2} name="Doors" />
              {fixed && <SrvRow id={6} name="Super Kart" state="new" extra={tr2({ uz: "+ yangi", ru: "+ новое" })} />}
            </div>
            {!found && (picked === "obj" || picked === "body" || picked === "end" ? <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Bu qator to'g'ri{picked === "obj" ? " — obyekt tayyor" : picked === "body" ? " — yuk stringify bilan joyida" : ""}. Lekin sozlamalar qutisida <b>nima yo'q</b>? Konsol GET deyapti…</>, ru: <>Эта строка верная{picked === "obj" ? " — объект готов" : picked === "body" ? " — груз со stringify на месте" : ""}. Но <b>чего нет</b> в коробке настроек? Консоль говорит GET…</> })}</p></div> : <div className="hint"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: <>Sir shu yerda: fe'l yozilmasa, fetch <b style={{ color: T.ink }}>jimgina GET yuboradi</b>. GET esa hech narsa qo'shmaydi.</>, ru: <>Секрет вот в чём: если глагол не написан, fetch <b style={{ color: T.ink }}>молча отправляет GET</b>. А GET ничего не добавляет.</> })}</p></div>)}
            {found && !fixed && <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.accent }}>{tr2({ uz: "✓ Topdingiz!", ru: "✓ Нашли!" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Sozlamalar qutisi ochilgan-u, ichida <b>method yo'q</b>! Fe'lsiz fetch sukut bo'yicha GET ishlatadi. Chapdagi tugma bilan tuzating →</>, ru: <>Коробка настроек открыта, а внутри <b>нет method</b>! Без глагола fetch по умолчанию использует GET. Исправьте кнопкой слева →</> })}</p></div>}
            {fixed && <div className="takeaway fade-step"><div className="ta-bulb">🛠️</div><p className="ta-h">{tr2({ uz: "Ko'rinmas xatoni ham topdingiz!", ru: "Вы нашли даже невидимую ошибку!" })}</p><p className="ta-sub">{tr2({ uz: "Konsoldagi fe'lni o'qish — dasturchilar odati", ru: "Читать глагол в консоли — привычка профессионала" })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen16 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const EMOJIS = ["🤖", "🏎️", "🐉", "⚽", "🎯"];
  const [gname, setGname] = useState3(typeof storedAnswer?.gameName === "string" ? storedAnswer.gameName : "");
  const [emoji, setEmoji] = useState3(storedAnswer?.gameEmoji || "🤖");
  ...
  const [loaded2, setLoaded] = useState3(!!storedAnswer);
  const timer = useRef3(null);
  ...
  useEffect4(() => {
    if (valid && !passed) {
      setPassed(true);
      onAnswer(screen, { stage: "final", screenIdx: screen, question: tr2({ uz: "O'z o'yiningizni POST bilan serverga qo'shing", ru: "Добавьте свою игру на сервер через POST" }), studentAnswer: `${gname} | ${value}`, gameName: gname, gameEmoji: emoji, correct: true, firstAttemptCorrect: true, solved: true, picked: value });
  ...
  return <Stage eyebrow={tr2({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!passed} label={passed ? { uz: "Davom etish", ru: "Продолжить" } : nameOk ? { uz: "Fe'lni yozing", ru: "Напишите глагол" } : { uz: "O'yiningizga nom bering", ru: "Дайте игре название" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Va'da qilingan daqiqa: <span className="italic" style={{ color: T.accent }}>o'z o'yiningizni</span> serverga qo'shing.</>, ru: <>Обещанный момент: добавьте <span className="italic" style={{ color: T.accent }}>свою игру</span> на сервер.</> })}</h2></div>
    ...
    placeholder={tr2({ uz: "O'yiningiz nomi…", ru: "Название Вашей игры…" })}
    spellCheck={false}
  ...
  />
              {EMOJIS.map((e) => <button key={e} className="gchip" style={emoji === e ? { background: T.accent } : void 0} onClick={() => setEmoji(e)}>{e}</button>)}
            </div>
            ...
            <div className="fade-up delay-2" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <span className="tagpill" style={{ opacity: hasMethod ? 1 : 0.4 }}>{hasMethod ? "✓" : "1"} {tr2({ uz: "method: — kichik harf", ru: "method: — строчными" })}</span>
              <span className="tagpill" style={{ opacity: hasPost && !lowerPost ? 1 : 0.4 }}>{hasPost && !lowerPost ? "✓" : "2"} {tr2({ uz: "'POST' — katta, qo'shtirnoqda", ru: "'POST' — большими, в кавычках" })}</span>
              <span className="tagpill" style={{ opacity: hasComma ? 1 : 0.4 }}>{hasComma ? "✓" : "3"} {tr2({ uz: ", — oxirida vergul", ru: ", — запятая в конце" })}</span>
            </div>
            {lowerPost && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Fe'llar doim <b>KATTA harflarda</b>: <span className="mono">'POST'</span> ('post' emas).</>, ru: <>Глаголы всегда <b>БОЛЬШИМИ буквами</b>: <span className="mono">'POST'</span> (не 'post').</> })}</p></div>}
            {capMethod && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <><span className="mono">method</span> — kichik harf bilan yoziladi.</>, ru: <><span className="mono">method</span> пишется со строчной буквы.</> })}</p></div>}
            {noQuotes && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Fe'l — matn: <b>qo'shtirnoq ichida</b> yozing: <span className="mono">'POST'</span></>, ru: <>Глагол — это текст: пишите <b>в кавычках</b>: <span className="mono">'POST'</span></> })}</p></div>}
            {lineOk && !nameOk && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Qator to'g'ri! Endi tepada o'yiningizga <b>nom bering</b> — posilka bo'sh ketmasin.</>, ru: <>Строка верная! Теперь дайте игре <b>название</b> сверху — чтобы посылка не улетела пустой.</> })}</p></div>}
            {passed && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "✓ Posilka yo'lda! Server qabul qilyapti…", ru: "✓ Посылка в пути! Сервер принимает…" })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "natija — localhost:5173", ru: "результат — localhost:5173" })}</p>
            <Win title="robo-games — localhost:5173" minH={130}>
              {!passed && <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center", lineHeight: 1.5 }}>{tr2({ uz: "4-qator yozilmaguncha o'yiningiz shu yerda paydo bo'lmaydi…", ru: "Пока не написана строка 4, Ваша игра здесь не появится…" })}</p>}
              {passed && !loaded2 && <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                  <SkelCard /><SkelCard /><SkelCard />
                </div>}
              {passed && loaded2 && <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 9 }}>
                  <RoCard name="Adopt Me!" />
                  <RoCard name="Doors" />
            ...
            {passed && loaded2 && <>
                ...
                <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>"<b>{gname}</b>" endi serverda — katalogda hammaga ko'rinadi. O'tgan dars va'dasi <b>bajarildi</b>!</>, ru: <>«<b>{gname}</b>» теперь на сервере — её видят все в каталоге. Обещание прошлого урока <b>выполнено</b>!</> })}</p></div>
              </>}
          </Col>
        </div>
        </Zoomable>
...
var MentorPracticeStats = ({ live, screen, label }) => {
  ...
  return <div className="lp-mstats fade-up">
      <div className="card-lbl" style={{ color: T.blue }}>{tr2(label || { uz: "👀 Kim bajardi", ru: "👀 Кто выполнил" })} — {doers.length}/{players.length}</div>
...
function ScreenLivePractice({ title, task, checklist, statsLabel, screen, storedAnswer, onAnswer, onNext, onPrev, live }) {
  ...
  return <Stage eyebrow={tr2({ uz: "Amaliyot · VS Code", ru: "Практика · VS Code" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Avval bajaring", ru: "Сначала выполните" }} onClick={onNext} /></>}>
    ...
    return <button key={i} className={`lp-step ${on ? "on" : ""}`} onClick={() => toggle(i)}>
            ...
            {done && !isMentor && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Juda yaxshi! Vazifani bajardingiz. Ustoz tekshirib, keyingi qadamga o'tkazadi.", ru: "Отлично! Задание выполнено. Наставник проверит и переведёт на следующий шаг." })}</p></div>}
          </Col>
        </div>
      </div>
    </Stage>;
...
var Screen17 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  const RECAP = [
    tr2({ uz: "4 fe'l: GET o'qiydi · POST qo'shadi · PUT almashtiradi · DELETE o'chiradi", ru: "4 глагола: GET читает · POST добавляет · PUT заменяет · DELETE удаляет" }),
    tr2({ uz: "Fe'l sozlamalar qutisida: fetch(url, { method: 'POST', … })", ru: "Глагол — в коробке настроек: fetch(url, { method: 'POST', … })" }),
    tr2({ uz: "Yuk — body: JSON.stringify(obyekt) — .json()ning teskarisi", ru: "Груз — body: JSON.stringify(объект) — операция, обратная .json()" }),
    tr2({ uz: "PUT va DELETE — doim ID bilan: /games/2", ru: "PUT и DELETE — всегда с ID: /games/2" }),
    tr2({ uz: "201 Created = yaratildi; DELETE qaytarilmaydi — avval tasdiqlash", ru: "201 Created = создано; DELETE не отменить — сначала подтверждение" }),
    tr2({ uz: "Bularning umumiy nomi — CRUD", ru: "Общее имя всего этого — CRUD" })
  ];
  const HOMEWORK = [
    { b: tr2({ uz: "Qo'shish formasi", ru: "Форма добавления" }), t: tr2({ uz: "— robo-games loyihangizga agent bilan forma qo'shing: nom + emoji → POST bilan serverga ketsin", ru: "— добавьте с агентом форму в свой проект robo-games: название + эмодзи → пусть уходит POST на сервер" }) },
    { b: tr2({ uz: "Jonli like", ru: "Живой лайк" }), t: tr2({ uz: "— like tugmasi PUT yuborsin; sahifani yangilab, saqlanganini tekshiring", ru: "— пусть кнопка лайка отправляет PUT; обновите страницу и проверьте, что он сохранился" }) },
    { b: tr2({ uz: "Xavfsiz o'chirish", ru: "Безопасное удаление" }), t: tr2({ uz: `— DELETE tugmasi qo'shing, lekin avval "Ishonchingiz komilmi?" so'ralsin`, ru: "— добавьте кнопку DELETE, но пусть сначала спрашивает «Вы уверены?»" }) }
  ];
  ...
  return <Stage eyebrow={tr2({ uz: "Tayyor", ru: "Готово" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Server endi sizning <span className="italic" style={{ color: T.accent }}>buyrug'ingizda</span>.</>, ru: <>Сервер теперь <span className="italic" style={{ color: T.accent }}>под Вашим командованием</span>.</> })}</h2>{
...
var ACH_TRIGGERS = { s4: "poster", s5b: "packer", s13: "dispatcher" };
function AchCelebrate({ ach, onDone }) {
  useEffect4(() => {
    const t = setTimeout(onDone, 4e3);
    return () => clearTimeout(t);
  }, []);
  return <div className="acu-overlay" onClick={onDone} role="status" aria-label={`${tr2({ uz: "Yangi nishon:", ru: "Новая награда:" })} ${tr2(ach.name)}`}>
...
var Q_LABELS = { 4: { uz: "1 — POST (qo'shish)", ru: "1 — POST (добавление)" }, 6: "2 — body/stringify", 11: { uz: "3 — manzil/ID", ru: "3 — адрес/ID" }, 15: "4 — DELETE", 19: { uz: "5 — Yakuniy amaliy", ru: "5 — Финальная практика" } };
var INLINE_KEYS = { s4: 1, s5b: 2, s9: 3, s12: 0, s16: -1, practice: -1 };
var ScreenPodium = ({ screen, answers, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} /></>}>
    ...
    return <div key={q} className="qstat-row">
              ...
              {live.mode === "mentor" && <p className="small" style={{ margin: "10px 0 0", color: T.ink2 }}>{tr2({ uz: "⚠️ belgili savollar — sinf qiynalgan mavzular. Qayta tushuntiring.", ru: "Вопросы со значком ⚠️ — темы, где класс споткнулся. Советуем объяснить их ещё раз." })}</p>}
            </div>
          </>}
      </div>
    </Stage>;
    ...
    /* ===== YAKUN — PODIUM ===== */
      ...
      {phase === "done" && <div className="qz-view fade-step">
          ...
          {solo ? <div className="qz-solo-res">
              ...
              <p className="qz-sub">{tr2({ uz: "ball", ru: "баллов" })} · {soloScore.ok}/{QUIZ_BANK.length} {tr2({ uz: "to'g'ri", ru: "верно" })}{soloScore.maxStreak >= 2 ? ` · ${tr2({ uz: "eng uzun streak", ru: "лучший streak" })} 🔥x${soloScore.maxStreak}` : ""}</p>
...
function ReactApiPostLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  return <LangContext.Provider value={lang}>
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={{ uz: "API darsi", ru: "Урок API" }} /> : <>
