...
//  Manba:  src/3-Modull/PmLesson8.homework.jsx
//  Kompilyator: TASHQI MODUL — https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx src/3-Modull/PmLesson8.homework.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/3-Modull/PmLesson8.homework.jsx
...
var katakOf = (k) => KATAKLAR.find((x) => x.k === k) || null;
...
var newDone = (d) => nomOk((d || {}).nom) && ((d || {}).foyda === "kop" || (d || {}).foyda === "kam") && ((d || {}).vaqt === "tez" || (d || {}).vaqt === "uzoq");
var allIshlar = (data) => {
...
var rejaIdx = (data) => allIshlar(data).map((it, i) => it.katak === "reja" && nomOk(it.nom) ? i : -1).filter((i) => i >= 0);
var BoardCard = ({ data }) => {
  ...
  return <div className="ac">
    ...
    return <div key={kk.k} className="ac-row">
            <span className="ac-k">{kk.ic}</span>
            <span className="ac-v">
              <span className="ac-cell">{tr(kk.t)}</span>
              {list.map((it) => <span key={it.i} className="ac-item">
                  <b className={it.isNew ? "ac-new" : ""}>{it.nom.trim()}</b>
                  {kk.k === "reja" && typeof split.src === "number" && split.src === it.i && (split.part || "").trim() && <span className="ac-part">{Ico.scissors(11)} {tr({ uz: "bir haftalik bo'lak:", ru: "недельная часть:" })} {(split.part || "").trim()}</span>}
                </span>)}
            </span>
          </div>;
  })}
  ...
  return <div className="col">
    ...
    return <div key={i} className="wrow" style={{ borderBottom: i < 2 ? `1px solid ${T.line}` : "none" }}>
              ...
              <div className="chips" role="radiogroup">
                {KATAKLAR.map((kk) => <button key={kk.k} type="button" role="radio" aria-checked={it.katak === kk.k} className={`chip sm ${it.katak === kk.k ? "on" : ""}`} onClick={() => setAt(i, { katak: kk.k })}>
...
var itemsDone = (arr) => [0, 1, 2].every((i) => {
  const it = (arr || [])[i] || {};
  return nomOk(it.nom) && !!katakOf(it.katak);
});
var StageNew = ({ data, setData, n, otherNom }) => {
  const nom = data.nom || "";
  const nOk = nomOk(nom);
  ...
  return <div className="col">
      <h2 className="title h-title h-center fade-up">
        {n === 1 ? tr({ uz: <>Saytga <span className="italic" style={{ color: T.accent }}>yangi ish</span> qo'shing</>, ru: <>Добавьте сайту <span className="italic" style={{ color: T.accent }}>новую задачу</span></> }) : tr({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>ikkinchi</span> yangi ish</>, ru: <>Теперь <span className="italic" style={{ color: T.accent }}>вторая</span> новая задача</> })}
      </h2>
      ...
      <div className="frame fade-up d1">
        ...
        <div className="chips" role="radiogroup">
          {FOYDA_OPTS.map((o) => <button key={o.key} type="button" role="radio" aria-checked={foyda === o.key} className={`chip ${foyda === o.key ? "on" : ""}`} onClick={() => setData({ ...data, foyda: o.key })}>
        ...
        <div className="chips" role="radiogroup">
          {VAQT_OPTS.map((o) => <button key={o.key} type="button" role="radio" aria-checked={vaqt === o.key} className={`chip ${vaqt === o.key ? "on" : ""}`} onClick={() => setData({ ...data, vaqt: o.key })}>
              <span className="chip-ic" aria-hidden="true">{o.ic}</span>
              <span>{tr(o.label)}</span>
            </button>)}
        </div>
        {kk && <div className="frame-success fade-step" style={{ marginTop: 14 }}>
...
var stageNewDone = (d, other) => newDone(d) && !(other && low((d || {}).nom) === low(other));
var QZ_HOLD_MS = 1500;
var QZ_OUT_MS = 380;
var splitDone = (data) => {
  const rl = rejaIdx(data);
  if (!rl.length) return true;
  const sp = (data.sum || {}).split || {};
  return rl.includes(sp.src) && (sp.part || "").trim().length >= PART_MIN;
};
var StageSum = ({ data, setData, full }) => {
  const d = data || {};
  const sp = d.split || {};
  ...
  return <div className="col">
      ...
      <div className="frame fade-up d1">
        {needSplit ? <>
            ...
            <div className="chips" role="radiogroup">
              {rl.map((i) => <button key={i} type="button" role="radio" aria-checked={sp.src === i} className={`chip ${sp.src === i ? "on" : ""}`} onClick={() => setSplit({ src: i })}>
                  <span>{(all[i].nom || "").trim()}</span>
                </button>)}
            </div>
            {typeof sp.src === "number" && rl.includes(sp.src) && <div style={{ marginTop: 12 }}>
...
var sumDone = (d, full) => splitDone(full) && QUIZ.every((q) => ((d || {}).ans || {})[q.id] === q.correct);
var STAGES = [
...
function PmLesson8Homework({ lang: langProp, onFinished }) {
  ...
  return <div className="hw-root">
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult ? <StageResult data={data} goStage={setStage} onFinishClick={finish} finished={finished} onCelebrated={() => setDataRaw((prev) => prev.celebrated ? prev : { ...prev, celebrated: true })} /> : cur.key === "items" ? <StageItems items={data.items} setItems={setItems} /> : cur.key === "new1" ? <StageNew data={data.new1 || {}} setData={setStageData("new1")} n={1} otherNom={null} /> : cur.key === "new2" ? <StageNew data={data.new2 || {}} setData={setStageData("new2")} n={2} otherNom={(data.new1 || {}).nom || ""} /> : <StageSum data={data.sum || {}} setData={setStageData("sum")} full={data} />}
        </div>
      </div>
