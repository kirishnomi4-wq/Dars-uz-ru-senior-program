...
//  Manba:  src/4c-Modull/PmLesson18.homework.jsx
//  Kompilyator: TASHQI MODUL — https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx src/4c-Modull/PmLesson18.homework.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/4c-Modull/PmLesson18.homework.jsx
...
var sababUzun = (s) => (s || "").trim().length >= SABAB_MIN;
var odamBor = (s) => sababUzun(s) && anyTest(ODAM_RE, normSab(s));
var boshMi = (s) => sababUzun(s) && anyTest(BOSH_RE, normSab(s)) && !odamBor(s);
var pastMi = (id, sonVal) => sonVal !== null && (id === "vaqt" && sonVal < 1 || id === "xato" && sonVal === 0);
var qoidaFull = (r) => sonOqi((r || {}).chegara) !== null && sababUzun((r || {}).sabab);
...
var yuklashHukm = (holat, vaqtMs, chegaraS) => {
  if (holat === null || vaqtMs === null || chegaraS === null) return null;
  return holat >= 400 || vaqtMs > chegaraS * 1e3;
};
var SignalCard = ({ data }) => {
  ...
  return <div className="ac">
      ...
      {rows.map(({ m, r }, i) => <div key={i} className="ac-row">
    ...
    return <div key={`y${i}`} className="ac-row" style={{ borderLeftColor: sig ? T.err : T.success }}>
      ...
      {((data.solish || {}).sabab || "").trim() && <div className="ac-row">
...
var StageRows = ({ rows, setRows }) => {
  ...
  return <div className="col">
    ...
    return <div key={m.id} className="wrow" style={{ borderBottom: i < 2 ? `1px solid ${T.line}` : "none" }}>
...
var rowsDone = (rows) => [0, 1, 2].every((i) => qoidaFull((rows || [])[i] || {}));
var StageOlchov = ({ data, setData }) => {
...
var StageSolish = ({ data, setData, rows, olchov }) => {
...
function PmLesson18Homework({ lang: langProp, onFinished }) {
  ...
  return <div className="hw-root">
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult ? <StageResult data={data} goStage={setStage} onFinishClick={finish} finished={finished} onCelebrated={() => setDataRaw((prev) => prev.celebrated ? prev : { ...prev, celebrated: true })} /> : cur.key === "rows" ? <StageRows rows={data.rows} setRows={setRows} /> : cur.key === "olchov" ? <StageOlchov data={data.olchov || {}} setData={setStageData("olchov")} /> : cur.key === "solish" ? <StageSolish data={data.solish || {}} setData={setStageData("solish")} rows={data.rows} olchov={data.olchov} /> : <StageSum data={data.sum || {}} setData={setStageData("sum")} />}
        </div>
      </div>
