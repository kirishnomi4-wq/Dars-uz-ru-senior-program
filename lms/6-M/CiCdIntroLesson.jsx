...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и число ✅/❌ скрыты — по нажатию «Открыть результат» всё появится сразу и у вас, и на экранах учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верно — класс не понял эту тему. Перед продолжением рекомендуем короткое повторение.</> })}</p>
    ...
    return <button key={i} className={cls} disabled={solved || isMentorLive} onClick={() => pick(i)} style={{ padding: "clamp(13px,1.9vw,17px) clamp(15px,2.2vw,20px)", fontSize: "clamp(15px,1.85vw,17px)", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <FeedbackBlock show={isMentorLive ? mReveal : picked !== null} isCorrect={isMentorLive ? true : solved && !wrongLocked} neutral={waiting}>
          <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: waiting ? T.blue : isMentorLive || solved && !wrongLocked ? T.success : T.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>
            {isMentorLive ? <>{tr({ uz: "✓ To'g'ri javob:", ru: "✓ Правильный ответ:" })} {String.fromCharCode(65 + correctIdx)} — {fmtCode(tr(options[correctIdx]))}</> : waiting ? tr({ uz: "📨 Javobingiz qabul qilindi", ru: "📨 Ваш ответ принят" }) : wrongLocked ? <>{tr({ uz: "To'g'ri javob:", ru: "Правильный ответ:" })} {String.fromCharCode(65 + correctIdx)} — {fmtCode(tr(options[correctIdx]))}</> : solved ? tr({ uz: "To'g'ri", ru: "Верно" }) : tr({ uz: "Qaytadan urinib ko'ring", ru: "Попробуйте ещё раз" })}
          </p>
          <p className="body" style={{ margin: 0 }}>
            {isMentorLive ? fmtCode(tr(explainCorrect)) : waiting ? tr({ uz: "Hozir to'g'ri javobni bilib olasiz.", ru: "Сейчас узнаете правильный ответ." }) : wrongLocked ? fmtCode(tr(explainWrong[picked] ?? explainWrong.default)) : solved ? fmtCode(tr(explainCorrect)) : fmtCode(tr(explainWrong[picked] ?? explainWrong.default))}
          </p>
          {
...
var Term = ({ title = "Terminal", children, minH }) => <div className="term"><div className="term-bar"><span className="bb-dots"><i /><i /><i /></span><span className="term-title">{tr(title)}</span></div><div className="term-body" style={{ minHeight: minH }}>{children}</div></div>;
...
var Belt = ({ statuses = {} }) => {
  ...
  return <div className="pipe-track">
      ...
      <div className="pipe">
        {POINTS.map((p, i) => <React.Fragment key={p.id}>
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Kirish", ru: "Введение" })} screen={screen} scrollSignal={sc} navContent={<NavNext optionalLive disabled={picked === null} label={tr({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up" style={{ maxWidth: 880 }}>{tr({ uz: <>Kodni yangiladingiz — endi uni internetga chiqarish uchun <span className="italic" style={{ color: T.accent }}>nechta amal</span> kerak?</>, ru: <>Вы обновили код — сколько теперь нужно <span className="italic" style={{ color: T.accent }}>действий</span>, чтобы выложить его в интернет?</> })}</h1>
        ...
        <Zoomable><Split>
          <Col>
            <Term title={{ uz: "qo'lda uchirish", ru: "запуск вручную" }} minH={150}>
              <TLine cmd="npm test" />
              {tried && <>
                <TLine out={tr({ uz: "✓ testlar yashil", ru: "✓ тесты зелёные" })} col={CODE.str} />
                <TLine cmd="npm run build" />
                <TLine cmd="scp ./dist server:/var/www" />
                <TLine cmd="deploy" />
                <TLine out={tr({ uz: "✓ chamadon qo'lda yetkazildi (5 qadam)", ru: "✓ чемодан доставлен вручную (5 шагов)" })} col={CODE.str} />
              </>}
            </Term>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Reja", ru: "План" })} screen={screen} mentorStatic scrollSignal={showSteps} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive label={tr({ uz: "Boshlaymiz →", ru: "Начнём →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Deploy'ni har safar qo'lda qilamizmi — yoki <span className="italic" style={{ color: T.accent }}>lentaga topshiramizmi</span>?</>, ru: <>Каждый раз деплоим вручную — или <span className="italic" style={{ color: T.accent }}>доверим ленте</span>?</> })}</h2></div>
        ...
        {!isNarrow ? <Zoomable><Split>{Preview}{StepsB}</Split></Zoomable> : !showSteps ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>{Preview}<button className="btn" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(true)}>{tr({ uz: "4 qadamni ko'rish", ru: "Посмотреть 4 шага" })}</button></div> : <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}><button className="btn-soft" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(false)}>{tr({ uz: "↩ Lentani ko'rish", ru: "↩ Посмотреть ленту" })}</button>{StepsB}</div>}
      </div>
    </Stage>;
};
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState(storedAnswer ? new Set(POINTS.map((p) => p.id)) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={tr({ uz: "Muammo · qo'lda", ru: "Проблема · вручную" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: `5 nuqtani qo'lda o'tkazing (${seen.size}/5)`, ru: `Проведите 5 точек вручную (${seen.size}/5)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1 cj-items">
              {POINTS.map((p) => <button key={p.id} className={`itm-card ${seen.has(p.id) ? "" : "tap-hint"}`} onClick={() => tap(p.id)} style={seen.has(p.id) ? { boxShadow: `inset 0 0 0 1.5px ${T.success}` } : void 0}><span className="itm-ico">{p.icon}</span><span className="itm-nm">{seen.has(p.id) ? "✓ " : ""}{tr(p.label)}</span></button>)}
            </div>
            <div className="timer-row fade-up delay-2">
              <div className={`timer-chip ${minutes >= 12 ? "danger" : ""}`}><span className="tc-lbl">{tr({ uz: "Qo'lda ketgan vaqt", ru: "Время вручную" })}</span><span className="tc-val">{minutes} {tr({ uz: "daq", ru: "мин" })}</span></div>
              <div className="timer-chip success"><span className="tc-lbl">{tr({ uz: "Lenta bilan (keyingi ekran)", ru: "С лентой (следующий экран)" })}</span><span className="tc-val">{tr({ uz: "4 son", ru: "4 сек" })}</span></div>
            </div>
          ...
          <Col>
            ...
            {skipped ? <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.danger }}>{tr({ uz: "⚠️ Skanerni tashlab ketdingiz", ru: "⚠️ Вы пропустили сканер" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: "Tez bo'ldi — lekin endi kod tekshirilmagan holda uchib ketadi. Agar xato bo'lsa, foydalanuvchi buzuq saytni ko'radi.", ru: "Быстро — но теперь код улетит непроверенным. Если там ошибка, пользователь увидит сломанный сайт." })}</p><PhoneMock state="broken" /></div> : cur ? <div className="sk-info fade-step" key={active}><p className="note-h"><span style={{ color: T.accent }}>{cur.icon} {tr(cur.label)}</span></p><p className="body" style={{ margin: 0, color: T.ink }}>{tr(cur.d)}</p></div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Nuqtani bosing ←", ru: "Нажмите на точку ←" })}</p></div>}
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Har nuqtani birma-bir bosing — qo'lda o'tkazganda diqqat qayerda charchaydi?", ru: "💡 Нажимайте каждую точку по очереди — где внимание устаёт, когда делаешь это вручную?" })}</p>}
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tushuncha · lenta", ru: "Понятие · лента" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Lentani yoqing", ru: "Включите ленту" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Bir marta sozlasangiz — keyin <span className="italic" style={{ color: T.accent }}>o'zi ishlasa-chi</span>?</>, ru: <>Настроить один раз — а дальше пусть <span className="italic" style={{ color: T.accent }}>работает сама</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            <div className="timer-row">
              ...
              <div className="timer-chip success"><span className="tc-lbl">{tr({ uz: "Lenta bilan", ru: "С лентой" })}</span><span className="tc-val">{tr({ uz: "4 son", ru: "4 сек" })}</span></div>
            </div>
          </Col>
          <Col>
            <Belt statuses={ptStatus} />
            {done ? <div className="frame fade-step" style={{ borderLeft: `4px solid ${T.success}` }}><p className="note-h" style={{ color: T.success }}>{tr({ uz: "⚙️ Lenta (CI/CD)", ru: "⚙️ Лента (CI/CD)" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: <>Bir marta sozlaysiz. Endi <b>har push</b> avtomatik: skaner → ramka → o'rash → uchirish. Qo'l tegmaydi, hech narsa unutilmaydi.</>, ru: <>Настраиваете один раз. Теперь <b>каждый push</b> — автоматически: сканер → рамка → упаковка → взлёт. Руки не нужны, ничего не забывается.</> })}</p></div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Tugmani bosing ←", ru: "Нажмите кнопку ←" })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Lentani ishga tushiring va nuqtalar ketma-ket yonishini kuzating.", ru: "💡 Запустите ленту и посмотрите, как точки загораются одна за другой." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState(storedAnswer ? new Set(SUITCASES.map((d) => d.id)) : /* @__PURE__ */ new Set());
  const [active, setActive] = useState(null);
  const [sc, setSc] = useState(0);
  ...
  return <Stage eyebrow={tr({ uz: "Tushuncha · CI", ru: "Понятие · CI" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: `Chamadonlarni ko'ring (${seen.size}/3)`, ru: `Посмотрите чемоданы (${seen.size}/3)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Har chamadonni oching — ichidagi yuk lentaning qaysi nuqtasida tekshiriladi?", ru: "💡 Откройте каждый чемодан — на какой точке ленты проверяется то, что внутри?" })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState(storedAnswer ? new Set(CD_MODES.map((m) => m.id)) : /* @__PURE__ */ new Set());
  const [active, setActive] = useState(null);
  const [sc, setSc] = useState(0);
  ...
  return <Stage eyebrow={tr({ uz: "Tushuncha · CD", ru: "Понятие · CD" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Ikkala rejimni sinang", ru: "Попробуйте оба режима" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Ikkala rejimni ham yoqing: biri faqat tekshiradi, ikkinchisi uchiradi ham.", ru: "💡 Включите оба режима: один только проверяет, второй ещё и отправляет в полёт." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const cur = POINTS.find((p) => p.id === active);
  return <Stage eyebrow={tr({ uz: "Tushuncha · lenta nuqtalari", ru: "Понятие · точки ленты" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: `Nuqtalarni ko'ring (${seen.size}/5)`, ru: `Посмотрите точки (${seen.size}/5)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1 cj-items">
              {POINTS.map((p) => <button key={p.id} className={`itm-card ${seen.has(p.id) ? "" : "tap-hint"}`} onClick={() => tap(p.id)} style={seen.has(p.id) ? { boxShadow: `inset 0 0 0 1.5px ${T.success}` } : void 0}><span className="itm-ico">{p.icon}</span><span className="itm-nm">{tr(p.label)}</span></button>)}
            </div>
          </Col>
          <Col>
            {cur ? <div className="sk-info fade-step" key={active}><p className="note-h"><span style={{ fontSize: 18, marginRight: 6 }}>{cur.icon}</span>{tr(cur.label)} <span className="mono" style={{ color: T.ink3, marginLeft: 6, fontSize: 12 }}>{cur.cmd}</span></p><p className="body" style={{ margin: 0, color: T.ink }}>{tr(cur.d)}</p></div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Nuqtani bosing ←", ru: "Нажмите на точку ←" })}</p></div>}
...
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [fixed, setFixed] = useState(() => /* @__PURE__ */ new Set());
  const [ptStatus, setPtStatus] = useState({});
  const [running, setRunning] = useState(false);
  const [log, setLog] = useState([]);
  const [result, setResult] = useState(null);
  const [everSucceeded, setEverSucceeded] = useState(!!storedAnswer);
  const [sc, setSc] = useState(0);
  const timer = useRef(null);
  ...
  const runBelt = () => {
    ...
    const tick = (i) => {
      if (i >= POINTS.length) {
        pushLog(tr({ uz: "▸ ✈️ Uchirish — samolyot ko'tarildi ✓", ru: "▸ ✈️ Взлёт — самолёт поднялся ✓" }), "ok");
        setResult("success");
      ...
      timer.current = setTimeout(() => {
        if (outcome && outcome.at === p.id) {
          ...
          pushLog(`▸ ${p.icon} ${tr(p.label)} — ${tr(outcome.item.bug)} ✗`, "bad");
          if (p.id !== "deploy") pushLog(tr({ uz: "▸ ✈️ Uchirish — o'tkazib yuborildi", ru: "▸ ✈️ Взлёт — пропущен" }), "skip");
          setResult("fail");
          setRunning(false);
          runningRef.current = false;
          setSc((n) => n + 1);
  ...
  const navLabel = done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : selected.size === 0 ? tr({ uz: "Buyumlarni tanlang", ru: "Выберите вещи" }) : running ? tr({ uz: "Lenta aylanmoqda…", ru: "Лента крутится…" }) : result === "fail" ? tr({ uz: "Tuzatib qayta urinib ko'ring", ru: "Исправьте и попробуйте снова" }) : tr({ uz: "Lentaga qo'ying", ru: "Положите на ленту" });
  return <Stage eyebrow={tr({ uz: "Markaziy · lenta o'yini", ru: "Главная · игра с лентой" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
    ...
    return <div key={it.id} style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                    ...
                    {on && it.failAt && !isFixed && result === "fail" && <button className="itm-fix" onClick={() => fixItem(it.id)}>{tr({ uz: "🔧 Tuzatish", ru: "🔧 Исправить" })}</button>}
                  </div>;
  })}
            </div>
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={running || selected.size === 0} onClick={runBelt}>{running ? tr({ uz: "● Lenta aylanmoqda…", ru: "● Лента крутится…" }) : result === "fail" ? tr({ uz: "↻ Qayta lentaga qo'ying", ru: "↻ Положить на ленту снова" }) : tr({ uz: "🧳 Lentaga qo'ying", ru: "🧳 Положить на ленту" })}</button>
            <Belt statuses={ptStatus} />
          </Col>
          <Col>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Chamadonga buyum solib lentani yuboring — nima o'tmasa, lenta shuni aytadi.", ru: "💡 Положите вещи в чемодан и отправьте по ленте — что не пройдёт, лента сама скажет." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState(storedAnswer ? new Set(WHY.map((w) => w.id)) : /* @__PURE__ */ new Set());
  const [active, setActive] = useState(null);
  const [sc, setSc] = useState(0);
  ...
  return <Stage eyebrow={tr({ uz: "Tushuncha · nega", ru: "Понятие · зачем" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: `3 foydani ko'ring (${seen.size}/3)`, ru: `Посмотрите 3 выгоды (${seen.size}/3)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            {cur ? <div className="frame fade-step" key={active}><p className="note-h"><span style={{ fontSize: 20, marginRight: 6 }}>{cur.icon}</span>{tr(cur.t)}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr(cur.d)}</p></div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Foydani bosing ←", ru: "Нажмите на выгоду ←" })}</p></div>}
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Uchala foydani ham oching — lenta nimani tejashini o'zingiz sanang.", ru: "💡 Откройте все три выгоды — сами посчитайте, что экономит лента." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Hayotiy · jamoa", ru: "Из жизни · команда" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Ikkinchi holatni ko'ring", ru: "Посмотрите второй случай" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
    ...
    setShow(true);
    setSc((n) => n + 1);
  }}>{show ? tr({ uz: "✓ Ko'rdingiz", ru: "✓ Просмотрено" }) : tr({ uz: "Lenta bilan-chi?", ru: "А если с лентой?" })}</button>
          </Col>
          <Col>
            {show ? <div className="frame fade-step" style={{ borderLeft: `4px solid ${T.success}` }}>
                ...
                </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Tugmani bosing ←", ru: "Нажмите кнопку ←" })}</p></div>}
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Ikkinchi holatni ham oching — ikki holat farqi asosiy gapni ko'rsatadi.", ru: "💡 Откройте и второй случай — разница между ними и есть главная мысль." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const pick = (blank, val) => {
    ...
    if (val === blank.correct) {
      ...
      setWrongMsg(tr(blank.wrong[val]) || tr({ uz: "Bu to'g'ri emas.", ru: "Это неверно." }));
      setTimeout(() => setWrongKey((k) => k === blank.key ? null : k), 500);
    }
  };
  return <Stage eyebrow={tr({ uz: "Amaliyot · yo'l xaritasi", ru: "Практика · маршрутная карта" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Bo'shliqlarni to'ldiring", ru: "Заполните пропуски" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Lentaning <span className="italic" style={{ color: T.accent }}>yo'l xaritasini</span> o'zingiz to'ldiring.</>, ru: <>Заполните <span className="italic" style={{ color: T.accent }}>маршрутную карту</span> ленты сами.</> })}</h2></div>
        <Mentor>{tr({ uz: <>Bu — <span className="mono">ci.yml</span>: lenta har push'da nima qilishini shu fayl aytadi. Uchta bo'shliqni to'g'ri chipdan tanlab to'ldiring.</>, ru: <>Это — <span className="mono">ci.yml</span>: этот файл говорит ленте, что делать на каждый push. Заполните три пропуска, выбирая правильные чипы.</> })}</Mentor>
        <Zoomable><div className="split">
          <Col>
            <p className="flow-label">.github/workflows/ci.yml</p>
            <pre className="code-box" style={{ lineHeight: 1.9 }}>
              <At>name</At>{": CI"}{"\n"}
              <At>on</At>{": "}<Jx>{filled.on || "____"}</Jx>{"        "}<Cm>{tr({ uz: "# start signali", ru: "# стартовый сигнал" })}</Cm>{"\n"}
              <At>jobs</At>{":"}{"\n"}
              {"  "}<At>test</At>{":"}{"\n"}
    ...
    return <button key={opt} className={`gchip ${wrongKey === b.key ? "shake" : ""} ${filled[b.key] === b.correct ? "" : "tap-hint"}`} disabled={filled[b.key] === b.correct} onClick={() => pick(b, opt)} style={okChosen ? { boxShadow: `inset 0 0 0 1.5px ${T.success}`, color: T.success } : void 0}>{okChosen ? "✓ " : ""}{opt}</button>;
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: "Yo'l xaritasi to'ldi! Endi bu fayl repo'ga qo'shilsa, lenta har push'da o'zi ishga tushadi.", ru: "Маршрутная карта заполнена! Теперь, если добавить этот файл в репозиторий, лента будет запускаться сама на каждый push." })}</p></div>}
          </Col>
        </div></Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Har bo'shliqqa qarang: bu qator lentaga qachon, qayerda yoki nima qilishni aytyaptimi?", ru: "💡 Посмотрите на каждый пропуск: эта строка говорит ленте когда, где или что делать?" })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen14 = (props) => <QuestionScreen
  ...
  eyebrow={tr({ uz: "Mashq · 4-savol", ru: "Практика · вопрос 4" })}
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done && !_resc} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : _resc ? tr({ uz: "Lentani birga yig'amiz →", ru: "Соберём ленту вместе →" }) : tr({ uz: "Lentani yig'ing", ru: "Соберите ленту" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
  ...
  />
        ...
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Lentani keyinroq birga yig'amiz — hozir davom etsangiz bo'ladi.", ru: "Соберём ленту вместе позже — сейчас можно продолжить." })}</p>}
      </div>
    </Stage>;
};
var ACHIEVEMENTS = {
...
function ScreenLivePractice({ title, task, checklist, screen, storedAnswer, onAnswer, onNext, onPrev, live }) {
  ...
  return <Stage eyebrow={tr({ uz: "Amaliyot · VS Code", ru: "Практика · VS Code" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Avval bajaring", ru: "Сначала выполните" })} onClick={onNext} /></>}>
...
var SummaryScreen = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  const RECAP = [
    tr({ uz: "CI/CD — kodni har o'zgarishda avtomatik tekshiradigan va uchiradigan lenta", ru: "CI/CD — лента, которая автоматически проверяет и отправляет код при каждом изменении" }),
    tr({ uz: "CI — har push birlashtiriladi va tekshiriladi (skaner, ramka)", ru: "CI — каждый push объединяется и проверяется (сканер, рамка)" }),
    tr({ uz: "CD — tekshiruvdan o'tgan yuk avtomatik uchadi", ru: "CD — прошедший проверку груз улетает автоматически" }),
    tr({ uz: "Lenta: Yig'ish → Skaner → Ramka → O'rash → Uchirish", ru: "Лента: Сборка → Сканер → Рамка → Упаковка → Взлёт" }),
    tr({ uz: "Bitta nuqta qizil bersa — lenta to'xtaydi, buzuq yuk hech qachon uchmaydi", ru: "Одна красная точка — лента останавливается, сломанный груз никогда не улетит" })
  ];
  const HOMEWORK = [
    { b: tr({ uz: "Sanang", ru: "Посчитайте" }), t: tr({ uz: "— o'z loyihangizda deploy hozir nechta qo'lda qadamdan iborat?", ru: "— из скольких ручных шагов сейчас состоит деплой вашего проекта?" }) },
    { b: tr({ uz: "Tartib", ru: "Порядок" }), t: tr({ uz: "— lenta 5 nuqtasini yoddan to'g'ri tartibda yozing", ru: "— напишите по памяти 5 точек ленты в правильном порядке" }) },
    { b: tr({ uz: "O'qing", ru: "Почитайте" }), t: tr({ uz: "— GitHub Actions haqida qisqa material ko'rib keling", ru: "— посмотрите короткий материал про GitHub Actions" }) }
  ];
  ...
  return <Stage eyebrow={tr({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: "CI/CD lentasini tushundingiz", ru: "Вы разобрались в ленте CI/CD" })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Endi kod o'zgarishini <span className="italic" style={{ color: T.accent }}>lenta</span> chiqaradi.</>, ru: <>Теперь изменения кода выкатывает <span className="italic" style={{ color: T.accent }}>лента</span>.</> })}</h2>{
    /* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>{tr({ uz: "📝 Uyga vazifa", ru: "📝 Домашнее задание" })}</div><ul>{HOMEWORK.map((h, i) => <li key={i}><b>{h.b}</b> <span className="t">{h.t}</span></li>)}</ul><p className="hw-note">{tr({ uz: "🚀 Keyingi dars — GitHub Actions: birinchi workflow'ni o'zimiz yozamiz va har push'da testlarni avtomatik ishga tushiramiz!", ru: "🚀 Следующий урок — GitHub Actions: сами напишем первый workflow и будем автоматически запускать тесты на каждый push!" })}</p></div>}
...
function CiCdIntroLesson({ lang: langProp, onFinished }) {
