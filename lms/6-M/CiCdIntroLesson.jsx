...
//  Manba:  src/4c-Modull/CiCdIntroLesson.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/4c-Modull/CiCdIntroLesson.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/4c-Modull/CiCdIntroLesson.jsx
...
var LESSON_META = { lessonId: "cicd-intro-4c-01-v18", lessonTitle: { uz: "CI/CD nima va nega kerak", ru: "Что такое CI/CD и зачем" } };
var HW_TOKENS = [
...
var INLINE_KEYS = { s4: 2, s8: 0, s10: 3, s14: 1, s15: 0, practice: -1 };
var RECAPS = {
  4: {
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr2({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и число ✅/❌ скрыты — по нажатию «Открыть результат» всё появится сразу и у вас, и на экранах учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верно — класс не понял эту тему. Перед продолжением рекомендуем короткое повторение.</> })}</p>
            ...
            {level === "maybe" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>🟡 <b>{pct}%</b> to'g'ri — yomon emas. Xohlasangiz, davom etishdan oldin qisqa takrorlab oling.</>, ru: <>🟡 <b>{pct}%</b> верно — неплохо. Если хотите, коротко повторите тему перед продолжением.</> })}</p>
            ...
            {level === "few" && <p className="mstats-verdict-t">{tr2({ uz: <>Javob berganlar kam ({answered} ta) — foiz bo'yicha xulosa chiqarish qiyin. O'zingiz baholang.</>, ru: <>Ответивших мало ({answered}) — рано делать выводы по процентам. Оцените сами.</> })}</p>}
          </div>;
  })()}
      ...
      {reveal && struggling && <p className="mstats-warn">{tr2({ uz: "⚠️ Ko'pchilik xato qildi — bu mavzu tushunarsiz bo'lgan ko'rinadi. Yana bir bor tushuntiring.", ru: "⚠️ Большинство ошиблось — похоже, тема осталась непонятной. Рекомендуем объяснить её ещё раз." })}</p>}
...
var QuestionScreen = ({ screen, idx, scope, eyebrow, question, questionText, options, correctIdx, explainCorrect, explainWrong, audioText, audioOk, audioWrong, storedAnswer, tip, onAnswer, onNext, onPrev }) => {
...
function ScoreRing({ correct, total }) {
  ...
  return <div className="ring-wrap">
      ...
      <div className="ring-center"><div className="ring-num"><span style={{ color: col }}>{correct}</span><span className="ring-den">/{total}</span></div><div className="ring-lbl">{tr2({ uz: "to'g'ri javob", ru: "правильных" })}</div></div>
    </div>;
}
var Mentor = ({ children }) => {
...
var Term = ({ title = "Terminal", children, minH }) => <div className="term"><div className="term-bar"><span className="bb-dots"><i /><i /><i /></span><span className="term-title">{tr2(title)}</span></div><div className="term-body" style={{ minHeight: minH }}>{children}</div></div>;
...
var POINTS = [
  ...
  { id: "lint", icon: "📐", label: { uz: "O'lcham ramkasi", ru: "Рамка" }, cmd: "eslint .", d: { uz: "Kod qoidalarga mos yozilganini tekshiradi.", ru: "Проверяет, что код написан по правилам." } },
...
var ITEMS = [
  { id: "cart", label: "savatcha.js", icon: "🟢" },
  { id: "price", label: "narx.js", icon: "🟢" },
  { id: "login", label: "login.js", icon: "🔴", failAt: "test", bug: { uz: 'login.test.js: kutildi "Ali", keldi undefined', ru: 'login.test.js: ожидалось "Ali", получено undefined' } },
  { id: "style", label: "stil.css", icon: "🟡", failAt: "lint", bug: { uz: "3-qatorda qoidaga zid yozuv (semicolon yo'q)", ru: "в строке 3 нарушение правила (нет точки с запятой)" } },
  { id: "image", label: "rasm.jsx", icon: "🟠", failAt: "build", bug: { uz: "import './logo-yangi.png' — fayl topilmadi", ru: "import './logo-yangi.png' — файл не найден" } }
];
...
function DragDropOrder({ items, hints, onSolved, doneText, onChange }) {
  ...
  return <div className="dd fade-up">
      ...
      <div className="dd-pool">
        {pool.length === 0 && !solved && <span className="dd-pool-empty">{tr2({ uz: "Tartib xato — bo'lakni bosib qaytaring va qayta joylang", ru: "Порядок неверный — нажмите на блок, чтобы вернуть его, и разложите заново" })}</span>}
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const OPTS = [
    { id: "a", label: { uz: "Har safar shu 5 qadamni qo'lda, diqqat bilan bajaraman", ru: "Каждый раз выполняю эти 5 шагов вручную, очень внимательно" } },
    { id: "b", label: { uz: "Lentaga topshiraman — har push'da o'zi tekshirib, uchiradi", ru: "Доверю ленте — на каждый push она сама проверит и отправит в полёт" } },
    { id: "c", label: { uz: "Umuman deploy qilmayman, lokalda ishlasa bo'ldi", ru: "Вообще не буду деплоить — работает локально, и ладно" } }
  ];
  const poke = () => {
    setTried(true);
    setSc((n) => n + 1);
  ...
  const ACK = {
    a: { uz: <>Diqqat charchaydi — beshinchi marta shu 5 qadamni qo'lda bajarganda bitta qadam <b>tushib qoladi</b>, va buni hech kim sezmaydi. Bugun shu ishni <b>lentaga</b> topshiramiz: u charchamaydi va tartibni buzmaydi.</>, ru: <>Внимание устаёт — на пятый раз один из этих 5 шагов <b>выпадает</b>, и никто этого не замечает. Сегодня передадим эту работу <b>ленте</b>: она не устаёт и не сбивает порядок.</> },
    b: { uz: <>Aynan! Bugun CI/CD — kodingizni <b>har push'da avtomatik</b> tekshiradigan va uchiradigan lentani o'rganamiz.</>, ru: <>Именно! Сегодня изучаем CI/CD — ленту, которая <b>автоматически на каждый push</b> проверяет ваш код и отправляет его в полёт.</> },
    c: { uz: <>Lokalda ishlagani — <b>faqat sizda</b> ishlagani. Boshqa kompyuterda paket yetishmaydi yoki versiya boshqa bo'ladi. Bugun ko'ramiz: lenta <b>hammani bir xil</b> tekshiradi va shu farqni ochib beradi.</>, ru: <>Работает локально — значит, работает <b>только у вас</b>. На другом компьютере не хватит пакета или окажется другая версия. Сегодня увидим: лента проверяет <b>всех одинаково</b> и вскрывает эту разницу.</> }
  };
  const pick = (v) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} scrollSignal={sc} navContent={<NavNext optionalLive disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up" style={{ maxWidth: 880 }}>{tr2({ uz: <>Kodni yangiladingiz — endi uni internetga chiqarish uchun <span className="italic" style={{ color: T.accent }}>nechta amal</span> kerak?</>, ru: <>Вы обновили код — сколько теперь нужно <span className="italic" style={{ color: T.accent }}>действий</span>, чтобы выложить его в интернет?</> })}</h1>
        <Mentor>{tr2({ uz: <>Kod tayyor, test ham yashil. Endi uni foydalanuvchiga yetkazish kerak. Bir martani <b style={{ color: T.ink }}>qo'lda</b> — chamadonni o'zingiz ko'tarib — bajarib ko'ring.</>, ru: <>Код готов, тесты зелёные. Теперь его нужно доставить пользователю. Попробуйте один раз сделать это <b style={{ color: T.ink }}>вручную</b> — сами пронесите чемодан.</> })}</Mentor>
        <Zoomable><Split>
          <Col>
            <Term title={{ uz: "qo'lda uchirish", ru: "запуск вручную" }} minH={150}>
              <TLine cmd="npm test" />
              {tried && <>
                <TLine out={tr2({ uz: "✓ testlar yashil", ru: "✓ тесты зелёные" })} col={CODE.str} />
                <TLine cmd="npm run build" />
                <TLine cmd="scp ./dist server:/var/www" />
                <TLine cmd="deploy" />
                <TLine out={tr2({ uz: "✓ chamadon qo'lda yetkazildi (5 qadam)", ru: "✓ чемодан доставлен вручную (5 шагов)" })} col={CODE.str} />
              </>}
            </Term>
            <button className={`btn-soft ${tried ? "" : "tap-hint"}`} style={{ alignSelf: "flex-start" }} onClick={poke} disabled={tried}>{tried ? tr2({ uz: "✓ Qo'lda uchirish bajarildi", ru: "✓ Запуск вручную выполнен" }) : tr2({ uz: "▶ Qo'lda uchirish", ru: "▶ Запуск вручную" })}</button>
            {tried && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>5 qadam bajarildi. Va buni <b>har push'da</b> takrorlaysiz. Bittasini unutsangiz (masalan testni) — buzuq yuk internetga chiqib ketadi.</>, ru: <>5 шагов выполнено. И вы будете повторять их на <b>каждый push</b>. Забудете один шаг (например, тесты) — сломанный груз улетит в интернет.</> })}</p></div>}
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Bu ishni qanday yaxshilaymiz?", ru: "Как улучшить этот процесс?" })}</p>
    ...
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null || !tried} style={{ opacity: !tried ? 0.55 : 1 }} onClick={() => pick(o.id)}><span className="radio">{on && <span className="radio-dot" />}</span><span>{tr2(o.label)}</span></button>;
            ...
            {!tried && <p className="small" style={{ color: T.ink3, fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Avval qo'lda uchirishni bosing ←", ru: "Сначала нажмите «Запуск вручную» ←" })}</p>}
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  const STEPS = [
    { text: { uz: "CI/CD nima va nega kerak", ru: "Что такое CI/CD и зачем" }, tag: { uz: "tushuncha", ru: "понятие" } },
    { text: { uz: "CI va CD — tekshirish va uchirish farqi", ru: "CI и CD — разница между проверкой и отправкой" }, tag: { uz: "farq", ru: "разница" } },
    { text: { uz: "Lenta nuqtalari — Yig'ish→Uchirish", ru: "Точки ленты — Сборка→Взлёт" }, tag: { uz: "5 nuqta", ru: "5 точек" } },
    { text: { uz: "Chamadonni yig'ing va lentaga qo'ying", ru: "Соберите чемодан и положите на ленту" }, tag: { uz: "markaziy o'yin", ru: "главная игра" } }
  ];
  const isNarrow = useIsMobile(768);
  const [showSteps, setShowSteps] = useState3(false);
  const Preview = <Col>
      <p className="flow-label">{tr2({ uz: "Dars oxirida — shu lentani tushunasiz", ru: "К концу урока вы будете понимать эту ленту" })}</p>
      <Belt statuses={ALL_PASS} />
      <div className="sk-info"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Push qildingiz → lenta o'zi ishladi: paket o'rnatildi, skaner yashil, yuk o'raldi va <b style={{ color: T.success }}>avtomatik</b> uchdi. Qo'l tegmaydi.</>, ru: <>Вы сделали push → лента сработала сама: пакеты установлены, сканер зелёный, груз упакован и <b style={{ color: T.success }}>автоматически</b> улетел. Руками ничего делать не нужно.</> })}</p></div>
    </Col>;
  const StepsB = <Col>
      <p className="flow-label">{tr2({ uz: "Bugungi 4 qadam", ru: "4 шага на сегодня" })}</p>
  ...
  return <Stage eyebrow={tr2({ uz: "Reja", ru: "План" })} screen={screen} mentorStatic scrollSignal={showSteps} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive label={tr2({ uz: "Boshlaymiz →", ru: "Начнём →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Deploy'ni har safar qo'lda qilamizmi — yoki <span className="italic" style={{ color: T.accent }}>lentaga topshiramizmi</span>?</>, ru: <>Каждый раз деплоим вручную — или <span className="italic" style={{ color: T.accent }}>доверим ленте</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>CI/CD — bu kodingizni avtomatik tekshiradigan va internetga chiqaradigan <b style={{ color: T.ink }}>lenta</b>. Bir marta sozlaysiz, u har push'da o'zi ishlaydi. Mana natija va unga olib boradigan 4 qadam.</>, ru: <>CI/CD — это <b style={{ color: T.ink }}>лента</b>, которая автоматически проверяет ваш код и выкладывает его в интернет. Настраиваете один раз — дальше она сама работает на каждый push. Вот результат и 4 шага, которые к нему ведут.</> })}</Mentor>
        {!isNarrow ? <Zoomable><Split>{Preview}{StepsB}</Split></Zoomable> : !showSteps ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>{Preview}<button className="btn" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(true)}>{tr2({ uz: "4 qadamni ko'rish", ru: "Посмотреть 4 шага" })}</button></div> : <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}><button className="btn-soft" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(false)}>{tr2({ uz: "↩ Lentani ko'rish", ru: "↩ Посмотреть ленту" })}</button>{StepsB}</div>}
      </div>
    </Stage>;
};
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Muammo · qo'lda", ru: "Проблема · вручную" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `5 nuqtani qo'lda o'tkazing (${seen.size}/5)`, ru: `Проведите 5 точек вручную (${seen.size}/5)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bitta uchirish — aslida <span className="italic" style={{ color: T.accent }}>nechta amal</span>?</>, ru: <>Один запуск — сколько это <span className="italic" style={{ color: T.accent }}>действий</span> на самом деле?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1 cj-items">
              {POINTS.map((p) => <button key={p.id} className={`itm-card ${seen.has(p.id) ? "" : "tap-hint"}`} onClick={() => tap(p.id)} style={seen.has(p.id) ? { boxShadow: `inset 0 0 0 1.5px ${T.success}` } : void 0}><span className="itm-ico">{p.icon}</span><span className="itm-nm">{seen.has(p.id) ? "✓ " : ""}{tr2(p.label)}</span></button>)}
            </div>
            <div className="timer-row fade-up delay-2">
              <div className={`timer-chip ${minutes >= 12 ? "danger" : ""}`}><span className="tc-lbl">{tr2({ uz: "Qo'lda ketgan vaqt", ru: "Время вручную" })}</span><span className="tc-val">{minutes} {tr2({ uz: "daq", ru: "мин" })}</span></div>
              <div className="timer-chip success"><span className="tc-lbl">{tr2({ uz: "Lenta bilan (keyingi ekran)", ru: "С лентой (следующий экран)" })}</span><span className="tc-val">{tr2({ uz: "4 son", ru: "4 сек" })}</span></div>
            </div>
            <button className="tempt-btn" onClick={tempt}>{tr2({ uz: "😮‍💨 Skanerni tashlab ketish (tez bo'ladi)", ru: "😮‍💨 Пропустить сканер (так быстрее)" })}</button>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "nuqta tafsiloti", ru: "детали точки" })}</p>
            {skipped ? <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.danger }}>{tr2({ uz: "⚠️ Skanerni tashlab ketdingiz", ru: "⚠️ Вы пропустили сканер" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Tez bo'ldi — lekin endi kod tekshirilmagan holda uchib ketadi. Agar xato bo'lsa, foydalanuvchi buzuq saytni ko'radi.", ru: "Быстро — но теперь код улетит непроверенным. Если там ошибка, пользователь увидит сломанный сайт." })}</p><PhoneMock state="broken" /></div> : cur ? <div className="sk-info fade-step" key={active}><p className="note-h"><span style={{ color: T.accent }}>{cur.icon} {tr2(cur.label)}</span></p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2(cur.d)}</p></div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Nuqtani bosing ←", ru: "Нажмите на точку ←" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "5 nuqta — har push'da qo'lda takrorlanadi. Sekin, zerikarli, va bir kuni bittasini unutasiz. Buni avtomatlashtirsak-chi?", ru: "5 точек — и так вручную на каждый push. Медленно, скучно, и однажды вы одну из них забудете. А если это автоматизировать?" })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Har nuqtani birma-bir bosing — qo'lda o'tkazganda diqqat qayerda charchaydi?", ru: "💡 Нажимайте каждую точку по очереди — где внимание устаёт, когда делаешь это вручную?" })}</p>}
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [ptStatus, setPtStatus] = useState3({});
  const [running, setRunning] = useState3(false);
  const [done, setDone] = useState3(!!storedAnswer);
  const { tip: _tip, rescue: _resc } = useStuckValve(done, Object.keys(ptStatus).length);
  const [sc, setSc] = useState3(0);
  ...
  const run = () => {
    ...
    const tick = (i) => {
      ...
      setPtStatus((st) => ({ ...st, [POINTS[i].id]: "run" }));
      timer.current = setTimeout(() => {
        setPtStatus((st) => ({ ...st, [POINTS[i].id]: "pass" }));
        setSc((n) => n + 1);
        timer.current = setTimeout(() => tick(i + 1), 60);
      }, 740);
    };
    tick(0);
  };
  return <Stage eyebrow={tr2({ uz: "Tushuncha · lenta", ru: "Понятие · лента" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Lentani yoqing", ru: "Включите ленту" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bir marta sozlasangiz — keyin <span className="italic" style={{ color: T.accent }}>o'zi ishlasa-chi</span>?</>, ru: <>Настроить один раз — а дальше пусть <span className="italic" style={{ color: T.accent }}>работает сама</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Aeroport bagaj lentasini tasavvur qiling: <b style={{ color: T.ink }}>kod = chamadon</b>. Siz uni push qilasiz, u <b style={{ color: T.ink }}>lentaga</b> tushadi. Har nuqta chamadon bilan o'z ishini qiladi — birortasi qizil bersa, keyingisiga o'tmaydi. Oxirida, hammasi yashil bo'lsa, samolyot avtomatik ko'tariladi. Tugmani bosing.</>, ru: <>Представьте багажную ленту в аэропорту: <b style={{ color: T.ink }}>код = чемодан</b>. Вы делаете push — и он попадает <b style={{ color: T.ink }}>на ленту</b>. Каждая точка делает с чемоданом свою работу; если какая-то даёт красный, дальше он не едет. А если в конце всё зелёное — самолёт взлетает автоматически. Нажмите кнопку.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <div className="frame" style={{ borderLeft: `4px solid ${T.danger}` }}><p className="note-h" style={{ color: T.danger }}>{tr2({ uz: "🐌 Qo'lda", ru: "🐌 Вручную" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Har push'dan keyin 5 nuqtani o'zingiz bajarasiz. Sekin, xatoga moyil — va bir kuni <b>unutib qo'yasiz</b>.</>, ru: <>После каждого push вы сами выполняете 5 точек. Медленно, легко ошибиться — и однажды вы просто <b>забудете</b>.</> })}</p></div>
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={running || done} onClick={run}>{done ? tr2({ uz: "✓ Lenta yoqildi", ru: "✓ Лента включена" }) : running ? tr2({ uz: "● Ishlayapti…", ru: "● Работает…" }) : tr2({ uz: "Lenta bilan-chi?", ru: "А если с лентой?" })}</button>
            <div className="timer-row">
              <div className="timer-chip danger"><span className="tc-lbl">{tr2({ uz: "Qo'lda", ru: "Вручную" })}</span><span className="tc-val">{tr2({ uz: "20 daq", ru: "20 мин" })}</span></div>
              <div className="timer-chip success"><span className="tc-lbl">{tr2({ uz: "Lenta bilan", ru: "С лентой" })}</span><span className="tc-val">{tr2({ uz: "4 son", ru: "4 сек" })}</span></div>
            </div>
          </Col>
          <Col>
            <Belt statuses={ptStatus} />
            {done ? <div className="frame fade-step" style={{ borderLeft: `4px solid ${T.success}` }}><p className="note-h" style={{ color: T.success }}>{tr2({ uz: "⚙️ Lenta (CI/CD)", ru: "⚙️ Лента (CI/CD)" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Bir marta sozlaysiz. Endi <b>har push</b> avtomatik: skaner → ramka → o'rash → uchirish. Qo'l tegmaydi, hech narsa unutilmaydi.</>, ru: <>Настраиваете один раз. Теперь <b>каждый push</b> — автоматически: сканер → рамка → упаковка → взлёт. Руки не нужны, ничего не забывается.</> })}</p></div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Tugmani bosing ←", ru: "Нажмите кнопку ←" })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Lentani ishga tushiring va nuqtalar ketma-ket yonishini kuzating.", ru: "💡 Запустите ленту и посмотрите, как точки загораются одна за другой." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
/>;
var SUITCASES = [
  { id: "ali", name: "Ali", work: { uz: "savatcha funksiyasi", ru: "функция корзины" } },
  { id: "vali", name: "Vali", work: { uz: "login sahifasi", ru: "страница логина" } },
  { id: "sora", name: "Sora", work: { uz: "mahsulot ro'yxati", ru: "список товаров" } }
];
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState3(storedAnswer ? new Set(SUITCASES.map((d) => d.id)) : /* @__PURE__ */ new Set());
  const [active, setActive] = useState3(null);
  const [sc, setSc] = useState3(0);
  const done = seen.size >= SUITCASES.length;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, seen.size);
  const tap = (id) => {
    setActive(id);
    setSeen((prev) => {
  ...
  const cur = SUITCASES.find((d) => d.id === active);
  const result = cur && cur.id === "vali" ? "bad" : cur ? "ok" : null;
  return <Stage eyebrow={tr2({ uz: "Tushuncha · CI", ru: "Понятие · CI" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Chamadonlarni ko'ring (${seen.size}/3)`, ru: `Посмотрите чемоданы (${seen.size}/3)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>CI — <span className="italic" style={{ color: T.accent }}>Continuous Integration</span>: uch chamadon, bitta lenta.</>, ru: <>CI — <span className="italic" style={{ color: T.accent }}>Continuous Integration</span>: три чемодана, одна лента.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1 cj-items">
              {SUITCASES.map((d) => <button key={d.id} className={`itm-card ${seen.has(d.id) ? "" : "tap-hint"}`} onClick={() => tap(d.id)} style={seen.has(d.id) ? { boxShadow: `inset 0 0 0 1.5px ${d.id === "vali" ? T.danger : T.success}` } : void 0}><span className="itm-ico">🧳</span><span className="itm-nm">{seen.has(d.id) ? "✓ " : ""}{d.name}</span></button>)}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "lenta natijasi", ru: "результат ленты" })}</p>
            {cur ? <div className={result === "bad" ? "frame-warn fade-step" : "sk-info fade-step"} key={active}><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>{cur.name}ning chamadoni ({tr2(cur.work)}) lentaga tushdi.</>, ru: <>Чемодан {cur.name} ({tr2(cur.work)}) попал на ленту.</> })} {result === "bad" ? tr2({ uz: <>🔍 Skaner nuqtasida <b style={{ color: T.danger }}>qizil</b> berdi — testi yiqildi.</>, ru: <>🔍 На точке «Сканер» загорелся <b style={{ color: T.danger }}>красный</b> — тест упал.</> }) : tr2({ uz: <><b style={{ color: T.success }}>✓ yashil</b>, hammasi joyida.</>, ru: <><b style={{ color: T.success }}>✓ зелёный</b>, всё в порядке.</> })}</p></div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Dasturchini bosing ←", ru: "Нажмите на разработчика ←" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Valiniki uyidagi kompyuterda ishlagan edi. Lenta esa hammani <b>bir xil sharoitda</b> sinaydi — shuning uchun aynan shu yerda xato topildi.</>, ru: <>У Вали код работал на домашнем компьютере. А лента проверяет всех <b>в одинаковых условиях</b> — поэтому ошибка нашлась именно здесь.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Har chamadonni oching — ichidagi yuk lentaning qaysi nuqtasida tekshiriladi?", ru: "💡 Откройте каждый чемодан — на какой точке ленты проверяется то, что внутри?" })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var CD_MODES = [
  { id: "delivery", label: { uz: "Yetkazish", ru: "Доставка" }, icon: "🖐️", d: { uz: "Lenta oxirigacha yetadi va tayyor turadi — lekin sizdan tasdiq so'raydi. Siz «Uchirish» tugmasini bosishingiz kerak.", ru: "Лента доезжает до конца, и всё готово — но она ждёт вашего подтверждения. Кнопку «Взлёт» нажимаете вы." } },
  { id: "deployment", label: "Deployment", icon: "⚙️", d: { uz: "Lenta oxirigacha yetsa — hech kimdan so'ramaydi, o'zi uchiradi. To'liq avtomatik.", ru: "Доехав до конца, лента никого не спрашивает — отправляет сама. Полностью автоматически." } }
];
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState3(storedAnswer ? new Set(CD_MODES.map((m) => m.id)) : /* @__PURE__ */ new Set());
  const [active, setActive] = useState3(null);
  const [sc, setSc] = useState3(0);
  const done = seen.size >= CD_MODES.length;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, seen.size);
  const tap = (id) => {
    setActive(id);
    setSeen((prev) => {
  ...
  const cur = CD_MODES.find((m) => m.id === active);
  return <Stage eyebrow={tr2({ uz: "Tushuncha · CD", ru: "Понятие · CD" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Ikkala rejimni sinang", ru: "Попробуйте оба режима" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>CD — <span className="italic" style={{ color: T.accent }}>Continuous Deployment</span>: kim uchiradi?</>, ru: <>CD — <span className="italic" style={{ color: T.accent }}>Continuous Deployment</span>: кто отправляет в полёт?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {CD_MODES.map((m) => <button key={m.id} className="vcard" onClick={() => tap(m.id)} style={{ boxShadow: active === m.id ? `inset 0 0 0 1.5px ${T.accent}, 0 8px 20px -6px rgba(${T.shadowBase},0.2)` : void 0 }}>
          ...
          <Col>
            {cur ? <div className="frame fade-step" key={active}><p className="note-h"><span style={{ fontSize: 20, marginRight: 6 }}>{cur.icon}</span>{tr2(cur.label)}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2(cur.d)}</p></div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Rejimni bosing ←", ru: "Нажмите на режим ←" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Ikkalasi ham CD. Farqi — oxirgi qadamda: <b>tasdiq kerakmi</b> yoki <b>yo'qmi</b>.</>, ru: <>Оба варианта — CD. Разница в последнем шаге: <b>нужно подтверждение</b> или <b>нет</b>.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Ikkala rejimni ham yoqing: biri faqat tekshiradi, ikkinchisi uchiradi ham.", ru: "💡 Включите оба режима: один только проверяет, второй ещё и отправляет в полёт." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const cur = POINTS.find((p) => p.id === active);
  return <Stage eyebrow={tr2({ uz: "Tushuncha · lenta nuqtalari", ru: "Понятие · точки ленты" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Nuqtalarni ko'ring (${seen.size}/5)`, ru: `Посмотрите точки (${seen.size}/5)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Lentada qanday <span className="italic" style={{ color: T.accent }}>nuqtalar</span> bor?</>, ru: <>Какие <span className="italic" style={{ color: T.accent }}>точки</span> есть на ленте?</> })}</h2></div>
        <Mentor>{tr2({ uz: "Pipeline (lenta) — ketma-ket nuqtalar. Chamadon (kod) har biridan o'tadi. Har nuqtani bosib, u nima qilishini ko'ring.", ru: "Pipeline (лента) — это точки одна за другой. Чемодан (код) проходит каждую. Нажимайте на точки и смотрите, что делает каждая." })}</Mentor>
        <div className="fade-up"><Belt statuses={statuses} /></div>
        <Zoomable>
        <div className="split">
          <Col>
            <div className="fade-up delay-1 cj-items">
              {POINTS.map((p) => <button key={p.id} className={`itm-card ${seen.has(p.id) ? "" : "tap-hint"}`} onClick={() => tap(p.id)} style={seen.has(p.id) ? { boxShadow: `inset 0 0 0 1.5px ${T.success}` } : void 0}><span className="itm-ico">{p.icon}</span><span className="itm-nm">{tr2(p.label)}</span></button>)}
            </div>
          </Col>
          <Col>
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Tartibi muhim: <b>Yig'ish → Skaner → Ramka → O'rash → Uchirish</b>. Avval tekshiramiz, keyin uchiramiz — hech qachon teskari emas.</>, ru: <>Порядок важен: <b>Сборка → Сканер → Рамка → Упаковка → Взлёт</b>. Сначала проверяем, потом отправляем — и никогда наоборот.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Har nuqtani bosib ta'rifini o'qing — qaysi buyruq nima qilishini solishtiring.", ru: "💡 Нажмите каждую точку и прочитайте описание — сравните, какая команда что делает." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen8 = (props) => <QuestionScreen
  ...
  idx={8}
  scope="module-mikro"
  tip={{ uz: "💡 Ikki harfning oxirgisiga qarang: biri tekshirishda to'xtaydi, ikkinchisi undan keyin ham davom etadi.", ru: "💡 Посмотрите на вторую букву: одна останавливается на проверке, вторая идёт дальше." }}
  eyebrow={tr2({ uz: "Mashq · 2-savol", ru: "Практика · вопрос 2" })}
  questionText="CI va CD orasidagi farq nima?"
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <><span className="mono" style={{ color: T.accent }}>CI</span> va <span className="mono" style={{ color: T.accent }}>CD</span> orasidagi <span className="italic" style={{ color: T.accent }}>farq</span> nima?</>, ru: <>В чём <span className="italic" style={{ color: T.accent }}>разница</span> между <span className="mono" style={{ color: T.accent }}>CI</span> и <span className="mono" style={{ color: T.accent }}>CD</span>?</> })}</h2></>}
  options={[{ uz: "CI — har o'zgarishni tekshiradi; CD — tekshirilgan yukni uchiradi", ru: "CI проверяет каждое изменение; CD отправляет проверенный груз в полёт" }, { uz: "CI va CD — ikkalasi ham bir xil narsaning boshqa nomi, xolos", ru: "CI и CD — просто два названия одного и того же" }, { uz: "CI uchirish ishini qiladi, CD esa faqat tekshirib qo'ya qoladi", ru: "CI занимается отправкой, а CD только проверяет" }, { uz: "Ikkalasi ham faqat sayt dizayni bilan shug'ullanadigan vositalar", ru: "Оба — инструменты только для дизайна сайта" }]}
  correctIdx={0}
...
function pointOkLine(p) {
  const MAP = {
    ...
    lint: { uz: "▸ 📐 O'lcham ramkasi — qoidalarga mos ✓", ru: "▸ 📐 Рамка — код соответствует правилам ✓" },
    build: { uz: "▸ 🎁 O'rash — yuk tayyor o'raldi ✓", ru: "▸ 🎁 Упаковка — груз аккуратно упакован ✓" },
...
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [fixed, setFixed] = useState3(() => /* @__PURE__ */ new Set());
  const [ptStatus, setPtStatus] = useState3({});
  const [running, setRunning] = useState3(false);
  const [log, setLog] = useState3([]);
  const [result, setResult] = useState3(null);
  const [everSucceeded, setEverSucceeded] = useState3(!!storedAnswer);
  const [sc, setSc] = useState3(0);
  const timer = useRef3(null);
  const runningRef = useRef3(false);
  useEffect4(() => () => clearTimeout(timer.current), []);
  const done = everSucceeded;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, (result ? 1 : 0) + (everSucceeded ? 1 : 0));
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { stage: "case", screenIdx: screen, question: "Chamadonni yig'ing va lentaga to'liq yashil o'tkazing", correct: true, solved: true, picked: true });
  }, [done]);
  const resetRunState = () => {
    setResult(null);
    setPtStatus({});
  ...
  const runBelt = () => {
    ...
    for (const p of POINTS) {
      const bad = chosen.find((it) => it.failAt === p.id && !fixed.has(it.id));
      if (bad) {
        outcome = { at: p.id, item: bad };
        break;
      }
    }
    const tick = (i) => {
      ...
      timer.current = setTimeout(() => {
        if (outcome && outcome.at === p.id) {
          setPtStatus((st) => {
            const ns = { ...st, [p.id]: "fail" };
            POINTS.slice(i + 1).forEach((sp) => {
              ns[sp.id] = "skip";
            });
            return ns;
          });
          pushLog(`▸ ${p.icon} ${tr2(p.label)} — ${tr2(outcome.item.bug)} ✗`, "bad");
          if (p.id !== "deploy") pushLog(tr2({ uz: "▸ ✈️ Uchirish — o'tkazib yuborildi", ru: "▸ ✈️ Взлёт — пропущен" }), "skip");
          setResult("fail");
          setRunning(false);
          runningRef.current = false;
          setSc((n) => n + 1);
  ...
  const navLabel = done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : selected.size === 0 ? tr2({ uz: "Buyumlarni tanlang", ru: "Выберите вещи" }) : running ? tr2({ uz: "Lenta aylanmoqda…", ru: "Лента крутится…" }) : result === "fail" ? tr2({ uz: "Tuzatib qayta urinib ko'ring", ru: "Исправьте и попробуйте снова" }) : tr2({ uz: "Lentaga qo'ying", ru: "Положите на ленту" });
  return <Stage eyebrow={tr2({ uz: "Markaziy · lenta o'yini", ru: "Главная · игра с лентой" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Chamadonni <span className="italic" style={{ color: T.accent }}>o'zingiz</span> yig'ing — lentaga qo'ying.</>, ru: <>Соберите чемодан <span className="italic" style={{ color: T.accent }}>сами</span> — и положите на ленту.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "buyumlar (bosib chamadonga soling)", ru: "вещи (нажмите, чтобы положить в чемодан)" })}</p>
            <div className="cj-items fade-up delay-1">
              {ITEMS.map((it) => {
    const on = selected.has(it.id);
    const isFixed = fixed.has(it.id);
    return <div key={it.id} style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                    <button className={`itm-card ${on ? "on" : ""} ${selected.size === 0 ? "tap-hint" : ""}`} disabled={running} onClick={() => toggleItem(it.id)}>
                      {on && <span className="itm-check">✓</span>}
                      <span className="itm-ico">{isFixed ? "🟢" : it.icon}</span>
                      <span className="itm-nm">{it.label}</span>
                    </button>
                    {on && it.failAt && !isFixed && result === "fail" && <button className="itm-fix" onClick={() => fixItem(it.id)}>{tr2({ uz: "🔧 Tuzatish", ru: "🔧 Исправить" })}</button>}
                  </div>;
  })}
            </div>
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={running || selected.size === 0} onClick={runBelt}>{running ? tr2({ uz: "● Lenta aylanmoqda…", ru: "● Лента крутится…" }) : result === "fail" ? tr2({ uz: "↻ Qayta lentaga qo'ying", ru: "↻ Положить на ленту снова" }) : tr2({ uz: "🧳 Lentaga qo'ying", ru: "🧳 Положить на ленту" })}</button>
            <Belt statuses={ptStatus} />
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "📟 LENTA JURNALI", ru: "📟 ЖУРНАЛ ЛЕНТЫ" })}</p>
            <div className="belt-log">
              {log.length === 0 && <span className="belt-log-empty">{tr2({ uz: "Chamadonni lentaga qo'ying — jurnal shu yerda yoziladi…", ru: "Положите чемодан на ленту — журнал будет писаться здесь…" })}</span>}
              {log.map((l) => <div key={l.k} className={`belt-log-line ${l.tone}`}>{l.text}</div>)}
            </div>
            <PhoneMock state={result === "fail" ? "broken" : everSucceeded ? "new" : "old"} />
            {result === "fail" && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Qizil chiroq — lenta to'xtadi. Foydalanuvchi <b>hech narsa sezmadi</b>: eski, ishlaydigan sayt joyida qoldi. Buzuq buyumni chamadondan chiqaring yoki «Tuzatish» bosing.</>, ru: <>Красный свет — лента остановилась. Пользователь <b>ничего не заметил</b>: старый рабочий сайт остался на месте. Уберите сломанную вещь из чемодана или нажмите «Исправить».</> })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>🎉 Hamma nuqta yashil — samolyot ko'tarildi! Siz buzuq kodni push qilgan bo'lsangiz ham, foydalanuvchi buni <b>umuman ko'rmadi</b> — lenta uni sizgacha tutib qoldi.</>, ru: <>🎉 Все точки зелёные — самолёт взлетел! Даже когда вы пушили сломанный код, пользователь его <b>вообще не увидел</b> — лента перехватила его раньше.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Chamadonga buyum solib lentani yuboring — nima o'tmasa, lenta shuni aytadi.", ru: "💡 Положите вещи в чемодан и отправьте по ленте — что не пройдёт, лента сама скажет." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
/>;
var WHY = [
  { id: "speed", icon: "⚡", t: { uz: "Tezlik", ru: "Скорость" }, d: { uz: "Push qildingiz — bir necha daqiqada skaner, o'rash va uchirish o'zi bo'ladi. Qo'lda 20 daqiqalik ish — soniyalarda.", ru: "Сделали push — через пару минут сканер, упаковка и взлёт проходят сами. 20 минут ручной работы — за секунды." } },
  { id: "trust", icon: "🛡", t: { uz: "Ishonch", ru: "Уверенность" }, d: { uz: "Nuqta qizil bo'lsa uchirish to'xtaydi. Buzuq yuk hech qachon uchmaydi — «juma kuni deploy qo'rquvi» yo'qoladi.", ru: "Если точка красная — взлёт останавливается. Сломанный груз никогда не улетит: страх «пятничного деплоя» исчезает." } },
  { id: "team", icon: "👥", t: { uz: "Jamoa", ru: "Команда" }, d: { uz: "Har kishi push qiladi, lenta hammani bir xil tekshiradi. «Mening kompimda ishlayapti-ku» muammosi tugaydi.", ru: "Каждый делает push, лента проверяет всех одинаково. Проблема «а у меня на компе работает» исчезает." } }
];
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState3(storedAnswer ? new Set(WHY.map((w) => w.id)) : /* @__PURE__ */ new Set());
  const [active, setActive] = useState3(null);
  const [sc, setSc] = useState3(0);
  ...
  return <Stage eyebrow={tr2({ uz: "Tushuncha · nega", ru: "Понятие · зачем" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `3 foydani ko'ring (${seen.size}/3)`, ru: `Посмотрите 3 выгоды (${seen.size}/3)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Nega bu mehnatni <span className="italic" style={{ color: T.accent }}>avtomatlashtiramiz</span>?</>, ru: <>Зачем <span className="italic" style={{ color: T.accent }}>автоматизировать</span> этот труд?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>CI/CD uchta katta narsani beradi: <b style={{ color: T.ink }}>tezlik, ishonch va jamoa hamohangligi</b>. Har birini bosib ko'ring.</>, ru: <>CI/CD даёт три больших плюса: <b style={{ color: T.ink }}>скорость, уверенность и слаженность команды</b>. Нажмите на каждый.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {WHY.map((w) => <button key={w.id} className="vcard" onClick={() => tap(w.id)} style={{ boxShadow: active === w.id ? `inset 0 0 0 1.5px ${T.accent}, 0 8px 20px -6px rgba(${T.shadowBase},0.2)` : void 0 }}>
                  <span className="role-ico">{w.icon}</span>
                  <span className="vlbl">{tr2(w.t)}</span>
                  <span className="vseen" style={{ color: seen.has(w.id) ? T.success : T.ink3 }}>{seen.has(w.id) ? "✓" : ""}</span>
                </button>)}
            </div>
          </Col>
          <Col>
            {cur ? <div className="frame fade-step" key={active}><p className="note-h"><span style={{ fontSize: 20, marginRight: 6 }}>{cur.icon}</span>{tr2(cur.t)}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2(cur.d)}</p></div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Foydani bosing ←", ru: "Нажмите на выгоду ←" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Tezlik · Ishonch · Jamoa. Shuning uchun haqiqiy ish jamoalarining deyarli barchasi CI/CD ishlatadi.", ru: "Скорость · Уверенность · Команда. Поэтому почти все рабочие команды используют CI/CD." })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Uchala foydani ham oching — lenta nimani tejashini o'zingiz sanang.", ru: "💡 Откройте все три выгоды — сами посчитайте, что экономит лента." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Hayotiy · jamoa", ru: "Из жизни · команда" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Ikkinchi holatni ko'ring", ru: "Посмотрите второй случай" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Juma, soat 18:00 — kod push qilindi. <span className="italic" style={{ color: T.accent }}>Lenta bormi</span>?</>, ru: <>Пятница, 18:00 — код запушен. <span className="italic" style={{ color: T.accent }}>Есть ли лента</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="frame" style={{ borderLeft: `4px solid ${T.danger}` }}>
              <p className="note-h" style={{ color: T.danger }}>{tr2({ uz: "❌ Lenta yo'q", ru: "❌ Ленты нет" })}</p>
              <p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Dasturchi shoshib qo'lda uchirdi — <b>skanerni o'tkazib yubordi</b>. Savatcha funksiyasida xato bor edi. Sayt buzildi, lekin kech — hamma uyga ketgan. Mijozlar dam olish kuni xarid qila olmadi. Dushanba ertalab muammo aniqlandi.</>, ru: <>Разработчик второпях задеплоил вручную — <b>пропустил сканер</b>. В функции корзины была ошибка. Сайт сломался, но поздно — все ушли домой. Все выходные клиенты не могли ничего купить. Проблему нашли только в понедельник утром.</> })}</p>
            </div>
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={show} onClick={() => {
    setShow(true);
    setSc((n) => n + 1);
  }}>{show ? tr2({ uz: "✓ Ko'rdingiz", ru: "✓ Просмотрено" }) : tr2({ uz: "Lenta bilan-chi?", ru: "А если с лентой?" })}</button>
          </Col>
          <Col>
            {show ? <div className="frame fade-step" style={{ borderLeft: `4px solid ${T.success}` }}>
                  <p className="note-h" style={{ color: T.success }}>{tr2({ uz: "✅ Lenta bor", ru: "✅ Лента есть" })}</p>
                  <p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Push qilindi → lenta skanerni ishga tushirdi → <b style={{ color: T.danger }}>qizil</b> → uchirish <b>to'xtadi</b>. Dasturchiga darhol xabar keldi. U 5 daqiqada tuzatdi, qayta push qildi → yashil → samolyot avtomatik ko'tarildi. Mijoz buzuq saytni umuman ko'rmadi.</>, ru: <>Сделан push → лента запустила сканер → <b style={{ color: T.danger }}>красный</b> → взлёт <b>остановлен</b>. Разработчику сразу пришло уведомление. Он исправил за 5 минут, снова запушил → зелёный → самолёт взлетел автоматически. Клиент так и не увидел сломанный сайт.</> })}</p>
                </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Tugmani bosing ←", ru: "Нажмите кнопку ←" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Bir xil xato — butunlay boshqa yakun. Lenta xatoni <b>uchishdan oldin</b> tutadi. Mana shuning uchun kerak.</>, ru: <>Одна и та же ошибка — совершенно разный финал. Лента ловит ошибку <b>до взлёта</b>. Вот зачем она нужна.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Ikkinchi holatni ham oching — ikki holat farqi asosiy gapni ko'rsatadi.", ru: "💡 Откройте и второй случай — разница между ними и есть главная мысль." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var YAML_BLANKS = [
  { key: "on", label: "on:", correct: "push", options: ["push", "once", "pull"], wrong: { once: { uz: "«once» degan signal yo'q — lenta qachon aylanishini bilmayapti.", ru: "Сигнала «once» не существует — лента не поймёт, когда запускаться." }, pull: { uz: "Bu darsda «pull» emas, oddiy push signalini o'rganamiz.", ru: "В этом уроке мы учим обычный сигнал push, а не «pull»." } } },
  { key: "runsOn", label: "runs-on:", correct: "ubuntu-latest", options: ["ubuntu-latest", "my-laptop", "windows-me"], wrong: { "my-laptop": { uz: "Lenta mashinasi sizning kompyuteringiz emas — alohida muhitda ishlaydi.", ru: "Машина ленты — не ваш компьютер, она работает в отдельной среде." }, "windows-me": { uz: "Bunday muhit yo'q — standart nomi kerak.", ru: "Такой среды нет — нужно стандартное имя." } } },
  { key: "run", label: "run:", correct: "npm test", options: ["npm test", "npm forget", "npm delete"], wrong: { "npm forget": { uz: "Bunday buyruq yo'q — skaner nuqtasida testlarni ishga tushiramiz.", ru: "Такой команды нет — на точке сканера мы запускаем тесты." }, "npm delete": { uz: "Bu hech narsani tekshirmaydi — skaner uchun test buyrug'i kerak.", ru: "Это ничего не проверяет — сканеру нужна команда тестов." } } }
];
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [filled, setFilled] = useState3(() => storedAnswer ? Object.fromEntries(YAML_BLANKS.map((b) => [b.key, b.correct])) : {});
  const [wrongKey, setWrongKey] = useState3(null);
  const [wrongMsg, setWrongMsg] = useState3("");
  const wrongEverRef = useRef3(storedAnswer ? storedAnswer.correct === false : false);
  const [sc, setSc] = useState3(0);
  const done = YAML_BLANKS.every((b) => filled[b.key] === b.correct);
  const { tip: _tip, rescue: _resc } = useStuckValve(done, Object.keys(filled).length);
  const fired = useRef3(!!storedAnswer);
  useEffect4(() => {
    if (done && !fired.current) {
      fired.current = true;
      onAnswer(screen, { stage: "builder", screenIdx: screen, question: "ci.yml bo'shliqlarini to'ldiring", correct: !wrongEverRef.current, solved: true, picked: true });
    }
  }, [done]);
  const pick = (blank, val) => {
    if (filled[blank.key] === blank.correct) return;
    if (val === blank.correct) {
      setFilled((f) => ({ ...f, [blank.key]: val }));
      setWrongKey(null);
      setSc((n) => n + 1);
    } else {
      wrongEverRef.current = true;
      setWrongKey(blank.key);
      setWrongMsg(tr2(blank.wrong[val]) || tr2({ uz: "Bu to'g'ri emas.", ru: "Это неверно." }));
      setTimeout(() => setWrongKey((k) => k === blank.key ? null : k), 500);
    }
  };
  return <Stage eyebrow={tr2({ uz: "Amaliyot · yo'l xaritasi", ru: "Практика · маршрутная карта" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Bo'shliqlarni to'ldiring", ru: "Заполните пропуски" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Lentaning <span className="italic" style={{ color: T.accent }}>yo'l xaritasini</span> o'zingiz to'ldiring.</>, ru: <>Заполните <span className="italic" style={{ color: T.accent }}>маршрутную карту</span> ленты сами.</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Bu — <span className="mono">ci.yml</span>: lenta har push'da nima qilishini shu fayl aytadi. Uchta bo'shliqni to'g'ri chipdan tanlab to'ldiring.</>, ru: <>Это — <span className="mono">ci.yml</span>: этот файл говорит ленте, что делать на каждый push. Заполните три пропуска, выбирая правильные чипы.</> })}</Mentor>
        <Zoomable><div className="split">
          <Col>
            <p className="flow-label">.github/workflows/ci.yml</p>
            <pre className="code-box" style={{ lineHeight: 1.9 }}>
              <At>name</At>{": CI"}{"\n"}
              <At>on</At>{": "}<Jx>{filled.on || "____"}</Jx>{"        "}<Cm>{tr2({ uz: "# start signali", ru: "# стартовый сигнал" })}</Cm>{"\n"}
              <At>jobs</At>{":"}{"\n"}
              {"  "}<At>test</At>{":"}{"\n"}
          ...
          <Col>
            {YAML_BLANKS.map((b) => <div key={b.key} className="blank-group">
                <span className="bg-lbl">{b.label}</span>
                <div className="blank-row">
                  {b.options.map((opt) => {
    const okChosen = filled[b.key] === opt;
    return <button key={opt} className={`gchip ${wrongKey === b.key ? "shake" : ""} ${filled[b.key] === b.correct ? "" : "tap-hint"}`} disabled={filled[b.key] === b.correct} onClick={() => pick(b, opt)} style={okChosen ? { boxShadow: `inset 0 0 0 1.5px ${T.success}`, color: T.success } : void 0}>{okChosen ? "✓ " : ""}{opt}</button>;
  })}
                </div>
              </div>)}
            {wrongKey && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{wrongMsg}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Yo'l xaritasi to'ldi! Endi bu fayl repo'ga qo'shilsa, lenta har push'da o'zi ishga tushadi.", ru: "Маршрутная карта заполнена! Теперь, если добавить этот файл в репозиторий, лента будет запускаться сама на каждый push." })}</p></div>}
          </Col>
        </div></Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Har bo'shliqqa qarang: bu qator lentaga qachon, qayerda yoki nima qilishni aytyaptimi?", ru: "💡 Посмотрите на каждый пропуск: эта строка говорит ленте когда, где или что делать?" })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen14 = (props) => <QuestionScreen
  ...
  idx={14}
  scope="module-mikro"
  tip={{ uz: "💡 `on:` qatori nimani anglatishini eslang — lentaga start signalini kim beradi?", ru: "💡 Вспомните, что означает строка `on:` — кто подаёт ленте стартовый сигнал?" }}
  eyebrow={tr2({ uz: "Mashq · 4-savol", ru: "Практика · вопрос 4" })}
  questionText="Lenta (pipeline) qachon ishga tushadi?"
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Lenta odatda <span className="italic" style={{ color: T.accent }}>qachon</span> ishga tushadi?</>, ru: <>Когда <span className="italic" style={{ color: T.accent }}>обычно</span> запускается лента?</> })}</h2></>}
  options={[{ uz: "Faqat men qo'lda maxsus tugma bossam", ru: "Только когда я вручную нажму специальную кнопку" }, { uz: "Har safar kod push qilinganda — avtomatik", ru: "Каждый раз, когда код запушен, — автоматически" }, { uz: "Yiliga bir marta", ru: "Раз в год" }, { uz: "Hech qachon — uchirishni baribir o'zim qilaman", ru: "Никогда — деплой я всё равно делаю сам" }]}
  correctIdx={1}
...
/>;
var BELT_ITEMS = POINTS.map((p) => ({ id: p.id, label: { uz: `${p.icon} ${p.label.uz}`, ru: `${p.icon} ${p.label.ru}` } }));
var BELT_ORDER = POINTS.map((p) => p.id);
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const onSolved = () => {
    ...
    setDone(true);
    onAnswer(screen, { stage: "final", screenIdx: screen, question: "Lenta nuqtalarini to'g'ri tartibda joylang", correct: firstOk, firstAttemptCorrect: firstOk, solved: true, picked: firstOk ? 0 : 1 });
  };
  const onChange = (slots) => {
    ...
    const solved = slots.every((s, i) => s === BELT_ORDER[i]);
    if (solved) {
      setConsequence(null);
      return;
    }
    hadWrongRef.current = true;
    const deployIdx = slots.indexOf("deploy");
    const testIdx = slots.indexOf("test");
    setConsequence(deployIdx >= 0 && testIdx >= 0 && deployIdx < testIdx ? "plane-early" : "wrong");
  };
  return <Stage eyebrow={tr2({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done && !_resc} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : _resc ? tr2({ uz: "Lentani birga yig'amiz →", ru: "Соберём ленту вместе →" }) : tr2({ uz: "Lentani yig'ing", ru: "Соберите ленту" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Oxirgi qadam: lentani <span className="italic" style={{ color: T.accent }}>to'g'ri tartibda</span> yig'ing.</>, ru: <>Последний шаг: соберите ленту <span className="italic" style={{ color: T.accent }}>в правильном порядке</span>.</> })}</h2></div>
        <Mentor>{tr2({ uz: "Nuqtalarni sudrab to'g'ri tartibga joylang. Diqqat: agar ✈️ Uchirishni 🔍 Skanerdan oldin qo'ysangiz — oqibatini ko'rasiz. Keyin to'g'rilaysiz.", ru: "Перетащите точки в правильный порядок. Внимание: если поставите ✈️ Взлёт раньше 🔍 Сканера — увидите последствия. Потом исправите." })}</Mentor>
        <DragDropOrder
    items={BELT_ITEMS}
    ...
    onChange={onChange}
  />
        {consequence === "plane-early" && !done && <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.danger }}>{tr2({ uz: "💥 Samolyot Skanergacha uchib ketdi!", ru: "💥 Самолёт улетел до Сканера!" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Tekshirilmagan yuk internetga chiqdi — foydalanuvchi buzuq saytni ko'rdi. Tartibni to'g'rilang.", ru: "Непроверенный груз попал в интернет — пользователь увидел сломанный сайт. Исправьте порядок." })}</p><PhoneMock state="broken" /></div>}
        {consequence === "wrong" && !done && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Tartib xato — bo'lakni bosib qaytaring va qaytadan joylang.", ru: "Порядок неверный — нажмите на блок, чтобы вернуть его, и разложите заново." })}</p></div>}
        {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>✓ Lenta tayyor: <b>Yig'ish → Skaner → Ramka → O'rash → Uchirish</b>. Har push'da shu tartibda avtomatik ishlaydi.</>, ru: <>✓ Лента готова: <b>Сборка → Сканер → Рамка → Упаковка → Взлёт</b>. На каждый push она работает автоматически именно в этом порядке.</> })}</p><PhoneMock state="new" />
        ...
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Nuqtalarni tartib bilan joylang: lenta xatoni yuk o'ralishidan OLDIN topishi kerak.", ru: "💡 Расставьте точки по порядку: лента должна найти ошибку ДО того, как груз упакован." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Lentani keyinroq birga yig'amiz — hozir davom etsangiz bo'ladi.", ru: "Соберём ленту вместе позже — сейчас можно продолжить." })}</p>}
      </div>
    </Stage>;
};
var ACHIEVEMENTS = {
  redLight: { icon: "🚦", name: "Red Light", desc: { uz: "Nega lenta to'xtashini tushundingiz", ru: "Вы поняли, почему лента останавливается" } },
  clearedForTakeoff: { icon: "🛫", name: "Cleared for Takeoff", desc: { uz: "Barcha buyumlarni tuzatib, samolyotni ko'tardingiz", ru: "Вы исправили все вещи и подняли самолёт" } },
  routeReader: { icon: "🗺️", name: "Route Reader", desc: { uz: "Yo'l xaritasini xatosiz o'qidingiz", ru: "Вы прочитали маршрутную карту без ошибок" } },
  orderMatters: { icon: "🔀", name: "Order Matters", desc: { uz: "Lenta tartibini 1-urinishda to'g'ri terdingiz", ru: "Вы собрали порядок ленты с первой попытки" } }
};
var ACH_TRIGGERS = { s9: "clearedForTakeoff", s10: "redLight", s13: "routeReader", s15: "orderMatters" };
...
var Q_LABELS = { 4: "1 — CI/CD", 8: { uz: "2 — CI/CD farqi", ru: "2 — разница CI/CD" }, 10: { uz: "3 — Qizil chiroq", ru: "3 — красный свет" }, 14: { uz: "4 — Start signali", ru: "4 — стартовый сигнал" }, 15: { uz: "5 — Tartib", ru: "5 — порядок" } };
var QUIZ_MS = 15e3;
var QZ_BG_SHAPES = [
  { ch: "on: push", l: 5, t: 10, s: 32, d: 19, dl: 0 },
  { ch: "📦", l: 85, t: 8, s: 32, d: 23, dl: 1.5 },
  { ch: "npm test", l: 8, t: 72, s: 26, d: 27, dl: 0.8 },
  { ch: "eslint", l: 76, t: 68, s: 26, d: 21, dl: 2.2 },
  { ch: "build", l: 45, t: 86, s: 24, d: 25, dl: 1.1 },
  { ch: "deploy", l: 66, t: 26, s: 26, d: 17, dl: 0.4 },
  { ch: "runs-on", l: 26, t: 34, s: 22, d: 20, dl: 1.9 },
  { ch: "ci.yml", l: 55, t: 5, s: 26, d: 22, dl: 0.6 },
  { ch: "✗", l: 91, t: 42, s: 26, d: 24, dl: 1.3 },
  { ch: "✓", l: 16, t: 52, s: 26, d: 26, dl: 2.6 },
  { ch: "pipeline", l: 34, t: 62, s: 20, d: 29, dl: 3.4 },
  { ch: "CI", l: 2, t: 30, s: 26, d: 28, dl: 3.1 },
  { ch: "CD", l: 60, t: 90, s: 20, d: 31, dl: 4.2 },
  { ch: "secret", l: 20, t: 16, s: 22, d: 18, dl: 2.9 }
];
var QUIZ_BANK = [
  { q: { uz: "Pipeline (lenta) nima?", ru: "Что такое pipeline (лента)?" }, opts: [{ uz: "Kodni chiroyli qilib formatlaydigan alohida dastur", ru: "Отдельная программа, которая красиво форматирует код" }, { uz: "Faqat testlarni yozib beradigan yordamchi vosita", ru: "Вспомогательный инструмент, который только пишет тесты" }, { uz: "Push qilingan kodni avtomatik tekshiradigan ketma-ket nuqtalar", ru: "Цепочка точек, автоматически проверяющая запушенный код" }, { uz: "Serverni qo'lda qayta ishga tushiruvchi maxsus buyruq, hech narsani tekshirmaydi", ru: "Специальная команда ручного перезапуска сервера, ничего не проверяет" }], correct: 2 },
  { q: { uz: "`npm install` qaysi nuqtada ishlaydi?", ru: "На какой точке работает `npm install`?" }, opts: [{ uz: "Yig'ish (install) — birinchi nuqtada ishlaydi", ru: "Сборка (install) — работает на первой точке" }, { uz: "Uchirish (deploy) nuqtasida, eng oxirida ishga tushadi", ru: "На точке Взлёт (deploy), запускается в самом конце" }, { uz: "O'lcham ramkasi (lint) nuqtasida ishlaydi", ru: "На точке рамки (lint)" }, { uz: "O'rash (build) nuqtasida bajariladi", ru: "Выполняется на точке Упаковка (build)" }], correct: 0 },
  { q: { uz: "Test qizil (FAIL) bo'lsa nima bo'ladi?", ru: "Что будет, если тест красный (FAIL)?" }, opts: [{ uz: "Lenta baribir davom etadi va uchirishni to'liq bajaraveradi", ru: "Лента всё равно продолжит и полностью выполнит взлёт" }, { uz: "Lenta shu nuqtada to'xtaydi, keyingisi o'tkazib yuboriladi", ru: "Лента остановится на этой точке, следующие будут пропущены" }, { uz: "Server butunlay o'chib, ishlamay qoladi", ru: "Сервер полностью выключится и перестанет работать" }, { uz: "Kod xatosi avtomatik ravishda o'zi tuzatiladi", ru: "Ошибка в коде исправится сама, автоматически" }], correct: 1 },
  { q: { uz: "CI (Continuous Integration) nimani anglatadi?", ru: "Что означает CI (Continuous Integration)?" }, opts: [{ uz: "Sayt dizaynini doimiy yangilab turish jarayoni", ru: "Процесс постоянного обновления дизайна сайта" }, { uz: "Serverni uzluksiz kuzatib turadigan monitoring", ru: "Мониторинг, который непрерывно следит за сервером" }, { uz: "Kodni qo'lda birlashtiradigan eski usul", ru: "Старый способ объединять код вручную" }, { uz: "Har push'ni birlashtirib avtomatik tekshirish", ru: "Объединять каждый push и автоматически проверять" }], correct: 3 },
  { q: { uz: "CD (Continuous Deployment) nima qiladi?", ru: "Что делает CD (Continuous Deployment)?" }, opts: [{ uz: "Tekshiruvdan o'tgan yukni avtomatik internetga chiqaradi", ru: "Автоматически выкладывает проверенный груз в интернет" }, { uz: "Kodni faqat serverga qo'lda, o'zi nusxalaydi doimo", ru: "Постоянно копирует код на сервер вручную" }, { uz: "Testlarni o'zi o'ylab yozib chiqadi", ru: "Сам придумывает и пишет тесты" }, { uz: "Loyihani repozitoriyga yuklab qo'yadi", ru: "Загружает проект в репозиторий" }], correct: 0 },
  { q: { uz: "`on: push` nimani bildiradi?", ru: "Что означает `on: push`?" }, opts: [{ uz: "Lenta faqat maxsus tugma bosilganda ishlaydi", ru: "Лента работает только по нажатию специальной кнопки" }, { uz: "Lenta yiliga bir marta ishga tushadi", ru: "Лента запускается раз в год" }, { uz: "Lenta har push'da avtomatik ishga tushadi", ru: "Лента запускается автоматически на каждый push" }, { uz: "Lenta hech qachon o'z-o'zidan ishlamaydi", ru: "Лента никогда не запускается сама" }], correct: 2 },
  { q: { uz: "`eslint .` qaysi nuqtada ishlatiladi?", ru: "На какой точке используется `eslint .`?" }, opts: [{ uz: "Uchirish (deploy) nuqtasida, oxirida ishlatiladi", ru: "На точке Взлёт (deploy), в самом конце" }, { uz: "Yig'ish (install) nuqtasida ishlatiladi", ru: "На точке Сборка (install)" }, { uz: "O'rash (build) nuqtasida ishlatiladi", ru: "На точке Упаковка (build)" }, { uz: "O'lcham ramkasi (lint) nuqtasida ishlatiladi", ru: "На точке рамки (lint)" }], correct: 3 },
  { q: { uz: "Artefakt (o'ralgan yuk) qaysi nuqtada tayyor bo'ladi?", ru: "На какой точке готов артефакт (упакованный груз)?" }, opts: [{ uz: "Skaner (test) nuqtasida tayyor bo'ladi", ru: "На точке Сканер (test)" }, { uz: "O'rash (build) nuqtasida tayyor bo'ladi", ru: "На точке Упаковка (build)" }, { uz: "Yig'ish (install) nuqtasida tayyor bo'ladi", ru: "На точке Сборка (install)" }, { uz: "Uchirish (deploy)dan keyin tayyor bo'ladi", ru: "После точки Взлёт (deploy)" }], correct: 1 },
  { q: { uz: "`runs-on: ubuntu-latest` nimani bildiradi?", ru: "Что означает `runs-on: ubuntu-latest`?" }, opts: [{ uz: "Loyihaning to'liq rasmiy nomini", ru: "Полное официальное название проекта" }, { uz: "Bazaga ulanish uchun maxfiy parolni", ru: "Секретный пароль для подключения к базе" }, { uz: "Lenta mashinasi qanday muhitda ishlashini bildiradi", ru: "В какой среде работает машина ленты" }, { uz: "Lentada nechta test borligini", ru: "Сколько тестов на ленте" }], correct: 2 },
  { q: { uz: "Uchirish avtomatik bo'lishi uchun nima shart?", ru: "Что нужно, чтобы взлёт был автоматическим?" }, opts: [{ uz: "Dasturchi maxsus tugmani bosishi shart", ru: "Разработчик должен нажать специальную кнопку" }, { uz: "Kod hajmi juda kichik bo'lishi shart", ru: "Код должен быть совсем маленьким" }, { uz: "Hafta oxiri kunlari bo'lishi shart", ru: "Должны быть выходные дни" }, { uz: "Barcha oldingi nuqtalar yashil bo'lishi shart", ru: "Все предыдущие точки должны быть зелёными" }], correct: 3 },
  { q: { uz: "Rollback (eski yukni qaytarish) qachon kerak bo'ladi?", ru: "Когда нужен rollback (возврат старого груза)?" }, opts: [{ uz: "Yangi versiya productionda muammo chiqarganda", ru: "Когда новая версия вызвала проблемы в production" }, { uz: "Har push'da doim, sharoitidan qat'i nazar", ru: "Всегда, на каждый push, независимо от ситуации" }, { uz: "Test hali yashil bo'lgan paytda", ru: "Пока тесты ещё зелёные" }, { uz: "Lenta hali ishga tushmagan paytda", ru: "Пока лента ещё не запускалась" }], correct: 0 },
  { q: { uz: "Secret (seyf/maxfiy kalit) qayerda saqlanadi?", ru: "Где хранится secret (секретный ключ)?" }, opts: [{ uz: "ci.yml faylining o'zida, hammaga ochiq holda turadi", ru: "Прямо в файле ci.yml, открыто для всех" }, { uz: "Maxsus xavfsiz seyf sozlamalarida", ru: "В специальных защищённых настройках-сейфе" }, { uz: "README faylining oxirgi qatorida", ru: "В последней строке файла README" }, { uz: "Terminal buyruqlar tarixida", ru: "В истории команд терминала" }], correct: 1 }
];
  ...
  return <div className="qz-arena">
    ...
    return <div key={i} className={`qz-tile rv ${win ? "win" : "lose"} ${pickedThis ? "picked" : ""}`} style={{ background: QUIZ_COLORS[i] }}>
      ...
      {phase === "done" && <div className="qz-view fade-step">
          {
    /* 141-qonun (F-0820-388): yakun-marosimi BITTA — konfetti podiumda qoladi.
...
function ScreenLivePractice({ title, task, checklist, screen, storedAnswer, onAnswer, onNext, onPrev, live }) {
  ...
  return <Stage eyebrow={tr2({ uz: "Amaliyot · VS Code", ru: "Практика · VS Code" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Avval bajaring", ru: "Сначала выполните" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "Bosqichlar — belgilab boring", ru: "Шаги — отмечайте" })}</p>
    ...
    return <button key={i} className={`lp-step ${on ? "on" : ""}`} onClick={() => toggle(i)}>
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Ajoyib! Vazifani bajardingiz. Ustoz tekshirib, keyingi qadamga o'tkazadi.", ru: "Отлично! Задание выполнено. Наставник проверит и переведёт вас на следующий шаг." })}</p></div>}
          </Col>
        </div>
      </div>
    </Stage>;
...
var ScreenCicdPractice = (props) => <ScreenLivePractice
  {...props}
  title={{ uz: "O'z loyihangizga kichik CI lenta qo'shing", ru: "Добавьте небольшую CI-ленту в свой проект" }}
  task={{ uz: "GitHub'dagi bitta repo uchun .github/workflows/ci.yml yozing: har push'da paketlar o'rnatilsin va testlar ishga tushsin. Keyin bitta o'zgarish push qiling va Actions bo'limida lentani jonli kuzating.", ru: "Для одного из ваших репозиториев на GitHub напишите .github/workflows/ci.yml: на каждый push пусть устанавливаются пакеты и запускаются тесты. Затем запушьте одно изменение и наблюдайте за лентой вживую во вкладке Actions." }}
  checklist={[
    { uz: "Repo ichida `.github/workflows/ci.yml` faylini yarating", ru: "Создайте файл `.github/workflows/ci.yml` в репозитории" },
    { uz: "`on: push`, `runs-on: ubuntu-latest` va `npm install` qadamini yozing", ru: "Напишите `on: push`, `runs-on: ubuntu-latest` и шаг `npm install`" },
    { uz: "`npm test` qadamini qo'shing — skaner nuqtasi", ru: "Добавьте шаг `npm test` — точка сканера" },
    { uz: "O'zgarishni push qiling", ru: "Запушьте изменение" },
    { uz: "`Actions` bo'limini oching va lentaning yashil/qizil bo'lishini kuzating", ru: "Откройте вкладку `Actions` и наблюдайте, как лента становится зелёной или красной" }
  ]}
/>;
var CICD_FLASHCARDS = [
  { front: { uz: "CI qisqartmasi qanday ishni bildiradi?", ru: "Какую работу обозначает сокращение CI?" }, back: "Continuous Integration", note: { uz: "har push birlashtiriladi va avtomatik tekshiriladi", ru: "каждый push объединяется и автоматически проверяется" } },
  { front: { uz: "CD qisqartmasi qanday ishni bildiradi?", ru: "Какую работу обозначает сокращение CD?" }, back: "Continuous Deployment", note: { uz: "tekshiruvdan o'tgan yuk o'zi internetga chiqadi", ru: "проверенный груз сам выкладывается в интернет" } },
  { front: { uz: "Ketma-ket nuqtalardan iborat avtomatik lenta qanday ataladi?", ru: "Как называется автоматическая лента из последовательных точек?" }, back: "Pipeline", note: { uz: "Yig'ish, Skaner, Ramka, O'rash, Uchirish", ru: "Сборка, Сканер, Рамка, Упаковка, Взлёт" } },
  { front: { uz: "Lentani nima ishga tushiradi?", ru: "Что запускает ленту?" }, back: "push", note: { uz: "ci.yml ichidagi on: push — start signali", ru: "on: push в ci.yml — стартовый сигнал" } },
  { front: { uz: "Lentaning birinchi nuqtasi nima qiladi?", ru: "Что делает первая точка ленты?" }, back: { uz: "Kerakli paketlarni o'rnatadi", ru: "Устанавливает нужные пакеты" }, note: "📦 npm install" },
  { front: { uz: "Testlar lentaning qaysi nuqtasida ishga tushadi?", ru: "На какой точке ленты запускаются тесты?" }, back: { uz: "Skaner nuqtasida", ru: "На точке Сканер" }, note: "🔍 npm test" },
  { front: { uz: "Kod uslubini qoidalarga solishtiradigan nuqta qaysi?", ru: "Какая точка сверяет стиль кода с правилами?" }, back: "Lint", note: "📐 eslint ." },
  { front: { uz: "Saytni internetga tayyor holatga qaysi nuqta o'raydi?", ru: "Какая точка упаковывает сайт для интернета?" }, back: "Build", note: "🎁 npm run build" },
  { front: { uz: "Tayyor yukni internetga chiqaradigan oxirgi nuqta qaysi?", ru: "Какая последняя точка выкладывает готовый груз в интернет?" }, back: "Deploy", note: { uz: "✈️ uchirish — faqat hammasi yashil bo'lsa", ru: "✈️ взлёт — только если всё зелёное" } },
  { front: { uz: "Lentaning ko'rsatmalari qaysi faylda yoziladi?", ru: "В каком файле записаны инструкции ленты?" }, back: "ci.yml", note: { uz: ".github/workflows papkasida — lentaning yo'l xaritasi", ru: "в папке .github/workflows — маршрутная карта ленты" } },
  { front: { uz: "Bitta nuqta qizil bersa, lenta bilan nima bo'ladi?", ru: "Что происходит с лентой, если одна точка даёт красный?" }, back: { uz: "To'xtaydi", ru: "Останавливается" }, note: { uz: "keyingi nuqtalar o'tkazib yuboriladi, buzuq yuk uchmaydi", ru: "следующие точки пропускаются, сломанный груз не улетает" } },
  { front: { uz: "Maxfiy kalitlarni qayerda saqlaysiz?", ru: "Где вы храните секретные ключи?" }, back: "Secret", note: { uz: "seyf: kalit ci.yml ichida ochiq yozilmaydi", ru: "сейф: ключ не пишется открыто в ci.yml" } }
];
var ScreenFlashcards = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Takrorlash", ru: "Повторение" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={false} label={tr2({ uz: "Yakunlash →", ru: "Завершить →" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="fc-center"><Flashcards cards={CICD_FLASHCARDS} /></div>
      </div>
    </Stage>;
};
var SummaryScreen = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  const RECAP = [
    tr2({ uz: "CI/CD — kodni har o'zgarishda avtomatik tekshiradigan va uchiradigan lenta", ru: "CI/CD — лента, которая автоматически проверяет и отправляет код при каждом изменении" }),
    tr2({ uz: "CI — har push birlashtiriladi va tekshiriladi (skaner, ramka)", ru: "CI — каждый push объединяется и проверяется (сканер, рамка)" }),
    tr2({ uz: "CD — tekshiruvdan o'tgan yuk avtomatik uchadi", ru: "CD — прошедший проверку груз улетает автоматически" }),
    tr2({ uz: "Lenta: Yig'ish → Skaner → Ramka → O'rash → Uchirish", ru: "Лента: Сборка → Сканер → Рамка → Упаковка → Взлёт" }),
    tr2({ uz: "Bitta nuqta qizil bersa — lenta to'xtaydi, buzuq yuk hech qachon uchmaydi", ru: "Одна красная точка — лента останавливается, сломанный груз никогда не улетит" })
  ];
  const HOMEWORK = [
    { b: tr2({ uz: "Sanang", ru: "Посчитайте" }), t: tr2({ uz: "— o'z loyihangizda deploy hozir nechta qo'lda qadamdan iborat?", ru: "— из скольких ручных шагов сейчас состоит деплой вашего проекта?" }) },
    { b: tr2({ uz: "Tartib", ru: "Порядок" }), t: tr2({ uz: "— lenta 5 nuqtasini yoddan to'g'ri tartibda yozing", ru: "— напишите по памяти 5 точек ленты в правильном порядке" }) },
    { b: tr2({ uz: "O'qing", ru: "Почитайте" }), t: tr2({ uz: "— GitHub Actions haqida qisqa material ko'rib keling", ru: "— посмотрите короткий материал про GitHub Actions" }) }
  ];
  ...
  return <Stage eyebrow={tr2({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "CI/CD lentasini tushundingiz", ru: "Вы разобрались в ленте CI/CD" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Endi kod o'zgarishini <span className="italic" style={{ color: T.accent }}>lenta</span> chiqaradi.</>, ru: <>Теперь изменения кода выкатывает <span className="italic" style={{ color: T.accent }}>лента</span>.</> })}</h2>{
    /* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>{tr2({ uz: "📝 Uyga vazifa", ru: "📝 Домашнее задание" })}</div><ul>{HOMEWORK.map((h, i) => <li key={i}><b>{h.b}</b> <span className="t">{h.t}</span></li>)}</ul><p className="hw-note">{tr2({ uz: "🚀 Keyingi dars — GitHub Actions: birinchi workflow'ni o'zimiz yozamiz va har push'da testlarni avtomatik ishga tushiramiz!", ru: "🚀 Следующий урок — GitHub Actions: сами напишем первый workflow и будем автоматически запускать тесты на каждый push!" })}</p></div>}
...
function CiCdIntroLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  const screens = [Screen0, Screen1, Screen2, Screen3, Screen4, Screen5, Screen6, Screen7, Screen8, Screen9, Screen10, Screen11, Screen12, Screen13, Screen14, Screen15, ScreenCicdPractice, ScreenPodium, ScreenFlashcards, SummaryScreen];
...
/* PRODUCTION: shu @import OLIB TASHLANADI — shriftlarni LMS yuklaydi (platform_contract). */
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={{ uz: "CI/CD darsi", ru: "Урок CI/CD" }} /> : <>
