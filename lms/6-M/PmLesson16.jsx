...
//  Manba:  src/4b-Modull/PmLesson16.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/4b-Modull/PmLesson16.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/4b-Modull/PmLesson16.jsx
...
var LESSON_META = { lessonId: "pm-m4b2-v1", lessonTitle: { uz: "Bitta xato — nechta odam ketadi?", ru: "Одна ошибка — сколько людей уйдёт?" } };
var SCREEN_META = [
...
var INLINE_KEYS = { s3: 2, s5: 0, s7: 1, s11: 0, tarozi: -1, practice: -1, navbat: -1, koding: -1 };
var RECAPS = {
  3: {
...
var somFmt = (n) => String(n).replace(/\B(?=(\d{3})+(?!\d))/g, " ");
var SkuterEkran = () => {
  const [som, setSom] = useState3(6200);
  useEffect4(() => {
    const t = setInterval(() => setSom((s) => s >= 9200 ? 6200 : s + 500), 1400);
    return () => clearInterval(t);
  }, []);
  return <div className="sph">
      <span className="sph-bar"><span className="bb-dots"><i /><i /><i /></span>{tr2({ uz: "Skuter · ijara", ru: "Самокат · аренда" })}</span>
      <div className="sph-rows">
        <span className="sph-row"><span className="sph-ic">⏱</span><span className="sph-t">{tr2({ uz: "Safar vaqti", ru: "Время поездки" })}</span><span className="sph-v mono">12:40</span></span>
        <span className="sph-row live"><span className="sph-ic">💸</span><span className="sph-t">{tr2({ uz: "Hisob", ru: "Счёт" })}</span><span className="sph-v mono">{tr2({ uz: `${somFmt(som)} so'm ▲`, ru: `${somFmt(som)} сум ▲` })}</span></span>
      </div>
      <span className="sph-btn">{tr2({ uz: "Tugatish ✓", ru: "Завершить ✓" })}</span>
      <span className="sph-note">{tr2({ uz: "bosildi — hisoblagich esa yuraveradi", ru: "нажата — а счётчик всё идёт" })}</span>
    </div>;
};
var HOOK_OPTS = [
...
var HOOK_KEY = "pm-m4b2-hook-choice";
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish · skuter ilovasi", ru: "Введение · приложение самокатов" })} screen={screen} navContent={<NavNext optionalLive turnBusy={picked === null && !isMentor} disabled={picked === null && !isMentor} label={opened ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Bittasini tanlang", ru: "Выберите один" })} onClick={onNext} />}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Ertaga yana shu ilovani <span className="italic" style={{ color: T.accent }}>ochasizmi?</span></>, ru: <>Откроете ли вы завтра это приложение <span className="italic" style={{ color: T.accent }}>снова?</span></> })}</h2></div>
        <Mentor>{tr2({ uz: "Skuterni QR kod bilan ochdingiz, yetib borib «Tugatish»ni bosdingiz — hisoblagich to'xtamadi, pul yozilib turibdi.", ru: "Вы открыли самокат по QR-коду, доехали и нажали «Завершить» — счётчик не остановился, деньги продолжают капать." })}</Mentor>
        ...
        {opened && <>
            ...
            <div className="frame-soft fade-step">
              <p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Ikkalasi ham bo'ladi: kimdir kechiradi, kimdir shu kuniyoq boshqasiga o'tadi. Ilovani yasagan jamoa buni bilmaydi — u faqat kamayib borayotgan odamlar sonini ko'radi. Nechta odam ketishi xato <b>qayerda tutilganiga</b> bog'liq — bugun shuni ko'rasiz.</>, ru: <>Бывает и так, и так: кто-то простит, а кто-то в тот же день перейдёт на другое приложение. Команда, которая его сделала, об этом не знает — она видит только убывающее число пользователей. Сколько людей уйдёт, зависит от того, <b>где поймали ошибку</b> — сегодня вы это увидите.</> })}</p>
            </div>
          </>}
        {
    ...
    return <div key={o.k} className={`hvote-row ${picked === i ? "mine" : ""} ${top ? "top" : ""}`}>
        ...
        <MentorNote>{tr2({ uz: "Ovozlar bo'linadi — ikkalasining ham hayotiy dalili bor: kechiradiganlar ham, boshqa ilovaga o'tadiganlar ham. Shu bo'linishning o'zi darsga eshik; javobni oldindan aytmang.", ru: "Голоса разделятся — у обеих сторон есть живой довод: и у тех, кто прощает, и у тех, кто уходит в другое приложение. Само это разделение — вход в урок; ответ заранее не говорите." })}</MentorNote>
      </div>
    </Stage>;
};
var DEMO_KARTA = [
  { t: { uz: "«Band qilish» bosilsa xato chiqadi", ru: "При нажатии «Забронировать» выходит ошибка" }, mark: "→", why: { uz: "Hammada · Ish to'xtaydi", ru: "У всех · Работа останавливается" } },
  { t: { uz: "Sharh yozish oynasi yopilmaydi", ru: "Окно отзыва не закрывается" }, mark: "→", why: { uz: "Ba'zilarda · Noqulay, lekin ishlaydi", ru: "У некоторых · Неудобно, но работает" } },
  { t: { uz: "Profil rasmi qo'yilsa yon ko'rinadi", ru: "Фото профиля встаёт боком" }, mark: "→", why: { uz: "Ba'zilarda · Noqulay, lekin ishlaydi", ru: "У некоторых · Неудобно, но работает" } }
];
var Screen1 = ({ screen, onNext, onPrev }) => <Stage eyebrow={tr2({ uz: "Maqsad", ru: "Цель" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Boshlaymiz →", ru: "Начнём →" })} onClick={onNext} /></>}>
    <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
      ...
      <Mentor>{tr2({ uz: "Pastdagi uch qatorni kuzating.", ru: "Понаблюдайте за тремя строками внизу." })}</Mentor>
      <div className="s1demo">
        <span className="s1demo-lbl">{tr2({ uz: "🗂 Har xato — bitta karta", ru: "🗂 Каждая ошибка — одна карточка" })}</span>
        <div className="s1demo-list">
          {DEMO_KARTA.map((r, i) => <span key={i} className="s1row" style={{ "--dd": `${0.5 + i * 0.85}s` }}>
              <span className="s1row-t" style={{ "--dd": `${0.5 + i * 0.85}s` }}>{tr2(r.t)}</span>
...
var S2_CARDS = [
  { ic: "😒", h: { uz: "Yoqmadi", ru: "Не понравилось" }, b: { uz: "Skuter uzoqda turibdi · narx kecha arzonroq edi — ilova aytganini qildi, sizga yoqmadi", ru: "Самокат стоит далеко · вчера цена была ниже — приложение сделало обещанное, просто вам это не понравилось" } },
  { ic: "⚠️", h: { uz: "Ishlamadi", ru: "Не сработало" }, b: { uz: "«Tugatish»ni bosdingiz, hisoblagich yuraverdi — ilova aytganini qilmadi", ru: "Вы нажали «Завершить», а счётчик пошёл дальше — приложение не сделало обещанного" } }
];
var Screen2 = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Muhokama · ikki xil holat", ru: "Обсуждение · два разных случая" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!allSeen && !isMentor} disabled={!allSeen && !isMentor} label={allSeen || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `👆 Yana ${qoldi} kartani oching`, ru: `👆 Откройте ещё ${qoldi} карточк${qoldi === 1 ? "у" : "и"}` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Ilovada nima <span className="italic" style={{ color: T.accent }}>xato</span> — nima faqat yoqmagan?</>, ru: <>Что в приложении <span className="italic" style={{ color: T.accent }}>ошибка</span>, а что просто не понравилось?</> })}</h2></div>
        <Mentor>{tr2({ uz: "Skuter safarida ikki xil holat bo'ladi. Ikki kartani bosib solishtiring.", ru: "В поездке на самокате бывают два разных случая. Нажмите на две карточки и сравните." })}</Mentor>
        ...
        {allSeen && <div className="xul fade-step">
            <span className="xul-h">{tr2({ uz: "Ilova har safar aytganini qilsa — buni sifat deyiladi.", ru: "Если приложение каждый раз делает обещанное — это называется качеством." })}</span>
            <p className="xul-b">{tr2({ uz: <>Ilova aytganini qilmasa — bu xato; bunday xatoni <b>nosozlik</b> (inglizchasi — bug) deyiladi. Yoqmagan narsa nosozlik emas.</>, ru: <>Если приложение не делает обещанного — это ошибка; такую ошибку называют <b>поломкой</b> (по-английски — bug). То, что просто не понравилось, поломкой не является.</> })}</p>
          </div>}
      </div>
    </Stage>;
};
...
var Screen3 = (props) => <QuestionScreen
  {...props}
  eyebrow={tr2({ uz: "Tekshiruv · nosozlik nima", ru: "Проверка · что такое поломка" })}
  scope="module-mikro"
  ctaLabel={tr2({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr2({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr2({ uz: "🛴 Skuter safarida uch holat bo'ldi. Qaysi biri — nosozlik?", ru: "🛴 В поездке на самокате было три случая. Какой из них — поломка?" })} />}
  questionText={tr2({ uz: "Uch holatdan qaysi biri nosozlik", ru: "Какой из трёх случаев — поломка" })}
  options={[tr2({ uz: "Ilova har ochilganda reklama ko'rsatadi", ru: "Приложение при каждом открытии показывает рекламу" }), tr2({ uz: "Ilova naqd pulni olmaydi — faqat karta", ru: "Приложение не принимает наличные — только карту" }), tr2({ uz: "QR skanerlandi, skuter qulfi ochilmadi", ru: "QR отсканирован, а замок самоката не открылся" })]}
  correctIdx={2}
  explainCorrect={tr2({ uz: "To'g'ri — ilova aytganini qilmadi; qolgan ikkitasi yoqmagan narsa, ilova ularda aytganini qilgan.", ru: "Верно — приложение не сделало обещанного; остальные два случая просто не понравились, там приложение обещанное выполнило." })}
  explainWrong={{
    0: tr2({ uz: "Reklama jahlni chiqaradi, lekin ilova shunday tuzilgan — aytganini qilgan; bu yoqmagan narsa.", ru: "Реклама раздражает, но приложение так и устроено — обещанное выполнено; это просто не нравится." }),
    1: tr2({ uz: "Ilova naqd pulni hech qachon va'da qilmagan — aytganini qilgan; bu yoqmagan narsa.", ru: "Приложение никогда не обещало наличные — обещанное выполнено; это просто не нравится." }),
    default: tr2({ uz: "Nosozlik — ilova aytganini qilmagan holat: qulf ochilishi kerak edi, ochilmadi.", ru: "Поломка — это когда приложение не сделало обещанного: замок должен был открыться и не открылся." })
  }}
/>;
var NUQTALAR = [
  {
    k: "kod",
    ic: "🧑‍💻",
    nom: { uz: "Kod yozilayotganda", ru: "Пока пишется код" },
    vaqt: { uz: "bir necha daqiqa", ru: "несколько минут" },
    odam: { uz: "0 odam", ru: "0 человек" },
    tilt: -6,
    dy: -8,
    ton: "ok",
    fakt: { uz: "Test tutdi — dasturchi shu zahoti tuzatdi. Odamlar hech narsani ko'rmadi", ru: "Тест поймал — программист тут же починил. Люди ничего не увидели" }
  },
  {
    k: "chiq",
    ic: "📦",
    nom: { uz: "Chiqarishdan oldin", ru: "Перед выпуском" },
    vaqt: { uz: "bir kun", ru: "один день" },
    odam: { uz: "0 odam", ru: "0 человек" },
    tilt: -13,
    dy: -18,
    ton: "ok",
    fakt: { uz: "Jamoa telefonda tekshirib topdi — chiqarish bir kunga surildi. Odamlar hech narsani ko'rmadi", ru: "Команда проверила на телефоне и нашла — выпуск сдвинули на день. Люди ничего не увидели" }
  },
  {
    k: "odam",
    ic: "📱",
    nom: { uz: "Odamlar qo'lida", ru: "В руках у людей" },
    vaqt: { uz: "haftalar", ru: "недели" },
    odam: { uz: "bir qism odam ketdi", ru: "часть людей ушла" },
    tilt: 15,
    dy: 26,
    ton: "bad",
    fakt: { uz: "Sharhlarda chiqdi: «pul yeyapti». Tuzatish va yangilanish haftalab tarqaldi — bir qism odam ketib bo'ldi", ru: "Вылезло в отзывах: «съедает деньги». Исправление и обновление расходились неделями — часть людей уже ушла" }
  }
];
var TAROZI_KEY = "pm-m4b2-tarozi";
var TIP_SEC = 45;
var RESCUE_SEC = 150;
var NUQTA_P = [0, 0.5, 1];
var Screen4 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  useEffect4(() => () => clearTimeout(runT.current), []);
  useEffect4(() => {
  ...
  useEffect4(() => {
    try {
      localStorage.setItem(TAROZI_KEY, JSON.stringify({ seen }));
    } catch {
    }
  }, [seen]);
  const put = (k) => {
  ...
  const navLabel = done || isMentor || rescue ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : !pos ? tr2({ uz: "① Yo'ldagi bitta nuqtani bosing", ru: "① Нажмите одну точку на пути" }) : shown !== pos ? tr2({ uz: "② «Chiqaramiz»ni bosing", ru: "② Нажмите «Выпускаем»" }) : tr2({ uz: `③ Yana ${NUQTALAR.length - seen.length} nuqtani ko'ring`, ru: `③ Посмотрите ещё ${NUQTALAR.length - seen.length} точ${NUQTALAR.length - seen.length === 1 ? "ку" : "ки"}` });
  return <Stage eyebrow={tr2({ uz: "Sinov · bitta nosozlik, uch nuqta", ru: "Испытание · одна поломка, три точки" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor && !rescue} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.3vw,13px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Tekshiruvni yo'lning <span className="italic" style={{ color: T.accent }}>uch joyiga</span> qo'yib ko'ring.</>, ru: <>Поставьте проверку в <span className="italic" style={{ color: T.accent }}>три места</span> пути.</> })}</h2></div>
        <Mentor>{tr2({ uz: "Ilovani odamlarga yetkazish — chiqarish deyiladi. Ilova chiqquncha uch nuqtadan o'tadi: bittasini bosing, keyin «Chiqaramiz».", ru: "Доставку приложения людям называют выпуском. Пока приложение выходит, оно проходит три точки: нажмите одну, потом «Выпускаем»." })}</Mentor>
    ...
    /* 72-qonun: belgi yorliqli idishda, diqqat-signali bilan; birinchi qo'yishdan keyin tinadi */
            ...
            <div className={`tdock${pos ? " calm" : ""}`}>
              <span className="tdock-lbl">{tr2({ uz: "👆 Nuqtani bosing — belgi o'sha yerga tushadi", ru: "👆 Нажмите точку — метка встанет туда" })}</span>
            ...
            <div className="yol">
              ...
              <span className={`bugc${run ? " run" : ""}${!run && cur && shown === pos && cur.ton === "ok" ? " tutildi" : ""}`} style={{ "--p": cardP }}>
                <span className="bugc-in">
                  <span className="bugc-t">{tr2({ uz: "«Tugatish» bosilganda hisoblagich to'xtamaydi", ru: "При нажатии «Завершить» счётчик не останавливается" })}</span>
                </span>
              </span>
              <div className="yol-nodes">
                {NUQTALAR.map((n) => <button
    key={n.k}
    type="button"
  ...
  >
                    ...
                    <span className="ynode-t">{tr2(n.nom)}</span>
            ...
            <button type="button" className={`chiq-btn${chiqTurn ? " turn-ring" : ""}`} disabled={isMentor || run || !pos || shown === pos} onClick={chiqar}>
              {run ? tr2({ uz: "Chiqmoqda…", ru: "Выпускаем…" }) : tr2({ uz: "▶ Chiqaramiz", ru: "▶ Выпускаем" })}
            </button>
            {tip && <p className="bhint fade-step">{tr2({ uz: "🤔 Boshqa nuqtani ham bosib ko'ring.", ru: "🤔 Попробуйте нажать и другую точку." })}</p>}
            {rescue && !done && <p className="small fade-step" style={{ margin: 0, color: T.ink3, fontWeight: 600 }}>{tr2({ uz: "Qolganini birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе — «Продолжить» открыто." })}</p>}
          </Col>
          <Col gap={9}>
            <div className="tar">
              <span className="tar-lbl">{tr2({ uz: "⚖️ Nosozlik narxi", ru: "⚖️ Цена поломки" })} <span className="tar-nm">{tr2({ uz: "namuna", ru: "пример" })}</span></span>
              <div className="tar-beam" style={{ "--tl": `${cur ? cur.tilt : 0}deg`, "--dy": `${cur ? cur.dy : 0}px` }}>
                ...
                <span className="tar-pan l">
                  <span className="tar-pan-h">{tr2({ uz: "Tuzatish vaqti", ru: "Время на починку" })}</span>
                  <span className="tar-pan-v">{cur ? tr2(cur.vaqt) : "—"}</span>
                </span>
                <span className={`tar-pan r${cur && cur.ton === "bad" ? " hot" : ""}`}>
                  <span className="tar-pan-h">{tr2({ uz: "Ketgan odamlar", ru: "Ушедшие люди" })}</span>
                  <span className="tar-pan-v">{cur ? tr2(cur.odam) : "—"}</span>
                </span>
              </div>
              <span className="tar-foot">{cur ? tr2({ uz: `🧪 ${tr2(NUQTALAR.find((n) => n.k === shown).nom)}`, ru: `🧪 ${tr2(NUQTALAR.find((n) => n.k === shown).nom)}` }) : tr2({ uz: "Tarozi «Chiqaramiz»dan keyin qiyshayadi", ru: "Весы наклонятся после «Выпускаем»" })}</span>
            </div>
            {cur && <p key={shown} className={`fakt ${cur.ton} fade-step`}>{cur.ton === "ok" ? "✅" : "🔴"} {tr2(cur.fakt)}</p>}
            <StudentPracticePulse live={live} screen={screen} />
            <MentorPracticeStats live={live} screen={screen} label={tr2({ uz: "⚖️ Uch nuqtani ko'rganlar", ru: "⚖️ Посмотрели три точки" })} />
          </Col>
        </div>
        {done && <div className="bdone fade-step">
            <span className="done-mini">{tr2({ uz: "✅ Buni o'zingiz topdingiz: nosozlik qancha kech topilsa, shuncha qimmatga tushadi", ru: "✅ Вы нашли это сами: чем позже найдена поломка, тем дороже она обходится" })}</span>
            <p className="bhint">{tr2({ uz: "Kod yozilayotganda tutadigan tekshiruv — o'tgan darsda o'zingiz yozgan test.", ru: "Проверка, которая ловит при написании кода, — это тест, который вы сами писали на прошлом уроке." })}</p>
          </div>}
        <MentorNote>{tr2({ uz: "Bolalar odatda 3-nuqtani birinchi tanlaydi va shu yerda to'xtaydi. «Endi birinchi nuqtani bosing» deb turtki bering — solishtirish aynan shu lahzada. Qaysi nuqta arzon ekanini AYTMANG. Bu ishni o'quvchilar bajaradi, siz kuzatasiz; «Davom etish» siz uchun ochiq.", ru: "Дети обычно сначала выбирают третью точку и на ней останавливаются. Подтолкните: «теперь нажмите первую точку» — сравнение происходит именно в этот момент. НЕ говорите, какая точка дешевле. Эту работу делают ученики, вы наблюдаете; «Продолжить» для вас открыто." })}</MentorNote>
      </div>
    </Stage>;
};
var Screen5 = (props) => <QuestionScreen
  {...props}
  eyebrow={tr2({ uz: "Tekshiruv · qayerda tutildi", ru: "Проверка · где поймали" })}
  scope="module-mikro"
  ctaLabel={tr2({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr2({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr2({ uz: "🧑‍💻 Dasturchi nosozlikni kod yozilayotganda tutdi. Ilovaga nima bo'ladi?", ru: "🧑‍💻 Программист поймал поломку при написании кода. Что будет с приложением?" })} />}
  questionText={tr2({ uz: "Kod yozilayotganda tutilgan nosozlikdan keyin ilovaga nima bo'ladi", ru: "Что будет с приложением после поломки, пойманной при написании кода" })}
  options={[tr2({ uz: "Hech kim sezmaydi — o'sha kuni tuzatiladi", ru: "Никто не заметит — починят в тот же день" }), tr2({ uz: "Sharhlarda odamlar shikoyat yoza boshlaydi", ru: "В отзывах люди начнут жаловаться" }), tr2({ uz: "Chiqarish bir necha kunga surilib ketadi", ru: "Выпуск сдвинется на несколько дней" })]}
  correctIdx={0}
  explainCorrect={tr2({ uz: "To'g'ri — kod yozilayotganda tutilgan nosozlik eng arzon: odamga yetmaydi, chiqarishni ham surmaydi.", ru: "Верно — поломка, пойманная при написании кода, самая дешёвая: до людей не доходит и выпуск не сдвигает." })}
  explainWrong={{
    1: tr2({ uz: "Sharhlar — nosozlik odamlar qo'lida chiqqanda boshlanadi. Bu yerda u dasturchi stolida tutildi.", ru: "Отзывы начинаются, когда поломка вылезает в руках у людей. Здесь её поймали ещё на столе у программиста." }),
    2: tr2({ uz: "Chiqarish surilishi — nosozlik chiqarishdan oldin tutilganda bo'ladi. Bu yerda u undan ham oldin tutildi.", ru: "Выпуск сдвигается, когда поломку ловят перед выпуском. Здесь её поймали ещё раньше." }),
    default: tr2({ uz: "Kod yozilayotganda tutilgan nosozlik odamga yetmaydi va chiqarishni surmaydi.", ru: "Поломка, пойманная при написании кода, до людей не доходит и выпуск не сдвигает." })
  }}
/>;
var K10_SLIDES = [
  {
    ic: "🎮",
    h: { uz: "2020-yil dekabr", ru: "Декабрь 2020 года" },
    body: { uz: <>Katta o'yin — <b>Cyberpunk 2077</b> — sotuvga chiqdi. Ichida esa juda ko'p nosozlik bor edi.</>, ru: <>Большая игра — <b>Cyberpunk 2077</b> — поступила в продажу. А внутри было очень много поломок.</> }
  },
  {
    ic: "🔮",
    h: null,
    body: null,
    predict: {
      ask: { uz: "Sizningcha, o'yindagi nosozliklarni kim birinchi topdi?", ru: "Как вы думаете, кто первым нашёл поломки в игре?" },
      chips: [
        { ic: "🧑‍💻", t: { uz: "O'yinni yasagan jamoa, chiqarishdan oldin", ru: "Команда, сделавшая игру, — до выпуска" } },
        { ic: "🕹", t: { uz: "Sotib olgan o'yinchilar, birinchi kunlarda", ru: "Купившие игроки — в первые дни" } },
        { ic: "🏪", t: { uz: "Do'kon tekshiruvchilari, sotuvga qo'yishdan oldin", ru: "Проверяющие магазина — до выкладки в продажу" } }
      ],
      ans: 1,
      hit: { uz: "🎯 Topdingiz! Sotib olgan o'yinchilar", ru: "🎯 Угадали! Купившие игроки" },
      miss: { uz: "Adashdingiz — asl javob: sotib olgan o'yinchilar", ru: "Не угадали — верный ответ: купившие игроки" }
    }
  },
  {
    ic: "🕹",
    h: { uz: "Sotib olgan o'yinchilar", ru: "Купившие игроки" },
    body: { uz: <>Nosozliklar odamlar qo'lida chiqdi — ayniqsa <b>PlayStation</b>'da o'yin aytganini qilmasdi.</>, ru: <>Поломки вылезли в руках у людей — особенно на <b>PlayStation</b> игра не делала обещанного.</> }
  },
  {
    ic: "💸",
    h: { uz: "Odamlar pulini qaytarishni so'radi", ru: "Люди потребовали вернуть деньги" },
    body: { uz: <>Qaytarish to'lqini boshlandi. Sony o'yinni PlayStation do'konidan <b>olib tashladi</b>.</>, ru: <>Началась волна возвратов. Sony <b>убрала</b> игру из магазина PlayStation.</> },
    predict: {
      ask: { uz: "Sizningcha, o'yin do'kondan qancha vaqtga olib tashlandi?", ru: "Как вы думаете, на сколько игру убрали из магазина?" },
      chips: [
        { ic: "📆", t: { uz: "Bir necha kunga", ru: "На несколько дней" } },
        { ic: "🗓", t: { uz: "Qariyb bir oyga", ru: "Почти на месяц" } },
        { ic: "⏳", t: { uz: "Qariyb yarim yilga", ru: "Почти на полгода" } },
        { ic: "🕰", t: { uz: "Bir yildan oshiq", ru: "Больше чем на год" } }
      ],
      ans: 2,
      hit: { uz: "🎯 Topdingiz! Qariyb yarim yilga", ru: "🎯 Угадали! Почти на полгода" },
      miss: { uz: "Adashdingiz — asl javob: qariyb yarim yilga", ru: "Не угадали — верный ответ: почти на полгода" }
    }
  },
  {
    ic: "⏳",
    h: { uz: "Qariyb yarim yilga", ru: "Почти на полгода" },
    body: { uz: <>O'yin shuncha vaqt do'konda yo'q edi. Nosozlik odamlar qo'lida chiqqanda <b>narxi shu bo'ldi</b>.</>, ru: <>Всё это время игры не было в магазине. Вот <b>какой была цена</b>, когда поломка вылезла в руках у людей.</> }
  },
  {
    ic: "⚖️",
    h: { uz: "Skuter ilovasi bilan bir xil tarozi", ru: "Те же весы, что и у приложения самокатов" },
    body: { uz: <>Cyberpunk nosozliklari yo'lning oxirgi nuqtasida — odamlar qo'lida — chiqdi, shuning uchun eng qimmatga tushdi. Endi navbatni o'zingiz qo'yasiz: qaysi nosozlik birinchi tuzatiladi.</>, ru: <>Поломки Cyberpunk вылезли в последней точке пути — в руках у людей, поэтому обошлись дороже всего. Теперь очередь выстроите вы сами: какую поломку чинят первой.</> }
  }
];
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "🎮 Biznes olamidan mashhur voqea", ru: "🎮 Известная история из мира бизнеса" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={betPending && !isMentorK} disabled={betPending && !isMentorK} label={betPending && !isMentorK ? tr2({ uz: "Avval o'zingiz belgilang", ru: "Сначала отметьте сами" }) : last ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Keyingi bosqich (${i + 1}/${K10_SLIDES.length})`, ru: `Следующий шаг (${i + 1}/${K10_SLIDES.length})` })} onClick={last ? onNext : () => setI(i + 1)} /></>}>
      <div className="screen k-fill" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bitta o'yin — nosozliklari <span className="italic" style={{ color: T.accent }}>qanchaga</span> tushdi?</>, ru: <>Одна игра — <span className="italic" style={{ color: T.accent }}>во сколько</span> обошлись её поломки?</> })}</h2></div>
        {c.h && <div className="k-slide fade-step" key={`s${i}`}>
        ...
        {c.predict && <div className={`kp-bet fade-step${bet !== void 0 ? " answered" : ""}`} key={`b${i}`}>
            <span className="k-slide-eyebrow">{bet === void 0 ? tr2({ uz: "🎲 Avval o'zingiz belgilab ko'ring", ru: "🎲 Сначала отметьте сами" }) : tr2({ uz: "🎮 Voqea davom etadi", ru: "🎮 История продолжается" })}{c.h ? "" : tr2({ uz: ` · Voqea — ${i + 1} / ${K10_SLIDES.length}`, ru: ` · История — ${i + 1} / ${K10_SLIDES.length}` })}</span>
    ...
    return <button key={k} className={`k-dot ${k === i ? "cur" : k < i ? "fill" : ""}`} disabled={!ochiq} onClick={() => ochiq && setI(k)} aria-label={tr2({ uz: `${k + 1}-bosqich`, ru: `Шаг ${k + 1}` })} title={ochiq ? void 0 : tr2({ uz: "Avval shu bosqichni tugating", ru: "Сначала завершите этот шаг" })} />;
        ...
        <MentorNote>{tr2({ uz: "Bashorat-javoblarini oldindan aytmang: bosqich-kartasi ochilgunicha kutinglar. O'yin mazmuni haqida savol chiqsa — burilmang: bu yerda faqat nosozlik qayerda chiqqani va qanchaga tushgani muhim.", ru: "Не называйте ответы на предсказания заранее: дождитесь, пока откроется карточка шага. Если пойдут вопросы про содержание игры — не сворачивайте туда: здесь важно только, где вылезла поломка и во сколько она обошлась." })}</MentorNote>
      </div>
    </Stage>;
};
var Screen7 = (props) => <QuestionScreen
  {...props}
  eyebrow={tr2({ uz: "Tekshiruv · qachon eng arzon", ru: "Проверка · когда дешевле всего" })}
  scope="module-mikro"
  ctaLabel={tr2({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr2({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr2({ uz: "🎮 Cyberpunk 2077 nosozliklari qachon tutilsa, eng arzon bo'lardi?", ru: "🎮 Когда поломки Cyberpunk 2077 обошлись бы дешевле всего?" })} />}
  questionText={tr2({ uz: "Cyberpunk nosozliklari qachon tutilsa eng arzon", ru: "Когда поломки Cyberpunk дешевле всего поймать" })}
  options={[tr2({ uz: "Sony o'yinni do'kondan olib tashlagan kuni", ru: "В день, когда Sony убрала игру из магазина" }), tr2({ uz: "O'yin sotuvga chiqarilishidan oldin", ru: "До того как игра поступила в продажу" }), tr2({ uz: "Birinchi sharhlar chiqqanidan bir kun oldin", ru: "За день до первых отзывов" })]}
  correctIdx={1}
  explainCorrect={tr2({ uz: "To'g'ri — o'yinchilar qo'lida chiqqan nosozlik pul qaytarish va yarim yillik yo'qlikka aylandi; oldin tutilsa, buning hech biri bo'lmasdi.", ru: "Верно — поломка, вылезшая в руках у игроков, обернулась возвратами и полугодом отсутствия в магазине; поймали бы раньше — ничего этого не было бы." })}
  explainWrong={{
    0: tr2({ uz: "O'sha kuni nosozliklar allaqachon o'yinchilar qo'lida edi — narx to'lanib bo'lgan.", ru: "В тот день поломки уже были в руках у игроков — цена уже заплачена." }),
    2: tr2({ uz: "Birinchi sharhlargacha ham o'yin sotuvda edi — nosozliklar odamlar qo'lida chiqib bo'lgan.", ru: "До первых отзывов игра уже была в продаже — поломки уже вылезли в руках у людей." }),
    default: "Eng arzoni — o'yin sotuvga chiqarilishidan oldin: hech kimning qo'liga yetmaydi."
  }}
/>;
var OUT_KEY = "pm-m4b2-sifat";
var DRAFT_KEY = "pm-m4b2-kartalar-draft";
var readDraft = () => {
  try {
...
var kimdaT = (v) => tr2((KIMDA_OPTS.find((o) => o.v === v) || {}).t || "");
var oqibatT = (v) => tr2((OQIBAT_OPTS.find((o) => o.v === v) || {}).t || "");
var SHARHLAR = [
  {
    id: "sh1",
    yul: "⭐",
    t: { uz: '«"Boshlash"ni bosaman — "Xatolik" chiqadi, skuter ochilmaydi. Sinfdoshlarimda ham xuddi shunday. Nima qilay??»', ru: '«Жму "Начать" — выходит "Ошибка", самокат не открывается. У одноклассников то же самое. Что делать??»' },
    kimda: "hammada",
    oqibat: "toxtaydi",
    kmsg: { uz: "Sharhni qayta o'qing: «sinfdoshlarimda ham» — bu bittasidami?", ru: "Перечитайте отзыв: «у одноклассников то же самое» — это у одного человека?" },
    omsg: { uz: "Sharhda «skuter ochilmaydi» deyilgan — ijara boshlanadimi?", ru: "В отзыве сказано «самокат не открывается» — аренда начинается?" }
  },
  {
    id: "sh2",
    yul: "⭐⭐⭐",
    t: { uz: "«Ilovani o'zbekchaga o'tkazdim, har ochganda yana ruscha bo'lib qoladi 🙄 Ishlaydi-ku, lekin charchatadi»", ru: "«Переключил приложение на узбекский, при каждом открытии снова становится русским 🙄 Работать работает, но утомляет»" },
    kimda: "bazilarda",
    oqibat: "noqulay",
    kmsg: { uz: "Sharhda faqat shu odamning telefoni haqida gap ketyapti — bu hammadami?", ru: "В отзыве речь только про телефон этого человека — это у всех?" },
    omsg: { uz: "Sharhda «ishlaydi-ku» deyilgan — ish to'xtadimi?", ru: "В отзыве сказано «работать работает» — работа остановилась?" }
  },
  {
    id: "sh3",
    yul: "⭐⭐",
    t: { uz: "«Kechqurun menda xarita qop-qora, skuterlar ko'rinmaydi — piyoda ketdim. Do'stimning telefonida esa ko'rinib turibdi»", ru: "«Вечером у меня карта совсем чёрная, самокатов не видно — пошёл пешком. А на телефоне друга они видны»" },
    kimda: "bazilarda",
    oqibat: "toxtaydi",
    kmsg: { uz: "Sharhda «do'stimning telefonida esa ko'rinib turibdi» deyilgan — bu hammadami?", ru: "В отзыве сказано «на телефоне друга они видны» — это у всех?" },
    omsg: { uz: "Sharhda «piyoda ketdim» deyilgan — ijara bo'ldimi?", ru: "В отзыве сказано «пошёл пешком» — аренда состоялась?" }
  }
];
var APO = "['\\u02BB\\u2019]";
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [list, setList] = useState3(() => storedAnswer && Array.isArray(storedAnswer.kartalar) ? storedAnswer.kartalar : readDraft());
  const [draft, setDraft] = useState3("");
  const [kimda, setKimda] = useState3(null);
  const [oqibat, setOqibat] = useState3(null);
  const [edit, setEdit] = useState3(null);
  const [focus, setFocus] = useState3(false);
  const [zidSeen, setZidSeen] = useState3(0);
  ...
  const nusxa = uzun && normS(draft).length >= 60 && normS(tr2(cur.t)).indexOf(normS(draft)) >= 0;
  ...
  useEffect4(() => {
    if (!done) return;
    try {
      localStorage.setItem(OUT_KEY, JSON.stringify({ kartalar: list.slice(0, 3), savedAt: Date.now() }));
    } catch {
    }
  }, [list, done]);
  const save = () => {
  ...
  const navLabel = done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : list.length === 0 ? tr2({ uz: "① Birinchi sharhni kartaga aylantiring", ru: "① Превратите первый отзыв в карточку" }) : tr2({ uz: `② Yana ${3 - list.length} karta qoldi`, ru: `② Осталось ещё ${3 - list.length} карточк${3 - list.length === 1 ? "а" : "и"}` });
  const needTxt = !uzun ? tr2({ uz: "matn yozilmagan", ru: "текст не написан" }) : nusxa ? tr2({ uz: "sharhning o'zi", ru: "это сам отзыв" }) : !kimda ? tr2({ uz: "«Kimda?» belgilanmagan", ru: "«У кого?» не отмечено" }) : !oqibat ? tr2({ uz: "«Nima bo'ladi?» belgilanmagan", ru: "«Что происходит?» не отмечено" }) : zidBlock ? tr2({ uz: "sharhga mos emas", ru: "не совпадает с отзывом" }) : "";
  const startEdit = (k) => {
    setEdit(k);
    setDraft(list[k].nima);
    setKimda(list[k].kimda);
    setOqibat(list[k].oqibat);
    setZidSeen(0);
  };
  return <Stage eyebrow={tr2({ uz: "Mustaqil ish · uch karta", ru: "Самостоятельная работа · три карточки" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Har sharhni <span className="italic" style={{ color: T.accent }}>nosozlik-kartasiga</span> aylantiring.</>, ru: <>Превратите каждый отзыв в <span className="italic" style={{ color: T.accent }}>карточку поломки</span>.</> })}</h2></div>
        <Mentor>{tr2({ uz: "Sharhda his-tuyg'u ko'p — siz undan faktni ajratasiz: nima bosildi, nima bo'ldi.", ru: "В отзыве много эмоций — вы вытаскиваете из него факт: что нажали и что произошло." })}</Mentor>
        {
    /* 80a: havoda uch doira — yozilgani yashil, joriysi pulsda, kelgusi punktir */
  }
        <div className="stps fade-up">
          {[0, 1, 2].map((k) => <span key={k} className={`stp ${list.length > k ? "done" : idx === k ? "on" : ""}`}><i>{list.length > k ? "✓" : k + 1}</i>{tr2({ uz: <>{k + 1}-karta</>, ru: <>Карточка {k + 1}</> })}</span>)}
        </div>
        <div className="split">
          <Col gap={9}>
            {!done || edit !== null ? <div className="sharh">
                <span className="sharh-lbl">{tr2({ uz: "💬 Ilova do'konidagi sharh", ru: "💬 Отзыв в магазине приложений" })} <span className="sharh-yul">{cur.yul}</span></span>
                <p className="sharh-t">{tr2(cur.t)}</p>
              </div> : <div className="wsp-list fade-step">
                <span className="wsp-list-h">{tr2({ uz: "🗂 Uch kartangiz", ru: "🗂 Ваши три карточки" })}</span>
    ...
    /* 👥 1-o'qish: qadam-indikator yuqorida turadi — bu yerda faqat YOZILGANLAR (referent) */
            ...
            {list.length > 0 && (!done || edit !== null) && <div className="wsp-task fade-step">
                <span className="wsp-task-lbl">{tr2({ uz: "🗂 Yozib bo'lgan kartalaringiz", ru: "🗂 Уже записанные карточки" })}</span>
                {list.slice(0, 3).map((r, k) => <span key={k} className="wsp-task-row done">
            ...
            <div className="wsxrow">
              <div className={`wsx ${yordamOpen ? "open" : ""}`}>
                ...
                {yordamOpen && <div className="wsx-body"><p>{tr2({ uz: <>Sharhdan ikki narsani toping: odam <b>nimani bosdi</b> va <b>nima bo'lmadi</b>. Shu ikkovi — kartaning birinchi qatori.</>, ru: <>Найдите в отзыве две вещи: <b>что человек нажал</b> и <b>что не произошло</b>. Эти двое — первая строка карточки.</> })}</p></div>}
              </div>
              <div className={`wsx star ${starOpen ? "open" : ""}`}>
                ...
                {starOpen && <div className="wsx-body"><p>{tr2({ uz: <>Yana bitta nosozlik: «Tugatish» bosildi — hisoblagich to'xtamadi. Bu kimda bo'ladi, ish to'xtaydimi? {solo ? "Javobingizni ovoz chiqarib ayting." : "Javobingizni sherigingizga ayting."}</>, ru: <>Ещё одна поломка: нажали «Завершить» — счётчик не остановился. У кого это происходит, останавливается ли работа? {solo ? "Скажите свой ответ вслух." : "Скажите свой ответ соседу."}</> })}</p></div>}
              </div>
            </div>
            <StudentPracticePulse live={live} screen={screen} />
            <MentorPracticeStats live={live} screen={screen} label={tr2({ uz: "✍️ Uch kartani yozganlar", ru: "✍️ Записали три карточки" })} />
          </Col>
          <Col gap={9}>
            {(!done || edit !== null) && <div className="wsp-ed">
                <span className="wsp-ed-h">{tr2({ uz: <>{idx + 1}-karta · nima bo'ldi</>, ru: <>Карточка {idx + 1} · что произошло</> })}</span>
                <input
    className={`reflect-input${inputTurn ? " await" : ""}${uzun ? " filled" : ""}`}
    value={draft}
    maxLength={140}
    placeholder={tr2({ uz: "Nima bosildi — nima bo'ldi?", ru: "Что нажали — что произошло?" })}
  ...
  />
                <span className="wsp-ed-q">{tr2({ uz: "Kimda?", ru: "У кого?" })}</span>
                <div className="mkbtns">
                  {KIMDA_OPTS.map((o) => <button key={o.v} type="button" className={`mkbtn a${kimda === o.v ? " on" : ""}`} onClick={() => setKimda(o.v)}>{tr2(o.t)}</button>)}
                </div>
                <span className="wsp-ed-q">{tr2({ uz: "Nima bo'ladi?", ru: "Что происходит?" })}</span>
                <div className="mkbtns">
                  {OQIBAT_OPTS.map((o) => <button key={o.v} type="button" className={`mkbtn b${oqibat === o.v ? " on" : ""}`} onClick={() => setOqibat(o.v)}>{tr2(o.t)}</button>)}
                </div>
                {
    /* 106d: ikki tomonlama javob — bloklamaydi, yo'naltiradi */
  }
                {uzun && nusxa && <p className="sfb ask">{tr2({ uz: "🤔 Bu sharhning o'zi. Uni bir qatorga qisqartiring: nima bosildi — nima bo'ldi.", ru: "🤔 Это сам отзыв. Сократите его до одной строки: что нажали — что произошло." })}</p>}
        ...
        {done && edit === null && <div className="done-mini fade-step">{tr2({ uz: <>✅ Uch kartangiz saqlandi <span className="dm-sub">— har birida nima bo'lgani, kimda va nima bo'lishi yozilgan</span></>, ru: <>✅ Ваши три карточки сохранены <span className="dm-sub">— в каждой записано, что произошло, у кого и что при этом происходит</span></> })}</div>}
        <MentorNote>{tr2({ uz: "«Ilova yomon ishlaydi» degan kartalar chiqadi — bu eng foydali xato. Javob-qatori uni tutadi; siz muhokama qiling: dasturchi bu kartadan nimani tuzatadi? Baholash-mezoni: uchta karta yozilgan · har kartada harakat va natija bor · kimda va nima bo'lishi belgilangan. Bu ishni o'quvchilar bajaradi, siz kuzatasiz; «Davom etish» siz uchun ochiq.", ru: "Появятся карточки вида «приложение плохо работает» — это самая полезная ошибка. Строка ответа её поймает; вы обсудите: что программист починит по такой карточке? Критерии оценки: три карточки записаны · в каждой есть действие и результат · отмечено, у кого и что происходит. Эту работу делают ученики, вы наблюдаете; «Продолжить» для вас открыто." })}</MentorNote>
      </div>
    </Stage>;
};
var NAVBAT_KEY = "pm-m4b2-navbat";
var JAVONLAR = [
...
var NAV_KARTA = [
  {
    id: "k1",
    ic: "💳",
    t: { uz: "To'lov sahifasi ochilmaydi — to'lovsiz ijara boshlanmaydi. Hamma odamda shunday", ru: "Страница оплаты не открывается — без оплаты аренда не начинается. Так у всех" },
    kimda: "hammada",
    oqibat: "toxtaydi",
    javon: "hozir",
    sabab: { uz: "Hammada va ijara boshlanmaydi — ikkalasi ham og'ir", ru: "У всех и аренда не начинается — оба ответа тяжёлые" },
    kmsg: { uz: "Kartani qayta o'qing: «hamma odamda shunday» — bu ba'zilaridami?", ru: "Перечитайте карточку: «так у всех» — это у некоторых?" },
    omsg: { uz: "Kartada «to'lovsiz ijara boshlanmaydi» deyilgan — ijara baribir bo'ladimi?", ru: "В карточке сказано «без оплаты аренда не начинается» — аренда всё равно состоится?" }
  },
  {
    id: "k2",
    ic: "🔔",
    t: { uz: "Ijara tugaganda ovozli signal chiqmaydi — faqat ovozni yoqqanlarda; ijara baribir tugaydi", ru: "При завершении аренды нет звукового сигнала — только у тех, кто включил звук; аренда всё равно завершается" },
    kimda: "bazilarda",
    oqibat: "noqulay",
    javon: "keyin",
    sabab: { uz: "Ba'zilarda va ijara tugayveradi — ikkalasi ham yengil", ru: "У некоторых, и аренда всё равно завершается — оба ответа лёгкие" },
    kmsg: { uz: "Kartada «faqat ovozni yoqqanlarda» deyilgan — bu hammadami?", ru: "В карточке сказано «только у тех, кто включил звук» — это у всех?" },
    omsg: { uz: "Kartada «ijara baribir tugaydi» deyilgan — ish to'xtadimi?", ru: "В карточке сказано «аренда всё равно завершается» — работа остановилась?" }
  },
  {
    id: "k3",
    ic: "📜",
    t: { uz: "Ijara tarixida eng eski safar tepada turadi — hammada shunday. Kechagisi eng pastda, uzoq surib topiladi", ru: "В истории аренд самая старая поездка стоит сверху — так у всех. Вчерашняя в самом низу, её долго ищут прокруткой" },
    kimda: "hammada",
    oqibat: "noqulay",
    javon: "bugun",
    sabab: { uz: "Hammada, lekin ijara ishlayveradi — bittasi og'ir", ru: "У всех, но аренда работает — тяжёлый только один ответ" },
    kmsg: { uz: "Kartada «hammada» deyilgan — bu ba'zilaridami?", ru: "В карточке сказано «так у всех» — это у некоторых?" },
    omsg: { uz: "Kartada «uzoq surib topiladi» deyilgan — safar topilsa, ijara to'xtadimi?", ru: "В карточке сказано «долго ищут прокруткой» — если поездка нашлась, аренда остановилась?" }
  },
  {
    id: "k4",
    ic: "📷",
    t: { uz: "QR kod eski telefon kamerasida o'qilmaydi — faqat eski telefonlarda; skuter ochilmaydi", ru: "QR-код не читается камерой старого телефона — только на старых телефонах; самокат не открывается" },
    kimda: "bazilarda",
    oqibat: "toxtaydi",
    javon: "bugun",
    sabab: { uz: "Ba'zilarda, lekin ijara boshlanmaydi — bittasi og'ir", ru: "У некоторых, но аренда не начинается — тяжёлый только один ответ" },
    kmsg: { uz: "Kartada «faqat eski telefonlarda» deyilgan — bu hammadami?", ru: "В карточке сказано «только на старых телефонах» — это у всех?" },
    omsg: { uz: "Kartada «skuter ochilmaydi» deyilgan — ijara boshlanadimi?", ru: "В карточке сказано «самокат не открывается» — аренда начинается?" }
  }
];
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === "mentor");
  const [i, setI] = useState3(() => storedAnswer && storedAnswer.solved ? NAV_KARTA.length : 0);
  const [landed, setLanded] = useState3(() => storedAnswer && storedAnswer.solved ? NAV_KARTA.map((k) => ({ id: k.id, javon: k.javon })) : []);
  const [kimda, setKimda] = useState3(null);
  const [oqibat, setOqibat] = useState3(null);
  const [msg, setMsg] = useState3(null);
  const [sabab, setSabab] = useState3(null);
  const [errs, setErrs] = useState3(0);
  const [yordamOpen, setYordamOpen] = useState3(false);
  const [mReveal, setMReveal] = useState3(false);
  const stepT = useRef3(null);
  const achMiss = useContext2(AchMissCtx);
  const done = landed.length >= NAV_KARTA.length;
  useEffect4(() => () => clearTimeout(stepT.current), []);
  useEffect4(() => {
    if (done && (storedAnswer === void 0 || !storedAnswer.solved)) {
  ...
  useEffect4(() => {
    try {
      localStorage.setItem(NAVBAT_KEY, JSON.stringify({ hukmlar: landed }));
    } catch {
    }
  }, [landed]);
  ...
  const pickK = (v) => {
    if (isMentor || done || sabab) return;
    setKimda(v);
    if (oqibat) judge(v, oqibat);
  };
  const pickO = (v) => {
    if (isMentor || done || sabab) return;
    setOqibat(v);
    if (kimda) judge(kimda, v);
  };
  const navLabel = done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : !kimda ? tr2({ uz: `① ${i + 1}-kartada «Kimda?» ni belgilang`, ru: `① В карточке ${i + 1} отметьте «У кого?»` }) : !oqibat ? tr2({ uz: "② «Nima bo'ladi?» ni belgilang", ru: "② Отметьте «Что происходит?»" }) : tr2({ uz: `③ Yana ${NAV_KARTA.length - landed.length} karta qoldi`, ru: `③ Осталось ещё ${NAV_KARTA.length - landed.length} карточк${NAV_KARTA.length - landed.length === 1 ? "а" : "и"}` });
  const kartaOf = (id) => NAV_KARTA.find((k) => k.id === id);
  return <Stage eyebrow={tr2({ uz: "Tekshiruv · nosozliklarga navbat", ru: "Проверка · очередь поломок" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(9px,1.4vw,14px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Nosozliklarga <span className="italic" style={{ color: T.accent }}>navbat</span> qo'ying.</>, ru: <>Выстройте поломкам <span className="italic" style={{ color: T.accent }}>очередь</span>.</> })}</h2></div>
        <Mentor>{tr2({ uz: "Uch kartangiz tayyor — endi shu ikki savolni jamoadan kelgan to'rt kartaga beramiz: kimda? nima bo'ladi?", ru: "Ваши три карточки готовы — теперь зададим те же два вопроса четырём карточкам от команды: у кого? что происходит?" })}</Mentor>
        <div className="stps fade-up">
          {NAV_KARTA.map((k, n) => <span key={k.id} className={`stp ${landed.length > n ? "done" : i === n ? "on" : ""}`}><i>{landed.length > n ? "✓" : n + 1}</i>{n + 1}-karta</span>)}
        </div>
    ...
    /* ETALON 44: mentor rejimida javob-kaliti FAQAT «Natijani ochish»dan keyin */
            ...
            {isMentor ? <div className="nq-card">
              ...
              </div> : !done ? <div className="nq-card">
                <span className="nq-lbl">{tr2({ uz: <>{i + 1}-karta · jamoadan</>, ru: <>Карточка {i + 1} · от команды</> })}</span>
                <span className="nq-karta">{cur.ic} {tr2(cur.t)}</span>
                <span className="nq-q">{tr2({ uz: "Kimda?", ru: "У кого?" })}</span>
                <div className="mkbtns">
                  {KIMDA_OPTS.map((o) => <button key={o.v} type="button" className={`mkbtn a${kimda === o.v ? " on" : ""}`} onClick={() => pickK(o.v)}>{tr2(o.t)}</button>)}
                </div>
                <span className="nq-q">{tr2({ uz: "Nima bo'ladi?", ru: "Что происходит?" })}</span>
                <div className="mkbtns">
                  {OQIBAT_OPTS.map((o) => <button key={o.v} type="button" className={`mkbtn b${oqibat === o.v ? " on" : ""}`} onClick={() => pickO(o.v)}>{tr2(o.t)}</button>)}
                </div>
                <AchRule screen={screen} />
                {msg && <p className="sfb ask">🤔 {tr2(msg)}</p>}
                {sabab && <p className="sfb ok">✅ {tr2(sabab)}</p>}
          ...
          <Col gap={9}>
            {landed.length === 0 && !isMentor ? <div className="nq-empty">{tr2({ uz: "🗄 Hukm berilgan karta shu yerdagi javonga o'zi tushadi", ru: "🗄 Карточка с приговором сама встанет на нужную полку" })}</div> : <div className="nq-shelves">
                {JAVONLAR.map((j) => <div key={j.k} className={`nq-shelf ${j.k}`}>
                    <span className="nq-shelf-h">{j.ic} {tr2(j.nom)}</span>
                    {landed.filter((x) => x.javon === j.k).map((x) => <span key={x.id} className="nq-chip">{kartaOf(x.id).ic} {tr2(kartaOf(x.id).t).split(" — ")[0]}</span>)}
                  </div>)}
              </div>}
            {errs > 0 && !done && <div className={`wsx ${yordamOpen ? "open" : ""}`}>
                <button className="wsx-toggle" onClick={() => setYordamOpen((o) => !o)}>💡 Yordam {yordamOpen ? "▾" : "▸"}</button>
                {yordamOpen && <div className="wsx-body"><p>{tr2({ uz: <>Kartani qayta o'qing va ikki savol bering: bu <b>hammada</b> bo'ladimi? U <b>ijarani to'xtatadimi</b> — yoki ijara baribir bo'ladimi?</>, ru: <>Перечитайте карточку и задайте два вопроса: это происходит <b>у всех</b>? Это <b>останавливает аренду</b> — или аренда всё равно состоится?</> })}</p></div>}
              </div>}
            <StudentPracticePulse live={live} screen={screen} />
            <MentorPracticeStats live={live} screen={screen} label={tr2({ uz: "🗄 To'rt kartaga hukm berganlar", ru: "🗄 Вынесли приговор четырём карточкам" })} />
          </Col>
        </div>
        {done && <div className="bdone fade-step">
            <span className="done-mini">{tr2({ uz: <>✅ Birinchi — 🔴 hammada ishni to'xtatadigani. Keyin — 🟠 bittasi og'ir bo'lgani. Eng oxiri — ⚪ ba'zilarda va ish baribir bo'ladigani <span className="dm-sub">— nosozliklarga shunday navbat qo'yiladi</span></>, ru: <>✅ Первой — 🔴 та, что останавливает работу у всех. Затем — 🟠 та, где тяжёлый только один ответ. В самом конце — ⚪ та, что у некоторых и работа всё равно идёт <span className="dm-sub">— так и выстраивают очередь поломок</span></> })}</span>
          </div>}
        <MentorNote>{tr2({ uz: "Eng ko'p adashiladigan joy — 3 va 4-kartalar: ikkalasi «Bugun» javonida, lekin sababi har xil. «Qaysi biri muhimroq?» degan savolga: ikkalasi bir javonda, tartibi jamoa qarori. Sinf ish-tartibi: har o'quvchi sherigining uch kartasini o'qib, har biriga «bu hammadami? ijarani to'xtatadimi?» deb so'raydi — javob kartada topilmasa, karta qayta yoziladi. Bu ishni o'quvchilar bajaradi, siz kuzatasiz; «Davom etish» siz uchun ochiq.", ru: "Чаще всего путают карточки 3 и 4: обе на полке «Сегодня», но причина разная. На вопрос «какая важнее?»: обе на одной полке, порядок — решение команды. Порядок работы в классе: каждый читает три карточки соседа и к каждой спрашивает «это у всех? это останавливает аренду?» — если ответа в карточке нет, карточку переписывают. Эту работу делают ученики, вы наблюдаете; «Продолжить» для вас открыто." })}</MentorNote>
      </div>
    </Stage>;
};
var KODING_KEY = "pm-m4b2-code";
var readKoding = () => {
  try {
...
var GATE_OPTS = [
  { t: { uz: "Odamlar qo'lida, birinchi sharhlarda", ru: "В руках у людей, в первых отзывах" } },
  { t: { uz: "Dasturchi stolida, kod yozilayotganda", ru: "На столе у программиста, пока пишется код" }, ok: true },
  { t: { uz: "Chiqarishdan keyin, birinchi yangilanishda", ru: "После выпуска, в первом обновлении" } }
];
var KD_FILES = [
  { k: "spec", nom: "narx.spec.ts", ic: "🧪", code: { uz: `// narx.spec.ts — o'tgan darsdagi shakl
...
});`, ru: `// narx.spec.ts — форма из прошлого урока
import { ijaraNarxi } from './narx';

describe('ijaraNarxi', () => {
  it('считает 5 минут вместе с платой за старт', () => {
    // ← это место заполняете вы: expect(...).toBe(...)
  });
});` } },
  { k: "src", nom: "narx.ts", ic: "📄", code: { uz: `// narx.ts — skuter ijarasi narxi (so'm)
...
}`, ru: `// narx.ts — цена аренды самоката (сум)
const BOSHLASH = 2000; // плата за открытие замка
const DAQIQA = 500;    // за каждую минуту
...
var tsHl = (ln) => ln.split(TS_TOKEN).filter((p) => p !== void 0 && p !== "").map((p, i) => {
...
var KD_SHART = [
  <>{tr2({ uz: <><code className="qcode">it</code> ichida <code className="qcode">expect(...).toBe(4500)</code> yozilgan</>, ru: <>Внутри <code className="qcode">it</code> написано <code className="qcode">expect(...).toBe(4500)</code></> })}</>,
  <>{tr2({ uz: "Test avval qizil chiqdi", ru: "Сначала тест был красным" })}</>,
  <>{tr2({ uz: <><code className="qcode">narx.ts</code> dagi nosozlik tuzatildi — test yashil chiqdi</>, ru: <>Поломка в <code className="qcode">narx.ts</code> исправлена — тест стал зелёным</> })}</>
];
var ScreenCoding = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [yordamOpen, setYordamOpen] = useState3(false);
  const [tab, setTab] = useState3("spec");
  ...
  const lines = tr2(file.code).split("\n");
  const doneTurn = useTurnHint(stage2 && !done && !isMentor);
  const navLabel = done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : !stage2 ? tr2({ uz: "🔒 Avval savolga javob bering", ru: "🔒 Сначала ответьте на вопрос" }) : tr2({ uz: "② Kodni yozing va tugmani bosing", ru: "② Напишите код и нажмите кнопку" });
  return <Stage eyebrow="Koding · 🧪 VS Code" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.5vw,15px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Nosozlikni odamlarga yetmasidan tutadigan <span className="italic" style={{ color: T.accent }}>test</span> yozamiz.</>, ru: <>Напишем <span className="italic" style={{ color: T.accent }}>тест</span>, который ловит поломку до того, как она дойдёт до людей.</> })}</h2></div>
        {!stage2 ? <>
            <Mentor>{tr2({ uz: "Avval bitta savol — keyin kod yoziladi.", ru: "Сначала один вопрос — потом пишем код." })}</Mentor>
            <div className="cmt hunt">
              <span className="cmt-lbl">{tr2({ uz: "🔎 Narx testi qizil chiqdi. Nosozlik qayerda tutildi?", ru: "🔎 Тест цены стал красным. Где поймали поломку?" })}</span>
              ...
              {missedOnce && <p className="cmt-tip">{tr2({ uz: "🤔 Test kod yozilayotgan joyda ishlaydi — qizil chiqqan lahzada nosozlik hali hech kimga yetmagan.", ru: "🤔 Тест работает там, где пишется код — в момент, когда он покраснел, поломка ещё ни до кого не дошла." })}</p>}
            </div>
          </> : <>
            <Mentor>{tr2({ uz: "Skuter jamoasi ijara narxini hisoblaydigan kod yozdi. Shu kod to'g'ri hisoblayaptimi — testda tekshirasiz: tarif kartasidan 5 daqiqalik narxni hisoblang.", ru: "Команда самокатов написала код, который считает цену аренды. Правильно ли он считает — проверите тестом: посчитайте по карточке тарифа цену за 5 минут." })}</Mentor>
            <div className="cmt-fold fade-step"><span className="cmt-done">{tr2({ uz: "✓ Dasturchi stolida, kod yozilayotganda", ru: "✓ На столе у программиста, пока пишется код" })}</span></div>
            <div className="split">
              <Col gap={10}>
                <div className={`kdpanel${done ? " is-done" : ""}`}>
                  <p className="flow-label">{tr2({ uz: "Nima bajarilishi kerak", ru: "Что нужно сделать" })}</p>
                  <div className="tarif">
                    <span className="tarif-h">{tr2({ uz: "🛴 Skuter tarifi", ru: "🛴 Тариф самоката" })} <span className="tarif-nm">{tr2({ uz: "namuna", ru: "пример" })}</span></span>
                    <span className="tarif-row"><span>{tr2({ uz: "Boshlash (qulfni ochish)", ru: "Старт (открытие замка)" })}</span><b>{tr2({ uz: "2000 so'm", ru: "2000 сум" })}</b></span>
                    <span className="tarif-row"><span>{tr2({ uz: "Har daqiqa", ru: "Каждая минута" })}</span><b>{tr2({ uz: "500 so'm", ru: "500 сум" })}</b></span>
                    <span className="tarif-row calc"><span>{tr2({ uz: "Misol: 5 daqiqa", ru: "Пример: 5 минут" })}</span><b>{tr2({ uz: "2000 + 5 × 500 = 4500 so'm", ru: "2000 + 5 × 500 = 4500 сум" })}</b></span>
                  </div>
                  ...
                  <div className={`wsx star ${yordamOpen ? "open" : ""}`}>
                    ...
                    {yordamOpen && <div className="wsx-body">
                      <p>{tr2({ uz: <>Tarif kartasidan hisoblang: boshlash haqi + daqiqalar soni × daqiqa narxi. Chiqqan sonni <code className="qcode">toBe</code> ichiga yozing, keyin <code className="qcode">npm test</code>.</>, ru: <>Посчитайте по карточке тарифа: плата за старт + число минут × цена минуты. Получившееся число впишите внутрь <code className="qcode">toBe</code>, затем <code className="qcode">npm test</code>.</> })}</p>
                      <p>{tr2({ uz: <>Test qizil chiqsa — nosozlik <code className="qcode">narx.ts</code> da: tarifda ikki raqam bor, hisobda ikkalasi ham qatnashyaptimi? <code className="qcode">toBe</code> ichidagi sonni o'zgartirmang.</>, ru: <>Если тест красный — поломка в <code className="qcode">narx.ts</code>: в тарифе два числа, участвуют ли оба в расчёте? Число внутри <code className="qcode">toBe</code> не меняйте.</> })}</p>
                      <p>{tr2({ uz: <>⭐ Qo'shimcha: ikkinchi <code className="qcode">it</code> yozing — 12 daqiqa uchun kutilgan narx 8000 so'm.</>, ru: <>⭐ Дополнительно: напишите второй <code className="qcode">it</code> — ожидаемая цена за 12 минут 8000 сум.</> })}</p>
                    </div>}
                  </div>
                  <button className={`lp-done-btn ${done ? "is-done" : ""}${!done && doneTurn ? " turn-ring" : ""}`} disabled={done} onClick={done ? void 0 : complete}>
                    {done ? tr2({ uz: "✓ Bajarildi", ru: "✓ Выполнено" }) : tr2({ uz: "✅ VS Code'da yozdim — test avval qizil, keyin yashil chiqdi", ru: "✅ Написал в VS Code — тест сначала был красным, потом стал зелёным" })}
                  </button>
                  {done && <div className="done-mini fade-step">{tr2({ uz: <>✅ Nosozlik kod yozilayotganda tutildi <span className="dm-sub">— tarozining birinchi nuqtasi</span></>, ru: <>✅ Поломку поймали, пока писался код <span className="dm-sub">— первая точка весов</span></> })}</div>}
                  {!done && isSelf && <button className="kd-skip" onClick={onNext}>{tr2({ uz: "✓ Bu kodni sinfda yozganman — davom etish →", ru: "✓ Этот код я писал(а) в классе — продолжить →" })}</button>}
                </div>
                <StudentPracticePulse live={live} screen={screen} />
                <MentorPracticeStats live={live} screen={screen} label={tr2({ uz: "🧪 Testni yozib bo'lganlar", ru: "🧪 Дописали тест" })} />
              </Col>
              <Col gap={10}>
                <div className="vsc no-copy" onCopy={(e) => e.preventDefault()} onCut={(e) => e.preventDefault()} onPaste={(e) => e.preventDefault()} onContextMenu={(e) => e.preventDefault()}>
                  <div className="vsc-bar">
                    {KD_FILES.map((f) => <button key={f.k} type="button" className={`vsc-tab btn${tab === f.k ? " on" : ""}`} onClick={() => setTab(f.k)}>
                  ...
                  <div className="vsc-body">
                    {lines.map((ln, i) => <div key={i} className="vsc-line"><span className="vsc-ln">{i + 1}</span><span className="vsc-code">{ln ? tsHl(ln) : " "}</span></div>)}
                  </div>
                </div>
                <p className="bhint">{tr2({ uz: "🔒 Kodni VS Code (kod yoziladigan dastur)da qo'lda yozasiz — qo'lda yozganda o'rganiladi.", ru: "🔒 Код вы пишете руками в VS Code (программа для написания кода) — руками и запоминается." })}</p>
              </Col>
            </div>
          </>}
        <MentorNote>{tr2({ uz: "Test qizil chiqqan lahzani ochiq nomlang: mana — nosozlik kod yozilayotganda tutildi, tarozining birinchi nuqtasi. Bolalar qizilni «xato qildim» deb tushunmasin. Kod 10 daqiqada yoziladi; ulgurmagan o'quvchi uyga qisqa variantni oladi. Bu ishni o'quvchilar bajaradi, siz kuzatasiz; «Davom etish» siz uchun ochiq.", ru: "Назовите вслух момент, когда тест покраснел: вот — поломку поймали, пока писался код, это первая точка весов. Пусть дети не понимают красный как «я ошибся». Код пишется за 10 минут; кто не успел, берёт короткий вариант домой. Эту работу делают ученики, вы наблюдаете; «Продолжить» для вас открыто." })}</MentorNote>
      </div>
    </Stage>;
};
var REFLECT_KEY = "pm-m4b2-reflection";
...
var ScreenReflection = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Mustahkamlash · 2 qadam", ru: "Закрепление · 2 шага" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext turnBusy={!written} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Uch kartangizni <span className="italic" style={{ color: T.accent }}>yoddan</span> ayta olasizmi?</>, ru: <>Сможете назвать свои три карточки <span className="italic" style={{ color: T.accent }}>по памяти</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: "Ekranga qaramasdan javob bering: qaysi nosozlik birinchi tuzatiladi va nega?", ru: "Ответьте, не глядя на экран: какую поломку чинят первой и почему?" })}</Mentor>
        <div className="rcp-flow">
          <div className="rcp-step fade-up delay-1">
            <div className="rcp-step-h"><span className="rcp-n">1</span><div><span className="rcp-t">🗣 {yakka ? tr2({ uz: "Ovoz chiqarib o'zingizga ayting: qaysi karta birinchi va nega", ru: "Скажите вслух себе: какая карточка первая и почему" }) : tr2({ uz: "Sherigingizga ayting: qaysi karta birinchi va nega", ru: "Скажите соседу: какая карточка первая и почему" })}</span></div></div>
            <PairTimer onStage={setPairStage} muted={written} solo={yakka} />
          </div>
          <div className="rcp-step fade-up delay-2">
            <div className="rcp-step-h"><span className="rcp-n">2</span><div><span className="rcp-t">{tr2({ uz: "✍️ Endi shu javobni bir qatorda yozing", ru: "✍️ Теперь запишите этот ответ одной строкой" })}</span></div></div>
            <span className={`turn-wrap${inputTurn ? " turn-ring" : ""}`}>
              <input className="reflect-input" value={text} onChange={(e) => save(e.target.value)} onFocus={() => setReflFocus(true)} onBlur={() => setReflFocus(false)} placeholder={tr2({ uz: "Birinchi ... kartasi, chunki ...", ru: "Первая — карточка ..., потому что ..." })} maxLength={160} />
            </span>
            {
    /* 106f(b): yozib bo'lgach mukofot — bitta tabrik-gap va bitta qoida-qatori */
  }
            {written && <div className="rwd fade-step">
                <p className="rwd-t">{tr2({ uz: "✓ Endi siz nosozlikni «yomon ishlaydi» deb emas, karta qilib ko'rasiz.", ru: "✓ Теперь вы видите поломку не как «плохо работает», а как карточку." })}</p>
                <span className="rwd-rule">🎯 Bugungi qoida: nosozlik qancha kech topilsa, shuncha qimmatga tushadi</span>
              </div>}
          </div>
        </div>
        <MentorNote>{tr2({ uz: "Uchdan biri «nega» savoliga javob berolmasa — navbat ekranini qayta oching va 1-karta bilan 3-kartani yonma-yon solishtiring.", ru: "Если треть класса не отвечает на вопрос «почему» — снова откройте экран очереди и сравните карточку 1 с карточкой 3 рядом." })}</MentorNote>
      </div>
    </Stage>;
};
...
var FLASHCARDS = [
  { front: { uz: "Sifat nima?", ru: "Что такое качество?" }, back: { uz: "Ilova har safar aytganini qilsa — buni sifat deyiladi", ru: "Если приложение каждый раз делает обещанное — это качество" } },
  { front: { uz: "Nosozlik nima?", ru: "Что такое поломка?" }, back: { uz: "Ilova aytganini qilmasa — bu nosozlik (inglizchasi — bug)", ru: "Если приложение не делает обещанного — это поломка (по-английски — bug)" } },
  { front: { uz: "Yoqmagan narsa nosozlikmi?", ru: "То, что не понравилось, — это поломка?" }, back: { uz: "Yo'q — ilova aytganini qilgan bo'lsa, bu nosozlik emas", ru: "Нет — если приложение сделало обещанное, это не поломка" } },
  { front: { uz: "Nosozlik narxi nimaga bog'liq?", ru: "От чего зависит цена поломки?" }, back: { uz: "Qancha kech topilsa, shuncha qimmatga tushadi", ru: "Чем позже найдена, тем дороже обходится" } },
  { front: { uz: "Eng arzon qayerda tutiladi?", ru: "Где ловить дешевле всего?" }, back: { uz: "Kod yozilayotganda — test tutadi, odam ko'rmaydi", ru: "Пока пишется код — ловит тест, люди не видят" } },
  { front: { uz: "Qayerda chiqsa odam ketadi?", ru: "Где вылезет — там люди уходят?" }, back: { uz: "Odamlar qo'lida — sharhlarda chiqqanda", ru: "В руках у людей — когда вылезает в отзывах" } },
  { front: { uz: "Nosozlik-kartasida nima yoziladi?", ru: "Что пишут в карточке поломки?" }, back: { uz: "Nima bosildi — nima bo'ldi · kimda · nima bo'ladi", ru: "Что нажали — что произошло · у кого · что при этом происходит" } },
  { front: { uz: "Qaysi nosozlik birinchi tuzatiladi?", ru: "Какую поломку чинят первой?" }, back: { uz: "Hammada ishni to'xtatadigani", ru: "Ту, что останавливает работу у всех" } },
  { front: { uz: "Cyberpunk 2077 nosozliklari qayerda chiqdi?", ru: "Где вылезли поломки Cyberpunk 2077?" }, back: { uz: "Sotib olgan o'yinchilar qo'lida (2020)", ru: "В руках у купивших игроков (2020)" } },
  { front: { uz: "Nosozliklarga navbat qanday qo'yiladi?", ru: "Как выстраивают очередь поломок?" }, back: { uz: "Har kartaga ikki savol: kimda? nima bo'ladi? (inglizchasi — bug triage)", ru: "К каждой карточке два вопроса: у кого? что происходит? (по-английски — bug triage)" } }
];
...
var ScreenFinalTest = (props) => <QuestionScreen
  ...
  ctaLabel={tr2({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr2({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr2({ uz: "📋 Uch nosozlik bir kunda keldi. Qaysi biri birinchi tuzatiladi?", ru: "📋 Три поломки пришли в один день. Какую чинят первой?" })} />}
  questionText={tr2({ uz: "Uch nosozlikdan qaysi biri birinchi tuzatiladi", ru: "Какую из трёх поломок чинят первой" })}
  options={[tr2({ uz: "Hammada: ilova ochilganda oq ekran chiqadi", ru: "У всех: при открытии приложения белый экран" }), tr2({ uz: "Hammada: safar cheki mayda, o'qish qiyin", ru: "У всех: чек поездки мелкий, читать трудно" }), tr2({ uz: "Ba'zilarda: eski telefonda xarita sekin ochiladi", ru: "У некоторых: на старом телефоне карта открывается медленно" })]}
  correctIdx={0}
  explainCorrect={tr2({ uz: "To'g'ri — hammada bo'ladi va ilova umuman ochilmaydi: ikkalasi ham og'ir.", ru: "Верно — это у всех, и приложение вообще не открывается: оба ответа тяжёлые." })}
  explainWrong={{
    1: tr2({ uz: "Chek mayda bo'lsa noqulay, lekin safar ham, ijara ham bo'laveradi — bittasi og'ir.", ru: "Мелкий чек неудобен, но и поездка, и аренда состоятся — тяжёлый только один ответ." }),
    2: tr2({ uz: "Sekin ochilsa charchatadi, lekin faqat eski telefonlarda va xarita baribir ochiladi.", ru: "Медленное открытие утомляет, но только на старых телефонах, и карта всё равно открывается." }),
    default: tr2({ uz: "Birinchi — hammada bo'ladigan va ishni to'xtatadigan nosozlik.", ru: "Первой — та поломка, что бывает у всех и останавливает работу." })
  }}
/>;
var HW_KEY = "pm-m4b2-hw-target";
var HW_VARIANT = [
  { k: "toliq", t: { uz: "To'liq · ~20 daqiqa", ru: "Полный · ~20 минут" } },
  { k: "qisqa", t: { uz: "Qisqa · ~10 daqiqa", ru: "Короткий · ~10 минут" } }
];
var HW_STEPS = {
  toliq: [{ uz: "O'zingiz ishlatadigan ilovada uchragan nosozlikni eslang", ru: "Вспомните поломку в приложении, которым пользуетесь сами" }, { uz: "Kartaga yozing: nima bosildi — nima bo'ldi, kimda, nima bo'ladi", ru: "Запишите карточку: что нажали — что произошло, у кого, что при этом происходит" }, { uz: "Javonini belgilang va sababini bir gap bilan yozing", ru: "Отметьте её полку и запишите причину одним предложением" }],
  qisqa: [{ uz: "Uch kartangizdan birinchi tuzatiladiganini toping", ru: "Найдите среди трёх карточек ту, что чинят первой" }, { uz: "Uni belgilang", ru: "Отметьте её" }, { uz: "Sababini bir gap bilan yozing", ru: "Запишите причину одним предложением" }]
};
var readHwTarget = () => {
  try {
...
var HwCard = ({ variant, onPick }) => {
  ...
  return <div className="card fade-step">
      ...
      <>
          <p className="body" style={{ margin: "0 0 10px", color: T.ink }}>{tr2({ uz: "Uyda kartalaringizni davom ettirasiz: o'zingiz ishlatadigan ilovada uchragan nosozlikni karta qilib yozasiz va uni qaysi javonga qo'yishni belgilaysiz. Qancha vaqtingiz bor — o'zingiz tanlaysiz.", ru: "Дома вы продолжите свои карточки: запишете карточкой поломку из приложения, которым пользуетесь сами, и отметите, на какую полку её поставить. Сколько у вас времени — выбираете сами." })}</p>
      ...
      {variant ? <div className="pmtask fade-step">
          ...
          <div className="pmtask-rows">
            <div className="pmtask-row"><span className="pmtask-k">{tr2({ uz: "Nechta", ru: "Сколько" })}</span><span className="pmtask-v"><b>{variant === "qisqa" ? tr2({ uz: "1 ta tayyor karta", ru: "1 готовая карточка" }) : tr2({ uz: "1 ta yangi karta", ru: "1 новая карточка" })}</b></span></div>
...
var ACHIEVEMENTS = {
  cheapFix: { icon: "⚖️", name: "Cheap Fix!", desc: { uz: "Nosozlik narxini uch nuqtada ko'rdingiz", ru: "Вы увидели цену поломки в трёх точках" } },
  bugReporter: { icon: "🗂", name: "Bug Reporter!", desc: { uz: "Uch sharhni karta qilib yozdingiz", ru: "Вы превратили три отзыва в карточки" } },
  priorityCall: { icon: "🔴", name: "Priority Call!", desc: { uz: "To'rt kartaga to'g'ri hukm berdingiz", ru: "Вы верно вынесли приговор четырём карточкам" } },
...
var ACH_TRIGGERS = { s4: "cheapFix", s8: "bugReporter", s9: "priorityCall", s10: "redToGreen" };
...
var Q_LABELS = { 3: { uz: "1 — Nosozlik nima", ru: "1 — Что такое поломка" }, 5: { uz: "2 — Qayerda tutildi", ru: "2 — Где поймали" }, 7: { uz: "3 — Cyberpunk misoli", ru: "3 — Пример Cyberpunk" }, 11: { uz: "4 — Yakuniy savol", ru: "4 — Итоговый вопрос" } };
var QUIZ_MS = 15e3;
var QZ_BG_SHAPES = [
...
var QUIZ_BANK = [
  { q: { uz: "Sifat nima?", ru: "Что такое качество?" }, opts: [{ uz: "Ilova har safar aytganini qilsa", ru: "Когда приложение каждый раз делает обещанное" }, { uz: "Ilovani juda ko'p odam yuklab olsa", ru: "Когда приложение скачивает очень много людей" }, { uz: "Ilova boshqalardan tez ochilsa", ru: "Когда приложение открывается быстрее других" }, { uz: "Ilovada tugmalar chiroyli qo'yilsa", ru: "Когда в приложении красиво расставлены кнопки" }], correct: 0 },
  { q: { uz: "Qaysi holat — nosozlik?", ru: "Какой случай — поломка?" }, opts: [{ uz: "Skuter ro'yxatda uzoqroqda ko'rinadi", ru: "Самокат в списке показан подальше" }, { uz: "Safar tugagach baho so'raladi", ru: "После поездки просят оценку" }, { uz: "Narx kecha arzonroq edi", ru: "Вчера цена была ниже" }, { uz: "Tugatish bosildi, hisoblagich yurdi", ru: "Нажали «Завершить», а счётчик пошёл дальше" }], correct: 3 },
  { q: { uz: "Ilova aytganini qildi, lekin yoqmadi — bu nima?", ru: "Приложение сделало обещанное, но не понравилось — что это?" }, opts: [{ uz: "Nosozlik — kartaga yoziladi", ru: "Поломка — записывается в карточку" }, { uz: "Yoqmagani muhim emas — bu nosozlik", ru: "Неважно, что не понравилось — это поломка" }, { uz: "Nosozlik emas — yoqmagan narsa", ru: "Не поломка — просто не понравилось" }, { uz: "Nosozlik — javonga qo'yiladi", ru: "Поломка — ставится на полку" }], correct: 2 },
  { q: { uz: "Nosozlik qayerda tutilsa eng arzon?", ru: "Где поломку поймать дешевле всего?" }, opts: [{ uz: "Chiqarishdan oldin", ru: "Перед выпуском" }, { uz: "Kod yozilayotganda", ru: "Пока пишется код" }, { uz: "Odamlar qo'lida", ru: "В руках у людей" }, { uz: "Birinchi sharhlar chiqqanda", ru: "Когда выйдут первые отзывы" }], correct: 1 },
  { q: { uz: "Nosozlik qayerda chiqsa odam ketadi?", ru: "Где вылезшая поломка уводит людей?" }, opts: [{ uz: "Kod yozilayotganda", ru: "Пока пишется код" }, { uz: "Odamlar qo'lida", ru: "В руках у людей" }, { uz: "Chiqarishdan oldin", ru: "Перед выпуском" }, { uz: "Test yozilayotganda", ru: "Пока пишется тест" }], correct: 1 },
  { q: { uz: "Nosozlik qancha kech topilsa nima bo'ladi?", ru: "Что будет, чем позже найдена поломка?" }, opts: [{ uz: "Shuncha qimmatga tushadi", ru: "Тем дороже она обойдётся" }, { uz: "Shuncha tez tuzatiladi", ru: "Тем быстрее её починят" }, { uz: "Shuncha kam odam ko'radi", ru: "Тем меньше людей её увидит" }, { uz: "Shuncha arzonga tushadi", ru: "Тем дешевле она обойдётся" }], correct: 0 },
  { q: { uz: "Karta birinchi qatorida nima yoziladi?", ru: "Что пишут в первой строке карточки?" }, opts: [{ uz: "Nosozlik qachon tuzatilishi", ru: "Когда поломку починят" }, { uz: "Nosozlikni kim topgani", ru: "Кто нашёл поломку" }, { uz: "Nima bosildi — nima bo'ldi", ru: "Что нажали — что произошло" }, { uz: "Nosozlik qaysi javonga tushishi", ru: "На какую полку попадёт поломка" }], correct: 2 },
  { q: { uz: "Qaysi nosozlik birinchi tuzatiladi?", ru: "Какую поломку чинят первой?" }, opts: [{ uz: "Ba'zilarda — ish to'xtaydi", ru: "У некоторых — работа останавливается" }, { uz: "Ba'zilarda — noqulay, lekin ishlaydi", ru: "У некоторых — неудобно, но работает" }, { uz: "Hammada — noqulay, lekin ishlaydi", ru: "У всех — неудобно, но работает" }, { uz: "Hammada — ish to'xtaydi", ru: "У всех — работа останавливается" }], correct: 3 },
  { q: { uz: "«Ba'zilarda · noqulay» karta qaysi javonga tushadi?", ru: "На какую полку попадает карточка «у некоторых · неудобно»?" }, opts: [{ uz: "Keyin javoniga", ru: "На полку «Потом»" }, { uz: "Kartaga yozilmaydi", ru: "Её не записывают в карточку" }, { uz: "Hozir javoniga", ru: "На полку «Сейчас»" }, { uz: "Bugun javoniga", ru: "На полку «Сегодня»" }], correct: 0 },
  { q: { uz: "Cyberpunk 2077 nosozliklari kimning qo'lida chiqdi?", ru: "В чьих руках вылезли поломки Cyberpunk 2077?" }, opts: [{ uz: "O'yinni yasagan jamoada", ru: "У команды, сделавшей игру" }, { uz: "Do'kon tekshiruvchilarida", ru: "У проверяющих магазина" }, { uz: "Sotib olgan o'yinchilarda", ru: "У купивших игроков" }, { uz: "Reklama yozganlarda", ru: "У тех, кто писал рекламу" }], correct: 2 },
  { q: { uz: "Sony o'yinni do'kondan qancha vaqtga olib tashladi?", ru: "На сколько Sony убрала игру из магазина?" }, opts: [{ uz: "Bir necha kunga", ru: "На несколько дней" }, { uz: "Qariyb yarim yilga", ru: "Почти на полгода" }, { uz: "Bir oyga yaqin", ru: "Примерно на месяц" }, { uz: "Butunlay, qaytarmadi", ru: "Совсем, не вернула" }], correct: 1 },
  { q: { uz: "Narx testi qizil chiqdi — bu nima degani?", ru: "Тест цены стал красным — что это значит?" }, opts: [{ uz: "Kod umuman ishga tushmay qoldi", ru: "Код вообще перестал запускаться" }, { uz: "Testni qaytadan yozish kerak bo'ldi", ru: "Тест придётся переписывать заново" }, { uz: "Odamlar shikoyat qila boshladi", ru: "Люди начали жаловаться" }, { uz: "Nosozlik dasturchi stolida tutildi", ru: "Поломку поймали на столе у программиста" }], correct: 3 }
];
...
function QuizArena({ live, onClose, startSolo }) {
  ...
  const closeArena = () => {
    if (isMentor && !solo && phase !== "done") {
      if (typeof window !== "undefined" && !window.confirm(tr2({ uz: "Arena hali yakunlanmadi — yopsangiz o'quvchilar arenada kutib qoladi.\nBaribir yopilsinmi?", ru: "Арена ещё не завершена — если закрыть, ученики останутся ждать на арене.\nВсё равно закрыть?" }))) return;
    }
    onClose();
  };
  return <div className="qz-arena">
      ...
      {classEnded && isStudent && !solo && phase !== "done" && <div className="qz-endnote fade-step">
          <span>{tr2({ uz: "⚠️ Jonli dars yakunlandi — savollarni o'zingiz davom ettiring:", ru: "⚠️ Живой урок завершён — продолжайте вопросы сами:" })}</span>
      ...
      {phase === "lobby" && <div className="qz-view fade-step">
          ...
          {isMentor && <button className="qz-btn big" disabled={players.length === 0} onClick={() => ctrl("q", 0)}>{tr2({ uz: "▶ Arenani boshlash", ru: "▶ Начать арену" })}</button>}
...
var ScreenSummary = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  const RECAP = [
    tr2({ uz: "Ilova har safar aytganini qilsa — buni sifat deyiladi.", ru: "Если приложение каждый раз делает обещанное — это качество." }),
    "Nosozlik qancha kech topilsa, shuncha qimmatga tushadi.",
    tr2({ uz: "Har nosozlik-kartasi nima bo'lganini, kimda va ishni to'xtatishini aytadi.", ru: "Каждая карточка поломки говорит, что произошло, у кого и останавливает ли это работу." }),
    tr2({ uz: "Birinchi — hammada ishni to'xtatadigan nosozlik tuzatiladi.", ru: "Первой чинят поломку, которая останавливает работу у всех." })
  ];
  ...
  return <Stage eyebrow={tr2({ uz: "Dars yakuni", ru: "Итог урока" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen s-fin" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="hero">
          <div className="hero-l">
            ...
            <h2 className="title h-title fade-up d1">{tr2({ uz: <>Uch kartangizni yozdingiz va <span className="italic" style={{ color: T.accent }}>navbat</span> qo'ydingiz.</>, ru: <>Вы записали три карточки и выстроили <span className="italic" style={{ color: T.accent }}>очередь</span>.</> })}</h2>
          </div>
    ...
    /* 103-qonun: darsni bitta gap yopadi */
        ...
        <div className="bigidea fade-up d2"><span className="bigidea-lbl">{tr2({ uz: "Bugungi asosiy fikr —", ru: "Главная мысль сегодня —" })}</span><p className="bigidea-t">{tr2({ uz: "Kech tutilgan nosozlik pul bilan emas, ketgan odam bilan to'lanadi.", ru: "Поздно пойманная поломка оплачивается не деньгами, а ушедшими людьми." })}</p></div>
    ...
    return <div key={id} className={`ach-badge ${got ? "got" : "locked"}`} title={tr2(a.desc)}>
        ...
        <MentorNote>{tr2({ uz: "Arena tugagach g'oliblarni nomlab tabriklang. Uy-vazifa: kod topshirig'ini sinfda tugatganlarga to'liq variant, ulgurmaganlarga qisqa variant. Muddat — keyingi darsgacha. Tekshirishda bitta savolga qarang: kartada harakat va natija bormi — nima bosildi, nima bo'ldi?", ru: "После арены назовите и поздравьте победителей. Домашнее задание: кто закончил код в классе — полный вариант, кто не успел — короткий. Срок — до следующего урока. При проверке смотрите на один вопрос: есть ли в карточке действие и результат — что нажали, что произошло?" })}</MentorNote>
      </div>
    </Stage>;
};
var CSS_BASE = `
...
`;
function PmLesson16({ lang: langProp, onFinished, liveToken }) {
