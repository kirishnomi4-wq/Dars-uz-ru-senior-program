...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и счёт ✅/❌ скрыты — по нажатию «Открыть результат» всё откроется сразу и у вас, и на экранах учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верных — класс не разобрался в этой теме. Прежде чем идти дальше, стоит коротко повторить.</> })}</p>
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Kirish", ru: "Введение" })} screen={screen} scrollSignal={sc} navContent={<NavNext optionalLive disabled={picked === null} label={tr({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
    ...
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null || !tried} style={{ opacity: !tried ? 0.55 : 1 }} onClick={() => pick(o.id)}><span className="radio">{on && <span className="radio-dot" />}</span><span>{tr(o.label)}</span></button>;
            ...
            {picked !== null && <p className="hook-ack fade-step">{picked === "b" ? tr({ uz: <>Aynan! Bugun lentangizni <b>ishonchli</b> qilamiz: parallel lentalar, yaqin javon, seyf va sinov reysi bilan.</>, ru: <>Именно! Сегодня сделаем вашу ленту <b>надёжной</b>: параллельные ленты, ближняя полка, сейф и тестовый рейс.</> }) : tr({ uz: <>Productionni bitta muhitga moslasangiz — boshqa server kelganda yana sinadi; e'tibor bermasangiz — xatoni foydalanuvchi topib beradi. Uchinchi yo'l: <b>bir nechta muhitda parallel tekshirish</b>. Bugun shundan boshlaymiz.</>, ru: <>Подогнать продакшен под одну среду — сломается снова на другом сервере; не обращать внимания — ошибку найдёт пользователь. Третий путь: <b>проверять параллельно в нескольких средах</b>. С этого сегодня и начнём.</> })}</p>}
          </Col>
        </Split></Zoomable>
      </div>
    </Stage>;
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Reja", ru: "План" })} screen={screen} mentorStatic scrollSignal={showSteps} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive label={tr({ uz: "Boshlaymiz →", ru: "Начнём →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Lentani <span className="italic" style={{ color: T.accent }}>ishonchli</span> qanday qilamiz?</>, ru: <>Как сделать ленту <span className="italic" style={{ color: T.accent }}>надёжной</span>?</> })}</h2></div>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState(storedAnswer ? new Set(MATRIX_ENVS.map((e) => e.id)) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={tr({ uz: "Tushuncha · matrix", ru: "Понятие · matrix" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${tr({ uz: "3 muhitni ko'ring", ru: "Посмотрите 3 среды" })} (${seen.size}/3)`} onClick={onNext} /></>}>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [status, setStatus] = useState(() => storedAnswer ? { node18: "fail", node20: "pass", node22: "pass" } : {});
  ...
  return <Stage eyebrow={tr({ uz: "Payoff · matrix", ru: "Эффект · matrix" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Matrixni ishga tushiring", ru: "Запустите matrix" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Muammo · kesh yo'q", ru: "Проблема · без кеша" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Install ishga tushiring", ru: "Запустите install" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
    ...
    setRan(true);
    setSc((n) => n + 1);
  }}>{ran ? tr({ uz: "✓ Yuklandi", ru: "✓ Загружено" }) : "▶ npm install"}</button>
          </Col>
          <Col>
            <div className="timer-row fade-up delay-2">
              <div className={`timer-chip ${ran ? "danger" : ""}`}><span className="tc-lbl">{tr({ uz: "Keshsiz reys", ru: "Рейс без кеша" })}</span><span className="tc-val">{ran ? tr({ uz: "40 s", ru: "40 сек" }) : "—"}</span></div>
            </div>
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Yechim · yaqin javon", ru: "Решение · ближняя полка" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Keshdan install qiling", ru: "Сделайте install из кеша" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 «▶ npm install (kesh bilan)» tugmasini bosing — ikki vaqt-chipini solishtiring.", ru: "💡 Нажмите «▶ npm install (с кешем)» — сравните два чипа времени." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
        <Zoomable>
        <div className="split">
          <Col>
    ...
    setRan(true);
    setSc((n) => n + 1);
  }}>{ran ? tr({ uz: "✓ Yuklandi", ru: "✓ Загружено" }) : tr({ uz: "▶ npm install (kesh bilan)", ru: "▶ npm install (с кешем)" })}</button>
          </Col>
          <Col>
            <div className="timer-row fade-up delay-2">
              <div className="timer-chip danger"><span className="tc-lbl">{tr({ uz: "Keshsiz", ru: "Без кеша" })}</span><span className="tc-val">{tr({ uz: "40 s", ru: "40 сек" })}</span></div>
              <div className={`timer-chip ${ran ? "success" : ""}`}><span className="tc-lbl">{tr({ uz: "Kesh bilan", ru: "С кешем" })}</span><span className="tc-val">{ran ? tr({ uz: "8 s", ru: "8 сек" }) : "—"}</span></div>
            </div>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tushuncha · cache kaliti", ru: "Понятие · ключ кеша" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Yo'l xaritasini ko'ring", ru: "Посмотрите маршрутную карту" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
    ...
    setShow(true);
    setSc((n) => n + 1);
  }}>{show ? tr({ uz: "✓ Ko'rdingiz", ru: "✓ Посмотрели" }) : tr({ uz: "▶ Bu nimani bildiradi?", ru: "▶ Что это значит?" })}</button>
          </Col>
          <Col>
            {show ? <div className="sk-info fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: <><span className="mono">path</span> — qaysi papka saqlanadi (<span className="mono">node_modules</span>). <span className="mono">key</span> — paketlar ro'yxati o'zgarmagan bo'lsa, oldingi keshdan foydalaniladi.</>, ru: <><span className="mono">path</span> — какая папка сохраняется (<span className="mono">node_modules</span>). <span className="mono">key</span> — если список пакетов не менялся, используется прежний кеш.</> })}</p></div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Tugmani bosing ←", ru: "Нажмите кнопку ←" })}</p></div>}
...
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Hayotiy · seyf", ru: "Из жизни · сейф" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : openSeen ? tr({ uz: "Seyfga joylang", ru: "Положите в сейф" }) : tr({ uz: "Ochiq holatni ko'ring", ru: "Посмотрите открытый вариант" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState(storedAnswer ? new Set(STAGE_MODES.map((m) => m.id)) : /* @__PURE__ */ new Set());
  const [active, setActive] = useState(null);
  const [sc, setSc] = useState(0);
  ...
  return <Stage eyebrow={tr({ uz: "Tushuncha · staging", ru: "Понятие · staging" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Ikkalasini ko'ring", ru: "Посмотрите оба" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Hayotiy · production", ru: "Из жизни · production" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Yangi yukni chiqarish", ru: "Выпустить новый багаж" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
    ...
    setBroke(true);
    setSc((n) => n + 1);
  }}>{broke ? tr({ uz: "✓ Yangi yuk production'ga chiqdi", ru: "✓ Новый багаж вышел в продакшен" }) : tr({ uz: "✈️ Yangi yukni production'ga chiqarish", ru: "✈️ Выпустить новый багаж в продакшен" })}</button>
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [on, setOn] = useState(() => storedAnswer ? new Set(IMPROVEMENTS.map((i) => i.id)) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={tr({ uz: "Markaziy · ishonchli lenta", ru: "Центральный · надёжная лента" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : allOn ? tr({ uz: "Eski yukni qaytaring", ru: "Верните старый багаж" }) : `${tr({ uz: "Yaxshilashlarni yoqing", ru: "Включите улучшения" })} (${on.size}/4)`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Lentani <span className="italic" style={{ color: T.accent }}>o'zingiz</span> ishonchli qiling.</>, ru: <>Сделайте ленту надёжной <span className="italic" style={{ color: T.accent }}>сами</span>.</> })}</h2></div>
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Lentani yig'ing", ru: "Соберите ленту" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var SummaryScreen = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: "Ishonchli lentani yig'dingiz", ru: "Вы собрали надёжную ленту" })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Endi lentangiz — <span className="italic" style={{ color: T.accent }}>tez, xavfsiz, qaytariladigan</span>.</>, ru: <>Теперь ваша лента — <span className="italic" style={{ color: T.accent }}>быстрая, безопасная, обратимая</span>.</> })}</h2>{
...
function FullProPipelineLesson({ lang: langProp, onFinished }) {
