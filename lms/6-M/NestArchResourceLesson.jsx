...
function LiveGate({ live, title = { uz: "Jonli dars", ru: "Живой урок" } }) {
  ...
  return <div style={wrap}><div style={card}>
    ...
    <button onClick={() => {
    live.selfStudy();
  }} style={link}>{tr({ uz: "Mustaqil o'qiyman →", ru: "Буду учиться сам →" })}</button>
    <button onClick={() => {
    setRole("mentor");
    setCode("");
...
var BoardCtx = createContext(null);
...
var CarSwagger = ({ available, openId, onToggle, triedIds, onTry }) => <div className="swg">
  ...
  return <div key={id} className="swg-row">
          ...
          {open && <div className="swg-detail el-in">
              {!tried ? <button className="btn-soft" onClick={() => onTry(id)} style={{ alignSelf: "flex-start" }}>▶ Try it out</button> : available ? <><div className="swg-code-lbl">{tr({ uz: "Javob", ru: "Ответ" })} · <span style={{ color: T.success }}>{e.m === "POST" ? "201" : "200"}</span></div><pre className="json">{e.resp}</pre></> : <><div className="swg-code-lbl">{tr({ uz: "Javob", ru: "Ответ" })} · <span style={{ color: T.danger }}>404</span></div><pre className="json">{'{\n  "statusCode": 404,\n  "message": "Cannot ' + e.m + " " + e.path + '"\n}'}</pre></>}
            </div>}
        </div>;
})}
  </div>;
var PickLines = ({ fileName, scaffoldTop, scaffoldBottom, candidates, agent, instruction, onComplete, completedInit }) => {
  ...
  const [picked, setPicked] = useState(() => completedInit ? new Set(correct.map((c) => c.id)) : /* @__PURE__ */ new Set());
    ...
    <div className="split">
      <Col>
        ...
        <CodeFile name={fileName} minH={120}>
          {scaffoldTop}{"\n"}
          {pickedCorrect.length === 0 ? <span className="line-empty">{tr({ uz: "  // qatorlarni o'ng tomondan tanlang →", ru: "  // выберите строки справа →" })}</span> : pickedCorrect.map((c, i) => <React.Fragment key={c.id}>{i > 0 ? "\n" : ""}{"  "}{c.node}</React.Fragment>)}
          {"\n"}{scaffoldBottom}
        </CodeFile>
...
var useBoard = () => useContext(BoardCtx) || { board: {}, fill: () => {
}, plus: false, plusOn: () => {
} };
var OpeningBoard = () => {
  const { board, plus } = useBoard();
  const done = BOARD_SLOTS.filter((s) => board[s.k]).length;
  return <div className="oc-board fade-up" aria-label={tr({ uz: "Ochilish taxtasi", ru: "Доска открытия" })}>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Reja · 5 qadam", ru: "План · 5 шагов" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive label={tr({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(storedAnswer ? new Set(FIELDS.map((f) => f.id)) : /* @__PURE__ */ new Set());
  const [sc, setSc] = useState(0);
  ...
  return <Stage eyebrow={tr({ uz: "Reja · mashina shakli", ru: "План · форма машины" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: `Ustunlarni belgilang (${seen.size}/${FIELDS.length})`, ru: `Отметьте столбцы (${seen.size}/${FIELDS.length})` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Har ustunni bosib belgilang — mashina haqida nimani saqlash kerakligini o'ylang.", ru: "💡 Отмечайте каждый столбец нажатием — подумайте, что нужно хранить о машине." })}</p>}
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "1-qadam · Entity", ru: "Шаг 1 · Entity" })} screen={screen} scrollSignal={done ? 1 : 0} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Faylni yig'ing", ru: "Соберите файл" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(storedAnswer ? /* @__PURE__ */ new Set(["ok", "bad"]) : /* @__PURE__ */ new Set());
  const [sc, setSc] = useState(0);
  const { fill } = useBoard();
  ...
  return <Stage eyebrow={tr({ uz: "2-qadam · DTO", ru: "Шаг 2 · DTO" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "To'g'ri va xato so'rovni sinang", ru: "Попробуйте верный и неверный запрос" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Ikkala tugmani ham bosing: avval to'g'ri so'rov, keyin xato — farqni o'zingiz ko'rasiz.", ru: "💡 Нажмите обе кнопки: сначала верный запрос, потом ошибочный — разницу увидите сами." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState(storedAnswer ? /* @__PURE__ */ new Set(["full", "partial"]) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={tr({ uz: "2-qadam · Update DTO", ru: "Шаг 2 · Update DTO" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: `Ikkala variantni sinang (${seen.size}/2)`, ru: `Попробуйте оба варианта (${seen.size}/2)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Ikkala variantni sinang: to'liq so'rov va faqat bitta maydonli so'rov.", ru: "💡 Попробуйте оба варианта: полный запрос и запрос с одним полем." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState(storedAnswer ? /* @__PURE__ */ new Set(["manual", "base"]) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={tr({ uz: "3-qadam · Service", ru: "Шаг 3 · Service" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: `Ikkala yo'lni ko'ring (${seen.size}/2)`, ru: `Посмотрите оба пути (${seen.size}/2)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            ...
            {cur === "manual" && <CodeFile name={tr({ uz: "src/api/car/car.service.ts  ·  ~40 qator", ru: "src/api/car/car.service.ts  ·  ~40 строк" })} minH={210}>
                <At>@Injectable</At>{"()"}{"\n"}
            ...
            {cur === "base" && <CodeFile name={tr({ uz: "src/api/car/car.service.ts  ·  4 qator", ru: "src/api/car/car.service.ts  ·  4 строки" })} minH={210}>
                <At>@Injectable</At>{"()"}{"\n"}
                <Jx>export class</Jx>{" CarService"}{"\n"}
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Ikkala yo'lni oching: qo'lda yozilgani va tayyor asosdan olingani.", ru: "💡 Откройте оба пути: написанный вручную и взятый из готовой основы." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "4-qadam · Controller", ru: "Шаг 4 · Controller" })} screen={screen} scrollSignal={done ? 1 : 0} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Eshiklarni yig'ing", ru: "Соберите двери" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "5-qadam · Module", ru: "Шаг 5 · Module" })} screen={screen} scrollSignal={done ? 1 : 0} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Module'ni yig'ing", ru: "Соберите Module" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Ochilish kuni · AppModule", ru: "День открытия · AppModule" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : phase === 0 ? tr({ uz: "Mijozni kiriting", ru: "Впустите посетителя" }) : phase === 1 ? tr({ uz: "Taxtachani osing", ru: "Повесьте табличку" }) : tr({ uz: "Mijozni qayta kiriting", ru: "Впустите посетителя снова" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
  ...
  >
                  {wired ? <span className="oc-plate-row on oc-plate-snap">{tr({ uz: "🚗 Mashinalar", ru: "🚗 Машины" })}</span> : <span className="oc-plate-empty">{canHang ? holding ? tr({ uz: "↓ shu yerga qo'ying", ru: "↓ отпустите здесь" }) : tr({ uz: "bo'sh joy — taxtacha yo'q", ru: "пустое место — таблички нет" }) : tr({ uz: "bo'sh joy — taxtacha yo'q", ru: "пустое место — таблички нет" })}</span>}
                </div>
              </div>
  ...
  >{tr({ uz: "🚗 Mashinalar — taxtachani sudrang", ru: "🚗 Машины — перетащите табличку" })}</button>}
          ...
          <Col>
            <CodeFile name="src/api/app.module.ts" minH={120}>
              ...
              {wired ? <span className="el-in oc-code-new" style={{ color: T.success }}>{tr({ uz: "    CarModule,   // ← qo'shildi ✓", ru: "    CarModule,   // ← добавлен ✓" })}</span> : <Cm>{tr({ uz: "    // CarModule, ← yo'q!", ru: "    // CarModule, ← нет!" })}</Cm>}{"\n"}
              {"  ],"}{"\n"}
              {"})"}{"\n"}
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 O'ylang: mijoz bo'limni topishi uchun undan oldin nima bajarilgan bo'lishi kerak?", ru: "💡 Подумайте: что должно быть сделано раньше, чтобы посетитель нашёл отдел?" })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen14 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr({ uz: "Mashq · 5-savol", ru: "Практика · вопрос 5" })}
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Markaziy · so'rov yo'li", ru: "Главное · путь запроса" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "So'rovni oxirigacha kuzating", ru: "Проследите запрос до конца" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
    ...
    return <div key={i} className="flow-stop" style={{ opacity: lit ? 1 : 0.35 }}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 So'rov yo'lini oxirigacha bosib boring — har bosishda u bitta bekatga siljiydi.", ru: "💡 Ведите запрос до конца нажатиями — с каждым он сдвигается на одну остановку." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
var Screen16 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Agent · yaratish", ru: "Агент · создание" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Playbookni yuboring", ru: "Отправьте playbook" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
    ...
    return <div key={i} className={`gen-file ${ready ? "ready" : ""} el-in`}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Yuborish tugmasini bosing — agent fayllarni ketma-ket yozadi, biroz kuting.", ru: "💡 Нажмите кнопку отправки — агент напишет файлы один за другим, немного подождите." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen17 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [tried, setTried] = useState(storedAnswer ? /* @__PURE__ */ new Set(["POST/car"]) : /* @__PURE__ */ new Set());
  const [sc, setSc] = useState(0);
  ...
  return <Stage eyebrow={tr({ uz: "Debugging · nazoratchi", ru: "Отладка · контролёр" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done && !_resc} label={done ? tr({ uz: "Oxirgi qadam →", ru: "Последний шаг →" }) : _resc ? tr({ uz: "Qatorlarni birga ko'ramiz →", ru: "Разберём строки вместе →" }) : found ? tr({ uz: "Endi tuzating", ru: "Теперь исправьте" }) : tr({ uz: "Xatoni toping", ru: "Найдите ошибку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Kod qatorlarini birma-bir oching: qaysi biri bu agentga tegishli emas?", ru: "💡 Открывайте строки кода одну за другой: какая из них не относится к этому агенту?" })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Bu qatorlarni keyinroq birga ko'rib chiqamiz — hozir davom etsangiz bo'ladi.", ru: "Эти строки разберём вместе позже — сейчас можно продолжить." })}</p>}
      </div>
    </Stage>;
};
var ACHIEVEMENTS = {
...
function ScreenLivePractice({ title, task, checklist, screen, storedAnswer, onAnswer, onNext, onPrev, live }) {
  ...
  return <Stage eyebrow={tr({ uz: "Amaliyot · VS Code", ru: "Практика · VS Code" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Avval bajaring", ru: "Сначала выполните" })} onClick={onNext} /></>}>
...
var Screen19 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  const RECAP = [
    tr({ uz: "Har resurs = 5 qadam: Entity → DTO → Service → Controller → Module", ru: "Каждый ресурс = 5 шагов: Entity → DTO → Service → Controller → Module" }),
    tr({ uz: "Entity — jadval shakli; id va vaqtlar BaseEntity'dan tekin", ru: "Entity — форма таблицы; id и время бесплатно из BaseEntity" }),
    tr({ uz: "DTO — kelgan ma'lumot qoidalari (bo'sh brand → 400)", ru: "DTO — правила входящих данных (пустой brand → 400)" }),
    tr({ uz: "Service — BaseService'dan meros: CRUD tekin keladi", ru: "Service — наследует BaseService: CRUD приходит бесплатно" }),
    tr({ uz: "CarModule'ni AppModule'ga ulash (aks holda 404)", ru: "Подключить CarModule к AppModule (иначе 404)" }),
    tr({ uz: "Agent yozgan kodni tekshirish: har qator o'z qatlamida (debug)", ru: "Проверять код агента: каждая строка в своём слое (отладка)" })
  ];
  const HOMEWORK = [
    { b: tr({ uz: "O'z resursingiz", ru: "Свой ресурс" }), t: tr({ uz: "— masalan Order (buyurtma) yoki Client (mijoz): 5 qadamni qog'ozga yozing", ru: "— например Order (заказ) или Client (клиент): распишите 5 шагов на бумаге" }) },
    { b: "Playbook", t: tr({ uz: "— har qadam uchun agentga beradigan promptni tayyorlang", ru: "— подготовьте промпт для агента на каждый шаг" }) },
    { b: tr({ uz: "Tekshiruv", ru: "Проверка" }), t: tr({ uz: "— AppModule'ga ulanganini va Swagger'da ko'rinishini tasavvur qiling", ru: "— представьте, как он подключён к AppModule и виден в Swagger" }) }
  ];
  ...
  return <Stage eyebrow={tr({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: "Mashinalar bo'limi ochildi", ru: "Отдел машин открыт" })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>yangi resurs qo'sha olasiz</span>.</>, ru: <>Теперь вы <span className="italic" style={{ color: T.accent }}>умеете добавлять новый ресурс</span>.</> })}</h2>{
...
function NestArchResourceLesson({ lang: langProp, onFinished }) {
