...
//  Manba:  src/4a-Modull/NestArchResourceLesson.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/4a-Modull/NestArchResourceLesson.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/4a-Modull/NestArchResourceLesson.jsx
...
var BoardCtx = createContext2(null);
...
var LESSON_META = { lessonId: "nest-arch-resource-4a-02-v18", lessonTitle: { uz: "Birinchi resursni qo'lda qo'shish — mashinalar", ru: "Добавляем первый ресурс вручную — машины" } };
var HW_TOKENS = [
...
var Q_LABELS = { 4: "1 — Entity", 7: "2 — DTO", 9: "3 — BaseService", 11: "4 — Controller", 14: { uz: "5 — Ulash", ru: "5 — Подключение" }, 17: "6 — Debug" };
var INLINE_KEYS = { s4: 2, s7: 3, s9: 1, s11: 0, s14: 1, s17: 0, practice: -1 };
var RECAPS = {
  4: {
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <p className="mstats-verdict-t">{tr2({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верно — класс не понял эту тему. Перед продолжением рекомендуем короткое повторение.</> })}</p>}
            {level === "maybe" && <p className="mstats-verdict-t">{tr2({ uz: <>🟡 <b>{pct}%</b> to'g'ri — yomon emas. Xohlasangiz, davom etishdan oldin qisqa takrorlab oling.</>, ru: <>🟡 <b>{pct}%</b> верно — неплохо. Если хотите, коротко повторите тему перед продолжением.</> })}</p>}
            ...
            {onOpenRecap && <button className="rc-open" onClick={onOpenRecap}>{tr2({ uz: "📖 Qayta tushuntirish", ru: "📖 Повторное объяснение" })}</button>}
          </div>;
  })()}
...
var CodeFile = ({ name, children, minH, maxH }) => <div className="editor">
    <div className="editor-bar"><span className="bb-dots"><i /><i /><i /></span><span className="editor-tab">{name}</span></div>
    <div className="editor-body" style={{ minHeight: minH, maxHeight: maxH, overflowY: maxH ? "auto" : void 0 }}><pre className="editor-code">{children}</pre></div>
  </div>;
var AgentCard = ({ children }) => {
...
var CarSwagger = ({ available, openId, onToggle, triedIds, onTry }) => <div className="swg">
    <div className="swg-top"><span className="swg-dot" style={{ background: available ? "#49cc90" : "#febc2e" }} /> {tr2({ uz: "Avtosalon API", ru: "API автосалона" })} <span className="swg-ver">/api/v1</span></div>
    {CAR_EPS.map((e) => {
  const id = e.m + e.path;
  ...
  return <div key={id} className="swg-row">
          ...
          {open && <div className="swg-detail el-in">
              {!tried ? <button className="btn-soft" onClick={() => onTry(id)} style={{ alignSelf: "flex-start" }}>▶ Try it out</button> : available ? <><div className="swg-code-lbl">{tr2({ uz: "Javob", ru: "Ответ" })} · <span style={{ color: T.success }}>{e.m === "POST" ? "201" : "200"}</span></div><pre className="json">{e.resp}</pre></> : <><div className="swg-code-lbl">{tr2({ uz: "Javob", ru: "Ответ" })} · <span style={{ color: T.danger }}>404</span></div><pre className="json">{'{\n  "statusCode": 404,\n  "message": "Cannot ' + e.m + " " + e.path + '"\n}'}</pre></>}
            </div>}
        </div>;
})}
  </div>;
var PickLines = ({ fileName, scaffoldTop, scaffoldBottom, candidates, agent, instruction, onComplete, completedInit, doneText }) => {
  ...
  return <Zoomable>
    <div className="split">
      ...
      <Col>
        <p className="flow-label">{instruction || tr2({ uz: "Shu faylga tegishli qatorlarni tanlang", ru: "Выберите строки, относящиеся к этому файлу" })}</p>
...
var useBoard = () => useContext2(BoardCtx) || { board: {}, fill: () => {
}, plus: false, plusOn: () => {
}, agent: null, setAgent: () => {
} };
var OpeningBoard = () => {
  ...
  return <div className="oc-board fade-up" aria-label={tr2({ uz: "Ochilish taxtasi", ru: "Доска открытия" })}>
      <div className="oc-board-h">
        <span className="oc-board-t">{tr2({ uz: "🚗 MASHINALAR BO'LIMI — ochilishga tayyorgarlik", ru: "🚗 ОТДЕЛ МАШИН — подготовка к открытию" })}</span>
    ...
    /* summary sarlavha qatorida (absolyut), ochilgan matn taxta ichida qatorlar ostida */
      ...
      {agent && <details className="oc-agent" key={agent}>
        <summary className="oc-agent-btn">{tr2({ uz: "💬 Agentni shunday yo'naltiring", ru: "💬 Направьте агента так" })}</summary>
        <p className="agent-msg oc-agent-msg">{agent}</p>
      </details>}
      <div className="oc-strip">
        {BOARD_SLOTS.map((s, i) => {
    const on = !!board[s.k];
    return <React3.Fragment key={s.k}>
              ...
              <div className={`oc-slot ${on ? "on oc-fly" : ""} ${s.plate ? "plate" : ""}`} title={s.file}>
                ...
                <span className="oc-nm">{tr2(s.name)}{s.k === "dto" && plus && <b className="oc-plus"> ⁺</b>}</span>
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const OPTS = [
    { id: "a", label: tr2({ uz: "admin faylini o'zgartirib, ichiga mashinani tiqamiz", ru: "поменяем файл admin и запихнём машину внутрь" }) },
    { id: "b", label: tr2({ uz: "Mashinalar uchun yangi resurs (Car) quramiz", ru: "построим для машин новый ресурс (Car)" }) },
    { id: "c", label: tr2({ uz: "Imkonsiz — skeletga yangi narsa qo'shib bo'lmaydi", ru: "Невозможно — к скелету ничего не добавишь" }) }
  ];
  const ACK = {
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} scrollSignal={sc} navContent={<NavNext optionalLive disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>Admin tizimi tayyor — lekin avtosalon <span className="italic" style={{ color: T.accent }}>mashinalarini</span> qayerda saqlaydi?</>, ru: <>Админка готова — но где автосалон хранит свои <span className="italic" style={{ color: T.accent }}>машины</span>?</> })}</h1>
        ...
        <Split>
          <Col>
            <p className="flow-label fade-up delay-1">{tr2({ uz: "Hozirgi holat — /car hali yo'q", ru: "Текущее состояние — /car пока нет" })}</p>
            <div className="swg fade-up delay-1">
              <div className="swg-top"><span className="swg-dot" /> {tr2({ uz: "Avtosalon API", ru: "API автосалона" })} <span className="swg-ver">/api/v1</span></div>
              <div className="swg-row"><div className="swg-head" style={{ cursor: "default" }}><span className="swg-m" style={{ background: M_COLOR.POST }}>POST</span><span className="swg-path">/admin/signin</span><span className="swg-sum">{tr2({ uz: "ishlaydi ✓", ru: "работает ✓" })}</span></div></div>
              <div className="swg-row"><div className="swg-head" style={{ cursor: "default" }}><span className="swg-m" style={{ background: M_COLOR.GET }}>GET</span><span className="swg-path">/admin</span><span className="swg-sum">{tr2({ uz: "ishlaydi ✓", ru: "работает ✓" })}</span></div></div>
              <div className="swg-row">
                <button className="swg-head" onClick={poke}><span className="swg-m" style={{ background: M_COLOR.POST }}>POST</span><span className="swg-path">/car</span><span className="swg-sum">{tr2({ uz: "sinab ko'ring", ru: "попробуйте" })}</span><span className="swg-chev">▸</span></button>
                {tried && <div className="swg-detail el-in"><div className="swg-code-lbl">{tr2({ uz: "Javob", ru: "Ответ" })} · <span style={{ color: T.danger }}>404</span></div><pre className="json">{'{\n  "statusCode": 404,\n  "message": "Cannot POST /car"\n}'}</pre></div>}
              </div>
            </div>
            {tried && <p className="small fade-step" style={{ color: T.accent, fontStyle: "italic", margin: 0 }}>{tr2({ uz: "404 — bunday eshik yo'q. Chunki mashinalar jadvalini hali hech kim qo'shmagan.", ru: "404 — такой двери нет. Потому что таблицу машин ещё никто не добавил." })}</p>}
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Endi /car ni qanday paydo qilamiz?", ru: "Как теперь создать /car?" })}</p>
    ...
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null || !tried} style={{ opacity: !tried ? 0.55 : 1 }} onClick={() => pick(o.id)}><span className="radio">{on && <span className="radio-dot" />}</span><span>{o.label}</span></button>;
            ...
            {!tried && <p className="small" style={{ color: T.ink3, fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Avval POST /car ni bosib ko'ring ←", ru: "Сначала нажмите POST /car ←" })}</p>}
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  const STEPS = [
    { text: tr2({ uz: "Entity — jadval shakli", ru: "Entity — форма таблицы" }), tag: "car.entity.ts" },
    { text: tr2({ uz: "DTO — anketa + qoidalar", ru: "DTO — анкета + правила" }), tag: "create/update dto" },
    { text: tr2({ uz: "Service — BaseService'dan meros", ru: "Service — наследует BaseService" }), tag: "car.service.ts" },
    { text: tr2({ uz: "Controller — eshiklar", ru: "Controller — двери" }), tag: "car.controller.ts" },
    { text: tr2({ uz: "Module — ulaydi + AppModule", ru: "Module — соединяет + AppModule" }), tag: "car.module.ts" }
  ];
  return <Stage eyebrow={tr2({ uz: "Reja · 5 qadam", ru: "План · 5 шагов" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive label={tr2({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Mashinalar jadvalini qo'shish — <span className="italic" style={{ color: T.accent }}>qayerdan boshlanadi</span>?</>, ru: <>Добавить таблицу машин — <span className="italic" style={{ color: T.accent }}>с чего начать</span>?</> })}</h2></div>
        ...
        <div className="frame-success fade-up delay-3"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Misol resurs — <span className="mono">Car</span> (mashina). Sodda: parol ham, kirish-chiqish ham yo'q — shu sababli <b>BaseService</b>ning kuchi to'liq ko'rinadi.</>, ru: <>Наш пример — ресурс <span className="mono">Car</span> (машина). Простой: ни пароля, ни входа-выхода — поэтому сила <b>BaseService</b> видна целиком.</> })}</p></div>
      </div>
    </Stage>;
};
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const FIELDS = [
    { id: "brand", label: "brand", d: tr2({ uz: "marka (matn, majburiy)", ru: "марка (текст, обязательно)" }) },
    { id: "model", label: "model", d: tr2({ uz: "model nomi (matn, majburiy)", ru: "название модели (текст, обязательно)" }) },
    { id: "price", label: "price", d: tr2({ uz: "narx (raqam)", ru: "цена (число)" }) },
    { id: "is_available", label: "is_available", d: tr2({ uz: "sotuvda bormi? (true/false)", ru: "есть в продаже? (true/false)" }) }
  ];
  const [seen, setSeen] = useState3(storedAnswer ? new Set(FIELDS.map((f) => f.id)) : /* @__PURE__ */ new Set());
  const [sc, setSc] = useState3(0);
  ...
  return <Stage eyebrow={tr2({ uz: "Reja · mashina shakli", ru: "План · форма машины" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Ustunlarni belgilang (${seen.size}/${FIELDS.length})`, ru: `Отметьте столбцы (${seen.size}/${FIELDS.length})` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Kod yozishdan oldin: bitta mashina haqida <span className="italic" style={{ color: T.accent }}>nimani saqlaymiz</span>?</>, ru: <>Прежде чем писать код: <span className="italic" style={{ color: T.accent }}>что мы храним</span> об одной машине?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "siz qo'shadigan ustunlar", ru: "столбцы, которые добавляете вы" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              {FIELDS.map((f) => <button key={f.id} className={`vcard ${!seen.has(f.id) ? "tap-hint" : ""}`} onClick={() => tap(f.id)} style={{ boxShadow: seen.has(f.id) ? `inset 0 0 0 1.5px ${T.success}, 0 5px 14px -6px rgba(${T.shadowBase},0.16)` : void 0 }}>
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "tekin keladi (BaseEntity)", ru: "приходят бесплатно (BaseEntity)" })}</p>
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Reja tayyor: 4 ta o'z ustunimiz + 3 ta tekin. Endi buni 1-faylga — Entity'ga yozamiz.", ru: "План готов: 4 наших столбца + 3 бесплатных. Теперь запишем это в первый файл — Entity." })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Har ustunni bosib belgilang — mashina haqida nimani saqlash kerakligini o'ylang.", ru: "💡 Отмечайте каждый столбец нажатием — подумайте, что нужно хранить о машине." })}</p>}
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const candidates = [
    ...
    { id: "id", correct: false, label: "id: string;", why: tr2({ uz: "id kerakmas — u BaseEntity'dan tekin keladi. Qayta yozsangiz, takror bo'ladi.", ru: "id не нужен — он бесплатно приходит из BaseEntity. Напишете снова — будет дубль." }) },
    { id: "username", correct: false, label: "username: string;", why: tr2({ uz: "Bu admin'ga tegishli. Mashinada username yo'q — bu qator boshqa resursdan.", ru: "Это из admin. У машины нет username — эта строка из другого ресурса." }) }
  ];
  return <Stage eyebrow={tr2({ uz: "1-qadam · Entity", ru: "Шаг 1 · Entity" })} screen={screen} scrollSignal={done ? 1 : 0} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Faylni yig'ing", ru: "Соберите файл" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Mashina haqida nimani saqlaymiz — jadval <span className="italic" style={{ color: T.accent }}>qanday ko'rinadi</span>?</>, ru: <>Что храним о машине — <span className="italic" style={{ color: T.accent }}>как выглядит</span> таблица?</> })}</h2></div>
    ...
    agent={tr2({ uz: "car.entity.ts yoz: BaseEntity'dan meros olsin; brand, model (matn), price (raqam), is_available (default true) ustunlari bo'lsin.", ru: "Напиши car.entity.ts: пусть наследуется от BaseEntity; со столбцами brand, model (текст), price (число), is_available (default true)." })}
    instruction={tr2({ uz: "car.entity.ts ga qaysi qatorlar tegishli?", ru: "Какие строки относятся к car.entity.ts?" })}
    ...
    doneText={tr2({ uz: <>1-fayl tayyor! Lekin <span className="mono">/car</span> hali yo'q — bitta fayl yetmaydi. Keyingi qadam: ma'lumot qoidalari (DTO).</>, ru: <>Первый файл готов! Но <span className="mono">/car</span> ещё нет — одного файла мало. Следующий шаг: правила данных (DTO).</> })}
  />
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Har qatorni o'qing: u mashinaning ustunimi? Ustun bo'lmasa — bu fayl uniki emas.", ru: "💡 Прочитайте каждую строку: это столбец машины? Если нет — строка не из этого файла." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen4 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <><span className="mono" style={{ color: T.accent }}>Entity</span> <span className="italic" style={{ color: T.accent }}>nimani</span> belgilaydi?</>, ru: <><span className="italic" style={{ color: T.accent }}>Что</span> определяет <span className="mono" style={{ color: T.accent }}>Entity</span>?</> })}</h2></>}
  options={[{ uz: "So'rovni butunlay qabul qilib olishni", ru: "Полностью принимать запросы" }, { uz: "Kelgan ma'lumot qoidalarini tekshirishni", ru: "Проверять правила входящих данных" }, { uz: "Bazadagi jadval qanday ko'rinishini", ru: "Как выглядит таблица в базе" }, { uz: "Loyihani ishga tushirish tartibini", ru: "Порядок запуска проекта" }]}
  correctIdx={2}
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [res, setRes] = useState3(storedAnswer ? "bad" : null);
  const [seen, setSeen] = useState3(storedAnswer ? /* @__PURE__ */ new Set(["ok", "bad"]) : /* @__PURE__ */ new Set());
  const [sc, setSc] = useState3(0);
  const { fill } = useBoard();
  ...
  return <Stage eyebrow={tr2({ uz: "2-qadam · DTO", ru: "Шаг 2 · DTO" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "To'g'ri va xato so'rovni sinang", ru: "Попробуйте верный и неверный запрос" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Noto'g'ri mashina ma'lumoti kelsa — <span className="italic" style={{ color: T.accent }}>kim to'xtatadi</span>?</>, ru: <>Пришли неверные данные о машине — <span className="italic" style={{ color: T.accent }}>кто их остановит</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button className="btn-soft" onClick={() => send("ok")}>{tr2({ uz: "✅ To'g'ri:", ru: "✅ Верно:" })} {`{ brand: 'Chevrolet', price: 15000 }`}</button>
              <button className="btn-soft" onClick={() => send("bad")}>{tr2({ uz: "❌ Xato:", ru: "❌ Ошибка:" })} {`{ brand: '', price: 'arzon' }`}</button>
            </div>
            <AgentCard>{tr2({ uz: "create-car.dto.ts yoz: brand — majburiy matn (@IsString, @IsNotEmpty), price — raqam (@IsNumber), is_available — ixtiyoriy.", ru: "Напиши create-car.dto.ts: brand — обязательный текст (@IsString, @IsNotEmpty), price — число (@IsNumber), is_available — необязательный." })}</AgentCard>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Nazoratchi javobi", ru: "Ответ контролёра" })}</p>
            {res === "ok" && <div className="frame-success fade-step"><p className="note-h" style={{ color: T.success }}>{tr2({ uz: "✓ 201 — qabul qilindi", ru: "✓ 201 — принято" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "brand to'ldirilgan, price raqam — anketa to'g'ri, ichkariga o'tdi.", ru: "brand заполнен, price — число: анкета верная, запрос прошёл внутрь." })}</p></div>}
            {res === "bad" && <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.danger }}>{tr2({ uz: "✗ 400 — rad etildi", ru: "✗ 400 — отклонено" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: `brand bo'sh, price esa raqam emas ("arzon") — qoidalar buzilgan. So'rov service'gacha ham bormadi.`, ru: 'brand пустой, а price — не число ("arzon") — правила нарушены. Запрос даже не дошёл до service.' })}</p></div>}
            {res === null && <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "So'rov yuboring ←", ru: "Отправьте запрос ←" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "DTO + nazoratchi ilovani ifloslangan ma'lumotdan himoya qiladi. Yomon ma'lumot bazaga umuman yetib bormaydi.", ru: "DTO + контролёр защищают приложение от грязных данных. Плохие данные до базы вообще не доходят." })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Ikkala tugmani ham bosing: avval to'g'ri so'rov, keyin xato — farqni o'zingiz ko'rasiz.", ru: "💡 Нажмите обе кнопки: сначала верный запрос, потом ошибочный — разницу увидите сами." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState3(storedAnswer ? /* @__PURE__ */ new Set(["full", "partial"]) : /* @__PURE__ */ new Set());
  const [cur, setCur] = useState3(storedAnswer ? "partial" : null);
  const [sc, setSc] = useState3(0);
  ...
  useEffect4(() => {
    if (storedAnswer) plusOn();
  }, []);
  const doneMsg = tr2({ uz: "Bitta qator — tahrirlash anketasi tayyor. Bir xil kodni ikki marta yozmaslik — tajribali dasturchilarning odati (DRY).", ru: "Одна строка — и анкета редактирования готова. Не писать один и тот же код дважды — привычка опытных разработчиков (DRY)." });
  useEffect4(() => {
    if (done) {
      plusOn();
      if (storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  ...
  return <Stage eyebrow={tr2({ uz: "2-qadam · Update DTO", ru: "Шаг 2 · Update DTO" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Ikkala variantni sinang (${seen.size}/2)`, ru: `Попробуйте оба варианта (${seen.size}/2)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Mijoz faqat narxni o'zgartirmoqchi — unga <span className="italic" style={{ color: T.accent }}>qaysi anketani</span> berasiz?</>, ru: <>Клиент хочет поменять только цену — <span className="italic" style={{ color: T.accent }}>какую анкету</span> вы ему дадите?</> })}</h2></div>
          ...
          <Col>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <button className={`pick-row ${cur === "full" ? "picked" : ""} ${!seen.has("full") ? "tap-hint" : ""}`} onClick={() => choose("full")}>
                <span style={{ flex: 1 }}>{tr2({ uz: "a) To'liq anketa — CreateCarDto", ru: "а) Полная анкета — CreateCarDto" })}</span>
              ...
              <button className={`pick-row ${cur === "partial" ? "picked" : ""} ${!seen.has("partial") ? "tap-hint" : ""}`} onClick={() => choose("partial")}>
                <span style={{ flex: 1 }}>{tr2({ uz: "b) PartialType(CreateCarDto) — nusxa", ru: "б) PartialType(CreateCarDto) — копия" })}</span>
                <span className="pick-plus">{seen.has("partial") ? "✓" : "?"}</span>
              </button>
            </div>
            {cur === "full" && <CodeFile name="src/api/car/dto/update-car.dto.ts" minH={110}>
                <Cm>{tr2({ uz: "// a) to'liq anketa — hamma katakcha MAJBURIY", ru: "// а) полная анкета — каждое поле ОБЯЗАТЕЛЬНО" })}</Cm>{"\n"}
            ...
            {cur === "partial" && <CodeFile name="src/api/car/dto/update-car.dto.ts" minH={110}>
                <Cm>{tr2({ uz: "// b) bir xil anketa — lekin har katakcha IXTIYORIY", ru: "// б) та же анкета — но каждое поле НЕОБЯЗАТЕЛЬНО" })}</Cm>{"\n"}
                <Jx>export class</Jx>{" UpdateCarDto "}{"\n"}
                {"  "}<Jx>extends</Jx>{" "}<At>PartialType</At>{"(CreateCarDto) {}"}
              </CodeFile>}
            {cur === null && <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Variantni tanlang ↑", ru: "Выберите вариант ↑" })}</p></div>}
            <AgentCard>{tr2({ uz: "update-car.dto.ts yoz: PartialType(CreateCarDto) — barcha maydon ixtiyoriy bo'lsin.", ru: "Напиши update-car.dto.ts: PartialType(CreateCarDto) — пусть все поля будут необязательными." })}</AgentCard>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "natija", ru: "результат" })} · PATCH /car/:id  {`{ price: 12000 }`}</p>
            {cur === "full" && <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.danger }}>{tr2({ uz: "✗ 400 — rad etildi", ru: "✗ 400 — отклонено" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>To'liq anketada <span className="mono">brand</span> va <span className="mono">model</span> ham majburiy. Mijoz faqat narxni yubordi — anketa chala, so'rov o'tmadi.</>, ru: <>В полной анкете <span className="mono">brand</span> и <span className="mono">model</span> тоже обязательны. Клиент отправил только цену — анкета неполная, запрос не прошёл.</> })}</p></div>}
            {cur === "partial" && <div className="frame-success fade-step"><p className="note-h" style={{ color: T.success }}>{tr2({ uz: "✓ 200 — o'zgartirildi", ru: "✓ 200 — изменено" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>PartialType create anketasini olib, har katakchani <b>ixtiyoriy</b> qildi. Faqat <span className="mono">price</span> yetarli.</>, ru: <>PartialType взял анкету create и сделал каждое поле <b>необязательным</b>. Достаточно одного <span className="mono">price</span>.</> })}</p>{done && <p className="body" style={{ margin: "8px 0 0", color: T.ink }}>{doneMsg}</p>}</div>}
            {!(done && cur === "full") && <div className="frame" style={{ padding: 10 }}>
              <div className="ent-row siz">brand, model, price <span>{tr2({ uz: "← create'da majburiy", ru: "← обязательны в create" })}</span></div>
              {seen.has("partial") && <><div className="ent-row free el-in">{tr2({ uz: "brand? (ixtiyoriy)", ru: "brand? (необязательно)" })} <span>← PartialType</span></div><div className="ent-row free el-in">{tr2({ uz: "price? (ixtiyoriy)", ru: "price? (необязательно)" })} <span>← PartialType</span></div></>}
            </div>}
            {
    /* §34: «b» ochiq bo'lsa xulosa natija qutisi ichida; «a» oxirgi bosilganda jadval o'rnida alohida quti */
        ...
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Ikkala variantni sinang: to'liq so'rov va faqat bitta maydonli so'rov.", ru: "💡 Попробуйте оба варианта: полный запрос и запрос с одним полем." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen7 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Bo'sh <span className="mono">brand</span> yuborilsa <span className="italic" style={{ color: T.accent }}>nima</span> bo'ladi?</>, ru: <><span className="italic" style={{ color: T.accent }}>Что</span> будет, если отправить пустой <span className="mono">brand</span>?</> })}</h2></>}
  options={[{ uz: "201 — mashina baribir saqlanadi", ru: "201 — машина всё равно сохранится" }, { uz: "Server butunlay ishdan chiqadi", ru: "Сервер полностью упадёт" }, { uz: "brand avtomatik ravishda qo'yiladi", ru: "brand подставится автоматически" }, { uz: "400 qaytadi — bazaga bormaydi", ru: "Вернётся 400 — до базы не дойдёт" }]}
  correctIdx={3}
...
/>;
var FREE_METHODS = ["create", "findAll", "findOneById", "update", "remove"];
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState3(storedAnswer ? /* @__PURE__ */ new Set(["manual", "base"]) : /* @__PURE__ */ new Set());
  const [cur, setCur] = useState3(storedAnswer ? "base" : null);
  const [sc, setSc] = useState3(0);
  const { fill } = useBoard();
  const done = seen.size >= 2;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, seen.size);
  ...
  useEffect4(() => {
    if (storedAnswer) fill("service");
  }, []);
  const doneMsg = tr2({ uz: "5 ta CRUD metod — bittasini ham yozmadingiz, BaseService'dan keldi. O'ziga xos mantiq kerak bo'lsagina qo'shasiz (mashinada kerak emas).", ru: "5 CRUD-методов — вы не написали ни одного, все пришли из BaseService. Свою логику добавляете, только когда она нужна (машинам — не нужна)." });
  useEffect4(() => {
    if (done) {
      fill("service");
      if (storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  ...
  return <Stage eyebrow={tr2({ uz: "3-qadam · Service", ru: "Шаг 3 · Service" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Ikkala yo'lni ko'ring (${seen.size}/2)`, ru: `Посмотрите оба пути (${seen.size}/2)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Yangi oshpaz keldi — unga <span className="italic" style={{ color: T.accent }}>nima berasiz</span>?</>, ru: <>Пришёл новый повар — <span className="italic" style={{ color: T.accent }}>что вы ему дадите</span>?</> })}</h2></div>
          ...
          <Col>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <button className={`pick-row ${cur === "manual" ? "picked" : ""} ${!seen.has("manual") ? "tap-hint" : ""}`} onClick={() => choose("manual")}>
                <span style={{ flex: 1 }}>{tr2({ uz: "a) Bo'sh daftar — 5 metodni qo'lda yozadi", ru: "а) Пустая тетрадь — пишет 5 методов вручную" })}</span>
              ...
              <button className={`pick-row ${cur === "base" ? "picked" : ""} ${!seen.has("base") ? "tap-hint" : ""}`} onClick={() => choose("base")}>
                <span style={{ flex: 1 }}>{tr2({ uz: "b) Tayyor retsept kitobi — extends BaseService", ru: "б) Готовая книга рецептов — extends BaseService" })}</span>
                <span className="pick-plus">{seen.has("base") ? "✓" : "?"}</span>
              </button>
            </div>
            {cur === "manual" && <CodeFile name={tr2({ uz: "src/api/car/car.service.ts  ·  ~40 qator", ru: "src/api/car/car.service.ts  ·  ~40 строк" })} minH={190} maxH={190}>
                <At>@Injectable</At>{"()"}{"\n"}
                ...
                {"  update(id, dto) { ... }"}{"\n"}
                {"  remove(id) { ... }"}{"\n"}
                <Cm>{tr2({ uz: "  // ...va bu HAR resursda qaytadan takrorlanadi", ru: "  // ...и это повторяется заново в КАЖДОМ ресурсе" })}</Cm>{"\n"}
                {"}"}
              </CodeFile>}
            {cur === "base" && <CodeFile name={tr2({ uz: "src/api/car/car.service.ts  ·  4 qator", ru: "src/api/car/car.service.ts  ·  4 строки" })} minH={190}>
                <At>@Injectable</At>{"()"}{"\n"}
                <Jx>export class</Jx>{" CarService"}{"\n"}
            ...
            {cur === null && <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Variantni tanlang ↑", ru: "Выберите вариант ↑" })}</p></div>}
            <AgentCard>{tr2({ uz: "CarService yoz: BaseService'dan meros olsin (Create/Update DTO, CarEntity), CarEntity repozitoriysini inject qilib super(repo) chaqirsin.", ru: "Напиши CarService: пусть наследуется от BaseService (Create/Update DTO, CarEntity), инжектирует репозиторий CarEntity и вызывает super(repo)." })}</AgentCard>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "natija", ru: "результат" })}</p>
            {cur === "manual" && <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.danger }}>{tr2({ uz: "~40 qator qo'l mehnati", ru: "~40 строк ручного труда" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "5 metodni o'zingiz yozasiz — va keyingi resursda (mijoz, buyurtma) HAMMASI qaytadan.", ru: "Все 5 методов пишете сами — а в следующем ресурсе (клиент, заказ) ВСЁ заново." })}</p></div>}
            {cur === "base" && <div className="frame-success fade-step"><p className="note-h" style={{ color: T.success }}>{tr2({ uz: "4 qator — 5 metod tekin", ru: "4 строки — 5 методов бесплатно" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Oshpaz har taomni noldan o'ylab topmaydi — tayyor retsept kitobidan oladi.", ru: "Повар не выдумывает каждое блюдо с нуля — берёт его из готовой книги рецептов." })}</p>{done && <p className="body" style={{ margin: "8px 0 0", color: T.ink }}>{doneMsg}</p>}</div>}
    ...
    /* §34: «b» ochiq bo'lsa yakuniy xulosa o'sha qutining ichida — alohida quti faqat «a» oxirgi bosilganda */
        ...
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Ikkala yo'lni oching: qo'lda yozilgani va tayyor asosdan olingani.", ru: "💡 Откройте оба пути: написанный вручную и взятый из готовой основы." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen9 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>CRUD kodini <span className="italic" style={{ color: T.accent }}>kim</span> yozadi?</>, ru: <><span className="italic" style={{ color: T.accent }}>Кто</span> пишет код CRUD?</> })}</h2></>}
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const { fill } = useBoard();
  useEffect4(() => {
    if (storedAnswer) fill("controller");
  }, []);
  const candidates = [
    ...
    { id: "col", correct: false, label: "@Column()  brand: string;", why: tr2({ uz: "Bu Entity'ga tegishli (jadval ustuni). Controller'da eshiklar (@Get/@Post) bo'ladi.", ru: "Это из Entity (столбец таблицы). В controller — двери (@Get/@Post)." }) },
    { id: "super", correct: false, label: "super(carRepo);", why: tr2({ uz: "Bu Service'ga tegishli (BaseService chaqiruvi). Controller'da bunday qator bo'lmaydi.", ru: "Это из Service (вызов BaseService). В controller такой строки не бывает." }) }
  ];
  return <Stage eyebrow={tr2({ uz: "4-qadam · Controller", ru: "Шаг 4 · Controller" })} screen={screen} scrollSignal={done ? 1 : 0} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Eshiklarni yig'ing", ru: "Соберите двери" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Admin mashina qo'shmoqchi — so'rov <span className="italic" style={{ color: T.accent }}>qaysi eshikdan</span> kiradi?</>, ru: <>Админ хочет добавить машину — <span className="italic" style={{ color: T.accent }}>в какую дверь</span> войдёт запрос?</> })}</h2></div>
    ...
    scaffoldBottom={<>{"}"}</>}
    candidates={candidates}
    agent={tr2({ uz: "CarController yoz: create, findAll, findOne, update, remove — standart CRUD endpointlar, har biri CarService'ning mos metodini chaqirsin.", ru: "Напиши CarController: create, findAll, findOne, update, remove — стандартные CRUD-эндпоинты, каждый вызывает соответствующий метод CarService." })}
    instruction={tr2({ uz: "car.controller.ts ga qaysi qatorlar tegishli?", ru: "Какие строки относятся к car.controller.ts?" })}
    completedInit={!!storedAnswer}
    onComplete={() => {
      setDone(true);
      fill("controller");
    ...
    doneText={tr2({ uz: <>5 ta eshik tayyor. Controller faqat <b>qabul qilib chaqiradi</b> — asosiy ishni service/BaseService bajaradi. Oxirgi qadam: Module.</>, ru: <>5 дверей готовы. Controller только <b>принимает и вызывает</b> — основную работу делает service/BaseService. Последний шаг: Module.</> })}
  />
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Har qatorga qarang: u tashqaridan keladigan so'rovni qabul qiladimi?", ru: "💡 Посмотрите на каждую строку: принимает ли она запрос извне?" })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen11 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Yangi mashina <span className="italic" style={{ color: T.accent }}>qo'shish</span> uchun qaysi dekorator?</>, ru: <>Какой декоратор — чтобы <span className="italic" style={{ color: T.accent }}>добавить</span> новую машину?</> })}</h2></>}
  options={[{ uz: "@Post() — yangi mashina qo'shish", ru: "@Post() — добавить новую машину" }, { uz: "@Get() — mashinalarni o'qish", ru: "@Get() — прочитать машины" }, { uz: "@Delete() — mashinani o'chirish", ru: "@Delete() — удалить машину" }, { uz: "@Column() — ustunni belgilash", ru: "@Column() — задать столбец" }]}
  correctIdx={0}
...
var Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const candidates = [
    ...
    { id: "isstring", correct: false, label: "@IsString()  brand: string;", why: tr2({ uz: "Bu DTO'ga tegishli (validatsiya qoidasi). Module'da ro'yxatlar bo'ladi.", ru: "Это из DTO (правило валидации). В module — списки." }) },
    { id: "col", correct: false, label: "@Column()  price: number;", why: tr2({ uz: "Bu Entity'ga tegishli (jadval ustuni). Module bo'limni ulaydi, ustun yozmaydi.", ru: "Это из Entity (столбец таблицы). Module подключает отдел, а не пишет столбцы." }) }
  ];
  return <Stage eyebrow={tr2({ uz: "5-qadam · Module", ru: "Шаг 5 · Module" })} screen={screen} scrollSignal={done ? 1 : 0} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Module'ni yig'ing", ru: "Соберите Module" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>5 fayl alohida turibdi — ularni <span className="italic" style={{ color: T.accent }}>kim bir-biriga ulaydi</span>?</>, ru: <>5 файлов лежат по отдельности — <span className="italic" style={{ color: T.accent }}>кто соединит их друг с другом</span>?</> })}</h2></div>
    ...
    candidates={candidates}
    agent={tr2({ uz: "CarModule yoz: imports'da TypeOrmModule.forFeature([CarEntity]), controllers'da CarController, providers'da CarService bo'lsin.", ru: "Напиши CarModule: в imports — TypeOrmModule.forFeature([CarEntity]), в controllers — CarController, в providers — CarService." })}
    instruction={tr2({ uz: "car.module.ts ga qaysi qatorlar tegishli?", ru: "Какие строки относятся к car.module.ts?" })}
    completedInit={!!storedAnswer}
    onComplete={() => {
      setDone(true);
      fill("module");
  ...
  />
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Module — ro'yxatlar joyi: nimani ulash kerakligini sanaydi, kod yozmaydi.", ru: "💡 Module — место списков: он перечисляет, что подключить, а не пишет код." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [walking, setWalking] = useState3(false);
  const [dragOver, setDragOver] = useState3(false);
  const [holding, setHolding] = useState3(false);
  const [typed, setTyped] = useState3(storedAnswer ? OK_JSON.length : 0);
  const [sc, setSc] = useState3(0);
  const { fill } = useBoard();
  const wired = phase >= 2;
  const done = phase >= 3;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, phase);
  useEffect4(() => {
    if (storedAnswer) fill("plate");
  }, []);
  useEffect4(() => {
    if (done) {
      fill("plate");
  ...
  return <Stage eyebrow={tr2({ uz: "Ochilish kuni · AppModule", ru: "День открытия · AppModule" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : phase === 0 ? tr2({ uz: "Mijozni kiriting", ru: "Впустите посетителя" }) : phase === 1 ? tr2({ uz: "Taxtachani osing", ru: "Повесьте табличку" }) : tr2({ uz: "Mijozni qayta kiriting", ru: "Впустите посетителя снова" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>5 fayl tayyor — nega <span className="mono" style={{ color: T.accent }}>/car</span> hali ham <span className="italic" style={{ color: T.accent }}>404</span> qaytaradi?</>, ru: <>5 файлов готовы — почему <span className="mono" style={{ color: T.accent }}>/car</span> всё ещё возвращает <span className="italic" style={{ color: T.accent }}>404</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "restoran kirishi", ru: "вход в ресторан" })}</p>
            <div className={`oc-door ${walking ? "oc-walk" : ""} ${phase === 1 && !walking ? "is-404" : ""} ${done ? "is-ok" : ""}`}>
              <div className="oc-plate-board">
                <span className="oc-plate-h">{tr2({ uz: "🚪 KIRISH TAXTASI", ru: "🚪 ВЫВЕСКА У ВХОДА" })}</span>
    ...
    onClick={() => {
      if (canHang && holding) hangPlate();
    }}
  >
                  {wired ? <span className="oc-plate-row on oc-plate-snap">{tr2({ uz: "🚗 Mashinalar", ru: "🚗 Машины" })}</span> : <span className="oc-plate-empty">{canHang ? holding ? tr2({ uz: "↓ shu yerga qo'ying", ru: "↓ отпустите здесь" }) : tr2({ uz: "bo'sh joy — taxtacha yo'q", ru: "пустое место — таблички нет" }) : tr2({ uz: "bo'sh joy — taxtacha yo'q", ru: "пустое место — таблички нет" })}</span>}
                </div>
              </div>
              <div className="oc-visitor" aria-hidden="true">{walking ? "🙋" : done ? "🛎️ 🙋" : phase === 1 ? "🙋‍♂️" : "🙋"}</div>
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button className="btn" disabled={walking || done} onClick={enterVisitor}>{done ? tr2({ uz: "✓ Mijoz kirdi", ru: "✓ Посетитель вошёл" }) : walking ? tr2({ uz: "🚶 Mijoz kirmoqda…", ru: "🚶 Посетитель заходит…" }) : tr2({ uz: "🚪 Mijozni kiriting", ru: "🚪 Впустите посетителя" })}</button>
              {canHang && <button
  ...
  >{tr2({ uz: "🚗 Mashinalar — taxtachani sudrang", ru: "🚗 Машины — перетащите табличку" })}</button>}
            </div>
            <AgentCard>{tr2({ uz: "CarModule'ni AppModule'ning imports ro'yxatiga qo'sh — endpointlar tirik bo'lsin.", ru: "Добавь CarModule в список imports у AppModule — пусть эндпоинты оживут." })}</AgentCard>
          </Col>
          <Col gap={12}>
            <CodeFile name="src/api/app.module.ts" minH={120}>
              ...
              {wired ? <span className="el-in oc-code-new" style={{ color: T.success }}>{tr2({ uz: "    CarModule,   // ← qo'shildi ✓", ru: "    CarModule,   // ← добавлен ✓" })}</span> : <Cm>{tr2({ uz: "    // CarModule, ← yo'q!", ru: "    // CarModule, ← нет!" })}</Cm>}{"\n"}
              {"  ],"}{"\n"}
              {"})"}{"\n"}
              <Jx>export class</Jx>{" AppModule {}"}
            </CodeFile>
            <p className="flow-label">{tr2({ uz: "GET /car natijasi", ru: "результат GET /car" })}</p>
            {phase === 0 && <div className="frame-dash" style={{ padding: "8px 12px" }}><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Mijozni kiriting ←", ru: "Впустите посетителя ←" })}</p></div>}
            {phase === 1 && <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.danger }}>{tr2({ uz: "✗ 404 — topilmadi", ru: "✗ 404 — не найдено" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Fayllar bor, xodimlar joyida — lekin mijoz bo'limni topa olmadi. Taxtada «Mashinalar» yo'q. Chapdagi taxtachani sudrab, kirish taxtasiga oling.", ru: "Файлы есть, сотрудники на местах — но посетитель не нашёл отдел. На вывеске нет «Машины». Перетащите табличку слева на вывеску у входа." })}</p></div>}
            {phase === 2 && <div className="frame-success fade-step"><p className="note-h" style={{ color: T.success }}>{tr2({ uz: "🪧 Taxtacha osildi", ru: "🪧 Табличка повешена" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Kodda <span className="mono">CarModule,</span> qatori paydo bo'ldi. Endi mijozni qayta kiriting — bo'limni topadimi?</>, ru: <>В коде появилась строка <span className="mono">CarModule,</span>. Теперь впустите посетителя снова — найдёт ли он отдел?</> })}</p></div>}
            {done && <>
              <pre className="json json-type">{OK_JSON.slice(0, typed).join("\n")}</pre>
              {typed >= OK_JSON.length && <div className="frame-success fade-step"><p className="note-h" style={{ color: T.success }}>{tr2({ uz: "✓ 200 OK — bo'lim ochildi", ru: "✓ 200 OK — отдел открыт" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Bitta qator hammasini ishga tushirdi. Ochilish taxtasi to'ldi — <b>6/6</b>.</>, ru: <>Одна строка запустила всё. Доска открытия заполнена — <b>6/6</b>.</> })}</p></div>}
            </>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 O'ylang: mijoz bo'limni topishi uchun undan oldin nima bajarilgan bo'lishi kerak?", ru: "💡 Подумайте: что должно быть сделано раньше, чтобы посетитель нашёл отдел?" })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen14 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr2({ uz: "Mashq · 5-savol", ru: "Практика · вопрос 5" })}
  questionText="5 fayl yozildi, lekin CarModule AppModule'ga qo'shilmadi. Nima bo'ladi?"
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>CarModule <span className="italic" style={{ color: T.accent }}>ulanmasa</span> nima bo'ladi?</>, ru: <>Что будет, если CarModule <span className="italic" style={{ color: T.accent }}>не подключить</span>?</> })}</h2></>}
  options={[{ uz: "Hammasi baribir avvalgidek ishlayveradi", ru: "Всё продолжит работать как раньше" }, { uz: "/car endpointlari ishlamaydi — 404", ru: "Эндпоинты /car не работают — 404" }, { uz: "Butun loyiha umuman ishga tushmay qoladi", ru: "Весь проект вообще не запустится" }, { uz: "Faqat GET so'rovlari ishlayveradi", ru: "Работать будут только GET-запросы" }]}
  correctIdx={1}
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [step, setStep] = useState3(storedAnswer ? FLOW.length : -1);
  const [sc, setSc] = useState3(0);
  ...
  return <Stage eyebrow={tr2({ uz: "Markaziy · so'rov yo'li", ru: "Главное · путь запроса" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "So'rovni oxirigacha kuzating", ru: "Проследите запрос до конца" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Admin <span className="mono" style={{ color: T.accent }}>POST /car</span> bosdi — so'rov qanday <span className="italic" style={{ color: T.accent }}>sayohat</span> qiladi?</>, ru: <>Админ нажал <span className="mono" style={{ color: T.accent }}>POST /car</span> — какое <span className="italic" style={{ color: T.accent }}>путешествие</span> совершит запрос?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Siz qurgan resurs orqali bitta so'rovni kuzatamiz. Har bekat — siz yozgan fayllardan biri. Tugmani bosib, oxirigacha boring.</>, ru: <>Проследим один запрос через ресурс, который вы построили. Каждая остановка — один из ваших файлов. Нажимайте кнопку и дойдите до конца.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
    ...
    return <div key={i} className="flow-stop" style={{ opacity: lit ? 1 : 0.35 }}>
          ...
          <Col>
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={done} onClick={adv}>{step < 0 ? tr2({ uz: "▶ So'rovni yuborish", ru: "▶ Отправить запрос" }) : done ? tr2({ uz: "✓ Javob qaytdi", ru: "✓ Ответ вернулся" }) : tr2({ uz: "Keyingi bekat →", ru: "Следующая остановка →" })}</button>
            {cur && <div className="sk-info fade-step" key={step}><p className="note-h"><span style={{ fontSize: 20, marginRight: 6 }}>{cur.icon}</span><span className="mono" style={{ color: T.accent }}>{tr2(cur.k)}</span> · {tr2(cur.r)}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2(cur.d)}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Mana butun yo'l — hammasi siz qurgan 5 fayl orqali o'tdi. Endi Swagger'da tirik ko'ramiz.", ru: "Вот весь путь — он прошёл через 5 файлов, которые построили вы. Теперь посмотрим вживую в Swagger." })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 So'rov yo'lini oxirigacha bosib boring — har bosishda u bitta bekatga siljiydi.", ru: "💡 Ведите запрос до конца нажатиями — с каждым он сдвигается на одну остановку." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
var Screen16 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [sc, setSc] = useState3(0);
  const done = n >= TOTAL;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, n);
  useEffect4(() => {
    if (!running) return;
    if (n >= TOTAL) {
      setRunning(false);
  ...
  return <Stage eyebrow={tr2({ uz: "Agent · yaratish", ru: "Агент · создание" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Playbookni yuboring", ru: "Отправьте playbook" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Sekin qo'lda qildik — agent buni necha <span className="italic" style={{ color: T.accent }}>soniyada</span> quradi?</>, ru: <>Вручную было медленно — за сколько <span className="italic" style={{ color: T.accent }}>секунд</span> это построит агент?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="prompt-box fade-up delay-1">
              <span className="agent-lbl">{tr2({ uz: "💬 Agentga playbook", ru: "💬 Playbook для агента" })}</span>
              <p className="agent-msg" style={{ marginBottom: 0 }}>{tr2({ uz: `"Avtosalon uchun Car resursini qo'sh: Entity (brand, model, price, is_available) → create/update DTO → BaseService'dan meros service → CRUD controller → module va uni AppModule'ga ula."`, ru: '"Добавь ресурс Car для автосалона: Entity (brand, model, price, is_available) → create/update DTO → service с наследованием от BaseService → CRUD controller → module и подключи его к AppModule."' })}</p>
            </div>
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={running || done} onClick={run}>{done ? tr2({ uz: "✓ Yozildi", ru: "✓ Написано" }) : running ? tr2({ uz: "⏳ Agent yozyapti…", ru: "⏳ Агент пишет…" }) : tr2({ uz: "▶ Playbookni agentga yuborish", ru: "▶ Отправить playbook агенту" })}</button>
            <AgentCard>{tr2({ uz: "Bir buyruqning o'zi — chunki agent 5 qadamni qaysi tartibda qilishni siz aytib berdingiz.", ru: "Всего одна команда — потому что вы сами объяснили агенту, в каком порядке делать 5 шагов." })}</AgentCard>
          </Col>
          <Col>
    ...
    return <div key={i} className={`gen-file ${ready ? "ready" : ""} el-in`}>
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Agent 5 qadamni soniyalarda bajardi! Lekin diqqat — agent ham shoshib xato qiladi. Keyingi ekranda uning kodini <b>tekshiramiz</b>.</>, ru: <>Агент выполнил 5 шагов за секунды! Но внимание — агент тоже спешит и ошибается. На следующем экране <b>проверим</b> его код.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Yuborish tugmasini bosing — agent fayllarni ketma-ket yozadi, biroz kuting.", ru: "💡 Нажмите кнопку отправки — агент напишет файлы один за другим, немного подождите." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen17 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [openId, setOpenId] = useState3(storedAnswer ? "POST/car" : null);
  const [tried, setTried] = useState3(storedAnswer ? /* @__PURE__ */ new Set(["POST/car"]) : /* @__PURE__ */ new Set());
  const [sc, setSc] = useState3(0);
  const [missed, setMissed] = useState3(0);
  const firstCorrectRef = useRef3(storedAnswer ? storedAnswer.firstAttemptCorrect ?? storedAnswer.correct ?? null : null);
  const mountTs = useRef3(Date.now());
  const done = fixed;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, (found ? 1 : 0) + (fixed ? 1 : 0));
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { stage: "final", screenIdx: screen, question: "Agent kodidagi begona qatorni toping", correct: firstCorrectRef.current === true, firstAttemptCorrect: firstCorrectRef.current === true, solved: true, picked: firstCorrectRef.current === true ? 0 : 1, elapsedMs: Date.now() - mountTs.current });
  }, [done]);
  const pickBad = () => {
    if (found) return;
  ...
  return <Stage eyebrow={tr2({ uz: "Debugging · nazoratchi", ru: "Отладка · контролёр" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done && !_resc} label={done ? tr2({ uz: "Oxirgi qadam →", ru: "Последний шаг →" }) : _resc ? tr2({ uz: "Qatorlarni birga ko'ramiz →", ru: "Разберём строки вместе →" }) : found ? tr2({ uz: "Endi tuzating", ru: "Теперь исправьте" }) : tr2({ uz: "Xatoni toping", ru: "Найдите ошибку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Agent yozdi — lekin <span className="mono" style={{ color: T.accent }}>/car</span> xato beryapti. <span className="italic" style={{ color: T.accent }}>Qayerda</span> adashgan?</>, ru: <>Агент написал — но <span className="mono" style={{ color: T.accent }}>/car</span> выдаёт ошибку. <span className="italic" style={{ color: T.accent }}>Где</span> он ошибся?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="ai-card fade-up delay-1">
              <div className="ai-row"><span className="ai-badge">AI</span><span className="ai-bubble">{tr2({ uz: "Mana CarController kodingiz! (lekin /car ishlamayapti 🤔)", ru: "Вот ваш код CarController! (но /car не работает 🤔)" })}</span></div>
              <div className="ai-code">
                <div className="ai-line" style={{ cursor: "default", opacity: 0.85 }}><span style={{ color: CODE.attr }}>@Controller</span>{"('car') {"}</div>
                <div className="ai-line" onClick={pickGood} title={tr2({ uz: "to'g'ri qator", ru: "правильная строка" })}><span style={{ color: CODE.attr }}>@Post</span>{"()  create("}<span style={{ color: CODE.attr }}>@Body</span>{"() dto) { ... }"}</div>
                <div className="ai-line" onClick={pickGood} title={tr2({ uz: "to'g'ri qator", ru: "правильная строка" })}><span style={{ color: CODE.attr }}>@Get</span>{"()  findAll() { ... }"}</div>
                ...
                <div className="ai-line" onClick={pickGood} title={tr2({ uz: "to'g'ri qator", ru: "правильная строка" })}><span style={{ color: CODE.attr }}>@Delete</span>{"(':id')  remove(...) { ... }"}</div>
                <div className="ai-line" style={{ cursor: "default", opacity: 0.85 }}>{"}"}</div>
              </div>
              {found && !fixed && <button className="btn fade-step" style={{ alignSelf: "flex-start" }} onClick={fix}>{tr2({ uz: "🔧 Begona @Column qatorini olib tashlash", ru: "🔧 Убрать чужую строку @Column" })}</button>}
              {fixed && <p className="ai-prompt" style={{ color: T.success, fontStyle: "normal", fontWeight: 600 }}>{tr2({ uz: "✓ Tuzatildi — controller endi toza!", ru: "✓ Исправлено — controller теперь чистый!" })}</p>}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "natija", ru: "результат" })}</p>
            {!found && <div className="hint"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: <>Har qatorni o'qing: <span className="mono">@Column</span> qaysi qatlamga tegishli edi? Begona qatorni bosing.</>, ru: <>Прочитайте каждую строку: к какому слою относился <span className="mono">@Column</span>? Нажмите на чужую строку.</> })}</p></div>}
            {found && !fixed && <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.accent }}>{tr2({ uz: "✓ Topdingiz!", ru: "✓ Нашли!" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <><span className="mono">@Column</span> — bu <b>Entity</b> qatori (jadval ustuni), controller'da turibdi. Shuning uchun kod buziladi. Chap tomondagi tugma bilan olib tashlang →</>, ru: <><span className="mono">@Column</span> — это строка <b>Entity</b> (столбец таблицы), а стоит в controller. Поэтому код ломается. Уберите её кнопкой слева →</> })}</p></div>}
            {fixed && <>
              <div className="takeaway fade-step"><div className="ta-bulb">🛠️</div><p className="ta-h">{tr2({ uz: "Debug qildingiz!", ru: "Вы сделали дебаг!" })}</p><p className="ta-sub">{tr2({ uz: "Kodni o'qib, begona qatorni topib, tuzatdingiz — agent ustidan nazorat shu", ru: "Прочитали код, нашли чужую строку и исправили — это и есть контроль над агентом" })}</p></div>
              <CarSwagger available={true} openId={openId} onToggle={toggle} triedIds={tried} onTry={onTry} />
            </>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Kod qatorlarini birma-bir oching: qaysi biri bu agentga tegishli emas?", ru: "💡 Открывайте строки кода одну за другой: какая из них не относится к этому агенту?" })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Bu qatorlarni keyinroq birga ko'rib chiqamiz — hozir davom etsangiz bo'ladi.", ru: "Эти строки разберём вместе позже — сейчас можно продолжить." })}</p>}
      </div>
    </Stage>;
};
var ACHIEVEMENTS = {
...
var ACH_TRIGGERS = { s4: "blueprintDrafter", s9: "recipeMaster", s14: "grandOpening", s17: "detective404" };
...
var QUIZ_BANK = [
  { q: { uz: "`Entity` nimani belgilaydi?", ru: "Что определяет `Entity`?" }, opts: [{ uz: "So'rov qaysi eshikdan kirishini bildiradi", ru: "Через какую дверь входит запрос" }, { uz: "Loyihani qaysi tartibda ishga tushishini", ru: "В каком порядке запускается проект" }, { uz: "Foydalanuvchi ekranidagi ranglarni", ru: "Цвета на экране пользователя" }, { uz: "Bazadagi jadval qanday ko'rinishini", ru: "Как выглядит таблица в базе" }], correct: 3 },
  { q: { uz: "`BaseEntity` qaysi ustunlarni tekin beradi?", ru: "Какие столбцы `BaseEntity` даёт бесплатно?" }, opts: [{ uz: "brand, model hamda price ustunlarini", ru: "Столбцы brand, model и price" }, { uz: "id, created_at va updated_at ustunlarini", ru: "Столбцы id, created_at и updated_at" }, { uz: "username hamda password ustunlarini", ru: "Столбцы username и password" }, { uz: "imports va controllers ro'yxatlarini", ru: "Списки imports и controllers" }], correct: 1 },
  { q: { uz: "`DTO` nima uchun kerak?", ru: "Зачем нужен `DTO`?" }, opts: [{ uz: "Kelgan ma'lumot qoidalarini tekshirish uchun", ru: "Чтобы проверять правила входящих данных" }, { uz: "Bazadagi jadval ustunlarini yaratish uchun", ru: "Чтобы создавать столбцы таблицы в базе" }, { uz: "CarModule'ni AppModule'ga ulab qo'yish uchun", ru: "Чтобы подключить CarModule к AppModule" }, { uz: "NestJS serverini ishga tushirish uchun", ru: "Чтобы запустить сервер NestJS" }], correct: 0 },
  { q: { uz: "Bo'sh `brand` bilan POST /car yuborilsa?", ru: "Отправили POST /car с пустым `brand` — что будет?" }, opts: [{ uz: "Mashina baribir bazaga saqlanadi", ru: "Машина всё равно сохранится в базу" }, { uz: "Server butunlay ishdan chiqadi", ru: "Сервер полностью упадёт" }, { uz: "400 qaytadi — bazaga bormaydi", ru: "Вернётся 400 — до базы не дойдёт" }, { uz: "brand qiymati avtomatik to'ldiriladi", ru: "Значение brand заполнится автоматически" }], correct: 2 },
  { q: { uz: "`PartialType(CreateCarDto)` nima qiladi?", ru: "Что делает `PartialType(CreateCarDto)`?" }, opts: [{ uz: "Barcha maydonni majburiy qiladi", ru: "Делает все поля обязательными" }, { uz: "Barcha maydonni ixtiyoriy qiladi", ru: "Делает все поля необязательными" }, { uz: "Bazada butunlay yangi jadval yaratadi", ru: "Создаёт в базе совершенно новую таблицу" }, { uz: "Mavjud endpointlarni o'chirib qo'yadi", ru: "Удаляет существующие эндпоинты" }], correct: 1 },
  { q: { uz: "CRUD metodlarini (create, findAll, remove) kim beradi?", ru: "Кто даёт CRUD-методы (create, findAll, remove)?" }, opts: [{ uz: "Controller o'zi to'liq yozib chiqadi", ru: "Controller сам всё напишет" }, { uz: "PostgreSQL o'zi avtomatik yozadi", ru: "PostgreSQL напишет их автоматически" }, { uz: "Har resursda qaytadan qo'lda yoziladi", ru: "В каждом ресурсе пишутся заново вручную" }, { uz: "`BaseService` — meros bo'lib keladi", ru: "`BaseService` — приходят по наследству" }], correct: 3 },
  { q: { uz: "Yangi mashina qo'shish uchun qaysi dekorator?", ru: "Какой декоратор — добавить новую машину?" }, opts: [{ uz: "`@Post()` — yangisini qo'shish uchun", ru: "`@Post()` — добавить новое" }, { uz: "`@Get()` — mavjudini o'qish uchun", ru: "`@Get()` — прочитать существующее" }, { uz: "`@Column()` — ustunni belgilash uchun", ru: "`@Column()` — задать столбец" }, { uz: "`@Module()` — bo'limni ulash uchun", ru: "`@Module()` — подключить отдел" }], correct: 0 },
  { q: { uz: "`Controller` nima qiladi?", ru: "Что делает `Controller`?" }, opts: [{ uz: "Jadval ustunlarini aniq belgilab beradi", ru: "Точно задаёт столбцы таблицы" }, { uz: "Ma'lumotni to'g'ridan-to'g'ri diskka yozadi", ru: "Пишет данные напрямую на диск" }, { uz: "So'rovni qabul qilib, service'ni chaqiradi", ru: "Принимает запрос и вызывает service" }, { uz: "Loyihani noldan build qilib ishga tushiradi", ru: "Собирает проект с нуля и запускает" }], correct: 2 },
  { q: { uz: "`CarModule` `AppModule` imports'iga qo'shilmasa?", ru: "`CarModule` не добавлен в imports `AppModule` — что будет?" }, opts: [{ uz: "Hammasi baribir avvalgidek ishlaydi", ru: "Всё продолжит работать как раньше" }, { uz: "/car endpointlari ishlamaydi — 404", ru: "Эндпоинты /car не работают — 404" }, { uz: "Faqat GET so'rovlari ishlayveradi", ru: "Работать будут только GET-запросы" }, { uz: "Butun loyiha ishga tushmay qoladi", ru: "Весь проект не запустится" }], correct: 1 },
  { q: { uz: "`super(carRepo)` nima uchun chaqiriladi?", ru: "Зачем вызывается `super(carRepo)`?" }, opts: [{ uz: "Bazada yangi jadval yaratish uchun", ru: "Чтобы создать в базе новую таблицу" }, { uz: "Serverni qaytadan ishga tushirish uchun", ru: "Чтобы перезапустить сервер" }, { uz: "Endpointni ro'yxatdan o'tkazish uchun", ru: "Чтобы зарегистрировать эндпоинт" }, { uz: "BaseService'ga repozitoriyni berish uchun", ru: "Чтобы дать BaseService репозиторий" }], correct: 3 },
  { q: { uz: "`car.module.ts` ichida nima bo'ladi?", ru: "Что находится внутри `car.module.ts`?" }, opts: [{ uz: "@Column'dagi ustunlar ro'yxati", ru: "Список столбцов из @Column" }, { uz: "@IsString kabi qoidalar ro'yxati", ru: "Список правил вроде @IsString" }, { uz: "imports, controllers, providers", ru: "imports, controllers, providers" }, { uz: "main.ts faylining sozlamalari", ru: "Настройки файла main.ts" }], correct: 2 },
  { q: { uz: "Yangi resurs qo'shishning to'g'ri tartibi?", ru: "Правильный порядок добавления нового ресурса?" }, opts: ["Entity → DTO → Service → Controller → Module", "Controller → Module → Entity → DTO → Service", "Module → Controller → Service → DTO → Entity", "DTO → Entity → Controller → Module → Service"], correct: 0 }
];
...
var ScreenCarPractice = (props) => <ScreenLivePractice
  {...props}
  title={{ uz: "Ochilish kuni — /car ni o'z loyihangizda tirik qiling", ru: "День открытия — оживите /car в своём проекте" }}
  task={{ uz: "VS Code'da IntroNestArxitechture loyihasini oching: mashinalar resursini 5 qadam bilan qo'shing va CarModule'ni AppModule imports ro'yxatiga ulang. Brauzerda /car ni ochib, 200 javobini ko'ring.", ru: "Откройте проект IntroNestArxitechture в VS Code: добавьте ресурс машин за 5 шагов и подключите CarModule к списку imports у AppModule. Откройте /car в браузере и убедитесь, что приходит ответ 200." }}
  checklist={[
    { uz: "`car.entity.ts` — `brand`, `model`, `price`, `is_available` ustunlarini yozing", ru: "`car.entity.ts` — напишите столбцы `brand`, `model`, `price`, `is_available`" },
    { uz: "`create-car.dto.ts` va `update-car.dto.ts` (`PartialType`) fayllarini qo'shing", ru: "добавьте файлы `create-car.dto.ts` и `update-car.dto.ts` (`PartialType`)" },
    { uz: "`car.service.ts` — `extends BaseService` va `super(carRepo)`", ru: "`car.service.ts` — `extends BaseService` и `super(carRepo)`" },
    { uz: "`car.controller.ts` — CRUD endpointlar (`@Post`, `@Get`, `@Patch`, `@Delete`)", ru: "`car.controller.ts` — CRUD-эндпоинты (`@Post`, `@Get`, `@Patch`, `@Delete`)" },
    { uz: "`car.module.ts` ni yozib, uni `AppModule` imports'iga ulang", ru: "напишите `car.module.ts` и подключите его к imports `AppModule`" },
    { uz: "Brauzerda `/car` ni oching — `200` javobini ko'ring (404 emas)", ru: "Откройте `/car` в браузере — увидите ответ `200` (а не 404)" }
  ]}
/>;
...
var Screen19 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  const RECAP = [
    tr2({ uz: "Har resurs = 5 qadam: Entity → DTO → Service → Controller → Module", ru: "Каждый ресурс = 5 шагов: Entity → DTO → Service → Controller → Module" }),
    tr2({ uz: "Entity — jadval shakli; id va vaqtlar BaseEntity'dan tekin", ru: "Entity — форма таблицы; id и время бесплатно из BaseEntity" }),
    tr2({ uz: "DTO — kelgan ma'lumot qoidalari (bo'sh brand → 400)", ru: "DTO — правила входящих данных (пустой brand → 400)" }),
    tr2({ uz: "Service — BaseService'dan meros: CRUD tekin keladi", ru: "Service — наследует BaseService: CRUD приходит бесплатно" }),
    tr2({ uz: "CarModule'ni AppModule'ga ulash (aks holda 404)", ru: "Подключить CarModule к AppModule (иначе 404)" }),
    tr2({ uz: "Agent yozgan kodni tekshirish: har qator o'z qatlamida (debug)", ru: "Проверять код агента: каждая строка в своём слое (отладка)" })
  ];
  const HOMEWORK = [
    { b: tr2({ uz: "O'z resursingiz", ru: "Свой ресурс" }), t: tr2({ uz: "— masalan Order (buyurtma) yoki Client (mijoz): 5 qadamni qog'ozga yozing", ru: "— например Order (заказ) или Client (клиент): распишите 5 шагов на бумаге" }) },
    { b: "Playbook", t: tr2({ uz: "— har qadam uchun agentga beradigan promptni tayyorlang", ru: "— подготовьте промпт для агента на каждый шаг" }) },
    { b: tr2({ uz: "Tekshiruv", ru: "Проверка" }), t: tr2({ uz: "— AppModule'ga ulanganini va Swagger'da ko'rinishini tasavvur qiling", ru: "— представьте, как он подключён к AppModule и виден в Swagger" }) }
  ];
  ...
  return <Stage eyebrow={tr2({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "Mashinalar bo'limi ochildi", ru: "Отдел машин открыт" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>yangi resurs qo'sha olasiz</span>.</>, ru: <>Теперь вы <span className="italic" style={{ color: T.accent }}>умеете добавлять новый ресурс</span>.</> })}</h2>{
    /* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>{tr2({ uz: "📝 Uyga vazifa", ru: "📝 Домашнее задание" })}</div><ul>{HOMEWORK.map((h, i) => <li key={i}><b>{h.b}</b> <span className="t">{h.t}</span></li>)}</ul><p className="hw-note">{tr2({ uz: "🚀 Keyingi — PRAKTIKA: o'z mini-loyihangiz uchun 2–3 resurs quring (Avtosalon / Kutubxona / Do'kon), agentni shu playbook bilan boshqarib!", ru: "🚀 Дальше — ПРАКТИКА: постройте 2–3 ресурса для своего мини-проекта (Автосалон / Библиотека / Магазин), управляя агентом по этому playbook!" })}</p></div>}
...
function NestArchResourceLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  const [board, setBoard] = useState3({});
  const [plus, setPlus] = useState3(false);
  const fill = useCallback2((k) => setBoard((b) => b[k] ? b : { ...b, [k]: true }), []);
  const plusOn = useCallback2(() => {
    setPlus((p) => p || true);
    setBoard((b) => b.dto ? b : { ...b, dto: true });
  }, []);
  ...
  return <LangContext.Provider value={lang}>
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={{ uz: "Birinchi resursni qo'shish darsi", ru: "Урок: добавляем первый ресурс" }} /> : <>
...
export {
  NestArchResourceLesson as default
};
