...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и число ✅/❌ пока скрыты — по кнопке «Открыть результат» всё появится сразу и у вас, и на экранах учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <p className="mstats-verdict-t">{tr({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Правильно только у <b>{pct}%</b> — тема осталась классу непонятной. Перед продолжением стоит коротко повторить.</> })}</p>}
...
var AgentCard = ({ children, title = { uz: "💬 Agentni shunday yo'naltiring", ru: "💬 Направьте агента вот так" } }) => <div className="agent-card"><span className="agent-lbl">{tr(title)}</span><p className="agent-msg">{children}</p></div>;
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const [seen, setSeen] = useState(storedAnswer ? /* @__PURE__ */ new Set([0, 1]) : /* @__PURE__ */ new Set());
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tushuncha · happy path", ru: "Понятие · happy path" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Happy path'ni ko'ring", ru: "Посмотрите happy path" }} onClick={onNext} /></>}>
    ...
    setShow(true);
    setSc((n) => n + 1);
  }}>{show ? tr({ uz: "✓ Ko'rdingiz", ru: "✓ Просмотрено" }) : tr({ uz: "Bu yetarlimi?", ru: "Этого достаточно?" })}</button>
          </Col>
          <Col>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tushuncha · edge", ru: "Понятие · edge" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `Chegaralarni sinang (${seen.size}/3)`, ru: `Проверьте границы (${seen.size}/3)` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            ...
            {active === null ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Chegarani bosing ←", ru: "Нажмите на границу ←" })}</p></div> : <div className={CASES[active].bad ? "frame-warn fade-step" : "frame-success fade-step"} key={active}><p className="body mono" style={{ margin: "0 0 5px", color: CASES[active].bad ? T.danger : T.success, fontWeight: 700 }}>{CASES[active].in} → {CASES[active].out}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr(CASES[active].note)}</p></div>}
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Yechim · throw", ru: "Решение · throw" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Funksiyani mustahkamlang", ru: "Укрепите функцию" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
    ...
    setShow(true);
    setSc((n) => n + 1);
  }}>{show ? tr({ uz: "✓ Mustahkamlandi", ru: "✓ Укреплено" }) : tr({ uz: "🛡️ Guard (throw) qo'shish", ru: "🛡️ Добавить guard (throw)" })}</button>
          </Col>
          <Col>
            <p className="flow-label">{tr({ uz: "endi nima bo'ladi", ru: "что будет теперь" })}</p>
            {!show ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Tugmani bosing ←", ru: "Нажмите кнопку ←" })}</p></div> : <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="toThrow" screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "To'g'ri yozuvni tanlang", ru: "Выберите правильную запись" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Savol: qaysi variantda funksiya darrov chaqirilmaydi, balki () => ichiga o'ralgan? Noto'g'risini bossangiz sabab chiqadi.", ru: "💡 Вопрос: в каком варианте функция не вызывается сразу, а обёрнута в () =>? Нажмёте не тот — появится причина." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
        <Zoomable>
        <div className="split">
          <Col>
            <button className={`vcard ${choice === "a" ? "shake" : ""}`} onClick={() => pick("a")} disabled={done} style={{ flexDirection: "column", alignItems: "flex-start", gap: 6, boxShadow: choice === "a" ? `inset 0 0 0 1.5px ${T.danger}` : void 0 }}>
              <span className="vlbl">{choice === "a" ? "✗ " : ""}{tr({ uz: "Variant A", ru: "Вариант A" })}</span>
...
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Mashq · guard yig'ish", ru: "Практика · собираем guard" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
          ...
          <Col>
            ...
            <div className="dd-pool fade-up delay-1">
              {pool.length ? pool.map((c) => <div key={c.id} className={`dd-chip ${shake === c.id ? "shake" : ""}`} onPointerDown={(e) => down(e, c)}>
                ...
                </div>) : <span className="small" style={{ color: T.success, fontWeight: 700 }}>{tr({ uz: "✓ Kerakli bloklar joylandi", ru: "✓ Нужные блоки на местах" })}</span>}
            </div>
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [a, setA] = useState(!!storedAnswer);
  const [b, setB] = useState(!!storedAnswer);
  const [sc, setSc] = useState(0);
  const done = a && b;
  ...
  return <Stage eyebrow={tr({ uz: "Tushuncha · chegara chizig'i", ru: "Понятие · линия границы" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Ikki tomonni sinang", ru: "Проверьте обе стороны" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Ikkala tugmani bosing — «▶ 1 ni sinash» va «▶ 0 ni sinash»: chegaraning ikki tomoni ham yashil bo'lishi kerak.", ru: "💡 Нажмите обе кнопки — «▶ Проверить 1» и «▶ Проверить 0»: обе стороны границы должны стать зелёными." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
        <Zoomable>
        <div className="split">
          <Col>
    ...
    setA(true);
    setSc((n) => n + 1);
  }}>{a ? "✓ 1 → PASS" : tr({ uz: "▶ 1 ni sinash", ru: "▶ Проверить 1" })}</button>
              <button className="btn-soft" disabled={b} onClick={() => {
    setB(true);
    setSc((n) => n + 1);
  }}>{b ? tr({ uz: "✓ 0 → xato tutildi", ru: "✓ 0 → ошибка поймана" }) : tr({ uz: "▶ 0 ni sinash", ru: "▶ Проверить 0" })}</button>
            </div>
...
var Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Xato yo'li · API", ru: "Путь ошибки · API" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Bog'lanishni ko'ring", ru: "Посмотрите связь" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Natija · bug", ru: "Итог · bug" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Hikoyani ko'ring", ru: "Посмотрите историю" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Tugmani ketma-ket bosing — avval faqat happy-path test, keyin edge test qo'shiladi: ikki natijani solishtiring.", ru: "💡 Нажимайте кнопку по очереди — сначала только happy-path тест, потом добавится edge-тест: сравните два результата." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
        <Zoomable>
        <div className="split">
          <Col>
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Amaliyot · haqiqiy edge test", ru: "Практика · настоящий edge-тест" })} screen={screen} scrollSignal={done ? 1 : 0} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Haqiqiy edge testlarni tanlang", ru: "Выберите настоящие edge-тесты" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Har testga ikki savol bering: () => bilan o'ralganmi? toThrow ishlatilganmi (toBe emas)? Ikkalasiga «ha» bo'lsa — haqiqiy edge test.", ru: "💡 Задайте каждому тесту два вопроса: обёрнут ли он в () =>? использован ли toThrow (а не toBe)? Если оба «да» — настоящий edge-тест." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
        <PickLines
  ...
  />
        {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: <>2 ta haqiqiy edge test: 0 va manfiy, ikkalasi ham <span className="mono">() =&gt; ...toThrow()</span> bilan. Yolg'onlar: o'ralmagan chaqiruv, noto'g'ri tekshiruvchi (toBe) va <span className="mono">console.log</span>.</>, ru: <>Два настоящих edge-теста: 0 и отрицательное, оба через <span className="mono">() =&gt; ...toThrow()</span>. Фальшивки: вызов без обёртки, неверный matcher (toBe) и <span className="mono">console.log</span>.</> })}</p></div>}
      </div>
    </Stage>;
};
...
var Screen16 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Yakuniy · chegara qiymati", ru: "Финал · граничное значение" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
    ...
    setTwist(true);
    setSc((n) => n + 1);
  }}>{tr({ uz: "▶ Eski testni ishga tushirish", ru: "▶ Запустить старый тест" })}</button>
                  </> : <>
  ...
  return <Stage eyebrow={tr({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        ...
        {!isLive ? <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 16, alignItems: "center" }}>
          ...
          </div> : !loaded ? <p className="mono small fade-up" style={{ color: T.ink2 }}>{tr({ uz: "Natijalar yuklanmoqda…", ru: "Загружаем результаты…" })}</p> : board.length === 0 ? <div className="frame-wait fade-up"><p className="body" style={{ margin: 0 }}>{tr({ uz: "Bu sessiyaga hali hech kim qo'shilmagan.", ru: "К этой сессии пока никто не подключился." })}</p></div> : <>
            <Confetti />
...
var Screen17 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: "Edge case va exception'larni o'rgandingiz", ru: "Вы освоили edge case и exception" })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Endi kodingiz <span className="italic" style={{ color: T.accent }}>g'alati kirishlardan</span> ham himoyalangan.</>, ru: <>Теперь ваш код защищён и от <span className="italic" style={{ color: T.accent }}>странного ввода</span>.</> })}</h2>{
...
function EdgeCasesTestLesson({ lang: langProp, onFinished }) {
