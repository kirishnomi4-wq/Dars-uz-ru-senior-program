...
//  Manba:  src/1-Modull/InternetLesson.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/1-Modull/InternetLesson.jsx
...
var LESSON_META = { lessonId: "internet-01-v18", lessonTitle: { uz: "Internet qanday ishlaydi", ru: "Как устроен интернет" } };
var HOMEWORK = {
  type: "text",
  title: { uz: "Internetni hayotda kuzating", ru: "Понаблюдайте за интернетом в жизни" },
  brief: {
    uz: "Uyda uchta kichik kuzatuv qilasiz va javoblaringizni platformadagi vazifa-savoliga yozib yuborasiz.",
    ru: "Дома сделаете три небольших наблюдения и отправите ответы в задание на платформе."
  },
  items: [
    { uz: "Kuzating — sevimli saytingiz domeni nima?", ru: "Понаблюдайте — какой домен у вашего любимого сайта?" },
    { uz: "Tasavvur qiling — sayt brauzeringizga qanday yo'l bosib keladi", ru: "Представьте — какой путь сайт проходит до вашего браузера" },
    { uz: "Tushuntiring — do'stingizga internet qanday ishlashini", ru: "Объясните — другу, как работает интернет" }
  ]
};
var HW_TOKENS = [
...
var Q_LABELS = { 4: { uz: "1 — Brauzer", ru: "1 — Браузер" }, 6: { uz: "2 — Domen", ru: "2 — Домен" }, 10: { uz: "3 — DNS", ru: "3 — DNS" }, 13: { uz: "4 — Server (HTML)", ru: "4 — Сервер (HTML)" }, 18: { uz: "5 — Yakuniy (so'rov tartibi)", ru: "5 — Итоговый (порядок запроса)" } };
...
var ouz = (o) => o && typeof o === "object" && !React3.isValidElement(o) ? o.uz ?? o.ru ?? "" : o;
var Tg = ({ children }) => <span style={{ color: CODE.tag }}>{children}</span>;
...
var ACHIEVEMENTS = {
  firstwin: { icon: "🎯", name: "Bullseye!", desc: { uz: "Birinchi test savoliga to'g'ri javob berdingiz", ru: "Вы правильно ответили на первый вопрос теста" } },
  packet: { icon: "📦", name: "Special Delivery!", desc: { uz: "So'rovni to'g'ri serverga yetkazdingiz", ru: "Вы доставили запрос на нужный сервер" } },
  router: { icon: "🧭", name: "Right Track!", desc: { uz: "So'rov bosqichlarini to'g'ri tartibladingiz", ru: "Вы верно расставили этапы запроса" } },
  graduate: { icon: "🏆", name: "Level Up!", desc: { uz: "Internet darsini to'liq yakunladingiz", ru: "Вы полностью прошли урок об интернете" } }
};
var ACH_TRIGGERS = { s4: "firstwin", s13b: "packet", s13c: "router" };
...
function AchCounter() {
  ...
  return <div className="ach-cnt-wrap">
      ...
      {open && <div className="ach-pop" onMouseLeave={() => setOpen(false)}>
          <div className="ach-pop-h">{tr2({ uz: "🏅 Badges", ru: "🏅 Значки" })} — {count}/{total}</div>
...
var Stage = ({ children, eyebrow, screen, totalScreens = TOTAL_SCREENS, navContent, narrow, mentorStatic, mentorCollapse }) => {
      ...
      <div className="stage">
        <div className="stage-header" style={{ paddingLeft: padH, paddingRight: padH }}>
          ...
          <div className="chrome">
            {
    /* F-0730-05: eyebrow {uz,ru} obyekt kelishi mumkin — tr() shart, aks holda React #31 oq-ekran */
  }
...
var QuestionScreen = ({ screen, idx, scope, eyebrow, question, questionText, options, correctIdx, explainCorrect, explainWrong, audioText, audioOk, audioWrong, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const practice = !!(_am && _am.practice);
  ...
  return <Stage eyebrow={eyebrow} screen={screen} narrow audioState={audioText ? audio : void 0} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={isMentorLive ? !mReveal : !solved} label={isMentorLive ? mReveal ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Avval natijani oching", ru: "Сначала откройте результат" } : solved ? { uz: "Davom etish", ru: "Продолжить" } : oneShot ? { uz: "Javob tanlang", ru: "Выберите ответ" } : { uz: "To'g'ri javobni toping", ru: "Найдите правильный ответ" }} onClick={onNext} /></>}>
      <div className="screen" style={{ justifyContent: isMentorLive ? "flex-start" : "safe center", gap: "clamp(16px,2.5vw,24px)" }}>
        ...
        {oneShot && !solved && <p className="small mono fade-up" style={{ margin: "-8px 0 0", color: T.accent, fontWeight: 600 }}>{tr2({ uz: "⚡ Jonli dars — bitta urinish, o'ylab bosing!", ru: "⚡ Живой урок — одна попытка, думайте перед выбором!" })}</p>}
    ...
    return <button key={i} className={cls} disabled={solved || isMentorLive} onClick={() => pick(i)} style={{ padding: picked !== null ? "clamp(9px,1.3vw,12px) clamp(15px,2.2vw,20px)" : "clamp(13px,1.9vw,17px) clamp(15px,2.2vw,20px)", fontSize: "clamp(15px,1.85vw,17px)", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <FeedbackBlock show={isMentorLive ? mReveal : picked !== null} isCorrect={isMentorLive ? true : solved && !wrongLocked} neutral={waiting}>
          <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: waiting ? T.blue : isMentorLive || solved && !wrongLocked ? T.success : T.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>
            {isMentorLive ? fmtCode(`${tr2({ uz: "✓ To'g'ri javob", ru: "✓ Правильный ответ" })}: ${String.fromCharCode(65 + correctIdx)} — ${tr2(options[correctIdx])}`) : waiting ? tr2({ uz: "📨 Javobingiz qabul qilindi", ru: "📨 Ваш ответ принят" }) : wrongLocked ? fmtCode(`${tr2({ uz: "To'g'ri javob", ru: "Правильный ответ" })}: ${String.fromCharCode(65 + correctIdx)} — ${tr2(options[correctIdx])}`) : solved ? tr2({ uz: "To'g'ri", ru: "Правильно" }) : tr2({ uz: "Qaytadan urinib ko'ring", ru: "Попробуйте ещё раз" })}
          </p>
          <p className="body" style={{ margin: 0 }}>
            {fmtCode(tr2(isMentorLive ? explainCorrect : waiting ? { uz: "Javobingiz yozib olindi. To'g'ri yoki xato ekani mentor «Natijani ochish»ni bosganda hammada birdan ko'rinadi.", ru: "Ваш ответ записан. Правильный он или нет — станет видно всем сразу, когда ментор нажмёт «Открыть результат»." } : wrongLocked ? explainWrong[picked] ?? explainWrong.default : solved ? explainCorrect : explainWrong[picked] ?? explainWrong.default))}
          </p>
          {
    /* Xato qilgan o'quvchi (yoki mustaqil o'quvchi) mavzuni qisqa kartalarda qayta ko'radi.
       Jonli darsda — javob sirini saqlash uchun faqat reveal'dan keyin chiqadi. */
  }
          {hasRecap && !isMentorLive && firstCorrectRef.current === false && (!oneShot || revealed) && <button className="rc-open-mini" onClick={() => setRecapOpen(true)}>{tr2({ uz: "📖 Qisqa takrorlash — mavzuni yana bir ko'rish", ru: "📖 Краткое повторение — ещё раз взглянуть на тему" })}</button>}
        </FeedbackBlock>
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
    ...
    return <div key={i} className={`mstats-row ${reveal && !isC ? "dimmed" : ""}`}>
              ...
              <span className="mono mstats-count" style={isC ? { color: T.success, fontWeight: 800 } : void 0}>{n > 0 ? `${n} ${tr2({ uz: "o'quvchi", ru: "учеников" })} · ${pct}%` : "—"}</span>
            </div>;
  })}
      </div>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlash tavsiya etiladi.</>, ru: <>⚠️ Только <b>{pct}%</b> верно — эта тема осталась классу непонятной. Перед продолжением стоит кратко повторить.</> })}</p>
            ...
            {level === "maybe" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>🟡 <b>{pct}%</b> to'g'ri — yomon emas. Xohlasangiz, davom etishdan oldin qisqa takrorlab oling.</>, ru: <>🟡 <b>{pct}%</b> верно — неплохо. При желании кратко повторите перед продолжением.</> })}</p>
              {onOpenRecap && <button className="rc-open soft" onClick={onOpenRecap}>{tr2({ uz: "📖 Qisqa takrorlash", ru: "📖 Краткое повторение" })}</button>}
            </>}
            {level === "good" && <p className="mstats-verdict-t">{tr2({ uz: <>✅ <b>{pct}%</b> to'g'ri — sinf mavzuni o'zlashtirdi. Bemalol davom eting!</>, ru: <>✅ <b>{pct}%</b> верно — класс усвоил тему. Спокойно продолжайте!</> })}</p>}
            {level === "few" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>Javob berganlar kam ({answered} ta) — foiz bo'yicha xulosa chiqarish qiyin. O'zingiz baholang:</>, ru: <>Ответивших мало ({answered}) — по проценту сложно судить. Оцените сами:</> })}</p>
    ...
    /* Kim hali javob bermadi — ismlari */
      ...
      {reveal && struggling && <p className="mstats-warn">{tr2({ uz: "⚠️ Ko'pchilik xato qildi — bu mavzu tushunarsiz bo'lgan ko'rinadi. Qayta tushuntirish tavsiya etiladi.", ru: "⚠️ Большинство ошиблось — похоже, тема осталась непонятной. Рекомендуется объяснить заново." })}</p>}
...
var Mentor = ({ children }) => {
  ...
  return <div data-tour="mentor" className={`mentor fade-up ${enabled ? "mentor-mob" : ""} ${collapsed ? "is-collapsed" : ""}`} onClick={collapsed ? expand : void 0} role={collapsed ? "button" : void 0}>
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  const audio = useAudio([{ id: "s0", text: `Brauzerga youtube.com yozib Enter bosasiz — bir soniyada sayt chiqadi. Lekin shu bir soniya ichida so'rovingiz xonangizdan chiqib ketadi, chet elgacha yo'l bosadi va javob bilan qaytib keladi. Sizningcha, sayt sizga qayerdan yetib keladi?`, trigger: "on_mount", waits_for: { type: "option_picked" } }]);
  const [picked, setPicked] = useState3(storedAnswer?.picked ?? null);
  const [phase, setPhase] = useState3(storedAnswer ? "site" : "idle");
  const OPTS = [
    { id: "a", label: { uz: "Kompyuterimning ichidan", ru: "Из моего компьютера" } },
    { id: "b", label: { uz: "Boshqa kompyuterdan — internet orqali", ru: "С другого компьютера — через интернет" } },
    { id: "c", label: { uz: "Hech qayerdan, o'zi paydo bo'ladi", ru: "Ниоткуда, появляется сам" } }
  ];
  const go = () => {
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Вступление" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>youtube.com yozib Enter bossangiz, sayt sizga <span className="italic" style={{ color: T.accent }}>qayerdan</span> yetib keladi?</>, ru: <>Вводите youtube.com и жмёте Enter — <span className="italic" style={{ color: T.accent }}>откуда</span> сайт добирается до вас?</> })}</h1>
        ...
        <Split>
          <Col>
            ...
            <Preview minH={150} title="youtube.com">
              {phase === "idle" && <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center" }}>{tr2({ uz: "↑ Manzilni yozib Enter bosing…", ru: "↑ Введите адрес и нажмите Enter…" })}</p>}
              {phase === "loading" && <p className="mono" style={{ color: T.ink2, textAlign: "center", margin: 0 }}><span className="gen-line">{tr2({ uz: "Sayt yuklanmoqda", ru: "Сайт загружается" })}</span></p>}
              {phase === "site" && <div className="fade-step">
          ...
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Sizningcha, sayt sizga qayerdan yetib keladi?", ru: "Как думаете, откуда сайт добирается до вас?" })}</p>
            <div className="fade-up delay-3" style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {OPTS.map((o) => {
    const on = picked === o.id;
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null} onClick={() => pick(o.id)}><span className="radio">{on && <span className="radio-dot" />}</span><span>{tr2(o.label)}</span></button>;
  })}
            </div>
            {picked !== null && <p className="hook-ack fade-step">{tr2({ uz: "To'g'ri yo'nalish! Sayt boshqa kompyuterda — serverda yashaydi. Buni internet yetkazib beradi.", ru: "Верное направление! Сайт живёт на другом компьютере — на сервере. А доставляет его интернет." })}</p>}
          </Col>
        </Split>
      </div>
    </Stage>;
...
var JR_R = 38;
var JR_STATIONS = [
  { k: "you", ic: "🧑", l: { uz: "Siz", ru: "Вы" }, a: 0 },
  { k: "browser", ic: "🌐", l: { uz: "Brauzer", ru: "Браузер" }, a: 90 },
  { k: "dns", ic: "🗂️", l: { uz: "DNS", ru: "DNS" }, a: 180 },
  { k: "server", ic: "🖥️", l: { uz: "Server", ru: "Сервер" }, a: 270 }
];
var jrPos = (a) => {
...
var JR_STEPS = [
  { active: "browser", kind: "req", pkt: "⌨️ youtube.com", cap: { uz: "1-qadam — Enter bosildi! Brauzer manzilni oldi.", ru: "Шаг 1 — Enter нажат! Браузер получил адрес." } },
  { active: "dns", kind: "req", pkt: "youtube.com = ?", cap: { uz: "2-qadam — Brauzer DNS'dan IP raqamini so'raydi.", ru: "Шаг 2 — Браузер спрашивает у DNS IP-номер." } },
  { active: "server", kind: "ip", pkt: "142.250.190.78", cap: { uz: "3-qadam — DNS IP'ni topdi — so'rov to'g'ri serverga ketdi.", ru: "Шаг 3 — DNS нашёл IP — запрос ушёл на нужный сервер." } },
  { active: "you", kind: "page", pkt: "▶️ YouTube", cap: { uz: "4-qadam — Server YouTube sahifasini qaytardi — mana, ekraningizda ochildi!", ru: "Шаг 4 — Сервер вернул страницу YouTube — вот, она открылась на вашем экране!" } }
];
var JR_STEP_MS = 1785;
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const STEPS = [
    { text: { uz: "Internet nima?", ru: "Что такое интернет?" }, tag: { uz: "tarmoq", ru: "сеть" } },
    { text: { uz: "Brauzer — saytlarni ochuvchi dastur", ru: "Браузер — программа, открывающая сайты" }, tag: "Chrome, Safari" },
    { text: { uz: "Domen — saytning manzili", ru: "Домен — адрес сайта" }, tag: "youtube.com" },
    { text: { uz: "DNS — manzilni topadi", ru: "DNS — находит адрес" }, tag: { uz: "domen → IP", ru: "домен → IP" } },
    { text: { uz: "Server — sayt shu yerda", ru: "Сервер — сайт находится здесь" }, tag: "" }
  ];
  const DONE = JR_STEPS.length;
  const [jstep, setJstep] = useState3(-1);
  const [turn, setTurn] = useState3(0);
  const [playing, setPlaying] = useState3(false);
  const baseRef = useRef3(0);
  const timer = useRef3(null);
  const animScrollRef = useRef3(null);
  ...
  const play = useCallback2(() => {
    ...
    const advance = () => {
      timer.current = setTimeout(() => {
        if (i < JR_STEPS.length - 1) {
          i++;
          setJstep(i);
          setTurn(base + (i + 1) * 90);
          advance();
        } else {
          setJstep(JR_STEPS.length);
          setTurn(base + 360);
          baseRef.current = base + 360;
          setPlaying(false);
        }
      }, JR_STEP_MS);
    };
  ...
  const caption = jstep < 0 ? { uz: "▶ tugmasini bosing — so'rov bosib o'tadigan butun aylanma yo'lni ko'ramiz.", ru: "▶ Нажмите кнопку — увидим весь круговой путь, который проходит запрос." } : jstep >= DONE ? { uz: "✓ Tayyor! YouTube ekraningizda ochildi — har bir sayt va ilova xuddi shu yo'l bilan, atigi 1 soniyada keladi.", ru: "✓ Готово! YouTube открылся на вашем экране — каждый сайт и приложение приходит этим же путём, всего за 1 секунду." } : JR_STEPS[jstep].cap;
  const isNarrow = useIsMobile(768);
  const [showSteps, setShowSteps] = useState3(false);
  useEffect4(() => {
    if (!isNarrow) return;
    const t = setTimeout(() => {
      if (animScrollRef.current) animScrollRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
    }, 850);
    return () => clearTimeout(t);
  }, [isNarrow]);
  const AnimBlock = <Zoomable>
    <div className="jr fade-up delay-1">
      <div className="jr-ring">
        <div className="jr-ring-circle" />
        <div className="jr-hub"><span className="jr-hub-ic">⏱️</span><span className="jr-hub-t">{tr2({ uz: "≈ 1 soniya", ru: "≈ 1 секунда" })}</span></div>
        {JR_STATIONS.map((s) => {
    const on = activeKey === s.k;
    const delivered = jstep >= DONE && s.k === "you";
    return <div key={s.k} className={`jr-st ${on ? "on" : ""} ${delivered ? "delivered" : ""}`} style={jrPos(s.a)}>
              {delivered && <span className="jr-badge">▶️ YouTube ✓</span>}
              <span className="jr-ic">{s.ic}</span>
              <span className="jr-l">{tr2(s.l)}</span>
            </div>;
  })}
      ...
      <div className={`jr-cap ${jstep >= DONE ? "done" : ""}`} key={jstep}>{tr2(caption)}</div>
      <div className="jr-dots">{JR_STEPS.map((_, i) => <span key={i} className={`jr-dot ${jstep > i || jstep >= DONE ? "fill" : ""} ${jstep === i ? "cur" : ""}`} />)}</div>
      <button className="btn" style={{ alignSelf: "center" }} onClick={play} disabled={playing}>{playing ? tr2({ uz: "So'rov yo'lda…", ru: "Запрос в пути…" }) : jstep >= DONE ? tr2({ uz: "↻ Yana ko'rish", ru: "↻ Ещё раз" }) : tr2({ uz: "▶ Yo'lni boshlash", ru: "▶ Начать путь" })}</button>
    </div>
    </Zoomable>;
  const StepsBlock = <Col>
      <p className="flow-label">{tr2({ uz: "Dars rejasi — 5 qadam", ru: "План урока — 5 шагов" })}</p>
      <ol className="roadmap">{STEPS.map((s, i) => <li key={i} className="step-card fade-up" style={{ animationDelay: `${0.08 + i * 0.05}s` }}><span className="step-num">{String(i + 1).padStart(2, "0")}</span><span className="step-body"><span className="step-text">{tr2(s.text)}</span>{s.tag && <span className="step-tag">{tr2(s.tag)}</span>}</span></li>)}</ol>
    </Col>;
  return <Stage eyebrow={tr2({ uz: "Reja", ru: "План" })} screen={screen} audioState={audio} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Boshlaymiz →", ru: "Начнём →" }} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up"><span className="italic" style={{ color: T.accent }}>{tr2({ uz: "Sayt sizgacha qanday yetib keladi?", ru: "Как сайт доходит до вас?" })}</span></h2></div>
        ...
        {!isNarrow ? <Split>{AnimBlock}{StepsBlock}</Split> : !showSteps ? <div ref={animScrollRef} className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>
            {AnimBlock}
            <button className="btn-soft" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(true)}>{tr2({ uz: "📋 Dars rejasini ko'rish", ru: "📋 Посмотреть план урока" })}</button>
          </div> : <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>
            <button className="btn-soft" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(false)}>{tr2({ uz: "↩ Animatsiyani ko'rish", ru: "↩ Посмотреть анимацию" })}</button>
            {StepsBlock}
          </div>}
      </div>
    </Stage>;
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const NODES = [
    { k: "phone", ic: "📱", l: { uz: "Telefon", ru: "Телефон" }, pos: [42, 36] },
    { k: "laptop", ic: "💻", l: { uz: "Noutbuk", ru: "Ноутбук" }, pos: [42, 120] },
    { k: "tablet", ic: "📲", l: { uz: "Planshet", ru: "Планшет" }, pos: [128, 28] },
    { k: "tv", ic: "📺", l: { uz: "TV", ru: "ТВ" }, pos: [128, 126] },
    { k: "server", ic: "🖥️", l: { uz: "Server", ru: "Сервер" }, pos: [222, 78] }
  ];
  ...
  const [connected, setConnected] = useState3(!!storedAnswer);
  const done = connected;
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  return <Stage eyebrow={tr2({ uz: "Internet", ru: "Интернет" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Ulanishlarni ko'rsating", ru: "Покажите связи" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Internet aslida <span className="italic" style={{ color: T.accent }}>nima</span>?</>, ru: <>Что такое интернет <span className="italic" style={{ color: T.accent }}>на самом деле</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Internet — bu butun dunyo bo'ylab <b style={{ color: T.ink }}>millionlab kompyuterlar</b> bir-biriga ulangan ulkan <b style={{ color: T.ink }}>tarmoq</b>. Xuddi shaharni bog'lab turgan yo'llar kabi — uyingizdagi telefon, TV va noutbuk ham shu tarmoqqa ulangan. Tugmani bosib, ulanishlarni ko'ring.</>, ru: <>Интернет — это огромная <b style={{ color: T.ink }}>сеть</b>, где по всему миру соединены <b style={{ color: T.ink }}>миллионы компьютеров</b>. Как дороги, связывающие город — телефон, ТВ и ноутбук у вас дома тоже подключены к этой сети. Нажмите кнопку и посмотрите на связи.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            <div className="web fade-up delay-2" style={{ height: 165 }}>
              ...
              {NODES.map((n) => <div key={n.k} className={`web-node ${connected ? "on" : ""}`} style={{ left: `${n.pos[0] / 260 * 100}%`, top: `${n.pos[1] / 154 * 100}%`, display: "flex", alignItems: "center", gap: 5 }}><span>{n.ic}</span><span>{tr2(n.l)}</span></div>)}
            </div>
            {!connected && <button className="btn" style={{ alignSelf: "flex-start" }} onClick={() => setConnected(true)}>{tr2({ uz: "🔗 Ulanishlarni ko'rsatish", ru: "🔗 Показать связи" })}</button>}
          </div>
          <div className="col">            {connected ? <div className="frame-success fade-step"><p className="small mono" style={{ margin: "0 0 4px", fontWeight: 600, color: T.success, textTransform: "uppercase", letterSpacing: "0.08em" }}>{tr2({ uz: "✓ Mana tarmoq", ru: "✓ Вот сеть" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Har bir qurilma boshqasiga <b>"yo'l"</b> bilan bog'langan. Ma'lumot shu yo'llar orqali yuradi. <b>Inter-net</b> = "tarmoqlararo" degani.</>, ru: <>Каждое устройство связано с другим <b>«дорогой»</b>. Данные идут по этим дорогам. <b>Inter-net</b> = «между сетями».</> })}</p></div> : <div className="hint"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: "Telefon, noutbuk, TV, server… hammasi ulangan. Ulanishlarni ko'rish uchun tugmani bosing.", ru: "Телефон, ноутбук, ТВ, сервер… всё соединено. Нажмите кнопку, чтобы увидеть связи." })}</p></div>}
            <div className="frame-soft"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <><b>Asosiy:</b> internet — qurilmalarni bog'lovchi tarmoq. Saytlar shu tarmoq orqali sizgacha keladi.</>, ru: <><b>Главное:</b> интернет — сеть, соединяющая устройства. Сайты доходят до вас через эту сеть.</> })}</p></div>
          </div>
        </div>
        </Zoomable>
      </div>
...
var BrowserLogo = ({ k, size = 30 }) => {
  ...
  if (k === "chrome") return <span style={{ ...ring, background: "conic-gradient(from -90deg, #EA4335 0 120deg, #34A853 120deg 240deg, #FBBC05 240deg 360deg)" }}>
  ...
  if (k === "firefox") return <span style={{ ...ring, background: "radial-gradient(circle at 62% 30%, #FFE259 0 8%, #FF9D17 30%, #FF4406 62%, #B5170B 96%)" }}>
  ...
  if (k === "edge") return <span style={{ ...ring, background: "conic-gradient(from 150deg, #36D6B7 0deg, #1B9DE2 130deg, #0C5B8F 250deg, #36D6B7 360deg)" }}>
  ...
  return <span style={{ ...ring, background: "radial-gradient(circle, #fbfdff 0 12%, #4aa3ff 24%, #1574E0 82%)" }}>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [br, setBr] = useState3(() => BROWSERS.some((b) => b.k === storedAnswer?.picked) ? storedAnswer.picked : null);
  const done = br !== null;
  const cur = BROWSERS.find((b) => b.k === br);
  const pick = (k) => {
    setBr(k);
    if (storedAnswer === void 0) onAnswer(screen, { correct: true, picked: k });
  };
  return <Stage eyebrow={tr2({ uz: "Brauzer", ru: "Браузер" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Brauzer tanlang", ru: "Выберите браузер" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Saytlarni qaysi <span className="italic" style={{ color: T.accent }}>dastur</span> ochib beradi?</>, ru: <>Какая <span className="italic" style={{ color: T.accent }}>программа</span> открывает сайты?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Saytlarni ochish uchun maxsus <b style={{ color: T.ink }}>dastur</b> kerak — bu <b style={{ color: T.ink }}>brauzer</b>. Telefondagi Chrome, Safari yoki kompyuterdagi Firefox — hammasi brauzer. Siz manzilni yozasiz, brauzer saytni <b style={{ color: T.ink }}>topib keltiradi</b> va ekranga chiroyli qilib chizadi. Birini tanlang.</>, ru: <>Чтобы открывать сайты, нужна особая <b style={{ color: T.ink }}>программа</b> — это <b style={{ color: T.ink }}>браузер</b>. Chrome и Safari на телефоне или Firefox на компьютере — всё это браузеры. Вы вводите адрес, браузер <b style={{ color: T.ink }}>находит и приносит</b> сайт и красиво рисует его на экране. Выберите один.</> })}</Mentor>
        <div className="split">
          <div className="col">
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Brauzeringizni tanlang", ru: "Выберите свой браузер" })}</p>
            <div className="fade-up delay-2" style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>{BROWSERS.map((b) => {
    const on = br === b.k;
    return <button key={b.k} onClick={() => pick(b.k)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 7, padding: "12px 16px", minWidth: 86, cursor: "pointer", background: T.paper, border: `2px solid ${on ? b.color : "rgba(58,53,48,0.14)"}`, borderRadius: 14, boxShadow: on ? `0 9px 22px -8px ${b.color}88` : "0 2px 8px -4px rgba(58,53,48,0.16)", transform: on ? "translateY(-2px)" : "none", transition: "transform .2s ease, box-shadow .2s ease, border-color .2s ease" }}>
            ...
            <div className="frame-soft fade-up delay-3"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Qaysi brauzer bo'lishidan qat'i nazar — vazifasi bir xil: <b>manzilni olib, saytni keltirib, ekranga chizish</b>.</>, ru: <>Каким бы ни был браузер — задача одна: <b>взять адрес, принести сайт и нарисовать его на экране</b>.</> })}</p></div>
          </div>
          <div className="col">            <div className="flow-label" style={{ display: "flex", alignItems: "center", gap: 8 }}>{done ? <><BrowserLogo k={br} size={18} /><span>{cur.l} {tr2({ uz: "youtube.com'ni ochdi", ru: "открыл youtube.com" })}</span></> : tr2({ uz: "Sayt shu yerda ochiladi", ru: "Сайт откроется здесь" })}</div>
            <Preview title={done ? `${cur.l} — youtube.com` : "brauzer"} minH={150}>
              {done ? <SiteMock site={{ mock: "youtube", ic: "▶️", name: "YouTube", bar: "#FF0000" }} /> : <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center" }}>{tr2({ uz: "Brauzer tanlang — sayt shu yerda ochiladi", ru: "Выберите браузер — сайт откроется здесь" })}</p>}
            </Preview>
          </div>
        </div>
      </div>
...
var Screen4 = (props) => <QuestionScreen
  ...
  options={[{ uz: "Server", ru: "Сервер" }, { uz: "Brauzer", ru: "Браузер" }, { uz: "Domen", ru: "Домен" }, { uz: "DNS", ru: "DNS" }]}
  correctIdx={1}
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const audio = useAudio([{ id: "s5", text: `Saytga borish uchun uning manzilini bilish kerak. Bu manzil — domen: youtube.com, coddycamp.uz. Xuddi uyingizning manzili kabi — uni yozsangiz, aynan o'sha saytga borasiz. Domenni tanlab, qismlarini ko'ring.`, trigger: "on_mount", waits_for: null }]);
  const DOMAINS = [
    { full: "youtube.com", name: "youtube", tld: ".com", note: { uz: ".com — tijoriy va umumiy saytlar uchun.", ru: ".com — для коммерческих и общих сайтов." } },
  ...
  const [sel, setSel] = useState3(() => DOMAINS.some((d) => d.full === storedAnswer?.picked) ? storedAnswer.picked : null);
  const done = sel !== null;
  const cur = DOMAINS.find((d) => d.full === sel);
  const pick = (f) => {
    setSel(f);
    if (storedAnswer === void 0) onAnswer(screen, { correct: true, picked: f });
  };
  return <Stage eyebrow={tr2({ uz: "Domen", ru: "Домен" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Domenni tanlang", ru: "Выберите домен" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Saytning <span className="italic" style={{ color: T.accent }}>manzili</span> qanday bo'ladi?</>, ru: <>Каким бывает <span className="italic" style={{ color: T.accent }}>адрес</span> сайта?</> })}</h2></div>
        ...
        <div className="split">
          <div className="col">
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Domenni tanlang", ru: "Выберите домен" })}</p>
            ...
            <div className="frame-soft fade-up delay-3"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <><b>Asosiy:</b> domen — odam eslab qoladigan sayt manzili. Bitta sayt = bitta domen.</>, ru: <><b>Главное:</b> домен — запоминаемый человеком адрес сайта. Один сайт = один домен.</> })}</p></div>
          </div>
          <div className="col">
            <div className="flow-label">{tr2({ uz: "Domen qismlari", ru: "Части домена" })}</div>
            {done ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <div className="dlabels"><span><b style={{ color: T.accent }}>{tr2({ uz: "nom", ru: "имя" })}</b> {tr2({ uz: "— sayt nomi", ru: "— имя сайта" })}</span><span><b style={{ color: T.blue }}>{tr2({ uz: "zona", ru: "зона" })}</b> {tr2({ uz: "— turi", ru: "— тип" })}</span></div>
                <div className="sk-info"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2(cur.note)}</p></div>
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Domenni tanlang — qismlarga ajraladi", ru: "Выберите домен — он разложится на части" })}</p></div>}
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen5b = (props) => <QuestionScreen
  ...
  options={[{ uz: "Brauzer", ru: "Браузер" }, { uz: "Parol", ru: "Пароль" }, { uz: "Domen", ru: "Домен" }, { uz: "Server", ru: "Сервер" }]}
  correctIdx={2}
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [scram, setScram] = useState3("•••.•••.•••.•••");
  const iv = useRef3(null);
  const done = phase === "done";
  useEffect4(() => () => clearInterval(iv.current), []);
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  const reveal = () => {
  ...
  return <Stage eyebrow={tr2({ uz: "IP manzil", ru: "IP-адрес" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "IP manzilni ko'ring", ru: "Посмотрите IP-адрес" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Kompyuterlar manzilni qanday <span className="italic" style={{ color: T.accent }}>yozadi</span>?</>, ru: <>Как компьютеры <span className="italic" style={{ color: T.accent }}>записывают</span> адрес?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Kompyuterlar saytning <b style={{ color: T.ink }}>nomini emas, raqamini</b> ishlatadi — bu <b style={{ color: T.ink }}>IP manzil</b>. Tugmani bosing, <b style={{ color: T.ink }}>youtube.com</b> qanday raqamga aylanishini ko'ring.</>, ru: <>Компьютеры используют <b style={{ color: T.ink }}>не имя сайта, а номер</b> — это <b style={{ color: T.ink }}>IP-адрес</b>. Нажмите кнопку и посмотрите, в какой номер превращается <b style={{ color: T.ink }}>youtube.com</b>.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            <div className="frame frame-col fade-up delay-2" style={{ alignItems: "center", gap: 12 }}>
              ...
              <p className={`mono small ip-conv ${phase !== "idle" ? "go" : ""}`} style={{ margin: 0, textAlign: "center" }}>{tr2({ uz: "↓ raqamga aylantiriladi", ru: "↓ превращается в номер" })}</p>
              {phase === "idle" ? <button className="btn" onClick={reveal}>{tr2({ uz: "🔢 IP manzilni ko'rsatish", ru: "🔢 Показать IP-адрес" })}</button> : <span className="ipbox" style={{ opacity: phase === "run" ? 0.72 : 1 }}><span className="ip-type" key={phase === "done" ? "d" : "r"}>{phase === "done" ? "142.250.190.78" : scram}</span></span>}
              {phase === "run" && <p className="mono small" style={{ margin: 0, color: T.blue }}>{tr2({ uz: "aylantirilmoqda…", ru: "превращается…" })}</p>}
          ...
          <div className="col" style={{ justifyContent: "center" }}>
            <div className="flow-label">{tr2({ uz: "Xuddi telefon kontaktidek", ru: "Как контакт в телефоне" })}</div>
            ...
            <p className="small" style={{ margin: "6px 2px 0", color: T.ink2 }}>{tr2({ uz: <>Siz <b style={{ color: T.ink }}>nom</b>ni eslaysiz, qurilma esa <b style={{ color: T.ink }}>raqam</b>ni ishlatadi.</>, ru: <>Вы запоминаете <b style={{ color: T.ink }}>имя</b>, а устройство использует <b style={{ color: T.ink }}>номер</b>.</> })}</p>
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const MAP = { "youtube.com": "142.250.190.78", "coddycamp.uz": "213.230.106.24", "wikipedia.org": "208.80.154.224" };
  const PLACE = "•••.•••.•••.••";
  const [q, setQ] = useState3(storedAnswer?.q || null);
  const [phase, setPhase] = useState3(storedAnswer ? "found" : "idle");
  const [scram, setScram] = useState3(PLACE);
  const timer = useRef3(null);
  const scramIv = useRef3(null);
  const done = phase === "found";
  const rnd = (m) => Math.floor(Math.random() * m);
  useEffect4(() => () => {
    clearTimeout(timer.current);
    clearInterval(scramIv.current);
  }, []);
  const lookup = (d) => {
    ...
    timer.current = setTimeout(() => {
      clearInterval(scramIv.current);
      setPhase("found");
      if (storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true, q: d });
    }, 950);
  };
  const rightIP = phase === "found" ? MAP[q] : phase === "looking" ? scram : PLACE;
  return <Stage eyebrow={tr2({ uz: "DNS", ru: "DNS" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Domenni qidiring", ru: "Найдите домен" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Domen nomidan IP'ni kim <span className="italic" style={{ color: T.accent }}>topadi</span>?</>, ru: <>Кто <span className="italic" style={{ color: T.accent }}>находит</span> IP по доменному имени?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Raqamlarni hech kim yodlamaydi! Shuning uchun <b style={{ color: T.ink }}>DNS</b> bor — internetning <b style={{ color: T.ink }}>telefon kitobi</b>: siz domen berasiz, u IP qaytaradi. Domenni tanlang.</>, ru: <>Номера никто не запоминает! Поэтому есть <b style={{ color: T.ink }}>DNS</b> — <b style={{ color: T.ink }}>телефонная книга</b> интернета: вы даёте домен, он возвращает IP. Выберите домен.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Qaysi saytni qidiramiz?", ru: "Какой сайт ищем?" })}</p>
            ...
            <div className="dns-card fade-up delay-3">
              <div className="dns-head">{tr2({ uz: "📖 DNS — Internet telefon kitobi", ru: "📖 DNS — телефонная книга интернета" })} {phase === "looking" && <span className="dns-status">{tr2({ uz: "🔎 qidirilmoqda…", ru: "🔎 ищем…" })}</span>}{phase === "found" && <span className="dns-status ok">{tr2({ uz: "topildi ✓", ru: "найдено ✓" })}</span>}</div>
    ...
    return <div key={dom} className={`dns-entry ${cls}`}>
              ...
              {!q && <p className="small" style={{ margin: 0, color: T.ink3, fontStyle: "italic" }}>{tr2({ uz: "Yuqoridan domen tanlang — DNS uni kitobdan topadi", ru: "Выберите домен сверху — DNS найдёт его в книге" })}</p>}
            </div>
          </div>
          <div className="col">            <div className="flow-label">{tr2({ uz: "DNS qanday aylantiradi", ru: "Как DNS превращает" })}</div>
            <div className="dnsconv fade-up delay-2">
              <div className="dc-node"><span className="dc-ic">🌐</span><span className="dc-k">{tr2({ uz: "domen", ru: "домен" })}</span><span className="dc-v">{q || "youtube.com"}</span></div>
              <div className={`dc-mid ${phase === "looking" ? "busy" : ""} ${phase === "found" ? "ok" : ""}`}>
                <span className="dc-arrow">↓</span>
                <span className="dc-dns">📖 DNS</span>
                <span className="dc-arrow">↓</span>
              </div>
              <div className={`dc-node dc-ip ${phase === "found" ? "hit" : ""}`}><span className="dc-ic">🔢</span><span className="dc-k">{tr2({ uz: "IP manzil", ru: "IP-адрес" })}</span><span className="dc-v" key={phase === "found" ? "f" : "x"}>{rightIP}</span></div>
            </div>
            <p className="small" style={{ margin: "4px 2px 0", color: T.ink2 }}>{tr2({ uz: <>Siz <b style={{ color: T.ink }}>nom</b> berasiz — DNS <b style={{ color: T.ink }}>raqam</b> qaytaradi. Brauzer keyin shu IP'ga boradi.</>, ru: <>Вы даёте <b style={{ color: T.ink }}>имя</b> — DNS возвращает <b style={{ color: T.ink }}>номер</b>. Затем браузер идёт на этот IP.</> })}</p>
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Server", ru: "Сервер" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "So'rov yuboring", ru: "Отправьте запрос" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Sayt o'zi <span className="italic" style={{ color: T.accent }}>qayerda</span> yashaydi?</>, ru: <>А <span className="italic" style={{ color: T.accent }}>где</span> живёт сам сайт?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Har bir sayt <b style={{ color: T.ink }}>serverda</b> — doim yoniq kuchli kompyuterda — saqlanadi. Siz so'rov yuborasiz, server <b style={{ color: T.ink }}>sahifani qaytaradi</b>. Tugmani bosing.</>, ru: <>Каждый сайт хранится на <b style={{ color: T.ink }}>сервере</b> — всегда включённом мощном компьютере. Вы отправляете запрос, сервер <b style={{ color: T.ink }}>возвращает страницу</b>. Нажмите кнопку.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            <div className="cs fade-up delay-2">
              <div className="cs-node"><span className="cs-ic">🧑‍💻</span><span className="cs-l">{tr2({ uz: <>Siz<br />(brauzer)</>, ru: <>Вы<br />(браузер)</> })}</span></div>
              <div className="cs-wire">
                <div className="cs-track">
                  <span className={`cs-chip cs-chip-req ${step >= 1 ? "sent" : ""} ${step >= 2 ? "gone" : ""}`}>{tr2({ uz: "📨 so'rov", ru: "📨 запрос" })}</span>
                  <span className={`cs-chip cs-chip-res ${step >= 2 ? "sent" : ""}`}>{tr2({ uz: "📄 sahifa", ru: "📄 страница" })}</span>
                </div>
              </div>
              <div className={`cs-node ${step >= 1 ? "cs-active" : ""} ${step === 1 ? "cs-proc" : ""}`}><span className="cs-ic">🖥️</span><span className="cs-l">{tr2({ uz: <>Server<br />(sayt shu yerda)</>, ru: <>Сервер<br />(сайт здесь)</> })}</span></div>
            </div>
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={step === 1} onClick={send}>{step === 1 ? tr2({ uz: "Yuborilmoqda…", ru: "Отправляется…" }) : done ? tr2({ uz: "↻ Yana yuborish", ru: "↻ Отправить ещё" }) : tr2({ uz: "📨 Serverga so'rov yuborish", ru: "📨 Отправить запрос серверу" })}</button>
          </div>
          <div className="col">            {done ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <div className="flow-label">{tr2({ uz: "🖥️ Server qaytargan sahifa", ru: "🖥️ Страница, которую вернул сервер" })}</div>
                <Preview title="google.com" minH={130}><SiteMock site={siteInfo("google.com")} /></Preview>
                <div className="frame-success"><p className="small mono" style={{ margin: "0 0 4px", fontWeight: 600, color: T.success, textTransform: "uppercase", letterSpacing: "0.08em" }}>{tr2({ uz: "✓ Server javob berdi", ru: "✓ Сервер ответил" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Server <b>google.com</b> sahifasini qaytardi. U <b>doim yoniq</b> — sayt istalgan vaqtda ochiladi.</>, ru: <>Сервер вернул страницу <b>google.com</b>. Он <b>всегда включён</b> — сайт открывается в любое время.</> })}</p></div>
              </div> : <div className="hint"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: "Siz (brauzer) chap tomonda, server o'ngda. So'rov yuborib, javobni kuzating.", ru: "Вы (браузер) слева, сервер справа. Отправьте запрос и проследите за ответом." })}</p></div>}
            <div className="frame-soft"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <><b>Server</b> — saytlarni saqlovchi va so'rovga javob beruvchi kuchli kompyuter.</>, ru: <><b>Сервер</b> — мощный компьютер, который хранит сайты и отвечает на запросы.</> })}</p></div>
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen9 = (props) => <QuestionScreen
  ...
  options={[{ uz: "Brauzer", ru: "Браузер" }, { uz: "Domen", ru: "Домен" }, { uz: "Server", ru: "Сервер" }, { uz: "DNS", ru: "DNS" }]}
  correctIdx={3}
...
/>;
var PIPE = [
  { k: "addr", ic: "⌨️", l: { uz: "Manzil", ru: "Адрес" }, pos: 10, pkt: "⌨️ youtube.com", cls: "req", cap: { uz: "1 — Siz brauzerga youtube.com yozib Enter bosdingiz", ru: "1 — Вы ввели в браузер youtube.com и нажали Enter" }, note: { uz: "Mana, siz brauzerimizga youtube.com yozib Enter bosdingiz. Endi brauzer ishga tushadi va saytni qidira boshlaydi.", ru: "Вот, вы ввели в наш браузер youtube.com и нажали Enter. Теперь браузер запускается и начинает искать сайт." } },
  { k: "dns", ic: "🗂️", l: { uz: "DNS", ru: "DNS" }, pos: 30, pkt: "🔢 142.250.190.78", cls: "ip", cap: { uz: "2 — DNS nomni IP raqamiga aylantiradi", ru: "2 — DNS превращает имя в IP-номер" }, note: { uz: "Hozir brauzerimizdan DNS'ga «youtube.com qaysi raqam?» degan so'rov oqib bormoqda. DNS uni 142.250.190.78 degan IP raqamiga aylantirib beryapti.", ru: "Сейчас от нашего браузера к DNS идёт запрос «youtube.com — это какой номер?». DNS превращает его в IP-номер 142.250.190.78." } },
  { k: "server", ic: "🖥️", l: { uz: "Server", ru: "Сервер" }, pos: 50, pkt: "📄 HTML kod", cls: "page", cap: { uz: "3 — Server shu IP'dan sahifani HTML kod sifatida yuboradi", ru: "3 — Сервер по этому IP отправляет страницу в виде HTML-кода" }, note: { uz: "Mana, brauzerimiz shu IP'dagi serverga bordi. Endi server bizga sahifani HTML kod ko'rinishida qaytarib yuboryapti — bu oddiy matn.", ru: "Вот, наш браузер пришёл на сервер по этому IP. Теперь сервер возвращает нам страницу в виде HTML-кода — это обычный текст." } },
  { k: "browser", ic: "🌐", l: { uz: "Brauzer", ru: "Браузер" }, pos: 70, pkt: { uz: "👀 o'qilmoqda", ru: "👀 читается" }, cls: "read", cap: { uz: "4 — Brauzer HTML kodni satrma-satr o'qiydi", ru: "4 — Браузер читает HTML-код строка за строкой" }, note: { uz: "Endi brauzerimiz server yuborgan HTML kodni satrma-satr o'qiyapti — har bir kodni tushunib chiqyapti.", ru: "Теперь наш браузер читает присланный сервером HTML-код строка за строкой — разбирает каждую строчку." } },
  { k: "screen", ic: "🖼️", l: { uz: "Ekran", ru: "Экран" }, pos: 90, pkt: { uz: "🎨 sahifa", ru: "🎨 страница" }, cls: "page", cap: { uz: "5 — O'qilgan koddan ekranga chiroyli sahifa chiziladi", ru: "5 — Из прочитанного кода на экране рисуется красивая страница" }, note: { uz: "Va mana — o'qilgan koddan chiroyli YouTube sahifasi ekraningizga chizildi! Hammasi tayyor.", ru: "И вот — из прочитанного кода на вашем экране нарисовалась красивая страница YouTube! Всё готово." } }
];
var PIPE_MS = 2100;
var PIPE_CODE = [
  { uz: <><Tg>{"<h1>"}</Tg>▶️ YouTube<Tg>{"</h1>"}</Tg></>, ru: <><Tg>{"<h1>"}</Tg>▶️ YouTube<Tg>{"</h1>"}</Tg></> },
  { uz: <><Tg>{"<video>"}</Tg>Mushuklar 🐱<Tg>{"</video>"}</Tg></>, ru: <><Tg>{"<video>"}</Tg>Котики 🐱<Tg>{"</video>"}</Tg></> },
  { uz: <><Tg>{"<video>"}</Tg>Futbol ⚽<Tg>{"</video>"}</Tg></>, ru: <><Tg>{"<video>"}</Tg>Футбол ⚽<Tg>{"</video>"}</Tg></> }
];
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [step, setStep] = useState3(storedAnswer ? PIPE.length - 1 : -1);
  const [playing, setPlaying] = useState3(false);
  const [paused, setPaused] = useState3(false);
  const [finished, setFinished] = useState3(!!storedAnswer);
  const doneRef = useRef3(null);
  const firstRef = useRef3(true);
  useEffect4(() => {
  ...
  const cap = step < 0 ? { uz: "▶ tugmani bosing — ma'lumot chapdan o'ngga, bosqichma-bosqich oqadi.", ru: "▶ Нажмите кнопку — данные текут слева направо, шаг за шагом." } : cur ? cur.cap : "";
  return <Stage eyebrow={tr2({ uz: "So'rov yo'li", ru: "Путь запроса" })} screen={screen} audioState={audio} mentorCollapse navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Yo'lni ishga tushiring", ru: "Запустите путь" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.7vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Enter'dan saytgacha — <span className="italic" style={{ color: T.accent }}>butun yo'l</span></>, ru: <>От Enter до сайта — <span className="italic" style={{ color: T.accent }}>весь путь</span></> })}</h2></div>
    ...
    /* Konveyer — oldinga oqadigan ma'lumot */
          ...
          <div className="pl-line">
            ...
            {step >= 0 && <div className={`pl-pkt ${cur ? cur.cls : ""}`} style={{ left: `${pktLeft}%` }}>{cur ? tr2(cur.pkt) : ""}</div>}
            {PIPE.map((s, i) => <div key={s.k} className={`pl-st ${step === i ? "on" : ""} ${step > i ? "done" : ""}`} style={{ left: `${s.pos}%` }}>
                <span className="pl-st-ic">{s.ic}</span>
                <span className="pl-st-l">{tr2(s.l)}</span>
              </div>)}
          </div>
          <div className="pl-dots">{PIPE.map((_, i) => <span key={i} className={`pl-dot ${step >= i ? "fill" : ""} ${step === i ? "cur" : ""}`} />)}</div>
          {step < 0 && <div className="pl-cap">{tr2(cap)}</div>}

          {
    /* Detal-karta — joriy bosqich jonli ko'rsatiladi */
  }
          <div className="pl-detail">
            {step < 0 && <p className="pl-idle">{tr2({ uz: "Har bosqich shu yerda jonli ochiladi 👇", ru: "Каждый шаг вживую откроется здесь 👇" })}</p>}
            ...
            {(step === 2 || step === 3) && <div className="pl-codewrap fade-step">
                <div className="pl-codehead">{step === 2 ? tr2({ uz: "📄 Server yuborgan HTML kod", ru: "📄 HTML-код, присланный сервером" }) : tr2({ uz: "🌐 Brauzer kodni o'qiyapti…", ru: "🌐 Браузер читает код…" })}</div>
                <pre className={`pl-code ${step === 3 ? "reading" : ""}`}>
                  {PIPE_CODE.map((c, i) => <div key={i} className="pl-codeline">{tr2(c)}</div>)}
                  {step === 3 && <span className="pl-scan" />}
                </pre>
              </div>}
            {step === 4 && <div className="pl-render fade-step">
                <div className="pl-codehead">{tr2({ uz: "🖼️ Ekranda — tayyor sahifa", ru: "🖼️ На экране — готовая страница" })}</div>
    ...
    /* Har bosqichda Mentor o'sha qadamni tushuntiradi (pauzada — to'liq o'qishga to'xtaydi) */
          ...
          {playing && cur && <div className={`mentor pl-pausebubble fade-step ${paused ? "is-paused" : ""}`} key={step}>
              <div className="mentor-ava" aria-hidden="true"><img src={MENTOR_IMG} alt="" /></div>
              <div className="mentor-col">
                <span className="mentor-name">{paused ? `⏸ ${step + 1}/${PIPE.length} — ${tr2({ uz: "o'qing, keyin «Davom etish»", ru: "прочитайте, потом «Продолжить»" })}` : `${tr2({ uz: "Mentor", ru: "Ментор" })} · ${step + 1}/${PIPE.length} ${tr2({ uz: "qadam", ru: "шаг" })}`}</span>
                <div className="mentor-msg body">{tr2(cur.note)}</div>
              </div>
            </div>}

          <div className="pl-controls">
            {playing ? <button className="btn" onClick={togglePause} style={{ alignSelf: "center" }}>{paused ? tr2({ uz: "▶ Davom etish", ru: "▶ Продолжить" }) : tr2({ uz: "⏸ Pauza", ru: "⏸ Пауза" })}</button> : <button className="btn" onClick={start} style={{ alignSelf: "center" }}>{finished ? tr2({ uz: "↻ Yana ko'rsatish", ru: "↻ Показать ещё раз" }) : tr2({ uz: "▶ Yo'lni boshlash", ru: "▶ Начать путь" })}</button>}
          </div>
        </div>
        </Zoomable>
        {done && <div ref={doneRef} className="frame-success fade-step"><p className="small mono" style={{ margin: "0 0 4px", fontWeight: 600, color: T.success, textTransform: "uppercase", letterSpacing: "0.08em" }}>{tr2({ uz: "✓ Mana butun yo'l", ru: "✓ Вот весь путь" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Siz <b>manzil</b> yozasiz → <b>DNS</b> IP topadi → <b>server</b> <b>HTML kodini</b> beradi → <b>brauzer</b> uni o'qib ekranga chizadi. Bularning bari — <b>bir soniyada</b>! Keyingi darsda brauzer HTML'ni qanday o'qishini ochamiz.</>, ru: <>Вы вводите <b>адрес</b> → <b>DNS</b> находит IP → <b>сервер</b> отдаёт <b>HTML-код</b> → <b>браузер</b> читает его и рисует на экране. И всё это — <b>за одну секунду</b>! На следующем уроке разберём, как браузер читает HTML.</> })}</p></div>}
      </div>
    </Stage>;
};
var YT_VIDEO = (title, views, dur) => <div style={{ display: "flex", gap: 10, alignItems: "flex-start", margin: "0 0 11px" }}>
    ...
    <div style={{ minWidth: 0, paddingTop: 1 }}>
      ...
      <div style={{ fontSize: 10.5, color: "#777", fontFamily: "'Manrope', sans-serif" }}>CoddyCamp · {views} {tr2({ uz: "ko'rish", ru: "просмотров" })}</div>
    </div>
  </div>;
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [rd, setRd] = useState3(storedAnswer ? HTML.length : 0);
  const timer = useRef3(null);
  ...
  return <Stage eyebrow={tr2({ uz: "Brauzer ichida", ru: "Внутри браузера" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Kodni sahifaga aylantiring", ru: "Превратите код в страницу" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Brauzer kodni qanday <span className="italic" style={{ color: T.accent }}>sahifaga</span> aylantiradi?</>, ru: <>Как браузер превращает код в <span className="italic" style={{ color: T.accent }}>страницу</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Hozirgina server brauzerga <b style={{ color: T.ink }}>HTML kod</b> qaytarganini ko'rdik. Endi aynan <b style={{ color: T.ink }}>shu qadamning ichiga</b> kiramiz: brauzer o'sha kodni <b style={{ color: T.ink }}>satrma-satr o'qib</b>, YouTube sahifasini chizadi.</>, ru: <>Мы только что видели, как сервер вернул браузеру <b style={{ color: T.ink }}>HTML-код</b>. Теперь заглянем <b style={{ color: T.ink }}>внутрь этого шага</b>: браузер читает этот код <b style={{ color: T.ink }}>строка за строкой</b> и рисует страницу YouTube.</> })}</Mentor>
        {
    /* 12-darsni eslatuvchi "ichiga zoom" strip */
  }
        <div className="zi fade-up delay-1">
          <div className="zi-mini">
            <div className="zi-node"><span className="zi-node-ic">🖥️</span><span className="zi-node-l">{tr2({ uz: "Server", ru: "Сервер" })}</span></div>
            <div className="zi-wire"><span className="zi-pkt">📄</span></div>
            <div className="zi-node focus"><span className="zi-node-ic">🌐</span><span className="zi-node-l">{tr2({ uz: "Brauzer", ru: "Браузер" })}</span><span className="zi-lens">🔍</span></div>
            <div className="zi-wire" />
            <div className="zi-node"><span className="zi-node-ic">🖼️</span><span className="zi-node-l">{tr2({ uz: "Ekran", ru: "Экран" })}</span></div>
          </div>
          <p className="zi-label">{tr2({ uz: <>🔍 Hozirgina ko'rgan <b>«brauzer o'qiydi»</b> qadamning <b>ichiga</b> kiramiz ↓</>, ru: <>🔍 Заглянем <b>внутрь</b> только что увиденного шага <b>«браузер читает»</b> ↓</> })}</p>
        </div>
        <Zoomable>
        <div style={{ display: "flex", flexDirection: "column", gap: "clamp(10px,1.6vw,16px)" }}>
          <div className="split" ref={animRef}>
            <div className="col">
              <div className="flow-label">{tr2({ uz: "📥 Server qaytargan kod (HTML)", ru: "📥 Код, присланный сервером (HTML)" })}{reading && <span className="dns-status">{tr2({ uz: "o'qilmoqda…", ru: "читается…" })}</span>}</div>
              <pre className="code-box fade-up delay-2">{HTML.map((h, i) => {
    const cls = i === active ? "reading" : i < rd ? "read" : phase === "idle" ? "" : "dim";
    return <div key={i} className={`htl-line ${cls}`}><span className="htl-code">{tr2(h.code)}</span>{i < rd && <span className="htl-tick">{tr2({ uz: "✓ chizildi", ru: "✓ нарисовано" })}</span>}</div>;
  })}</pre>
            </div>
            <div className="col" ref={resultRef}>
              <div className="flow-label">{tr2({ uz: "🌐 Brauzer chizgan sahifa", ru: "🌐 Страница, нарисованная браузером" })}</div>
              <Preview title="youtube.com" minH={150}>
                {rd === 0 ? <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center" }}>{tr2({ uz: "Tugmani bosing — brauzer kodni o'qib, YouTube sahifasini shu yerda chizadi", ru: "Нажмите кнопку — браузер прочитает код и нарисует страницу YouTube здесь" })}</p> : <div style={{ display: "block" }}>{HTML.slice(0, rd).map((h, i) => <div key={i} className="htl-pop">{tr2(h.el)}</div>)}</div>}
              </Preview>
            </div>
          </div>
          <button className="btn" style={{ alignSelf: "center" }} onClick={start} disabled={reading}>{reading ? tr2({ uz: "Brauzer o'qiyapti…", ru: "Браузер читает…" }) : done ? tr2({ uz: "↻ Qaytadan", ru: "↻ Заново" }) : tr2({ uz: "🖥️ Brauzer bilan o'qib chizish", ru: "🖥️ Прочитать и нарисовать браузером" })}</button>
        </div>
        </Zoomable>
        {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>✓ Chapdagi <b>kod</b> = o'ngdagi <b>YouTube sahifasi</b>. Server faqat HTML matn yuboradi, chiroyli sahifani esa <b>brauzer chizadi</b>. Aynan shunday HTML'ni keyingi modulda <b>o'zingiz yozasiz</b>!</>, ru: <>✓ <b>Код</b> слева = <b>страница YouTube</b> справа. Сервер шлёт лишь HTML-текст, а красивую страницу рисует <b>браузер</b>. Точно такой же HTML в следующем модуле вы <b>напишете сами</b>!</> })}</p></div>}
      </div>
    </Stage>;
};
var Screen12 = (props) => <QuestionScreen
  ...
  options={[{ uz: "Saytning domen nomi", ru: "Доменное имя сайта" }, { uz: "Sahifa kodi (HTML)", ru: "Код страницы (HTML)" }, { uz: "DNS server manzili", ru: "Адрес DNS-сервера" }, { uz: "Boshqa brauzer dasturi", ru: "Другую программу-браузер" }]}
  correctIdx={1}
...
var SITE_CATALOG = {
  ...
  "coddycamp.uz": { ic: "🎓", name: "CoddyCamp", ip: "213.230.106.24", bar: "#FF4F28", mock: "generic" },
...
var siteInfo = (raw) => {
  ...
  let h = 0;
  for (let i = 0; i < t.length; i++) h = (h * 31 + t.charCodeAt(i)) % 1e5;
...
var SiteMock = ({ site }) => {
  if (!site) return null;
  if (site.mock === "google") {
    return <div className="fade-step" style={{ textAlign: "center", padding: "18px 6px 14px" }}>
        ...
        <div style={{ display: "flex", gap: 9, justifyContent: "center", marginTop: 16 }}>
          {tr2({ uz: ["Google Qidiruv", "Menga omad"], ru: ["Поиск Google", "Мне повезёт"] }).map((l) => <div key={l} style={{ background: "#f8f9fa", border: "1px solid #f1f1f1", borderRadius: 4, padding: "7px 12px", fontSize: 10, color: "#5f6368", fontFamily: "Manrope, sans-serif" }}>{l}</div>)}
        </div>
      </div>;
  }
  if (site.mock === "youtube") {
    return <div className="fade-step">
        ...
        <div style={{ display: "flex", gap: 9 }}>
          {[0, 1].map((i) => <div key={i} style={{ flex: 1 }}>
  ...
  if (site.mock === "wikipedia") {
    return <div className="fade-step">
        <div style={{ display: "flex", alignItems: "center", gap: 9, borderBottom: "1px solid #eaecf0", paddingBottom: 8, marginBottom: 9 }}>
          ...
          <div><div style={{ fontFamily: "Georgia, serif", fontSize: 13, color: "#202122", lineHeight: 1.1 }}>WIKIPEDIA</div><div style={{ fontSize: 8, color: "#72777d" }}>{tr2({ uz: "Erkin ensiklopediya", ru: "Свободная энциклопедия" })}</div></div>
        </div>
        <div style={{ fontFamily: "Georgia, serif", fontSize: 14, fontWeight: 700, color: "#000", marginBottom: 8 }}>{tr2({ uz: "Maqola", ru: "Статья" })}</div>
...
var RECAPS = {
  ...
  4: {
    title: { uz: "Internet va Brauzer", ru: "Интернет и браузер" },
    cards: [
      {
        ic: "🕸️",
        h: { uz: "Internet — kompyuterlarni bog'lovchi tarmoq", ru: "Интернет — сеть, связывающая компьютеры" },
      ...
      {
        ic: "📲",
        h: { uz: "Brauzer — saytlarni ochuvchi dastur", ru: "Браузер — программа, открывающая сайты" },
      ...
      {
        ic: "🎨",
        h: { uz: "Brauzer 3 ta ish qiladi", ru: "Браузер делает 3 вещи" },
  ...
  6: {
    title: { uz: "Domen — sayt manzili", ru: "Домен — адрес сайта" },
    cards: [
      {
        ic: "🏠",
        h: { uz: "Domen — saytning manzili", ru: "Домен — адрес сайта" },
      ...
      {
        ic: "🧩",
        h: { uz: "Domen ikki qismdan iborat", ru: "Домен состоит из двух частей" },
      ...
      {
        ic: "☝️",
        h: { uz: "Adashtirmang!", ru: "Не перепутайте!" },
  ...
  10: {
    title: { uz: "IP manzil va DNS", ru: "IP-адрес и DNS" },
    cards: [
      {
        ic: "🔢",
        h: { uz: "Kompyuter faqat raqamni tushunadi", ru: "Компьютер понимает только числа" },
      ...
      {
        ic: "📖",
        h: { uz: "DNS — internetning telefon kitobi", ru: "DNS — телефонная книга интернета" },
      ...
      {
        ic: "⚡",
        h: { uz: "Uchalasini eslab qoling", ru: "Запомните все три" },
  ...
  13: {
    title: { uz: "Server va HTML", ru: "Сервер и HTML" },
    cards: [
      {
        ic: "🖥️",
        h: { uz: "Sayt serverda yashaydi", ru: "Сайт живёт на сервере" },
      ...
      {
        ic: "📄",
        h: { uz: "Server chiroyli sahifa EMAS — kod qaytaradi", ru: "Сервер отдаёт не красивую страницу, а КОД" },
      ...
      {
        ic: "🎨",
        h: { uz: "Chiroyli qiladigan — brauzer", ru: "Красивым делает — браузер" },
  ...
  18: {
    title: { uz: "So'rovning to'liq yo'li", ru: "Полный путь запроса" },
    cards: [
      {
        ic: "🗺️",
        h: { uz: "4 qadam — doim shu tartibda", ru: "4 шага — всегда в этом порядке" },
      ...
      {
        ic: "🤔",
        h: { uz: "Nega aynan shu tartib?", ru: "Почему именно такой порядок?" },
      ...
      {
        ic: "⏱️",
        h: { uz: "Va bularning bari — 1 soniyada", ru: "И всё это — за 1 секунду" },
...
function RecapOverlay({ screenIdx, onClose }) {
  ...
  return <div className="rc-overlay">
      <div className="rc-head">
        <span className="rc-tag">{tr2({ uz: "📖 Qayta tushuntirish", ru: "📖 Объяснение заново" })}</span>
...
var sim13Steps = (s) => [
  { active: "browser", kind: "req", pkt: `⌨️ ${s.domain}`, cap: { uz: "1-qadam — Enter bosildi! Brauzer manzilni oldi.", ru: "Шаг 1 — Enter нажат! Браузер получил адрес." } },
  { active: "dns", kind: "req", pkt: `${s.domain} = ?`, cap: { uz: "2-qadam — Brauzer DNS'dan IP raqamini so'raydi.", ru: "Шаг 2 — Браузер спрашивает у DNS IP-номер." } },
  { active: "server", kind: "ip", pkt: s.ip, cap: { uz: "3-qadam — DNS IP'ni topdi — so'rov to'g'ri serverga ketdi.", ru: "Шаг 3 — DNS нашёл IP — запрос ушёл на нужный сервер." } },
  { active: "you", kind: "page", pkt: `${s.ic} ${s.name}`, cap: { uz: `4-qadam — Server ${s.name} sahifasini qaytardi — ekraningizda ochildi!`, ru: `Шаг 4 — Сервер вернул страницу ${s.name} — она открылась на вашем экране!` } }
];
var SIM13_STEP_MS = 1500;
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [site, setSite] = useState3(storedAnswer?.picked ? siteInfo(storedAnswer.picked) : null);
  const [steps, setSteps] = useState3(storedAnswer?.picked ? sim13Steps(siteInfo(storedAnswer.picked)) : []);
    ...
    const advance = () => {
      timer.current = setTimeout(() => {
        if (i < st.length - 1) {
          ...
          setRunning(false);
          setLoaded(s.domain);
          if (storedAnswer === void 0) onAnswer(screen, { correct: true, picked: s.domain });
  ...
  const caption = jstep < 0 ? { uz: "Manzil yuboring — so'rov bosib o'tadigan butun aylanma yo'lni ko'rasiz.", ru: "Отправьте адрес — увидите весь круговой путь, который проходит запрос." } : jstep >= DONE ? { uz: `✓ Tayyor! ${site?.name || "Sayt"} ekraningizda ochildi — aynan shu yo'l bilan, 1 soniyada.`, ru: `✓ Готово! ${site?.name || "Сайт"} открылся на вашем экране — этим же путём, за 1 секунду.` } : steps[jstep]?.cap || "";
  return <Stage eyebrow={tr2({ uz: "Amaliyot · so'rov yuboring", ru: "Практика · отправьте запрос" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "So'rov yuboring", ru: "Отправьте запрос" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>O'zingiz <span className="italic" style={{ color: T.accent }}>so'rov</span> yuboring.</>, ru: <>Отправьте <span className="italic" style={{ color: T.accent }}>запрос</span> сами.</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Endi navbat sizda! Sayt manzilini yozing yoki tayyorlaridan tanlang va <b style={{ color: T.ink }}>"Yubor"</b> tugmasini bosing — so'rovingiz <b style={{ color: T.ink }}>aylanma yo'lni</b> bosib o'tib, aynan o'sha sayt ochilishini ko'rasiz.</>, ru: <>Теперь ваша очередь! Введите адрес сайта или выберите готовый и нажмите <b style={{ color: T.ink }}>«Отправить»</b> — ваш запрос пройдёт <b style={{ color: T.ink }}>круговой путь</b>, и вы увидите, как откроется именно этот сайт.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            <div className="urlbar fade-up delay-2"><span className="urlbar-lock">🔒</span><input className="urlbar-input" value={url} placeholder={tr2({ uz: "masalan: google.com", ru: "например: google.com" })} onChange={(e) => setUrl(e.target.value)} onKeyDown={(e) => {
    if (e.key === "Enter") send();
  }} /><button className="urlbar-go" onClick={() => send()} disabled={running}>{tr2({ uz: "Yubor", ru: "Отправить" })}</button></div>
...
var NET_EDGES = [["you", "dns"], ["dns", "srv0"], ["dns", "srv1"], ["dns", "srv2"], ["srv0", "screen"], ["srv1", "screen"], ["srv2", "screen"]];
var SRV_POOL = [
...
var NET_TARGETS = ["youtube.com", "google.com", "wikipedia.org"];
var netShuffle = (a) => {
  const r = a.slice();
...
var netPick = (not) => {
  const o = NET_TARGETS.filter((s) => s !== not);
  return o[Math.floor(Math.random() * o.length)];
};
var nearMissIP = (ip) => {
  const p = ip.split(".");
  p[3] = String((parseInt(p[3], 10) + 17) % 200 + 11);
  return p.join(".");
};
var buildRound = (domain) => {
...
var Screen13b = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [round, setRound] = useState3(() => buildRound("youtube.com"));
  const target = round.target;
  const site = siteInfo(target.domain) || { ...target, ic: "🌐" };
  const [cur, setCur] = useState3(won ? "screen" : "you");
  const [pktPos, setPktPos] = useState3(won ? NET_POS.screen : NET_POS.you);
  const [hasIP, setHasIP] = useState3(won);
  const [hasPage, setHasPage] = useState3(won);
  const [visited, setVisited] = useState3(won ? ["you", "dns", "screen"] : ["you"]);
  const [traveled, setTraveled] = useState3([]);
  const [moving, setMoving] = useState3(false);
  const [phase, setPhase] = useState3(won ? "win" : "play");
  const [msg, setMsg] = useState3(null);
  const [moves, setMoves] = useState3(0);
  const [shakeNode, setShakeNode] = useState3(null);
  const [matchNode, setMatchNode] = useState3(null);
  const timer = useRef3(null);
  const winRef = useRef3(null);
  const firstRef = useRef3(true);
  ...
  const nodes = {
    you: { ic: "🧑", l: { uz: "Siz", ru: "Вы" }, note: { uz: "boshlanish", ru: "старт" }, kind: "start", pos: NET_POS.you },
    dns: { ic: "🗂️", l: { uz: "DNS", ru: "DNS" }, note: { uz: "manzil beradi", ru: "даёт адрес" }, kind: "dns", pos: NET_POS.dns },
    screen: { ic: "🖼️", l: { uz: "Ekran", ru: "Экран" }, note: { uz: "chizadi", ru: "рисует" }, kind: "screen", pos: NET_POS.screen }
  };
  round.servers.forEach((s) => {
    nodes["srv" + s.slot] = { ic: "🖥️", l: { uz: "Server", ru: "Сервер" }, ip: s.ip, name: s.name, mock: s.mock, kind: "server", pos: NET_POS["srv" + s.slot] };
  });
  ...
  const replay = () => {
    firstRef.current = false;
    resetRound(netPick(target.domain));
  };
  ...
  const go = (to) => {
    ...
    timer.current = setTimeout(() => {
      ...
      const bounce = (t, type = "bad") => {
        ...
        setMsg({ t, type });
        setShakeNode(to);
        setPktPos(nodes[from].pos);
        timer.current = setTimeout(() => setShakeNode(null), 520);
        setMoving(false);
      };
      if (node.kind === "dns") {
        setHasIP(true);
        setMsg({ t: `${tr2({ uz: "🗂️ DNS telefon kitobidan topdi", ru: "🗂️ DNS нашёл в телефонной книге" })}: ${target.domain} → ${target.ip}`, type: "good" });
      } else if (node.kind === "server") {
        if (!hasIP) return bounce(tr2({ uz: "🔒 Qaysi server kerakligini bilmaysiz — avval IP-manzilni oling.", ru: "🔒 Вы не знаете, какой сервер нужен — сначала получите IP-адрес." }));
        if (node.ip === target.ip) {
          setHasPage(true);
          setMatchNode(to);
          setMsg({ t: tr2({ uz: "🤝 IP mos keldi! Aynan shu server — sahifa olindi.", ru: "🤝 IP совпал! Это тот самый сервер — страница получена." }), type: "good" });
        } else if (node.name) return bounce(`${tr2({ uz: "❌", ru: "❌" })} ${node.ip} — ${tr2({ uz: `bu ${node.name}'niki, sizniki emas! (404)`, ru: `это ${node.name}, а не ваш! (404)` })}`);
        else return bounce(`❌ ${node.ip} — ${tr2({ uz: "juda o'xshash, lekin boshqa kompyuter! Raqamni oxirigacha solishtiring.", ru: "очень похоже, но другой компьютер! Сравните номер до конца." })}`);
      } else if (node.kind === "screen") {
        if (!hasPage) return bounce(tr2({ uz: "🖼️ Hali sahifa yo'q — uni to'g'ri serverdan olib keling.", ru: "🖼️ Страницы ещё нет — принесите её с правильного сервера." }));
      }
      setTraveled((tr3) => Array.from(/* @__PURE__ */ new Set([...tr3, ek(from, to)])));
      setVisited((v) => Array.from(/* @__PURE__ */ new Set([...v, to])));
      setCur(to);
      setMoving(false);
      if (node.kind === "screen") {
        setPhase("win");
  ...
  return <Stage eyebrow={tr2({ uz: "Sinov · saytni top", ru: "Испытание · найди сайт" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={phase !== "win"} label={phase === "win" ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Saytni ekranga keltiring", ru: "Выведите сайт на экран" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <><span className="italic" style={{ color: T.accent }}>{target.domain}</span> ni topib, ekranga keltira olasizmi?</>, ru: <>Сможете найти <span className="italic" style={{ color: T.accent }}>{target.domain}</span> и вывести его на экран?</> })}</h2></div>
    ...
    /* Inventar — paket nimani tashiydi */
          ...
          <div className="net-hud zb-gap">
            <span className={`net-slot ${hasIP ? "on" : ""}`}><span className="net-slot-ic">📇</span>{hasIP ? <span className="net-hud-ip">{target.ip}</span> : tr2({ uz: "IP-manzil yo'q", ru: "Нет IP-адреса" })}</span>
            <span className={`net-slot ${hasPage ? "on" : ""}`}><span className="net-slot-ic">📄</span>{hasPage ? tr2({ uz: "Sahifa ✓", ru: "Страница ✓" }) : tr2({ uz: "Sahifa yo'q", ru: "Нет страницы" })}</span>
            <span className="net-moves">{tr2({ uz: "Qadam:", ru: "Шагов:" })} {moves}</span>
          </div>
    ...
    return <button key={k} className={`net-node ${n.kind} ${isCur ? "cur" : ""} ${isVisited ? "visited" : ""} ${reachable ? "reachable" : ""} ${matchNode === k ? "match" : ""} ${shakeNode === k ? "shake" : ""}`} style={{ left: `${n.pos[0]}%`, top: `${n.pos[1]}%` }} onClick={() => go(k)} disabled={!reachable}>
                  <span className="net-node-ic">{n.ic}</span>
                  <span className="net-node-l">{tr2(n.l)}</span>
                  {n.kind === "server" ? <span className="net-ip">{n.ip}</span> : <span className="net-node-note">{tr2(n.note)}</span>}
                </button>;
  })}
    ...
    /* Xabar / maslahat + boshqaruv */
          ...
          {msg ? <p className={`net-msg ${msg.type}`} key={msg.t}>{msg.t}</p> : phase === "play" && <p className="net-hint">{hasIP ? tr2({ uz: "📇 Kartangizdagi IP'ga MOS serverni tanlang — raqamlarni oxirigacha solishtiring!", ru: "📇 Выберите сервер, СОВПАДАЮЩИЙ с IP на вашей карточке — сравните номера до конца!" }) : tr2({ uz: "🟢 yonayotgan DNS tugunini bosing", ru: "🟢 нажмите на подсвеченный узел DNS" })}</p>}
          <div className="dlv-run-row">
            <button className="dlv-mini" onClick={restart} disabled={moving || cur === "you" && !hasIP && !hasPage}>{tr2({ uz: "↺ Boshidan", ru: "↺ Сначала" })}</button>
            {phase === "win" && <button className="btn" onClick={replay} style={{ marginLeft: "auto" }}>{tr2({ uz: "↻ Boshqa sayt bilan yana", ru: "↻ Ещё раз с другим сайтом" })}</button>}
          </div>
        </div>
        </Zoomable>

        {phase === "win" && <div ref={winRef} className="dlv-result fade-step">
            <Confetti />
            <p className="dlv-open-lbl">{tr2({ uz: <>🎉 Topdingiz! Brauzer <b>{target.name}</b>'ni ekraningizda ochdi</>, ru: <>🎉 Нашли! Браузер открыл <b>{target.name}</b> на вашем экране</> })}</p>
            ...
            <div className="dlv-win">
              ...
              <p className="dlv-win-b"><b className="dlv-win-h">{moves <= 3 ? tr2({ uz: "Aniq nishonga!", ru: "Точно в цель!" }) : tr2({ uz: "Topdingiz!", ru: "Нашли!" })}</b> {tr2({ uz: <>🗂️ DNS <b>{target.domain}</b> ning manzilini berdi — <b className="mono">{target.ip}</b>. Har bir kompyuterning <b>o'z IP-manzili</b> bor; aynan shu raqam internetda <b>bitta serverni</b> aniqlaydi. Bitta raqam farq qilsa ham — butunlay boshqa kompyuter! Shuning uchun to'g'ri IP = to'g'ri sayt.</>, ru: <>🗂️ DNS дал адрес сайта <b>{target.domain}</b> — <b className="mono">{target.ip}</b>. У каждого компьютера <b>свой IP-адрес</b>; именно этот номер определяет в интернете <b>один сервер</b>. Отличается хоть одна цифра — уже совсем другой компьютер! Поэтому правильный IP = правильный сайт.</> })}</p>
            </div>
          </div>}
      </div>
    </Stage>;
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [stage, setStage] = useState3(storedAnswer ? "fixed" : "idle");
  ...
  const fail = () => {
    ...
    if (!audio.muted) setTimeout(() => {
      const e = getAudioEngine();
      if (e && !audio.muted) e.pushOneOff(`Mana muammo: DNS "youtub.com" ni topolmadi, chunki bunday domen yo'q. "e" harfi tushib qolgan.`);
    }, 300);
  };
  const fix = () => {
    setStage("fixed");
    ...
    if (!audio.muted) setTimeout(() => {
      const e = getAudioEngine();
      if (e && !audio.muted) e.pushOneOff(`Tuzatildi! Endi youtube.com to'g'ri — DNS IP'ni topdi va sayt ochildi.`);
    }, 300);
  };
  return <Stage eyebrow={tr2({ uz: "Debugging", ru: "Отладка" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : stage === "failed" ? { uz: "Endi tuzating", ru: "Теперь исправьте" } : { uz: "Ochib ko'ring", ru: "Попробуйте открыть" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Sayt ochilmadi — <span className="italic" style={{ color: T.accent }}>nega</span>?</>, ru: <>Сайт не открылся — <span className="italic" style={{ color: T.accent }}>почему</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Ba'zan sayt ochilmaydi — qo'rqinchli emas. Eng ko'p sabab: <b style={{ color: T.ink }}>domen xato yozilgan</b>, shuning uchun DNS uni topolmaydi. Mana <span className="mono">youtub.com</span> — bitta harf yetishmayapti. Avval ochib ko'ring, keyin xatoni o'zingiz toping.</>, ru: <>Иногда сайт не открывается — это не страшно. Чаще всего причина: <b style={{ color: T.ink }}>домен написан с ошибкой</b>, поэтому DNS его не находит. Вот <span className="mono">youtub.com</span> — не хватает одной буквы. Сначала попробуйте открыть, потом сами найдите ошибку.</> })}</Mentor>
        <div className="split">
          <div className="col">
            ...
            {stage === "idle" && <button className="btn" style={{ alignSelf: "flex-start" }} onClick={fail}>{tr2({ uz: "↵ Saytni ochish", ru: "↵ Открыть сайт" })}</button>}
            {stage === "failed" && <button className="btn fade-step" style={{ alignSelf: "flex-start" }} onClick={fix}>{tr2({ uz: `🔧 Yetishmagan "e" harfini qo'shish`, ru: "🔧 Добавить недостающую букву «e»" })}</button>}
            {stage === "fixed" && <p className="mono small fade-step" style={{ color: T.success, margin: 0, fontWeight: 600 }}>{tr2({ uz: "✓ youtube.com — endi to'g'ri!", ru: "✓ youtube.com — теперь верно!" })}</p>}
          </div>
          <div className="col">
            {stage === "idle" && <div className="hint"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: `Manzilga diqqat bilan qarang. "Saytni ochish"ni bosib, nima bo'lishini ko'ring.`, ru: "Внимательно посмотрите на адрес. Нажмите «Открыть сайт» и посмотрите, что будет." })}</p></div>}
            {stage === "failed" && <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.accent }}>{tr2({ uz: "❌ DNS: domen topilmadi", ru: "❌ DNS: домен не найден" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>DNS <span className="mono">youtub.com</span> ni topolmadi — bunday domen yo'q. Oxirida <b>"e"</b> harfi yetishmayapti (qizil katak ⌶). Chap tugma bilan qo'shing →</>, ru: <>DNS не нашёл <span className="mono">youtub.com</span> — такого домена нет. В конце не хватает буквы <b>«e»</b> (красная клетка ⌶). Добавьте её кнопкой слева →</> })}</p></div>}
            {stage === "fixed" && <><Preview title="youtube.com" minH={130}><SiteMock site={siteInfo("youtube.com")} /></Preview><div className="takeaway fade-step"><div className="ta-bulb">🛠️</div><p className="ta-h">{tr2({ uz: "Xatoni topdingiz va tuzatdingiz!", ru: "Вы нашли и исправили ошибку!" })}</p><p className="ta-sub">{tr2({ uz: "Domen to'g'ri bo'lsa — DNS topadi, sayt ochiladi", ru: "Если домен верный — DNS находит, сайт открывается" })}</p></div></>}
          </div>
        </div>
      </div>
    </Stage>;
...
var Screen15 = (props) => <QuestionScreen
  {...props}
  scope="final"
  eyebrow={{ uz: "Yakuniy savol", ru: "Итоговый вопрос" }}
  ...
  options={[{ uz: "Server → DNS → Brauzer → Ekran", ru: "Сервер → DNS → Браузер → Экран" }, { uz: "DNS → Brauzer → Server → Ekran", ru: "DNS → Браузер → Сервер → Экран" }, { uz: "Brauzer → DNS → Server → Ekran", ru: "Браузер → DNS → Сервер → Экран" }, { uz: "Brauzer → Server → DNS → Ekran", ru: "Браузер → Сервер → DNS → Экран" }]}
  correctIdx={2}
...
var ScreenPodium = ({ screen, answers, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Kim <span className="italic" style={{ color: T.accent }}>g'olib</span>?</>, ru: <>Кто <span className="italic" style={{ color: T.accent }}>победил</span>?</> })}</h2></div>
    ...
    return <div key={rank} className={`pod-col pod-${rank + 1} ${b && live.playerId === b.id ? "me" : ""}`}>
            ...
            {myIdx >= 0 && <p className="pod-my fade-up">{tr2({ uz: "Siz —", ru: "Вы —" })} <b>{myIdx + 1}-{tr2({ uz: "o'rin", ru: "место" })}</b> ({board[myIdx].okCount}/{totalQ} {tr2({ uz: "to'g'ri", ru: "верно" })})</p>}
    ...
    return <div key={q} className="qstat-row">
              ...
              {live.mode === "mentor" && <p className="small" style={{ margin: "10px 0 0", color: T.ink2 }}>{tr2({ uz: "⚠️ belgili savollar — sinf qiynalgan mavzular. Qayta tushuntirish tavsiya etiladi.", ru: "⚠️ отмеченные вопросы — темы, с которыми класс справился хуже. Рекомендуется объяснить заново." })}</p>}
            </div>
          </>}
      </div>
    </Stage>;
...
var INLINE_KEYS = { s4: 1, s5b: 2, s9: 3, s12: 1, s15: 2 };
var QUIZ_BANK = [
  { q: { uz: "youtube.com yozib Enter bosdingiz. Brauzer ENG BIRINCHI kimga murojaat qiladi?", ru: "Вы набрали youtube.com и нажали Enter. К кому браузер обращается ПЕРВЫМ?" }, opts: [{ uz: "Serverga", ru: "К серверу" }, { uz: "DNS'ga", ru: "К DNS" }, { uz: "Ekranga", ru: "К экрану" }, { uz: "Wi-Fi routerga", ru: "К Wi-Fi роутеру" }], correct: 1 },
  { q: { uz: "DNS'ning asl vazifasi qaysi qatorda?", ru: "В какой строке настоящая задача DNS?" }, opts: [{ uz: "Saytni ekranga chizish", ru: "Нарисовать сайт на экране" }, { uz: "Sahifalarni saqlash", ru: "Хранить страницы" }, { uz: "Domenni IP raqamga aylantirish", ru: "Превратить домен в IP-адрес" }, { uz: "Internet tezligini oshirish", ru: "Повысить скорость интернета" }], correct: 2 },
  { q: { uz: "142.250.190.78 va 142.250.190.87 — bular…", ru: "142.250.190.78 и 142.250.190.87 — это…" }, opts: [{ uz: "Ikki XIL kompyuter", ru: "Два РАЗНЫХ компьютера" }, { uz: "Bitta kompyuter", ru: "Один компьютер" }, { uz: "Bitta saytning ikki nomi", ru: "Два имени одного сайта" }, { uz: "Xato yozilgan bitta IP", ru: "Один IP, записанный с ошибкой" }], correct: 0 },
  { q: { uz: "Server brauzerga aniq nima yuboradi?", ru: "Что именно сервер отправляет браузеру?" }, opts: [{ uz: "HTML kodni", ru: "HTML-код" }, { uz: "Tayyor suratni", ru: "Готовую картинку" }, { uz: "IP manzilni", ru: "IP-адрес" }, { uz: "Yangi brauzerni", ru: "Новый браузер" }], correct: 0 },
  { q: { uz: "«Inter-net» so'zining asl ma'nosi nima?", ru: "Что на самом деле означает слово «Интер-нет»?" }, opts: [{ uz: "Tezkor aloqa", ru: "Быстрая связь" }, { uz: "Simsiz aloqa", ru: "Беспроводная связь" }, { uz: "Butunjahon kompyuteri", ru: "Всемирный компьютер" }, { uz: "Tarmoqlararo", ru: "Между сетями" }], correct: 3 },
  { q: { uz: "youtub.com (bir harf kam!) yozsangiz nima bo'ladi?", ru: "Что будет, если набрать youtub.com (одной буквы не хватает)?" }, opts: [{ uz: "YouTube baribir ochiladi", ru: "YouTube всё равно откроется" }, { uz: "Kompyuter sekinlashadi", ru: "Компьютер замедлится" }, { uz: "Butun internet uziladi", ru: "Весь интернет отключится" }, { uz: "DNS saytni topolmaydi", ru: "DNS не найдёт сайт" }], correct: 3 },
  { q: { uz: "Sayt «yashaydigan», DOIM yoniq turadigan kompyuter?", ru: "Компьютер, где «живёт» сайт и который ВСЕГДА включён?" }, opts: [{ uz: "Router", ru: "Роутер" }, { uz: "Server", ru: "Сервер" }, { uz: "Brauzer", ru: "Браузер" }, { uz: "Planshet", ru: "Планшет" }], correct: 1 },
  { q: { uz: "Qaysi biri DOMEN?", ru: "Что из этого — ДОМЕН?" }, opts: [{ uz: "142.250.190.78", ru: "142.250.190.78" }, { uz: "Chrome", ru: "Chrome" }, { uz: "wikipedia.org", ru: "wikipedia.org" }, { uz: "HTML", ru: "HTML" }], correct: 2 },
  { q: { uz: "HTML kodni o'qib, chiroyli sahifaga aylantiradigan kim?", ru: "Кто читает HTML-код и превращает его в красивую страницу?" }, opts: [{ uz: "Server", ru: "Сервер" }, { uz: "DNS", ru: "DNS" }, { uz: "Domen", ru: "Домен" }, { uz: "Brauzer", ru: "Браузер" }], correct: 3 },
  { q: { uz: "So'rovning TO'G'RI tartibi qaysi?", ru: "Какой ПРАВИЛЬНЫЙ порядок запроса?" }, opts: [{ uz: "Brauzer → DNS → Server → Ekran", ru: "Браузер → DNS → Сервер → Экран" }, { uz: "DNS → Brauzer → Ekran → Server", ru: "DNS → Браузер → Экран → Сервер" }, { uz: "Server → Brauzer → DNS → Ekran", ru: "Сервер → Браузер → DNS → Экран" }, { uz: "Brauzer → Server → DNS → Ekran", ru: "Браузер → Сервер → DNS → Экран" }], correct: 0 },
  { q: { uz: "coddycamp.uz'dagi «.uz» nimani bildiradi?", ru: "Что означает «.uz» в coddycamp.uz?" }, opts: [{ uz: "Saytning nomi", ru: "Название сайта" }, { uz: "O'zbekiston zonasi", ru: "Зона Узбекистана" }, { uz: "Parol", ru: "Пароль" }, { uz: "Serverning IP'si", ru: "IP сервера" }], correct: 1 },
  { q: { uz: "DNS «telefon kitobi» bo'lsa, undagi ism va raqam nima?", ru: "Если DNS — «телефонная книга», что в ней имя и номер?" }, opts: [{ uz: "Ism=IP · Raqam=domen", ru: "Имя=IP · Номер=домен" }, { uz: "Ism=brauzer · Raqam=server", ru: "Имя=браузер · Номер=сервер" }, { uz: "Ism=domen · Raqam=IP", ru: "Имя=домен · Номер=IP" }, { uz: "Ism=sayt · Raqam=parol", ru: "Имя=сайт · Номер=пароль" }], correct: 2 }
];
  ...
  return <div className="qz-arena">
    ...
    /* ===== LOBBY ===== */
      ...
      {phase === "lobby" && <div className="qz-view fade-step">
          <CsWordmark />
          <p className="qz-sub" style={{ marginTop: -4 }}>{tr2({ uz: "Tezroq to'g'ri bossangiz — ko'proq ball. Ketma-ket to'g'ri javoblar 🔥 bonus beradi!", ru: "Чем быстрее верно ответите — тем больше баллов. Ответы подряд дают 🔥 бонус!" })}</p>
    ...
    return <button key={i} className={`qz-tile ${my ? pickedThis ? "picked" : "faded" : ""}`} style={{ background: QUIZ_COLORS[i] }} disabled={isMentor || !!my} onClick={() => answer(i)}>
                  ...
                  <span className="qz-opt">{tr2(o)}</span>
    ...
    return <div key={i} className={`qz-tile rv ${win ? "win" : "lose"} ${pickedThis ? "picked" : ""}`} style={{ background: QUIZ_COLORS[i] }}>
          ...
          {!isMentor && <div className={`qz-res ${my?.correct ? "good" : "bad"}`}>
              {my?.correct ? <><span className="qz-res-pts">+{myPtsFor(qi)}</span><span className="qz-res-t">{tr2({ uz: "ball", ru: "баллов" })}{streakUpTo(qi) >= 2 ? ` · 🔥 x${streakUpTo(qi)} streak` : ""}</span></> : <span className="qz-res-t">{my ? tr2({ uz: "Adashdingiz — 0 ball. Keyingisida olasiz! 💪", ru: "Неверно — 0 баллов. В следующий раз получится! 💪" }) : tr2({ uz: "Vaqt tugadi — 0 ball. Tezroq bo'ling! ⏱", ru: "Время вышло — 0 баллов. Будьте быстрее! ⏱" })}</span>}
              {!solo && myRank >= 0 && <span className="qz-res-rank">{tr2({ uz: "Siz hozir:", ru: "Вы сейчас:" })} {myRank + 1}-{tr2({ uz: "o'rin", ru: "место" })}</span>}
            </div>}
    ...
    /* ===== YAKUN — PODIUM ===== */
      ...
      {phase === "done" && <div className="qz-view fade-step">
          ...
          {solo ? <div className="qz-solo-res">
              ...
              <p className="qz-sub">{tr2({ uz: "ball", ru: "баллов" })} · {soloScore.ok}/{QUIZ_BANK.length} {tr2({ uz: "to'g'ri", ru: "верно" })}{soloScore.maxStreak >= 2 ? ` · ${tr2({ uz: "eng uzun streak", ru: "самая длинная серия" })} 🔥x${soloScore.maxStreak}` : ""}</p>
    ...
    return <div key={rank} className={`qz-pod-col p${rank + 1} ${b && b.id === live.playerId ? "me" : ""}`}>
              ...
              {myRank >= 0 && <p className="qz-mypl">{tr2({ uz: "Siz —", ru: "Вы —" })} <b>{myRank + 1}-{tr2({ uz: "o'rin", ru: "место" })}</b> · {board[myRank].pts} {tr2({ uz: "ball", ru: "баллов" })}</p>}
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  const RECAP = [{ uz: "Internet — qurilmalarni bog'lovchi tarmoq", ru: "Интернет — сеть, связывающая устройства" }, { uz: "Brauzer — saytni ochib beruvchi dastur", ru: "Браузер — программа, открывающая сайт" }, { uz: "Domen — saytning manzili (youtube.com)", ru: "Домен — адрес сайта (youtube.com)" }, { uz: "IP — kompyuterlar uchun raqamli manzil", ru: "IP — числовой адрес для компьютеров" }, { uz: "DNS — domenni IP'ga aylantiradi", ru: "DNS — превращает домен в IP" }, { uz: "Server — saytni saqlaydi va qaytaradi", ru: "Сервер — хранит и отдаёт сайт" }];
  const HOMEWORK2 = [{ b: { uz: "Kuzating", ru: "Понаблюдайте" }, t: { uz: "— sevimli saytingiz domeni nima?", ru: "— какой домен у вашего любимого сайта?" } }, { b: { uz: "Tasavvur qiling", ru: "Представьте" }, t: { uz: "— sayt brauzeringizga qanday yo'l bosib kelishini", ru: "— какой путь сайт проходит до вашего браузера" } }, { b: { uz: "Tushuntiring", ru: "Объясните" }, t: { uz: "— do'stingizga internet qanday ishlashini", ru: "— другу, как работает интернет" } }];
  const GLOSSARY = [{ b: "Internet", t: { uz: "— tarmoqlar tarmog'i", ru: "— сеть сетей" } }, { b: "Brauzer", t: { uz: "— saytni ochuvchi dastur", ru: "— программа, открывающая сайт" } }, { b: "Domen", t: { uz: "— sayt manzili", ru: "— адрес сайта" } }, { b: "IP", t: { uz: "— raqamli manzil", ru: "— числовой адрес" } }, { b: "DNS", t: { uz: "— domen → IP", ru: "— домен → IP" } }, { b: "Server", t: { uz: "— saytni saqlovchi kompyuter", ru: "— компьютер, хранящий сайт" } }, { b: "So'rov", t: { uz: "— brauzer → DNS → server → ekran", ru: "— браузер → DNS → сервер → экран" } }];
  const correct = SCORED_IDX.filter((i) => answers[i]?.correct).length;
  const total = SCORED_IDX.length;
  const PASSED = (total ? correct / total : 0) >= 0.6;
  const [open, setOpen] = useState3(false);
  const [showDone, setShowDone] = useState3(false);
  return <Stage eyebrow={tr2({ uz: "Tayyor", ru: "Готово" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={() => setShowDone(true)} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "🎉 Darsni yakunlash", ru: "🎉 Завершить урок" })}</button></>}>
      <div className="screen">
        {PASSED && <Confetti />}
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "Kirish darsi tugadi", ru: "Вводный урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Internet <span className="italic" style={{ color: T.accent }}>qanday ishlashini</span> bilib oldingiz.</>, ru: <>Вы узнали, <span className="italic" style={{ color: T.accent }}>как работает</span> интернет.</> })}</h2>{
    ...
    hint={studentWait ? "⏳ Mentorni kuting" : void 0}
  />
        </div>
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>{tr2({ uz: "🔎 Uyga vazifa", ru: "🔎 Домашнее задание" })}</div><p className="body" style={{ margin: "0 0 10px", color: T.ink }}>{tr2({ uz: "Internetni hayotda kuzating:", ru: "Понаблюдайте за интернетом в жизни:" })}</p><ul>{HOMEWORK2.map((h, i) => <li key={i}><b>{tr2(h.b)}</b> <span className="t">{tr2(h.t)}</span></li>)}</ul><p className="hw-note">{tr2({ uz: "Javoblaringizni platformadagi vazifa-savoliga yozib yuborasiz.", ru: "Ответы отправите в задание на платформе." })}</p></div>}
    ...
    return <div key={id} className={`ach-badge ${got ? "got" : "locked"}`} title={tr2(a.desc)}>
      ...
      {showDone && <div className="fin-backdrop" onClick={() => setShowDone(false)}>
          <Confetti />
          <div className="fin-card" onClick={(e) => e.stopPropagation()}>
            <button className="fin-x" onClick={() => setShowDone(false)} aria-label={tr2({ uz: "Yopish", ru: "Закрыть" })}>✕</button>
            <div className="fin-emoji">🎉</div>
            <h3 className="fin-h">{tr2({ uz: "Tabriklaymiz!", ru: "Поздравляем!" })}</h3>
            <p className="fin-sub">{tr2({ uz: "Internet darsini muvaffaqiyatli tugatdingiz. Endi sayt sizgacha qanday yetib kelishini bilasiz.", ru: "Вы успешно завершили урок об интернете. Теперь вы знаете, как сайт доходит до вас." })}</p>
            <button className="btn-white-accent" onClick={onFinish} style={{ padding: "12px 32px", fontSize: 15 }}>{tr2({ uz: "Tamom", ru: "Готово" })}</button>
          </div>
        </div>}
...
function DragDropOrder({ items, hints, onSolved, onWrong }) {
  ...
  return <div className={`dd fade-up ${pristine ? "dd-hint-on" : ""}`}>
      <div className="dd-slots">
        {slots.map((sid, i) => <div key={i} ref={(el) => slotRefs.current[i] = el} className={`dd-slot ${sid ? "filled" : ""} ${solved && sid ? "ok" : ""} ${wrong && sid && sid !== order[i] ? "bad" : ""}`}>
            ...
            {sid ? <button className="dd-chip in" onPointerDown={(e) => down(e, sid, i)}>{tr2(byId[sid].label)}</button> : <span className="dd-hint">{hints ? tr2(hints[i]) : tr2({ uz: "bu yerga joylang", ru: "поместите сюда" })}</span>}
          </div>)}
      </div>
      <div className="dd-pool">
        {pool.length === 0 && !solved && <span className="dd-pool-empty">{tr2({ uz: "Tartib xato — bo'lakni bosib qaytaring va qayta joylang", ru: "Порядок неверный — верните часть нажатием и разместите заново" })}</span>}
        {pool.map((id) => <button key={id} className="dd-chip" onPointerDown={(e) => down(e, id, "pool")}>{tr2(byId[id].label)}</button>)}
      </div>
      {solved && <div className="dd-done">{tr2({ uz: "✓ To'g'ri! So'rov aynan shu tartibda yuradi.", ru: "✓ Верно! Запрос идёт именно в таком порядке." })}</div>}
      {wrong && !solved && <div className="dd-wrong">{tr2({ uz: "⚠️ Tartib xato — qayta joylang.", ru: "⚠️ Порядок неверный — разместите заново." })}</div>}
    </div>;
}
var REQ_ORDER_PIECES = [
...
var ScreenReqOrder = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Mustahkamlash", ru: "Закрепление" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Tartibni yig'ing", ru: "Соберите порядок" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>So'rov qanday <span className="italic" style={{ color: T.accent }}>tartibda</span> yuradi?</>, ru: <>В каком <span className="italic" style={{ color: T.accent }}>порядке</span> идёт запрос?</> })}</h2></div>
        ...
        <div className="sk-buildbox" style={{ maxWidth: 480 }}>
          <p className="eyebrow" style={{ color: T.accent, margin: "0 0 10px" }}>{tr2({ uz: "🧲 Bo'laklarni sudrab yoki bosib joylang", ru: "🧲 Разместите части перетаскиванием или нажатием" })}</p>
          <DragDropOrder items={REQ_ORDER_PIECES} hints={[{ uz: "so'rov shu yerdan boshlanadi", ru: "запрос начинается здесь" }, { uz: "manzilni (IP) topadi", ru: "находит адрес (IP)" }, { uz: "sahifani beradi", ru: "отдаёт страницу" }, { uz: "chizib ko'rsatadi", ru: "рисует на экране" }]} onSolved={() => setDone(true)} onWrong={() => {
    if (achMiss && !done) achMiss.miss(screen);
  }} />
...
function Flashcards({ cards }) {
  ...
  if (!card) return <div className="fc-done fade-up"><span className="fc-done-emoji">🎉</span><p className="fc-done-h">{tr2({ uz: "Hammasini bilasiz!", ru: "Вы знаете всё!" })}</p><p className="fc-done-s">{total}/{total} {tr2({ uz: "atama yodlandi", ru: "терминов запомнено" })}</p><button className="fc-btn ghost" onClick={restart}>{tr2({ uz: "↻ Qaytadan takrorlash", ru: "↻ Повторить заново" })}</button></div>;
...
var TOUR = {
  learner: [
    { sel: '[data-tour="next"]', ic: "▶", h: { uz: "Oldinga o'tish", ru: "Двигаться вперёд" }, body: { uz: <>Tayyor bo'lsangiz, <b>«Davom etish»</b> bilan keyingi qadamga o'tasiz. Yonidagi <b>«Orqaga»</b> bilan qaytasiz.</>, ru: <>Когда готовы, кнопкой <b>«Продолжить»</b> переходите к следующему шагу. Рядом <b>«Назад»</b> вернёт обратно.</> } },
    { sel: '[data-tour="mentor"]', ic: "🧑‍🏫", h: { uz: "Mentor yordami", ru: "Помощь ментора" }, body: { uz: <>Har ekranda mentor nima qilishni tushuntiradi — avval uni <b>o'qing</b>, keyin bajaring.</>, ru: <>На каждом экране ментор объясняет, что делать — сначала <b>прочитайте</b>, затем выполняйте.</> } },
    { sel: '[data-tour="progress"]', ic: "📊", h: { uz: "Progress chizig'i", ru: "Полоса прогресса" }, body: { uz: <>Bu chiziq dars <b>qancha qolganini</b> ko'rsatadi — to'lgani sari yakunlaysiz.</>, ru: <>Эта полоса показывает, <b>сколько осталось</b> — чем полнее, тем ближе финал.</> } },
  ...
  mentor: [
    { sel: '[data-tour="live"]', ic: "🔢", h: { uz: "Qo'shilish kodi", ru: "Код подключения" }, body: { uz: <>O'quvchilar shu <b>PIN kod</b> bilan qo'shiladi. Kim qo'shilganini shu yerda ko'rasiz.</>, ru: <>Ученики подключаются по этому <b>PIN-коду</b>. Здесь видно, кто присоединился.</> } },
    { sel: '[data-tour="next"]', ic: "▶", h: { uz: "Siz boshqarasiz", ru: "Вы управляете" }, body: { uz: <><b>«Davom etish»</b> bosganingizda, barcha o'quvchilar birga oldinga o'tadi.</>, ru: <>Когда вы нажмёте <b>«Продолжить»</b>, все ученики перейдут вперёд вместе.</> } },
...
function TourGuide({ role, onClose }) {
  ...
  const vw = typeof window !== "undefined" ? window.innerWidth : 1e3;
  ...
  if (rect) {
    ...
    callStyle = below ? { top: rect.bottom + 16, left: cx, transform: "translateX(-50%)", width: CW } : { top: Math.max(12, rect.top - 16), left: cx, transform: "translate(-50%,-100%)", width: CW };
  }
  return <div className="tg-root">
      {rect ? <div className="tg-hole" style={{ top: rect.top - 7, left: rect.left - 7, width: rect.width + 14, height: rect.height + 14 }} /> : <div className="tg-dim" />}
      <div className="tg-call" style={callStyle}>
        <div className="tg-head"><span className="tg-ic">{step.ic}</span><span className="tg-h">{tr2(step.h)}</span><span className="tg-count">{i + 1}/{steps.length}</span></div>
        <p className="tg-body">{tr2(step.body)}</p>
        <div className="tg-nav">
          <button className="tg-skip" onClick={onClose}>{tr2({ uz: "O'tkazib yuborish", ru: "Пропустить" })}</button>
          <div className="tg-navr">
            {i > 0 && <button className="tg-btn ghost" onClick={() => setI(i - 1)} aria-label={tr2({ uz: "Oldingi", ru: "Предыдущий" })}>←</button>}
            <button className="tg-btn go" onClick={next}>{last ? tr2({ uz: "Boshladik!", ru: "Начали!" }) : tr2({ uz: "Keyingisi →", ru: "Дальше →" })}</button>
          </div>
        </div>
      </div>
    </div>;
}
function HtmlLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  const [practice, setPractice] = useState3(!!saved?.firstPass);
  ...
  const [onboard, setOnboard] = useState3(false);
  const onboardShownRef = useRef3(false);
  ...
  useEffect4(() => {
    if (live.mode !== "choosing" && !onboardShownRef.current) {
      ...
      let show = true;
      try {
        if (localStorage.getItem("inetOnboarded_" + onboardRole)) show = false;
      } catch {
      }
      if (show) {
  ...
  const closeOnboard = () => {
    try {
      localStorage.setItem("inetOnboarded_" + onboardRole, "1");
    } catch {
    }
    setOnboard(false);
  };
  ...
  return <LangContext.Provider value={lang}>
      <style>{`
        ...
        .net-msg.net-msg { font-family: 'Manrope'; font-weight: 600; font-size: clamp(13px,1.7vw,15px); text-align: center; margin: 0; padding: 10px 14px; border-radius: 10px; animation: fade-step 0.3s ease-out; }
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        ...
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={{ uz: "Internet darsi", ru: "Урок об интернете" }} /> : <>
