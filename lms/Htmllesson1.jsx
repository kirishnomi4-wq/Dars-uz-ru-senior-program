...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Yozing", ru: "Напишите" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={isMentorLive ? false : !correct} label={isMentorLive ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : correct ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Yopuvchi tegni yozing", ru: "Напишите закрывающий тег" })} onClick={onNext} /></>}>
      <div className="screen">
        ...
        <div className="split">
          <div className="col"><div className="yz-card fade-up delay-2"><div className="yz-line"><span className="yz-code"><span className="t-tag">&lt;h1&gt;</span>{tr3({ uz: "Salom!", ru: "Привет!" })}</span>{!correct ? <input className="yz-input" value={val} onChange={(e) => setVal(e.target.value)} placeholder={tr3({ uz: "yopuvchi teg…", ru: "закрывающий тег…" })} spellCheck={false} /> : <span className="yz-code yz-done"><span className="t-tag">&lt;/h1&gt;</span></span>}</div>{!correct && <p className="yz-hint">{touched ? tr3({ uz: "Deyarli! Yopuvchi teg / belgisi bilan boshlanadi: </h1>", ru: "Почти! Закрывающий тег начинается со знака /: </h1>" }) : tr3({ uz: "Maslahat: avval / yozing, keyin teg nomi va >", ru: "Подсказка: сначала напишите /, потом имя тега и >" })}</p>}{correct && <p className="yz-ok">✓ {tr3({ uz: "To'g'ri! Endi element yopildi:", ru: "Верно! Теперь элемент закрыт:" })} &lt;h1&gt;...&lt;/h1&gt;</p>}</div></div>
