...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr3({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up" style={{ maxWidth: 760 }}>{tr3({ uz: <>Bu sayt nimadan <span className="italic" style={{ color: T.accent }}>yasalgan</span>?</>, ru: <>Из чего <span className="italic" style={{ color: T.accent }}>сделан</span> этот сайт?</> })}</h1>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [val, setVal] = useState4(storedAnswer?.studentAnswer ?? "");
...
function HtmlLesson({ lang: langProp, onFinished, onPractice, liveToken }) {
  ...
  const runPractice = (entry, fromScreen) => {
    ...
    if (typeof onPractice === "function") {
      Promise.resolve(onPractice(entry.task)).then(done);
    } else {
  ...
  const openHomeworkPractice = () => {
    ...
    if (typeof onPractice === "function") {
      Promise.resolve(onPractice(entry.task)).catch(() => {
      });
      return;
    }
