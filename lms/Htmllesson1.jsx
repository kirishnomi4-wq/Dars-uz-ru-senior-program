...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const [view, setView] = useState2("site");
  const OPTS = [
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up" style={{ maxWidth: 760 }}>{tr2({ uz: <>Bu sayt nimadan <span className="italic" style={{ color: T.accent }}>yasalgan</span>?</>, ru: <>Из чего <span className="italic" style={{ color: T.accent }}>сделан</span> этот сайт?</> })}</h1>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [revealed, setRevealed] = useState2(!!storedAnswer);
  const answerRef = useRef2(null);
  const xulosaRef = useRef2(null);
  const [solved, setSolved] = useState2(!!storedAnswer);
  const [program, setProgram] = useState2([]);
  const [pos, setPos] = useState2(RG_START);
  const [phase, setPhase] = useState2("idle");
  const [jumping, setJumping] = useState2(false);
  const [active, setActive] = useState2(-1);
  const botTimer = useRef2(null);
  const running = phase === "running";
  const isCactus = (c) => RG_CACTI.indexOf(c) !== -1;
  useEffect2(() => () => clearTimeout(botTimer.current), []);
  useEffect2(() => {
    if (phase === "win") {
      setSolved(true);
...
var Screen3 = ({ screen, onNext, onPrev }) => {
  ...
  const [name, setName] = useState2("Aziz");
...
function DebugChallenge({ lines, fixed, explain, onSolved }) {
  ...
  const [picked, setPicked] = useState2(-1);
  const [wrongIdx, setWrongIdx] = useState2(-1);
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [clicked, setClicked] = useState2(/* @__PURE__ */ new Set());
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [wrapped, setWrapped] = useState2(false);
  const [active, setActive] = useState2(null);
  const [dbgDone, setDbgDone] = useState2(!!storedAnswer);
  const isNarrow = useIsMobile(768);
  const done = dbgDone;
  const tap = (k) => {
    if (!wrapped) return;
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const prevCollapsedRef = useRef2(mctx.collapsed);
  useEffect2(() => {
    if (isNarrow && done) {
...
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [fmt, setFmt] = useState2({ imtihon: { b: false, i: false }, albatta: { b: false, i: false } });
  const [sel, setSel] = useState2(null);
  const [usedB, setUsedB] = useState2(false);
  const [usedI, setUsedI] = useState2(false);
...
var Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [page, setPage] = useState2("bosh");
  const [jumped, setJumped] = useState2(false);
  const done = jumped;
  const [revealed, setRevealed] = useState2(false);
  const [travel, setTravel] = useState2(null);
  const travelKey = useRef2(0);
  const travelTimer = useRef2(null);
  const mctx = useContext(MentorCtx) || {};
  const isNarrow = useIsMobile(768);
  const showResult = !isNarrow || revealed;
  useEffect2(() => () => clearTimeout(travelTimer.current), []);
  const go = (to) => {
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  const HOMEWORK = [{ b: tr2({ uz: "Sarlavha (h1)", ru: "Заголовок (h1)" }), t: tr2({ uz: "— ismingiz", ru: "— ваше имя" }) }, { b: tr2({ uz: "2–3 paragraf", ru: "2–3 абзаца" }), t: tr2({ uz: "— o'zingiz haqingizda", ru: "— о себе" }) }, { b: tr2({ uz: "Ro'yxat", ru: "Список" }), t: tr2({ uz: "— sevimli mashg'ulotlaringiz", ru: "— ваши любимые занятия" }) }, { b: tr2({ uz: "Havola", ru: "Ссылка" }), t: tr2({ uz: "— sevimli saytingizga", ru: "— на любимый сайт" }) }];
...
function HtmlLesson({ lang: langProp, onFinished, onPractice }) {
  ...
  const [onboard, setOnboard] = useState2(false);
  const onboardShownRef = useRef2(false);
