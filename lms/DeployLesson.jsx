...
var ACH_TRIGGERS = { s2: "hostpick", s6: "filefind", s10: "shipit" };
function AchCounter() {
...
var RECAPS = {
  ...
  10: {
    ...
    cards: [
      {
        ic: "🧭",
        h: { uz: "To'rt qadam, aniq tartibda", ru: "Четыре шага, строгий порядок" },
      ...
      {
        ic: "⬆️",
        h: { uz: "Nega avval GitHub?", ru: "Почему сначала GitHub?" },
      ...
      {
        ic: "🔗",
        h: { uz: "Havola — eng oxirida", ru: "Ссылка — в самом конце" },
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} navContent={<NavNext optionalLive disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
    ...
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null} onClick={() => pick(o.id)}>
            ...
            {picked !== null && <p className="hook-ack fade-step">{tr2({ uz: <>Javob — ikkinchisi. Sayt internetga chiqsa, bitta <b>havola</b> yetadi: do'stingiz uni telefonida ochadi. Bugun shuni qilamiz.</>, ru: <>Ответ — второй. Когда сайт в интернете, хватает одной <b>ссылки</b>: друг откроет её на телефоне. Сегодня мы это и сделаем.</> })}</p>}
          </Col>
        </Split>
      </div>
    </Stage>;
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [turi, setTuri] = useState3(saved.turi || null);
  ...
  const demo = demoRead();
  const projList = demo && demo.muammo ? [{ id: "demo", ic: "⭐", name: { uz: "Demo Day loyiham", ru: "Мой проект Demo Day" }, sub: { uz: demo.yechim || "", ru: demo.yechim || "" } }, ...PROJECTS] : PROJECTS;
  ...
  const navLabel = !proj ? tr2({ uz: "Loyihani tanlang", ru: "Выберите проект" }) : !done ? tr2({ uz: `Yana ${MIN_PAGES - filled.length} ta sahifa nomi`, ru: `Ещё названий страниц: ${MIN_PAGES - filled.length}` }) : tr2({ uz: "Davom etish", ru: "Продолжить" });
  return <Stage eyebrow={tr2({ uz: "Sizning loyihangiz", ru: "Ваш проект" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bugun <span className="italic" style={{ color: T.accent }}>qanday</span> sayt quramiz?</>, ru: <>Какой сайт мы <span className="italic" style={{ color: T.accent }}>соберём</span> сегодня?</> })}</h2></div>
        {
    /* F-0803-30: `!proj` sharti — turi saqlanib, lekin ro'yxatdan topilmasa (masalan
       demo-g'oya o'chirilgan bo'lsa) tanlov qaytadan ochiladi, proj.ic da qulamaydi. */
        ...
        <Mentor>{!proj ? tr2({ uz: "Loyihangizni tanlang — shu sayt dars oxirida internetda turadi.", ru: "Выберите свой проект — этот сайт к концу урока окажется в интернете." }) : tr2({ uz: "Endi sayt qaysi sahifalardan iborat bo'lishini yozing. Kamida to'rttasi kerak: masalan «Bosh sahifa».", ru: "Теперь напишите, из каких страниц состоит сайт. Нужно минимум четыре: например «Главная»." })}</Mentor>
        <Split>
          <Col>
            {!proj ? <div className="prj-grid fade-up delay-1">
                {projList.map((p) => <button key={p.id} className="prj-card" onClick={() => choose(p.id)}>
                    <span className="prj-ic">{p.ic}</span>
                    <span className="prj-nm">{tr2(p.name)}</span>
                    <span className="prj-sub">{tr2(p.sub)}</span>
                  </button>)}
              </div> : <>
                <div className="pick-row fade-step">
                  <span style={{ fontSize: 20 }}>{proj.ic}</span>
                  <span className="body" style={{ margin: 0 }}>{tr2({ uz: "Loyiha:", ru: "Проект:" })} <b>{tr2(proj.name)}</b></span>
                  <button className="pick-redo" onClick={() => {
    setTuri(null);
    projWrite({ turi: null });
  }}>↻ {tr2({ uz: "o'zgartirish", ru: "изменить" })}</button>
                </div>
    ...
    placeholder={i < MIN_PAGES ? tr2({ uz: "Sahifa nomi…", ru: "Название страницы…" }) : tr2({ uz: "Sahifa nomi (ixtiyoriy)…", ru: "Название страницы (не обязательно)…" })}
  />
                    </div>)}
                </div>
              </>}
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "Sayt menyusi shunday chiqadi", ru: "Меню сайта получится таким" })}</p>
            <Preview title={turi ? `${turi}.netlify.app` : tr2({ uz: "hali tanlanmadi", ru: "пока не выбрано" })} minH={150}>
              {filled.length === 0 ? <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center" }}>{tr2({ uz: "Sahifa nomlarini yozing — menyu shu yerda yig'iladi", ru: "Напишите названия страниц — меню соберётся здесь" })}</p> : <>
                  <div className="menu-mock">{filled.map((p, i) => <span key={i} className={`menu-tab ${i === 0 ? "on" : ""}`}>{p.trim()}</span>)}</div>
                  <p className="body" style={{ margin: "12px 0 0", color: T.ink2 }}>{tr2({ uz: "Har nomga bitta sahifa. Hammasi shu menyu orqali bir-biriga ulanadi.", ru: "На каждое название — своя страница. Все они связаны через это меню." })}</p>
                </>}
            </Preview>
          </Col>
        </Split>
...
var buildPrompt = (proj, style) => {
  ...
  const ttl = demo && demo.yechim ? { uz: demo.yechim, ru: demo.yechim } : PROJ_TITLE[proj && proj.turi] || { uz: "Mening saytim", ru: "Мой сайт" };
  return [
    tr2({ uz: "Men veb-sayt qurmoqchiman. Menga uning to'liq kodini yozib ber.", ru: "Я хочу сделать веб-сайт. Напиши мне его полный код." }),
    "",
    tr2({ uz: "LOYIHA: " + ttl.uz, ru: "ПРОЕКТ: " + ttl.ru }),
    tr2({ uz: "SAHIFALAR: " + pages, ru: "СТРАНИЦЫ: " + pages }),
    "",
    ...
    tr2({ uz: "9. Kod toza va o'qiladigan bo'lsin, muhim joylariga izoh yoz.", ru: "9. Код должен быть чистым и читаемым, в важных местах — комментарии." }),
  ...
  const STEPS = [
    {
      ...
      res: () => <Preview title="AI chat" minH={150}><div style={{ ...winBox, minHeight: 112, alignItems: "stretch" }}><span className="mono small" style={{ color: T.ink3 }}>{tr2({ uz: "siz:", ru: "вы:" })}</span><span className="mono small" style={{ background: T.bg, borderRadius: 8, padding: "7px 10px" }}>{tr2({ uz: "Men veb-sayt qurmoqchiman…", ru: "Я хочу сделать веб-сайт…" })}</span><span className="mono small" style={{ color: T.success, fontWeight: 700 }}>AI: index.html · style.css · …</span></div></Preview>
    },
    {
      id: "folder",
...
var Screen6 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: "Sayt papkasi ochilganda brauzer birinchi qaysi faylni qidiradi?", ru: "Какой файл браузер ищет первым, когда открывает папку сайта?" })}</h2></>}
  options={[
...
var Screen7 = (props) => {
  const STEPS = [
    {
      ...
      res: () => <Preview title="github.com/new" minH={150}><div className="repo" style={{ boxShadow: "none" }}><div className="repo-top">📦 <b>mening-saytim</b> <span style={{ marginLeft: "auto", color: T.ink3, fontSize: 11 }}>Public</span></div><div className="repo-row" style={{ cursor: "default" }}>{tr2({ uz: "hozircha fayl yo'q", ru: "файлов пока нет" })}</div></div></Preview>
    },
    {
      id: "commit",
      nav: { uz: "commit", ru: "коммит" },
  ...
  return <DoScreen
    ...
    practice
    steps={STEPS}
    eyebrow={tr2({ uz: "Amaliyot · GitHub", ru: "Практика · GitHub" })}
    taskLabel={tr2({ uz: "Saytni GitHub'ga qo'yish", ru: "Загрузка сайта на GitHub" })}
    title={tr2({ uz: <>Sayt fayllarini <span className="italic" style={{ color: T.accent }}>GitHub'ga</span> qo'ying</>, ru: <>Положите файлы сайта <span className="italic" style={{ color: T.accent }}>на GitHub</span></> })}
    mentor={tr2({ uz: "Bu Git darsining davomi. Netlify kodni aynan GitHub'dan oladi.", ru: "Это продолжение урока Git. Код Netlify забирает именно с GitHub." })}
  />;
};
var Screen8 = (props) => {
  const STEPS = [
    ...
    },
    {
      ...
      res: () => <NetWin title="app.netlify.com"><span className="nfy-live">✓ {tr2({ uz: "kirdingiz", ru: "вы вошли" })}</span><span className="nfy-btn">Add new site</span><span className="nfy-row">{tr2({ uz: "hali sayt yo'q", ru: "сайтов пока нет" })}</span></NetWin>
    }
  ];
  return <DoScreen
    {...props}
...
var Screen9 = (props) => {
  const STEPS = [
    {
      id: "add",
      nav: { uz: "Add new site", ru: "Add new site" },
    ...
    },
    {
      ...
      res: () => <NetWin><span className="nfy-row">⏳ Building…</span><span className="nfy-row">📦 {tr2({ uz: "fayllar ko'chirilmoqda", ru: "файлы копируются" })}</span></NetWin>
    },
    {
      id: "live",
      ...
      res: () => <NetWin><span className="nfy-live">✓ Site is live</span><span className="nfy-link">https://mening-saytim.netlify.app</span><span className="nfy-row">{tr2({ uz: "havolani nusxalang", ru: "скопируйте ссылку" })} 📋</span></NetWin>
    }
  ];
  return <DoScreen
    {...props}
    practice
    steps={STEPS}
    eyebrow={tr2({ uz: "Amaliyot · deploy", ru: "Практика · деплой" })}
    taskLabel={tr2({ uz: "Deploy va havola", ru: "Деплой и ссылка" })}
    title={tr2({ uz: <>Reponi tanlang va saytni <span className="italic" style={{ color: T.accent }}>internetga chiqaring</span></>, ru: <>Выберите репозиторий и <span className="italic" style={{ color: T.accent }}>опубликуйте сайт</span></> })}
    mentor={tr2({ uz: <>Saytni serverga joylab internetga chiqarish <b style={{ color: T.ink }}>deploy</b> deyiladi. Besh qadamdan keyin havolangiz tayyor.</>, ru: <>Разместить сайт на сервере и открыть в интернете — это <b style={{ color: T.ink }}>деплой</b>. Через пять шагов ссылка будет готова.</> })}
  />;
};
var Screen10 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr2({ uz: "Tekshiruv · 3-savol", ru: "Проверка · вопрос 3" })}
  questionText="Sayt Netlify orqali internetga qanday tartibda chiqadi?"
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri tartibni tanlang", ru: "Выберите верный порядок" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: "Sayt Netlify orqali internetga qanday tartibda chiqadi?", ru: "В каком порядке сайт выходит в интернет через Netlify?" })}</h2></>}
  options={[
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [url, setUrl] = useState3(() => storedAnswer?.picked || siteUrlRead());
  ...
  useEffect4(() => {
    if (storedAnswer === void 0) onAnswer(screen, { correct: true, picked: "" });
  }, []);
  return <Stage eyebrow={tr2({ uz: "Havolangiz", ru: "Ваша ссылка" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Havolani <span className="italic" style={{ color: T.accent }}>telefonda</span> oching</>, ru: <>Откройте ссылку <span className="italic" style={{ color: T.accent }}>на телефоне</span></> })}</h2></div>
        <Mentor>{tr2({ uz: "Havolani telefoningizda oching va do'stingizga yuboring. Keyin uni pastdagi maydonga yozib qo'ying.", ru: "Откройте ссылку на телефоне и отправьте другу. Потом впишите её в поле ниже." })}</Mentor>
        <Split>
          <Col>
            <p className="flow-label">{tr2({ uz: "Telefonda shunday ko'rinadi", ru: "На телефоне это выглядит так" })}</p>
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "Havolangizni saqlab qo'ying", ru: "Сохраните свою ссылку" })}</p>
            ...
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={!looksOk || isMentorLive} onClick={save}>{saved ? tr2({ uz: "✓ Saqlandi", ru: "✓ Сохранено" }) : tr2({ uz: "Saqlash", ru: "Сохранить" })}</button>
            <p className="small" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Maydon ixtiyoriy: sayt hali tayyor bo'lmasa ham davom etasiz.", ru: "Поле не обязательное: даже если сайт ещё не готов, вы идёте дальше." })}</p>
            {clean && looksOk && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Endi shu havolani bilgan <b>har kim</b> saytingizni ochadi.</>, ru: <>Теперь ваш сайт откроет <b>каждый</b>, кто знает эту ссылку.</> })}</p></div>}
          </Col>
        </Split>
      </div>
    </Stage>;
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Uyga vazifa", ru: "Домашнее задание" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Amaliy topshiriqni bajarish →", ru: "Выполнить практическое задание →" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <Mentor>{tr2({ uz: "Sayt bugun bitmagan bo'lsa ham xavotir olmang: uyda tugatasiz. Topshiriq bitta — havolani yuborish.", ru: "Если сайт сегодня не закончен, ничего страшного: доделаете дома. Задание одно — отправить ссылку." })}</Mentor>
        <Split>
...
var Q_LABELS = { 2: { uz: "Netlify vazifasi", ru: "Задача Netlify" }, 6: { uz: "Bosh sahifa fayli", ru: "Файл главной страницы" }, 10: { uz: "Deploy tartibi", ru: "Порядок деплоя" }, 12: { uz: "Havola (yakuniy)", ru: "Ссылка (финал)" } };
var Confetti = () => {
...
var INLINE_KEYS = { s2: 0, s6: 2, s10: 1, s12: 3 };
