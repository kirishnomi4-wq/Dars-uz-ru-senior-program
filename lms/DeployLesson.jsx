...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">🙈 {tr({ uz: "Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "Кто что выбрал и число ✅/❌ скрыты — после «Открыть результат» всё откроется сразу и у вас, и на экранах учеников." })}</p>}
...
var Screen1 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="Netlify" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Yo'lni ko'rsating", ru: "Покажите путь" })} onClick={onNext} /></>}>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const navLabel = !proj ? tr({ uz: "Loyihani tanlang", ru: "Выберите проект" }) : !done ? tr({ uz: `Yana ${MIN_PAGES - filled.length} ta sahifa nomi`, ru: `Ещё названий страниц: ${MIN_PAGES - filled.length}` }) : tr({ uz: "Davom etish", ru: "Продолжить" });
  return <Stage eyebrow={tr({ uz: "Sizning loyihangiz", ru: "Ваш проект" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
    ...
    /* F-0803-30: `!proj` sharti — turi saqlanib, lekin ro'yxatdan topilmasa (masalan
       demo-g'oya o'chirilgan bo'lsa) tanlov qaytadan ochiladi, proj.ic da qulamaydi. */
        ...
        <Mentor>{!proj ? tr({ uz: "Loyihangizni tanlang — shu sayt dars oxirida internetda turadi.", ru: "Выберите свой проект — этот сайт к концу урока окажется в интернете." }) : tr({ uz: "Endi sayt qaysi sahifalardan iborat bo'lishini yozing. Kamida to'rttasi kerak: masalan «Bosh sahifa».", ru: "Теперь напишите, из каких страниц состоит сайт. Нужно минимум четыре: например «Главная»." })}</Mentor>
        <Split>
          <Col>
    ...
    setTuri(null);
    projWrite({ turi: null });
  }}>↻ {tr({ uz: "o'zgartirish", ru: "изменить" })}</button>
                </div>
  ...
  />
          ...
          <Col>
            ...
            <Preview title={turi ? `${turi}.netlify.app` : tr({ uz: "hali tanlanmadi", ru: "пока не выбрано" })} minH={150}>
              {filled.length === 0 ? <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center" }}>{tr({ uz: "Sahifa nomlarini yozing — menyu shu yerda yig'iladi", ru: "Напишите названия страниц — меню соберётся здесь" })}</p> : <>
...
var Screen4 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tayyor topshiriq", ru: "Готовое задание" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!st} label={st ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Uslubni tanlang", ru: "Выберите стиль" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <Split>
          ...
          <Col>
            {st ? <div className="pr-panel fade-step" key={style}>
              ...
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Chapdan uslubni tanlang — topshiriq shu yerda chiqadi", ru: "Выберите стиль слева — задание появится здесь" })}</p></div>}
          </Col>
        </Split>
      </div>
    </Stage>;
...
var Screen10 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr({ uz: "Tekshiruv · 3-savol", ru: "Проверка · вопрос 3" })}
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Havolangiz", ru: "Ваша ссылка" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive label={tr({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Uyga vazifa", ru: "Домашнее задание" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr({ uz: "Amaliy topshiriqni bajarish →", ru: "Выполнить практическое задание →" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
function Flashcards({ cards }) {
  ...
  if (!card) return <div className="fc-done fade-up"><span className="fc-done-emoji">🎉</span><p className="fc-done-h">{tr({ uz: "Hammasini bilasiz!", ru: "Вы знаете всё!" })}</p><p className="fc-done-s">{total}/{total} {tr({ uz: "tushuncha yodlandi", ru: "понятий выучено" })}</p><button className="fc-btn ghost" onClick={restart}>↻ {tr({ uz: "Qaytadan takrorlash", ru: "Повторить заново" })}</button></div>;
...
var ScreenFlashcards = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Takrorlash", ru: "Повторение" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={false} label={tr({ uz: "Yakunlash →", ru: "К финалу →" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var INLINE_KEYS = { s2: 0, s6: 2, s10: 1, s12: 3, s3: -1, s4: -1, s5: -1, s7: -1, s8: -1, s9: -1, s11: -1 };
...
function DeployLesson({ lang: langProp, onFinished }) {
