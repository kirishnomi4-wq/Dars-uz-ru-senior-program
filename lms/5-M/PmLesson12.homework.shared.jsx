...
//  Manba:  src/4-Modull/PmLesson12.homework.jsx
//  Kompilyator: TASHQI MODUL — https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx src/4-Modull/PmLesson12.homework.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/4-Modull/PmLesson12.homework.jsx
...
var sababBaho = (s) => sababUzun(s) && anyTest(BAHO_SOZ, (s || "").trim());
var sababOdamsiz = (s) => sababUzun(s) && !anyTest(BAHO_SOZ, (s || "").trim()) && !anyTest(ODAM_RE, s || "");
var ruxsatOk = (r) => r === "ochiq" || r === "yopiq";
...
var TrustCard = ({ data }) => {
  ...
  return <div className="ac">
      ...
      {bands.map((b, i) => <div key={i} className="ac-row">
...
var sababOk = (s, others) => sababUzun(s) && !sababNote(s, others);
var MarkRow = ({ label, row, setRow, others, pulse }) => {
...
var StageMarks = ({ data, setData, store }) => {
...
function PmLesson12Homework({ lang: langProp, onFinished }) {
  ...
  return <div className="hw-root">
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult ? <StageResult data={data} goStage={setStage} onFinishClick={finish} finished={finished} onCelebrated={() => setDataRaw((prev) => prev.celebrated ? prev : { ...prev, celebrated: true })} /> : cur.key === "rows" ? <StageRows rows={data.rows} setRows={setRows} /> : cur.key === "store" ? <StageStore data={data.store || {}} setData={setStageData("store")} /> : cur.key === "marks" ? <StageMarks data={data.marks || {}} setData={setStageData("marks")} store={data.store || {}} /> : <StageSum data={data.sum || {}} setData={setStageData("sum")} />}
        </div>
      </div>
