...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
    ...
    return <button key={o.id} className={`hook-option ${sel ? "on" : ""}`} disabled={picked !== null || !tried} style={{ opacity: !tried ? 0.55 : 1 }} onClick={() => pick(o.id)}>
            ...
            {picked !== null && <p className="hook-ack fade-step">{picked === "c" ? tr({ uz: <>Aynan! fetch'ga <b>server dasturi</b> javob beradi — o'sha o'chsa, hech kim javob bermaydi (ECONNREFUSED). Bugun shu serverni Node.js bilan o'zingiz yozasiz va yoqasiz.</>, ru: <>Именно! На fetch отвечает <b>программа-сервер</b> — если она выключится, никто не ответит (ECONNREFUSED). Сегодня Вы сами напишете этот сервер на Node.js и запустите его.</> }) : tr({ uz: <>Aslida brauzer faqat <b>so'raydi</b> — javobni narigi tomondagi <b>server dasturi</b> beradi. Tugmani eslang: u o'chganda ECONNREFUSED chiqdi. O'sha javob beruvchini bugun o'zingiz qurasiz.</>, ru: <>На самом деле браузер только <b>спрашивает</b> — отвечает <b>программа-сервер</b> на той стороне. Вспомните кнопку: когда она была выключена, появился ECONNREFUSED. Этого отвечающего Вы сегодня построите сами.</> })}</p>}
          </Col>
        </Split>
        </Zoomable>
      </div>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Server nima", ru: "Что такое сервер" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={navLabel} onClick={onNext} /></>}>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="Node.js" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Kodni ishga tushiring", ru: "Запустите код" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="npm" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "express'ni o'rnating", ru: "Установите express" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(storedAnswer ? new Set(Object.keys(PARTS)) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow="Express" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${seen.size}/5 ${tr({ uz: "qism o'rganildi", ru: "— изучите части" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            ...
            {active ? <div className="sk-info" key={active}><span className="sk-tagbig"><span className="sk-wordbadge mono">{PARTS[active].word}</span></span><p className="body" style={{ color: T.ink, margin: "11px 0 0" }}>{tr(PARTS[active].info)}</p></div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Koddan bir qismni bosing", ru: "Нажмите на часть кода" })}</p></div>}
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [active, setActive] = useState(null);
  const [seen, setSeen] = useState(storedAnswer ? new Set(EPS.map((e) => e.path)) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow="Endpoint" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${seen.size}/3 ${tr({ uz: "eshik ochildi", ru: "— откройте двери" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="NestJS" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "NestJS kartasini oching", ru: "Откройте карту NestJS" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Lokalda yurgizish", ru: "Локальный запуск" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Serverni ishga tushiring", ru: "Запустите сервер" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            ...
            <Win title="localhost:3000/salom" minH={92} hotTitle={done}>
              {done ? <div className="demo-swap" style={{ fontFamily: "Georgia, serif", fontSize: 19, color: T.ink }}>Salom, dunyo!</div> : <p style={{ color: T.ink3, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13 }}>{tr({ uz: "Server o'chiq — sahifa ochilmaydi…", ru: "Сервер выключен — страница не открывается…" })}</p>}
            </Win>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Keyingi qadam · AI", ru: "Следующий шаг · AI" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "AI'ga endpoint buyuring", ru: "Закажите endpoint у AI" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen12 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "Ishlab turgan serverni butunlay o'chirib qo'yadi", ru: "Полностью выключает работающий сервер" }), tr({ uz: "Yangi endpoint (eshik) yaratib, unga javob beradi", ru: "Создаёт новый endpoint (дверь) и отвечает на него" }), tr({ uz: "Saqlangan barcha ma'lumotni o'chirib tashlaydi", ru: "Удаляет все сохранённые данные" }), tr({ uz: "Serverni yoqadi va 3000-portda so'rov kuta boshlaydi", ru: "Включает сервер, и тот ждёт запросы на порту 3000" })]}
  correctIdx={3}
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Amaliyot · tartiblash", ru: "Практика · порядок" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${placed.length}/4 ${tr({ uz: "joylandi", ru: "на месте" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Debugging", ru: "Дебаггинг" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : found ? tr({ uz: "Endi tuzating", ru: "Теперь почините" }) : tr({ uz: "Muammoni toping", ru: "Найдите проблему" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Birinchi serveringiz <span className="italic" style={{ color: T.accent }}>javob berdi</span>.</>, ru: <>Ваш первый сервер <span className="italic" style={{ color: T.accent }}>ответил</span>.</> })}</h2>{
...
function NodeServerLesson({ lang: langProp, onFinished }) {
  ...
  return <LangContext.Provider value={lang}>
      ...
      <LiveGateCtx.Provider value={{ locked, live, earn }}>
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={tr({ uz: "Node.js server darsi", ru: "Урок: сервер на Node.js" })} /> : <>
