...
//  Manba:  src/4-Modull/NodeServerLesson.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/4-Modull/NodeServerLesson.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/4-Modull/NodeServerLesson.jsx
...
var LESSON_META = { lessonId: "node-server-04-03-v18", lessonTitle: { uz: "Node.js — birinchi serveringiz", ru: "Node.js — первый сервер" } };
var HW_TOKENS = [
...
var INLINE_KEYS = { s4: 0, s5b: 1, s9: 2, s12: 3, s15: -1, practice: -1 };
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <p className="mstats-verdict-t">{tr2({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верных — тема осталась непонятной классу. Перед продолжением коротко повторите.</> })}</p>}
...
var EpBtn = ({ method = "GET", path, desc, active, onClick }) => <button className={`epwin ${active ? "on" : ""}`} onClick={onClick}>
    <span className="epmethod">{method}</span>
    <span className="eppath">{path}</span>
    {desc && <span className="epdesc">{desc}</span>}
  </button>;
var ServerDot = ({ on }) => <span className="srv-dot" style={{ background: on ? T.success : T.ink3, boxShadow: on ? `0 0 8px ${T.success}` : "none" }} />;
var StoreSign = ({ on }) => <div className={`store-sign ${on ? "open" : "closed"}`} aria-hidden="true">
    <span className="store-sign__dot" />
    <span className="store-sign__txt">{on ? tr2({ uz: "OCHIQ", ru: "ОТКРЫТО" }) : tr2({ uz: "YOPIQ", ru: "ЗАКРЫТО" })}</span>
  </div>;
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const [seen, setSeen] = useState3(() => new Set(storedAnswer ? ["on", "off"] : ["off"]));
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <><span className="mono" style={{ color: T.accent }}>fetch</span> yozganingizda — narigi tomonda <span className="italic" style={{ color: T.accent }}>kim javob beradi</span>?</>, ru: <>Вы пишете <span className="mono" style={{ color: T.accent }}>fetch</span> — а на той стороне <span className="italic" style={{ color: T.accent }}>кто отвечает</span>?</> })}</h1>
        ...
        <Split>
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
              <button className={`btn ${!tried ? "tap-hint" : ""}`} onClick={toggle}><ServerDot on={on} /> &nbsp;{on ? tr2({ uz: "Serverni o'chirish", ru: "Выключить сервер" }) : tr2({ uz: "Serverni yoqish", ru: "Включить сервер" })}</button>
              <StoreSign on={on} />
            </div>
            <Win title={tr2({ uz: "brauzer — fetch so'rovi", ru: "браузер — fetch-запрос" })} minH={120}>
              <pre className="code-box" style={{ marginBottom: 10 }}><Jx>{"fetch"}</Jx>{"("}<St>{"'http://localhost:3000/salom'"}</St>{")"}</pre>
              {on ? <div className="frame-success demo-swap" style={{ padding: "10px 13px" }}><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "✓ Javob: ", ru: "✓ Ответ: " })}<b style={{ color: T.success }}>"Salom, dunyo!"</b></p></div> : <div className="frame-warn demo-swap" style={{ padding: "10px 13px" }}><p className="body" style={{ margin: 0, color: T.ink }}>❌ <span className="mono">ECONNREFUSED</span>{tr2({ uz: " — ulanib bo'lmadi", ru: " — не удалось подключиться" })}</p></div>}
            </Win>
            <p className="mono small" style={{ margin: 0, color: on ? T.success : T.accent }}>{on ? tr2({ uz: "server yoniq → javob bor", ru: "сервер включён → ответ есть" }) : tr2({ uz: "server o'chiq → javob yo'q", ru: "сервер выключен → ответа нет" })}</p>
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Sizningcha, fetch'ga kim javob beradi?", ru: "Как Вы думаете, кто отвечает на fetch?" })}</p>
    ...
    return <button key={o.id} className={`hook-option ${sel ? "on" : ""}`} disabled={picked !== null || !tried} style={{ opacity: !tried ? 0.55 : 1 }} onClick={() => pick(o.id)}>
            ...
            {!tried && <p className="small" style={{ color: T.ink3, fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Avval serverni yoqib-o'chirib ko'ring ←", ru: "Сначала повключайте и повыключайте сервер ←" })}</p>}
            {picked !== null && <p className="hook-ack fade-step">{picked === "c" ? tr2({ uz: <>Aynan! fetch'ga <b>server dasturi</b> javob beradi — o'sha o'chsa, hech kim javob bermaydi (ECONNREFUSED). Bugun shu serverni Node.js bilan o'zingiz yozasiz va yoqasiz.</>, ru: <>Именно! На fetch отвечает <b>программа-сервер</b> — если она выключится, никто не ответит (ECONNREFUSED). Сегодня Вы сами напишете этот сервер на Node.js и запустите его.</> }) : tr2({ uz: <>Aslida brauzer faqat <b>so'raydi</b> — javobni narigi tomondagi <b>server dasturi</b> beradi. Tugmani eslang: u o'chganda ECONNREFUSED chiqdi. O'sha javob beruvchini bugun o'zingiz qurasiz.</>, ru: <>На самом деле браузер только <b>спрашивает</b> — отвечает <b>программа-сервер</b> на той стороне. Вспомните кнопку: когда она была выключена, появился ECONNREFUSED. Этого отвечающего Вы сегодня построите сами.</> })}</p>}
          </Col>
        </Split>
        </Zoomable>
      </div>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const PreviewBlock = <Col>
      <p className="flow-label">{tr2({ uz: "Dars oxirida — sizning serveringiz", ru: "В конце урока — Ваш сервер" })}</p>
      <Win title={tr2({ uz: "brauzer — localhost:3000/salom", ru: "браузер — localhost:3000/salom" })} minH={96}>
        <div className="fade-up delay-1" style={{ fontFamily: "Georgia, serif", fontSize: 20, color: T.ink, padding: "6px 2px" }}>Salom, dunyo!</div>
        <p className="mono small" style={{ margin: "4px 0 0", color: T.success }}>{tr2({ uz: "✓ serveringiz javob berdi", ru: "✓ Ваш сервер ответил" })}</p>
      </Win>
      <p className="mono small" style={{ color: T.accent, margin: 0 }}>{tr2({ uz: "→ shu javobni dars oxirida o'zingiz yozasiz va ishga tushirasiz", ru: "→ в конце урока Вы сами напишете и запустите этот ответ" })}</p>
    </Col>;
  ...
  return <Stage eyebrow={tr2({ uz: "Reja", ru: "План" })} screen={screen} audioState={audio} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Birinchi serveringizni <span className="italic" style={{ color: T.accent }}>5 qadamda</span> quramiz</>, ru: <>Построим Ваш первый сервер <span className="italic" style={{ color: T.accent }}>за 5 шагов</span></> })}</h2></div>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const QUEUE = ["🧑‍🎤", "🧑‍🚀", "🧑‍🎨"];
  const [on, setOn] = useState3(true);
  const [served, setServed] = useState3(storedAnswer ? 3 : 0);
  const [refused, setRefused] = useState3(false);
  const [pop, setPop] = useState3(-1);
  const done = served >= 3;
  const serve = () => {
    if (done) return;
    if (!on) {
    ...
    if (served < 3) {
      setPop(served);
      setTimeout(() => setPop(-1), 720);
      setServed(served + 1);
    }
  };
  const toggle = () => setOn((v) => {
    const nv = !v;
    setRefused(!nv && served < 3);
    return nv;
  });
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  const navLabel = done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : !on ? tr2({ uz: "Avval tabloni yoqing", ru: "Сначала включите табло" }) : `${served}/3 — ${tr2({ uz: "mijozga javob bering", ru: "ответьте клиентам" })}`;
  ...
  return <Stage eyebrow={tr2({ uz: "Server nima", ru: "Что такое сервер" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Server <span className="italic" style={{ color: T.accent }}>aslida nima</span> — maxsus mashinami?</>, ru: <>Что такое сервер <span className="italic" style={{ color: T.accent }}>на самом деле</span> — особая машина?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <button className={`btn fade-up delay-1 ${on ? "" : "btn-soft"}`} style={{ alignSelf: "flex-start" }} onClick={toggle}><ServerDot on={on} /> &nbsp;{on ? tr2({ uz: "Tablo: OCHIQ", ru: "Табло: ОТКРЫТО" }) : tr2({ uz: "Tablo: YOPIQ", ru: "Табло: ЗАКРЫТО" })}</button>
            <Win title={on ? tr2({ uz: "do'kon — OCHIQ", ru: "магазин — ОТКРЫТ" }) : tr2({ uz: "do'kon — YOPIQ", ru: "магазин — ЗАКРЫТ" })} minH={140} hotTitle={done}>
    ...
    return <span key={i} className={cls}>
                    ...
                    <span className="mono small" style={{ color: T.ink3, marginLeft: "auto" }}>{served}/3 {tr2({ uz: "xizmat ko'rdi", ru: "обслужено" })}</span>
                  </div>
                </div>
                {!done && <button className={`btn-soft ${on && served === 0 && !refused ? "tap-hint" : ""}`} style={{ alignSelf: "flex-start" }} onClick={serve}>{tr2({ uz: "📨 Javob ber (res.send)", ru: "📨 Ответить (res.send)" })}</button>}
                {refused && <div className="frame-warn" style={{ padding: "9px 12px" }}><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>❌ <span className="mono">ECONNREFUSED</span> — tablo YOPIQ, mijoz javob ololmadi. Server DOIM ishlashi kerak!</>, ru: <>❌ <span className="mono">ECONNREFUSED</span> — табло ЗАКРЫТО, клиент не получил ответ. Сервер должен работать ВСЕГДА!</> })}</p></div>}
                {done && <div className="frame-success" style={{ padding: "9px 12px" }}><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "✓ Hamma mijozga javob berdingiz — do'koningiz ishlayapti!", ru: "✓ Вы ответили всем клиентам — Ваш магазин работает!" })}</p></div>}
              </div>
            </Win>
            <p className="mono small" style={{ margin: 0, color: on ? T.success : T.accent }}>{on ? tr2({ uz: "tablo yoniq → so'rovga javob bor", ru: "табло горит → на запрос есть ответ" }) : tr2({ uz: "tablo yopiq → hech kim javob ololmaydi", ru: "табло погасло → никто не получит ответ" })}</p>
          </Col>
          <Col>
            <div className="sk-info">
              <span className="sk-tagbig"><span className="sk-wordbadge">{tr2({ uz: "Server = dastur", ru: "Сервер = программа" })}</span></span>
              <p className="body" style={{ color: T.ink, margin: "11px 0 0" }}>{tr2({ uz: <>Doim ishlab turadigan, <b>so'rovlarni kutadigan</b> va <b>javob qaytaradigan</b> dastur. Do'kon yopilsa — javob ham yo'qoladi (ECONNREFUSED).</>, ru: <>Программа, которая работает всегда, <b>ждёт запросы</b> и <b>возвращает ответы</b>. Закрылся магазин — пропал и ответ (ECONNREFUSED).</> })}</p>
            </div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Mana shu — server. Mijoz (so'rov) keladi, javob beradi. Endi savol: bunday dasturni qaysi tilda yozamiz? Javob — <b>JavaScript</b> (Node.js bilan)!</>, ru: <>Вот это и есть сервер. Приходит клиент (запрос) — он отвечает. Теперь вопрос: на каком языке пишут такую программу? Ответ — <b>JavaScript</b> (с Node.js)!</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="Node.js" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Kodni ishga tushiring", ru: "Запустите код" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>JavaScript faqat <span className="italic" style={{ color: T.accent }}>brauzerda yashaydimi</span>?</>, ru: <>JavaScript живёт <span className="italic" style={{ color: T.accent }}>только в браузере</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            {!ran && <button className="btn fade-up delay-2" style={{ alignSelf: "flex-start" }} onClick={() => setRan(true)}>{tr2({ uz: "▶ Kodni ishga tushirish", ru: "▶ Запустить код" })}</button>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Brauzer (frontend)", ru: "Браузер (фронтенд)" })}</p>
            <Win title={tr2({ uz: "brauzer konsoli", ru: "консоль браузера" })} minH={52}>{ran ? <TLine out={<span style={{ color: CODE.str }}>Salom!</span>} /> : <span style={{ color: T.ink3, fontFamily: "Georgia, serif", fontStyle: "italic", fontSize: 13 }}>{tr2({ uz: "kutilmoqda…", ru: "ожидание…" })}</span>}</Win>
            <p className="flow-label" style={{ marginTop: 2 }}>{tr2({ uz: "Server (Node.js)", ru: "Сервер (Node.js)" })}</p>
            <div className="code-box" style={{ padding: "9px 13px", minHeight: 46 }}>{ran ? <TLine cmd="node server.js" /> : <span style={{ color: CODE.comment, fontFamily: "'JetBrains Mono'", fontSize: 12 }}>{tr2({ uz: "terminal…", ru: "терминал…" })}</span>}{ran && <TLine out={<span style={{ color: CODE.str }}>Salom!</span>} />}</div>
...
var Screen4 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <><span className="mono" style={{ color: T.accent }}>Node.js</span> nima imkon beradi?</>, ru: <>Что даёт <span className="mono" style={{ color: T.accent }}>Node.js</span>?</> })}</h2></>}
  audioText="Node.js nima imkon beradi? JavaScript endi qayerda ishlaydi deb o'ylaysiz? To'g'ri javobni tanlang."
  options={[tr2({ uz: "JavaScript'ni brauzerdan tashqarida, serverda ishlatish", ru: "Запускать JavaScript вне браузера — на сервере" }), tr2({ uz: "Faqat HTML sahifalarni chiroyli qilib yozish uchun kerak", ru: "Нужен только чтобы красиво писать HTML-страницы" }), tr2({ uz: "Internet tezligini oshirib, barcha saytlarni tezlatadi", ru: "Ускоряет интернет и все сайты сразу" }), tr2({ uz: "Rasm va videolarni tahrirlash imkonini beradi", ru: "Позволяет редактировать фото и видео" })]}
  correctIdx={0}
  explainCorrect={tr2({ uz: "To'g'ri! Node.js JavaScript'ni brauzerdan tashqarida — serverda/kompyuterda ishlatish imkonini beradi. Shuning uchun backend ham JS'da yoziladi.", ru: "Верно! Node.js позволяет запускать JavaScript вне браузера — на сервере/компьютере. Поэтому бэкенд тоже пишут на JS." })}
  explainWrong={{
    1: tr2({ uz: "Yo'q — Node JS uchun. HTML alohida narsa.", ru: "Нет — Node для JS. HTML — отдельная история." }),
    2: tr2({ uz: "Yo'q — Node internetni tezlatmaydi; u JS'ni serverda ishlatadi.", ru: "Нет — Node не ускоряет интернет; он запускает JS на сервере." }),
    3: tr2({ uz: "Yo'q — bu boshqa dasturlar ishi. Node — JS'ni serverda ishlatadi.", ru: "Нет — это дело других программ. Node запускает JS на сервере." }),
    default: tr2({ uz: "Node.js — JS'ni serverda ishlatish imkoni.", ru: "Node.js — возможность запускать JS на сервере." })
  }}
/>;
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="npm" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "express'ni o'rnating", ru: "Установите express" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Har narsani <span className="italic" style={{ color: T.accent }}>noldan yozish</span> shartmi?</>, ru: <>Обязательно ли всё <span className="italic" style={{ color: T.accent }}>писать с нуля</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            <div className="code-box" style={{ minHeight: 80 }}>
              {phase === "idle" && <TLine out={<span style={{ color: CODE.comment }}>{tr2({ uz: "# express'ni o'rnatishga tayyor", ru: "# готово к установке express" })}</span>} />}
              {phase !== "idle" && <TLine cmd="npm install express" />}
              {phase === "installing" && <TLine out={<span style={{ color: CODE.attr }}>{tr2({ uz: "⏳ yuklanmoqda…", ru: "⏳ загрузка…" })}</span>} />}
              {phase === "done" && <><TLine out={<span style={{ color: CODE.str }}>{tr2({ uz: "+ express o'rnatildi ✓", ru: "+ express установлен ✓" })}</span>} /><TLine out={<span style={{ color: CODE.comment }}>added 57 packages</span>} /></>}
            </div>
            {phase === "idle" && <button className="btn fade-up delay-1" style={{ alignSelf: "flex-start" }} onClick={install}>⬇ npm install express</button>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "package.json — o'rnatilgan paketlar", ru: "package.json — установленные пакеты" })}</p>
            <pre className="code-box">{"{"}{"\n"}{"  "}<At>"dependencies"</At>{": {"}{"\n"}{"    "}{done ? <St>"express": "^4.18.0"</St> : <span style={{ color: CODE.comment }}>{tr2({ uz: "// hali bo'sh", ru: "// пока пусто" })}</span>}{"\n"}{"  }"}{"\n"}{"}"}</pre>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>express o'rnatildi va <span className="mono">package.json</span>'ga yozildi. Endi uni kodimizda ishlatsak bo'ladi. npm — sizning <b>tayyor paketlar to'plamingiz</b>.</>, ru: <>express установлен и записан в <span className="mono">package.json</span>. Теперь его можно использовать в коде. npm — Ваш <b>набор готовых пакетов</b>.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen5b = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "Mustahkamlash", ru: "Закрепление" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <><span className="mono" style={{ color: T.accent }}>npm</span> nima qiladi?</>, ru: <>Что делает <span className="mono" style={{ color: T.accent }}>npm</span>?</> })}</h2></>}
  audioText="npm nima qiladi? Asboblar do'konini eslang. To'g'ri javobni tanlang."
  options={[tr2({ uz: "Ishlab turgan serverni butunlay o'chirib qo'yadi", ru: "Полностью выключает работающий сервер" }), tr2({ uz: "Tayyor asboblarni (paketlarni) o'rnatadi, masalan express", ru: "Устанавливает готовые инструменты (пакеты), например express" }), tr2({ uz: "Yozib qo'ygan barcha kodingizni o'chirib tashlaydi", ru: "Удаляет весь написанный Вами код" }), tr2({ uz: "Kompyuterni internetga ulab, aloqa o'rnatadi", ru: "Подключает компьютер к интернету" })]}
  correctIdx={1}
  explainCorrect={tr2({ uz: "To'g'ri! npm (Node Package Manager) — tayyor paketlarni o'rnatadigan vosita. npm install express bilan Express'ni o'rnatdik.", ru: "Верно! npm (Node Package Manager) — инструмент, который устанавливает готовые пакеты. Командой npm install express мы установили Express." })}
  explainWrong={{
    0: tr2({ uz: "Yo'q — npm serverni o'chirmaydi. U tayyor paketlarni o'rnatadi.", ru: "Нет — npm не выключает сервер. Он устанавливает готовые пакеты." }),
    2: tr2({ uz: "Aksincha — npm kod qo'shadi (paket o'rnatadi), o'chirmaydi.", ru: "Наоборот — npm добавляет код (устанавливает пакеты), а не удаляет." }),
    3: tr2({ uz: "Yo'q — npm paketlarni o'rnatadi (tayyor paketlar to'plami).", ru: "Нет — npm устанавливает пакеты (набор готовых пакетов)." }),
    default: tr2({ uz: "npm — tayyor paketlarni o'rnatuvchi.", ru: "npm — установщик готовых пакетов." })
  }}
/>;
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState3(storedAnswer ? new Set(Object.keys(PARTS)) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow="Express" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : `${seen.size}/5 ${tr2({ uz: "qism o'rganildi", ru: "— изучите части" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Butun serverni <span className="italic" style={{ color: T.accent }}>5 qatorda</span> yozsa bo'ladimi?</>, ru: <>Можно ли написать целый сервер <span className="italic" style={{ color: T.accent }}>в 5 строк</span>?</> })}</h2></div>
        ...
        <div className="split">
          ...
          <Col>
            ...
            {active ? <div className="sk-info" key={active}><span className="sk-tagbig"><span className="sk-wordbadge mono">{PARTS[active].word}</span></span><p className="body" style={{ color: T.ink, margin: "11px 0 0" }}>{tr2(PARTS[active].info)}</p></div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Koddan bir qismni bosing", ru: "Нажмите на часть кода" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Mana butun server! Asbobni chaqir → app yarat → endpoint och → javob ber → yoq. Endi <b>endpoint</b>ni chuqurroq ko'ramiz.</>, ru: <>Вот и весь сервер! Подключи инструмент → создай app → открой endpoint → ответь → включи. Теперь разберём <b>endpoint</b> поглубже.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [active, setActive] = useState3(null);
  const [seen, setSeen] = useState3(storedAnswer ? new Set(EPS.map((e) => e.path)) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow="Endpoint" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : `${seen.size}/3 ${tr2({ uz: "eshik ochildi", ru: "— откройте двери" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bitta server <span className="italic" style={{ color: T.accent }}>nechta so'rovga</span> javob bera oladi?</>, ru: <>На <span className="italic" style={{ color: T.accent }}>сколько запросов</span> может отвечать один сервер?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Server eshiklari — bosing", ru: "Двери сервера — нажимайте" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {EPS.map((e) => <EpBtn key={e.path} path={e.path} desc={tr2(e.desc)} active={active === e.path} onClick={() => tap(e.path)} />)}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Javob", ru: "Ответ" })}</p>
            <Win title={cur ? `${tr2({ uz: "brauzer", ru: "браузер" })} — localhost:3000${cur.path}` : tr2({ uz: "brauzer", ru: "браузер" })} minH={90}>
              {cur ? <div className="demo-swap"><p className="mono small" style={{ color: T.ink3, margin: "0 0 6px" }}>GET {cur.path}</p><div style={{ fontFamily: "Georgia, serif", fontSize: 16, color: T.ink }}>{cur.reply}</div></div> : <p style={{ color: T.ink3, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13 }}>{tr2({ uz: "Eshikni bosing — javobi shu yerda chiqadi", ru: "Нажмите на дверь — здесь появится её ответ" })}</p>}
            </Win>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Har endpoint — bitta manzil, bitta javob. Kodda har biri <span className="mono">app.get('/manzil', ...)</span> bilan ochiladi. Frontend shu manzillarga so'rov yuboradi.</>, ru: <>Каждый endpoint — один адрес, один ответ. В коде каждый открывается через <span className="mono">app.get('/адрес', ...)</span>. Фронтенд шлёт запросы на эти адреса.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="NestJS" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "NestJS kartasini oching", ru: "Откройте карту NestJS" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Loyiha kattalashsa — <span className="italic" style={{ color: T.accent }}>Express yetadimi</span>?</>, ru: <>А если проект вырастет — <span className="italic" style={{ color: T.accent }}>хватит ли Express</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <div className="cmp-card"><p className="cmp-h">Express</p><p className="small" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Kichik, tez, oddiy. Birinchi qadam uchun ayni kerakli.", ru: "Маленький, быстрый, простой. Для первого шага — то, что нужно." })}</p></div>
              <div className={`cmp-card hot cmp-tap ${opened ? "on" : "tap-hint"}`} role="button" tabIndex={0} onClick={open} onKeyDown={(e) => {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      open();
    }
  }}><p className="cmp-h" style={{ color: T.accent }}>NestJS ★</p><p className="small" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Tartibli va kuchli. Katta loyihalar uchun.", ru: "Организованный и мощный. Для больших проектов." })}</p></div>
            </div>
          </Col>
          <Col>
            {opened ? <div className="sk-info fade-step"><span className="sk-tagbig"><span className="sk-wordbadge">{tr2({ uz: "Tartib (modullar)", ru: "Порядок (модули)" })}</span></span><p className="body" style={{ color: T.ink, margin: "10px 0 0" }}>{tr2({ uz: "Kod papkalar va modullarga ajraladi — katta loyihada ham adashmaysiz.", ru: "Код делится на папки и модули — не запутаетесь даже в большом проекте." })}</p></div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "NestJS kartasini bosing", ru: "Нажмите на карту NestJS" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Bugun — <b>Express</b> bilan birinchi server. <b>NestJS</b> bilan katta loyihalarda keyin tanishasiz.</>, ru: <>Сегодня — первый сервер на <b>Express</b>. С <b>NestJS</b> познакомитесь позже, в больших проектах.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen9 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <><span className="mono" style={{ color: T.accent }}>Endpoint</span> nima?</>, ru: <>Что такое <span className="mono" style={{ color: T.accent }}>endpoint</span>?</> })}</h2></>}
  audioText="Endpoint aslida nima? Xuddi do'kondagi alohida eshik kabi. To'g'ri javobni tanlang."
  options={[tr2({ uz: "Serverni butunlay o'chirib-yoquvchi maxsus qizil tugma", ru: "Особая красная кнопка включения-выключения сервера" }), tr2({ uz: "Barcha ma'lumotlarni saqlab turadigan katta ombor baza", ru: "Большая база-склад, где хранятся все данные" }), tr2({ uz: "Serverning aniq manzili (eshik) — bir so'rovga javob beradi", ru: "Точный адрес сервера (дверь) — отвечает на один запрос" }), tr2({ uz: "Brauzerning bir turi yoki eng so'nggi yangi versiyasi", ru: "Вид браузера или его самая новая версия" })]}
  correctIdx={2}
  explainCorrect={tr2({ uz: "To'g'ri! Endpoint — serverning aniq manzili (masalan /salom), u bitta so'rovga javob beradi. Frontend shu manzilga fetch qiladi.", ru: "Верно! Endpoint — точный адрес сервера (например /salom), он отвечает на один запрос. Фронтенд делает fetch на этот адрес." })}
  explainWrong={{
    0: tr2({ uz: "Yo'q — endpoint manzil (eshik), o'chirish tugmasi emas.", ru: "Нет — endpoint это адрес (дверь), а не кнопка выключения." }),
    1: tr2({ uz: "Yo'q — baza alohida narsa. Endpoint — serverga kirish manzili.", ru: "Нет — база это отдельная вещь. Endpoint — адрес входа на сервер." }),
    3: tr2({ uz: "Yo'q — brauzer turi emas. Endpoint = server manzili (/salom).", ru: "Нет — это не вид браузера. Endpoint = адрес сервера (/salom)." }),
    default: tr2({ uz: "Endpoint — serverning manzili (eshigi), bitta so'rovga javob beradi.", ru: "Endpoint — адрес (дверь) сервера, отвечает на один запрос." })
  }}
/>;
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Lokalda yurgizish", ru: "Локальный запуск" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Serverni ishga tushiring", ru: "Запустите сервер" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Serveringizni qanday <span className="italic" style={{ color: T.accent }}>"yoqasiz"</span>?</>, ru: <>Как <span className="italic" style={{ color: T.accent }}>«включить»</span> Ваш сервер?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            <div className="code-box" style={{ minHeight: 78 }}>
              {phase === "idle" && <TLine out={<span style={{ color: CODE.comment }}>{tr2({ uz: "# serverni ishga tushirishga tayyor", ru: "# готово к запуску сервера" })}</span>} />}
              {phase !== "idle" && <TLine cmd="node server.js" />}
              {phase === "starting" && <TLine out={<span style={{ color: CODE.attr }}>{tr2({ uz: "⏳ ishga tushyapti…", ru: "⏳ запускается…" })}</span>} />}
              {phase === "done" && <TLine out={<span style={{ color: CODE.str }}>{tr2({ uz: "✓ Server localhost:3000 da ishlayapti", ru: "✓ Сервер работает на localhost:3000" })}</span>} />}
            </div>
            {phase === "idle" && <button className="btn fade-up delay-1" style={{ alignSelf: "flex-start" }} onClick={run}>▶ node server.js</button>}
          </Col>
          <Col>
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}><p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "Brauzer", ru: "Браузер" })}</p><StoreSign on={done} /></div>
            <Win title="localhost:3000/salom" minH={92} hotTitle={done}>
              {done ? <div className="demo-swap" style={{ fontFamily: "Georgia, serif", fontSize: 19, color: T.ink }}>Salom, dunyo!</div> : <p style={{ color: T.ink3, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13 }}>{tr2({ uz: "Server o'chiq — sahifa ochilmaydi…", ru: "Сервер выключен — страница не открывается…" })}</p>}
            </Win>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Server yoqildi! Brauzerda <span className="mono">localhost:3000/salom</span> ochilsa — sizning serveringiz javob beryapti. <b>localhost = o'z kompyuteringiz</b>.</>, ru: <>Сервер включён! Если в браузере открывается <span className="mono">localhost:3000/salom</span> — отвечает Ваш сервер. <b>localhost = Ваш собственный компьютер</b>.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Keyingi qadam · AI", ru: "Следующий шаг · AI" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "AI'ga endpoint buyuring", ru: "Закажите endpoint у AI" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Yangi endpoint kerakmi? <span className="italic" style={{ color: T.accent }}>AI'ga buyuring</span> — siz tekshiring.</>, ru: <>Нужен новый endpoint? <span className="italic" style={{ color: T.accent }}>Закажите AI</span> — а проверьте сами.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "1. AI'ga buyuring", ru: "1. Дайте команду AI" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {cur && <div className="ai-card fade-step" key={task || "stored"}>
                <div className="ai-row"><span className="ai-badge">AI</span><span className="ai-bubble">{phase === "planned" ? tr2({ uz: "Mana endpoint kodi — tasdiqlaysizmi?", ru: "Вот код endpoint — подтверждаете?" }) : phase === "building" ? tr2({ uz: "Yozyapman…", ru: "Пишу…" }) : tr2({ uz: "Qo'shdim — tekshiring", ru: "Добавил — проверяйте" })}</span></div>
                ...
                {phase === "planned" && <button className="btn fade-step" style={{ alignSelf: "flex-start" }} onClick={approve}>{tr2({ uz: "Kodni qo'shish", ru: "Добавить код" })}</button>}
                {phase === "done" && <p className="ai-prompt" style={{ color: T.success, fontStyle: "normal", fontWeight: 600 }}>{tr2({ uz: "✓ endpoint qo'shildi", ru: "✓ endpoint добавлен" })}</p>}
              </div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "2. Natija — brauzerda sinang", ru: "2. Результат — проверьте в браузере" })}</p>
            <Win title={cur ? `localhost:3000${cur.path}` : tr2({ uz: "brauzer", ru: "браузер" })} minH={92}>
              {done && cur ? <div className="demo-swap"><p className="mono small" style={{ color: T.ink3, margin: "0 0 6px" }}>GET {cur.path}</p><div style={{ fontFamily: "Georgia, serif", fontSize: 16, color: T.ink }}>{cur.reply}</div></div> : <p style={{ color: T.ink3, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13 }}>{tr2({ uz: "Buyruq bering va kodni tasdiqlang…", ru: "Дайте команду и подтвердите код…" })}</p>}
            </Win>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>AI endpoint yozdi, siz <b>kodni o'qib tekshirdingiz</b> va brauzerda sinadingiz. <span className="mono">app.get</span> + <span className="mono">res.send</span> — hammasi joyida!</>, ru: <>AI написал endpoint, Вы <b>прочитали и проверили код</b> и испытали его в браузере. <span className="mono">app.get</span> + <span className="mono">res.send</span> — всё на месте!</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen12 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <><span className="mono" style={{ color: T.accent }}>app.listen(3000)</span> nima qiladi?</>, ru: <>Что делает <span className="mono" style={{ color: T.accent }}>app.listen(3000)</span>?</> })}</h2></>}
  audioText="app.listen(3000) — do'kondagi OCHIQ tablosini yoqadigan qator. Nima qiladi deb o'ylaysiz? To'g'ri javobni tanlang."
  options={[tr2({ uz: "Ishlab turgan serverni butunlay o'chirib qo'yadi", ru: "Полностью выключает работающий сервер" }), tr2({ uz: "Yangi endpoint (eshik) yaratib, unga javob beradi", ru: "Создаёт новый endpoint (дверь) и отвечает на него" }), tr2({ uz: "Saqlangan barcha ma'lumotni o'chirib tashlaydi", ru: "Удаляет все сохранённые данные" }), tr2({ uz: "Serverni yoqadi va 3000-portda so'rov kuta boshlaydi", ru: "Включает сервер, и тот ждёт запросы на порту 3000" })]}
  correctIdx={3}
  explainCorrect={tr2({ uz: "To'g'ri! app.listen(3000) serverni ishga tushiradi va 3000-portda so'rovlarni kuta boshlaydi. Busiz server umuman ishlamaydi.", ru: "Верно! app.listen(3000) запускает сервер, и тот начинает ждать запросы на порту 3000. Без этого сервер вообще не работает." })}
  explainWrong={{
    0: tr2({ uz: "Aksincha — listen serverni YOQADI, o'chirmaydi.", ru: "Наоборот — listen ВКЛЮЧАЕТ сервер, а не выключает." }),
    1: tr2({ uz: "Yo'q — endpoint app.get bilan yaratiladi. listen — serverni yoqadi.", ru: "Нет — endpoint создаётся через app.get. listen — включает сервер." }),
    2: tr2({ uz: "Yo'q — listen ma'lumotga tegmaydi, u serverni ishga tushiradi.", ru: "Нет — listen не трогает данные, он запускает сервер." }),
    default: tr2({ uz: "app.listen — serverni yoqadi va port kuta boshlaydi.", ru: "app.listen — включает сервер и начинает слушать порт." })
  }}
/>;
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [placed, setPlaced] = useState3(storedAnswer ? ORDER.map((o) => o.id) : []);
  ...
  return <Stage eyebrow={tr2({ uz: "Amaliyot · tartiblash", ru: "Практика · порядок" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : `${placed.length}/4 ${tr2({ uz: "joylandi", ru: "на месте" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Server qismlarini <span className="italic" style={{ color: T.accent }}>to'g'ri tartibda</span> yig'a olasizmi?</>, ru: <>Соберёте части сервера <span className="italic" style={{ color: T.accent }}>в правильном порядке</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Server kodi aniq tartibda yoziladi — har bo'lak o'zidan oldingisiga tayanadi. Pastdagi to'rt bo'lakni to'g'ri ketma-ketlikda bosing.</>, ru: <>Код сервера пишется в строгом порядке — каждый блок опирается на предыдущий. Нажмите четыре блока ниже в правильной последовательности.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Bo'laklar — to'g'ri tartibda bosing", ru: "Блоки — нажимайте в правильном порядке" })}</p>
    ...
    return <button key={id} className={`chip mono ${shake === id ? "shake" : ""}`} disabled={used} style={used ? { opacity: 0.35 } : void 0} onClick={() => click(id)}>{o.label}</button>;
  })}
            </div>
            {!done && nextNeeded && missedOnce && <p className="small" style={{ color: T.ink3, fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Keyingi: ", ru: "Следующий: " })}{tr2(ORDER[placed.length].hint)}</p>}
            {!done && <AchRule screen={screen} />}
          </Col>
          <Col>
            <p className="flow-label">server.js</p>
            <pre className="code-box" style={{ minHeight: 110 }}>
              {placed.length === 0 && <span style={{ color: CODE.comment }}>{tr2({ uz: "// bo'laklarni tartibda joylang…", ru: "// расставьте блоки по порядку…" })}</span>}
              {placed.map((id, i) => {
    const o = ORDER.find((x) => x.id === id);
    return <div key={id} className="el-in"><span style={{ color: CODE.str }}>{i + 1}</span>{"  "}<span style={{ color: CODE.text }}>{o.label}</span></div>;
  })}
            </pre>
            <div className="store-build" aria-hidden="true">
              {["📦", "🛒", "🚪"].map((ic, i) => <React3.Fragment key={i}>
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>To'g'ri tartib! Chaqir → yarat → endpoint → yoq. Mana butun server — to'rt qadamda. Endi oxirgi qadam — o'zingiz yozasiz!</>, ru: <>Правильный порядок! Подключи → создай → endpoint → включи. Вот целый сервер — в четыре шага. Теперь последний шаг — напишете сами!</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const achMiss = useContext2(AchMissCtx);
  const [picked, setPicked] = useState3(storedAnswer ? "listen" : null);
  ...
  return <Stage eyebrow={tr2({ uz: "Debugging", ru: "Дебаггинг" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : found ? tr2({ uz: "Endi tuzating", ru: "Теперь почините" }) : tr2({ uz: "Muammoni toping", ru: "Найдите проблему" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>AI server yozdi, lekin brauzer <span className="italic" style={{ color: T.accent }}>"ulanib bo'lmadi"</span> deyapti. Nega?</>, ru: <>AI написал сервер, но браузер говорит <span className="italic" style={{ color: T.accent }}>«не удалось подключиться»</span>. Почему?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="ai-card fade-up delay-1">
              <div className="ai-row"><span className="ai-badge">AI</span><span className="ai-bubble">{tr2({ uz: "Serveringiz:", ru: "Ваш сервер:" })}</span></div>
              <pre className="code-box" style={{ fontSize: 12.5, boxShadow: "none" }}>
                ...
                {fixed && <div className="el-in" style={{ marginTop: 8 }}><At>app</At>{"."}<Jx>{"listen"}</Jx>{"("}<St>3000</St>{")   "}<span style={{ color: CODE.str }}>{tr2({ uz: "// ✓ qo'shildi", ru: "// ✓ добавлено" })}</span></div>}
              </pre>
              {found && !fixed && <button className="btn fade-step" style={{ alignSelf: "flex-start" }} onClick={() => setFixed(true)}>{tr2({ uz: "🔧 app.listen(3000) qo'shish", ru: "🔧 Добавить app.listen(3000)" })}</button>}
              {fixed && <p className="ai-prompt" style={{ color: T.success, fontStyle: "normal", fontWeight: 600 }}>{tr2({ uz: "✓ Endi server yoqildi!", ru: "✓ Теперь сервер включён!" })}</p>}
            </div>
          ...
          <Col>
            {!found && <>
              <p className="flow-label">{tr2({ uz: "Nima yetishmayapti?", ru: "Чего не хватает?" })}</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {OPTS.map((o) => <button key={o.id} className={`pick-row ${picked === o.id ? "on" : ""}`} onClick={() => {
    if (o.id !== "listen" && achMiss) achMiss.miss(screen);
    setPicked(o.id);
  }}><span className="pick-box">{picked === o.id && "•"}</span><span className="body" style={{ color: T.ink }}>{tr2(o.label)}</span></button>)}
              </div>
              {picked && picked !== "listen" && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Bu qator kodda bor. Yana qarang: serverni <b>yoqadigan</b> qator (app.listen) bormi?</>, ru: <>Эта строка в коде есть. Посмотрите ещё раз: есть ли строка, которая <b>включает</b> сервер (app.listen)?</> })}</p></div>}
            </>}
            {!done && <AchRule screen={screen} />}
            {found && !fixed && <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.accent }}>{tr2({ uz: "✓ Topdingiz!", ru: "✓ Нашли!" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <><span className="mono">app.listen(3000)</span> yo'q — shuning uchun server umuman <b>yoqilmagan</b>, brauzer ulana olmaydi. Chapdagi tugma bilan qo'shing →</>, ru: <>Нет <span className="mono">app.listen(3000)</span> — поэтому сервер вообще <b>не включён</b>, браузер не может подключиться. Добавьте кнопкой слева →</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  useEffect4(() => {
    if (valid && !passed) {
      setPassed(true);
      onAnswer(screen, { stage: "final", screenIdx: screen, question: "app.get('/salom', ...) endpoint'ini yozing", studentAnswer: value, correct: true, firstAttemptCorrect: true, solved: true, picked: value });
    }
  }, [valid]);
  const Ln = ({ n, children }) => <div className="vsc-line"><span className="vsc-ln">{n}</span><span style={{ whiteSpace: "pre" }}>{children}</span></div>;
  const navLabel = ran ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : valid ? tr2({ uz: "▶ Run bosing", ru: "▶ Нажмите Run" }) : tr2({ uz: "Endpoint qatorini yozing", ru: "Напишите строку endpoint" });
  ...
  return <Stage eyebrow={tr2({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Birinchi serveringizni <span className="italic" style={{ color: T.accent }}>javob berdiring</span>.</>, ru: <>Заставьте первый сервер <span className="italic" style={{ color: T.accent }}>ответить Вам</span>.</> })}</h2></div>
          ...
          <Col>
            <div className="vsc fade-up delay-1">
              ...
              <div className="vsc-body">
                ...
                <Ln n={3}>{" "}</Ln>
                <div className="vsc-line"><span className="vsc-ln">4</span><input className={`vsc-input ${valid ? "ok" : ""}`} value={value} onChange={(e) => {
    setValue(e.target.value);
    setRan(false);
  }} placeholder="app.get('/salom', (req, res) => res.send('...'))" spellCheck={false} autoCapitalize="off" autoCorrect="off" /></div>
                <Ln n={5}>{" "}</Ln>
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "Terminal", ru: "Терминал" })}</p>
            <div className="code-box" style={{ minHeight: 44 }}>{ran ? <TLine out={<span style={{ color: CODE.str }}>{tr2({ uz: "✓ Server localhost:3000 da ishlayapti", ru: "✓ Сервер работает на localhost:3000" })}</span>} /> : <span style={{ color: CODE.comment, fontFamily: "'JetBrains Mono'", fontSize: 12 }}>{tr2({ uz: "Run bosilmagan…", ru: "Run не нажат…" })}</span>}</div>
            <p className="flow-label" style={{ marginTop: 2 }}>{tr2({ uz: "Brauzer", ru: "Браузер" })}</p>
            <Win title="localhost:3000/salom" minH={86} hotTitle={ran}>
              {ran ? <div className="demo-swap" style={{ fontFamily: "Georgia, serif", fontSize: 19, color: T.ink }}>{reply}</div> : <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, fontSize: 13 }}>{valid ? tr2({ uz: "▶ Run bosing — serveringiz javob beradi", ru: "▶ Нажмите Run — Ваш сервер ответит" }) : tr2({ uz: "Endpoint qatorini yozing…", ru: "Напишите строку endpoint…" })}</p>}
            </Win>
            {ran && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>🎉 Tabriklaymiz! Sizning birinchi serveringiz ishga tushdi va <b>"{reply}"</b> deb javob berdi. Siz endi backend yoza olasiz!</>, ru: <>🎉 Поздравляем! Ваш первый сервер запустился и ответил: <b>"{reply}"</b>. Теперь Вы умеете писать бэкенд!</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Birinchi serveringiz <span className="italic" style={{ color: T.accent }}>javob berdi</span>.</>, ru: <>Ваш первый сервер <span className="italic" style={{ color: T.accent }}>ответил</span>.</> })}</h2>{
    /* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>{tr2({ uz: "📝 Uyga vazifa", ru: "📝 Домашнее задание" })}</div><p className="body" style={{ margin: "0 0 10px", color: T.ink }}>{tr2({ uz: "O'z serveringizni quring:", ru: "Постройте свой сервер:" })}</p><ul>{HOMEWORK.map((h, i) => <li key={i}><b>{tr2(h.b)}</b> <span className="t">{tr2(h.t)}</span></li>)}</ul><p className="hw-note">{tr2({ uz: "Keyingi darsda serverni PostgreSQL bazaga ulab, haqiqiy ma'lumot qaytaramiz! 🚀", ru: "На следующем уроке подключим сервер к базе PostgreSQL и вернём настоящие данные! 🚀" })}</p></div>}
        {!isMentorL && <div className="card ach-coll fade-up d3">
          <div className="card-lbl" style={{ color: T.accent }}>{tr2({ uz: "🏅 Nishonlaringiz — ", ru: "🏅 Ваши бейджи — " })}{achievements ? achievements.size : 0}/{Object.keys(ACHIEVEMENTS).length}</div>
...
var ScreenNodePractice = (props) => <ScreenLivePractice
  {...props}
...
var ACH_TRIGGERS = { s13: "serverArchitect", s14: "bugHunter", s15: "shopOpener" };
...
var Q_LABELS = { 4: "1 — Node.js", 6: "2 — npm", 10: "3 — endpoint", 13: "4 — app.listen", 16: "5 — server" };
var QUIZ_MS = 15e3;
var QZ_BG_SHAPES = [
  ...
  { ch: "node server.js", l: 34, t: 58, s: 20, d: 18, dl: 1 }
];
var QUIZ_BANK = [
...
function QuizArena({ live, onClose, startSolo }) {
  const _achGate = useContext2(LiveGateCtx) || {};
  ...
  const answer = (i) => {
    ...
    if (correct) {
      ...
      for (let k = qi - 1; k >= 0; k--) {
        if (myAnswers[k]?.correct) _st++;
        else break;
      }
      if (_st >= 2 && _earn) _earn("lightningClerk");
    }
...
function NodeServerLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  return <LangContext.Provider value={lang}>
      ...
      <LiveGateCtx.Provider value={{ locked, live, earn }}>
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={tr2({ uz: "Node.js server darsi", ru: "Урок: сервер на Node.js" })} /> : <>
