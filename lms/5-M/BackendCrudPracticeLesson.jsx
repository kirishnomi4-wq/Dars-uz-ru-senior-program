...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  }}>{"pool.query('INSERT INTO cars (nom, "}<At>price</At>{", yil)...');"}{"  "}<Cm>{"// price?"}</Cm></div> : <div className="ai-line ok fixed-line">{"pool.query('INSERT INTO cars (nom, "}<At>narx</At>{", yil)...');"}{"  "}<Cm>{"// ✓ narx"}</Cm></div>}
                <div className={`ai-line ${picked === "c" ? "ok" : ""} ${!found ? "scan" : ""}`} style={{ animationDelay: "1s" }} onClick={() => {
    if (!found) setPicked("c");
