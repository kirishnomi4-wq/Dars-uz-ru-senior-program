...
function RecapOverlay({ screenIdx, onClose }) {
  ...
  return <div className="rc-overlay">
      ...
      <div className="rc-nav">
        ...
        {last ? <button className="rc-btn done" onClick={onClose}>{tr({ uz: "✓ Tushunarli — davom etamiz", ru: "✓ Понятно — продолжаем" })}</button> : <button className="rc-btn" onClick={() => setI(i + 1)}>{tr({ uz: "Keyingisi →", ru: "Далее →" })}</button>}
      </div>
    </div>;
}
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <p className="mstats-verdict-t">{tr({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верно — класс не разобрался в теме. Перед продолжением рекомендуем короткое повторение.</> })}</p>}
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Kirish", ru: "Вступление" })} screen={screen} scrollSignal={sc} navContent={<NavNext optionalLive disabled={picked === null} label={tr({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Asos · 2 dastur", ru: "Основа · 2 программы" })} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Ikkalasini ishga tushiring", ru: "Запустите обе" })} onClick={onNext} /></>}>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "1-qadam · ME'MOR", ru: "Шаг 1 · АРХИТЕКТОР" })} screen={screen} scrollSignal={done || wrong} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Ma'lumot manbasini tanlang", ru: "Выберите источник данных" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Ma'lumot qayerdan kelishini tanlang: brauzerdami yoki serverdami?", ru: "💡 Выберите, откуда приходят данные: из браузера или с сервера?" })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "2-qadam · REJISSYOR", ru: "Шаг 2 · РЕЖИССЁР" })} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "AI'ga buyruq bering", ru: "Дайте команду AI" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            ...
            <div className="ai-card fade-up delay-1">
              <div className="ai-row"><span className="you-badge">{tr({ uz: "Siz", ru: "Вы" })}</span><span className="ai-bubble">{tr({ uz: `"Katalogda const cars o'rniga, sahifa ochilganda GET /api/cars dan fetch qilib useState'ga saqla."`, ru: "«В каталоге вместо const cars при открытии страницы сделай fetch из GET /api/cars и сохрани в useState.»" })}</span></div>
              {!done ? <button className="btn fade-step" style={{ alignSelf: "flex-start" }} onClick={() => setDone(true)}>{tr({ uz: "AI'ga yuborish →", ru: "Отправить AI →" })}</button> : <>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 AI'ga buyruq bering — qaysi manzildan o'qishini aniq ayting.", ru: "💡 Дайте команду AI — точно скажите, с какого адреса читать." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tuzilishi · oqim", ru: "Строение · поток" })} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "So'rov yo'lini kuzating", ru: "Проследите путь запроса" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
    ...
    return <React.Fragment key={i}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 So'rov yo'lining keyingi bo'g'inini bosing — front'dan bazagacha boradi.", ru: "💡 Нажмите следующее звено пути запроса — он идёт от фронта до базы." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(storedAnswer ? /* @__PURE__ */ new Set(["loading", "error", "data"]) : /* @__PURE__ */ new Set(["data"]));
  const [sc, setSc] = useState(0);
  ...
  return <Stage eyebrow={tr({ uz: "Holatlar", ru: "Состояния" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${tr({ uz: "Uch holatni ko'ring", ru: "Посмотрите три состояния" })} (${seen.size}/3)`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Uchala holatni ham oching: kutish · muvaffaqiyat · xato.", ru: "💡 Откройте все три состояния: ожидание · успех · ошибка." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "3-qadam · NAZORATCHI", ru: "Шаг 3 · КОНТРОЛЁР" })} screen={screen} scrollSignal={phase} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Saytni ishga tushiring va tuzating", ru: "Запустите сайт и почините" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            <button className={`btn ${phase === 0 ? "tap-hint" : ""}`} style={{ alignSelf: "flex-start" }} disabled={phase >= 1 && phase < 3 ? false : phase === 0 ? false : true} onClick={phase === 0 ? run : void 0}>{phase === 0 ? tr({ uz: "▶ Saytni ishga tushirish", ru: "▶ Запустить сайт" }) : phase < 3 ? tr({ uz: "Yuklanmoqda / xato…", ru: "Загрузка / ошибка…" }) : tr({ uz: "✓ Ishladi", ru: "✓ Заработало" })}</button>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Avval saytni ishga tushiring, keyin konsoldagi xatoni o'qib tuzating.", ru: "💡 Сначала запустите сайт, потом прочитайте ошибку в консоли и исправьте." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "4-qadam · forma", ru: "Шаг 4 · форма" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Qo'shing va yangilang", ru: "Добавьте и обновите" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Ikki amal kerak: formaga yozib qo'shing, keyin ro'yxatni yangilang.", ru: "💡 Нужны два действия: добавьте через форму, затем обновите список." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
var Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Ma'lumot yo'li", ru: "Путь данных" })} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Yo'lni kuzating", ru: "Проследите путь" })} onClick={onNext} /></>}>
    ...
    return isArr ? <span key={i} className="flow-arr" style={{ color: step > i ? T.accent : T.ink3, fontWeight: 700 }}>{tr(c)}</span> : <div key={i} className="flow-node" style={{ background: lit ? T.accent : T.paper, color: lit ? "#fff" : T.ink3 }}>{tr(c)}</div>;
  })}
          </div>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Doira to'liq aylanishi kerak — keyingi qadamni bosing.", ru: "💡 Круг должен замкнуться — нажмите следующий шаг." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Amaliyot · to'liq ilova", ru: "Практика · полное приложение" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Qo'shing va yangilang", ru: "Добавьте и обновите" })} onClick={onNext} /></>}>
...
/>;
var Screen15 = ({ screen, onNext, onPrev }) => <Stage eyebrow={tr({ uz: "Qoida · xulosa", ru: "Правило · итог" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr({ uz: "Oxirgi qadam →", ru: "Последний шаг →" })} onClick={onNext} /></>}>
    <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
...
function ScreenLivePractice({ title, task, checklist, screen, storedAnswer, onAnswer, onNext, onPrev, live }) {
  ...
  return <Stage eyebrow={tr({ uz: "Amaliyot · VS Code", ru: "Практика · VS Code" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Avval bajaring", ru: "Сначала выполните" })} onClick={onNext} /></>}>
    ...
    return <button key={i} className={`lp-step ${on ? "on" : ""}`} onClick={() => toggle(i)}>
            ...
            {done && !isMentor && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: "Juda yaxshi! Saytingizni serverga o'zingiz uladingiz. Ustoz tekshirib, keyingi qadamga o'tkazadi.", ru: "Отлично! Вы сами подключили свой сайт к серверу. Наставник проверит и переведёт на следующий шаг." })}</p></div>}
          </Col>
        </div>
      </div>
    </Stage>;
...
function AchCelebrate({ ach, onDone }) {
  ...
  return <div className="acu-overlay" onClick={onDone} role="status" aria-label={`${tr({ uz: "Yangi nishon:", ru: "Новый бейдж:" })} ${ach.name}`}>
...
var Screen17 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: "Birinchi fullstack ilovangiz", ru: "Ваше первое fullstack-приложение" })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Front va backni <span className="italic" style={{ color: T.accent }}>o'zingiz uladingiz</span>.</>, ru: <>Фронт и бэк <span className="italic" style={{ color: T.accent }}>вы соединили сами</span>.</> })}</h2>{
...
function FullstackConnectPracticeLesson({ lang: langProp, onFinished }) {
  ...
  return <LangContext.Provider value={lang}>
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={tr({ uz: "Fullstack ulash — praktika", ru: "Fullstack связка — практика" })} /> : <>
