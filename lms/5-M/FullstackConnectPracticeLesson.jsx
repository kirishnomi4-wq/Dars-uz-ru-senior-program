...
//  Manba:  src/4-Modull/FullstackConnectPracticeLesson.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/4-Modull/FullstackConnectPracticeLesson.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/4-Modull/FullstackConnectPracticeLesson.jsx
...
var CODE = { bg: "#1A2436", text: "#E8E5DD", tag: "#FF7755", attr: "#FFD380", str: "#7DD181", comment: "#6B7585", punct: "#9FB4D8", deco: "#C9A6F5" };
...
var LESSON_META = { lessonId: "fullstack-connect-practice-p2-v18", lessonTitle: { uz: "Praktika: Fullstack ulash — AvtoIjara", ru: "Практика: Fullstack связка — AvtoIjara" } };
var HW_TOKENS = [
...
var INLINE_KEYS = { s4: 2, s7: 1, s9: 3, s14: 0, practice: -1 };
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <p className="mstats-verdict-t">{tr2({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верно — класс не разобрался в теме. Перед продолжением рекомендуем короткое повторение.</> })}</p>}
            ...
            {level === "few" && <p className="mstats-verdict-t">{tr2({ uz: <>Javob berganlar kam ({answered} ta) — foiz bo'yicha xulosa chiqarish qiyin. O'zingiz baholang.</>, ru: <>Ответивших мало ({answered}) — по процентам судить сложно. Оцените сами.</> })}</p>}
...
var QuestionScreen = ({ screen, idx, scope, eyebrow, question, questionText, options, correctIdx, explainCorrect, explainWrong, audioText, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const pick = (i) => {
    ...
    if (oneShot) {
      setSolved(true);
      onAnswer(screen, { stage: scope, screenIdx: screen, question: tr2(questionText), options: options.map((o) => tr2(o)), correctIndex: correctIdx, correctAnswer: tr2(options[correctIdx]), picked: i, studentAnswerIndex: i, studentAnswer: tr2(options[i]), correct: isCorrect, firstAttemptCorrect: isCorrect, solved: true, lastPicked: i });
      if (!fpPractice) live.submitAnswer(screen, SCREEN_META[screen]?.id || `s${screen}`, i, isCorrect, Date.now() - mountTs.current);
    } else {
      if (isCorrect) setSolved(true);
      onAnswer(screen, { stage: scope, screenIdx: screen, question: tr2(questionText), options: options.map((o) => tr2(o)), correctIndex: correctIdx, correctAnswer: tr2(options[correctIdx]), picked: i, studentAnswerIndex: i, studentAnswer: tr2(options[i]), correct: firstCorrectRef.current, firstAttemptCorrect: firstCorrectRef.current, solved: isCorrect, lastPicked: i });
    }
    if (live && live.recordAttempt && !fpPractice) live.recordAttempt(screen, SCREEN_META[screen]?.id || `s${screen}`, i, Date.now() - mountTs.current, { question: tr2(questionText), options: options.map((o) => tr2(o)), picked: tr2(options[i]), correct: tr2(options[correctIdx]), lang: typeof __lang !== "undefined" && __lang === "ru" ? "ru" : "uz" });
  };
...
var CARS = [
  { id: 1, nom: "Chevrolet Cobalt", narx: 28e4, yil: 2022, emoji: "🚗", bandmi: false, rating: "4.8", bg: "linear-gradient(135deg,#7EA6F4,#2E4A9E)" },
  { id: 2, nom: "Chevrolet Malibu", narx: 52e4, yil: 2023, emoji: "🚙", bandmi: true, rating: "4.9", bg: "linear-gradient(135deg,#8FD3A8,#2E7A4E)" },
  { id: 3, nom: "Kia K5", narx: 61e4, yil: 2023, emoji: "🚘", bandmi: false, rating: "4.7", bg: "linear-gradient(135deg,#F4B26A,#C9622E)" }
];
var SPARK = { id: 4, nom: "Chevrolet Spark", narx: 19e4, yil: 2021, emoji: "🚐", bandmi: false, rating: "4.6", bg: "linear-gradient(135deg,#F4A6C0,#B5446E)" };
var TRACKER = { id: 5, nom: "Chevrolet Tracker", narx: 45e4, yil: 2024, emoji: "🚓", bandmi: false, rating: "5.0", bg: "linear-gradient(135deg,#B9A8F4,#6A4AC9)" };
var CARS_DB = [...CARS, SPARK];
var SiteCard = ({ car, isNew }) => <div className={`scard el-in ${isNew ? "scard-new" : ""}`}>
    <div className="scard-img" style={{ background: car.bg }}>
      <span className="scard-emoji">{car.emoji}</span>
      <span className={`scard-tag ${car.bandmi ? "busy" : "free"}`}>{car.bandmi ? tr2({ uz: "Band", ru: "Занята" }) : tr2({ uz: "Bo'sh", ru: "Свободна" })}</span>
      {isNew && <span className="scard-newbadge">{tr2({ uz: "yangi", ru: "новая" })}</span>}
    </div>
    <div className="scard-info">
      <div className="scard-top"><span className="scard-name">{car.nom}</span><span className="scard-year">{car.yil}</span></div>
      <div className="scard-meta">★ {car.rating} {tr2({ uz: "· Avtomat · Konditsioner", ru: "· Автомат · Кондиционер" })}</div>
      <div className="scard-bottom">
        <span className="scard-price">{sp(car.narx)}<small> {tr2({ uz: "so'm/kun", ru: "сум/день" })}</small></span>
        <span className="scard-btn">{tr2({ uz: "Ijaraga olish", ru: "Арендовать" })}</span>
      </div>
    </div>
  </div>;
var AvtoSite = ({ cars = [], state = "data", newId, cols = 3 }) => <div className="site">
    <div className="site-nav">
      <span className="site-logo">🚗 Avto<b>Ijara</b></span>
      <span className="site-links"><span>{tr2({ uz: "Bosh sahifa", ru: "Главная" })}</span><span className="on">{tr2({ uz: "Mashinalar", ru: "Машины" })}</span><span>{tr2({ uz: "Aloqa", ru: "Контакты" })}</span></span>
    </div>
    <div className="site-hero"><span className="site-h">{tr2({ uz: "Ijaraga mashinalar", ru: "Машины в аренду" })}</span><span className="site-sub">{tr2({ uz: "Toshkent bo'ylab · kunlik ijara", ru: "По Ташкенту · посуточная аренда" })}</span></div>
    <div className="site-main">
      {state === "loading" ? <div className="site-grid" style={{ gridTemplateColumns: `repeat(${cols},1fr)` }}>{[0, 1, 2].slice(0, cols).map((i) => <div key={i} className="scard skel"><div className="scard-img skel-box" /><div className="scard-info"><div className="skel-line" /><div className="skel-line short" /><div className="skel-line" style={{ width: "50%" }} /></div></div>)}</div> : state === "error" ? <div className="site-msg err"><span className="site-msg-ico">⚠️</span>{tr2({ uz: "Ma'lumotni yuklab bo'lmadi", ru: "Не удалось загрузить данные" })}<small>{tr2({ uz: "serverga ulanib bo'lmadi", ru: "нет соединения с сервером" })}</small></div> : cars.length ? <div className="site-grid" style={{ gridTemplateColumns: `repeat(${cols},1fr)` }}>{cars.map((c) => <SiteCard key={c.id} car={c} isNew={c.id === newId} />)}</div> : <div className="site-msg"><span className="site-msg-ico">🚗</span>{tr2({ uz: "Hozircha mashina yo'q", ru: "Пока машин нет" })}</div>}
    </div>
  </div>;
var DbTable = ({ rows, flashId }) => <div className="db">
    <div className="db-cap">🗄️ <b>cars</b> <span>· {rows.length} {tr2({ uz: "qator", ru: "строк" })}</span></div>
    <div className="db-row db-head"><span>id</span><span>nom</span><span>narx</span><span>yil</span></div>
    {rows.map((r) => <div key={r.id} className={`db-row el-in ${flashId === r.id ? "flash" : ""}`}>
        <span>{r.id}</span><span>{r.nom}</span><span>{sp(r.narx)}</span><span>{r.yil}</span>
      </div>)}
  </div>;
var Konsol = ({ children, error }) => <div className="kons"><div className="kons-bar"><span className="kons-dot" /> Console</div><div className={`kons-body ${error ? "err" : ""}`}>{children}</div></div>;
var ZAPS = [
  { x: "8px", y: "-9px", d: "0s" },
  { x: "-7px", y: "-6px", d: "0.14s" },
  { x: "6px", y: "7px", d: "0.28s" },
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Вступление" })} screen={screen} scrollSignal={sc} navContent={<NavNext optionalLive disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>Bazaga mashina qo'shdik — lekin saytda <span className="italic" style={{ color: T.accent }}>ko'rinmayapti</span>. Nega?</>, ru: <>Добавили машину в базу — а на сайте её <span className="italic" style={{ color: T.accent }}>не видно</span>. Почему?</> })}</h1>
        ...
        <div className="wire cut fade-up delay-1">
          <span className="wire-end"><span className="wire-ico">💻</span><span className="wire-t">{tr2({ uz: "Sayt — vitrina", ru: "Сайт — витрина" })}<small>localhost:5173</small></span></span>
          <span className="wire-mid">
            ...
            <span className="wire-lbl">{tr2({ uz: "sim uzilgan — vitrina omborni ko'rmayapti", ru: "провод оборван — витрина не видит склад" })}</span>
          </span>
          <span className="wire-end"><span className="wire-ico">🗄️</span><span className="wire-t">{tr2({ uz: "Baza — ombor", ru: "База — склад" })}<small>localhost:3000</small></span></span>
        </div>
        <Zoomable>
        <Split three>
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "Sayt — front (localhost:5173)", ru: "Сайт — фронт (localhost:5173)" })}</p>
              <button className={`chip ${shaking ? "shake" : ""} ${!tried ? "tap-hint" : ""}`} onClick={refresh} style={{ padding: "5px 10px", fontSize: 12.5, flexShrink: 0, whiteSpace: "nowrap" }}>{tr2({ uz: "🔄 Yangilash", ru: "🔄 Обновить" })}</button>
            </div>
            <div className={`fade-up delay-1 ${shaking ? "shake" : ""}`}><Win title="avtoijara.uz" minH={150} maxH={180}><AvtoSite cars={CARS} cols={1} /></Win></div>
            {tried && <p className="small fade-step" style={{ color: T.accent, fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Yangiladingiz — lekin baribir 3 ta mashina. Spark yo'q!", ru: "Обновили — а машин всё равно 3. Spark так и нет!" })}</p>}
          </Col>
          <Col>
            <p className="flow-label fade-up delay-2" style={{ margin: 0 }}>{tr2({ uz: "Baza — back (localhost:3000)", ru: "База — бэк (localhost:3000)" })}</p>
            <div className="fade-up delay-2"><DbTable rows={CARS_DB} flashId={SPARK.id} /></div>
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-3" style={{ color: T.ink2, margin: "4px 0 0", paddingRight: 40 }}>{tr2({ uz: "Sayt 3 ta, baza 4 ta. Nega ular bir xil emas?", ru: "На сайте 3, в базе 4. Почему не совпадают?" })}</p>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const PreviewBlock = <Col>
      <p className="flow-label">{tr2({ uz: "Dars oxirida — sayt serverdan o'qiydi", ru: "К концу урока — сайт читает с сервера" })}</p>
      <Win title="avtoijara.uz" minH={150} maxH={230}><AvtoSite cars={CARS_DB} newId={SPARK.id} cols={2} /></Win>
      <p className="mono small" style={{ color: T.success, margin: 0 }}>{tr2({ uz: "→ Spark paydo bo'ldi! Sayt endi bazadagi 4 mashinani ko'rsatadi", ru: "→ Spark появился! Сайт теперь показывает 4 машины из базы" })}</p>
    </Col>;
  ...
  return <Stage eyebrow={tr2({ uz: "Reja", ru: "План" })} screen={screen} mentorStatic scrollSignal={showSteps} navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Frontni backendga <span className="italic" style={{ color: T.accent }}>qanday</span> ulaymiz?</>, ru: <>Как <span className="italic" style={{ color: T.accent }}>соединить</span> фронт с бэкендом?</> })}</h2></div>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [front, setFront] = useState3(!!storedAnswer);
  const [back, setBack] = useState3(!!storedAnswer);
  const done = front && back;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, (front ? 1 : 0) + (back ? 1 : 0));
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  return <Stage eyebrow={tr2({ uz: "Asos · 2 dastur", ru: "Основа · 2 программы" })} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Ikkalasini ishga tushiring", ru: "Запустите обе" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Sayt va server — <span className="italic" style={{ color: T.accent }}>bitta dasturmi</span> yoki ikkita?</>, ru: <>Сайт и сервер — <span className="italic" style={{ color: T.accent }}>одна программа</span> или две?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="run-card fade-up delay-1">
              <div className="run-top"><span className="run-name">{tr2({ uz: "💻 Front — sayt", ru: "💻 Фронт — сайт" })}</span><span className="run-port">:5173</span></div>
              ...
              <button className={`btn-soft ${front ? "" : "tap-hint"}`} disabled={front} onClick={() => setFront(true)} style={{ alignSelf: "flex-start" }}>{front ? tr2({ uz: "✓ Ishlayapti", ru: "✓ Работает" }) : tr2({ uz: "▶ Ishga tushirish", ru: "▶ Запустить" })}</button>
            </div>
            <div className="run-card fade-up delay-2">
              <div className="run-top"><span className="run-name">{tr2({ uz: "🟢 Back — server", ru: "🟢 Бэк — сервер" })}</span><span className="run-port">:3000</span></div>
              <div className="code-box" style={{ padding: "9px 12px", minHeight: 40 }}>
                <TLine cmd="node server.js" />
                {back && <TLine out={<span style={{ color: CODE.str }}>{tr2({ uz: "✓ Server :3000 da ishlayapti", ru: "✓ Сервер работает на :3000" })}</span>} />}
              </div>
              <button className={`btn-soft ${back ? "" : "tap-hint"}`} disabled={back} onClick={() => setBack(true)} style={{ alignSelf: "flex-start" }}>{back ? tr2({ uz: "✓ Ishlayapti", ru: "✓ Работает" }) : tr2({ uz: "▶ Ishga tushirish", ru: "▶ Запустить" })}</button>
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Ular qanday gaplashadi", ru: "Как они общаются" })}</p>
            <div className="fade-up delay-2">
            <div className={`bridge ${done ? "live" : ""}`}>
              <div className="bridge-end" style={{ opacity: front ? 1 : 0.4 }}><span className="bridge-ico">💻</span><span>{tr2({ uz: "Front", ru: "Фронт" })}<br /><small>:5173</small></span></div>
              ...
              <div className="bridge-end" style={{ opacity: back ? 1 : 0.4 }}><span className="bridge-ico">🟢</span><span>{tr2({ uz: "Back", ru: "Бэк" })}<br /><small>:3000</small></span></div>
            </div>
            </div>
            {done ? <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Ikkalasi ishlayapti! Front <span className="mono">fetch</span> bilan so'raydi, back <span className="mono">JSON</span> bilan javob beradi. Biri o'chsa — gaplashuv uziladi.</>, ru: <>Обе работают! Фронт спрашивает через <span className="mono">fetch</span>, бэк отвечает <span className="mono">JSON</span>. Выключится одна — разговор оборвётся.</> })}</p></div> : <div className="hint"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: "Faqat bittasi ishlasa — ulanish bo'lmaydi. Ikkalasini ham yoqing.", ru: "Если работает только одна — связи не будет. Включите обе." })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Ikkala oyna ham yonishi kerak — hali o'chig'ini bosib ishga tushiring.", ru: "💡 Оба окна должны гореть — включите то, что ещё выключено." })}</p>}
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [pick, setPick] = useState3(storedAnswer ? "server" : null);
  const [wrong, setWrong] = useState3(false);
  const done = pick === "server";
  const { tip: _tip, rescue: _resc } = useStuckValve(done, pick ? 1 : 0);
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  const choose = (v) => {
    if (done) return;
    if (v === "server") setPick("server");
    else {
      setPick("hard");
      setWrong(true);
    }
  };
  return <Stage eyebrow={tr2({ uz: "1-qadam · ME'MOR", ru: "Шаг 1 · АРХИТЕКТОР" })} screen={screen} scrollSignal={done || wrong} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Ma'lumot manbasini tanlang", ru: "Выберите источник данных" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Katalog mashinalarni <span className="italic" style={{ color: T.accent }}>qayerdan</span> olishi kerak?</>, ru: <>Откуда каталог <span className="italic" style={{ color: T.accent }}>должен брать</span> машины?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Ma'lumot manbasi", ru: "Источник данных" })}</p>
            <button className="vcard" onClick={() => choose("hard")} style={{ boxShadow: pick === "hard" ? `inset 0 0 0 1.5px ${T.accent}` : void 0, alignItems: "flex-start", flexDirection: "column", gap: 4 }}>
              <span className="vlbl">{tr2({ uz: "Kod ichiga yozilgan ro'yxat (eski usul)", ru: "Список, зашитый в код (старый способ)" })}</span>
              <span className="mono small" style={{ color: T.ink3 }}>const cars = [ ... ]</span>
            </button>
            <button className="vcard" onClick={() => choose("server")} style={{ boxShadow: pick === "server" ? `inset 0 0 0 1.5px ${T.success}` : void 0, alignItems: "flex-start", flexDirection: "column", gap: 4 }}>
              <span className="vlbl">{tr2({ uz: "Serverdan, bazadan olish (fetch)", ru: "С сервера, из базы (fetch)" })}</span>
              <span className="mono small" style={{ color: T.ink3 }}>fetch('/api/cars')</span>
            </button>
            {wrong && !done && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Bu — eski usul: mashinalar to'g'ridan-to'g'ri kodga yozilgan. Bazaga Spark qo'shilsa ham, sayt buni ko'rmaydi — chunki u koddagi ro'yxatdan o'qiydi. Bazadagi <b>jonli</b> ma'lumot uchun — <span className="mono">fetch</span>.</>, ru: <>Это старый способ: машины записаны прямо в код. Даже если в базу добавят Spark, сайт этого не увидит — он читает список из кода. Для <b>живых</b> данных из базы — <span className="mono">fetch</span>.</> })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Ma'lumot oqimi", ru: "Поток данных" })}</p>
            <div className="chain fade-up delay-1">
              <div className="chain-node" style={{ background: T.paper, color: T.ink }}>{tr2({ uz: "React katalog", ru: "React-каталог" })}</div>
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>To'g'ri! Sayt <span className="mono">fetch</span> bilan serverga boradi, server bazadan oladi. Endi baza yangilansa — sayt ham yangilanadi. Bu — <b>jonli</b> ulanish.</>, ru: <>Верно! Сайт идёт на сервер через <span className="mono">fetch</span>, сервер берёт из базы. Теперь обновится база — обновится и сайт. Это <b>живая</b> связь.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Ma'lumot qayerdan kelishini tanlang: brauzerdami yoki serverdami?", ru: "💡 Выберите, откуда приходят данные: из браузера или с сервера?" })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen4 = (props) => <QuestionScreen
  ...
  question={tr2({ uz: <><p className="eyebrow" style={{ color: T.accent }}>To'g'ri javobni tanlang</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Front va back o'zaro <span className="italic" style={{ color: T.accent }}>qanday</span> gaplashadi?</h2></>, ru: <><p className="eyebrow" style={{ color: T.accent }}>Выберите правильный ответ</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Как фронт и бэк <span className="italic" style={{ color: T.accent }}>общаются</span> между собой?</h2></> })}
  options={[{ uz: "Bir xil fayl ichida — to'g'ridan-to'g'ri", ru: "Внутри одного файла — напрямую" }, { uz: "Hech qanday — har biri alohida ishlaydi", ru: "Никак — каждый работает сам по себе" }, { uz: "fetch — HTTP so'rov orqali", ru: "fetch — через HTTP-запрос" }, { uz: "Faqat CSS fayli orqali", ru: "Только через CSS-файл" }]}
  correctIdx={2}
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "2-qadam · REJISSYOR", ru: "Шаг 2 · РЕЖИССЁР" })} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "AI'ga buyruq bering", ru: "Дайте команду AI" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Mashinalarni serverdan <span className="italic" style={{ color: T.accent }}>qanday yuklab</span> olamiz?</>, ru: <>Как <span className="italic" style={{ color: T.accent }}>загрузить</span> машины с сервера?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Aniq buyruq (prompt)", ru: "Чёткая команда (промпт)" })}</p>
            <div className="ai-card fade-up delay-1">
              <div className="ai-row"><span className="you-badge">{tr2({ uz: "Siz", ru: "Вы" })}</span><span className="ai-bubble">{tr2({ uz: `"Katalogda const cars o'rniga, sahifa ochilganda GET /api/cars dan fetch qilib useState'ga saqla."`, ru: "«В каталоге вместо const cars при открытии страницы сделай fetch из GET /api/cars и сохрани в useState.»" })}</span></div>
              {!done ? <button className="btn fade-step" style={{ alignSelf: "flex-start" }} onClick={() => setDone(true)}>{tr2({ uz: "AI'ga yuborish →", ru: "Отправить AI →" })}</button> : <>
                    <div className="ai-row"><span className="ai-badge">AI</span><span className="ai-bubble">{tr2({ uz: "Mana ulanish kodi:", ru: "Вот код подключения:" })}</span></div>
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "Har bir qator nima qiladi", ru: "Что делает каждая строка" })}</p>
            <div className="sk-info"><p className="body" style={{ margin: 0, color: T.ink }}><span className="mono" style={{ color: T.accent }}>useState([])</span>{tr2({ uz: " — boshida ro'yxat bo'sh.", ru: " — в начале список пуст." })}</p></div>
            <div className="sk-info"><p className="body" style={{ margin: 0, color: T.ink }}><span className="mono" style={{ color: T.accent }}>useEffect(...[])</span>{tr2({ uz: " — sahifa ochilganda bir marta ishlaydi.", ru: " — срабатывает один раз при открытии страницы." })}</p></div>
            <div className="sk-info"><p className="body" style={{ margin: 0, color: T.ink }}><span className="mono" style={{ color: T.accent }}>fetch → json → setCars</span>{tr2({ uz: " — serverdan oladi, JSON'ga aylantiradi, ro'yxatga saqlaydi.", ru: " — получает с сервера, превращает в JSON, сохраняет в список." })}</p></div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Kod tayyor ko'rinadi. Lekin haqiqatan ishlaydimi? Buni NAZORATCHI sifatida brauzerda sinaymiz.", ru: "Код выглядит готовым. Но правда ли он работает? Проверим это в браузере — как КОНТРОЛЁР." })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 AI'ga buyruq bering — qaysi manzildan o'qishini aniq ayting.", ru: "💡 Дайте команду AI — точно скажите, с какого адреса читать." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var FLOW = [{ uz: "React: fetch", ru: "React: fetch" }, { uz: "GET /api/cars", ru: "GET /api/cars" }, { uz: "Server", ru: "Сервер" }, { uz: "JSON javob", ru: "JSON-ответ" }, { uz: "setCars → ekran", ru: "setCars → экран" }];
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Tuzilishi · oqim", ru: "Строение · поток" })} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "So'rov yo'lini kuzating", ru: "Проследите путь запроса" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bitta <span className="italic" style={{ color: T.accent }}>fetch</span> bosilganda nima sodir bo'ladi?</>, ru: <>Что происходит при одном <span className="italic" style={{ color: T.accent }}>fetch</span>?</> })}</h2></div>
        ...
        <Col>
          <div className="flow-strip fade-up delay-1">
            {FLOW.map((f, i) => {
    const lit = step >= i;
    return <React3.Fragment key={i}>
                  <div className="flow-node" style={{ background: lit ? T.accent : T.paper, color: lit ? "#fff" : T.ink3, boxShadow: lit ? `0 6px 16px -5px rgba(255,79,40,0.45)` : `0 4px 12px -6px rgba(${T.shadowBase},0.16)` }}>{tr2(f)}</div>
                  {i < FLOW.length - 1 && <span className="flow-arr" style={{ color: step > i ? T.accent : T.ink3 }}>→</span>}
                </React3.Fragment>;
  })}
          </div>
          <button className="btn" style={{ alignSelf: "flex-start" }} disabled={done} onClick={() => setStep((s) => Math.min(s + 1, FLOW.length - 1))}>{step < 0 ? tr2({ uz: "▶ So'rovni yuborish", ru: "▶ Отправить запрос" }) : done ? tr2({ uz: "✓ Javob keldi", ru: "✓ Ответ пришёл" }) : tr2({ uz: "Keyingi qadam →", ru: "Следующий шаг →" })}</button>
          {step >= 0 && <div className="sk-info fade-step" key={step}><p className="body" style={{ margin: 0, color: T.ink }}><b style={{ color: T.accent }}>{step < FLOW.length ? tr2(FLOW[Math.min(step, FLOW.length - 1)]) : ""}</b> — {tr2(NOTES[Math.min(step, NOTES.length - 1)])}</p></div>}
          {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Mana to'liq yo'l: so'rov chiqadi → server javob beradi → ekran yangilanadi. Endi buni haqiqatan ishga tushiramiz.", ru: "Вот полный путь: запрос уходит → сервер отвечает → экран обновляется. Теперь запустим это по-настоящему." })}</p></div>}
        </Col>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 So'rov yo'lining keyingi bo'g'inini bosing — front'dan bazagacha boradi.", ru: "💡 Нажмите следующее звено пути запроса — он идёт от фронта до базы." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen7 = (props) => <QuestionScreen
  ...
  question={tr2({ uz: <><p className="eyebrow" style={{ color: T.accent }}>Mustahkamlash</p><h2 className="title h-ask" style={{ marginTop: 8 }}>fetch <span className="italic" style={{ color: T.accent }}>qachon</span> ishga tushsin?</h2></>, ru: <><p className="eyebrow" style={{ color: T.accent }}>Закрепление</p><h2 className="title h-ask" style={{ marginTop: 8 }}><span className="italic" style={{ color: T.accent }}>Когда</span> должен срабатывать fetch?</h2></> })}
  options={[{ uz: "Har soniyada uzluksiz qayta-qayta", ru: "Каждую секунду, снова и снова" }, { uz: "Sahifa ochilganda bir marta — useEffect ichida", ru: "Один раз при открытии страницы — внутри useEffect" }, { uz: "Foydalanuvchi biror tugma bosmaguncha hech qachon", ru: "Никогда, пока пользователь не нажмёт кнопку" }, { uz: "CSS fayli yuklanib bo'lganda", ru: "Когда загрузится CSS-файл" }]}
  correctIdx={1}
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [st, setSt] = useState3("data");
  const [seen, setSeen] = useState3(storedAnswer ? /* @__PURE__ */ new Set(["loading", "error", "data"]) : /* @__PURE__ */ new Set(["data"]));
  const [sc, setSc] = useState3(0);
  ...
  return <Stage eyebrow={tr2({ uz: "Holatlar", ru: "Состояния" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : `${tr2({ uz: "Uch holatni ko'ring", ru: "Посмотрите три состояния" })} (${seen.size}/3)`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Ma'lumot kelguncha foydalanuvchi <span className="italic" style={{ color: T.accent }}>nimani ko'radi</span>?</>, ru: <>Что видит пользователь, <span className="italic" style={{ color: T.accent }}>пока данные в пути</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Holatni sinab ko'ring", ru: "Попробуйте состояние" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {STATES.map((s) => <button key={s.k} className="vcard" onClick={() => show(s.k)} style={{ boxShadow: st === s.k ? `inset 0 0 0 1.5px ${T.accent}, 0 8px 20px -6px rgba(${T.shadowBase},0.2)` : void 0 }}>
                  <span className="vlbl">{tr2(s.lbl)}</span>
                  <span className="vseen" style={{ color: seen.has(s.k) ? T.success : T.ink3 }}>{seen.has(s.k) ? "✓" : ""}</span>
                </button>)}
            </div>
            {!done && <div className="sk-info" key={st}><p className="body" style={{ margin: 0, color: T.ink }}>{tr2(STATES.find((s) => s.k === st).note)}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Uch holat ham tayyor! <span className="mono">if (loading)</span> → skeleton, <span className="mono">if (error)</span> → xabar, aks holda → kartochkalar. Foydalanuvchi hech qachon "buzuq" sayt ko'rmaydi.</>, ru: <>Все три состояния готовы! <span className="mono">if (loading)</span> → skeleton, <span className="mono">if (error)</span> → сообщение, иначе → карточки. Пользователь никогда не увидит «сломанный» сайт.</> })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Sayt — shu holatda", ru: "Сайт — в этом состоянии" })}</p>
            <Win title="avtoijara.uz" minH={150} maxH={200}><AvtoSite cars={CARS_DB} state={st} cols={2} /></Win>
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Uchala holatni ham oching: kutish · muvaffaqiyat · xato.", ru: "💡 Откройте все три состояния: ожидание · успех · ошибка." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen9 = (props) => <QuestionScreen
  ...
  question={tr2({ uz: <><p className="eyebrow" style={{ color: T.accent }}>To'g'ri javobni tanlang</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Sayt serverdan ma'lumot olishi uchun <span className="italic" style={{ color: T.accent }}>nima shart</span>?</h2></>, ru: <><p className="eyebrow" style={{ color: T.accent }}>Выберите правильный ответ</p><h2 className="title h-ask" style={{ marginTop: 8 }}><span className="italic" style={{ color: T.accent }}>Что нужно</span>, чтобы сайт получил данные с сервера?</h2></> })}
  options={[{ uz: "Faqat front ishlab tursa ham kifoya qiladi", ru: "Достаточно, чтобы работал только фронт" }, { uz: "Faqat back ishlab tursa ham yetarli bo'ladi", ru: "Хватит и того, что работает только бэк" }, { uz: "Internet aloqasi butunlay o'chiq bo'lishi kerak", ru: "Интернет должен быть полностью выключен" }, { uz: "Front va back — ikkalasi ham ishlab turishi kerak", ru: "Фронт и бэк — работать должны оба" }]}
  correctIdx={3}
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [boom, setBoom] = useState3(false);
  const timer = useRef3(null);
  const cfTimer = useRef3(null);
  useEffect4(() => () => {
    clearTimeout(timer.current);
  ...
  const run = () => {
    setPhase(1);
    clearTimeout(timer.current);
    timer.current = setTimeout(() => setPhase(2), 900);
  };
  const fix = () => {
    setPhase(3);
    setBoom(true);
    clearTimeout(cfTimer.current);
    cfTimer.current = setTimeout(() => setBoom(false), 3600);
  };
  const siteState = phase === 1 ? "loading" : phase === 3 ? "data" : "error";
  const siteCars = phase === 3 ? CARS_DB : [];
  return <Stage eyebrow={tr2({ uz: "3-qadam · NAZORATCHI", ru: "Шаг 3 · КОНТРОЛЁР" })} screen={screen} scrollSignal={phase} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Saytni ishga tushiring va tuzating", ru: "Запустите сайт и почините" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Saytni ochdik — konsoldagi <span className="italic" style={{ color: T.accent }}>qizil xato</span> nimani aytmoqchi?</>, ru: <>Открыли сайт — о чём говорит <span className="italic" style={{ color: T.accent }}>красная ошибка</span> в консоли?</> })}</h2></div>
        ...
        <div className={`gate ${phase === 1 ? "sending" : ""} ${phase === 2 ? "blocked" : ""} ${phase >= 3 ? "open" : ""}`}>
          <span className="gate-end"><span className="gate-ico">💻</span><span className="gate-t">{tr2({ uz: "Front — sayt", ru: "Фронт — сайт" })}<small>localhost:5173</small></span></span>
          <span className="gate-mid">
            <span className="gate-road">
              ...
              {phase >= 3 && <>
                ...
                {Array.from({ length: 8 }).map((_, i) => <i key={i} className="gate-spark" aria-hidden="true" style={{ "--a": `${i * 45}deg`, animationDelay: `${0.12 + i % 3 * 0.05}s` }}>✦</i>)}
              </>}
            </span>
            <span className="gate-status">
              {phase >= 3 ? tr2({ uz: "✓ 200 OK — shlagbaum ochiq, ma'lumot o'tdi", ru: "✓ 200 OK — шлагбаум открыт, данные прошли" }) : phase === 2 ? tr2({ uz: "⛔ CORS policy — brauzer so'rovni to'sdi", ru: "⛔ CORS policy — браузер заблокировал запрос" }) : phase === 1 ? tr2({ uz: "so'rov yo'lda…", ru: "запрос в пути…" }) : tr2({ uz: "so'rov hali yuborilmagan", ru: "запрос ещё не отправлен" })}
            </span>
          </span>
          <span className="gate-end"><span className="gate-ico">🟢</span><span className="gate-t">{tr2({ uz: "Back — server", ru: "Бэк — сервер" })}<small>localhost:3000</small></span></span>
        </div>
        </div>
        <Zoomable>
        <div className="split">
          <Col>
            <button className={`btn ${phase === 0 ? "tap-hint" : ""}`} style={{ alignSelf: "flex-start" }} disabled={phase >= 1 && phase < 3 ? false : phase === 0 ? false : true} onClick={phase === 0 ? run : void 0}>{phase === 0 ? tr2({ uz: "▶ Saytni ishga tushirish", ru: "▶ Запустить сайт" }) : phase < 3 ? tr2({ uz: "Yuklanmoqda / xato…", ru: "Загрузка / ошибка…" }) : tr2({ uz: "✓ Ishladi", ru: "✓ Заработало" })}</button>
            <Konsol error={phase === 2}>
              {phase === 0 && <span style={{ color: CODE.comment }}>{tr2({ uz: "— konsol bo'sh —", ru: "— консоль пуста —" })}</span>}
              ...
              {phase === 3 && <span style={{ color: CODE.str }}>{tr2({ uz: "✓ GET /api/cars → 200 OK (4 mashina yuklandi)", ru: "✓ GET /api/cars → 200 OK (загружено 4 машины)" })}</span>}
            </Konsol>
            <p className="flow-label" style={{ margin: "2px 0 0" }}>server.js</p>
            {!done && <pre className="code-box" style={{ padding: "8px 13px", lineHeight: 1.6 }}>
              {phase < 3 ? <><Cm>{tr2({ uz: "// CORS hali ruxsat berilmagan", ru: "// CORS пока не разрешён" })}</Cm>{"\n"}<Jx>{"const"}</Jx>{" app = express();"}</> : <><Jx>{"const"}</Jx>{" app = express();"}{"\n"}<span style={{ background: "rgba(31,122,77,0.18)", borderRadius: 5, padding: "1px 5px" }}>{"app.use("}<At>cors()</At>{");"}</span>{"  "}<Cm>{tr2({ uz: "// ✓ :5173 ga ruxsat", ru: "// ✓ доступ для :5173" })}</Cm></>}
            </pre>}
            {done && <div className="takeaway ok fade-step"><div className="ta-bulb">🎉</div><p className="ta-h">{tr2({ uz: "Sayt endi serverdan o'qiyapti!", ru: "Сайт теперь читает с сервера!" })}</p><p className="ta-sub">{tr2({ uz: "Spark ham paydo bo'ldi — S0'dagi muammo hal bo'ldi.", ru: "Spark появился — проблема с первого экрана решена." })}</p></div>}
            {phase === 2 && <button className="btn fade-step tap-hint" style={{ alignSelf: "flex-start", background: T.success }} onClick={fix}>{tr2({ uz: "🔧 app.use(cors()) qo'shish", ru: "🔧 Добавить app.use(cors())" })}</button>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Sayt — front (localhost:5173)", ru: "Сайт — фронт (localhost:5173)" })}</p>
            <Win title="avtoijara.uz" minH={160} maxH={190}><AvtoSite cars={siteCars} state={siteState} newId={SPARK.id} cols={2} /></Win>
            {phase === 2 && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Katalog bo'sh — lekin xato matni aniq aytyapti: <b>CORS policy</b> to'sib qo'ygan. Chap tomondagi tugma bilan tuzating →</>, ru: <>Каталог пуст — но текст ошибки говорит прямо: заблокировала <b>CORS policy</b>. Почините кнопкой слева →</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Avval saytni ishga tushiring, keyin konsoldagi xatoni o'qib tuzating.", ru: "💡 Сначала запустите сайт, потом прочитайте ошибку в консоли и исправьте." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [cars, setCars] = useState3(storedAnswer ? [...CARS_DB, TRACKER] : CARS_DB);
  const [added, setAdded] = useState3(!!storedAnswer);
  const [refreshed, setRefreshed] = useState3(!!storedAnswer);
  const [sc, setSc] = useState3(0);
  const done = added && refreshed;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, (added ? 1 : 0) + (refreshed ? 1 : 0));
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  const post = () => {
  ...
  return <Stage eyebrow={tr2({ uz: "4-qadam · forma", ru: "Шаг 4 · форма" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Qo'shing va yangilang", ru: "Добавьте и обновите" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Forma orqali qo'shilgan mashina <span className="italic" style={{ color: T.accent }}>darrov ko'rinadimi</span>?</>, ru: <>Машина, добавленная через форму, <span className="italic" style={{ color: T.accent }}>появится сразу</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Yangi mashina formasi", ru: "Форма новой машины" })}</p>
            <div className="frame" style={{ padding: 14 }}>
              <div className="form-row"><span className="form-lbl">{tr2({ uz: "Nomi", ru: "Название" })}</span><span className="form-val">Chevrolet Tracker</span></div>
              <div className="form-row"><span className="form-lbl">{tr2({ uz: "Narx", ru: "Цена" })}</span><span className="form-val">{tr2({ uz: "450 000 so'm/kun", ru: "450 000 сум/день" })}</span></div>
              <div className="form-row"><span className="form-lbl">{tr2({ uz: "Yil", ru: "Год" })}</span><span className="form-val">2024</span></div>
              <button className={`btn ${added ? "" : "tap-hint"}`} disabled={added} onClick={post} style={{ marginTop: 10, width: "100%", background: added ? T.success : T.ink3 }}>{added ? tr2({ uz: "✓ POST yuborildi (201)", ru: "✓ POST отправлен (201)" }) : tr2({ uz: "Qo'shish — POST /api/cars", ru: "Добавить — POST /api/cars" })}</button>
            </div>
            {!done && <pre className="code-box" style={{ padding: "8px 13px", lineHeight: 1.6 }}>
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Yangilandi — Tracker baribir joyida! Chunki u faqat ekranda emas, <b>bazada</b> saqlangan. Mana to'liq ulanish: forma → POST → baza → qayta GET → ekran.</>, ru: <>Обновили — а Tracker всё равно на месте! Потому что он сохранён не только на экране, а <b>в базе</b>. Вот полная связка: форма → POST → база → повторный GET → экран.</> })}</p></div>}
          </Col>
          <Col>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}><p className="flow-label">{tr2({ uz: "Sayt — front (localhost:5173)", ru: "Сайт — фронт (localhost:5173)" })}</p><button className={`btn-soft ${added && !refreshed ? "tap-hint" : ""}`} disabled={!added || refreshed} onClick={refresh} style={{ flexShrink: 0 }}>{refreshed ? tr2({ uz: "✓ Yangilandi — Tracker joyida", ru: "✓ Обновили — Tracker на месте" }) : tr2({ uz: "🔄 Sahifani yangilash", ru: "🔄 Обновить страницу" })}</button>
</div>
            <Win title="avtoijara.uz" minH={160} maxH={220}><AvtoSite cars={cars} newId={added ? TRACKER.id : void 0} cols={2} /></Win>
            {added && !refreshed && <p className="small fade-step" style={{ color: T.success, fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Tracker chiqdi! Endi 🔄 yangilang — saqlanib qoladimi?", ru: "Tracker появился! Теперь обновите 🔄 — сохранится ли?" })}</p>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Ikki amal kerak: formaga yozib qo'shing, keyin ro'yxatni yangilang.", ru: "💡 Нужны два действия: добавьте через форму, затем обновите список." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var CIRCLE = [{ uz: "Forma (front)", ru: "Форма (фронт)" }, { uz: "POST →", ru: "POST →" }, { uz: "Express", ru: "Express" }, { uz: "PostgreSQL", ru: "PostgreSQL" }, { uz: "GET →", ru: "GET →" }, { uz: "Katalog (front)", ru: "Каталог (фронт)" }];
var Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [step, setStep] = useState3(storedAnswer ? CIRCLE.length : -1);
  const done = step >= CIRCLE.length - 1;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, step);
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  return <Stage eyebrow={tr2({ uz: "Ma'lumot yo'li", ru: "Путь данных" })} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Yo'lni kuzating", ru: "Проследите путь" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Mashina qo'shilganda ma'lumot <span className="italic" style={{ color: T.accent }}>qaysi yo'lni</span> bosib o'tadi?</>, ru: <>Какой <span className="italic" style={{ color: T.accent }}>путь</span> проходят данные при добавлении машины?</> })}</h2></div>
    ...
    return isArr ? <span key={i} className="flow-arr" style={{ color: step > i ? T.accent : T.ink3, fontWeight: 700 }}>{tr2(c)}</span> : <div key={i} className="flow-node" style={{ background: lit ? T.accent : T.paper, color: lit ? "#fff" : T.ink3 }}>{tr2(c)}</div>;
  })}
          </div>
          <button className="btn" style={{ alignSelf: "flex-start" }} disabled={done} onClick={() => setStep((s) => Math.min(s + 1, CIRCLE.length - 1))}>{step < 0 ? tr2({ uz: "▶ Boshlash", ru: "▶ Начать" }) : done ? tr2({ uz: "✓ Yo'l tugadi", ru: "✓ Путь пройден" }) : tr2({ uz: "Keyingi qadam →", ru: "Следующий шаг →" })}</button>
          {done && <Zoomable><Split>
            <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Mana ma'lumotning to'liq yo'li. Har bo'lak o'z ishini qiladi: <b>front</b> ko'rsatadi, <b>server</b> boshqaradi, <b>baza</b> saqlaydi.</>, ru: <>Вот полный путь данных. Каждая часть делает своё дело: <b>фронт</b> показывает, <b>сервер</b> управляет, <b>база</b> хранит.</> })}</p></div>
            <div className="frame fade-step" style={{ padding: 14 }}><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: <>Ma'lumot bazada saqlanadi, shuning uchun u <b style={{ color: T.ink }}>yo'qolmaydi</b>: sahifani yangilasangiz ham, do'stingiz boshqa telefonda ochsa ham — hamma bir xil mashinalarni ko'radi.</>, ru: <>Данные хранятся в базе, поэтому они <b style={{ color: T.ink }}>не теряются</b>: обновите ли вы страницу, откроет ли друг сайт на другом телефоне — все увидят одни и те же машины.</> })}</p></div>
          </Split></Zoomable>}
        </Col>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Doira to'liq aylanishi kerak — keyingi qadamni bosing.", ru: "💡 Круг должен замкнуться — нажмите следующий шаг." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [cars, setCars] = useState3(storedAnswer ? [...CARS_DB, TRACKER] : CARS_DB);
  const [didLoad] = useState3(true);
  const [didAdd, setDidAdd] = useState3(!!storedAnswer);
  const [didRefresh, setDidRefresh] = useState3(!!storedAnswer);
  const [sc, setSc] = useState3(0);
  const done = didAdd && didRefresh;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, (didAdd ? 1 : 0) + (didRefresh ? 1 : 0));
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  const add = () => {
  ...
  return <Stage eyebrow={tr2({ uz: "Amaliyot · to'liq ilova", ru: "Практика · полное приложение" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Qo'shing va yangilang", ru: "Добавьте и обновите" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Endi o'zingiz — <span className="italic" style={{ color: T.accent }}>to'liq ulangan</span> saytni boshqaring.</>, ru: <>Теперь сами — управляйте <span className="italic" style={{ color: T.accent }}>полностью подключённым</span> сайтом.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Amallar", ru: "Действия" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <button className="btn-soft" disabled style={{ alignSelf: "flex-start", opacity: 1 }}>{tr2({ uz: "✓ Serverdan yuklandi (avtomatik)", ru: "✓ Загружено с сервера (автоматически)" })}</button>
            <button className={`btn ${didAdd ? "" : "tap-hint"}`} disabled={didAdd} onClick={add} style={{ alignSelf: "flex-start", background: didAdd ? T.success : T.ink3 }}>{didAdd ? tr2({ uz: "✓ Qo'shildi (POST)", ru: "✓ Добавлено (POST)" }) : tr2({ uz: "Mashina qo'shish — POST", ru: "Добавить машину — POST" })}</button>
            <button className={`btn-soft ${didAdd && !didRefresh ? "tap-hint" : ""}`} disabled={!didAdd || didRefresh} onClick={refresh} style={{ alignSelf: "flex-start" }}>{didRefresh ? tr2({ uz: "✓ Saqlanib qoldi", ru: "✓ Сохранилось" }) : tr2({ uz: "🔄 Sahifani yangilash", ru: "🔄 Обновить страницу" })}</button>
            </div>
            <p className="flow-label" style={{ margin: "4px 0 0" }}>{tr2({ uz: "Bajarildi", ru: "Выполнено" })}</p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}><Tick ok={didLoad} label={tr2({ uz: "Yuklandi", ru: "Загрузилось" })} /><Tick ok={didAdd} label={tr2({ uz: "Qo'shdim", ru: "Добавили" })} /><Tick ok={didRefresh} label={tr2({ uz: "Saqlandi", ru: "Сохранилось" })} /></div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "🎉 Fullstack ilova tayyor! Sayt serverdan o'qiydi, formaga yozsangiz bazaga yoziladi, refresh'da yo'qolmaydi. Front + back + baza — bittasi bo'lib ishlaydi.", ru: "🎉 Fullstack-приложение готово! Сайт читает с сервера, запись из формы попадает в базу и не теряется при refresh. Фронт + бэк + база работают как одно целое." })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Sizning saytingiz", ru: "Ваш сайт" })}</p>
            <Win title="avtoijara.uz" minH={170} maxH={230}><AvtoSite cars={cars} newId={didAdd ? TRACKER.id : void 0} cols={2} /></Win>
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Ikki amal kerak: formaga yozib qo'shing, keyin ro'yxatni yangilang.", ru: "💡 Нужны два действия: добавьте через форму, затем обновите список." })}</p>}
...
/>;
var Screen15 = ({ screen, onNext, onPrev }) => <Stage eyebrow={tr2({ uz: "Qoida · xulosa", ru: "Правило · итог" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Oxirgi qadam →", ru: "Последний шаг →" })} onClick={onNext} /></>}>
    <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
      <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Fullstack — <span className="italic" style={{ color: T.accent }}>bir qarashda</span> nima?</>, ru: <>Fullstack — что это <span className="italic" style={{ color: T.accent }}>одним взглядом</span>?</> })}</h2></div>
      ...
      <Split>
        <Col>
          <p className="flow-label">{tr2({ uz: "3 ta asosiy g'oya", ru: "3 главные идеи" })}</p>
          <div className="roadmap">
            <div className="step-card"><span className="step-num">01</span><span className="step-body"><span className="step-text">{tr2({ uz: "2 dastur bir vaqtda", ru: "2 программы одновременно" })}</span><span className="step-tag">front :5173 · back :3000</span></span></div>
        ...
        <div className="split">
          <Col>
            ...
            {!isMentor && <button className={`lp-done-btn ${done ? "is-done" : ""}`} disabled={done} onClick={complete}>
              {done ? tr2({ uz: "✓ Bajarildi — ustozni kuting", ru: "✓ Готово — ждите наставника" }) : tr2({ uz: "✅ Bajardim", ru: "✅ Готово" })}
            </button>}
            {done && !isMentor && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Juda yaxshi! Saytingizni serverga o'zingiz uladingiz. Ustoz tekshirib, keyingi qadamga o'tkazadi.", ru: "Отлично! Вы сами подключили свой сайт к серверу. Наставник проверит и переведёт на следующий шаг." })}</p></div>}
...
var ScreenConnectPractice = (props) => <ScreenLivePractice
  {...props}
...
var ACH_TRIGGERS = { s4: "connected", s9: "bothOnline", s14: "fullCircle", s16: "fullStack" };
function AchCelebrate({ ach, onDone }) {
  useEffect4(() => {
    const t = setTimeout(onDone, 4e3);
    return () => clearTimeout(t);
  }, []);
  return <div className="acu-overlay" onClick={onDone} role="status" aria-label={`${tr2({ uz: "Yangi nishon:", ru: "Новый бейдж:" })} ${ach.name}`}>
...
var Q_LABELS = { 4: { uz: "1 — fetch", ru: "1 — fetch" }, 7: { uz: "2 — useEffect", ru: "2 — useEffect" }, 9: { uz: "3 — 2 dastur", ru: "3 — 2 программы" }, 14: { uz: "4 — serverdan", ru: "4 — с сервера" } };
var QUIZ_MS = 15e3;
var QZ_BG_SHAPES = [
...
function QuizArena({ live, onClose, startSolo }) {
  ...
  return <div className="qz-arena">
      ...
      {phase === "lobby" && <div className="qz-view fade-step">
          <CsWordmark />
          <p className="qz-sub" style={{ marginTop: -4 }}>{tr2({ uz: "Tezroq to'g'ri bossangiz — ko'proq ball. Ketma-ket to'g'ri javoblar 🔥 bonus beradi!", ru: "Ответите верно быстрее — получите больше баллов. Серия верных ответов даёт 🔥 бонус!" })}</p>
...
var Screen17 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "Birinchi fullstack ilovangiz", ru: "Ваше первое fullstack-приложение" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Front va backni <span className="italic" style={{ color: T.accent }}>o'zingiz uladingiz</span>.</>, ru: <>Фронт и бэк <span className="italic" style={{ color: T.accent }}>вы соединили сами</span>.</> })}</h2>{
...
function FullstackConnectPracticeLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  return <LangContext.Provider value={lang}>
      <style>{`
        ...
        .split3 { grid-template-columns: minmax(0,1.14fr) minmax(0,1.19fr) minmax(0,1fr); gap: clamp(14px,2vw,24px); } /* §34 s0: sayt · baza · savol — uch ustun */
        .split3 .hook-option { padding: clamp(9px,1.3vw,12px) clamp(13px,1.8vw,16px); } .split3 .db-row { gap: 6px; }
        ...
        .takeaway { background: ${T.accentSoft}; border-radius: 14px; padding: 14px 16px; display: flex; flex-direction: column; align-items: center; text-align: center; gap: 5px; } .ta-bulb { font-size: 26px; } .ta-h { font-family: 'Source Serif 4', serif; font-weight: 600; font-size: clamp(16px,2.2vw,20px); color: ${T.ink}; margin: 0; } .ta-sub { color: ${T.accent}; font-weight: 600; font-size: 13px; margin: 0; }
        ...
        /* KLAPAN-IPUCHASI (13-band). Ogohlantirish emas — YORDAM: accent-kontur, qizil emas.
           Modifikator .calm — rescue xabari, undan ham yumshoqroq.
           Selektor ikki marta: .lesson-root p reseti padding-ni jim o'chiradi (F-0803-27). */
        ...
        /* BITTA KLASS — BITTA ROL (134-qonun). «Siz» — personaj emas, ekran egasining
           navbat-signali: KONTUR-accent (yorliq, tugma emas). */
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={tr2({ uz: "Fullstack ulash — praktika", ru: "Fullstack связка — практика" })} /> : <>
...
export {
  FullstackConnectPracticeLesson as default
};
