...
var Phone = ({ children, quiet, ping, hint }) => <div className={`pho${quiet ? " quiet" : ""}`}>
    ...
    <span className="pho-app">{tr2({ uz: "🎧 Musiqa", ru: "🎧 Музыка" })}</span>
    <div className="pho-body">
      {ping ? <span key={ping} className="pho-ping" aria-hidden="true" /> : null}
      {quiet && <span className="pho-empty"><i className="pho-empty-ic">🎧</i>{hint || tr2({ uz: "Bo'lim hali qurilmadi", ru: "Раздел ещё не построен" })}</span>}
      {children}
    </div>
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish · bitta qo'shiq", ru: "Вступление · одна песня" })} screen={screen} navContent={<NavNext optionalLive disabled={picked === null && !isMentor} label={opened ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Bittasini tanlang", ru: "Выберите один вариант" })} onClick={onNext} />}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Ilova qo'shiqni <span className="italic" style={{ color: T.accent }}>eslab qolsinmi?</span></>, ru: <>Приложению <span className="italic" style={{ color: T.accent }}>запомнить песню?</span></> })}</h2></div>
        <Mentor>{tr2({ uz: "Sevimli qo'shig'ingizni ilova eslab qolsinmi?", ru: "Пусть приложение запомнит вашу любимую песню?" })}</Mentor>
    ...
    /* IMZO-SAHNA: ikkala tanlovda ham BIR XIL natija ochiladi (104-qonun) */
            ...
            <div className="h0pay fade-step">
              <span className="h0pay-row"><b>{tr2({ uz: "Eslab qolsin", ru: "Пусть запомнит" })}</b><i className="h0pay-arw">→</i><span>{tr2({ uz: "ertaga qo'shiqni darhol topasiz", ru: "завтра найдёте песню сразу" })}</span></span>
              <span className="h0pay-row"><b>{tr2({ uz: "Eslamasin", ru: "Пусть не запоминает" })}</b><i className="h0pay-arw">→</i><span>{tr2({ uz: "ertaga uni yana qidirasiz", ru: "завтра будете искать её снова" })}</span></span>
            </div>
          </>}
        {
...
var Screen1 = ({ screen, onNext, onPrev }) => <Stage eyebrow={tr2({ uz: "Maqsad", ru: "Цель" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Boshlaymiz →", ru: "Начнём →" })} onClick={onNext} /></>}>
    <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
      <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Dars oxirida ilovangiz uchun <span className="italic" style={{ color: T.accent }}>3 ta maydon</span> yozasiz.</>, ru: <>В конце урока вы напишете <span className="italic" style={{ color: T.accent }}>3 поля</span> для своего приложения.</> })}</h2></div>
      <Mentor>{tr2({ uz: <>Musiqa ilovasi siz haqingizda nimalarni eslab qoladi — mana shu uchtasi <b style={{ color: T.ink }}>maydon</b> deyiladi.</>, ru: <>Что музыкальное приложение запоминает о вас — вот эти три вещи называются <b style={{ color: T.ink }}>полями</b>.</> })}</Mentor>
      <div className="s1demo">
        <span className="s1demo-lbl">{tr2({ uz: "🎧 Musiqa ilovasi eslab qoladi:", ru: "🎧 Музыкальное приложение запоминает:" })}</span>
        <div className="s1demo-list">
          {DEMO_MAYDON.map((m, i) => <span key={i} className="s1row" style={{ "--dd": `${0.4 + i * 0.45}s` }}>
              <span className="s1row-t">{tr2(m)}</span>
              <span className="s1row-ok" style={{ "--dd3": `${0.7 + i * 0.45}s` }}>✅</span>
            </span>)}
        </div>
      </div>
      <div className="takeaway fade-up delay-2"><span className="ta-bulb">🎯</span><p className="ta-h">{tr2({ uz: "Dars oxirida siz ham o'z ilovangiz eslab qoladigan uchta maydonni yozasiz.", ru: "В конце урока вы тоже напишете три поля, которые будет запоминать ваше приложение." })}</p></div>
      <MentorNote>{tr2({ uz: "Ro'yxat yozilib bo'lgunicha gapirmang — vizual o'zi tanishtiradi.", ru: "Не говорите, пока список не допишется, — визуал представит всё сам." })}</MentorNote>
    </div>
  </Stage>;
var S2_CARDS = [
...
var Screen2 = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Muhokama · ikki karta", ru: "Обсуждение · две карточки" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!allSeen && !isMentor} label={allSeen || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `👆 Yana ${qoldi} kartani oching`, ru: `👆 Откройте ещё ${qoldi} карточк${qoldi === 1 ? "у" : "и"}` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Ilova nimani <span className="italic" style={{ color: T.accent }}>eslab qoladi</span>?</>, ru: <>Что приложение <span className="italic" style={{ color: T.accent }}>запоминает</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: "Ikki kartani solishtiring.", ru: "Сравните две карточки." })}</Mentor>
        ...
        {allSeen && <div className="xul fade-step">
            <span className="xul-h">{tr2({ uz: "Ilova faqat eslab qolgan narsasini keyin yana ko'rsata oladi.", ru: "Приложение может снова показать только то, что запомнило." })}</span>
            <p className="xul-b">{tr2({ uz: <>Eslab qolinmagan narsa faqat o'sha payt ko'rinadi. Ilova eslab qoladigan har bir narsa — <b>maydon</b> deb ataladi.</>, ru: <>Незапомненное видно только в тот момент. Каждая вещь, которую приложение запоминает, называется <b>полем</b>.</> })}</p>
          </div>}
      </div>
    </Stage>;
};
...
var Screen3 = (props) => <QuestionScreen
  {...props}
  eyebrow={tr2({ uz: "Tekshiruv · nima qaytadi", ru: "Проверка · что возвращается" })}
  scope="module-mikro"
  ctaLabel={tr2({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr2({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr2({ uz: "💾 Ilova keyin yana nimani ko'rsata oladi?", ru: "💾 Что приложение сможет показать снова?" })} />}
  questionText={tr2({ uz: "Ilova keyin yana nimani ko'rsata oladi", ru: "Что приложение сможет показать снова" })}
  options={[tr2({ uz: "Bir marta ko'rilgan hamma narsani", ru: "Всё, что один раз увидели" }), tr2({ uz: "Eslab qolgan ma'lumotini", ru: "Данные, которые оно запомнило" }), tr2({ uz: "Foydalanuvchi so'ragan hamma narsani", ru: "Всё, что попросит пользователь" })]}
  correctIdx={1}
  explainCorrect={tr2({ uz: "To'g'ri — eslab qolinmagan narsa faqat o'sha payt ko'rinadi.", ru: "Верно — незапомненное видно только в тот момент." })}
  explainWrong={{
    0: tr2({ uz: "Bir marta ko'rilgani eslab qolinmagan bo'lsa, keyin uni qaytadan chiqarib bo'lmaydi.", ru: "Если увиденное один раз не запомнили, потом его уже не вывести снова." }),
    2: tr2({ uz: "So'rash yetmaydi: ilova faqat eslab qolgan ma'lumotini ko'rsata oladi.", ru: "Просьбы мало: приложение показывает только те данные, которые запомнило." }),
    default: tr2({ uz: "Ilova faqat eslab qolgan ma'lumotini keyin yana ko'rsata oladi.", ru: "Приложение может снова показать только те данные, которые запомнило." })
  }}
/>;
var TIP1_SEC = 40;
var TIP1_TRY = 3;
...
var Screen4 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === "mentor");
  const [on, setOn] = useState3({});
  const [tried, setTried] = useState3(() => storedAnswer?.tried || {});
  const [offSeen, setOffSeen] = useState3(() => !!storedAnswer?.offSeen);
  ...
  useEffect3(() => () => clearTimeout(justT.current), []);
  useEffect3(() => {
  ...
  const navLabel = done || isMentor || rescue ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : qolgan > 0 ? tr2({ uz: `① Yana ${qolgan} tugmani yoqib ko'ring`, ru: `① Включите ещё ${qolgan} ${qolgan === 1 ? "переключатель" : "переключателя"}` }) : tr2({ uz: "② Endi bittasini o'chirib ko'ring", ru: "② Теперь попробуйте один выключить" });
  return <Stage eyebrow={tr2({ uz: "Sinov · ilova nimani eslab qoladi", ru: "Опыт · что запоминает приложение" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor && !rescue} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.3vw,13px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Ilova nimani eslab qolishini <span className="italic" style={{ color: T.accent }}>o'zingiz</span> tanlang.</>, ru: <>Выберите <span className="italic" style={{ color: T.accent }}>сами</span>, что приложению запоминать.</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Faqat <b style={{ color: T.ink }}>kerakli</b> ma'lumotlarni yoqing.</>, ru: <>Включите только <b style={{ color: T.ink }}>нужные</b> данные.</> })}</Mentor>
    ...
    /* 72-qonun: yorliqli idish + diqqat-pulsi; birinchi bosishdan keyin puls tinadi */
            ...
            <div className={`mtg${Object.keys(tried).length > 0 ? " calm" : ""}`}>
              <span className="mtg-lbl">{tr2({ uz: "🎛 Ilova nimani yozib qo'ysin?", ru: "🎛 Что приложению записывать?" })}</span>
              {TUGMALAR.map((m) => <div key={m.id} className={`mtg-row${on[m.id] ? " on" : ""}${just === m.id ? " just" : ""}`}>
                  <button type="button" className="mtg-btn" onClick={() => flip(m.id)} aria-pressed={!!on[m.id]}>
                    ...
                    <span className="mtg-t">{tr2(m.t)}</span>
                    <span className="mtg-sw"><i /></span>
                  </button>
                  {
    /* 106d/71: har bosishdan keyin yonida bitta qator — nimadan qurilishini aytadi */
  }
                  {tried[m.id] && <span className={`mtg-res ${m.bolim ? "ok" : "nil"}`}>{m.bolim ? "✅" : "⬜"} {tr2(m.res)}</span>}
                </div>)}
            </div>
          </Col>
          <Col gap={9}>
       ...
       so'nib pasayadi. Ikkala harakat ham bir xil egri bilan — yo'qolish KO'RINADI. */
            ...
            <Phone quiet={bolimlar.length === 0} ping={nilPing}>
              {TUGMALAR.filter((m) => m.bolim).map((m) => <div key={m.id} className={`pho-sec${on[m.id] ? " on" : ""}`} aria-hidden={!on[m.id]}>
                  <span className="pho-sec-h">{tr2(m.bolim)}</span>
        ...
        {tipOn && <p className="bhint fade-step">
            {nextOpen ? tr2({ uz: <>💡 Hali sinalmagan tugma bor: {nextOpen.ic} {tr2(nextOpen.t)} — uni ham yoqib ko'ring.</>, ru: <>💡 Остался непроверенный переключатель: {nextOpen.ic} {tr2(nextOpen.t)} — включите и его.</> }) : tr2({ uz: "💡 Yoqib ko'rdingiz. Endi bittasini o'chirib ham ko'ring — ilovada nima o'zgaradi?", ru: "💡 Включать попробовали. Теперь попробуйте один выключить — что изменится в приложении?" })}
          </p>}
        {rescue && <p className="small fade-step" style={{ margin: 0, color: T.ink3, fontWeight: 600 }}>{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём позже вместе — «Продолжить» открыто." })}</p>}
        {done && <div className="bdone fade-step">
            <span className="done-mini">{tr2({ uz: <>✅ Uch tugma bittadan bo'lim ochdi, ikkitasi hech narsa ochmadi <span className="dm-sub">— har saqlanadigan maydon ortida bitta bo'lim turadi</span></>, ru: <>✅ Три переключателя открыли по разделу, два не открыли ничего <span className="dm-sub">— за каждым сохраняемым полем стоит один раздел</span></> })}</span>
          </div>}
...
var Screen5 = (props) => <QuestionScreen
  {...props}
  eyebrow={tr2({ uz: "Tekshiruv · foyda bermagan ma'lumot", ru: "Проверка · бесполезные данные" })}
  scope="module-mikro"
  ctaLabel={tr2({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr2({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr2({ uz: "📇 Kontaktlarni saqladik, lekin musiqa ilovasida hech narsa o'zgarmadi. Nega?", ru: "📇 Контакты сохранили, но в музыкальном приложении ничего не изменилось. Почему?" })} />}
  questionText={tr2({ uz: "Kontaktlar saqlandi, lekin ilovada hech narsa o'zgarmadi — nega", ru: "Контакты сохранены, но в приложении ничего не изменилось — почему" })}
  options={[tr2({ uz: "Chunki bu ma'lumotdan foydali bo'lim chiqmaydi", ru: "Потому что из этих данных не получается полезный раздел" }), tr2({ uz: "Chunki ilova buzilgan", ru: "Потому что приложение сломалось" }), tr2({ uz: "Chunki keyinroq o'zi ishlay boshlaydi", ru: "Потому что позже оно заработает само" })]}
  correctIdx={0}
  explainCorrect={tr2({ uz: "To'g'ri — saqlangan ma'lumotdan bo'lim chiqmasa, u bekorga joy egallaydi.", ru: "Верно — если из сохранённых данных не выходит раздел, они зря занимают место." })}
  explainWrong={{
    1: tr2({ uz: "Ilova buzilmagan: kontaktlarni bemalol saqladi. Faqat ulardan musiqa ilovasiga foydali hech narsa chiqmaydi.", ru: "Приложение не сломалось: контакты оно спокойно сохранило. Просто из них для музыкального приложения не выходит ничего полезного." }),
    2: tr2({ uz: "Bo'lim o'zidan paydo bo'lmaydi — uni saqlangan ma'lumot quradi. Kontaktlardan qo'shiq bo'limi chiqmaydi.", ru: "Раздел не появляется сам по себе — его строят сохранённые данные. Из контактов раздел с песнями не выходит." }),
    default: tr2({ uz: "Har foydali bo'lim ortida bitta kerakli ma'lumot turadi — kontaktlardan bunday bo'lim chiqmadi.", ru: "За каждым полезным разделом стоят одни нужные данные — из контактов такого раздела не вышло." })
  }}
/>;
var K6_SLIDES = [
  {
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "🎬 Haqiqiy voqea", ru: "🎬 Реальная история" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={betPending && !isMentorK} label={betPending && !isMentorK ? tr2({ uz: "Avval o'zingiz belgilang", ru: "Сначала сделайте свой выбор" }) : last ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Keyingi bosqich (${i + 1}/${K6_SLIDES.length})`, ru: `Следующий шаг (${i + 1}/${K6_SLIDES.length})` })} onClick={last ? onNext : () => setI(i + 1)} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Kino olamidan <span className="italic" style={{ color: T.accent }}>mashhur voqea</span></>, ru: <>Известная история <span className="italic" style={{ color: T.accent }}>из мира кино</span></> })}</h2></div>
    ...
    /* 🔴 ETALON 22 (sanoq-mosligi): bashoratli bosqichda ham hisoblagich uzluksiz
       turadi (1·2·3·4·5·6) va har bosqichda AYNAN BITTA joyda ko'rinadi. */
            ...
            <span className="k-slide-eyebrow">{bet === void 0 ? tr2({ uz: "🎲 Avval o'zingiz belgilab ko'ring", ru: "🎲 Сначала попробуйте угадать сами" }) : tr2({ uz: "🎬 Haqiqiy voqea", ru: "🎬 Реальная история" })} · {i + 1} / {K6_SLIDES.length}</span>
    ...
    return <button key={k} className={cls} disabled={locked} onClick={() => setBets((p) => ({ ...p, [i]: k }))}>
        ...
        {showSlide && <div className="k-slide fade-step" key={`s${i}`}>
            {!c.predict && <span className="k-slide-eyebrow">{tr2({ uz: "🎬 Haqiqiy voqea", ru: "🎬 Реальная история" })} · {i + 1} / {K6_SLIDES.length}</span>}
    ...
    /* ETALON 22: ko'prik-bosqichi ham sanoqqa kiradi — zanjir uzilmaydi */
            ...
            <span className="k-slide-eyebrow">{tr2({ uz: "🎬 Haqiqiy voqea", ru: "🎬 Реальная история" })} · {i + 1} / {K6_SLIDES.length}</span>
            <p className="body" style={{ margin: "10px 0 0", color: T.ink }}>{tr2({ uz: <>Netflix ham qaysi ma'lumot muhimligini tanlaydi. <b>Endi shu ishni siz qilasiz.</b></>, ru: <>Netflix тоже выбирает, какие данные важны. <b>Теперь это сделаете вы.</b></> })}</p>
          </div>}
      </div>
    </Stage>;
};
var Screen7 = (props) => <QuestionScreen
  {...props}
  eyebrow={tr2({ uz: "Tekshiruv · bosh sahifa", ru: "Проверка · главная страница" })}
  scope="module-mikro"
  ctaLabel={tr2({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr2({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr2({ uz: "🎬 Netflix bosh sahifasi nimadan yig'iladi?", ru: "🎬 Из чего собирается главная Netflix?" })} />}
  questionText={tr2({ uz: "Netflix bosh sahifasi nimadan yig'iladi", ru: "Из чего собирается главная Netflix" })}
  options={[tr2({ uz: "Har kim ilgari ko'rgan kinolardan", ru: "Из фильмов, которые каждый смотрел раньше" }), tr2({ uz: "Eng ko'p pul ishlagan kinolardan", ru: "Из фильмов, которые заработали больше всего денег" }), tr2({ uz: "Hamma uchun tuzilgan bitta ro'yxatdan", ru: "Из одного списка, составленного для всех" })]}
  correctIdx={0}
  explainCorrect={tr2({ uz: "To'g'ri. Shu sababli ikki odam Netflix'ni ochsa, bosh sahifalari har xil bo'ladi.", ru: "Верно. Поэтому, когда два человека открывают Netflix, главные у них разные." })}
  explainWrong={{
    1: tr2({ uz: "Pul haqida voqeada gap yo'q: bosh sahifa har kimning ilgari ko'rganidan yig'iladi.", ru: "О деньгах в истории ни слова: главная собирается из того, что каждый смотрел раньше." }),
    2: tr2({ uz: "Bitta umumiy ro'yxat bo'lganda ikki odam bir xil bosh sahifani ko'rardi.", ru: "Будь список один на всех, два человека видели бы одинаковую главную." }),
    default: tr2({ uz: "Bosh sahifa har kimning ilgari ko'rgan kinolaridan quriladi — shuning uchun u har kimda har xil.", ru: "Главная строится из фильмов, которые каждый смотрел раньше, — поэтому у всех она разная." })
  }}
/>;
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === "mentor");
  const [list, setList] = useState3(() => storedAnswer && Array.isArray(storedAnswer.maydonlar) ? storedAnswer.maydonlar : []);
  const [dMaydon, setDMaydon] = useState3("");
  const [dBolim, setDBolim] = useState3("");
  const [edit, setEdit] = useState3(null);
  const [focus, setFocus] = useState3(false);
  const [yordamOpen, setYordamOpen] = useState3(true);
  ...
  const navLabel = done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : list.length === 0 ? tr2({ uz: "① Birinchi maydonni yozing va saqlang", ru: "① Напишите и сохраните первое поле" }) : tr2({ uz: `② Yana ${3 - list.length} maydon yozing`, ru: `② Напишите ещё ${3 - list.length} ${3 - list.length === 1 ? "поле" : "поля"}` });
  return <Stage eyebrow={tr2({ uz: "Mustaqil ish · uch maydon", ru: "Самостоятельная работа · три поля" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Musiqa ilovasiga <span className="italic" style={{ color: T.accent }}>uchta maydon</span> yozing.</>, ru: <>Напишите <span className="italic" style={{ color: T.accent }}>три поля</span> для музыкального приложения.</> })}</h2></div>
        <Mentor>{tr2({ uz: "Har maydonga bitta savol bering: buni saqlasak, ilovada qaysi bo'lim ochiladi?", ru: "Задайте каждому полю один вопрос: если это сохранить, какой раздел откроется в приложении?" })}</Mentor>
        {
    /* 80a: havoda uch doira — yozilgani yashil, joriysi pulsda, kelgusi punktir */
  }
        <div className="stps fade-up">
          {[0, 1, 2].map((k) => <span key={k} className={`stp ${list.length > k ? "done" : (edit === null ? list.length : edit) === k ? "on" : ""}`}><i>{list.length > k ? "✓" : k + 1}</i>{tr2({ uz: <>{k + 1}-maydon{list.length > k ? " tayyor" : ""}</>, ru: <>Поле {k + 1}{list.length > k ? " готово" : ""}</> })}</span>)}
        </div>
        <div className="split">
          <Col gap={9}>
            {
    /* 80b: ekranning yagona kartasi — ikki yozuv-joyi + jonli javob-qatori */
            ...
            {(!done || edit !== null) && <div className="wsp-ed">
                <span className="wsp-ed-h">{tr2({ uz: <>{nQadam}-maydon</>, ru: <>Поле {nQadam}</> })}</span>
                <span className="wsp-q">{tr2({ uz: "Ilova nimani eslab qolsin?", ru: "Что приложению запомнить?" })}</span>
                <input
    className={`reflect-input${inputTurn ? " await" : ""}${uzunM ? " filled" : ""}`}
    value={dMaydon}
    maxLength={90}
    placeholder={tr2({ uz: "Masalan: qo’shiqchi nomi", ru: "Например: имя исполнителя" })}
    onFocus={() => setFocus(true)}
    onBlur={() => setFocus(false)}
    onChange={(e) => setDMaydon(e.target.value)}
  />
                <span className="wsp-q">{tr2({ uz: "Ilovada qaysi bo'lim ochiladi?", ru: "Какой раздел откроется в приложении?" })}</span>
                <input
    className={`reflect-input${bolimBor ? " filled" : ""}`}
    value={dBolim}
    maxLength={90}
    placeholder={tr2({ uz: "Masalan: Qo’shiqchining boshqa qo’shiqlari", ru: "Например: Другие песни исполнителя" })}
  ...
  />
    ...
    /* 106d: ikki tomonlama javob — bloklamaydi, yo'naltiradi */
                ...
                {dMaydon.trim().length > 0 && !uzunM && <p className="sfb ask">{tr2({ uz: "🤔 Juda qisqa qoldi — ilova aynan nimani eslab qolishini aniqroq yozing.", ru: "🤔 Слишком коротко — напишите точнее, что именно приложению запомнить." })}</p>}
                {uzunM && umumiy && <p className="sfb ask">{tr2({ uz: "🤔 Bu hali bo'lim nomi emas. Bundan qaysi bo'lim ochiladi? Masalan: «Kechqurun tinglaganlaringiz».", ru: "🤔 Это ещё не название раздела. Какой раздел из этого откроется? Например: «Слушали вечером»." })}</p>}
                {uzunM && !umumiy && takror && <p className="sfb ask">{tr2({ uz: "🤔 Bu bo'lim yuqorida allaqachon yozilgan — boshqa bo'limni oling.", ru: "🤔 Этот раздел уже записан выше — возьмите другой." })}</p>}
                {canSave && <p className="sfb ok">{tr2({ uz: "✅ Bo'lim nomi aytilgan — bu maydon kerak.", ru: "✅ Раздел назван — это поле нужно." })}</p>}
    ...
    /* 80c: yozilganlar YOZISH PAYTIDA ko'rinmaydi; uchtasi yozilgach ro'yxat ochiladi */
            ...
            {done && edit === null && <div className="wsp-list fade-step">
                <span className="wsp-list-h">{tr2({ uz: "🎧 Musiqa ilovasi eslab qoladigan uchta narsa", ru: "🎧 Три вещи, которые запоминает музыкальное приложение" })}</span>
          ...
          <Col gap={9}>
            <div className="wsp-task">
              ...
              <span className="wsp-task-nom">{tr2({ uz: "Har maydondan bitta foydali bo'lim chiqsin", ru: "Из каждого поля — один полезный раздел" })}</span>
              {
    /* 106c-b: holat ko'rsatkichi */
            ...
            <div className="wsxrow">
              <div className={`wsx ${yordamOpen ? "open" : ""}`}>
                ...
                {yordamOpen && <div className="wsx-body"><p>{tr2({ uz: "O'zingizga savol bering: bu maydonni o'chirsam, ilovada qaysi bo'lim yo'qoladi? Bo'lim nomi topilmasa — o'sha maydon kerak emas.", ru: "Спросите себя: если удалить это поле, какой раздел исчезнет из приложения? Не нашли название раздела — это поле не нужно." })}</p></div>}
              </div>
              <div className={`wsx star ${starOpen ? "open" : ""}`}>
                ...
                {starOpen && <div className="wsx-body"><p>{tr2({ uz: "To'rtinchi maydon yozing — ilova saqlamaydigan narsa va nega saqlamasligi.", ru: "Напишите четвёртое поле — то, что приложение НЕ сохраняет, и почему." })}</p></div>}
              </div>
            </div>
            <StudentPracticePulse live={live} screen={screen} />
        ...
        {done && edit === null && <div className="done-mini fade-step">{tr2({ uz: <>✅ Uch maydoningiz yozildi <span className="dm-sub">— har biridan bittadan bo'lim quriladi</span></>, ru: <>✅ Ваши три поля записаны <span className="dm-sub">— из каждого строится по разделу</span></> })}</div>}
        <MentorNote>{tr2({ uz: "«Foydalanuvchi haqida umumiy ma'lumot» kabi javoblar chiqadi — bu eng foydali xato. Javob-qatori uni tutadi, siz muhokama qiling: bundan qaysi bo'lim quriladi? Bu ishni o'quvchilar bajaradi, siz kuzatasiz; «Davom etish» siz uchun ochiq.", ru: "Появятся ответы вроде «общая информация о пользователе» — это самая полезная ошибка. Строка-ответ её поймает, а вы обсудите: какой раздел из этого строится? Эту работу делают ученики, вы наблюдаете; «Продолжить» для вас открыто." })}</MentorNote>
      </div>
    </Stage>;
};
var YOZUVLAR = [
...
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [built, setBuilt] = useState3(null);
  const [miss, setMiss] = useState3("");
  ...
  const qur = () => {
    ...
    setMissedOnce(true);
    setMiss(ortiqcha.length > 0 ? tr2({ uz: "🤔 Bu yozuv bo'lim nomiga mos kelmaydi — uni yana bir bor o'qing.", ru: "🤔 Эта запись не подходит под название раздела — перечитайте его ещё раз." }) : tr2({ uz: "🤔 Bo'limga mos yana bitta yozuv bor.", ru: "🤔 Есть ещё одна запись, которая подходит разделу." }));
  };
  const keyingi = () => {
    setBuilt(null);
    setSel([]);
  ...
  const navLabel = done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Yana ${RAUNDLAR.length - ri} bo'limni quring`, ru: `Постройте ещё ${RAUNDLAR.length - ri} ${RAUNDLAR.length - ri === 1 ? "раздел" : "раздела"}` });
  const korsat = built ? built : done ? RAUNDLAR[RAUNDLAR.length - 1].togri : null;
  return <Stage eyebrow={tr2({ uz: "Tekshiruv · bo'limni quring", ru: "Проверка · постройте раздел" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(9px,1.4vw,14px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bo'limni jadvaldan <span className="italic" style={{ color: T.accent }}>quring</span>.</>, ru: <><span className="italic" style={{ color: T.accent }}>Постройте</span> раздел из таблицы.</> })}</h2></div>
        <Mentor>{tr2({ uz: <>«{tr2(cur.bolim)}» bo'limiga mos qatorlarni belgilang.</>, ru: <>Отметьте строки, которые подходят разделу «{tr2(cur.bolim)}».</> })}</Mentor>
        <div className="split s9">
          <Col gap={9}>
            <div className={`s9ask${korsat ? " ok" : ""}`}>
              <span className="s9ask-n">{tr2({ uz: "Bo'lim", ru: "Раздел" })} {Math.min(ri + 1, RAUNDLAR.length)} / {RAUNDLAR.length}</span>
              <span className="s9ask-t">«{tr2(cur.bolim)}» <i>({tr2(cur.shart)})</i></span>
            </div>
            <div className="tbl">
              <div className="tbl-head">
                <span className="tbl-c c0">#</span>
                <span className="tbl-c">{tr2({ uz: "🎵 Qaysi qo'shiq", ru: "🎵 Какая песня" })}</span>
                <span className="tbl-c">{tr2({ uz: "⏰ Qachon", ru: "⏰ Когда" })}</span>
                <span className="tbl-c">{tr2({ uz: "☑️ Oxirigacha tinglandimi", ru: "☑️ Дослушана до конца" })}</span>
              </div>
              {YOZUVLAR.map((y) => {
    ...
    return <button key={y.n} type="button" className={`tbl-row${chosen ? " on" : ""}${inBuilt ? " fixed" : ""}`} onClick={() => toggleRow(y.n)} disabled={isMentor || !!built || done}>
                    ...
                    <span className="tbl-c" data-l={tr2({ uz: "🎵 Qaysi qo'shiq", ru: "🎵 Какая песня" })}>{y.qoshiq}</span>
                    <span className="tbl-c mono" data-l={tr2({ uz: "⏰ Qachon", ru: "⏰ Когда" })}>{y.vaqt}</span>
                    <span className="tbl-c" data-l={tr2({ uz: "☑️ Oxirigacha tinglandimi", ru: "☑️ Дослушана до конца" })}><i className={`yn ${y.oxir ? "ok" : "no"}`}>{y.oxir ? tr2({ uz: "ha", ru: "да" }) : tr2({ uz: "yo'q", ru: "нет" })}</i></span>
                  </button>;
  })}
            </div>
            {!built && !done && <button type="button" className="wsp-save" disabled={sel.length === 0 || isMentor} onClick={qur}>{tr2({ uz: "Bo'limni qurish →", ru: "Построить раздел →" })}</button>}
            {built && ri < RAUNDLAR.length - 1 && <button type="button" className="wsp-save" onClick={keyingi}>{tr2({ uz: "Keyingi bo'lim →", ru: "Следующий раздел →" })}</button>}
            {built && ri === RAUNDLAR.length - 1 && !done && <button type="button" className="wsp-save" onClick={keyingi}>{tr2({ uz: "✓ Uch bo'lim qurildi", ru: "✓ Три раздела построены" })}</button>}
          </Col>
          <Col gap={9}>
            <Phone quiet={!korsat}>
              {korsat && <div className="pho-sec on pop">
                  <span className="pho-sec-h">{tr2(cur.bolim)}</span>
...
var KOD_TASK = {
  ...
  requirements: [
    ...
    },
    {
      ...
      check: checks.custom((x) => /oxirigacha/.test(yoqqanlarTanasi(x.js)) ? true : tr2({ uz: "Funksiya ichida yozuvning oxirigacha maydonini tekshiring", ru: "Внутри функции проверьте поле oxirigacha у записи" }))
    },
    {
      id: "uch",
...
var ScreenCoding = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const navLabel = done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : !stage2 ? tr2({ uz: "① Maydonni belgilang", ru: "① Отметьте поле" }) : tr2({ uz: "② Kodni yozing", ru: "② Напишите код" });
  return <Stage eyebrow={tr2({ uz: "Koding · 🛠 kod oynasi", ru: "Кодинг · 🛠 окно кода" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.5vw,15px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Yoqqan qo'shiqlarni ajratadigan <span className="italic" style={{ color: T.accent }}>kod</span> yozamiz.</>, ru: <>Напишем <span className="italic" style={{ color: T.accent }}>код</span>, который отбирает понравившиеся песни.</> })}</h2></div>
        {!stage2 ? <>
            <Mentor>{tr2({ uz: "Kod bitta maydonga qarab ishlaydi. Qaysi maydondan «Sizga yoqadi» bo'limi qurilishini belgilang.", ru: "Код смотрит на одно поле. Отметьте, из какого поля строится раздел «Вам нравится»." })}</Mentor>
            <div className={`cmt hunt${missedOnce ? " calm" : ""}`}>
              <span className="cmt-lbl">{tr2({ uz: "🔎 «Sizga yoqadi» qaysi maydondan quriladi?", ru: "🔎 Из какого поля строится «Вам нравится»?" })}</span>
              <div className="gt-rows">
                {GATE_ITEMS.map((g) => <button key={g.id} type="button" className={`fchoice${miss === g.id ? " miss" : ""}`} onClick={() => pickGate(g)}>
                    {g.ic} {tr2(g.t)}
                  </button>)}
              </div>
              {missedOnce && <p className="cmt-tip">{tr2({ uz: "🤔 Bu maydondan boshqa bo'lim quriladi. Qo'shiq yoqqanini qaysi belgi ko'rsatadi?", ru: "🤔 Из этого поля строится другой раздел. Какой признак показывает, что песня понравилась?" })}</p>}
            </div>
          </> : <>
            <Mentor>{tr2({ uz: "Hozir siz tanladingiz. Endi shu tanlovni kod bajaradi.", ru: "Сейчас выбрали вы. Теперь этот выбор выполнит код." })}</Mentor>
            <div className="cmt-fold fade-step"><span className="cmt-done">{tr2({ uz: "✓ Maydon belgilandi: ☑️ Qo'shiq oxirigacha tinglandimi", ru: "✓ Поле отмечено: ☑️ Дослушана ли песня до конца" })}</span></div>
            <div className="split">
              <Col gap={10}>
                <div className={`kdpanel${done ? " is-done" : ""}`}>
                  <p className="flow-label">{tr2({ uz: "Kod nima qilsin", ru: "Что должен делать код" })}</p>
                  <ul className="kdreq">
                    <li>{tr2({ uz: "Funksiya ro'yxat qaytaradi", ru: "Функция возвращает список" })}</li>
                    <li>{tr2({ uz: "Faqat oxirigacha tinglanganlar qoladi", ru: "Остаются только дослушанные до конца" })}</li>
                    <li>{tr2({ uz: "Natijada uch nom chiqadi", ru: "В результате выходят три названия" })}</li>
                  </ul>
                  <div className={`wsx star ${yordamOpen ? "open" : ""}`}>
                    <button className="wsx-toggle" onClick={() => setYordamOpen((o) => !o)}>{tr2({ uz: "💡 Yordam", ru: "💡 Подсказка" })} {yordamOpen ? "▾" : "▸"}</button>
                    {yordamOpen && <div className="wsx-body">
                      <p>{tr2({ uz: <>Bitta yozuvdan boshlang: uning <code className="qcode">oxirigacha</code> maydoni rostmi? Ishlagach qolganlariga o'ting.</>, ru: <>Начните с одной записи: её поле <code className="qcode">oxirigacha</code> истинно? Заработало — переходите к остальным.</> })}</p>
                      <p>{tr2({ uz: "⭐ Qo'shimcha: ikkinchi funksiya yozing — kechqurun (soat 20 dan keyin) tinglangan qo'shiqlarni qaytarsin.", ru: "⭐ Дополнительно: напишите вторую функцию — пусть возвращает песни, прослушанные вечером (после 20:00)." })}</p>
                    </div>}
                  </div>
                  {done && <div className="done-mini fade-step">{tr2({ uz: <>✅ Uch nom chiqdi <span className="dm-sub">— kod endi yoqqan qo'shiqlarni o'zi ajratadi</span></>, ru: <>✅ Три названия вышли <span className="dm-sub">— теперь код сам отбирает понравившиеся песни</span></> })}</div>}
              ...
              <Col gap={10}>
                <div className="klaunch">
                  <span className="klaunch-lbl">{tr2({ uz: "🎧 Kod oynasi", ru: "🎧 Окно кода" })}</span>
                  <ul className="klaunch-pre">
                    <li><i>📄</i>{tr2({ uz: "4 ta yozuv", ru: "4 записи" })}</li>
                    <li><i>⚙️</i>{tr2({ uz: "1 ta funksiya", ru: "1 функция" })}</li>
                    <li><i>🎯</i>{tr2({ uz: "Natija: 3 ta qo'shiq", ru: "Результат: 3 песни" })}</li>
                  </ul>
...
var ScreenReflection = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Mustahkamlash · 2 qadam", ru: "Закрепление · 2 шага" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext turnBusy={!written} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Uch maydoningizni <span className="italic" style={{ color: T.accent }}>yoddan</span> ayta olasizmi?</>, ru: <>Сможете назвать свои три поля <span className="italic" style={{ color: T.accent }}>по памяти</span>?</> })}</h2></div>
        ...
        <div className="rcp-flow">
          <div className="rcp-step fade-up delay-1">
            <div className="rcp-step-h"><span className="rcp-n">1</span><div><span className="rcp-t">🗣 {yakka ? tr2({ uz: "Ovoz chiqarib ayting: qaysi uch maydon va qaysi bo'lim", ru: "Скажите вслух: какие три поля и какой раздел" }) : tr2({ uz: "Sherigingizga ayting: qaysi uch maydon va qaysi bo'lim", ru: "Скажите напарнику: какие три поля и какой раздел" })}</span></div></div>
          ...
          <div className="rcp-step fade-up delay-2">
            ...
            <span className={`turn-wrap${inputTurn ? " turn-ring" : ""}`}>
              <input className="reflect-input" value={text} onChange={(e) => save(e.target.value)} onFocus={() => setReflFocus(true)} onBlur={() => setReflFocus(false)} placeholder={tr2({ uz: "Ilova ... ni eslab qoladi, undan ... bo'limi quriladi", ru: "Приложение запоминает ..., из этого строится раздел ..." })} maxLength={160} />
            </span>
            {written && <p className="small" style={{ margin: 0, color: T.success, fontWeight: 700 }}>{tr2({ uz: "✓ Yozildi!", ru: "✓ Записано!" })}</p>}
          </div>
        </div>
        <MentorNote>{tr2({ uz: "Uchdan biri «qaysi bo'lim quriladi» degan savolga javob berolmasa — tugmalar ekranini qayta oching va bittasini birga o'chirib ko'ring.", ru: "Если треть класса не может ответить на вопрос «какой раздел строится» — снова откройте экран с переключателями и выключите один вместе." })}</MentorNote>
      </div>
    </Stage>;
};
...
var ScreenFinalTest = (props) => <QuestionScreen
  ...
  ctaLabel={tr2({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr2({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr2({ uz: "📋 Qaysi maydonni saqlashga arziydi?", ru: "📋 Какое поле стоит хранить?" })} />}
  questionText={tr2({ uz: "Qaysi maydonni saqlashga arziydi", ru: "Какое поле стоит хранить" })}
  options={[tr2({ uz: "Telefondagi kontaktlar ro'yxati", ru: "Список контактов в телефоне" }), tr2({ uz: "Qo'shiq oxirigacha tinglandimi", ru: "Дослушана ли песня до конца" }), tr2({ uz: "Qo'shiq qayerda tinglandi", ru: "Где прослушана песня" })]}
  correctIdx={1}
  explainCorrect={tr2({ uz: "To'g'ri — bundan «Sizga yoqadi» bo'limi quriladi; qolgan ikkitasi bo'lim bermaydi.", ru: "Верно — из этого строится раздел «Вам нравится»; два других раздела не дают." })}
  explainWrong={{
    0: tr2({ uz: "Kontaktlardan bitta ham bo'lim qurilmaydi — bu maydon bekorga saqlanadi.", ru: "Из контактов не строится ни один раздел — это поле хранится зря." }),
    2: tr2({ uz: "Qayerda tinglanganidan bitta ham bo'lim qurilmaydi — buni beshta tugma sinovida ko'rgansiz.", ru: "Из места прослушивания не строится ни один раздел — вы видели это в опыте с пятью переключателями." }),
    default: tr2({ uz: "Saqlashga arziydigan maydon ortida bitta bo'lim turadi.", ru: "За полем, которое стоит хранить, стоит один раздел." })
  }}
/>;
  ...
  return <div className="card hw fade-step" ref={innerRef}>
      ...
      {variant ? <div className="pmtask fade-step">
          ...
          <div className="pmtask-rows">
            <div className="pmtask-row"><span className="pmtask-k">{tr2({ uz: "Nechta", ru: "Сколько" })}</span><span className="pmtask-v"><b>{variant === "qisqa" ? tr2({ uz: "1 ta maydon", ru: "1 поле" }) : tr2({ uz: "3 maydon + 1 qo'shimcha", ru: "3 поля + 1 дополнительное" })}</b></span></div>
...
function QzFX() {
  ...
  useEffect3(() => {
    ...
    const TOK = tr2({ uz: ["maydon", "bo'lim", "saqlash", "tinglash", "yozuv", "tavsiya", "jadval", "ilova", "✅", "⬜"], ru: ["поле", "раздел", "хранение", "прослушивание", "запись", "рекомендация", "таблица", "приложение", "✅", "⬜"] });
    const em = [], toks = [];
      ...
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="hero">
          <div className="hero-l">
            ...
            <h2 className="title h-title fade-up d1">{tr2({ uz: <>Uchta <span className="italic" style={{ color: T.accent }}>maydoningiz</span> yozildi.</>, ru: <>Ваши три <span className="italic" style={{ color: T.accent }}>поля</span> записаны.</> })}</h2>
          </div>
...
`;
function PmLesson11({ lang: langProp, onFinished }) {
