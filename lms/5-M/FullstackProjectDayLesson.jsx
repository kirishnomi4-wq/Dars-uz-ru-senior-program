...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и число ✅/❌ скрыты — по кнопке «Открыть результат» всё появится сразу и у Вас, и на экранах учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верных — класс не разобрался в теме. Перед тем как идти дальше, стоит коротко повторить.</> })}</p>
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Loyiha kuni · kirish", ru: "Проектный день · введение" })} screen={screen} scrollSignal={sc} navContent={<NavNext optionalLive disabled={picked === null} label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} />}>
    ...
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null || !tried} style={{ opacity: !tried ? 0.55 : 1 }} onClick={() => pick(o.id)}>
            ...
            {picked !== null && <p className="hook-ack fade-step">{picked === "b" ? tr({ uz: <>Aynan! Bugun <b>qorovul uchun</b> stoyanka ilovasini quramiz: bo'sh/band joylar rangda ko'rinadi, mashina kirsa bir bosishda band bo'ladi, chiqsa to'lov yoziladi. To'liq fullstack — front, server va baza birga.</>, ru: <>Именно! Сегодня мы построим приложение стоянки <b>для охранника</b>: свободные/занятые места видны цветом, машина въехала — место занимается одним нажатием, выехала — записывается оплата. Полный fullstack — фронт, сервер и база вместе.</> }) : tr({ uz: <>Ikkinchi daftar chalkashlikni ikki baravar qiladi; joylarni kamaytirish — yechim emas, daromad kamayadi. Qorovulga <b>bir qarashda ko'rinadigan</b> panel kerak: bo'sh/band joylar rangda, kirish-chiqish bir bosishda. Bugun shuni quramiz — front, server va baza birga.</>, ru: <>Вторая тетрадь удваивает путаницу; сократить места — не решение, упадёт выручка. Охраннику нужна панель, где всё <b>видно с одного взгляда</b>: свободные/занятые места цветом, въезд-выезд одним нажатием. Сегодня мы её построим — фронт, сервер и база вместе.</> })}</p>}
          </Col>
        </Split>
        </Zoomable>
      </div>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Reja", ru: "План" })} screen={screen} mentorStatic scrollSignal={showSteps} navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Boshlaymiz →", ru: "Начинаем →" }} onClick={onNext} /></>}>
      <div className="screen">
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(storedAnswer ? new Set(PRINC.map((p) => p.id)) : /* @__PURE__ */ new Set());
  const [sc, setSc] = useState(0);
  ...
  return <Stage eyebrow={tr({ uz: "PM · foydalanuvchi", ru: "PM · пользователь" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : `${tr({ uz: "Qoidalarni ko'ring", ru: "Посмотрите правила" })} (${seen.size}/3)`} onClick={onNext} /></>}>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [active, setActive] = useState(null);
  const [seen, setSeen] = useState(storedAnswer ? new Set(JOY_COLS.map((c) => c.key)) : /* @__PURE__ */ new Set());
  const [sc, setSc] = useState(0);
  ...
  return <Stage eyebrow={tr({ uz: "1-qadam · ME'MOR", ru: "Шаг 1 · АРХИТЕКТОР" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : `${tr({ uz: "Ustunlarni ko'ring", ru: "Посмотрите столбцы" })} (${seen.size}/${JOY_COLS.length})`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            ...
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              {JOY_COLS.map((c) => <button key={c.key} className={`vcard ${!seen.has(c.key) ? "tap-hint" : ""}`} onClick={() => tap(c.key)} style={{ boxShadow: active === c.key ? `inset 0 0 0 1.5px ${T.accent}, 0 8px 20px -6px rgba(${T.shadowBase},0.2)` : void 0 }}>
                  <span className={`vbadge ${c.key === "id" ? "t-id" : "t-col"}`}>{tr(c.type)}</span>
          ...
          <Col>
            ...
            <pre className="code-box fade-up delay-1" style={{ lineHeight: 1.9 }}>
              ...
              {JOY_COLS.map((c) => <React.Fragment key={c.key}>
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [active, setActive] = useState(null);
  const [seen, setSeen] = useState(storedAnswer ? new Set(SES_COLS.map((c) => c.key)) : /* @__PURE__ */ new Set());
  const [sc, setSc] = useState(0);
  ...
  return <Stage eyebrow={tr({ uz: "1-qadam · ME'MOR", ru: "Шаг 1 · АРХИТЕКТОР" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : `${tr({ uz: "Ustunlarni ko'ring", ru: "Посмотрите столбцы" })} (${seen.size}/${SES_COLS.length})`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
          ...
          <Col>
            ...
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              {SES_COLS.map((c) => <button key={c.key} className={`vcard ${!seen.has(c.key) ? "tap-hint" : ""}`} onClick={() => tap(c.key)} style={{ boxShadow: active === c.key ? `inset 0 0 0 1.5px ${T.accent}, 0 8px 20px -6px rgba(${T.shadowBase},0.2)` : c.fk ? `inset 0 0 0 1.5px ${T.blue}55` : void 0 }}>
                  <span className={`vbadge ${c.fk ? "t-fk" : c.key === "id" ? "t-id" : "t-col"}`}>{tr(c.type)}</span>
                  <span className="vlbl mono">{c.key}</span>
                  <span className={`vseen ${seen.has(c.key) ? "on" : ""}`} style={{ color: seen.has(c.key) ? T.success : T.ink3 }}>{seen.has(c.key) ? "✓" : ""}</span>
                </button>)}
            </div>
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(storedAnswer ? /* @__PURE__ */ new Set([2, 5]) : /* @__PURE__ */ new Set());
  const [sc, setSc] = useState(0);
  ...
  return <Stage eyebrow={tr({ uz: "Bog'lanish", ru: "Связь" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : `${tr({ uz: "2 ta joyni tekshiring", ru: "Проверьте 2 места" })} (${seen.size}/2)`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "2-qadam · REJISSYOR (backend)", ru: "Шаг 2 · РЕЖИССЁР (бэкенд)" })} screen={screen} scrollSignal={phase} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={navLbl} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            ...
            {phase >= 2 && <div className="ai-card fade-step"><div className="ai-row"><span className="ai-badge">AI</span><span className="ai-bubble">{tr({ uz: "To'g'riladim — kirishda joy ham band bo'ladi:", ru: "Исправил — при въезде место тоже занимается:" })}</span></div><div className="ai-code"><div className="ai-line" style={{ whiteSpace: "pre-wrap" }}>{AI_V2_HEAD}</div><div className="ai-line ok" style={{ whiteSpace: "pre-wrap" }}>{AI_V2_FIX}</div><div className="ai-line" style={{ whiteSpace: "pre-wrap" }}>{AI_V2_TAIL}</div></div></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr({ uz: "Bu backend nima qilishi kerak", ru: "Что должен делать этот бэкенд" })}</p>
...
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "2-qadam · REJISSYOR (backend)", ru: "Шаг 2 · РЕЖИССЁР (бэкенд)" })} screen={screen} scrollSignal={step} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Kirish va chiqishni ko'ring", ru: "Посмотрите въезд и выезд" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen9b = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "2-qadam · REJISSYOR (front)", ru: "Шаг 2 · РЕЖИССЁР (фронт)" })} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Promptni AI'ga bering", ru: "Отправьте промпт ИИ" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            ...
            {!done ? <button className="btn fade-step" style={{ alignSelf: "flex-start" }} onClick={() => setDone(true)}>{tr({ uz: "📤 Promptni AI'ga yuborish", ru: "📤 Отправить промпт ИИ" })}</button> : <div className="ai-card fade-step"><div className="ai-row"><span className="ai-badge">AI</span><span className="ai-bubble">{tr({ uz: "Panel komponenti yozildi:", ru: "Компонент панели написан:" })}</span></div><div className="ai-code"><div className="ai-line ok" style={{ cursor: "default", whiteSpace: "pre-wrap" }}>{"function Panel(){\n  const [joylar,setJoylar]=useState([]);\n  const yukla=()=>fetch('/api/joylar')...;  // joylarni olish\n  useEffect(()=>{ yukla(); },[]);\n  const kirgiz=(joy_id,mashina)=>fetch(...);  // POST → yukla\n  const chiqar=(id,joy_id)=>fetch(...);  // PUT → yukla\n  return <Grid joylar={joylar} .../>;\n}"}</div></div></div>}
          </Col>
          <Col>
...
var Screen10 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "GET — joylar ro'yxatini o'qish", ru: "GET — прочитать список мест" }), tr({ uz: "DELETE — sessiyani o'chirish", ru: "DELETE — удалить сеанс" }), tr({ uz: "CSS — panelni chiroyli bezash", ru: "CSS — красиво оформить панель" }), tr({ uz: "POST — yangi sessiya yaratish", ru: "POST — создать новый сеанс" })]}
  correctIdx={3}
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "3-qadam · NAZORATCHI", ru: "Шаг 3 · КОНТРОЛЁР" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Bir kirish + bir chiqishni sinang", ru: "Проверьте один въезд + один выезд" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "JOIN · tarix", ru: "JOIN · история" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "JOIN bilan birlashtiring", ru: "Объедините через JOIN" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen13 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "DELETE — eski sessiyalarni o'chirish", ru: "DELETE — удалить старые сеансы" }), tr({ uz: "Sessiyalarga yangi ustun qo'shish", ru: "Добавить в sessiyalar новый столбец" }), tr({ uz: "JOIN — ikki jadvalni birlashtirish", ru: "JOIN — объединить две таблицы" }), tr({ uz: "CSS — jadvalga rang berish", ru: "CSS — раскрасить таблицу" })]}
  correctIdx={2}
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Amaliyot · to'liq panel", ru: "Практика · полная панель" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : `${tr({ uz: "Kamida 3 amal qiling", ru: "Сделайте минимум 3 действия" })} (${Math.min(acts, 3)}/3)`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen15 = ({ screen, onNext, onPrev }) => <Stage eyebrow={tr({ uz: "Qoida · xulosa", ru: "Правило · вывод" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Oxirgi qadam →", ru: "Последний шаг →" }} onClick={onNext} /></>}>
...
var Screen16 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} scrollSignal={passed} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!passed} label={passed ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Bog'lanish shartini yozing", ru: "Напишите условие связи" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var ScreenPodium = ({ screen, answers, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} /></>}>
    ...
    return <div key={q} className="qstat-row">
              ...
              {live.mode === "mentor" && <p className="small" style={{ margin: "10px 0 0", color: T.ink2 }}>{tr({ uz: "⚠️ belgili savollar — sinf qiynalgan mavzular. Ularni qayta tushuntirib o'ting.", ru: "Вопросы со знаком ⚠️ — темы, где класс споткнулся. Объясните их ещё раз." })}</p>}
            </div>
          </>}
      </div>
    </Stage>;
...
function OpeningAct() {
  ...
  return <div className={`oact fade-step ${act ? "act" : ""}`} role="status" aria-label={tr({ uz: "Stoyanka ochildi", ru: "Стоянка открыта" })}>
      <span className="oact-gate" aria-hidden="true"><i className="oact-arm" /></span>
      <span className="oact-ship">{tr({ uz: "🚦 Stoyanka ochildi", ru: "🚦 Стоянка открыта" })}</span>
      <span className="oact-sub">{tr({ uz: "Sxema · Backend · Panel · Sinov — 4/4", ru: "Схема · Бэкенд · Панель · Проверка — 4/4" })}</span>
...
/>;
function FullstackProjectDayLesson({ lang: langProp, onFinished }) {
