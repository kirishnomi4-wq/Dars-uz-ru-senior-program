...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и счёт ✅/❌ скрыты — при нажатии «Открыть результат» всё откроется сразу и у вас, и на экранах учеников." })}</p>}
...
var sectionPromptWords = (s = {}) => Object.keys(SECTION_PROMPT).filter((k) => s[k]).map((k) => tr(SECTION_PROMPT[k]));
...
var LandingPreview = ({ topic, style: styleIn, color: colorIn, sections = {}, noaniq = false, draft = false }) => {
  if (draft) {
    ...
    const notes = missing.length ? [tr({ uz: `${missUz.charAt(0).toUpperCase()}${missUz.slice(1)}ni aytmadingiz — o'zim to'ldirdim`, ru: `Вы не назвали ${missing.join(" и ")} — заполнил сам` })] : [];
    return <div className="lp-draft lp-live" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        <div style={{ background: heroBg, color: heroColor, borderRadius: hasStyle ? radius2 : 5, padding: "clamp(16px,3vw,22px)", border: heroBorder, textAlign: "center", transform: heroTilt, position: "relative" }}>
          ...
          <span style={{ fontSize: 10.5, fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", opacity: 0.9 }}>{hasTopic ? tr(p2.tag) : tr({ uz: "Sayt", ru: "Сайт" })}</span>
          <h3 style={{ fontFamily: "'Source Serif 4',serif", fontWeight: 700, fontSize: "clamp(20px,4vw,30px)", margin: "5px 0 6px", lineHeight: 1.08, transform: hasStyle ? "none" : "rotate(1.4deg)" }}>{hasTopic ? tr(p2.title) : tr({ uz: "SAYT", ru: "САЙТ" })}</h3>
          <p style={{ fontSize: 13, margin: 0, opacity: 0.92, fontStyle: hasTopic ? "normal" : "italic" }}>{hasTopic ? tr(p2.sub) : tr({ uz: "Bla-bla lorem — bu yerda matn bo'lishi kerak edi…", ru: "Бла-бла лорем — тут должен был быть текст…" })}</p>
          {sections.button && <button style={{ marginTop: 13, border: "none", borderRadius: Math.max((hasStyle ? radius2 : 5) - 4, 6), padding: "9px 18px", fontFamily: "'Manrope'", fontWeight: 700, fontSize: 13, cursor: "pointer", background: hasColor ? "#fff" : "#12121a", color: hasColor ? c2 : "#c6ff00", transform: hasStyle ? "none" : "rotate(-1.6deg)" }}>{hasTopic ? tr(p2.cta) : tr({ uz: "Tugma", ru: "Кнопка" })}</button>}
        </div>
        ...
        {sections.banner && <div style={{ background: hasColor ? `${c2}1f` : "rgba(255,20,147,0.14)", color: hasColor ? c2 : "#ff1493", borderRadius: 8, padding: "9px 12px", fontSize: 12.5, fontWeight: 700, textAlign: "center", transform: hasStyle ? "none" : "rotate(-1deg)" }}>{tr({ uz: "★ Maxsus taklif — faqat shu hafta!", ru: "★ Спецпредложение — только на этой неделе!" })}</div>}
  ...
  return <Stage eyebrow={{ uz: "Kirish", ru: "Введение" }} screen={screen} navContent={<NavNext optionalLive disabled={picked === null} label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} />}>
    ...
    /* maxWidth 820 → 1000: sarlavha ikki qatordan bittaga tushadi va quyidagi savol-variantlar
       ko'rish zonasiga sig'adi (F-0803-26) */
        ...
        <h1 className="title h-title fade-up" style={{ maxWidth: 1e3 }}>{tr({ uz: <>AI'ga shunchaki <span className="italic" style={{ color: T.accent }}>«sayt yasab ber»</span> desangiz — nima chiqadi?</>, ru: <>Что получится, если сказать AI просто <span className="italic" style={{ color: T.accent }}>«сделай сайт»</span>?</> })}</h1>
        ...
        <div className="split">
          <Col>
            ...
            {part ? <div className="sk-info fade-step" key={part}>
              ...
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Buyruqdagi 4 ta rangli qismni bosing", ru: "Нажмите на 4 цветные части команды" })}</p></div>}
          </Col>
          <Col>
...
var Screen4 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "Aniq tafsilot beradi: nima, uslub, rang, qismlar", ru: "Даёт точные детали: что, стиль, цвет, части" }), tr({ uz: "Shunchaki uzunroq bo'ladi", ru: "Просто длиннее" }), tr({ uz: "Faqat inglizcha yoziladi", ru: "Пишется только по-английски" }), tr({ uz: "Hech qanday farqi yo'q", ru: "Никакой разницы" })]}
  correctIdx={0}
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [mode, setMode] = useState("yomon");
  const [sent, setSent] = useState(null);
  const [phase, setPhase] = useState("idle");
  const [seenBuilt, setSeenBuilt] = useState(/* @__PURE__ */ new Set());
  const timer = useRef(null);
      ...
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            ...
            {mode === "yaxshi" ? <PromptLine topic="oyin" style="oynoqi" color="yashil" sections={{ button: true, cards: true, banner: true }} /> : <div className="promptbox">{tr({ uz: <>Menga <span className="pb-slot">o'yin sayti</span> yasab ber.</>, ru: <>Сделай мне <span className="pb-slot">сайт про игру</span>.</> })}</div>}
...
var Screen7 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "Menga biror narsa qilib ber", ru: "Сделай мне что-нибудь" }), tr({ uz: "Ko'k rangli, zamonaviy o'yin promo sahifasi — sarlavha, tugma va 3 ta karta bilan", ru: "Синяя современная промо-страница игры — с заголовком, кнопкой и 3 карточками" }), tr({ uz: "Chiroyli va zamonaviy qilib ber", ru: "Сделай красиво и современно" }), tr({ uz: "O'yin haqida sayt qil", ru: "Сделай сайт про игру" })]}
  correctIdx={1}
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [usedIds, setUsedIds] = useState(/* @__PURE__ */ new Set());
...
var Screen9 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "Hammasini qo'lda qayta yozaman", ru: "Перепишу всё руками" }), tr({ uz: "Tashlab ketaman", ru: "Брошу и уйду" }), tr({ uz: "Qayta, aniqroq so'rayman (iteratsiya)", ru: "Попрошу снова, точнее (итерация)" }), tr({ uz: `"AI yomon" deb xafa bo'laman`, ru: "Обижусь: «AI плохой»" })]}
  correctIdx={2}
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [sent, setSent] = useState(null);
  const [phase, setPhase] = useState("idle");
  const [builtTopics, setBuiltTopics] = useState(/* @__PURE__ */ new Set());
  const timer = useRef(null);
...
function PracticeLesson2({ lang: langProp, onFinished }) {
