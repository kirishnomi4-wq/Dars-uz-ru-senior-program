...
//  Manba:  src/1-Modull/VsCodeLesson.jsx
//          src/compilator/HtmlCompiler.jsx
//  Qayta yig'ish:  node scripts/build-lms.mjs src/1-Modull/VsCodeLesson.jsx
...
var LESSON_META = { lessonId: "vscode-start-01-v1", lessonTitle: { uz: "VS Code — professional start", ru: "VS Code — профессиональный старт" } };
var HOMEWORK = {
  type: "file",
...
var SCREEN_META = [
  ...
  // 0  Dasturchilar kodni qayerga yozadi?
  { id: "s1", type: "rule", template: "custom", scored: false, scope: null },
  // 1  Maqsad: namuna-card + 6 qadam
  { id: "s2", type: "exploration", template: "custom", scored: false, scope: null },
  // 2  QADAM 1 — VS Code o'rnatish (checklist)
  { id: "s3", type: "exploration", template: "custom", scored: false, scope: null },
  // 3  QADAM 2 — interfeys-ekskursiya (hotspot)
  { id: "s4", type: "test", template: "MCScreen", scored: true, scope: "module-mikro" },
  // 4  TEST 1 — Explorer
  { id: "s5", type: "exploration", template: "custom", scored: false, scope: null },
  // 5  QADAM 3 — papka + fayllar (checklist)
  { id: "s6", type: "exploration", template: "custom", scored: false, scope: null },
  // 6  Emmet: ! + Tab + shablonni yig'ish (drag)
  { id: "s7", type: "exploration", template: "custom", scored: false, scope: null },
  // 7  QADAM 4 — Live Server / Go Live (checklist) → praktika
  { id: "s8", type: "test", template: "MCScreen", scored: true, scope: "module-mikro" },
  // 8  TEST 2 — Go Live nima qiladi
  { id: "s9", type: "exploration", template: "custom", scored: false, scope: null },
  // 9  QADAM 5 — card HTML → praktika
  { id: "s10", type: "exploration", template: "custom", scored: false, scope: null },
  // 10 QADAM 6 — card CSS → praktika
  { id: "s11", type: "test", template: "MCScreen", scored: true, scope: "module-mikro" },
  // 11 TEST 3 — link/stylesheet
  { id: "s12", type: "exploration", template: "custom", scored: false, scope: null },
  // 12 Tezkor tugmalar (shortcut)
  { id: "s13", type: "test", template: "custom", scored: true, scope: "final" },
  // 13 YAKUNIY yozma — Go Live
  { id: "s14", type: "case", template: "custom", scored: false, scope: null },
  // 14 FINAL — jonli card tasdig'i + tantana
  { id: "sproj", type: "case", template: "custom", scored: false, scope: null },
  // 15 Demo Day loyihasi (F-0803-30)
...
var RECAPS = {
  ...
  4: {
    title: { uz: "VS Code oynasi", ru: "Окно VS Code" },
    cards: [
      {
        ic: "🗂️",
        h: { uz: "Explorer — fayllar ro'yxati", ru: "Explorer — список файлов" },
        body: {
      ...
      {
        ic: "📝",
        h: { uz: "Editor — kod yoziladigan panel", ru: "Editor — панель, где пишется код" },
        body: {
      ...
      {
        ic: "🧩",
        h: { uz: "Extensions — qo'shimcha dasturlar paneli", ru: "Extensions — панель дополнений" },
        body: {
  ...
  8: {
    title: { uz: "Jonli server", ru: "Живой сервер" },
    cards: [
      {
        ic: "⚡",
        h: { uz: "Go Live — sahifa brauzerda jonlanadi", ru: "Go Live — страница оживает в браузере" },
        body: {
      ...
      {
        ic: "🏠",
        h: { uz: "127.0.0.1 — «mening kompyuterim»", ru: "127.0.0.1 — «мой компьютер»" },
        body: {
      ...
      {
        ic: "💾",
        h: { uz: "Ctrl+S — saqlang, brauzer o'zi yangilanadi", ru: "Ctrl+S — сохраните, браузер обновится сам" },
        body: {
  ...
  11: {
    title: { uz: "HTML va CSS ulanishi", ru: "Связь HTML и CSS" },
    cards: [
      {
        ic: "🔗",
        h: { uz: "<link> — ikki fayl orasidagi ko'prik", ru: "<link> — мост между двумя файлами" },
        body: {
      ...
      {
        ic: "🎨",
        h: { uz: "HTML — nima bor, CSS — qanday ko'rinadi", ru: "HTML — что есть, CSS — как выглядит" },
        body: {
      ...
      {
        ic: "🎯",
        h: { uz: "class — CSS uchun nishon", ru: "class — мишень для CSS" },
        body: {
  ...
  13: {
    title: { uz: "Bir bosishda jonli sahifa", ru: "Живая страница в одно нажатие" },
    cards: [
      {
        ic: "⚡",
        h: { uz: "Tugma nomi — Go Live", ru: "Кнопка называется Go Live" },
        body: {
      ...
      {
        ic: "📖",
        h: { uz: "Manzilni endi o'qiy olasiz", ru: "Теперь вы умеете читать адрес" },
        body: {
      ...
      {
        ic: "🔁",
        h: { uz: "Har kunlik ish tartibi", ru: "Ежедневный порядок работы" },
        body: {
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">🙈 {tr3({ uz: "Kim nimani tanlagani va ✅/❌ soni hozircha yashirin. «Natijani ochish» bosilganda — sizda ham, o'quvchilarda ham birdan ochiladi.", ru: "Кто что выбрал и число ✅/❌ пока скрыто. Нажмёте «Открыть результат» — появится сразу и у вас, и у учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr3({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верных — тема осталась непонятной классу. Перед продолжением сделайте короткое повторение.</> })}</p>
      ...
      {reveal && struggling && <p className="mstats-warn">⚠️ {tr3({ uz: "Ko'pchilik adashdi — mavzu tushunarsiz qolganga o'xshaydi. Yana bir bor tushuntiring.", ru: "Большинство ошиблись — похоже, тема осталась непонятной. Объясните ещё раз." })}</p>}
...
function MentorPracticeOverlay({ entry, live, onClose }) {
  ...
  return <div className="mp-overlay">
      <div className="mp-card">
        ...
        <p className="mp-tip">💡 {tr3({ uz: "Ko'pchilik tugatgach, shu mashqni doskada birga yozing. Shunda o'quvchilar o'zini tekshiradi va mavzu esida mahkam qoladi.", ru: "Когда закончит большинство, напишите это же упражнение вместе на доске. Так ученики проверят себя, а тема запомнится крепко." })}</p>
      </div>
    </div>;
}
...
function DragDropOrder({ items, hints, onSolved }) {
  ...
  return <div className="dd fade-up">
      ...
      {solved && <div className="dd-done">✓ {tr3({ uz: "To'g'ri! Shablon aynan shu tartibda.", ru: "Верно! Шаблон именно в таком порядке." })}</div>}
      {wrong && !solved && <div className="dd-wrong">⚠️ {tr3({ uz: "Tartib xato — qayta joylang.", ru: "Порядок неверный — расставьте заново." })}</div>}
    </div>;
}
var VSCODE_FLASHCARDS = [
  ...
  { front: { uz: "VS Code'ga yangi imkoniyat qo'shadigan mini-dastur qanday ataladi?", ru: "Как называется мини-программа, добавляющая VS Code новые возможности?" }, back: { uz: "Extension (qo'shimcha dastur)", ru: "Extension (дополнение)" }, note: { uz: "kubiklar belgisidagi Extensions panelidan o'rnatiladi", ru: "ставится из панели Extensions со значком кубиков" } },
  { front: { uz: "Sahifani brauzerda jonli ochadigan qo'shimcha dastur qaysi?", ru: "Какое дополнение живо открывает страницу в браузере?" }, back: "Live Server", note: { uz: "pastki ko'k chiziqdagi Go Live tugmasi bilan ishga tushadi", ru: "запускается кнопкой Go Live на нижней синей полосе" } },
  ...
  { front: { uz: "127.0.0.1:5500 manzilidagi 5500 raqami nima?", ru: "Что такое число 5500 в адресе 127.0.0.1:5500?" }, back: { uz: "Eshik raqami (port)", ru: "Номер двери (порт)" }, note: { uz: "sahifa aynan shu eshikdan chiqadi", ru: "страница выходит именно через эту дверь" } },
  { front: { uz: "Bo'sh HTML faylda tayyor shablonni nima bilan chiqarasiz?", ru: "Чем вы выводите готовый шаблон в пустом HTML-файле?" }, back: "! + Tab", note: { uz: "Emmet yordamchisi butun shablonni o'zi yozadi", ru: "помощник Emmet сам пишет весь шаблон" } },
  ...
  { front: { uz: "Har yangi loyiha uchun nima yaratamiz?", ru: "Что мы создаём для каждого нового проекта?" }, back: { uz: "Alohida papka", ru: "Отдельную папку" }, note: { uz: "nomi inglizcha, bo'sh joysiz — masalan `my-site`", ru: "имя английское, без пробелов — например `my-site`" } },
...
var CARD_NAME = "Steve Jobs";
var CARD_PROF = "Frontend developer";
var VSC = { bg: "#1E1E1E", side: "#252526", act: "#333333", line: "#3C3C3C", text: "#D4D4D4", dim: "#858585", blue: "#007ACC", tag: "#569CD6", attr: "#9CDCFE", str: "#CE9178", kw: "#C586C0" };
var VscIc = ({ k }) => {
  const P = {
...
var VscFile = ({ name, on, css }) => <div className={`vsc-file ${on ? "on" : ""}`}>
    <span className="vsc-fic" style={{ color: css ? "#519ABA" : "#E37933" }}>{css ? "#" : "<>"}</span>
    <span>{name}</span>
  </div>;
var VscShell = ({ title = "Visual Studio Code", act = 0, side, main, panel, statusRight, statusPulse }) => <div className="vsc">
...
var VL = ({ n, ind = 0, children }) => <div className="vsc-ln"><span className="vsc-lnum">{n}</span><span className="vsc-lcode" style={{ paddingLeft: ind * 14 }}>{children}</span></div>;
var Vt = ({ children }) => <span style={{ color: VSC.tag }}>{children}</span>;
var Va = ({ children }) => <span style={{ color: VSC.attr }}>{children}</span>;
var Vs = ({ children }) => <span style={{ color: VSC.str }}>{children}</span>;
var BrowserWin = ({ url = "127.0.0.1:5500/index.html", tab, children, minH }) => <div className="brw">
    ...
    <div className="brw-body" style={minH ? { minHeight: minH } : void 0}>{children}</div>
  </div>;
var VCard = ({ name = CARD_NAME, prof = CARD_PROF, flags, plain, anim }) => {
  const f = flags || { bg: true, center: true, radius: true, shadow: true };
  if (plain) {
    return <div className="vcard-plainwrap">
  ...
  return <div className={`vcard-stage ${anim ? "vc-anim" : ""}`}>
...
function StepChecklist({ screen, storedAnswer, onAnswer, steps, taskLabel }) {
  ...
  const live = gate.live;
  const isMentorLive = !!(live && live.mode === "mentor");
  const [done, setDone] = useState4(() => new Set(storedAnswer?.doneIds || (storedAnswer?.correct ? steps.map((s) => s.id) : [])));
  const sentRef = useRef4(!!(storedAnswer && storedAnswer.correct));
  ...
  return <div className="clk fade-up delay-2">
      <div className="clk-head">
        <span className="clk-lbl">✅ {tr3({ uz: "Kompyuteringizda bajaring", ru: "Выполните на своём компьютере" })}</span>
        <span className={`clk-count ${allDone ? "ok" : ""}`}>{done.size}/{steps.length}</span>
      </div>
      {steps.map((s, i) => {
    const on = done.has(s.id);
    return <div key={s.id} className={`clk-row ${on ? "on" : ""}`}>
            <span className="clk-num">{on ? "✓" : i + 1}</span>
            <span className="clk-txt">
              <span className="clk-t">{fmtCode(tr3(s.t))}</span>
              {s.d && <span className="clk-d">{fmtCode(tr3(s.d))}</span>}
            </span>
            <button className={`clk-btn ${on ? "on" : ""}`} onClick={() => toggle(s.id)} disabled={isMentorLive}>
              {on ? tr3({ uz: "✓ Bajarildi", ru: "✓ Готово" }) : tr3({ uz: "Bajardim", ru: "Сделал(а)" })}
            </button>
          </div>;
  })}
      {allDone && !isMentorLive && <p className="clk-done fade-step">🎉 {tr3({ uz: "Zo'r! Hamma qadam bajarildi — davom etamiz.", ru: "Отлично! Все шаги выполнены — продолжаем." })}</p>}
      {isMentorLive && <MentorWorkStats live={live} screenIdx={screen} taskLabel={taskLabel} />}
    </div>;
}
var ScreenHook = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const OPTS = [
    ...
    { id: "b", label: { uz: "Maxsus dasturda — kod redaktorida yozadi", ru: "Пишут в специальном редакторе кода" } },
  ...
  return <Stage eyebrow={tr3({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr3({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr3({ uz: <>Dasturchilar kodni <span className="italic" style={{ color: T.accent }}>qayerga</span> yozadi?</>, ru: <>Куда разработчики <span className="italic" style={{ color: T.accent }}>пишут</span> код?</> })}</h1>
        <Mentor>{tr3({ uz: <>Shu paytgacha kodni <b style={{ color: T.ink }}>dars ichidagi kompilatorga</b> yozdik — u mashq joyi. Bugun kodni o'z kompyuteringizga yozasiz. Sizningcha, dasturchilar buni qayerga yozadi?</>, ru: <>До этого мы писали код <b style={{ color: T.ink }}>в компиляторе внутри урока</b> — это место для тренировки. Сегодня вы будете писать код на своём компьютере. Как думаете, куда его пишут разработчики?</> })}</Mentor>
        <Split>
          <Col>
            <p className="eyebrow fade-up delay-1" style={{ color: T.ink2, margin: 0 }}>{tr3({ uz: "Sizningcha qaysi biri?", ru: "Как вы думаете, где?" })}</p>
    ...
    main={<>
                  ...
                  <div className="vsc-code">
                    ...
                    <VL n={2}><Vt>&lt;p&gt;</Vt><span style={{ color: VSC.text }}>{tr3({ uz: "Mening birinchi loyiham", ru: "Мой первый проект" })}</span><Vt>&lt;/p&gt;</Vt></VL>
  ...
  />
            ...
            {picked === null && <p className="mono small fade-up delay-3" style={{ color: T.ink3, textAlign: "center" }}>{tr3({ uz: "↑ javob tanlasangiz — ochiladi", ru: "↑ выберите ответ — и он откроется" })}</p>}
          </Col>
        </Split>
      </div>
    </Stage>;
};
var ScreenGoal = ({ screen, onNext, onPrev }) => {
  const audio = useAudio([{ id: "s1", text: "Mana shu card — o'zingiz haqingizdagi kichkina sahifa. Dars oxirida u o'z kompyuteringizda ishlaydi. Unga 6 qadamda yetib boramiz.", trigger: "on_mount", waits_for: null }]);
  const STEPS = [
  ...
  const PreviewBlock = <Col>
      <p className="flow-label">{tr3({ uz: "Natija — dars oxirida kompyuteringizda shunday bo'ladi", ru: "Результат — в конце урока на вашем компьютере будет так" })}</p>
      <BrowserWin tab="Mening cardim" minH={230}>
        <VCard anim />
      </BrowserWin>
    </Col>;
  const StepsBlock = <Col>
      <p className="flow-label">{tr3({ uz: "6 qadam", ru: "6 шагов" })}</p>
      <ol className="roadmap">
        {STEPS.map((s, i) => <li key={i} className="step-card fade-up" style={{ animationDelay: `${0.08 + i * 0.05}s` }}>
  ...
  return <Stage eyebrow={tr3({ uz: "Maqsad", ru: "Цель" })} screen={screen} audioState={audio} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr3({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head">
          <h2 className="title h-title fade-up">{tr3({ uz: <>Bugun <span className="italic" style={{ color: T.accent }}>o'z kompyuteringizda</span> birinchi sahifangizni yasaysiz.</>, ru: <>Сегодня вы сделаете первую страницу <span className="italic" style={{ color: T.accent }}>на своём компьютере</span>.</> })}</h2>
        </div>
        <Mentor>{tr3({ uz: <>Mana shu <b style={{ color: T.ink }}>card</b> — o'zingiz haqingizdagi kichkina sahifa. Dars oxirida u <b style={{ color: T.ink }}>o'z kompyuteringizda</b> ishlaydi. Unga <b style={{ color: T.ink }}>6 qadamda</b> yetib boramiz.</>, ru: <>Вот эта <b style={{ color: T.ink }}>card</b> — маленькая страница о вас самих. К концу урока она заработает <b style={{ color: T.ink }}>на вашем компьютере</b>. Дойдём до неё за <b style={{ color: T.ink }}>6 шагов</b>.</> })}</Mentor>
        {!isNarrow ? <Split>{PreviewBlock}{StepsBlock}</Split> : !showSteps ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>
            {PreviewBlock}
            <button className="btn" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(true)}>📋 {tr3({ uz: "Bugungi 6 qadamni ko'rish", ru: "Посмотреть 6 шагов на сегодня" })}</button>
          </div> : <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>
            <button className="btn-ghost" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(false)}>↩ {tr3({ uz: "Natijani ko'rish", ru: "Посмотреть результат" })}</button>
            {StepsBlock}
          </div>}
      </div>
    </Stage>;
        ...
        <Split>
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {VIEWS.map((v) => <button key={v.id} className={`chip ${view === v.id ? "chip-on" : ""}`} onClick={() => setView(v.id)}>{tr3(v.chip)}</button>)}
            </div>
            <div className="demo-swap" key={view}>
              {view === 0 && <BrowserWin url="code.visualstudio.com" tab="Visual Studio Code" minH={190}>
                  <div className="inst-site">
                    <div className="inst-logo">{"</>"}</div>
                    <p className="inst-h">Visual Studio Code</p>
                    <p className="inst-sub">{tr3({ uz: "Free. Built on open source. Runs everywhere.", ru: "Free. Built on open source. Runs everywhere." })}</p>
                    <span className="inst-dl">⬇ Download for Windows</span>
                  </div>
                </BrowserWin>}
              {view === 1 && <BrowserWin url="code.visualstudio.com" tab="Visual Studio Code" minH={190}>
                  <div className="inst-site">
                    ...
                    <p className="inst-sub">{tr3({ uz: "Fayl brauzerning pastida yoki «Yuklanmalar» papkasida ko'rinadi", ru: "Файл появится внизу браузера или в папке «Загрузки»" })}</p>
                  </div>
                </BrowserWin>}
              {view === 2 && <div className="inst-setup">
                  ...
                  <div className="inst-setbody">
                    ...
                    <p className="inst-sub" style={{ marginTop: 8 }}>{tr3({ uz: "Bir daqiqadan so’ng — tayyor!", ru: "Через минуту — готово!" })}</p>
                  </div>
                </div>}
            </div>
          </Col>
...
var ScreenTour = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const audio = useAudio([{ id: "s3", text: "Bu — sizning yangi ish stolingiz. Oynaning 4 panelini birma-bir bosib chiqing: Explorer, Editor, Extensions va Terminal. Har birini bossangiz, nima ekanini tushuntiraman.", trigger: "on_mount", waits_for: null }]);
  const HOTS = {
    explorer: {
      name: { uz: "Explorer — fayllar paneli", ru: "Explorer — панель файлов" },
      ic: "🗂️",
      body: { uz: <>Loyihangizdagi <b>barcha fayllar ro'yxati</b>. Xuddi papka oynasi kabi: faylni bossangiz — <b>Editor</b> panelida ochiladi. Tepadagi birinchi ikonka shu panelni ochib-yopadi.</>, ru: <>Список <b>всех файлов</b> вашего проекта. Как окно папки: нажмёте на файл — он откроется в панели <b>Editor</b>. Первая иконка сверху открывает и закрывает эту панель.</> }
    },
    editor: {
      name: { uz: "Editor — kod paneli", ru: "Editor — панель кода" },
      ic: "📝",
      body: { uz: <>Eng katta panel — kod <b>aynan shu yerga yoziladi</b>. Har ochilgan fayl tepada o'z varag'i (tab) bilan turadi. Qator raqamlari chapda — xato qidirishda juda asqotadi.</>, ru: <>Самая большая панель — код <b>пишется именно здесь</b>. Каждый открытый файл держит сверху свою вкладку (tab). Номера строк слева — очень помогают искать ошибки.</> }
    },
    ext: {
      name: { uz: "Extensions — qo'shimcha dasturlar", ru: "Extensions — дополнения" },
      ic: "🧩",
      body: { uz: <>Kubiklar belgisi — <b>Extensions</b> paneli. Extension (qo'shimcha dastur) — VS Code'ga yangi imkoniyat qo'shadigan mini-dastur. Birozdan keyin shu yerdan <b>Live Server</b>'ni o'rnatamiz.</>, ru: <>Значок из кубиков — панель <b>Extensions</b>. Extension (дополнение) — мини-программа, добавляющая VS Code новую возможность. Чуть позже мы поставим отсюда <b>Live Server</b>.</> }
    },
    term: {
      name: { uz: "Terminal — buyruq paneli", ru: "Terminal — панель команд" },
      ic: "⌨️",
      body: { uz: <>Pastdagi panel — <b>Terminal</b>: kompyuterga yozma buyruq beriladigan joy. Hozircha shunchaki tanishib qo'ying — tez orada juda kerak bo'ladi.</>, ru: <>Нижняя панель — <b>Terminal</b>: место, где компьютеру дают письменные команды. Пока просто познакомьтесь — совсем скоро он очень пригодится.</> }
    }
  };
  const _gate = useContext2(LiveGateCtx) || {};
  const _isMentorLive = !!(_gate.live && _gate.live.mode === "mentor");
  const [active, setActive] = useState4(null);
  const [seen, setSeen] = useState4(() => new Set(storedAnswer?.seenIds || (storedAnswer?.correct ? Object.keys(HOTS) : [])));
  const sentRef = useRef4(!!(storedAnswer && storedAnswer.correct));
  ...
  return <Stage eyebrow={tr3({ uz: "2-qadam · Tanishuv", ru: "2-й шаг · Знакомство" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !allSeen} label={_isMentorLive || allSeen ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "4 panelni bosib chiqing", ru: "Нажмите на 4 панели" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>VS Code oynasida <span className="italic" style={{ color: T.accent }}>nimalar bor</span>?</>, ru: <>Что <span className="italic" style={{ color: T.accent }}>есть</span> в окне VS Code?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Bu — sizning yangi ish stolingiz. Oynaning <b style={{ color: T.ink }}>4 panelini</b> birma-bir bosib chiqing — har birining vazifasini aytib beraman.</>, ru: <>Это ваш новый рабочий стол. Нажмите по очереди на <b style={{ color: T.ink }}>4 панели</b> окна — расскажу, для чего каждая.</> })}</Mentor>
        <p className="sk-tapguide fade-up delay-1">👆 {tr3({ uz: <>Oynaning panellarini <b>bosing</b></>, ru: <>Нажимайте на панели окна</> })}<span className="sk-tapcount">{seen.size}/4</span></p>
        <Split>
          <Col>
          ...
          <Col>
            {active ? <div className="sk-info fade-step" key={active}>
                <div className="sk-tagbig"><span style={{ fontSize: 22 }}>{HOTS[active].ic}</span><span className="sk-wordbadge">{tr3(HOTS[active].name)}</span></div>
                <p className="body" style={{ marginTop: 8 }}>{tr3(HOTS[active].body)}</p>
              </div> : <div className="hint fade-up delay-2">{tr3({ uz: "Chapdagi oynaning istalgan panelini bosing — shu yerda tushuntirish chiqadi.", ru: "Нажмите на любую панель окна слева — здесь появится объяснение." })}</div>}
            {allSeen && <div className="frame-success fade-step">
                <p className="body" style={{ margin: 0 }}>{tr3({ uz: <>Ajoyib — <b>4 panelning hammasini</b> bilib oldingiz! Endi bu oyna siz uchun begona emas.</>, ru: <>Отлично — вы узнали <b>все 4 панели</b>! Теперь это окно для вас не чужое.</> })}</p>
              </div>}
          </Col>
        </Split>
      </div>
...
var ScreenTest1 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr3({ uz: "Mashq · 1-savol", ru: "Упражнение · вопрос 1" })}
  audioText="Loyihangizdagi fayllar ro'yxatini VS Code'ning qaysi paneli ko'rsatadi? To'g'ri variantni tanlang."
  questionText="Loyihadagi fayllar ro'yxatini VS Code'ning qaysi paneli ko'rsatadi?"
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "Oynani eslang", ru: "Вспомните окно" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr3({ uz: "Loyihadagi fayllar ro'yxatini VS Code'ning qaysi paneli ko'rsatadi?", ru: "Какая панель VS Code показывает список файлов проекта?" })}</h2></>}
  options={[
    { uz: "Terminal — pastdagi buyruq paneli", ru: "Terminal — панель команд внизу" },
  ...
  correctIdx={1}
  explainCorrect={tr3({ uz: "To'g'ri. Explorer — chapdagi panel: loyihangizdagi barcha fayllar shu yerda. Faylni bossangiz — Editor panelida ochiladi.", ru: "Верно. Explorer — панель слева: там все файлы проекта. Нажмёте на файл — он откроется в панели Editor." })}
  explainWrong={{
    0: tr3({ uz: "Terminal — kompyuterga yozma buyruq beriladigan panel. Fayllar ro'yxatini Explorer ko'rsatadi.", ru: "Terminal — панель для письменных команд компьютеру. Список файлов показывает Explorer." }),
    2: tr3({ uz: "Extensions — qo'shimcha dasturlar paneli: Live Server ham shu yerdan o'rnatiladi. Fayllar ro'yxati — Explorer'da.", ru: "Extensions — панель дополнений: отсюда ставится и Live Server. Список файлов — в Explorer." }),
    3: tr3({ uz: "Status bar — pastki ko'k chiziq, u yerda Go Live tugmasi turadi. Fayllar ro'yxati — Explorer'da.", ru: "Status bar — нижняя синяя полоса, там живёт кнопка Go Live. Список файлов — в Explorer." }),
    default: tr3({ uz: "Fayllar ro'yxatini chapdagi Explorer paneli ko'rsatadi.", ru: "Список файлов показывает панель Explorer слева." })
  }}
/>;
var ScreenFolder = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const _gate = useContext2(LiveGateCtx) || {};
  const _isMentorLive = !!(_gate.live && _gate.live.mode === "mentor");
  ...
  const STEPS = [
    { id: "folder", t: { uz: "Desktopda yangi papka yarating: `my-site`", ru: "Создайте на рабочем столе новую папку: `my-site`" }, d: { uz: "Papka nomi — inglizcha, bo'sh joysiz (dasturchilar shunday yozadi)", ru: "Имя папки — английское, без пробелов (разработчики пишут так)" } },
  ...
  return <Stage eyebrow={tr3({ uz: "3-qadam · Loyiha", ru: "3-й шаг · Проект" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !(storedAnswer && storedAnswer.correct)} label={_isMentorLive || storedAnswer && storedAnswer.correct ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Qadamlarni belgilang", ru: "Отметьте шаги" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Birinchi <span className="italic" style={{ color: T.accent }}>loyihangizni</span> ochamiz.</>, ru: <>Открываем ваш первый <span className="italic" style={{ color: T.accent }}>проект</span>.</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Dasturchilar qoidasi: <b style={{ color: T.ink }}>har loyiha — alohida papka</b>. Papkani VS Code'da ochsangiz, u chapda — Explorer'da ko'rinadi. Ichiga ikkita fayl yaratamiz: <b style={{ color: T.ink }}>index.html</b> (sahifa) va <b style={{ color: T.ink }}>style.css</b> (bezaklar).</>, ru: <>Правило разработчиков: <b style={{ color: T.ink }}>каждый проект — отдельная папка</b>. Откроете папку в VS Code — она появится слева, в Explorer. Внутри создадим два файла: <b style={{ color: T.ink }}>index.html</b> (страница) и <b style={{ color: T.ink }}>style.css</b> (оформление).</> })}</Mentor>
        <Split>
          <Col>
            <p className="flow-label fade-up delay-1">{tr3({ uz: "Natijada Explorer shunday ko'rinadi", ru: "В итоге Explorer будет выглядеть так" })}</p>
    ...
    main={<>
                  <div className="vsc-tabs"><span className="vsc-tab on"><span style={{ color: "#E37933" }}>{"<>"}</span> index.html</span></div>
                  <div className="vsc-code">
                    <VL n={1}><span className="vsc-caret" /><span style={{ color: VSC.dim, fontStyle: "italic", marginLeft: 8 }}>{tr3({ uz: "bo'sh — keyingi qadamda to'ldiramiz", ru: "пусто — заполним на следующем шаге" })}</span></VL>
                  </div>
                </>}
    statusRight={<>⚡ Go Live</>}
  />
            </div>
            <div className="hl-note fade-up delay-2"><p className="body" style={{ margin: 0 }}>{fmtCode(tr3({ uz: "💡 `index.html` — loyihaning bosh sahifasi. Brauzer papkani ochganda birinchi bo'lib shu faylni qidiradi. Shuning uchun nomi doim shunday bo'ladi.", ru: "💡 `index.html` — главная страница проекта. Открывая папку, браузер первым делом ищет именно этот файл. Поэтому имя всегда такое." }))}</p></div>
          </Col>
          <Col>
            <StepChecklist screen={screen} storedAnswer={storedAnswer} onAnswer={onAnswer} steps={STEPS} taskLabel={tr3({ uz: "Papka va fayllar", ru: "Папка и файлы" })} />
          </Col>
        </Split>
      </div>
    </Stage>;
...
var ScreenEmmet = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const _gate = useContext2(LiveGateCtx) || {};
  const _isMentorLive = !!(_gate.live && _gate.live.mode === "mentor");
  const audio = useAudio([{ id: "s6", text: "Endi natijani ko'ring: bo'sh index.html'da undov belgisini yozib, Tab tugmasini bossangiz — VS Code butun HTML shablonni o'zi yozib beradi. Bu yordamchining nomi Emmet. Avval natijani ko'ring, keyin shablon bo'laklarini o'zingiz to'g'ri tartibda joylang.", trigger: "on_mount", waits_for: null }]);
  const [phase, setPhase] = useState4(storedAnswer ? 2 : 0);
  const [solved, setSolved] = useState4(!!(storedAnswer && storedAnswer.correct));
  const timerRef = useRef4(null);
  useEffect5(() => () => clearTimeout(timerRef.current), []);
  const play = () => {
    setPhase(1);
  ...
  return <Stage eyebrow={tr3({ uz: "3-qadam · Shablon", ru: "3-й шаг · Шаблон" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !solved} label={_isMentorLive || solved ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Shablonni yig'ing", ru: "Соберите шаблон" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <><span className="mono" style={{ color: T.accent }}>!</span> + Tab bosilsa, <span className="italic" style={{ color: T.accent }}>butun shablon</span> o'zi yoziladimi?</>, ru: <><span className="mono" style={{ color: T.accent }}>!</span> + Tab — и <span className="italic" style={{ color: T.accent }}>весь шаблон</span> напишется сам?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Shablonni har safar qo'lda yozish shart emas. VS Code ichida <b style={{ color: T.ink }}>Emmet</b> degan yordamchi bor: bo'sh index.html'da <b className="mono" style={{ color: T.ink }}>!</b> yozib <b style={{ color: T.ink }}>Tab</b> bossangiz — butun HTML shablon o'zi yoziladi. Avval natijani ko'ring, keyin bo'laklarni o'zingiz tartibga soling — shablonda nima borligini bilishingiz kerak.</>, ru: <>Не обязательно каждый раз писать шаблон вручную. Внутри VS Code живёт помощник <b style={{ color: T.ink }}>Emmet</b>: напишите в пустом index.html <b className="mono" style={{ color: T.ink }}>!</b> и нажмите <b style={{ color: T.ink }}>Tab</b> — весь HTML-шаблон напишется сам. Сначала посмотрите результат, потом расставьте части сами — вы должны знать, что внутри шаблона.</> })}</Mentor>
        <Split>
          <Col>
            <div className="fade-up delay-1">
              <VscShell
    ...
    main={<>
                  ...
                  <div className="vsc-code" key={phase}>
                    ...
                    {phase === 2 && TPL.map(([n, ind, jsx], i) => <div key={n} className="fade-step" style={{ animationDelay: `${i * 0.06}s` }}><VL n={n} ind={ind}>{jsx}</VL></div>)}
                  </div>
                </>}
    statusRight={<>⚡ Go Live</>}
  />
            ...
            <div className="br-controls fade-up delay-2">
              {phase === 0 && <button className="btn" onClick={play}>✨ {tr3({ uz: "Natijani ko'rish", ru: "Посмотреть результат" })}</button>}
              {phase === 2 && <button className="btn-ghost" onClick={() => {
    setPhase(0);
  }}>↻ {tr3({ uz: "Yana bir bor ko'rish", ru: "Посмотреть ещё раз" })}</button>}
              {phase === 2 && <span className="mono small" style={{ color: T.success, fontWeight: 700 }}>✓ {tr3({ uz: "10 qator — 1 soniyada!", ru: "10 строк — за 1 секунду!" })}</span>}
            </div>
            {
    /* F-0810-06: `Ctrl+Z` ilgari faqat `s12` (Bonus) ekranida bir marta
       uchrardi, flashcard esa undan javob so'rardi. Eslatma aynan shu yerda:
       bir bosishda 10 qator paydo bo'lgan payt — bola «orqaga qaytarish»ni
       birinchi marta shu yerda qidiradi. */
  }
            {phase === 2 && <p className="yz-hint fade-up delay-2" style={{ marginTop: 6 }}>{fmtCode(tr3({
    uz: "Noto'g'ri chiqdimi? `Ctrl+Z` bosing — hammasi orqaga qaytadi.",
    ru: "Получилось не то? Нажмите `Ctrl+Z` — всё вернётся назад."
  }))}</p>}
          </Col>
          <Col>
            {phase < 2 ? <div className="hint fade-up delay-2">{tr3({ uz: "Avval chapdagi «Natijani ko'rish» tugmasini bosing — shablon ochilgach, bu yerda mashq chiqadi.", ru: "Сначала нажмите «Посмотреть результат» слева — когда шаблон раскроется, здесь появится упражнение." })}</div> : <div className="sk-buildbox">
                <p className="flow-label" style={{ marginBottom: 8 }}>{tr3({ uz: "Endi o'zingiz: shablon bo'laklarini TO'G'RI tartibda joylang", ru: "Теперь сами: расставьте части шаблона в ВЕРНОМ порядке" })}</p>
                ...
                {solved && <div className="frame-success fade-step" style={{ marginTop: 10 }}><p className="body" style={{ margin: 0 }}>{tr3({ uz: <>Esladingiz! Emmet ham aynan shu tartibda yozadi: kod <b>body</b> ichiga — kursor turgan joyga yoziladi.</>, ru: <>Вспомнили! Emmet пишет именно в этом порядке: код пишется внутрь <b>body</b> — туда, где стоит курсор.</> })}</p></div>}
              </div>}
          </Col>
        </Split>
      </div>
...
var ScreenLive = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const _gate = useContext2(LiveGateCtx) || {};
  const _isMentorLive = !!(_gate.live && _gate.live.mode === "mentor");
  const audio = useAudio([{ id: "s7", text: "Endi eng muhim qadam. Extensions panelidan Live Server'ni o'rnatasiz. Keyin pastdagi Go Live tugmasini bossangiz, sahifangiz brauzerda 127 nol nol bir manzilda ochiladi. Bu — o'z kompyuteringizning manzili, sahifani hozircha faqat siz ko'rasiz. Uch ko'rinishni ko'rib chiqib, qadamlarni kompyuteringizda bajaring.", trigger: "on_mount", waits_for: null }]);
  const nm = CARD_NAME;
  const [view, setView] = useState4(0);
  const [viewed, setViewed] = useState4(() => /* @__PURE__ */ new Set([0]));
  const go = (v) => {
    setView(v);
    setViewed((prev) => {
  ...
  const VIEWS = [
    { chip: { uz: "1 · Extensions", ru: "1 · Extensions" } },
  ...
  return <Stage eyebrow={tr3({ uz: "4-qadam · Jonli server", ru: "4-й шаг · Живой сервер" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !(storedAnswer && storedAnswer.correct)} label={_isMentorLive || storedAnswer && storedAnswer.correct ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Qadamlarni belgilang", ru: "Отметьте шаги" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <><span className="italic" style={{ color: T.accent }}>Go Live</span> bilan sahifani brauzerda ochamiz.</>, ru: <>Открываем страницу в браузере через <span className="italic" style={{ color: T.accent }}>Go Live</span>.</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Endi eng muhim qadam. <b style={{ color: T.ink }}>Live Server</b> — sahifani <b style={{ color: T.ink }}>jonli serverda</b> ochib beradigan qo'shimcha dastur: kodni saqlashingiz bilan brauzer o'zi yangilanadi. Uch ko'rinishni ko'rib chiqing, keyin kompyuteringizda bajaring.</>, ru: <>Теперь самый важный шаг. <b style={{ color: T.ink }}>Live Server</b> — дополнение, открывающее страницу на <b style={{ color: T.ink }}>живом сервере</b>: сохраните код — и браузер обновится сам. Просмотрите три вида, затем выполните на своём компьютере.</> })}</Mentor>
        <Split>
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {VIEWS.map((v, i) => <button key={i} className={`chip ${view === i ? "chip-on" : ""}`} onClick={() => go(i)}>{viewed.has(i) && i !== view ? "✓ " : ""}{tr3(v.chip)}</button>)}
            </div>
            <div className="demo-swap" key={view}>
              {view === 0 && <VscShell
    act={3}
    side={<>
                    ...
                    <div className="vsc-extrow"><span className="vsc-extic">🎨</span><span className="vsc-extinfo"><b>Prettier</b><i>{tr3({ uz: "keyinroq kerak", ru: "пригодится позже" })}</i></span></div>
                  </>}
    main={<div className="vsc-code" style={{ paddingTop: 20 }}><p className="vsc-note">{tr3({ uz: "← Qidiruvdan toping va Install tugmasini bosing", ru: "← Найдите в поиске и нажмите Install" })}</p></div>}
    statusRight={<>…</>}
  />}
              {view === 1 && <VscShell
    title="my-site — Visual Studio Code"
    ...
    main={<>
                    ...
                    <div className="vsc-code">
                      ...
                      <VL n={8} ind={2}><Vt>&lt;h1&gt;</Vt><span style={{ color: VSC.text }}>{tr3({ uz: `Salom, men — ${nm}!`, ru: `Привет, я — ${nm}!` })}</span><Vt>&lt;/h1&gt;</Vt></VL>
    ...
    statusPulse
  />}
              {view === 2 && <BrowserWin url="127.0.0.1:5500/index.html" tab={tr3({ uz: "Mening sahifam", ru: "Моя страница" })} minH={170}>
                  <h1 className="pv-h1" style={{ padding: "18px 20px" }}>{tr3({ uz: `Salom, men — ${nm}!`, ru: `Привет, я — ${nm}!` })}</h1>
                </BrowserWin>}
            </div>
            {view === 1 && <p className="mono small fade-step" style={{ color: T.accent, fontWeight: 700, textAlign: "right" }}>↑ {tr3({ uz: "mana shu tugma — pastki ko'k chiziqda", ru: "вот эта кнопка — на нижней синей полосе" })}</p>}
            {view === 2 && <div className="frame-soft fade-step">
                <p className="body" style={{ margin: 0 }}>{fmtCode(tr3({ uz: "🏠 `127.0.0.1` — «mening kompyuterim» degan manzil. Internet emas, o'zingizniki. `5500` — eshik raqami (port): sahifa aynan shu eshikdan chiqadi. Demak bu sahifani hozircha faqat SIZ ko'rasiz.", ru: "🏠 `127.0.0.1` — адрес «мой компьютер». Это не интернет, а ваш собственный компьютер. `5500` — номер двери (порт): страница выходит именно через неё. Значит, эту страницу пока видите только ВЫ." }))}</p>
              </div>}
          </Col>
          <Col>
            <StepChecklist screen={screen} storedAnswer={storedAnswer} onAnswer={onAnswer} steps={STEPS} taskLabel={tr3({ uz: "Live Server + Go Live", ru: "Live Server + Go Live" })} />
          </Col>
        </Split>
      </div>
    </Stage>;
};
var ScreenTest2 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr3({ uz: "Mashq · 2-savol", ru: "Упражнение · вопрос 2" })}
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "Jonli server", ru: "Живой сервер" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr3({ uz: "Pastdagi «Go Live» tugmasi bosilganda nima bo’ladi?", ru: "Что происходит, когда вы нажимаете «Go Live» внизу?" })}</h2></>}
  options={[
  ...
  correctIdx={2}
  explainCorrect={tr3({ uz: "To'g'ri. Go Live jonli serverni yoqadi: sahifa brauzerda 127.0.0.1:5500 manzilida ochiladi. Har saqlashda esa o'zi yangilanadi.", ru: "Верно. Go Live включает живой сервер: страница открывается в браузере на 127.0.0.1:5500. А при каждом сохранении обновляется сама." })}
  explainWrong={{
    0: tr3({ uz: "Yo'q — 127.0.0.1 faqat o'z kompyuteringiz. Internetga chiqarish alohida qadam, unga hali yetamiz.", ru: "Нет — 127.0.0.1 это только ваш компьютер. Публикация в интернете — отдельный шаг, до него ещё дойдём." }),
    1: tr3({ uz: "Xatolarni tekshirish — boshqa vositalar ishi. Go Live sahifani jonli serverda ochadi.", ru: "Проверка ошибок — работа других инструментов. Go Live открывает страницу на живом сервере." }),
    3: tr3({ uz: "Saqlash — Ctrl+S ishi. Go Live esa sahifani brauzerda jonli ochadi.", ru: "Сохранение — работа Ctrl+S. А Go Live живо открывает страницу в браузере." }),
    default: tr3({ uz: "Go Live sahifani 127.0.0.1:5500 manzilida jonli ochadi.", ru: "Go Live живо открывает страницу на адресе 127.0.0.1:5500." })
  }}
/>;
var ScreenCardHtml = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const _gate = useContext2(LiveGateCtx) || {};
  const _isMentorLive = !!(_gate.live && _gate.live.mode === "mentor");
  const audio = useAudio([{ id: "s9", text: "Endi cardni bo'laklardan yig'amiz. Cardda uchta bo'lak bor: logo, ism va orzu-kasb. Hammasi div class card qutisining ichida. Style css esa link tegi bilan ulanadi. Uch bo'lakni bosib o'rganing, keyin praktikada o'zingiz yig'asiz.", trigger: "on_mount", waits_for: null }]);
  const nm = CARD_NAME;
  const PARTS = {
    link: {
      name: { uz: "<link> — ko'prik", ru: "<link> — мост" },
      ic: "🔗",
    ...
    card: {
      name: { uz: '<div class="card"> — quti', ru: '<div class="card"> — коробка' },
      ic: "📦",
    ...
    inner: {
      name: { uz: "Ichki 3 bo'lak", ru: "3 части внутри" },
      ic: "🧱",
      body: { uz: <>Quti ichida: <b className="mono">div class="logo"</b> — oq logo-maydon, <b className="mono">h2</b> — ismingiz, <b className="mono">p</b> — kim bo'lmoqchi ekaningiz. Tartib muhim: logo tepada, ism o'rtada, orzu-kasb pastda.</>, ru: <>Внутри коробки: <b className="mono">div class="logo"</b> — белое поле логотипа, <b className="mono">h2</b> — ваше имя, <b className="mono">p</b> — кем вы хотите стать. Порядок важен: логотип сверху, имя в середине, профессия мечты внизу.</> }
    }
  };
  const [active, setActive] = useState4(null);
  const [seen, setSeen] = useState4(() => new Set(storedAnswer?.seenIds || (storedAnswer?.correct ? Object.keys(PARTS) : [])));
  ...
  return <Stage eyebrow={tr3({ uz: "5-qadam · Card HTML", ru: "5-й шаг · HTML карточки" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !allSeen} label={_isMentorLive || allSeen ? tr3({ uz: "Praktikaga →", ru: "К практике →" }) : tr3({ uz: "3 bo'lakni bosib o'rganing", ru: "Изучите 3 части" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Card <span className="italic" style={{ color: T.accent }}>nechta qismdan</span> iborat?</>, ru: <>Из <span className="italic" style={{ color: T.accent }}>скольких частей</span> состоит card?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Har bir web-sahifa kichik bo'laklardan yig'iladi. Keling, bu card qanday qismlardan tuzilganini ko'ramiz: kodda <b style={{ color: T.ink }}>belgilangan bo'laklarni</b> bosib chiqing — keyin praktikada o'zingiz yig'asiz.</>, ru: <>Каждая веб-страница собирается из маленьких частей. Давайте посмотрим, из каких частей состоит эта card: нажмите на <b style={{ color: T.ink }}>выделенные части</b> в коде — потом соберёте сами в практике.</> })}</Mentor>
        <p className="sk-tapguide fade-up delay-1">👆 {tr3({ uz: <>Koddagi yorqin bo'laklarni <b>bosing</b></>, ru: <>Нажимайте на яркие части кода</> })}<span className="sk-tapcount">{seen.size}/3</span></p>
        <Split>
          <Col>
            <div className="code-box fade-up delay-1" style={{ fontSize: "clamp(12px,1.5vw,13.5px)" }}>
              ...
              <div style={{ display: "inline" }}>
                <span className={`ck cd-hot ${seen.has("inner") ? "cd-seen" : ""} ${active === "inner" ? "active" : ""}`} onClick={() => tap("inner")}>
                  ...
                  {"    "}<Tg>&lt;p&gt;</Tg>Frontend developer<Tg>&lt;/p&gt;</Tg>
                </span>
              </div>
              <div>{"  "}<Tg>&lt;/div&gt;</Tg></div>
              <div><Tg>&lt;/body&gt;</Tg></div>
            ...
            {active && <div className="sk-info fade-step" key={active}>
                <div className="sk-tagbig"><span style={{ fontSize: 20 }}>{PARTS[active].ic}</span><span className="sk-wordbadge">{tr3(PARTS[active].name)}</span></div>
                <p className="body" style={{ marginTop: 8 }}>{tr3(PARTS[active].body)}</p>
              </div>}
          </Col>
          <Col>
            <p className="flow-label fade-up delay-2">{tr3({ uz: "Hozircha shunday ko'rinadi (style.css bo'sh)", ru: "Пока выглядит так (style.css пуст)" })}</p>
            <BrowserWin url="127.0.0.1:5500/index.html" minH={190}>
              <VCard name={nm} prof={CARD_PROF} plain />
            </BrowserWin>
            <div className="hl-note fade-up delay-3"><p className="body" style={{ margin: 0 }}>{tr3({ uz: <>Oddiy va rangsiz — chunki <b className="mono">style.css</b> hali bo'sh. Keyingi qadamda uni to'ldiramiz. 😉</>, ru: <>Просто и бесцветно — потому что <b className="mono">style.css</b> пока пуст. На следующем шаге мы его заполним. 😉</> })}</p></div>
          </Col>
        </Split>
      </div>
    </Stage>;
};
var ScreenCardCss = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const _gate = useContext2(LiveGateCtx) || {};
  const _isMentorLive = !!(_gate.live && _gate.live.mode === "mentor");
  const audio = useAudio([{ id: "s10", text: "Endi eng qiziq joyi — cardni bezaymiz. To'rtta bezakni birma-bir yoqib ko'ring: yashil fon, markazlash, yumaloq burchak va soya. Har birini yoqqaningizda kod ham, natija ham o'zgaradi. Keyin praktikada o'zingiz yozasiz.", trigger: "on_mount", waits_for: null }]);
  const nm = CARD_NAME;
  const STEPS = [
  ...
  const [on, setOn] = useState4(() => new Set(storedAnswer?.onIds || (storedAnswer?.correct ? STEPS.map((s) => s.id) : [])));
  ...
  return <Stage eyebrow={tr3({ uz: "6-qadam · Card CSS", ru: "6-й шаг · CSS карточки" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !allOn} label={_isMentorLive || allOn ? tr3({ uz: "Praktikaga →", ru: "К практике →" }) : tr3({ uz: "4 bezakni yoqing", ru: "Включите 4 оформления" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Cardni qanday <span className="italic" style={{ color: T.accent }}>chiroyli</span> qilamiz?</>, ru: <>Как сделать card <span className="italic" style={{ color: T.accent }}>красивой</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Hozir card ishlayapti, lekin oddiy ko'rinadi. <b style={{ color: T.ink }}>CSS</b> uni bezaydi. To'rt bezakni <b style={{ color: T.ink }}>birma-bir yoqib</b> ko'ring: har bosishda chapda CSS qatori qo'shiladi, o'ngda card o'zgaradi.</>, ru: <>Сейчас card работает, но выглядит просто. Оформляет её <b style={{ color: T.ink }}>CSS</b>. Включите четыре оформления <b style={{ color: T.ink }}>по одному</b>: с каждым нажатием слева добавится строка CSS, а справа изменится card.</> })}</Mentor>
        <Split>
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {STEPS.map((s) => <button key={s.id} className={`chip ${on.has(s.id) ? "chip-on" : ""}`} onClick={() => toggle(s.id)}>{on.has(s.id) ? "✓ " : ""}{tr3(s.chip)}</button>)}
            </div>
            <div className="code-box fade-up delay-1" style={{ fontSize: "clamp(12.5px,1.6vw,14px)" }}>
              <div><span style={{ color: "#D7BA7D" }}>.card</span> {"{"}</div>
              {STEPS.filter((s) => on.has(s.id)).map((s) => <div key={s.id} className="fade-step">{"  "}<span style={{ color: CODE.attr }}>{s.css.split(":")[0]}</span>:<span style={{ color: CODE.str }}>{s.css.split(":").slice(1).join(":").replace(";", "")};</span> <span className="cm">{"/* "}{tr3(s.d)}{" */"}</span></div>)}
              {on.size === 0 && <div className="cm">{"  /* "}{tr3({ uz: "bezaklarni tepadan yoqing…", ru: "включайте оформления сверху…" })}{" */"}</div>}
              <div>{"}"}</div>
            </div>
            {allOn && <div className="frame-success fade-step"><p className="body" style={{ margin: 0 }}>{tr3({ uz: <>Mana bu — <b>rasmdagi card</b>! Endi praktikada shu 4 qatorni o'zingiz yozasiz, keyin VS Code'dagi style.css'ga ham ko'chirasiz.</>, ru: <>Вот она — <b>карточка с образца</b>! Теперь в практике вы напишете эти 4 строки сами, а потом перенесёте их в свой style.css в VS Code.</> })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label fade-up delay-2">{tr3({ uz: "Jonli natija", ru: "Живой результат" })}</p>
            <BrowserWin url="127.0.0.1:5500/index.html" minH={230}>
              <VCard name={nm} prof={CARD_PROF} flags={flags} />
            </BrowserWin>
          </Col>
        </Split>
      </div>
...
var ScreenTest3 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "Ikki fayl do’stligi", ru: "Дружба двух файлов" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr3({ uz: "style.css'dagi bezaklar sahifaga qaysi qator orqali ulanadi?", ru: "Через какую строку оформление из style.css подключается к странице?" })}</h2></>}
  options={[
  ...
  correctIdx={0}
  explainCorrect={tr3({ uz: "To'g'ri. `link` tegi head ichida turadi: `rel=\"stylesheet\"` — «bu bezak fayli», `href` — fayl manzili. Shu ko'prik orqali CSS sahifaga yetib boradi.", ru: 'Верно. Тег `link` стоит внутри head: `rel="stylesheet"` — «это файл оформления», `href` — адрес файла. По этому мосту CSS доходит до страницы.' })}
  explainWrong={{
    1: tr3({ uz: "`style` tegida `src` atributi ishlamaydi. CSS faylni `head` ichidagi `link` tegi ulaydi.", ru: "У тега `style` атрибут `src` не работает. CSS-файл подключает тег `link` внутри `head`." }),
    2: tr3({ uz: "`css` degan teg mavjud emas. CSS faylni `link` tegi ulaydi.", ru: "Тега `css` не существует. CSS-файл подключает тег `link`." }),
    3: tr3({ uz: "`script` — JavaScript uchun. CSS faylni `head` ichidagi `link` tegi ulaydi.", ru: "`script` — для JavaScript. CSS-файл подключает тег `link` внутри `head`." }),
    default: tr3({ uz: "CSS faylni `head` ichidagi `link` tegi ulaydi.", ru: "CSS-файл подключает тег `link` внутри `head`." })
  }}
/>;
var ScreenKeys = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const _gate = useContext2(LiveGateCtx) || {};
  const _isMentorLive = !!(_gate.live && _gate.live.mode === "mentor");
  const audio = useAudio([{ id: "s12", text: "Bu tugmalar vaqtni tejaydi. To'rttasi bor: har birini bosib, nima qilishini ko'ring.", trigger: "on_mount", waits_for: null }]);
  const KEYS = [
  ...
  const [pressed, setPressed] = useState4(() => new Set(storedAnswer?.pressedIds || (storedAnswer?.correct ? KEYS.map((k) => k.id) : [])));
  const [active, setActive] = useState4(null);
  const allPressed = pressed.size >= KEYS.length;
  const tap = (id) => {
    setActive(id);
    const n = new Set(pressed);
    n.add(id);
    setPressed(n);
    onAnswer(screen, { correct: n.size >= KEYS.length, pressedIds: [...n], picked: n.size, stage: "exploration", screenIdx: screen });
  };
  return <Stage eyebrow={tr3({ uz: "Bonus · Tezkor tugmalar", ru: "Бонус · Горячие клавиши" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !allPressed} label={_isMentorLive || allPressed ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "4 birikmani bosing", ru: "Нажмите 4 сочетания" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Dasturchining <span className="italic" style={{ color: T.accent }}>maxfiy tugmalari</span> qanday?</>, ru: <>Какие у разработчика <span className="italic" style={{ color: T.accent }}>секретные клавиши</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Bu tugmalar <b style={{ color: T.ink }}>vaqtni tejaydi</b>. Har birini <b style={{ color: T.ink }}>bosib</b>, nima qilishini ko'ring.</>, ru: <>Эти клавиши <b style={{ color: T.ink }}>экономят время</b>. <b style={{ color: T.ink }}>Нажмите</b> на каждую и посмотрите, что она делает.</> })}</Mentor>
        {
    /* 🔴 F-0810-04: ilgari bu ekran `narrow` (680px) edi va butun dars ichida
       yolg'iz o'zi qisilib turardi. Endi u s3 «tanishuv» ekrani bilan bir xil
       naqshda: chapda bosiladigan narsalar, o'ngda tushuntirish. */
  }
        <Split>
          <Col>
            <p className="sk-tapguide fade-up delay-1">👆 {tr3({ uz: <>Birikmalarni <b>bosing</b></>, ru: <>Нажимайте на сочетания</> })}<span className="sk-tapcount">{pressed.size}/4</span></p>
            <div className="kcap-grid fade-up delay-1">
              {KEYS.map((k) => <button key={k.id} className={`kcap-card ${pressed.has(k.id) ? "seen" : ""} ${active === k.id ? "on" : ""}`} onClick={() => tap(k.id)}>
                  <span className="kcap-keys">{k.keys.map((kk, i) => <React3.Fragment key={i}><span className="kcap">{kk}</span>{i < k.keys.length - 1 && <b>+</b>}</React3.Fragment>)}</span>
                  <span className="kcap-name">{pressed.has(k.id) ? "✓ " : ""}{tr3(k.name)}</span>
                </button>)}
            </div>
          </Col>
          <Col>
            {active ? <div className="sk-info fade-step" key={active}>
                <div className="sk-tagbig"><span className="sk-wordbadge">{tr3(KEYS.find((k) => k.id === active).name)}</span></div>
                <p className="body" style={{ marginTop: 8 }}>{tr3(KEYS.find((k) => k.id === active).d)}</p>
              </div> : <div className="hint fade-up delay-2">{tr3({ uz: "Chapdagi istalgan birikmani bosing — shu yerda tushuntirish chiqadi.", ru: "Нажмите на любое сочетание слева — здесь появится объяснение." })}</div>}
            {allPressed && <div className="frame-success fade-step">
...
var ScreenFinalTest = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Yakuniy sinov", ru: "Финальная проверка" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={isMentorLive ? false : !correct} label={isMentorLive ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : correct ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Tugma nomini yozing", ru: "Напишите имя кнопки" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>yozib</span> ko'ring.</>, ru: <>Теперь напишите <span className="italic" style={{ color: T.accent }}>сами</span>.</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Yakuniy savol: sahifani brauzerda ochish uchun <b style={{ color: T.ink }}>qaysi tugmani</b> bosamiz? Javob <b style={{ color: T.ink }}>ikki so'zdan</b> iborat.</>, ru: <>Финальный вопрос: <b style={{ color: T.ink }}>какую кнопку</b> мы нажимаем, чтобы открыть страницу в браузере? Ответ — <b style={{ color: T.ink }}>два слова</b>.</> })}</Mentor>
        <div className="ft-statwrap fade-up delay-1">
          ...
          {!correct ? <div className="ft-inputrow">
              <input className="text-input" style={{ maxWidth: 260 }} value={val} onChange={(e) => setVal(e.target.value)} placeholder={tr3({ uz: "tugma nomi…", ru: "имя кнопки…" })} spellCheck={false} disabled={isMentorLive} />
              <p className="yz-hint">{touched ? tr3({ uz: "Deyarli! Ikki inglizcha so'z: «jonli holatga o't» degan ma'noda.", ru: "Почти! Два английских слова — в смысле «переходи в живой режим»." }) : tr3({ uz: "Maslahat: birinchi so'zi — «Go»", ru: "Подсказка: первое слово — «Go»" })}</p>
            </div> : <div className="fade-step">
              <BrowserWin url="127.0.0.1:5500/index.html" minH={90}>
                <p className="yz-ok" style={{ padding: "14px 18px", margin: 0 }}>✓ {tr3({ uz: "To'g'ri — Go Live! Sahifa jonli serverda ochildi.", ru: "Верно — Go Live! Страница открылась на живом сервере." })}</p>
              </BrowserWin>
            </div>}
        </div>
        {isMentorLive && <MentorWorkStats live={live} screenIdx={screen} taskLabel={tr3({ uz: "«Go Live» yozish (yakuniy)", ru: "Написать «Go Live» (финал)" })} />}
      </div>
    </Stage>;
};
var ScreenFinale = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const audio = useAudio([{ id: "s14", text: "Qarang — bugun bosib o'tgan yo'lingiz: VS Code o'rnatildi, loyiha ochildi, jonli server ishladi va o'z cardingiz tayyor bo'ldi. Cardingizni brauzerda ochib, rangini o'zgartirib ko'ring.", trigger: "on_mount", waits_for: null }]);
  const gate = useContext2(LiveGateCtx) || {};
  const live = gate.live;
  const isMentorLive = !!(live && live.mode === "mentor");
  const nm = CARD_NAME;
  ...
  useEffect5(() => {
    ...
    sentRef.current = true;
    onAnswer(screen, { correct: true, picked: "reached", stage: "case", screenIdx: screen });
    if (live && live.mode === "student") live.submitAnswer(screen, SCREEN_META[screen]?.id || `s${screen}`, 0, true, 0);
  }, []);
  const DONE = [
    ...
    { uz: "O'z ismingiz yozilgan yashil card", ru: "Зелёная card с вашим именем" }
  ];
  return <Stage eyebrow={tr3({ uz: "Marra", ru: "Финиш" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive label={tr3({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen">
        <Confetti />
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Birinchi <span className="italic" style={{ color: T.accent }}>web-cardingiz</span> tayyor.</>, ru: <>Ваша первая <span className="italic" style={{ color: T.accent }}>web-card</span> готова.</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Tabriklaymiz! Endi cardingiz brauzerda ishlayapti. Rangini o'zgartirib ko'ring va <b className="mono" style={{ color: T.ink }}>Ctrl+S</b> bosganda natija qanday yangilanishini kuzating.</>, ru: <>Поздравляем! Теперь ваша card работает в браузере. Попробуйте поменять её цвет и посмотрите, как обновится результат после <b className="mono" style={{ color: T.ink }}>Ctrl+S</b>.</> })}</Mentor>
        <Split>
          <Col>
            <ul className="recipe-list fade-up delay-1">
              {DONE.map((d, i) => <li key={i} className="on"><span className="recipe-num">✓</span><span className="recipe-text">{fmtCode(tr3(d))}</span></li>)}
            </ul>
            {!isMentorLive && <div className="frame-success fade-step"><p className="body" style={{ margin: 0 }}>{tr3({ uz: <>Bugundan boshlab kodni <b>o'z kompyuteringizga</b> yozasiz. Endi har darsda shu qurollar bilan ishlaymiz.</>, ru: <>С сегодняшнего дня вы пишете код <b>на своём компьютере</b>. Теперь на каждом уроке работаем этими инструментами.</> })}</p></div>}
            {isMentorLive && <MentorWorkStats live={live} screenIdx={screen} taskLabel={tr3({ uz: "Yakuniy ekranga yetdi", ru: "Дошёл до финального экрана" })} />}
          </Col>
          <Col>
            <p className="flow-label fade-up delay-2">{tr3({ uz: "Sizning natijangiz", ru: "Ваш результат" })}</p>
            <BrowserWin url="127.0.0.1:5500/index.html" tab={tr3({ uz: "Mening cardim", ru: "Моя карточка" })} minH={230}>
              <VCard name={nm} prof={CARD_PROF} anim />
            </BrowserWin>
          </Col>
        </Split>
      </div>
...
var ScreenSummary = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  const audio = useAudio([{ id: "s16", text: "Dars tugadi. Endi kompyuteringizda hammasi bor: VS Code, jonli server va o'z cardingiz. Asosiyni eslab qoling: har loyiha — alohida papka, Go Live — jonli ko'rish, Ctrl+S — saqlash.", trigger: "on_mount", waits_for: null }]);
  const RECAP = [
    tr3({ uz: "VS Code'ni o'rnatish va sozlash", ru: "Устанавливать и настраивать VS Code" }),
    tr3({ uz: "Loyiha papkasi va fayllar yaratish", ru: "Создавать папку проекта и файлы" }),
    tr3({ uz: "! + Tab bilan tayyor shablon olish", ru: "Получать готовый шаблон через ! + Tab" }),
    tr3({ uz: "Live Server bilan sahifani jonli ko'rish", ru: "Живо смотреть страницу через Live Server" }),
    tr3({ uz: "Card yasab, CSS bilan bezash", ru: "Делать card и оформлять её CSS" })
  ];
  const HOMEWORK2 = [
    { b: tr3({ uz: "Hobbi ro'yxati", ru: "Список хобби" }), t: tr3({ uz: "— card ichiga `ul`/`li` bilan 3 ta hobbi", ru: "— 3 хобби внутри карточки через `ul`/`li`" }) },
    { b: tr3({ uz: "Havola", ru: "Ссылка" }), t: tr3({ uz: "— Telegram yoki sevimli saytingizga `a href`", ru: "— `a href` на Telegram или любимый сайт" }) },
    { b: tr3({ uz: "Rang tajribasi", ru: "Эксперимент с цветом" }), t: tr3({ uz: "— fonni boshqa rangga o'zgartirib ko'ring", ru: "— попробуйте другой цвет фона" }) },
    { b: tr3({ uz: "Sahifa nomi", ru: "Имя страницы" }), t: tr3({ uz: "— `title` ichiga ismingizni yozing", ru: "— напишите своё имя в `title`" }) }
  ];
  ...
  return <Stage eyebrow={tr3({ uz: "Tayyor", ru: "Готово" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Keyingi dars →", ru: "Следующий урок →" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr3({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr3({ uz: <>Endi sizda <span className="italic" style={{ color: T.accent }}>haqiqiy dasturchi quroli</span> bor.</>, ru: <>Теперь у вас есть <span className="italic" style={{ color: T.accent }}>настоящий инструмент разработчика</span>.</> })}</h2>{
...
var ScreenDemoIdea = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const gate = useContext2(LiveGateCtx) || {};
  const live = gate.live;
  const isMentorLive = !!(live && live.mode === "mentor");
  const saved = demoRead();
  const hasSaved = !!(saved && saved.muammo);
  const [picking, setPicking] = useState4(!hasSaved);
  const [sel, setSel] = useState4(hasSaved ? saved.manba : null);
  const [ownM, setOwnM] = useState4(hasSaved && saved.manba === "ozim" ? saved.muammo : "");
  const [ownY, setOwnY] = useState4(hasSaved && saved.manba === "ozim" ? saved.yechim : "");
  const [confirmed, setConfirmed] = useState4(!!(storedAnswer && storedAnswer.correct));
  ...
  const confirm = () => {
    ...
    if (picking) {
      if (sel === "ozim") demoWrite({ muammo: ownM.trim(), yechim: ownY.trim(), manba: "ozim", holat: "tasdiqlandi" });
        ...
        const b = IDEA_BANK.find((x) => x.id === sel);
        demoWrite({ muammo: tr3(b.muammo), yechim: tr3(b.yechim), manba: b.id, holat: "tasdiqlandi" });
      }
  ...
  const cur = demoRead();
  return <Stage eyebrow={tr3({ uz: "Demo Day loyihasi", ru: "Проект Demo Day" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!confirmed && !isMentorLive} label={confirmed || isMentorLive ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Loyihani tasdiqlang", ru: "Подтвердите проект" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Demo Day uchun <span className="italic" style={{ color: T.accent }}>g'oyangiz</span> tayyormi?</>, ru: <>Готова ли ваша <span className="italic" style={{ color: T.accent }}>идея</span> для Demo Day?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Demo Day'da siz loyihangiz haqida gapirasiz va uni ko'rsatasiz. Hozirdan o'ylab ko'ring: <b style={{ color: T.ink }}>saytingiz kimga yordam beradi</b> va <b style={{ color: T.ink }}>qanday muammoni hal qiladi</b>?</>, ru: <>На Demo Day вы расскажете о своём проекте и покажете его. Подумайте уже сейчас: <b style={{ color: T.ink }}>кому помогает ваш сайт</b> и <b style={{ color: T.ink }}>какую задачу он решает</b>?</> })}</Mentor>
        {isMentorLive ? <MentorWorkStats live={live} screenIdx={screen} taskLabel={tr3({ uz: "Demo Day loyihasini aytish", ru: "Назвать проект Demo Day" })} /> : <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 8, maxWidth: 680 }}>
          {!picking && hasSaved && <>
              <div className="frame-success"><p className="body" style={{ margin: 0, color: T.ink }}>💡 <b>{cur ? cur.muammo : ""}</b> → {cur ? cur.yechim : ""}</p></div>
              {!confirmed && <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <button className="btn" style={{ background: T.success }} onClick={confirm}>{tr3({ uz: "✓ Shu — mening loyiham", ru: "✓ Это мой проект" })}</button>
                  <button className="chip" onClick={() => {
    setPicking(true);
    setSel(null);
  }}>{tr3({ uz: "Boshqasini tanlayman", ru: "Выберу другой" })}</button>
                </div>}
            </>}
          {picking && <>
              <p className="body" style={{ margin: 0, color: T.ink2, fontSize: 14 }}>{tr3({ uz: "Qoida bitta: tayyor brendning saytini ko'chirmaymiz — o'z atrofingizdagi muammoni tanlang.", ru: "Правило одно: не копируем сайт готового бренда — выберите проблему вокруг себя." })}</p>
              {
    /* Bank 2 ustunda — 720px oynada «Tasdiqlash» skrollsiz ko'rinsin (110-B) */
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(270px, 1fr))", gap: 8 }}>
                {IDEA_BANK.map((b) => {
    const on = sel === b.id;
    return <button key={b.id} className={"hook-option " + (on ? "on" : "")} onClick={() => setSel(b.id)}>
                      <span className="radio">{on && <span className="radio-dot" />}</span>
                      <span>{b.ic} {tr3(b.muammo)} <span style={{ color: on ? void 0 : T.ink2 }}>→ {tr3(b.yechim)}</span></span>
                    </button>;
  })}
                <button className={"hook-option " + (sel === "ozim" ? "on" : "")} onClick={() => setSel("ozim")}>
                  <span className="radio">{sel === "ozim" && <span className="radio-dot" />}</span>
                  <span>✍️ {tr3({ uz: "O'z g'oyam bor", ru: "У меня своя идея" })}</span>
                </button>
              </div>
              {sel === "ozim" && <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 8, paddingLeft: 6 }}>
                  <input className="text-input" value={ownM} onChange={(e) => setOwnM(e.target.value)} placeholder={tr3({ uz: "Qaysi muammo?", ru: "Какая проблема?" })} />
                  <input className="text-input" value={ownY} onChange={(e) => setOwnY(e.target.value)} placeholder={tr3({ uz: "Sayt nima qiladi?", ru: "Что сделает сайт?" })} />
                </div>}
              {!confirmed && <button className="btn" disabled={!ready} style={{ background: ready ? T.success : void 0, alignSelf: "flex-start", opacity: ready ? 1 : 0.5 }} onClick={confirm}>{tr3({ uz: "✓ Tasdiqlash", ru: "✓ Подтвердить" })}</button>}
            </>}
          {confirmed && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Tasdiqlandi. Deploy darsida aynan shu loyihani internetga chiqarasiz.</>, ru: <>Подтверждено. На уроке деплоя вы выложите в интернет именно этот проект.</> })}</p></div>}
        </div>}
      </div>
    </Stage>;
};
var ScreenPodium = ({ screen, answers, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr3({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Bugungi <span className="italic" style={{ color: T.accent }}>g'oliblarimiz</span>.</>, ru: <>Наши <span className="italic" style={{ color: T.accent }}>победители</span> сегодня.</> })}</h2></div>
    ...
    return <div key={q} className="qstat-row">
              ...
              {live.mode === "mentor" && <p className="small" style={{ margin: "10px 0 0", color: T.ink2 }}>{tr3({ uz: "⚠️ belgili savollar — sinf qiynalgan mavzular. Ularni yana bir bor tushuntiring.", ru: "Вопросы со значком ⚠️ — темы, где класс споткнулся. Объясните их ещё раз." })}</p>}
            </div>
          </>}
      </div>
    </Stage>;
...
var QUIZ_BANK = [
  { q: { uz: "Dunyodagi dasturchilar eng ko'p ishlatadigan kod redaktori qaysi?", ru: "Каким редактором кода пользуется больше всего разработчиков в мире?" }, opts: ["Word", "VS Code", "Photoshop", "Paint"], correct: 1 },
  ...
  { q: { uz: "Yangi qo'shimcha dastur (extension) qayerdan o'rnatiladi?", ru: "Откуда устанавливается новое дополнение (extension)?" }, opts: [{ uz: "Explorer panelidan", ru: "Из панели Explorer" }, { uz: "Terminal panelidan", ru: "Из панели Terminal" }, { uz: "Status bar'dagi tugmadan", ru: "С кнопки на status bar" }, { uz: "Extensions panelidan", ru: "Из панели Extensions" }], correct: 3 },
...
function QuizArena({ live, onClose, startSolo }) {
  ...
  const closeArena = () => {
    if (isMentor && !solo && phase !== "done") {
      if (!window.confirm(tr3({ uz: "Test hali tugamadi. Yopsangiz — o'quvchilar arenada kutib qoladi.\nKeyin «⚔️ Davom ettirish» bilan shu joydan qaytasiz.\n\nBaribir yopilsinmi?", ru: "Тест ещё не завершён. Если закрыть — ученики останутся ждать в арене.\nПотом вернётесь к этому месту через «⚔️ Продолжить».\n\nВсё равно закрыть?" }))) return;
    }
    onClose();
  };
...
var TASK_CARD = {
  ...
  title: { uz: "Card'ni bo'laklardan yig'ing", ru: "Соберите card из частей" },
  brief: {
    uz: "Cardda uchta bo'lak bor: logo, ism va orzu-kasb. Hammasi bitta qutining ichida. Avval shu yerda yig'ing — xato bo'lsa, ✓ belgilari darrov ko'rsatadi. Tayyor kodni keyin VS Code'dagi index.html'ga ko'chirasiz.",
    ru: "В карточке три части: логотип, имя и профессия мечты. Всё внутри одной коробки. Сначала соберите здесь — если есть ошибка, значки ✓ сразу покажут. Готовый код потом перенесёте в index.html в VS Code."
  },
  requirements: [
    { id: "card", label: { uz: '<div class="card"> — quti', ru: '<div class="card"> — коробка' }, check: checks.has(".card", { uz: '`<div class="card">` qutisini oching va yoping', ru: 'Откройте и закройте коробку `<div class="card">`' }) },
    { id: "logo", label: { uz: 'quti ichida <div class="logo">', ru: 'внутри коробки <div class="logo">' }, check: checks.nested(".card", ".logo", { uz: '`.card` ichiga `<div class="logo">&lt;/&gt;</div>` maydonini joylang', ru: 'Поместите внутрь `.card` поле `<div class="logo">&lt;/&gt;</div>`' }) },
    { id: "h2", label: { uz: "quti ichida <h2> — ism", ru: "внутри коробки <h2> — имя" }, check: checks.nested(".card", "h2", { uz: "`.card` ichiga `<h2>` bilan ismingizni yozing", ru: "Напишите внутри `.card` своё имя в `<h2>`" }) },
    { id: "prof", label: { uz: "quti ichida <p> — orzu-kasb", ru: "внутри коробки <p> — профессия мечты" }, check: checks.nested(".card", "p", { uz: "`.card` ichiga `<p>` bilan kim bo'lmoqchi ekaningizni yozing (masalan: Frontend developer)", ru: "Напишите внутри `.card` в `<p>`, кем хотите стать (например: Frontend developer)" }) }
  ]
};
var STARTER_CARD = {
  uz: `<!-- Bu yerga yozing -->
...
var TASK_STYLE = {
  ...
  brief: {
    uz: "index.html tayyor — unga tegmang. style.css'da `.card` uchun 4 ta bezak yozing; ✓ belgilari qaysi biri bajarilganini ko'rsatib turadi. Tayyor kodni keyin VS Code'dagi style.css'ga ko'chirasiz va Ctrl+S bosasiz.",
    ru: "index.html готов — его не трогайте. В style.css напишите 4 оформления для `.card`; значки ✓ покажут, что уже сделано. Готовый код потом перенесёте в style.css в VS Code и нажмёте Ctrl+S."
  },
  files: [
    {
      name: "index.html",
  ...
  <h2 style="text-align:center; color:#1E7BD7; margin:14px 0 4px">Steve Jobs</h2>
  <p style="text-align:center; color:#14301a; margin:0">Frontend developer</p>
</div>
`
    },
    {
      name: "style.css",
      lang: "css",
      starter: `/* Bu yerga yozing */
`
    }
  ],
  requirements: [
    { id: "bg", label: { uz: ".card — yashil fon (background)", ru: ".card — зелёный фон (background)" }, check: checks.custom((x) => {
      const rule = x.cssRules.find((r) => r.selector.split(",").map((s) => s.trim()).includes(".card"));
      if (!rule) return tr3({ uz: "style.css'da `.card { ... }` qoidasini oching", ru: "Откройте правило `.card { ... }` в style.css" });
      const v = rule.props["background"] || rule.props["background-color"];
      if (!v) return tr3({ uz: "`.card` ichiga `background` xossasini yozing", ru: "Напишите свойство `background` внутри `.card`" });
      return isGreenish(v) ? true : tr3({ uz: "Rang yashil oilasidan bo'lsin — masalan `#3FAE2A`", ru: "Возьмите цвет из зелёной семьи — например `#3FAE2A`" });
    }) },
    { id: "radius", label: { uz: ".card — yumaloq burchak (border-radius)", ru: ".card — скруглённые углы (border-radius)" }, check: checks.cssProp(".card", "border-radius", { uz: "`.card` ga `border-radius` yozing — masalan `18px`", ru: "Задайте `.card` свойство `border-radius` — например `18px`" }) },
    { id: "center", label: { uz: ".card — markazda (width + margin auto)", ru: ".card — по центру (width + margin auto)" }, check: checks.custom((x) => {
      const rule = x.cssRules.find((r) => r.selector.split(",").map((s) => s.trim()).includes(".card"));
      if (!rule) return tr3({ uz: "style.css'da `.card { ... }` qoidasini oching", ru: "Откройте правило `.card { ... }` в style.css" });
      if (!rule.props["width"]) return tr3({ uz: "Avval `width` bering — masalan `260px`", ru: "Сначала задайте `width` — например `260px`" });
      const m = [rule.props["margin"], rule.props["margin-left"], rule.props["margin-right"]].filter(Boolean).join(" ");
      return m.includes("auto") ? true : tr3({ uz: "Markazlash uchun `margin: 40px auto` yozing", ru: "Для центрирования напишите `margin: 40px auto`" });
    }) },
    { id: "shadow", label: { uz: ".card — soya (box-shadow)", ru: ".card — тень (box-shadow)" }, check: checks.cssProp(".card", "box-shadow", { uz: "`.card` ga `box-shadow` yozing — masalan `0 10px 30px rgba(0,0,0,0.18)`", ru: "Задайте `.card` свойство `box-shadow` — например `0 10px 30px rgba(0,0,0,0.18)`" }) }
  ]
};
var PRACTICE_AFTER = {
  // 🔴 F-0810-03: 2 ta praktika-compiler (Card HTML + Card CSS). Uchinchisi — `s7`
  // dagi «h1 + ism» — olib tashlandi: u VS Code'da qilingan ishning aynan takrori edi.
  9: { task: TASK_CARD, starter: STARTER_CARD },
  // 1) Card HTML tuzilishi
  10: { task: TASK_STYLE, starter: null }
  // 2) Card CSS (2 fayl — files ichida)
};
var ACHIEVEMENTS = {
  ...
  cardmaster: { icon: "🪪", name: "Card Wizard!", desc: { uz: "Card yasab, unga stil berdingiz", ru: "Вы сделали card и оформили её" } },
  graduate: { icon: "🏆", name: "Level Up!", desc: { uz: "VS Code darsini to'liq tugatdingiz", ru: "Вы полностью прошли урок VS Code" } }
};
var ACH_TRIGGERS = { s2: "installed", s3: "pilot" };
...
var INLINE_KEYS = { s4: 1, s8: 2, s11: 0, s13: -1 };
function VsCodeLesson({ lang: langProp, onFinished, onPractice, liveToken }) {
  ...
  const runPractice = (entry, fromScreen) => {
    const done = () => {
      ...
      if (fromScreen === 10) earn("cardmaster");
  ...
  return <LangContext.Provider value={lang}>
      <style>{`
        ...
        .cmp-codelbl { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 10px; font-weight: 700; letter-spacing: 0.08em; color: ${T.ink3}; }
        ...
        .cmp2-pvlbl { display: block; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: 9.5px; font-weight: 700; letter-spacing: 0.06em; color: ${T.ink3}; margin-bottom: 5px; }
        ...
        .rg-cmt { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 11px; color: ${CODE.comment}; margin-left: auto; }
        ...
        .tb-lbl { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: 9px; text-transform: uppercase; letter-spacing: 0.06em; color: ${T.ink3}; transition: opacity 0.3s 0.4s; }
      ...
      <style>{`
        ...
        .vsc { border-radius: 13px; overflow: hidden; background: ${VSC.bg}; box-shadow: 0 14px 34px -10px rgba(${T.shadowBase},0.35); font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; }
        ...
        .brw-url { flex: 1; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: 11.5px; font-weight: 600; color: ${T.ink}; background: #fff; border-radius: 99px; padding: 4px 12px; box-shadow: inset 0 0 0 1px #E4E1DA; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        ...
        .vcard-plainlogo { display: inline-block; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 20px; color: ${T.ink2}; border: 1.5px dashed ${T.ink3}; border-radius: 6px; padding: 8px 12px; }
        ...
        .inst-logo { width: 44px; height: 44px; border-radius: 11px; background: linear-gradient(135deg,#2A9FE8,#0065A9); color: #fff; display: flex; align-items: center; justify-content: center; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 17px; }
        ...
        .inst-dlfile { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: 10.5px; color: ${T.ink}; background: #F1EFE8; border-radius: 7px; padding: 6px 10px; white-space: nowrap; }
        ...
        .kcap { display: inline-flex; align-items: center; justify-content: center; min-width: 34px; padding: 7px 10px; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: clamp(12px,1.6vw,14px); color: ${T.ink}; background: linear-gradient(180deg,#FFFFFF,#EDEAE2); border-radius: 8px; box-shadow: 0 3px 0 #CFCBC1, 0 5px 10px -4px rgba(${T.shadowBase},0.3); }
    ...
    lang={__lang2}
    task={practice.task}
    starterCode={practice.starter ? tr3(practice.starter) : void 0}
