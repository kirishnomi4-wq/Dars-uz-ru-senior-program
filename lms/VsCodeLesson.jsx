...
function useLiveSession(lessonId, answerKey) {
  ...
  const joinStudent = useCallback(async (raw, rawNick) => {
    ...
    try {
      ...
      if (row.status !== "live") {
        setJoinError(tr2({ uz: "Bu dars allaqachon tugagan.", ru: "Этот урок уже завершён." }));
        setBusy(false);
        return;
      }
...
function LiveBadge({ live, total }) {
  ...
  if (live.mode === "mentor") {
    if (live.ended) return <div data-tour="live" className="live-badge" style={_liveBadgeS}><span style={_liveDot(LT.ink3)} /> 🔓 {tr2({ uz: "O'quvchilar erkin qilindi", ru: "Ученики отпущены в свободный режим" })}</div>;
    return <>
  ...
  if (live.mode === "student") {
    ...
    if (!live.mentorAlive) return <div data-tour="live" className="live-badge" style={_liveBadgeS}><span style={_liveDot(LT.ink3)} /> ⚠️ {tr2({ uz: "Mentor uzildi — erkin rejim", ru: "Ментор отключился — свободный режим" })}</div>;
    if (!live.connected) return <div data-tour="live" className="live-badge" style={_liveBadgeS}><span style={_liveDot("#FFD380")} /> 🔄 {tr2({ uz: "Qayta ulanmoqda…", ru: "Переподключаемся…" })}</div>;
...
var Stage = ({ children, eyebrow, screen, totalScreens = TOTAL_SCREENS, navContent, narrow, mentorStatic }) => {
  ...
  const [mForced, setMForced] = useState2(false);
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">🙈 {tr2({ uz: "Kim nimani tanlagani va ✅/❌ soni hozircha yashirin. «Natijani ochish» bosilganda — sizda ham, o'quvchilarda ham birdan ochiladi.", ru: "Кто что выбрал и число ✅/❌ пока скрыто. Нажмёте «Открыть результат» — появится сразу и у вас, и у учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верных — тема осталась непонятной классу. Перед продолжением сделайте короткое повторение.</> })}</p>
      ...
      {reveal && struggling && <p className="mstats-warn">⚠️ {tr2({ uz: "Ko'pchilik adashdi — mavzu tushunarsiz qolganga o'xshaydi. Yana bir bor tushuntiring.", ru: "Большинство ошиблись — похоже, тема осталась непонятной. Объясните ещё раз." })}</p>}
...
function MentorPracticeOverlay({ entry, live, onClose }) {
  ...
  if (view === "demo") {
    return <div style={{ position: "fixed", inset: 0, zIndex: 2e3, background: T.bg }}>
        <HtmlCompiler_default lang={__lang2} task={entry.task} starterCode={tr2(entry.starter)} onContinue={() => setView("watch")} onBack={() => setView("watch")} />
      </div>;
  }
  ...
  return <div className="mp-overlay">
      <div className="mp-card">
        <div className="mp-eyebrow">✍️ {tr2({ uz: "Amaliyot · jonli", ru: "Практика · live" })}</div>
        ...
        <div className="mp-flow">
          ...
          <span className="mp-step">2 · {tr2({ uz: "Mentor doskada yozib ko'rsatadi", ru: "Ментор показывает решение на доске" })}</span>
        </div>
        {data.players === null ? <p className="mstats-wait">{tr2({ uz: "Ulanish…", ru: "Подключение…" })}</p> : <div className="mstats" style={{ marginTop: 2 }}>
            ...
            {total === 0 && <p className="mstats-wait">{tr2({ uz: "Hali o'quvchi qo'shilmagan — ular praktikani boshlashi bilan bu yerda ✓ chiqadi…", ru: "Ученики ещё не подключились — как только они начнут практику, здесь появится ✓…" })}</p>}
          </div>}
        ...
        <p className="mp-tip">💡 {tr2({ uz: "Ko'pchilik tugatgach, shu mashqni doskada birga yozing. Shunda o'quvchilar o'zini tekshiradi va mavzu esida mahkam qoladi.", ru: "Когда закончит большинство, напишите это же упражнение вместе на доске. Так ученики проверят себя, а тема запомнится крепко." })}</p>
      </div>
    </div>;
}
  ...
  return <div className="dd fade-up">
      <div className="dd-slots">
        {slots.map((sid, i) => <div key={i} ref={(el) => slotRefs.current[i] = el} className={`dd-slot ${sid ? "filled" : ""} ${solved && sid ? "ok" : ""} ${wrong && sid && sid !== order[i] ? "bad" : ""}`}>
            ...
            {sid ? <button className="dd-chip in" onPointerDown={(e) => down(e, sid, i)}>{byId[sid].label}</button> : <span className="dd-hint">{hints ? tr2(hints[i]) : tr2({ uz: "bu yerga joylang", ru: "поместите сюда" })}</span>}
          </div>)}
      </div>
      <div className="dd-pool">
        {pool.length === 0 && !solved && <span className="dd-pool-empty">{tr2({ uz: "Tartib xato — bo'lakni bosib qaytaring va qayta joylang", ru: "Порядок неверный — нажмите на блок, верните его и расставьте заново" })}</span>}
        {pool.map((id) => <button key={id} className="dd-chip" onPointerDown={(e) => down(e, id, "pool")}>{byId[id].label}</button>)}
      </div>
      {solved && <div className="dd-done">✓ {tr2({ uz: "To'g'ri! Shablon aynan shu tartibda.", ru: "Верно! Шаблон именно в таком порядке." })}</div>}
      {wrong && !solved && <div className="dd-wrong">⚠️ {tr2({ uz: "Tartib xato — qayta joylang.", ru: "Порядок неверный — расставьте заново." })}</div>}
    </div>;
}
...
function StepChecklist({ screen, storedAnswer, onAnswer, steps, taskLabel }) {
  ...
  const live = gate.live;
  const isMentorLive = !!(live && live.mode === "mentor");
  const [done, setDone] = useState2(() => new Set(storedAnswer?.doneIds || (storedAnswer?.correct ? steps.map((s) => s.id) : [])));
  const sentRef = useRef2(!!(storedAnswer && storedAnswer.correct));
  ...
  return <div className="clk fade-up delay-2">
      <div className="clk-head">
        <span className="clk-lbl">✅ {tr2({ uz: "Kompyuteringizda bajaring", ru: "Выполните на своём компьютере" })}</span>
    ...
    return <div key={s.id} className={`clk-row ${on ? "on" : ""}`}>
            ...
            <span className="clk-txt">
              <span className="clk-t">{fmtCode(tr2(s.t))}</span>
              {s.d && <span className="clk-d">{fmtCode(tr2(s.d))}</span>}
            </span>
            <button className={`clk-btn ${on ? "on" : ""}`} onClick={() => toggle(s.id)} disabled={isMentorLive}>
              {on ? tr2({ uz: "✓ Bajarildi", ru: "✓ Готово" }) : tr2({ uz: "Bajardim", ru: "Сделал(а)" })}
            </button>
          </div>;
  })}
      {allDone && !isMentorLive && <p className="clk-done fade-step">🎉 {tr2({ uz: "Zo'r! Hamma qadam bajarildi — davom etamiz.", ru: "Отлично! Все шаги выполнены — продолжаем." })}</p>}
...
var ScreenHook = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up" style={{ maxWidth: 780 }}>{tr2({ uz: <>Dasturchilar kodni <span className="italic" style={{ color: T.accent }}>qayerga</span> yozadi?</>, ru: <>Куда разработчики <span className="italic" style={{ color: T.accent }}>пишут</span> код?</> })}</h1>
        ...
        <Split>
          <Col>
            <p className="eyebrow fade-up delay-1" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Sizningcha qaysi biri?", ru: "Как вы думаете, где?" })}</p>
    ...
    main={<>
                  ...
                  <div className="vsc-code">
                    ...
                    <VL n={2}><Vt>&lt;p&gt;</Vt><span style={{ color: VSC.text }}>{tr2({ uz: "Mening birinchi loyiham", ru: "Мой первый проект" })}</span><Vt>&lt;/p&gt;</Vt></VL>
  ...
  />
            ...
            {picked === null && <p className="mono small fade-up delay-3" style={{ color: T.ink3, textAlign: "center" }}>{tr2({ uz: "↑ javob tanlasangiz — ochiladi", ru: "↑ выберите ответ — и он откроется" })}</p>}
          </Col>
        </Split>
      </div>
    </Stage>;
...
var ScreenGoal = ({ screen, onNext, onPrev }) => {
  ...
  const PreviewBlock = <Col>
      <p className="flow-label">{tr2({ uz: "Natija — dars oxirida kompyuteringizda shunday bo'ladi", ru: "Результат — в конце урока на вашем компьютере будет так" })}</p>
      <BrowserWin tab="Mening cardim" minH={230}>
        <VCard anim />
      </BrowserWin>
    </Col>;
  const StepsBlock = <Col>
      <p className="flow-label">{tr2({ uz: "6 qadam", ru: "6 шагов" })}</p>
      <ol className="roadmap">
        {STEPS.map((s, i) => <li key={i} className="step-card fade-up" style={{ animationDelay: `${0.08 + i * 0.05}s` }}>
            <span className="step-num">{String(i + 1).padStart(2, "0")}</span>
            <span className="step-body">
              <span className="step-text">{tr2(s.text)}</span>
              {s.tag && <span className="step-tag">{s.tag}</span>}
            </span>
          </li>)}
      </ol>
    </Col>;
  return <Stage eyebrow={tr2({ uz: "Maqsad", ru: "Цель" })} screen={screen} audioState={audio} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head">
          <h2 className="title h-title fade-up">{tr2({ uz: <>Bugun <span className="italic" style={{ color: T.accent }}>o'z kompyuteringizda</span> birinchi sahifangizni yasaysiz.</>, ru: <>Сегодня вы сделаете первую страницу <span className="italic" style={{ color: T.accent }}>на своём компьютере</span>.</> })}</h2>
        </div>
        ...
        {!isNarrow ? <Split>{PreviewBlock}{StepsBlock}</Split> : !showSteps ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>
            {PreviewBlock}
            <button className="btn" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(true)}>📋 {tr2({ uz: "Bugungi 6 qadamni ko'rish", ru: "Посмотреть 6 шагов на сегодня" })}</button>
          </div> : <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>
            <button className="btn-ghost" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(false)}>↩ {tr2({ uz: "Natijani ko'rish", ru: "Посмотреть результат" })}</button>
            {StepsBlock}
          </div>}
      </div>
    </Stage>;
...
var ScreenInstall = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "1-qadam · O'rnatish", ru: "1-й шаг · Установка" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !(storedAnswer && storedAnswer.correct)} label={_isMentorLive || storedAnswer && storedAnswer.correct ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Qadamlarni belgilang", ru: "Отметьте шаги" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>VS Code'ni <span className="italic" style={{ color: T.accent }}>qayerdan</span> yuklaymiz?</>, ru: <>Откуда <span className="italic" style={{ color: T.accent }}>скачать</span> VS Code?</> })}</h2></div>
        ...
        <Split>
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {VIEWS.map((v) => <button key={v.id} className={`chip ${view === v.id ? "chip-on" : ""}`} onClick={() => setView(v.id)}>{tr2(v.chip)}</button>)}
            </div>
            <div className="demo-swap" key={view}>
              {view === 0 && <BrowserWin url="code.visualstudio.com" tab="Visual Studio Code" minH={190}>
                  <div className="inst-site">
                    ...
                    <p className="inst-sub">{tr2({ uz: "Free. Built on open source. Runs everywhere.", ru: "Free. Built on open source. Runs everywhere." })}</p>
              ...
              {view === 1 && <BrowserWin url="code.visualstudio.com" tab="Visual Studio Code" minH={190}>
                  <div className="inst-site">
                    ...
                    <p className="inst-sub">{tr2({ uz: "Fayl brauzerning pastida yoki «Yuklanmalar» papkasida ko'rinadi", ru: "Файл появится внизу браузера или в папке «Загрузки»" })}</p>
                  </div>
                </BrowserWin>}
              {view === 2 && <div className="inst-setup">
                  ...
                  <div className="inst-setbody">
                    ...
                    <p className="inst-sub" style={{ marginTop: 8 }}>{tr2({ uz: "Bir daqiqadan so’ng — tayyor!", ru: "Через минуту — готово!" })}</p>
                  </div>
                </div>}
            </div>
          </Col>
          <Col>
            <StepChecklist screen={screen} storedAnswer={storedAnswer} onAnswer={onAnswer} steps={STEPS} taskLabel={tr2({ uz: "VS Code o'rnatish", ru: "Установка VS Code" })} />
          </Col>
        </Split>
      </div>
    </Stage>;
...
var ScreenTour = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState2(() => new Set(storedAnswer?.seenIds || (storedAnswer?.correct ? Object.keys(HOTS) : [])));
  const sentRef = useRef2(!!(storedAnswer && storedAnswer.correct));
  ...
  return <Stage eyebrow={tr2({ uz: "2-qadam · Tanishuv", ru: "2-й шаг · Знакомство" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !allSeen} label={_isMentorLive || allSeen ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "4 panelni bosib chiqing", ru: "Нажмите на 4 панели" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>VS Code oynasida <span className="italic" style={{ color: T.accent }}>nimalar bor</span>?</>, ru: <>Что <span className="italic" style={{ color: T.accent }}>есть</span> в окне VS Code?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Bu — sizning yangi ish stolingiz. Oynaning <b style={{ color: T.ink }}>4 panelini</b> birma-bir bosib chiqing — har birining vazifasini aytib beraman.</>, ru: <>Это ваш новый рабочий стол. Нажмите по очереди на <b style={{ color: T.ink }}>4 панели</b> окна — расскажу, для чего каждая.</> })}</Mentor>
        <p className="sk-tapguide fade-up delay-1">👆 {tr2({ uz: <>Oynaning panellarini <b>bosing</b></>, ru: <>Нажимайте на панели окна</> })}<span className="sk-tapcount">{seen.size}/4</span></p>
        <Split>
          <Col>
          ...
          <Col>
            {active ? <div className="sk-info fade-step" key={active}>
                <div className="sk-tagbig"><span style={{ fontSize: 22 }}>{HOTS[active].ic}</span><span className="sk-wordbadge">{tr2(HOTS[active].name)}</span></div>
                <p className="body" style={{ marginTop: 8 }}>{tr2(HOTS[active].body)}</p>
              </div> : <div className="hint fade-up delay-2">{tr2({ uz: "Chapdagi oynaning istalgan panelini bosing — shu yerda tushuntirish chiqadi.", ru: "Нажмите на любую панель окна слева — здесь появится объяснение." })}</div>}
            {allSeen && <div className="frame-success fade-step">
                <p className="body" style={{ margin: 0 }}>{tr2({ uz: <>Ajoyib — <b>4 panelning hammasini</b> bilib oldingiz! Endi bu oyna siz uchun begona emas.</>, ru: <>Отлично — вы узнали <b>все 4 панели</b>! Теперь это окно для вас не чужое.</> })}</p>
              </div>}
          </Col>
        </Split>
      </div>
...
var ScreenTest1 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "Oynani eslang", ru: "Вспомните окно" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: "Loyihadagi fayllar ro'yxatini VS Code'ning qaysi paneli ko'rsatadi?", ru: "Какая панель VS Code показывает список файлов проекта?" })}</h2></>}
  options={[
  ...
  correctIdx={1}
  explainCorrect={tr2({ uz: "To'g'ri. Explorer — chapdagi panel: loyihangizdagi barcha fayllar shu yerda. Faylni bossangiz — Editor panelida ochiladi.", ru: "Верно. Explorer — панель слева: там все файлы проекта. Нажмёте на файл — он откроется в панели Editor." })}
  explainWrong={{
    0: tr2({ uz: "Terminal — kompyuterga yozma buyruq beriladigan panel. Fayllar ro'yxatini Explorer ko'rsatadi.", ru: "Terminal — панель для письменных команд компьютеру. Список файлов показывает Explorer." }),
    2: tr2({ uz: "Extensions — qo'shimcha dasturlar paneli: Live Server ham shu yerdan o'rnatiladi. Fayllar ro'yxati — Explorer'da.", ru: "Extensions — панель дополнений: отсюда ставится и Live Server. Список файлов — в Explorer." }),
    3: tr2({ uz: "Status bar — pastki ko'k chiziq, u yerda Go Live tugmasi turadi. Fayllar ro'yxati — Explorer'da.", ru: "Status bar — нижняя синяя полоса, там живёт кнопка Go Live. Список файлов — в Explorer." }),
    default: tr2({ uz: "Fayllar ro'yxatini chapdagi Explorer paneli ko'rsatadi.", ru: "Список файлов показывает панель Explorer слева." })
  }}
/>;
var ScreenFolder = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "3-qadam · Loyiha", ru: "3-й шаг · Проект" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !(storedAnswer && storedAnswer.correct)} label={_isMentorLive || storedAnswer && storedAnswer.correct ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Qadamlarni belgilang", ru: "Отметьте шаги" })} onClick={onNext} /></>}>
    ...
    main={<>
                  <div className="vsc-tabs"><span className="vsc-tab on"><span style={{ color: "#E37933" }}>{"<>"}</span> index.html</span></div>
                  <div className="vsc-code">
                    <VL n={1}><span className="vsc-caret" /><span style={{ color: VSC.dim, fontStyle: "italic", marginLeft: 8 }}>{tr2({ uz: "bo'sh — keyingi qadamda to'ldiramiz", ru: "пусто — заполним на следующем шаге" })}</span></VL>
                  </div>
                </>}
    statusRight={<>⚡ Go Live</>}
  />
            </div>
            <div className="hl-note fade-up delay-2"><p className="body" style={{ margin: 0 }}>{fmtCode(tr2({ uz: "💡 `index.html` — loyihaning bosh sahifasi. Brauzer papkani ochganda birinchi bo'lib shu faylni qidiradi. Shuning uchun nomi doim shunday bo'ladi.", ru: "💡 `index.html` — главная страница проекта. Открывая папку, браузер первым делом ищет именно этот файл. Поэтому имя всегда такое." }))}</p></div>
          </Col>
          <Col>
            <StepChecklist screen={screen} storedAnswer={storedAnswer} onAnswer={onAnswer} steps={STEPS} taskLabel={tr2({ uz: "Papka va fayllar", ru: "Папка и файлы" })} />
          </Col>
        </Split>
      </div>
    </Stage>;
...
var ScreenEmmet = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [phase, setPhase] = useState2(storedAnswer ? 2 : 0);
  const [solved, setSolved] = useState2(!!(storedAnswer && storedAnswer.correct));
  ...
  return <Stage eyebrow={tr2({ uz: "3-qadam · Shablon", ru: "3-й шаг · Шаблон" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !solved} label={_isMentorLive || solved ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Shablonni yig'ing", ru: "Соберите шаблон" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <><span className="mono" style={{ color: T.accent }}>!</span> + Tab bosilsa, <span className="italic" style={{ color: T.accent }}>butun shablon</span> o'zi yoziladimi?</>, ru: <><span className="mono" style={{ color: T.accent }}>!</span> + Tab — и <span className="italic" style={{ color: T.accent }}>весь шаблон</span> напишется сам?</> })}</h2></div>
  ...
  />
            ...
            <div className="br-controls fade-up delay-2">
              {phase === 0 && <button className="btn" onClick={play}>✨ {tr2({ uz: "Natijani ko'rish", ru: "Посмотреть результат" })}</button>}
              {phase === 2 && <button className="btn-ghost" onClick={() => {
    setPhase(0);
  }}>↻ {tr2({ uz: "Yana bir bor ko'rish", ru: "Посмотреть ещё раз" })}</button>}
              {phase === 2 && <span className="mono small" style={{ color: T.success, fontWeight: 700 }}>✓ {tr2({ uz: "10 qator — 1 soniyada!", ru: "10 строк — за 1 секунду!" })}</span>}
            </div>
            {
    /* F-0810-06: `Ctrl+Z` ilgari faqat `s12` (Bonus) ekranida bir marta
       ...
       birinchi marta shu yerda qidiradi. */
            ...
            {phase === 2 && <p className="yz-hint fade-up delay-2" style={{ marginTop: 6 }}>{fmtCode(tr2({
    uz: "Noto'g'ri chiqdimi? `Ctrl+Z` bosing — hammasi orqaga qaytadi.",
    ru: "Получилось не то? Нажмите `Ctrl+Z` — всё вернётся назад."
  }))}</p>}
          </Col>
          <Col>
            {phase < 2 ? <div className="hint fade-up delay-2">{tr2({ uz: "Avval chapdagi «Natijani ko'rish» tugmasini bosing — shablon ochilgach, bu yerda mashq chiqadi.", ru: "Сначала нажмите «Посмотреть результат» слева — когда шаблон раскроется, здесь появится упражнение." })}</div> : <div className="sk-buildbox">
                <p className="flow-label" style={{ marginBottom: 8 }}>{tr2({ uz: "Endi o'zingiz: shablon bo'laklarini TO'G'RI tartibda joylang", ru: "Теперь сами: расставьте части шаблона в ВЕРНОМ порядке" })}</p>
                ...
                {solved && <div className="frame-success fade-step" style={{ marginTop: 10 }}><p className="body" style={{ margin: 0 }}>{tr2({ uz: <>Esladingiz! Emmet ham aynan shu tartibda yozadi: kod <b>body</b> ichiga — kursor turgan joyga yoziladi.</>, ru: <>Вспомнили! Emmet пишет именно в этом порядке: код пишется внутрь <b>body</b> — туда, где стоит курсор.</> })}</p></div>}
              </div>}
          </Col>
        </Split>
      </div>
...
var ScreenLive = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const nm = CARD_NAME;
  const [view, setView] = useState2(0);
  const [viewed, setViewed] = useState2(() => /* @__PURE__ */ new Set([0]));
  const go = (v) => {
    setView(v);
    setViewed((prev) => {
  ...
  return <Stage eyebrow={tr2({ uz: "4-qadam · Jonli server", ru: "4-й шаг · Живой сервер" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !(storedAnswer && storedAnswer.correct)} label={_isMentorLive || storedAnswer && storedAnswer.correct ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Qadamlarni belgilang", ru: "Отметьте шаги" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <><span className="italic" style={{ color: T.accent }}>Go Live</span> bilan sahifani brauzerda ochamiz.</>, ru: <>Открываем страницу в браузере через <span className="italic" style={{ color: T.accent }}>Go Live</span>.</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Endi eng muhim qadam. <b style={{ color: T.ink }}>Live Server</b> — sahifani <b style={{ color: T.ink }}>jonli serverda</b> ochib beradigan qo'shimcha dastur: kodni saqlashingiz bilan brauzer o'zi yangilanadi. Uch ko'rinishni ko'rib chiqing, keyin kompyuteringizda bajaring.</>, ru: <>Теперь самый важный шаг. <b style={{ color: T.ink }}>Live Server</b> — дополнение, открывающее страницу на <b style={{ color: T.ink }}>живом сервере</b>: сохраните код — и браузер обновится сам. Просмотрите три вида, затем выполните на своём компьютере.</> })}</Mentor>
        <Split>
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {VIEWS.map((v, i) => <button key={i} className={`chip ${view === i ? "chip-on" : ""}`} onClick={() => go(i)}>{viewed.has(i) && i !== view ? "✓ " : ""}{tr2(v.chip)}</button>)}
            </div>
            <div className="demo-swap" key={view}>
              {view === 0 && <VscShell
    act={3}
    side={<>
                    ...
                    <div className="vsc-extrow"><span className="vsc-extic">🎨</span><span className="vsc-extinfo"><b>Prettier</b><i>{tr2({ uz: "keyinroq kerak", ru: "пригодится позже" })}</i></span></div>
                  </>}
    main={<div className="vsc-code" style={{ paddingTop: 20 }}><p className="vsc-note">{tr2({ uz: "← Qidiruvdan toping va Install tugmasini bosing", ru: "← Найдите в поиске и нажмите Install" })}</p></div>}
    statusRight={<>…</>}
  />}
              {view === 1 && <VscShell
    title="my-site — Visual Studio Code"
    ...
    main={<>
                    ...
                    <div className="vsc-code">
                      ...
                      <VL n={8} ind={2}><Vt>&lt;h1&gt;</Vt><span style={{ color: VSC.text }}>{tr2({ uz: `Salom, men — ${nm}!`, ru: `Привет, я — ${nm}!` })}</span><Vt>&lt;/h1&gt;</Vt></VL>
    ...
    statusPulse
  />}
              {view === 2 && <BrowserWin url="127.0.0.1:5500/index.html" tab={tr2({ uz: "Mening sahifam", ru: "Моя страница" })} minH={170}>
                  <h1 className="pv-h1" style={{ padding: "18px 20px" }}>{tr2({ uz: `Salom, men — ${nm}!`, ru: `Привет, я — ${nm}!` })}</h1>
                </BrowserWin>}
            </div>
            {view === 1 && <p className="mono small fade-step" style={{ color: T.accent, fontWeight: 700, textAlign: "right" }}>↑ {tr2({ uz: "mana shu tugma — pastki ko'k chiziqda", ru: "вот эта кнопка — на нижней синей полосе" })}</p>}
            {view === 2 && <div className="frame-soft fade-step">
                <p className="body" style={{ margin: 0 }}>{fmtCode(tr2({ uz: "🏠 `127.0.0.1` — «mening kompyuterim» degan manzil. Internet emas, o'zingizniki. `5500` — eshik raqami (port): sahifa aynan shu eshikdan chiqadi. Demak bu sahifani hozircha faqat SIZ ko'rasiz.", ru: "🏠 `127.0.0.1` — адрес «мой компьютер». Это не интернет, а ваш собственный компьютер. `5500` — номер двери (порт): страница выходит именно через неё. Значит, эту страницу пока видите только ВЫ." }))}</p>
              </div>}
          </Col>
          <Col>
            <StepChecklist screen={screen} storedAnswer={storedAnswer} onAnswer={onAnswer} steps={STEPS} taskLabel={tr2({ uz: "Live Server + Go Live", ru: "Live Server + Go Live" })} />
          </Col>
        </Split>
      </div>
    </Stage>;
...
var ScreenTest2 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "Jonli server", ru: "Живой сервер" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: "Pastdagi «Go Live» tugmasi bosilganda nima bo’ladi?", ru: "Что происходит, когда вы нажимаете «Go Live» внизу?" })}</h2></>}
  options={[
  ...
  correctIdx={2}
  explainCorrect={tr2({ uz: "To'g'ri. Go Live jonli serverni yoqadi: sahifa brauzerda 127.0.0.1:5500 manzilida ochiladi. Har saqlashda esa o'zi yangilanadi.", ru: "Верно. Go Live включает живой сервер: страница открывается в браузере на 127.0.0.1:5500. А при каждом сохранении обновляется сама." })}
  explainWrong={{
    0: tr2({ uz: "Yo'q — 127.0.0.1 faqat o'z kompyuteringiz. Internetga chiqarish alohida qadam, unga hali yetamiz.", ru: "Нет — 127.0.0.1 это только ваш компьютер. Публикация в интернете — отдельный шаг, до него ещё дойдём." }),
    1: tr2({ uz: "Xatolarni tekshirish — boshqa vositalar ishi. Go Live sahifani jonli serverda ochadi.", ru: "Проверка ошибок — работа других инструментов. Go Live открывает страницу на живом сервере." }),
    3: tr2({ uz: "Saqlash — Ctrl+S ishi. Go Live esa sahifani brauzerda jonli ochadi.", ru: "Сохранение — работа Ctrl+S. А Go Live живо открывает страницу в браузере." }),
    default: tr2({ uz: "Go Live sahifani 127.0.0.1:5500 manzilida jonli ochadi.", ru: "Go Live живо открывает страницу на адресе 127.0.0.1:5500." })
  }}
/>;
var ScreenCardHtml = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [active, setActive] = useState2(null);
  const [seen, setSeen] = useState2(() => new Set(storedAnswer?.seenIds || (storedAnswer?.correct ? Object.keys(PARTS) : [])));
  ...
  return <Stage eyebrow={tr2({ uz: "5-qadam · Card HTML", ru: "5-й шаг · HTML карточки" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !allSeen} label={_isMentorLive || allSeen ? tr2({ uz: "Praktikaga →", ru: "К практике →" }) : tr2({ uz: "3 bo'lakni bosib o'rganing", ru: "Изучите 3 части" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Card <span className="italic" style={{ color: T.accent }}>nechta qismdan</span> iborat?</>, ru: <>Из <span className="italic" style={{ color: T.accent }}>скольких частей</span> состоит card?</> })}</h2></div>
        ...
        <p className="sk-tapguide fade-up delay-1">👆 {tr2({ uz: <>Koddagi yorqin bo'laklarni <b>bosing</b></>, ru: <>Нажимайте на яркие части кода</> })}<span className="sk-tapcount">{seen.size}/3</span></p>
        <Split>
          <Col>
            ...
            {active && <div className="sk-info fade-step" key={active}>
                <div className="sk-tagbig"><span style={{ fontSize: 20 }}>{PARTS[active].ic}</span><span className="sk-wordbadge">{tr2(PARTS[active].name)}</span></div>
                <p className="body" style={{ marginTop: 8 }}>{tr2(PARTS[active].body)}</p>
              </div>}
          </Col>
          <Col>
            <p className="flow-label fade-up delay-2">{tr2({ uz: "Hozircha shunday ko'rinadi (style.css bo'sh)", ru: "Пока выглядит так (style.css пуст)" })}</p>
            <BrowserWin url="127.0.0.1:5500/index.html" minH={190}>
              <VCard name={nm} prof={CARD_PROF} plain />
            </BrowserWin>
            <div className="hl-note fade-up delay-3"><p className="body" style={{ margin: 0 }}>{tr2({ uz: <>Oddiy va rangsiz — chunki <b className="mono">style.css</b> hali bo'sh. Keyingi qadamda uni to'ldiramiz. 😉</>, ru: <>Просто и бесцветно — потому что <b className="mono">style.css</b> пока пуст. На следующем шаге мы его заполним. 😉</> })}</p></div>
          </Col>
        </Split>
      </div>
    </Stage>;
...
var ScreenCardCss = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [on, setOn] = useState2(() => new Set(storedAnswer?.onIds || (storedAnswer?.correct ? STEPS.map((s) => s.id) : [])));
  ...
  return <Stage eyebrow={tr2({ uz: "6-qadam · Card CSS", ru: "6-й шаг · CSS карточки" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !allOn} label={_isMentorLive || allOn ? tr2({ uz: "Praktikaga →", ru: "К практике →" }) : tr2({ uz: "4 bezakni yoqing", ru: "Включите 4 оформления" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Cardni qanday <span className="italic" style={{ color: T.accent }}>chiroyli</span> qilamiz?</>, ru: <>Как сделать card <span className="italic" style={{ color: T.accent }}>красивой</span>?</> })}</h2></div>
        ...
        <Split>
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {STEPS.map((s) => <button key={s.id} className={`chip ${on.has(s.id) ? "chip-on" : ""}`} onClick={() => toggle(s.id)}>{on.has(s.id) ? "✓ " : ""}{tr2(s.chip)}</button>)}
            </div>
            <div className="code-box fade-up delay-1" style={{ fontSize: "clamp(12.5px,1.6vw,14px)" }}>
              <div><span style={{ color: "#D7BA7D" }}>.card</span> {"{"}</div>
              {STEPS.filter((s) => on.has(s.id)).map((s) => <div key={s.id} className="fade-step">{"  "}<span style={{ color: CODE.attr }}>{s.css.split(":")[0]}</span>:<span style={{ color: CODE.str }}>{s.css.split(":").slice(1).join(":").replace(";", "")};</span> <span className="cm">{"/* "}{tr2(s.d)}{" */"}</span></div>)}
              {on.size === 0 && <div className="cm">{"  /* "}{tr2({ uz: "bezaklarni tepadan yoqing…", ru: "включайте оформления сверху…" })}{" */"}</div>}
              <div>{"}"}</div>
            </div>
            {allOn && <div className="frame-success fade-step"><p className="body" style={{ margin: 0 }}>{tr2({ uz: <>Mana bu — <b>rasmdagi card</b>! Endi praktikada shu 4 qatorni o'zingiz yozasiz, keyin VS Code'dagi style.css'ga ham ko'chirasiz.</>, ru: <>Вот она — <b>карточка с образца</b>! Теперь в практике вы напишете эти 4 строки сами, а потом перенесёте их в свой style.css в VS Code.</> })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label fade-up delay-2">{tr2({ uz: "Jonli natija", ru: "Живой результат" })}</p>
...
var ScreenTest3 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "Ikki fayl do’stligi", ru: "Дружба двух файлов" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: "style.css'dagi bezaklar sahifaga qaysi qator orqali ulanadi?", ru: "Через какую строку оформление из style.css подключается к странице?" })}</h2></>}
  options={[
  ...
  correctIdx={0}
  explainCorrect={tr2({ uz: "To'g'ri. `link` tegi head ichida turadi: `rel=\"stylesheet\"` — «bu bezak fayli», `href` — fayl manzili. Shu ko'prik orqali CSS sahifaga yetib boradi.", ru: 'Верно. Тег `link` стоит внутри head: `rel="stylesheet"` — «это файл оформления», `href` — адрес файла. По этому мосту CSS доходит до страницы.' })}
  explainWrong={{
    1: tr2({ uz: "`style` tegida `src` atributi ishlamaydi. CSS faylni `head` ichidagi `link` tegi ulaydi.", ru: "У тега `style` атрибут `src` не работает. CSS-файл подключает тег `link` внутри `head`." }),
    2: tr2({ uz: "`css` degan teg mavjud emas. CSS faylni `link` tegi ulaydi.", ru: "Тега `css` не существует. CSS-файл подключает тег `link`." }),
    3: tr2({ uz: "`script` — JavaScript uchun. CSS faylni `head` ichidagi `link` tegi ulaydi.", ru: "`script` — для JavaScript. CSS-файл подключает тег `link` внутри `head`." }),
    default: tr2({ uz: "CSS faylni `head` ichidagi `link` tegi ulaydi.", ru: "CSS-файл подключает тег `link` внутри `head`." })
  }}
/>;
var ScreenKeys = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [pressed, setPressed] = useState2(() => new Set(storedAnswer?.pressedIds || (storedAnswer?.correct ? KEYS.map((k) => k.id) : [])));
  const [active, setActive] = useState2(null);
  ...
  return <Stage eyebrow={tr2({ uz: "Bonus · Tezkor tugmalar", ru: "Бонус · Горячие клавиши" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !allPressed} label={_isMentorLive || allPressed ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "4 birikmani bosing", ru: "Нажмите 4 сочетания" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Dasturchining <span className="italic" style={{ color: T.accent }}>maxfiy tugmalari</span> qanday?</>, ru: <>Какие у разработчика <span className="italic" style={{ color: T.accent }}>секретные клавиши</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Bu tugmalar <b style={{ color: T.ink }}>vaqtni tejaydi</b>. Har birini <b style={{ color: T.ink }}>bosib</b>, nima qilishini ko'ring.</>, ru: <>Эти клавиши <b style={{ color: T.ink }}>экономят время</b>. <b style={{ color: T.ink }}>Нажмите</b> на каждую и посмотрите, что она делает.</> })}</Mentor>
        {
    /* 🔴 F-0810-04: ilgari bu ekran `narrow` (680px) edi va butun dars ichida
       ...
       naqshda: chapda bosiladigan narsalar, o'ngda tushuntirish. */
        ...
        <Split>
          <Col>
            <p className="sk-tapguide fade-up delay-1">👆 {tr2({ uz: <>Birikmalarni <b>bosing</b></>, ru: <>Нажимайте на сочетания</> })}<span className="sk-tapcount">{pressed.size}/4</span></p>
            <div className="kcap-grid fade-up delay-1">
              {KEYS.map((k) => <button key={k.id} className={`kcap-card ${pressed.has(k.id) ? "seen" : ""} ${active === k.id ? "on" : ""}`} onClick={() => tap(k.id)}>
                  ...
                  <span className="kcap-name">{pressed.has(k.id) ? "✓ " : ""}{tr2(k.name)}</span>
                </button>)}
            </div>
          </Col>
          <Col>
            {active ? <div className="sk-info fade-step" key={active}>
                <div className="sk-tagbig"><span className="sk-wordbadge">{tr2(KEYS.find((k) => k.id === active).name)}</span></div>
                <p className="body" style={{ marginTop: 8 }}>{tr2(KEYS.find((k) => k.id === active).d)}</p>
              </div> : <div className="hint fade-up delay-2">{tr2({ uz: "Chapdagi istalgan birikmani bosing — shu yerda tushuntirish chiqadi.", ru: "Нажмите на любое сочетание слева — здесь появится объяснение." })}</div>}
...
var ScreenFinalTest = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [val, setVal] = useState2(storedAnswer?.studentAnswer ?? "");
  ...
  return <Stage eyebrow={tr2({ uz: "Yakuniy sinov", ru: "Финальная проверка" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={isMentorLive ? false : !correct} label={isMentorLive ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : correct ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Tugma nomini yozing", ru: "Напишите имя кнопки" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>yozib</span> ko'ring.</>, ru: <>Теперь напишите <span className="italic" style={{ color: T.accent }}>сами</span>.</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Yakuniy savol: sahifani brauzerda ochish uchun <b style={{ color: T.ink }}>qaysi tugmani</b> bosamiz? Javob <b style={{ color: T.ink }}>ikki so'zdan</b> iborat.</>, ru: <>Финальный вопрос: <b style={{ color: T.ink }}>какую кнопку</b> мы нажимаем, чтобы открыть страницу в браузере? Ответ — <b style={{ color: T.ink }}>два слова</b>.</> })}</Mentor>
        <div className="ft-statwrap fade-up delay-1">
          ...
          {!correct ? <div className="ft-inputrow">
              <input className="text-input" style={{ maxWidth: 260 }} value={val} onChange={(e) => setVal(e.target.value)} placeholder={tr2({ uz: "tugma nomi…", ru: "имя кнопки…" })} spellCheck={false} disabled={isMentorLive} />
              <p className="yz-hint">{touched ? tr2({ uz: "Deyarli! Ikki inglizcha so'z: «jonli holatga o't» degan ma'noda.", ru: "Почти! Два английских слова — в смысле «переходи в живой режим»." }) : tr2({ uz: "Maslahat: birinchi so'zi — «Go»", ru: "Подсказка: первое слово — «Go»" })}</p>
            </div> : <div className="fade-step">
              <BrowserWin url="127.0.0.1:5500/index.html" minH={90}>
                <p className="yz-ok" style={{ padding: "14px 18px", margin: 0 }}>✓ {tr2({ uz: "To'g'ri — Go Live! Sahifa jonli serverda ochildi.", ru: "Верно — Go Live! Страница открылась на живом сервере." })}</p>
              </BrowserWin>
            </div>}
        </div>
        {isMentorLive && <MentorWorkStats live={live} screenIdx={screen} taskLabel={tr2({ uz: "«Go Live» yozish (yakuniy)", ru: "Написать «Go Live» (финал)" })} />}
      </div>
    </Stage>;
};
var ScreenFinale = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Marra", ru: "Финиш" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen">
        <Confetti />
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Birinchi <span className="italic" style={{ color: T.accent }}>web-cardingiz</span> tayyor.</>, ru: <>Ваша первая <span className="italic" style={{ color: T.accent }}>web-card</span> готова.</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Tabriklaymiz! Endi cardingiz brauzerda ishlayapti. Rangini o'zgartirib ko'ring va <b className="mono" style={{ color: T.ink }}>Ctrl+S</b> bosganda natija qanday yangilanishini kuzating.</>, ru: <>Поздравляем! Теперь ваша card работает в браузере. Попробуйте поменять её цвет и посмотрите, как обновится результат после <b className="mono" style={{ color: T.ink }}>Ctrl+S</b>.</> })}</Mentor>
        <Split>
          <Col>
            <ul className="recipe-list fade-up delay-1">
              {DONE.map((d, i) => <li key={i} className="on"><span className="recipe-num">✓</span><span className="recipe-text">{fmtCode(tr2(d))}</span></li>)}
            </ul>
            {!isMentorLive && <div className="frame-success fade-step"><p className="body" style={{ margin: 0 }}>{tr2({ uz: <>Bugundan boshlab kodni <b>o'z kompyuteringizga</b> yozasiz. Endi har darsda shu qurollar bilan ishlaymiz.</>, ru: <>С сегодняшнего дня вы пишете код <b>на своём компьютере</b>. Теперь на каждом уроке работаем этими инструментами.</> })}</p></div>}
            {isMentorLive && <MentorWorkStats live={live} screenIdx={screen} taskLabel={tr2({ uz: "Yakuniy ekranga yetdi", ru: "Дошёл до финального экрана" })} />}
          </Col>
          <Col>
            <p className="flow-label fade-up delay-2">{tr2({ uz: "Sizning natijangiz", ru: "Ваш результат" })}</p>
            <BrowserWin url="127.0.0.1:5500/index.html" tab={tr2({ uz: "Mening cardim", ru: "Моя карточка" })} minH={230}>
...
var ScreenSummary = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  const RECAP = [
    tr2({ uz: "VS Code'ni o'rnatish va sozlash", ru: "Устанавливать и настраивать VS Code" }),
    tr2({ uz: "Loyiha papkasi va fayllar yaratish", ru: "Создавать папку проекта и файлы" }),
    tr2({ uz: "! + Tab bilan tayyor shablon olish", ru: "Получать готовый шаблон через ! + Tab" }),
    tr2({ uz: "Live Server bilan sahifani jonli ko'rish", ru: "Живо смотреть страницу через Live Server" }),
    tr2({ uz: "Card yasab, CSS bilan bezash", ru: "Делать card и оформлять её CSS" })
  ];
  const HOMEWORK = [
    { b: tr2({ uz: "Hobbi ro'yxati", ru: "Список хобби" }), t: tr2({ uz: "— card ichiga `ul`/`li` bilan 3 ta hobbi", ru: "— 3 хобби внутри карточки через `ul`/`li`" }) },
    { b: tr2({ uz: "Havola", ru: "Ссылка" }), t: tr2({ uz: "— Telegram yoki sevimli saytingizga `a href`", ru: "— `a href` на Telegram или любимый сайт" }) },
    { b: tr2({ uz: "Rang tajribasi", ru: "Эксперимент с цветом" }), t: tr2({ uz: "— fonni boshqa rangga o'zgartirib ko'ring", ru: "— попробуйте другой цвет фона" }) },
    { b: tr2({ uz: "Sahifa nomi", ru: "Имя страницы" }), t: tr2({ uz: "— `title` ichiga ismingizni yozing", ru: "— напишите своё имя в `title`" }) }
  ];
  const correct = SCORED_IDX.filter((i) => answers[i]?.correct).length;
  const total = SCORED_IDX.length;
  const PASSED = (total ? correct / total : 0) >= 0.6;
  return <Stage eyebrow={tr2({ uz: "Tayyor", ru: "Готово" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Keyingi dars →", ru: "Следующий урок →" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Endi sizda <span className="italic" style={{ color: T.accent }}>haqiqiy dasturchi quroli</span> bor.</>, ru: <>Теперь у вас есть <span className="italic" style={{ color: T.accent }}>настоящий инструмент разработчика</span>.</> })}</h2>{
  ...
  />
        ...
        <div className="card fade-up d3"><div className="card-lbl" style={{ color: T.success }}><span className="tick" style={{ width: 16, height: 16, borderRadius: "50%", background: T.success, color: "#fff", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 10 }}>✓</span> {tr2({ uz: "Endi siz bilasiz", ru: "Теперь вы умеете" })}</div><ul className="recap">{RECAP.map((r, i) => <li key={i} style={{ animationDelay: `${0.3 + i * 0.07}s` }}><span className="ck">✓</span><span>{r}</span></li>)}</ul></div>
       ...
       KO'RSATILMAYDI: uy ishi shaxsiy (sahna ↔ daftar tamoyili). */
        ...
        {!isMentorL && onHomework && <div className="hw-big-wrap fade-up d4">
            <button className="hw-big" onClick={onHomework}>
              <span className="hw-big-shine" aria-hidden="true" />
              <span className="hw-big-t">{tr2({ uz: "Uyga vazifa", ru: "Домашнее задание" })}</span>
              <span className="hw-big-s">{tr2({ uz: "Amaliy topshiriqni boshlash →", ru: "Начать практическое задание →" })}</span>
            </button>
          </div>}
...
var ScreenDemoIdea = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [picking, setPicking] = useState2(!hasSaved);
  const [sel, setSel] = useState2(hasSaved ? saved.manba : null);
  const [ownM, setOwnM] = useState2(hasSaved && saved.manba === "ozim" ? saved.muammo : "");
  const [ownY, setOwnY] = useState2(hasSaved && saved.manba === "ozim" ? saved.yechim : "");
  const [confirmed, setConfirmed] = useState2(!!(storedAnswer && storedAnswer.correct));
  ...
  const confirm = () => {
    ...
    if (picking) {
      if (sel === "ozim") demoWrite({ muammo: ownM.trim(), yechim: ownY.trim(), manba: "ozim", holat: "tasdiqlandi" });
        ...
        demoWrite({ muammo: tr2(b.muammo), yechim: tr2(b.yechim), manba: b.id, holat: "tasdiqlandi" });
      }
  ...
  const cur = demoRead();
  return <Stage eyebrow={tr2({ uz: "Demo Day loyihasi", ru: "Проект Demo Day" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!confirmed && !isMentorLive} label={confirmed || isMentorLive ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Loyihani tasdiqlang", ru: "Подтвердите проект" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Demo Day uchun <span className="italic" style={{ color: T.accent }}>g'oyangiz</span> tayyormi?</>, ru: <>Готова ли ваша <span className="italic" style={{ color: T.accent }}>идея</span> для Demo Day?</> })}</h2></div>
        ...
        {isMentorLive ? <MentorWorkStats live={live} screenIdx={screen} taskLabel={tr2({ uz: "Demo Day loyihasini aytish", ru: "Назвать проект Demo Day" })} /> : <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 8, maxWidth: 680 }}>
          {!picking && hasSaved && <>
              ...
              {!confirmed && <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <button className="btn" style={{ background: T.success }} onClick={confirm}>{tr2({ uz: "✓ Shu — mening loyiham", ru: "✓ Это мой проект" })}</button>
                  <button className="chip" onClick={() => {
    setPicking(true);
    setSel(null);
  }}>{tr2({ uz: "Boshqasini tanlayman", ru: "Выберу другой" })}</button>
                </div>}
            </>}
          {picking && <>
              <p className="body" style={{ margin: 0, color: T.ink2, fontSize: 14 }}>{tr2({ uz: "Qoida bitta: tayyor brendning saytini ko'chirmaymiz — o'z atrofingizdagi muammoni tanlang.", ru: "Правило одно: не копируем сайт готового бренда — выберите проблему вокруг себя." })}</p>
              {
    ...
    return <button key={b.id} className={"hook-option " + (on ? "on" : "")} onClick={() => setSel(b.id)}>
              ...
              {sel === "ozim" && <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 8, paddingLeft: 6 }}>
                  <input className="text-input" value={ownM} onChange={(e) => setOwnM(e.target.value)} placeholder={tr2({ uz: "Qaysi muammo?", ru: "Какая проблема?" })} />
                  <input className="text-input" value={ownY} onChange={(e) => setOwnY(e.target.value)} placeholder={tr2({ uz: "Sayt nima qiladi?", ru: "Что сделает сайт?" })} />
                </div>}
              {!confirmed && <button className="btn" disabled={!ready} style={{ background: ready ? T.success : void 0, alignSelf: "flex-start", opacity: ready ? 1 : 0.5 }} onClick={confirm}>{tr2({ uz: "✓ Tasdiqlash", ru: "✓ Подтвердить" })}</button>}
            </>}
          {confirmed && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Tasdiqlandi. Deploy darsida aynan shu loyihani internetga chiqarasiz.</>, ru: <>Подтверждено. На уроке деплоя вы выложите в интернет именно этот проект.</> })}</p></div>}
        </div>}
      </div>
    </Stage>;
};
var ScreenPodium = ({ screen, answers, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bugungi <span className="italic" style={{ color: T.accent }}>g'oliblarimiz</span>.</>, ru: <>Наши <span className="italic" style={{ color: T.accent }}>победители</span> сегодня.</> })}</h2></div>
    ...
    return <div key={q} className="qstat-row">
              ...
              {live.mode === "mentor" && <p className="small" style={{ margin: "10px 0 0", color: T.ink2 }}>{tr2({ uz: "⚠️ belgili savollar — sinf qiynalgan mavzular. Ularni yana bir bor tushuntiring.", ru: "Вопросы со значком ⚠️ — темы, где класс споткнулся. Объясните их ещё раз." })}</p>}
            </div>
          </>}
      </div>
    </Stage>;
...
`
  ...
  requirements: [
    { id: "bg", label: { uz: ".card — yashil fon (background)", ru: ".card — зелёный фон (background)" }, check: checks.custom((x) => {
      ...
      if (!rule) return tr2({ uz: "style.css'da `.card { ... }` qoidasini oching", ru: "Откройте правило `.card { ... }` в style.css" });
      const v = rule.props["background"] || rule.props["background-color"];
      if (!v) return tr2({ uz: "`.card` ichiga `background` xossasini yozing", ru: "Напишите свойство `background` внутри `.card`" });
      return isGreenish(v) ? true : tr2({ uz: "Rang yashil oilasidan bo'lsin — masalan `#3FAE2A`", ru: "Возьмите цвет из зелёной семьи — например `#3FAE2A`" });
    }) },
    ...
    { id: "center", label: { uz: ".card — markazda (width + margin auto)", ru: ".card — по центру (width + margin auto)" }, check: checks.custom((x) => {
      const rule = x.cssRules.find((r) => r.selector.split(",").map((s) => s.trim()).includes(".card"));
      if (!rule) return tr2({ uz: "style.css'da `.card { ... }` qoidasini oching", ru: "Откройте правило `.card { ... }` в style.css" });
      if (!rule.props["width"]) return tr2({ uz: "Avval `width` bering — masalan `260px`", ru: "Сначала задайте `width` — например `260px`" });
      const m = [rule.props["margin"], rule.props["margin-left"], rule.props["margin-right"]].filter(Boolean).join(" ");
      return m.includes("auto") ? true : tr2({ uz: "Markazlash uchun `margin: 40px auto` yozing", ru: "Для центрирования напишите `margin: 40px auto`" });
    }) },
...
function VsCodeLesson({ lang: langProp, onFinished, onPractice }) {
  ...
  return <LangContext.Provider value={lang}>
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        ...
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={tr2({ uz: "1-Modul", ru: "Модуль 1" })} /> : <>
    ...
    lang={__lang2}
    task={practice.task}
    starterCode={practice.starter ? tr2(practice.starter) : void 0}
