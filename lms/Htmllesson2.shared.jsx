...
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx src/1-Modull/Htmllesson2.jsx
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>Sayt faqat <span className="italic" style={{ color: T.accent }}>matndan</span> iboratmi?</>, ru: <>Сайт состоит только из <span className="italic" style={{ color: T.accent }}>текста</span>?</> })}</h1>
      ...
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <div className="col">
            <div className="ai-card fade-up delay-2">
              ...
              <div className="ai-code">
                ...
                <div className={`ai-line ${flashLine === 1 ? "bad" : ""}`} onClick={() => pickWrong(1)}><span className="tg">&lt;h1&gt;</span>{tr2({ uz: "Mening mushugim", ru: "Мой кот" })}<span className="tg">&lt;/h1&gt;</span></div>
              </div>
              {!found && <p className="ai-prompt">{wrongMsg ? tr2({ uz: "Bu qatorda xato yo'q — yana qarang.", ru: "В этой строке ошибки нет — посмотрите ещё раз." }) : tr2({ uz: "Rasm nega ko'rinmayapti? Xato qatorni bosing.", ru: "Почему картинка не видна? Нажмите на строку с ошибкой." })}</p>}
          ...
          <div className="col">
            ...
            {!found && <div className="hint"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: "Sahifada nima ko'rinmayapti? Shuni chiqaradigan qatorni kodda toping.", ru: "Что не видно на странице? Найдите в коде строку, которая это выводит." })}</p></div>}
