...
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/e069aaaa4d94bd163588a759a75419dc.jsx src/2-Modull/PeanStackLesson.jsx
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и число ✅/❌ скрыты — после «Открыть результат» всё появится сразу и у Вас, и на экранах учеников." })}</p>}
    ...
    return <button key={i} className={cls} disabled={solved || isMentorLive} onClick={() => pick(i)} style={{ padding: "clamp(13px,1.9vw,17px) clamp(15px,2.2vw,20px)", fontSize: "clamp(15px,1.85vw,17px)", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <FeedbackBlock show={isMentorLive ? mReveal : picked !== null} isCorrect={isMentorLive ? true : solved && !wrongLocked} neutral={waiting}>
          ...
          <p className="body" style={{ margin: 0 }}>
            {fmtCode(isMentorLive ? explainCorrect : waiting ? tr({ uz: "Hozir to'g'ri javobni bilib olasiz.", ru: "Сейчас узнаете правильный ответ." }) : wrongLocked ? explainWrong[picked] ?? explainWrong.default : solved ? explainCorrect : explainWrong[picked] ?? explainWrong.default)}
          </p>
          {
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Kirish", ru: "Вступление" })} screen={screen} navContent={<NavNext optionalLive disabled={picked === null} label={tr({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        ...
        <Split>
          <Col>
            <BWindow minH={170}>
              {spinning ? <div className="bw-spin"><span className="spinner" /> {tr({ uz: "yuklanmoqda…", ru: "загрузка…" })}</div> : <>
                  <p className="bw-h">{tr({ uz: "Mening saytim", ru: "Мой сайт" })}</p>
                  <p className="bw-sub">{tr({ uz: "Izohlar", ru: "Комментарии" })} ({comments.length}):</p>
                  {comments.length === 0 ? <p className="bw-empty">{lost ? tr({ uz: "Izohlar yo'q… hammasi yo'qoldi!", ru: "Комментариев нет… всё пропало!" }) : tr({ uz: "Hozircha izoh yo'q", ru: "Пока нет комментариев" })}</p> : comments.map((c, i) => <div key={i} className="cmt el-in">
            ...
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              <button className="btn-soft" onClick={refresh} disabled={comments.length === 0 || spinning} style={{ boxShadow: comments.length > 0 && !lost ? `0 0 0 1.5px ${T.accent}` : void 0 }}>{tr({ uz: "Sahifani yangilash ⟳", ru: "Обновить страницу ⟳" })}</button>
            </div>
          </Col>
          <Col>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Frontend va Backend", ru: "Frontend и Backend" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Ikkala xonani ko'ring", ru: "Посмотрите обе комнаты" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            {cur ? <div className="sk-info fade-step" key={active}>
              ...
              </div> : !isNarrow ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Xonani bosing", ru: "Нажмите на комнату" })}</p></div> : null}
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="React" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${added.length}/3 ${tr({ uz: "blokni qo'shing", ru: "блока добавьте" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen4 = (props) => <QuestionScreen
  ...
  options={["Backend", "Frontend", tr({ uz: "Baza (database)", ru: "База (database)" }), tr({ uz: "Server", ru: "Сервер" })]}
  correctIdx={1}
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="Node.js" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Ikkala joyda ishlating", ru: "Запустите в обоих местах" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            {place ? <div className="demo-swap" key={place} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              </div> : !isNarrow ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Joyni tanlang", ru: "Выберите место" })}</p></div> : null}
...
var Screen5b = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "Sahifani bezaydi (`CSS` kabi)", ru: "Украшает страницу (как `CSS`)" }), tr({ uz: "`JavaScript`'ni serverda ishlatadi", ru: "Запускает `JavaScript` на сервере" }), tr({ uz: "Rasmlarni tahrirlab beradi", ru: "Редактирует картинки" }), tr({ uz: "Internet tezligini oshiradi", ru: "Ускоряет интернет" })]}
  correctIdx={1}
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="Express" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${seen.size}/2 ${tr({ uz: "buyurtma bering", ru: "заказа сделайте" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            {cur ? <div className="demo-swap" key={order} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              </div> : !isNarrow ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Taomni tanlang", ru: "Выберите блюдо" })}</p></div> : null}
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="PostgreSQL" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : rows.length < 2 ? tr({ uz: "Buyurtma qo'shing", ru: "Добавьте заказ" }) : tr({ uz: "Serverni o'chirib-yoqing", ru: "Перезапустите сервер" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              <button className="btn-soft" onClick={restart} disabled={rows.length === 0 || spinning} style={{ boxShadow: rows.length >= 2 && !restarted ? `0 0 0 1.5px ${T.accent}` : void 0 }}>{tr({ uz: "Serverni o'chirib-yoqish ⏻", ru: "Перезапустить сервер ⏻" })}</button>
            </div>
          ...
          <Col>
            ...
            <div className="dbt fade-up delay-2" style={{ opacity: spinning ? 0.35 : 1 }}>
              ...
              {rows.length === 0 ? <div className="dbt-empty">{tr({ uz: "bo'sh jadval…", ru: "пустая таблица…" })}</div> : rows.map((r) => <div key={r.id} className="dbt-row el-in"><span className="mono">{r.id}</span><span>{tr(r.item)}</span><span>{r.who}</span></div>)}
            </div>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "To'liq sayohat", ru: "Полное путешествие" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Sayohatni boshlang", ru: "Запустите путешествие" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen9 = (props) => <QuestionScreen
  ...
  options={[tr({ uz: "Brauzerda", ru: "В браузере" }), tr({ uz: "Telefon ekranida", ru: "На экране телефона" }), tr({ uz: "`PostgreSQL` bazasida", ru: "В базе `PostgreSQL`" }), tr({ uz: "`CSS` faylida", ru: "В файле `CSS`" })]}
  correctIdx={2}
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "PERN jamoasi", ru: "Команда PERN" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${seen.size}/4 ${tr({ uz: "a'zoni taniqing", ru: "участника изучите" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            {cur ? <div className="sk-info fade-step" key={active}>
              ...
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "A'zoni bosing", ru: "Нажмите на участника" })}</p></div>}
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Vazifa taqsimoti", ru: "Распределение задач" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : `${tr({ uz: "Vazifa", ru: "Задача" })} ${Math.min(idx + 1, TASKS.length)}/${TASKS.length}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Amaliyot · stack yig'ish", ru: "Практика · сборка стека" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Stackni yig'ing", ru: "Соберите стек" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="Debugging" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : found ? tr({ uz: "Endi tuzating", ru: "Теперь исправьте" }) : tr({ uz: "Xatoni toping", ru: "Найдите ошибку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
    ...
    /* F-0803-26 (109-qonun): boshlang'ich «Eslang…» qutisi olib tashlandi — u xato qatorni
       ...
       noto'g'ri bosilganda chiqadi (110-qonun: har bosishga ko'rinadigan javob). */
            ...
            {!found && picked !== null && <div className="frame-warn fade-step" key={picked}><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: <>Bu qator to'g'ri — rol o'z egasida. Yana o'ylang: <b>chizish</b> kimning ishi edi, <b>saqlash</b> kimning?</>, ru: <>Эта строка верная — роль на своём месте. Подумайте ещё: <b>рисовать</b> — чья это работа, а <b>хранить</b> — чья?</> })}</p></div>}
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!solved} label={solved ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Sayohatni tuzing", ru: "Соберите путь" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var ScreenFlashcards = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Takrorlash", ru: "Повторение" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={false} label={tr({ uz: "Yakunlash →", ru: "Завершить →" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: "Dars tugadi", ru: "Урок пройден" })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Katta saytlarning <span className="italic" style={{ color: T.accent }}>xaritasi</span> endi qo'lingizda.</>, ru: <>Теперь <span className="italic" style={{ color: T.accent }}>карта</span> больших сайтов у Вас в руках.</> })}</h2>{
