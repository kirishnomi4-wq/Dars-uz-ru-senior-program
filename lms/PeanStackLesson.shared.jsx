...
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx src/2-Modull/PeanStackLesson.jsx
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const [spinning, setSpinning] = useState3(false);
  const [lost, setLost] = useState3(false);
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Вступление" })} screen={screen} navContent={<NavNext optionalLive disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>Izohingiz <span className="italic" style={{ color: T.accent }}>qayoqqa</span> yo'qoldi?</>, ru: <>Куда <span className="italic" style={{ color: T.accent }}>исчез</span> Ваш комментарий?</> })}</h1>
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [place, setPlace] = useState3(null);
  const [seen, setSeen] = useState3(/* @__PURE__ */ new Set());
  const isNarrow = useIsMobile(768);
  const done = seen.size >= 2;
  const tap = (k) => {
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [order, setOrder] = useState3(null);
  const [seen, setSeen] = useState3(/* @__PURE__ */ new Set());
  const isNarrow = useIsMobile(768);
  const done = seen.size >= 2;
  const tap = (k) => {
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [spinning, setSpinning] = useState3(false);
  const [restarted, setRestarted] = useState3(false);
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [solvedAll, setSolvedAll] = useState3(false);
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [assign, setAssign] = useState3({});
  const [activeSlot, setActiveSlot] = useState3("view");
  const [checked, setChecked] = useState3(false);
  const [allOk, setAllOk] = useState3(false);
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [picked, setPicked] = useState3(storedAnswer ? "pg" : null);
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [solved, setSolved] = useState3(!!storedAnswer?.solved);
  const achMiss = useContext2(AchMissCtx);
  const firstCorrectRef = useRef3(storedAnswer ? storedAnswer.firstAttemptCorrect ?? storedAnswer.correct ?? null : achMiss && achMiss.missed.has(SCREEN_META[screen].id) ? false : null);
  const items = useMemo(() => STEPS.map((s) => ({ id: s.key, label: s.label })), []);
  const onWrong = () => {
