...
var Navbar = ({ flex = true, dir = "row", justify = "space-between", align = "center", gap = 10, snap = false }) => {
  ...
  return <div style={{ display: flex ? "flex" : "block", flexDirection: dir, justifyContent: justify, alignItems: align, gap, background: "#fff", borderRadius: 10, padding: "9px 12px", transition: "all 0.4s cubic-bezier(.34,1.1,.4,1)", boxShadow: "inset 0 0 0 1px rgba(0,0,0,0.05)" }}>
      ...
      <div className={itemCls} style={{ ...iStyle(1), gap: 11 }}><span style={{ display: "inline-flex", gap: 11 }}>{[{ uz: "Asosiy", ru: "Главная" }, { uz: "Darslar", ru: "Уроки" }].map((x, i) => <span key={i} style={{ fontFamily: "'Manrope'", fontWeight: 600, fontSize: 12, color: T.ink2 }}>{tr2(x)}</span>)}</span></div>
      <div className={itemCls} style={iStyle(2)}><span style={{ background: T.accent, color: "#fff", borderRadius: 7, padding: "5px 11px", fontFamily: "'Manrope'", fontWeight: 700, fontSize: 12, display: "inline-block", whiteSpace: "nowrap" }}>{tr2({ uz: "Kirish", ru: "Войти" })}</span></div>
    </div>;
};
var AxisDemo = ({ justify = "flex-start", align = "stretch", dir = "row", varied = false, gap = 12, axis = null }) => {
  ...
  return <div className="axis-stage">
      {axis === "main" && <div className="axis-main"><span className="axis-head">{tr2({ uz: "asosiy o'q (justify-content)", ru: "главная ось (justify-content)" })}</span><span className="axis-line"><span className="axis-bead" /><span className="axis-tip">▶</span></span></div>}
      {axis === "cross" ? <div className="axis-crosswrap">
          <div className="axis-cross"><span className="axis-vline"><span className="axis-bead v" /><span className="axis-tip v">▼</span></span><span className="axis-head v">{tr2({ uz: "ko'ndalang o'q", ru: "поперечная ось" })}</span></div>
          {boxes}
        </div> : boxes}
    </div>;
};
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const [row, setRow] = useState2(false);
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up" style={{ maxWidth: 760 }}>{tr2({ uz: <>Tugmalar nega <span className="italic" style={{ color: T.accent }}>yonma-yon</span> turmaydi?</>, ru: <>Почему кнопки не встают <span className="italic" style={{ color: T.accent }}>в ряд</span>?</> })}</h1>
        ...
        <Split>
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8 }}>
              <button className={`chip ${!row ? "chip-on" : ""}`} onClick={() => setRow(false)}>{tr2({ uz: "Ustma-ust", ru: "Друг под другом" })}</button>
              <button className={`chip ${row ? "chip-on" : ""}`} onClick={() => setRow(true)}>✨ {tr2({ uz: "Yonma-yon", ru: "В ряд" })}</button>
            </div>
            ...
            <p className="mono small" style={{ color: T.ink3, margin: 0, textAlign: "center" }}>{row ? tr2({ uz: "✨ display: flex — menyu bir qatorda", ru: "✨ display: flex — меню в одну строку" }) : tr2({ uz: "Sukut bo'yicha — ustma-ust (block)", ru: "По умолчанию — друг под другом (block)" })}</p>
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Elementlarni qatorga nima tizadi?", ru: "Что выстраивает элементы в ряд?" })}</p>
    ...
    return <button key={o.id} className={`hook-option ${sel ? "on" : ""}`} disabled={picked !== null} onClick={() => pick(o.id)}><span className="radio">{sel && <span className="radio-dot" />}</span><span>{tr2(o.label)}</span></button>;
            ...
            {picked !== null && <p className="hook-ack fade-step">{tr2({ uz: "Aynan! CSS Flexbox elementlarni qatorga tizadi va joylashtiradi. Bugun shuni o'rganamiz.", ru: "Именно! CSS Flexbox выстраивает элементы в ряд и расставляет их. Сегодня этим и займёмся." })}</p>}
          </Col>
        </Split>
        </Zoomable>
      </div>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const PreviewBlock = <Col>
      <p className="flow-label">{tr2({ uz: "Manzil — dars oxirida shu menyuni yasaysiz", ru: "Цель — в конце урока вы соберёте это меню" })}</p>
      <Zoomable>
      <Preview title="coddy.uz" minH={200}>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <span style={{ alignSelf: "center", fontFamily: "'Manrope', sans-serif", fontWeight: 600, fontSize: 11.5, color: T.accent, background: T.accentSoft, padding: "3px 12px", borderRadius: 99 }}>{tr2({ uz: "↑ Logo · havolalar · Kirish — yonma-yon menyu", ru: "↑ Лого · ссылки · Войти — меню в один ряд" })}</span>
        </div>
      </Preview>
      </Zoomable>
    </Col>;
  ...
  return <Stage eyebrow={tr2({ uz: "Reja", ru: "План" })} screen={screen} audioState={audio} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up"><span className="italic" style={{ color: T.accent }}>{tr2({ uz: "Bugun chinakam sayt menyusini yasaymiz!", ru: "Сегодня сделаем настоящее меню сайта!" })}</span></h2></div>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [mode, setMode] = useState2("block");
  ...
  return <Stage eyebrow="Block / inline" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Ikkalasini ko'ring", ru: "Посмотрите оба" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Nega menyu <span className="italic" style={{ color: T.accent }}>ustma-ust</span> tushdi?</>, ru: <>Почему меню свалилось <span className="italic" style={{ color: T.accent }}>в столбик</span>?</> })}</h2></div>
        ...
        <div className="split">
          <div className="col">
            ...
            <Preview title="element.html" minH={150}>
              {mode === "block" ? <div key="b" className="demo-swap"><p className="bi-cap">{tr2({ uz: <>Har biri <b>butun qatorni</b> egallaydi — shuning uchun ustma-ust ↓</>, ru: <>Каждый занимает <b>всю строку</b> — поэтому друг под другом ↓</> })}</p>{["A", "B", "C"].map((t, i) => <div key={i} className="bi-block" style={{ animationDelay: `${i * 0.08}s` }}><span><span className="bi-tag">&lt;div&gt;</span> {tr2({ uz: "Blok", ru: "Блок" })} {t}</span><span className="bi-full">↔ 100%</span></div>)}</div> : <div key="i" className="demo-swap"><p className="bi-cap">{tr2({ uz: <>Faqat <b>o'z kengligini</b> egallaydi — matn ichida yonma-yon ↓</>, ru: <>Занимает только <b>свою ширину</b> — стоит в строке текста ↓</> })}</p><div style={{ lineHeight: 2.6, fontFamily: "'Georgia, serif'", fontSize: 15, color: T.ink }}>{tr2({ uz: "Bu matn ichida", ru: "В этом тексте" })} {["span A", "span B", "span C"].map((t, i) => <span key={i} className="bi-inline" style={{ animationDelay: `${i * 0.08}s` }}>{t}</span>)} {tr2({ uz: "yonma-yon turibdi.", ru: "стоят рядом." })}</div></div>}
            </Preview>
          </div>
          <div className="col">
            <div className={mode === "block" ? "frame-soft fade-step" : "frame-success fade-step"} key={mode}><p className="body" style={{ margin: 0, color: T.ink }}>{mode === "block" ? tr2({ uz: <><b>block</b> — har biri to'liq qatorni egallaydi, shuning uchun ustma-ust tushadi. div, h1, p, ul — barchasi block.</>, ru: <><b>block</b> — каждый занимает всю строку, поэтому они встают друг под другом. div, h1, p, ul — все block.</> }) : tr2({ uz: <><b>inline</b> — faqat o'z kengligini egallaydi, shuning uchun yonma-yon turadi. span, a, strong — inline.</>, ru: <><b>inline</b> — занимает только свою ширину, поэтому стоит рядом. span, a, strong — inline.</> })}</p></div>
            <div className="frame-soft"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <><b>Muammo:</b> menyu bo'laklari (div) block bo'lgani uchun ustma-ust tushdi. Ularni yonma-yon qilish uchun — <b>Flexbox</b> kerak. Keyingi qadam!</>, ru: <><b>Проблема:</b> части меню (div) — block, поэтому они свалились друг под друга. Чтобы поставить их в ряд, нужен <b>Flexbox</b>. Следующий шаг!</> })}</p></div>
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [flex, setFlex] = useState2(!!storedAnswer);
  const [part, setPart] = useState2(null);
  const [seen, setSeen] = useState2(() => new Set(storedAnswer ? ["box", "kid"] : []));
  const isNarrow = useIsMobile(768);
  const done = flex;
  const tap = (k) => {
    setPart(k);
  ...
  return <Stage eyebrow="display: flex" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "flex'ni yoqing", ru: "Включите flex" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Elementlarni qatorga qanday <span className="italic" style={{ color: T.accent }}>tizamiz</span>?</>, ru: <>Как <span className="italic" style={{ color: T.accent }}>выстроить</span> элементы в ряд?</> })}</h2></div>
        ...
        <div className="split">
          <div className="col">
            <button className={`btn ${!flex ? "btn-pulse" : ""}`} style={{ alignSelf: "flex-start" }} onClick={() => setFlex((f) => !f)}>{flex ? tr2({ uz: "↩ flex'ni o'chirish", ru: "↩ выключить flex" }) : tr2({ uz: "🎯 display: flex yoqish", ru: "🎯 включить display: flex" })}</button>
            ...
            {part && <div className="sk-info fade-step" key={part}><span className="sk-tagbig"><span className="sk-wordbadge">{tr2(PART[part].lbl)}</span></span><p className="body" style={{ color: T.ink, margin: "10px 0 0" }}>{tr2(PART[part].txt)}</p></div>}
          </div>
          <div className="col">
            <div className="flow-label">{tr2({ uz: "Natija — bosib o'rganing", ru: "Результат — нажимайте и изучайте" })}</div>
            <Preview title="qator.html" minH={120}>
              <div className={`cdiag ${part === "box" ? "on" : ""} ${flex && part === null ? "tapme" : ""}`} onClick={() => tap("box")} title="konteyner">
                <span className="cdiag-tag">{tr2({ uz: ".row (konteyner)", ru: ".row (контейнер)" })}</span>
    ...
    tap("kid");
            ...
            <div className={flex ? "frame-success fade-step" : "hint"}><p className="body" style={{ margin: 0, color: T.ink }}>{flex ? <>✓ <span className="mono">display: flex</span> — {tr2({ uz: "uchala element bir qatorga tizildi!", ru: "все три элемента встали в один ряд!" })} {seen.size < 2 && tr2({ uz: "Endi konteyner va ichki elementni bosib, farqini ko'ring.", ru: "Теперь нажмите на контейнер и внутренний элемент — увидите разницу." })}</> : tr2({ uz: <>Hozir elementlar ustma-ust (block). <span className="mono">display: flex</span> ularni qatorga tizadi.</>, ru: <>Сейчас элементы друг под другом (block). <span className="mono">display: flex</span> выстроит их в ряд.</> })}</p></div>
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen3b = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Qoida ustaxonasi", ru: "Мастерская правила" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "qoidani yig'ing", ru: "Соберите правило" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <><span className="italic" style={{ color: T.accent }}>display: flex</span> qoidasini o'zingiz yig'ing</>, ru: <>Соберите правило <span className="italic" style={{ color: T.accent }}>display: flex</span> сами</> })}</h2></div>
        ...
        <div className="split">
          <div className="col">
            <div className="flow-label">{tr2({ uz: "bo'laklarni tartibga soling", ru: "расставьте блоки по порядку" })}</div>
          ...
          <div className="col">
            <div className="flow-label">{tr2({ uz: "Natija — menyu preview", ru: "Результат — превью меню" })}</div>
            <Preview title="menyu.html" minH={120}><Navbar flex={flex} snap /></Preview>
            <div className={flex ? "frame-success fade-step" : "hint"}><p className="body" style={{ margin: 0, color: T.ink }}>{flex ? tr2({ uz: <>✓ Qoida to'g'ri! <span className="mono">display: flex</span> menyuni bir qatorga tizdi.</>, ru: <>✓ Правило верное! <span className="mono">display: flex</span> выстроил меню в один ряд.</> }) : tr2({ uz: <>Hozir menyu bo'laklari ustma-ust (block). Qoidani to'g'ri yig'ing — qatorga tiziladi.</>, ru: <>Сейчас пункты меню друг под другом (block). Соберите правило верно — они встанут в ряд.</> })}</p></div>
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen4 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Elementlarni <span className="italic" style={{ color: T.accent }}>yonma-yon</span> qatorga tizish uchun konteynerga qaysi xususiyat beriladi?</>, ru: <>Какое свойство задаётся контейнеру, чтобы выстроить элементы <span className="italic" style={{ color: T.accent }}>в ряд</span>?</> })}</h2></>}
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [dir, setDir] = useState2(storedAnswer?.dir || "row");
  const [touched, setTouched] = useState2(!!storedAnswer);
  const done = touched;
  const isNarrow = useIsMobile(768);
  const set = (d) => {
  ...
  return <Stage eyebrow="flex-direction" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Yo'nalishni almashtiring", ru: "Переключите направление" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Qatorga yoki <span className="italic" style={{ color: T.accent }}>ustunga</span>?</>, ru: <>В ряд или <span className="italic" style={{ color: T.accent }}>в столбик</span>?</> })}</h2></div>
        ...
        <div className="split">
          ...
          <div className="col">
            <div className="flow-label">{tr2({ uz: "Natija", ru: "Результат" })} <span className={`dir-badge ${dir}`}>{dir === "row" ? tr2({ uz: "→ qator", ru: "→ ряд" }) : tr2({ uz: "↓ ustun", ru: "↓ столбик" })}</span></div>
...
var Screen5b = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "Mustahkamlash", ru: "Закрепление" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Flex elementlarni <span className="italic" style={{ color: T.accent }}>vertikal ustunga</span> tizish uchun nima yoziladi?</>, ru: <>Что написать, чтобы выстроить flex-элементы <span className="italic" style={{ color: T.accent }}>в вертикальный столбик</span>?</> })}</h2></>}
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [gap, setGap] = useState2(storedAnswer ? storedAnswer.gap ?? 20 : 8);
  const [touched, setTouched] = useState2(!!storedAnswer);
  const done = touched;
  const isNarrow = useIsMobile(768);
  const set = (v) => {
  ...
  return <Stage eyebrow="gap" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Bo'shliqni o'zgartiring", ru: "Измените отступ" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Element orasini qanday <span className="italic" style={{ color: T.accent }}>ochamiz</span>?</>, ru: <>Как <span className="italic" style={{ color: T.accent }}>раздвинуть</span> элементы?</> })}</h2></div>
        ...
        <div className="split">
          <div className="col">
            <div className="fade-up delay-2" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>{GAPS.map((g) => <button key={g.v} className={`chip ${gap === g.v ? "chip-on" : ""} ${!touched ? "btn-pulse" : ""}`} onClick={() => set(g.v)}>{tr2(g.l)} ({g.v}px)</button>)}</div>
            {!isNarrow && <pre className="code-box fade-up delay-2" style={{ fontSize: "clamp(12px,1.7vw,14px)" }}><span style={{ color: CODE.tag }}>.row</span> {"{"}{"\n  "}<span style={{ color: CODE.attr }}>display</span>: <span style={{ color: CODE.str }}>flex</span>;{"\n  "}<span style={{ color: CODE.attr }}>gap</span>: <span style={{ color: CODE.str }}>{gap}px</span>;{"\n"}{"}"}</pre>}
          </div>
          <div className="col">
            <div className="flow-label">{tr2({ uz: "Natija (qizil — gap bo'shlig'i)", ru: "Результат (красным — отступ gap)" })}</div>
            <Preview title="gap.html" minH={150}><div className="gapviz" style={{ ["--g"]: `${gap}px` }}><FBOX flex gap={gap} /></div></Preview>
            <p className="mono small" style={{ color: T.ink3, margin: 0, textAlign: "center" }}>gap: {gap}px — {gap === 0 ? tr2({ uz: "har element orasida bo'shliq yo'q", ru: "между элементами нет отступа" }) : tr2({ uz: `har element orasida ${gap}px joy`, ru: `между элементами по ${gap}px` })}</p>
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [jc, setJc] = useState2(storedAnswer?.jc || "flex-start");
  const [solved, setSolved] = useState2(!!storedAnswer);
  const [burst, setBurst] = useState2(false);
  const done = solved;
  ...
  return <Stage eyebrow="justify-content" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Namunaga moslang", ru: "Подгоните под образец" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Menyuni <span className="italic" style={{ color: T.accent }}>namunaga</span> moslang</>, ru: <>Подгоните меню <span className="italic" style={{ color: T.accent }}>под образец</span></> })}</h2></div>
        ...
        <div className="split">
          <div className="col">
            <div className="flow-label">{tr2({ uz: "justify-content qiymatini tanlang", ru: "выберите значение justify-content" })}</div>
          ...
          <div className="col">
            <div className="flow-label">🎯 {tr2({ uz: "Namuna (shunga moslang)", ru: "Образец (подгоните под него)" })}</div>
            <div style={{ opacity: 0.5, filter: "grayscale(1)", border: "1.5px dashed " + T.ink3, borderRadius: 12, padding: 4 }}><Navbar justify={TARGET} /></div>
            <div className="flow-label" style={{ marginTop: 8 }}>{tr2({ uz: "Sizning menyungiz", ru: "Ваше меню" })}</div>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [ai, setAi] = useState2(storedAnswer?.ai || "center");
  const [touched, setTouched] = useState2(!!storedAnswer);
  const done = touched;
  const isNarrow = useIsMobile(768);
  const set = (v) => {
  ...
  return <Stage eyebrow="align-items" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Variantni sinang", ru: "Попробуйте варианты" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Elementlarni <span className="italic" style={{ color: T.accent }}>ko'ndalang o'q</span> bo'ylab tekislash</>, ru: <>Выравнивание элементов по <span className="italic" style={{ color: T.accent }}>поперечной оси</span></> })}</h2></div>
        ...
        <div className="split">
          ...
          <div className="col">
            <div className="flow-label">{tr2({ uz: "Natija (balandligi har xil)", ru: "Результат (высота разная)" })}</div>
...
var Screen9 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Flex elementlarni qator bo'ylab <span className="italic" style={{ color: T.accent }}>gorizontal markazga</span> joylashtirish uchun?</>, ru: <>Как разместить flex-элементы вдоль ряда <span className="italic" style={{ color: T.accent }}>по центру горизонтали</span>?</> })}</h2></>}
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [opened, setOpened] = useState2(!!storedAnswer);
  const done = opened;
  useEffect2(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  return <Stage eyebrow="DevTools · CSS" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Styles'ni oching", ru: "Откройте Styles" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Element CSS'ini qayerda <span className="italic" style={{ color: T.accent }}>ko'ramiz</span>?</>, ru: <>Где <span className="italic" style={{ color: T.accent }}>увидеть</span> CSS элемента?</> })}</h2></div>
        ...
        <div className="split">
          <div className="col">
            ...
            {!opened && <button className="btn btn-pulse" style={{ alignSelf: "flex-start" }} onClick={() => setOpened(true)}>🔍 {tr2({ uz: "Menyuni Inspect qilish", ru: "Inspect меню" })}</button>}
            {opened && <p className="mono small" style={{ color: T.accent, margin: 0 }}>↑ {tr2({ uz: "tanlangan element belgilandi", ru: "выбранный элемент подсвечен" })}</p>}
          </div>
          <div className="col">
            {opened ? <div className="fade-step">
                ...
                <div className="frame-soft"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Mana menyuning butun CSS'i! Styles paneli har bir elementning qoidalarini ko'rsatadi — saytlar qanday yasalganini shu yerdan o'rganasiz.", ru: "Вот весь CSS меню! Панель Styles показывает правила каждого элемента — здесь вы изучаете, как устроены сайты." })}</p></div>
              </div> : <div className="hint"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: <>Hozir faqat menyu ko'rinyapti. <b style={{ color: T.ink }}>Inspect</b> bossangiz, uning CSS qoidalari ochiladi.</>, ru: <>Сейчас видно только меню. Нажмите <b style={{ color: T.ink }}>Inspect</b> — откроются его CSS-правила.</> })}</p></div>}
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "DevTools · tahrir", ru: "DevTools · правка" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Qiymatni o'zgartiring", ru: "Измените значение" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>CSS'ni DevTools'da <span className="italic" style={{ color: T.accent }}>jonli</span> o'zgartiring</>, ru: <>Меняйте CSS в DevTools <span className="italic" style={{ color: T.accent }}>вживую</span></> })}</h2></div>
        ...
        <div className="split">
          <div className="col">
            <div className="flow-label">{tr2({ uz: "DevTools — Styles (bosib o'zgartiring)", ru: "DevTools — Styles (нажмите и меняйте)" })}</div>
...
var Screen12 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>DevTools'ning qaysi paneli elementning <span className="italic" style={{ color: T.accent }}>CSS qoidalarini</span> ko'rsatadi va o'zgartiradi?</>, ru: <>Какая панель DevTools показывает и меняет <span className="italic" style={{ color: T.accent }}>CSS-правила</span> элемента?</> })}</h2></>}
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Amaliyot · layout yig'", ru: "Практика · соберите макет" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Kamida 3 ta xususiyat (${count}/3)`, ru: `Минимум 3 свойства (${count}/3)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.4vw,14px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>O'zingiz <span className="italic" style={{ color: T.accent }}>layout yig'ing</span>.</>, ru: <>Соберите <span className="italic" style={{ color: T.accent }}>макет сами</span>.</> })}</h2></div>
        ...
        <div className="split">
          <div className="col">
            <p className="flow-label">{tr2({ uz: "Xususiyatlarni yoqing", ru: "Включите свойства" })}</p>
            ...
            {!isNarrow && <><div className="flow-label" style={{ marginTop: 4 }}>{tr2({ uz: "CSS kodi", ru: "CSS-код" })}</div>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="Debugging" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : found ? tr2({ uz: "Endi tuzating", ru: "Теперь исправьте" }) : tr2({ uz: "Xatoni toping", ru: "Найдите ошибку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Menyu yonma-yon bo'lmadi — <span className="italic" style={{ color: T.accent }}>nega</span>?</>, ru: <>Меню не встало в ряд — <span className="italic" style={{ color: T.accent }}>почему</span>?</> })}</h2></div>
        ...
        <div className="split">
          <div className="col">
            <div className="ai-card fade-up delay-2">
              <div className="ai-row"><span className="ai-badge">AI</span><span className="ai-bubble">{tr2({ uz: "Menyuni markazga qo'ydim! (lekin ustma-ust 🤔)", ru: "Я поставил меню по центру! (но оно в столбик 🤔)" })}</span></div>
              ...
              {!found && <p className="ai-prompt">{tr2({ uz: "Qaysi qator xato? Bosing.", ru: "Какая строка с ошибкой? Нажмите." })}</p>}
              {found && !fixed && <button className="btn fade-step btn-pulse" style={{ alignSelf: "flex-start" }} onClick={fix}>🔧 {tr2({ uz: "display: flex ga tuzatish", ru: "Исправить на display: flex" })}</button>}
              {fixed && <p className="ai-prompt" style={{ color: T.success, fontStyle: "normal", fontWeight: 600 }}>✓ {tr2({ uz: "Tuzatildi — endi flex ishlaydi!", ru: "Исправлено — теперь flex работает!" })}</p>}
            </div>
          </div>
          <div className="col">
            <div className="flow-label">{tr2({ uz: "Natija", ru: "Результат" })}</div>
            <Preview title="menyu.html" minH={120}><Navbar flex={fixed} justify="center" /></Preview>
            {!found && !isNarrow && <div className="hint"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: <>Menyu ustma-ust — flex ishlamayapti. <span className="mono">justify-content</span> faqat <span className="mono">display: flex</span> bo'lganda ishlaydi. Qaysi qatorda muammo?</>, ru: <>Меню в столбик — flex не работает. <span className="mono">justify-content</span> работает только при <span className="mono">display: flex</span>. В какой строке проблема?</> })}</p></div>}
            {found && !fixed && <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.accent }}>✓ {tr2({ uz: "Topdingiz!", ru: "Нашли!" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <><span className="mono">display: block</span> — flex emas! Shuning uchun justify-content e'tiborsiz qoldi. <span className="mono">flex</span> ga tuzating →</>, ru: <><span className="mono">display: block</span> — это не flex! Поэтому justify-content был проигнорирован. Исправьте на <span className="mono">flex</span> →</> })}</p></div>}
            {fixed && <div className="takeaway fade-step"><div className="ta-bulb">🛠️</div><p className="ta-h">{tr2({ uz: "display: flex — hammasining kaliti!", ru: "display: flex — ключ ко всему!" })}</p><p className="ta-sub">{tr2({ uz: "justify/align/gap faqat flex konteynerda ishlaydi", ru: "justify/align/gap работают только во flex-контейнере" })}</p></div>}
          </div>
        </div>
      </div>
    </Stage>;
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Tayyor", ru: "Готово" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Keyingi dars", ru: "Следующий урок" })} →</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "CSS 2-dars tugadi", ru: "Урок CSS 2 завершён" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>joylashtira</span> olasiz.</>, ru: <>Теперь вы умеете <span className="italic" style={{ color: T.accent }}>расставлять</span>.</> })}</h2>{
  ...
  />
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>🧩 {tr2({ uz: "Uyga vazifa", ru: "Домашнее задание" })}</div><p className="body" style={{ margin: "0 0 10px", color: T.ink }}>{tr2({ uz: "Saytingiz menyusini Flexbox bilan yasang:", ru: "Сделайте меню своего сайта на Flexbox:" })}</p><ul>{HOMEWORK.map((h, i) => <li key={i}><b>{tr2(h.b)}</b> <span className="t">{tr2(h.t)}</span></li>)}</ul><p className="hw-note">{tr2({ uz: "Flexbox — deyarli har bir zamonaviy saytda ishlatiladi!", ru: "Flexbox используется почти на каждом современном сайте!" })}</p></div>}
        {
