...
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/4340b314d6398ef53f3db530789e9e0e.jsx src/2-Modull/JsFunctionsLesson.jsx
    ...
    return <button key={i} className={cls} disabled={solved || isMentorLive} onClick={() => pick(i)} style={{ padding: "clamp(13px,1.9vw,17px) clamp(15px,2.2vw,20px)", fontSize: "clamp(15px,1.85vw,17px)", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <FeedbackBlock show={isMentorLive ? mReveal : picked !== null} isCorrect={isMentorLive ? true : solved && !wrongLocked} neutral={waiting}>
          <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: waiting ? T.blue : isMentorLive || solved && !wrongLocked ? T.success : T.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>
            {isMentorLive ? fmtCode(`✓ ${tr({ uz: "To'g'ri javob:", ru: "Верный ответ:" })} ${String.fromCharCode(65 + correctIdx)} — ${options[correctIdx]}`) : waiting ? tr({ uz: "📨 Javobingiz qabul qilindi", ru: "📨 Ваш ответ принят" }) : wrongLocked ? fmtCode(`${tr({ uz: "To'g'ri javob:", ru: "Верный ответ:" })} ${String.fromCharCode(65 + correctIdx)} — ${options[correctIdx]}`) : solved ? tr({ uz: "To'g'ri", ru: "Верно" }) : tr({ uz: "Qaytadan urinib ko'ring", ru: "Попробуйте ещё раз" })}
          </p>
...
var Terminal = ({ lines, empty = { uz: "// natija shu yerda chiqadi…", ru: "// результат появится здесь…" }, title = "console" }) => <div className="term">
    ...
    <div className="term-body">
      {lines.length === 0 ? <p className="term-empty">{tr(empty)}</p> : lines.map((l, i) => <div key={i} className="term-line"><span className="term-arrow">›</span><span>{tr(l)}</span></div>)}
    </div>
  </div>;
        ...
        <Split>
          <Col>
            ...
            <div className="msg-list fade-up delay-1">
              {count === 0 ? <p style={{ color: T.ink3, fontStyle: "italic", margin: 0, fontFamily: "'JetBrains Mono',monospace", fontSize: 13 }}>{tr({ uz: "// hali bitta urish ham hisoblanmadi", ru: "// пока не посчитан ни один" })}</p> : Array.from({ length: count }).map((_, i) => <div key={i} className="msg-line el-in"><span className="msg-ok">⚔️</span><span>{tr({ uz: <>{i + 1}-urish — 4 amal qo'lda bajarildi</>, ru: <>Удар #{i + 1} — 4 действия вручную</> })}</span></div>)}
            </div>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(/* @__PURE__ */ new Set(["manual"]));
        ...
        <div className="split">
          <Col>
            ...
            {part ? <div className="sk-info fade-step" key={part}>
              ...
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "👆 Koddagi 3 ta rangli qismni bosing", ru: "👆 Нажмите на 3 цветные части в коде" })}</p></div>}
          </Col>
          <Col>
            {
        ...
        <div className="split">
          ...
          <Col>
            {active ? <div className="sk-info fade-step" key={active}>
              ...
              </div> : !isNarrow ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Bir kartani bosing", ru: "Нажмите на одну из карточек" })}</p></div> : null}
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(/* @__PURE__ */ new Set(["5-3"]));
  const done = seen.size >= 2;
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [step, setStep] = useState(storedAnswer ? PRAC_STEPS.length : 0);
  ...
  return <Stage eyebrow={{ uz: "Amaliyot · kod yozing", ru: "Практика · пишем код" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!allDone} label={allDone ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `${step}/${PRAC_STEPS.length} bosqich yozildi`, ru: `Написано шагов: ${step}/${PRAC_STEPS.length}` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.4vw,14px)" }}>
        ...
        <div className="fade-up delay-1" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {PRAC_STEPS.map((s, i) => <span key={i} className="tagpill" style={{ opacity: i < step ? 1 : i === step ? 0.9 : 0.4, background: i < step ? T.successSoft : T.paper, color: i < step ? T.success : T.ink }}>{i < step ? "✓" : "✏️"} {tr(s.lbl)}</span>)}
        </div>
        <Zoomable>
