...
//  Qayta yig'ish:  node scripts/build-lms.mjs src/2-Modull/JsFunctionsLesson.jsx
      ...
      <div className="stage">
        <div className="stage-header" style={{ paddingLeft: padH, paddingRight: padH }}>
          ...
          <div className="chrome">
            <div className="chrome-left eyebrow"><span className="dot" /><span>{tr3(eyebrow)}</span></div>
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr3({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlash tavsiya etiladi.</>, ru: <>⚠️ Только <b>{pct}%</b> верных — классу эта тема пока непонятна. Перед продолжением стоит коротко повторить.</> })}</p>
            ...
            {level === "maybe" && <>
              <p className="mstats-verdict-t">{tr3({ uz: <>🟡 <b>{pct}%</b> to'g'ri — yomon emas. Xohlasangiz, davom etishdan oldin qisqa takrorlab oling.</>, ru: <>🟡 <b>{pct}%</b> верных — неплохо. Если хотите, коротко повторите перед тем, как идти дальше.</> })}</p>
            ...
            {level === "few" && <>
              <p className="mstats-verdict-t">{tr3({ uz: <>Javob berganlar kam ({answered} ta) — foiz bo'yicha xulosa chiqarish qiyin. O'zingiz baholang:</>, ru: <>Ответов мало ({answered}) — делать выводы по проценту трудно. Оцените сами:</> })}</p>
    ...
    return <button key={i} className={cls} disabled={solved || isMentorLive} onClick={() => pick(i)} style={{ padding: picked !== null ? "clamp(9px,1.3vw,12px) clamp(15px,2.2vw,20px)" : "clamp(13px,1.9vw,17px) clamp(15px,2.2vw,20px)", fontSize: "clamp(15px,1.85vw,17px)", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <FeedbackBlock show={isMentorLive ? mReveal : picked !== null} isCorrect={isMentorLive ? true : solved && !wrongLocked} neutral={waiting}>
          <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: waiting ? T.blue : isMentorLive || solved && !wrongLocked ? T.success : T.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>
            {isMentorLive ? fmtCode(`✓ ${tr3({ uz: "To'g'ri javob:", ru: "Верный ответ:" })} ${String.fromCharCode(65 + correctIdx)} — ${options[correctIdx]}`) : waiting ? tr3({ uz: "📨 Javobingiz qabul qilindi", ru: "📨 Ваш ответ принят" }) : wrongLocked ? fmtCode(`${tr3({ uz: "To'g'ri javob:", ru: "Верный ответ:" })} ${String.fromCharCode(65 + correctIdx)} — ${options[correctIdx]}`) : solved ? tr3({ uz: "To'g'ri", ru: "Верно" }) : tr3({ uz: "Qaytadan urinib ko'ring", ru: "Попробуйте ещё раз" })}
          </p>
...
var Terminal = ({ lines, empty = { uz: "// natija shu yerda chiqadi…", ru: "// результат появится здесь…" }, title = "console" }) => <div className="term">
    ...
    <div className="term-body">
      {lines.length === 0 ? <p className="term-empty">{tr3(empty)}</p> : lines.map((l, i) => <div key={i} className="term-line"><span className="term-arrow">›</span><span>{tr3(l)}</span></div>)}
    </div>
  </div>;
  ...
  return <div className="dd fade-up">
      <div className="dd-slots">
        {slots.map((sid, i) => <div key={i} ref={(el) => slotRefs.current[i] = el} className={`dd-slot ${sid ? "filled" : ""} ${solved && sid ? "ok" : ""} ${wrong && sid && sid !== order[i] ? "bad" : ""}`}>
            ...
            {sid ? <button className="dd-chip in" onPointerDown={(e) => down(e, sid, i)}>{byId[sid].label}</button> : <span className="dd-hint">{hints ? tr3(hints[i]) : tr3({ uz: "bu yerga joylang", ru: "положите сюда" })}</span>}
          </div>)}
      </div>
      <div className="dd-pool">
        {pool.length === 0 && !solved && <span className="dd-pool-empty">{tr3({ uz: "Tartib xato — bo'lakni bosib qaytaring va qayta joylang", ru: "Порядок неверный — нажмите на блок, верните его и разложите заново" })}</span>}
        {pool.map((id) => <button key={id} className="dd-chip" onPointerDown={(e) => down(e, id, "pool")}>{byId[id].label}</button>)}
      </div>
      {solved && <div className="dd-done">{tr3({ uz: "✓ To'g'ri! Funksiya aynan shu tartibda yig'iladi.", ru: "✓ Верно! Функция собирается именно в этом порядке." })}</div>}
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={{ uz: "Kirish", ru: "Введение" }} screen={screen} navContent={<NavNext optionalLive disabled={picked === null} label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr3({ uz: <>Har urishda zararni <span className="italic" style={{ color: T.accent }}>qo'lda</span> hisoblaysizmi?</>, ru: <>Считаете стоимость каждого заказа <span className="italic" style={{ color: T.accent }}>вручную</span>?</> })}</h1>
        <Mentor>{tr3({ uz: <>O'yin yozyapsiz. Har urishda <b style={{ color: T.ink }}>bir xil 4 amal</b> — kuchni ol, zarbaga ko'paytir, jonni kamaytir, ekranga chiqar — qo'lda takrorlanadi. Tugmani bosib, 5 urishni o'zingiz hisoblang.</>, ru: <>Вы пишете игру. При каждом ударе повторяются <b style={{ color: T.ink }}>одни и те же 4 действия</b> — взять силу, умножить на удар, отнять здоровье, вывести на экран. Нажмите кнопку и посчитайте 5 ударов сами.</> })}</Mentor>
        <Zoomable>
        <Split>
          <Col>
            <p className="flow-label">{tr3({ uz: "Qo'lda hisoblangan urishlar", ru: "Удары, посчитанные вручную" })}</p>
            <div className="msg-list fade-up delay-1">
              {count === 0 ? <p style={{ color: T.ink3, fontStyle: "italic", margin: 0, fontFamily: "'JetBrains Mono',monospace", fontSize: 13 }}>{tr3({ uz: "// hali bitta urish ham hisoblanmadi", ru: "// пока не посчитан ни один" })}</p> : Array.from({ length: count }).map((_, i) => <div key={i} className="msg-line el-in"><span className="msg-ok">⚔️</span><span>{tr3({ uz: <>{i + 1}-urish — 4 amal qo'lda bajarildi</>, ru: <>Удар #{i + 1} — 4 действия вручную</> })}</span></div>)}
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
              <button className={`btn ${count >= 3 ? "btn-tired" : ""}`} onClick={make} disabled={count >= NEED} style={{ alignSelf: "flex-start" }}>{count >= NEED ? tr3({ uz: "😕 Charchadim…", ru: "😕 Я устал…" }) : tr3({ uz: "🧾 Yana bittasini qo'lda hisoblash", ru: "🧾 Посчитать ещё один вручную" })}</button>
            ...
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span className="flow-label" style={{ margin: 0 }}>{tr3({ uz: "Charchoq darajasi", ru: "Уровень усталости" })} <span className="face-pop" key={count} style={{ fontSize: 15 }}>{count < 2 ? "🙂" : count < 4 ? "😐" : count < 5 ? "😓" : "😕"}</span></span>
            ...
            {count >= 3 && count < NEED && <p className="hook-ack fade-step">{tr3({ uz: <>Har safar <b>aynan bir xil 4 amal</b>… Zerikarli, to'g'rimi? 😅</>, ru: <>Каждый раз <b>одни и те же 4 действия</b>… Скучно, правда? 😅</> })}</p>}
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr3({ uz: "Dasturchi buni qanday hal qiladi?", ru: "Как это решает программист?" })}</p>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const PreviewBlock = <Col>
      <p className="flow-label">{tr3({ uz: "Bugungi asosiy fikr", ru: "Главная мысль дня" })}</p>
      <div className="frame fade-up" style={{ display: "flex", alignItems: "center", gap: 16, padding: "clamp(16px,2.4vw,22px)" }}>
        ...
        <p className="body" style={{ margin: 0, color: T.ink, fontSize: "clamp(15px,2vw,18px)" }}>{tr3({ uz: <>Bir marta <b>yozasiz</b> — <b>xohlagancha</b> ishlatasiz.</>, ru: <>Пишете <b>один раз</b> — используете <b>сколько угодно</b>.</> })}</p>
      </div>
      <div className="fade-up delay-1" style={{ display: "flex", gap: 7, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <span className="mono small" style={{ color: T.ink3 }}>{tr3({ uz: "… xohlagancha", ru: "… сколько угодно" })}</span>
      </div>
    </Col>;
  ...
  return <Stage eyebrow={{ uz: "Reja", ru: "План" }} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Boshlaymiz →", ru: "Начинаем →" }} onClick={onNext} /></>}>
      <div className="screen">
        ...
        {!isNarrow ? <Zoomable><Split>{PreviewBlock}{StepsBlock}</Split></Zoomable> : !showSteps ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>
            ...
            <button className="btn-soft" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(false)}>{tr3({ uz: "↩ Tushunchalarni ko'rish", ru: "↩ Посмотреть понятия" })}</button>
            {StepsBlock}
          </div>}
      </div>
    </Stage>;
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [mode, setMode] = useState4("manual");
  const [seen, setSeen] = useState4(/* @__PURE__ */ new Set(["manual"]));
  ...
  return <Stage eyebrow={{ uz: "Funksiya nima", ru: "Что такое функция" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Ikkala usulni ko'ring", ru: "Посмотрите оба способа" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Bir ishni <span className="italic" style={{ color: T.accent }}>3 marta</span> — qanday yozamiz?</>, ru: <>Одно действие <span className="italic" style={{ color: T.accent }}>3 раза</span> — как напишем?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>O'yinda dushmanga uch marta urdingiz — har urishda bir xil uch qator chiqadi. <b style={{ color: T.ink }}>Funksiyasiz</b> — o'sha uch qatorni qayta-qayta ko'chirasiz (9 qator). <b style={{ color: T.ink }}>Funksiya bilan</b> — bir marta yozib, <b style={{ color: T.ink }}>nom</b> berasiz va shu nomni 3 marta chaqirasiz.</>, ru: <>Вы ударили врага три раза — при каждом ударе выводятся одни и те же три строки. <b style={{ color: T.ink }}>Без функции</b> — копируете эти три строки снова и снова (9 строк). <b style={{ color: T.ink }}>С функцией</b> — пишете один раз, даёте <b style={{ color: T.ink }}>имя</b> и вызываете его 3 раза.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8 }}>
              <button className={`chip ${mode === "manual" ? "chip-on" : ""}`} onClick={() => set("manual")}>{tr3({ uz: "😕 Funksiyasiz", ru: "😕 Без функции" })}</button>
              <button className={`chip ${mode === "loop" ? "chip-on" : ""}`} onClick={() => set("loop")}>{tr3({ uz: "🛠️ Funksiya bilan", ru: "🛠️ С функцией" })}</button>
            </div>
            <div className="codebox demo-swap" key={mode}>
              {mode === "manual" ? <>
                  ...
                  <div><CM>{tr3({ uz: "// ... yana, yana — 9 qator", ru: "// ... снова и снова — 9 строк" })}</CM></div>
                  <div><CM>{tr3({ uz: "// zarar o'zgarsa — 3 joyda tuzatamiz 🥲", ru: "// изменится урон — чиним в 3 местах 🥲" })}</CM></div>
                </> : <>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [part, setPart] = useState4(null);
  ...
  const PARTS = {
    name: { color: T.blue, num: "1", name: tr3({ uz: "Funksiya nomi", ru: "Имя функции" }), code: "zarar", desc: tr3({ uz: "Mashinaning nomi. Aynan shu nom bilan funksiyani chaqiramiz: zarar().", ru: "Имя машины. Именно по этому имени мы вызываем функцию: zarar()." }) },
    param: { color: T.accent, num: "2", name: tr3({ uz: "Parametr", ru: "Параметр" }), code: "kuch", desc: tr3({ uz: "Mashinaga beriladigan kirish (input). Chaqirganda haqiqiy qiymat solamiz, masalan 5.", ru: "Вход машины (input). При вызове кладём настоящее значение, например 5." }) },
    ret: { color: T.success, num: "3", name: "return", code: "return", desc: tr3({ uz: "Mashinadan chiqadigan natija (output). Funksiya javobni shu yerda tashqariga qaytaradi.", ru: "Результат на выходе машины (output). Здесь функция возвращает ответ наружу." }) }
  };
  ...
  return <Stage eyebrow={{ uz: "Funksiya tuzilishi", ru: "Строение функции" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : ran ? { uz: "Funksiyani yig'ing", ru: "Соберите функцию" } : { uz: "Avval chaqiring", ru: "Сначала вызовите" }} onClick={onNext} /></>}>
        ...
        <div className="split">
          <Col>
            ...
            {part ? <div className="sk-info fade-step" key={part}>
              ...
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr3({ uz: "👆 Koddagi 3 ta rangli qismni bosing", ru: "👆 Нажмите на 3 цветные части в коде" })}</p></div>}
          </Col>
          <Col>
            {
    ...
    if (achMiss) achMiss.miss(screen);
            ...
            <button className="btn" onClick={run} disabled={running} style={{ alignSelf: "flex-start" }}>{running ? tr3({ uz: "Ishlayapti…", ru: "Работает…" }) : ran ? tr3({ uz: "↻ Yana urish", ru: "↻ Ударить ещё раз" }) : tr3({ uz: "▶ zarar(5) ni chaqirish", ru: "▶ Вызвать zarar(5)" })}</button>
...
var Screen4 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr3({ uz: <><span className="mono" style={{ color: T.accent }}>zarar()</span> deb yozsak nima bo'ladi?</>, ru: <>Что произойдёт, если написать <span className="mono" style={{ color: T.accent }}>zarar()</span>?</> })}</h2></>}
  options={[tr3({ uz: "Hech narsa — u shunchaki nom bo'lib turadi", ru: "Ничего — это просто имя, оно так и останется" }), tr3({ uz: "Funksiya ichidagi kod ishga tushadi (chaqiriladi)", ru: "Код внутри функции запустится (вызов)" }), tr3({ uz: "Funksiya butunlay o'chib, yo'qoladi", ru: "Функция полностью сотрётся и исчезнет" }), tr3({ uz: "Noldan yangi funksiya yaratiladi", ru: "С нуля создастся новая функция" })]}
  correctIdx={1}
  explainCorrect={tr3({ uz: "To'g'ri! Nomdan keyingi `()` — bu chaqirish (call). Funksiya ichida yozilgan kod aynan shu paytda ishga tushadi.", ru: "Верно! `()` после имени — это вызов (call). Код, записанный внутри функции, запускается именно в этот момент." })}
  explainWrong={{
    0: tr3({ uz: "Yo'q — `()` qo'shilsa, bu chaqirish bo'ladi: funksiya ichidagi kod ishlaydi.", ru: "Нет — если добавить `()`, это вызов: код внутри функции сработает." }),
    2: tr3({ uz: "Yo'q — chaqirish funksiyani o'chirmaydi. Aksincha, uni ishga tushiradi va keyin yana chaqirsa bo'ladi.", ru: "Нет — вызов не удаляет функцию. Наоборот, он её запускает, и потом можно вызвать снова." }),
    3: tr3({ uz: "Yo'q — funksiya `function` bilan bir marta yaratiladi. `zarar()` esa borini chaqiradi.", ru: "Нет — функция создаётся один раз через `function`. А `zarar()` вызывает уже существующую." }),
    default: tr3({ uz: "`zarar()` — funksiyani chaqiradi: ichidagi kod ishga tushadi.", ru: "`zarar()` — вызывает функцию: код внутри запускается." })
  }}
/>;
      ...
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>✓ Ko'rdingizmi? Funksiya <b>bitta</b>, lekin har xil <b>kuch</b> bilan har xil zarar chiqadi. Bir mashina — barcha urishlar uchun!</>, ru: <>✓ Видели? Функция <b>одна</b>, но с разной <b>силой</b> даёт разный урон. Одна машина — на все удары!</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="return" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Avval ishga tushiring", ru: "Сначала запустите" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Funksiya javobini qanday <span className="italic" style={{ color: T.accent }}>saqlab qolamiz?</span></>, ru: <>Как <span className="italic" style={{ color: T.accent }}>сохранить</span> ответ функции?</> })}</h2></div>
        <Mentor>{tr3({ uz: <><span className="mono" style={{ color: T.accent }}>return</span> funksiyaning <b style={{ color: T.ink }}>javobini tashqariga qaytaradi</b>. Eng muhimi: bu javobni <b style={{ color: T.ink }}>o'zgaruvchiga saqlash</b> va keyin ishlatish mumkin! <span className="mono">zarar</span> mashinasiga kuch kiriting va ishga tushiring — natija <span className="mono">natija</span> o'zgaruvchisida saqlanadi.</>, ru: <><span className="mono" style={{ color: T.accent }}>return</span> <b style={{ color: T.ink }}>возвращает ответ функции наружу</b>. Самое главное: этот ответ можно <b style={{ color: T.ink }}>сохранить в переменную</b> и использовать дальше! Введите число в машину <span className="mono">zarar</span> и запустите — результат сохранится в переменной <span className="mono">natija</span>.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <p className="flow-label">{tr3({ uz: "Qaysi kuch bilan uramiz?", ru: "С какой силой бьём?" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8 }}>
              {[4, 5, 6].map((v) => <button key={v} className={`chip ${n === v ? "chip-on" : ""}`} onClick={() => setNN(v)}>{v}</button>)}
            </div>
            <div className="codebox" style={{ marginTop: 6 }}>
              <div><KW>function</KW> <FN>zarar</FN>(kuch) {"{"}</div>
              <div style={{ paddingLeft: 18 }}><KW>return</KW> kuch * <NUM>3</NUM> <CM>{tr3({ uz: "// natijani qaytaradi", ru: "// возвращает результат" })}</CM></div>
              <div>{"}"}</div>
              <div style={{ marginTop: 8 }}><KW>let</KW> ochko = <FN>zarar</FN>(<NUM>{n}</NUM>) <CM>{tr3({ uz: "// javobni saqlaydi", ru: "// сохраняет ответ" })}</CM></div>
            </div>
            <button className="btn" onClick={run} disabled={running} style={{ alignSelf: "flex-start" }}>{running ? tr3({ uz: "Ishlayapti…", ru: "Работает…" }) : done ? tr3({ uz: "↻ Yana", ru: "↻ Ещё раз" }) : tr3({ uz: "▶ Ishga tushir", ru: "▶ Запустить" })}</button>
          </Col>
          <Col>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="log ⚔️ return" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `${seen.size}/2 ko'ring`, ru: `Посмотрите ${seen.size}/2` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Ekranga <span className="italic" style={{ color: T.accent }}>ko'rsatish</span> va kodga <span className="italic" style={{ color: T.accent }}>qaytarish</span> — bir xilmi?</>, ru: <>Показать <span className="italic" style={{ color: T.accent }}>на экране</span> и <span className="italic" style={{ color: T.accent }}>вернуть</span> в код — одно и то же?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Yangi dasturchilar <span className="mono">console.log</span> va <span className="mono">return</span> ni adashtiradi. Farq oddiy: <b style={{ color: T.accent }}>console.log</b> natijani <b style={{ color: T.ink }}>ekranga ko'rsatadi</b> (ko'z uchun), <b style={{ color: T.accent }}>return</b> esa qiymatni <b style={{ color: T.ink }}>kodga qaytaradi</b> (saqlash uchun). Ikkala kartani bosing.</>, ru: <>Новички путают <span className="mono">console.log</span> и <span className="mono">return</span>. Разница проста: <b style={{ color: T.accent }}>console.log</b> <b style={{ color: T.ink }}>показывает результат на экране</b> (для глаз), а <b style={{ color: T.accent }}>return</b> <b style={{ color: T.ink }}>возвращает значение в код</b> (для сохранения). Нажмите обе карточки.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
          ...
          <Col>
            {active ? <div className="sk-info fade-step" key={active}>
                ...
                <p className="body" style={{ color: T.ink, margin: "11px 0 9px", fontWeight: 600 }}>{tr3(CARDS[active].when)}</p>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {CARDS[active].ex.map((e, i) => <div key={i} className="ex-row" style={{ display: "flex", gap: 8, alignItems: "center", background: T.bg, borderRadius: 8, padding: "8px 11px", animationDelay: `${0.05 + i * 0.09}s` }}><span style={{ color: T.accent }}>•</span><span className="body" style={{ margin: 0, color: T.ink2 }}>{tr3(e)}</span></div>)}
                </div>
              </div> : !isNarrow ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr3({ uz: "Bir kartani bosing", ru: "Нажмите на одну из карточек" })}</p></div> : null}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Esda tuting: <b>console.log</b> = ko'rsatadi (yo'qoladi), <b>return</b> = qaytaradi (saqlanadi va ishlatiladi).</>, ru: <>Запомните: <b>console.log</b> = показывает (и пропадает), <b>return</b> = возвращает (сохраняется и используется).</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [a, setA] = useState4(5);
  const [b, setB] = useState4(3);
  const [seen, setSeen] = useState4(/* @__PURE__ */ new Set(["5-3"]));
  const done = seen.size >= 2;
...
var Screen9 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><div className="codebox" style={{ marginTop: 10, marginBottom: 6 }}><div><KW>function</KW> <FN>zarar</FN>(kuch, bonus) {"{"} <KW>return</KW> kuch * <NUM>3</NUM> + bonus {"}"}</div></div><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr3({ uz: "zarar(10, 3) nimani qaytaradi?", ru: "Что вернёт zarar(10, 3)?" })}</h2></>}
  options={["30", "33", "19", "103"]}
  correctIdx={1}
  explainCorrect={tr3({ uz: "To'g'ri! Tartib bo'yicha `kuch = 10`, `bonus = 3`. Funksiya `10 * 3 + 3` = 33 ni qaytaradi.", ru: "Верно! По порядку `kuch = 10`, `bonus = 3`. Функция возвращает `10 * 3 + 3` = 33." })}
  explainWrong={{
    0: tr3({ uz: "Yo'q — 30 bu faqat `kuch * 3`. Bonus ham qo'shiladi: 30 + 3 = 33.", ru: "Нет — 30 это только `kuch * 3`. Бонус тоже прибавляется: 30 + 3 = 33." }),
    2: tr3({ uz: "Yo'q — bu qiymatlar almashib ketgani: 3 * 3 + 10 = 19. Tartib muhim: `kuch` birinchi = 10.", ru: "Нет — здесь значения переставлены: 3 * 3 + 10 = 19. Порядок важен: `kuch` первое = 10." }),
    3: tr3({ uz: "Yo'q — 103 bu matn ulanishi. Bizda sonlar hisoblanadi: 10 * 3 + 3 = 33.", ru: "Нет — 103 это склейка текста. А у нас считаются числа: 10 * 3 + 3 = 33." }),
    default: "`kuch = 10`, `bonus = 3` → 10 * 3 + 3 = 33."
  }}
/>;
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const BONUS = 10;
  const [soni, setSoni] = useState4(5);
  const [jami, setJami] = useState4(storedAnswer ? 5 * 3 + BONUS : null);
  const [running, setRunning] = useState4(false);
  const [ran, setRan] = useState4(!!storedAnswer);
  const timer = useRef4(null);
  const done = ran;
  ...
  return <Stage eyebrow={{ uz: "Hammasi birga", ru: "Всё вместе" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Avval hisoblang", ru: "Сначала посчитайте" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Uchalasi birga — <span className="italic" style={{ color: T.accent }}>zarar mashinasi</span>ni quramizmi?</>, ru: <>Все три вместе — соберём <span className="italic" style={{ color: T.accent }}>кассовую машину</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Mana to'liq mashina! <span className="mono">zarar(kuch, bonus)</span> ikkita parametr oladi, hisoblaydi va javobni <b style={{ color: T.ink }}>return</b> bilan qaytaradi. Kuchni tanlang va ishga tushiring.</>, ru: <>Вот и полная машина! <span className="mono">zarar(kuch, bonus)</span> берёт два параметра, считает и возвращает ответ через <b style={{ color: T.ink }}>return</b>. Выберите силу и запустите.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <p className="flow-label">{tr3({ uz: <>Urish kuchi? (krit bonusi {BONUS})</>, ru: <>Сила удара? (крит-бонус {BONUS})</> })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8 }}>
            ...
            <button className="btn" onClick={run} disabled={running} style={{ alignSelf: "flex-start" }}>{running ? tr3({ uz: "Hisoblanyapti…", ru: "Считаем…" }) : done ? tr3({ uz: "↻ Yana", ru: "↻ Ещё раз" }) : tr3({ uz: "🧮 Jami narxni hisoblash", ru: "🧮 Посчитать итоговую цену" })}</button>
          </Col>
          <Col>
            <div className="iwatch fade-up delay-1">
              <span className="iwatch-lbl">ochko</span>
            ...
            <p className="mono small" style={{ color: T.ink3, margin: 0 }}>{jami !== null ? `${jami} ${tr3({ uz: "zarar", ru: "урона" })}` : tr3({ uz: "// hisoblanmadi", ru: "// не посчитано" })}</p>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>✓ <b>Funksiya</b> (zarar), <b>parametrlar</b> (kuch, bonus) va <b>return</b> — uchalasi bitta mashinada ishladi: {soni} × 3 + {BONUS} = {jami}.</>, ru: <>✓ <b>Функция</b> (zarar), <b>параметры</b> (kuch, bonus) и <b>return</b> — все трое сработали в одной машине: {soni} × 3 + {BONUS} = {jami}.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const XILLAR = [{ e: "👊", s: 4, n: tr3({ uz: "Oddiy urish", ru: "Обычный удар" }) }, { e: "⚔️", s: 6, n: tr3({ uz: "Qilich zarbasi", ru: "Удар мечом" }) }, { e: "🔥", s: 9, n: tr3({ uz: "Olovli zarba", ru: "Огненный удар" }) }];
  const KOEF = 3;
  const som = (v) => v.toLocaleString("ru-RU").replace(/ /g, " ");
  const N = XILLAR.length;
  const line = (x) => `⚔️ ${x.n} — ${x.s * KOEF} ${tr3({ uz: "zarar", ru: "урона" })}`;
  const [out, setOut] = useState4(storedAnswer ? XILLAR.map(line) : []);
  const [running, setRunning] = useState4(false);
  const timer = useRef4(null);
  ...
  return <Stage eyebrow={{ uz: "Hayotiy misol", ru: "Пример из жизни" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Hisoblagichni ishlating", ru: "Запустите расчёт" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Esingizdagi urishlar? Endi zararni <span className="italic" style={{ color: T.accent }}>funksiya</span> hisoblaydi.</>, ru: <>Помните удары? Теперь урон считает <span className="italic" style={{ color: T.accent }}>функция</span>.</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Dars boshida har urishni qo'lda hisoblagandingiz. Endi hisob bir marta funksiya ichiga yozildi — uni <b style={{ color: T.ink }}>uch xil kuch</b> bilan chaqiramiz.</>, ru: <>В начале урока Вы считали каждый удар вручную. Теперь расчёт записан внутри функции один раз — вызовем её с <b style={{ color: T.ink }}>тремя разными силами</b>.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <div className="codebox fade-up delay-1" style={{ lineHeight: 1.9 }}>
              <div><KW>function</KW> <FN>zarar</FN>(<span style={{ color: "#FF9777", fontWeight: 700 }}>kuch</span>) {"{"}</div>
              <div style={{ paddingLeft: 16 }}><CM>{tr3({ uz: "// har kuch birligi 3 zarar", ru: "// каждая единица силы — 3 урона" })}</CM></div>
            ...
            <button className="btn" onClick={run} disabled={running} style={{ alignSelf: "flex-start" }}>{running ? tr3({ uz: "Hisoblanyapti…", ru: "Считается…" }) : done ? tr3({ uz: "↻ Yana hisoblash", ru: "↻ Посчитать ещё" }) : tr3({ uz: "🧾 Funksiya bilan hisoblash", ru: "🧾 Посчитать функцией" })}</button>
          </Col>
          <Col>
            <p className="flow-label">{tr3({ uz: "Funksiya har urishni hisoblayapti", ru: "Функция считает каждый удар" })}</p>
    ...
    return <div key={i} className={`flavor-card ${got ? "got" : ""}`}>
                    ...
                    <div><div className="flavor-name">{x.n}</div><div className="flavor-msg">{got ? `${x.s * KOEF} ${tr3({ uz: "zarar", ru: "урона" })}` : tr3({ uz: "navbatda…", ru: "в очереди…" })}</div></div>
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>✓ Uchala urish — <b>bitta funksiya bilan</b>. Kuch o'zgardi, mashina o'zgarmadi.</>, ru: <>✓ Все три удара — <b>одной функцией</b>. Сила менялась, машина — нет.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen12 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><div className="codebox" style={{ marginTop: 10, marginBottom: 6 }}><div><KW>function</KW> <FN>salom</FN>(ism) {"{"} <KW>return</KW> <STR>"Salom, "</STR> + ism {"}"}</div><div style={{ marginTop: 4 }}><KW>let</KW> x = <FN>salom</FN>(<STR>"Olim"</STR>)</div></div><h2 className="title h-ask" style={{ marginTop: 6 }}>{tr3({ uz: <><span className="mono" style={{ color: T.accent }}>x</span> o'zgaruvchisida nima saqlanadi?</>, ru: <>Что сохранится в переменной <span className="mono" style={{ color: T.accent }}>x</span>?</> })}</h2></>}
  options={['`"salom"`', '`"ism"`', '`"Salom, Olim"`', "`undefined`"]}
  correctIdx={2}
  explainCorrect={tr3({ uz: `To'g'ri! "Olim" \`ism\` parametriga tushadi, funksiya \`"Salom, " + ism\` = "Salom, Olim" ni qaytaradi. \`return\` qiymati \`x\` ga saqlanadi.`, ru: 'Верно! "Olim" попадает в параметр `ism`, функция возвращает `"Salom, " + ism` = "Salom, Olim". Значение `return` сохраняется в `x`.' })}
  explainWrong={{
    0: tr3({ uz: "Yo'q — funksiya nomi «salom», lekin u matn qaytarmaydi. Qaytadigan qiymat «Salom, Olim».", ru: "Нет — «salom» это имя функции, но возвращает она не его. Возвращаемое значение — «Salom, Olim»." }),
    1: tr3({ uz: "Yo'q — «ism» bu parametr nomi. Uning qiymati «Olim», natija esa «Salom, Olim».", ru: "Нет — «ism» это имя параметра. Его значение «Olim», а результат — «Salom, Olim»." }),
    3: tr3({ uz: "Yo'q — `return` bor, demak `undefined` emas. Funksiya «Salom, Olim» ni qaytaradi.", ru: "Нет — `return` есть, значит не `undefined`. Функция возвращает «Salom, Olim»." }),
    default: tr3({ uz: "`return` «Salom, Olim» ni qaytaradi → `x` = «Salom, Olim».", ru: "`return` возвращает «Salom, Olim» → `x` = «Salom, Olim»." })
  }}
/>;
var PRAC_STEPS = [
  {
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [step, setStep] = useState4(storedAnswer ? PRAC_STEPS.length : 0);
  ...
  return <Stage eyebrow={{ uz: "Amaliyot · kod yozing", ru: "Практика · пишем код" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!allDone} label={allDone ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `${step}/${PRAC_STEPS.length} bosqich yozildi`, ru: `Написано шагов: ${step}/${PRAC_STEPS.length}` }} onClick={onNext} /></>}>
        ...
        {allDone ? <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>✓ Uch bosqich ham tayyor! Siz funksiyani <b>e'lon qildingiz</b>, <b>parametr</b> berdingiz va <b>chaqirib</b> return natijasini to'g'ri bashorat qildingiz. Endi haqiqiy mashinasozsiz! 🛠️</>, ru: <>✓ Все три шага готовы! Вы <b>объявили</b> функцию, дали ей <b>параметры</b> и, <b>вызвав</b>, верно предсказали результат return. Теперь вы настоящий машиностроитель! 🛠️</> })}</p></div> : <div className="split" key={step}>
          <Col>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {checks2.map((c, i) => <span key={i} className="tagpill" style={{ opacity: c.ok ? 1 : 0.4 }}>{c.ok ? "✓" : i + 1} {tr3(c.lbl)}</span>)}
            </div>
            {needPredict && <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                <span className="body" style={{ color: T.ink }}>{tr3({ uz: "return nimani qaytaradi?", ru: "что вернёт return?" })}</span>
    ...
    /* Xato-izoh: birinchi yetishmayotgan qism bo'yicha aniq maslahat */
            ...
            {!stepOk && (value.trim() || guess.trim()) && (firstBad ? <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3(firstBad.hint)}</p></div> : needPredict && guess.trim() && !predictOk && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Hisoblab ko'ring: <span className="mono">a = 3</span>, <span className="mono">b = 4</span> → <span className="mono">return a + b</span> qancha qaytaradi?</>, ru: <>Посчитайте: <span className="mono">a = 3</span>, <span className="mono">b = 4</span> → сколько вернёт <span className="mono">return a + b</span>?</> })}</p></div>)}
            {stepOk && <div className="frame-success fade-step" style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                <p className="body" style={{ margin: 0, color: T.ink, flex: 1, minWidth: 180 }}>{tr3({ uz: "✓ To'g'ri yozdingiz!", ru: "✓ Написано верно!" })}</p>
                <button className="btn" onClick={nextStep}>{step === PRAC_STEPS.length - 1 ? tr3({ uz: "🏁 Yakunlash", ru: "🏁 Завершить" }) : tr3({ uz: "Keyingi bosqich →", ru: "Следующий шаг →" })}</button>
              </div>}
          </Col>
          <Col>
            <p className="flow-label">{tr3({ uz: "Mashina quvuri", ru: "Конвейер машины" })}</p>
            <div className="pipe fade-up delay-1">
              <div className="pipe-box"><span className="pipe-lbl">{tr3({ uz: "kirish", ru: "вход" })}</span><span className="pipe-chip" style={{ background: T.accentSoft, color: T.accent }}>{step === 0 ? "—" : step === 1 ? "a, b" : "3, 4"}</span></div>
              <span className={`pipe-arrow ${codeOk ? "flow" : ""}`}>→</span>
              <div className="pipe-box"><span className="pipe-lbl">{tr3({ uz: "mashina", ru: "машина" })}</span><span className={`pipe-machine ${codeOk ? "busy" : ""}`} key={codeOk ? "b" : "i"}>🛠️</span></div>
              <span className={`pipe-arrow ${stepOk ? "flow" : ""}`}>→</span>
              <div className="pipe-box"><span className="pipe-lbl">{tr3({ uz: "chiqish · return", ru: "выход · return" })}</span><span className="pipe-chip" style={{ background: stepOk ? T.successSoft : T.bg, color: stepOk ? T.success : T.ink3 }}>{stepOk ? S.out : "?"}</span></div>
            </div>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [picked, setPicked] = useState4(storedAnswer ? "body" : null);
        ...
        <div className="split">
          <Col>
            <div className="ai-card fade-up delay-1">
              ...
              {!found && <p className="ai-prompt">{tr3({ uz: "Qaysi qator xato? Ustiga bosing.", ru: "Какая строка с ошибкой? Нажмите на неё." })}</p>}
              {found && !fixed && <button className="btn fade-step" style={{ alignSelf: "flex-start" }} onClick={fix}>{tr3({ uz: `🔧 "return" so'zini qo'shish`, ru: '🔧 Добавить слово "return"' })}</button>}
              {fixed && <p className="ai-prompt" style={{ color: T.success, fontStyle: "normal", fontWeight: 600 }}>{tr3({ uz: "✓ Tuzatildi — endi funksiya natijani qaytaradi!", ru: "✓ Починено — теперь функция возвращает результат!" })}</p>}
            </div>
            {!fixed ? <div className="term fade-up delay-2">
                ...
                <div className="term-body"><div className="term-line"><span className="term-arrow" style={{ color: T.accent }}>›</span><span>undefined</span></div><p className="term-empty" style={{ color: T.accent }}>{tr3({ uz: "⚠️ funksiya hisobladi, lekin hech narsa qaytarmadi!", ru: "⚠️ функция посчитала, но ничего не вернула!" })}</p></div>
              </div> : <div className="term fade-step">
                <div className="term-bar"><span className="term-dot" style={{ background: "#FF5F56" }} /><span className="term-dot" style={{ background: "#FFBD2E" }} /><span className="term-dot" style={{ background: "#27C93F" }} /><span className="term-title">console</span></div>
                <div className="term-body"><div className="term-line"><span className="term-arrow">›</span><span>25</span></div><p className="term-empty" style={{ color: T.success }}>{tr3({ uz: "✓ endi natija (5 × 5 = 25) qaytdi", ru: "✓ теперь результат (5 × 5 = 25) вернулся" })}</p></div>
              </div>}
          </Col>
          <Col>
            {!found && (picked && picked !== "body" ? <div className="frame-warn fade-step"><p className="body zb-notch" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Bu qism to'g'ri. {picked === "name" ? "Funksiya nomi (zarar) — joyida." : "Parametr (n) — joyida."} Xato esa <b>ichki qatorida</b> — natija qaytarilyaptimi?</>, ru: <>Эта часть верна. {picked === "name" ? "Имя функции (zarar) — на месте." : "Параметр (n) — на месте."} А ошибка — <b>во внутренней строке</b>: возвращается ли результат?</> })}</p></div> : <div className="hint"><p className="body zb-notch" style={{ margin: 0, color: T.ink2 }}>{tr3({ uz: <>Eslang: funksiya natija berishi uchun <b style={{ color: T.ink }}>return</b> kerak. Bu yerda <span className="mono">n * n</span> hisoblanyapti, lekin qaytarilyaptimi?</>, ru: <>Вспомните: чтобы функция отдала результат, нужен <b style={{ color: T.ink }}>return</b>. Здесь <span className="mono">n * n</span> считается — но возвращается ли?</> })}</p></div>)}
            {found && !fixed && <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.accent }}>{tr3({ uz: "✓ Topdingiz!", ru: "✓ Нашли!" })}</p><p className="body zb-notch" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <><span className="mono">n * n</span> hisoblanadi, ammo <b>return yo'q</b> — natija tashqariga chiqmaydi, shuning uchun x = undefined. To'g'risi: <span className="mono">return n * n</span>. Chap tugmani bosing →</>, ru: <><span className="mono">n * n</span> считается, но <b>return отсутствует</b> — результат не выходит наружу, поэтому x = undefined. Правильно: <span className="mono">return n * n</span>. Нажмите кнопку слева →</> })}</p></div>}
            {fixed && <div className="takeaway fade-step"><div className="ta-bulb">🛠️</div><p className="ta-h">{tr3({ uz: "Topdingiz va tuzatdingiz — bu debugging!", ru: "Нашли и починили — это дебаггинг!" })}</p><p className="ta-sub">{tr3({ uz: "return bo'lmasa — funksiya undefined qaytaradi", ru: "без return функция возвращает undefined" })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  const RECAP = [tr3({ uz: "Funksiya — nomlangan, qayta ishlatiladigan kod bloki (mashina)", ru: "Функция — именованный, многоразовый блок кода (машина)" }), tr3({ uz: "Funksiyani nomi bilan chaqiramiz — zarar()", ru: "Функцию вызываем по имени — zarar()" }), tr3({ uz: "Parametr — funksiyaga beriladigan kirish (input)", ru: "Параметр — вход, который передают функции (input)" }), tr3({ uz: "return — funksiya qaytaradigan natija (output)", ru: "return — результат, который возвращает функция (output)" }), tr3({ uz: "console.log ko'rsatadi, return qaytaradi (saqlash uchun)", ru: "console.log показывает, return возвращает (для сохранения)" })];
  const HOMEWORK = [{ b: tr3({ uz: "Oddiy urish", ru: "Обычный удар" }), t: tr3({ uz: "— kuch oladigan va kuch * 3 qaytaradigan zarar(kuch) yozing", ru: "— напишите zarar(kuch): берёт силу и возвращает kuch * 3" }) }, { b: tr3({ uz: "Krit bonus", ru: "Крит-бонус" }), t: tr3({ uz: "— ikkinchi parametr qo'shing: zarar(kuch, bonus)", ru: "— добавьте второй параметр: zarar(kuch, bonus)" }) }, { b: tr3({ uz: "Uch urish", ru: "Три удара" }), t: tr3({ uz: "— bitta funksiyani 3 xil kuch bilan chaqirib, natijalarni chop eting", ru: "— вызовите одну функцию с тремя разными силами и выведите результаты" }) }];
  const GLOSSARY = [{ b: tr3({ uz: "Funksiya", ru: "Функция" }), t: tr3({ uz: "— qayta ishlatiladigan kod bloki", ru: "— многоразовый блок кода" }) }, { b: "function", t: tr3({ uz: "— funksiya e'lon qilish kaliti", ru: "— ключевое слово объявления функции" }) }, { b: tr3({ uz: "Parametr", ru: "Параметр" }), t: tr3({ uz: "— kirish o'zgaruvchisi", ru: "— входная переменная" }) }, { b: tr3({ uz: "Argument", ru: "Аргумент" }), t: tr3({ uz: "— chaqiruvda beriladigan haqiqiy qiymat", ru: "— настоящее значение при вызове" }) }, { b: "return", t: tr3({ uz: "— natijani qaytarish", ru: "— возврат результата" }) }, { b: tr3({ uz: "Chaqirish", ru: "Вызов" }), t: tr3({ uz: "— funksiyani ishga tushirish (call)", ru: "— запуск функции (call)" }) }, { b: "undefined", t: tr3({ uz: `— return bo'lmasa qaytadigan "hech narsa"`, ru: "— «ничего», если нет return" }) }];
  ...
  return <Stage eyebrow={{ uz: "Tayyor", ru: "Готово" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr3({ uz: "Dars tugadi", ru: "Урок окончен" })}</span><h2 className="title h-title fade-up d1">{tr3({ uz: <>Endi murakkab ishni bitta <span className="italic" style={{ color: T.accent }}>nom</span> ortiga yashira olasiz.</>, ru: <>Теперь вы умеете прятать сложную работу за одним <span className="italic" style={{ color: T.accent }}>именем</span>.</> })}</h2>{
  ...
  />
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>{tr3({ uz: "📝 Uyga vazifa", ru: "📝 Домашнее задание" })}</div><p className="body" style={{ margin: "0 0 10px", color: T.ink }}>{tr3({ uz: "Funksiyalar bilan mashq qiling:", ru: "Потренируйтесь с функциями:" })}</p><ul>{HOMEWORK.map((h, i) => <li key={i}><b>{h.b}</b> <span className="t">{h.t}</span></li>)}</ul><p className="hw-note">{tr3({ uz: "Funksiya — kodni tartibli va qayta ishlatiladigan qiladi. Mashq qilsangiz, qo'lingizga o'tirib qoladi! 🚀", ru: "Функция делает код аккуратным и многоразовым. Потренируетесь — и рука сама запомнит! 🚀" })}</p></div>}
    ...
    return <div key={id} className={`ach-badge ${got ? "got" : "locked"}`} title={tr3(a.desc)}>
        ...
        <div ref={glossRef} className="gloss fade-up d4" style={{ scrollMarginBottom: 16 }}><div className="gloss-head" onClick={toggleGloss}><span className="lbl">{tr3({ uz: "💡 Kalit so'zlar (funksiya)", ru: "💡 Ключевые слова (функция)" })}</span><span className="gloss-toggle">{open ? "−" : "+"}</span></div>{open && <div className="gloss-body">{GLOSSARY.map((g, i) => <span key={i}><b>{g.b}</b> {g.t}{i < GLOSSARY.length - 1 ? " · " : ""}</span>)}</div>}</div>
      </div>
    </Stage>;
};
  ...
  return <Stage eyebrow={{ uz: "Natijalar", ru: "Результаты" }} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        ...
        {!isLive ? <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 16, alignItems: "center" }}>
          ...
          </div> : !loaded2 ? <p className="mono small fade-up" style={{ color: T.ink2 }}>{tr3({ uz: "Natijalar yuklanmoqda…", ru: "Загружаем результаты…" })}</p> : board.length === 0 ? <div className="frame-soft fade-up"><p className="body" style={{ margin: 0 }}>{tr3({ uz: "Bu sessiyaga hali hech kim qo'shilmagan.", ru: "К этой сессии пока никто не присоединился." })}</p></div> : <>
            <Confetti />
            {
    ...
    return <div key={q} className="qstat-row">
              ...
              {live.mode === "mentor" && <p className="small" style={{ margin: "10px 0 0", color: T.ink2 }}>{tr3({ uz: "⚠️ belgili savollar — sinf qiynalgan mavzular. Qayta tushuntirish tavsiya etiladi.", ru: "Вопросы с ⚠️ — темы, где класс споткнулся. Рекомендуется объяснить ещё раз." })}</p>}
            </div>
          </>}
      </div>
    </Stage>;
...
function AchCelebrate({ ach, onDone }) {
  ...
  return <div className="acu-overlay" onClick={onDone} role="status" aria-label={`${tr3({ uz: "Yangi nishon:", ru: "Новая награда:" })} ${ach.name}`}>
...
var CsWordmark = ({ onClick, disabled, hint, stats = true, bolt = true, liveOn = false }) => {
  ...
  >
      ...
      {hint && <span className={`cs-enter ${disabled ? "wait" : ""}`}>{tr3(hint)}</span>}
  ...
  return <div className="qz-arena">
    ...
    /* ===== LOBBY ===== */
      ...
      {phase === "lobby" && <div className="qz-view fade-step">
          <CsWordmark />
          <p className="qz-sub" style={{ marginTop: -4 }}>{tr3({ uz: "Tezroq to'g'ri bossangiz — ko'proq ball. Ketma-ket to'g'ri javoblar 🔥 bonus beradi!", ru: "Чем быстрее верный ответ — тем больше баллов. Серия верных ответов подряд даёт 🔥 бонус!" })}</p>
    ...
    /* ===== YAKUN — PODIUM ===== */
      ...
      {phase === "done" && <div className="qz-view fade-step">
          ...
          {solo ? <div className="qz-solo-res">
              ...
              <p className="qz-sub">{tr3({ uz: "ball", ru: "баллов" })} · {soloScore.ok}/{QUIZ_BANK.length} {tr3({ uz: "to'g'ri", ru: "верных" })}{soloScore.maxStreak >= 2 ? tr3({ uz: ` · eng uzun streak 🔥x${soloScore.maxStreak}`, ru: ` · лучшая серия 🔥x${soloScore.maxStreak}` }) : ""}</p>
...
function JsFunctionsLesson({ lang: langProp, onFinished, onPractice, liveToken }) {
