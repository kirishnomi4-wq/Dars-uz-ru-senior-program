...
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared react src/2-Modull/PmLesson5.jsx
...
var QuestionScreen = ({ screen, scope, eyebrow, question, questionText, payloadQuestion, payloadOptions, options, correctIdx, explainCorrect, explainWrong, audioText, audioOk, audioWrong, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const pick = (i) => {
    ...
    if (live && live.recordAttempt) live.recordAttempt(screen, SCREEN_META[screen]?.id || `s${screen}`, i, Date.now() - mountTs.current, { question: pQ, options: pOpts, picked: pOpts[i], correct: pOpts[correctIdx], lang: typeof __lang !== "undefined" && __lang === "ru" ? "ru" : "uz" });
    if (audioText) {
    ...
    return <button key={i} className={cls} disabled={solved || isMentorLive} onClick={() => pick(i)} style={{ padding: picked !== null ? "clamp(9px,1.3vw,12px) clamp(15px,2.2vw,20px)" : "clamp(13px,1.9vw,17px) clamp(15px,2.2vw,20px)", fontSize: "clamp(15px,1.85vw,17px)", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <FeedbackBlock show={isMentorLive ? mReveal : picked !== null} isCorrect={isMentorLive ? true : solved && !wrongLocked} neutral={waiting}>
          <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: waiting ? T.blue : isMentorLive || solved && !wrongLocked ? T.success : T.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>
            {isMentorLive ? fmtCode(tr2({ uz: `✓ To'g'ri javob: ${String.fromCharCode(65 + correctIdx)} — ${options[correctIdx]}`, ru: `✓ Верный ответ: ${String.fromCharCode(65 + correctIdx)} — ${options[correctIdx]}` })) : waiting ? tr2({ uz: "📨 Javobingiz qabul qilindi", ru: "📨 Ваш ответ принят" }) : wrongLocked ? fmtCode(tr2({ uz: `To'g'ri javob: ${String.fromCharCode(65 + correctIdx)} — ${options[correctIdx]}`, ru: `Верный ответ: ${String.fromCharCode(65 + correctIdx)} — ${options[correctIdx]}` })) : solved ? tr2({ uz: "To'g'ri", ru: "Верно" }) : tr2({ uz: "Qaytadan urinib ko'ring", ru: "Попробуйте ещё раз" })}
          </p>
          <p className="body" style={{ margin: 0 }}>
            {isMentorLive ? fmtCode(explainCorrect) : waiting ? tr2({ uz: "📨 Javobingiz qabul qilindi. Hozir to'g'ri javobni bilib olasiz.", ru: "📨 Ваш ответ принят. Сейчас узнаете верный ответ." }) : wrongLocked ? fmtCode(explainWrong[picked] ?? explainWrong.default) : solved ? fmtCode(explainCorrect) : fmtCode(explainWrong[picked] ?? explainWrong.default)}
          </p>
          {
...
var MentorPracticeStats = ({ live, screen, label }) => {
  ...
  return <div className="mstats fade-up">
      <div className="mstats-head">
        <span className="mstats-lbl">{label || tr2({ uz: "👀 Kim bajardi", ru: "👀 Кто выполнил" })}</span>
        <span className="mstats-n">{allIn ? tr2({ uz: "✓ Hamma bajardi!", ru: "✓ Все выполнили!" }) : <>{tr2({ uz: "Bajardi: ", ru: "Выполнили: " })}<b>{doneN}</b> / {total}</>}</span>
      </div>
      ...
      {doneN === 0 && <p className="mstats-wait">{tr2({ uz: "O'quvchilar bajarishi bilan shu yerda ✓ belgisi chiqadi…", ru: "Как только ученики выполнят, здесь появится ✓…" })}</p>}
    </div>;
};
var StudentPracticePulse = ({ live, screen }) => {
  const isStudent = !!(live && live.mode === "student" && live.pin);
  const pin = live ? live.pin : null;
  const [st, setSt] = useState3({ total: 0, done: 0, ok: false });
  useEffect4(() => {
    if (!isStudent) return;
    let on = true, t = null;
    const tick = async () => {
  ...
  return <p className="cls-pulse fade-step">{tr2({ uz: "👥 Sinfda: ", ru: "👥 В классе: " })}<b>{st.done}</b>{tr2({ uz: " bajardi · ✏️ ", ru: " выполнили · ✏️ " })}<b>{busy}</b>{tr2({ uz: " hali bajarmoqda", ru: " ещё выполняют" })}</p>;
};
var MentorBypassLine = ({ live }) => {
  if (!live || live.mode !== "mentor") return null;
  return <p className="mbypass">{tr2({ uz: "👨‍🏫 Jonli darsda bu amaliyotni o'quvchilar bajaradi — siz kuzatasiz; «Davom etish» siz uchun ochiq", ru: "👨‍🏫 В живом уроке это задание выполняют ученики — вы наблюдаете; «Продолжить» для вас открыто" })}</p>;
};
var HOOK_OPTS = [
...
var ScrHook = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage
    eyebrow={tr2({ uz: "Ochilish kuni 🚀", ru: "День открытия 🚀" })}
    screen={screen}
    navContent={<NavNext optionalLive disabled={isMentorLive ? !mReveal : picked === null} label={isMentorLive ? mReveal ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Avval natijani oching", ru: "Сначала откройте результат" }) : picked === null ? tr2({ uz: "Bittasini tanlang", ru: "Выберите один" }) : tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}
  >
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>Bitta odam bir haftada 12 ta ishning <span className="italic" style={{ color: T.accent }}>nechtasini</span> bajaradi?</>, ru: <>Сколько из 12 дел <span className="italic" style={{ color: T.accent }}>сделает</span> один человек за неделю?</> })}</h1>
        <Mentor>{tr2({ uz: <>Kinoteatr egasi sayt uchun <b style={{ color: T.ink }}>12 ta</b> ish aytdi. Ochilishgacha bir hafta, sayt ustida bitta odam ishlaydi.</>, ru: <>Владелец кинотеатра назвал для сайта <b style={{ color: T.ink }}>12 дел</b>. До открытия неделя, над сайтом работает один человек.</> })}</Mentor>
        <MentorNote>{tr2({ uz: "Javobni oldindan aytmang. Hamma ovoz bergach «Natijani ochish»ni bosing va ayting: «Uchta. Demak qolgan to'qqiztasi bilan nima qilamiz — shuni bugun hal qilamiz.»", ru: "Не называйте ответ заранее. Когда все проголосуют, нажмите «Открыть результат» и скажите: «Три. Значит, что делать с оставшимися девятью — решим сегодня»." })}</MentorNote>
        <div className="hk-work fade-up delay-1">
          <div className="hk-list">
            {["🎬", "🎫", "🕒", "🍿", "🎟", "⭐", "📸", "💬", "🎥", "🏷", "📞", "🧾"].map((e, i) => <span key={i} className="hk-dot" style={{ animationDelay: `${0.1 + i * 0.05}s` }}>{e}</span>)}
          </div>
          <span className="hk-cap">{tr2({ uz: "12 ta ish · 1 hafta · 1 odam", ru: "12 дел · 1 неделя · 1 человек" })}</span>
        </div>
    ...
    return <button key={o.id} className={`hook-option ${on ? "on" : ""} ${win ? "hk-win" : ""}`} disabled={picked !== null || isMentorLive} onClick={() => pick(o.id)}>
                <span className="radio">{on && <span className="radio-dot" />}</span><span>{tr2(o)}</span>
    ...
    setMReveal(true);
    if (live) live.mentorReveal(screen);
  }}>{tr2({ uz: "🔓 Natijani ochish", ru: "🔓 Открыть результат" })}</button>}
        {revealed && <p className="hook-ack fade-step">{tr2({ uz: <>Uchta. Qolgan to'qqizta ish bilan nima qilamiz — <b>bugun shuni hal qilamiz</b>.</>, ru: <>Три. Что делать с оставшимися девятью — <b>решим сегодня</b>.</> })}</p>}
      </div>
    </Stage>;
};
var DEMO_LIST = {
...
var ScrGoal = ({ screen, onNext, onPrev }) => <Stage
  eyebrow={tr2({ uz: "Bugungi natija", ru: "Сегодняшний результат" })}
  screen={screen}
  mentorStatic
  navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}
>
    <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
      <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Dars oxirida <span className="italic" style={{ color: T.accent }}>nimani</span> bilib olasiz?</>, ru: <>Что вы <span className="italic" style={{ color: T.accent }}>узнаете</span> к концу урока?</> })}</h2></div>
      <Mentor>{tr2({ uz: <>Katta ro'yxatni uch bo'lakka ajratishni bilib olasiz: uchta ish ochilish kuniga, qolganlari navbatiga qarab. Quyida namunasi o'z-o'zidan yozilib chiqadi.</>, ru: <>Вы научитесь делить большой список на три части: три дела — ко дню открытия, остальные — по очереди. Ниже образец заполняется сам.</> })}</Mentor>
      <div className="lp-card fade-up delay-1">
        <div className="lp-head"><span className="lp-ic">🚀</span><span className="lp-h">{tr2({ uz: "Ochilish ro'yxati", ru: "Список к открытию" })}</span><span className="lp-src">{tr2({ uz: "namuna", ru: "образец" })}</span></div>
        <div className="lp-rows">
          {DEMO_LIST.v1.map((s, i) => <span key={i} className="lp-row" style={{ "--fd": `${0.9 + i * 0.8}s` }}>
              <span className="lp-n">{i + 1}</span>
              <span className="lp-slot"><span className="lp-ph">{tr2({ uz: "…", ru: "…" })}</span><span className="lp-fill">{tr2(s)}</span></span>
              <span className="lp-stamp">{tr2({ uz: "OCHILISHGA", ru: "К ОТКРЫТИЮ" })}</span>
            </span>)}
        </div>
        <div className="lp-rest">
          <span className="lp-mini lvl lvl-v2" style={{ "--fd": "3.4s" }}>⚡ <b>{tr2({ uz: "Keyingi versiya", ru: "Следующая версия" })}</b>: {DEMO_LIST.v2.map((x) => tr2(x)).join(", ")}</span>
          <span className="lp-mini lvl lvl-backlog" style={{ "--fd": "3.9s" }}>🌱 <b>{tr2({ uz: "Keyinga qoldirilganlar", ru: "Отложенные" })}</b>: {DEMO_LIST.backlog.map((x) => tr2(x)).join(", ")}</span>
        </div>
      </div>
      <div className="takeaway fade-up delay-2"><span className="ta-bulb">🎯</span><p className="ta-h">{tr2({ uz: "Dars oxirida shunday ro'yxatni o'zingiz tuzasiz.", ru: "К концу урока такой список вы составите сами." })}</p></div>
    </div>
  </Stage>;
var ScrSplit = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [split, setSplit] = useState3(!!storedAnswer);
  const [seen, setSeen] = useState3(() => new Set(storedAnswer ? FALLBACK_FEATURES.map((f) => f.id) : []));
  ...
  return <Stage
    eyebrow={tr2({ uz: "Katta ish", ru: "Большая работа" })}
    screen={screen}
    navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : !split ? tr2({ uz: "Avval kartani bosing", ru: "Сначала нажмите карточку" }) : tr2({ uz: `Yana ${FALLBACK_FEATURES.length - seen.size} bo'lakni oching`, ru: `Откройте ещё ${FALLBACK_FEATURES.length - seen.size} частей` })} onClick={onNext} /></>}
  >
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bitta ish yoki <span className="italic" style={{ color: T.accent }}>oltita</span> ish?</>, ru: <>Одна работа или <span className="italic" style={{ color: T.accent }}>шесть</span> работ?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>«Kinoteatrga sayt qilish» bitta ish bo'lib ko'rinadi. Kartani bosing — u alohida ishlarga bo'linadi.</>, ru: <>«Сделать сайт для кинотеатра» выглядит как одно дело. Нажмите на карточку — она разделится на отдельные дела.</> })}</Mentor>
        {!split ? (
    ...
    <div className="dc-stage fade-up delay-1">
            <button className="dc-big tap-hint-card" onClick={() => setSplit(true)}>
              ...
              <span className="dc-big-t">{tr2({ uz: "Kinoteatrga sayt qilish", ru: "Сделать сайт для кинотеатра" })}</span>
              <span className="dc-big-cue">{tr2({ uz: "👆 bosing — bo'laklarga ajraladi", ru: "👆 нажмите — разделится на части" })}</span>
            </button>
          </div>
  ) : <>
    ...
    return <button key={f.id} className={`dc-piece ${on ? "on" : ""} ${f.id === nextId ? "tap-hint-card" : ""}`} style={{ "--fd": `${i * 0.07}s` }} onClick={() => open(f.id)}>
                    ...
                    <span className="dc-piece-t">{on ? tr2(f) : tr2({ uz: "bo'lak", ru: "часть" })}</span>
                    {on && <span className="dc-piece-ok">✓</span>}
                  </button>;
  })}
            </div>
            <p className="small mono" style={{ color: T.ink2, margin: 0 }}>{seen.size}/{FALLBACK_FEATURES.length} {tr2({ uz: "bo'lak ochildi", ru: "частей открыто" })}</p>
          </>}
        <MentorBypassLine live={live} />
        {done && <div className="frame-success fade-step">
            <p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Katta ishni shunday bo'laklarga bo'lish <b>dekompozitsiya</b> deyiladi. Har bo'lak — saytning bitta <b>imkoniyati</b>.</>, ru: <>Такое разбиение большой работы на части называется <b>декомпозиция</b>. Каждая часть — одна <b>возможность</b> сайта.</> })}</p>
          </div>}
      </div>
    </Stage>;
};
...
var ScrPiece = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === "mentor");
  const [opened, setOpened] = useState3(() => /* @__PURE__ */ new Set());
  const [tried, setTried] = useState3(() => new Set(storedAnswer ? S3_ITEMS.map((i) => i.id) : []));
  ...
  return <Stage
    eyebrow={tr2({ uz: "Bo'lak qanday yoziladi", ru: "Как записывается часть" })}
    screen={screen}
    navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Oxiri ko'rinadigan yana ${2 - goodFound} yozuvni toping`, ru: `Найдите ещё ${2 - goodFound} записи с видимым концом` })} onClick={onNext} /></>}
  >
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Qaysi ishning <span className="italic" style={{ color: T.accent }}>oxiri</span> ko'rinadi?</>, ru: <>У какой работы <span className="italic" style={{ color: T.accent }}>виден конец</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Yaxshi bo'lakni alohida qilib, tugatib bo'ladi: qilindi — tamom. To'rt yozuvni bosib ko'ring, ikkitasining oxiri ko'rinadi.</>, ru: <>Хорошую часть можно сделать отдельно и завершить: сделано — всё. Нажмите на четыре записи: у двух конец виден.</> })}</Mentor>
    ...
    return <div key={it.id} className={`s3item ${wasTried ? it.good ? "good" : "plain" : it.id === nextId ? "tap-hint-card" : ""}`}>
                <button className="s3btn" onClick={() => toggle(it.id)}>
                  ...
                  <span className="s3txt">{tr2(it)}</span>
                  <span className="s3caret">{isOpen ? "▴" : "▾"}</span>
                </button>
                {isOpen && <p className="s3note fade-step">{tr2(it.note)}</p>}
              </div>;
  })}
        </div>
        <MentorBypassLine live={live} />
        {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0 }}>{tr2({ uz: <>Bo'lakning <b>boshi ham, oxiri ham</b> ko'rinib turishi kerak. Aks holda u qachon tugaganini hech kim ayta olmaydi.</>, ru: <>У части должны быть видны <b>и начало, и конец</b>. Иначе никто не скажет, когда она закончилась.</> })}</p></div>}
      </div>
    </Stage>;
};
var TESTS = {
...
var ScaleCard = ({ ic, title, need, cost, onNeed, onCost }) => <div className="tz-wrap">
    ...
    <div className="tz-beam" data-tilt={!need ? "flat" : need === "must" ? "left" : "right"}>
      <span className={`tz-pan left ${need === "must" ? "heavy" : ""}`}>🎯 <b>{tr2({ uz: "foyda", ru: "польза" })}</b></span>
      <span className={`tz-pan right ${cost === "week" ? "heavy" : ""}`}>🧱 <b>{tr2({ uz: "yuk", ru: "нагрузка" })}</b></span>
    </div>
    <div className="tz-qs">
      {!need ? <div className="tz-q fade-step">
          <span className="tz-q-t">1. {tr2(Q1_TEXT)}</span>
          <div className="tz-q-btns">{NEED_OPTS.map((o) => <button key={o.id} className="tz-opt" onClick={() => onNeed(o.id)}>{tr2(o)}</button>)}</div>
        </div> : <div className="tz-done"><span>✓ {tr2(NEED_OPTS.find((o) => o.id === need))}</span>{onNeed && <button className="tz-redo" onClick={() => onNeed(null)} title={tr2({ uz: "qayta tanlash", ru: "выбрать заново" })}>↻</button>}</div>}
      {!need ? <div className="tz-lock">⏳ {tr2({ uz: "Ikkinchi savol — javobdan keyin", ru: "Второй вопрос — после ответа" })}</div> : !cost ? <div className="tz-q fade-step">
          <span className="tz-q-t">2. {tr2(Q2_TEXT)}</span>
          <div className="tz-q-btns">{COST_OPTS.map((o) => <button key={o.id} className="tz-opt" onClick={() => onCost(o.id)}>{tr2(o)}</button>)}</div>
        </div> : <div className="tz-done"><span>✓ {tr2(COST_OPTS.find((o) => o.id === cost))}</span>{onCost && <button className="tz-redo" onClick={() => onCost(null)} title={tr2({ uz: "qayta tanlash", ru: "выбрать заново" })}>↻</button>}</div>}
    </div>
  </div>;
var ScrScaleDemo = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === "mentor");
  const [need, setNeed] = useState3(null);
  const [cost, setCost] = useState3(null);
  ...
  return <Stage
    eyebrow={tr2({ uz: "Ikki savol", ru: "Два вопроса" })}
    screen={screen}
    navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : !need ? tr2({ uz: "Birinchi savolga javob bering", ru: "Ответьте на первый вопрос" }) : tr2({ uz: "Ikkinchi savolga javob bering", ru: "Ответьте на второй вопрос" })} onClick={onNext} /></>}
  >
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Har bo'lakdan <span className="italic" style={{ color: T.accent }}>nima</span> so'raymiz?</>, ru: <>Что мы <span className="italic" style={{ color: T.accent }}>спрашиваем</span> у каждой части?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Har bo'lak ikki savoldan o'tadi. Pastdagi imkoniyatni sinab ko'ring — javoblaringizdan keyin karta o'zi joyini topadi.</>, ru: <>Каждая часть проходит два вопроса. Попробуйте на возможности ниже — после ваших ответов карточка сама найдёт своё место.</> })}</Mentor>
        <div className="fade-up delay-1">
          <ScaleCard ic="📸" title={tr2({ uz: "Kinoteatr fotosuratlari", ru: "Фотографии кинотеатра" })} need={need} cost={cost} onNeed={setNeed} onCost={setCost} />
        </div>
        <MentorBypassLine live={live} />
        {done && <div className="frame-success fade-step">
            <p className="body" style={{ margin: 0 }}><span className={`lv-chip lvl lvl-${lvl}`}>{bucketOf(lvl).ic} {tr2(bucketOf(lvl))}</span> — {tr2(noteFor(need, cost))}</p>
            <p className="body" style={{ margin: "8px 0 0" }}>{tr2({ uz: <>Sayt ish beradigan eng sodda birinchi versiya — shu ro'yxat <b>MVP</b> deb ataladi.</>, ru: <>Самая простая рабочая первая версия — этот список называется <b>MVP</b>.</> })}</p>
          </div>}
      </div>
    </Stage>;
};
...
var ScrCase = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const isMentor = !!(live && live.mode === "mentor");
  const [i, setI] = useState3(0);
  const [seen, setSeen] = useState3(storedAnswer ? K3_SLIDES.length - 1 : 0);
  const [guess, setGuess] = useState3({});
  ...
  return <Stage
    eyebrow={tr2({ uz: "Biznes olamidan mashhur voqea 📸", ru: "Известная история из мира бизнеса 📸" })}
    screen={screen}
    navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Voqeani oxirigacha ko'ring", ru: "Досмотрите историю до конца" })} onClick={onNext} /></>}
  >
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Hamma imkoniyatni <span className="italic" style={{ color: T.accent }}>o'chirgan</span> ilova</>, ru: <>Приложение, которое <span className="italic" style={{ color: T.accent }}>удалило</span> все возможности</> })}</h2></div>
        {pr && !answered ? <div className="k-predict fade-step" key={`p${i}`}>
            <span className="k-predict-lbl">{tr2({ uz: "🎲 Avval o'zingiz belgilab ko'ring", ru: "🎲 Сначала отметьте свой вариант" })}</span>
            <p className="k-predict-q">{tr2(pr.q)}</p>
            <div className="k-predict-opts">
              {pr.opts.map((o, k) => <button key={k} className="k-predict-opt" onClick={() => setGuess((g) => ({ ...g, [i]: k }))}>{tr2(o)}</button>)}
            </div>
            ...
            <h3 className="k-slide-h">{tr2(cur.h)}</h3>
            <p className="k-slide-body">{tr2(cur.body)}</p>
            {pr && guess[i] !== pr.right && <p className="k-miss">{tr2(pr.miss)}</p>}
          </div>}
        <div className="k-nav">
          <button className="btn-soft" disabled={i === 0} onClick={() => go(i - 1)}>{tr2({ uz: "← Orqaga", ru: "← Назад" })}</button>
          <span className="k-dots">{K3_SLIDES.map((_, k) => <span key={k} className={`k-dot ${k === i ? "cur" : k < i ? "fill" : ""}`} />)}</span>
          <button className="btn" disabled={last || !answered} onClick={() => go(i + 1)}>{tr2({ uz: "Keyingisi →", ru: "Дальше →" })}</button>
        </div>
        {
...
var ScrScale = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === "mentor");
  const [items] = useState3(() => readIncomingFeatures() || FALLBACK_FEATURES);
  const own = !!readIncomingFeatures();
  const [res, setRes] = useState3(() => readDraft() || {});
  const [need, setNeed] = useState3(null);
  const [cost, setCost] = useState3(null);
  ...
  const [lastNote, setLastNote] = useState3(null);
  const commit = (n, c) => {
  ...
  return <Stage
    eyebrow={tr2({ uz: "Tarozi", ru: "Весы" })}
    screen={screen}
    navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Yana ${items.length - doneN} imkoniyatni tarozidan o'tkazing`, ru: `Взвесьте ещё ${items.length - doneN} возможностей` })} onClick={onNext} /></>}
  >
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Har imkoniyatni <span className="italic" style={{ color: T.accent }}>tarozidan</span> o'tkazing</>, ru: <>Взвесьте <span className="italic" style={{ color: T.accent }}>каждую</span> возможность</> })}</h2></div>
        <Mentor>{own ? tr2({ uz: <>Ro'yxat boshida — 2-darsda o'zingiz yozgan imkoniyatlar, qolganini kinoteatr namunasidan oldik. Ikki savoldan keyin karta o'zi tushadigan joyini topadi.</>, ru: <>В начале списка — возможности, которые вы записали на втором уроке, остальные взяты из образца кинотеатра. После двух вопросов карточка сама найдёт своё место.</> }) : tr2({ uz: <>Bu ro'yxat — kinoteatr sayti uchun tayyor namuna. Ikki savoldan keyin karta o'zi tushadigan joyini topadi.</>, ru: <>Этот список — готовый образец для сайта кинотеатра. После двух вопросов карточка сама найдёт своё место.</> })}</Mentor>
  ...
  />
          </div> : <div className="frame-success fade-step"><p className="body" style={{ margin: 0 }}>{tr2({ uz: "Oltala imkoniyat tarozidan o'tdi. Endi ochilish ro'yxatini uchtaga keltiramiz.", ru: "Все шесть возможностей взвешены. Теперь доведём список к открытию до трёх." })}</p></div>}
        {lastNote && cur && <p className={`tz-note lvl lvl-${lastNote.lvl} fade-step`}>{bucketOf(lastNote.lvl).ic} {tr2(lastNote.note)}</p>}
        <div className="bkt-row fade-up delay-2">
          {counts.map((b) => <span key={`${b.key}-${b.n}`} className={`bkt lvl lvl-${b.key} ${b.n ? "has" : ""} ${lastNote && lastNote.lvl === b.key ? "land" : ""}`}>{b.ic} {tr2(b)} <b>{b.n}</b></span>)}
        </div>
        <MentorBypassLine live={live} />
        <MentorNote>{tr2({ uz: "Bu ekranda to'g'ri javob yo'q — bahslashmang. Uchtaga sig'may qolgan o'quvchiga vaqt-chegarasini eslatish kifoya.", ru: "На этом экране нет правильного ответа — не спорьте. Ученику, у кого не влезает в три, достаточно напомнить об ограничении времени." })}</MentorNote>
        <StudentPracticePulse live={live} screen={screen} />
        <MentorPracticeStats live={live} screen={screen} label={tr2({ uz: "⚖️ Tarozidan o'tkazganlar", ru: "⚖️ Кто взвесил" })} />
      </div>
    </Stage>;
};
var V1_CAP = 3;
var ScrLaunchList = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const lockLabel = v1.length > V1_CAP ? tr2({ uz: `🔒 Yana ${v1.length - V1_CAP} tasini ⚡ ga suring`, ru: `🔒 Переместите ещё ${v1.length - V1_CAP} в ⚡` }) : tr2({ uz: `🔒 Yana ${V1_CAP - v1.length} tasini 🔥 ga ko'taring`, ru: `🔒 Поднимите ещё ${V1_CAP - v1.length} в 🔥` });
  const save = () => {
  ...
  const Col2 = ({ b, list, up, down, gloss }) => <div className={`ll-col lvl lvl-${b.key} ${b.key}`}>
      <div className="ll-col-h">{b.ic} {tr2(b)} <b>{list.length}{b.key === "v1" ? `/${V1_CAP}` : ""}</b></div>
      {gloss && <span className="ll-gloss">{gloss}</span>}
      {list.length === 0 && <span className="ll-empty">{tr2({ uz: "hozircha bo'sh", ru: "пока пусто" })}</span>}
      {list.map((f) => <div key={f.id} className="ll-item">
          <span className="ll-ic">{f.ic}</span><span className="ll-t">{tr2(f)}</span>
          {!saved && up && <button className="ll-mv" title={tr2({ uz: "yuqoriga", ru: "выше" })} onClick={() => move(f.id, up)}>▲</button>}
          {!saved && down && <button className="ll-mv" title={tr2({ uz: "pastga", ru: "ниже" })} onClick={() => move(f.id, down)}>▼</button>}
        </div>)}
    </div>;
  return <Stage
    eyebrow={tr2({ uz: "Ochilish ro'yxati", ru: "Список к открытию" })}
    screen={screen}
    navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!saved && !isMentor} label={saved || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : ready ? tr2({ uz: "🔒 Ro'yxatni saqlang", ru: "🔒 Сохраните список" }) : lockLabel} onClick={onNext} /></>}
  >
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Ochilish ro'yxatini <span className="italic" style={{ color: T.accent }}>uchtaga</span> keltiring</>, ru: <>Доведите список к открытию <span className="italic" style={{ color: T.accent }}>до трёх</span></> })}</h2></div>
        <Mentor>{tr2({ uz: <>Ochilishgacha bir hafta bor, sayt ustida bitta odam ishlaydi. U bir haftada atigi uchta ishni bajaradi.</>, ru: <>До открытия неделя, над сайтом работает один человек. За неделю он сделает всего три дела.</> })}</Mentor>
        <div className="ll-grid fade-up delay-1">
          <Col2 b={BUCKETS[0]} list={v1} down="v2" />
          <Col2 b={BUCKETS[1]} list={v2} up="v1" down="backlog" />
          <Col2 b={BUCKETS[2]} list={bl} up="v2" gloss={tr2({ uz: "uni backlog deyishadi · hech narsa o'chirilmaydi, navbati suriladi", ru: "его называют бэклог · ничего не удаляется, сдвигается очередь" })} />
        </div>
        {!saved ? <button className="btn" style={{ alignSelf: "flex-start" }} disabled={!ready} onClick={save}>{ready ? tr2({ uz: "✓ Ro'yxatni saqlash", ru: "✓ Сохранить список" }) : lockLabel}</button> : <div className="done-mini fade-step">✅ {tr2({ uz: "Saqlandi!", ru: "Сохранено!" })} <span className="dm-sub">{tr2({ uz: "— ochilish ro'yxatida uchta ish bor", ru: "— в списке к открытию три дела" })}</span></div>}
        <MentorBypassLine live={live} />
        <MentorNote>{tr2({ uz: "Baholash mezoni: 3/3 = qabul · 2/3 = joyida to'g'rilanadi · undan kam = mentor bilan qaytadan.", ru: "Критерий оценки: 3/3 — принято · 2/3 — правится на месте · меньше — заново с ментором." })}</MentorNote>
        <StudentPracticePulse live={live} screen={screen} />
        <MentorPracticeStats live={live} screen={screen} label={tr2({ uz: "🚀 Ro'yxatni saqlaganlar", ru: "🚀 Кто сохранил список" })} />
      </div>
    </Stage>;
};
var FE_CARDS = [
...
var ScrFindError = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [placed, setPlaced] = useState3(!!storedAnswer);
  const [miss, setMiss] = useState3(null);
  const [wrongPlace, setWrongPlace] = useState3(false);
  ...
  return <Stage
    eyebrow={tr2({ uz: "Boshqa guruhning ro'yxati", ru: "Список другой группы" })}
    screen={screen}
    navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : !found ? tr2({ uz: "Avval noto'g'ri kartani toping", ru: "Сначала найдите неверную карточку" }) : tr2({ uz: "Endi unga to'g'ri ro'yxatni tanlang", ru: "Теперь выберите ей верный список" })} onClick={onNext} /></>}
  >
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <><span className="italic" style={{ color: T.accent }}>O'z joyida</span> turmagan kartani toping</>, ru: <>Найдите карточку <span className="italic" style={{ color: T.accent }}>не на своём месте</span></> })}</h2></div>
        <Mentor>{tr2({ uz: <>Boshqa guruh xuddi shu kinoteatr uchun ro'yxat tuzdi. Bitta karta noto'g'ri ro'yxatga tushib qolgan.</>, ru: <>Другая группа составила список для того же кинотеатра. Одна карточка попала не в тот список.</> })}</Mentor>
        <div className="fe-grid fade-up delay-1">
          {BUCKETS.map((b) => <div key={b.key} className={`fe-col lvl lvl-${b.key} ${b.key}`}>
              <div className="fe-col-h">{b.ic} {tr2(b)}</div>
              {FE_CARDS.filter((c) => c.b === b.key || found && placed && c.bad && b.key === "backlog").filter((c) => !(c.bad && found && placed && b.key === "v1")).map((c) => <button key={c.id} className={`fe-card ${c.bad && found ? "ok" : ""} ${miss === c.id ? "miss" : ""}`} disabled={found} onClick={() => tap(c)}>
                  <span>{c.ic} {tr2(c)}</span>{c.bad && found && <span className="fe-tick">✓</span>}
                </button>)}
            </div>)}
        </div>
        {miss && <p className="fe-note fade-step">{tr2({ uz: FE_CARDS.find((c) => c.id === miss).neutralUz, ru: FE_CARDS.find((c) => c.id === miss).neutralRu })}</p>}
        {found && !placed && <div className="fe-place fade-step">
            <p className="body" style={{ margin: "0 0 8px" }}>{tr2({ uz: "Topdingiz. Endi «Tomoshabin sharhlari» qaysi ro'yxatga tushishi kerak?", ru: "Нашли. Теперь: в какой список должны попасть «Отзывы зрителей»?" })}</p>
            <div className="fe-place-btns">{BUCKETS.map((b) => <button key={b.key} className={`tz-opt lvl lvl-${b.key}`} onClick={() => place(b.key)}>{b.ic} {tr2(b)}</button>)}</div>
            {wrongPlace && <p className="fe-note">{tr2({ uz: "Yana o'ylab ko'ring: busiz sayt ish beradimi va qancha vaqt oladi?", ru: "Подумайте ещё: работает ли сайт без этого и сколько времени это займёт?" })}</p>}
          </div>}
        <MentorBypassLine live={live} />
        {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0 }}>{tr2({ uz: "Yangi ochilgan kinoteatrda sharh yozadigan tomoshabin hali yo'q — busiz ham sayt ish beradi.", ru: "В только что открывшемся кинотеатре ещё некому писать отзывы — без этого сайт работает." })}</p></div>}
        <MentorNote>{tr2({ uz: "Tekshirish qoidasi: har kartadan so'rang — busiz sayt ochilish kuni ish beradimi? Javob «ha» bo'lsa, u ochilish ro'yxatida turmaydi.", ru: "Правило проверки: спросите у каждой карточки — работает ли сайт без неё в день открытия? Если «да» — ей не место в списке к открытию." })}</MentorNote>
      </div>
    </Stage>;
};
...
var ScrCoding = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage
    eyebrow={tr2({ uz: "Koding · 🛠 kompilyator", ru: "Кодинг · 🛠 компилятор" })}
    screen={screen}
    navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Avval kompilyatorda bajaring", ru: "Сначала выполните в компиляторе" })} onClick={onNext} /></>}
  >
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Ochilish ro'yxatini <span className="italic" style={{ color: T.accent }}>kod</span> o'zi ajratib beradi</>, ru: <>Список к открытию <span className="italic" style={{ color: T.accent }}>код</span> отберёт сам</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Ikki massiv tayyor: nomlar va darajalar. Shu ikkovidan ochilish ro'yxatini yig'adigan kod yozasiz.</>, ru: <>Два массива готовы: названия и уровни. Вы напишете код, который соберёт из них список к открытию.</> })}</Mentor>
        <div className="kdx fade-up delay-1">
          ...
          <div className="kdx-out">
            {preview.map((s, i) => <div key={i} className="kdx-card" style={{ "--kd": `${0.5 + i * 0.3}s` }}>
                <span className="hc-prev-badge lvl lvl-v1">🔥 {tr2({ uz: "ochilishga", ru: "к открытию" })}</span>{s}
              </div>)}
          </div>
        </div>
    ...
    writeKodingOpen(true);
          ...
          {!done && isSelf && <button className="kdx-skip" onClick={onNext}>{tr2({ uz: "✓ Bu mashqni sinfda bajarganman — davom etish →", ru: "✓ Это задание я выполнил в классе — продолжить →" })}</button>}
        </div>
        <div className="takeaway fade-up delay-2"><span className="ta-bulb">📌</span><p className="ta-h">{tr2({ uz: "Ochilish ro'yxati kod yozishdan oldin tuziladi — kod faqat tanlanganini quradi.", ru: "Список к открытию составляют до написания кода — код лишь строит выбранное." })}</p></div>
        {done && <div className="done-mini fade-step">✅ {tr2({ uz: "Ishladi!", ru: "Сработало!" })} <span className="dm-sub">{tr2({ uz: "— koddan uchta nom chiqdi", ru: "— из кода вышли три названия" })}</span></div>}
        <MentorBypassLine live={live} />
        <MentorNote>{tr2({ uz: "Vaqt qoidasi: 10 daqiqa. Obyekt va .push bu darsgacha o'tilmagan — so'ralsa «keyingi modulda» deng, kodga kiritmang.", ru: "Правило времени: 10 минут. Объект и .push до этого урока не проходили — если спросят, скажите «в следующем модуле», в код не вводите." })}</MentorNote>
...
function PairTimer({ onStage }) {
  ...
  return <div className="pair-timer">
      {st.running ? <div className="pair-live">
        ...
        </div> : <p className="pair-now" style={{ margin: 0 }}>{st.done ? tr2({ uz: "✓ Vaqt tugadi — ikkalangiz ham aytib bo'ldingiz. Barakalla!", ru: "✓ Время вышло — вы оба рассказали. Молодцы!" }) : tr2({ uz: "Har biringizga 30 soniyadan — avval A, keyin B.", ru: "По 30 секунд каждому — сначала A, потом B." })}</p>}
      <div className="pair-timer-btns">
        {!st.running && <button className={st.done ? "btn-soft" : "pair-start"} onClick={() => setSt({ running: true, left: 60, done: false })}>{st.done ? tr2({ uz: "↻ Yana 1 daqiqa", ru: "↻ Ещё 1 минута" }) : tr2({ uz: "▶ 1 daqiqani boshlash", ru: "▶ Запустить 1 минуту" })}</button>}
...
var ScrRecap = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage
    eyebrow={tr2({ uz: "Yoddan", ru: "По памяти" })}
    screen={screen}
    navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!written && !isMentor} label={written || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Bir qator yozing", ru: "Напишите одну строку" })} onClick={onNext} /></>}
  >
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Uchta ishni <span className="italic" style={{ color: T.accent }}>yoddan</span> ayta olasizmi?</>, ru: <>Сможете назвать три дела <span className="italic" style={{ color: T.accent }}>по памяти</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Ekranga qaramasdan sherigingizga ayting: qaysi uchta ishni tanladingiz va nega aynan ular ochilish kuniga tushdi.</>, ru: <>Не глядя на экран, расскажите напарнику: какие три дела вы выбрали и почему именно они попали ко дню открытия.</> })}</Mentor>
        <div className="rcp-flow">
          <div className="rcp-step fade-up delay-1">
            <div className="rcp-step-h"><span className="rcp-n">1</span><div><span className="rcp-t">{tr2({ uz: "🗣 Sherigingizga ayting", ru: "🗣 Расскажите напарнику" })}</span></div></div>
            <PairTimer />
          </div>
          <div className="rcp-step fade-up delay-2">
            ...
            <input className="reflect-input" value={text} onChange={(e) => save(e.target.value)} placeholder={tr2({ uz: "Ochilish kuniga ... tanladim, chunki ...", ru: "Ко дню открытия я выбрал ..., потому что ..." })} maxLength={160} />
            {written && <p className="small" style={{ margin: 0, color: T.success, fontWeight: 700 }}>{tr2({ uz: "✓ Yozildi!", ru: "✓ Записано!" })}</p>}
          </div>
        </div>
        <MentorBypassLine live={live} />
        <MentorNote>{tr2({ uz: "Sinfning uchdan biridan ko'pi «nega aynan bu uchtasi» degan savolga javob berolmasa — vaqt-chegarasi gapini qayta tushuntiring.", ru: "Если больше трети класса не отвечает на вопрос «почему именно эти три» — объясните ограничение по времени заново." })}</MentorNote>
      </div>
    </Stage>;
};
var HW_OPTS = [
...
var ScrSummary = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  const [hwPick, setHwPick] = useState3(() => {
    try {
  ...
  return <Stage eyebrow={tr2({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash", ru: "Завершить" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l">
          <span className="done-chip fade-up"><span className="tick">{Ico.check(11)}</span> {tr2({ uz: "Dars tugadi", ru: "Урок завершён" })}</span>
          <h2 className="title h-title fade-up d1">{tr2({ uz: <>Endi siz ochilish ro'yxatini <span className="italic" style={{ color: T.accent }}>tuza olasiz</span>.</>, ru: <>Теперь вы <span className="italic" style={{ color: T.accent }}>умеете составлять</span> список к открытию.</> })}</h2>
          {
    /* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */
        ...
        <Zoomable>
          <div className="split">
            <div className="card fade-up d3"><div className="card-lbl" style={{ color: T.success }}><span style={{ color: T.success, display: "inline-flex" }}>{Ico.check(15)}</span> {tr2({ uz: "Endi siz bilasiz", ru: "Теперь вы знаете" })}</div><ul className="recap">{SUM_RECAP.map((r, i) => <li key={i} style={{ animationDelay: `${0.3 + i * 0.07}s` }}><span className="ck" style={{ display: "inline-flex" }}>{Ico.check(15)}</span><span>{tr2(r)}</span></li>)}</ul></div>
            <div className="card fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>🚀 {tr2({ uz: "Sizning ochilish ro'yxatingiz", ru: "Ваш список к открытию" })}</div>
              {mvp && Array.isArray(mvp.v1) && mvp.v1.length ? <ul>{mvp.v1.map((x, i) => <li key={i}><b>{i + 1}.</b> <span className="t">{x}</span></li>)}</ul> : <p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: "Ro'yxat hali saqlanmagan.", ru: "Список ещё не сохранён." })}</p>}
            </div>
          </div>
          {
          ...
          {hwOpen && <div className="card hw fade-up d4" style={{ marginTop: "clamp(12px,2vw,18px)" }}>
            ...
            <div className="hw-cards" style={{ marginTop: 8 }}>
              {HW_OPTS.map((o) => <button key={o.id} className={`hw-card ${hwPick === o.id ? "on" : ""}`} onClick={() => chooseHw(o.id)}>
                  <span className="hw-card-h">{o.ic} {tr2(o.title)}</span>
                  <ul className="hw-card-list">{o.items.map((it, i) => <li key={i}>{tr2(it)}</li>)}</ul>
                  {hwPick === o.id && <span className="hw-card-ok">✓ {tr2({ uz: "tanlandi", ru: "выбрано" })}</span>}
                </button>)}
            </div>
          </div>}
        </Zoomable>
...
var ScreenPodium = ({ screen, answers, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
    ...
    return <div key={rank} className={`pod-col pod-${rank + 1} ${b && live.playerId === b.id ? "me" : ""}`}>
            ...
            {myIdx >= 0 && <p className="pod-my fade-up">{tr2({ uz: "Siz — ", ru: "Вы — " })}<b>{myIdx + 1}</b>{tr2({ uz: "-o'rin", ru: "-е место" })} ({board[myIdx].okCount}/{totalQ})</p>}
...
function QuizArena({ live, onClose, startSolo }) {
  ...
  return <div className="qz-arena">
    ...
    return <div key={rank} className={`qz-pod-col p${rank + 1} ${b && b.id === live.playerId ? "me" : ""}`}>
              ...
              {myRank >= 0 && <p className="qz-mypl">{tr2({ uz: "Siz —", ru: "Вы —" })} <b>{myRank + 1}-o'rin</b> · {board[myRank].pts} ball</p>}
