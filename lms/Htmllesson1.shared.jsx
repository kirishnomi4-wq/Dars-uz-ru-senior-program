...
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/4340b314d6398ef53f3db530789e9e0e.jsx src/1-Modull/Htmllesson1.jsx
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [pos, setPos] = useState(RG_START);
    ...
    /* DASTUR — kod tahririday ko'rinish */
              ...
              <div className="rg-code">
                ...
                <div className="rg-code-body">
                  {program.length === 0 ? <div className="rg-code-empty">{tr({ uz: "// yuqoridagi tugmalardan buyruq qo'shing…", ru: "// добавьте команды кнопками выше…" })}</div> : program.map((c, i) => <div key={i} className={`rg-line ${active === i ? "now" : ""}`}>
...
var Screen3 = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="HTML" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
        ...
        <div className="split">
          ...
          <div className="col" style={{ gap: 7 }}>
            ...
            <Preview title="profil.html" minH={isNarrow ? 124 : 150}>
              {step === 0 && !running ? <div className="br-empty"><span style={{ fontSize: 24 }}>🌐</span><span>{tr({ uz: <>Brauzer hali kodni o'qimadi.<br />Pastdagi tugmani bosing.</>, ru: <>Браузер ещё не прочитал код.<br />Нажмите кнопку ниже.</> })}</span></div> : <div className="profile-card">
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Teg", ru: "Тег" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : wrapped ? tr({ uz: "🐞 Xatoni toping", ru: "🐞 Найдите ошибку" }) : tr({ uz: "Avval tegga o'rang", ru: "Сначала оберните в тег" })} onClick={onNext} /></>}>
      <div className="screen">
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const LADDER = [
    { n: 1, size: 28, tag: tr({ uz: "eng katta", ru: "самый большой" }) },
    { n: 2, size: 23, tag: "" },
    { n: 3, size: 19, tag: "" },
    { n: 4, size: 16.5, tag: "" },
    { n: 5, size: 14.5, tag: "" },
    { n: 6, size: 13, tag: tr({ uz: "eng kichik", ru: "самый маленький" }) }
  ];
  const LADDER_COMPACT = [
    { n: 1, size: 26, tag: tr({ uz: "eng katta", ru: "самый большой" }) },
    { n: 2, size: 21, tag: "" },
    { ellipsis: true },
    { n: 6, size: 13, tag: tr({ uz: "eng kichik", ru: "самый маленький" }) }
  ];
  ...
  return <Stage eyebrow={tr({ uz: "Sarlavhalar", ru: "Заголовки" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "✍️ Praktika →", ru: "✍️ Практика →" }) : tr({ uz: "Avval tanlang", ru: "Сначала выберите" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <div className="col">
            {!showAnswer ? <>
                ...
                {[{ id: "a", label: tr({ uz: "Eng katta sarlavha (tepadagi)", ru: "Самый большой заголовок (сверху)" }) }, { id: "b", label: tr({ uz: "«Kim qatnashishi mumkin?»", ru: "«Кто может участвовать?»" }) }].map((o) => <button key={o.id} className="hook-option fade-up delay-3" onClick={() => pick(o.id)}><span className="radio" /><span>{o.label}</span></button>)}
...
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Matn", ru: "Текст" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "✍️ Praktika →", ru: "✍️ Практика →" }) : tr({ uz: "B va I ni sinang", ru: "Попробуйте B и I" })} onClick={onNext} /></>}>
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Ro'yxatlar", ru: "Списки" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr({ uz: "✍️ Praktika →", ru: "✍️ Практика →" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Havolalar", ru: "Ссылки" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "✍️ Praktika →", ru: "✍️ Практика →" }) : tr({ uz: "Havolani bosing", ru: "Нажмите на ссылку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        ...
        <div className="split">
          <div className="col"><div className="flow-label">{tr({ uz: `Veb — bu "to'r"`, ru: "Веб — это «паутина»" })}</div><div className="web fade-up delay-2"><svg className="web-svg" viewBox="0 0 260 150" preserveAspectRatio="none">{EDGES.map(([a, b], i) => {
  ...
  })}{travel && <div className="web-travel" key={travel.key} style={{ "--fx": travel.fx, "--fy": travel.fy, "--tx": travel.tx, "--ty": travel.ty }} />}</div><p className="web-cap">{tr({ uz: <>Har sahifa boshqasiga <b>havola</b> bilan bog'langan. Shu bog'lanishlar <b>"to'r"</b> hosil qiladi — <b>Veb</b> (World Wide Web) shundan nom olgan.</>, ru: <>Каждая страница связана с другой <b>ссылкой</b>. Эти связи образуют <b>«паутину»</b> — отсюда и имя <b>Веб</b> (World Wide Web).</> })}</p></div>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const elCode = (type) => {
    switch (type) {
      ...
      case "p":
        return <><Tg>{"<p>"}</Tg>{tr({ uz: "Men HTML o'rganyapman.", ru: "Я изучаю HTML." })}<Tg>{"</p>"}</Tg></>;
      case "ul":
  ...
  const elView = (type, i) => {
    switch (type) {
      ...
      case "p":
        return <p key={i} style={{ fontFamily: "Georgia, serif", margin: "0 0 6px", color: T.ink, fontSize: "clamp(13px,1.8vw,15px)" }}>{tr({ uz: "Men HTML o'rganyapman.", ru: "Я изучаю HTML." })}</p>;
      case "ul":
        return <ul key={i} style={{ fontFamily: "Georgia, serif", color: T.ink, margin: "0 0 6px", paddingLeft: 22, fontSize: "clamp(13px,1.8vw,15px)" }}><li>{tr({ uz: "Futbol", ru: "Футбол" })}</li><li>{tr({ uz: "Kitob", ru: "Книги" })}</li></ul>;
      case "a":
  ...
  const submit = () => {
    ...
    if (!type) {
      setHint(tr({ uz: "Tushunmadim 🙂 Mana shulardan yozing: sarlavha, matn, ro’yxat, havola, rasm.", ru: "Не понял 🙂 Напишите что-то из этого: заголовок, текст, список, ссылка, картинка." }));
      return;
    }
    generate(type);
    setText("");
  ...
  return <Stage eyebrow={tr({ uz: "Amaliyot · sahifa quramiz", ru: "Практика · строим страницу" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: `Kamida 3 ta bo’lak (${items.length}/3)`, ru: `Минимум 3 блока (${items.length}/3)` })} onClick={onNext} /></>}>
    ...
    if (e.key === "Enter") submit();
            ...
            <div className="fade-up delay-2"><p className="flow-label" style={{ margin: "2px 0 7px" }}>{tr({ uz: "yoki tayyor buyruqlardan tanlang", ru: "или выберите из готовых команд" })}</p><div className="chips">{CHIPS.map((c) => <button key={c.key} className="gchip" disabled={items.length >= MAX} onClick={() => {
    setText(tr({ uz: c.label.toLowerCase() + " qo'sh", ru: "добавь " + c.label.toLowerCase() }));
    setHint("");
  }}>{c.label} <span className="gt">&lt;{c.tag}&gt;</span></button>)}{items.length > 0 && <button className="gchip" onClick={reset}>↺ {tr({ uz: "Tozalash", ru: "Очистить" })}</button>}</div></div>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tayyor", ru: "Готово" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Keyingi dars →", ru: "Следующий урок →" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Birinchi <span className="italic" style={{ color: T.accent }}>saytingizni</span> yasadingiz.</>, ru: <>Вы сделали свой первый <span className="italic" style={{ color: T.accent }}>сайт</span>.</> })}</h2>{
