...
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx src/1-Modull/Htmllesson1.jsx
...
function MentorPracticeOverlay({ entry, live, onClose }) {
  ...
  return <div className="mp-overlay">
      <div className="mp-card">
        ...
        <p className="mp-tip">💡 {tr2({ uz: "Ko'pchilik tugatgach, aynan shu mashqni doskada birga yozing — shunda o'quvchilar o'zini tekshiradi va mavzu mustahkamlanadi.", ru: "Когда закончит большинство, напишите это же упражнение вместе на доске — так ученики проверят себя, а тема закрепится." })}</p>
      </div>
    </div>;
}
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>Bu sayt nimadan <span className="italic" style={{ color: T.accent }}>yasalgan</span>?</>, ru: <>Из чего <span className="italic" style={{ color: T.accent }}>сделан</span> этот сайт?</> })}</h1>
            ...
            <div className="demo-swap" key={view}>
              {view === "site" ? <Preview minH={170} title="maktab.uz">
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingBottom: 10, borderBottom: `1px solid ${T.ink3}40`, marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}><span style={{ width: 22, height: 22, borderRadius: 6, background: T.accent, color: "#fff", display: "inline-flex", alignItems: "center", justifyContent: "center", fontFamily: "'Manrope', sans-serif", fontWeight: 800, fontSize: 13 }}>M</span><b style={{ fontFamily: "'Manrope', sans-serif", color: T.ink, fontSize: 15 }}>{tr2({ uz: "Maktab", ru: "Школа" })}</b></span>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 11, fontFamily: "'Manrope', sans-serif", fontSize: 12, color: T.ink2 }}><span>{tr2({ uz: "Asosiy", ru: "Главная" })}</span><span>{tr2({ uz: "Darslar", ru: "Уроки" })}</span><span style={{ background: T.ink, color: T.bg, padding: "5px 11px", borderRadius: 6, fontWeight: 600 }}>{tr2({ uz: "Kirish", ru: "Войти" })}</span></span>
                  </div>
                  <h1 style={{ fontFamily: "Georgia, serif", fontSize: "clamp(20px,3.2vw,28px)", margin: "0 0 6px", color: T.ink }}>{tr2({ uz: "Xush kelibsiz!", ru: "Добро пожаловать!" })} 👋</h1>
                  <p style={{ fontFamily: "Georgia, serif", margin: "0 0 12px", color: T.ink2, fontSize: "clamp(13px,1.8vw,15px)" }}>{tr2({ uz: "Bilim — bir bosishda.", ru: "Знания — в один клик." })}</p>
                  <span style={{ display: "inline-block", fontFamily: "'Manrope', sans-serif", fontWeight: 700, background: T.accent, color: "#fff", padding: "8px 16px", borderRadius: 8, fontSize: "clamp(12px,1.7vw,14px)" }}>{tr2({ uz: "Boshlash", ru: "Начать" })}</span>
                </Preview> : <>
                  <CodeBox><Tg>{"<header>"}</Tg>{"\n  "}{tr2({ uz: "Maktab · Asosiy · Darslar · Kirish", ru: "Школа · Главная · Уроки · Войти" })}{"\n"}<Tg>{"</header>"}</Tg>{"\n"}<Tg>{"<h1>"}</Tg>{tr2({ uz: "Xush kelibsiz!", ru: "Добро пожаловать!" })} 👋<Tg>{"</h1>"}</Tg>{"\n"}<Tg>{"<p>"}</Tg>{tr2({ uz: "Bilim — bir bosishda.", ru: "Знания — в один клик." })}<Tg>{"</p>"}</Tg>{"\n"}<Tg>{"<button>"}</Tg>{tr2({ uz: "Boshlash", ru: "Начать" })}<Tg>{"</button>"}</Tg></CodeBox>
                  <p className="mono small" style={{ color: T.ink3, marginTop: 6, textAlign: "center" }}>↑ {tr2({ uz: "shu saytning kodi — boshqa hech narsa emas!", ru: "это код того самого сайта — и ничего больше!" })}</p>
                </>}
            </div>
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Sizningcha, nima ishlatiladi?", ru: "Как вы думаете, что здесь используется?" })}</p>
    ...
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null} onClick={() => pick(o.id)}>
            ...
            {picked !== null && <p className="hook-ack fade-step">{tr2({ uz: "Yaxshi! Hozir hammasini birga ko'rib chiqamiz.", ru: "Отлично! Сейчас разберём всё вместе." })}</p>}
          </Col>
        </Split>
      </div>
    </Stage>;
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const PreviewBlock = <Col>
      <p className="flow-label">{tr2({ uz: "Natija — dars oxirida shunday bo'ladi", ru: "Результат — в конце урока будет так" })}</p>
      <Preview title="mening-saytim.html" minH={260}>
        <h1 style={{ fontFamily: G, fontSize: "clamp(20px,3vw,26px)", margin: "0 0 8px", color: T.ink }}>{tr2({ uz: "Salom, men Aziza!", ru: "Привет, я Азиза!" })}</h1>
        <p style={{ fontFamily: G, margin: "0 0 14px", color: T.ink2, fontSize: "clamp(13px,1.8vw,15px)", lineHeight: 1.5 }}>{tr2({ uz: "Web-dasturlashni endi o'rganyapman. Bu — mening birinchi saytim.", ru: "Я только начинаю изучать веб-разработку. Это мой первый сайт." })}</p>
        <p style={{ fontFamily: G, fontWeight: 700, margin: "0 0 6px", color: T.ink, fontSize: "clamp(14px,1.9vw,16px)" }}>{tr2({ uz: "Sevimli mashg'ulotlarim:", ru: "Мои любимые занятия:" })}</p>
        <ul style={{ fontFamily: G, color: T.ink, margin: "0 0 14px", paddingLeft: 22, fontSize: "clamp(13px,1.8vw,15px)", lineHeight: 1.6 }}><li>Minecraft</li><li>{tr2({ uz: "Futbol", ru: "Футбол" })}</li><li>{tr2({ uz: "Shaxmat", ru: "Шахматы" })}</li></ul>
        <a style={{ fontFamily: G, color: T.link, textDecoration: "underline", fontSize: "clamp(13px,1.8vw,15px)" }}>{tr2({ uz: "Mening Telegram kanalim", ru: "Мой Telegram-канал" })}</a>
      </Preview>
    </Col>;
  const StepsBlock = <Col>
      <p className="flow-label">{tr2({ uz: "7 qadam", ru: "7 шагов" })}</p>
  ...
  return <Stage eyebrow={tr2({ uz: "Reja", ru: "План" })} screen={screen} audioState={audio} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head">
          <h2 className="title h-title fade-up"><span className="italic" style={{ color: T.accent }}>{tr2({ uz: "Bugun haqiqiy sayt yasaymiz!", ru: "Сегодня сделаем настоящий сайт!" })}</span></h2>
        </div>
        ...
        {!isNarrow ? <Split>{PreviewBlock}{StepsBlock}</Split> : !showSteps ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>
            {PreviewBlock}
            <button className="btn" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(true)}>📋 {tr2({ uz: "Bugungi 7 qadamni ko'rish", ru: "Посмотреть 7 шагов на сегодня" })}</button>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [revealed, setRevealed] = useState3(!!storedAnswer);
  const answerRef = useRef3(null);
  const xulosaRef = useRef3(null);
  const [solved, setSolved] = useState3(!!storedAnswer);
  const [program, setProgram] = useState3([]);
  const [pos, setPos] = useState3(RG_START);
  const [phase, setPhase] = useState3("idle");
  const [jumping, setJumping] = useState3(false);
  const [active, setActive] = useState3(-1);
  const botTimer = useRef3(null);
  const running = phase === "running";
  const isCactus = (c) => RG_CACTI.indexOf(c) !== -1;
  useEffect4(() => () => clearTimeout(botTimer.current), []);
  useEffect4(() => {
    if (phase === "win") {
      setSolved(true);
  ...
  const RG_MSG = {
    running: { cls: "idle", text: tr2({ uz: "🦖 Dinozavr buyruqlaringizni bajaryapti…", ru: "🦖 Динозавр выполняет ваши команды…" }) },
    win: { cls: "ok", text: tr2({ uz: "🎉 Yetib keldi! Buyruqlaringiz to'g'ri ketma-ketlikda edi.", ru: "🎉 Добрался! Ваши команды были в правильном порядке." }) },
    "fail-cactus": { cls: "bad", text: tr2({ uz: "💥 Dinozavr kaktusga urildi! Undan «Sakra» bilan oshish kerak edi. U o'zi o'ylab sakramaydi — aniq buyurasiz.", ru: "💥 Динозавр врезался в кактус! Через него нужно было перепрыгнуть командой «Прыгай». Сам он не догадается — командуйте точно." }) },
    "fail-short": { cls: "bad", text: tr2({ uz: "🤏 Dinozavr 🍖 gacha yetmadi. Yana buyruq qo'shing va qaytadan urinib ko'ring.", ru: "🤏 Динозавр не добрался до 🍖. Добавьте ещё команду и попробуйте снова." }) }
  };
  const msg = RG_MSG[phase];
  return <Stage eyebrow={tr2({ uz: "Hayotdan misol", ru: "Пример из жизни" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!solved} label={solved ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : revealed ? tr2({ uz: "Dinozavrni 🍖 ga yetkazing", ru: "Доведите динозавра до 🍖" }) : tr2({ uz: "Avval o'ylab ko'ring", ru: "Сначала подумайте" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Kompyuter <span className="italic" style={{ color: T.accent }}>o'zicha</span> o'ylay oladimi?</>, ru: <>Умеет ли компьютер думать <span className="italic" style={{ color: T.accent }}>сам</span>?</> })}</h2></div>
        ...
        {!revealed && <div className="frame frame-col savo fade-up delay-2">
            <p className="eyebrow" style={{ color: T.accent, margin: 0 }}>{tr2({ uz: "Savol", ru: "Вопрос" })}</p>
            <p className="body" style={{ margin: 0, color: T.ink, fontSize: "clamp(15px,1.9vw,18px)", lineHeight: 1.5 }}>{tr2({ uz: <>Kompyuter o'zi hech narsa o'ylab topmaydi. Unga biror ishni <b>qanday buyuramiz</b> — qanday qilib «mana shuni qil» deb tushuntiramiz?</>, ru: <>Компьютер сам ничего не придумывает. <b>Как отдать ему приказ</b> — как объяснить «сделай вот это»?</> })}</p>
            <button className="btn" onClick={reveal} style={{ alignSelf: "flex-start", marginTop: 4 }}>{tr2({ uz: "Dinozavrda sinab ko'ramiz", ru: "Проверим на динозавре" })} →</button>
          </div>}
        {revealed && <>
            <Zoomable>
    ...
    /* Buyruq tugmalari — dino ostida */
              ...
              <div className="rg-pad">
                <button className="rg-key rg-key-yur" onClick={() => addCmd("yur")} disabled={running || program.length >= RG_MAX}><span className="rg-key-ic">🐾</span>{tr2(RG_CMDS.yur.label)}</button>
                <button className="rg-key rg-key-jump" onClick={() => addCmd("sakra")} disabled={running || program.length >= RG_MAX}><span className="rg-key-ic">⤴️</span>{tr2(RG_CMDS.sakra.label)}</button>
                <span className="rg-legend">{tr2({ uz: <>🐾 Yur = <b>1 qadam</b> · ⤴️ Sakra = <b>2 qadam</b></>, ru: <>🐾 Иди = <b>1 шаг</b> · ⤴️ Прыгай = <b>2 шага</b></> })}</span>
              </div>
    ...
    /* DASTUR — kod tahririday ko'rinish */
              ...
              <div className="rg-code">
                ...
                <div className="rg-code-body">
                  {program.length === 0 ? <div className="rg-code-empty">{tr2({ uz: "// yuqoridagi tugmalardan buyruq qo'shing…", ru: "// добавьте команды кнопками выше…" })}</div> : program.map((c, i) => <div key={i} className={`rg-line ${active === i ? "now" : ""}`}>
                          <span className="rg-ln">{i + 1}</span>
                          <span className="rg-call"><span className="rg-fn">{c}</span><span className="rg-paren">()</span></span>
                          <span className="rg-cmt">{RG_CMDS[c].icon} {tr2(RG_CMDS[c].note)}</span>
                        </div>)}
                </div>
              </div>
    ...
    /* Boshqaruv — tozalash + ishga tushirish */
              ...
              <div className="rg-run-row">
                <button className="rg-mini" onClick={clearProgram} disabled={running || !program.length}>↺ {tr2({ uz: "Tozalash", ru: "Очистить" })}</button>
                <button className="btn" onClick={runProgram} disabled={running || !program.length} style={{ marginLeft: "auto" }}>{running ? tr2({ uz: "Bajaryapti…", ru: "Выполняет…" }) : tr2({ uz: "▶ Ishga tushirish", ru: "▶ Запустить" })}</button>
              </div>
              </div>
              </div>
...
var Screen3 = ({ screen, onNext, onPrev }) => {
  ...
  const [name, setName] = useState3("Aziz");
  ...
  const codeLines = [
    ...
    <><Tg>{"<p>"}</Tg>{tr2({ uz: "HTML o'rganyapman", ru: "Изучаю HTML" })}<Tg>{"</p>"}</Tg></>,
    <><Tg>{"<button>"}</Tg>{tr2({ uz: "Obuna bo'lish", ru: "Подписаться" })}<Tg>{"</button>"}</Tg></>
  ];
  const runBrowser = () => {
  ...
  return <Stage eyebrow="HTML" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.5vw,14px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Yozgan so'zingiz qanday <span className="italic" style={{ color: T.accent }}>saytga</span> aylanadi?</>, ru: <>Как написанное слово превращается в <span className="italic" style={{ color: T.accent }}>сайт</span>?</> })}</h2></div>
        ...
        <div className="split">
          <div className="col" style={{ gap: 8 }}>
            <div className="fade-up delay-1">
              <p className="eyebrow" style={{ color: T.ink2, margin: "0 0 8px" }}>{tr2({ uz: "Ismingizni yozing", ru: "Напишите своё имя" })}</p>
              <input className="text-input" value={name} onChange={(e) => setName(e.target.value)} maxLength={18} placeholder={tr2({ uz: "Ismingiz", ru: "Ваше имя" })} disabled={running} />
            </div>
            <p className="flow-label">{tr2({ uz: "HTML kod", ru: "HTML-код" })} {isNarrow ? "↓" : "→"} {tr2({ uz: "brauzer o'qiydi", ru: "браузер читает" })}</p>
          ...
          <div className="col" style={{ gap: 7 }}>
            <p className="flow-label">{tr2({ uz: "Brauzer oynasi", ru: "Окно браузера" })}</p>
            <Preview title="profil.html" minH={isNarrow ? 124 : 150}>
              {step === 0 && !running ? <div className="br-empty"><span style={{ fontSize: 24 }}>🌐</span><span>{tr2({ uz: <>Brauzer hali kodni o'qimadi.<br />Pastdagi tugmani bosing.</>, ru: <>Браузер ещё не прочитал код.<br />Нажмите кнопку ниже.</> })}</span></div> : <div className="profile-card">
                    ...
                    {step >= 2 && <p className="pf-bio br-el">{tr2({ uz: "HTML o'rganyapman", ru: "Изучаю HTML" })}</p>}
                    {step >= 3 && <button className="pf-btn br-el">{tr2({ uz: "Obuna bo'lish", ru: "Подписаться" })}</button>}
                  </div>}
            </Preview>
            <div className="br-controls">
              {built && !running && <button className="rg-mini" onClick={reset}>↺ {tr2({ uz: "Tozalash", ru: "Очистить" })}</button>}
              <button className="btn" onClick={runBrowser} disabled={running} style={{ marginLeft: built && !running ? "auto" : 0 }}>
                {running ? tr2({ uz: "Brauzer o'qiyapti…", ru: "Браузер читает…" }) : built ? tr2({ uz: "↻ Qaytadan qurish", ru: "↻ Построить заново" }) : tr2({ uz: "▶ Brauzerni ishga tushirish", ru: "▶ Запустить браузер" })}
              </button>
            </div>
            {built && <p className="br-status">{tr2({ uz: <>✓ Brauzer kodni <b>yuqoridan pastga</b> o'qib, har bir qatorni sahifaga aylantirdi.</>, ru: <>✓ Браузер прочитал код <b>сверху вниз</b> и превратил каждую строку в часть страницы.</> })}</p>}
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen4 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "Kim nima qiladi?", ru: "Кто что делает?" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: "HTML kodini o'qib, sahifani ekranda ko'rsatadigan dastur qaysi?", ru: "Какая программа читает HTML-код и показывает страницу на экране?" })}</h2></>}
  options={[{ uz: "Server", ru: "Сервер" }, "Photoshop", { uz: "Brauzer", ru: "Браузер" }, { uz: "Klaviatura", ru: "Клавиатура" }]}
  correctIdx={2}
  explainCorrect={tr2({ uz: "To'g'ri. Brauzer (Chrome, Safari, Firefox, Edge) HTML kodini o'qib, sahifa qilib ekraningizda ko'rsatadi.", ru: "Верно. Браузер (Chrome, Safari, Firefox, Edge) читает HTML-код и показывает его на экране в виде страницы." })}
  explainWrong={{ 0: tr2({ uz: "Server faqat HTML faylni saqlaydi va jo’natadi. Uni o’qib sahifaga aylantirish — brauzerning ishi.", ru: "Сервер лишь хранит и отправляет HTML-файл. Читать его и превращать в страницу — работа браузера." }), 1: tr2({ uz: "Photoshop — bu rasm muharriri, HTML’ga aloqasi yo’q.", ru: "Photoshop — это редактор изображений, к HTML он отношения не имеет." }), 3: tr2({ uz: "Klaviatura — bu siz yozadigan qurilma. HTML’ni o’qib ko’rsatadigan — brauzer.", ru: "Клавиатура — устройство, на котором вы печатаете. А читает и показывает HTML — браузер." }), default: tr2({ uz: "HTML kodini o’qib, sahifa qilib ko’rsatadigan — brauzer.", ru: "HTML-код читает и показывает как страницу именно браузер." }) }}
/>;
var SKELET_PIECES = [
...
function DragDropOrder({ items, hints, onSolved, onWrong }) {
  ...
  return <div className="dd fade-up">
      ...
      {solved && <div className="dd-done">✓ {tr2({ uz: "To'g'ri! Skelet aynan shu tartibda.", ru: "Верно! Скелет именно в таком порядке." })}</div>}
...
function Flashcards({ cards }) {
  ...
  if (!card) return <div className="fc-done fade-up"><span className="fc-done-emoji">🎉</span><p className="fc-done-h">{tr2({ uz: "Hammasini bilasiz!", ru: "Вы знаете всё!" })}</p><p className="fc-done-s">{total}/{total} {tr2({ uz: "karta yodlandi", ru: "карточек выучено" })}</p><button className="fc-btn ghost" onClick={restart}>↻ {tr2({ uz: "Qaytadan takrorlash", ru: "Повторить заново" })}</button></div>;
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const PARTS = {
    doctype: { tag: "<!DOCTYPE html>", word: tr2({ uz: "E'lon", ru: "Объявление" }), role: tr2({ uz: 'Eng boshida turadi va "bu HTML5 sahifa" deb bildiradi.', ru: "Стоит в самом начале и объявляет: «это страница HTML5»." }) },
    html: { tag: "<html>", word: tr2({ uz: "Butun hujjat", ru: "Весь документ" }), role: tr2({ uz: "Butun sahifani o’rab turadi — head ham, body ham shuning ichida.", ru: "Оборачивает всю страницу — и head, и body находятся внутри него." }) },
    head: { tag: "<head>", word: tr2({ uz: "Ko’rinmaydigan qism", ru: "Невидимая часть" }), role: tr2({ uz: "Sahifa sozlamalari: title (nom), til, shriftlar. Xuddi restoran oshxonasi kabi — mehmon ko’rmaydi. title faqat brauzer tabchasida chiqadi, sahifa ichida ko’rinmaydi.", ru: "Настройки страницы: title (имя), язык, шрифты. Как кухня ресторана — гость её не видит. title появляется только на вкладке браузера, внутри страницы его не видно." }) },
    body: { tag: "<body>", word: tr2({ uz: "Ko’rinadigan qism", ru: "Видимая часть" }), role: tr2({ uz: "Ekranda ko’rinadigan hamma narsa shu yerda: sarlavha, matn, rasm, havola. Xuddi restoran zali kabi — mehmon shuni ko’radi.", ru: "Всё, что видно на экране, живёт здесь: заголовок, текст, картинка, ссылка. Как зал ресторана — именно его видит гость." }) }
  };
  const [active, setActive] = useState3(null);
  const [clicked, setClicked] = useState3(/* @__PURE__ */ new Set());
  const [dragDone, setDragDone] = useState3(!!storedAnswer);
  const isNarrow = useIsMobile(768);
  ...
  return <Stage eyebrow={tr2({ uz: "Struktura", ru: "Структура" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : explored ? tr2({ uz: "Skeletni yig'ing", ru: "Соберите скелет" }) : tr2({ uz: `${clicked.size}/4 qismi ko'rilgan`, ru: `Просмотрено частей: ${clicked.size}/4` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Saytning <span className="italic" style={{ color: T.accent }}>ko'rinmaydigan</span> tomoni bormi?</>, ru: <>Есть ли у сайта <span className="italic" style={{ color: T.accent }}>невидимая</span> сторона?</> })}</h2></div>
        ...
        <div className="split">
          <div className="col">
            {explored ? <div className="sk-buildbox">
                <p className="eyebrow" style={{ color: T.accent, margin: "0 0 2px" }}>🧲 {tr2({ uz: "Endi o'zingiz yig'ing", ru: "Теперь соберите сами" })}</p>
                <p className="body" style={{ margin: "0 0 10px", color: T.ink2, fontSize: 13.5 }}>{tr2({ uz: "Bo'laklarni to'g'ri tartibda joylang — sudrab yoki bosib.", ru: "Расставьте блоки в правильном порядке — перетаскивая или нажимая." })}</p>
    ...
    if (achMiss && !done) achMiss.miss(screen);
            ...
            <p className="sk-tapguide fade-up delay-1">{tr2({ uz: <>👆 Chizmadagi <b>4 ta qismni birma-bir bosing</b> — bosilmaganlari yonib turadi</>, ru: <>👆 <b>Нажмите по очереди на 4 части</b> схемы — ненажатые подсвечиваются</> })} <span className="sk-tapcount">{clicked.size}/4</span></p>
            <div className="bskel fade-up delay-2">
              ...
              <div className={fc("html", "bskel-html")} onClick={() => tap("html")}>
                <span className="bskel-htmllabel">{tr2({ uz: "<html> — butun hujjat", ru: "<html> — весь документ" })}</span>
                <div className="bskel-win" onClick={(e) => e.stopPropagation()}>
                  <div className={fc("head", "bskel-tab")} onClick={() => tap("head")}><span className="bskel-dots"><i /><i /><i /></span><span className="bskel-tabpill">{tr2({ uz: "Mening sahifam", ru: "Моя страница" })}</span><span className="bskel-zone">&lt;head&gt;</span></div>
                  <div className={fc("body", "bskel-page")} onClick={() => tap("body")}><p className="bskel-ptitle">{tr2({ uz: "Salom!", ru: "Привет!" })} 👋</p><p className="bskel-ptext">{tr2({ uz: "Bu mening sahifam.", ru: "Это моя страница." })}</p><span className="bskel-zone bskel-zone-b">&lt;body&gt;</span></div>
                </div>
              </div>
            </div>
            </>}
          ...
          <div className="col" style={{ gap: 8 }}>
            <div className="zb-gap" style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
              <div className="flow-label">{tr2({ uz: "HTML kodi", ru: "HTML-код" })}</div>
              <span className="small mono" style={{ color: explored ? T.success : T.ink3 }}>{clicked.size} / 4 {tr2({ uz: "ko'rildi", ru: "просмотрено" })}</span>
            </div>
...
var ScreenSkeletTest = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "Mustahkamlash", ru: "Закрепление" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Sahifada <span className="italic" style={{ color: T.accent }}>ko'rinadigan</span> matn qaysi qismga yoziladi?</>, ru: <>В какую часть пишется <span className="italic" style={{ color: T.accent }}>видимый</span> текст страницы?</> })}</h2></>}
  options={["`<head>`", "`<title>`", "`<!DOCTYPE>`", "`<body>`"]}
  correctIdx={3}
  explainCorrect={tr2({ uz: "To'g'ri! `body` — sahifada ko'rinadigan hamma narsa (sarlavha, matn, rasm) shu yerda yoziladi.", ru: "Верно! `body` — всё видимое на странице (заголовок, текст, картинка) пишется здесь." })}
  explainWrong={{
    0: tr2({ uz: "`<head>` — bu ko’rinmaydigan qism: `title` va sozlamalar. Ko’rinadigan matn `body` ichida.", ru: "`<head>` — невидимая часть: `title` и настройки. Видимый текст — внутри `body`." }),
    1: tr2({ uz: "`<title>` — faqat brauzer tabchasidagi nom. To’g’risi — `body`.", ru: "`<title>` — лишь имя на вкладке браузера. Правильный ответ — `body`." }),
    2: tr2({ uz: '`<!DOCTYPE>` — bu "men HTML5 man" degan e’lon, matn joyi emas. To’g’risi — `body`.', ru: "`<!DOCTYPE>` — это объявление «я HTML5», а не место для текста. Правильный ответ — `body`." }),
    default: tr2({ uz: "Ko’rinadigan matn `body` ichiga yoziladi.", ru: "Видимый текст пишется внутрь `body`." })
  }}
/>;
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const PARTS = {
    open: { role: tr2({ uz: "Ochuvchi teg — element shu yerdan boshlanadi.", ru: "Открывающий тег — элемент начинается здесь." }) },
    content: { role: tr2({ uz: "Kontent — ekranda ko'rinadigan matn. Teglar uni yashirmaydi, balki nima ekanini aytadi.", ru: "Контент — видимый на экране текст. Теги его не прячут, а говорят, что это такое." }) },
    close: { role: tr2({ uz: "Yopuvchi teg — / belgili. Element shu yerda tugaydi.", ru: "Закрывающий тег — со знаком /. Элемент заканчивается здесь." }) }
  };
  const [wrapped, setWrapped] = useState3(false);
  const [active, setActive] = useState3(null);
  const [dbgDone, setDbgDone] = useState3(!!storedAnswer);
  const isNarrow = useIsMobile(768);
  const done = dbgDone;
  const achMiss = useContext2(AchMissCtx);
  const tap = (k) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Teg", ru: "Тег" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : wrapped ? tr2({ uz: "🐞 Xatoni toping", ru: "🐞 Найдите ошибку" }) : tr2({ uz: "Avval tegga o'rang", ru: "Сначала оберните в тег" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Brauzer matn qayerda <span className="italic" style={{ color: T.accent }}>tugashini</span> qanday biladi?</>, ru: <>Как браузер узнаёт, где текст <span className="italic" style={{ color: T.accent }}>заканчивается</span>?</> })}</h2></div>
            ...
            <div className={`tegbuild-wrap ${wrapped ? "on" : ""} fade-up delay-2`}>
              <div className={`tegbuild ${wrapped ? "on" : ""}`}>
                <div className={ic("open", `tb-chip tb-tag tb-open ${wrapped ? "" : "hide"}`)} onClick={() => tap("open")}>
                  <span className="tb-code">&lt;h1&gt;</span><span className="tb-lbl">{tr2({ uz: "ochuvchi", ru: "открывающий" })}</span>
                </div>
                <div className={ic("content", "tb-chip tb-content")} onClick={() => tap("content")}>
                  <span className="tb-code">{tr2({ uz: "Salom!", ru: "Привет!" })}</span><span className="tb-lbl">{wrapped ? tr2({ uz: "kontent", ru: "контент" }) : tr2({ uz: "oddiy matn", ru: "обычный текст" })}</span>
                </div>
                <div className={ic("close", `tb-chip tb-tag tb-close ${wrapped ? "" : "hide"}`)} onClick={() => tap("close")}>
                  <span className="tb-code">&lt;<span className="tb-slash">/</span>h1&gt;</span><span className="tb-lbl">{tr2({ uz: "yopuvchi", ru: "закрывающий" })}</span>
                </div>
              </div>
              <div className="tb-bracket"><div className="tb-brace" /><span className="tb-brace-lbl">{tr2({ uz: "= bitta element", ru: "= один элемент" })}</span></div>
            </div>
    ...
    setActive(null);
              ...
              {wrapped ? tr2({ uz: "↻ Qaytadan", ru: "↻ Заново" }) : tr2({ uz: "▶ Tegga o’rash", ru: "▶ Обернуть в тег" })}
            </button>

            {wrapped ? active ? <div className="role-line fade-step" key={active}><p className="body" style={{ color: T.ink, margin: 0 }}>{PARTS[active].role}</p></div> : <p className="body" style={{ margin: 0, color: T.ink3, fontSize: 13, fontStyle: "italic" }}>{tr2({ uz: "Yuqoridagi uchta qismdan birini bosib, vazifasini ko'ring.", ru: "Нажмите на одну из трёх частей выше и посмотрите её роль." })}</p> : <div className="hint"><p className="body" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: <>Hozir "Salom!" — oddiy matn, brauzer u bilan nima qilishni bilmaydi. Tugmani bosing — teglar ikki tomondan kelib uni <b style={{ color: T.ink }}>qamrab oladi</b>.</>, ru: <>Сейчас "Привет!" — обычный текст, браузер не знает, что с ним делать. Нажмите кнопку — теги подойдут с двух сторон и <b style={{ color: T.ink }}>обнимут его</b>.</> })}</p></div>}
          </div>

          {(!isNarrow || wrapped) && <div className="col fade-step">
            <div className="flow-label">{tr2({ uz: "Sahifada qanday ko'rinadi", ru: "Как выглядит на странице" })}</div>
            <Preview title="sahifa.html" minH={92}>{wrapped ? <p className="pv-h1 fade-step">{tr2({ uz: "Salom!", ru: "Привет!" })}</p> : <p className="pv-plain">{tr2({ uz: "Salom!", ru: "Привет!" })}</p>}</Preview>
            {wrapped && <div className="frame-ok fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>✓ Endi brauzer biladi: <span className="mono">&lt;h1&gt;</span> boshladi, <span className="mono">&lt;/h1&gt;</span> tugatdi — orasidagi <b>"Salom!"</b> katta sarlavhaga aylandi.</>, ru: <>✓ Теперь браузер знает: <span className="mono">&lt;h1&gt;</span> начал, <span className="mono">&lt;/h1&gt;</span> закончил — "Привет!" между ними стал большим заголовком.</> })}</p></div>}
          </div>}
          {wrapped && <div className="col fade-step">
          <div className="dbg-box">
            <p className="eyebrow" style={{ color: T.accent, margin: "2px 0 0" }}>🐞 {tr2({ uz: "Endi xatoni toping", ru: "Теперь найдите ошибку" })}</p>
            <p className="body" style={{ margin: "2px 0 8px", color: T.ink2, fontSize: 13.5 }}>{tr2({ uz: <>Bu kodda bitta teg <b style={{ color: T.ink }}>noto'g'ri yopilgan</b>. Xato qatorni bosing.</>, ru: <>В этом коде один тег <b style={{ color: T.ink }}>закрыт неправильно</b>. Нажмите на строку с ошибкой.</> })}</p>
            <DebugChallenge
    lines={[{ text: tr2({ uz: "<h1>Mening saytim<h1>", ru: "<h1>Мой сайт<h1>" }), bug: true }, { text: tr2({ uz: "<p>Xush kelibsiz!</p>", ru: "<p>Добро пожаловать!</p>" }) }]}
    fixed={tr2({ uz: "<h1>Mening saytim</h1>", ru: "<h1>Мой сайт</h1>" })}
    explain={tr2({ uz: "Yopuvchi tegda / belgisi bo'lishi kerak: </h1>", ru: "В закрывающем теге должен быть знак /: </h1>" })}
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Yozing", ru: "Напишите" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={isMentorLive ? false : !correct} label={isMentorLive ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : correct ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Yopuvchi tegni yozing", ru: "Напишите закрывающий тег" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>o'zingiz</span> yozib ko'ring.</>, ru: <>Теперь попробуйте написать <span className="italic" style={{ color: T.accent }}>сами</span>.</> })}</h2></div>
        ...
        <div className="split">
          <div className="col"><div className="yz-card fade-up delay-2"><div className="yz-line"><span className="yz-code"><span className="t-tag">&lt;h1&gt;</span>{tr2({ uz: "Salom!", ru: "Привет!" })}</span>{!correct ? <input className="yz-input" value={val} onChange={(e) => setVal(e.target.value)} placeholder={tr2({ uz: "yopuvchi teg…", ru: "закрывающий тег…" })} spellCheck={false} /> : <span className="yz-code yz-done"><span className="t-tag">&lt;/h1&gt;</span></span>}</div>{!correct && <p className="yz-hint">{touched ? tr2({ uz: "Deyarli! Yopuvchi teg / belgisi bilan boshlanadi: </h1>", ru: "Почти! Закрывающий тег начинается со знака /: </h1>" }) : tr2({ uz: "Maslahat: avval / yozing, keyin teg nomi va >", ru: "Подсказка: сначала напишите /, потом имя тега и >" })}</p>}{correct && <p className="yz-ok">✓ {tr2({ uz: "To'g'ri! Endi element yopildi:", ru: "Верно! Теперь элемент закрыт:" })} &lt;h1&gt;...&lt;/h1&gt;</p>}</div></div>
          <div className="col"><div className="flow-label">{tr2({ uz: "Natija", ru: "Результат" })}</div><div className="bp-window fade-up delay-2"><div className="bp-bar"><span className="bb-dots"><i /><i /><i /></span><span className="bp-title">sahifa.html</span></div><div className="bp-body" style={{ minHeight: 110, display: "flex", alignItems: "center", justifyContent: "center" }}>{correct ? <p className="pv-h1 fade-step">{tr2({ uz: "Salom!", ru: "Привет!" })}</p> : <p className="yz-placeholder">{tr2({ uz: "Natija shu yerda chiqadi…", ru: "Результат появится здесь…" })}</p>}</div></div></div>
        </div>
        {isMentorLive && <MentorWorkStats live={live} screenIdx={screen} taskLabel={tr2({ uz: "Yopuvchi teg yozish", ru: "Написать закрывающий тег" })} />}
      </div>
    </Stage>;
};
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const LADDER = [
    { n: 1, size: 28, tag: tr2({ uz: "eng katta", ru: "самый большой" }) },
    { n: 2, size: 23, tag: "" },
    { n: 3, size: 19, tag: "" },
    { n: 4, size: 16.5, tag: "" },
    { n: 5, size: 14.5, tag: "" },
    { n: 6, size: 13, tag: tr2({ uz: "eng kichik", ru: "самый маленький" }) }
  ];
  const LADDER_COMPACT = [
    { n: 1, size: 26, tag: tr2({ uz: "eng katta", ru: "самый большой" }) },
    { n: 2, size: 21, tag: "" },
    { ellipsis: true },
    { n: 6, size: 13, tag: tr2({ uz: "eng kichik", ru: "самый маленький" }) }
  ];
  ...
  const [revealed, setRevealed] = useState3(false);
  ...
  const prevCollapsedRef = useRef3(mctx.collapsed);
  useEffect4(() => {
    if (isNarrow && done) {
  ...
  return <Stage eyebrow={tr2({ uz: "Sarlavhalar", ru: "Заголовки" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "✍️ Praktika →", ru: "✍️ Практика →" }) : tr2({ uz: "Avval tanlang", ru: "Сначала выберите" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Telegram postida qaysi yozuv <span className="italic" style={{ color: T.accent }}>birinchi</span> ko'zga tashlanadi?</>, ru: <>Какая надпись в посте Telegram бросается в глаза <span className="italic" style={{ color: T.accent }}>первой</span>?</> })}</h2></div>
          ...
          <div className="col">
            <div className={`news-card tg-post frame fade-up delay-1 ${showAnswer ? "tagged" : ""}`}>
              <div className="tgp-head">
                ...
                <div><p className="tgp-name">{tr2({ uz: "Maktab kanali", ru: "Школьный канал" })}</p><p className="tgp-sub">{tr2({ uz: "bugun 09:14", ru: "сегодня 09:14" })}</p></div>
              </div>
              <div className="news-line news-headline">
                <h3 style={{ fontFamily: G, fontWeight: 700, fontSize: "clamp(20px,3vw,26px)", lineHeight: 1.15, color: T.ink, margin: 0 }}>{tr2({ uz: "Maktabimizda robototexnika to'garagi ochildi", ru: "В нашей школе открылся кружок робототехники" })}</h3>
                <span className="tag-badge accent" style={{ transitionDelay: "0.05s" }}>&lt;h1&gt;</span>
              </div>
              <div className="news-line">
                <p style={{ fontFamily: G, fontWeight: 700, fontSize: "clamp(15px,2vw,17px)", color: T.ink, margin: 0 }}>{tr2({ uz: "Kim qatnashishi mumkin?", ru: "Кто может участвовать?" })}</p>
                <span className="tag-badge" style={{ transitionDelay: "0.18s" }}>&lt;h2&gt;</span>
              </div>
              <div className="news-line">
                <p style={{ fontFamily: G, fontSize: "clamp(13px,1.7vw,14.5px)", color: T.ink2, margin: 0, lineHeight: 1.5 }}>{tr2({ uz: "To'garak har shanba soat 10:00 da. Ro'yxatdan o'tish hammaga ochiq.", ru: "Кружок — каждую субботу в 10:00. Запись открыта для всех." })}</p>
                <span className="tag-badge soft" style={{ transitionDelay: "0.31s" }}>&lt;p&gt;</span>
              </div>
              <p className="tgp-foot">👁 <b>412</b> · {tr2({ uz: "Maktab kanali", ru: "Школьный канал" })}</p>
              {showAnswer && <p className="news-hint fade-step">{tr2({ uz: <>↑ Har bir yozuvning o'z "darajasi" bor — eng kattasi <b style={{ color: T.accent }}>&lt;h1&gt;</b>.</>, ru: <>↑ У каждой надписи свой «уровень» — самый большой это <b style={{ color: T.accent }}>&lt;h1&gt;</b>.</> })}</p>}
            </div>
            {showAnswer && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <><b>Sizning loyihangiz:</b> saytingizdagi eng katta sarlavha — bu ismingiz. Demak u <span className="mono">&lt;h1&gt;</span>.</>, ru: <><b>Ваш проект:</b> самый большой заголовок на вашем сайте — это ваше имя. Значит, он будет <span className="mono">&lt;h1&gt;</span>.</> })}</p></div>}
          </div>
          <div className="col">
            {!showAnswer ? <>
                <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Sizningcha, qaysi yozuv eng muhim?", ru: "Как вы думаете, какая надпись самая важная?" })}</p>
                {[{ id: "a", label: tr2({ uz: "Eng katta sarlavha (tepadagi)", ru: "Самый большой заголовок (сверху)" }) }, { id: "b", label: tr2({ uz: "«Kim qatnashishi mumkin?»", ru: "«Кто может участвовать?»" }) }].map((o) => <button key={o.id} className="hook-option fade-up delay-3" onClick={() => pick(o.id)}><span className="radio" /><span>{o.label}</span></button>)}
              </> : <div ref={revealRef} className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 12, scrollMarginTop: 16 }}>
                {!isNarrow && <div className="frame-success"><p className="small mono" style={{ margin: "0 0 4px", fontWeight: 600, color: T.success, textTransform: "uppercase", letterSpacing: "0.08em" }}>{picked === "a" ? tr2({ uz: "✓ To'g'ri", ru: "✓ Верно" }) : tr2({ uz: "Mana gap", ru: "Вот в чём суть" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Telegramda eng muhim gap <b>qalin</b> va birinchi turadi. HTML sahifasida esa uni <b>kattaligi</b> ko'rsatadi: <span className="mono">h1</span> eng katta va muhim, keyin <span className="mono">h2</span>, <span className="mono">h3</span>… <span className="mono">h6</span> eng kichik.</>, ru: <>В Telegram самая важная фраза — <b>жирная</b> и первая. На HTML-странице её показывает <b>размер</b>: <span className="mono">h1</span> — самый большой и важный, дальше <span className="mono">h2</span>, <span className="mono">h3</span>… <span className="mono">h6</span> — самый маленький.</> })}</p></div>}
                <p className="flow-label">{isNarrow ? tr2({ uz: "h1 → h6 — sarlavha darajalari", ru: "h1 → h6 — уровни заголовков" }) : tr2({ uz: "h1 dan h6 gacha — darajalar narvoni", ru: "от h1 до h6 — лестница уровней" })}</p>
                <div className="ladder ladder-stair">{(isNarrow ? LADDER_COMPACT : LADDER).map((h, i) => h.ellipsis ? <div key="el" className="hl-row hl-stair" style={{ animationDelay: `${i * 0.1}s`, justifyContent: "center", padding: "2px 0", background: "transparent", boxShadow: "none" }}><span style={{ color: T.ink3, fontSize: 18, letterSpacing: 3 }}>⋮</span></div> : <div key={h.n} className="hl-row hl-stair" style={{ animationDelay: `${i * 0.1}s`, marginLeft: isNarrow ? 0 : `${i * 15}px` }}><span className="hl-bar" style={{ opacity: Math.max(0.18, 1 - i * 0.15) }} /><span className="hl-chip">{`<h${h.n}>`}</span><span className="hl-text" style={{ fontSize: h.size }}>{tr2({ uz: "Sarlavha", ru: "Заголовок" })}</span>{h.tag && <span className="hl-tag">{h.tag}</span>}</div>)}</div>
              </div>}
          </div>
        </div>
      </div>
...
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const labels = { imtihon: tr2({ uz: "imtihon", ru: "контрольная" }), albatta: tr2({ uz: "Albatta", ru: "Обязательно" }) };
  const [fmt, setFmt] = useState3({ imtihon: { b: false, i: false }, albatta: { b: false, i: false } });
  const [sel, setSel] = useState3(null);
  const [usedB, setUsedB] = useState3(false);
  const [usedI, setUsedI] = useState3(false);
  ...
  const chatWord = (id) => <span key={id} className={`tgm-word ${sel === id ? "sel" : ""} ${fmt[id].b ? "bold" : ""} ${fmt[id].i ? "ital" : ""}`} onClick={(e) => {
    ...
    setSel(id);
      ...
      {sel === id && <span className="tgm-menu" onClick={(e) => e.stopPropagation()}>
          <button className={`${fmt[id].b ? "on" : ""}${usedB ? "" : " pulse"}`} onClick={() => applyFmt("b")} title={tr2({ uz: "Qalin", ru: "Жирный" })}><b>B</b></button>
          <button className={`${fmt[id].i ? "on" : ""}${usedI ? "" : " pulse"}`} onClick={() => applyFmt("i")} title={tr2({ uz: "Yotiq", ru: "Курсив" })}><i>I</i></button>
        </span>}
    </span>;
  ...
  return <Stage eyebrow={tr2({ uz: "Matn", ru: "Текст" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "✍️ Praktika →", ru: "✍️ Практика →" }) : tr2({ uz: "B va I ni sinang", ru: "Попробуйте B и I" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }} onClick={() => setSel(null)}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Muhim so'zni qanday <span className="italic" style={{ color: T.accent }}>ajratasiz</span>?</>, ru: <>Как <span className="italic" style={{ color: T.accent }}>выделить</span> важное слово?</> })}</h2></div>
    ...
    /* TELEGRAM muhiti */
          ...
          <div className="cmp-side">
            ...
            <div className="tgc-window">
              <div className="tgc-head"><span className="tgc-ava">🧑‍🏫</span><div><p className="tgc-name">{tr2({ uz: "Sinfdosh", ru: "Одноклассник" })}</p><p className="tgc-status">online</p></div></div>
              <div className="tgc-body">
                <div className="tgc-row in"><div className="tgc-bubble in">{tr2({ uz: "Ertaga maktabda muhim narsa bormi?", ru: "Завтра в школе есть что-то важное?" })}</div></div>
                {sent && <div className="tgc-row out"><div className="tgc-bubble out">{tr2({ uz: "Ertaga", ru: "Завтра" })} {renderWord("imtihon")}{tr2({ uz: " bor! ", ru: "! " })}{renderWord("albatta")} {tr2({ uz: "keling.", ru: "приходите." })}<span className="tgc-time">✓✓</span></div></div>}
              </div>
              <div className="tgc-composer" onClick={(e) => e.stopPropagation()}>
                <span className="tgc-cic">😊</span>
                <div className="tgc-input">{tr2({ uz: "Ertaga", ru: "Завтра" })} {chatWord("imtihon")}{tr2({ uz: " bor! ", ru: "! " })}{chatWord("albatta")} {tr2({ uz: "keling.", ru: "приходите." })}</div>
                <button className="tgc-send" onClick={() => setSent(true)} title={tr2({ uz: "Yuborish", ru: "Отправить" })} aria-label={tr2({ uz: "Yuborish", ru: "Отправить" })}><svg viewBox="0 0 24 24" fill="currentColor"><path d="M2.01 21 23 12 2.01 3 2 10l15 2-15 2z" /></svg></button>
              </div>
            </div>
            <p className="tgc-hint">{sel ? tr2({ uz: "👆 Yuqoridagi B yoki I tugmasini bosing", ru: "👆 Нажмите кнопку B или I сверху" }) : tr2({ uz: "👆 xabardagi so'zni bosib belgilang", ru: "👆 нажмите на слово в сообщении, чтобы выделить" })}</p>
          </div>
    ...
    /* VEB-SAYT muhiti */
          ...
          <div className="cmp-side">
            <div className="cmp-hd cmp-hd-html">🌐 {tr2({ uz: "Veb-saytda HTML buni teg bilan qiladi", ru: "На сайте HTML делает это тегом" })}</div>
            <div className="bp-window">
              ...
              <div className="cmp-siteview"><p>{tr2({ uz: "Ertaga", ru: "Завтра" })} {renderWord("imtihon")}{tr2({ uz: " bor! ", ru: "! " })}{renderWord("albatta")} {tr2({ uz: "keling.", ru: "приходите." })}</p></div>
            </div>
            <div className="cmp-codeblock">
              <span className="cmp-codelbl">&lt;/&gt; {tr2({ uz: "HTML kod", ru: "HTML-код" })}</span>
              <pre className="code-box cmp-code"><span className="tg">&lt;p&gt;</span>{tr2({ uz: "Ertaga", ru: "Завтра" })} {codeWord("imtihon")}{tr2({ uz: " bor! ", ru: "! " })}{codeWord("albatta")} {tr2({ uz: "keling.", ru: "приходите." })}<span className="tg">&lt;/p&gt;</span></pre>

              <p className="cmp-leg"><span className="cmp-leg-tag">&lt;strong&gt;</span> — {tr2({ uz: "qalin qiladi", ru: "делает жирным" })} · <span className="cmp-leg-tag">&lt;em&gt;</span> — {tr2({ uz: "yotiq-kursiv qiladi", ru: "делает курсивом" })}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className={`cmp-merge fade-up delay-3 ${done ? "done" : ""}`}>
          {done ? "✓ " : ""}{tr2({ uz: <>Telegramda so'zni belgilab <b>tugma</b> bosasiz; veb-saytda esa o'sha so'z <b>teg</b> ichiga olinadi.</>, ru: <>В Telegram вы выделяете слово и жмёте <b>кнопку</b>; на сайте то же слово берётся в <b>тег</b>.</> })}
        </div>
      </div>
    </Stage>;
};
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const SHOP = [tr2({ uz: "Non", ru: "Хлеб" }), tr2({ uz: "Sut", ru: "Молоко" }), tr2({ uz: "Tuxum", ru: "Яйца" })];
  const RECIPE = [tr2({ uz: "Limon siqing", ru: "Выжмите лимон" }), tr2({ uz: "Shakar soling", ru: "Добавьте сахар" }), tr2({ uz: "Suv quying", ru: "Налейте воду" })];
  const isNarrow = useIsMobile(768);
  ...
  return <Stage eyebrow={tr2({ uz: "Ro'yxatlar", ru: "Списки" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "✍️ Praktika →", ru: "✍️ Практика →" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Xarid ro'yxati va retsept — <span className="italic" style={{ color: T.accent }}>farqi</span> nimada?</>, ru: <>Список покупок и рецепт — в чём <span className="italic" style={{ color: T.accent }}>разница</span>?</> })}</h2></div>
    ...
    /* XARID — ul */
          ...
          <div className="cmp2-card fade-up delay-2">
            <div className="cmp2-hd">🛒 {tr2({ uz: "Xarid ro'yxati", ru: "Список покупок" })} <span className="cmp2-chip">&lt;ul&gt;</span></div>
            <span className="cmp2-order no">{tr2({ uz: "tartib muhim EMAS", ru: "порядок НЕ важен" })}</span>
            ...
            <div className="cmp2-bp">
              ...
              <div className="cmp2-site shop">
                <div className="cmp2-top"><span className="cmp2-logo">🛒</span><span className="cmp2-brand">{tr2({ uz: "Mahsulotlar", ru: "Продукты" })}</span><span className="cmp2-nav"><span>{tr2({ uz: "Bosh", ru: "Главная" })}</span><span>{tr2({ uz: "Savat", ru: "Корзина" })}</span></span></div>
                <div className="cmp2-body">
                  <p className="cmp2-site-h">{tr2({ uz: "Bugungi xaridlar", ru: "Покупки на сегодня" })}</p>
            ...
            <div className="cmp2-codewrap">
              <span className="cmp2-pvlbl">&lt;/&gt; {tr2({ uz: "HTML kod", ru: "HTML-код" })}</span>
              {codeList("ul", SHOP)}
            </div>
            </div>
          </div>
    ...
    /* RETSEPT — ol */
          ...
          <div className="cmp2-card fade-up delay-3">
            <div className="cmp2-hd">🥤 {tr2({ uz: "Limonad retsepti", ru: "Рецепт лимонада" })} <span className="cmp2-chip">&lt;ol&gt;</span></div>
            <span className="cmp2-order yes">{tr2({ uz: "tartib MUHIM", ru: "порядок ВАЖЕН" })}</span>
            <div className="cmp2-pair">
            <div className="cmp2-bp">
              ...
              <div className="cmp2-site lemon">
                <div className="cmp2-top"><span className="cmp2-logo">🥤</span><span className="cmp2-brand">{tr2({ uz: "Limonad", ru: "Лимонад" })}</span><span className="cmp2-nav"><span>{tr2({ uz: "Retseptlar", ru: "Рецепты" })}</span></span></div>
                <div className="cmp2-body">
                  <p className="cmp2-site-h">{tr2({ uz: "Limonad retsepti", ru: "Рецепт лимонада" })}</p>
        ...
        <div className="cmp2-replaywrap">
          <button className="cmp2-replay" onClick={replay}>↻ {tr2({ uz: "Qaytadan ko'rsatish", ru: "Показать ещё раз" })}</button>
        </div>

        <div className="cmp2-concl fade-up delay-3">
          <p>{tr2({ uz: <>💡 <b>🛒 ul</b> — belgili (•), tartib muhim emas. <b>🥤 ol</b> — raqamli (1, 2, 3), tartib muhim (limonadda avval shakar, keyin limon bo'lsa — boshqacha chiqadi!). Har bir element — <span className="mono" style={{ color: T.accent }}>&lt;li&gt;</span>.</>, ru: <>💡 <b>🛒 ul</b> — с маркерами (•), порядок не важен. <b>🥤 ol</b> — с номерами (1, 2, 3), порядок важен (если в лимонаде сначала сахар, а потом лимон — получится иначе!). Каждый пункт — <span className="mono" style={{ color: T.accent }}>&lt;li&gt;</span>.</> })}</p>
        </div>
      </div>
    </Stage>;
};
var Screen11 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri tegni tanlang", ru: "Выберите верный тег" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: "Retsept qadamlarini raqamli tartibda ko'rsatish uchun qaysi tegdan boshlash kerak?", ru: "С какого тега начать, чтобы показать шаги рецепта пронумерованным списком?" })}</h2></>}
  options={["`<li>`", "`<ol>`", "`<ul>`", "`<a>`"]}
  correctIdx={1}
  explainCorrect={tr2({ uz: "To'g'ri. `<ol>` — ordered list, ya'ni raqamli ro'yxat. Tartib muhim bo'lgan joylarda ishlatiladi.", ru: "Верно. `<ol>` — ordered list, то есть нумерованный список. Используется там, где важен порядок." })}
  explainWrong={{ 2: tr2({ uz: "`<ul>` — bu belgili (bullet) ro’yxat. Raqam emas, nuqta chiqaradi.", ru: "`<ul>` — маркированный (bullet) список. Выводит точки, а не номера." }), 0: tr2({ uz: "`<li>` — bu alohida element. Avval uni o’rab oluvchi `<ol>` yoki `<ul>` kerak.", ru: "`<li>` — отдельный пункт. Сначала нужен обёртывающий `<ol>` или `<ul>`." }), 3: tr2({ uz: "`<a>` — bu havola tegi, ro’yxatga aloqasi yo’q.", ru: "`<a>` — тег ссылки, к спискам он не относится." }), default: tr2({ uz: "Raqamli ro’yxat uchun `<ol>` ishlatiladi.", ru: "Для нумерованного списка используется `<ol>`." }) }}
/>;
var Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const PAGES = {
    bosh: { title: tr2({ uz: "Bosh sahifa", ru: "Главная" }), file: "index.html", url: "sayt.uz", body: tr2({ uz: "Salom! Bu mening birinchi saytim. O'zim haqimda yozaman.", ru: "Привет! Это мой первый сайт. Пишу о себе." }), links: [{ to: "oyinlar", label: tr2({ uz: "O'yinlar", ru: "Игры" }) }, { to: "men", label: tr2({ uz: "Men haqimda", ru: "Обо мне" }) }] },
    oyinlar: { title: tr2({ uz: "O'yinlar", ru: "Игры" }), file: "oyinlar.html", url: "sayt.uz/oyinlar.html", body: tr2({ uz: "Bo'sh vaqtimda Minecraft va futbol o'ynayman.", ru: "В свободное время играю в Minecraft и футбол." }), links: [{ to: "minecraft", label: tr2({ uz: "Minecraft haqida", ru: "Про Minecraft" }) }, { to: "bosh", label: tr2({ uz: "Bosh sahifa", ru: "Главная" }) }] },
    minecraft: { title: "Minecraft", file: "minecraft.html", url: "sayt.uz/minecraft.html", body: tr2({ uz: "Minecraft — menga eng yoqadigan o'yin.", ru: "Minecraft — моя самая любимая игра." }), links: [{ to: "oyinlar", label: tr2({ uz: "O'yinlar", ru: "Игры" }) }, { to: "bosh", label: tr2({ uz: "Bosh sahifa", ru: "Главная" }) }] },
    men: { title: tr2({ uz: "Men haqimda", ru: "Обо мне" }), file: "men.html", url: "sayt.uz/men.html", body: tr2({ uz: "Men CoddyCamp o'quvchisiman. Web-dasturchi bo'lmoqchiman.", ru: "Я ученик CoddyCamp. Хочу стать веб-разработчиком." }), links: [{ to: "bosh", label: tr2({ uz: "Bosh sahifa", ru: "Главная" }) }] }
  };
  const POS = { bosh: [55, 72], oyinlar: [150, 34], minecraft: [216, 90], men: [108, 122] };
  const EDGES = [["bosh", "oyinlar"], ["bosh", "men"], ["oyinlar", "minecraft"]];
  const [page, setPage] = useState3("bosh");
  const [jumped, setJumped] = useState3(false);
  const done = jumped;
  const [revealed, setRevealed] = useState3(false);
  const [travel, setTravel] = useState3(null);
  const travelKey = useRef3(0);
  const travelTimer = useRef3(null);
  const mctx = useContext2(MentorCtx) || {};
  const isNarrow = useIsMobile(768);
  const showResult = !isNarrow || revealed;
  useEffect4(() => () => clearTimeout(travelTimer.current), []);
  const go = (to) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Havolalar", ru: "Ссылки" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "✍️ Praktika →", ru: "✍️ Практика →" }) : tr2({ uz: "Havolani bosing", ru: "Нажмите на ссылку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bir bosishda boshqa sahifaga qanday <span className="italic" style={{ color: T.accent }}>o'tamiz</span>?</>, ru: <>Как одним кликом <span className="italic" style={{ color: T.accent }}>перейти</span> на другую страницу?</> })}</h2></div>
        ...
        <div className="split">
          <div className="col"><div className="flow-label">{tr2({ uz: `Veb — bu "to'r"`, ru: "Веб — это «паутина»" })}</div><div className="web fade-up delay-2"><svg className="web-svg" viewBox="0 0 260 150" preserveAspectRatio="none">{EDGES.map(([a, b], i) => {
  ...
  })}{travel && <div className="web-travel" key={travel.key} style={{ "--fx": travel.fx, "--fy": travel.fy, "--tx": travel.tx, "--ty": travel.ty }} />}</div><p className="web-cap">{tr2({ uz: <>Har sahifa boshqasiga <b>havola</b> bilan bog'langan. Shu bog'lanishlar <b>"to'r"</b> hosil qiladi — <b>Veb</b> (World Wide Web) shundan nom olgan.</>, ru: <>Каждая страница связана с другой <b>ссылкой</b>. Эти связи образуют <b>«паутину»</b> — отсюда и имя <b>Веб</b> (World Wide Web).</> })}</p></div>
          {showResult && <div className="col">
            {!isNarrow && <div className="bp-window fade-up delay-2"><div className="bp-bar"><span className="bb-dots"><i /><i /><i /></span><span className="bp-url" key={`u-${page}`}><span className="lock">●</span>{cur.url}</span></div><div className="bp-body pg-in" key={`p-${page}`}><div className="site-top"><span className="site-wordmark">{tr2({ uz: "Mening saytim", ru: "Мой сайт" })}</span><span className="site-tag">{tr2({ uz: "o'quvchi · CoddyCamp", ru: "ученик · CoddyCamp" })}</span></div><h1 className="pg-h1">{cur.title}</h1><p className="pg-body">{cur.body}</p><div className="pg-divider" /><p className="pg-linklabel">{tr2({ uz: "Boshqa sahifalar", ru: "Другие страницы" })}</p><div className="pg-links">{cur.links.map((l, i) => <a key={i} className="pg-a" onClick={() => go(l.to)}>{l.label} <span className="arr">→</span></a>)}</div></div></div>}
            <div className="codecard" key={`c-${page}`}><p className="codecard-top"><span className="dotf" />{cur.file} — {tr2({ uz: "havolalar kodi", ru: "код ссылок" })}</p><pre className="codeblock">{cur.links.map((l, i) => <span className="ln" key={i}><span className="tg">&lt;a </span><span className="at">href</span><span className="tx">=</span><span className="st">"{PAGES[l.to].file}"</span><span className="tg">&gt;</span><span className="tx">{l.label}</span><span className="tg">&lt;/a&gt;</span></span>)}</pre><p className="codecap">{tr2({ uz: <>Har bir havola = bitta <span className="mn">&lt;a&gt;</span> teg. U ikki qismdan iborat: <span className="mn">href="…"</span> ichida — <b>manzil</b> (bosilganda qayerga borish), teglar <b>orasida</b> — sahifada <b>ko'rinadigan, bosiladigan yozuv</b>.</>, ru: <>Каждая ссылка = один тег <span className="mn">&lt;a&gt;</span>. Он состоит из двух частей: внутри <span className="mn">href="…"</span> — <b>адрес</b> (куда идти по клику), а <b>между</b> тегами — <b>видимая кликабельная надпись</b>.</> })}</p></div>
          </div>}
        </div>
        {showResult && <div className="frame-success fade-up delay-3" style={{ padding: "9px 15px" }}><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <><b>Sizning loyihangiz:</b> saytingizga sevimli sayt yoki Telegram kanalingizga havola qo'shasiz.</>, ru: <><b>Ваш проект:</b> вы добавите на свой сайт ссылку на любимый сайт или свой Telegram-канал.</> })}</p></div>}
      </div>
    </Stage>;
};
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const MAX = 6;
  const CHIPS = [{ key: "h1", label: tr2({ uz: "Sarlavha", ru: "Заголовок" }), tag: "h1" }, { key: "p", label: tr2({ uz: "Matn", ru: "Текст" }), tag: "p" }, { key: "ul", label: tr2({ uz: "Ro'yxat", ru: "Список" }), tag: "ul" }, { key: "a", label: tr2({ uz: "Havola", ru: "Ссылка" }), tag: "a" }, { key: "img", label: tr2({ uz: "Rasm", ru: "Картинка" }), tag: "img" }];
  const detect = (txt) => {
  ...
  const elCode = (type) => {
    switch (type) {
      ...
      case "p":
        return <><Tg>{"<p>"}</Tg>{tr2({ uz: "Men HTML o'rganyapman.", ru: "Я изучаю HTML." })}<Tg>{"</p>"}</Tg></>;
      case "ul":
        return <><Tg>{"<ul>"}</Tg>{"\n    "}<Tg>{"<li>"}</Tg>{tr2({ uz: "Futbol", ru: "Футбол" })}<Tg>{"</li>"}</Tg>{"\n    "}<Tg>{"<li>"}</Tg>{tr2({ uz: "Kitob", ru: "Книги" })}<Tg>{"</li>"}</Tg>{"\n  "}<Tg>{"</ul>"}</Tg></>;
      case "a":
  ...
  const elView = (type, i) => {
    switch (type) {
      ...
      case "p":
        return <p key={i} style={{ fontFamily: "Georgia, serif", margin: "0 0 6px", color: T.ink, fontSize: "clamp(13px,1.8vw,15px)" }}>{tr2({ uz: "Men HTML o'rganyapman.", ru: "Я изучаю HTML." })}</p>;
      case "ul":
        return <ul key={i} style={{ fontFamily: "Georgia, serif", color: T.ink, margin: "0 0 6px", paddingLeft: 22, fontSize: "clamp(13px,1.8vw,15px)" }}><li>{tr2({ uz: "Futbol", ru: "Футбол" })}</li><li>{tr2({ uz: "Kitob", ru: "Книги" })}</li></ul>;
      case "a":
  ...
  const submit = () => {
    ...
    if (!type) {
      setHint(tr2({ uz: "Tushunmadim 🙂 Mana shulardan yozing: sarlavha, matn, ro’yxat, havola, rasm.", ru: "Не понял 🙂 Напишите что-то из этого: заголовок, текст, список, ссылка, картинка." }));
      return;
    }
    generate(type);
    setText("");
  ...
  return <Stage eyebrow={tr2({ uz: "Amaliyot · sahifa quramiz", ru: "Практика · строим страницу" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Kamida 3 ta bo’lak (${items.length}/3)`, ru: `Минимум 3 блока (${items.length}/3)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Buyruq bering — <span className="italic" style={{ color: T.accent }}>kod o'zi yaraladi</span>.</>, ru: <>Дайте команду — <span className="italic" style={{ color: T.accent }}>код появится сам</span>.</> })}</h2></div>
    ...
    if (e.key === "Enter") submit();
            ...
            <div className="fade-up delay-2"><p className="flow-label" style={{ margin: "2px 0 7px" }}>{tr2({ uz: "yoki tayyor buyruqlardan tanlang", ru: "или выберите из готовых команд" })}</p><div className="chips">{CHIPS.map((c) => <button key={c.key} className="gchip" disabled={items.length >= MAX} onClick={() => {
    setText(tr2({ uz: c.label.toLowerCase() + " qo'sh", ru: "добавь " + c.label.toLowerCase() }));
    setHint("");
  }}>{c.label} <span className="gt">&lt;{c.tag}&gt;</span></button>)}{items.length > 0 && <button className="gchip" onClick={reset}>↺ {tr2({ uz: "Tozalash", ru: "Очистить" })}</button>}</div></div>
            <p className="body fade-up delay-3" style={{ margin: "2px 0 0", color: T.ink3, fontSize: 13 }}>{tr2({ uz: <><b style={{ color: T.ink2 }}>Tez tugatdingizmi?</b> 5 xil teg ishlating yoki yangi teg topib sinab ko'ring.</>, ru: <><b style={{ color: T.ink2 }}>Закончили быстро?</b> Используйте 5 разных тегов или найдите и попробуйте новый.</> })}</p>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  const RECAP = [tr2({ uz: "HTML bilan veb-sahifa yasash", ru: "Делать веб-страницу на HTML" }), tr2({ uz: "Sahifa skeletini qurish (head, body)", ru: "Строить скелет страницы (head, body)" }), tr2({ uz: "Teglarni ochish va yopish", ru: "Открывать и закрывать теги" }), tr2({ uz: "Sarlavha (h1–h6) va matn (strong, em)", ru: "Заголовки (h1–h6) и текст (strong, em)" }), tr2({ uz: "Ro'yxat (ul/ol) va havola (a) qo'shish", ru: "Добавлять списки (ul/ol) и ссылки (a)" })];
  const HOMEWORK2 = [{ b: tr2({ uz: "Sarlavha (h1)", ru: "Заголовок (h1)" }), t: tr2({ uz: "— ismingiz", ru: "— ваше имя" }) }, { b: tr2({ uz: "2–3 paragraf", ru: "2–3 абзаца" }), t: tr2({ uz: "— o'zingiz haqingizda", ru: "— о себе" }) }, { b: tr2({ uz: "Ro'yxat", ru: "Список" }), t: tr2({ uz: "— sevimli mashg'ulotlaringiz", ru: "— ваши любимые занятия" }) }, { b: tr2({ uz: "Havola", ru: "Ссылка" }), t: tr2({ uz: "— sevimli saytingizga", ru: "— на любимый сайт" }) }];
  ...
  return <Stage eyebrow={tr2({ uz: "Tayyor", ru: "Готово" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Keyingi dars →", ru: "Следующий урок →" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Birinchi <span className="italic" style={{ color: T.accent }}>saytingizni</span> yasadingiz.</>, ru: <>Вы сделали свой первый <span className="italic" style={{ color: T.accent }}>сайт</span>.</> })}</h2>{
...
var TASK_HW = {
  ...
  requirements: [
    ...
    { id: "p2", label: { uz: "kamida 2 ta <p> — o'zingiz haqingizda", ru: "минимум 2 <p> — о себе" }, check: C.count("p", 2, { uz: "O'zingiz haqingizda kamida 2 ta `<p>` paragraf yozing", ru: "Напишите о себе минимум 2 абзаца `<p>`" }) },
    { id: "li2", label: { uz: "ro'yxatda kamida 2 ta <li> — sevimli mashg'ulotlaringiz", ru: "в списке минимум 2 <li> — любимые занятия" }, check: C.count("li", 2, { uz: "Sevimli mashg'ulotlaringizni `<ul>` ichida kamida 2 ta `<li>` qilib yozing", ru: "Напишите любимые занятия — минимум 2 пункта `<li>` внутри `<ul>`" }) },
    { id: "a", label: { uz: "<a> — sevimli saytingizga havola", ru: "<a> — ссылка на любимый сайт" }, check: C.attr("a", "href", { uz: 'Sevimli saytingizga `<a href="https://...">nomi</a>` havola qo\'shing', ru: 'Добавьте ссылку на любимый сайт: `<a href="https://...">имя</a>`' }) }
  ]
};
var STARTER_HW = {
  uz: `<!-- Bu yerga yozing -->
      ...
      <div className="tg-call" style={callStyle}>
        ...
        <div className="tg-nav">
          ...
          <div className="tg-navr">
            ...
            <button className="tg-btn go" onClick={next}>{last ? tr2({ uz: "Boshladik!", ru: "Поехали!" }) : tr2({ uz: "Keyingisi →", ru: "Дальше →" })}</button>
          </div>
        </div>
      </div>
    </div>;
