...
var ScreenHook = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>Dasturchilar kodni <span className="italic" style={{ color: T.accent }}>qayerga</span> yozadi?</>, ru: <>Куда разработчики <span className="italic" style={{ color: T.accent }}>пишут</span> код?</> })}</h1>
...
var ScreenFinalTest = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [val, setVal] = useState3(typeof storedAnswer?.studentAnswer === "string" ? storedAnswer.studentAnswer : "");
