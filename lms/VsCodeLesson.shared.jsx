...
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/e069aaaa4d94bd163588a759a75419dc.jsx src/1-Modull/VsCodeLesson.jsx
...
function useLiveSession(lessonId, answerKey) {
  ...
  const joinStudent = useCallback(async (raw, rawNick) => {
    ...
    try {
      ...
      if (row.status !== "live") {
        setJoinError(tr({ uz: "Bu dars allaqachon tugagan.", ru: "Этот урок уже завершён." }));
        setBusy(false);
        return;
      }
...
var Stage = ({ children, eyebrow, screen, totalScreens = TOTAL_SCREENS, navContent, narrow, mentorStatic }) => {
  ...
  const [mForced, setMForced] = useState(false);
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">🙈 {tr({ uz: "Kim nimani tanlagani va ✅/❌ soni hozircha yashirin. «Natijani ochish» bosilganda — sizda ham, o'quvchilarda ham birdan ochiladi.", ru: "Кто что выбрал и число ✅/❌ пока скрыто. Нажмёте «Открыть результат» — появится сразу и у вас, и у учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
      ...
      {reveal && struggling && <p className="mstats-warn">⚠️ {tr({ uz: "Ko'pchilik adashdi — mavzu tushunarsiz qolganga o'xshaydi. Yana bir bor tushuntiring.", ru: "Большинство ошиблись — похоже, тема осталась непонятной. Объясните ещё раз." })}</p>}
...
function MentorPracticeOverlay({ entry, live, onClose }) {
  ...
  return <div className="mp-overlay">
      <div className="mp-card">
        ...
        <p className="mp-tip">💡 {tr({ uz: "Ko'pchilik tugatgach, shu mashqni doskada birga yozing. Shunda o'quvchilar o'zini tekshiradi va mavzu esida mahkam qoladi.", ru: "Когда закончит большинство, напишите это же упражнение вместе на доске. Так ученики проверят себя, а тема запомнится крепко." })}</p>
      </div>
    </div>;
}
...
function DragDropOrder({ items, hints, onSolved }) {
  ...
  return <div className="dd fade-up">
      ...
      {solved && <div className="dd-done">✓ {tr({ uz: "To'g'ri! Shablon aynan shu tartibda.", ru: "Верно! Шаблон именно в таком порядке." })}</div>}
...
var ScreenHook = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up" style={{ maxWidth: 780 }}>{tr({ uz: <>Dasturchilar kodni <span className="italic" style={{ color: T.accent }}>qayerga</span> yozadi?</>, ru: <>Куда разработчики <span className="italic" style={{ color: T.accent }}>пишут</span> код?</> })}</h1>
...
var ScreenGoal = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Maqsad", ru: "Цель" })} screen={screen} audioState={audio} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head">
          <h2 className="title h-title fade-up">{tr({ uz: <>Bugun <span className="italic" style={{ color: T.accent }}>o'z kompyuteringizda</span> birinchi sahifangizni yasaysiz.</>, ru: <>Сегодня вы сделаете первую страницу <span className="italic" style={{ color: T.accent }}>на своём компьютере</span>.</> })}</h2>
        </div>
...
var ScreenTour = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "2-qadam · Tanishuv", ru: "2-й шаг · Знакомство" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !allSeen} label={_isMentorLive || allSeen ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "4 panelni bosib chiqing", ru: "Нажмите на 4 панели" })} onClick={onNext} /></>}>
      <div className="screen">
        ...
        <Split>
          ...
          <Col>
            {active ? <div className="sk-info fade-step" key={active}>
              ...
              </div> : <div className="hint fade-up delay-2">{tr({ uz: "Chapdagi oynaning istalgan panelini bosing — shu yerda tushuntirish chiqadi.", ru: "Нажмите на любую панель окна слева — здесь появится объяснение." })}</div>}
            {allSeen && <div className="frame-success fade-step">
                <p className="body" style={{ margin: 0 }}>{tr({ uz: <>Ajoyib — <b>4 panelning hammasini</b> bilib oldingiz! Endi bu oyna siz uchun begona emas.</>, ru: <>Отлично — вы узнали <b>все 4 панели</b>! Теперь это окно для вас не чужое.</> })}</p>
              </div>}
          </Col>
        </Split>
      </div>
...
var ScreenTest1 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr({ uz: "Oynani eslang", ru: "Вспомните окно" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr({ uz: "Loyihadagi fayllar ro'yxatini VS Code'ning qaysi paneli ko'rsatadi?", ru: "Какая панель VS Code показывает список файлов проекта?" })}</h2></>}
  options={[
...
var ScreenEmmet = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [phase, setPhase] = useState(storedAnswer ? 2 : 0);
  ...
  return <Stage eyebrow={tr({ uz: "3-qadam · Shablon", ru: "3-й шаг · Шаблон" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !solved} label={_isMentorLive || solved ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Shablonni yig'ing", ru: "Соберите шаблон" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <><span className="mono" style={{ color: T.accent }}>!</span> + Tab bosilsa, <span className="italic" style={{ color: T.accent }}>butun shablon</span> o'zi yoziladimi?</>, ru: <><span className="mono" style={{ color: T.accent }}>!</span> + Tab — и <span className="italic" style={{ color: T.accent }}>весь шаблон</span> напишется сам?</> })}</h2></div>
  ...
  />
            ...
            <div className="br-controls fade-up delay-2">
              {phase === 0 && <button className="btn" onClick={play}>✨ {tr({ uz: "Natijani ko'rish", ru: "Посмотреть результат" })}</button>}
              {phase === 2 && <button className="btn-ghost" onClick={() => {
    setPhase(0);
  }}>↻ {tr({ uz: "Yana bir bor ko'rish", ru: "Посмотреть ещё раз" })}</button>}
    ...
    /* F-0810-06: `Ctrl+Z` ilgari faqat `s12` (Bonus) ekranida bir marta
       ...
       birinchi marta shu yerda qidiradi. */
            ...
            {phase === 2 && <p className="yz-hint fade-up delay-2" style={{ marginTop: 6 }}>{fmtCode(tr({
    uz: "Noto'g'ri chiqdimi? `Ctrl+Z` bosing — hammasi orqaga qaytadi.",
    ru: "Получилось не то? Нажмите `Ctrl+Z` — всё вернётся назад."
  }))}</p>}
          </Col>
          <Col>
            {phase < 2 ? <div className="hint fade-up delay-2">{tr({ uz: "Avval chapdagi «Natijani ko'rish» tugmasini bosing — shablon ochilgach, bu yerda mashq chiqadi.", ru: "Сначала нажмите «Посмотреть результат» слева — когда шаблон раскроется, здесь появится упражнение." })}</div> : <div className="sk-buildbox">
...
var ScreenLive = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const nm = CARD_NAME;
  const [view, setView] = useState(0);
  const [viewed, setViewed] = useState(() => /* @__PURE__ */ new Set([0]));
  const go = (v) => {
    setView(v);
    setViewed((prev) => {
  ...
  return <Stage eyebrow={tr({ uz: "4-qadam · Jonli server", ru: "4-й шаг · Живой сервер" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !(storedAnswer && storedAnswer.correct)} label={_isMentorLive || storedAnswer && storedAnswer.correct ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Qadamlarni belgilang", ru: "Отметьте шаги" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <><span className="italic" style={{ color: T.accent }}>Go Live</span> bilan sahifani brauzerda ochamiz.</>, ru: <>Открываем страницу в браузере через <span className="italic" style={{ color: T.accent }}>Go Live</span>.</> })}</h2></div>
  ...
  />}
    ...
    statusPulse
  />}
              {view === 2 && <BrowserWin url="127.0.0.1:5500/index.html" tab={tr({ uz: "Mening sahifam", ru: "Моя страница" })} minH={170}>
...
var ScreenTest2 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr({ uz: "Jonli server", ru: "Живой сервер" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr({ uz: "Pastdagi «Go Live» tugmasi bosilganda nima bo’ladi?", ru: "Что происходит, когда вы нажимаете «Go Live» внизу?" })}</h2></>}
  options={[
      ...
      <div className="screen">
        ...
        <Split>
          ...
          <Col>
            ...
            <div className="hl-note fade-up delay-3"><p className="body" style={{ margin: 0 }}>{tr({ uz: <>Oddiy va rangsiz — chunki <b className="mono">style.css</b> hali bo'sh. Keyingi qadamda uni to'ldiramiz. 😉</>, ru: <>Просто и бесцветно — потому что <b className="mono">style.css</b> пока пуст. На следующем шаге мы его заполним. 😉</> })}</p></div>
          </Col>
        </Split>
      </div>
    </Stage>;
...
var ScreenCardCss = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "6-qadam · Card CSS", ru: "6-й шаг · CSS карточки" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !allOn} label={_isMentorLive || allOn ? tr({ uz: "Praktikaga →", ru: "К практике →" }) : tr({ uz: "4 bezakni yoqing", ru: "Включите 4 оформления" })} onClick={onNext} /></>}>
      <div className="screen">
        ...
        <Split>
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {STEPS.map((s) => <button key={s.id} className={`chip ${on.has(s.id) ? "chip-on" : ""}`} onClick={() => toggle(s.id)}>{on.has(s.id) ? "✓ " : ""}{tr(s.chip)}</button>)}
            </div>
            <div className="code-box fade-up delay-1" style={{ fontSize: "clamp(12.5px,1.6vw,14px)" }}>
              <div><span style={{ color: "#D7BA7D" }}>.card</span> {"{"}</div>
              {STEPS.filter((s) => on.has(s.id)).map((s) => <div key={s.id} className="fade-step">{"  "}<span style={{ color: CODE.attr }}>{s.css.split(":")[0]}</span>:<span style={{ color: CODE.str }}>{s.css.split(":").slice(1).join(":").replace(";", "")};</span> <span className="cm">{"/* "}{tr(s.d)}{" */"}</span></div>)}
...
var ScreenTest3 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr({ uz: "Ikki fayl do’stligi", ru: "Дружба двух файлов" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr({ uz: "style.css'dagi bezaklar sahifaga qaysi qator orqali ulanadi?", ru: "Через какую строку оформление из style.css подключается к странице?" })}</h2></>}
  options={[
...
var ScreenKeys = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Bonus · Tezkor tugmalar", ru: "Бонус · Горячие клавиши" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={_isMentorLive ? false : !allPressed} label={_isMentorLive || allPressed ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "4 birikmani bosing", ru: "Нажмите 4 сочетания" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Dasturchining <span className="italic" style={{ color: T.accent }}>maxfiy tugmalari</span> qanday?</>, ru: <>Какие у разработчика <span className="italic" style={{ color: T.accent }}>секретные клавиши</span>?</> })}</h2></div>
        <Mentor>{tr({ uz: <>Bu tugmalar <b style={{ color: T.ink }}>vaqtni tejaydi</b>. Har birini <b style={{ color: T.ink }}>bosib</b>, nima qilishini ko'ring.</>, ru: <>Эти клавиши <b style={{ color: T.ink }}>экономят время</b>. <b style={{ color: T.ink }}>Нажмите</b> на каждую и посмотрите, что она делает.</> })}</Mentor>
        {
    /* 🔴 F-0810-04: ilgari bu ekran `narrow` (680px) edi va butun dars ichida
       ...
       naqshda: chapda bosiladigan narsalar, o'ngda tushuntirish. */
        ...
        <Split>
          ...
          <Col>
            {active ? <div className="sk-info fade-step" key={active}>
                <div className="sk-tagbig"><span className="sk-wordbadge">{tr(KEYS.find((k) => k.id === active).name)}</span></div>
                <p className="body" style={{ marginTop: 8 }}>{tr(KEYS.find((k) => k.id === active).d)}</p>
              </div> : <div className="hint fade-up delay-2">{tr({ uz: "Chapdagi istalgan birikmani bosing — shu yerda tushuntirish chiqadi.", ru: "Нажмите на любое сочетание слева — здесь появится объяснение." })}</div>}
...
var ScreenFinalTest = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Yakuniy sinov", ru: "Финальная проверка" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={isMentorLive ? false : !correct} label={isMentorLive ? tr({ uz: "Davom etish", ru: "Продолжить" }) : correct ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Tugma nomini yozing", ru: "Напишите имя кнопки" })} onClick={onNext} /></>}>
      <div className="screen">
        ...
        <Mentor>{tr({ uz: <>Yakuniy savol: sahifani brauzerda ochish uchun <b style={{ color: T.ink }}>qaysi tugmani</b> bosamiz? Javob <b style={{ color: T.ink }}>ikki so'zdan</b> iborat.</>, ru: <>Финальный вопрос: <b style={{ color: T.ink }}>какую кнопку</b> мы нажимаем, чтобы открыть страницу в браузере? Ответ — <b style={{ color: T.ink }}>два слова</b>.</> })}</Mentor>
...
var ScreenFinale = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Marra", ru: "Финиш" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive label={tr({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen">
        <Confetti />
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Birinchi <span className="italic" style={{ color: T.accent }}>web-cardingiz</span> tayyor.</>, ru: <>Ваша первая <span className="italic" style={{ color: T.accent }}>web-card</span> готова.</> })}</h2></div>
        ...
        <Split>
          <Col>
            <ul className="recipe-list fade-up delay-1">
              {DONE.map((d, i) => <li key={i} className="on"><span className="recipe-num">✓</span><span className="recipe-text">{fmtCode(tr(d))}</span></li>)}
            </ul>
            {!isMentorLive && <div className="frame-success fade-step"><p className="body" style={{ margin: 0 }}>{tr({ uz: <>Bugundan boshlab kodni <b>o'z kompyuteringizga</b> yozasiz. Endi har darsda shu qurollar bilan ishlaymiz.</>, ru: <>С сегодняшнего дня вы пишете код <b>на своём компьютере</b>. Теперь на каждом уроке работаем этими инструментами.</> })}</p></div>}
            {isMentorLive && <MentorWorkStats live={live} screenIdx={screen} taskLabel={tr({ uz: "Yakuniy ekranga yetdi", ru: "Дошёл до финального экрана" })} />}
          </Col>
          <Col>
...
var ScreenSummary = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  const RECAP = [
    ...
    tr({ uz: "Card yasab, CSS bilan bezash", ru: "Делать card и оформлять её CSS" })
  ];
  const HOMEWORK = [
  ...
  return <Stage eyebrow={tr({ uz: "Tayyor", ru: "Готово" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Keyingi dars →", ru: "Следующий урок →" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Endi sizda <span className="italic" style={{ color: T.accent }}>haqiqiy dasturchi quroli</span> bor.</>, ru: <>Теперь у вас есть <span className="italic" style={{ color: T.accent }}>настоящий инструмент разработчика</span>.</> })}</h2>{
...
var ScreenDemoIdea = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [picking, setPicking] = useState(!hasSaved);
  ...
  const confirm = () => {
    ...
    if (picking) {
      if (sel === "ozim") demoWrite({ muammo: ownM.trim(), yechim: ownY.trim(), manba: "ozim", holat: "tasdiqlandi" });
        ...
        demoWrite({ muammo: tr(b.muammo), yechim: tr(b.yechim), manba: b.id, holat: "tasdiqlandi" });
      }
  ...
  const cur = demoRead();
  return <Stage eyebrow={tr({ uz: "Demo Day loyihasi", ru: "Проект Demo Day" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!confirmed && !isMentorLive} label={confirmed || isMentorLive ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Loyihani tasdiqlang", ru: "Подтвердите проект" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Demo Day uchun <span className="italic" style={{ color: T.accent }}>g'oyangiz</span> tayyormi?</>, ru: <>Готова ли ваша <span className="italic" style={{ color: T.accent }}>идея</span> для Demo Day?</> })}</h2></div>
        ...
        {isMentorLive ? <MentorWorkStats live={live} screenIdx={screen} taskLabel={tr({ uz: "Demo Day loyihasini aytish", ru: "Назвать проект Demo Day" })} /> : <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 8, maxWidth: 680 }}>
          {!picking && hasSaved && <>
    ...
    setPicking(true);
    setSel(null);
  }}>{tr({ uz: "Boshqasini tanlayman", ru: "Выберу другой" })}</button>
                </div>}
            </>}
          {picking && <>
    ...
    return <button key={b.id} className={"hook-option " + (on ? "on" : "")} onClick={() => setSel(b.id)}>
                      <span className="radio">{on && <span className="radio-dot" />}</span>
                      <span>{b.ic} {tr(b.muammo)} <span style={{ color: on ? void 0 : T.ink2 }}>→ {tr(b.yechim)}</span></span>
                    </button>;
  })}
              ...
              {!confirmed && <button className="btn" disabled={!ready} style={{ background: ready ? T.success : void 0, alignSelf: "flex-start", opacity: ready ? 1 : 0.5 }} onClick={confirm}>{tr({ uz: "✓ Tasdiqlash", ru: "✓ Подтвердить" })}</button>}
            </>}
...
var ScreenPodium = ({ screen, answers, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Bugungi <span className="italic" style={{ color: T.accent }}>g'oliblarimiz</span>.</>, ru: <>Наши <span className="italic" style={{ color: T.accent }}>победители</span> сегодня.</> })}</h2></div>
    ...
    return <div key={q} className="qstat-row">
              ...
              {live.mode === "mentor" && <p className="small" style={{ margin: "10px 0 0", color: T.ink2 }}>{tr({ uz: "⚠️ belgili savollar — sinf qiynalgan mavzular. Ularni yana bir bor tushuntiring.", ru: "Вопросы со значком ⚠️ — темы, где класс споткнулся. Объясните их ещё раз." })}</p>}
            </div>
          </>}
      </div>
    </Stage>;
...
function VsCodeLesson({ lang: langProp, onFinished, onPractice }) {
  ...
  return <LangContext.Provider value={lang}>
    ...
    lang={__lang}
    task={practice.task}
    starterCode={practice.starter ? tr(practice.starter) : void 0}
