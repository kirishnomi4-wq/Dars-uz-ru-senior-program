...
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx src/1-Modull/VsCodeLesson.jsx
...
var Stage = ({ children, eyebrow, screen, totalScreens = TOTAL_SCREENS, navContent, narrow, mentorStatic }) => {
  ...
  const [mForced, setMForced] = useState3(false);
...
function StepChecklist({ screen, storedAnswer, onAnswer, steps, taskLabel }) {
  ...
  const live = gate.live;
  const isMentorLive = !!(live && live.mode === "mentor");
  const [done, setDone] = useState3(() => new Set(storedAnswer?.doneIds || (storedAnswer?.correct ? steps.map((s) => s.id) : [])));
  const sentRef = useRef3(!!(storedAnswer && storedAnswer.correct));
...
var ScreenTour = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState3(() => new Set(storedAnswer?.seenIds || (storedAnswer?.correct ? Object.keys(HOTS) : [])));
  const sentRef = useRef3(!!(storedAnswer && storedAnswer.correct));
...
var ScreenEmmet = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [phase, setPhase] = useState3(storedAnswer ? 2 : 0);
  const [solved, setSolved] = useState3(!!(storedAnswer && storedAnswer.correct));
...
var ScreenLive = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const nm = CARD_NAME;
  const [view, setView] = useState3(0);
  const [viewed, setViewed] = useState3(() => /* @__PURE__ */ new Set([0]));
  const go = (v) => {
    setView(v);
    setViewed((prev) => {
...
var ScreenCardHtml = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [active, setActive] = useState3(null);
  const [seen, setSeen] = useState3(() => new Set(storedAnswer?.seenIds || (storedAnswer?.correct ? Object.keys(PARTS) : [])));
...
var ScreenCardCss = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [on, setOn] = useState3(() => new Set(storedAnswer?.onIds || (storedAnswer?.correct ? STEPS.map((s) => s.id) : [])));
...
var ScreenKeys = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [pressed, setPressed] = useState3(() => new Set(storedAnswer?.pressedIds || (storedAnswer?.correct ? KEYS.map((k) => k.id) : [])));
  const [active, setActive] = useState3(null);
...
var ScreenFinalTest = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [val, setVal] = useState3(storedAnswer?.studentAnswer ?? "");
...
var ScreenDemoIdea = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [picking, setPicking] = useState3(!hasSaved);
  const [sel, setSel] = useState3(hasSaved ? saved.manba : null);
  const [ownM, setOwnM] = useState3(hasSaved && saved.manba === "ozim" ? saved.muammo : "");
  const [ownY, setOwnY] = useState3(hasSaved && saved.manba === "ozim" ? saved.yechim : "");
  const [confirmed, setConfirmed] = useState3(!!(storedAnswer && storedAnswer.correct));
...
var TASK_CARD = {
  ...
  requirements: [
    ...
    { id: "prof", label: { uz: "quti ichida <p> — orzu-kasb", ru: "внутри коробки <p> — профессия мечты" }, check: C.nested(".card", "p", { uz: "`.card` ichiga `<p>` bilan kim bo'lmoqchi ekaningizni yozing (masalan: Frontend developer)", ru: "Напишите внутри `.card` в `<p>`, кем хотите стать (например: Frontend developer)" }) }
  ]
};
var STARTER_CARD = {
  uz: `<!-- Bu yerga yozing -->
