...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const [row, setRow] = useState3(false);
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>Tugmalar nega <span className="italic" style={{ color: T.accent }}>yonma-yon</span> turmaydi?</>, ru: <>Почему кнопки не встают <span className="italic" style={{ color: T.accent }}>в ряд</span>?</> })}</h1>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [mode, setMode] = useState3("block");
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [flex, setFlex] = useState3(!!storedAnswer);
  const [part, setPart] = useState3(null);
  const [seen, setSeen] = useState3(() => new Set(storedAnswer ? ["box", "kid"] : []));
  const isNarrow = useIsMobile(768);
  const done = flex;
  const tap = (k) => {
    setPart(k);
...
var Screen3b = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Qoida ustaxonasi", ru: "Мастерская правила" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "qoidani yig'ing", ru: "Соберите правило" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <Mentor>{tr2({ uz: <>Bo'laklarni <b style={{ color: T.ink }}>to'g'ri tartibda</b> kataklarga sudrang. To'g'ri yig'ilsa, yonidagi menyu qatorga tiziladi.</>, ru: <>Перетащите блоки в ячейки <b style={{ color: T.ink }}>в правильном порядке</b>. Соберёте верно — меню рядом выстроится в ряд.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [dir, setDir] = useState3(storedAnswer?.dir || "row");
  const [touched, setTouched] = useState3(!!storedAnswer);
  const done = touched;
  const isNarrow = useIsMobile(768);
  const set = (d) => {
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [gap, setGap] = useState3(storedAnswer ? storedAnswer.gap ?? 20 : 8);
  const [touched, setTouched] = useState3(!!storedAnswer);
  const done = touched;
  const isNarrow = useIsMobile(768);
  const set = (v) => {
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [jc, setJc] = useState3(storedAnswer?.jc || "flex-start");
  const [solved, setSolved] = useState3(!!storedAnswer);
  const [burst, setBurst] = useState3(false);
  const done = solved;
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [ai, setAi] = useState3(storedAnswer?.ai || "center");
  const [touched, setTouched] = useState3(!!storedAnswer);
  const done = touched;
  const isNarrow = useIsMobile(768);
  const set = (v) => {
        ...
        <div className="split">
          <div className="col">
            <div className="ai-card fade-up delay-2">
              ...
              {!found && <p className="ai-prompt">{wrongMsg ? tr2({ uz: "Bu qatorda xato yo'q — yana qarang.", ru: "В этой строке ошибки нет — посмотрите ещё раз." }) : tr2({ uz: "Qaysi qator xato? Bosing.", ru: "Какая строка с ошибкой? Нажмите." })}</p>}
              {!done && <AchRule screen={screen} />}
