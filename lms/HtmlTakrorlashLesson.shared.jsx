...
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/81a985c6a19b3e7f7d39be9fda07af4e.jsx src/1-Modull/HtmlTakrorlashLesson.jsx
...
var ScreenHook = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const [found, setFound] = useState3(() => storedAnswer ? new Set(ANAT.map((x) => x.id)) : /* @__PURE__ */ new Set());
...
var ScreenBlitz = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [i, setI] = useState3(storedAnswer ? BLITZ_QS.length : 0);
  const [hits, setHits] = useState3(storedAnswer?.hits ?? 0);
  const [streak, setStreak] = useState3(0);
  const [best, setBest] = useState3(storedAnswer?.best ?? 0);
...
var ScreenDebug = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [fixed, setFixed] = useState3(() => storedAnswer ? new Set(bugIdxs) : /* @__PURE__ */ new Set());
  const [active, setActive] = useState3(null);
  const [wrongLine, setWrongLine] = useState3(-1);
  const [wrongFix, setWrongFix] = useState3(-1);
...
var ScreenSkeletInfo = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [clicked, setClicked] = useState3(() => storedAnswer ? new Set(LAYERS.map((l) => l.id)) : /* @__PURE__ */ new Set());
  const [active, setActive] = useState3(null);
