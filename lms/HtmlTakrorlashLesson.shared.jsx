...
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/e069aaaa4d94bd163588a759a75419dc.jsx src/1-Modull/HtmlTakrorlashLesson.jsx
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr({ uz: "Javoblar yozib olinmoqda. ✅/❌ taqsimoti «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "Ответы записываются. Распределение ✅/❌ откроется по кнопке «Открыть результат» — и у вас, и на экранах учеников." })}</p>}
...
function PageBuild({ built = 0, glow = -1, intro = false, big = false }) {
  ...
  return <div className={`pb-window ${big ? "pb-big" : ""}`}>
      ...
      <div className="pb-body">
        {parts.map((node, i) => i < built ? <div key={i} className={`pb-part on ${glow === i ? "pb-glow" : ""} ${intro ? "pb-intro" : ""}`} style={intro ? { animationDelay: `${0.3 + i * 0.55}s` } : void 0}>{node}</div> : <div key={i} className="pb-part ghost"><span className="pb-ghost-ic">{PB_GHOSTS[i].ic}</span><span className="pb-ghost-lbl">{tr(PB_GHOSTS[i].lbl)}</span></div>)}
      </div>
    </div>;
}
var ScreenHook = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const [found, setFound] = useState(() => storedAnswer ? new Set(ANAT.map((x) => x.id)) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={tr({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={!allFound} label={allFound ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "4 ta qismni toping", ru: "Найдите 4 части" })} onClick={onNext} />}>
...
var ScreenPlan = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Reja", ru: "План" })} screen={screen} audioState={audio} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive label={tr({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
...
var ScreenBlitz = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Eslab olish", ru: "Вспоминаем" })} screen={screen} audioState={audio} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "1-bosqichga →", ru: "К 1-му этапу →" }) : tr({ uz: "Savollarga javob bering", ru: "Ответьте на вопросы" })} onClick={onNext} /></>}>
...
function StageShell({ screen, n, title, mentor, audioText, task, need, built, nextLabel, optional = true, onNext, onPrev }) {
  ...
  return <Stage eyebrow={tr({ uz: `${n}-bosqich / 5`, ru: `Этап ${n} / 5` })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} />{nav}</>}>
      <div className="screen">
...
var ScreenTest1 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr({ uz: "Tekshiruv · 1-savol", ru: "Проверка · вопрос 1" })}
  ...
  question={<>
      ...
      <h2 className="title h-ask" style={{ marginTop: 8 }}>{tr({ uz: "Shu kod uchun brauzer nimani ko'rsatadi?", ru: "Что покажет браузер для этого кода?" })}</h2>
...
var ScreenDebug = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [fixed, setFixed] = useState(() => storedAnswer ? new Set(bugIdxs) : /* @__PURE__ */ new Set());
  const [active, setActive] = useState(null);
  ...
  return <Stage eyebrow={tr({ uz: "Xato topish sinovi", ru: "Испытание: найти ошибку" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: `Xatolarni tuzating (${fixed.size}/2)`, ru: `Исправьте ошибки (${fixed.size}/2)` })} onClick={onNext} /></>}>
...
var ScreenTest2 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr({ uz: "Tekshiruv · 2-savol", ru: "Проверка · вопрос 2" })}
  ...
  question={<>
      ...
      <h2 className="title h-ask" style={{ marginTop: 8 }}>{tr({ uz: <>Havola to'rt xil yozilgan. Qaysi biri <span className="italic" style={{ color: T.accent }}>TO'G'RI</span>?</>, ru: <>Ссылка написана четырьмя способами. Какой из них <span className="italic" style={{ color: T.accent }}>ВЕРНЫЙ</span>?</> })}</h2>
    </>}
  options={[
...
var ScreenStage4 = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "4-bosqich / 5", ru: "Этап 4 / 5" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive label={tr({ uz: "Kompilyatorni ochish →", ru: "Открыть компилятор →" })} onClick={onNext} /></>}>
      <div className="screen">
...
var ScreenForm = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Forma-takror", ru: "Повтор формы" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!built} label={built ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Formani yig'ing", ru: "Соберите форму" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var ScreenSkeletInfo = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [clicked, setClicked] = useState(() => storedAnswer ? new Set(LAYERS.map((l) => l.id)) : /* @__PURE__ */ new Set());
  const [active, setActive] = useState(null);
  ...
  return <Stage eyebrow={tr({ uz: "Qobiq — eslatma", ru: "Оболочка — напоминание" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!all} label={all ? tr({ uz: "Imtihonga →", ru: "К экзамену →" }) : tr({ uz: `Qatlamlarni bosing (${clicked.size}/4)`, ru: `Нажмите слои (${clicked.size}/4)` })} onClick={onNext} /></>}>
      <div className="screen">
        ...
        <Split>
          ...
          <Col>
            {act ? <div className="card fade-step" key={act.id}><div className="card-lbl mono" style={{ color: T.accent }}>{act.tag}</div><p className="body" style={{ margin: 0 }}>{tr(act.role)}</p></div> : <div className="hint fade-up delay-2"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr({ uz: "Chapdagi qatlamni bosing — vazifasi shu yerda chiqadi.", ru: "Нажмите слой слева — его задача появится здесь." })}</p></div>}
...
var ScreenExam = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Usta-imtihoni", ru: "Экзамен мастера" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={isMentorLive ? false : !solved} label={solved || isMentorLive ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Qobiqni yig'ing", ru: "Соберите оболочку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        {isMentorLive ? <div className="frame-soft"><p className="body" style={{ margin: 0 }}>{tr({ uz: "O'quvchilar qobiqni o'z qurilmasida yig'moqda — pastda kim tugatganini ko'rasiz.", ru: "Ученики собирают скелет на своих устройствах — ниже видно, кто закончил." })}</p></div> : <DragDropOrder items={EXAM_PIECES} hints={[
...
var ScreenParty = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Sahifa tayyor!", ru: "Страница готова!" })} screen={screen} audioState={audio} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr({ uz: "Natijalarga →", ru: "К результатам →" })} onClick={onNext} /></>}>
  ...
  return <LangContext.Provider value={lang}>
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        ...
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={tr({ uz: "1-Modul · Takrorlash", ru: "Модуль 1 · Повторение" })} /> : <>
