...
var SiklZavodi = ({ count = 0, max = 5, init = "i = 1", cond = "i <= 5", step = "i++", load: load2 = null, cheksiz = false, whileMode = false, done = false, onStep, onAuto, manual = false, compact = false }) => {
  ...
  return <div className={`zavod ${overflow ? "zavod-cheksiz" : ""} ${done && !cheksiz ? "zavod-done" : ""} ${compact ? "zavod-sm" : ""} ${idle ? "zv-idle" : ""}`}>
      ...
      <div className="zavod-bin">
        {boxes.length === 0 ? <span className="zv-bin-empty">{tr2({ uz: "// yashik hali bo'sh", ru: "// ящик пока пуст" })}</span> : boxes.map((b, k) => <span key={k} className="zv-box" style={{ animationDelay: `${Math.min(k * 0.025, 0.32)}s` }}>{load2 ? "🎁" : "📦"}<b>{String(b)}</b></span>)}
      </div>
      {overflow ? <div className="zavod-alert">⚠️ {tr2({ uz: "CHEKSIZ! — mashina to'xtamayapti", ru: "БЕСКОНЕЧНО! — машина не останавливается" })}</div> : done && <div className="zavod-ding">🔔 {tr2({ uz: "Tayyor! — shart buzildi, sikl to'xtadi", ru: "Готово! — условие нарушилось, цикл остановился" })}</div>}
    </div>;
};
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} navContent={<NavNext optionalLive disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>30 ta do'stingizga bir xil xabarni <span className="italic" style={{ color: T.accent }}>bittalab</span> yozasizmi?</>, ru: <>Будете писать 30 друзьям одно и то же <span className="italic" style={{ color: T.accent }}>по одному</span>?</> })}</h1>
        ...
        <Split three>
          ...
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0, paddingRight: 40 }}>{tr2({ uz: "Dasturchi 1000 ta xabarni qanday yozadi?", ru: "Как программист напишет 1000 сообщений?" })}</p>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [mode, setMode] = useState3("manual");
  const [seen, setSeen] = useState3(/* @__PURE__ */ new Set(["manual"]));
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const N = 5;
  const [out, setOut] = useState3(storedAnswer ? Array.from({ length: N }, (_, i) => i + 1) : []);
  const [iVal, setIVal] = useState3(storedAnswer ? N : 0);
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [end, setEnd] = useState3(5);
  const [step, setStep] = useState3(1);
  const [seen, setSeen] = useState3(/* @__PURE__ */ new Set(["5-1"]));
  const done = seen.size >= 2;
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const STEP = 20;
  const [suv, setSuv] = useState3(storedAnswer ? 100 : 0);
  const [iter, setIter] = useState3(storedAnswer ? 5 : 0);
  const [running, setRunning] = useState3(false);
  const timer = useRef3(null);
  const done = suv >= 100;
  useEffect4(() => () => clearTimeout(timer.current), []);
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const ARR = [{ e: "🍎", n: "olma" }, { e: "🍌", n: "banan" }, { e: "🍇", n: "uzum" }, { e: "🍓", n: "qulupnay" }];
  const N = ARR.length;
  const [hi, setHi] = useState3(-1);
  const [out, setOut] = useState3(storedAnswer ? ARR.map((a) => a.n) : []);
  const [running, setRunning] = useState3(false);
  const timer = useRef3(null);
  const done = out.length >= N;
  useEffect4(() => () => clearTimeout(timer.current), []);
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [out, setOut] = useState3(storedAnswer ? NAMES.map((n) => `${tr2({ uz: "Bayram muborak", ru: "С праздником" })}, ${n}`) : []);
  const [running, setRunning] = useState3(false);
  const timer = useRef3(null);
  const done = out.length >= N;
  useEffect4(() => () => clearTimeout(timer.current), []);
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [n, setN] = useState3(5);
  const [msgIdx, setMsgIdx] = useState3(0);
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [picked, setPicked] = useState3(storedAnswer ? "step" : null);
        ...
        <div className="split">
          <Col>
            ...
            {!fixed ? <div className="term fade-up delay-2">
                ...
                <div className="term-body" style={{ padding: "8px 14px", gap: 3 }}>{[1, 0, -1, -2].map((v, k) => <div key={k} className="term-line"><span className="term-arrow" style={{ color: T.accent }}>›</span><span>{v}</span></div>)}<div className="term-line warn-pulse" style={{ color: T.accent }}><span className="term-arrow" style={{ color: T.accent }}>›</span><span>⋮</span></div><p className="term-empty warn-pulse" style={{ color: T.accent }}>⚠️ {tr2({ uz: "5 ga hech yetmaydi — cheksiz!", ru: "До 5 не дойдёт никогда — бесконечно!" })}</p></div>
