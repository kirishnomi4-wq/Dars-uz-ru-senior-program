...
//  Qayta yig'ish:  node scripts/build-lms.mjs --shared https://go.coddycamp.uz/uploads/course_artifacts/f9e30f4aaecfeada4e3482bfe60877d2.jsx src/2-Modull/JsLoopsLesson.jsx
...
var Terminal = ({ lines, empty = { uz: "// natija shu yerda chiqadi…", ru: "// результат появится здесь…" }, title = "console" }) => <div className="term">
    ...
    <div className="term-body">
      {lines.length === 0 ? <p className="term-empty">{tr(empty)}</p> : lines.map((l, i) => <div key={i} className="term-line"><span className="term-arrow">›</span><span>{l}</span></div>)}
    </div>
  </div>;
...
var SiklZavodi = ({ count = 0, max = 5, init = "i = 1", cond = "i <= 5", step = "i++", load = null, cheksiz = false, whileMode = false, done = false, onStep, onAuto, manual = false, compact = false }) => {
  ...
  return <div className={`zavod ${overflow ? "zavod-cheksiz" : ""} ${done && !cheksiz ? "zavod-done" : ""} ${compact ? "zavod-sm" : ""} ${idle ? "zv-idle" : ""}`}>
      ...
      <div className="zavod-bin">
        {boxes.length === 0 ? <span className="zv-bin-empty">{tr({ uz: "// yashik hali bo'sh", ru: "// ящик пока пуст" })}</span> : boxes.map((b, k) => <span key={k} className="zv-box" style={{ animationDelay: `${Math.min(k * 0.025, 0.32)}s` }}>{load ? "🎁" : "📦"}<b>{String(b)}</b></span>)}
      </div>
      {overflow ? <div className="zavod-alert">⚠️ {tr({ uz: "CHEKSIZ! — qadam shartga yaqinlashmayapti, mashina to'xtamayapti", ru: "БЕСКОНЕЧНО! — шаг не приближает к условию, машина не останавливается" })}</div> : done && <div className="zavod-ding">🔔 {tr({ uz: "Tayyor! — shart buzildi, sikl to'xtadi", ru: "Готово! — условие нарушилось, цикл остановился" })}</div>}
    </div>;
};
        ...
        <Split>
          <Col>
            ...
            <div className="msg-list fade-up delay-1">
              {count === 0 ? <p style={{ color: T.ink3, fontStyle: "italic", margin: 0, fontFamily: "'JetBrains Mono',monospace", fontSize: 13 }}>{tr({ uz: "// hali bittasi ham yuborilmadi", ru: "// пока не отправлено ни одного" })}</p> : Array.from({ length: count }).map((_, i) => <div key={i} className="msg-line el-in"><span className="msg-ok">✅</span><span>{tr({ uz: `Do'st #${i + 1} — "Bayram muborak!"`, ru: `Друг #${i + 1} — «С праздником!»` })}</span></div>)}
            </div>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Sikl nima", ru: "Что такое цикл" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Ikkala usulni ko'ring", ru: "Посмотрите оба способа" })} onClick={onNext} /></>}>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "for sikli", ru: "Цикл for" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Avval ishga tushiring", ru: "Сначала запустите" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            ...
            {part ? <div className="sk-info fade-step" key={part}>
              ...
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>👆 {tr({ uz: "Koddagi 3 ta rangli qismni bosing", ru: "Нажмите на 3 цветные части кода" })}</p></div>}
          </Col>
          <Col>
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(/* @__PURE__ */ new Set(["5-1"]));
  const done = seen.size >= 2;
  ...
  return <Stage eyebrow={tr({ uz: "Siklni boshqarish", ru: "Управление циклом" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Parametrni o'zgartiring", ru: "Измените параметр" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "while sikli", ru: "Цикл while" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Idishni to'ldiring", ru: "Наполните сосуд" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
    ...
    /* F-0803-17: avval QO'LDA bir qadam (o'quvchi shartni o'zi tekshiradi), keyin to'liq animatsiya */
            ...
            <div className="fade-up delay-2" style={{ display: "flex", gap: 9, flexWrap: "wrap", alignItems: "center" }}>
              {done ? <button className="btn" onClick={again}>{tr({ uz: "↻ Boshidan", ru: "↻ Сначала" })}</button> : <>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="for ⚔️ while" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: `${seen.size}/2 ko'ring`, ru: `Просмотрите ${seen.size}/2` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Massiv", ru: "Массив" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Elementlarni bosing", ru: "Нажимайте на элементы" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <Col>
            {active !== null ? <div className="sk-info fade-step" key={active}>
              ...
              </div> : !isNarrow ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Bir qutini bosing — indeksini ko'ring", ru: "Нажмите на коробку — увидите её индекс" })}</p></div> : null}
...
var Screen9 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><div className="codebox" style={{ marginTop: 10, marginBottom: 6 }}><div><KW>const</KW> mevalar = [<STR>"olma"</STR>, <STR>"banan"</STR>, <STR>"uzum"</STR>]</div></div><h2 className="title h-ask" style={{ marginTop: 6 }}>{tr({ uz: <><span className="mono" style={{ color: T.accent }}>mevalar[0]</span> nimaga teng?</>, ru: <>Чему равно <span className="mono" style={{ color: T.accent }}>mevalar[0]</span>?</> })}</h2></>}
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Aylanib chiqish", ru: "Обход массива" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Avval ishga tushiring", ru: "Сначала запустите" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Hayotiy misol", ru: "Пример из жизни" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Hammaga yuboring", ru: "Отправьте всем" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Amaliyot · o'z siklingiz", ru: "Практика · ваш цикл" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Davom etish", ru: "Продолжить" }) : tr({ uz: "Siklni ishga tushiring", ru: "Запустите цикл" })} onClick={onNext} /></>}>
    ...
    setN(v);
    setRan(false);
  }}>{tr({ uz: `${v} marta`, ru: `${v} ${v === 3 ? "раза" : "раз"}` })}</button>)}
            </div>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Debugging", ru: "Дебаггинг" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: "Yakuniy sinov →", ru: "Финальное испытание →" }) : found ? tr({ uz: "Endi tuzating", ru: "Теперь исправьте" }) : tr({ uz: "Xatoni toping", ru: "Найдите ошибку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr({ uz: "Yakunlash", ru: "Завершить" })} ✓</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>takrorlashni</span> kompyuterga topshirasiz.</>, ru: <>Теперь <span className="italic" style={{ color: T.accent }}>повторение</span> вы поручаете компьютеру.</> })}</h2>{
