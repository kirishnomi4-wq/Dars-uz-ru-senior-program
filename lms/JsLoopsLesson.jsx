...
//  Qayta yig'ish:  node scripts/build-lms.mjs src/2-Modull/JsLoopsLesson.jsx
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {reveal ? <div className="mstats-big">
          ...
          <div className="mstats-chip badc"><span className="mstats-chip-n">{bad}</span><span className="mstats-chip-t">{tr3({ uz: "xato ❌", ru: "неверно ❌" })}</span></div>
...
var SiklZavodi = ({ count = 0, max = 5, init = "i = 1", cond = "i <= 5", step = "i++", load: load2 = null, cheksiz = false, whileMode = false, done = false, onStep, onAuto, manual = false, compact = false }) => {
  const n = Math.max(0, count);
  const zeroBased = /=\s*0/.test(init);
  const boxes = Array.from({ length: n }, (_, k) => load2 ? load2[k] ?? "?" : zeroBased ? k : k + 1);
  ...
  return <div className={`zavod ${overflow ? "zavod-cheksiz" : ""} ${done && !cheksiz ? "zavod-done" : ""} ${compact ? "zavod-sm" : ""} ${idle ? "zv-idle" : ""}`}>
      <div className="zavod-levers">
        <div className="zv-lever zv-init"><span className="zv-lbl">{tr3({ uz: "Boshlanish", ru: "Старт" })}</span><span className="zv-code">{init}</span></div>
        <div className="zv-lever zv-cond"><span className="zv-lbl">{whileMode ? tr3({ uz: "👀 Tekshiruvchi", ru: "👀 Проверяющий" }) : tr3({ uz: "Shart", ru: "Условие" })}</span><span className="zv-code">{cond}</span></div>
        <div className="zv-lever zv-step"><span className="zv-lbl">{tr3({ uz: "Qadam", ru: "Шаг" })}</span><span className="zv-code">{step}</span></div>
      </div>
      <div className="zavod-body">
        ...
        {manual && <div className="zavod-controls">
            {onStep && <button className="btn-soft" onClick={onStep} disabled={done || overflow}>🔧 {tr3({ uz: "Bir marta aylantirish", ru: "Прокрутить один раз" })}</button>}
            {onAuto && <button className="zv-start" onClick={onAuto} disabled={done || overflow}>⚡ {tr3({ uz: "AUTO — hammasini", ru: "AUTO — всё сразу" })}</button>}
          </div>}
      </div>
      <div className="zavod-bin">
        {boxes.length === 0 ? <span className="zv-bin-empty">{tr3({ uz: "// yashik hali bo'sh", ru: "// ящик пока пуст" })}</span> : boxes.map((b, k) => <span key={k} className="zv-box" style={{ animationDelay: `${Math.min(k * 0.025, 0.32)}s` }}>{load2 ? "🎁" : "📦"}<b>{String(b)}</b></span>)}
      </div>
      {overflow ? <div className="zavod-alert">⚠️ {tr3({ uz: "CHEKSIZ! — mashina to'xtamayapti", ru: "БЕСКОНЕЧНО! — машина не останавливается" })}</div> : done && <div className="zavod-ding">🔔 {tr3({ uz: "Tayyor! — shart buzildi, sikl to'xtadi", ru: "Готово! — условие нарушилось, цикл остановился" })}</div>}
    </div>;
};
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Kirish", ru: "Введение" })} screen={screen} navContent={<NavNext optionalLive disabled={picked === null} label={tr3({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
        ...
        <Split three>
          <Col>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
              <button className={`btn ${count >= 20 ? "btn-tired" : ""}`} onClick={write} disabled={count >= NEED} style={{ alignSelf: "flex-start" }}>{count >= NEED ? tr3({ uz: "😮‍💨 Charchadim…", ru: "😮‍💨 Я устал…" }) : tr3({ uz: "✍️ Yana bittasini yozish", ru: "✍️ Написать ещё одно" })}</button>
            ...
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span className="flow-label" style={{ margin: 0 }}>{tr3({ uz: "Charchoq darajasi", ru: "Уровень усталости" })} <span className="face-pop" key={count === 0 ? 0 : count < 12 ? 1 : count < 21 ? 2 : count < 30 ? 3 : 4} style={{ fontSize: 15 }}>{count < 12 ? "🙂" : count < 21 ? "😐" : count < 30 ? "😓" : "😮‍💨"}</span></span>
    ...
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null} onClick={() => pick(o.id)}>
            ...
            {picked !== null && <p className="hook-ack fade-step">{tr3({ uz: <>To'g'ri yo'l — <b>sikl</b>! Bir marta yozasiz, kompyuter uni 1000 marta takrorlaydi. Bugun shuni o'rganamiz.</>, ru: <>Верный путь — <b>цикл</b>! Вы пишете один раз, а компьютер повторит 1000 раз. Этому сегодня и научимся.</> })}</p>}
          </Col>
        </Split>
        </Zoomable>
      </div>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const PreviewBlock = <Col>
      <p className="flow-label">{tr3({ uz: "Bugungi 2 katta vosita", ru: "Два больших инструмента дня" })}</p>
      <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div className="frame" style={{ display: "flex", alignItems: "center", gap: 14, padding: "16px 18px" }}>
          ...
          <div><p style={{ fontFamily: "'Source Serif 4',serif", fontWeight: 600, color: T.ink, margin: 0, fontSize: "clamp(16px,2.2vw,19px)" }}>{tr3({ uz: "SIKL", ru: "ЦИКЛ" })}</p><p className="body" style={{ margin: "2px 0 0", color: T.ink2 }}>{tr3({ uz: "Bir amalni ko'p marta takrorlaydi (for, while)", ru: "Повторяет действие много раз (for, while)" })}</p></div>
        </div>
        <div className="frame" style={{ display: "flex", alignItems: "center", gap: 14, padding: "16px 18px" }}>
          <span className="ic-float" style={{ fontSize: 32 }}>📚</span>
          <div style={{ flex: 1 }}><p style={{ fontFamily: "'Source Serif 4',serif", fontWeight: 600, color: T.ink, margin: 0, fontSize: "clamp(16px,2.2vw,19px)" }}>{tr3({ uz: "MASSIV", ru: "МАССИВ" })}</p><p className="body" style={{ margin: "2px 0 0", color: T.ink2 }}>{tr3({ uz: "Bitta o'zgaruvchida qiymatlar ro'yxati", ru: "Список значений в одной переменной" })}</p>
            <div className="mini-arr">{[0, 1, 2].map((i) => <span key={i} className="mini-cell" style={{ animationDelay: `${i * 0.45}s` }}>{i}</span>)}</div>
          </div>
        </div>
      </div>
      <p className="mono small" style={{ color: T.accent, margin: 0 }}>→ {tr3({ uz: "ikkalasini birga ishlatsak — haqiqiy kuch!", ru: "вместе они — настоящая сила!" })}</p>
    </Col>;
  ...
  return <Stage eyebrow={tr3({ uz: "Reja", ru: "План" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr3({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        ...
        {!isNarrow ? <Zoomable><Split>{PreviewBlock}{StepsBlock}</Split></Zoomable> : !showSteps ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>
            {PreviewBlock}
            <button className="btn" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(true)}>📋 {tr3({ uz: "Bugungi 5 qadamni ko'rish", ru: "Посмотреть 5 шагов урока" })}</button>
          </div> : <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>
            <button className="btn-soft" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(false)}>↩ {tr3({ uz: "Vositalarni ko'rish", ru: "Посмотреть инструменты" })}</button>
            {StepsBlock}
          </div>}
      </div>
    </Stage>;
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Sikl nima", ru: "Что такое цикл" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Ikkala usulni ko'ring", ru: "Посмотрите оба способа" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Bitta ishni <span className="italic" style={{ color: T.accent }}>5 marta</span> — qanday yozamiz?</>, ru: <>Одно действие <span className="italic" style={{ color: T.accent }}>5 раз</span> — как это написать?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>"Salom" so'zini 5 marta chop etmoqchimiz. <b style={{ color: T.ink }}>Siklsiz</b> — har birini alohida yozasiz (5 qator). <b style={{ color: T.ink }}>Sikl bilan</b> — bir marta yozib, "5 marta takrorla" deysiz. Ikkala tugmani bosib solishtiring.</>, ru: <>Хотим напечатать слово «Salom» 5 раз. <b style={{ color: T.ink }}>Без цикла</b> — пишете каждое отдельно (5 строк). <b style={{ color: T.ink }}>С циклом</b> — пишете один раз и говорите «повтори 5 раз». Нажмите обе кнопки и сравните.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8 }}>
              <button className={`chip ${mode === "manual" ? "chip-on" : ""}`} onClick={() => set("manual")}>😕 {tr3({ uz: "Siklsiz", ru: "Без цикла" })}</button>
              <button className={`chip ${mode === "loop" ? "chip-on" : ""}`} onClick={() => set("loop")}>🔁 {tr3({ uz: "Sikl bilan", ru: "С циклом" })}</button>
            </div>
            <div className="codebox demo-swap" key={mode}>
              {mode === "manual" ? <>
                  <div><FN>console</FN>.<FN>log</FN>(<STR>{tr3({ uz: '"Salom"', ru: '"Привет"' })}</STR>)</div>
                  <div><FN>console</FN>.<FN>log</FN>(<STR>{tr3({ uz: '"Salom"', ru: '"Привет"' })}</STR>)</div>
                  <div><FN>console</FN>.<FN>log</FN>(<STR>{tr3({ uz: '"Salom"', ru: '"Привет"' })}</STR>)</div>
                  <div><FN>console</FN>.<FN>log</FN>(<STR>{tr3({ uz: '"Salom"', ru: '"Привет"' })}</STR>)</div>
                  <div><FN>console</FN>.<FN>log</FN>(<STR>{tr3({ uz: '"Salom"', ru: '"Привет"' })}</STR>)</div>
                  <div><CM>{tr3({ uz: "// 5 qator… 100 marta bo'lsa-chi?", ru: "// 5 строк… а если надо 100 раз?" })}</CM></div>
                </> : <>
                  <div><KW>for</KW> (<KW>let</KW> i = <NUM>1</NUM>; i &lt;= <NUM>5</NUM>; i++) {"{"}</div>
                  <div style={{ paddingLeft: 18 }}><FN>console</FN>.<FN>log</FN>(<STR>{tr3({ uz: '"Salom"', ru: '"Привет"' })}</STR>)</div>
                  <div>{"}"}</div>
                  <div><CM>{tr3({ uz: "// 1 qator → 5 marta. 100 marta ham shu!", ru: "// 1 строка → 5 раз. И для 100 — та же!" })}</CM></div>
                </>}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr3({ uz: "Natija (ikkalasida bir xil)", ru: "Результат (одинаковый в обоих)" })}</p>
            <Terminal lines={Array.from({ length: 5 }, () => tr3({ uz: "Salom", ru: "Привет" }))} />
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>✓ Natija aynan bir xil! Lekin <b>sikl</b> bilan kod qisqa, o'zgartirishi oson. Mana shuning uchun sikl kerak.</>, ru: <>✓ Результат абсолютно одинаковый! Но с <b>циклом</b> код короче и его легко менять. Вот зачем нужен цикл.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const N = 5;
  const [out, setOut] = useState4(storedAnswer ? Array.from({ length: N }, (_, i) => i + 1) : []);
  const [iVal, setIVal] = useState4(storedAnswer ? N : 0);
        ...
        <div className="split">
          <Col>
            <div className="codebox fade-up delay-1" style={{ fontSize: "clamp(13px,1.8vw,15px)" }}>
              ...
              <div style={{ paddingLeft: 18 }}><FN>console</FN>.<FN>log</FN>(<STR>{tr3({ uz: '"Salom"', ru: '"Привет"' })}</STR>, i)</div>
              <div>{"}"}</div>
            </div>
            {part ? <div className="sk-info fade-step" key={part}>
                <span className="sk-tagbig"><span className="lg-dot" style={{ background: PARTS[part].color, width: 14, height: 14 }} /><span className="sk-wordbadge" style={{ color: PARTS[part].color, background: PARTS[part].color + "22" }}>{PARTS[part].num}. {tr3(PARTS[part].name)}</span><span className="mono" style={{ color: T.ink2 }}>{PARTS[part].code}</span></span>
                <p className="body" style={{ color: T.ink, margin: "10px 0 0" }}>⚙️ <b>{tr3(PARTS[part].stair)}.</b> {tr3(PARTS[part].desc)}</p>
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>👆 {tr3({ uz: "Koddagi 3 ta rangli qismni bosing", ru: "Нажмите на 3 цветные части кода" })}</p></div>}
          </Col>
          <Col>
            <div className="fade-up delay-1"><SiklZavodi count={iVal} max={N} init="i = 1" cond="i <= 5" step="i++" done={done} /></div>
            <Terminal lines={out.map((v) => `${tr3({ uz: "Salom", ru: "Привет" })} ${v}`)} empty={{ uz: "// ▶ ishga tushiring", ru: "// ▶ запустите" }} />
            <button className="btn" onClick={run} disabled={running} style={{ alignSelf: "flex-start" }}>{running ? tr3({ uz: "Bajarilmoqda…", ru: "Выполняется…" }) : done ? tr3({ uz: "↻ Yana ishga tushir", ru: "↻ Запустить ещё раз" }) : tr3({ uz: "▶ Ishga tushir", ru: "▶ Запустить" })}</button>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>✓ i 1→2→3→4→5 bo'ldi, har safar bir marta ishladi. i = 6 bo'lganda shart (<span className="mono">i &lt;= 5</span>) buzildi — sikl to'xtadi.</>, ru: <>✓ i прошёл 1→2→3→4→5, каждый раз срабатывая по одному. При i = 6 условие (<span className="mono">i &lt;= 5</span>) нарушилось — цикл остановился.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen4 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr3({ uz: <>for siklidagi <span className="mono" style={{ color: T.accent }}>i++</span> nima qiladi?</>, ru: <>Что делает <span className="mono" style={{ color: T.accent }}>i++</span> в цикле for?</> })}</h2></>}
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [end, setEnd] = useState4(5);
  const [step, setStep] = useState4(1);
  const [seen, setSeen] = useState4(/* @__PURE__ */ new Set(["5-1"]));
  const done = seen.size >= 2;
  ...
  return <Stage eyebrow={tr3({ uz: "Siklni boshqarish", ru: "Управление циклом" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Parametrni o'zgartiring", ru: "Измените параметр" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Siklni <span className="italic" style={{ color: T.accent }}>o'zingiz</span> boshqarib ko'ring</>, ru: <>Попробуйте управлять циклом <span className="italic" style={{ color: T.accent }}>сами</span></> })}</h2></div>
        <Mentor>{tr3({ uz: <>3 qismni o'zgartirsangiz — sikl boshqacha ishlaydi. <b style={{ color: T.ink }}>Shart</b>ni o'zgartiring (qachongacha) yoki <b style={{ color: T.ink }}>qadam</b>ni (qancha sakraydi). Pastdagi natija darhol o'zgaradi.</>, ru: <>Поменяете эти части — цикл заработает иначе. Измените <b style={{ color: T.ink }}>условие</b> (до каких пор) или <b style={{ color: T.ink }}>шаг</b> (на сколько прыгать). Результат внизу изменится сразу.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <p className="flow-label">{tr3({ uz: "Shart — i qachongacha?", ru: "Условие — до какого i?" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8 }}>
              {[5, 8, 12].map((e) => <button key={e} className={`chip ${end === e ? "chip-on" : ""}`} onClick={() => setE(e)}>i &lt;= {e}</button>)}
            </div>
            <p className="flow-label" style={{ marginTop: 4 }}>{tr3({ uz: "Qadam — qancha sakraydi?", ru: "Шаг — на сколько прыгает?" })}</p>
            <div className="fade-up delay-2" style={{ display: "flex", gap: 8 }}>
              <button className={`chip ${step === 1 ? "chip-on" : ""}`} onClick={() => setS(1)}>i++ {tr3({ uz: "(bir-bir)", ru: "(по одному)" })}</button>
              <button className={`chip ${step === 2 ? "chip-on" : ""}`} onClick={() => setS(2)}>i += 2 {tr3({ uz: "(ikki-ikki)", ru: "(по два)" })}</button>
            </div>
...
var Screen5b = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "Siklni o'qing", ru: "Прочитайте цикл" })}</p><h2 className="title h-ask" style={{ margin: "8px 0 2px" }}>{tr3({ uz: <>Bu sikl konsolga <span className="italic" style={{ color: T.accent }}>qaysi sonlarni</span> yozadi?</>, ru: <><span className="italic" style={{ color: T.accent }}>Какие числа</span> этот цикл выведет в консоль?</> })}</h2><div className="codebox" style={{ marginTop: 10, marginBottom: 4 }}><div><KW>for</KW> (<KW>let</KW> i = <NUM>1</NUM>; i &lt;= <NUM>3</NUM>; i++) {"{"}</div><div style={{ paddingLeft: 18 }}><FN>console</FN>.<FN>log</FN>(i)</div><div>{"}"}</div></div></>}
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const STEP = 20;
  const [suv, setSuv] = useState4(storedAnswer ? 100 : 0);
  const [iter, setIter] = useState4(storedAnswer ? 5 : 0);
  const [running, setRunning] = useState4(false);
  const timer = useRef4(null);
  const done = suv >= 100;
  useEffect5(() => () => clearTimeout(timer.current), []);
  ...
  return <Stage eyebrow={tr3({ uz: "while sikli", ru: "Цикл while" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Idishni to'ldiring", ru: "Наполните сосуд" })} onClick={onNext} /></>}>
        ...
        <div className="split">
          <Col>
            <div className="codebox fade-up delay-1">
              ...
              <div style={{ paddingLeft: 18 }}>suv += <NUM>20</NUM> <CM>{tr3({ uz: "// yana quyamiz", ru: "// подливаем ещё" })}</CM></div>
              <div>{"}"}</div>
            </div>
            {
    /* F-0803-17: avval QO'LDA bir qadam (o'quvchi shartni o'zi tekshiradi), keyin to'liq animatsiya */
            ...
            <div className="fade-up delay-2" style={{ display: "flex", gap: 9, flexWrap: "wrap", alignItems: "center" }}>
              {done ? <button className="btn" onClick={again}>{tr3({ uz: "↻ Boshidan", ru: "↻ Сначала" })}</button> : <>
                    <button className="btn" onClick={step} disabled={running}>👣 {tr3({ uz: "Bir marta quy", ru: "Налить один раз" })}</button>
                    <button className="btn-ghost" onClick={run} disabled={running} style={{ padding: "11px 18px", fontSize: 14 }}>{running ? tr3({ uz: "Quyilmoqda…", ru: "Наливается…" }) : tr3({ uz: "▶ To'liq quyib ko'rsat", ru: "▶ Показать целиком" })}</button>
                  </>}
            </div>
          ...
          <Col>
            <div className="glass-wrap fade-up delay-1">
              ...
              <p className="cond-label">👀 {tr3({ uz: "Tekshiruvchi", ru: "Проверяющий" })}</p>
              <div className={`cond-pill${done ? " stop" : ""}`} style={{ background: suv < 100 ? T.successSoft : T.accentSoft, color: suv < 100 ? T.success : T.accent }}>suv {suv} &lt; 100 → {suv < 100 ? tr3({ uz: "✓ rost — yana quy", ru: "✓ истина — лей ещё" }) : tr3({ uz: "✗ yolg'on — to'xta", ru: "✗ ложь — стоп" })}</div>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="for ⚔️ while" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: `${seen.size}/2 ko'ring`, ru: `Просмотрите ${seen.size}/2` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Qaysi birini <span className="italic" style={{ color: T.accent }}>qachon</span> ishlatamiz?</>, ru: <>Какой из них <span className="italic" style={{ color: T.accent }}>когда</span> использовать?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Ikkalasi ham takrorlaydi. Farq <b style={{ color: T.ink }}>bitta savolda</b>: necha marta takrorlashni <b style={{ color: T.ink }}>oldindan bilamizmi?</b> Bilsak — <b style={{ color: T.accent }}>for</b>. Bilmasak, faqat shart bo'lsa — <b style={{ color: T.accent }}>while</b>. Ikkala kartani bosing.</>, ru: <>Оба повторяют. Разница — в <b style={{ color: T.ink }}>одном вопросе</b>: <b style={{ color: T.ink }}>знаем ли заранее</b>, сколько раз повторять? Знаем — <b style={{ color: T.accent }}>for</b>. Не знаем, есть только условие — <b style={{ color: T.accent }}>while</b>. Нажмите обе карточки.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
          ...
          <Col>
            ...
            {done && <div className="frame-soft fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Esda tuting: <b>for</b> = sanab bo'ladigan ishlar, <b>while</b> = "qachongacha?" deb so'raydigan ishlar.</>, ru: <>Запомните: <b>for</b> — задачи, которые можно сосчитать, <b>while</b> — задачи с вопросом «до каких пор?».</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Massiv", ru: "Массив" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Elementlarni bosing", ru: "Нажимайте на элементы" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Ko'p narsani <span className="italic" style={{ color: T.accent }}>bitta joyda</span> qanday saqlaymiz?</>, ru: <>Как хранить много всего <span className="italic" style={{ color: T.accent }}>в одном месте</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>4 ta meva uchun 4 ta alohida o'zgaruvchi (<span className="mono">meva1, meva2…</span>) yasash — noqulay. 100 ta bo'lsa-chi? Yaxshisi — hammasini bitta <b style={{ color: T.ink }}>massivga</b>, raqamlangan qator qutilarga joylaymiz. Eng qizig'i: qutilar <b style={{ color: T.accent }}>1 dan emas, 0 dan</b> sanaladi! Dasturlashda shunday qabul qilingan: birinchi element — 0-o'rinda. Har bir qutini bosib ko'ring.</>, ru: <>Делать 4 отдельные переменные для 4 фруктов (<span className="mono">meva1, meva2…</span>) — неудобно. А если их 100? Лучше сложить всё в один <b style={{ color: T.ink }}>массив</b> — ряд пронумерованных коробок. Самое интересное: коробки считаются <b style={{ color: T.accent }}>не с 1, а с 0</b>! Так принято в программировании: первый элемент — на месте 0. Нажмите на каждую коробку.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <div className="codebox fade-up delay-1">
            ...
            <p className="flow-label">{tr3({ uz: "Qutilar — indeksini bosing", ru: "Коробки — нажмите на индекс" })}</p>
          ...
          <Col>
            {active !== null ? <div className="sk-info fade-step" key={active}>
                <p className="flow-label" style={{ margin: "0 0 8px" }}>{tr3({ uz: "Indeks orqali olamiz", ru: "Получаем по индексу" })}</p>
                ...
                <p className="body" style={{ color: T.ink, margin: "12px 0 0" }}>{active === 0 ? tr3({ uz: <>🎯 <b>Indeks 0</b> — eng birinchi element! Sanash noldan boshlanadi, shuning uchun "olma" — nolinchi.</>, ru: <>🎯 <b>Индекс 0</b> — самый первый элемент! Счёт идёт с нуля, поэтому "olma" — нулевая.</> }) : tr3({ uz: <>Bu <b>{active + 1}-element</b>, lekin indeksi <b>{active}</b> — chunki 0 dan sanadik.</>, ru: <>Это <b>{active + 1}-й элемент</b>, но его индекс <b>{active}</b> — потому что считали с 0.</> })}</p>
              </div> : !isNarrow ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr3({ uz: "Bir qutini bosing — indeksini ko'ring", ru: "Нажмите на коробку — увидите её индекс" })}</p></div> : null}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>✓ Birinchi element <b>[0]</b>, oxirgisi <b>[3]</b>. Bu "0 dan sanash" — dasturlashning eng mashhur "tuzog'i". Endi bilasiz! 😉</>, ru: <>✓ Первый элемент <b>[0]</b>, последний <b>[3]</b>. Этот «счёт с нуля» — самая известная «ловушка» программирования. Теперь вы в курсе! 😉</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen9 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><div className="codebox" style={{ marginTop: 10, marginBottom: 6 }}><div><KW>const</KW> mevalar = [<STR>"olma"</STR>, <STR>"banan"</STR>, <STR>"uzum"</STR>]</div></div><h2 className="title h-ask" style={{ marginTop: 6 }}>{tr3({ uz: <><span className="mono" style={{ color: T.accent }}>mevalar[0]</span> nimaga teng?</>, ru: <>Чему равно <span className="mono" style={{ color: T.accent }}>mevalar[0]</span>?</> })}</h2></>}
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const ARR = [{ e: "🍎", n: "olma" }, { e: "🍌", n: "banan" }, { e: "🍇", n: "uzum" }, { e: "🍓", n: "qulupnay" }];
  const N = ARR.length;
  const [hi, setHi] = useState4(-1);
  const [out, setOut] = useState4(storedAnswer ? ARR.map((a) => a.n) : []);
  const [running, setRunning] = useState4(false);
  const timer = useRef4(null);
  const done = out.length >= N;
  useEffect5(() => () => clearTimeout(timer.current), []);
      ...
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            ...
            <button className="btn" onClick={run} disabled={running} style={{ alignSelf: "flex-start" }}>{running ? tr3({ uz: "Aylanyapti…", ru: "Обходит…" }) : done ? tr3({ uz: "↻ Yana", ru: "↻ Ещё раз" }) : tr3({ uz: "▶ Ishga tushir", ru: "▶ Запустить" })}</button>
          </Col>
          <Col>
            <p className="flow-label">{tr3({ uz: "Natija (har bir element)", ru: "Результат (каждый элемент)" })}</p>
            <Terminal lines={out} empty={{ uz: "// ▶ ishga tushiring", ru: "// ▶ запустите" }} />
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>✓ Sikl massivning 4 ta elementini birma-bir chop etdi. <span className="mono">i &lt; .length</span> tufayli oxiriga yetganda o'zi to'xtadi — qancha element bo'lsa ham ishlaydi!</>, ru: <>✓ Цикл напечатал 4 элемента массива по одному. Благодаря <span className="mono">i &lt; .length</span> он сам остановился в конце — и так с любым числом элементов!</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [out, setOut] = useState4(storedAnswer ? NAMES.map((n) => `${tr3({ uz: "Bayram muborak", ru: "С праздником" })}, ${n}`) : []);
  const [running, setRunning] = useState4(false);
  const timer = useRef4(null);
  const done = out.length >= N;
  useEffect5(() => () => clearTimeout(timer.current), []);
  const run = () => {
    ...
    setRunning(true);
    const tick = (i) => {
      setOut((prev) => [...prev, `${tr3({ uz: "Bayram muborak", ru: "С праздником" })}, ${NAMES[i]}`]);
  ...
  return <Stage eyebrow={tr3({ uz: "Hayotiy misol", ru: "Пример из жизни" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Hammaga yuboring", ru: "Отправьте всем" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Esingizdami — <span className="italic" style={{ color: T.accent }}>30 ta xabar?</span> Mana yechim!</>, ru: <>Помните — <span className="italic" style={{ color: T.accent }}>30 сообщений?</span> Вот решение!</> })}</h2></div>
    ...
    return <div key={i} className={`friend-card ${got ? "got" : ""}`}>
                    ...
                    <div><div className="friend-name">{nm}</div><div className="friend-msg">{got ? `"${tr3({ uz: "Bayram muborak", ru: "С праздником" })}, ${nm}"` : tr3({ uz: "navbatini kutyapti…", ru: "ждёт своей очереди…" })}</div></div>
...
var Screen12 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><div className="codebox" style={{ marginTop: 10, marginBottom: 6 }}><div><CM>{tr3({ uz: "// dostlar = 5 ta nom", ru: "// dostlar = 5 имён" })}</CM></div><div><KW>for</KW> (<KW>let</KW> i = <NUM>0</NUM>; i &lt; dostlar.<FN>length</FN>; i++) {"{ … }"}</div></div><h2 className="title h-ask" style={{ marginTop: 6 }}>{tr3({ uz: <>Sikl <span className="italic" style={{ color: T.accent }}>necha marta</span> ishlaydi?</>, ru: <>Цикл сработает <span className="italic" style={{ color: T.accent }}>сколько раз</span>?</> })}</h2></>}
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [msgIdx, setMsgIdx] = useState4(0);
  ...
  return <Stage eyebrow={tr3({ uz: "Amaliyot · o'z siklingiz", ru: "Практика · ваш цикл" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Siklni ishga tushiring", ru: "Запустите цикл" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.4vw,14px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>siz</span> sikl quring</>, ru: <>Теперь цикл строите <span className="italic" style={{ color: T.accent }}>вы</span></> })}</h2></div>
        <Mentor>{tr3({ uz: <>Navbat sizga! <b style={{ color: T.ink }}>Necha marta</b> takrorlashni va <b style={{ color: T.ink }}>qaysi xabarni</b> tanlang, keyin "Ishga tushir"ni bosing. Kod o'zgarishini va natijani kuzating.</>, ru: <>Ваш ход! Выберите, <b style={{ color: T.ink }}>сколько раз</b> повторять и <b style={{ color: T.ink }}>какое сообщение</b>, затем нажмите «Запустить». Следите, как меняется код и результат.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <p className="flow-label">{tr3({ uz: "Necha marta? (shart)", ru: "Сколько раз? (условие)" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8 }}>
              {[3, 5, 7].map((v) => <button key={v} className={`chip ${n === v ? "chip-on" : ""}`} onClick={() => {
    setN(v);
    setRan(false);
  }}>{tr3({ uz: `${v} marta`, ru: `${v} ${v === 3 ? "раза" : "раз"}` })}</button>)}
            </div>
            <p className="flow-label" style={{ marginTop: 4 }}>{tr3({ uz: "Qaysi xabar?", ru: "Какое сообщение?" })}</p>
    ...
    setRan(false);
            ...
            <div className="codebox" style={{ marginTop: 6 }}>
              ...
              <div style={{ paddingLeft: 18 }}><FN>console</FN>.<FN>log</FN>(i + <STR>". {tr3(MSGS[msgIdx])}"</STR>)</div>
              <div>{"}"}</div>
            </div>
            <button className="btn" onClick={run} disabled={running} style={{ alignSelf: "flex-start" }}>{running ? tr3({ uz: "Bajarilmoqda…", ru: "Выполняется…" }) : tr3({ uz: "▶ Ishga tushir", ru: "▶ Запустить" })}</button>
          </Col>
          <Col>
            <p className="flow-label">{tr3({ uz: "Sizning natijangiz", ru: "Ваш результат" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", alignItems: "center", gap: 13, background: T.paper, borderRadius: 12, padding: "12px 18px", boxShadow: `0 8px 20px -6px rgba(${T.shadowBase},0.14)` }}>
              ...
              <div><div className="flow-label" style={{ margin: 0 }}>{tr3({ uz: "marta bajarildi", ru: "раз выполнено" })}</div><div className="mono small" style={{ color: T.ink2 }}>{tr3({ uz: `${n} martadan`, ru: `из ${n}` })}</div></div>
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>✓ Zo'r! Siz haqiqiy sikl qurdingiz va ishga tushirdingiz. Parametrni o'zgartirib, yana sinab ko'ring.</>, ru: <>✓ Класс! Вы собрали настоящий цикл и запустили его. Поменяйте параметры и попробуйте ещё.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [picked, setPicked] = useState4(storedAnswer ? "step" : null);
  ...
  const click = (part) => {
    if (found) return;
    if (part !== "step" && achMiss) achMiss.miss(screen);
    setPicked(part);
  };
  ...
  return <Stage eyebrow={tr3({ uz: "Debugging", ru: "Дебаггинг" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr3({ uz: "Yakuniy sinov →", ru: "Финальное испытание →" }) : found ? tr3({ uz: "Endi tuzating", ru: "Теперь исправьте" }) : tr3({ uz: "Xatoni toping", ru: "Найдите ошибку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Bu sikl <span className="italic" style={{ color: T.accent }}>to'xtamayapti</span> — nega?</>, ru: <>Этот цикл <span className="italic" style={{ color: T.accent }}>не останавливается</span> — почему?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>AI 1 dan 5 gacha sanaydigan sikl yozdi, lekin u <b style={{ color: T.ink }}>cheksiz</b> aylanyapti! Diqqat bilan o'qing: i 5 ga <b style={{ color: T.ink }}>yaqinlashyaptimi</b>? Xato qismni toping va bosing.</>, ru: <>ИИ написал цикл, считающий от 1 до 5, но он крутится <b style={{ color: T.ink }}>бесконечно</b>! Читайте внимательно: i вообще <b style={{ color: T.ink }}>приближается</b> к 5? Найдите ошибочную часть и нажмите на неё.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <div className="ai-card fade-up delay-1">
              <div className="ai-row"><span className="ai-badge">AI</span><span className="ai-bubble">{tr3({ uz: "1 dan 5 gacha sanaymiz:", ru: "Считаем от 1 до 5:" })}</span></div>
              ...
              {!found && <p className="ai-prompt">{tr3({ uz: "Qaysi qism xato? Ustiga bosing.", ru: "Какая часть с ошибкой? Нажмите на неё." })}</p>}
              {!done && <AchRule screen={screen} />}
              {found && !fixed && <button className="btn fade-step" style={{ alignSelf: "flex-start" }} onClick={fix}>🔧 {tr3({ uz: "i-- ni i++ ga almashtirish", ru: "Заменить i-- на i++" })}</button>}
              {fixed && <p className="ai-prompt" style={{ color: T.success, fontStyle: "normal", fontWeight: 600 }}>✓ {tr3({ uz: "Tuzatildi — endi i oshadi va sikl 5 da to'xtaydi!", ru: "Исправлено — теперь i растёт, и цикл остановится на 5!" })}</p>}
            </div>
            {!fixed ? <div className="term fade-up delay-2">
                ...
                <div className="term-body" style={{ padding: "8px 14px", gap: 3 }}>{[1, 0, -1, -2].map((v, k) => <div key={k} className="term-line"><span className="term-arrow" style={{ color: T.accent }}>›</span><span>{v}</span></div>)}<div className="term-line warn-pulse" style={{ color: T.accent }}><span className="term-arrow" style={{ color: T.accent }}>›</span><span>⋮</span></div><p className="term-empty warn-pulse" style={{ color: T.accent }}>⚠️ {tr3({ uz: "5 ga hech yetmaydi — cheksiz!", ru: "До 5 не дойдёт никогда — бесконечно!" })}</p></div>
          ...
          <Col>
            {!found && (picked && picked !== "step" ? <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Bu qism to'g'ri. {picked === "init" ? "Boshlanish (i = 1) — joyida." : "Shart (i <= 5) — joyida."} Xato boshqa qismda — i qaysi tomonga o'zgaryapti?</>, ru: <>Эта часть верна. {picked === "init" ? "Старт (i = 1) — на месте." : "Условие (i <= 5) — на месте."} Ошибка в другой части — в какую сторону меняется i?</> })}</p></div> : picked === null ? null : <div className="hint"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr3({ uz: <>Eslang: sikl to'xtashi uchun i <b style={{ color: T.ink }}>shartga yaqinlashishi</b> kerak. Bu yerda i 5 ga tomon ketyaptimi yoki undan <b style={{ color: T.ink }}>uzoqlashyaptimi?</b></>, ru: <>Помните: чтобы цикл остановился, i должен <b style={{ color: T.ink }}>приближаться к условию</b>. Здесь i идёт к 5 или <b style={{ color: T.ink }}>удаляется от неё?</b></> })}</p></div>)}
            {found && !fixed && <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.accent }}>✓ {tr3({ uz: "Topdingiz!", ru: "Нашли!" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <><span className="mono">i--</span> i ni <b>kamaytiradi</b> (1, 0, -1, …) — 5 ga hech qachon yetmaydi. To'g'risi: <span className="mono">i++</span>. Chap tugmani bosing →</>, ru: <><span className="mono">i--</span> <b>уменьшает</b> i (1, 0, -1, …) — до 5 не дойдёт никогда. Правильно: <span className="mono">i++</span>. Нажмите кнопку слева →</> })}</p></div>}
            {fixed && <div className="takeaway fade-step"><div className="ta-bulb">🛠️</div><p className="ta-h">{tr3({ uz: "Topdingiz va tuzatdingiz — bu debugging!", ru: "Нашли и исправили — это дебаггинг!" })}</p><p className="ta-sub">{tr3({ uz: "Cheksiz sikl — qadam shartga yaqinlashmaganda yuz beradi", ru: "Бесконечный цикл случается, когда шаг не приближает к условию" })}</p></div>}
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Yakunlash", ru: "Завершить" })} ✓</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr3({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr3({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>takrorlashni</span> kompyuterga topshirasiz.</>, ru: <>Теперь <span className="italic" style={{ color: T.accent }}>повторение</span> вы поручаете компьютеру.</> })}</h2>{
  ...
  />
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>📝 {tr3({ uz: "Uyga vazifa", ru: "Домашнее задание" })}</div><p className="body" style={{ margin: "0 0 10px", color: T.ink }}>{tr3({ uz: "Sikllar bilan mashq qiling:", ru: "Потренируйтесь с циклами:" })}</p><ul>{HOMEWORK.map((h, i) => <li key={i}><b>{tr3(h.b)}</b> <span className="t">{tr3(h.t)}</span></li>)}</ul><p className="hw-note">{tr3({ uz: "Sikllar — eng ko'p ishlatiladigan vosita. Mashq qilsangiz, qo'lingizga o'tirib qoladi! 🚀", ru: "Циклы — самый используемый инструмент. Потренируетесь — и рука набьётся! 🚀" })}</p></div>}
...
function JsLoopsLesson({ lang: langProp, onFinished, onPractice, liveToken }) {
