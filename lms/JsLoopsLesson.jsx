...
var SiklZavodi = ({ count = 0, max = 5, init = "i = 1", cond = "i <= 5", step = "i++", load: load2 = null, cheksiz = false, whileMode = false, done = false, onStep, onAuto, manual = false, compact = false }) => {
  ...
  return <div className={`zavod ${overflow ? "zavod-cheksiz" : ""} ${done && !cheksiz ? "zavod-done" : ""} ${compact ? "zavod-sm" : ""} ${idle ? "zv-idle" : ""}`}>
      ...
      {overflow ? <div className="zavod-alert">⚠️ {tr3({ uz: "CHEKSIZ! — qadam shartga yaqinlashmayapti, mashina to'xtamayapti", ru: "БЕСКОНЕЧНО! — шаг не приближает к условию, машина не останавливается" })}</div> : done && <div className="zavod-ding">🔔 {tr3({ uz: "Tayyor! — shart buzildi, sikl to'xtadi", ru: "Готово! — условие нарушилось, цикл остановился" })}</div>}
    </div>;
};
