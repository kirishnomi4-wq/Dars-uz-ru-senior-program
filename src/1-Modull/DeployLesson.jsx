...
const Screen2 = (props) => (
  <QuestionScreen {...props} scope="module-mikro" eyebrow={tr({ uz: 'Tekshiruv · 1-savol', ru: 'Проверка · вопрос 1' })}
    ...
    question={<><p className="eyebrow" style={{ color: T.accent }}>{tr({ uz: "To'g'ri javobni tanlang", ru: 'Выберите верный ответ' })}</p><h2 className="title h-sub" style={{ marginTop: 8 }}>{tr({ uz: 'Netlify nima qiladi?', ru: 'Что делает Netlify?' })}</h2></>}
    options={[
...
const Screen6 = (props) => (
  <QuestionScreen {...props} scope="module-mikro" eyebrow={tr({ uz: 'Tekshiruv · 2-savol', ru: 'Проверка · вопрос 2' })}
    ...
    question={<><p className="eyebrow" style={{ color: T.accent }}>{tr({ uz: "To'g'ri javobni tanlang", ru: 'Выберите верный ответ' })}</p><h2 className="title h-sub" style={{ marginTop: 8 }}>{tr({ uz: 'Sayt papkasi ochilganda brauzer birinchi qaysi faylni qidiradi?', ru: 'Какой файл браузер ищет первым, когда открывает папку сайта?' })}</h2></>}
    options={[
...
const Screen10 = (props) => (
  <QuestionScreen {...props} scope="module-mikro" eyebrow={tr({ uz: 'Tekshiruv · 3-savol', ru: 'Проверка · вопрос 3' })}
    ...
    question={<><p className="eyebrow" style={{ color: T.accent }}>{tr({ uz: "To'g'ri tartibni tanlang", ru: 'Выберите верный порядок' })}</p><h2 className="title h-sub" style={{ marginTop: 8 }}>{tr({ uz: 'Sayt Netlify orqali internetga qanday tartibda chiqadi?', ru: 'В каком порядке сайт выходит в интернет через Netlify?' })}</h2></>}
    options={[
...
const Screen12 = (props) => (
  <QuestionScreen {...props} scope="final" eyebrow={tr({ uz: 'Yakuniy savol', ru: 'Финальный вопрос' })}
    ...
    question={<><p className="eyebrow" style={{ color: T.accent }}>{tr({ uz: 'Yakuniy savol', ru: 'Финальный вопрос' })}</p><h2 className="title h-sub" style={{ marginTop: 8 }}>{tr({ uz: "Do'stingiz saytingizni telefonida ochishi uchun unga nima yuborasiz?", ru: 'Что вы отправите другу, чтобы он открыл ваш сайт на телефоне?' })}</h2></>}
    options={[
...
const Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tayyor', ru: 'Готово' })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: 'clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Qaytadan', ru: 'Заново' })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: 'auto', padding: 'clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Yakunlash ✓', ru: 'Завершить ✓' })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: 'Dars tugadi', ru: 'Урок завершён' })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Saytingiz endi <span className="italic" style={{ color: T.accent }}>internetda</span>.</>, ru: <>Ваш сайт теперь <span className="italic" style={{ color: T.accent }}>в интернете</span>.</> })}</h2><p className="body h-sub fade-up d2">{PASSED ? tr({ uz: "Endi istalgan saytni o'zingiz internetga chiqara olasiz.", ru: 'Теперь вы сами можете вывести в интернет любой сайт.' }) : tr({ uz: "Yaxshi harakat! Bir-ikki joyni mustahkamlash uchun darsni qayta ko'ring.", ru: 'Хорошая попытка! Пересмотрите урок, чтобы закрепить пару мест.' })}</p></div><ScoreRing correct={correct} total={total} /></div>
        ...
        <div className="split">
          ...
          {!isMentorL && <div className="card hw fade-up d4">
