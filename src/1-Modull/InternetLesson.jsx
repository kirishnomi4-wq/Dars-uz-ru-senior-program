...
// 🏠 LMS uchun statik uy-vazifa deklaratsiyasi. Bu darsda kod yozilmaydi — vazifa
// kuzatuv-savollarga YOZMA javob (LMS'da «Text» savol-turi). Yakun-kartaga 1:1 mos.
...
const Q_LABELS = { 4: { uz: '1 — Brauzer', ru: '1 — Браузер' }, 6: { uz: '2 — Domen', ru: '2 — Домен' }, 10: { uz: '3 — DNS', ru: '3 — DNS' }, 13: { uz: '4 — Server (HTML)', ru: '4 — Сервер (HTML)' }, 18: { uz: "5 — Yakuniy (so'rov tartibi)", ru: '5 — Итоговый (порядок запроса)' } };
...
const fmtCode = (s) => (typeof s === 'string' && s.includes('`'))
  ...
  : s;
// UZ-RU: analitika payload'i uchun doim UZ-etalon qiymatni ajratadi (til almashsa ham server-yozuv barqaror)
const ouz = (o) => (o && typeof o === 'object' && !React.isValidElement(o)) ? (o.uz ?? o.ru ?? '') : o;
        ...
        <FeedbackBlock show={isMentorLive ? mReveal : picked !== null} isCorrect={isMentorLive ? true : (solved && !wrongLocked)} neutral={waiting}>
          ...
          <p className="body" style={{ margin: 0 }}>
            {fmtCode(tr(isMentorLive
              ? explainCorrect
              : waiting
                ? { uz: "Javobingiz yozib olindi. To'g'ri yoki xato ekani mentor «Natijani ochish»ni bosganda hammada birdan ko'rinadi.", ru: 'Ваш ответ записан. Правильный он или нет — станет видно всем сразу, когда ментор нажмёт «Открыть результат».' }
                : wrongLocked
...
const Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const caption = jstep < 0
    ? { uz: "▶ tugmasini bosing — so'rov bosib o'tadigan butun aylanma yo'lni ko'ramiz.", ru: '▶ Нажмите кнопку — увидим весь круговой путь, который проходит запрос.' }
    : jstep >= DONE
      ? { uz: "✓ Tayyor! YouTube ekraningizda ochildi — har bir sayt va ilova xuddi shu yo'l bilan, atigi 1 soniyada keladi.", ru: '✓ Готово! YouTube открылся на вашем экране — каждый сайт и приложение приходит этим же путём, всего за 1 секунду.' }
      : JR_STEPS[jstep].cap;
...
const Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Internet', ru: 'Интернет' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: "Ulanishlarni ko'rsating", ru: 'Покажите связи' }} onClick={onNext} /></>}>
...
const Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Brauzer', ru: 'Браузер' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: 'Brauzer tanlang', ru: 'Выберите браузер' }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Domen', ru: 'Домен' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: 'Domenni tanlang', ru: 'Выберите домен' }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'IP manzil', ru: 'IP-адрес' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: "IP manzilni ko'ring", ru: 'Посмотрите IP-адрес' }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'DNS', ru: 'DNS' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: 'Domenni qidiring', ru: 'Найдите домен' }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Server', ru: 'Сервер' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: "So'rov yuboring", ru: 'Отправьте запрос' }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Brauzer ichida', ru: 'Внутри браузера' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: 'Kodni sahifaga aylantiring', ru: 'Превратите код в страницу' }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(10px,1.6vw,16px)' }}>
          <div className="split" ref={animRef}>
            ...
            <div className="col" ref={resultRef}>
              ...
              <Preview title="youtube.com" minH={150}>
                {rd === 0
                  ? (<p style={{ fontFamily: 'Georgia, serif', color: T.ink3, fontStyle: 'italic', margin: 0, textAlign: 'center' }}>{tr({ uz: "Tugmani bosing — brauzer kodni o'qib, YouTube sahifasini shu yerda chizadi", ru: 'Нажмите кнопку — браузер прочитает код и нарисует страницу YouTube здесь' })}</p>)
                  : (<div style={{ display: 'block' }}>{HTML.slice(0, rd).map((h, i) => <div key={i} className="htl-pop">{tr(h.el)}</div>)}</div>)}
              </Preview>
            </div>
          </div>
...
const Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: "Amaliyot · so'rov yuboring", ru: 'Практика · отправьте запрос' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: "So'rov yuboring", ru: 'Отправьте запрос' }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen13b = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  round.servers.forEach((s) => { nodes['srv' + s.slot] = { ic: '🖥️', l: { uz: 'Server', ru: 'Сервер' }, ip: s.ip, name: s.name, mock: s.mock, kind: 'server', pos: NET_POS['srv' + s.slot] }; });
  ...
  const go = (to) => {
    ...
    timer.current = setTimeout(() => {
      ...
      if (node.kind === 'dns') { setHasIP(true); setMsg({ t: `${tr({ uz: '🗂️ DNS telefon kitobidan topdi', ru: '🗂️ DNS нашёл в телефонной книге' })}: ${target.domain} → ${target.ip}`, type: 'good' }); }
        ...
        if (node.ip === target.ip) { setHasPage(true); setMatchNode(to); setMsg({ t: tr({ uz: '🤝 IP mos keldi! Aynan shu server — sahifa olindi.', ru: '🤝 IP совпал! Это тот самый сервер — страница получена.' }), type: 'good' }); }
...
const Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Debugging', ru: 'Отладка' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : (stage === 'failed' ? { uz: 'Endi tuzating', ru: 'Теперь исправьте' } : { uz: "Ochib ko'ring", ru: 'Попробуйте открыть' })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        ...
        {!isLive ? (
          ...
          <>
            ...
            <div className="card fade-up d2">
              ...
              {live.mode === 'mentor' && <p className="small" style={{ margin: '10px 0 0', color: T.ink2 }}>{tr({ uz: '⚠️ belgili savollar — sinf qiynalgan mavzular. Qayta tushuntiring.', ru: '⚠️ отмеченные вопросы — темы, с которыми класс справился хуже. Объясните заново.' })}</p>}
            </div>
          </>
        )}
      </div>
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tayyor', ru: 'Готово' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: 'clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Qaytadan', ru: 'Заново' })}</button><button className="btn-white-accent" onClick={() => setShowDone(true)} style={{ marginLeft: 'auto', padding: 'clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: '🎉 Darsni yakunlash', ru: '🎉 Завершить урок' })}</button></>}>
      <div className="screen">
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>{tr({ uz: '🔎 Uyga vazifa', ru: '🔎 Домашнее задание' })}</div><p className="body" style={{ margin: '0 0 10px', color: T.ink }}>{tr({ uz: 'Internetni hayotda kuzating:', ru: 'Понаблюдайте за интернетом в жизни:' })}</p><ul>{HOMEWORK.map((h, i) => (<li key={i}><b>{tr(h.b)}</b> <span className="t">{tr(h.t)}</span></li>))}</ul><p className="hw-note">{tr({ uz: "Javoblaringizni platformadagi vazifa-savoliga yozib yuborasiz.", ru: 'Ответы отправите в задание на платформе.' })}</p></div>}
...
export default function HtmlLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  return (
    <LangContext.Provider value={lang}>
      <style>{`
        ...
        .hz-node { width: 34px; height: 34px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 14px; box-shadow: inset 0 0 0 2px rgba(${T.shadowBase},0.30); color: ${T.ink2}; background: ${T.paper}; transition: all 0.4s cubic-bezier(.34,1.35,.4,1); }
        ...
        .urlbar-text { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: clamp(14px,1.8vw,16px); color: ${T.ink}; flex: 1; min-width: 0; }
        .urlbar-input { flex: 1; min-width: 0; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: clamp(14px,1.8vw,16px); border: none; background: transparent; color: ${T.ink}; outline: none; }
        ...
        .dompill { display: inline-flex; align-self: center; border-radius: 10px; overflow: hidden; box-shadow: 0 6px 16px -6px rgba(${T.shadowBase},0.18); font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: clamp(15px,2.2vw,19px); }
        ...
        .domsplit { display: inline-flex; align-self: center; align-items: center; gap: 7px; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: clamp(15px,2.2vw,19px); }
        ...
        .ipbox { align-self: flex-start; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: clamp(16px,2.4vw,20px); color: ${T.blue}; background: #E2F1F7; padding: 10px 16px; border-radius: 10px; }
        ...
        .dns-arr { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; color: ${T.accent}; }
        ...
        .dns-dom { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 13px; color: ${T.ink}; flex-shrink: 0; }
        .dns-dots { flex: 1; height: 0; border-bottom: 1.5px dotted rgba(${T.shadowBase},0.28); margin: 6px 2px 0; }
        .dns-ip { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: 12.5px; color: ${T.ink3}; letter-spacing: 0.5px; flex-shrink: 0; transition: color 0.3s; }
        ...
        .dc-v { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 14px; color: ${T.ink}; }
        ...
        .jr-packet { position: absolute; left: 50%; top: 12%; white-space: nowrap; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: clamp(10.5px,1.5vw,13px); padding: 6px 11px; border-radius: 99px; opacity: 0; box-shadow: 0 7px 18px -4px rgba(${T.shadowBase},0.4); transition: transform 1.25s cubic-bezier(.45,0,.3,1), opacity 0.4s ease; }
        ...
        .dlv-attempts { margin-left: auto; font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 11px; color: ${T.ink3}; flex-shrink: 0; }
        ...
        .dlv-route-lbl { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 11px; color: ${CODE.comment}; }
        .dlv-route-empty { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 12px; color: ${CODE.comment}; font-style: italic; }
        ...
        .dlv-arr { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; color: ${CODE.attr}; font-size: 13px; }
        ...
        .net-ip { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: clamp(8.5px,2vw,10.5px); color: ${T.ink}; background: ${T.bg}; padding: 1px 6px; border-radius: 6px; box-shadow: inset 0 0 0 1px rgba(${T.shadowBase},0.14); white-space: nowrap; }
        ...
        .net-hud-ip { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; }
        ...
        .pl-pkt { position: absolute; top: 20%; transform: translate(-50%,-50%); white-space: nowrap; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: clamp(10px,1.6vw,12.5px); padding: 5px 11px; border-radius: 99px; color: #fff; box-shadow: 0 8px 18px -4px rgba(${T.shadowBase},0.4); transition: left 1.15s cubic-bezier(.45,0,.3,1), background 0.4s ease; z-index: 6; }
        ...
        .pl-conv-arr { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; color: ${T.accent}; font-size: 18px; }
        .pl-pill { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: clamp(13px,2vw,16px); padding: 8px 14px; border-radius: 10px; }
        ...
        .pl-code { position: relative; background: ${CODE.bg}; color: ${CODE.text}; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: clamp(12px,1.6vw,13.5px); line-height: 1.6; padding: 11px 13px; border-radius: 10px; margin: 0; overflow: hidden; }
        ...
        .tg-count { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: 11.5px; font-weight: 700; color: #fff; background: ${T.accent}; border-radius: 99px; padding: 2px 8px; }
