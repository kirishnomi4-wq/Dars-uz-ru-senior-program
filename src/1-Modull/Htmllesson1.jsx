  ...
  return (
    <Stage eyebrow={tr({ uz: 'Kirish', ru: 'Введение' })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr({ uz: 'Davom etish', ru: 'Продолжить' })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr({ uz: <>Bu sayt nimadan <span className="italic" style={{ color: T.accent }}>yasalgan</span>?</>, ru: <>Из чего <span className="italic" style={{ color: T.accent }}>сделан</span> этот сайт?</> })}</h1>
        ...
        {revealed && (
          <>
            ...
            <div ref={answerRef} className="frame frame-col fade-step" style={{ scrollMarginTop: 12 }}>
              ...
              <div className="rg-grid">{/* §34: keng ekranda sahna+tugmalar chapda, kod o'ngda — bir ekranga sig'adi */}
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Teg', ru: 'Тег' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : (wrapped ? tr({ uz: '🐞 Xatoni toping', ru: '🐞 Найдите ошибку' }) : tr({ uz: "Avval tegga o'rang", ru: 'Сначала оберните в тег' }))} onClick={onNext} /></>}>
      <div className="screen">
        ...
        <div className={`split${wrapped ? ' split3' : ''}`}>{/* §34: xato-topish quti uchinchi ustunda */}
          <div className="col">
...
const Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  // «Tugallangan, lekin noto'g'ri» urinish: o'quvchi tegni yopdi (>) — javob esa mos emas.
  // Shundan keyingina to'liq javob ko'rsatiladi (19.09 naqshi), oldin — faqat shakl-maslahati.
  const [missed, setMissed] = useState(false);
  useEffect(() => { if (!correct && val.includes('>')) setMissed(true); }, [val, correct]);
  useEffect(() => {
...
const Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tayyor', ru: 'Готово' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: 'clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Qaytadan', ru: 'Заново' })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: 'auto', padding: 'clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Keyingi dars →', ru: 'Следующий урок →' })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: 'Dars tugadi', ru: 'Урок завершён' })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Birinchi <span className="italic" style={{ color: T.accent }}>saytingizni</span> yasadingiz.</>, ru: <>Вы сделали свой первый <span className="italic" style={{ color: T.accent }}>сайт</span>.</> })}</h2>{/* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */}</div><ScoreRing correct={correct} total={total} /></div>
...
//   TASK_FINAL bilan adashtirmang: u sinf-praktikasi (mavzu erkin, 1 ta p yetadi);
//   uy vazifasi — O'ZI haqida sahifa, kamida 2 paragraf (shart-karta va'dasi bilan bir xil).
const TASK_HW = {
...
// 🏠 LMS uchun statik uy-vazifa deklaratsiyasi: «Uyga vazifa» tugmasi bosilganda LMS shu
// shartni ko'rsatadi, dars tugagach (onFinished) kompilyatorni task/starter bilan ochadi.
...
export default function HtmlLesson({ lang: langProp, onFinished, onPractice, liveToken }) {
  ...
  // 🏠 UYGA VAZIFA PRAKTIKASI (yakun-sahifadagi tugma) — TASK_HW: o'zi haqida sahifa.
  ...
  // (status==='ended') yoki bola mustaqil o'qiyotgan bo'lsa —
  ...
  const next = () => {
    ...
    // Sinfda, sessiya davom etayotgan holat — mashq majburiy.
    ...
    if (!inLiveClass) { advance(); return; } // erkin qilingan / mustaqil → mashq yo'q
