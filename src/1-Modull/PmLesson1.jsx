...
// 156-qonun 4-band: izoh NOMLAYDI, rasm KO'RSATADI. Garvard — chizib bo'lmaydigan referent
// (real joy), shuning uchun maket emas, FOTO. Rasm yuklanmasa emoji qoladi va dars TO'XTAMAYDI
// (155-qonun 3-bandi bilan bir mantiq). Manba: foydalanuvchining o'z media-kutubxonasi.
const PHOTO_SET = {
  garvard: {
...
const Photo = ({ kind }) => {
  ...
  if (sc.img && !failed) {
    return (
      <span className="k-photo" style={{ background: sc.bg }}>
        <img src={sc.img} alt={tr(sc.alt)} onError={() => setFailed(true)} />
      </span>
    );
  }
  return <span className="k-ic">{sc.emoji}</span>;
};
...
const K_SLIDES = [
  ...
  { ic: '🎓', photo: 'garvard', t: { uz: "Avval sayt faqat Garvard universiteti (Amerikadagi mashhur universitet) talabalari uchun ochiq edi. Boshqalar ro'yxatdan o'ta olmasdi.", ru: 'Сначала сайт был открыт только для студентов Гарварда (известный университет в Америке). Остальные не могли зарегистрироваться.' } },
        ...
        <div className="k-slide fade-up" key={i}>
          ...
          {cur.bet ? (
            ...
            <div className="k-body">
              {cur.photo ? <Photo kind={cur.photo} /> : <span className="k-ic">{cur.ic}</span>}
...
const Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [st, setSt] = useState(() => {
    const saved = ((Array.isArray(storedAnswer?.cards) ? storedAnswer.cards : readFullCards()) || []).filter(cardFull).slice(0, 2); // F-0914-10: saqlangan javob massiv bo'lmasa — bo'sh (oq ekran himoyasi)
...
const ScreenCoding = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [st, setSt] = useState(() => {
    ...
    return { code: (typeof storedAnswer?.code === 'string' ? storedAnswer.code : null) || (saved && saved.code) || kodStart(), done: !!(storedAnswer && storedAnswer.solved) || !!(saved && saved.done) }; // F-0914-10: saqlangan javob matn bo'lmasa — zaxira-zanjir (oq ekran himoyasi)
  });
  const { code, done } = st;
...
const INLINE_KEYS = { s4: 2, s5b: 1, s9: 2, s12: 2, s15: -1 }; // o'lik mashq-kalitlar olib tashlandi (jsx-lint, 2026-09-07): s6
const QUIZ_BANK = [
...
// ============================================================ LESSON ROOT
export default function PmLesson1({ lang: langProp, onFinished, liveToken }) {
  ...
  return (
    <LangContext.Provider value={lang}>
      <style>{`
        ...
        /* Foto — emoji o'rnida, o'sha ustunda. Balandligi matnni pastga surib yubormaydi. */
