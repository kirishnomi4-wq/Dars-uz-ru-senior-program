// Jonli-dars UI: darvoza (PIN/mentor-kod), yuqori belgi, katta PIN ekrani. Darslardagi inline nusxadan
// ko'chirildi (2026-09-03). Yangi: darvozada «Kodsiz, o'zim ko'raman» — o'quvchi uyda darsni ocha oladi (F-0903-01).
import React, { useState, useEffect } from 'react';
import { tr } from './i18n.js';
import { livePlayers, fmtPin, nickRead } from './liveClient.js';
...
export function LiveGate({ live, title = 'Jonli dars' }) {
  ...
  const [pinFallback, setPinFallback] = useState(false); // LMS-tanlovdan «kod bilan kiraman»ga o'tish

  // LMS orqali avto-kirish ketmoqda — kutish kartasi (token kelib, server javob berguncha)
  ...
  // O'quvchi 2+ guruhda va ikkalasida jonli dars — xavfsiz tanlov (guruh nomi oshkor qilinmaydi)
  if (lms.state === 'choose' && !pinFallback && Array.isArray(lms.choices)) {
    const when = (iso) => { try { return new Date(iso).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }); } catch { return ''; } };
    return (<div style={wrap}><div style={card} data-live="lms-choose">
      ...
      {lms.choices.map((c) => (
        <button key={c.session_id} onClick={() => live.joinWithToken(c.session_id)} style={{ ..._liveBtnPri, display: 'flex', justifyContent: 'space-between', gap: 12 }}>
  ...
  const lmsNote = lms.state === 'none'
    ? tr({ uz: "Guruhingizda hozir jonli dars yo'q. Kod bilan kiring yoki o'zingiz ko'ring.", ru: 'В вашей группе сейчас нет живого урока. Войдите по коду или смотрите сами.' })
    : lms.state === 'error'
      ? tr({ uz: "Avtomatik kirish bo'lmadi. Kod bilan kiring.", ru: 'Автоматический вход не удался. Войдите по коду.' })
      : '';
  if (role === 'mentor') {
...
export function LiveBadge({ live, total }) {
  ...
  useEffect(() => {
    ...
    let on = true, t = null;
    const tick = async () => {
      try { const rows = await livePlayers(live.pin); if (on) setNPlayers(rows.length); } catch { /* keyingi urinish */ }
  ...
  // LMS: mustaqil urinish (server-ball, oxirgi ekranda tugaydi) va ko'rish rejimi (F-0903-01)
  const restartBtn = (
    <button data-live="restart" disabled={live.busy} onClick={() => live.restartAttempt && live.restartAttempt()} style={{ background: LT.accentSoft, color: LT.accent, border: 'none', borderRadius: 99, padding: '4px 10px', fontSize: 12, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}>{tr({ uz: '↻ Qaytadan boshlash', ru: '↻ Начать заново' })}</button>
  );
  if (live.mode === 'solo') {
