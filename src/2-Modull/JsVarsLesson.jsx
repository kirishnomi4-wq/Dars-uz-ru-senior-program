...
// F-0803-11: «🌯 Loyihaga ko'prik» bloki OLIB TASHLANDI (foydalanuvchi qarori) —
// s15 ning ishi bitta: o'quvchi birinchi o'zgaruvchisini O'ZI yozadi. Yozib bo'lgan
// lahzada kelajakdagi loyiha haqida qo'shimcha matn chiqarish g'alabani suyultiradi.
...
// Muammo edi: `.fc-tag` hamma javobga 46px monoshrift berardi — u bir so'zlik javob
// (`let`, `=`, `string`) uchun tanlangan o'lcham. 32 belgilik gap 2-3 qatorga bo'linib,
// qat'iy balandlikdagi kartaga sig'masdi va izoh pastki chetga yopishib qolardi.
// Yechim: (1) uzunlik bo'yicha 4 pog'onali o'lcham · (2) bitta token — mono (kod),
...
const Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tayyor', ru: 'Готово' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: 'clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Qaytadan', ru: 'Заново' })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: 'auto', padding: 'clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Yakunlash ✓', ru: 'Завершить ✓' })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: 'Dars tugadi', ru: 'Урок завершён' })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Birinchi <span className="italic" style={{ color: T.accent }}>JavaScript</span> kodingizni yozdingiz.</>, ru: <>Вы написали свой первый код на <span className="italic" style={{ color: T.accent }}>JavaScript</span>.</> })}</h2>{/* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */}</div><ScoreRing correct={correct} total={total} /></div>
...
//  JsVars uchun yengil rejim: task.files = faqat `script.js` → 🖥️ Console paneli.
// ============================================================
...
function HtmlCompiler({
  ...
  // F-0803-16: YENGIL REJIM — topshiriqda faqat `script.js` bo'lsa, ko'rinadigan sahifa
  // YO'Q (HTML yozilmaydi). Bo'sh oq preview yarim ekranni egallamasin: iframe jimgina
  // ishlashda davom etadi (JS bajariladi, console uzatiladi), lekin ko'rinmaydi —
  // o'rniga konsol butun panelni oladi.
  const jsOnly = useMemo(() => files.length > 0 && files.every((f) => f.lang === 'js'), [files]);
    ...
    <div className="hc-root">
      ...
      <main className="hc-split">
        ...
        <section className="hc-pane hc-preview-pane">
          ...
          <iframe
            className={`hc-frame${jsOnly ? ' hc-frame-off' : ''}`}
            title="natija"
            sandbox="allow-scripts allow-popups allow-popups-to-escape-sandbox"
            srcDoc={doc}
          />
          {showConsole && (
            <div className={`hc-console${jsOnly ? ' full' : ''}`}>
...
function StyleTag() {
  return (
    <style>{`
      ...
      .hc-frame-off{display:none}
      .hc-console.full{height:auto;flex:1;border-top:none}
...
// ============================================================
//  ✏️ PRAKTIKA — 3 ta YENGIL topshiriq (F-0803-09).
//  JsVars — o'quvchining BIRINCHI JS darsi, shuning uchun praktika yengil
//  variantda: BITTA fayl (`script.js`), HTML/DOM yo'q, natija 🖥️ Console
//  panelida ko'rinadi. Shartlar 2-3 tadan oshmaydi.
...
// — P1: BALL QUTISI (Screen5b — let testidan — keyin). Faqat let: const hali o'tilmagan. —
const TASK_BALL = {
...
// — P2: QO'SHTIRNOQ SINOVI (Screen12 — tur/qo'shtirnoq testidan — keyin) —
const TASK_QUOTES = {
...
// (indekslar `screens[]` bo'yicha: 6 = s5b, 13 = s12, 16 = s15)
...
export default function JsVarsLesson({ lang: langProp, onFinished, onPractice }) {
  ...
  // mustaqil o'qiyotgan bola uchun mashq yakun-sahifadagi «Uyga vazifa» tugmasi orqali ochiladi.
  const next = () => {
