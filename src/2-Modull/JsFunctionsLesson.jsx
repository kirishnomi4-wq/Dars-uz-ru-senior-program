...
// Hook: har urishda zararni qo'lda hisoblash — takror dardi. Dars ipi: o'yin zarari (F-0803-20).
...
// `...` bilan belgilangan kod atamalarini (function, return, zarar(3)) matndan ajratib chip qilib ko'rsatadi.
...
const Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  // F-0803-19 (TMI): avval bu yerda 3 ta ATAMA-TA'RIFI (funksiya/parametr/return) + 2 xatboshi
  // turardi — ya'ni dars boshlanmasdan turib butun mazmun ta'rif bilan aytib qo'yilardi
  // (918 belgi, darsning eng katta matni). Endi bitta KO'RSATILADIGAN nuqta qoldi;
  // atamalar o'z ekranida, misol ustida ochiladi (induktiv tartib).
  const PreviewBlock = (
    <Col>
            ...
            <div className="codebox demo-swap" key={mode}>
              {mode === 'manual' ? (
                <>
                  ...
                  <div><CM>{tr({ uz: "// zarar o'zgarsa — 3 joyda tuzatamiz 🥲", ru: '// изменится урон — чиним в 3 местах 🥲' })}</CM></div>
                </>
              ) : (
                <>
                  ...
                  <div>{'}'}</div>
                  <div style={{ marginTop: 8 }}><FN>zarar</FN>() <CM>{tr({ uz: '// 1-urish', ru: '// 1-й удар' })}</CM></div>
                  <div><FN>zarar</FN>() <CM>{tr({ uz: '// 2-urish', ru: '// 2-й удар' })}</CM></div>
                  <div><FN>zarar</FN>() <CM>{tr({ uz: '// 3-urish', ru: '// 3-й удар' })}</CM></div>
                </>
              )}
            </div>
          </Col>
...
const Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const showBuild = ran && !dragDone;   // mashina ishlagach — o'ng ustun yig'ish maydoniga aylanadi
  const PARTS = {
    name: { color: T.blue, num: '1', name: tr({ uz: 'Funksiya nomi', ru: 'Имя функции' }), code: 'zarar', desc: tr({ uz: "Mashinaning nomi. Aynan shu nom bilan funksiyani chaqiramiz: zarar().", ru: 'Имя машины. Именно по этому имени мы вызываем функцию: zarar().' }) },
    param: { color: T.accent, num: '2', name: tr({ uz: 'Parametr', ru: 'Параметр' }), code: 'kuch', desc: tr({ uz: 'Mashinaga beriladigan kirish (input). Chaqirganda haqiqiy qiymat solamiz, masalan 5.', ru: 'Вход машины (input). При вызове кладём настоящее значение, например 5.' }) },
  ...
  const run = () => {
    ...
    timer.current = setTimeout(() => { setOut(['15']); setRunning(false); }, 700);
  };
  useEffect(() => { if (done && storedAnswer === undefined) onAnswer(screen, { correct: true, picked: true }); }, [done]);
  return (
        ...
        <div className="split">
          <Col>
            <div className="codebox fade-up delay-1" style={{ fontSize: 'clamp(13px,1.8vw,15px)' }}>
              <div>
                <KW>function</KW>{' '}
                <span className={part ? undefined : 'cp-hint'} onClick={() => setPart('name')} style={{ cursor: 'pointer', color: '#5BC8EC', fontWeight: 700, outline: part === 'name' ? `2px solid ${T.blue}` : 'none', borderRadius: 4, padding: '0 2px' }}>zarar</span>(
                <span className={part ? undefined : 'cp-hint'} onClick={() => setPart('param')} style={{ cursor: 'pointer', color: '#FF9777', fontWeight: 700, outline: part === 'param' ? `2px solid ${T.accent}` : 'none', borderRadius: 4, padding: '0 2px' }}>kuch</span>) {'{'}
              </div>
...
const Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const KUCHLAR = [4, 5, 8, 12];   // urish kuchi — parametrga tushadigan qiymatlar
  ...
  return (
    <Stage eyebrow={{ uz: 'Parametr', ru: 'Параметр' }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: "Kuchni o'zgartiring", ru: 'Поменяйте силу' }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Bir mashinaga <span className="italic" style={{ color: T.accent }}>har xil kuch</span> bersak-chi?</>, ru: <>А если дать одной машине <span className="italic" style={{ color: T.accent }}>разную силу</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr({ uz: 'Qanday kuch bilan uramiz?', ru: 'С какой силой бьём?' })}</p>
...
const Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const run = () => {
    ...
    timer.current = setTimeout(() => { setNatija(n * 3); setRunning(false); setRan(true); }, 750);
  };
  ...
  return (
    <Stage eyebrow="return" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: 'Avval ishga tushiring', ru: 'Сначала запустите' }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr({ uz: 'Qaysi kuch bilan uramiz?', ru: 'С какой силой бьём?' })}</p>
            <div className="fade-up delay-1" style={{ display: 'flex', gap: 8 }}>
              {[4, 5, 6].map(v => <button key={v} className={`chip ${n === v ? 'chip-on' : ''}`} onClick={() => setNN(v)}>{v}</button>)}
            </div>
            <div className="codebox" style={{ marginTop: 6 }}>
              <div><KW>function</KW> <FN>zarar</FN>(kuch) {'{'}</div>
              <div style={{ paddingLeft: 18 }}><KW>return</KW> kuch * <NUM>3</NUM> <CM>{tr({ uz: '// natijani qaytaradi', ru: '// возвращает результат' })}</CM></div>
              <div>{'}'}</div>
              <div style={{ marginTop: 8 }}><KW>let</KW> ochko = <FN>zarar</FN>(<NUM>{n}</NUM>) <CM>{tr({ uz: '// javobni saqlaydi', ru: '// сохраняет ответ' })}</CM></div>
            </div>
...
const Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow={{ uz: "Ko'p parametr", ru: 'Много параметров' }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: "Sonlarni o'zgartiring", ru: 'Поменяйте числа' }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Mashinaga <span className="italic" style={{ color: T.accent }}>ikkita qiymat</span> bersak-chi?</>, ru: <>А если дать машине <span className="italic" style={{ color: T.accent }}>два числа</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr({ uz: 'Urish kuchi — kuch', ru: 'Сила удара — kuch' })}</p>
            <div className="fade-up delay-1" style={{ display: 'flex', gap: 8 }}>
              {[2, 5, 10].map(v => <button key={v} className={`chip ${a === v ? 'chip-on' : ''}`} onClick={() => setAA(v)}>{v}</button>)}
            </div>
            <p className="flow-label" style={{ marginTop: 4 }}>{tr({ uz: 'Krit bonus — bonus', ru: 'Крит-бонус — bonus' })}</p>
...
const Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const BONUS = 10;   // krit bonusi - ikkinchi parametr
  const [soni, setSoni] = useState(5);   // urish kuchi
  const [jami, setJami] = useState(storedAnswer ? 5 * 3 + BONUS : null);
  const [running, setRunning] = useState(false);
  const [ran, setRan] = useState(!!storedAnswer);
  const timer = useRef(null);
  const done = ran;
  ...
  const run = () => {
    ...
    timer.current = setTimeout(() => { setJami(soni * 3 + BONUS); setRunning(false); setRan(true); }, 750);
  };
  ...
  return (
    <Stage eyebrow={{ uz: 'Hammasi birga', ru: 'Всё вместе' }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: 'Avval hisoblang', ru: 'Сначала посчитайте' }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr({ uz: <>Urish kuchi? (krit bonusi {BONUS})</>, ru: <>Сила удара? (крит-бонус {BONUS})</> })}</p>
            <div className="fade-up delay-1" style={{ display: 'flex', gap: 8 }}>
...
const Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  // Bitta funksiya — uch xil kuch bilan chaqiriladi, har safar boshqa zarar (kuch × 3)
  const XILLAR = [{ e: '👊', s: 4, n: tr({ uz: 'Oddiy urish', ru: 'Обычный удар' }) }, { e: '⚔️', s: 6, n: tr({ uz: 'Qilich zarbasi', ru: 'Удар мечом' }) }, { e: '🔥', s: 9, n: tr({ uz: 'Olovli zarba', ru: 'Огненный удар' }) }];
  const KOEF = 3;   // har kuch birligi 3 zarar beradi
  const som = (v) => v.toLocaleString('ru-RU').replace(/ /g, ' ');
  const N = XILLAR.length;
  const line = (x) => `⚔️ ${x.n} — ${x.s * KOEF} ${tr({ uz: 'zarar', ru: 'урона' })}`;
  ...
  return (
    <Stage eyebrow={{ uz: 'Hayotiy misol', ru: 'Пример из жизни' }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: 'Hisoblagichni ishlating', ru: 'Запустите расчёт' }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Esingizdagi urishlar? Endi zararni <span className="italic" style={{ color: T.accent }}>funksiya</span> hisoblaydi.</>, ru: <>Помните удары? Теперь урон считает <span className="italic" style={{ color: T.accent }}>функция</span>.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="codebox fade-up delay-1" style={{ lineHeight: 1.9 }}>
              <div><KW>function</KW> <FN>zarar</FN>(<span style={{ color: '#FF9777', fontWeight: 700 }}>kuch</span>) {'{'}</div>
              <div style={{ paddingLeft: 16 }}><CM>{tr({ uz: '// har kuch birligi 3 zarar', ru: '// каждая единица силы — 3 урона' })}</CM></div>
              ...
              {XILLAR.map((x, i) => {
                ...
                return (
                  <div key={i} className={`flavor-card ${got ? 'got' : ''}`}>
                    ...
                    <div><div className="flavor-name">{x.n}</div><div className="flavor-msg">{got ? `${x.s * KOEF} ${tr({ uz: 'zarar', ru: 'урона' })}` : tr({ uz: 'navbatda…', ru: 'в очереди…' })}</div></div>
        ...
        <div className="split">
          <Col>
            <div className="ai-card fade-up delay-1">
              <div className="ai-row"><span className="ai-badge">AI</span><span className="ai-bubble">{tr({ uz: 'Zararni qaytaramiz:', ru: 'Возвращаем урон:' })}</span></div>
...
const Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  useEffect(() => { if (valid && !passed) { setPassed(true); onAnswer(screen, { stage: 'final', screenIdx: screen, question: 'zarar funksiyasini yozing', studentAnswer: value, correct: true, firstAttemptCorrect: true, solved: true, picked: value }); } }, [valid]);
  return (
        ...
        <div className="split">
          ...
          <Col>
            ...
            <div style={{ background: T.paper, borderRadius: 14, minHeight: 130, padding: '20px', boxShadow: `0 8px 22px -10px rgba(${T.shadowBase},0.16)`, display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
              {valid
                ? <div className="fade-step"><div style={{ fontSize: 36 }}>🛠️</div><p style={{ fontFamily: "'Source Serif 4',serif", color: T.success, fontWeight: 700, margin: '8px 0 4px', fontSize: 'clamp(16px,2.4vw,20px)' }}>{tr({ uz: 'Funksiya tayyor!', ru: 'Функция готова!' })}</p><p className="mono small" style={{ margin: 0, color: T.ink2 }}>zarar(4) → 16</p></div>
...
const Q_LABELS = { 4: { uz: "1 — Chaqirish", ru: '1 — Вызов' }, 6: "2 — zarar(3)", 10: "3 — zarar(10,3)", 13: { uz: "4 — return x da", ru: '4 — return в x' }, 16: { uz: "5 — Funksiya yoz", ru: '5 — Напиши функцию' } };
...
//  bu dars funksiya · parametr · return ni o'rgatadi (massiv/obyekt bu yerda YO'Q).
// ============================================================

// — P1: BIRINCHI FUNKSIYA (Screen5b — testdan — keyin) —
const TASK_SALOM = {
  ...
  requirements: [
    { id: 'fn', label: { uz: 'funksiya yozildi', ru: 'функция написана' }, check: C.js(/function\s+\w+\s*\(/, { uz: "`function salom() {` deb boshlang", ru: 'Начните с `function salom() {`' }) },
    { id: 'log', label: { uz: 'chaqirildi — konsolda Salom', ru: 'вызвана — в консоли Salom' }, check: C.logs('Salom', { uz: "Funksiyani chaqirishni unutmang: pastda `salom()` yozing", ru: 'Не забудьте вызвать: ниже напишите `salom()`' }) },
  ],
};

// — P2: PARAMETR VA RETURN (Screen12 — testdan — keyin) —
const TASK_KVADRAT = {
...
// — P3: BIR FUNKSIYA — KO'P MARTA (Screen15 — dan keyin, YAKUNIY) —
const TASK_QAYTA = {
  ...
  requirements: [
    { id: 'param', label: { uz: 'parametrli funksiya', ru: 'функция с параметром' }, check: C.js(/function\s+\w+\s*\(\s*\w+/, { uz: "`function salomla(ism) {` deb boshlang", ru: 'Начните с `function salomla(ism) {`' }) },
    { id: 'log', label: { uz: 'ikkala salom konsolda', ru: 'оба приветствия в консоли' }, check: C.logs('Salom, Aziza Salom, Bobur', { uz: "Funksiyani ikki marta chaqiring: `salomla(\"Aziza\")` va `salomla(\"Bobur\")`", ru: 'Вызовите функцию дважды: `salomla(\"Aziza\")` и `salomla(\"Bobur\")`' }) },
  ],
};

// Praktika handoff xaritasi: shu ekran INDEKSIDAN keyin qaysi praktika chaqiriladi.
const PRACTICE_AFTER = {
  6:  { task: TASK_SALOM,   starter: '' }, // 1) funksiya yoz + chaqir
  13: { task: TASK_KVADRAT, starter: '' }, // 2) parametr + return
  16: { task: TASK_QAYTA,   starter: '' }, // 3) yakuniy: bitta funksiya, ikki chaqiruv
};
const HW_TASK = TASK_QAYTA;

export default function JsFunctionsLesson({ lang: langProp, onFinished, onPractice, liveToken }) {
