  ...
  return (
    <Stage eyebrow={tr({ uz: 'Ochilish kuni 🚀', ru: 'День открытия 🚀' })} screen={screen}
      ...
      <div className="screen">
        <h1 className="title h-title fade-up">{tr({ uz: <>Bitta odam bir haftada 12 ta ishning <span className="italic" style={{ color: T.accent }}>nechtasini</span> bajaradi?</>, ru: <>Сколько из 12 дел <span className="italic" style={{ color: T.accent }}>сделает</span> один человек за неделю?</> })}</h1>
...
const ScrCoding = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [st, setSt] = useState(() => {
    ...
    return { code: (typeof storedAnswer?.code === 'string' ? storedAnswer.code : null) || (saved && saved.code) || KODING_STARTER, done: !!(storedAnswer && storedAnswer.solved) || !!(saved && saved.done) }; // F-0914-10: saqlangan javob matn bo'lmasa — zaxira-zanjir (oq ekran himoyasi)
  });
  const { code, done } = st;
  useEffect(() => {
...
const INLINE_KEYS = { s4: 2, s7: 2, s10: 1, s15: 1 }; // o'lik mashq-kalitlar olib tashlandi (jsx-lint, 2026-09-07): s8, s9, s11, s12
...
export default function PmLesson5({ lang: langProp, onFinished, liveToken }) {
