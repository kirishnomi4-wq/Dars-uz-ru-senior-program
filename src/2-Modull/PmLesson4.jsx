...
// 11.1 — mentor avatar: hostlangan rasm (LMS'da assets papkasi bo'lmaydi, lokal import EMAS)
...
function LiveBadge({ live, total }) {
  ...
  // PIN faqat «📺 Ko'rsatish» tugmasi bilan katta ochiladi — avto-ochilmaydi (11.14)
...
const ACHIEVEMENTS = {
  sorter:   { icon: '🗂️', name: 'Sort It!',    desc: "Feature'larni dori va shirinlikka to'g'ri saraladingiz" },
  matcher:  { icon: '💊', name: 'Right Med!',  desc: "Har og'riqqa uni yopadigan to'g'ri feature'ni topdingiz" },
  debugger: { icon: '🐞', name: 'Nice Catch!', desc: "Hech qanday og'riqni yopmaydigan behuda feature'ni topdingiz" },
  graduate: { icon: '🏆', name: 'Level Up!',   desc: 'Muammo → Yechim darsini to\'liq yakunladingiz' },
};
// Ekran id → nishon — FAQAT haqiqiy solve/match ekranlariga (recordAnswer'da avtomatik beriladi).
// s5 (saralash), s8 (og'riq↔feature moslash), s10 (behuda feature debug) — ularda correct:true faqat to'g'ri yechimda chiqadi.
const ACH_TRIGGERS = { s5: 'sorter', s8: 'matcher', s10: 'debugger' };
...
const LESSON_META = { lessonId: 'pm-problem-solution-04-v18', lessonTitle: { uz: 'Muammo → Yechim', ru: 'Проблема → Решение' } };
const SCREEN_META = [
...
const RECAPS = {
  4: {
    title: "Avval og'riq, keyin feature",
    cards: [
      { ic: "🩺", h: "Feature — bu og'riqqa dori", body: <>Feature (funksiya) qurishdan oldin bitta narsa aniq bo'lishi shart: <b>qaysi foydalanuvchi og'rig'ini</b> yopadi? Xuddi shifokor dori berishdan oldin "qayering og'riyapti?" deb so'raganday — avval og'riqni topamiz, keyin dori tanlaymiz.</>, vis: <RcFlow items={["Foydalanuvchi", "Og'riq", "Feature (dori)"]} />, ask: "Instagram qaysi og'riqni yopadi deb o'ylaysiz?" },
      { ic: "🎨", h: "Rang, tugma, logo — bular emas", body: <>"Qaysi rangda bo'lsin?", "Nechta tugma?", "Logo qanaqa?" — bular <b>keyin</b> hal bo'ladigan mayda savollar. Ular og'riqni yopmaydi. Avval "kimga va nima uchun kerak?" degan asosiy savolga javob beramiz.</>, vis: <RcFlow items={["Og'riq nima?", "Kimga kerak?", "Qanday yopamiz?"]} /> },
      { ic: "🎯", h: "Og'riqni bilmasangiz — nishonsiz o'q", body: <>Og'riqni bilmay feature qurish — ko'zni yumib o'q otganday. Balki tegar, balki yo'q. <b>Aniq og'riq</b> — bu nishon. Nishon bor bo'lsa, feature to'g'ri joyga tegadi.</> },
    ]
  },
  6: {
    title: "Dori-mi yoki shirinlik?",
    cards: [
      { ic: "💊", h: "Dori og'riqni yopadi", body: <>Har bir feature ikki xil bo'ladi. <b>Dori</b> — real og'riqni yopadi (masalan, mahsulot rasmini yuklash: xaridor ko'radi va ishonadi). <b>Shirinlik</b> — shirin, chiroyli, lekin hech qanday og'riqni yopmaydi.</>, vis: <RcFlow items={["Rasm yuklash", "Xaridor ko'radi", "Ishonadi → sotib oladi"]} /> },
      { ic: "🍬", h: "3D logo, qor, 32 shrift — shirinlik", body: <>Logoni 3D aylantirish, sahifada qor yog'dirish, 32 xil shrift — bularning hammasi <b>bezak</b>. Ko'zga yoqadi, lekin foydalanuvchining birorta muammosini hal qilmaydi. Shirinlikni yeb qorin to'ymaydi.</>, ask: "Sevimli ilovangizda qaysi narsa 'shirinlik', qaysi biri 'dori'?" },
      { ic: "🤔", h: "Sinov savoli: buni olib tashlasak nima bo'ladi?", body: <>Feature dori yoki shirinlik ekanini bilish oson. O'zingizga savol bering: <b>"Buni olib tashlasam, kimningdir ishi to'xtaydimi?"</b> Ha bo'lsa — dori. Yo'q, faqat kamroq chiroyli bo'ladi bo'lsa — shirinlik.</> },
    ]
  },
  10: {
    title: "Og'riqni to'g'ri feature bilan yop",
    cards: [
      { ic: "🛒", h: "\"Ko'rmasdan ishonmayman\"", body: <>Xaridor: "Mahsulotni <b>ko'rmasdan</b> ishonmayman" deydi. Bu — aniq og'riq. Endi shu og'riqqa mos dorini tanlaymiz: <b>mahsulot rasmini yuklash</b>. Rasm bo'lsa, xaridor ko'radi, ishonadi va sotib oladi.</>, vis: <RcFlow items={["Ko'rmasdan ishonmayman", "Rasm yuklaymiz", "Ko'rdi → ishondi"]} /> },
      { ic: "🚫", h: "Rang, logo, reklama — bu og'riqni yopmaydi", body: <>Sahifa rangini o'zgartirish yoki logoni kattalashtirish — xaridorga mahsulotni <b>ko'rsatmaydi</b>. Ko'proq reklama esa ishonchni oshirmaydi, aksincha bezdiradi. Og'riq "ko'rmayapman" bo'lsa, yechim ham "ko'rsatish" bo'lishi kerak.</> },
      { ic: "🔑", h: "Har og'riqning o'z kaliti bor", body: <>Feature tanlashda qoida oddiy: <b>og'riq bilan yechim bir-biriga mos</b> bo'lsin. "Ko'rmayapman" → rasm. "Topolmayapman" → qidiruv. "Yetdimi bilmayman" → "o'qildi" belgisi. Kalit qulfga mos kelishi kerak.</>, ask: "\"Minglab e'lon ichidan topolmayapman\" og'rig'iga qaysi feature dori bo'ladi?" },
    ]
  },
  13: {
    title: "Muammosiz yechim — behuda mehnat",
    cards: [
      { ic: "🏚️", h: "Og'riq yo'q bo'lsa, feature keraksiz", body: <>"Muammosiz yechim" — bu hech kimga og'rimagan joyga qo'yilgan dori. <b>Vaqt va kuch ketadi</b>, lekin <b>hech kim ishlatmaydi</b>. Cho'lning o'rtasida chiroyli avtobus bekati qurganday — bekat bor, lekin odam yo'q.</>, vis: <RcFlow items={["Og'riq yo'q", "Feature qurildi", "Hech kim ishlatmaydi"]} /> },
      { ic: "⏳", h: "Vaqt — eng qimmat narsa", body: <>Og'riqsiz feature'ga sarflangan har bir soat — real og'riqni yopadigan feature'dan <b>o'g'irlangan soat</b>. Chiroyli ko'rinsa ham, foyda keltirmasa, u faqat resursni yeydi.</> },
      { ic: "✅", h: "Yaxshi feature'ning belgisi", body: <>To'g'ri feature'ni topdingizmi tekshirish oson: <b>real odam</b> bormi va u shu narsani <b>chindan ishlatadi</b>mi? Ha bo'lsa — bu dori. Yo'q bo'lsa — bu shunchaki chiroyli, lekin bo'sh quti.</>, ask: "O'zingiz o'ylab topgan g'oyangizda qaysi og'riq bor — kim buni chindan xohlaydi?" },
    ]
  },
};
...
// ===== 🤖 DORIBOT — DORIXONA robot-dorishunosi (s8 asosiy, s6/s13 kichik identifikator). Tibbiy-xoch YO'Q — dori-shisha belgisi. =====
const DoriBot = ({ size = 88, mood = 'idle', say }) => (
  <div className={`doribot doribot-${mood}`} style={{ width: size }}>
    <svg viewBox="0 0 100 112" width={size} height={size * 1.12} aria-hidden="true">
      <line x1="50" y1="15" x2="50" y2="6" stroke={T.accent} strokeWidth="2.6" strokeLinecap="round" />
      <circle className="db-spark" cx="50" cy="5" r="4.2" fill={T.accent} />
      <rect x="11" y="13" width="78" height="76" rx="31" fill="#FFFFFF" stroke={`${T.ink3}66`} strokeWidth="2" />
      <rect x="23" y="29" width="54" height="31" rx="15.5" fill="#101828" />
      <circle className="db-eye" cx="39" cy="44" r="4.6" fill="#7FE7C4" />
      <circle className="db-eye" cx="61" cy="44" r="4.6" fill="#7FE7C4" />
      <path d="M42 52.5 q8 6.5 16 0" fill="none" stroke="#7FE7C4" strokeWidth="2.2" strokeLinecap="round" />
      <g transform="translate(50 74) rotate(-32)"><rect x="-13" y="-5.5" width="26" height="11" rx="5.5" fill={T.accentSoft} stroke={T.accent} strokeWidth="2.2" /><line x1="0" y1="-5.5" x2="0" y2="5.5" stroke={T.accent} strokeWidth="2.2" /></g>
      <rect x="30" y="90" width="40" height="9" rx="4.5" fill={`${T.ink3}33`} />
    </svg>
    {say && <span className="db-say fade-step">{say}</span>}
  </div>
);
...
const Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow="Og'riq → feature" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} optionalLive label={done ? 'Davom etish' : `${seen.size}/4 ko'ring`} onClick={onNext} /></>}>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {KEYS.map(k => (<button key={k} onClick={() => tap(k)} className={!seen.has(k) ? 'tap-hint-card' : ''} style={{ display: 'flex', alignItems: 'center', gap: 9, cursor: 'pointer', border: 'none', borderRadius: 13, padding: '13px 12px', background: T.paper, boxShadow: active === k ? `inset 0 0 0 2px ${T.accent}, 0 8px 20px -7px rgba(255,79,40,0.22)` : `0 6px 16px -8px rgba(${T.shadowBase},0.16)`, transition: 'all 0.18s' }}><span style={{ display: 'inline-flex' }}>{APPS[k].ic}</span><span style={{ fontFamily: "'Manrope',sans-serif", fontWeight: 600, fontSize: 13.5, color: T.ink }}>{APPS[k].name}</span>{seen.has(k) && <span style={{ marginLeft: 'auto', color: T.success, display: 'inline-flex' }}>{Ico.check(13)}</span>}</button>))}
            </div>
          </Col>
          <Col>
            {active ? (
...
const Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  useEffect(() => { if (done && storedAnswer === undefined) onAnswer(screen, { correct: true, picked: true }); }, [done]);
  return (
    <Stage eyebrow="Og'riq = o'zak" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} optionalLive label={done ? 'Davom etish' : 'Ikkalasini ko\'ring'} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: 'flex', gap: 8 }}>
              <button className={`chip ${hasPain ? 'chip-on' : ''} ${!seen.has('on') ? 'tap-hint' : ''}`} onClick={() => set(true)}>Og'riq bor</button>
              <button className={`chip ${!hasPain ? 'chip-on' : ''} ${!seen.has('off') ? 'tap-hint' : ''}`} onClick={() => set(false)}>Og'riq yo'q</button>
            </div>
          ...
          <Col>
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>Demak <b>og'riq — feature'ning o'zagi</b>. Og'riq yo'qolsa, feature ham keraksiz bo'ladi.</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
const Screen4 = (props) => (
  <QuestionScreen {...props} scope="module-mikro" eyebrow="Mashq · 1-savol"
    ...
    options={['Sahifa qaysi rangda bo\'lishi', 'Nechta tugma qo\'yish kerakligi', 'Qaysi foydalanuvchi og\'rig\'ini yopishi', 'Logo qanday ko\'rinishga ega bo\'lishi']} correctIdx={2}
    ...
    explainWrong={{ 0: 'Rang — bezak. Avval qaysi og\'riqni yopishini aniqlang.', 1: 'Tugmalar soni muhim emas. Muhimi — qaysi muammoni yechishi.', 3: 'Logo — tashqi ko\'rinish. Avval og\'riqni toping.', default: 'Avval feature qaysi og\'riqni yopishini aniqlang.' }} />
);
...
const Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow="Saralash" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} optionalLive label={done ? 'Davom etish' : `${cnt}/${SORT_ITEMS.length} saralang`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        <div className="head" style={{ display: 'flex', alignItems: 'center', gap: 12 }}><DoriBot size={44} /><h2 className="title h-title fade-up" style={{ margin: 0 }}>Javchani saralaymiz: bu feature <span className="italic" style={{ color: T.success }}>dori</span>mi yoki <span className="italic" style={{ color: T.honey }}>shirinlik</span>mi?</h2></div>
        <Mentor>Dorixona javchasida ikki tokcha bor. Og'riqni yopsa — <b style={{ color: T.success }}>dori-tokcha</b>, shunchaki bezak bo'lsa — <b style={{ color: T.honey }}>shirinlik-tokcha</b>. Har feature'ni to'g'ri tokchaga qo'ying.</Mentor>
        <Zoomable>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div className="shelf-tabs fade-up">
          <span className="shelf-tab dori">{p4.pill(15)} Dori-tokcha <span className="shelf-tab-sub">og'riqni yopadi</span></span>
          <span className="shelf-tab candy">{p4.candy(15)} Shirinlik-tokcha <span className="shelf-tab-sub">faqat bezak</span></span>
        </div>
        <div className="fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
          {SORT_ITEMS.map(it => {
              ...
              <div key={it.id} className={isWrong ? 'shake-x' : ''} style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', background: ok ? (it.kind === 'dori' ? T.successSoft : T.honeySoft) : T.paper, borderRadius: 12, padding: '11px 14px', boxShadow: ok ? 'none' : `0 6px 16px -8px rgba(${T.shadowBase},0.16)`, transition: 'background 0.25s' }}>
                ...
                {ok ? (
                  ...
                  <span style={{ display: 'flex', gap: 7 }}>
                    <button className="kindbtn" onClick={() => place(it, 'dori')} style={{ background: T.successSoft, color: T.success }}>{p4.pill(14)} Dori-tokcha</button>
                    <button className="kindbtn" onClick={() => place(it, 'shirinlik')} style={{ background: T.honeySoft, color: T.honey }}>{p4.candy(14)} Shirinlik-tokcha</button>
                  </span>
                )}
              </div>
            );
...
const Screen5b = (props) => (
  <QuestionScreen {...props} scope="module-mikro" eyebrow="Tekshiruv"
    ...
    options={['Mahsulot rasmini yuklash', 'Logoni 3D aylantirish', '32 xil shrift qo\'shish', 'Sahifada qor yog\'dirish']} correctIdx={0}
    ...
    explainWrong={{ 1: '3D logo — chiroyli, lekin hech qanday og\'riqni yopmaydi. Bu shirinlik.', 2: '32 shrift hech kimga yordam bermaydi — bu bezak.', 3: 'Qor animatsiyasi ko\'zga chiroyli, lekin muammo yechmaydi.', default: 'Dori — og\'riqni yopadigan feature. Rasm yuklash shunday.' }} />
);
...
const Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  useEffect(() => { if (done && storedAnswer === undefined) onAnswer(screen, { correct: true, picked: true }); }, [done]);
  return (
    <Stage eyebrow="Feature tug'iladi" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} optionalLive label={done ? 'Davom etish' : 'Avval kuzating'} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(8px,1.4vw,13px)' }}>
        <div className="head" style={{ display: 'flex', alignItems: 'center', gap: 12 }}><DoriBot size={46} /><h2 className="title h-title fade-up" style={{ margin: 0 }}>Feature qayerdan <span className="italic" style={{ color: T.accent }}>tug'iladi</span>?</h2></div>
...
const Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  useEffect(() => { if (done && storedAnswer === undefined) onAnswer(screen, { correct: true, picked: true }); }, [done]);
  return (
    <Stage eyebrow="Qaysini ishlatasiz" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} optionalLive label={done ? 'Davom etish' : 'Ikkalasini ko\'ring'} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: 'flex', gap: 8 }}>
              <button className={`chip ${v === 'dori' ? 'chip-on' : ''} ${!seen.has('dori') ? 'tap-hint' : ''}`} onClick={() => set('dori')}>Doriga to'la</button>
              <button className={`chip ${v === 'shirinlik' ? 'chip-on' : ''} ${!seen.has('shirinlik') ? 'tap-hint' : ''}`} onClick={() => set('shirinlik')}>Shirinlikka to'la</button>
            </div>
          ...
          <Col>
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>Mahsulotni <b>dori</b> ushlab turadi, shirinlik emas. Avval og'riqlarni yoping.</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
// ===== SCREEN 8 — 🏥 DORIXONA (signature o'yin): og'riqli foydalanuvchiga to'g'ri dorini (feature) ber =====
const Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  // Navbatdagi personajlar — yuz emojilari (og'riq matni = mahsulot-frustatsiyasi, mavjud PAIRS)
  const FACES = { rasm: '🧑', qidiruv: '👩', oqildi: '🧔', tungi: '👵' };
  // Javcha (shelf): DORI-shishalar (og'riqni yopadigan feature) + SHIRINLIK-bankalar (bezak, og'riqsiz)
  const SHELF = [
    { id: 'rasm',    kind: 'dori',      label: 'Rasm yuklash' },
    { id: 'qidiruv', kind: 'dori',      label: 'Qidiruv' },
    { id: 'qor',     kind: 'shirinlik', label: 'Qor yog\'dirish' },
    { id: 'oqildi',  kind: 'dori',      label: '"O\'qildi" belgisi' },
    { id: 'logo3d',  kind: 'shirinlik', label: 'Logoni 3D aylantirish' },
    { id: 'tungi',   kind: 'dori',      label: 'Tungi rejim' },
  ];
  const [healed, setHealed] = useState(storedAnswer ? Object.fromEntries(PAIRS.map(p => [p.id, true])) : {});
  const [thanks, setThanks] = useState(false);        // joriy personaj davolanib «Rahmat!» deb ketmoqda
  const [wrongShake, setWrongShake] = useState(null); // noto'g'ri dori — shisha silkinib qaytadi
  const [candyId, setCandyId] = useState(null);       // shirinlik berildi — konfet eriydi, o'lchagich qimirlamaydi
  const reliefN = Object.keys(healed).length;
  const done = reliefN >= PAIRS.length; // 4 foydalanuvchi davolangach (eski matched >= PAIRS.length ekvivalenti)
  const queue = PAIRS.filter(p => !healed[p.id]);
  const cur = queue[0] || null;
  const give = (item) => {
    if (done || thanks || !cur) return;
    if (item.kind === 'shirinlik') { setCandyId(item.id); setTimeout(() => setCandyId(c => (c === item.id ? null : c)), 1100); return; } // og'riq joyida qoladi, urinish bloklanmaydi
    if (item.id === cur.id) {
      setThanks(true); setCandyId(null);
      setTimeout(() => { setHealed(h => ({ ...h, [cur.id]: true })); setThanks(false); }, 900); // pufak yashilga → personaj ketadi → o'lchagich +1
    } else {
      setWrongShake(item.id); setTimeout(() => setWrongShake(w => (w === item.id ? null : w)), 460);
    }
  };
  useEffect(() => { if (done && storedAnswer === undefined) onAnswer(screen, { correct: true, picked: true }); }, [done]);
  return (
    <Stage eyebrow="Dorixona" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} optionalLive label={done ? 'Davom etish' : `${reliefN}/${PAIRS.length} davolandi`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        <div className="head"><h2 className="title h-title fade-up">🏥 <span className="italic" style={{ color: T.accent }}>Dorixona</span>: har foydalanuvchiga og'rig'ini yopadigan dorini bering</h2></div>
        <Mentor>Navbatdagi foydalanuvchining boshi uzra <b style={{ color: T.accent }}>og'riq</b> ko'rinadi. Javchadan to'g'ri <b style={{ color: T.success }}>dori</b>ni (feature) tanlab bering. <b style={{ color: T.honey }}>Shirinlik</b> chiroyli, lekin og'riqni yopmaydi.</Mentor>
        <Zoomable>
        <div className="pharm">
          <div className="pharm-left">
            <DoriBot size={92} mood={thanks ? 'happy' : (wrongShake ? 'oops' : 'idle')} say={done ? 'Hammasi davolandi! 🎉' : (thanks ? 'Mana dori!' : 'Kim keyingi?')} />
            <div className="relief">
              <div className="relief-top"><span className="relief-lbl">💚 Yengillik o'lchagichi</span><span className="relief-n">{reliefN}/{PAIRS.length}</span></div>
              <div className="relief-bar">{PAIRS.map((p, i) => <span key={p.id} className={`relief-seg ${i < reliefN ? 'on' : ''}`} />)}</div>
            </div>
            <div className="queue">
              <span className="queue-lbl">Navbat:</span>
              {queue.length ? queue.map((p, i) => <span key={p.id} className={`queue-face ${i === 0 ? 'now' : ''}`}>{FACES[p.id]}</span>) : <span className="queue-empty">bo'sh 🎉</span>}
            </div>
          </div>
          <div className="pharm-stage">
            {cur ? (
              <div className="patient" key={cur.id}>
                <div className={`pain-bubble ${thanks ? 'relieved' : ''}`}>
                  {thanks ? <>😊 Rahmat! Og'rig'im ketdi.</> : cur.pain}
                  {candyId && !thanks && <span className="candy-note">Chiroyli ekan… lekin og'rig'im joyida.</span>}
                </div>
                <div className={`patient-av ${thanks ? 'leaving' : ''}`}>{FACES[cur.id]}</div>
              </div>
            ) : (
              <div className="patient done-state fade-step"><div className="pain-bubble relieved">Hammaga dori yetdi!</div><div className="patient-av">🙂</div></div>
            )}
          </div>
          <div className="shelf">
            <div className="shelf-lbl"><span>🧴 Javcha</span><span className="shelf-hint">dori vs shirinlik</span></div>
            <div className="shelf-grid">
              {SHELF.map(it => {
                const isDori = it.kind === 'dori';
                const shaking = wrongShake === it.id;
                const melting = candyId === it.id;
                return (
                  <button key={it.id} onClick={() => give(it)} disabled={done || thanks} className={`bottle ${isDori ? 'is-dori' : 'is-candy'} ${shaking ? 'shake-x' : ''} ${melting ? 'melt' : ''}`}>
                    <span className="bottle-ic">{isDori ? p4.pill(20) : p4.candy(20)}</span>
                    <span className="bottle-lbl">{it.label}</span>
                    <span className="bottle-kind">{isDori ? 'DORI' : 'SHIRINLIK'}</span>
                  </button>
                );
              })}
            </div>
        ...
        {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>Zo'r! Har og'riqqa aniq bir <b>dori</b> (feature) to'g'ri keldi. Shirinliklar hech kimni davolamadi — og'riq yo'q joyda feature behuda.</p></div>}
        </Zoomable>
      </div>
    </Stage>
  );
...
const Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  useEffect(() => { if (done && storedAnswer === undefined) onAnswer(screen, { correct: true, picked: true }); }, [done]);
  return (
    <Stage eyebrow="Tuzatish" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} optionalLive label={done ? 'Davom etish' : (found ? 'Endi tuzating' : 'Behuda feature\'ni toping')} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow="Feature-karta" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!allGood} optionalLive label={allGood ? 'Davom etish' : (allPicked ? 'Eng kuchli variantni tanlang' : 'Har qatordan tanlang')} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen12 = (props) => (
  <QuestionScreen {...props} scope="module-mikro" eyebrow="Mashq · 3-savol"
    ...
    options={['Sayt ancha chiroyliroq ko\'rinadi', 'Mahsulotni tezlashtiradi', 'Foydalanuvchilarga juda foydali', 'Kuch ketadi, lekin hech kim ishlatmaydi']} correctIdx={3}
    ...
    explainWrong={{ 0: 'Chiroyli bo\'lishi mumkin, lekin behuda. Resurs bekorga ketadi.', 1: 'Ko\'pincha sekinlashtiradi. Eng yomoni — keraksiz.', 2: 'Aksincha — og\'riqsiz feature foydasiz.', default: 'Og\'riqsiz feature — behuda mehnat, hech kim ishlatmaydi.' }} />
);
...
const Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow="Namuna" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} optionalLive label={done ? 'Endi navbat sizga →' : `${seen.size}/${CASE_ROWS.length} qatorni oching`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        <div className="head" style={{ display: 'flex', alignItems: 'center', gap: 12 }}><DoriBot size={46} /><h2 className="title h-title fade-up" style={{ margin: 0 }}>Tayyor mahsulot: har feature qaysi <span className="italic" style={{ color: T.accent }}>og'riqni</span> yopadi?</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {CASE_ROWS.map(s => { const open = seen.has(s.key); return (<button key={s.key} onClick={() => tap(s.key)} className={!open ? 'tap-hint-card' : ''} style={{ textAlign: 'left', cursor: 'pointer', border: 'none', borderRadius: 12, padding: '11px 14px', background: T.paper, display: 'flex', flexDirection: 'column', gap: 3, boxShadow: active === s.key ? `inset 0 0 0 2px ${s.color}, 0 8px 20px -8px ${s.color}44` : (open ? `inset 0 0 0 1px ${T.success}55` : `0 6px 16px -8px rgba(${T.shadowBase},0.16)`), transition: 'all 0.18s' }}><span style={{ display: 'flex', alignItems: 'center', gap: 9 }}><span style={{ color: s.color, display: 'inline-flex' }}>{s.ic}</span><span className="mono" style={{ fontSize: 11, fontWeight: 700, color: s.color, textTransform: 'uppercase' }}>{s.label}</span>{open && <span className="feat-pop" style={{ marginLeft: 'auto', color: T.success, display: 'inline-flex' }}>{Ico.check(13)}</span>}</span><span style={{ fontFamily: G, fontSize: 12.5, color: T.ink2 }}>og'riq: {s.pain}</span></button>); })}
            </div>
          </Col>
          <Col>
...
const Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = _gate.live; // jonli darsda: yakuniy ishni bajargani serverga «tugatdi» belgisi bo'lib boradi
  const isMentorLive = !!(live && live.mode === 'mentor'); // mentor proyektorda ko'rsatadi — to'ldirmasdan o'ta oladi
  ...
  useEffect(() => {
    if (passed && !prevPassed.current) {
      ...
      if (live) live.submitAnswer(screen, 's15', 0, true, 0);
      if (typeof window !== 'undefined' && window.innerWidth < 768 && workRef.current) { const el = workRef.current; setTimeout(() => { if (el) el.scrollIntoView({ behavior: 'smooth', block: 'end' }); }, 360); }
    }
  }, [passed]);
  ...
  return (
    <Stage eyebrow="Yakuniy ish" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!passed && !isMentorLive} label={(passed || isMentorLive) ? 'Davom etish' : `To'ldiring (${filled}/3)`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Q_LABELS = { 4: '1 — Feature asosi (og\'riq)', 6: '2 — Dori vs shirinlik', 10: '3 — Og\'riqqa mos feature', 13: '4 — Muammosiz yechim xatosi', 16: '5 — 3 og\'riq→yechim' };
...
const INLINE_KEYS = { s4: 2, s5b: 0, s9: 1, s12: 3, s15: -1 };
...
const QZ_BG_SHAPES = [
  { ch: "og'riq",    l: 5,  t: 10, s: 40, c: 'rgba(203,173,255,0.16)', d: 19, dl: 0 },
  { ch: '💊',        l: 86, t: 7,  s: 40, c: 'rgba(203,173,255,0.13)', d: 23, dl: 1.5 },
  { ch: 'dori',      l: 8,  t: 72, s: 42, c: 'rgba(255,110,70,0.15)',  d: 27, dl: 0.8 },
  { ch: 'shirinlik', l: 76, t: 68, s: 28, c: 'rgba(203,173,255,0.11)', d: 21, dl: 2.2 },
  { ch: 'yechim',    l: 44, t: 86, s: 32, c: 'rgba(120,235,175,0.13)', d: 25, dl: 1.1 },
  { ch: '→',         l: 66, t: 26, s: 46, c: 'rgba(80,200,255,0.14)',  d: 17, dl: 0.4 },
  { ch: 'feature',   l: 26, t: 34, s: 30, c: 'rgba(203,173,255,0.12)', d: 20, dl: 1.9 },
  { ch: 'muammo',    l: 55, t: 5,  s: 30, c: 'rgba(203,173,255,0.14)', d: 22, dl: 0.6 },
  { ch: '🍬',        l: 93, t: 42, s: 40, c: 'rgba(203,173,255,0.10)', d: 24, dl: 1.3 },
  { ch: 'natija',    l: 2,  t: 45, s: 28, c: 'rgba(203,173,255,0.11)', d: 26, dl: 2.6 },
];
const QUIZ_BANK = [
  { q: "Feature (funksiya) aslida nima?", opts: ["Chiroyli ko'rinish bezagi", "Og'riqqa beriladigan yechim", "Sahifadagi logo turi", "Ranglar sxemasi"], correct: 1 },
  { q: "«Dori» (painkiller) feature nima qiladi?", opts: ["Aniq og'riqni yopadi", "Sahifaga rang beradi", "Saytni sekinlashtiradi", "Faqat bezaydi"], correct: 0 },
  { q: "Quyidagidan qaysi biri «shirinlik» (keraksiz bezak)?", opts: ["Rasm yuklash imkoni", "Qidiruv funksiyasi", "Logoni 3D aylantirish", "«O'qildi ✓✓» belgisi"], correct: 2 },
  { q: "Og'riq butunlay yo'qolsa, feature nima bo'ladi?", opts: ["Yanada kerak bo'ladi", "Keraksiz bo'ladi", "O'zi o'chadi", "Qimmatlashadi"], correct: 1 },
  { q: "Instagram qaysi og'riqni yopadi?", opts: ["Uzoqdagi do'st bilan gaplashish", "Lahzani do'stlarga ko'rsatish", "Yangi mavzu o'rganish", "Ovqat buyurtma qilish"], correct: 1 },
  { q: "Xaridor «mahsulotni ko'rmasdan ishonmayman» deydi. Qaysi feature?", opts: ["Ko'proq reklama qo'shish", "Sahifa rangini o'zgartirish", "Mahsulot rasmini yuklash", "Kattaroq logo qo'yish"], correct: 2 },
  { q: "Telegramning asosiy og'riq→yechimi qaysi?", opts: ["Bepul tez xabar yuborish", "Cheksiz qisqa video oqimi", "Onlayn to'lov qilish", "Aylanuvchi 3D logo"], correct: 0 },
  { q: "«O'qildi ✓✓» belgisi qaysi og'riqni yopadi?", opts: ["Kechasi ko'z qamashadi", "Narx juda qimmat ko'rinadi", "Xabarim yetdimi — bilmayman", "Shrift xunuk ko'rinadi"], correct: 2 },
  { q: "Feature qanday tug'iladi — to'g'ri tartib?", opts: ["Feature → og'riq → savol → natija", "Bezak → feature → savol → og'riq", "Natija → savol → og'riq → feature", "Og'riq → savol → feature → natija"], correct: 3 },
  { q: "«Muammosiz yechim» nega yomon?", opts: ["Sayt ancha chiroyliroq ko'rinadi", "Mahsulotni tezlashtiradi", "Foydalanuvchilarga juda foydali", "Kuch ketadi, lekin hech kim ishlatmaydi"], correct: 3 },
  { q: "Tungi rejim (dark mode) qaysi og'riqni yopadi?", opts: ["Kechasi ekran ko'zni charchatadi", "Xabar do'stga yetmaydi", "Mahsulot rasmi ko'rinmaydi", "Kerakli narsa topolmaydi"], correct: 0 },
  { q: "Yaxshi PM feature qo'shishdan oldin nimani so'raydi?", opts: ["Chiroyli ko'rinadimi?", "Nechta animatsiya bor?", "Nechta rang ishlatilgan?", "Qaysi og'riqni yopadi?"], correct: 3 },
];
...
const QzBolt = ({ size = 72 }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" aria-hidden="true" className="qz-bolt">
    <defs><linearGradient id="qzbg4" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#FF8A3D" /><stop offset="1" stopColor="#FF4F28" /></linearGradient></defs>
    <rect x="6" y="6" width="88" height="88" rx="24" fill="url(#qzbg4)" />
...
function QzFX() {
  ...
  useEffect(() => {
    ...
    const TOK = ['og\'riq', 'feature', 'dori', 'shirinlik', 'yechim', '💊', 'muammo', '→'];
    const em = [], toks = [];
...
export default function PmLesson4({ lang: langProp, onFinished }) {
  ...
  // 🏅 Nishonlar — dars davomida haqiqiy solve/match bosqichlarida beriladi
  ...
  return (
    <LangContext.Provider value={lang}>
      <style>{`
        ...
        /* === 👆 TAP-HINT affordance (bosilmagan interaktiv elementlar «meni bos» deb chaqiradi) === */
        ...
        /* === 🏥 DORIXONA (Screen8 signature o'yin) + 🤖 DORIBOT + javcha === */
        .doribot { display: flex; flex-direction: column; align-items: center; gap: 6px; flex-shrink: 0; }
        .doribot svg { display: block; animation: db-bob 3s ease-in-out infinite; }
        @keyframes db-bob { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-4px); } }
        .db-spark { animation: db-spark 1.6s ease-in-out infinite; transform-origin: center; }
        @keyframes db-spark { 0%,100% { opacity: 0.5; } 50% { opacity: 1; } }
        .db-eye { animation: db-blink 3.4s ease-in-out infinite; transform-origin: center; }
        @keyframes db-blink { 0%,92%,100% { transform: scaleY(1); } 96% { transform: scaleY(0.15); } }
        .doribot-happy svg { animation: db-happy 0.6s ease; }
        @keyframes db-happy { 30% { transform: translateY(-8px) scale(1.05); } }
        .doribot-oops svg { animation: shake 0.42s; }
        .db-say { font-family: 'Manrope'; font-weight: 700; font-size: 12.5px; color: ${T.ink2}; background: ${T.paper}; border-radius: 99px; padding: 4px 12px; box-shadow: 0 5px 14px -7px rgba(${T.shadowBase},0.3); white-space: nowrap; }
        @media (prefers-reduced-motion: reduce) { .doribot svg, .db-spark, .db-eye { animation: none; } }
        .pharm { display: grid; grid-template-columns: minmax(150px,0.9fr) minmax(180px,1.2fr) minmax(200px,1.3fr); gap: clamp(10px,1.8vw,18px); align-items: start; }
        @media (max-width: 860px) { .pharm { grid-template-columns: 1fr; } }
        .pharm-left { display: flex; flex-direction: column; align-items: center; gap: 13px; }
        .relief { width: 100%; background: ${T.paper}; border-radius: 14px; padding: 11px 13px; box-shadow: 0 6px 16px -8px rgba(${T.shadowBase},0.16); }
        .relief-top { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 7px; }
        .relief-lbl { font-family: 'Manrope'; font-weight: 800; font-size: 12px; color: ${T.success}; }
        .relief-n { font-family: 'Manrope'; font-weight: 800; font-size: 13px; color: ${T.ink2}; font-variant-numeric: tabular-nums; }
        .relief-bar { display: flex; gap: 5px; }
        .relief-seg { flex: 1; height: 12px; border-radius: 6px; background: rgba(167,166,162,0.28); transition: background 0.4s, transform 0.4s; }
        .relief-seg.on { background: linear-gradient(90deg, #4FD08A, ${T.success}); transform: scaleY(1.08); box-shadow: 0 4px 12px -5px ${T.success}; }
        .queue { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; justify-content: center; }
        .queue-lbl { font-family: 'Manrope'; font-weight: 700; font-size: 12px; color: ${T.ink3}; }
        .queue-face { font-size: 22px; opacity: 0.55; transition: all 0.25s; filter: grayscale(0.4); }
        .queue-face.now { opacity: 1; filter: none; transform: scale(1.2); }
        .queue-empty { font-family: 'Manrope'; font-weight: 700; font-size: 13px; color: ${T.success}; }
        .pharm-stage { display: flex; flex-direction: column; align-items: center; justify-content: flex-start; gap: 12px; min-height: 190px; padding-top: 6px; }
        .patient { display: flex; flex-direction: column; align-items: center; gap: 10px; animation: patient-in 0.4s cubic-bezier(.2,.8,.3,1); }
        @keyframes patient-in { from { opacity: 0; transform: translateX(24px); } }
        .pain-bubble { position: relative; max-width: 260px; text-align: center; font-family: 'Manrope'; font-weight: 600; font-size: 13.5px; line-height: 1.4; color: ${T.accent}; background: ${T.accentSoft}; border: 1.5px solid ${T.accent}55; border-radius: 16px; padding: 11px 15px; box-shadow: 0 8px 22px -10px rgba(255,79,40,0.35); transition: all 0.45s; animation: pain-throb 1.8s ease-in-out infinite; }
        .pain-bubble::after { content: ''; position: absolute; bottom: -8px; left: 50%; transform: translateX(-50%); border: 8px solid transparent; border-top-color: ${T.accentSoft}; }
        @keyframes pain-throb { 0%,100% { box-shadow: 0 8px 22px -10px rgba(255,79,40,0.35); } 50% { box-shadow: 0 8px 30px -8px rgba(255,79,40,0.6); } }
        .pain-bubble.relieved { color: ${T.success}; background: ${T.successSoft}; border-color: ${T.success}66; animation: none; }
        .pain-bubble.relieved::after { border-top-color: ${T.successSoft}; }
        .candy-note { display: block; margin-top: 6px; font-family: 'Manrope'; font-weight: 600; font-size: 11.5px; font-style: italic; color: ${T.honey}; animation: fade-step 0.3s ease; }
        .patient-av { font-size: clamp(48px,9vw,64px); line-height: 1; filter: drop-shadow(0 8px 16px rgba(${T.shadowBase},0.22)); }
        .patient-av.leaving { animation: patient-leave 0.85s ease forwards; }
        @keyframes patient-leave { 40% { transform: translateY(-6px) scale(1.06); } 100% { transform: translateX(70px) scale(0.7); opacity: 0; } }
        @media (prefers-reduced-motion: reduce) { .patient, .patient-av.leaving, .pain-bubble { animation: none !important; } }
        .shelf { background: linear-gradient(180deg, ${T.paper}, ${T.bg}); border-radius: 16px; padding: 12px; box-shadow: inset 0 0 0 1px rgba(${T.shadowBase},0.08), 0 8px 22px -12px rgba(${T.shadowBase},0.2); }
        .shelf-lbl { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 10px; font-family: 'Manrope'; font-weight: 800; font-size: 13px; color: ${T.ink}; }
        .shelf-hint { font-weight: 600; font-size: 11px; color: ${T.ink3}; font-style: italic; }
        .shelf-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 9px; }
        .bottle { display: flex; flex-direction: column; align-items: center; gap: 4px; text-align: center; border: 1.5px solid transparent; border-radius: 13px; padding: 12px 8px 9px; cursor: pointer; transition: transform 0.15s, box-shadow 0.15s, background 0.2s; }
        .bottle.is-dori { background: ${T.successSoft}; border-color: ${T.success}44; color: ${T.success}; }
        .bottle.is-candy { background: ${T.honeySoft}; border-color: ${T.honey}44; color: ${T.honey}; }
        .bottle:hover:not(:disabled) { transform: translateY(-3px); box-shadow: 0 10px 22px -10px rgba(${T.shadowBase},0.3); }
        .bottle:disabled { opacity: 0.5; cursor: default; }
        .bottle-ic { display: inline-flex; }
        .bottle-lbl { font-family: 'Manrope'; font-weight: 700; font-size: 12.5px; color: ${T.ink}; line-height: 1.25; }
        .bottle-kind { font-family: 'Manrope'; font-weight: 800; font-size: 9px; letter-spacing: 0.08em; }
        .bottle.melt { animation: candy-melt 1.1s ease; }
        @keyframes candy-melt { 30% { transform: scale(0.9) translateY(4px); opacity: 0.7; } 60% { transform: scale(0.96) translateY(6px); filter: blur(1px); } 100% { transform: none; opacity: 1; filter: none; } }
        @media (prefers-reduced-motion: reduce) { .bottle.melt, .shake-x, .doribot-oops svg { animation-duration: 0.01s !important; } .relief-seg { transition: none !important; } }
        .shelf-tabs { display: flex; gap: 10px; flex-wrap: wrap; }
        .shelf-tab { flex: 1; min-width: 150px; display: inline-flex; align-items: center; gap: 7px; font-family: 'Manrope'; font-weight: 800; font-size: 13px; border-radius: 12px; padding: 10px 14px; }
        .shelf-tab.dori { background: ${T.successSoft}; color: ${T.success}; border: 1.5px solid ${T.success}44; }
        .shelf-tab.candy { background: ${T.honeySoft}; color: ${T.honey}; border: 1.5px solid ${T.honey}44; }
        .shelf-tab-sub { font-weight: 600; font-size: 11px; opacity: 0.8; margin-left: auto; }
        ...
        .acu-overlay { position: fixed; inset: 0; z-index: 11000; display: flex; align-items: center; justify-content: center; overflow: hidden; cursor: pointer;
          background: radial-gradient(60% 55% at 50% 42%, rgba(60,32,120,0.62) 0%, rgba(18,10,40,0.9) 62%, rgba(8,4,20,0.96) 100%); animation: acu-bg-in 0.3s ease-out; }
        .acu-tap { font-family: 'Manrope', sans-serif; font-weight: 600; font-size: 12px; letter-spacing: 0.14em; text-transform: uppercase; color: rgba(255,255,255,0.55); animation: acu-blink 1.6s ease-in-out infinite 0.8s; }
        @keyframes acu-bg-in { from { opacity: 0; } to { opacity: 1; } }
        @keyframes acu-bg-out { to { opacity: 0; } }
        .acu-rays { position: absolute; top: 50%; left: 50%; width: 170vmax; height: 170vmax; transform: translate(-50%,-50%); pointer-events: none;
          background: repeating-conic-gradient(from 0deg at 50% 50%, rgba(255,210,120,0.16) 0deg 6deg, transparent 6deg 14deg); animation: acu-spin 15s linear infinite, acu-fade 0.5s ease-out; }
        @keyframes acu-spin { to { transform: translate(-50%,-50%) rotate(360deg); } }
        @keyframes acu-fade { from { opacity: 0; } to { opacity: 1; } }
        .acu-glow { position: absolute; top: 42%; left: 50%; width: 78vmin; height: 78vmin; transform: translate(-50%,-50%); pointer-events: none; filter: blur(4px);
          background: radial-gradient(circle, rgba(255,201,77,0.5) 0%, rgba(255,140,60,0.22) 38%, transparent 68%); animation: acu-glow-pulse 2.4s ease-in-out infinite; }
        ...
        .acu-medal { position: relative; width: clamp(112px,26vw,152px); height: clamp(112px,26vw,152px); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: clamp(54px,13vw,74px); overflow: hidden;
          background: radial-gradient(circle at 38% 30%, #FFF4D6 0%, #FFD35A 40%, #F5A623 72%, #E2830E 100%); box-shadow: 0 0 0 6px rgba(255,255,255,0.14), 0 0 0 13px rgba(255,201,77,0.14), 0 18px 44px -10px rgba(245,166,35,0.7); animation: acu-medal-pop 0.7s cubic-bezier(.2,1.3,.4,1) both, acu-float 3s ease-in-out infinite 0.7s; }
        ...
        @media (prefers-reduced-motion: reduce) { .acu-rays, .acu-glow, .acu-medal, .acu-shine, .acu-spark, .acu-ring { animation-duration: 0.01s !important; animation-iteration-count: 1 !important; } }
