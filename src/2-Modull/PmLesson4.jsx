          ...
          <div className="kp-bet fade-step" key={`b${i}`}>
            ...
            <div className="kp-chips">
              {c.predict.chips.map((ch, k) => (
                <button key={k} className={`kp-chip${!isMentorK && betHint ? ' turn-ring' : ''}`} onClick={() => setBets(p => ({ ...p, [i]: k }))}>
...
const ScreenClean = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(() => Array.isArray(storedAnswer?.seen) ? storedAnswer.seen : []); // F-0914-10: saqlangan javob massiv bo'lmasa — bo'sh (oq ekran himoyasi)
  const [shelf, setShelf] = useState(() => Array.isArray(storedAnswer?.shelf) ? storedAnswer.shelf : []); // F-0914-10: saqlangan javob massiv bo'lmasa — bo'sh (oq ekran himoyasi)
...
// Tekshiruv mantig'i (`checkList`) AYNAN o'zgarmadi; u shartlarni bitta o'tishda
// hisoblaydi, shuning uchun natija oxirgi matn bo'yicha eslab qolinadi.
let _listSrc = null, _listRes = null;
const listOf = (html) => {
  if (html !== _listSrc) { _listSrc = html; _listRes = checkList(html || ''); }
  return _listRes;
};
const KOD_TASK = {
...
const ScreenCoding = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [st, setSt] = useState(() => {
    ...
    return { code: (typeof storedAnswer?.code === 'string' ? storedAnswer.code : null) || (saved && saved.code) || kodStarter(), done: !!(storedAnswer && storedAnswer.solved) || !!(saved && saved.done) }; // F-0914-10: saqlangan javob matn bo'lmasa — zaxira-zanjir (oq ekran himoyasi)
  });
  const { code, done } = st;
    ...
    <Stage eyebrow={tr({ uz: 'Koding · 🛠 kompilyator', ru: 'Кодинг · 🛠 компилятор' })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: 'Avval kompilyatorda yozing', ru: 'Сначала напишите в компиляторе' }} onClick={onNext} /></>}>
      ...
      {open && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: T.bg }}>
          <HtmlCompiler lang={__lang} task={KOD_TASK} starterCode={code || kodStarter()} storageKey={`${KODING_KEY}:code`}
...
const INLINE_KEYS = { s3: 2, s5: 1, s9: 1, s12: 0 }; // o'lik mashq-kalitlar olib tashlandi (jsx-lint, 2026-09-07): s8, s10, s11
...
export default function PmLesson4({ lang: langProp, onFinished, liveToken }) {
