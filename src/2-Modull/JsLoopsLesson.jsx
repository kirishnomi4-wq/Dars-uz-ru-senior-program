...
const Terminal = ({ lines, empty = { uz: '// natija shu yerda chiqadi…', ru: '// результат появится здесь…' }, title = 'console', maxH }) => (
  <div className="term">
    ...
    <div className="term-body" style={maxH ? { maxHeight: maxH, overflowY: 'auto' } : undefined}>
...
// whileMode → «Shart» dastagi «👀 Tekshiruvchi»ga aylanadi (F-0803-17: «datchik» —
...
const Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const listRef = useRef(null); // §34: ro'yxat ichki skroll — oxirgi xabar ko'rinsin
  useEffect(() => { const el = listRef.current; if (el) el.scrollTop = el.scrollHeight; }, [count]);
  const OPTS = [
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Kirish', ru: 'Введение' })} screen={screen} navContent={<NavNext optionalLive disabled={picked === null} label={tr({ uz: 'Davom etish', ru: 'Продолжить' })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr({ uz: <>30 ta do'stingizga bir xil xabarni <span className="italic" style={{ color: T.accent }}>bittalab</span> yozasizmi?</>, ru: <>Будете писать 30 друзьям одно и то же <span className="italic" style={{ color: T.accent }}>по одному</span>?</> })}</h1>
        ...
        <Split three>
          <Col>
            ...
            <div className="msg-list fade-up delay-1" ref={listRef} style={{ maxHeight: 96 }}>
              {count === 0 ? (
                <p style={{ color: T.ink3, fontStyle: 'italic', margin: 0, fontFamily: "'JetBrains Mono',monospace", fontFeatureSettings: '"liga" 0, "calt" 0', fontSize: 13 }}>{tr({ uz: '// hali bittasi ham yuborilmadi', ru: '// пока не отправлено ни одного' })}</p>
...
const Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const done = suv >= 100;
  useEffect(() => () => clearTimeout(timer.current), []);
  // 👣 Bir qadam: o'quvchi shartni O'ZI tekshirib, aylanishni qo'lda bosadi.
  // Sabab (F-0803-17): o'rganish shart YOLG'ON bo'lgan lahzani ko'rishdan keladi —
  // uni qo'lda bosib kelgan bola bu lahzani kutadi va payqaydi.
  const step = () => { if (running || suv >= 100) return; setSuv(v => Math.min(v + STEP, 100)); setIter(c => c + 1); };
  const again = () => { clearTimeout(timer.current); setRunning(false); setSuv(0); setIter(0); };
  const run = () => {
        ...
        <div className="split">
          <Col>
            ...
            <div className={`loop-count fade-up delay-2 ${iter ? 'bump' : ''}`} key={iter}>
              ...
              <span className="loop-count-l">{tr({ uz: 'aylanish bajarildi', ru: 'итераций выполнено' })}</span>
            </div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: <>✓ Idish to'ldi — sikl 5 marta ishladi. Biz sanamadik: <b>Tekshiruvchi</b> shart yolg'on bo'lgan lahzada o'zi to'xtatdi.</>, ru: <>✓ Сосуд полон — цикл сработал 5 раз. Мы не считали: <b>Проверяющий</b> сам остановил его в момент, когда условие стало ложным.</> })}</p></div>}
          </Col>
          <Col>
    ...
    <Stage eyebrow={tr({ uz: 'Aylanib chiqish', ru: 'Обход массива' })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: 'Avval ishga tushiring', ru: 'Сначала запустите' })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        ...
        <div className="split">
          <Col>
            <div className="codebox fade-up delay-1">
              <div><KW>let</KW> mevalar = [{ARR.map((it, i) => (
                <span key={i}><span className={`arr-lit${hi === i ? ' scan' : ''}`}><STR>"{it.n}"</STR></span>{i < N - 1 ? ', ' : ''}</span>
              ))}]</div>
        ...
        <div className="split">
          <Col>
            <div className="codebox fade-up delay-1" style={{ lineHeight: 1.7 }}>
              <div><KW>let</KW> dostlar = [<STR>"Ali"</STR>, <STR>"Laylo"</STR>, <STR>"Bobur"</STR>]</div>
...
// s15 ning ishi bitta: o'quvchi siklni O'ZI yozadi. Yozib bo'lgan lahzada kelajakdagi
// loyiha haqida qo'shimcha matn chiqarish g'alaba-lahzasini suyultiradi.

// F-0803-15: eski `s15` («namunani ko'chirib yozing» inputi) OLIB TASHLANDI.
// O'rniga 2 ta yengil praktika-kompilyator keldi (PRACTICE_AFTER: 6 va 15) — o'quvchi
// endi kodni ISHGA TUSHIRIB, natijasini 🖥️ Console panelida o'z ko'zi bilan ko'radi.
...
const Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tayyor', ru: 'Готово' })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: 'clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Qaytadan', ru: 'Заново' })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: 'auto', padding: 'clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Yakunlash', ru: 'Завершить' })} ✓</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: 'Dars tugadi', ru: 'Урок завершён' })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>takrorlashni</span> kompyuterga topshirasiz.</>, ru: <>Теперь <span className="italic" style={{ color: T.accent }}>повторение</span> вы поручаете компьютеру.</> })}</h2>{/* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */}</div><ScoreRing correct={correct} total={total} /></div>
...
const INLINE_KEYS = { s4: 1, s5b: 0, s9: 1, s12: 1 }; // F-0803-15: s15 olib tashlandi (o'rnida praktika-kompilyator)
...
// ============================================================
//  ✏️ PRAKTIKA — 2 ta YENGIL topshiriq (F-0803-15).
//  Yengil variant (JsVars naqshi): BITTA fayl (`script.js`), HTML/DOM yo'q,
//  natija 🖥️ Console panelida ko'rinadi, har topshiriqda 2 ta shart.
//  Bu ikkovi eski `s15` («namunani ko'chirib yozing» inputi) O'RNIGA keldi:
//  o'quvchi endi kodni ISHGA TUSHIRIB, natijasini o'z ko'zi bilan ko'radi.
//  Pedagogik tartib (527-qonun): har topshiriq faqat o'sha ekrangacha
//  o'tilgan tushunchani so'raydi.
// ============================================================

// — P1: 1 DAN 5 GACHA SANASH (Screen5b — for testidan — keyin) —
const TASK_SANOQ = {
  ...
  requirements: [
    { id: 'for', label: { uz: 'for sikli yozildi', ru: 'цикл for написан' }, check: C.js(/for\s*\([^)]*;[^)]*;[^)]*\)/, { uz: "`for (let i = 1; i <= 5; i++) {` deb boshlang", ru: 'Начните с `for (let i = 1; i <= 5; i++) {`' }) },
    { id: 'log', label: { uz: 'konsolda 1 2 3 4 5', ru: 'в консоли 1 2 3 4 5' }, check: C.logs('1 2 3 4 5', { uz: "Sikl ichida `console.log(i)` yozing — har qadamda bitta son chiqadi", ru: 'Внутри цикла напишите `console.log(i)` — на каждом шаге выводится число' }) },
  ],
};

// — P2: MASSIVNI SIKL BILAN O'QISH (Screen14 — debugging — dan keyin, YAKUNIY) —
const TASK_MEVA = {
  ...
  requirements: [
    { id: 'arr', label: { uz: 'massiv yozildi', ru: 'массив написан' }, check: C.js(/\[\s*["'][^"']+["']\s*,/, { uz: "Massivni shunday yozing: `let mevalar = [\"olma\", \"banan\", \"uzum\"]`", ru: 'Напишите массив так: `let mevalar = [\"olma\", \"banan\", \"uzum\"]`' }) },
    { id: 'log', label: { uz: 'konsolda uchala meva', ru: 'в консоли все три фрукта' }, check: C.logs('olma banan uzum', { uz: "Sikl ichida `console.log(mevalar[i])` yozing", ru: 'Внутри цикла напишите `console.log(mevalar[i])`' }) },
  ],
};

// Praktika handoff xaritasi: shu ekran INDEKSIDAN keyin qaysi praktika chaqiriladi.
// (indekslar `screens[]` bo'yicha — s15 olib tashlangandan KEYINGI holat: 6 = s5b, 15 = s14)
const PRACTICE_AFTER = {
  6:  { task: TASK_SANOQ, starter: '' }, // 1) for bilan 1..5 sanash
  15: { task: TASK_MEVA,  starter: '' }, // 2) yakuniy: massivni sikl bilan o'qish
};

export default function JsLoopsLesson({ lang: langProp, onFinished, onPractice, liveToken }) {
  ...
  const runPractice = (entry, fromScreen) => {
    const done = () => {
      ...
      earn('assemblymaster'); // 🏅 F-0803-15: siklni o'z qo'li bilan yozib, ishga tushirdi
  ...
  // 🏠 UYGA VAZIFA PRAKTIKASI (yakun-sahifadagi tugma) — yakuniy topshiriq (TASK_MEVA).
  ...
  return (
    <LangContext.Provider value={lang}>
      <style>{`
        ...
        /* Kod ichidagi massiv elementi — sikl o'qiyotganda karta bilan BIRGA yonadi (F-0922-75) */
        .arr-lit { border-radius: 4px; padding: 0 2px; transition: background 0.18s ease, box-shadow 0.18s ease; }
        .arr-lit.scan { background: rgba(255,79,40,0.22); box-shadow: inset 0 0 0 1.5px ${T.accent}; }
        ...
        .glass-pct { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 18px; color: ${T.ink}; mix-blend-mode: difference; filter: invert(1); }
        ...
        .zv-tablo-lbl { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: 10px; color: #7DD181; letter-spacing: 0.16em; opacity: 0.85; }
        .zv-tablo-num { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 34px; line-height: 1; color: #7DD181; text-shadow: 0 0 14px rgba(125,209,129,0.7); animation: zv-flip 0.3s cubic-bezier(.34,1.4,.4,1); }
        ...
        .zv-belt-arrow { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; color: ${T.ink3}; font-size: 15px; transition: color 0.3s; animation: zv-arrow-flow 1.05s linear infinite; }
        ...
        .zv-bin-empty { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: 12px; color: ${T.ink3}; font-style: italic; }
        .zv-box { position: relative; display: inline-flex; align-items: center; gap: 3px; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: 12px; font-weight: 700; color: ${T.ink}; background: ${T.paper}; border-radius: 8px; padding: 4px 8px; box-shadow: 0 3px 8px -3px rgba(${T.shadowBase},0.25); transform-origin: top center; animation: zv-box-drop 0.5s cubic-bezier(.34,1.56,.5,1) both; }
        ...
        .mini-cell { width: 27px; height: 27px; border-radius: 7px; display: flex; align-items: center; justify-content: center; font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 12px; color: ${T.ink2}; background: ${T.bg}; animation: cellwave 2.4s ease-in-out infinite; }
        ...
        .stair-n { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 10px; font-weight: 700; color: ${T.ink3}; }
        ...
        .splash { position: absolute; top: 10px; left: 50%; font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 15px; color: ${T.blue}; animation: floatup 0.72s ease-out; }
        @keyframes floatup { from { opacity: 1; transform: translate(-50%, 8px);} to { opacity: 0; transform: translate(-50%, -24px);} }
        .cond-pill { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 12px; font-weight: 700; padding: 6px 13px; border-radius: 99px; transition: all 0.3s ease; }
        ...
        .loop-count-n { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 800; font-size: clamp(28px,4.2vw,40px); line-height: 1; color: ${T.accent}; font-variant-numeric: tabular-nums; }
