...
// ============================================================
// PM M4B-D2 — UYGA VAZIFA: «NOSOZLIK-KARTALARI DAVOM ETADI» (PmLesson16 davomi)
// Darsning To'liq-kapsulasi (HW_STEPS.toliq AYNAN): «① o'zingiz ishlatadigan ilovada uchragan
// nosozlikni eslang ② kartaga yozing: nima bosildi — nima bo'ldi, kimda, nima bo'ladi
// ③ javonini belgilang va sababini bir gap bilan yozing» — vazifa AYNAN shu.
// 4 bosqich · mezon: TO'RTTALASI bajarilsa «Bajarildi» (HW_PASS_MIN = 4).
//   1) Kartalarim — darsdagi 3 karta (avto: `pm-m4b2-sifat`)
//   2) Yangi karta — o'z ilovasidagi nosozlik
//   3) Javon — yangi kartaning javoni (hukmdan O'ZI chiqadi — moslik shart) + sabab
//   4) Xulosa — 3 savol (Kahoot)
// Validatorlar DARSDAN AYNAN: BOSH_RE (savol, bloklamaydi) · HARAKAT_RE+NATIJA_RE (maslahat) ·
// normS-takror · javon-hukm mosligi (hammada+to'xtaydi→hozir · ba'zilarda+noqulay→keyin · qolgani→bugun).
// Senariy: pm-senariylar/M4B-D2-Sifat-UY.md · Naqsh: PmLesson2.homework (ETALON).
...
const uzunOk = (s) => (s || '').trim().length >= NIMA_MIN;
const boshMi = (s) => uzunOk(s) && anyRe(BOSH_RE, (s || '').trim());
const faktMi = (s) => uzunOk(s) && !boshMi(s) && anyRe(HARAKAT_RE, s) && anyRe(NATIJA_RE, s);
...
// Darsdagi hukm-qoida AYNAN: javon hukmdan O'ZI chiqadi, o'quvchi tartib tanlamaydi.
const javonOf = (kimda, oqibat) => {
...
const kartaFull = (r) => uzunOk((r || {}).nima) && !!(r || {}).kimda && !!(r || {}).oqibat;
const nimaTakror = (list, i) => {
...
// Bitta karta-blok (1- va 2-bosqichda). BOSH_RE/fakt — darsdagidek MASLAHAT, bloklamaydi.
const KartaBlock = ({ r, setR, pulse, phNima, takror }) => {
    ...
    <>
      ...
      <div className="krow">
        ...
        <div className="chips" role="radiogroup">
          {KIMDA_OPTS.map(o => (
            <button key={o.v} type="button" role="radio" aria-checked={r.kimda === o.v} className={`chip sm ${r.kimda === o.v ? 'on' : ''}`} onClick={() => setR({ ...r, kimda: o.v })}>{tr(o.t)}</button>
          ))}
        </div>
      </div>
      <div className="krow">
        ...
        <div className="chips" role="radiogroup">
          {OQIBAT_OPTS.map(o => (
            <button key={o.v} type="button" role="radio" aria-checked={r.oqibat === o.v} className={`chip sm ${r.oqibat === o.v ? 'on' : ''}`} onClick={() => setR({ ...r, oqibat: o.v })}>{tr(o.t)}</button>
          ))}
        </div>
      </div>
    </>
...
// — 3-BOSQICH: javon —
const StageJavon = ({ data, setData, yangi }) => {
  const y = yangi || {};
  ...
  return (
    <div className="col">
      ...
      {!ready ? (
        <div className="frame-warn fade-up d1"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: 'Avval 2-bosqichda yangi kartani tugating — javon o\'sha kartaga qo\'yiladi.', ru: 'Сначала завершите новую карточку на 2-м этапе — полка ставится этой карточке.' })}</p></div>
      ) : (
        <div className="frame fade-up d1">
          ...
          <div className="chips" role="radiogroup">
            {JAVONLAR.map(j => (
              <button key={j.k} type="button" role="radio" aria-checked={pick === j.k} className={`chip ${pick === j.k ? 'on' : ''}`} onClick={() => setData({ ...data, pick: j.k })}>
...
// ILDIZ-KOMPONENT
// ============================================================
export default function PmLesson16Homework({ lang: langProp, onFinished }) {
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult
            ...
            : cur.key === 'javon' ? <StageJavon data={data.javon || {}} setData={setStageData('javon')} yangi={data.yangi} />
