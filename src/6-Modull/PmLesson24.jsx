...
function LiveBigCode({ pin, onClose }) {
  ...
  return (
    <div style={overlay}>
      ...
      <p style={{ color: '#fff', opacity: 0.85, fontSize: 'clamp(15px,2.2vw,22px)', maxWidth: 640, margin: 'clamp(20px,4vw,36px) 0 0', lineHeight: 1.5 }}>Shu darsni o'z qurilmangizda oching → <b style={{ color: '#fff' }}>«Darsga qo'shilish»</b> oynasida kodni va ismingizni kiriting.</p>
...
const Screen4 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [bosilgan, setBosilgan] = useState(() => (storedAnswer && storedAnswer.bosilgan) || (saved && saved.bosilgan) || []);
  const [qaror, setQaror] = useState(() => (storedAnswer && storedAnswer.qaror) || (saved && saved.qaror) || null);
...
const Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === 'mentor');
  const [i, setI] = useState(() => (storedAnswer && storedAnswer.qoyilgan ? Math.min(storedAnswer.qoyilgan.length, ISHLAR9.length - 1) : 0));
  const [qoyilgan, setQoyilgan] = useState(() => (storedAnswer && storedAnswer.qoyilgan) || []);
