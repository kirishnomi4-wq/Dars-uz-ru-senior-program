...
// ============================================================
// PM · M6-D12 — BUGUN QAYSI ISH BOSHLANADI?
// Senariy-manba: pm-senariylar/M6-D12-Roadmap.md (AVTO-GATE S yopilgan, 2026-08-19).
// Misol-ip: sartaroshxonaga navbat oladigan ilova — demo-tizim (s0/s1/s2/s4/s9/s10);
//   o'quvchining O'Z tizimi s8 va uy-vazifada (91/95/96c/108-qonun).
// Imzo-vizual: UCH UFQ YO'LI (oltita ish bugun boshlab ko'riladi, yo'l o'zi quriladi).
// Bosh keys: K17 · Tesla — «uch ufq: bir varaqdagi uzoq reja» (raqamsiz bank).
// Kirish-artefakt: pm-m6d6-chegara.chegaralar (jim zaxira — topilmasa panel chiqmaydi).
// Chiqish-artefakt: pm-m6d12-yol { ufqlar: [{ufq, ish} x3], savedAt }.
// INFRA MANBAI: src/4-Modull/PmLesson12.jsx (P0 zanjiri: Stage, QuestionScreen,
//   MentorTestStats, RecapOverlay, PairTimer, ScreenPodium, CodeStrike-arena, nishonlar)
//   + src/3-Modull/PmLesson9.jsx s6 (uzluksiz keys-hisoblagichi).
// KODING: VS Code-topshirig'i (26-qonun R1 navbati) — reja.js, kod nusxalanmaydi.
...
import { useLiveSession, useServerProgress, LiveGateCtx, LiveGate, LiveBadge, LIVE_ENABLED, liveGet, liveRead, progRead, progWrite, progClear, livePlayers, liveAnswers, liveQuizAnswers , buildResultDetails } from '../live/index.js';
...
const SCREEN_META = [
  ...
  { id: 's4',  type: 'exploration', template: 'custom', scored: false, scope: null },          // 4  · YADRO: uch ufq yo'li
  { id: 's5',  type: 'test',        template: 'custom', scored: true,  scope: 'module-mikro' },// 5  · TEST-2
  { id: 's6',  type: 'case',        template: 'custom', scored: false, scope: null },          // 6  · K17 Tesla
  ...
  { id: 's9',  type: 'practice',    template: 'custom', scored: false, scope: null },          // 9  · BLOK 5 ufq-joylash
...
export const SCREEN_INTENTS = {
  ...
  s11: "Bola ishni ufqqa kerak narsaning payti qo'yishini tanlaydi",
  s12: "Bola uch qatorini yoddan aytadi va bir qatorda yozib qoldiradi",
...
const INLINE_KEYS = { s3: 1, s5: 2, s7: 0, s11: 1, yol: -1, reja: -1, joylash: -1, koding: -1 };
...
// ============================================================
// 🎒 DARS MA'LUMOTLARI — sartaroshxonaga navbat oladigan ilova (bitta misol-ip, 108-qonun)
// ============================================================
...
// ===== SCREEN 4 — YADRO: UCH UFQ YO'LI (markaziy mexanika) =====
// 1-bosqich: har ishning ▶ tugmasi bosiladi — belgi VA bitta fakt-qator darhol chiqadi (106d/71),
// to'xtagan ish o'zi kutgan narsa tayyor bo'ladigan ufqqa borib turadi.
// 2-bosqich: yo'l qurilgach ko'chirish-qarori ochiladi (94-qonun progressiv ochilish).
// 🔴 Yo'l o'quvchi BOSGAN ishlardan quriladi (korpus §139): oldindan yozilgan natija yo'q.
const YOL_KEY = 'pm-m6d12-yol-holat';
...
const Screen4 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === 'mentor');
  const [saved] = useState(() => readYolHolat());
  const [bosilgan, setBosilgan] = useState(() => Array.isArray(storedAnswer?.bosilgan) ? storedAnswer.bosilgan : ((saved && saved.bosilgan) || [])); // F-0914-10: saqlangan javob massiv bo'lmasa — bo'sh (oq ekran himoyasi)
  const [qaror, setQaror] = useState(() => (typeof storedAnswer?.qaror === 'string' ? storedAnswer.qaror : null) || (saved && saved.qaror) || null); // F-0914-10: saqlangan javob matn bo'lmasa — zaxira-zanjir (oq ekran himoyasi)
  const [just, setJust] = useState(null);
  const [idle, setIdle] = useState(0);
  const justT = useRef(null);
  const hammasi = bosilgan.length === ISHLAR.length;
  const done = hammasi && !!qaror;
  useEffect(() => () => clearTimeout(justT.current), []);
  useEffect(() => {
    if (hammasi || isMentor) return;
    const t = setInterval(() => setIdle(s => s + 1), 1000);
    return () => clearInterval(t);
  }, [hammasi, isMentor, bosilgan.length]);
  useEffect(() => { try { localStorage.setItem(YOL_KEY, JSON.stringify({ bosilgan, qaror })); } catch {} }, [bosilgan, qaror]);
  useEffect(() => {
    if (!done) return;
    if (storedAnswer === undefined || !storedAnswer.solved) {
      onAnswer(screen, { stage: 'yol', screenIdx: screen, bosilgan, qaror, solved: true, correct: true });
      if (live && live.mode === 'student') live.submitAnswer(PRACTICE_BASE + screen, 'yol', 0, true, 0);
    }
  }, [done]); // eslint-disable-line
  const boshla = (ish) => {
    if (isMentor || bosilgan.includes(ish.id)) return;
    setBosilgan(p => [...p, ish.id]);
    setIdle(0);
    setJust({ id: ish.id, ok: ish.ok, fakt: ish.fakt });
    clearTimeout(justT.current);
    justT.current = setTimeout(() => setJust(null), 7000);
  };
  const qolgan = ISHLAR.filter(i => !bosilgan.includes(i.id));
...
// ===== SCREEN 6 — KEYS: K17 · Tesla (4 bosqich-kartasi + 2 bashorat, 33/56/91b-qonun qolipi) =====
// 🔴 Bank «raqamsiz»: yagona sonlar — 2006 va «o'n yildan ortiq», ikkalasi ham bank matnidan.
...
const Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  // Nuqta faqat ALLAQACHON ko'rilgan bosqichga yo'l beradi: bashoratdan oldin javob-kartasiga
...
// ===== KIRISH-ARTEFAKT: m6-06 uch chegarasi (jim zaxira — korpus §69) =====
// BOR bo'lsa panel ochiladi va uchala qaror qatori ko'rinadi; YO'Q bo'lsa panel umuman
// chiqmaydi va ekran u haqda hech narsa demaydi. Panel faqat O'QIYDI.
const IN_KEY = 'pm-m6d6-chegara';
const readChegaralar = () => {
  try {
    const d = JSON.parse(localStorage.getItem(IN_KEY) || 'null');
    if (!d || typeof d !== 'object' || !Array.isArray(d.chegaralar)) return null;
    const qatorlar = d.chegaralar
      .map(c => (typeof c === 'string' ? c : (c && typeof c.qaror === 'string' ? c.qaror : '')))
      .map(s => s.trim()).filter(s => s.length >= 2);
    return qatorlar.length >= 3 ? qatorlar.slice(0, 3) : null;
  } catch { return null; }
};
...
const ishOxshash = (a, b) => {
  ...
  if (A.length === 0) return normIsh(a) === normIsh(b);
...
const Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === 'mentor');
  const [chegaralar] = useState(() => readChegaralar());
  const [list, setList] = useState(() => (storedAnswer && Array.isArray(storedAnswer.ufqlar)) ? storedAnswer.ufqlar : []);
  ...
  const cur = UFQLAR[idx];
  const uzun = draft.trim().length >= 8;
  const bosh = uzun && normIsh(draft).split(' ').every(w => BOSH_SET.includes(w));
  const kutadi = uzun && idx === 0 && KUTISH_RE.test(draft);
  const takror = uzun && list.some((r, k) => k !== edit && ishOxshash(draft, r.ish));
  // Uchinchi karta: uchala ish ham hech narsa kutmasa — yumshoq muvozanat-hinti (bloklamaydi).
  const hammasiBugun = uzun && idx === 2 && list.length === 2 && !KUTISH_RE.test(list[1].ish) && !KUTISH_RE.test(draft);
  ...
  useEffect(() => {
    ...
    if (storedAnswer === undefined || !storedAnswer.solved) {
      onAnswer(screen, { stage: 'reja', screenIdx: screen, ufqlar: list.slice(0, 3), solved: true, correct: true });
      if (live && live.mode === 'student') live.submitAnswer(PRACTICE_BASE + screen, 'reja', 0, true, 0);
    }
  }, [done]); // eslint-disable-line
  useEffect(() => {
    if (!done) return;
    const payload = { ufqlar: list.slice(0, 3).map(r => ({ ufq: r.ufq, ish: r.ish })), savedAt: Date.now() };
    try { localStorage.setItem(OUT_KEY, JSON.stringify(payload)); } catch {}
  }, [list, done]);
  const save = () => {
    if (!canSave) return;
    const yozuv = { ufq: UFQLAR[idx].k, ish: draft.trim() };
    const nx = edit === null ? [...list, yozuv] : list.map((r, k) => (k === edit ? yozuv : r));
    setList(nx);
    // Yarim yozilgan holat «Orqaga»da yo'qolmasin: har saqlashda dars-holatiga yoziladi.
    if (nx.length < 3) onAnswer(screen, { stage: 'reja', screenIdx: screen, ufqlar: nx, solved: false });
...
// ===== SCREEN 9 — TEKSHIRUV: UFQ-JOYLASH (26/59-qonun: yangi mexanika) =====
// Timeline (M3-D10) tartib quradi, ikki o'qli doska (M3-D5) katak-kesishmasi qidiradi.
// Bu yerda o'q BITTA: «buni bugun boshlab bo'ladimi?» — oltita MUSTAQIL ish, bitta ufqqa
...
const Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === 'mentor');
  const [i, setI] = useState(() => (Array.isArray(storedAnswer?.qoyilgan) ? Math.min(storedAnswer.qoyilgan.length, ISHLAR9.length - 1) : 0)); // F-0914-10: saqlangan javob massiv bo'lmasa — bo'sh (oq ekran himoyasi)
  const [qoyilgan, setQoyilgan] = useState(() => Array.isArray(storedAnswer?.qoyilgan) ? storedAnswer.qoyilgan : []); // F-0914-10: saqlangan javob massiv bo'lmasa — bo'sh (oq ekran himoyasi)
  const [pick, setPick] = useState(null);
  const [xatoBor, setXatoBor] = useState(false);
  const [yordamOpen, setYordamOpen] = useState(false);
  const [yakun, setYakun] = useState(() => !!(storedAnswer && storedAnswer.solved));
  const cur = ISHLAR9[i];
  const hammasi = qoyilgan.length === ISHLAR9.length;
  // Oxirgi ishning sabab-qatori ham o'qilsin: yo'l faqat o'quvchi tugmani bosgach ochiladi.
  const done = hammasi && yakun;
  useEffect(() => {
    if (done && (storedAnswer === undefined || !storedAnswer.solved)) {
      onAnswer(screen, { stage: 'joylash', screenIdx: screen, qoyilgan, solved: true, correct: true });
      if (live && live.mode === 'student') live.submitAnswer(PRACTICE_BASE + screen, 'joylash', 0, true, 0);
    }
  }, [done]); // eslint-disable-line
  const qoy = (k) => {
  ...
  const javob = pick !== null;
  const togri = javob && pick === cur.ufq;
...
// Kod NUSXALANMAYDI (82d): qo'lda yozganda o'rganiladi. Natija terminalda ko'rinadi (82b).
// 87-qonun: massiv, obyekt, `for`, `.length`, matn qo'shish `+`, `console.log` — M2 materiali;
// `node fayl.js` — M4 da o'tilgan. O'tilmagan hech narsa so'ralmaydi.
const KODING_KEY = 'pm-m6d12-code';
...
const jsHl = (ln) => ln.split(JS_TOKEN).filter(p => p !== undefined && p !== '').map((p, i) => {
  ...
  if (/^(const|let|for|return)$/.test(p)) return <span key={i} style={{ color: '#C586C0' }}>{p}</span>;
...
const REFLECT_KEY = 'pm-m6d12-reflection';
...
// hero (h-sub YO'Q) -> asosiy fikr -> CodeStrike -> («Endi siz bilasiz» + nishonlar) -> uy-vazifa kapsulasi.
const ScreenSummary = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  // 77-qonun (tekshiruvchi topilmasi, M6-D12): kapsula bosilganda topshiriq-karta yakun
  // sahifasini uzaytirib yuborardi (+439/+439/+436 skroll) va karta ekran ostida qolardi.
  // Yechim tuzilmaviy: karta sahifaga qo'shilmaydi — RecapOverlay naqshidagi to'liq-ekran
  // qatlamida ochiladi. Sahifa uzunligi o'zgarmaydi => skroll 0.
  useEffect(() => {
    if (!hwOpen) return;
...
const CSS_BASE = `
  ...
  /* Reset: <p> uchun FAQAT margin nollanadi. Ilgari padding ham nollanardi va u element-selektor
     bo'lgani uchun klass-paddinglarni (.jres/.sfb/.bhint/.cmt-tip) yeb qo'yardi —
...
// Dars-vizuallari: uch ufq yo'li (imzo-vizual), yozish-ekrani, ufq-joylash, VS Code paneli.
const CSS_LESSON = `
  ...
  .ibtn { display: flex; flex-direction: column; align-items: flex-start; gap: 6px; text-align: left; background: ${T.paper}; border: none; border-radius: 12px; padding: 10px 12px; cursor: pointer; font-family: 'Manrope', sans-serif; box-shadow: inset 0 0 0 1.5px ${T.line}, 0 6px 16px -8px rgba(${T.shadowBase},0.18); transition: transform 0.14s, box-shadow 0.16s; min-width: 0; animation: fade-in-up 0.3s ease-out both; }
  .ibtn:hover:not(:disabled) { transform: translateY(-2px); box-shadow: inset 0 0 0 1.5px ${T.accent}66, 0 10px 22px -8px rgba(${T.shadowBase},0.26); }
  .ibtn:focus-visible { outline: none; box-shadow: inset 0 0 0 2px ${T.accent}; }
  .ibtn:active:not(:disabled) { transform: translateY(0) scale(0.98); }
  .ibtn:disabled { cursor: default; }
  .ibtn-ic { font-size: 20px; line-height: 1; }
  .ibtn-t { font-weight: 700; font-size: clamp(12.5px,1.5vw,14px); color: ${T.ink}; line-height: 1.35; min-width: 0; overflow-wrap: anywhere; }
  .ibtn-go { font-weight: 800; font-size: 11.5px; color: ${T.accent}; background: ${T.accentSoft}; border-radius: 99px; padding: 4px 10px; }
  @media (prefers-reduced-motion: reduce) { .itray, .ibtn { animation: none; transition: none; } .ibtn:hover:not(:disabled), .ibtn:active:not(:disabled) { transform: none; } }
  /* 🔴 UCH UFQ HUDUDI — bu «bekat chizig'i» EMAS (mexanika-chegarasi: M3-D10 Timeline'dan farq):
     hududlarni bog'laydigan chiziq yo'q, hudud ichida tartib yo'q, chipda raqam yo'q — uch alohida maydon.
     §134 (rang-ko'rlik): rangdan tashqari SHAKL ham farqlaydi — belgi (doira · kvadrat · olti burchak),
     hoshiya (to'liq · uzuq · nuqtali) va maydon-naqshi (tekis · nuqta · qiya chiziq).
  ...
  .road .stop-n { grid-area: num; justify-self: end; font-family: 'JetBrains Mono', monospace; font-size: 11px; font-weight: 700; color: ${T.ink3}; }
  ...
  .mvq-b:hover:not(:disabled) { box-shadow: inset 0 0 0 1.5px ${T.accent}66; }
  .mvq-b.on { background: ${T.accentSoft}; color: ${T.accent}; box-shadow: inset 0 0 0 2px ${T.accent}; }
  .mvq-res { margin: 0; font-family: 'Manrope'; font-weight: 600; font-size: clamp(12.5px,1.5vw,13.5px); line-height: 1.5; color: ${T.ink2}; background: ${T.bg}; border-radius: 10px; padding: 9px 12px; min-width: 0; overflow-wrap: anywhere; animation: fade-step 0.3s ease-out; }
  @media (prefers-reduced-motion: reduce) { .mvq-b { transition: none; } .mvq-res { animation: none; } }
  ...
  .uj-card { position: relative; display: flex; flex-direction: column; align-items: flex-start; gap: 9px; background: ${T.paper}; border-radius: 16px; padding: clamp(13px,2vw,18px); box-shadow: 0 16px 34px -16px rgba(${T.shadowBase},0.28), inset 0 0 0 2px ${T.accent}44; min-width: 0; }
  .uj-n { position: absolute; top: 10px; right: 14px; font-size: 11.5px; font-weight: 700; color: ${T.ink3}; }
  .uj-ic { font-size: clamp(26px,3.6vw,34px); line-height: 1; }
  .uj-t { font-family: 'Manrope'; font-weight: 800; font-size: clamp(14px,1.9vw,17px); line-height: 1.35; color: ${T.ink}; min-width: 0; overflow-wrap: anywhere; }
  .uj-btns { display: flex; gap: 8px; flex-wrap: wrap; }
  /* Uch tugma ham yo'l bilan BIR KODDA: hoshiya-uslubi ufqni rangsiz ham aytadi (§134) */
  .uj-b { font-family: 'Manrope'; font-weight: 700; font-size: clamp(12.5px,1.5vw,14px); color: ${T.ink}; background: ${T.bg}; border: 2px solid ${T.line}; border-radius: 11px; padding: 8px 13px; cursor: pointer; transition: transform 0.14s, border-color 0.16s, background 0.16s, color 0.16s; min-width: 0; overflow-wrap: anywhere; }
  .uj-b.hozir { border-style: solid; }
  .uj-b.uch-oy { border-style: dashed; }
  .uj-b.olti-oy { border-style: dotted; }
  .uj-b:hover:not(:disabled) { transform: translateY(-1px); border-color: ${T.accent}66; }
  .uj-b:active:not(:disabled) { transform: translateY(0) scale(0.98); }
  .uj-b.hozir.on { background: ${T.successSoft}; color: ${T.success}; border-color: ${T.success}; }
  .uj-b.uch-oy.on { background: #FBEED6; color: #8F5F12; border-color: #E8A13A; }
  .uj-b.olti-oy.on { background: ${T.blueSoft}; color: ${T.blue}; border-color: ${T.blue}; }
  .uj-ans { display: flex; flex-direction: column; gap: 8px; align-items: flex-start; width: 100%; min-width: 0; }
  /* Asl javob ham o'z ufqining kodida chiqadi — bola rangni belgiga bog'lab qo'yadi */
  .uj-real { font-family: 'Manrope'; font-weight: 800; font-size: clamp(12.5px,1.5vw,14px); color: ${T.ink}; background: ${T.bg}; border: 2px solid ${T.line}; border-radius: 99px; padding: 5px 13px; min-width: 0; overflow-wrap: anywhere; }
  .uj-real.hozir { border-style: solid; border-color: ${T.success}; background: ${T.successSoft}; color: ${T.success}; }
  .uj-real.uch-oy { border-style: dashed; border-color: #E8A13A; background: #FBEED6; color: #8F5F12; }
  .uj-real.olti-oy { border-style: dotted; border-color: ${T.blue}; background: ${T.blueSoft}; color: ${T.blue}; }
  .uj-sabab { margin: 0; font-family: 'Manrope'; font-weight: 600; font-size: clamp(12.5px,1.5vw,13.5px); line-height: 1.5; color: ${T.ink2}; min-width: 0; overflow-wrap: anywhere; }
  @media (prefers-reduced-motion: reduce) { .uj-b { transition: none; } .uj-b:hover:not(:disabled), .uj-b:active:not(:disabled) { transform: none; } }

  /* KODING (s10): VS Code ostidagi terminal-tasmasi — natija shu yerda ko'rinadi (82b) */
  /* Terminal-tasmasi VS Code panel rangida (#252526 — tab-panel bilan bir xil), matn terminal-yashilida: asbob-xrom, PM palitrasidan tashqarida (kompilyator-istisnosi bilan bir mantiq) */
  .vsc-term { display: flex; align-items: center; gap: 10px; background: #252526; padding: 8px 14px; box-shadow: inset 0 1px 0 rgba(255,255,255,0.08); color: #8FE38F; font-family: 'JetBrains Mono', monospace; font-size: 11.5px; }
  .vsc-term-lbl { font-family: 'Manrope'; font-weight: 800; font-size: 10px; letter-spacing: 0.12em; color: #8B949E; }

  /* HOOK — ikki tanlov (104-qonun: teng og'irlik) */
  .hrow { display: grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap: clamp(8px,1.4vw,14px); }
  @media (max-width: 560px) { .hrow { grid-template-columns: 1fr; } }
  ...
  .s1demo-list .s1row:nth-child(2) .s1row-t { color: #8F5F12; }
  .s1demo-list .s1row:nth-child(3) .s1row-t { color: ${T.blue}; }
  ...
  .vsc-body { padding: 10px 14px 12px 6px; font-family: 'JetBrains Mono', monospace; font-size: clamp(11px,1.35vw,12.5px); color: #D4D4D4; line-height: 1.58; overflow: auto; max-height: clamp(190px, 34vh, 380px); scrollbar-width: thin; scrollbar-color: #4A4A4A #1E1E1E; }
  ...
  /* Uzun satr YASHIRINMAYDI: yon skroll o'rniga o'ralib tushadi (bola yon skrollni topmaydi) */
  ...
  /* s8 yozish-ekrani: gap inline emas — kirish-artefakt paneli chizilganda past ekranda
     (1280x800) qatlam siqiladi, panelsiz ko'rinish o'zgarmaydi (§147-B skroll-o'lchovi). */
  ...
  @media (max-height: 860px) {
    ...
    .s8-scr.has-ch .wsp-ed { padding: 11px 13px; gap: 7px; }
  }
...
const CSS_ARENA = `
  ...
  /* 58/60-qonun (past ekran: 1280x800 va 1440x900) — yakun-sahifasi BIR KO'Z bilan sig'sin.
     Qisqaradigan narsa BO'SHLIQ: matn butun qoladi (nishon tavsiflari 👦 uchun zarur),
     CODE STRIKE so'ziga esa umuman tegilmaydi (20-qonun). 2560x1440 da hech nima o'zgarmaydi. */
  @media (max-height: 900px) {
    .screen:has(> .hw-big-wrap) { gap: 10px !important; }
    .ring-wrap { width: 104px; height: 104px; }
    .ring-num { font-size: 26px; }
    .ring-den { font-size: 17px; }
    .bigidea { padding: 11px clamp(14px,2.2vw,22px) 10px; }
    ...
    .split.sum2 .card { padding: 12px 16px; }
    .split.sum2 .card-lbl { margin-bottom: 7px; }
    .split.sum2 .recap { gap: 6px; }
    .ach-coll { gap: 8px; }
    .ach-grid { gap: 8px; }
    .ach-badge { padding: 9px 8px; gap: 3px; }
    .ach-badge-ic { font-size: 25px; }
    .hw-big { padding: clamp(13px,1.8vw,17px) clamp(26px,3.4vw,44px); gap: 4px; }
    .hw-big-t { font-size: clamp(21px,2.8vw,28px); }
    .hw-big-s { font-size: clamp(13px,1.7vw,15px); }
  }

  /* 58-qonun (1280x800) — nishon olingach tavsif ochiladi va nishon-kartochka bo'yiga
     cho'ziladi. Tavsif 👦 uchun ZARUR (nishon nomini u faqat tavsif bilan tushunadi):
     shuning uchun MATN qisqarmaydi, JOYLASHUV yig'iladi — ikonka chapga chiqadi,
     nom + tavsif o'ng ustunda ustma-ust turadi (indeks-karta qatori). 900px bo'yli
     ekranda ustunli ko'rinish o'z joyida qoladi. */
  @media (max-height: 850px) {
    .ring-wrap { width: 94px; height: 94px; }
    .bigidea { padding: 9px clamp(14px,2.2vw,22px) 8px; }
    ...
    .split.sum2 .ach-grid { gap: 7px; }
    .split.sum2 .ach-badge { display: grid; grid-template-columns: auto minmax(0,1fr); align-items: center; column-gap: 9px; row-gap: 1px; text-align: left; padding: 6px 10px; }
