...
// ============================================================
// PM · M6-D14 — RAQAMINGIZ NIMANI ISBOTLAYDI? (gapiradigan slayd)
// Senariy-manba: pm-senariylar/M6-D14-MetrikaliPitch.md ([AVTO-GATE S] yopilgan, 2026-08-19).
// Misol-ip: Demo Day sahnasi va o'quvchining O'Z tizimi (91/95/96c/108-qonun).
// Imzo-vizual: GAPIRADIGAN SLAYD — uch qator birma-bir ochiladi, keyin ikki rost raqam duel qiladi.
// Bosh keys: K12 Airbnb — BESHINCHI burchak: besh qadamda raqamli qadam bitta va u o'rtada turadi.
//   🔴 Raqam-chegarasi: varaqdagi raqamning NECHALIGI hech qayerda aytilmaydi (§101/§122).
// Kirish-artefakt: pm-m6d12-yol (uch ufqli yo'l) — JIM zaxira, faqat `ufq === 'hozir'` qatori (§69).
// Chiqish-artefakt: pm-m6d14-pitch = { slayd: { raqam, nima, isbot }, savedAt } — `raqam` MATN
//   sifatida saqlanadi. M6 modulini va Demo Day 3 ni YOPADI.
// INFRA MANBAI: src/5-Modull/PmLesson21.jsx (M5-D11) va src/4a-Modull/PmLesson15.jsx (fixed-qobiq) —
//   ular o'z navbatida src/pm/PmUserStoryLesson.jsx (P0) va src/4-Modull/PmLesson11.jsx dan:
...
//   o'rgatilgan qoidaga almashtirildi (korpus §145, F-0819-27).
//   «Qator» faqat slaydniki: kod satri — «satr», sayt sahifasi — «sahifa» (§105/§121).
...
const SCREEN_META = [
  ...
  { id: 's4',  type: 'exploration', template: 'custom', scored: false, scope: null },          // 4  · YADRO: gapiradigan slayd
  { id: 's5',  type: 'test',        template: 'custom', scored: true,  scope: 'module-mikro' },// 5  · TEST-2
  { id: 's6',  type: 'case',        template: 'custom', scored: false, scope: null },          // 6  · K12 keys (Airbnb)
  { id: 's7',  type: 'test',        template: 'custom', scored: true,  scope: 'module-mikro' },// 7  · TEST-3
  { id: 's8',  type: 'practice',    template: 'custom', scored: false, scope: null },          // 8  · BLOK 4 slaydning uch qatori
...
const INLINE_KEYS = { s3: 1, s5: 2, s7: 0, s11: 1, slayd: -1, practice: -1, juft: -1, koding: -1 };
  ...
  return (
    <Stage eyebrow="Kirish · Demo Day sahnasi" screen={screen} navContent={<NavNext optionalLive turnBusy={picked === null && !isMentor} disabled={picked === null && !isMentor} label={opened ? 'Davom etish' : 'Bittasini tanlang'} onClick={onNext} />}>
      <div className="screen" style={{ gap: 'clamp(14px,2.2vw,20px)' }}>
        ...
        {/* Tanlov ishini bajarib bo'lgach o'rnini javobga bo'shatadi (ekran-matn chegarasi).
            Mentor (proyektor) rejimida qoladi — sinf undan ovoz beradi. */}
        {(!opened || isMentor) && (
...
const Screen2 = ({ screen, onNext, onPrev }) => {
  ...
  // 46-qonun: karta QULFLANMAYDI — qayta bosilsa yopiladi (akkordeon).
  const toggle = (i) => {
...
const Screen4 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === 'mentor');
  const [n, setN] = useState(() => (storedAnswer && storedAnswer.rows ? S4_ROWS.length : 0));
  const [pick, setPick] = useState(() => (storedAnswer && Number.isInteger(storedAnswer.tanlov) && S4_DUO[storedAnswer.tanlov] ? storedAnswer.tanlov : null)); /* F-0915-02 */
  ...
  useEffect(() => {
    try { localStorage.setItem(SLAYD_KEY, JSON.stringify({ qator: n, tanlov: pick, savedAt: Date.now() })); } catch {}
  }, [n, pick]);
  useEffect(() => {
    if (!stage2) return;
    ...
    if (!newRows && !newDuel && !newProof) return;
...
// ===== SCREEN 8 — SLAYDNING UCH QATORI (48/80/85/92/106d-qonun) =====
// Chiqish-artefakt: { slayd: { raqam, nima, isbot }, savedAt } — M6 ni va Demo Day 3 ni yopadi.
...
const readHozirgiIsh = () => {
  try {
    ...
    if (!v || !Array.isArray(v.ufqlar)) return null;
    const row = v.ufqlar.find(x => x && x.ufq === 'hozir' && typeof x.ish === 'string' && x.ish.trim());
    if (!row) return null;
    const s = row.ish.trim();
...
const Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === 'mentor');
  const [ish] = useState(() => readHozirgiIsh());
  ...
  const done = list.length >= S8_STEPS.length;
  const savedRef = useRef(false);
  const qadam = edit === null ? list.length : edit;
  const spec = S8_STEPS[Math.min(qadam, S8_STEPS.length - 1)];
  const v = d.trim();
  const sonBor = /\d/.test(v);
  const takror = qadam === 2 && list[1] !== undefined && sodda(v).length > 0 && sodda(v) === sodda(list[1]);
  const canSave = qadam === 0 ? sonBor : (v.length >= 3 && !takror);
  const inputTurn = useTurnHint(!done && v.length === 0 && !focus && !isMentor);
  useEffect(() => {
    if (!done || savedRef.current) return;
    savedRef.current = true;
    try { localStorage.setItem(OUT_KEY, JSON.stringify({ slayd: { raqam: String(list[0]), nima: list[1], isbot: list[2] }, savedAt: Date.now() })); } catch {}
...
// ===== SCREEN 9 — TEKSHIRUV: IKKI RAQAM — BIR JOY (26-qonun: yangi mexanika) =====
// Har raundda yonma-yon ikki ROST raqam va slaydda bitta joy. 3-raundda ikkala nomzod ham
// birinchi mezondan («kimning ishi») o'tadi — hukmni ikkinchi, nozikroq qoida chiqaradi.
...
const Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === 'mentor');
  const [i, setI] = useState(() => (Number.isInteger(storedAnswer?.raund) && storedAnswer.raund >= 0 ? storedAnswer.raund : 0)); /* F-0915-02 */
  const [pick, setPick] = useState(null);
  const [missedOnce, setMissedOnce] = useState(false);
  const [yordamOpen, setYordamOpen] = useState(false);
  const done = i >= JUFTLIKLAR.length;
  const raund = done ? null : JUFTLIKLAR[i];
  useEffect(() => {
    if (done && (storedAnswer === undefined || !storedAnswer.solved)) {
      try { localStorage.setItem(JUFT_KEY, JSON.stringify({ raund: JUFTLIKLAR.length, savedAt: Date.now() })); } catch {}
      onAnswer(screen, { stage: 'juft', screenIdx: screen, raund: JUFTLIKLAR.length, solved: true, correct: true });
...
const REFLECT_KEY = 'pm-m6d14-reflection';
...
`;
// Dars-vizuallari: gapiradigan slayd (imzo-vizual), ikki raqam dueli, yozish-kartasi, keys-slaydlari.
const CSS_LESSON = `
  ...
  /* s1 (18-qonun): uch qator o'z-o'zidan yozilib chiqadi; kam-harakat rejimida darhol to'liq */
  .sl.auto .sl-row.demo { opacity: 0; animation: sl-in 0.5s cubic-bezier(.2,.8,.3,1) both; animation-delay: var(--dd, 0s); }
  @keyframes sl-in { from { opacity: 0; transform: translateX(-12px); } to { opacity: 1; transform: translateX(0); } }
  @media (prefers-reduced-motion: reduce) { .sl.auto .sl-row.demo { animation: none; opacity: 1; transform: none; } }
  ...
  /* YADRO (s4): qator-tugmalari, «Slayd nima aytdi?» javob-qatori va ikki raqam dueli */
  .rowsel { display: flex; flex-direction: column; gap: 8px; }
  .rowbtn { display: flex; align-items: center; gap: 10px; text-align: left; background: ${T.paper}; border: none; border-radius: 13px; padding: 11px 14px; cursor: pointer; font-family: 'Manrope'; box-shadow: inset 0 0 0 1.5px ${T.line}; transition: transform 0.15s, box-shadow 0.15s; min-width: 0; }
  .rowbtn:disabled { cursor: default; }
  .rowbtn.cur { box-shadow: inset 0 0 0 2px ${T.accent}; animation: rowbtn-pulse 1.8s ease-in-out infinite; }
  .rowbtn.cur:hover { transform: translateY(-2px); }
  .rowbtn.done { background: ${T.successSoft}; box-shadow: inset 0 0 0 1.5px ${T.success}66; }
  @keyframes rowbtn-pulse { 0%, 100% { box-shadow: inset 0 0 0 2px ${T.accent}; } 50% { box-shadow: inset 0 0 0 2px ${T.accent}, 0 10px 22px -10px rgba(91,61,230,0.5); } }
  .rowbtn-n { font-style: normal; flex-shrink: 0; width: 22px; height: 22px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; font-weight: 800; font-size: 11.5px; background: ${T.bg}; color: ${T.ink2}; }
  .rowbtn.cur .rowbtn-n { background: ${T.accent}; color: #fff; }
  .rowbtn.done .rowbtn-n { background: ${T.success}; color: #fff; }
  .rowbtn-t { font-weight: 800; font-size: clamp(13px,1.6vw,14.5px); color: ${T.ink}; min-width: 0; overflow-wrap: anywhere; }
  .rowbtn-cue { margin-left: auto; flex-shrink: 0; font-weight: 700; font-size: 11px; color: ${T.accent}; }
  @media (prefers-reduced-motion: reduce) { .rowbtn, .rowbtn.cur { animation: none; transition: none; } .rowbtn.cur:hover { transform: none; } }
  .say { display: flex; flex-direction: column; gap: 6px; background: ${T.paper}; border-radius: 14px; padding: clamp(11px,1.8vw,15px); box-shadow: inset 0 0 0 1.5px ${T.line}; min-width: 0; }
  .say-lbl { font-family: 'Manrope'; font-weight: 800; font-size: 11px; letter-spacing: 0.07em; text-transform: uppercase; color: ${T.ink3}; }
  ...
  .duocard.win { background: ${T.successSoft}; box-shadow: inset 0 0 0 2px ${T.success}; animation: duo-win-pop 0.34s cubic-bezier(.34,1.5,.4,1); }
  .duocard.lose { opacity: 0.74; }
  /* 🔴 134-qonun: hukm FAQAT rang bilan aytilmaydi — yutgan kartada to'ldirilgan ✓ belgisi,
     ikkinchisida bo'sh halqa turadi. Ikkinchisi XATO EMAS (qizil yo'q), u shunchaki
     sahnadagi bitta joyni olmadi. */
  .duocard.win::after, .duocard.lose::after { position: absolute; top: 8px; right: 9px; width: 19px; height: 19px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 11.5px; font-weight: 800; line-height: 1; }
  .duocard.win::after { content: '✓'; background: ${T.success}; color: #fff; }
  .duocard.lose::after { content: '–'; background: ${T.paper}; color: ${T.ink3}; box-shadow: inset 0 0 0 1.5px ${T.line}; }
  @keyframes duo-win-pop { 0% { transform: scale(1); } 45% { transform: scale(1.035); } 100% { transform: scale(1); } }
  .duo-n { font-weight: 800; font-size: clamp(25px,4vw,34px); line-height: 1; color: ${T.accent}; font-variant-numeric: tabular-nums; }
  .duocard.win .duo-n { color: ${T.success}; }
  .duo-t { font-weight: 700; font-size: clamp(12.5px,1.5vw,14px); line-height: 1.35; color: ${T.ink}; min-width: 0; overflow-wrap: anywhere; }
  .duo-why { font-weight: 600; font-size: 11.5px; line-height: 1.4; color: ${T.ink2}; background: ${T.paper}; border-radius: 9px; padding: 6px 9px; animation: fade-step 0.28s ease-out; min-width: 0; overflow-wrap: anywhere; }
  .duel-h { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; align-self: center; }
  .duel-n { font-weight: 800; font-size: 12px; color: ${T.accent}; background: ${T.accentSoft}; border-radius: 99px; padding: 3px 11px; }
  .duel-t { font-family: 'Manrope'; font-weight: 800; font-size: clamp(13px,1.7vw,15px); color: ${T.ink}; }
  .duel-strip { display: flex; gap: 7px; flex-wrap: wrap; }
  .duel-chip { display: inline-flex; align-items: center; gap: 6px; font-family: 'Manrope'; font-weight: 700; font-size: 11.5px; color: ${T.ink2}; background: ${T.paper}; border-radius: 99px; padding: 5px 12px; box-shadow: inset 0 0 0 1.5px ${T.line}; min-width: 0; overflow-wrap: anywhere; }
  .duel-chip i { font-style: normal; font-weight: 800; color: ${T.success}; }
  /* Yakun-lentasi: uch juftlik sahnaga birma-bir chiqadi */
  .bdone .duel-chip { animation: duel-chip-in 0.34s cubic-bezier(.2,.8,.3,1) both; }
  .bdone .duel-chip:nth-child(2) { animation-delay: 0.13s; }
  .bdone .duel-chip:nth-child(3) { animation-delay: 0.26s; }
  @keyframes duel-chip-in { from { opacity: 0; transform: translateY(7px); } to { opacity: 1; transform: none; } }
  @media (prefers-reduced-motion: reduce) { .bdone .duel-chip { animation: none; } }
  @media (prefers-reduced-motion: reduce) { .duocard, .duocard:hover:not(:disabled), .duocard.win { transition: none; transform: none; animation: none; } .duo-why { animation: none; } }
  /* MOBIL (s4): sahna slaydi tepaga chiqadi — bosish va o'zgarish bir ko'rish maydonida qoladi */
  ...
  /* s8 juftlik: ikki maslahat-chipi BITTA guruh — biri ochilganda ikkinchisi yopiladi.
     Ochilgan chip qatorning to'liq kengligini oladi (matni 5 satrdan 2 satrga tushadi),
     yopiq sherigi esa shu qatorning o'ng chetiga «yorliq» bo'lib chiqadi: ustun
     balandligi o'smaydi, ikkinchi maslahat esa doim ko'z oldida qoladi. */
  ...
  /* Airbnb besh qadami: matn ichidagi «1) … 5)» o'rniga ajralgan qatorlar; uchinchisi ko'z bilan o'rtada. */
  .k-steps { list-style: none; margin: 2px 0 0; padding: 0; display: flex; flex-wrap: wrap; justify-content: center; gap: 7px; max-width: 640px; min-width: 0; }
  .k-steps li { display: inline-flex; align-items: center; gap: 7px; min-width: 0; background: ${T.bg}; border-radius: 99px; padding: 5px 13px 5px 5px; font-family: 'Manrope', sans-serif; font-weight: 700; font-size: clamp(11.5px,1.5vw,13.5px); line-height: 1.25; color: ${T.ink2}; box-shadow: inset 0 0 0 1.5px ${T.line}; overflow-wrap: anywhere; }
  .k-step-n { flex-shrink: 0; display: inline-flex; align-items: center; justify-content: center; width: 19px; height: 19px; border-radius: 99px; background: ${T.paper}; color: ${T.ink3}; font-size: 11px; box-shadow: inset 0 0 0 1.5px ${T.line}; }
  .k-steps li.on { background: ${T.accentSoft}; color: ${T.ink}; box-shadow: inset 0 0 0 2px ${T.accent}; }
  .k-steps li.on .k-step-n { background: ${T.accent}; color: ${T.paper}; box-shadow: none; }
  ...
  @media (min-width: 861px) {
    ...
    /* Muharrir-kartasi TABIIY balandligida qoladi: cho'zilsa ichida bo'sh oq maydon
       hosil bo'lardi (kartaning yarmi quruq). Ustun balandligini o'ng tomondagi
       topshiriq-paneli ushlab turadi. */
    .wsp-ed { flex-grow: 0; }
  ...
  @media (max-height: 850px) {
    ...
    .sl-row { min-height: 33px; }
    .sl-row.num { min-height: 44px; }
    .mentor-msg { padding: 8px 13px; }
    .mentor-ava { width: 32px; height: 32px; }
    .say-t { font-size: 12px; }
...
const CSS_ARENA = `
  ...
  /* «Endi siz bilasiz» kengroq ustunga o'tadi — to'rt xulosa BIR QATORDAN bo'lib qoladi
     (matn o'zgarmaydi, faqat qator uzunligi yetadi); nishonlar esa bitta qatorga tushadi. */
...
export default function PmLesson25({ lang: langProp, onFinished, liveToken }) {
  ...
  const recordAnswer = (idx, data) => {
    ...
    if (_m && ACH_EXTRA[_m.id] && data && data[ACH_EXTRA[_m.id].key]) earn(ACH_EXTRA[_m.id].id);
  };
