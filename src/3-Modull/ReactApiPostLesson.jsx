  ...
  return (
    <Stage eyebrow={tr({ uz: 'Kirish', ru: 'Введение' })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={{ uz: 'Davom etish', ru: 'Продолжить' }} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr({ uz: <>Siz o'yin yaratdingiz. Nega u katalogga <span className="italic" style={{ color: T.accent }}>chiqmayapti</span>?</>, ru: <>Вы создали игру. Почему она <span className="italic" style={{ color: T.accent }}>не появляется</span> в каталоге?</> })}</h1>
    ...
    <Stage eyebrow={tr({ uz: "Fe'llar", ru: 'Глаголы' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: `${seen.size}/4 fe'lni sinang`, ru: `Попробуйте глаголы: ${seen.size}/4` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(8px,1.2vw,12px)' }}>
        ...
        <div className="split">
          ...
          <Col>
            <p className="flow-label">{tr({ uz: 'robo-api.example — server jadvali', ru: 'robo-api.example — таблица сервера' })}</p>
      ...
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        ...
        <div className="split">
          ...
          <Col>
            ...
            <p className="flow-label" style={{ margin: 0 }}>{tr({ uz: 'robo-api.example — jadval', ru: 'robo-api.example — таблица' })}</p>
  ...
  return (
    <Stage eyebrow="ID" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: `Manzilni toping (${taskIdx}/2)`, ru: `Найдите адрес (${taskIdx}/2)` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr({ uz: 'robo-api.example — jadval', ru: 'robo-api.example — таблица' })}</p>
          ...
          <Col>
            ...
            {found && !fixed && (
              <pre className="code-box fade-step" style={{ lineHeight: 1.9 }}>
                {'fetch('}<St>'https://robo-api.example/games/</St><span style={{ borderRadius: 5, padding: '0 4px', background: 'rgba(255,79,40,0.2)' }}><St>4</St></span><St>'</St>{', {'}{'\n'}
...
const Screen9 = (props) => (
  <QuestionScreen {...props} idx={9} scope="module-mikro" eyebrow={tr({ uz: 'Mashq · 3-savol', ru: 'Практика · вопрос 3' })}
    questionText={tr({ uz: "robo-api.example/games/7 — bu manzil nima degani?", ru: 'robo-api.example/games/7 — что означает этот адрес?' })}
        ...
        <div className="split">
          <Col>
            ...
            <pre className="code-box fade-up delay-2" style={{ lineHeight: 1.9, padding: '10px 14px' }}>
              {'fetch('}<St>'https://robo-api.example/games/5'</St>{', {'}{'\n'}
...
const Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const DEL_LINES = [
    { text: "fetch('https://robo-api.example/games/5', {" },
    ...
    <Stage eyebrow={tr({ uz: 'Amaliyot · pult', ru: 'Практика · пульт' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: 'Davom etish', ru: 'Продолжить' } : { uz: `Posilka ${Math.min(stage + 1, 3)}/3`, ru: `Посылка ${Math.min(stage + 1, 3)}/3` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(8px,1.2vw,12px)' }}>
        ...
        <div className="split">
          <Col>
            ...
            {stage === 1 && (
              <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <button className={`gchip ${putShake ? 'shake' : ''}`} onClick={() => pickAddr(false)} style={{ fontFamily: "'JetBrains Mono',monospace", fontFeatureSettings: '"liga" 0, "calt" 0', fontSize: 13, padding: '10px 14px', justifyContent: 'flex-start' }}>robo-api.example/games</button>
                  <button className="gchip" onClick={() => pickAddr(true)} style={{ fontFamily: "'JetBrains Mono',monospace", fontFeatureSettings: '"liga" 0, "calt" 0', fontSize: 13, padding: '10px 14px', justifyContent: 'flex-start' }}>robo-api.example/games/<b>2</b></button>
                </div>
          ...
          <Col>
            <p className="flow-label">{tr({ uz: 'robo-api.example — jadval (jonli)', ru: 'robo-api.example — таблица (живая)' })}</p>
...
const Screen16 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const EMOJIS = ['🤖', '🏎️', '🐉', '⚽', '🎯'];
  const [gname, setGname] = useState(typeof storedAnswer?.gameName === 'string' ? storedAnswer.gameName : ''); // F-0914-10: saqlangan javob matn bo'lmasa — bo'sh (oq ekran himoyasi)
          ...
          <Col>
            ...
            <div className="vsc fade-up delay-2">
              ...
              <div className="vsc-body" style={{ fontSize: 'clamp(11px,1.35vw,12px)', lineHeight: 1.82 }}>
                ...
                <Ln n={2}>{''}</Ln>
                <Ln n={3}>{'fetch('}<St>'https://robo-api.example/games'</St>{', {'}</Ln>
...
export default function ReactApiPostLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  return (
    <LangContext.Provider value={lang}>
      <style>{`
        ...
        .vbadge { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 11px; color: #fff; padding: 4px 9px; border-radius: 7px; letter-spacing: 0.04em; flex-shrink: 0; min-width: 52px; text-align: center; }
        ...
        .srow { display: flex; align-items: center; gap: 10px; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: 12.5px; color: ${CODE.text}; padding: 5px 8px; border-radius: 7px; transition: all 0.3s; }
