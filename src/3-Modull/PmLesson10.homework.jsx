...
// ============================================================
// PM M3-D14 — UYGA VAZIFA: «KO'RSATUVNI HAQIQIY SAYTDA SINANG» (PmLesson10 davomi)
// Darsning To'liq-kapsulasi: «3 kadrni oching-o'qing → aytgan harakatingizni chindan bajaring →
// mos kelmagan kadrni tuzating» — vazifa AYNAN shu.
// 4 bosqich · mezon: TO'RTTALASI bajarilsa «Bajarildi» (HW_PASS_MIN = 4).
//   1) 3 kadrim — darsdagi ish + kadrlar (avto: `pm-m3d14-pitch`)
//   2) Haqiqiy sinov — har kadr harakati saytda bajarilib 🙂/😕 belgilanadi
//   3) Tuzatish — 😕 bo'lsa «o'zgartirmayman» yopiq (halol-qulf), kadr qayta yoziladi
//   4) Xulosa — 3 savol (Kahoot)
// Validatorlar DARSDAN AYNAN: gap ≥12 · ekranniTakror OGOHLANTIRISH (darsda ham blok emas) ·
// harakat 1–5 so'z. Senariy: pm-senariylar/M3-D14-Korsatuv-UY.md
// Naqsh: src/1-Modull/PmLesson2.homework.jsx (ETALON) · PmLesson6.homework halol-qulf relsi.
...
// ===== Darsdagi validatorlar (PmLesson10 bilan AYNAN) =====
const ISH_MIN = 4, GAP_MIN = 12;
const APO = "['\\u02BB\\u2018\\u2019`]";
...
const gapOk = (s) => (s || '').trim().length >= GAP_MIN;
const actOk = (s) => { const n = sozSoni(s); return n >= 1 && n <= 5 && (s || '').trim().length >= 3; };
const kadrOk = (k) => !!k && gapOk(k.gap) && actOk(k.harakat);

// Kadr nomlari va yo'riqlari darsdan (KADR_QADAM)
const KADR_QADAM = [
        ...
        return (
          <div key={i} className="ac-row">
            ...
            <span className="ac-v">
              ...
              {isRew
                ? <><s className="ac-old">{(k.gap || '').trim()}</s> <span className="ac-new">{(rew.gap || '').trim()}</span></>
                : <span>{(k.gap || '').trim()}</span>}
...
// Kadr-maydonlari (1- va 3-bosqichda)
const KadrFields = ({ k, setK, idx, pulse }) => {
...
const pitchDone = (w) => ishOk((w || {}).ish) && [0, 1, 2].every(i => kadrOk(((w || {}).kadrlar || [])[i]));

// — 2-BOSQICH: haqiqiy sinov —
const StageTest = ({ data, setData, pitch }) => {
          ...
          return (
            <div key={i} className="wrow" style={{ borderBottom: i < 2 ? `1px solid ${T.line}` : 'none' }}>
              ...
              <div className="chips" role="radiogroup">
                {MATCH.map((m, mi) => (
                  <button key={mi} type="button" role="radio" aria-checked={marks[i] === mi} className={`chip ${marks[i] === mi ? 'on' : ''}`} onClick={() => setMark(i, mi)}>
...
const testDone = (d) => [0, 1, 2].every(i => ((d || {}).marks || [])[i] === 0 || ((d || {}).marks || [])[i] === 1);
const allMatch = (d) => [0, 1, 2].every(i => ((d || {}).marks || [])[i] === 0);
...
const StageRew = ({ data, setData, pitch, test }) => {
  const d = data || {};
      ...
      <div className="frame fade-up d1">
        <div className="chips" role="radiogroup">
          {MODES.map((m, i) => {
            ...
            return (
              <button key={i} type="button" role="radio" aria-checked={mode === i} disabled={locked} className={`chip ${mode === i ? 'on' : ''} ${locked ? 'off' : ''}`} onClick={() => setData({ ...d, mode: i })} title={locked ? tr({ uz: 'Sinovda mos kelmagan kadr bor — uni tuzating', ru: 'В проверке есть несовпавший кадр — исправьте его' }) : undefined}>
                <span className="chip-ic" aria-hidden="true">{m.ic}</span>
                <span>{tr(m.label)}</span>
              </button>
            );
        ...
        {mode === 1 && (
          <div style={{ marginTop: 12 }}>
            ...
            <div className="chips" role="radiogroup">
              {(badIdx.length ? badIdx : [0, 1, 2]).map(i => (
                <button key={i} type="button" role="radio" aria-checked={piece === i} className={`chip ${piece === i ? 'on' : ''}`} onClick={() => setData({ ...d, piece: i })}>
...
// ILDIZ-KOMPONENT
// ============================================================
export default function PmLesson10Homework({ lang: langProp, onFinished }) {
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult
            ...
            : cur.key === 'test' ? <StageTest data={data.test || {}} setData={setStageData('test')} pitch={data.pitch} />
            : cur.key === 'rew' ? <StageRew data={data.rew || {}} setData={setStageData('rew')} pitch={data.pitch} test={data.test || {}} />
