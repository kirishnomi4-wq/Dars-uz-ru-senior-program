  ...
  return (
    <Stage eyebrow={tr({ uz: 'Kirish', ru: 'Введение' })} screen={screen} audioState={audio} navContent={<NavNext disabled={picked === null} label={tr({ uz: 'Davom etish', ru: 'Продолжить' })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr({ uz: <>Bitta buyruq <span className="italic" style={{ color: T.accent }}>butun loyihani</span> yaratib beradimi?</>, ru: <>Одна команда создаст <span className="italic" style={{ color: T.accent }}>целый проект</span>?</> })}</h1>
            ...
            <div className="frame fade-up delay-2" style={{ padding: '11px 14px', minHeight: 138 }}>
              {phase < 2 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  {TREE.map((f, i) => (
                    <div key={f.label} className="fade-up" style={{ animationDelay: `${i * 0.18}s`, display: 'flex', alignItems: 'center', gap: 8, padding: '4px 9px', paddingLeft: 9 + f.d * 20, borderRadius: 7, background: f.hot ? T.accentSoft : 'transparent', fontFamily: "'JetBrains Mono',monospace", fontFeatureSettings: '"liga" 0, "calt" 0', fontSize: 12.5, color: T.ink }}>
            ...
            <div className="code-box fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {STEPS.map((s, i) => (
                <div key={i} onClick={() => { if (i === step) setStep(v => v + 1); }}
                  ...
                  <span style={{ fontFamily: "'JetBrains Mono',monospace", fontFeatureSettings: '"liga" 0, "calt" 0', fontSize: 12, color: i < step ? CODE.str : CODE.comment, minWidth: 14 }}>{i < step ? '✓' : i + 1}</span>
                  <span style={{ fontFamily: "'JetBrains Mono',monospace", fontFeatureSettings: '"liga" 0, "calt" 0', fontSize: 'clamp(11.5px,1.4vw,13px)', color: CODE.text }}><span style={{ color: CODE.str }}>$</span> {s.cmd}</span>
        ...
        <div className="split">
          <Col>
            ...
            <div className="fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <div style={{ fontFamily: "'JetBrains Mono',monospace", fontFeatureSettings: '"liga" 0, "calt" 0', fontSize: 12, color: T.ink3, padding: '2px 11px' }}>📁 robo-games/</div>
              <div style={{ fontFamily: "'JetBrains Mono',monospace", fontFeatureSettings: '"liga" 0, "calt" 0', fontSize: 12, color: T.ink3, padding: '2px 11px', paddingLeft: 29, opacity: 0.6 }}>📁 node_modules/ <span style={{ fontSize: 10 }}>{tr({ uz: '(kutubxonalar — tegmaymiz)', ru: '(библиотеки — не трогаем)' })}</span></div>
              <Row k="pkg" depth={1} />
              <Row k="html" depth={1} />
              <div style={{ fontFamily: "'JetBrains Mono',monospace", fontFeatureSettings: '"liga" 0, "calt" 0', fontSize: 12, color: T.ink3, padding: '2px 11px', paddingLeft: 29 }}>📁 src/</div>
              <Row k="main" depth={2} />
              <Row k="app" depth={2} />
            </div>
          </Col>
...
const Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [value, setValue] = useState(() => (typeof storedAnswer?.picked === 'string' ? storedAnswer.picked : '')); /* F-0915-02 */
...
export default function ReactFirstComponentLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  return (
    <LangContext.Provider value={lang}>
      <style>{`
        ...
        .cm-plate { position: relative; z-index: 1; text-align: center; background: ${T.bg}; color: ${T.ink}; border: 1px solid ${T.line}; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 800; font-size: 12px; letter-spacing: 0.02em; color: #fff; background: linear-gradient(180deg, #1f2740, #171d30); border-radius: 8px; padding: 4px 0; margin: 0 6px 8px; box-shadow: inset 0 0 0 1px rgba(255,255,255,0.08); transition: all 0.2s; }
        ...
        .cm-mold-shape { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: 11px; letter-spacing: 0; color: ${T.ink3}; }
        ...
        .cm-fl { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 10.5px; padding: 4px 9px; border-radius: 7px; background: ${T.bg}; color: ${T.ink3}; border: 1px solid ${T.line}; white-space: nowrap; transition: all 0.3s ease; }
