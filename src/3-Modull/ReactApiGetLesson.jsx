...
const INLINE_KEYS = { s4: 1, s5b: 2, s9: 3, s12: 0 }; // o'lik mashq-kalitlar olib tashlandi (jsx-lint, 2026-09-07): s15
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Kirish', ru: 'Введение' })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr({ uz: 'Davom etish', ru: 'Продолжить' })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr({ uz: <>Yangi o'yin chiqdi — millionlab ekranda <span className="italic" style={{ color: T.accent }}>bir zumda</span> paydo bo'ldi. Qanday?</>, ru: <>Вышла новая игра — и <span className="italic" style={{ color: T.accent }}>мгновенно</span> появилась на миллионах экранов. Как?</> })}</h1>
...
// ===== SCREEN 3 — FETCH TUZILISHI (3 bosiladigan qism) =====
...
const MentorPracticeStats = ({ live, screen }) => {
  ...
  // paydo bo'ladi (har 3 s da yangilanadi) — F-0819-57, m3-07:1704 dan ko'chirildi.
...
const Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tayyor', ru: 'Готово' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: 'clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Qaytadan', ru: 'Заново' })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: 'auto', padding: 'clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Yakunlash ✓', ru: 'Завершить ✓' })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: 'Dars tugadi', ru: 'Урок завершён' })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Katalogingiz endi <span className="italic" style={{ color: T.accent }}>jonli</span>.</>, ru: <>Ваш каталог теперь <span className="italic" style={{ color: T.accent }}>живой</span>.</> })}</h2>{/* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */}</div><ScoreRing correct={correct} total={total} /></div>
...
export default function ReactApiGetLesson({ lang: langProp, onFinished, liveToken }) {
