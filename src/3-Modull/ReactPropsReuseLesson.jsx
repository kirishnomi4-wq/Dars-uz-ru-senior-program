  ...
  return (
    <Stage eyebrow={tr({ uz: 'Kirish', ru: 'Введение' })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr({ uz: 'Davom etish', ru: 'Продолжить' })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr({ uz: <>6 ta har xil kartochka — <span className="italic" style={{ color: T.accent }}>nechta kod</span>?</>, ru: <>6 разных карточек — <span className="italic" style={{ color: T.accent }}>сколько кода</span>?</> })}</h1>
...
const Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tayyor', ru: 'Готово' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: 'clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Qaytadan', ru: 'Заново' })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: 'auto', padding: 'clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Yakunlash ✓', ru: 'Завершить ✓' })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: 'Dars tugadi', ru: 'Урок завершён' })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Katalogingiz <span className="italic" style={{ color: T.accent }}>tayyor</span>.</>, ru: <>Ваш каталог <span className="italic" style={{ color: T.accent }}>готов</span>.</> })}</h2>{/* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */}</div><ScoreRing correct={correct} total={total} /></div>
...
export default function ReactPropsReuseLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  return (
    <LangContext.Provider value={lang}>
      <style>{`
        ...
        .parcel2-lbl { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 9px; font-weight: 700; color: ${T.accent}; background: #fff; padding: 2px 7px; border-radius: 99px; box-shadow: 0 2px 6px -1px rgba(255,79,40,0.4); white-space: nowrap; }
        ...
        .dbg-line { display: flex; align-items: center; gap: 10px; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: clamp(11.5px,1.5vw,13.5px); color: ${CODE.text}; padding: 8px 12px; border-radius: 9px; cursor: pointer; border: 1.5px solid transparent; transition: background .15s, border-color .15s; white-space: nowrap; }
        ...
        .cf-slip { position: absolute; top: -26px; left: 50%; transform: translateX(-50%); background: #fff; color: ${T.ink}; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 9px; padding: 4px 10px; border-radius: 7px; white-space: nowrap; box-shadow: 0 5px 12px -3px rgba(${T.shadowBase},0.4), inset 0 0 0 1px ${T.line}; border-top: 3px solid #DFB068; }
        ...
        .cf-plate { position: relative; z-index: 1; text-align: center; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 800; font-size: 12px; color: ${T.ink}; background: ${T.bg}; border: 1px solid ${T.line}; border-radius: 8px; padding: 5px 0; margin: 0 4px; }
        ...
        .cf-hopper-slip { position: relative; background: #fff; color: ${T.ink2}; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 8px; padding: 3px 6px; border-radius: 5px; box-shadow: 0 3px 7px -2px rgba(${T.shadowBase},0.35), inset 0 0 0 1px ${T.line}; border-top: 2px solid #DFB068; }
