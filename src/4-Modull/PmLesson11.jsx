...
// Maket: bitta xizmat — IKKI XIL bosh sahifa (156-qonun · KORPUS §189). Foto emas, chizma:
// mualliflik muammosi yo'q, qorong'i rejimda to'g'ri ko'rinadi, LMS'ga qo'shimcha fayl kerak emas.
// Logotip qo'yilmaydi — brend nomi slayd matnida allaqachon aytilgan.
// 5.8-qonun: ustun-sarlavhasi «Siz / Sinfdoshingiz» — o'ylab topilgan personaj EMAS, o'quvchining
// o'zi; hech kim «gapirmaydi».
const NF_PANES = [
  { who: { uz: 'Siz', ru: 'Вы' }, seen: { uz: 'kulgili kino', ru: 'комедии' }, tiles: ['🤡', '🎭', '🍿'] },
  { who: { uz: 'Sinfdoshingiz', ru: 'Одноклассник' }, seen: { uz: "qo'rqinchli kino", ru: 'ужасы' }, tiles: ['👻', '🧟', '🦇'] },
];
const NfMock = () => (
    ...
    <div className="nf-panes">
      {NF_PANES.map((pane, k) => (
        <div key={k} className="nf-scr">
          ...
          <div className="nf-row">{pane.tiles.map((t, j) => <i key={j} className="nf-tile">{t}</i>)}</div>
        </div>
      ))}
    </div>
    <p className="nf-note">{tr({ uz: 'bitta xizmat — ikki xil bosh sahifa', ru: 'один сервис — две разные главные' })}</p>
  </div>
);
...
const CSS_LESSON = `
  ...
  /* MAKET: ikki bosh sahifa. Ekran-taqlidi ataylab quyuq (kino xizmati oqshom fonida
     ko'riladi) — bu tugma emas, shuning uchun accent qoidasiga bo'ysunmaydi. */
  ...
  .nf-tile { flex: 1 1 0; aspect-ratio: 2 / 3; border-radius: 5px; background: linear-gradient(160deg,#4C4468,#312A45); display: flex; align-items: center; justify-content: center; font-size: clamp(15px,2.4vw,21px); font-style: normal; box-shadow: inset 0 0 0 1px rgba(255,255,255,0.06); }
...
`;
// ============================================================ LESSON ROOT
export default function PmLesson11({ lang: langProp, onFinished, liveToken }) {
