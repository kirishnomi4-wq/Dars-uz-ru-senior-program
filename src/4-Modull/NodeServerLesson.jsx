  ...
  return (
    <Stage eyebrow={tr({ uz: 'Kirish', ru: 'Введение' })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr({ uz: 'Davom etish', ru: 'Продолжить' })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr({ uz: <><span className="mono" style={{ color: T.accent }}>fetch</span> yozganingizda — narigi tomonda <span className="italic" style={{ color: T.accent }}>kim javob beradi</span>?</>, ru: <>Вы пишете <span className="mono" style={{ color: T.accent }}>fetch</span> — а на той стороне <span className="italic" style={{ color: T.accent }}>кто отвечает</span>?</> })}</h1>
    ...
    <Stage eyebrow={tr({ uz: 'Yakuniy · amaliy', ru: 'Финал · практика' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        ...
        <div className="split">
          ...
          <Col>
            <p className="flow-label">{tr({ uz: 'Terminal', ru: 'Терминал' })}</p>
            <div className="code-box" style={{ minHeight: 44 }}>{ran ? <TLine out={<span style={{ color: CODE.str }}>{tr({ uz: '✓ Server localhost:3000 da ishlayapti', ru: '✓ Сервер работает на localhost:3000' })}</span>} /> : <span style={{ color: CODE.comment, fontFamily: "'JetBrains Mono'", fontFeatureSettings: '"liga" 0, "calt" 0', fontSize: 12 }}>{tr({ uz: 'Run bosilmagan…', ru: 'Run не нажат…' })}</span>}</div>
...
const Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tayyor', ru: 'Готово' })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: 'clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Qaytadan', ru: 'Заново' })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: 'auto', padding: 'clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Yakunlash ✓', ru: 'Завершить ✓' })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: 'Dars tugadi', ru: 'Урок завершён' })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Birinchi serveringiz <span className="italic" style={{ color: T.accent }}>javob berdi</span>.</>, ru: <>Ваш первый сервер <span className="italic" style={{ color: T.accent }}>ответил</span>.</> })}</h2>{/* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */}</div><ScoreRing correct={correct} total={total} /></div>
...
export default function NodeServerLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  return (
    <LangContext.Provider value={lang}>
      <style>{`
        ...
        .store-sign { display: inline-flex; align-items: center; gap: 8px; padding: 6px 15px; border-radius: 10px; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 800; font-size: 14px; letter-spacing: 2.5px; border: 1.5px solid; transition: color 0.28s, background 0.28s, box-shadow 0.28s, border-color 0.28s; }
        ...
        .epmethod { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 10px; color: ${T.success}; background: ${T.successSoft}; padding: 2px 7px; border-radius: 5px; flex-shrink: 0; }
