...
function TrackerStrip() {
  ...
  if (!resolved || resolved.size === 0) return null; // B2 / F-0820-193: strip birinchi fikr hal bo'lganda tug'iladi (129-ruhi); 4/4 yakun-dalil bo'lib qoladi
...
const INLINE_KEYS = { s4: 2, s6: 1, s10: 3, s13: 0, s16: -1, practice: -1 };
...
// feedback kartasi
const FbCard = ({ fb, onClick, on, seen, hint }) => (
...
const MentorPracticeStats = ({ live, screen }) => {
  ...
  // 129-qonun: bo'sh apparat ko'rsatilmaydi — birinchi o'quvchi qo'shilgach panel o'zi chiqadi (F-0820-190)
...
// O'QUVCHI ko'radigan sinf-holati — sof O'QISH, ball-relsga yozmaydi (m3-09 kanonik nusxa, F-0820-189).
...
// ============================================================ LESSON ROOT
export default function FullstackFeedbackLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  const recordAnswer = (idx, data) => {
    ...
    if (_m && data && data.correct) {
      if (ACH_TRIGGERS[_m.id] && !missedRef.current.has(_m.id)) earn(ACH_TRIGGERS[_m.id]); // 🏅 nishon (faqat REAL solve)
  ...
  const reset = () => { if (!firstPassRef.current) { firstPassRef.current = { answers, durationSec: Math.floor((Date.now() - startTimeRef.current) / 1000) }; setFpPractice(true); } progClear(LESSON_META.lessonId); setAnswers({}); setScreen(0); startTimeRef.current = Date.now(); trkRef.current = new Set(); setResolved(new Set()); };
  ...
  return (
    <LangContext.Provider value={lang}>
      <style>{`
        ...
        .set-val { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 13px; color: ${T.ink}; }
        ...
        .spot-num { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 800; font-size: 14px; } .spot-sm .spot-num { font-size: 11px; }
        ...
        .jemoji { font-size: 22px; } .jn { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 800; font-size: 12px; color: ${T.accent}; flex-shrink: 0; white-space: nowrap; }
        ...
        .trk-n { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 11px; color: ${T.ink2}; display: inline-block; }
