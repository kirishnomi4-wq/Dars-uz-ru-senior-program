...
const INLINE_KEYS = { s4: 2, s7: 1, s9: 3, s14: 0, practice: -1 };   // F-0820-216: o'lik `s16` kaliti olib tashlandi
      ...
      {reveal && answered > 0 && (() => {
        ...
        return (
          <div className={`mstats-verdict ${level}`}>
            {level === 'need' && <p className="mstats-verdict-t">{tr({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верно — класс не разобрался в теме. Перед продолжением коротко повторите.</> })}</p>}
...
const QuestionScreen = ({ screen, idx, scope, eyebrow, question, questionText, options, correctIdx, explainCorrect, explainWrong, audioText, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const pick = (i) => {
    ...
    if (live && live.recordAttempt && !fpPractice) live.recordAttempt(screen, SCREEN_META[screen]?.id || `s${screen}`, i, Date.now() - mountTs.current, { question: tr(questionText), options: options.map(o => tr(o)), picked: tr(options[i]), correct: tr(options[correctIdx]), lang: (typeof __lang !== 'undefined' && __lang === 'ru') ? 'ru' : 'uz' });
  };
...
//
// Uch daraja (138-qonun): QULF-YORLIQ harakatni aytadi -> IPUCHA chuqurlashtiradi
// -> RESCUE yo'l ochadi. Bu darsda 1-daraja ALLAQACHON bajarilgan (to'qqizala
// ekranda yorliq harakatni aytadi), shuning uchun bu yerga 2 va 3-daraja qo'shiladi.
//
...
// (progress o'sib turadi) hech qachon «tiqilib qolgan» deb belgilanmaydi.
//
// 🔴 MUSTAQILLIK: hook komponent ICHIDA yashaydi — taymer faqat o'sha ekran ochiq
// turganda yuradi va ekran almashganda unmount bo'ladi. Ya'ni bir vaqtda FAQAT
// joriy ekranning ipuchasi ko'rinadi; to'qqizta ipucha to'planib qolmaydi.
//
...
const Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const { tip: _tip, rescue: _resc } = useStuckValve(done, (front ? 1 : 0) + (back ? 1 : 0));   // 13-band klapan
  useEffect(() => { if (done && storedAnswer === undefined) onAnswer(screen, { correct: true, picked: true }); }, [done]);
  return (
    <Stage eyebrow={tr({ uz: 'Asos · 2 dastur', ru: 'Основа · 2 программы' })} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: 'Ikkalasini ishga tushiring', ru: 'Запустите обе' })} onClick={onNext} /></>}>
...
const Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const { tip: _tip, rescue: _resc } = useStuckValve(done, pick ? 1 : 0);   // 13-band klapan
  useEffect(() => { if (done && storedAnswer === undefined) onAnswer(screen, { correct: true, picked: true }); }, [done]);
  const choose = (v) => { if (done) return; if (v === 'server') setPick('server'); else { setPick('hard'); setWrong(true); } };
  return (
    <Stage eyebrow={tr({ uz: "1-qadam · ME'MOR", ru: 'Шаг 1 · АРХИТЕКТОР' })} screen={screen} scrollSignal={done || wrong} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: "Ma'lumot manbasini tanlang", ru: 'Выберите источник данных' })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tuzilishi · oqim', ru: 'Строение · поток' })} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: "So'rov yo'lini kuzating", ru: 'Проследите путь запроса' })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
      ...
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        ...
        <div className="split">
          <Col>
            ...
            {!done && <div className="sk-info" key={st}><p className="body" style={{ margin: 0, color: T.ink }}>{tr(STATES.find(s => s.k === st).note)}</p></div>}
...
const Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: '3-qadam · NAZORATCHI', ru: 'Шаг 3 · КОНТРОЛЁР' })} screen={screen} scrollSignal={phase} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: 'Saytni ishga tushiring va tuzating', ru: 'Запустите сайт и почините' })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const { tip: _tip, rescue: _resc } = useStuckValve(done, (added ? 1 : 0) + (refreshed ? 1 : 0));   // 13-band klapan
  useEffect(() => { if (done && storedAnswer === undefined) onAnswer(screen, { correct: true, picked: true }); }, [done]);
  ...
  return (
    <Stage eyebrow={tr({ uz: '4-qadam · forma', ru: 'Шаг 4 · форма' })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: "Qo'shing va yangilang", ru: 'Добавьте и обновите' })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const { tip: _tip, rescue: _resc } = useStuckValve(done, (didAdd ? 1 : 0) + (didRefresh ? 1 : 0));   // 13-band klapan
  useEffect(() => { if (done && storedAnswer === undefined) onAnswer(screen, { correct: true, picked: true }); }, [done]);
      ...
      <div className="screen" style={{ gap: 'clamp(8px,1.2vw,12px)' }}>
        ...
        <div className="split">
          ...
          <Col>
            ...
            <Win title="avtoijara.uz" minH={170} maxH={230}><AvtoSite cars={cars} newId={didAdd ? TRACKER.id : undefined} cols={2} /></Win>
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr({ uz: "💡 Ikki amal kerak: formaga yozib qo'shing, keyin ro'yxatni yangilang.", ru: '💡 Нужны два действия: добавьте через форму, затем обновите список.' })}</p>}
...
const Screen17 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tayyor', ru: 'Готово' })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: 'clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Qaytadan', ru: 'Заново' })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: 'auto', padding: 'clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Yakunlash ✓', ru: 'Завершить ✓' })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: 'Birinchi fullstack ilovangiz', ru: 'Ваше первое fullstack-приложение' })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Front va backni <span className="italic" style={{ color: T.accent }}>o'zingiz uladingiz</span>.</>, ru: <>Фронт и бэк <span className="italic" style={{ color: T.accent }}>вы соединили сами</span>.</> })}</h2>{/* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */}</div><ScoreRing correct={correct} total={total} /></div>
...
// ============================================================ LESSON ROOT
export default function FullstackConnectPracticeLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  return (
    <LangContext.Provider value={lang}>
      <style>{`
        ...
        .db-row { display: grid; grid-template-columns: 36px 1.5fr 1fr 0.7fr; gap: 8px; padding: 8px 12px; align-items: center; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: 12px; color: ${T.ink}; border-top: 1px solid #eee; }
        ...
        .kons-bar { background: #2D2D2D; color: #C9D1D9; font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 11px; padding: 6px 11px; display: flex; align-items: center; gap: 7px; }
        .kons-dot { width: 8px; height: 8px; border-radius: 50%; background: #28c840; }
        .kons-body { background: #1E1E1E; color: #D4D4D4; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: 11.5px; line-height: 1.65; padding: 11px 12px; min-height: 54px; word-break: break-word; }
        ...
        .run-port { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 11px; color: ${T.ink3}; background: ${T.bg}; padding: 2px 8px; border-radius: 6px; }
        ...
        .chain-node, .flow-node { font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: 11.5px; font-weight: 700; padding: 8px 12px; border-radius: 9px; transition: all 0.3s; box-shadow: 0 4px 12px -6px rgba(${T.shadowBase},0.16); }
        .chain-arr, .flow-arr { font-size: 13px; font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; transition: color 0.3s; }
        ...
        .pechat-404 { display: inline-block; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-weight: 800; font-size: 0.92em; letter-spacing: 0.05em; text-transform: uppercase; color: ${T.accent}; background: ${T.accentSoft}; border: 2px solid ${T.accent}; border-radius: 6px; padding: 1px 8px; margin: 0 2px; transform: rotate(-3.5deg); box-shadow: inset 0 0 0 1.5px ${T.paper}, inset 0 0 0 3px ${T.accent}44; }
