...
// ============================================================
// PM M4-D15 — UYGA VAZIFA: «UCH GAPNI JONLI ODAMDA SINANG» (PmLesson14 davomi)
// Darsning To'liq-kapsulasi: «① kod bilmaydigan odamga uch gapingizni aytib bering ② u qaytadan
// so'ragan gapni tuzating ③ tuzatilgan gapni qayta saqlang» — vazifa AYNAN shu.
// 4 bosqich · mezon: TO'RTTALASI bajarilsa «Bajarildi» (HW_PASS_MIN = 4).
//   1) Uch gapim — darsdagi qavat-gaplar (avto: `pm-m4d15-pitch`)
//   2) Suhbat — kimga aytildi + har gap: Tushunishdi 🙂 / Qayta so'rashdi 🤔 (hurmat-shakl, F-0828-09)
//   3) Tuzatish — halol-qulf: qayta so'ralgan gap majburiy tuzatiladi
//   4) Xulosa — 3 savol (Kahoot)
// Validatorlar DARSDAN AYNAN: gap ≥14 · TEXNIK_RE elagi (topilgan so'z ko'rsatiladi).
// Senariy: pm-senariylar/M4-D15-UchQavat-UY.md · Naqsh: PmLesson2.homework (ETALON).
...
// ===== Darsdagi validatorlar (PmLesson14 bilan AYNAN) =====
const GAP_MIN = 14;
const TEXNIK_RE = {
...
const gapUzun = (s) => (s || '').trim().length >= GAP_MIN;
const gapOk = (s) => gapUzun(s) && !texnikSoz(s);

// Qavatlar — darsdagi nom + fe'l bilan aynan
const QAVATLAR = [
...
// ===== 2-bosqich: tinglovchi va gap-belgilari (hurmat-shakl — F-0828-09) =====
const WHO = [
...
// ===== Uch-qavat kartasi (yakun) =====
const FloorsCard = ({ data }) => {
        ...
        return (
          <div key={i} className="ac-row">
            ...
            <span className="ac-v">
              ...
              {isRew
                ? <><s className="ac-old">{gaps[i]}</s> <span className="ac-new">{(rew.text || '').trim()}</span></>
                : <span>{gaps[i]}</span>}
            </span>
          </div>
        );
...
// Gap-maydoni (1- va 3-bosqichda) — texnik-so'z elagi darsdagidek so'zni ko'rsatadi
const GapField = ({ v, setV, idx, pulse }) => {
...
const pitchDone = (pitch) => [0, 1, 2].every(i => gapOk((pitch || [])[i]));

// — 2-BOSQICH: suhbat —
const StageTalk = ({ data, setData, pitch }) => {
      ...
      <div className="frame fade-up d1">
        ...
        {[0, 1, 2].map(i => (
          <div key={i} className="wrow" style={{ borderBottom: i < 2 ? `1px solid ${T.line}` : 'none' }}>
            ...
            <div className="chips" role="radiogroup">
              {MARK.map((m, mi) => (
                <button key={mi} type="button" role="radio" aria-checked={marks[i] === mi} className={`chip sm ${marks[i] === mi ? 'on' : ''}`} onClick={() => setMark(i, mi)}>
...
const talkDone = (d) => typeof (d || {}).who === 'number' && [0, 1, 2].every(i => ((d || {}).marks || [])[i] === 0 || ((d || {}).marks || [])[i] === 1);
const allUnderstood = (d) => [0, 1, 2].every(i => ((d || {}).marks || [])[i] === 0);
...
const StageRew = ({ data, setData, pitch, talk }) => {
  const d = data || {};
      ...
      <div className="frame fade-up d1">
        <div className="chips" role="radiogroup">
          {MODES.map((m, i) => {
            ...
            return (
              <button key={i} type="button" role="radio" aria-checked={mode === i} disabled={locked} className={`chip ${mode === i ? 'on' : ''} ${locked ? 'off' : ''}`} onClick={() => setData({ ...d, mode: i })} title={locked ? tr({ uz: 'Qayta so\'ralgan gap bor — uni tuzating', ru: 'Есть переспрошенная фраза — исправьте её' }) : undefined}>
                <span className="chip-ic" aria-hidden="true">{m.ic}</span>
                <span>{tr(m.label)}</span>
              </button>
            );
...
// ILDIZ-KOMPONENT
// ============================================================
export default function PmLesson14Homework({ lang: langProp, onFinished }) {
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult
            ...
            : cur.key === 'pitch' ? <StagePitch pitch={data.pitch} setPitch={setPitch} />
            : cur.key === 'talk' ? <StageTalk data={data.talk || {}} setData={setStageData('talk')} pitch={data.pitch} />
            : cur.key === 'rew' ? <StageRew data={data.rew || {}} setData={setStageData('rew')} pitch={data.pitch} talk={data.talk || {}} />
