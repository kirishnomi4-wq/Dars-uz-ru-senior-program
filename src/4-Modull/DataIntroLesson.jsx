      ...
      {reveal && answered > 0 && (() => {
        ...
        return (
          <div className={`mstats-verdict ${level}`}>
            {level === 'need' && <p className="mstats-verdict-t">{tr({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> верно — эта тема осталась для класса непонятной. Перед продолжением коротко повторите.</> })}</p>}
    ...
    <Col>
      ...
      <Win title={tr({ uz: "ilova ma'lumot sxemasi", ru: 'схема данных приложения' })} minH={148}>
        <div className="fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, color: T.accent, fontFamily: "'JetBrains Mono', monospace", fontFeatureSettings: '"liga" 0, "calt" 0', fontSize: 10.5 }}>
...
const Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState(storedAnswer ? new Set(['username', 'rasm', 'sarlavha']) : new Set());
          ...
          <Col>
            ...
            <Win title={tr({ uz: "bitta → ko'p", ru: 'один → много' })} minH={150}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, justifyContent: 'center' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: u ? T.accent : T.ink3, fontFamily: "'JetBrains Mono'", fontFeatureSettings: '"liga" 0, "calt" 0', fontSize: 11 }}>
...
const fieldPos = (tableName, colKey, nodes = SCHEMA_NODES) => {
...
const relLine = (fromId, toId, nodes = SCHEMA_NODES) => {
...
// ===== SCREEN 15b — MUSTAQIL: BOG'LOVCHINI O'ZI TOPADI, KEYIN ULAYDI =====
// Farqi s15 dan: FK ustunlar OLDINDAN belgilanmagan. O'quvchi avval qaysi ustun
// bog'lovchi ekanini o'zi aniqlaydi (2 ta), keyingina ularni ulaydi.
const FK_NODES = {
...
const FK_TARGETS = new Set(Object.keys(FK_EXPECTED));

const ScreenFindFk = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const ALL = Object.keys(FK_EXPECTED);
  const [marked, setMarked] = useState(storedAnswer ? new Set(ALL) : new Set());
  ...
  const clickField = (id, col) => {
    ...
    if (marking) {
      ...
      if (col.pk) { setMiss('pk'); bump(id); return; }
      if (FK_TARGETS.has(id)) { setMarked(prev => { const n = new Set(prev); n.add(id); return n; }); return; }
      setMiss('plain'); bump(id); return;
    }
    if (doneRels.has(id)) return;
    if (sel === null) { if (marked.has(id)) setSel(id); return; }
    if (id === FK_EXPECTED[sel]) {
      setDoneRels(prev => { const n = new Set(prev); n.add(sel); return n; });
      setSel(null);
      return;
    }
    if (marked.has(id)) { setSel(id); return; }
    setMiss('wire'); bump(sel); setSel(null);
  };
  // 136-qonun: reset wrongRef ni NOLLAMAYDI — qayta urinish «birinchi urinish» emas.
  const reset = () => { setMarked(new Set()); setDoneRels(new Set()); setSel(null); setMiss(null); };
  const renderNode = (name) => {
  ...
  const navLabel = allDone
    ? tr({ uz: 'Davom etish', ru: 'Продолжить' })
    : marking
      ? `${marked.size}/2 ${tr({ uz: "bog'lovchi", ru: 'связующих' })}`
      : `${doneRels.size}/2 ${tr({ uz: "bog'lanish", ru: 'связей' })}`;
  return (
    <Stage eyebrow={tr({ uz: "Mustaqil · bog'lovchini toping", ru: 'Самостоятельно · найдите связующий' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!allDone} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(8px,1.4vw,14px)' }}>
        ...
        <Mentor>{allDone
          ? tr({ uz: <>Ajoyib! Endi bog'lovchi ustunlarni <b style={{ color: T.ink }}>o'zingiz</b> topa oldingiz. Backendda jadvallar aynan shunday bog'lanadi.</>, ru: <>Отлично! Теперь связующие столбцы вы находите <b style={{ color: T.ink }}>сами</b>. В backend таблицы связываются именно так.</> })
          : marking
...
const Q_LABELS = { 4: "1 — JSON", 6: { uz: "2 — Qator", ru: '2 — Строка' }, 11: { uz: "3 — Bog'lanish", ru: '3 — Связь' }, 14: "4 — FK", 17: { uz: "5 — Sxema", ru: '5 — Схема' }, 18: { uz: "6 — FK topish", ru: '6 — Найти FK' } };
const QUIZ_MS = 15000;
...
export default function DataIntroLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  return (
    <LangContext.Provider value={lang}>
      <style>{`
        ...
        /* === JSON KO'RINISHI === */
        .json-view { background: ${CODE.bg}; color: ${CODE.text}; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: clamp(12px,1.5vw,13.5px); line-height: 1.85; padding: 14px 16px; border-radius: 12px; margin: 0; overflow-x: auto; white-space: pre; box-shadow: 0 8px 22px -6px rgba(${T.shadowBase},0.2); }
        ...
        .dtable th { background: #F0EEE8; color: ${T.ink2}; font-weight: 700; text-align: left; padding: 8px 12px; font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 11.5px; white-space: nowrap; }
        ...
        .tcard-h { background: ${T.ink}; color: #fff; font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 12.5px; padding: 8px 12px; letter-spacing: 0.03em; }
        .tcard-row { display: flex; align-items: center; gap: 7px; padding: 6px 11px; font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 12px; color: ${T.ink}; border-top: 1px solid #EFECE5; transition: background 0.15s; }
        ...
        .cdrag-zone-h { background: ${T.ink}; color: #fff; font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 12.5px; padding: 8px 12px; letter-spacing: 0.03em; }
        .cdrag-zone-body { padding: 9px; display: flex; flex-direction: column; gap: 6px; min-height: 74px; }
        .cdrag-fixed { display: flex; align-items: center; gap: 7px; font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 12px; font-weight: 700; color: ${T.ink2}; padding: 5px 9px; border-radius: 8px; background: ${T.blueSoft}; }
        .cdrag-placed { display: flex; align-items: center; gap: 7px; font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 12px; color: ${T.success}; padding: 6px 9px; border-radius: 8px; background: ${T.successSoft}; }
