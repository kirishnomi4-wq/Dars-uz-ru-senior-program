...
// ============================================================
// PM M4-D12 — UYGA VAZIFA: «SXEMANGIZ O'SADI» (PmLesson13 davomi)
// Darsning To'liq-kapsulasi: «① odamning yangi savolini toping ② unga ustun oching: nomi,
// belgisi, savoli ③ sxemangizni uch shartdan o'tkazing» — vazifa AYNAN shu.
// 4 bosqich · mezon: TO'RTTALASI bajarilsa «Bajarildi» (HW_PASS_MIN = 4).
//   1) Sxemam — darsdagi 3 ustun (avto: `pm-m4d12-sxema`)
//   2) Yangi ustun — odamning yangi savoli → nom + belgi + savol
//   3) Uch shart — halol chip (O'tdi/O'tmadi; o'tmagani izoh bilan)
//   4) Xulosa — 3 savol (Kahoot)
// Validatorlar DARSDAN AYNAN: savol ≥10 va ≥3 so'z · takrorNom · juftOxshash 70%.
// Senariy: pm-senariylar/M4-D12-Sxema-UY.md · Naqsh: PmLesson2.homework (ETALON).
...
// ===== Darsdagi validatorlar (PmLesson13 bilan AYNAN) =====
const NOM_MIN = 4, SAVOL_MIN = 10, SAVOL_SOZ = 3, IZOH_MIN = 8, YANGI_MIN = 10;
const APO = "['\\u02BB\\u2019]";
const normQ = (s) => (s || '').toLowerCase().replace(new RegExp(APO, 'g'), '').replace(/[^a-z0-9а-џ ]+/gi, ' ').replace(/\s+/g, ' ').trim();
...
const savolUzun = (s) => (s || '').trim().length >= SAVOL_MIN && (s || '').trim().split(/\s+/).filter(Boolean).length >= SAVOL_SOZ;
const kimOk = (k) => k === 'ochiq' || k === 'yopiq';
...
// ===== Uch shart (darsdagi s9 shartlari bilan aynan) =====
const SHARTLAR = [
...
const savolNote = (savol, nom, others) => {
...
const colOk = (c, others) => nomOk((c || {}).nom) && kimOk((c || {}).kim) && savolUzun((c || {}).savol) && !savolNote((c || {}).savol, (c || {}).nom, others);

// Ustun-maydonlari (1- va 2-bosqichda)
const ColFields = ({ c, setC, others, pulse, ph }) => {
...
// — 2-BOSQICH: yangi ustun —
const StageYangi = ({ data, setData, cols }) => {
  ...
  return (
    <div className="col">
      ...
      <div className="frame fade-up d1">
        ...
        {yOk && (
          <div className="wrow" style={{ borderTop: `1px solid ${T.line}` }}>
            ...
            <ColFields c={data} setC={(nc) => setData(nc)} others={oldSavollar} pulse={pulseCol}
...
// ILDIZ-KOMPONENT
// ============================================================
export default function PmLesson13Homework({ lang: langProp, onFinished }) {
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult
            ...
            : cur.key === 'cols' ? <StageCols cols={data.cols} setCols={setCols} />
            : cur.key === 'yangi' ? <StageYangi data={data.yangi || {}} setData={setStageData('yangi')} cols={data.cols} />
            : cur.key === 'check' ? <StageCheck data={data.check || {}} setData={setStageData('check')} />
