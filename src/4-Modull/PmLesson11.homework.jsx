...
// ============================================================
// PM M4-D2 — UYGA VAZIFA: «HAR MAYDONGA BITTA SAVOL» (PmLesson11 davomi)
// Darsning To'liq-kapsulasi: «Har maydonga bitta javob (bu maydon bo'lmasa, ilovada nima
// yo'qoladi?) · har javobda bo'lim nomi bo'lsin · yana bittasi — ilova saqlamaydigan narsa» — AYNAN shu.
// 4 bosqich · mezon: TO'RTTALASI bajarilsa «Bajarildi» (HW_PASS_MIN = 4).
//   1) Maydonlarim — darsdagi 3 juftlik (avto: `pm-m4d2-data`)
//   2) Javoblar — har maydonga «bo'lmasa nima yo'qoladi?» (javobda bo'lim nomi SHART)
//   3) Saqlanmaydigan narsa — ataylab saqlanmaydigan yozuv + sababi
//   4) Xulosa — 3 savol (Kahoot)
// Validatorlar DARSDAN AYNAN: maydon ≥8 · bo'lim ≥4 · UMUMIY_SOZ taqiqi · normBolim takror-taqiqi.
// Senariy: pm-senariylar/M4-D2-Maydonlar-UY.md · Naqsh: PmLesson2.homework (ETALON).
...
// ===== Darsdagi validatorlar (PmLesson11 bilan AYNAN) =====
const MAYDON_MIN = 8, BOLIM_MIN = 4, JAVOB_MIN = 10, SABAB_MIN = 10;
const APO = "['\\u02BB\\u2019]";
...
const bolimLen = (s) => (s || '').trim().length >= BOLIM_MIN;
const bolimUmumiy = (s) => bolimLen(s) && anyTest(UMUMIY_SOZ, (s || '').trim());
const javobLen = (s) => (s || '').trim().length >= JAVOB_MIN;
// Kapsula-sharti: javobda O'SHA bo'lim nomining so'zi (>3 harf) qatnashsin
const javobBolimlik = (javob, bolim) => {
...
// — 2-BOSQICH: javoblar —
const StageJavob = ({ data, setData, rows }) => {
...
const skipDone = (d) => ((d || {}).narsa || '').trim().length >= MAYDON_MIN && ((d || {}).sabab || '').trim().length >= SABAB_MIN;
...
// ILDIZ-KOMPONENT
// ============================================================
export default function PmLesson11Homework({ lang: langProp, onFinished }) {
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult
            ...
            : cur.key === 'javob' ? <StageJavob data={data.javob || {}} setData={setStageData('javob')} rows={data.rows} />
            : cur.key === 'skip' ? <StageSkip data={data.skip || {}} setData={setStageData('skip')} />
