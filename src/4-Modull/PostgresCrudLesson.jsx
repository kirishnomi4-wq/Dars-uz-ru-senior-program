...
const TLine = ({ cmd, out, dim }) => (
  <div style={{ fontFamily: "'JetBrains Mono', monospace", fontFeatureSettings: '"liga" 0, "calt" 0', fontSize: 12.5, lineHeight: 1.7 }}>
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tekshiruv · AI xatosini tut', ru: 'Проверка · поймайте ошибку ИИ' })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: 'Xatoni toping va tuzating', ru: 'Найдите и исправьте ошибку' })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        ...
        <Mentor>{tr({ uz: <>AI arzon mahsulotlarni so'radi, lekin so'rov <b style={{ color: T.accent }}>ishlamayapti</b> — baza "bunday jadval yo'q" deyapti. Xato qatorni toping va bosing — u tuzatiladi, so'rov ishga tushadi.</>, ru: <>ИИ запросил дешёвые товары, но запрос <b style={{ color: T.accent }}>не работает</b> — база говорит «такой таблицы нет». Найдите строку с ошибкой и нажмите — она исправится, запрос заработает.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
      ...
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        ...
        <div className="split">
          ...
          <Col>
            ...
            <div className="code-box" style={{ minHeight: 40 }}>{ran ? <TLine out={<span style={{ color: CODE.str }}>{tr({ uz: "✓ INSERT 0 1 — 1 qator qo'shildi", ru: '✓ INSERT 0 1 — добавлена 1 строка' })}</span>} /> : <span style={{ color: CODE.comment, fontFamily: "'JetBrains Mono'", fontFeatureSettings: '"liga" 0, "calt" 0', fontSize: 12 }}>{tr({ uz: 'Run bosilmagan…', ru: 'Run ещё не нажат…' })}</span>}</div>
...
export default function PostgresCrudLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  return (
    <LangContext.Provider value={lang}>
      <style>{`
        ...
        .sql-box { background: ${CODE.bg}; color: ${CODE.text}; font-family: 'JetBrains Mono', monospace; font-feature-settings: "liga" 0, "calt" 0; font-size: clamp(12px,1.5vw,13.5px); line-height: 1.7; padding: 14px 16px; border-radius: 12px; margin: 0; overflow-x: auto; white-space: pre-wrap; box-shadow: 0 8px 22px -6px rgba(${T.shadowBase},0.2); }
        ...
        .shop-narx { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 12px; color: ${T.accent}; font-weight: 700; }
