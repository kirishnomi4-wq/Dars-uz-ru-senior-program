...
// ============================================================
// 4c-MODUL (CI/CD) · DARS 1 — «CI/CD NIMA VA NEGA KERAK» — PLATFORM STANDARD v18 (AUDIOSIZ)
// Maqsad: o'quvchi CI/CD nima ekanini, CI va CD farqini, pipeline (lenta) tushunchasini va
//         nega avtomatlashtirish kerakligini tushunadi — hamda o'zi bir lenta ishga tushirishni yig'adi.
// ✈️ BIRLASHTIRUVCHI DUNYO — UCHISH LENTASI (5 darsga yagona lug'at):
...
// INTERAKTIV BEAT'lar: s2 «Qo'lda yuk tashish» (5 nuqta + o'suvchi taymer) · s3 «Lentani yoqing» (4s auto) ·
//   s5 CI «Uch chamadon, bitta lenta» · s6 CD ikki rejim · s9 MARKAZIY: «Chamadonni yig'ing va lentaga qo'ying» ·
//   s13 «Yo'l xaritasini to'ldiring» (ci.yml bo'shliqlari) · s15 FINAL: tartib + oqibat (DragDropOrder).
...
const LESSON_META = { lessonId: 'cicd-intro-4c-01-v18', lessonTitle: { uz: 'CI/CD nima va nega kerak', ru: 'Что такое CI/CD и зачем' } };
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return (
    <div className="mstats fade-up">
      ...
      {reveal && struggling && <p className="mstats-warn">{tr({ uz: "⚠️ Ko'pchilik xato qildi — bu mavzu tushunarsiz bo'lgan ko'rinadi. Yana bir bor tushuntiring.", ru: '⚠️ Большинство ошиблось — похоже, тема осталась непонятной. Объясните её ещё раз.' })}</p>}
...
// ══ KLAPAN — IPUCHA-ZINAPOYA + RESCUE (KATTA_TOZALASH 13-band) ══
//
// Uch daraja (138-qonun): QULF-YORLIQ aytadi -> IPUCHA chuqurlashtiradi -> RESCUE yo'l ochadi.
// Bu darsda 1-daraja TUG'MA bajarilgan (12 qulf-joyining hammasida yorliq harakatni aytadi).
//
// 🔴 BALLIK EKRANLAR: `QuestionScreen` (s4·s8·s10·s14) va `Screen15` (s15) — 138-qonun
// qo'shimchasi bo'yicha ularning rescue-yorlig'i «Davom etish» EMAS, harakat-nom:
// «Javobni birga ko'ramiz →» va «Lentani birga yig'amiz →». Ipuchalar umumiy
// komponentga `tip` propi bilan keladi — har savol O'Z mazmunidan gapiradi.
//
// IKKI O'LCHOV: `sec` — ekran ochiq turgan vaqt · `idle` — oxirgi HAQIQIY siljishdan beri.
// 🔴 MUSTAQILLIK: hook komponent ICHIDA — setInterval faqat o'sha ekran ochiq turganda
// yuradi va ekran almashganda cleanup bilan to'xtaydi.
...
const QuestionScreen = ({ screen, idx, scope, eyebrow, question, questionText, options, correctIdx, explainCorrect, explainWrong, audioText, audioOk, audioWrong, storedAnswer, tip, onAnswer, onNext, onPrev }) => {
  ...
  const { tip: _tip, rescue: _resc } = useStuckValve(solved, picked === null ? -1 : picked);   // 13-band klapan
...
// ===== ★ CHAMADON BUYUMLARI (s9 markaziy o'yin manbai) =====
const ITEMS = [
...
// ===== SCREEN 0 — HOOK: qo'lda uchirish charchatadi =====
const Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  // 137-qonun: har tanlovga O'Z javobi; «Aynan!» faqat to'g'ri tanlovda; xato tanlovga
  // ko'prik-gap (uyaltirish emas — dars nimani ko'rsatishini VA'DA qiladi).
  // Naqsh-manba: 4a-02 `NestArchResourceLesson` ACK xaritasi (F-0820-387).
  const ACK = {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Kirish', ru: 'Введение' })} screen={screen} scrollSignal={sc} navContent={<NavNext optionalLive disabled={picked === null} label={tr({ uz: 'Davom etish', ru: 'Продолжить' })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr({ uz: <>Kodni yangiladingiz — endi uni internetga chiqarish uchun <span className="italic" style={{ color: T.accent }}>nechta amal</span> kerak?</>, ru: <>Вы обновили код — сколько теперь нужно <span className="italic" style={{ color: T.accent }}>действий</span>, чтобы выложить его в интернет?</> })}</h1>
...
// ===== SCREEN 2 — QO'LDA YUK TASHISH (charchatadi) =====
const Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const tap = (id) => { setActive(id); setSkipped(false); setSc(n => n + 1); setSeen(prev => { const s = new Set(prev); s.add(id); return s; }); };
  const tempt = () => { setSkipped(true); setSc(n => n + 1); };
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1 cj-items">
              {POINTS.map(p => <button key={p.id} className={`itm-card ${seen.has(p.id) ? '' : 'tap-hint'}`} onClick={() => tap(p.id)} style={seen.has(p.id) ? { boxShadow: `inset 0 0 0 1.5px ${T.success}` } : undefined}><span className="itm-ico">{p.icon}</span><span className="itm-nm">{seen.has(p.id) ? '✓ ' : ''}{tr(p.label)}</span></button>)}
            </div>
...
// ===== SCREEN 3 — LENTANI YOQING (avtomatlashtirish) =====
const Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [ptStatus, setPtStatus] = useState({});
  const [running, setRunning] = useState(false);
  const [done, setDone] = useState(!!storedAnswer);
  const { tip: _tip, rescue: _resc } = useStuckValve(done, Object.keys(ptStatus).length);   // 13-band klapan
  const [sc, setSc] = useState(0);
  ...
  const run = () => {
    ...
    const tick = (i) => {
      if (i >= POINTS.length) { setRunning(false); setDone(true); setSc(n => n + 1); return; }
      setPtStatus(st => ({ ...st, [POINTS[i].id]: 'run' }));
      timer.current = setTimeout(() => { setPtStatus(st => ({ ...st, [POINTS[i].id]: 'pass' })); setSc(n => n + 1); timer.current = setTimeout(() => tick(i + 1), 60); }, 740);
    };
    tick(0);
  };
  return (
    <Stage eyebrow={tr({ uz: 'Tushuncha · lenta', ru: 'Понятие · лента' })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: 'Lentani yoqing', ru: 'Включите ленту' })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
// ===== SCREEN 4 — TEST 1 =====
const Screen4 = (props) => (
  <QuestionScreen {...props} idx={4} scope="module-mikro" tip={{ uz: "💡 Dars boshida qo'lda 5 nuqtani o'tkazgansiz — lenta o'sha ishning qaysi qismini o'z ustiga oladi?", ru: '💡 В начале урока вы вручную прошли 5 точек — какую часть этой работы лента берёт на себя?' }} eyebrow={tr({ uz: 'Mashq · 1-savol', ru: 'Практика · вопрос 1' })}
...
// ===== SCREEN 5 — CI (uch chamadon, bitta lenta) =====
const SUITCASES = [
...
const Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState(storedAnswer ? new Set(SUITCASES.map(d => d.id)) : new Set());
  const [active, setActive] = useState(null);
  const [sc, setSc] = useState(0);
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tushuncha · CI', ru: 'Понятие · CI' })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: `Chamadonlarni ko'ring (${seen.size}/3)`, ru: `Посмотрите чемоданы (${seen.size}/3)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [seen, setSeen] = useState(storedAnswer ? new Set(CD_MODES.map(m => m.id)) : new Set());
  const [active, setActive] = useState(null);
  const [sc, setSc] = useState(0);
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tushuncha · CD', ru: 'Понятие · CD' })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: 'Ikkala rejimni sinang', ru: 'Попробуйте оба режима' })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
// ===== ★ SCREEN 9 — MARKAZIY O'YIN: «CHAMADONNI YIG'ING VA LENTAGA QO'YING» =====
...
const Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const { tip: _tip, rescue: _resc } = useStuckValve(done, (result ? 1 : 0) + (everSucceeded ? 1 : 0));   // 13-band klapan
  useEffect(() => { if (done && storedAnswer === undefined) onAnswer(screen, { stage: 'case', screenIdx: screen, question: "Chamadonni yig'ing va lentaga to'liq yashil o'tkazing", correct: true, solved: true, picked: true }); }, [done]); // eslint-disable-line

  const resetRunState = () => { setResult(null); setPtStatus({}); setLog([]); };
  const toggleItem = (id) => { if (running) return; setSelected(prev => { const s = new Set(prev); if (s.has(id)) s.delete(id); else s.add(id); return s; }); resetRunState(); };
  const fixItem = (id) => { if (running) return; setFixed(prev => new Set(prev).add(id)); resetRunState(); };
  const pushLog = (text, tone) => setLog(l => [...l, { text, tone, k: l.length }]);

  const runBelt = () => {
    if (running || selected.size === 0) return;
    runningRef.current = true; setRunning(true); resetRunState(); setSc(n => n + 1);
    const chosen = ITEMS.filter(it => selected.has(it.id));
    let outcome = null;
    for (const p of POINTS) {
      const bad = chosen.find(it => it.failAt === p.id && !fixed.has(it.id));
      if (bad) { outcome = { at: p.id, item: bad }; break; }
    }
    const tick = (i) => {
      if (i >= POINTS.length) {
        pushLog(tr({ uz: "▸ ✈️ Uchirish — samolyot ko'tarildi ✓", ru: '▸ ✈️ Взлёт — самолёт поднялся ✓' }), 'ok');
        setResult('success'); setEverSucceeded(true); setRunning(false); runningRef.current = false; setSc(n => n + 1);
        return;
      }
      const p = POINTS[i];
      ...
      timer.current = setTimeout(() => {
        if (outcome && outcome.at === p.id) {
          setPtStatus(st => { const ns = { ...st, [p.id]: 'fail' }; POINTS.slice(i + 1).forEach(sp => { ns[sp.id] = 'skip'; }); return ns; });
          ...
          setResult('fail'); setRunning(false); runningRef.current = false; setSc(n => n + 1);
          return;
        }
        setPtStatus(st => ({ ...st, [p.id]: 'pass' }));
        pushLog(pointOkLine(p), 'ok'); setSc(n => n + 1);
...
// ===== SCREEN 11 — NEGA AVTOMATLASHTIRAMIZ (3 foyda) =====
const WHY = [
...
const Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tushuncha · nega', ru: 'Понятие · зачем' })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: `3 foydani ko'ring (${seen.size}/3)`, ru: `Посмотрите 3 выгоды (${seen.size}/3)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [sc, setSc] = useState(0);
  const done = show;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, show ? 1 : 0);   // 13-band klapan
  useEffect(() => { if (done && storedAnswer === undefined) onAnswer(screen, { correct: true, picked: true }); }, [done]); // eslint-disable-line
  return (
    <Stage eyebrow={tr({ uz: 'Hayotiy · jamoa', ru: 'Из жизни · команда' })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: "Ikkinchi holatni ko'ring", ru: 'Посмотрите второй случай' })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
        ...
        <div className="split">
          <Col>
            ...
            <button className="btn" style={{ alignSelf: 'flex-start' }} disabled={show} onClick={() => { setShow(true); setSc(n => n + 1); }}>{show ? tr({ uz: "✓ Ko'rdingiz", ru: '✓ Просмотрено' }) : tr({ uz: 'Lenta bilan-chi?', ru: 'А если с лентой?' })}</button>
          </Col>
          <Col>
            {show
...
// ===== SCREEN 13 — BUILDER: YO'L XARITASINI TO'LDIRING (ci.yml) — Route Reader badge =====
const YAML_BLANKS = [
...
const Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const { tip: _tip, rescue: _resc } = useStuckValve(done, Object.keys(filled).length);   // 13-band klapan
  ...
  const pick = (blank, val) => {
    ...
    else { if (achMiss) achMiss.miss(screen); wrongEverRef.current = true; setWrongKey(blank.key); setWrongMsg(tr(blank.wrong[val]) || tr({ uz: "Bu to'g'ri emas.", ru: 'Это неверно.' })); setTimeout(() => setWrongKey(k => (k === blank.key ? null : k)), 500); }
  };
  return (
    <Stage eyebrow={tr({ uz: "Amaliyot · yo'l xaritasi", ru: 'Практика · маршрутная карта' })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: "Bo'shliqlarni to'ldiring", ru: 'Заполните пропуски' })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen14 = (props) => (
  <QuestionScreen {...props} idx={14} scope="module-mikro" tip={{ uz: "💡 `on:` qatori nimani anglatishini eslang — lentaga start signalini kim beradi?", ru: '💡 Вспомните, что означает строка `on:` — кто подаёт ленте стартовый сигнал?' }} eyebrow={tr({ uz: 'Mashq · 4-savol', ru: 'Практика · вопрос 4' })}
...
// ===== SCREEN 15 — FINAL: tartib + oqibat (Order Matters badge) =====
const BELT_ITEMS = POINTS.map(p => ({ id: p.id, label: { uz: `${p.icon} ${p.label.uz}`, ru: `${p.icon} ${p.label.ru}` } }));
const BELT_ORDER = POINTS.map(p => p.id);
const Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [done, setDone] = useState(!!storedAnswer);
  const { tip: _tip, rescue: _resc } = useStuckValve(done, done ? 1 : 0);   // 13-band klapan
  const [consequence, setConsequence] = useState(null); // null | 'plane-early' | 'wrong'
...
// Ekran id → nishon. ❗ FAQAT ma'noli ekranlar: s10 (SCORED test) · s9 (markaziy o'yin — to'liq yashil) ·
// s13 (builder — 3 bo'shliq xatosiz) · s15 (final — 1-urinishda to'g'ri tartib). Exploration ekranlarga BOG'LANMAYDI.
const ACH_TRIGGERS = { s9: 'clearedForTakeoff', s10: 'redLight', s13: 'routeReader', s15: 'orderMatters' };
...
const Q_LABELS = { 4: '1 — CI/CD', 8: { uz: '2 — CI/CD farqi', ru: '2 — разница CI/CD' }, 10: { uz: '3 — Qizil chiroq', ru: '3 — красный свет' }, 14: { uz: '4 — Start signali', ru: '4 — стартовый сигнал' }, 15: { uz: '5 — Tartib', ru: '5 — порядок' } };
const QUIZ_MS = 15000;
// Kapsula ichida suzuvchi tokenlar — darsning "DNK"si (CI/CD atamalari)
const QZ_BG_SHAPES = [
...
// Manba-etalon: m3-09 `ReactApiPostLesson` (kanonik nusxa).
// 🔴 F-0820-283 saboqi: komponent ko'chirilganda `.done-mini` CSS'i ham BIRGA ko'chadi.
...
const ScreenCicdPractice = (props) => (
...
// 🃏 FLASHCARD KARTALARI — 12 savol (uchish lentasi tili); front=savol, back=qisqa javob, note=izoh
const CICD_FLASHCARDS = [
...
const SummaryScreen = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tayyor', ru: 'Готово' })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: 'clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Qaytadan', ru: 'Заново' })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: 'auto', padding: 'clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Yakunlash ✓', ru: 'Завершить ✓' })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: 'CI/CD lentasini tushundingiz', ru: 'Вы разобрались в ленте CI/CD' })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Endi kod o'zgarishini <span className="italic" style={{ color: T.accent }}>lenta</span> chiqaradi.</>, ru: <>Теперь изменения кода выкатывает <span className="italic" style={{ color: T.accent }}>лента</span>.</> })}</h2>{/* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */}</div><ScoreRing correct={correct} total={total} /></div>
...
export default function CiCdIntroLesson({ lang: langProp, onFinished, liveToken }) {
...
/* PRODUCTION: shu @import OLIB TASHLANADI — shriftlarni LMS yuklaydi (platform_contract). */
        ...
        .pipe-badge { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 11px; font-weight: 800; min-height: 13px; line-height: 1; color: ${T.ink3}; }
        ...
        .timer-chip .tc-val { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 800; font-size: clamp(20px,3vw,28px); color: ${T.ink}; }
        ...
        .phone-ok { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 9px; font-weight: 700; }
        ...
        .belt-log-line { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 12px; line-height: 1.7; color: ${CODE.text}; animation: el-pop 0.25s ease-out; white-space: pre-wrap; word-break: break-word; }
        .belt-log-line.ok { color: #7DD181; }
        .belt-log-line.bad { color: #FF8B7A; }
        .belt-log-line.skip { color: ${CODE.comment}; font-style: italic; }
        .belt-log-empty { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 12px; color: ${CODE.comment}; font-style: italic; }
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        <div className="lesson-root">
          {live.mode === 'choosing' ? (
            <LiveGate live={live} title={{ uz: 'CI/CD darsi', ru: 'Урок CI/CD' }} />
          ) : (
            <>
