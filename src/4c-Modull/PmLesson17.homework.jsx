...
// ============================================================
// PM M4C-D2 — UYGA VAZIFA: «REJANGIZ DAVOM ETADI» (PmLesson17 davomi)
// Darsning To'liq-kapsulasi (HW_STEPS.toliq AYNAN): «① to'rtinchi haftaga bitta bo'lak yozing
// ② har bo'lakka savol bering: odam buni bosganda nima bo'ladi? ③ javobini bir gapda yozib
// qo'ying» — vazifa AYNAN shu.
// 4 bosqich · mezon: TO'RTTALASI bajarilsa «Bajarildi» (HW_PASS_MIN = 4).
//   1) Bo'laklarim — darsdagi 3 haftalik bo'lak (avto: `pm-m4c2-reliz`)
//   2) 4-hafta — yangi bo'lak
//   3) Bosganda nima bo'ladi? — to'rtala bo'lakka bir gapli javob
//   4) Xulosa — 3 savol (Kahoot)
// Validatorlar DARSDAN AYNAN (canSave bloklari): sozSoni≥3 · KATTA_SOZ · ikkiIsh · normIsh-takror;
// 3-bosqich javobi darsning fe'l-lug'ati (NATIJA/HARAKAT) bilan tekshiriladi.
// Senariy: pm-senariylar/M4C-D2-Reliz-UY.md · Naqsh: PmLesson2.homework (ETALON).
...
// ===== Darsdagi validatorlar (PmLesson17 bilan AYNAN) =====
const SOZ_MIN = 3;
const APO = "['\\u02BB\\u2019]";
...
const uzunOk = (s) => sozSoni(s) >= SOZ_MIN;
const kattaMi = (s) => uzunOk(s) && anyTest(KATTA_SOZ, s);
// «Va» tekshiruvi darsdagidek: harakat+natija bitta ish, ikki AKTIV harakat — ikkita ish.
const ikkiIsh = (s) => {
...
const juftMi = (s) => uzunOk(s) && ikkiIsh(s);
const ishTakror = (list, i) => {
  const me = normIsh((list[i] || {}).ish);
  return !!me && list.some((r, j) => j !== i && normIsh((r || {}).ish) === me);
};
const ishFull = (r) => uzunOk((r || {}).ish) && !kattaMi((r || {}).ish) && !juftMi((r || {}).ish);
// 3-bosqich javobi: bir gap + darsning fe'l-lug'atidan belgi (nima BO'LADI).
const javobOk = (s) => uzunOk(s) && (anyTest(NATIJA, s) || anyTest(HARAKAT, s));
...
// Bitta bo'lak-qator (1- va 2-bosqichda) — darsdagi 🤔 savol-notelari bilan.
const IshBlock = ({ r, setR, pulse, ph, takror }) => {
  const s = (r || {}).ish || '';
...
// — 3-BOSQICH: bosganda nima bo'ladi? —
const StageJavob = ({ data, setData, rows, yangi }) => {
  ...
  return (
    <div className="col">
      ...
      {!ready ? (
        <div className="frame-warn fade-up d1"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: "Avval 1–2-bosqichlarda to'rtala bo'lakni tugating — savol o'sha bo'laklarga beriladi.", ru: 'Сначала завершите все четыре кусочка на 1–2-м этапах — вопрос задаётся этим кусочкам.' })}</p></div>
      ) : (
        <div className="frame fade-up d1" style={{ padding: 'clamp(6px,1.2vw,10px) clamp(14px,2.2vw,20px)' }}>
          {all.map((r, i) => {
            const v = jav[i] || '';
            const jOk = javobOk(v);
            return (
              <div key={i} className="wrow" style={{ borderBottom: i < 3 ? `1px solid ${T.line}` : 'none' }}>
...
// ILDIZ-KOMPONENT
// ============================================================
export default function PmLesson17Homework({ lang: langProp, onFinished }) {
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult
            ...
            : cur.key === 'javob' ? <StageJavob data={data.javob || {}} setData={setStageData('javob')} rows={data.rows} yangi={data.yangi} />
