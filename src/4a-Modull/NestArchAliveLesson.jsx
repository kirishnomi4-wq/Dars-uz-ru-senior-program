...
// Bu darsda 1-daraja ALLAQACHON bajarilgan: 13 qulflangan ekranning HAMMASIDA yorliq
// harakatni aytadi (Screen9 da u hisoblanadigan `navLabel`). Shuning uchun 2 va 3-daraja.
//
...
// yuradi, ekran almashganda cleanup bilan to'xtaydi. 12 ta ipucha TO'PLANMAYDI.
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Kirish', ru: 'Введение' })} screen={screen} scrollSignal={sc} navContent={<NavNext optionalLive disabled={picked === null} label={tr({ uz: 'Davom etish', ru: 'Продолжить' })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr({ uz: <>Katta backend bitta faylga sig'adimi — uni <span className="italic" style={{ color: T.accent }}>boshqarib bo'ladimi</span>?</>, ru: <>Поместится ли большой бэкенд в один файл — и можно ли им <span className="italic" style={{ color: T.accent }}>управлять</span>?</> })}</h1>
...
const Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: '1-qadam · clone', ru: 'Шаг 1 · clone' })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: '2 qadamni bajaring', ru: 'Выполните 2 шага' })} onClick={onNext} /></>}>
...
const Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const go = () => { setStep(s => Math.min(s + 1, 2)); setSc(n => n + 1); };
  return (
    <Stage eyebrow={tr({ uz: '1-qadam · ishga tushirish', ru: 'Шаг 1 · запуск' })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: 'Serverni yoqing', ru: 'Включите сервер' })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: '2-qadam · Swagger', ru: 'Шаг 2 · Swagger' })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: 'Bitta eshikni sinang', ru: 'Попробуйте одну дверь' })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  useEffect(() => { if (done && storedAnswer === undefined) onAnswer(screen, { correct: true, picked: true }); }, [done]); // eslint-disable-line
  return (
    <Stage eyebrow={tr({ uz: 'Tushuncha · Entity', ru: 'Понятие · Entity' })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: "BaseEntity nima berishini ko'ring", ru: 'Посмотрите, что даёт BaseEntity' })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  useEffect(() => { if (done && storedAnswer === undefined) onAnswer(screen, { correct: true, picked: true }); }, [done]); // eslint-disable-line
  return (
    <Stage eyebrow={tr({ uz: 'Tushuncha · DTO', ru: 'Понятие · DTO' })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={(done || _resc) ? tr({ uz: 'Davom etish', ru: 'Продолжить' }) : tr({ uz: "To'g'ri va xato so'rovni sinang", ru: 'Попробуйте верный и неверный запросы' })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: 'clamp(10px,1.6vw,16px)' }}>
...
const Screen18 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [resName, setResName] = useState(() => (storedAnswer ? (typeof storedAnswer.picked === 'string' && storedAnswer.picked ? storedAnswer.picked : 'Task') : '')); /* F-0915-02 */
  ...
  const { tip: _tip, rescue: _resc } = useStuckValve(done, built.length);   // 13-band klapan
...
const Screen19 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const done = moved;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, moved ? 1 : 0);   // 13-band klapan
  const pickLine = (id) => {
    if (found) return;
...
// Manba-etalon: m3-09 `ReactApiPostLesson`; 4-Modul va 4a da standart.
...
const Screen20 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return (
    <Stage eyebrow={tr({ uz: 'Tayyor', ru: 'Готово' })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: 'clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Qaytadan', ru: 'Заново' })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: 'auto', padding: 'clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)', fontSize: 'clamp(13px,1.5vw,15px)' }}>{tr({ uz: 'Modulni yakunlash →', ru: 'Завершить модуль →' })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr({ uz: "Arxitekturani tirik ko'rdingiz", ru: 'Вы увидели архитектуру вживую' })}</span><h2 className="title h-title fade-up d1">{tr({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>kim qayerda turishini</span> bilasiz.</>, ru: <>Теперь вы знаете, <span className="italic" style={{ color: T.accent }}>кто где стоит</span>.</> })}</h2>{/* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */}</div><ScoreRing correct={correct} total={total} /></div>
...
export default function NestArchAliveLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  return (
    <LangContext.Provider value={lang}>
      <style>{`
        ...
        .tree-root { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 12.5px; color: ${T.ink}; padding: 4px 6px; }
        ...
        .tree-folder { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 12.5px; color: ${T.ink}; }
        ...
        .tree-kid { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 11px; color: ${T.ink2}; background: ${T.bg}; padding: 5px 9px; border-radius: 7px; }
        ...
        .wire-box { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 11.5px; background: var(--zbg, ${T.bg}); color: var(--zink, ${T.ink}); padding: 8px 11px; border-radius: 8px; box-shadow: inset 0 0 0 1.5px var(--zline, ${T.line}); opacity: 0.5; filter: saturate(0.45); transition: opacity 0.25s, filter 0.25s, box-shadow 0.25s; }
        ...
        .wire-arr { position: relative; font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 10.5px; font-weight: 700; color: ${T.ink3}; transition: color 0.3s; }
        ...
        .prompt-input { flex: 1; min-width: 0; font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 14px; padding: 11px 14px; border: none; border-radius: 12px; background: ${T.paper}; color: ${T.ink}; outline: none; box-shadow: inset 0 0 0 1.5px ${T.ink3}40; transition: box-shadow 0.2s; }
        ...
        .gen-step { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 11px; padding: 6px 10px; border-radius: 99px; background: ${T.paper}; color: ${T.ink3}; box-shadow: inset 0 0 0 1px ${T.ink3}30; transition: all 0.25s; }
        ...
        .gen-line { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: 12px; color: ${CODE.attr}; margin: 4px 2px 0; } .gen-line::after { content: '…'; animation: blink 1s steps(3) infinite; } @keyframes blink { 0% { opacity: 0.3; } 50% { opacity: 1; } 100% { opacity: 0.3; } }
        ...
        .fs-name { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 700; font-size: 11.5px; color: #C9D1D9; background: #243049; padding: 7px 11px; display: flex; align-items: center; gap: 7px; } .fs-ico { font-size: 14px; }
        .fs-code { font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-size: clamp(11px,1.4vw,12.5px); line-height: 1.55; color: ${CODE.text}; padding: 10px 12px; margin: 0; white-space: pre-wrap; word-break: break-word; }
        ...
        .rz-n { position: absolute; top: 4px; left: 6px; font-family: 'JetBrains Mono'; font-feature-settings: "liga" 0, "calt" 0; font-weight: 800; font-size: 10px; color: ${T.ink3}; z-index: 1; }
