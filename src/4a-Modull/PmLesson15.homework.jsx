...
// ============================================================
// PM M4A-D2 — UYGA VAZIFA: «QARORLARINGIZ DAVOM ETADI» (PmLesson15 davomi)
// Darsning To'liq-kapsulasi: «① chipta saytining yana bir qismini toping ② unga qaror va sabab
// yozing ③ qarorlaringizdan birinchi sinadiganini belgilang» — vazifa AYNAN shu.
// 4 bosqich · mezon: TO'RTTALASI bajarilsa «Bajarildi» (HW_PASS_MIN = 4).
//   1) Qarorlarim — darsdagi 3 qaror (avto: `pm-m4a2-yuk`)
//   2) 4-qaror — saytning yana bir qismi + qaror + sabab
//   3) Birinchi sinadigan — 4 talikdan belgilash + yuk-belgili sabab
//   4) Xulosa — 3 savol (Kahoot)
// Validatorlar DARSDAN AYNAN: normQism-takror · BOSH_SOZ · ZID (OGIR/YENGIL belgi) · hammaKuch.
// Senariy: pm-senariylar/M4A-D2-Yuk-UY.md · Naqsh: PmLesson2.homework (ETALON).
...
// ===== Darsdagi validatorlar (PmLesson15 bilan AYNAN) =====
const QISM_MIN = 3, SABAB_MIN = 6;
const APO = "['\\u02BB\\u2019]";
const rx = (s) => new RegExp(s.replace(/'/g, APO), 'i');
...
const BOSH_SOZ = { uz: rx('muhim|kerak|yaxshi|foydali|qulay'), ru: rx('важн|нужн|хорош|полезн|удобн') };
...
const qismOk = (s) => (s || '').trim().length >= QISM_MIN;
const sababLen = (s) => (s || '').trim().length >= SABAB_MIN;
const KUCH = 'kuchaytiramiz', ODDIY = 'oddiy'; // darsdagi saqlov-qiymatlar AYNAN (setDQaror)
const qarorOk = (q) => q === KUCH || q === ODDIY;
// ZID: qaror «kuchaytiramiz» + sabab faqat yengil-belgili (yoki aksincha)
const zidMi = (qaror, sabab) => {
...
// BOSH_SOZ: sabab FAQAT bosh-so'zdan iborat bo'lsa (yuk-belgisi yo'q)
const boshMi = (sabab) => sababLen(sabab) && anyTest(BOSH_SOZ, sabab) && !anyTest(OGIR_BELGI, sabab) && !anyTest(YENGIL_BELGI, sabab);
...
const sababNote = (qaror, sabab) => {
  ...
  if (zidMi(qaror, sabab)) return qaror === KUCH
    ? tr({ uz: 'Sabab yengil yukni aytyapti, qaror esa «kuchaytiramiz» — ikkovi zid. Qaysi biri to\'g\'ri: qarormi, sababmi?', ru: 'Причина говорит о лёгкой нагрузке, а решение — «усиливаем». Они противоречат. Что верно: решение или причина?' })
    : tr({ uz: 'Sabab og\'ir yukni aytyapti, qaror esa «oddiy qoladi» — ikkovi zid. Qaysi biri to\'g\'ri: qarormi, sababmi?', ru: 'Причина говорит о тяжёлой нагрузке, а решение — «остаётся простым». Они противоречат. Что верно?' });
  return null;
};
const rowFull = (r) => qismOk((r || {}).qism) && qarorOk((r || {}).qaror) && sababLen((r || {}).sabab) && !sababNote((r || {}).qaror, (r || {}).sabab);
const qismTakror = (list, i) => {
  const me = normQism((list[i] || {}).qism);
  return !!me && list.some((r, j) => j !== i && normQism((r || {}).qism) === me);
};

// Bitta qaror-blok (1- va 2-bosqichda)
const QarorBlock = ({ r, setR, pulse, phQism, takror }) => {
  ...
  return (
    <>
      ...
      <div className="chips" role="radiogroup" style={{ marginBottom: 8 }}>
        {QAROR_OPTS.map(o => (
          <button key={o.key} type="button" role="radio" aria-checked={r.qaror === o.key} className={`chip sm ${r.qaror === o.key ? 'on' : ''}`} onClick={() => setR({ ...r, qaror: o.key })}>
...
const hammaKuch = (rows) => rows.filter(r => (r || {}).qaror === KUCH).length === rows.length && rows.length >= 3;
...
// — 2-BOSQICH: 4-qaror —
const StageYangi = ({ data, setData, rows }) => {
...
const yangiDone = (d, rows) => {
  const all = [...[0, 1, 2].map(i => (rows || [])[i] || {}), d || {}];
  return rowFull(d || {}) && !qismTakror(all, 3);
};

// — 3-BOSQICH: birinchi sinadigan —
const StageFirst = ({ data, setData, rows, yangi }) => {
    ...
    <div className="col">
      ...
      <div className="frame fade-up d1">
        ...
        <div className="chips" role="radiogroup">
          {all.map((r, i) => (
            <button key={i} type="button" role="radio" aria-checked={idx === i} disabled={!qismOk(r.qism)} className={`chip ${idx === i ? 'on' : ''}`} onClick={() => setData({ ...data, idx: i })}>
...
const firstDone = (d) => typeof (d || {}).idx === 'number' && (d || {}).idx >= 0 && sababLen((d || {}).sabab) && (anyTest(OGIR_BELGI, (d || {}).sabab) || !anyTest(BOSH_SOZ, (d || {}).sabab));
...
// ILDIZ-KOMPONENT
// ============================================================
export default function PmLesson15Homework({ lang: langProp, onFinished }) {
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult
            ...
            : cur.key === 'yangi' ? <StageYangi data={data.yangi || {}} setData={setStageData('yangi')} rows={data.rows} />
            : cur.key === 'first' ? <StageFirst data={data.first || {}} setData={setStageData('first')} rows={data.rows} yangi={data.yangi} />
