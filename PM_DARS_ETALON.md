...
**2026-07-30 EKRAN-YUKI (92) — PmLesson3 fidbek-raundidan chiqqan QAT'IY TAQIQ:**

92. 🔴 **BIR EKRAN — BIR ISH. EKRAN BULG'ANIB TURMAYDI.** Ildiz-sabab: quruvchi «foydali» bo'laklarni bitta ekranga qalashtiradi (yozuv-maydonlari + namuna-chiplar + yon-karta + taymer + ro'yxat), natijada o'quvchi ekranni ochib **o'qishni boshlamaydi** — foydalanuvchi dalili: «juda ko'p ma'lumot, kirgim kelmayapti», «bo'limlarni bir qarashda o'qigim ham kelmayapti». Qat'iy bandlar: **(a)** 🔴 **har ekranda o'quvchidan SO'RALADIGAN ish BITTA bo'ladi** — yozish YOKI tanlash YOKI gapirish; ikkitasi kerak bo'lsa, ekran ikkiga bo'linadi (ekran soni ko'paygani yomon emas — ekran zichligi yomon); **(b)** 🔴 **yakuniy/ustaxona ekranida yig'ma-karta TO'LIQ holatda turmaydi** — dars o'rtasida u yig'ilgan (bir qatorlik holat: bo'lak nomlari + ✓ + «3/6»), to'liq matn faqat o'sha matn ustida ish qilinadigan ekranda ochiladi; **(c)** 🔴 **maydon ipuchasi qisqa buyruq bo'ladi** («Savolingizni yozing…»), tayyor javob-namunasi maydonda TURMAYDI va ustiga bosiladigan namuna-chiplar QO'YILMAYDI — namuna mentor gapida BIR marta aytiladi; **(d)** 🔴 **majburiy maydon faqat o'quvchida ANIQ bor bo'lgan ma'lumot uchun qo'yiladi.** Tashqi ma'lumot (sayt manzili, hisob nomi) so'ralsa — **ixtiyoriy**, aks holda o'sha ekran o'tib bo'lmas devorga aylanadi (PmLesson3: netlify manzili majburiy edi — foydalanuvchining o'zi ham o'tolmadi); **(e)** 🔴 **gapirish ekranida yozish maydoni ochiq turmaydi** — matn o'qish holatida (teleprompter) ko'rsatiladi, tahrir «✎ Tahrirlash» ortida yashiriladi. Tekshiruv-usuli: har ekranni SSR-render qilib **matn uzunligini o'lchang**; interaktiv ekran ~400–800 belgidan oshsa — bo'lish nomzodi. Mas'ul: `pm-quruvchi` + `pm-dizayn`; tekshiruv: `pm-tekshiruvchi` (yangi ov-bandi), `pm-qabulchi`. 📌 Texnik darslarga ham tegishli — `DARS_ETALON.md` ga ko'chirilsin. *(2026-07-30 foydalanuvchi qarori: «qat'iy taqiq etalonga ham yoz — bunaqa page bulg'anib tursa»; tatbiq: PmLesson3 s4/s5/s6/s8/s9/s12 — 1413→328 belgigacha yengillashtirildi)*

93. 🔴 **YOZILGAN NARSA ETALONDAN OLINADI, QAYTA IXTIRO QILINMAYDI.** Yangi darsda TAKRORLANADIGAN element (podium sarlavhasi, uy-vazifa kartasi, mentor-panellari, ball-fidbeki, arena matnlari) yozilayotganda **avval etalon darsdan aynan shu element grep qilinadi** va so'zma-so'z ko'chiriladi. Sabab: PmLesson3'da podium «Kim g'olib?» deb yozilgan edi — etalonda esa jonli darsda «Bugungi **g'oliblarimiz**», yakka rejimda «Bugungi **natijangiz**» (o'quvchi yolg'iz o'qiyotganda «kim g'olib?» degan savol ma'nosiz). Qoida: **etalon = birinchi manba, ijod = faqat darsning O'Z mazmunida**. Mas'ul: `pm-quruvchi`; tekshiruv: `pm-tekshiruvchi` — takrorlanuvchi elementlarni etalon bilan diff qilib chiqadi. *(2026-07-30 foydalanuvchi e'tirozi: «nega etalonga qaramayapsan?»)*

94. 🔴 **KO'P-BOSQICHLI TANLOV BOSQICHMA-BOSQICH OCHILADI (progressiv ochilish).** Ildiz-sabab: bir mashqda 2-3 guruh tanlov bo'lsa (masalan KIM+MUAMMO+YECHIM × 3 karta) va hammasi birdan ko'rsatilsa, ekranga 9+ tugma to'kiladi — o'quvchi qayerdan boshlashni bilmaydi, UI «to'lib» qo'rqitadi (92-qonunning tanlov-varianti). Qoidalar: **(a)** 🔴 kirganda faqat BIRINCHI bosqich kartalari ko'rinadi; keyingi bosqich oldingisi tasdiqlangachgina ochiladi; **(b)** tasdiqlangan bosqich ✓ bilan BIR QATORGA yig'iladi (tanlangan qiymat + o'zgartirish tugmasi ↻); **(c)** kelajak bosqichlar qulf-qator holida ko'rinadi («MUAMMO — keyingi qadam») — o'quvchi yo'lni oldindan ko'radi, lekin kartalarini emas; **(d)** har bosqichda tekshiruv-mexanika (masalan «👀 Egasiga ko'rsat») ALOHIDA ishlaydi — xato darhol o'z bosqichida bilinadi, oxirigacha yig'ilib qolmaydi; **(e)** mentor gapi bosqich bilan birga ALMASHADI va aynan joriy qadamga yo'naltiradi («Endi shu odamning muammosini toping…»). Mas'ul: `pm-quruvchi` + `pm-dizayn`; tekshiruv: `pm-tekshiruvchi`. 📌 Texnik darslarga ham tegishli. *(2026-07-31 foydalanuvchi tashxisi F-0731-04: «kirganda 3×3 karta ochilib yotibti — UI buzilishi»; birinchi tatbiq: PmLesson1 s11 karta-yig'ish ekrani)*

95. 🔴 **MISOL-OLAM — TOSHKENT O'SMIRINING O'Z HAYOTIDAN.** Auditoriyamiz — Toshkent shahrida yashaydigan o'smir. Misol-olam (91-qonun bo'yicha darsning bitta ipi) tanlanayotganda sinov-savoli: **«bizning o'quvchi shu joyga O'ZI boradimi / shu narsani o'zi ishlatadimi?»** Javob «yo'q» bo'lsa — olam almashadi, dars qurilmaydi. Ildiz-sabab: PmLesson1 «mahalla novvoyxonasi» ipida qurilgan edi — shahar o'smiri novvoyxonaga bormaydi, «non qachon issiq chiqadi» tashvishi unga begona, dars boshdanoq «meniki emas» tuyg'usini beradi. Yaqin olamlar (namuna): maktab yonidagi fastfud/lavash · savdo markazi · o'yin-klub · telefon/ilovalar · avtobus-bekat · maktab hayoti. Uzoq olamlar (asossiz ishlatilmaydi): qishloq-mahalla turmushi, kattalar kasb-tashvishlari (soliq, ijara-shartnoma), o'quvchi ko'rmagan texnika. Istisno: keys-janr (91b) — real biznes-voqea o'z olamida qoladi, lekin freym bilan kiradi. 📌 **Barcha darslarga tegishli** (texnik ham, PM ham) — yangi dars senariy-bosqichida (GATE S) misol-olam shu testdan o'tadi. Mas'ul: senariy-bosqichi + `pm-metodist`; tekshiruv: `pm-auditor`/`pm-tekshiruvchi`. *(2026-07-31 foydalanuvchi qarori F-0731-05: «o'quvchilarimiz Toshkent shahrida — novvoyxonaga ishi tushmaydi, qiziq ham emas; buni barcha darslarda o'ylash kerak»; birinchi tatbiq: PmLesson1 to'liq lavash-olamiga ko'chirildi)*

96. 🔴 **MODUL-IPI: BITTA MAHSULOT MODULLAR OSHA O'SADI.** 91-qonun bitta DARS ichidagi ipni qo'riqlaydi; bu qonun undan yuqori qavat — **modul (va imkon bo'lsa, qo'shni modullar) bitta mahsulot ustida quriladi**. Sabab: o'quvchi har modulda yangi olamga ko'chsa, oldingi mehnati «o'yin edi» bo'lib qoladi; bitta mahsulot o'ssa — u o'z ishining davomini ko'radi va motivatsiya modul chegarasida uzilmaydi. Qoidalar: **(a)** modulning LOYIHA kunlari qanday mahsulot qursa, o'sha modulning nazariy darslari ham shu mahsulotning bo'laklarida o'rgatiladi (o'zgaruvchi — o'sha mahsulotning soni/narxi; shart — o'sha mahsulotning holati; sikl — o'sha mahsulotning ro'yxati; funksiya — o'sha mahsulotning takrorlanadigan hisobi); **(b)** oldingi modul artefakti KIRISH bo'lib ishlaydi — yangi modul «siz yozgan X ni endi ishlaydigan qilamiz» deb boshlanadi; **(c)** mahsulot 95-qonun testidan o'tgan bo'lishi shart; **(d)** modul-ipi tanlovi darslar QURILISHIDAN OLDIN yozib qo'yiladi (STATE-yozuv + memory), aks holda har dars o'z olamini tanlab yuboradi. 📌 Texnik darslarga ham tegishli. Mas'ul: modul-rejasi bosqichi; tekshiruv: `pm-auditor`/`darslik-auditor` — modul ichidagi mahsulot-nomlarini grep qilib bitta ip ekanini tasdiqlaydi. *(2026-07-31 foydalanuvchi qarori: 1-Modul lavash do'koni auditoriya-kartasi → 2-Modul o'sha do'konga jonli sayt quradi)*

**96c — 🔴 IP O'QUVCHINING ARTEFAKTIDA, DEMO-MISOLDA EMAS.** 96-qonunning eng muhim aniqligi: modul-ipi **o'quvchining O'Z loyihasi** orqali yuradi (localStorage artefakt-zanjiri: karta → feature'lar → MVP → sayt → pitch), **mentorning demo-misoli orqali EMAS**. Shuning uchun: **(a)** o'quvchining artefakti modul (va modullar) bo'ylab UZILMAYDI — bu qat'iy; **(b)** mentor ekranda ko'rsatadigan DEMO-MISOL esa **har darsda YANGILANADI** — bitta demo uch-to'rt darsda takrorlansa, o'quvchida «yana shumi?» hissi paydo bo'ladi va qiziqish so'nadi (foydalanuvchi dalili: «uf, yana lavash saytmi deb xo'rsinishi mumkin»); **(c)** dars ICHIDA demo bitta bo'ladi (91-qonun kuchda qoladi) — almashish darslar ORASIDA bo'ladi; **(d)** zaxira-namuna (o'quvchida artefakt bo'lmaganda) o'sha darsning O'Z demo-olamidan olinadi, aks holda bir darsda ikki olam paydo bo'ladi; **(e)** yangi demo tanlashda **95-qonun testi** («o'quvchi buni oxirgi bir haftada ko'rganmi?») + **to'qnashuv tekshiruvi**: boshqa darsning bosh-misoli, keysi yoki abrazetsi band bo'lsa olinmaydi (grep bilan tasdiqlanadi). Mas'ul: senariy-bosqichi; tekshiruv: `pm-auditor` — modul darslarining demo-olamlarini yonma-yon qo'yib takror yo'qligini tasdiqlaydi. *(2026-07-31 foydalanuvchi kuzatuvi: lavash M1-D2 · M2-D2 · M2-D7 da takrorlangan edi; tatbiq: M2-D2 → kino olami)*

**96b — KO'PRIK NAQSHI (modul-ipini majburlamasdan ulash).** 96-qonun har ekranni mahsulot misoliga ko'chirishni TALAB QILMAYDI. Ba'zi tushuncha o'quvchining O'ZIGA bog'langanda kuchliroq o'rgatiladi — birinchi o'zgaruvchi `let ism = "Aziza"` (o'z ismi), shart — metro turniketi, sikl — 30 do'stga xabar. Bunday holda: **(a)** hook va BIRINCHI misol shaxsiy/yaqin olamda qoladi (95-qonun bo'yicha tanlangan); **(b)** modul-ipi darsning **yakuniy amaliy ekranida** — mashq BAJARILGANDAN keyin chiqadigan **«🌯 Loyihaga ko'prik»** kartasi bilan ulanadi: bitta gap + o'sha tushunchaning mahsulotdagi ko'rinishi (`let narx = 25000` · `if (soni > 10)` · `for` menyuni chiqaradi); **(c)** ko'prik mashqdan OLDIN ko'rsatilmaydi (92-qonun: bir ekran — bir ish) va bitta gapdan oshmaydi; **(d)** amaliyot/loyiha kunlari esa TO'LIQ mahsulot olamida bo'ladi — ko'prik faqat nazariy darslar uchun yon-yo'l. Tatbiq: `src/2-Modull/JsVarsLesson.jsx · JsConditionsLesson.jsx · JsLoopsLesson.jsx` — `ProjectBridge` komponenti. *(2026-07-31 foydalanuvchi qarori: «ko'prik variant bilan davom et»)*
