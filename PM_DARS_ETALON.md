...
64. 🔴 **TUZOQ-CHIP ASL BO'LAKNING QAYTA AYTILISHI BO'LADI, LEKIN SO'ZMA-SO'Z NUSXASI EMAS.** Klinika/konstruktor tuzog'i «takror NATIJA»ni o'rgatadi — shuning uchun u NIMA bilan **ma'nodosh**, ammo **boshqa so'zlar** bilan yozilishi shart: ❌ NIMA «sahifani tez ochish» + tuzoq «sahifa tez ochilishi» (o'quvchi ikkisini bir xil o'qiydi va tanlovni tasodifga qo'yadi) → ✅ tuzoq «saytning tez ishlashi». Umumiy shakl: **bir ekranda ikki variant bir-birining harf-nusxasi bo'lmaydi.** Tekshiruv: har ekran variantlarini juftlab solishtiring — 3 so'zdan ortig'i ustma-ust tushsa, qayta yozing.
...
67. 🟡 **HARAKAT-TUGMASI VA BAHO-SHKALASI O'Z NOMINI YOZADI.** Yolg'iz ikonka (`✎`, `★`) nima qilishini aytmaydi: tugmaga matn qo'shiladi («✎ Tahrirlash»), shkalaga esa **o'lchov gapi** beriladi («1 yulduz — unchalik muhim emas, 5 yulduz — eng muhimi») va `aria-label`/`title` shu ma'noni takrorlaydi («Muhimligi: 3 yulduz», ❌ «3 ball» — bu ball-tizimi bilan chalkashtiradi). Mas'ul: `pm-dizayn`.
