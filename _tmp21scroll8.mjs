...
for (const [nom, kunlar, act] of CASES) {
  ...
  const p = join(TMP, 'g' + VW + nom + '.html');
  ...
  const sc = await pg.evaluate(() => { const st = document.querySelector('.stage-content'); const el = document.scrollingElement || document.documentElement; return { stage: st ? st.scrollHeight - st.clientHeight : -1, doc: el.scrollHeight - el.clientHeight }; });
  ...
  out.push({ nom: VW + 'x' + VH + ' ' + nom, sc, stage: cnt(t.txt), phold: cnt(t.ph), jami: cnt(t.txt) + cnt(t.ph), tas: cnt(t.tas), txt: t.txt.replace(/\n/g, ' | ') });
  await pg.close();
}
await ctx.close();
}
for (const o of out) console.log(o.nom + ': tasmasiz=' + (o.jami - o.tas) + ' · skroll stage=' + o.sc.stage + ' doc=' + o.sc.doc);
console.log(NL + 'SKROLL NUQSON: ' + out.filter(o => o.sc.stage > 0 || o.sc.doc > 0).length);
await browser.close();
