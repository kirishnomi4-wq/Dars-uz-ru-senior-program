...
const EXTRA = {
  ...
  'pm-m5d11-belgi': JSON.stringify({ qator: 4, savedAt: Date.now() }),
  'pm-m5d11-code': JSON.stringify({ gpick: 'g2', done: true, open: false }),
  'pm-m5d11-hw-target': 'toliq',
};
...
const SELS = process.argv.includes('--hwonly') ? ['.hw-big', '.hw-chip'] : ['.hopt', '.dfc', '.qcard', '.wq-row', '.elakb', '.nextsig', '.option', '.tap', '.acc-h', '.flip', '.rfb', '.fc-card', '.hw-big', '.hw-chip', '.vsc-run', '.kd-opt', '.gate-opt', '.ctl-btn', '.mrk-cell', '.wsp-save', '.btn-soft', '.fchoice', '.kdpanel .wsx-toggle', '.stp'];
const rows = [];
