...
- **Qoida 3 — bir o'quvchi, bir dars, bitta tanga-hodisa:** jonli har doim ketadi; solo faqat shu dars bo'yicha oldin TUGALLANGAN natija (jonli — har qanday; solo — completed) bo'lmasa ketadi; qolgan urinishlar bizda (`attempts.result_event_id = 'skipped:already_rewarded'`), ko'rish/analitika uchun. Tashlab ketilgan (auto_7d, completed:false) hodisa keyingi to'liq urinishni bloklamaydi; `restarted` yuborilmaydi.
  - 🔁 **2026-09-18 yangilanish (F-0918-04, foydalanuvchi qarori: «bir darsdan faqat bir marta jo'natamiz, bo'lmasa nishonlar ko'payib ketadi»):** «jonli har doim ketadi» **bekor**. Jonli hodisada ham shu dars bo'yicha oldin **tugallangan** (`completed: true`, jonli yoki solo) natijasi yuborilgan/yuborilayotgan (`pending · retry_wait · delivered`) o'quvchi hodisadan **chiqariladi**; qolganlarning o'rni va nishonlari o'zgarmaydi. Hammasi takror bo'lsa hodisa yuborilmaydi, yozuv `status = 'skipped'`, `last_error = 'already_rewarded'` bilan qoladi (migratsiya **0009**; analitika + sweeper sessiyani qayta olmaydi). **To'smaydi:** uzilgan natija (`completed: false` — mentor noutbuki o'chdi) va LMS'ga yetmagan natija (`manual_review`, masalan 422) — keyingi haqiqiy dars baribir ketadi. Bayroq: `RESULT_LIVE_REPEAT=skip` (standart, prod) · `send` = eski xulq, **faqat sinov muhiti** uchun (bitta test-o'quvchi bilan bitta darsni qayta-qayta sinash). Kod: `result-service.js` `rewardedStudents` + `enqueueLiveSession`; test: `test/integration/results_repeat.test.js` (4 holat + teskari isbot). Ma'lum chegara: `manual_review` yozuvi keyinroq qo'lda `requeue` qilinsa va orada o'sha o'quvchining boshqa natijasi yetkazilgan bo'lsa — ikkinchi natija ketadi (operator qarori).
...
**2026-09-10 20:15 — Axadullaga bog'liq bo'lmagan ishlar (foydalanuvchi buyrug'i):** KATTA §30 ✅ — 96 darsga `arenaBank: QUIZ_BANK`
(unit 6/6, jsx-lint 0, esbuild 96/96, smoke 109/109, smoke-onfinished 2 dars ✓; UNCOMMITTED). CRM-yuklash papkasi yangilandi
(`feedback/lms-sinov-2026-09-10/CRM-yuklash/`): `1-Internet-YANGI-2026-09-10.jsx` (= `lms/InternetLesson.jsx` 16:31 bilan bir xil) va
`2-HtmlLesson1-YANGI-2026-09-10.jsx` (§5.2 15-qadam uchun, staging manzili, yangi modul) — ikkalasida staging=1 · ccDetails=1 ·
LMS_SOLO_RECHECK_MS · arenaBank=1 · supabase=0; eski nusxalar `eski/` ga. `3-Internet-ESKI-177ee1a.jsx` (16-qadam) o'zgarmadi.
Staging tirik (3f751291, migratsiya 7, katalog 110, result_details off). Axadulla javobi hali yo'q — A-variant bo'yicha hech narsa
qilinmadi (ataylab). Yon topilma: `esbuild-gate src/main.jsx` qizil M7 `MvpBuild2Lesson.jsx:127` takror-kalit — HEAD'da ham bor, M7 konveyerida.

**2026-09-10 21:00 — to'liq sinov-aylanishi (foydalanuvchi: «davom et, testla»):** lokal PG 17 klaster scratchpad'da (5433, `initdb --auth=trust`,
dars_dev + dars_test, migratsiya 7/7, katalog 110) → server unit **52/52** · integratsion **69/69** · E2E LMS m1-01 **18/18** (soxta School API 3999 +
server 3001 + vite 5300; F-0910-01 qadami 19.8 s) · `vite build` ✓ (scratch outDir, 116 asset) · `esbuild-gate` 156/156 (darvoza tuzatildi: tmp-papka,
`/dev/null.css` yolg'on qizili yo'q) · `jsx-lint` 0 · `smoke` 109/109 · cutover-mashq **90/90 + smoke 68 + 22** (yig'malarda arenaBank/RECHECK/ccDetails 90/90).
Diff-tozaligi tekshirildi: `src/` dagi 97 faylda faqat `arenaBank` satri + M7 o'lik `map` satri. `dark`/`til` lintlar 97 faylda 61/47 topilma — **HEAD'da ham
