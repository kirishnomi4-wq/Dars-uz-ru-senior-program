...
//  Manba:  src/3-Modull/ReactFirstComponentLesson.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/3-Modull/ReactFirstComponentLesson.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/3-Modull/ReactFirstComponentLesson.jsx
import React3, { useState as useState3, useEffect as useEffect4, useRef as useRef3, useCallback as useCallback2, createContext as createContext2, useContext as useContext2, useMemo } from "react";
...
var LESSON_META = { lessonId: "react-first-component-02-v18", lessonTitle: { uz: "Birinchi komponent: Vite, JSX, props", ru: "Первый компонент: Vite, JSX, props" } };
var HW_TOKENS = [
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr2({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и сколько ✅/❌ — скрыто. По кнопке «Открыть результат» всё откроется одновременно и у вас, и на экранах учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Верно только у <b>{pct}%</b> — класс не понял эту тему. Перед тем как идти дальше, стоит коротко повторить.</> })}</p>
...
var RoCard = ({ name, emoji, players, likeable }) => {
  ...
  return <div className="rocard el-in">
      ...
      <div className="robody">
        ...
        <div className="rostats">
          {likeable ? <button className={`rolike ${liked ? "on" : ""}`} onClick={() => setLiked((v) => !v)} title={tr2({ uz: "Like bosib ko'ring", ru: "Попробуйте поставить лайк" })}>
              <span className={liked ? "hpop" : void 0} style={{ display: "inline-block" }}>👍</span> {pct}%
            </button> : <span className="rolike-static">👍 {baseLikes}%</span>}
...
function CardMachine({ mode = "anatomy", active = null, seen = null, n = 0, cards = [], phase = 0, running = false, onPull }) {
  ...
  const prevCount = useRef3(0);
  const trayCards = mode === "lever" ? Array.from({ length: n }, () => "Adopt Me!") : mode === "capsule" ? cards : mode === "xray" ? phase >= 3 ? ["Blox Fruits"] : [] : ["Adopt Me!"];
  ...
  return <div className={`cm cm--${mode} ${shake ? "cm-chunk" : ""}`}>
      {
    /* TEPA — kapsula teshigi (S8/S10) */
  }
      <div className={`cm-top ${mode === "capsule" || mode === "xray" ? "cm-top-live" : ""}`}>
        <span className="cm-hole" />
        {mode === "capsule" && count > 0 && <span className="cm-capsule" key={count} style={gameByName(lastName) ? { background: gameByName(lastName).bg } : void 0}>{String(lastName).split(" ")[0]}</span>}
        {(mode === "capsule" || mode === "xray") && <span className="cm-top-lbl">{tr2({ uz: "nom-kapsula", ru: "капсула-имя" })}</span>}
      </div>
      {
    /* TANA — robot yuzi */
  }
      <div className="cm-body">
        ...
        <div className={`cm-mold ${glow("jsx")}`}>
          {mode === "xray" ? <div className="cm-flow">
            ...
            </div> : <span className="cm-mold-shape">{"<div> … </div>"}</span>}
        </div>
      </div>
      {
    /* RICHAG — old tomonda (S7) */
  }
      {mode === "lever" && <button type="button" className={`cm-lever ${shake ? "pulled" : ""}`} onClick={pull} disabled={n >= 4} aria-label={tr2({ uz: "Richagni torting", ru: "Потяните рычаг" })}>
          <span className="cm-lever-arm"><span className="cm-lever-knob" /></span>
          <span className="cm-lever-lbl">{n >= 4 ? tr2({ uz: "TO'LDI", ru: "ПОЛНО" }) : tr2({ uz: "TORT", ru: "ТЯНИ" })}</span>
        </button>}
      {
    /* CHUNK porti */
  }
    ...
    /* CHIQISH LATOGI (S6 ret + barcha modelar chiqishi) */
      ...
      <div className={`cm-chute ${glow("ret")}`}>
        ...
        <div className="cm-tray">
          {count === 0 ? <span className="cm-tray-empty">{tr2({ uz: "latok bo'sh…", ru: "лоток пуст…" })}</span> : trayCards.map((nm, i) => <div className="cm-card-slot" key={`${mode}-${i}`}><RoCard name={nm} /></div>)}
        </div>
      </div>
    </div>;
}
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const pick = (v) => {
    if (picked !== null || phase < 2) return;
    setPicked(v);
    onAnswer(screen, { stage: "hook", screenIdx: screen, picked: v, correct: true });
    audio.triggerEvent("option_picked");
  };
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>Bitta buyruq <span className="italic" style={{ color: T.accent }}>butun loyihani</span> yaratib beradimi?</>, ru: <>Одна команда создаст <span className="italic" style={{ color: T.accent }}>целый проект</span>?</> })}</h1>
        ...
        <Split>
          <Col>
            <div className="code-box fade-up delay-1" style={{ padding: "11px 14px" }}>
              ...
              {phase === 1 && <TLine out={<span style={{ color: CODE.attr }}>{tr2({ uz: "→ papka va fayllar yaratilmoqda…", ru: "→ создаём папки и файлы…" })}</span>} />}
              {phase >= 2 && <TLine out={<span style={{ color: CODE.str }}>{tr2({ uz: "✔ Tayyor! Pastga qarang —", ru: "✔ Готово! Посмотрите вниз —" })}</span>} />}
            </div>
            {phase === 0 && <button className="btn fade-up delay-2" style={{ alignSelf: "flex-start" }} onClick={run}>{tr2({ uz: "⏎ Enter — ishga tushirish", ru: "⏎ Enter — запустить" })}</button>}
            <p className="flow-label fade-up delay-2" style={{ margin: 0 }}>{tr2({ uz: "Kompyuteringizda", ru: "На вашем компьютере" })}</p>
            <div className="frame fade-up delay-2" style={{ padding: "11px 14px", minHeight: 138 }}>
              {phase < 2 ? <p style={{ color: T.ink3, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13, textAlign: "center", paddingTop: 38 }}>{phase === 0 ? tr2({ uz: "Hozircha bo'sh — buyruqni ishga tushiring…", ru: "Пока пусто — запустите команду…" }) : tr2({ uz: "yaratilmoqda…", ru: "создаётся…" })}</p> : <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                  {TREE.map((f, i) => <div key={f.label} className="fade-up" style={{ animationDelay: `${i * 0.18}s`, display: "flex", alignItems: "center", gap: 8, padding: "4px 9px", paddingLeft: 9 + f.d * 20, borderRadius: 7, background: f.hot ? T.accentSoft : "transparent", fontFamily: "'JetBrains Mono',monospace", fontSize: 12.5, color: T.ink }}>
                      <span>{f.icon}</span><span style={{ fontWeight: f.hot ? 700 : 500 }}>{f.label}</span>
                      {f.sub && <span style={{ marginLeft: "auto", fontFamily: "'Manrope',sans-serif", fontSize: 10, color: f.hot ? T.accent : T.ink3, fontWeight: 600 }}>← {tr2(f.sub)}</span>}
                    </div>)}
                </div>}
            </div>
            {phase === 2 && <span className="tagpill fade-up" style={{ color: T.success, animationDelay: "1.2s" }}>{tr2({ uz: "✓ 1 buyruq → butun loyiha tuzilishi", ru: "✓ 1 команда → структура всего проекта" })}</span>}
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Sizningcha, hozir nima yuz berdi?", ru: "Как вы думаете, что сейчас произошло?" })}</p>
            <div className="fade-up delay-3" style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {OPTS.map((o) => {
    const on = picked === o.id;
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null || phase < 2} style={{ opacity: phase < 2 ? 0.55 : 1 }} onClick={() => pick(o.id)}>
                    <span className="radio">{on && <span className="radio-dot" />}</span>
                    <span>{tr2(o.label)}</span>
                  </button>;
  })}
            </div>
            {phase < 2 && <p className="small" style={{ color: T.ink3, fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Avval buyruqni ishga tushiring ←", ru: "Сначала запустите команду ←" })}</p>}
            {picked !== null && <p className="hook-ack fade-step">{tr2({ uz: <>Aynan shunday! Bu yordamchining nomi — <b>Vite</b> (talaffuzi: "vit", fransuzcha "tez"). Xuddi Roblox Studio'ning tayyor maydonchasiday: tuzilish bor — <b>ichini esa siz to'ldirasiz</b>. Bugun shu yerga birinchi komponentingizni yozasiz.</>, ru: <>Именно так! Этого помощника зовут <b>Vite</b> (произносится «вит», по-французски «быстрый»). Как готовая площадка в Roblox Studio: структура есть — <b>а наполняете его вы</b>. Сегодня вы напишете сюда свой первый компонент.</> })}</p>}
          </Col>
        </Split>
        </Zoomable>
      </div>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const PreviewBlock = <Col>
      ...
      <p className="mono small" style={{ color: T.accent, margin: 0 }}>{tr2({ uz: "→ mana shu qatorni dars oxirida to'liq o'zingiz yozasiz", ru: "→ именно эту строку в конце урока вы напишете полностью сами" })}</p>
    </Col>;
  ...
  return <Stage eyebrow={tr2({ uz: "Reja", ru: "План" })} screen={screen} audioState={audio} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head">
          <h2 className="title h-title fade-up">{tr2({ uz: <>Roblox'dagiday o'yin kartochkasini <span className="italic" style={{ color: T.accent }}>o'zingiz</span> yasay olasizmi?</>, ru: <>Сможете <span className="italic" style={{ color: T.accent }}>сами</span> сделать игровую карточку, как в Roblox?</> })}</h2>
        </div>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "O'rnatish", ru: "Установка" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : `${tr2({ uz: "Qadamlar", ru: "Шаги" })}: ${step}/4`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Loyiha <span className="italic" style={{ color: T.accent }}>to'rt qadamda</span> tayyor bo'ladimi?</>, ru: <>Проект будет готов <span className="italic" style={{ color: T.accent }}>за четыре шага</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Terminal — buyruqlarni bosing", ru: "Терминал — нажимайте команды" })}</p>
            <div className="code-box fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {STEPS.map((s, i) => <div
    key={i}
    onClick={() => {
      if (i === step) setStep((v) => v + 1);
    }}
  ...
  >
                  ...
                  {i === step && <span style={{ marginLeft: "auto", fontFamily: "'Manrope',sans-serif", fontSize: 10, color: CODE.attr, whiteSpace: "nowrap" }}>{tr2({ uz: "← bosing", ru: "← нажмите" })}</span>}
                </div>)}
            </div>
            {step > 0 && <div className="hint fade-step" key={step}><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2(STEPS[step - 1].info)}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Natija — brauzeringizda", ru: "Результат — в вашем браузере" })}</p>
            {done ? <Win title="localhost:5173" minH={120}>
                <div className="fade-step" style={{ textAlign: "center", padding: "8px 0" }}>
                  ...
                  <p className="small" style={{ color: T.ink3, margin: "0 0 10px" }}>{tr2({ uz: "Loyihangiz ishlayapti!", ru: "Ваш проект работает!" })}</p>
                  <button className="likebtn" onClick={() => setCount((c) => c + 1)}>count is {count}</button>
                  <p className="small" style={{ color: T.ink3, margin: "10px 0 0", fontStyle: "italic" }}>{tr2({ uz: "↑ Vite'ning tayyor tugmasi — bosib ko'ring.", ru: "↑ Готовая кнопка Vite — попробуйте нажать." })}</p>
                </div>
              </Win> : <div className="frame-dash" style={{ minHeight: 120, display: "flex", alignItems: "center", justifyContent: "center" }}><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "4 qadamni bajaring — sayt shu yerda ochiladi", ru: "Выполните 4 шага — сайт откроется здесь" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <><span className="mono">localhost:5173</span> — saytingizning <b>kompyuteringizdagi</b> manzili. Hali internetda emas — faqat siz ko'rasiz. Xuddi Roblox Studio'da hali e'lon qilinmagan o'yiningizday!</>, ru: <><span className="mono">localhost:5173</span> — адрес вашего сайта <b>на вашем компьютере</b>. Он ещё не в интернете — видите его только вы. Как ваша ещё не опубликованная игра в Roblox Studio!</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const Row = ({ k, depth }) => <button onClick={() => tap(k)} className="fade-up" style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", textAlign: "left", border: "none", cursor: "pointer", borderRadius: 8, padding: "8px 11px", paddingLeft: 11 + depth * 18, fontFamily: "'JetBrains Mono',monospace", fontSize: "clamp(12px,1.5vw,13.5px)", background: active === k ? T.accentSoft : T.paper, boxShadow: seen.has(k) ? `inset 0 0 0 1.5px ${active === k ? T.accent : T.success}` : `0 3px 9px -4px rgba(${T.shadowBase},0.14)`, color: T.ink, transition: "all 0.18s" }}>
      <span>{FILES[k].icon}</span><span>{FILES[k].label}</span>
      {seen.has(k) && <span style={{ marginLeft: "auto", color: T.success, fontSize: 11 }}>✓</span>}
    </button>;
  return <Stage eyebrow={tr2({ uz: "Loyiha tuzilishi", ru: "Структура проекта" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : `${seen.size}/4 ${tr2({ uz: "fayl ko'rildi", ru: "файла просмотрено" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bu fayllarning qaysi biri <span className="italic" style={{ color: T.accent }}>sizniki</span>?</>, ru: <>Какой из этих файлов — <span className="italic" style={{ color: T.accent }}>ваш</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "robo-games/ papkasi", ru: "папка robo-games/" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 12, color: T.ink3, padding: "2px 11px", paddingLeft: 29, opacity: 0.6 }}>📁 node_modules/ <span style={{ fontSize: 10 }}>{tr2({ uz: "(kutubxonalar — tegmaymiz)", ru: "(библиотеки — не трогаем)" })}</span></div>
              <Row k="pkg" depth={1} />
              <Row k="html" depth={1} />
          ...
          <Col>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
              <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "Fayl nima qiladi?", ru: "Что делает файл?" })}</p>
            ...
            {active ? <div className="sk-info" key={active}>
                ...
                <p className="body" style={{ color: T.ink, margin: "11px 0 0" }}>{tr2(FILES[active].info)}</p>
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Chapdan faylni bosing", ru: "Нажмите на файл слева" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Yo'l aniq: <span className="mono">index.html</span> → <span className="mono">main.jsx</span> → <span className="mono">App.jsx</span>. Bugun butun ishimiz — <b>App.jsx</b> ichida.</>, ru: <>Путь ясен: <span className="mono">index.html</span> → <span className="mono">main.jsx</span> → <span className="mono">App.jsx</span>. Сегодня вся наша работа — внутри <b>App.jsx</b>.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen4 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <><span className="mono" style={{ color: T.accent }}>npm run dev</span> buyrug'i nima qiladi?</>, ru: <>Что делает команда <span className="mono" style={{ color: T.accent }}>npm run dev</span>?</> })}</h2></>}
  options={[tr2({ uz: "Saytni internetga chiqarib e'lon qiladi", ru: "Публикует сайт в интернете" }), tr2({ uz: "Loyihani kompyuteringizda ishga tushiradi", ru: "Запускает проект на вашем компьютере" }), tr2({ uz: "Koddagi xatolarni avtomatik tuzatadi", ru: "Автоматически исправляет ошибки в коде" }), tr2({ uz: "React'ni loyihadan o'chirib tashlaydi", ru: "Удаляет React из проекта" })]}
  correctIdx={1}
  explainCorrect={tr2({ uz: "To'g'ri! npm run dev loyihani localhost:5173 manzilida ishga tushiradi — faqat sizning kompyuteringizda. Internetga chiqarish — boshqa jarayon (deploy, esingizdami?).", ru: "Верно! npm run dev запускает проект по адресу localhost:5173 — только на вашем компьютере. Публикация в интернете — отдельный процесс (deploy, помните?)." })}
  explainWrong={{
    0: tr2({ uz: "Yo'q — internetga chiqarish deploy deyiladi (Git darsida ko'rganmiz). dev — faqat kompyuteringizda ishga tushiradi.", ru: "Нет — публикация в интернете называется deploy (видели на уроке про Git). dev запускает проект только на вашем компьютере." }),
    2: tr2({ uz: "Yo'q — xatolarni tuzatish dasturchining (ya'ni sizning) ishingiz. dev faqat loyihani ishga tushiradi.", ru: "Нет — исправлять ошибки — работа программиста (то есть ваша). dev только запускает проект." }),
    3: tr2({ uz: "Aksincha! dev loyihani ishga tushiradi — React ishlay boshlaydi.", ru: "Наоборот! dev запускает проект — React начинает работать." }),
    default: tr2({ uz: "npm run dev — loyihani localhost'da ishga tushiradi.", ru: "npm run dev — запускает проект на localhost." })
  }}
/>;
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const RULES = {
    curly: {
      chip: { uz: "{ } — jingalak qavs", ru: "{ } — фигурные скобки" },
      word: { uz: "JS ichida", ru: "JS внутри" },
    ...
    cls: {
      chip: "className",
      word: { uz: "class emas", ru: "не class" },
    ...
    wrap: {
      chip: { uz: "Bitta tashqi teg", ru: "Один внешний тег" },
      word: { uz: "yagona ildiz", ru: "единый корень" },
      ...
      code: <><Jx>{"<div>"}</Jx>{" "}<Cm>{tr2({ uz: "// hammasi shu teg ichida", ru: "// всё внутри этого тега" })}</Cm>{"\n"}{"  "}<Jx>{"<h3>"}</Jx>…<Jx>{"</h3>"}</Jx>{"\n"}{"  "}<Jx>{"<p>"}</Jx>…<Jx>{"</p>"}</Jx>{"\n"}<Jx>{"</div>"}</Jx></>,
  ...
  return <Stage eyebrow="JSX" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : `${seen.size}/3 ${tr2({ uz: "qoida o'rganildi", ru: "правила изучено" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bu HTML'mi yoki <span className="italic" style={{ color: T.accent }}>JavaScript'mi</span>?</>, ru: <>Это HTML или <span className="italic" style={{ color: T.accent }}>JavaScript</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {KEYS.map((k) => <button key={k} className={`chip ${active === k ? "chip-on" : ""}`} onClick={() => tap(k)}>{tr2(RULES[k].chip)} {seen.has(k) ? "✓" : ""}</button>)}
            </div>
            {active ? <pre className="code-box fade-step" key={active} style={{ minHeight: 70 }}>{RULES[active].code}</pre> : <pre className="code-box fade-up delay-2" style={{ minHeight: 70 }}>
                <Cm>{tr2({ uz: "// App.jsx — JS ichida HTML?!", ru: "// App.jsx — HTML внутри JS?!" })}</Cm>{"\n"}
                <Jx>{"function"}</Jx>{" App() {"}{"\n"}
                {"  "}<Jx>{"return"}</Jx>{" "}<Jx>{"<h1>"}</Jx>{tr2({ uz: "Salom!", ru: "Привет!" })}<Jx>{"</h1>"}</Jx>{";"}{"\n"}
                {"}"}
              </pre>}
          </Col>
          <Col>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
              <p className="flow-label" style={{ margin: 0 }}>{tr2({ uz: "Qoida nima deydi?", ru: "О чём говорит правило?" })}</p>
              <span className="small mono" style={{ color: done ? T.success : T.ink3 }}>{seen.size} / 3</span>
            </div>
            {active ? <div className="sk-info" key={active}>
                <span className="sk-tagbig"><span className="sk-wordbadge">{tr2(RULES[active].word)}</span></span>
                <p className="body" style={{ color: T.ink, margin: "11px 0 0" }}>{tr2(RULES[active].info)}</p>
                <div className="hint" style={{ marginTop: 10 }}><p className="small" style={{ margin: 0, color: T.ink2 }}>{tr2(RULES[active].result)}</p></div>
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Chapdan qoidani tanlang", ru: "Выберите правило слева" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>JSX = HTML ko'rinishli JavaScript. 3 qoida: <b>{"{ }"} ichida JS</b> · <b>className</b> · <b>bitta tashqi teg</b>. Shu uchtasi bilan deyarli hamma JSX o'qiladi!</>, ru: <>JSX = JavaScript, похожий на HTML. 3 правила: <b>JS внутри {"{ }"}</b> · <b>className</b> · <b>один внешний тег</b>. С этими тремя читается почти весь JSX!</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen5b = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "Mustahkamlash", ru: "Закрепление" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>JSX'da nima uchun <span className="mono" style={{ color: T.accent }}>className</span> yoziladi?</>, ru: <>Почему в JSX пишется <span className="mono" style={{ color: T.accent }}>className</span>?</> })}</h2></>}
  options={[tr2({ uz: "Chunki class so'zi JavaScript'da band", ru: "Потому что слово class занято в JavaScript" }), tr2({ uz: "Chunki className chiroyliroq eshitiladi", ru: "Потому что className звучит красивее" }), tr2({ uz: "Chunki HTML'da class degan so'z yo'q", ru: "Потому что в HTML нет слова class" }), tr2({ uz: "Farqi yo'q — ikkalasi ham ishlayveradi", ru: "Разницы нет — работают оба" })]}
  correctIdx={0}
  explainCorrect={tr2({ uz: "To'g'ri! JSX — JavaScript ichida, class esa JS'ning band so'zi. Shuning uchun React className'ni tanlagan.", ru: "Верно! JSX живёт внутри JavaScript, а class — занятое слово JS. Поэтому React выбрал className." })}
  explainWrong={{
    1: tr2({ uz: "Yo'q — gap chiroyda emas. class so'zi JavaScript'da band, JSX esa JS ichida yashaydi.", ru: "Нет — дело не в красоте. Слово class занято в JavaScript, а JSX живёт внутри JS." }),
    2: tr2({ uz: "Aksincha — HTML'da aynan class ishlatiladi. JSX JavaScript ichida bo'lgani uchun className kerak.", ru: "Наоборот — в HTML используется именно class. className нужен потому, что JSX находится внутри JavaScript." }),
    3: tr2({ uz: "Farqi bor: JSX'da class yozsangiz, React ogohlantirish beradi — to'g'risi className.", ru: "Разница есть: напишете class в JSX — React выдаст предупреждение, правильно className." }),
    default: tr2({ uz: "class — JavaScript'ning band so'zi, shuning uchun JSX'da className.", ru: "class — занятое слово JavaScript, поэтому в JSX пишут className." })
  }}
/>;
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [active, setActive] = useState3(null);
  const [seen, setSeen] = useState3(storedAnswer ? /* @__PURE__ */ new Set(["name", "ret", "jsx"]) : /* @__PURE__ */ new Set());
  const done = seen.size >= 3;
  const tap = (k) => {
    setActive(k);
    setSeen((prev) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Komponent", ru: "Компонент" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : `${seen.size}/3 ${tr2({ uz: "qism topildi", ru: "части найдено" })}`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Komponent ichida <span className="italic" style={{ color: T.accent }}>nima yashaydi</span>?</>, ru: <>Что <span className="italic" style={{ color: T.accent }}>живёт внутри</span> компонента?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            {!done && (active ? <div className="sk-info" key={active}>
              ...
              </div> : <div className="frame-dash" style={{ padding: "10px 14px" }}><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Koddan bir qismni bosing", ru: "Нажмите на часть кода" })}</p></div>)}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Komponent = <b>Katta harfli funksiya + return JSX</b>. Tamom — butun formula shu!</>, ru: <>Компонент = <b>функция с Заглавной буквы + return JSX</b>. Всё — вот и вся формула!</> })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Bloxy-Press — kod = mashina", ru: "Bloxy-Press — код = машина" })}</p>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [n, setN] = useState3(storedAnswer ? 2 : 0);
  const done = n >= 2;
  const add = () => setN((v) => Math.min(v + 1, 4));
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  return <Stage eyebrow={tr2({ uz: "Chaqirish", ru: "Вызов" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : `${tr2({ uz: "Kamida 2 marta chaqiring", ru: "Вызовите минимум 2 раза" })} (${n}/2)`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Yozdik — endi qanday <span className="italic" style={{ color: T.accent }}>ekranga chiqaramiz</span>?</>, ru: <>Написали — а как теперь <span className="italic" style={{ color: T.accent }}>вывести на экран</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <button className="btn fade-up delay-1" style={{ alignSelf: "flex-start" }} onClick={add} disabled={n >= 4}>+ {"<GameCard />"} {tr2({ uz: "chaqirish", ru: "вызвать" })}</button>
            <pre className="code-box fade-up delay-2">
              <Jx>{"function"}</Jx>{" App() {"}{"\n"}
              {"  "}<Jx>{"return"}</Jx>{" ("}{"\n"}
              {"    "}<Jx>{"<div>"}</Jx>{"\n"}
              {n === 0 ? <>{"      "}<Cm>{tr2({ uz: "// hozircha bo'sh…", ru: "// пока пусто…" })}</Cm>{"\n"}</> : Array.from({ length: n }, (_, i) => <React3.Fragment key={i}>{"      "}<Jx>{"<GameCard />"}</Jx>{"\n"}</React3.Fragment>)}
              {"    "}<Jx>{"</div>"}</Jx>{"\n"}
              {"  );"}{"\n"}
              {"}"}
            </pre>
            {done && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Ishladi! Lekin bir muammo bor: hamma kartochka <b>bir xil — Adopt Me!</b>. Blox Fruits va Brookhaven qani? Keyingi ekranda yechamiz.</>, ru: <>Работает! Но есть проблема: все карточки <b>одинаковые — Adopt Me!</b>. А где Blox Fruits и Brookhaven? Решим на следующем экране.</> })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Bloxy-Press — har TORT = bitta chaqiruv", ru: "Bloxy-Press — каждое ТЯНИ = один вызов" })}</p>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [cards, setCards] = useState3(storedAnswer ? ["Adopt Me!", "Blox Fruits"] : ["Adopt Me!"]);
  const [tried, setTried] = useState3(storedAnswer ? /* @__PURE__ */ new Set(["Adopt Me!", "Blox Fruits"]) : /* @__PURE__ */ new Set(["Adopt Me!"]));
  ...
  return <Stage eyebrow="Props" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : `${tr2({ uz: "3 xil nom sinang", ru: "Попробуйте 3 разных имени" })} (${tried.size}/3)`} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bitta komponentdan <span className="italic" style={{ color: T.accent }}>turli kartochka</span> qanday chiqadi?</>, ru: <>Как из одного компонента получаются <span className="italic" style={{ color: T.accent }}>разные карточки</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              ...
              {cards.length > 1 && <button className="gchip" onClick={() => {
    setCards(["Adopt Me!"]);
  }}>{tr2({ uz: "↺ Tozalash", ru: "↺ Очистить" })}</button>}
            </div>
            <pre className="code-box fade-up delay-2">
              <Cm>{tr2({ uz: "// App ichida:", ru: "// внутри App:" })}</Cm>{"\n"}
              {cards.map((nm, i) => <React3.Fragment key={i}><Jx>{"<GameCard "}</Jx><At>name</At>=<St>"{nm}"</St><Jx>{" />"}</Jx>{"\n"}</React3.Fragment>)}
            </pre>
            <span className="tagpill fade-step" key={cards.length} style={{ color: T.success }}>{tr2({ uz: "Komponent kodi: 1 dona", ru: "Код компонента: 1" })} · {tr2({ uz: "chaqiruv:", ru: "вызовов:" })} {cards.length}</span>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <><b>Props</b> — komponentga uzatiladigan ma'lumot. Kartochkalar har xil bo'ldi, kod esa bitta. 1-darsdagi va'da bajarildi: <b>bir marta yoz — ming marta ishlat</b>!</>, ru: <><b>Props</b> — данные, которые передают компоненту. Карточки стали разными, а код один. Обещание из 1-го урока выполнено: <b>напиши один раз — используй тысячу раз</b>!</> })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Bloxy-Press — nom-kapsula = props", ru: "Bloxy-Press — капсула-имя = props" })}</p>
...
var Screen9 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <><span className="italic" style={{ color: T.accent }}>Props</span> nima?</>, ru: <>Что такое <span className="italic" style={{ color: T.accent }}>props</span>?</> })}</h2></>}
  options={[tr2({ uz: "Komponentga uzatiladigan ma'lumot", ru: "Данные, которые передают компоненту" }), tr2({ uz: "Komponentning tashqi rangi", ru: "Внешний цвет компонента" }), tr2({ uz: "React'ni tezlashtiruvchi sozlama", ru: "Настройка, ускоряющая React" }), tr2({ uz: "Brauzerga qo'shiladigan qo'shimcha dastur", ru: "Дополнение для браузера" })]}
  correctIdx={0}
  explainCorrect={tr2({ uz: "To'g'ri! Props — komponentga uzatiladigan ma'lumot: <GameCard name='Blox Fruits' />. Komponent bitta, ma'lumot har xil.", ru: "Верно! Props — данные, которые передают компоненту: <GameCard name='Blox Fruits' />. Компонент один, данные разные." })}
  explainWrong={{
    1: tr2({ uz: "Yo'q — rang emas. Props orqali istalgan ma'lumot uzatiladi: nom, rasm, son…", ru: "Нет — не цвет. Через props передают любые данные: имя, картинку, число…" }),
    2: tr2({ uz: "Yo'q — tezlikka aloqasi yo'q. Props — komponentga ma'lumot uzatish usuli.", ru: "Нет — к скорости это не относится. Props — способ передать компоненту данные." }),
    3: tr2({ uz: "Yo'q — brauzerga aloqasi yo'q. Props — <GameCard name='…' /> dagi name kabi ma'lumot.", ru: "Нет — браузер тут ни при чём. Props — данные вроде name в <GameCard name='…' />." }),
    default: tr2({ uz: "Props — komponentga tashqaridan uzatiladigan ma'lumot.", ru: "Props — данные, которые передают компоненту снаружи." })
  }}
/>;
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Ma'lumot yo'li", ru: "Путь данных" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Yo'lni kuzating", ru: "Проследите путь" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>name="Blox Fruits" komponent ichiga <span className="italic" style={{ color: T.accent }}>qanday yetib boradi</span>?</>, ru: <>Как name="Blox Fruits" <span className="italic" style={{ color: T.accent }}>попадает внутрь</span> компонента?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <button className="btn fade-up delay-1" style={{ alignSelf: "flex-start" }} onClick={run} disabled={running}>{running ? tr2({ uz: "Ishlayapti…", ru: "Работает…" }) : done ? tr2({ uz: "↻ Yana ko'rsatish", ru: "↻ Показать ещё раз" }) : tr2({ uz: "▶ Ma'lumot yo'lini kuzating", ru: "▶ Проследить путь данных" })}</button>
    ...
    return <React3.Fragment key={i}>
                    ...
                    <div className={`prop-step ${reached ? "on" : ""} ${now ? "now" : ""}`}>
                      ...
                      <span className="prop-txt">{tr2(s)}</span>
                      {hasToken && <span className="prop-token">📦 "Blox Fruits"</span>}
                    </div>
                  </React3.Fragment>;
  })}
            </div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Yo'l aniq: <span className="mono">name="Blox Fruits"</span> → <span className="mono">props</span> → <span className="mono">{"{props.name}"}</span> → ekranda <b>Blox Fruits</b>!</>, ru: <>Путь ясен: <span className="mono">name="Blox Fruits"</span> → <span className="mono">props</span> → <span className="mono">{"{props.name}"}</span> → на экране <b>Blox Fruits</b>!</> })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Kod ichida", ru: "Внутри кода" })}</p>
            ...
            <p className="flow-label" style={{ marginTop: 2 }}>{tr2({ uz: "Bloxy-Press — rentgen rejimi", ru: "Bloxy-Press — режим рентгена" })}</p>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Keyingi qadam · AI", ru: "Следующий шаг · AI" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Agent bilan ishlab ko'ring", ru: "Поработайте с агентом" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Komponentni <span className="italic" style={{ color: T.accent }}>AI'ga yozdirsak</span>-chi?</>, ru: <>А что если <span className="italic" style={{ color: T.accent }}>поручить компонент AI</span>?</> })}</h2></div>
          ...
          <Col>
            ...
            <Win title="robo-games — localhost:5173" minH={130}>
              {done && cur ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {cur.id === "t2" && <p style={{ fontFamily: "'Manrope',sans-serif", fontWeight: 800, fontSize: 14, color: T.ink, margin: 0 }}>🎮 Robo Games <span className="mono small" style={{ color: T.success, fontWeight: 700 }}>{tr2({ uz: "+ yangi", ru: "+ новое" })}</span></p>}
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 9 }}>
                    <RoCard name="Adopt Me!" players={cur.id === "t1" ? "402K" : void 0} />
                    <RoCard name="Blox Fruits" players={cur.id === "t1" ? "750K" : void 0} />
                  </div>
                  {cur.id === "t1" && <span className="mono small" style={{ color: T.success, fontWeight: 700 }}>{tr2({ uz: "+ 👥 o'yinchilar soni props orqali keldi", ru: "+ 👥 число игроков пришло через props" })}</span>}
                  {cur.id === "t3" && <p style={{ fontFamily: "'Manrope',sans-serif", fontSize: 10.5, color: T.ink3, margin: 0, textAlign: "center" }}>© robo-games · 2026 <span className="mono" style={{ color: T.success, fontWeight: 700 }}>{tr2({ uz: "+ yangi", ru: "+ новое" })}</span></p>}
            ...
            {done ? <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Agent kodini <b>o'qiy oldingizmi</b>? Komponent Katta harf bilan, props to'g'ri — siz buni endi <b>tekshira olasiz</b>. Aynan shu — vibecoding'dagi eng kuchli mahorat.</>, ru: <>Смогли <b>прочитать</b> код агента? Компонент с Заглавной буквы, props верные — теперь вы умеете это <b>проверять</b>. Именно это — самый сильный навык в вайбкодинге.</> })}</p></div> : <p className="body" style={{ margin: 0, color: T.ink3, fontSize: 13 }}>{tr2({ uz: "Natija shu yerda paydo bo'ladi — keyin uni o'zingiz tekshirasiz.", ru: "Результат появится здесь — потом вы сами его проверите." })}</p>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen12 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Nima uchun komponent nomi <span className="italic" style={{ color: T.accent }}>Katta harf</span> bilan boshlanadi?</>, ru: <>Почему имя компонента начинается с <span className="italic" style={{ color: T.accent }}>Заглавной буквы</span>?</> })}</h2></>}
  options={[tr2({ uz: "Shunchaki chiroyliroq ko'rinishi uchun", ru: "Просто чтобы выглядело красивее" }), tr2({ uz: "React katta harfdan komponentligini biladi", ru: "По заглавной букве React понимает, что это компонент" }), tr2({ uz: "Katta harf yozilgan kodni tezlashtiradi", ru: "Заглавная буква ускоряет код" }), tr2({ uz: "Klaviaturada shunday yozish qulayroq", ru: "Так удобнее печатать на клавиатуре" })]}
  correctIdx={1}
  explainCorrect={tr2({ uz: "To'g'ri! <gamecard /> deb yozsangiz, React uni oddiy HTML teg deb o'ylaydi va komponentingizni topa olmaydi. <GameCard /> — Katta harf — komponent!", ru: "Верно! Напишете <gamecard /> — React примет его за обычный HTML-тег и не найдёт ваш компонент. <GameCard /> — Заглавная буква — компонент!" })}
  explainWrong={{
    0: tr2({ uz: "Yo'q — bu chiroy uchun emas, qoida: React katta harfdan komponentligini ajratadi.", ru: "Нет — это не ради красоты, а правило: по заглавной букве React отличает компонент." }),
    2: tr2({ uz: "Yo'q — tezlikka aloqasi yo'q. Katta harf — React uchun 'bu komponent' degan belgi.", ru: "Нет — к скорости это не относится. Заглавная буква — знак для React: «это компонент»." }),
    3: tr2({ uz: "Yo'q — qulaylik emas, qoida: kichik harfli teg HTML deb qabul qilinadi.", ru: "Нет — не удобство, а правило: тег со строчной буквы считается HTML." }),
    default: tr2({ uz: "Katta harf — React uchun komponent belgisi; kichik harf — oddiy HTML teg.", ru: "Заглавная буква — знак компонента для React; строчная — обычный HTML-тег." })
  }}
/>;
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const EMOJIS = ["🐾", "🍇", "🏠", "🗼", "🐶", "🕵️"];
  const [nm, setNm] = useState3("Adopt Me!");
  const [em, setEm] = useState3("🐾");
  const [nmTried, setNmTried] = useState3(storedAnswer ? /* @__PURE__ */ new Set(["Adopt Me!", "Blox Fruits"]) : /* @__PURE__ */ new Set(["Adopt Me!"]));
  const [emTried, setEmTried] = useState3(storedAnswer ? /* @__PURE__ */ new Set(["🐾", "🍇"]) : /* @__PURE__ */ new Set(["🐾"]));
  ...
  return <Stage eyebrow={tr2({ uz: "Amaliyot · props", ru: "Практика · props" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Ikkala props'ni almashtiring", ru: "Поменяйте оба props" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>O'z kartochkangizni <span className="italic" style={{ color: T.accent }}>props bilan</span> sozlay olasizmi?</>, ru: <>Сможете настроить свою карточку <span className="italic" style={{ color: T.accent }}>через props</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            <div className="fade-up delay-1" style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {GAMES.map((g) => <button key={g.name} className={`gchip ${nm === g.name ? "chip-on" : ""}`} style={nm === g.name ? { background: T.accent, color: "#fff" } : void 0} onClick={() => pickNm(g.name)}>{g.name}</button>)}
            </div>
            ...
            <div className="fade-up delay-2" style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {EMOJIS.map((e) => <button key={e} className="gchip" style={em === e ? { background: T.accent } : void 0} onClick={() => pickEm(e)}>{e}</button>)}
            </div>
            <pre className="code-box fade-up delay-2">
              <Jx>{"<GameCard"}</Jx>{"\n"}
              {"  "}<At>name</At>=<St>"{nm}"</St>{"\n"}
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "Kartochkangiz", ru: "Ваша карточка" })}</p>
            <Win title="robo-games — localhost:5173" minH={110}>
              <div style={{ maxWidth: 170 }}><RoCard key={`${nm}-${em}`} name={nm} emoji={em} likeable /></div>
            </Win>
            <span className="tagpill fade-step" style={{ color: done ? T.success : T.ink }}>{nmTried.size >= 2 ? "✓" : "○"} {tr2({ uz: "name almashtirildi", ru: "name изменён" })} · {emTried.size >= 2 ? "✓" : "○"} {tr2({ uz: "emoji almashtirildi", ru: "emoji изменён" })}</span>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Siz hozir <b>props bilan boshqardingiz</b>: ma'lumot o'zgardi — ko'rinish o'zgardi, kod esa bitta.</>, ru: <>Вы только что <b>управляли через props</b>: данные изменились — вид изменился, а код один. На следующем уроке карточка будет <b>меняться и при нажатии</b> (state)!</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Q_LABELS = { 5: { uz: "1 — Vite / loyiha", ru: "1 — Vite / проект" }, 7: { uz: "2 — JSX qoidasi", ru: "2 — правило JSX" }, 12: "3 — Props", 15: { uz: "4 — Katta harf", ru: "4 — Заглавная буква" }, 19: { uz: "5 — Yakuniy amaliy", ru: "5 — финальное практическое" } };
var INLINE_KEYS = { s4: 1, s5b: 0, s9: 0, s12: 1, s15: -1 };
var QUIZ_MS = 15e3;
var QZ_BG_SHAPES = [
  ...
  return <div className="qz-arena">
    ...
    /* ===== LOBBY ===== */
      ...
      {phase === "lobby" && <div className="qz-view fade-step">
          <CsWordmark />
          <p className="qz-sub" style={{ marginTop: -4 }}>{tr2({ uz: "Tezroq to'g'ri bossangiz — ko'proq ball. Ketma-ket to'g'ri javoblar 🔥 bonus beradi!", ru: "Быстрее ответите верно — больше баллов. Верные ответы подряд дают 🔥 бонус!" })}</p>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Debugging", ru: "Дебаггинг" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Xatoni toping", ru: "Найдите ошибку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            <div className="ai-row"><span className="ai-badge">AI</span><span className="ai-bubble">{tr2({ uz: "GameCard komponentini yozdim:", ru: "Я написал компонент GameCard:" })}</span></div>
            <DebugChallenge
    lines={LINES}
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  useEffect4(() => {
    if (valid && !passed) {
      ...
      if (!audio.muted) setTimeout(() => {
        const e = getAudioEngine();
        if (e && !audio.muted) e.pushOneOff(`Juda yaxshi! Birinchi komponent chaqiruvingizni xuddi haqiqiy dasturchiday yozdingiz.`);
      }, 300);
    }
  }, [valid]);
  const Ln = ({ n, children }) => <div className="vsc-line"><span className="vsc-ln">{n}</span><span style={{ whiteSpace: "pre" }}>{children}</span></div>;
  return <Stage eyebrow={tr2({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!passed} label={passed ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Chaqiruvni yozing", ru: "Напишите вызов" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Oxirgi qadam: komponentni <span className="italic" style={{ color: T.accent }}>o'zingiz</span> chaqiring.</>, ru: <>Последний шаг: вызовите компонент <span className="italic" style={{ color: T.accent }}>сами</span>.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            <div className="fade-up delay-2" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <span className="tagpill" style={{ opacity: hasComp ? 1 : 0.4 }}>{hasComp ? "✓" : "1"} {"<GameCard"} — {tr2({ uz: "Katta harf", ru: "Заглавная буква" })}</span>
            ...
            {hasLowerComp && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Esingizdami? Komponent nomi <b>Katta harf</b> bilan: <span className="mono">GameCard</span> — aks holda React uni HTML teg deb o'ylaydi.</>, ru: <>Помните? Имя компонента — с <b>Заглавной буквы</b>: <span className="mono">GameCard</span> — иначе React примет его за HTML-тег.</> })}</p></div>}
            {passed && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "✓ Mukammal! Komponent + props — chaqiruvni to'liq o'zingiz yozdingiz. Bu endi sizning mahoratingiz.", ru: "✓ Превосходно! Компонент + props — вы полностью сами написали вызов. Теперь это ваш навык." })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "natija — localhost:5173", ru: "результат — localhost:5173" })}</p>
            <Win title="robo-games — localhost:5173" minH={130}>
              {valid ? <div key={inner} className="fade-step" style={{ maxWidth: 170 }}><RoCard name={inner} likeable /></div> : <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center", lineHeight: 1.5 }}>{tr2({ uz: "4-qatorni to'liq yozing:", ru: "Напишите 4-ю строку полностью:" })} <span className="mono" style={{ fontStyle: "normal" }}>{"<GameCard"}</span> + <span className="mono" style={{ fontStyle: "normal" }}>name="…"</span> + <span className="mono" style={{ fontStyle: "normal" }}>{"/>"}</span></p>}
            </Win>
          </Col>
        </div>
        </Zoomable>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Tayyor", ru: "Готово" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "Dars tugadi", ru: "Урок окончен" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Birinchi komponentingiz <span className="italic" style={{ color: T.accent }}>tayyor</span>.</>, ru: <>Ваш первый компонент <span className="italic" style={{ color: T.accent }}>готов</span>.</> })}</h2>{
    /* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>{tr2({ uz: "📝 Uyga vazifa", ru: "📝 Домашнее задание" })}</div><p className="body" style={{ margin: "0 0 10px", color: T.ink }}>{tr2({ uz: "Bugungi qadamlarni o'z kompyuteringizda takrorlang:", ru: "Повторите сегодняшние шаги на своём компьютере:" })}</p><ul>{HOMEWORK.map((h, i) => <li key={i}><b>{tr2(h.b)}</b> <span className="t">{tr2(h.t)}</span></li>)}</ul><p className="hw-note">{tr2({ uz: "Keyingi darsda kartochkangiz jonlanadi: 👍 bosilganda son o'zi yangilanadi — bu State! 🚀", ru: "На следующем уроке ваша карточка оживёт: при нажатии 👍 число обновится само — это State! 🚀" })}</p></div>}
...
function ReactFirstComponentLesson({ lang: langProp, onFinished, liveToken }) {
...
export {
  ReactFirstComponentLesson as default
};
