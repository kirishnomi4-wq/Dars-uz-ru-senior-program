...
//  Manba:  src/4c-Modull/PmLesson18.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/4c-Modull/PmLesson18.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/4c-Modull/PmLesson18.jsx
...
var LESSON_META = { lessonId: "pm-m4c6-v1", lessonTitle: { uz: "Saytingiz hozir ochilyaptimi?", ru: "Ваш сайт сейчас открывается?" } };
var SCREEN_META = [
...
var SCREEN_INTENTS = {
  s0: "Bola o'z sayti shu daqiqada ochilyaptimi degan savolga javob tanlaydi va uni hech kim aytmasligini ko'radi",
  s1: "Bola dars oxirida saytiga uchta qoida yozib olishini oldindan ko'radi",
  s2: "Bola chiqarishgacha va chiqqandan keyingi tekshiruvni solishtirib, o'lchagich nima ekanini biladi",
  s3: "Bola o'lchagichsiz sayt ochilmay qolganini birinchi bo'lib kim ko'rishini tanlaydi",
  s4: "Bola bir kunlik o'lchovlarni ko'radi va uch chegarani qo'yib qaysi xabarni odam sezganini o'zi sanaydi",
  s5: "Bola sekin javobda kirgan odam nima qilishini aniqlaydi",
  s6: "Bola brauzeri har kirishda holat va vaqtni o'zi o'lchab yozishini o'z ekranida tekshiradi",
  s7: "Bola xatoni holat raqami aytishini, vaqt emasligini tanlaydi",
  s8: "Bola o'z saytiga uchta signal-qoidasini bittalab yozadi: chegara-son va sabab",
  s9: "Bola to'rt signalning har biriga yo'l tanlaydi: hozir xabar yoki jurnalga",
  s10: "Bola VS Code'da saytini o'lchaydigan uch shartni yozadi va terminalda natijani ko'radi",
  s11: "Bola juda past chegara qanday xabarlar berishini tanlaydi",
...
var INLINE_KEYS = { s3: 1, s5: 0, s7: 2, s11: 1, kun: -1, qoida: -1, yol: -1, koding: -1 };
var RECAPS = {
  3: {
...
var SITE_KEY = "cc-site-url";
var SITE_ZAXIRA = "mening-saytim.netlify.app";
var SITE_YORLIQ = "Saytingiz (Netlify)";
var siteHavola = () => {
  try {
    const raw = (localStorage.getItem(SITE_KEY) || "").trim();
    if (!raw) return "";
    const t = raw.replace(/^https?:\/\//i, "").replace(/\/+$/, "");
    return t.length >= 4 ? t : "";
  } catch {
    return "";
  }
};
var siteNom = () => siteHavola() || SITE_ZAXIRA;
var OLCHAGICH = [
  { id: "ochilish", ic: "🟢", nom: { uz: "Sayt ochiladimi", ru: "Открывается ли сайт" }, oddiy: { uz: "ochilyapti", ru: "открывается" } },
  { id: "vaqt", ic: "⏱", nom: { uz: "Javob vaqti", ru: "Время ответа" }, oddiy: { uz: "0,4 s", ru: "0,4 с" } },
  { id: "xato", ic: "❌", nom: { uz: "Xatolar", ru: "Ошибки" }, oddiy: { uz: "100 dan 0 tasi", ru: "0 из 100" } }
];
var SiteBar = ({ nom, suffix }) => <div className="sitebar">
    <span className="bb-dots"><i /><i /><i /></span>
    <span className="sitebar-u mono">🌐 {nom}{suffix || ""}</span>
  </div>;
var HOOK_OPTS = [
  { k: "ochilyapti", ic: "🟢", t: { uz: "Ochilyapti — o'zim vaqti-vaqti bilan kirib turaman", ru: "Открывается — я сам время от времени захожу" } },
  { k: "bilmayman", ic: "🤷", t: { uz: "Bilmayman — chiqarganimdan beri ochib ko'rmaganman", ru: "Не знаю — с момента выпуска ни разу не открывал" } }
];
var HOOK_KEY = "pm-m4c6-hook-choice";
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  const gate = useContext2(LiveGateCtx) || {};
  const live = gate.live;
  const [nom] = useState3(() => siteHavola());
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish · saytingiz", ru: "Введение · ваш сайт" })} screen={screen} navContent={<NavNext optionalLive turnBusy={picked === null && !isMentor} disabled={picked === null && !isMentor} label={opened ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Bittasini tanlang", ru: "Выберите один" })} onClick={onNext} />}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Saytingiz hozir <span className="italic" style={{ color: T.accent }}>ochilyaptimi?</span></>, ru: <>Ваш сайт сейчас <span className="italic" style={{ color: T.accent }}>открывается?</span></> })}</h2></div>
        <Mentor>{nom ? tr2({ uz: <>Saytingizni Netlify'ga chiqarib qo'ygansiz — havolasi {nom}.</>, ru: <>Вы выложили свой сайт на Netlify — ссылка {nom}.</> }) : tr2({ uz: <>Saytingizni Netlify'ga chiqarib qo'ygansiz — u hozir ham o'sha yerda turibdi.</>, ru: <>Вы выложили свой сайт на Netlify — он и сейчас там.</> })}</Mentor>
    ...
    return <div key={o.k} className={`hvote-row ${picked === i ? "mine" : ""} ${top ? "top" : ""}`}>
        ...
        <MentorNote>{tr2({ uz: "Ovozlar bo'linadi — ikkalasi ham halol javob. «Hech kim aytmaydi» degan joyda bir lahza jim turing: aynan shu bo'shliqni bugungi dars to'ldiradi. Qanday bilish mumkinligini oldindan aytmang.", ru: "Голоса разделятся — оба ответа честные. На словах «вам никто не скажет» сделайте паузу: именно эту пустоту заполняет сегодняшний урок. Как это можно узнать, заранее не говорите." })}</MentorNote>
      </div>
    </Stage>;
};
var DEMO_QOIDA = [
  { ic: "🟢", nom: { uz: "Sayt ochilmay qolsa", ru: "Если сайт перестанет открываться" }, why: { uz: "xabar keladi", ru: "придёт сообщение" } },
  { ic: "⏱", nom: { uz: "Sahifa uzoq kelsa", ru: "Если страница идёт долго" }, why: { uz: "xabar keladi", ru: "придёт сообщение" } },
  { ic: "❌", nom: { uz: "Xato ko'payib ketsa", ru: "Если ошибок станет больше" }, why: { uz: "xabar keladi", ru: "придёт сообщение" } }
];
var Screen1 = ({ screen, onNext, onPrev }) => <Stage eyebrow={tr2({ uz: "Maqsad", ru: "Цель" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Boshlaymiz →", ru: "Начнём →" })} onClick={onNext} /></>}>
    <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
      <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Dars oxirida siz <span className="italic" style={{ color: T.accent }}>nima</span> qila olasiz?</>, ru: <><span className="italic" style={{ color: T.accent }}>Что</span> вы сможете к концу урока?</> })}</h2></div>
      <Mentor>{tr2({ uz: "Pastdagi uch qatorga qarang.", ru: "Посмотрите на три строки ниже." })}</Mentor>
      <div className="s1demo">
        <span className="s1demo-lbl">{tr2({ uz: "🗂 Saytingiz uchun uchta qoida", ru: "🗂 Три правила для вашего сайта" })}</span>
        <div className="s1demo-list">
          {DEMO_QOIDA.map((r, i) => <span key={i} className="s1row" style={{ "--dd": `${0.5 + i * 0.85}s` }}>
              <span className="s1row-ic" style={{ "--dd": `${0.5 + i * 0.85}s` }} aria-hidden="true">{r.ic}</span>
              <span className="s1row-t" style={{ "--dd": `${0.5 + i * 0.85}s` }}>{tr2(r.nom)}</span>
              <span className="s1row-mark" style={{ "--dd2": `${0.95 + i * 0.85}s` }}>→</span>
              <span className="s1row-why" style={{ "--dd3": `${1.15 + i * 0.85}s` }}>{tr2(r.why)}</span>
            </span>)}
        </div>
      </div>
      <div className="takeaway fade-up delay-2"><span className="ta-bulb">🎯</span><p className="ta-h">{tr2({ uz: "Dars oxirida saytingiz uchun uchta qoida yozib olasiz: sayt bilan nima bo'lganda sizga xabar kelishini o'zingiz belgilaysiz.", ru: "В конце урока вы запишете три правила для своего сайта: сами определите, при чём именно с сайтом вам придёт сообщение." })}</p></div>
...
var S2_CARDS = [
  { ic: "✈️", h: { uz: "Chiqarishgacha", ru: "До выпуска" }, b: { uz: "Kodni lenta tekshiradi — yashil bo'lsa sayt uchadi. Xato bo'lsa, u odamlarga yetmaydi", ru: "Код проверяет конвейер — если зелёный, сайт улетает. Если ошибка, до людей она не доходит" } },
  { ic: "🌐", h: { uz: "Chiqqandan keyin", ru: "После выпуска" }, b: { uz: "Sayt odamlar qo'lida. Uni endi hech kim tekshirmaydi — ochilmay qolsa, buni birinchi bo'lib kirgan odam ko'radi", ru: "Сайт в руках у людей. Теперь его никто не проверяет — если перестанет открываться, первым это увидит зашедший человек" } }
];
var Screen2 = ({ screen, onNext, onPrev }) => {
  ...
  useEffect4(() => {
    ...
    const t = setTimeout(() => {
      if (xulRef.current) xulRef.current.scrollIntoView({ behavior: kam ? "auto" : "smooth", block: "nearest" });
    }, 320);
    return () => clearTimeout(t);
  }, [allSeen]);
  return <Stage eyebrow={tr2({ uz: "Muhokama · ikki payt", ru: "Обсуждение · два момента" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!allSeen && !isMentor} disabled={!allSeen && !isMentor} label={allSeen || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `👆 Yana ${qoldi} kartani oching`, ru: `👆 Откройте ещё ${qoldi} карточк${qoldi === 1 ? "у" : "и"}` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Sayt chiqqandan keyin uni <span className="italic" style={{ color: T.accent }}>kim</span> tekshiradi?</>, ru: <><span className="italic" style={{ color: T.accent }}>Кто</span> проверяет сайт после выпуска?</> })}</h2></div>
        <Mentor>{tr2({ uz: "Chiqarishgacha kodni lenta tekshirardi. Chiqqandan keyin sayt odamlar qo'lida — ikki kartani bosib solishtiring.", ru: "До выпуска код проверял конвейер. После выпуска сайт в руках у людей — нажмите на две карточки и сравните." })}</Mentor>
        ...
        {allSeen && <div className="xul fade-step" ref={xulRef}>
            <span className="xul-h">{tr2({ uz: "Sayt chiqqandan keyin uni to'xtovsiz o'lchab turadigan asbob — o'lchagich deyiladi.", ru: "Прибор, который непрерывно измеряет сайт после выпуска, называется измерителем." })}</span>
            <p className="xul-b">{tr2({ uz: "Uchta o'lchagich bor: sayt ochiladimi · javob necha soniyada keladi · 100 kirishdan nechtasi xato.", ru: "Измерителей три: открывается ли сайт · за сколько секунд приходит ответ · сколько из 100 заходов с ошибкой." })}</p>
          </div>}
      </div>
    </Stage>;
};
...
var Screen3 = (props) => <QuestionScreen
  {...props}
  eyebrow={tr2({ uz: "Tekshiruv · kim ko'radi", ru: "Проверка · кто увидит" })}
  scope="module-mikro"
  ctaLabel={tr2({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr2({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr2({ uz: "🌙 Kechasi sayt 20 daqiqa ochilmadi, ertalab yana ishlayapti. O'lchagichsiz buni kim bildi?", ru: "🌙 Ночью сайт 20 минут не открывался, утром снова работает. Кто узнал об этом без измерителя?" })} />}
  questionText={tr2({ uz: "O'lchagichsiz sayt ochilmay qolganini kim bildi", ru: "Кто узнал без измерителя, что сайт не открывался" })}
  options={[tr2({ uz: "Ertalab saytga kirgan dasturchi", ru: "Программист, зашедший на сайт утром" }), tr2({ uz: "O'sha paytda saytga kirgan odamlar", ru: "Люди, заходившие на сайт в тот момент" }), tr2({ uz: "Kodni chiqarishdan oldin tekshirgan lenta", ru: "Конвейер, проверявший код перед выпуском" })]}
  correctIdx={1}
  explainCorrect={tr2({ uz: "To'g'ri — o'lchagich bo'lmasa, sayt ochilmay qolganini birinchi bo'lib kirgan odam ko'radi, dasturchi emas.", ru: "Верно — без измерителя первым увидит, что сайт не открывается, зашедший человек, а не программист." })}
  explainWrong={{
    0: tr2({ uz: "Dasturchi ertalab kirganda sayt allaqachon ishlayapti — u kechasi nima bo'lganini ko'rmaydi.", ru: "Когда программист заходит утром, сайт уже работает — что было ночью, он не увидит." }),
    2: tr2({ uz: "Lenta kodni chiqarishdan oldin tekshiradi; sayt chiqqandan keyin u qaramaydi.", ru: "Конвейер проверяет код перед выпуском; после выпуска он на сайт не смотрит." }),
    default: tr2({ uz: "Sayt ochilmay qolganini o'sha payt kirgan odamlar ko'radi.", ru: "Что сайт не открывается, видят люди, зашедшие в тот момент." })
  }}
/>;
var KUN_KEY = "pm-m4c6-kun";
var KUN_BOSH = 480;
var KUN_OXIR = 1200;
var KUN_MS = 4e4;
var fmtSoat = (m) => `${String(Math.floor(m / 60)).padStart(2, "0")}:${String(Math.floor(m) % 60).padStart(2, "0")}`;
var HODISA = [
  { id: "h1", t: 570, olchov: "vaqt", ic: "⏱", val: { uz: "1,5 s", ru: "1,5 с" }, sek: 1.5, win: 10, dav: { uz: "5 soniya turdi", ru: "держалось 5 секунд" }, fakt: { uz: "Hech kim sezmadi — sahifa odatdagidek ochildi", ru: "Никто не заметил — страница открылась как обычно" } },
  { id: "h2", t: 670, olchov: "vaqt", ic: "⏱", val: { uz: "6 s", ru: "6 с" }, sek: 6, win: 14, dav: { uz: "12 daqiqa turdi", ru: "держалось 12 минут" }, fakt: { uz: "Kirgan odam: «uzoq kutdim, yopib ketdim»", ru: "Зашедший человек: «долго ждал, закрыл и ушёл»" } },
  { id: "h3", t: 800, olchov: "vaqt", ic: "⏱", val: { uz: "1,2 s", ru: "1,2 с" }, sek: 1.2, win: 8, dav: { uz: "bir zum", ru: "на мгновение" }, fakt: { uz: "Hech kim sezmadi", ru: "Никто не заметил" } },
  { id: "h4", t: 845, olchov: "ochilish", ic: "🔴", val: { uz: "ochilmayapti", ru: "не открывается" }, sek: 0, win: 15, dav: { uz: "15 daqiqa ochilmadi", ru: "15 минут не открывался" }, fakt: { uz: "Kirganlar sahifa o'rniga xato ko'rdi", ru: "Зашедшие увидели ошибку вместо страницы" } },
  { id: "h5", t: 960, olchov: "xato", ic: "❌", val: { uz: "100 dan 1 tasi xato", ru: "1 из 100 с ошибкой" }, sek: 0, win: 14, dav: { uz: "", ru: "" }, fakt: { uz: "Bir kishi bo'lmagan manzilni terdi — saytda hamma sahifa joyida", ru: "Один человек набрал несуществующий адрес — на сайте все страницы на месте" } },
  { id: "h6", t: 1120, olchov: "xato", ic: "❌", val: { uz: "100 dan 9 tasi xato", ru: "9 из 100 с ошибкой" }, sek: 0, win: 14, dav: { uz: "", ru: "" }, fakt: { uz: "Bitta sahifa ochilmay qoldi — kirganlar xato ko'rdi", ru: "Одна страница перестала открываться — зашедшие увидели ошибку" } }
];
var CHEGARA = [
  { v: 1, t: { uz: "1 s", ru: "1 с" }, ok: false, res: { uz: "📣 Uch marta xabar keldi — ikkitasini hech kim sezmagan edi (09:30, 13:20).", ru: "📣 Сообщение пришло три раза — двух из них никто не заметил (09:30, 13:20)." } },
  { v: 3, t: { uz: "3 s", ru: "3 с" }, ok: true, res: { uz: "📣 Bir marta xabar keldi — aynan odam kutib yopib ketgan payt (11:10).", ru: "📣 Сообщение пришло один раз — ровно в тот момент, когда человек не дождался и закрыл (11:10)." } },
  { v: 10, t: { uz: "10 s", ru: "10 с" }, ok: false, res: { uz: "📣 Xabar kelmadi — 6 soniyalik kutish 10 soniyaga yetmadi; odam sizdan oldin sezgan edi.", ru: "📣 Сообщение не пришло — ожидание в 6 секунд не дотянуло до 10; человек заметил раньше вас." } }
];
var gaugeHolat = (id, mins) => {
  const hs = HODISA.filter((e) => e.olchov === id && mins >= e.t && mins <= e.t + e.win);
  const h = hs[hs.length - 1];
  return h ? { val: tr2(h.val), hot: true, dav: tr2(h.dav), lev: h.olchov === "ochilish" ? "bad" : "warn" } : id === "vaqt" ? { val: tr2({ uz: ["0,3 s", "0,5 s", "0,4 s", "0,6 s"][Math.floor(mins / 9) % 4], ru: ["0,3 с", "0,5 с", "0,4 с", "0,6 с"][Math.floor(mins / 9) % 4] }), hot: false, dav: "", lev: "" } : { val: tr2(OLCHAGICH.find((x) => x.id === id).oddiy), hot: false, dav: "", lev: "" };
};
var Screen4 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const gate = useContext2(LiveGateCtx) || {};
  const live = gate.live;
  const isMentor = !!(live && live.mode === "mentor");
  const [nom] = useState3(() => siteNom());
  const [reduce] = useState3(() => {
    try {
  ...
  if (savedRef.current === void 0) {
    try {
      savedRef.current = JSON.parse(localStorage.getItem(KUN_KEY) || "null") || {};
    } catch {
      savedRef.current = {};
    }
  }
  const s0 = savedRef.current;
  const [mins, setMins] = useState3(() => s0.dayDone ? KUN_OXIR : KUN_BOSH);
  const [running, setRunning] = useState3(false);
  const [dayDone, setDayDone] = useState3(!!s0.dayDone);
  const [picked, setPicked] = useState3(() => Array.isArray(s0.picked) ? s0.picked : []);
  const [rp, setRp] = useState3(null);
  const [sec, setSec] = useState3(0);
  const [eskiOpen, setEskiOpen] = useState3(false);
  const done = picked.length === CHEGARA.length;
  useEffect4(() => {
    if (!running || reduce) return;
    const step = (KUN_OXIR - KUN_BOSH) / (KUN_MS / 100);
    const id = setInterval(() => setMins((m) => Math.min(KUN_OXIR, m + step)), 100);
    return () => clearInterval(id);
  }, [running, reduce]);
  useEffect4(() => {
    if (running && mins >= KUN_OXIR) {
      setRunning(false);
      setDayDone(true);
    }
  }, [mins, running]);
  useEffect4(() => {
    ...
    const t = setTimeout(() => setRp((r) => r ? { ...r, mins: Math.min(KUN_OXIR, r.mins + (reduce ? 720 : 12)) } : r), reduce ? 50 : 100);
    return () => clearTimeout(t);
  }, [rp, reduce]);
  useEffect4(() => {
    if (!dayDone || done || isMentor) return;
    const t = setInterval(() => setSec((s) => s + 1), 1e3);
    return () => clearInterval(t);
  }, [dayDone, done, isMentor]);
  useEffect4(() => {
    try {
      localStorage.setItem(KUN_KEY, JSON.stringify({ dayDone, picked }));
    } catch {
    }
  }, [dayDone, picked]);
  useEffect4(() => {
    if (done && (storedAnswer === void 0 || !storedAnswer.solved)) {
      onAnswer(screen, { stage: "kun", screenIdx: screen, picked, solved: true, correct: true });
      if (live && live.mode === "student") live.submitAnswer(PRACTICE_BASE + screen, "kun", 0, true, 0);
    }
  }, [done]);
  const boshla = () => {
    if (running || dayDone) return;
    setRunning(true);
    if (reduce) setMins((m) => Math.min(KUN_OXIR, m + 60));
  };
  const soatOldinga = () => setMins((m) => Math.min(KUN_OXIR, m + 60));
  const olChegara = (v) => {
    if (!dayDone || rp || picked.includes(v)) return;
    setRp({ v, mins: KUN_BOSH });
  };
  const faktlar = HODISA.filter((e) => mins >= e.t);
  const aktiv = rp ? rp.v : picked.length ? picked[picked.length - 1] : null;
  const pos = (t) => (t - KUN_BOSH) / (KUN_OXIR - KUN_BOSH) * 100;
  const prog = pos(mins);
  const sigN = aktiv === null ? 0 : HODISA.filter((e) => e.olchov === "vaqt" && e.sek > aktiv && (!rp || rp.mins >= e.t) && mins >= e.t).length;
  const natijalar = picked.map((v) => CHEGARA.find((c) => c.v === v)).filter(Boolean);
  const thrTurn = useTurnHint(dayDone && !done && !rp && !isMentor);
  const startTurn = useTurnHint(!running && !dayDone && !isMentor);
  const tip = dayDone && !done && !rp && picked.length > 0 && sec >= 40;
  const xulRef = useRef3(null);
  useEffect4(() => {
  ...
  const resRef = useRef3(null);
  useEffect4(() => {
    if (done || !picked.length || !resRef.current) return;
    const kam = typeof window !== "undefined" && window.matchMedia && window.matchMedia("(prefers-reduced-motion:reduce)").matches;
    const t = setTimeout(() => {
      if (resRef.current) resRef.current.scrollIntoView({ behavior: kam ? "auto" : "smooth", block: "nearest" });
    }, 320);
    return () => clearTimeout(t);
  }, [picked.length]);
  useEffect4(() => {
    if (done) setEskiOpen(false);
  }, [done]);
  const resBlok = natijalar.length > 0 ? <div className="thr-res" ref={resRef}>
      {natijalar.map((c) => <span key={c.v} className={`res-line${c.ok ? " ok" : ""}`}><b>{tr2(c.t)}</b> — {tr2(c.res)}</span>)}
    </div> : null;
  const navLabel = done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : !dayDone ? running ? tr2({ uz: "① Kun o'tmoqda…", ru: "① День идёт…" }) : tr2({ uz: "① «Kunni boshlash»ni bosing", ru: "① Нажмите «Запустить день»" }) : tr2({ uz: `② Uch sonni ham bosib ko'ring (${picked.length}/3)`, ru: `② Нажмите все три числа (${picked.length}/3)` });
  return <Stage eyebrow={tr2({ uz: "Sinov · bir kunlik o'lchov", ru: "Испытание · измерения за один день" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.3vw,13px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Kunni boshlang va <span className="italic" style={{ color: T.accent }}>o'lchagichlarni</span> o'qing.</>, ru: <>Запустите день и <span className="italic" style={{ color: T.accent }}>читайте измерители</span>.</> })}</h2></div>
        <Mentor>{tr2({ uz: "Tepada saytingizning uch o'lchagichi, pastda bir kunlik vaqt chizig'i.", ru: "Сверху три измерителя вашего сайта, снизу — линия одного дня." })}</Mentor>
        <div className="gpanel fade-up delay-1">
          <SiteBar nom={nom} />
          <div className="grow">
            {OLCHAGICH.map((o) => {
    const g = gaugeHolat(o.id, mins);
    return <div key={o.id} className={`gauge${g.hot ? " " + g.lev : ""}`}>
                  <span className="gauge-h">{o.ic} {tr2(o.nom)}<i className="gauge-led" aria-hidden="true" /></span>
                  <span className="gauge-v mono" key={String(g.val)}>{g.val}</span>
                  {g.hot && g.dav ? <span className="gauge-d">{g.dav}</span> : <span className="gauge-d" />}
                </div>;
  })}
          </div>
          <div className="tl">
            <span className="tl-lbl mono">08:00</span>
            <span className="tl-track">
              <span className="tl-fill" style={{ width: `${prog}%` }} />
              {rp && <span className="tl-scan" style={{ left: `${pos(rp.mins)}%` }} />}
              {HODISA.map((e) => {
    if (mins < e.t) return null;
    const sig = aktiv !== null && e.olchov === "vaqt" && e.sek > aktiv && (!rp || rp.mins >= e.t);
    return <span key={e.id} className={`tl-mark${sig ? " sig" : e.olchov === "ochilish" ? " bad" : ""}`} style={{ left: `${pos(e.t)}%` }} title={fmtSoat(e.t)}>
                    <i>{sig ? "📣" : e.ic}</i>
                  </span>;
  })}
              {!dayDone && <span className="tl-now mono" style={{ left: `${Math.max(4, Math.min(96, prog))}%` }}>{fmtSoat(mins)}</span>}
            </span>
            <span className="tl-lbl mono">20:00</span>
            {aktiv !== null && <span className={`tl-cnt${sigN ? "" : " zero"}`}>📣 {sigN} ta xabar</span>}
          </div>
          {!dayDone && (reduce && running ? <button type="button" className="day-btn" onClick={soatOldinga}>{tr2({ uz: "Keyingi soat ▸", ru: "Следующий час ▸" })}</button> : <button type="button" className={`day-btn${running ? " calm" : ""}${startTurn ? " turn-ring" : ""}`} disabled={running} onClick={boshla}>{running ? tr2({ uz: "Kun o'tmoqda…", ru: "День идёт…" }) : tr2({ uz: "▶ Kunni boshlash", ru: "▶ Запустить день" })}</button>)}
        </div>
        {
    /* Fakt-jurnali: kun yurayotganda oxirgi 3 hodisa ko'rinadi (ekran o'smasin),
       avvalgilari yig'ma qatlamda; kun tugagach hammasi ochiq — yakun-karta uchun. */
  }
        {faktlar.length > 0 && (() => {
    const ochiq = eskiOpen || dayDone && !done;
    const yashirin = ochiq ? 0 : done ? faktlar.length : Math.max(0, faktlar.length - 3);
    const rows = ochiq ? faktlar : done ? [] : faktlar.slice(-3);
    const nMore = ochiq ? faktlar.length : yashirin;
    return <div className="fakts">
              {(done || !dayDone && yashirin > 0) && <button type="button" className="fakts-more" aria-expanded={eskiOpen} onClick={() => setEskiOpen((o) => !o)}>
                  {eskiOpen ? "▾" : "▸"} oldingi {nMore} hodisa
                </button>}
              {rows.map((e) => <span key={e.id} className={`fakt${e.olchov === "ochilish" ? " red" : ""}`}>
                  <b className="mono">{fmtSoat(e.t)}</b> {e.ic} {tr2(e.val)}{e.dav ? ` (${tr2(e.dav)})` : ""} — {tr2(e.fakt)}
                </span>)}
            </div>;
  })()}
        {dayDone && !done && <div className="thr fade-step">
            <p className="flow-label">{tr2({ uz: "⏱ Javob vaqti", ru: "⏱ Время ответа" })}</p>
            <span className="thr-q">{tr2({ uz: "Javob necha soniyadan uzoq kelsa, sizga xabar kelsin?", ru: "Дольше скольких секунд должен идти ответ, чтобы вам пришло сообщение?" })}</span>
            <div className="thr-btns">
              {CHEGARA.map((c, i) => <button key={c.v} type="button" className={`thr-b${picked.includes(c.v) ? " used" : ""}${rp && rp.v === c.v ? " live" : ""}${!picked.includes(c.v) && !rp ? waveCls(thrTurn, i, CHEGARA.length) : ""}`} disabled={!!rp || picked.includes(c.v) || isMentor} onClick={() => olChegara(c.v)}>{tr2(c.t)}</button>)}
            </div>
            <p className="bhint">{tr2({ uz: "☝️ Bu son faqat javob vaqti uchun. Sayt ochilmay qolgan payt uchun keyin alohida son qo'yasiz.", ru: "☝️ Это число только для времени ответа. Для случая, когда сайт не открывается, число вы поставите отдельно позже." })}</p>
            {tip && <p className="bhint fade-step">{tr2({ uz: "🤔 Boshqa sonni ham bosib, natijani solishtiring.", ru: "🤔 Нажмите и другое число, сравните результат." })}</p>}
          </div>}
        {
    /* Uch son bosilgach natijalar va yakun-karta yonma-yon turadi — ikkalasi bitta
       ekranga sig'adi; shungacha natija-qatorlari to'liq kenglikda. */
  }
        {done ? <div className="split">
            <Col gap={9}>{resBlok}</Col>
            <Col gap={9}>
              <div className="xul fade-step" ref={xulRef}>
                <span className="xul-h">{tr2({ uz: "Siz qo'ygan son — chegara.", ru: "Число, которое поставили вы, — это граница." })}</span>
                <p className="xul-b">{tr2({ uz: "O'lchagich chegaradan o'tganda keladigan xabar — signal. Odam sezgan signal — chin, hech kim sezmagan signal — quruq. Chegara odam seza boshlaydigan joyga qo'yiladi.", ru: "Сообщение, которое приходит, когда измеритель перешёл границу, — это сигнал. Сигнал, который заметил человек, — настоящий; который не заметил никто — пустой. Границу ставят туда, где человек начинает замечать." })}</p>
              </div>
            </Col>
          </div> : resBlok}
        {done && <StudentPracticePulse live={live} screen={screen} />}
        <MentorPracticeStats live={live} screen={screen} label={tr2({ uz: "📟 Uch sonni ham bosganlar", ru: "📟 Нажали все три числа" })} />
        <MentorNote>{tr2({ uz: "Bolalar odatda 1 s ni bosib «xabar ko'p — zo'r» deydi. Uchalasini bosgach so'rang: 09:30 dagi xabarni kim sezdi? Javob — hech kim. Chin va quruq farqi shu yerda tug'iladi, siz aytmang. Bu ishni o'quvchilar bajaradi, siz kuzatasiz; «Davom etish» siz uchun ochiq.", ru: "Дети обычно жмут 1 с и говорят «много сообщений — отлично». Когда нажмут все три, спросите: кто заметил сообщение в 09:30? Ответ — никто. Разница между настоящим и пустым рождается здесь, вы её не называйте. Эту работу делают ученики, вы наблюдаете; «Продолжить» для вас открыто." })}</MentorNote>
      </div>
    </Stage>;
};
var Screen5 = (props) => <QuestionScreen
  {...props}
  eyebrow={tr2({ uz: "Tekshiruv · sekin javob", ru: "Проверка · медленный ответ" })}
  scope="module-mikro"
  ctaLabel={tr2({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr2({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr2({ uz: "⏱ Besh daqiqa davomida sahifa 8 soniyada ochildi. Kirgan odam nima qildi?", ru: "⏱ Пять минут подряд страница открывалась за 8 секунд. Что сделал зашедший человек?" })} />}
  questionText={tr2({ uz: "Sahifa 8 soniyada ochilganda kirgan odam nima qildi", ru: "Что сделал зашедший человек, когда страница открывалась за 8 секунд" })}
  options={[tr2({ uz: "Kutib turolmadi — sahifani yopib ketdi", ru: "Не стал ждать — закрыл страницу и ушёл" }), tr2({ uz: "Hech narsa sezmadi — sahifani odatdagidek o'qidi", ru: "Ничего не заметил — читал страницу как обычно" }), tr2({ uz: "Xato yozuvini ko'rdi — sahifani qayta yukladi", ru: "Увидел надпись об ошибке — перезагрузил страницу" })]}
  correctIdx={0}
  explainCorrect={tr2({ uz: "To'g'ri — 3 soniyadan uzoq kutish odamga seziladi: bu chin signal.", ru: "Верно — ожидание дольше 3 секунд человек замечает: это настоящий сигнал." })}
  explainWrong={{
    1: tr2({ uz: "8 soniya odatdagi emas: bir zumlik sakrashni odam sezmaydi, sakkiz soniyalik kutishni sezadi.", ru: "8 секунд — это не «как обычно»: мгновенный скачок человек не замечает, а восьмисекундное ожидание замечает." }),
    2: tr2({ uz: "8 soniyada sahifa baribir keladi — xato yozuvi chiqmaydi, odam shunchaki kutadi.", ru: "За 8 секунд страница всё же приходит — надписи об ошибке нет, человек просто ждёт." }),
    default: tr2({ uz: "8 soniyalik kutishni odam sezadi va sahifani yopib ketadi.", ru: "Восьмисекундное ожидание человек замечает и закрывает страницу." })
  }}
/>;
var holatSteps = (nom) => [
  {
    ic: "🖥",
    h: { uz: "Kompyuteringizda saytingizni oching", ru: "Откройте свой сайт на компьютере" },
    body: { uz: <>Sayt bo'lmasa — istalgan saytni. <code className="qcode">F12</code> ni bosing: brauzerning ish paneli (<span className="uic">DevTools</span>) ochiladi. Undagi <span className="uic">Network</span> bo'limini tanlang. Telefonda bu panel yo'q.</>, ru: <>Если сайта нет — любой сайт. Нажмите <code className="qcode">F12</code>: откроется рабочая панель браузера (<span className="uic">DevTools</span>). Выберите в ней раздел <span className="uic">Network</span>. На телефоне такой панели нет.</> }
  },
  { predict: {
    ask: { uz: "Sahifa necha soniyada kelganini kim o'lchab yozib qo'yadi?", ru: "Кто измеряет и записывает, за сколько секунд пришла страница?" },
    chips: [
      { ic: "🧑‍💻", t: { uz: "Saytni yozgan dasturchi yozib qo'ygan", ru: "Записал программист, написавший сайт" } },
      { ic: "🌐", t: { uz: "Saytning o'zi yozib qo'ygan", ru: "Записал сам сайт" } },
      { ic: "🖥", t: { uz: "Brauzeringiz o'zi o'lchab yozgan", ru: "Ваш браузер измерил и записал сам" } }
    ],
    ans: 2,
    hit: { uz: "🎯 Topdingiz! Brauzeringiz o'zi o'lchab yozgan", ru: "🎯 Угадали! Ваш браузер измерил и записал сам" },
    miss: { uz: "Adashdingiz — asl javob: brauzeringiz o'zi o'lchab yozgan", ru: "Не угадали — верный ответ: ваш браузер измерил и записал сам" }
  } },
  {
    ic: "🔄",
    h: { uz: "Sahifani qayta yuklang (F5)", ru: "Перезагрузите страницу (F5)" },
    body: { uz: <>Panelda qatorlar chiqadi: brauzer sahifa uchun nimani so'ragan bo'lsa, har so'rov — bitta qator. Ikki ustunni toping: <span className="uic">Status</span> — holat (<span className="stc ok">200</span>) va <span className="uic">Time</span> — vaqt. Vaqt <b>ms</b> da: millisekund, ya'ni soniyaning mingdan biri. Buni brauzeringiz o'zi o'lchab yozgan.</>, ru: <>В панели появятся строки: каждый запрос, который браузер сделал для страницы, — это одна строка. Найдите два столбца: <span className="uic">Status</span> — состояние (<span className="stc ok">200</span>) и <span className="uic">Time</span> — время. Время в <b>мс</b>: миллисекунда, то есть тысячная доля секунды. Это измерил и записал ваш браузер сам.</> }
  },
  { predict: {
    ask: { uz: "Manzil oxiriga bo'lmagan sahifa nomini yozsangiz, yangi qatorda qanday holat turadi?", ru: "Если дописать в конец адреса имя несуществующей страницы, какое состояние будет в новой строке?" },
    chips: [
      { ic: "📄", t: { uz: "200 — sahifa odatdagidek keldi", ru: "200 — страница пришла как обычно" } },
      { ic: "🔎", t: { uz: "404 — bunday sahifa topilmadi", ru: "404 — такая страница не найдена" } },
      { ic: "⬜", t: { uz: "Hech narsa — qator umuman chiqmaydi", ru: "Ничего — строка вообще не появится" } }
    ],
    ans: 1,
    hit: { uz: "🎯 Topdingiz! 404 — bunday sahifa topilmadi", ru: "🎯 Угадали! 404 — такая страница не найдена" },
    miss: { uz: "Adashdingiz — asl javob: 404 — bunday sahifa topilmadi", ru: "Не угадали — верный ответ: 404 — такая страница не найдена" }
  } },
  {
    ic: "🔎",
    h: { uz: "Endi manzil oxiriga bo'lmagan sahifa nomini yozing", ru: "Теперь допишите в конец адреса имя несуществующей страницы" },
    body: { uz: <><code className="qcode">{nom}/yoq</code> — yangi qatorda <span className="uic">Status</span> ustunida <span className="stc bad">404</span> turadi. Brauzer xatoni ham o'zi yozib qo'yadi.</>, ru: <><code className="qcode">{nom}/yoq</code> — в новой строке в столбце <span className="uic">Status</span> будет <span className="stc bad">404</span>. Ошибку браузер тоже записывает сам.</> }
  },
  {
    ic: "🧾",
    h: { uz: "Brauzer har kirishda uchala narsani o'zi o'lchaydi", ru: "При каждом заходе браузер сам измеряет все три вещи" },
    body: { uz: <>Sahifa keldimi · necha ms da keldi · qaysi holat bilan. Lekin bu raqamlar faqat sizning ekraningizda va faqat siz qaraganda turadi. O'lchagich xuddi shu ishni sizsiz, har daqiqa qiladi.</>, ru: <>Пришла ли страница · за сколько мс пришла · с каким состоянием. Но эти числа есть только на вашем экране и только пока вы смотрите. Измеритель делает ровно ту же работу без вас, каждую минуту.</> }
  },
  {
    bridge: true,
    ic: "🌉",
    body: { uz: <>Brauzer buni allaqachon o'lchayapti — faqat siz qaraganda. O'lchagich shu raqamlarni to'xtovsiz o'qiydi va chegaradan o'tsa sizga xabar beradi. Endi saytingizga shu chegaralarni o'zingiz yozasiz.</>, ru: <>Браузер уже это измеряет — только пока вы смотрите. Измеритель читает те же числа непрерывно и, если перейдена граница, шлёт вам сообщение. Теперь эти границы для своего сайта запишете вы сами.</> }
  }
];
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [steps] = useState3(() => holatSteps(siteNom()));
  const [i, setI] = useState3(0);
  ...
  return <Stage eyebrow={tr2({ uz: "🖥 Haqiqiy holat", ru: "🖥 Реальная ситуация" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={betPending && !isMentorK} disabled={betPending && !isMentorK} label={betPending && !isMentorK ? tr2({ uz: "Avval o'zingiz belgilang", ru: "Сначала отметьте сами" }) : last ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Keyingi bosqich (${i + 1}/${steps.length})`, ru: `Следующий шаг (${i + 1}/${steps.length})` })} onClick={last ? onNext : () => setI(i + 1)} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Shu raqamlarni hozir <span className="italic" style={{ color: T.accent }}>o'zingiz</span> topa olasizmi?</>, ru: <>Сможете <span className="italic" style={{ color: T.accent }}>сами</span> найти эти числа прямо сейчас?</> })}</h2></div>
        {c.predict && <div className={`kp-bet fade-step${bet !== void 0 ? " answered" : ""}`} key={`b${i}`}>
            <span className="k-slide-eyebrow">{bet === void 0 ? tr2({ uz: "🎲 Avval o'zingiz belgilab ko'ring", ru: "🎲 Сначала отметьте сами" }) : tr2({ uz: "🖥 Haqiqiy holat", ru: "🖥 Реальная ситуация" })} · {i + 1} / {steps.length}</span>
    ...
    return <button key={k} className={cls} disabled={locked} onClick={() => setBets((p) => ({ ...p, [i]: k }))}>
        ...
        {showSlide && <div className="k-slide fade-step" key={`s${i}`}>
            <span className="k-slide-eyebrow">{tr2({ uz: "🖥 Haqiqiy holat", ru: "🖥 Реальная ситуация" })} · {i + 1} / {steps.length}</span>
            <div className="k-slide-ic">{c.ic}</div>
            {c.h && <h3 className="k-slide-h">{tr2(c.h)}</h3>}
    ...
    return <button key={k} className={`k-dot ${k === i ? "cur" : k < i ? "fill" : ""}`} disabled={!ochiq} onClick={() => ochiq && setI(k)} aria-label={tr2({ uz: `${k + 1}-bosqich`, ru: `Шаг ${k + 1}` })} title={ochiq ? void 0 : tr2({ uz: "Avval shu bosqichni tugating", ru: "Сначала завершите этот шаг" })} />;
        ...
        <MentorNote>{tr2({ uz: "Hozir kompyuterida F12 bosib ko'rmoqchi bo'lganlar bo'ladi — ruxsat bering, bu darsning eng foydali daqiqasi. Telefonda bu panel yo'q — kompyuterda.", ru: "Найдутся те, кто прямо сейчас захочет нажать F12 у себя на компьютере — разрешите, это самая полезная минута урока. На телефоне такой панели нет — только на компьютере." })}</MentorNote>
      </div>
    </Stage>;
};
var Screen7 = (props) => <QuestionScreen
  {...props}
  eyebrow={tr2({ uz: "Tekshiruv · holat va vaqt", ru: "Проверка · состояние и время" })}
  scope="module-mikro"
  ctaLabel={tr2({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr2({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr2({ uz: "🖥 Bir qator 200 · 420 ms, ikkinchisi 404 · 35 ms. Qaysi birida xato bor?", ru: "🖥 Одна строка 200 · 420 мс, другая 404 · 35 мс. В какой из них ошибка?" })} />}
  questionText={tr2({ uz: "Ikki qatorning qaysi birida xato bor", ru: "В какой из двух строк ошибка" })}
  options={[tr2({ uz: "200 turgan qatorda — u sekinroq keldi", ru: "В строке с 200 — она пришла медленнее" }), tr2({ uz: "Ikkalasida ham — vaqtlari har xil", ru: "В обеих — у них разное время" }), tr2({ uz: "404 turgan qatorda — sahifa topilmadi", ru: "В строке с 404 — страница не найдена" })]}
  correctIdx={2}
  explainCorrect={tr2({ uz: "To'g'ri — xatoni holat raqami aytadi, vaqt emas: 404 — so'ralgan sahifa yo'q.", ru: "Верно — об ошибке говорит число состояния, а не время: 404 — запрошенной страницы нет." })}
  explainWrong={{
    0: tr2({ uz: "Sekinlik va xato — ikki alohida o'lchagich; 420 ms esa uzoq kutish ham emas.", ru: "Медленность и ошибка — два разных измерителя; а 420 мс — это даже не долгое ожидание." }),
    1: tr2({ uz: "Vaqtlarning har xilligi xato emas — xatoni holat raqami aytadi.", ru: "Разное время — это не ошибка; об ошибке говорит число состояния." }),
    default: tr2({ uz: "Xatoni holat raqami aytadi: 404 — so'ralgan sahifa yo'q.", ru: "Об ошибке говорит число состояния: 404 — запрошенной страницы нет." })
  }}
/>;
var IN_KEY = "pm-m4c2-reliz";
var readReliz = () => {
  try {
    const d = JSON.parse(localStorage.getItem(IN_KEY) || "null");
    if (!d || typeof d !== "object" || !Array.isArray(d.bolaklar)) return null;
    const rows = d.bolaklar.map((b, k) => {
      const ish = b && typeof b === "object" ? String(b.ish || "").trim() : typeof b === "string" ? b.trim() : "";
      const hRaw = b && typeof b === "object" ? String(b.hafta || "").trim() : "";
      const hafta = /hafta/i.test(hRaw) ? hRaw : `${hRaw || k + 1}-hafta`;
      return { hafta, ish };
    }).filter((b) => b.ish.length >= 2);
    return rows.length >= 3 ? rows.slice(0, 3) : null;
  } catch {
    return null;
  }
};
var qisqart = (s, n = 24) => s.length > n ? `${s.slice(0, n - 1).trimEnd()}…` : s;
var OUT_KEY = "pm-m4c6-signal";
var DRAFT_KEY = "pm-m4c6-draft";
var QOIDA_MAYDON = [
  { id: "ochilish", ic: "🟢", nom: { uz: "Sayt ochiladimi", ru: "Открывается ли сайт" }, ph: { uz: "Sonni yozing", ru: "Введите число" }, birlik: { uz: "daqiqa", ru: "минут" }, savol: { uz: "Necha daqiqa ochilmasa, xabar kelsin?", ru: "Сколько минут не открывается — и приходит сообщение?" }, shart: (n) => tr2({ uz: `${n} daqiqa ochilmasa`, ru: `не открывается ${n} минут` }) },
  { id: "vaqt", ic: "⏱", nom: { uz: "Javob vaqti", ru: "Время ответа" }, ph: { uz: "Sonni yozing", ru: "Введите число" }, birlik: { uz: "soniya", ru: "секунд" }, savol: { uz: "Necha soniyadan uzoq kelsa, xabar kelsin?", ru: "Дольше скольких секунд идёт ответ — и приходит сообщение?" }, shart: (n) => tr2({ uz: `${n} soniyadan uzoq kelsa`, ru: `ответ идёт дольше ${n} секунд` }) },
  { id: "xato", ic: "❌", nom: { uz: "Xatolar", ru: "Ошибки" }, ph: { uz: "Sonni yozing", ru: "Введите число" }, birlik: { uz: "tasi (100 kirishdan)", ru: "из 100 заходов" }, savol: { uz: "100 kirishdan nechtasi xato bo'lsa, xabar kelsin?", ru: "Сколько из 100 заходов с ошибкой — и приходит сообщение?" }, shart: (n) => tr2({ uz: `100 kirishdan ${n} tasi xato bo'lsa`, ru: `${n} из 100 заходов с ошибкой` }) }
];
var M_APO = "['ʻ’]";
var normSab = (s) => s.toLowerCase().replace(new RegExp(M_APO, "g"), "'").replace(/\s+/g, " ").trim();
var ODAM_RE = { uz: /(odam|kishi|kirgan|kiruvchi|sezad|sezm|sezib|kutad|kutib|kutm|yopib|ketad|ko'rad|ko'rm|ko'rib|chiqib ketad)/, ru: /(человек|люди|людей|зашед|заход|замет|ждат|ждёт|ждет|ожида|закро|закрыв|уйд|уход|увид)/ };
var BOSH_RE = { uz: /(muhim|kerak|yaxshi|xavfli|zarur)/, ru: /(важн|нужн|хорош|опасн|необходим)/ };
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === "mentor");
  const [reliz] = useState3(() => readReliz());
  const draftRef = useRef3(void 0);
  if (draftRef.current === void 0) {
    try {
      draftRef.current = JSON.parse(localStorage.getItem(DRAFT_KEY) || "null") || {};
    } catch {
      draftRef.current = {};
    }
  }
  const d0 = draftRef.current;
  const [list, setList] = useState3(() => storedAnswer && Array.isArray(storedAnswer.qoidalar) ? storedAnswer.qoidalar : Array.isArray(d0.list) ? d0.list : []);
  const [son, setSon] = useState3(() => typeof d0.son === "string" ? d0.son : "");
  const [sabab, setSabab] = useState3(() => typeof d0.sabab === "string" ? d0.sabab : "");
  ...
  const savedRef = useRef3(false);
  const idx = edit === null ? Math.min(list.length, QOIDA_MAYDON.length - 1) : edit;
  const cur = QOIDA_MAYDON[idx];
  const sonVal = sonOqi(son);
  const sababTxt = sabab.trim();
  const uzun = sababTxt.length >= 10;
  const n = normSab(sababTxt);
  const odamBor = uzun && anyTest(ODAM_RE, n);
  const boshSoz = uzun && anyTest(BOSH_RE, n) && !odamBor;
  const past = sonVal !== null && (cur.id === "vaqt" && sonVal < 1 || cur.id === "xato" && sonVal === 0);
  const canSave = sonVal !== null && uzun;
  const inputTurn = useTurnHint(!done && sonVal !== null && !uzun && !focus && !isMentor);
  useEffect4(() => {
    if (!done || savedRef.current) return;
    savedRef.current = true;
    if (storedAnswer === void 0 || !storedAnswer.solved) {
      onAnswer(screen, { stage: "qoida", screenIdx: screen, qoidalar: list.slice(0, 3), solved: true, correct: true });
      if (live && live.mode === "student") live.submitAnswer(PRACTICE_BASE + screen, "qoida", 0, true, 0);
    }
  }, [done]);
  useEffect4(() => {
    if (!done) return;
    const payload = { signallar: list.slice(0, 3).map((r) => ({ olchov: r.olchov, chegara: r.chegara, sabab: r.sabab })), savedAt: Date.now() };
    try {
  ...
  useEffect4(() => {
    try {
      localStorage.setItem(DRAFT_KEY, JSON.stringify({ list, son, sabab }));
    } catch {
    }
  }, [list, son, sabab]);
  const save = () => {
    if (!canSave) return;
    const yozuv = { olchov: cur.id, nom: tr2(cur.nom), ic: cur.ic, chegara: sonVal, matn: cur.shart(son.replace(".", ",")), sabab: sababTxt };
  ...
  const startEdit = (k) => {
    setEdit(k);
    setSon(String(list[k].chegara).replace(".", ","));
    setSabab(list[k].sabab);
  };
  const xulRef = useRef3(null);
  useEffect4(() => {
    if (!done || !xulRef.current) return;
  ...
  const navLabel = done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : list.length === 0 ? tr2({ uz: "① Birinchi o'lchagichga qoida yozing", ru: "① Напишите правило для первого измерителя" }) : tr2({ uz: `② Yana ${3 - list.length} qoida qoldi`, ru: `② Осталось ещё ${3 - list.length} правил${3 - list.length === 1 ? "о" : "а"}` });
  ...
  return <Stage eyebrow={tr2({ uz: "Mustaqil ish · uch qoida", ru: "Самостоятельная работа · три правила" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Saytingizga uchta <span className="italic" style={{ color: T.accent }}>qoida</span> yozing.</>, ru: <>Запишите для своего сайта три <span className="italic" style={{ color: T.accent }}>правила</span>.</> })}</h2></div>
        {
    /* 92b: kirish-artefakt bir qatorlik tasma — ish-maydoni emas, ko'prik */
  }
        {reliz && <div className="tasma fade-up">
            {reliz.map((b, k) => <span key={k} className="tasma-i">✈️ {b.hafta}: {qisqart(b.ish)}</span>)}
          </div>}
        <Mentor>{reliz ? tr2({ uz: "Haftalarga bo'lib yozgan uch ishingiz endi saytda — ular ishlayaptimi, buni o'lchagich aytadi.", ru: "Три задачи, которые вы расписали по неделям, теперь на сайте — работают ли они, скажет измеритель." }) : tr2({ uz: "Saytingiz chiqdi va endi odamlar qo'lida — u yaxshi ishlayaptimi, buni o'lchagich aytadi.", ru: "Ваш сайт вышел и теперь в руках у людей — хорошо ли он работает, скажет измеритель." })}</Mentor>
        {
    /* 80a: havoda uch doira — har biri o'z o'lchagich-belgisi bilan */
  }
        <div className="stps fade-up">
          {QOIDA_MAYDON.map((m, k) => <span key={m.id} className={`stp ${list.length > k ? "done" : idx === k ? "on" : ""}`}><i>{list.length > k ? "✓" : m.ic}</i>{tr2(m.nom)}</span>)}
        </div>
        <div className="split">
          <Col gap={9}>
            {(!done || edit !== null) && <div className="wsp-ed">
                <span className="wsp-ed-h">{cur.ic} {tr2(cur.nom)}</span>
                <span style={{ fontFamily: "Manrope", fontWeight: 700, fontSize: 13.5, lineHeight: 1.45, color: T.ink, minWidth: 0, overflowWrap: "anywhere" }}>{tr2(cur.savol)}</span>
                <div className="numrow">
                  <input
    className={`reflect-input num-in${sonVal !== null ? " filled" : ""}`}
    value={son}
    maxLength={6}
    inputMode="decimal"
    placeholder={tr2(cur.ph)}
    onChange={(e) => setSon(e.target.value)}
  />
                  <span className="numrow-u">{tr2(cur.birlik)}</span>
                </div>
                {sonVal === null && son.trim().length > 0 && <p className="sfb ask">{tr2({ uz: "🤔 Bu yerga son yozing: necha daqiqa, necha soniya yoki nechta xato.", ru: "🤔 Сюда впишите число: сколько минут, сколько секунд или сколько ошибок." })}</p>}
                {sonVal !== null && <>
                    <input
    className={`reflect-input${inputTurn ? " await" : ""}${uzun ? " filled" : ""}`}
    value={sabab}
    maxLength={140}
    placeholder={tr2({ uz: "Nega aynan shu son?", ru: "Почему именно это число?" })}
  ...
  />
    ...
    /* 106d: ikki tomonlama javob — bloklamaydi, yo'naltiradi */
                    ...
                    {!uzun && sababTxt.length > 0 && <p className="sfb ask">{tr2({ uz: "🤔 Qisqa qoldi: to'liq gap bilan yozing.", ru: "🤔 Слишком коротко: напишите полным предложением." })}</p>}
                    {boshSoz && <p className="sfb ask">{tr2({ uz: "🤔 Bu hali sabab emas. Bu sonda saytga kirgan odam nimani sezadi? Shuni yozing.", ru: "🤔 Это ещё не причина. Что при таком числе заметит зашедший на сайт человек? Вот это и напишите." })}</p>}
                    {uzun && !boshSoz && !odamBor && <p className="sfb ask">{tr2({ uz: "🤔 Bu sonda saytga kirgan odam nimani sezadi? Shuni yozing.", ru: "🤔 Что при таком числе заметит зашедший на сайт человек? Вот это и напишите." })}</p>}
                    {odamBor && <p className="sfb ok">{tr2({ uz: "✅ Sababingiz odam nimani sezishini aytyapti.", ru: "✅ Ваша причина говорит о том, что заметит человек." })}</p>}
                    {past && <p className="sfb ask">{tr2({ uz: "🤔 Bu chegarada har mayda sakrash signal beradi — odam buni sezadimi? Sonni yoki sababni qayta ko'ring.", ru: "🤔 При такой границе сигнал даёт каждый мелкий скачок — заметит ли это человек? Пересмотрите число или причину." })}</p>}
                  </>}
    ...
    /* 80c: yozilganlar yozish paytida ko'rinmaydi; uchtasi tayyor bo'lgach ro'yxat ochiladi */
            ...
            {done && edit === null && <div className="wsp-list fade-step" ref={xulRef}>
                <span className="wsp-list-h">{tr2({ uz: "🗂 Uch qoidangiz", ru: "🗂 Ваши три правила" })}</span>
                {list.slice(0, 3).map((r, k) => <span key={k} className="wsp-item">
                    <span className="wsp-item-m">{r.ic}</span>
                    <span className="wsp-item-t"><b>{r.nom}</b> → {r.matn} — {r.sabab}</span>
          ...
          <Col gap={9}>
            <div className="wsp-task">
              ...
              {QOIDA_MAYDON.map((m, k) => <span key={m.id} className={`wsp-task-row${list.length > k ? " done" : ""}`}>
                  <span>{m.ic} {tr2(m.nom)}</span>
                  {list[k] && <span className="wsp-task-m mono" aria-hidden="true">{list[k].chegara}</span>}
                </span>)}
              <div className="spec">
                {[tr2({ uz: "Uchta qoida yozilgan", ru: "Записаны три правила" }), tr2({ uz: "Har chegarada son bor", ru: "В каждой границе есть число" }), tr2({ uz: "Har sabab odam sezganini aytadi", ru: "Каждая причина говорит о том, что заметил человек" })].map((t, k) => <span key={t} className={`spec-c${specOk[k] ? " ok" : ""}`}>{specOk[k] ? "✓" : "○"} {t}</span>)}
              </div>
              {
    /* 106c-b: holat ko'rsatkichi */
            ...
            <div className="wsxrow">
              <div className={`wsx ${yordamOpen ? "open" : ""}`}>
                ...
                {yordamOpen && <div className="wsx-body"><p>{tr2({ uz: "Bu sonda saytga kirgan odam nimani sezadi?", ru: "Что при таком числе заметит зашедший на сайт человек?" })}</p><p>{tr2({ uz: "U kutadimi, yopib ketadimi, xato ko'radimi?", ru: "Он подождёт, закроет страницу или увидит ошибку?" })}</p></div>}
              </div>
              <div className={`wsx star ${starOpen ? "open" : ""}`}>
                ...
                {starOpen && <div className="wsx-body"><p>{tr2({ uz: "Uch chegarangizdan bittasini ikki barobar pasaytirib ko'ring: endi qanday signallar ko'payadi? Javobingizni bir qatorda yozing.", ru: "Попробуйте снизить одну из трёх границ вдвое: каких сигналов теперь станет больше? Запишите свой ответ одной строкой." })}</p></div>}
              </div>
            </div>
            <StudentPracticePulse live={live} screen={screen} />
            <MentorPracticeStats live={live} screen={screen} label={tr2({ uz: "✍️ Uch qoidani yozganlar", ru: "✍️ Записали три правила" })} />
          </Col>
        </div>
        {done && edit === null && <div className="done-mini fade-step">{tr2({ uz: <>✅ Uch qoidangiz saqlandi <span className="dm-sub">— har birida chegara-son va sabab bor</span></>, ru: <>✅ Ваши три правила сохранены <span className="dm-sub">— в каждом есть число-граница и причина</span></> })}</div>}
        <MentorNote>{tr2({ uz: "«0 soniya», «1 ta xato ham bo'lmasin» kabi juda past chegaralar chiqadi — eng foydali xato. Javob-qatori uni tutadi; siz 1 s natijasini eslating: hech kim sezmagan xabarlar. Baholash-mezoni: uchala chegarada son bor · har sababda odam nimani sezishi aytilgan. Bu ishni o'quvchilar bajaradi, siz kuzatasiz; «Davom etish» siz uchun ochiq.", ru: "Появятся слишком низкие границы вроде «0 секунд», «пусть не будет ни одной ошибки» — самая полезная ошибка. Строка ответа её поймает; напомните результат для 1 с: сообщения, которых никто не заметил. Критерии оценки: во всех трёх границах есть число · в каждой причине написано, что заметит человек. Эту работу делают ученики, вы наблюдаете; «Продолжить» для вас открыто." })}</MentorNote>
      </div>
    </Stage>;
};
var YOL_KEY = "pm-m4c6-yol";
var YOLLAR = [
  { k: "hozir", ic: "📣", t: { uz: "Hozir xabar berish", ru: "Сообщить сейчас" } },
  { k: "jurnal", ic: "📒", t: { uz: "Jurnalga yozish", ru: "Записать в журнал" } }
];
var SIGNALLAR = [
  { id: "g1", ic: "⏱", vaqt: "02:15", t: { uz: "Javob 1,3 soniyaga chiqdi, 4 soniyadan keyin 0,4 ga qaytdi", ru: "Ответ вырос до 1,3 секунды, через 4 секунды вернулся к 0,4" }, yol: "jurnal", sabab: { uz: "Hech kim sezmadi — ertalab ko'rasiz", ru: "Никто не заметил — посмотрите утром" } },
  { id: "g2", ic: "🔴", vaqt: "06:40", t: { uz: "Sayt 20 daqiqadan beri ochilmayapti", ru: "Сайт не открывается уже 20 минут" }, yol: "hozir", sabab: { uz: "Kirgan har bir odam buni ko'ryapti", ru: "Это видит каждый зашедший человек" } },
  { id: "g3", ic: "❌", vaqt: "07:05", t: { uz: "100 kirishdan 8 tasi xato: «Aloqa» sahifasi ochilmayapti", ru: "8 из 100 заходов с ошибкой: страница «Контакты» не открывается" }, yol: "hozir", sabab: { uz: "Bitta sahifa ishlamayapti — kirganlar xato ko'radi", ru: "Одна страница не работает — зашедшие видят ошибку" } },
  { id: "g4", ic: "❌", vaqt: "07:40", t: { uz: "100 kirishdan 1 tasi xato: bir kishi bo'lmagan manzilni terdi", ru: "1 из 100 заходов с ошибкой: один человек набрал несуществующий адрес" }, yol: "jurnal", sabab: { uz: "Saytda hamma sahifa joyida — bu odamning o'z xatosi", ru: "На сайте все страницы на месте — это ошибка самого человека" } }
];
var YOL_TUZAT = {
  jurnal: { uz: "🤔 Bu signalni hech kim sezmagan — u jurnalga yoziladi", ru: "🤔 Этот сигнал никто не заметил — он пишется в журнал" },
  hozir: { uz: "🤔 Buni kirgan odam hozir ko'ryapti — hozir xabar", ru: "🤔 Это зашедший человек видит прямо сейчас — сообщать сейчас" }
};
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === "mentor");
  const [tanlov, setTanlov] = useState3(() => storedAnswer && storedAnswer.tanlov || {});
  const [i, setI] = useState3(() => {
    const t = storedAnswer && storedAnswer.tanlov || {};
    const k = SIGNALLAR.findIndex((s) => !t[s.id]);
    return k < 0 ? SIGNALLAR.length - 1 : k;
  });
  const [missedOnce, setMissedOnce] = useState3(false);
  const [yordamOpen, setYordamOpen] = useState3(false);
  const n = Object.keys(tanlov).length;
  const done = n === SIGNALLAR.length;
  const cur = SIGNALLAR[i];
  const pick = tanlov[cur.id];
  const togri = pick === cur.yol;
  useEffect4(() => {
    try {
      localStorage.setItem(YOL_KEY, JSON.stringify({ yol: tanlov }));
    } catch {
    }
  }, [tanlov]);
  useEffect4(() => {
    if (done && (storedAnswer === void 0 || !storedAnswer.solved)) {
      const hammasi = SIGNALLAR.every((s) => tanlov[s.id] === s.yol);
      onAnswer(screen, { stage: "yol", screenIdx: screen, tanlov, solved: true, correct: hammasi });
      if (live && live.mode === "student") live.submitAnswer(PRACTICE_BASE + screen, "yol", 0, true, 0);
    }
  }, [done]);
  const tanla = (k) => {
  ...
  const nextTurn = useTurnHint(!!pick && i < SIGNALLAR.length - 1);
  const xulRef = useRef3(null);
  useEffect4(() => {
    if (!done || !xulRef.current) return;
    const kam = typeof window !== "undefined" && window.matchMedia && window.matchMedia("(prefers-reduced-motion:reduce)").matches;
  ...
  const navLabel = done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : n === 0 ? tr2({ uz: "① Birinchi signalga yo'l tanlang", ru: "① Выберите путь для первого сигнала" }) : tr2({ uz: `② Yana ${SIGNALLAR.length - n} signal qoldi`, ru: `② Осталось ещё ${SIGNALLAR.length - n} сигнал${SIGNALLAR.length - n === 1 ? "" : "а"}` });
  return <Stage eyebrow={tr2({ uz: "Tekshiruv · tungi signallar", ru: "Проверка · ночные сигналы" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(9px,1.4vw,14px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Har signalga <span className="italic" style={{ color: T.accent }}>yo'l</span> tanlang.</>, ru: <>Выберите <span className="italic" style={{ color: T.accent }}>путь</span> для каждого сигнала.</> })}</h2></div>
        <Mentor>{tr2({ uz: "Uch qoidangiz tayyor — endi tunda va ertalab kelgan to'rt signalga yo'l tanlaysiz.", ru: "Ваши три правила готовы — теперь выберите путь для четырёх сигналов, пришедших ночью и утром." })}</Mentor>
        <p className="task-line fade-up delay-1">{tr2({ uz: "Odam sezgan signal sizni hozir uyg'otsin. Hech kim sezmagani jurnalga tushsin — jurnal keyin o'qiydigan ro'yxat.", ru: "Сигнал, который заметил человек, пусть разбудит вас сейчас. Тот, что не заметил никто, пусть попадёт в журнал — журнал это список, который читают потом." })}</p>
        <div className="split">
          <Col gap={9}>
            <div className={`sigc${!pick ? " wait" : ""}`}>
              <span className="sigc-bar"><i className="sigc-ic" aria-hidden="true">📟</i> {tr2({ uz: "Tunda va ertalab kelgan signallar", ru: "Сигналы, пришедшие ночью и утром" })} <b className="mono">{Math.min(n + (pick ? 0 : 1), SIGNALLAR.length)} / {SIGNALLAR.length}</b></span>
              <div className="sigc-body" key={cur.id}>
                <span className="sigc-t"><b className="mono">{cur.vaqt}</b> {cur.ic} {tr2(cur.t)}</span>
              </div>
              <div className="yolbtns">
                {YOLLAR.map((y, k) => <button key={y.k} type="button" className={`yolb yolb-${y.k}${pick === y.k ? " on" : ""}${pick && y.k === cur.yol ? " ans" : ""}${!pick ? waveCls(yolTurn, k, YOLLAR.length) : ""}`} disabled={!!pick || isMentor} onClick={() => tanla(y.k)}>
                    <span className="yolb-ic">{y.ic}</span><span className="yolb-t">{tr2(y.t)}</span>
                  </button>)}
              </div>
              <AchRule screen={screen} once />
              {pick && <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {!togri && <p className="sfb ask">{tr2(YOL_TUZAT[cur.yol])}</p>}
                  <p className={`sfb ${togri ? "ok" : "ask"}`}>{togri ? "✅" : "📒"} {tr2(cur.sabab)}</p>
                  {i < SIGNALLAR.length - 1 && <button type="button" className={`nextsig${nextTurn ? " turn-ring" : ""}`} onClick={() => setI(i + 1)}>Keyingisi ▸</button>}
                </div>}
            </div>
          </Col>
          <Col gap={9}>
    ...
    /* YORDAM ekran boshida turmaydi — faqat birinchi xatodan keyin ochiladi */
            ...
            {missedOnce && !done && <div className={`wsx ${yordamOpen ? "open" : ""}`}>
                <button className="wsx-toggle" onClick={() => setYordamOpen((o) => !o)}>💡 Yordam {yordamOpen ? "▾" : "▸"}</button>
                {yordamOpen && <div className="wsx-body"><p>{tr2({ uz: "Ikki savol bering: buni saytga kirgan odam sezadimi?", ru: "Задайте два вопроса: заметит ли это зашедший на сайт человек?" })}</p><p>{tr2({ uz: "Saytda biror sahifa ochilmay qoldimi?", ru: "Перестала ли открываться какая-то страница сайта?" })}</p></div>}
              </div>}
            {done && <div className="bdone fade-step" ref={xulRef}>
                <div className="strip">
                  {SIGNALLAR.map((s, k) => <span key={s.id} className={`strip-i${tanlov[s.id] === s.yol ? " ok" : ""}`} style={{ "--i": k }}>
                      <b className="mono">{s.vaqt}</b> {s.ic} → {s.yol === "hozir" ? "📣" : "📒"}
                    </span>)}
                </div>
                <span className="done-mini">{tr2({ uz: <>✅ Odam sezgan signal hozir uyg'otadi <span className="dm-sub">— hech kim sezmagani jurnalga yoziladi: u yo'qolmaydi, keyin o'qiysiz</span></>, ru: <>✅ Сигнал, который заметил человек, будит сейчас <span className="dm-sub">— тот, что не заметил никто, пишется в журнал: он не теряется, но ночью не будит</span></> })}</span>
              </div>}
            <StudentPracticePulse live={live} screen={screen} />
            <MentorPracticeStats live={live} screen={screen} label={tr2({ uz: "📟 To'rt signalni saralaganlar", ru: "📟 Разобрали четыре сигнала" })} />
          </Col>
        </div>
        <MentorNote>{tr2({ uz: "Eng ko'p adashiladigan joy — oxirgi signal: «xato-ku, demak hozir». Ikkinchi savolni eslating: saytda biror narsa ochilmay qoldimi? Yo'q — bir kishi manzilni noto'g'ri terdi. Sinf ish-tartibi: har o'quvchi sherigining uch qoidasini o'qib, har chegaraga «shu sonda odam nimani sezadi?» deb so'raydi; javob topilmasa — chegara qayta yoziladi. Bu ishni o'quvchilar bajaradi, siz kuzatasiz; «Davom etish» siz uchun ochiq.", ru: "Чаще всего ошибаются на последнем сигнале: «ошибка же, значит сейчас». Напомните второй вопрос: перестало ли что-то на сайте открываться? Нет — один человек набрал адрес неправильно. Порядок работы в классе: каждый читает три правила соседа и к каждой границе спрашивает «что при этом числе заметит человек?»; если ответа нет — границу переписывают. Эту работу делают ученики, вы наблюдаете; «Продолжить» для вас открыто." })}</MentorNote>
      </div>
    </Stage>;
};
var KODING_KEY = "pm-m4c6-code";
var readKoding = () => {
  try {
...
var kodMatn = (nom) => tr2({ uz: `// tekshir.js — o'lchagichning bir marta ishlashi
...
olchagich();`, ru: `// tekshir.js — одно срабатывание измерителя
const MANZIL = 'https://${nom}'; // ← ссылка на ваш сайт

async function olchagich() {
  const boshlandi = Date.now();          // текущее время, в миллисекундах
  const javob = await fetch(MANZIL);     // запрос к сайту — одна строка в Network
  const vaqt = Date.now() - boshlandi;   // за сколько мс пришёл ответ

  console.log('Holat:', javob.status, '· Vaqt:', vaqt, 'ms');

  // 1) если javob.status не 200:      '📣 сигнал: ошибка ' + javob.status
  // 2) если vaqt больше 3000:         '📣 сигнал: медленный ответ'
  // 3) если оба в порядке:            '✅ Сайт работает'
}

olchagich();` });
var JS_TOKEN = /(\/\/[^\n]*|'[^']*'|\b(?:const|async|function|await|return|if|else)\b|\b\d+\b)/g;
var jsHl = (ln) => ln.split(JS_TOKEN).filter((p) => p !== void 0 && p !== "").map((p, i) => {
  if (p.startsWith("//")) return <span key={i} style={{ color: "#6A9955" }}>{p}</span>;
  if (p.startsWith("'")) return <span key={i} style={{ color: "#CE9178" }}>{p}</span>;
  if (/^(const|async|function|await|return|if|else)$/.test(p)) return <span key={i} style={{ color: "#C586C0" }}>{p}</span>;
  if (/^\d+$/.test(p)) return <span key={i} style={{ color: "#B5CEA8" }}>{p}</span>;
  return <span key={i}>{p}</span>;
});
var KD_SHART = [
  <>{tr2({ uz: <><code className="qcode">node tekshir.js</code> holat va vaqtni chiqaradi</>, ru: <><code className="qcode">node tekshir.js</code> выводит состояние и время</> })}</>,
  <>{tr2({ uz: <>Uchta alohida <code className="qcode">if</code> uch xil qator chiqaradi</>, ru: <>Три отдельных <code className="qcode">if</code> выводят три разные строки</> })}</>,
  <>{tr2({ uz: "O'z saytingiz uchun natija chiqdi", ru: "Результат получен для вашего сайта" })}</>
];
var ScreenCoding = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [done, setDone] = useState3(!!(storedAnswer && storedAnswer.solved) || !!(saved && saved.done));
  const [gateOk, setGateOk] = useState3(!!(saved && saved.gateOk));
  const [miss, setMiss] = useState3(null);
  const [missedOnce, setMissedOnce] = useState3(false);
  const [yordamOpen, setYordamOpen] = useState3(false);
  const missT = useRef3(null);
  useEffect4(() => () => clearTimeout(missT.current), []);
  ...
  const pickGate = (i) => {
    ...
    if (GATE_OPTS[i].ok) {
      setGateOk(true);
      try {
        localStorage.setItem(KODING_KEY, JSON.stringify({ ...readKoding() || {}, gateOk: true }));
      } catch {
      }
    } else {
      setMiss(i);
  ...
  const complete = () => {
    ...
    setDone(true);
    try {
      localStorage.setItem(KODING_KEY, JSON.stringify({ ...readKoding() || {}, gateOk: true, done: true }));
    } catch {
    }
  };
  const lines = kodMatn(nom).split("\n");
  const doneTurn = useTurnHint(stage2 && !done && !isMentor);
  const navLabel = done || isMentor ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : !stage2 ? tr2({ uz: "🔒 Avval kod-savolini yeching", ru: "🔒 Сначала ответьте на вопрос о коде" }) : tr2({ uz: "② Kodni yozing va tugmani bosing", ru: "② Напишите код и нажмите кнопку" });
  return <Stage eyebrow="Koding · ⌨️ VS Code" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.5vw,15px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Saytingizni o'lchaydigan <span className="italic" style={{ color: T.accent }}>kod</span> yozamiz.</>, ru: <>Напишем <span className="italic" style={{ color: T.accent }}>код</span>, который измеряет ваш сайт.</> })}</h2></div>
        {!stage2 ? <>
            <Mentor>{tr2({ uz: "Avval bitta savol — keyin kod yoziladi.", ru: "Сначала один вопрос — потом пишем код." })}</Mentor>
            <div className="cmt hunt">
              <span className="cmt-lbl">{tr2({ uz: "🔎 Holat 200, vaqt 4200 ms keldi. Kod nima chiqarishi kerak?", ru: "🔎 Пришли состояние 200 и время 4200 мс. Что должен вывести код?" })}</span>
              <div className="gt-btns col3">
                {GATE_OPTS.map((g, i) => <button key={i} type="button" className={`gt-b${miss === i ? " miss" : ""}`} onClick={() => pickGate(i)}>{tr2(g.t)}</button>)}
              </div>
              {missedOnce && <p className="cmt-tip">{tr2({ uz: "🤔 Ikki raqamni alohida o'qing: 200 — sayt ochildi; 4200 ms — bu uch soniyadan uzoqmi?", ru: "🤔 Прочитайте два числа по отдельности: 200 — сайт открылся; 4200 мс — это дольше трёх секунд?" })}</p>}
            </div>
          </> : <>
            <Mentor>{tr2({ uz: "Network'da o'zingiz ko'rgan ikki raqamni endi kod o'lchaydi — sizga uchta if qoladi.", ru: "Два числа, которые вы сами видели в Network, теперь измеряет код — вам остаются три if." })}</Mentor>
            <div className="cmt-fold fade-step"><span className="cmt-done">{tr2({ uz: "✓ 4200 ms — uch soniyadan uzoq: sekin javob", ru: "✓ 4200 мс — дольше трёх секунд: медленный ответ" })}</span></div>
            <div className="split">
              <Col gap={10}>
                <div className={`kdpanel${done ? " is-done" : ""}`}>
                  <p className="flow-label">{tr2({ uz: "Kod nima chiqarsin", ru: "Что должен вывести код" })}</p>
                  <ol className="kdreq">{KD_SHART.map((sh, i) => <li key={i}>{tr2(sh)}</li>)}</ol>
                  <div className={`wsx star ${yordamOpen ? "open" : ""}`}>
                    ...
                    {yordamOpen && <div className="wsx-body">
                      <p>{tr2({ uz: <>Kodni <b>node</b> buyrug'i bilan yurgizgansiz — bu ham shunday: pastdagi qora oynaga (terminal) <b>node tekshir.js</b> deb yozasiz.</>, ru: <>Код вы уже запускали командой <b>node</b> — здесь так же: в чёрное окно снизу (терминал) пишете <b>node tekshir.js</b>.</> })}</p>
                      <p>{tr2({ uz: <>React darsida ko'rgan <b>fetch</b> saytga so'rov yuboradi, <b>await</b> esa javob kelguncha kutadi — shuning uchun funksiya <b>async</b>.</>, ru: <><b>fetch</b>, который вы видели на уроке React, отправляет запрос сайту, а <b>await</b> ждёт ответа — поэтому функция <b>async</b>.</> })}</p>
                      <p>{tr2({ uz: <>Bitta <b>if</b> dan boshlang: <b>javob.status</b> 200 emasmi? Ishlagach <b>vaqt &gt; 3000</b> ni qo'shing. Uchtasi ham alohida <b>if</b> — uchinchisi «ikkalasi joyida» degani: xato ham, sekinlik ham yo'q.</>, ru: <>Начните с одного <b>if</b>: <b>javob.status</b> не 200? Заработало — добавьте <b>vaqt &gt; 3000</b>. Все три — отдельные <b>if</b>; третий срабатывает, когда не сработали первые два.</> })}</p>
                      <p>{tr2({ uz: <>⭐ Qo'shimcha: <b>for</b> bilan 3 marta ketma-ket o'lchang va eng uzun vaqtni alohida chiqaring.</>, ru: <>⭐ Дополнительно: измерьте <b>for</b>-ом три раза подряд и выведите самое долгое время отдельно.</> })}</p>
                    </div>}
                  </div>
                  <button className={`lp-done-btn ${done ? "is-done" : ""}${!done && doneTurn ? " turn-ring" : ""}`} disabled={done} onClick={done ? void 0 : complete}>
                    {done ? "✓ Bajarildi" : "✅ Yozdim — terminalda signal-qatori chiqdi"}
                  </button>
                  {done && <div className="done-mini fade-step">{tr2({ uz: <>✅ Kod saytingizni o'lchadi <span className="dm-sub">— holat va vaqt terminalda chiqdi</span></>, ru: <>✅ Код измерил ваш сайт <span className="dm-sub">— состояние и время вышли в терминале</span></> })}</div>}
                  {!done && isSelf && <button className="kd-skip" onClick={onNext}>{tr2({ uz: "✓ Bu mashqni sinfda bajarganman — davom etish →", ru: "✓ Я делал это упражнение в классе — продолжить →" })}</button>}
                </div>
                <StudentPracticePulse live={live} screen={screen} />
                <MentorPracticeStats live={live} screen={screen} label={tr2({ uz: "⌨️ Kodni yozib bo'lganlar", ru: "⌨️ Дописали код" })} />
              </Col>
              <Col gap={10}>
                <div className="vsc no-copy" onCopy={(e) => e.preventDefault()} onCut={(e) => e.preventDefault()} onPaste={(e) => e.preventDefault()} onContextMenu={(e) => e.preventDefault()}>
                  <div className="vsc-bar">
                    <span className="vsc-tab on"><span style={{ color: "#4FC1FF" }}>⌨️</span> tekshir.js</span>
                    <span className="vsc-lock" title={tr2({ uz: "Kod nusxalanmaydi — o'zingiz terib yozasiz", ru: "Код не копируется — набираете сами" })}>🔒 {tr2({ uz: "qo'lda yoziladi", ru: "пишется руками" })}</span>
                  </div>
                  <div className="vsc-body">
                    {lines.map((ln, i) => <div key={i} className="vsc-line"><span className="vsc-ln">{i + 1}</span><span className="vsc-code">{ln ? jsHl(ln) : " "}</span></div>)}
                  </div>
                  <div className="term">
                    <span className="term-l mono"><b>$</b> node tekshir.js</span>
                    <span className="term-c" aria-hidden="true" />
                  </div>
                </div>
                <p className="bhint">{tr2({ uz: "🔒 Kodni VS Code (kod yoziladigan dastur)da qo'lda yozasiz — nusxalab bo'lmaydi.", ru: "🔒 Код вы пишете вручную в VS Code (программе для написания кода) — скопировать нельзя." })}</p>
              </Col>
            </div>
          </>}
        <MentorNote>{tr2({ uz: "Internet bo'lmasa fetch javob olmaydi — bu ham o'lchagich natijasi («sayt ochilmadi»), shuni ochiq ayting. Sayt-havolasi yo'q bolalar MANZIL qatorini o'zgartirmaydi. Kod 10 daqiqada yoziladi; ulgurmaganlar uyga qisqa variantni oladi. Bu ishni o'quvchilar bajaradi, siz kuzatasiz; «Davom etish» siz uchun ochiq.", ru: "Если интернета нет, fetch не получит ответа — это тоже результат измерителя («сайт не открылся»), скажите это прямо. Те, у кого нет ссылки на сайт, строку MANZIL не меняют. Код пишется за 10 минут; кто не успел, берёт короткий вариант домой. Эту работу делают ученики, вы наблюдаете; «Продолжить» для вас открыто." })}</MentorNote>
      </div>
    </Stage>;
};
var REFLECT_KEY = "pm-m4c6-reflection";
...
var ScreenReflection = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Mustahkamlash · 2 qadam", ru: "Закрепление · 2 шага" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext turnBusy={!written} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Uch chegarangizni <span className="italic" style={{ color: T.accent }}>yoddan</span> ayta olasizmi?</>, ru: <>Сможете назвать свои три границы <span className="italic" style={{ color: T.accent }}>по памяти</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: "Ekranga qaramay javob bering: javob vaqtiga qanday chegara qo'ydingiz va o'sha sonda odam nimani sezadi?", ru: "Ответьте, не глядя на экран: какую границу вы поставили для времени ответа и что при этом числе заметит человек?" })}</Mentor>
        <div className="rcp-flow">
          <div className="rcp-step fade-up delay-1">
            <div className="rcp-step-h"><span className="rcp-n">1</span><div><span className="rcp-t">🗣 {yakka ? tr2({ uz: "Ovoz chiqarib o'zingizga ayting: chegara va odam", ru: "Скажите вслух себе: ваша граница и что заметит человек" }) : tr2({ uz: "Sherigingizga ayting: chegara va odam", ru: "Скажите соседу: ваша граница и что заметит человек" })}</span></div></div>
          ...
          <div className="rcp-step fade-up delay-2">
            ...
            <span className={`turn-wrap${inputTurn ? " turn-ring" : ""}`}>
              <input className="reflect-input" value={text} onChange={(e) => save(e.target.value)} onFocus={() => setReflFocus(true)} onBlur={() => setReflFocus(false)} placeholder={tr2({ uz: "Javob vaqtiga ... qo'ydim, chunki odam ...", ru: "Если ответ идёт дольше ... секунд, человек ..." })} maxLength={160} />
            </span>
            {
    /* 106f(b): yozib bo'lgach mukofot — bitta tabrik-gap va bitta qoida-qatori */
  }
            {written && <div className="rwd fade-step">
                <p className="rwd-t">{tr2({ uz: "✓ Endi siz saytni chiqarib qo'yib ketmaysiz — chiqqandan keyin ham o'lchab turasiz.", ru: "✓ Теперь вы не просто выпускаете сайт и уходите — вы измеряете его и после выпуска." })}</p>
                <span className="rwd-rule">{tr2({ uz: "🎯 Bugungi qoida: chegara odam seza boshlaydigan joyga qo'yiladi", ru: "🎯 Правило дня: границу ставят туда, где человек начинает замечать" })}</span>
              </div>}
          </div>
        </div>
        <MentorNote>{tr2({ uz: "Uchdan biri «odam nimani sezadi» savoliga javob berolmasa — o'lchagich-paneli ekranini qayta oching va 11:10 hodisasini birga ko'ring.", ru: "Если треть класса не отвечает на вопрос «что заметит человек» — снова откройте экран с панелью измерителей и вместе посмотрите событие в 11:10." })}</MentorNote>
      </div>
    </Stage>;
};
      ...
      <div className="fc-cardwrap">
        <div className={`fc-fly ${exiting === "knew" ? "out-knew" : ""} ${exiting === "again" ? "out-again" : ""}`} key={swapRef.current}>
          <div className={`fc-card ${flipped ? "flip" : ""}`} onClick={() => !flipped && !exiting && setFlipped(true)} role="button" tabIndex={0}>
            <div className="fc-face fc-front"><span className="fc-q">{fmtCode(tr2(card.front))}</span><span className="fc-cue">{tr2({ uz: "Javobni o'ylang 🤔", ru: "Подумайте над ответом 🤔" })} <span className="fc-tap">{tr2({ uz: "bosing", ru: "нажмите" })}</span></span></div>
            <div className="fc-face fc-back"><span className={`fc-tag ${fcTier(tr2(card.back))}`}>{tr2(card.back)}</span></div>
          </div>
        </div>
      </div>
      {flipped ? <div className="fc-actions"><button className="fc-btn again" disabled={!!exiting} onClick={() => advance(false)}>✗ Takrorlash</button><button className="fc-btn knew" disabled={!!exiting} onClick={() => advance(true)}>✓ Bildim</button></div> : <p className="fc-hint">{tr2({ uz: "👆 Kartani bosing — javobni ko'rasiz", ru: "👆 Нажмите на карточку — увидите ответ" })}</p>}
    </div>;
}
var FLASHCARDS = [
  { front: { uz: "O'lchagich nima?", ru: "Что такое измеритель?" }, back: { uz: "Sayt chiqqandan keyin uni to'xtovsiz o'lchab turadigan asbob", ru: "Прибор, который непрерывно измеряет сайт после выпуска" } },
  { front: { uz: "Uch o'lchagich nimani ko'rsatadi?", ru: "Что показывают три измерителя?" }, back: { uz: "Sayt ochiladimi · javob necha soniyada · 100 kirishdan nechtasi xato", ru: "Открывается ли сайт · за сколько секунд ответ · сколько из 100 заходов с ошибкой" } },
  { front: { uz: "Signal nima?", ru: "Что такое сигнал?" }, back: { uz: "O'lchagich chegaradan o'tganda keladigan xabar", ru: "Сообщение, которое приходит, когда измеритель перешёл границу" } },
  { front: { uz: "Qaysi signal chin?", ru: "Какой сигнал настоящий?" }, back: { uz: "Odam sezgan signal", ru: "Тот, который заметил человек" } },
  { front: { uz: "Qaysi signal quruq?", ru: "Какой сигнал пустой?" }, back: { uz: "Hech kim sezmagan signal", ru: "Тот, который не заметил никто" } },
  { front: { uz: "Chegara qayerga qo'yiladi?", ru: "Куда ставят границу?" }, back: { uz: "Odam seza boshlaydigan joyga", ru: "Туда, где человек начинает замечать" } },
  { front: { uz: "Chegara juda past bo'lsa?", ru: "А если граница слишком низкая?" }, back: { uz: "Quruq signallar ko'payadi", ru: "Пустых сигналов становится больше" } },
  { front: { uz: "Chegara juda baland bo'lsa?", ru: "А если граница слишком высокая?" }, back: { uz: "Odam sizdan oldin sezadi", ru: "Человек заметит раньше вас" } },
  { front: { uz: "Network'da har qator haqida qaysi ikki raqam turadi?", ru: "Какие два числа стоят в Network для каждой строки?" }, back: { uz: "Holat (200 · 404) va vaqt (ms)", ru: "Состояние (200 · 404) и время (мс)" } },
  { front: { uz: "O'lchagich saytni qachongacha o'lchaydi?", ru: "До каких пор измеритель измеряет сайт?" }, back: { uz: "To'xtovsiz — kunu-tun (bu ishning inglizchasi — monitoring)", ru: "Непрерывно — днём и ночью (по-английски это monitoring)" } }
];
var ScreenFlashcards = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Takrorlash", ru: "Повторение" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>O'zingizni <span className="italic" style={{ color: T.accent }}>sinab ko'ring</span>.</>, ru: <><span className="italic" style={{ color: T.accent }}>Проверьте</span> себя.</> })}</h2></div>
...
var ScreenFinalTest = (props) => <QuestionScreen
  ...
  ctaLabel={tr2({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr2({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr2({ uz: "⏱ Javob vaqtiga 0,5 soniya chegara qo'ydingiz. Kun bo'yi kelgan xabarlar qanday bo'ladi?", ru: "⏱ Вы поставили границу времени ответа 0,5 секунды. Какими будут сообщения за день?" })} />}
  questionText={tr2({ uz: "0,5 soniya chegarada kun bo'yi kelgan xabarlar qanday bo'ladi", ru: "Какими будут сообщения за день при границе 0,5 секунды" })}
  options={[tr2({ uz: "Kam va aniq — har biri odam sezgan payt", ru: "Мало и точно — каждое в момент, который заметил человек" }), tr2({ uz: "Ko'p — ko'pini hech kim sezmaydi", ru: "Много — большинство из них никто не заметит" }), tr2({ uz: "Umuman kelmaydi — chegara juda baland", ru: "Не придут вовсе — граница слишком высокая" })]}
  correctIdx={1}
  explainCorrect={tr2({ uz: "To'g'ri — chegara odam sezadigan joydan pastda: hech kim sezmagan sakrashlar ham signal beradi. Bunday signal — quruq.", ru: "Верно — граница ниже того, что замечает человек: сигнал дают и те скачки, которых никто не заметил. Такой сигнал — пустой." })}
  explainWrong={{
    0: tr2({ uz: "0,5 soniya — sahifaning odatdagi vaqti ichida (0,3–0,6 s): xabar tinmay keladi.", ru: "0,5 секунды — это внутри обычного времени страницы (0,3–0,6 с): сообщения будут идти без остановки." }),
    2: tr2({ uz: "0,5 soniya baland emas, past chegara — xabar kamaymaydi, aksincha ko'payadi.", ru: "0,5 секунды — не высокая, а низкая граница: сообщений станет не меньше, а больше." }),
    default: tr2({ uz: "Past chegarada xabar ko'p keladi va ko'pini hech kim sezmaydi.", ru: "При низкой границе сообщений приходит много, и большинство из них никто не замечает." })
  }}
/>;
var HW_KEY = "pm-m4c6-hw-target";
var HW_VARIANT = [
  { k: "toliq", t: { uz: "To'liq · ~20 daqiqa", ru: "Полный · ~20 минут" } },
  { k: "qisqa", t: { uz: "Qisqa · ~10 daqiqa", ru: "Короткий · ~10 минут" } }
];
var HW_STEPS = {
  toliq: [{ uz: "Saytingizni oching, F12 → Network'ni oching va sahifani uch marta yuklang", ru: "Откройте свой сайт, нажмите F12 → Network и загрузите страницу три раза" }, { uz: "Har yuklashda holat va vaqtni yozib oling", ru: "При каждой загрузке запишите состояние и время" }, { uz: "Har birini chegarangiz bilan solishtiring: signal chiqdimi? Sababini bir gapda yozing", ru: "Сравните каждое со своей границей времени ответа" }],
  qisqa: [{ uz: "Saytingizni oching va Network'dagi holat va vaqtni yozib oling", ru: "Откройте свой сайт и запишите состояние и время из Network" }, { uz: "Uni javob vaqti chegarangiz bilan solishtiring", ru: "Сравните со своей границей времени ответа" }, { uz: "Chegaradan o'tgan-o'tmaganini bir gapda yozing", ru: "Одним предложением запишите, перешло ли это границу" }]
};
var readHwTarget = () => {
  try {
...
var HW_TOKENS = [
  ...
  { t: { uz: "signal", ru: "сигнал" }, l: 80, tp: 12, s: 11, d: 7.5 },
  { t: { uz: "chin", ru: "настоящий" }, l: 12, tp: 70, s: 11, d: 8 },
  { t: { uz: "quruq", ru: "пустой" }, l: 64, tp: 76, s: 12, d: 6 },
  { t: { uz: "holat", ru: "состояние" }, l: 86, tp: 52, s: 10, d: 9 },
...
var HwCard = ({ variant, onPick }) => {
  ...
  return <div className="card fade-step">
      <div className="card-lbl" style={{ color: T.accent }}>📝 {tr2({ uz: "Uyda nima qilasiz?", ru: "Что сделаете дома?" })}</div>
      <p className="body" style={{ margin: "0 0 10px", color: T.ink }}>{tr2({ uz: "Uyda o'lchagich ishini o'zingiz bajarasiz. Saytingizni brauzerda ochib, Network'dagi holat va vaqtni yozib olasiz va chegarangiz bilan solishtirasiz. Qancha vaqtingiz bor — o'zingiz tanlaysiz.", ru: "Дома вы сами сделаете работу измерителя. Откроете свой сайт в браузере и сравните состояние и время из Network со своей границей. Сколько у вас времени — выбираете сами." })}</p>
      ...
      {variant ? <div className="pmtask fade-step">
          ...
          <div className="pmtask-rows">
            <div className="pmtask-row"><span className="pmtask-k">Necha marta</span><span className="pmtask-v"><b>{variant === "qisqa" ? "1 marta" : "3 marta"}</b></span></div>
            ...
            <div className="pmtask-row"><span className="pmtask-k">{tr2({ uz: "Sayt", ru: "Сайт" })}</span><span className="pmtask-v"><b>{tr2({ uz: "Saytingiz bo'lmasa — istalgan saytda", ru: "Если своего сайта нет — на любом сайте" })}</b></span></div>
          </div>
...
var ACHIEVEMENTS = {
  eagleEye: { icon: "🦅", name: "Eagle Eye!", desc: { uz: "Uch sonni sinab, farqini ko'rdingiz", ru: "Вы попробовали три числа и увидели разницу" } },
  ruleWriter: { icon: "✍️", name: "Rule Writer!", desc: { uz: "Uch qoidani sabab bilan yozdingiz", ru: "Вы записали три правила с причинами" } },
  coolHead: { icon: "🧊", name: "Cool Head!", desc: { uz: "To'rt signalning yo'lini tanladingiz", ru: "Вы выбрали путь для четырёх сигналов" } },
  siteChecker: { icon: "🛰", name: "Site Checker!", desc: { uz: "Saytingizni kod bilan o'lchadingiz", ru: "Вы измерили свой сайт кодом" } }
};
var ACH_TRIGGERS = { s4: "eagleEye", s8: "ruleWriter", s9: "coolHead", s10: "siteChecker" };
...
var Q_LABELS = { 3: { uz: "1 — O'lchagichsiz kim biladi", ru: "1 — Кто узнает без измерителя" }, 5: { uz: "2 — Sekin javob", ru: "2 — Медленный ответ" }, 7: { uz: "3 — Holat va vaqt", ru: "3 — Состояние и время" }, 11: { uz: "4 — Yakuniy savol", ru: "4 — Итоговый вопрос" } };
var QUIZ_MS = 15e3;
var QZ_BG_SHAPES = [
  { ch: { uz: "chegara", ru: "граница" }, l: 5, t: 10, s: 30, d: 19, dl: 0 },
  { ch: { uz: "signal", ru: "сигнал" }, l: 85, t: 8, s: 28, d: 23, dl: 1.5 },
  { ch: { uz: "chin", ru: "настоящий" }, l: 8, t: 72, s: 26, d: 27, dl: 0.8 },
  { ch: { uz: "quruq", ru: "пустой" }, l: 74, t: 68, s: 26, d: 21, dl: 2.2 },
  { ch: { uz: "holat", ru: "состояние" }, l: 45, t: 86, s: 22, d: 25, dl: 1.1 },
  { ch: { uz: "vaqt", ru: "время" }, l: 66, t: 26, s: 24, d: 17, dl: 0.4 },
  { ch: { uz: "sayt", ru: "сайт" }, l: 26, t: 34, s: 26, d: 20, dl: 1.9 },
  { ch: { uz: "xato", ru: "ошибка" }, l: 55, t: 5, s: 20, d: 22, dl: 0.6 },
  { ch: "📣", l: 91, t: 42, s: 26, d: 24, dl: 1.3 },
  { ch: "📒", l: 16, t: 52, s: 28, d: 26, dl: 2.6 },
  { ch: "⏱", l: 2, t: 30, s: 30, d: 28, dl: 3.1 }
];
var QUIZ_BANK = [
  { q: { uz: "O'lchagich nima?", ru: "Что такое измеритель?" }, opts: [{ uz: "Chiqqandan keyin saytni o'lchab turadigan asbob", ru: "Прибор, который измеряет сайт после выпуска" }, { uz: "Sayt kodini chiqarishdan oldin tekshiradigan asbob", ru: "Прибор, который проверяет код сайта перед выпуском" }, { uz: "Sayt sahifalarini yozib beradigan dastur", ru: "Программа, которая пишет страницы сайта" }, { uz: "Sayt havolasini saqlab qo'yadigan ro'yxat", ru: "Список, где хранится ссылка на сайт" }], correct: 0 },
  // §107: ikki variantlik olam — 2/2 yoziladi, farq SABABda qoladi.
  { q: { uz: "O'lchagich qachon ishlaydi?", ru: "Когда работает измеритель?" }, opts: [{ uz: "Chiqarishgacha — kodni tekshirib beradi", ru: "До выпуска — проверяет код" }, { uz: "Chiqarishgacha — xatoni oldin topadi", ru: "До выпуска — находит ошибку раньше" }, { uz: "Chiqqandan keyin — sayt tez ochilsin uchun", ru: "После выпуска — чтобы сайт открывался быстрее" }, { uz: "Chiqqandan keyin — sayt odamlar qo'lida", ru: "После выпуска — сайт в руках у людей" }], correct: 3 },
  { q: { uz: "«Javob vaqti» nimani ko'rsatadi?", ru: "Что показывает «время ответа»?" }, opts: [{ uz: "100 kirishdan nechtasi xato bilan tugaganini", ru: "Сколько из 100 заходов закончились ошибкой" }, { uz: "Sahifada nechta rasm borligini", ru: "Сколько на странице картинок" }, { uz: "Sahifa necha soniyada kelganini", ru: "За сколько секунд пришла страница" }, { uz: "Sayt bir kunda necha marta ochilganini", ru: "Сколько раз за день открыли сайт" }], correct: 2 },
  { q: { uz: "Signal nima?", ru: "Что такое сигнал?" }, opts: [{ uz: "Chegara qilib qo'ygan soningiz", ru: "Число, которое вы поставили границей" }, { uz: "Chegaradan o'tganda keladigan xabar", ru: "Сообщение, которое приходит при переходе границы" }, { uz: "Saytga kirgan odam yozib qoldirgan izoh-xabar", ru: "Отзыв, который оставил зашедший человек" }, { uz: "Sayt ochilmay qolgan payt", ru: "Момент, когда сайт перестал открываться" }], correct: 1 },
  { q: { uz: "Qaysi signal chin?", ru: "Какой сигнал настоящий?" }, opts: [{ uz: "Kechasi odam uxlayotganda kelgan signal", ru: "Сигнал, пришедший ночью, когда человек спит" }, { uz: "Saytga kirgan odam sezgan signal", ru: "Сигнал, который заметил зашедший на сайт человек" }, { uz: "Har mayda sakrashdan kelgan signal", ru: "Сигнал от каждого мелкого скачка" }, { uz: "Ketma-ket ikki marta kelgan signal", ru: "Сигнал, пришедший дважды подряд" }], correct: 1 },
  // 2 «quruq» / 2 «chin»: bola atama-nomini emas, SABABni tanlaydi.
  { q: { uz: "Kechasi javob bir zumga sekinlashdi va o'ziga qaytdi — bu qanday signal?", ru: "Ночью ответ на мгновение замедлился и вернулся к норме — какой это сигнал?" }, opts: [{ uz: "Quruq — uni hech kim sezmadi", ru: "Пустой — его никто не заметил" }, { uz: "Quruq — u kechasi keldi", ru: "Пустой — он пришёл ночью" }, { uz: "Chin — javob sekinlashdi", ru: "Настоящий — ответ замедлился" }, { uz: "Chin — o'lchagich xabar berdi", ru: "Настоящий — измеритель прислал сообщение" }], correct: 0 },
  { q: { uz: "Chegara juda past bo'lsa nima bo'ladi?", ru: "Что будет, если граница слишком низкая?" }, opts: [{ uz: "Quruq signal umuman kelmaydi", ru: "Пустые сигналы вообще не придут" }, { uz: "O'lchagich xatoni ko'rmay qoladi", ru: "Измеритель перестанет видеть ошибки" }, { uz: "Quruq signallar ko'payadi", ru: "Пустых сигналов станет больше" }, { uz: "Sayt tezroq ochiladigan bo'ladi", ru: "Сайт станет открываться быстрее" }], correct: 2 },
  { q: { uz: "Chegara juda baland bo'lsa nima bo'ladi?", ru: "Что будет, если граница слишком высокая?" }, opts: [{ uz: "Har mayda sakrash xabar beradi", ru: "Каждый мелкий скачок будет слать сообщение" }, { uz: "Sayt umuman ochilmay qoladi", ru: "Сайт вообще перестанет открываться" }, { uz: "O'lchagich uch o'lchovni almashtiradi", ru: "Измеритель поменяет три измерения местами" }, { uz: "Odam sizdan oldin sezadi", ru: "Человек заметит раньше вас" }], correct: 3 },
  { q: { uz: "Network'da har qator haqida qaysi ikki raqam turadi?", ru: "Какие два числа стоят в Network для каждой строки?" }, opts: [{ uz: "Holat (200 · 404) va vaqt (ms)", ru: "Состояние (200 · 404) и время (мс)" }, { uz: "Sahifa nomi va rasm o'lchami", ru: "Название страницы и размер картинки" }, { uz: "Sahifa holati va kirganlar soni", ru: "Состояние страницы и число зашедших" }, { uz: "Sayt havolasi va ochilgan vaqt", ru: "Ссылка на сайт и время открытия" }], correct: 0 },
  { q: { uz: "Holat 404 nimani bildiradi?", ru: "Что означает состояние 404?" }, opts: [{ uz: "Sahifa odatdagidek keldi", ru: "Страница пришла как обычно" }, { uz: "Sahifa juda sekin keldi", ru: "Страница пришла очень медленно" }, { uz: "Bunday sahifa topilmadi", ru: "Такая страница не найдена" }, { uz: "Sayt butunlay ochilmadi", ru: "Сайт не открылся совсем" }], correct: 2 },
  // 2 «jurnalga» / 2 «hozir xabarga»; har distraktorning sababi savol-materialida yolg'on.
  { q: { uz: "Javob bir zumga sekinlashdi va qaytdi — bu signalga qaysi yo'l to'g'ri?", ru: "Ответ на мгновение замедлился и вернулся — какой путь верен для этого сигнала?" }, opts: [{ uz: "Hozir xabarga — javob sekinlashdi", ru: "Сообщить сейчас — ответ замедлился" }, { uz: "Jurnalga — uni hech kim sezmadi", ru: "В журнал — его никто не заметил" }, { uz: "Hozir xabarga — sayt ochilmay qoldi", ru: "Сообщить сейчас — сайт перестал открываться" }, { uz: "Jurnalga — saytda xato ko'paydi", ru: "В журнал — на сайте стало больше ошибок" }], correct: 1 },
  { q: { uz: "Chiqqandan keyin saytni o'lchashni kim hal qiladi?", ru: "Кто решает, измерять ли сайт после выпуска?" }, opts: [{ uz: "Brauzer — u xabar yuborib turadi", ru: "Браузер — он присылает сообщения" }, { uz: "Saytga kirgan odamlar", ru: "Люди, зашедшие на сайт" }, { uz: "Hech kim — sayt o'zi ishlab ketaveradi", ru: "Никто — сайт работает сам" }, { uz: "Saytni chiqargan odam — siz", ru: "Тот, кто выпустил сайт, — вы" }], correct: 3 }
];
...
function QzFX() {
  ...
  useEffect4(() => {
    ...
    const TOK = tr2({ uz: ["ochilish", "javob", "xato", "chegara", "signal", "chin", "quruq", "sayt", "📣", "📒"], ru: ["открытие", "ответ", "ошибка", "граница", "сигнал", "настоящий", "пустой", "сайт", "📣", "📒"] });
    const em = [], toks = [];
...
var ScreenSummary = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  const RECAP = [
    tr2({ uz: "Sayt chiqqandan keyin uni to'xtovsiz o'lchab turadigan asbob — o'lchagich.", ru: "Прибор, который непрерывно измеряет сайт после выпуска, — это измеритель." }),
    tr2({ uz: "Odam sezgan signal — chin; hech kim sezmagan signal — quruq.", ru: "Сигнал, который заметил человек, — настоящий; который не заметил никто — пустой." }),
    tr2({ uz: "Chegara odam seza boshlaydigan joyga qo'yiladi.", ru: "Границу ставят туда, где человек начинает замечать." }),
    tr2({ uz: "Saytni chiqarish — ishning yarmi; chiqqandan keyin o'lchab turish — ikkinchi yarmi.", ru: "Выпустить сайт — половина работы; измерять его после выпуска — вторая половина." })
  ];
  ...
  return <Stage eyebrow={tr2({ uz: "Dars yakuni", ru: "Итог урока" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="hero">
          <div className="hero-l">
            ...
            <h2 className="title h-title fade-up d1">Saytingizga uchta <span className="italic" style={{ color: T.accent }}>qoida</span> yozdingiz.</h2>
          </div>
    ...
    /* 103-qonun: darsni bitta gap yopadi */
        ...
        <div className="bigidea fade-up d2"><span className="bigidea-lbl">{tr2({ uz: "Bugungi asosiy fikr —", ru: "Главная мысль сегодня —" })}</span><p className="bigidea-t">{tr2({ uz: "Chiqqandan keyin saytni kim o'lchashini siz hal qilasiz.", ru: "Кто измеряет сайт после выпуска, решаете вы." })}</p></div>
    ...
    return <div key={id} className={`ach-badge ${got ? "got" : "locked"}`} title={tr2(a.desc)}>
        ...
        <MentorNote>{tr2({ uz: "Arena tugagach g'oliblarni nomlab tabriklang. Uy-vazifa: kod topshirig'ini sinfda tugatganlarga to'liq variant, ulgurmaganlarga qisqa. Tekshirishda bitta savolga qarang: yozilgan vaqt chegaradan o'tdimi-yo'qmi — va buni o'quvchi bir gapda ayta oldimi?", ru: "После арены назовите и поздравьте победителей. Домашнее задание: кто закончил код в классе — полный вариант, кто не успел — короткий. При проверке смотрите на один вопрос: перешло ли записанное время границу — и смог ли ученик сказать это одним предложением?" })}</MentorNote>
      </div>
    </Stage>;
};
var CSS_BASE = `
...
var CSS_LESSON = `
  ...
  /* Bir tushuncha — bir rang: ✈️ CHIQARISHGACHA yashil (lentaning o'z so'zi — «yashil bo'lsa uchadi») ·
     🌐 CHIQQANDAN KEYIN indigo (bugungi dars olami: bu yerda o'lchagich kerak bo'ladi). */
  .dfc-grid .dfc:first-child.open { box-shadow: inset 0 0 0 1.5px ${T.success}66, 0 12px 26px -14px rgba(31,122,77,0.3); }
  .dfc-grid .dfc:first-child.open .dfc-b { background: ${T.successSoft}; }
  .dfc-grid .dfc:last-child.open { box-shadow: inset 0 0 0 1.5px ${T.accent}66, 0 12px 26px -14px rgba(91,61,230,0.3); }
  .dfc-grid .dfc:last-child.open .dfc-b { background: ${T.accentSoft}; }
  ...
  /* SAYT-QATORI (s0 · s4): brauzer-qatori — saytning O'Z nomi turadi (96c) */
  .sitebar { display: flex; align-items: center; gap: 6px; background: ${T.bg}; border-radius: 10px; padding: 7px 12px; box-shadow: inset 0 0 0 1px ${T.line}; min-width: 0; }
  .sitebar-u { font-size: clamp(11.5px,1.4vw,13px); color: ${T.ink2}; min-width: 0; overflow-wrap: anywhere; }

  /* 🔴 IMZO-VIZUAL (s4): O'LCHAGICH-PANELI — uch o'lchagich + bir kunlik vaqt chizig'i */
  .gpanel { display: flex; flex-direction: column; gap: 9px; background: ${T.paper}; border-radius: 16px; padding: clamp(11px,1.8vw,16px); box-shadow: 0 16px 34px -16px rgba(${T.shadowBase},0.28), inset 0 0 0 1.5px ${T.line}; min-width: 0; }
  .grow { display: grid; grid-template-columns: repeat(3, minmax(0,1fr)); gap: clamp(8px,1.4vw,14px); }
  @media (max-width: 640px) { .grow { grid-template-columns: 1fr; } }
  .gauge { display: flex; flex-direction: column; gap: 3px; background: ${T.bg}; border-radius: 12px; padding: 10px 12px; box-shadow: inset 0 0 0 1.5px ${T.line}; transition: background 0.25s, box-shadow 0.25s; min-width: 0; }
  .gauge-h { display: flex; align-items: center; gap: 7px; font-family: 'Manrope'; font-weight: 800; font-size: clamp(11.5px,1.4vw,13px); color: ${T.ink2}; overflow-wrap: anywhere; min-width: 0; }
  /* Chiroq — uzluksiz olchov ko:rinishi: u dars davomida nafas oladi.
     Rang-semantikasi (pasport): yashil = odatdagidek · sariq = odatdan chetga chiqdi ·
     qizil = sayt ochilmadi (yagona haqiqiy nosozlik; oquvchi xatosi EMAS). */
  .gauge-led { font-style: normal; margin-left: auto; width: 9px; height: 9px; border-radius: 50%; flex-shrink: 0; background: ${T.success}; box-shadow: 0 0 0 3px rgba(18,169,104,0.16); animation: led-live 2.4s ease-in-out infinite; }
  @keyframes led-live { 0%, 100% { opacity: 1; } 50% { opacity: 0.34; } }
  .gauge-v { font-size: clamp(16px,2.4vw,22px); line-height: 1.22; font-weight: 700; color: ${T.ink}; animation: fade-step 0.3s ease-out; overflow-wrap: anywhere; min-width: 0; }
  .gauge-d { font-family: 'Manrope'; font-weight: 600; font-size: 11.5px; color: ${T.ink3}; min-height: 16px; overflow-wrap: anywhere; min-width: 0; }
  /* Sariq — P0 amber-oilasi (#E8A13A/#FBEED6/#B77A16): chetga chiqish, nosozlik emas. */
  .gauge.warn { background: #FBEED6; box-shadow: inset 0 0 0 2px rgba(232,161,58,0.55); }
  .gauge.warn .gauge-v { color: #B77A16; }
  .gauge.warn .gauge-d { color: #B77A16; }
  .gauge.warn .gauge-led { background: #E8A13A; box-shadow: 0 0 0 3px rgba(232,161,58,0.22); }
  .gauge.bad { background: ${T.errSoft}; box-shadow: inset 0 0 0 2px ${T.err}66; }
  .gauge.bad .gauge-v { color: ${T.err}; }
  .gauge.bad .gauge-d { color: ${T.err}; }
  .gauge.bad .gauge-led { background: ${T.err}; box-shadow: 0 0 0 3px rgba(229,72,77,0.24); animation-duration: 0.9s; }
  ...
  .tl-mark i { font-style: normal; display: inline-flex; align-items: center; justify-content: center; width: 22px; height: 22px; border-radius: 50%; background: ${T.paper}; font-size: 12px; box-shadow: inset 0 0 0 1.5px ${T.line}, 0 4px 10px -5px rgba(${T.shadowBase},0.3); }
  ...
  .day-btn:hover:not(:disabled) { transform: translateY(-2px); }
  ...
  .sigc-bar b { margin-left: auto; color: ${T.ink3}; font-weight: 700; font-size: 11.5px; }
  /* Javob kutilayotganda panel-belgisi sekin uriladi: yangi xabar turibdi. */
  .sigc-ic { font-style: normal; display: inline-block; }
  .sigc.wait .sigc-ic { animation: sig-beat 1.8s ease-in-out infinite; }
  @keyframes sig-beat { 0%, 100% { transform: scale(1); } 45% { transform: scale(1.18); } }
  /* Xabar tepadan tushib joyiga o'tiradi — paneldan kelishining o'zi. */
  .sigc-body { background: ${T.bg}; border-radius: 12px; padding: 11px 13px; animation: sig-in 0.42s cubic-bezier(.3,1.3,.45,1); min-width: 0; }
  @keyframes sig-in { 0% { opacity: 0; transform: translateY(-10px); box-shadow: inset 0 0 0 2px rgba(91,61,230,0.35); } 60% { opacity: 1; box-shadow: inset 0 0 0 2px rgba(91,61,230,0.2); } 100% { opacity: 1; transform: translateY(0); box-shadow: inset 0 0 0 0 rgba(91,61,230,0); } }
  .sigc-t { font-family: 'Manrope'; font-weight: 700; font-size: clamp(13px,1.6vw,15px); line-height: 1.45; color: ${T.ink}; overflow-wrap: anywhere; min-width: 0; }
  .yolbtns { display: grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap: 9px; }
  @media (max-width: 520px) { .yolbtns { grid-template-columns: 1fr; } }
  .yolb { display: flex; align-items: center; gap: 9px; text-align: left; font-family: 'Manrope'; font-weight: 800; font-size: clamp(12.5px,1.5vw,14px); color: ${T.ink}; background: ${T.bg}; border: none; border-radius: 12px; padding: 11px 14px; cursor: pointer; box-shadow: inset 0 0 0 1.5px ${T.line}; transition: box-shadow 0.16s, transform 0.14s, background 0.16s; min-width: 0; }
  .yolb:hover:not(:disabled) { transform: translateY(-2px); }
  .yolb:disabled { cursor: default; }
  /* Ikki yo'l — ikki xarakter: 📣 hozir (indigo, shoshilinch) · 📒 keyin (ko'k, xotirjam).
     DINAMIK OILA (o'lik-CSS auditiga eslatma): klass yolb-<kalit> shaklida yig'iladi,
     kalitlar YOLLAR dagi 'hozir' va 'jurnal' — grep bilan to'g'ridan-to'g'ri topilmaydi. */
  .yolb-hozir:hover:not(:disabled) { box-shadow: inset 0 0 0 1.5px ${T.accent}; }
  .yolb-jurnal:hover:not(:disabled) { box-shadow: inset 0 0 0 1.5px ${T.blue}; }
  .yolb.on { background: ${T.accentSoft}; box-shadow: inset 0 0 0 2px ${T.accent}; }
  .yolb-jurnal.on { background: ${T.blueSoft}; box-shadow: inset 0 0 0 2px ${T.blue}; }
  .yolb.ans { background: ${T.successSoft}; box-shadow: inset 0 0 0 2px ${T.success}; animation: yol-land 0.42s cubic-bezier(.34,1.5,.4,1); }
  @keyframes yol-land { 0% { transform: scale(0.97); } 48% { transform: scale(1.03); } 100% { transform: scale(1); } }
  .yolb-ic { font-size: 18px; flex-shrink: 0; width: 30px; height: 30px; border-radius: 10px; display: inline-flex; align-items: center; justify-content: center; background: ${T.paper}; box-shadow: inset 0 0 0 1.5px ${T.line}; }
  ...
  @media (prefers-reduced-motion: reduce) {
    .term-c { animation: none; }
    .gauge-v, .fakt, .res-line, .sigc-body, .tl-mark.sig i, .gauge-led { animation: none; }
    .tl-fill { transition: none; }
    .day-btn:hover:not(:disabled), .thr-b:hover:not(:disabled), .yolb:hover:not(:disabled) { transform: none; }
    .sigc.wait .sigc-ic, .yolb.ans { animation: none; }
...
`;
function PmLesson18({ lang: langProp, onFinished, liveToken }) {
