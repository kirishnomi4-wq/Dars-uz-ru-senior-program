...
//  Manba:  src/4b-Modull/EdgeCasesTestLesson.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/4b-Modull/EdgeCasesTestLesson.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/4b-Modull/EdgeCasesTestLesson.jsx
...
var LESSON_META = { lessonId: "edge-cases-04b-02-v18", lessonTitle: { uz: "Edge cases va error path", ru: "Граничные случаи и обработка ошибок" } };
var HW_TOKENS = [
...
var SCREEN_META = [
  ...
  // guard funksiyasini yig'ish (DragDrop)
  ...
  // haqiqiy edge testlarni tanlash (PickLines)
  { id: "s16", type: "test", template: "custom", scored: true, scope: "final" },
  // etalon kartochkasi — boundary
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr2({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и число ✅/❌ пока скрыты — по кнопке «Открыть результат» всё появится сразу и у вас, и на экранах учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <p className="mstats-verdict-t">{tr2({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Правильно только у <b>{pct}%</b> — тема осталась классу непонятной. Перед продолжением стоит коротко повторить.</> })}</p>}
            {level === "maybe" && <p className="mstats-verdict-t">{tr2({ uz: <>🟡 <b>{pct}%</b> to'g'ri — yomon emas. Xohlasangiz, davom etishdan oldin qisqa takrorlab oling.</>, ru: <>🟡 <b>{pct}%</b> правильных — неплохо. При желании коротко повторите, прежде чем идти дальше.</> })}</p>}
            {level === "good" && <p className="mstats-verdict-t">{tr2({ uz: <>✅ <b>{pct}%</b> to'g'ri — sinf mavzuni o'zlashtirdi. Bemalol davom eting!</>, ru: <>✅ <b>{pct}%</b> правильных — класс освоил тему. Смело продолжайте!</> })}</p>}
            {level === "few" && <p className="mstats-verdict-t">{tr2({ uz: <>Javob berganlar kam ({answered} ta) — foiz bo'yicha xulosa chiqarish qiyin. O'zingiz baholang.</>, ru: <>Ответивших мало ({answered}) — делать выводы по процентам рано. Оцените сами.</> })}</p>}
          </div>;
  })()}
...
var AgentCard = ({ children, title = { uz: "💬 Agentni shunday yo'naltiring", ru: "💬 Направьте агента вот так" } }) => <div className="agent-card"><span className="agent-lbl">{tr2(title)}</span><p className="agent-msg">{children}</p></div>;
...
var PickLines = ({ fileName, scaffoldTop, scaffoldBottom, candidates, agent, instruction, onComplete, completedInit, onProgress, doneNote, onWrong }) => {
  ...
  return <Zoomable>
    <div className="split">
      ...
      <Col>
        <p className="flow-label">{instruction ? tr2(instruction) : tr2({ uz: "Testga tegishli qatorlarni tanlang", ru: "Выберите строки, относящиеся к тесту" })}</p>
        {
    /* bajarilgach tanlanmagan qatorlar ixcham (147 (e) 2-naqsh) — to'g'ri tanlanganlar va xulosa to'liq qoladi */
        ...
        {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{doneNote || tr2({ uz: "✓ Test tayyor — chaqirdik va natijani tekshirdik.", ru: "✓ Тест готов — вызвали и проверили результат." })}</p></div>}
      </Col>
    </div>
    </Zoomable>;
};
var OrderPlain = ({ minH }) => <CodeFile name="order.ts" minH={minH || 90}>
    <Jx>export function</Jx>{" orderTotal(price, quantity) {"}{"\n"}
    {"  "}<Jx>return</Jx>{" price * quantity;"}{"  "}<Cm>{"// himoyasiz"}</Cm>{"\n"}
    {"}"}
  </CodeFile>;
var OrderGuarded = ({ minH }) => <CodeFile name="order.ts" minH={minH || 130}>
    <Jx>export function</Jx>{" orderTotal(price, quantity) {"}{"\n"}
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const [seen, setSeen] = useState3(storedAnswer ? /* @__PURE__ */ new Set([0, 1]) : /* @__PURE__ */ new Set());
  ...
  const firstUnseen = WEIRD.findIndex((_, i) => !seen.has(i));
  const OPTS = [
    ...
    { id: "c", label: { uz: "Farqi yo'q — haqiqiy mijoz baribir bunday qilmaydi", ru: "Без разницы — настоящий клиент так всё равно не сделает" } }
  ];
  const tap = (i) => {
    setActive(i);
    setSeen((prev) => {
        ...
        <Split>
          <Col>
            ...
            <div className="fade-up delay-1" style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {WEIRD.map((w, i) => <button key={i} className={`gchip ${i === firstUnseen ? "tap-hint" : ""}`} onClick={() => tap(i)} style={seen.has(i) ? { boxShadow: `inset 0 0 0 1.5px ${T.danger}`, color: T.danger } : void 0}>orderTotal{w.in}</button>)}
            </div>
            {active !== null && <div className="frame-warn fade-step" key={active}><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Natija:", ru: "Результат:" })} <b className="mono" style={{ color: T.danger }}>{WEIRD[active].out}</b> — {tr2(WEIRD[active].note)}</p></div>}
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Funksiya bunday kirishga qanday javob berishi kerak?", ru: "Как функция должна ответить на такой ввод?" })}</p>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const Preview = <Col>
      <p className="flow-label">{tr2({ uz: "Dars oxirida — exceptionni ham sinaysiz", ru: "К концу урока — протестируете и exception" })}</p>
      <JestRun status="pass" testName="0 ta buyurtmada xato beradi" />
      <div className="sk-info"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Endi test faqat "to'g'ri ishlaydimi" emas, "<b>noto'g'rini rad etadimi</b>" ni ham tekshiradi.</>, ru: <>Теперь тест проверяет не только «работает ли правильно», но и «<b>отклоняет ли неверное</b>».</> })}</p></div>
    </Col>;
  const StepsB = <Col>
  ...
  return <Stage eyebrow={tr2({ uz: "Reja", ru: "План" })} screen={screen} mentorStatic scrollSignal={showSteps} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive label={{ uz: "Boshlaymiz →", ru: "Начинаем →" }} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Test faqat "to'g'ri ishlaydimi" ni tekshiradimi — yoki <span className="italic" style={{ color: T.accent }}>"noto'g'rini rad etadimi"</span> ham?</>, ru: <>Тест проверяет только «работает ли правильно» — или ещё и <span className="italic" style={{ color: T.accent }}>«отклоняет ли неверное»</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Yaxshi dasturchi ikkalasini ham sinaydi: oddiy kirish (happy path) <b style={{ color: T.ink }}>va</b> g'alati kirish (edge cases). Mana natija va 4 qadam.</>, ru: <>Хороший разработчик проверяет и то, и другое: обычный ввод (happy path) <b style={{ color: T.ink }}>и</b> странный ввод (edge cases). Вот результат и 4 шага.</> })}</Mentor>
        {!isNarrow ? <Zoomable><Split>{Preview}{StepsB}</Split></Zoomable> : !showSteps ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>{Preview}<button className="btn" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(true)}>{tr2({ uz: "4 qadamni ko'rish", ru: "Смотреть 4 шага" })}</button></div> : <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}><button className="btn-soft" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(false)}>{tr2({ uz: "↩ Natijani ko'rish", ru: "↩ Смотреть результат" })}</button>{StepsB}</div>}
      </div>
    </Stage>;
};
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Tushuncha · happy path", ru: "Понятие · happy path" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Happy path'ni ko'ring", ru: "Посмотрите happy path" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>"Happy path" — bu <span className="italic" style={{ color: T.accent }}>nima</span>?</>, ru: <>«Happy path» — это <span className="italic" style={{ color: T.accent }}>что</span>?</> })}</h2></div>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 «Bu yetarlimi?» tugmasini bosing — happy path testi natijasi o'ngda chiqadi.", ru: "💡 Нажмите «Этого достаточно?» — справа появится результат happy-path теста." })}</p>}
    ...
    setShow(true);
    setSc((n) => n + 1);
  }}>{show ? tr2({ uz: "✓ Ko'rdingiz", ru: "✓ Просмотрено" }) : tr2({ uz: "Bu yetarlimi?", ru: "Этого достаточно?" })}</button>
          </Col>
          <Col>
            <JestRun status={show ? "pass" : void 0} testName="2 kitob narxini hisoblaydi" />
            {!show && <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Tugmani bosing ←", ru: "Нажмите кнопку ←" })}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Yashil! Lekin bu faqat <b>oddiy</b> kirish. Haqiqiy foydalanuvchilar har doim ham "happy" emas — keyingisi: g'alati kirishlar.</>, ru: <>Зелёный! Но это только <b>обычный</b> ввод. Настоящие пользователи не всегда «happy» — дальше: странный ввод.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Tushuncha · edge", ru: "Понятие · edge" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `Chegaralarni sinang (${seen.size}/3)`, ru: `Проверьте границы (${seen.size}/3)` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Funksiya <span className="italic" style={{ color: T.accent }}>chegarada</span> qanday ishlaydi — 0, manfiy, eng kichik?</>, ru: <>Как функция ведёт себя <span className="italic" style={{ color: T.accent }}>на границе</span> — 0, отрицательное, наименьшее?</> })}</h2></div>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Qolgan chegara-qiymatni bosing — 0, manfiy va 1: har biri mashinaning javobini ko'rsatadi.", ru: "💡 Нажмите оставшееся граничное значение — 0, отрицательное и 1: каждое покажет ответ машины." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
        <Zoomable>
        <div className="split">
          <Col>
            <OrderPlain />
            <div className="fade-up delay-1" style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {CASES.map((c, i) => <button key={i} className={`gchip ${i === firstUnseen ? "tap-hint" : ""}`} onClick={() => tap(i)} style={seen.has(i) ? { boxShadow: `inset 0 0 0 1.5px ${c.bad ? T.danger : T.success}`, color: c.bad ? T.danger : T.success } : void 0}>orderTotal{c.in}</button>)}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "natija", ru: "результат" })}</p>
            {active === null ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Chegarani bosing ←", ru: "Нажмите на границу ←" })}</p></div> : <div className={CASES[active].bad ? "frame-warn fade-step" : "frame-success fade-step"} key={active}><p className="body mono" style={{ margin: "0 0 5px", color: CASES[active].bad ? T.danger : T.success, fontWeight: 700 }}>{CASES[active].in} → {CASES[active].out}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2(CASES[active].note)}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Mana muammo: <b>0</b> va <b>manfiy</b>da funksiya jim ravishda noto'g'ri ishlaydi. Lekin <b>1</b> to'g'ri. Demak chegara — 0 bilan 1 orasida.</>, ru: <>Вот и проблема: на <b>0</b> и <b>отрицательных</b> функция молча работает неверно. А <b>1</b> — правильно. Значит, граница проходит между 0 и 1.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Tushuncha · noto'g'ri ma'lumot", ru: "Понятие · неверные данные" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `Ikkalasini sinang (${seen.size}/2)`, ru: `Проверьте оба (${seen.size}/2)` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Mijoz raqam o'rniga <span className="italic" style={{ color: T.accent }}>"ikki"</span> deb yozsa nima bo'ladi?</>, ru: <>Что будет, если клиент вместо числа напишет <span className="italic" style={{ color: T.accent }}>«ikki»</span>?</> })}</h2></div>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Qolgan g'alati kirishni bosing — matn va null: mashina nima qaytarishini ko'ring.", ru: "💡 Нажмите оставшийся странный ввод — текст и null: посмотрите, что вернёт машина." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
        <Zoomable>
        <div className="split">
          <Col>
            <OrderPlain />
            <div className="fade-up delay-1" style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {CASES.map((c, i) => <button key={i} className={`gchip ${i === firstUnseen ? "tap-hint" : ""}`} onClick={() => tap(i)} style={seen.has(i) ? { boxShadow: `inset 0 0 0 1.5px ${T.danger}`, color: T.danger } : void 0}>orderTotal{c.in}</button>)}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "natija", ru: "результат" })}</p>
            {active === null ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Noto'g'ri kirishni bosing ←", ru: "Нажмите на неверный ввод ←" })}</p></div> : <div className="frame-warn fade-step" key={active}><p className="body mono" style={{ margin: "0 0 5px", color: T.danger, fontWeight: 700 }}>{CASES[active].in} → {CASES[active].out}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2(CASES[active].note)}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Demak funksiya noto'g'ri kirishni <b>o'zi to'xtatishi</b> kerak. Buni qanday qilamiz? — Keyingi qadam.</>, ru: <>Значит, функция должна <b>сама останавливать</b> неверный ввод. Как это сделать? — Следующий шаг.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Yechim · throw", ru: "Решение · throw" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Funksiyani mustahkamlang", ru: "Укрепите функцию" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Funksiya noto'g'ri kirishni qanday <span className="italic" style={{ color: T.accent }}>rad etadi</span>?</>, ru: <>Как функция <span className="italic" style={{ color: T.accent }}>отклоняет</span> неверный ввод?</> })}</h2></div>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 «🛡️ Guard (throw) qo'shish» tugmasini bosing — funksiya boshiga tekshiruv qo'shiladi va o'ngda ikki natija chiqadi.", ru: "💡 Нажмите «🛡️ Добавить guard (throw)» — в начало функции добавится проверка, а справа появятся два результата." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
        <Zoomable>
        <div className="split">
          <Col>
    ...
    setShow(true);
    setSc((n) => n + 1);
  }}>{show ? tr2({ uz: "✓ Mustahkamlandi", ru: "✓ Укреплено" }) : tr2({ uz: "🛡️ Guard (throw) qo'shish", ru: "🛡️ Добавить guard (throw)" })}</button>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "endi nima bo'ladi", ru: "что будет теперь" })}</p>
            {!show ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Tugmani bosing ←", ru: "Нажмите кнопку ←" })}</p></div> : <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                <div className="frame-warn" style={{ padding: 12 }}><p className="body mono" style={{ margin: 0, fontSize: 12, color: T.ink }}>{tr2({ uz: <>orderTotal(10000, 0) → <b style={{ color: T.danger }}>Error tashlaydi</b> ✋</>, ru: <>orderTotal(10000, 0) → <b style={{ color: T.danger }}>бросает Error</b> ✋</> })}</p></div>
              </div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Endi funksiya kuchli: to'g'ri kirishda hisoblaydi, noto'g'rida xato beradi. Lekin buni <b>qanday sinaymiz</b>? Xato tashlasa, test buzilmaydimi?</>, ru: <>Теперь функция сильная: правильный ввод считает, неверный отклоняет с ошибкой. Но <b>как это протестировать</b>? Если она бросит ошибку — не сломается ли сам тест?</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [choice, setChoice] = useState3(storedAnswer?.picked ?? null);
  const [sc, setSc] = useState3(0);
  const done = choice === "b";
  const { tip: _tip, rescue: _resc } = useStuckValve(done, choice === null ? 0 : choice === "b" ? 2 : 1);
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: "b" });
  }, [done]);
  const pick = (v) => {
    if (choice === "b") return;
    setChoice(v);
  ...
  return <Stage eyebrow="toThrow" screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "To'g'ri yozuvni tanlang", ru: "Выберите правильную запись" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Funksiya xato tashlasa — uni qanday <span className="italic" style={{ color: T.accent }}>sinaymiz</span>?</>, ru: <>Функция бросает ошибку — как её <span className="italic" style={{ color: T.accent }}>протестировать</span>?</> })}</h2></div>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Savol: qaysi variantda funksiya darrov chaqirilmaydi, balki () => ichiga o'ralgan? Noto'g'risini bossangiz sabab chiqadi.", ru: "💡 Вопрос: в каком варианте функция не вызывается сразу, а обёрнута в () =>? Нажмёте не тот — появится причина." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
        <Zoomable>
        <div className="split">
          <Col>
            <button className={`vcard ${choice === "a" ? "shake" : ""}`} onClick={() => pick("a")} disabled={done} style={{ flexDirection: "column", alignItems: "flex-start", gap: 6, boxShadow: choice === "a" ? `inset 0 0 0 1.5px ${T.danger}` : void 0 }}>
              <span className="vlbl">{choice === "a" ? "✗ " : ""}{tr2({ uz: "Variant A", ru: "Вариант A" })}</span>
              <span className="agent-msg">expect(orderTotal(10000, 0)).toThrow()</span>
            </button>
            {choice === "a" && <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Bu yerda <span className="mono">orderTotal(10000, 0)</span> <b>darrov</b> chaqiriladi va xato tashlaydi — expect ushlab ulgurmaydi, test qulaydi. <span className="mono">() =&gt;</span> kerak.</>, ru: <>Здесь <span className="mono">orderTotal(10000, 0)</span> вызывается <b>сразу</b> и бросает ошибку — expect не успевает её поймать, тест падает. Нужен <span className="mono">() =&gt;</span>.</> })}</p></div>}
          </Col>
          <Col>
            <button className="vcard" onClick={() => pick("b")} disabled={done} style={{ flexDirection: "column", alignItems: "flex-start", gap: 6, boxShadow: choice === "b" ? `inset 0 0 0 1.5px ${T.success}` : void 0 }}>
              <span className="vlbl">{done ? "✓ " : ""}{tr2({ uz: "Variant B", ru: "Вариант B" })}</span>
              <span className="agent-msg">expect(() =&gt; orderTotal(10000, 0)).toThrow()</span>
            </button>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>✓ To'g'ri! <span className="mono">() =&gt;</span> funksiyani "o'rab" beradi — Jest uni nazorat ostida chaqiradi va xato chiqqanini ko'rib, testni <b>PASS</b> qiladi.</>, ru: <>✓ Верно! <span className="mono">() =&gt;</span> «оборачивает» функцию — Jest вызывает её под контролем, видит ошибку и ставит тесту <b>PASS</b>.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen8 = (props) => <QuestionScreen
  ...
  question={tr2({ uz: <><p className="eyebrow" style={{ color: T.accent }}>To'g'ri javobni tanlang</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Exceptionni <span className="italic" style={{ color: T.accent }}>qanday</span> sinaymiz?</h2></>, ru: <><p className="eyebrow" style={{ color: T.accent }}>Выберите правильный ответ</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Как <span className="italic" style={{ color: T.accent }}>проверить</span> exception?</h2></> })}
  options={[
...
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [sawPartial, setSawPartial] = useState3(!!storedAnswer);
  ...
  const runTests = () => {
    setRun(has("throw") ? "real" : "nothrow");
    if (!has("throw")) setSawPartial(true);
    setSc((n) => n + 1);
  };
  const pool = DD_CHIPS.filter((c) => placed[c.id] === void 0);
  const navLabel = done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : !full ? { uz: `Guardni yig'ing (${Object.keys(placed).length}/3)`, ru: `Соберите guard (${Object.keys(placed).length}/3)` } : { uz: "Testni ishga tushiring", ru: "Запустите тест" };
  return <Stage eyebrow={tr2({ uz: "Mashq · guard yig'ish", ru: "Практика · собираем guard" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "order.ts — varaqa", ru: "order.ts — лист" })}</p>
    ...
    return <div
      ...
      style={{ marginLeft: 14 + s.i * 8, ...sel && !chip ? { cursor: "pointer", boxShadow: `inset 0 0 0 2px ${T.accent}` } : null }}
    >
  ...
  >
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Endi guard to'liq: shart → xato tashlash → to'g'ri hisob.", ru: "Теперь guard полный: условие → бросок ошибки → правильный расчёт." })}{sawPartial ? tr2({ uz: " Shartsiz throw ishlamasligini o'z ko'zingiz bilan ko'rdingiz.", ru: " Вы своими глазами увидели: без throw защита не работает." }) : ""}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [a, setA] = useState3(!!storedAnswer);
  const [b, setB] = useState3(!!storedAnswer);
  const [sc, setSc] = useState3(0);
  const done = a && b;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, (a ? 1 : 0) + (b ? 1 : 0));
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  return <Stage eyebrow={tr2({ uz: "Tushuncha · chegara chizig'i", ru: "Понятие · линия границы" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Ikki tomonni sinang", ru: "Проверьте обе стороны" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Chegara qayerda — <span className="italic" style={{ color: T.accent }}>1 to'g'ri, 0 noto'g'ri</span>. Ikkalasini sinaymiz.</>, ru: <>Где проходит граница — <span className="italic" style={{ color: T.accent }}>1 верно, 0 неверно</span>. Проверим обе стороны.</> })}</h2></div>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Ikkala tugmani bosing — «▶ 1 ni sinash» va «▶ 0 ni sinash»: chegaraning ikki tomoni ham yashil bo'lishi kerak.", ru: "💡 Нажмите обе кнопки — «▶ Проверить 1» и «▶ Проверить 0»: обе стороны границы должны стать зелёными." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
        <Zoomable>
        <div className="split">
          <Col>
            ...
            {
    /* 147 (e) 1-naqsh: xulosa bo'sh chap ustunda — o'ngdagi ikki natija-kartasi ostidan pastga tushmaydi */
  }
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: "Ikkalasi ham yashil: 1 to'g'ri ishladi, 0 xato berdi (va test buni kutgan edi). Chegara puxta sinaldi.", ru: "Оба зелёные: 1 отработал правильно, 0 дал ошибку (и тест этого ждал). Граница проверена надёжно." })}</p></div>}
          </Col>
          <Col>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button className="btn-soft" disabled={a} onClick={() => {
    setA(true);
    setSc((n) => n + 1);
  }}>{a ? "✓ 1 → PASS" : tr2({ uz: "▶ 1 ni sinash", ru: "▶ Проверить 1" })}</button>
              <button className="btn-soft" disabled={b} onClick={() => {
    setB(true);
    setSc((n) => n + 1);
  }}>{b ? tr2({ uz: "✓ 0 → xato tutildi", ru: "✓ 0 → ошибка поймана" }) : tr2({ uz: "▶ 0 ni sinash", ru: "▶ Проверить 0" })}</button>
            </div>
...
var Screen11 = (props) => <QuestionScreen
  ...
  question={tr2({ uz: <><p className="eyebrow" style={{ color: T.accent }}>To'g'ri javobni tanlang</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Qaysi holatlarni <span className="italic" style={{ color: T.accent }}>tekshiramiz</span>?</h2></>, ru: <><p className="eyebrow" style={{ color: T.accent }}>Выберите правильный ответ</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Какие случаи <span className="italic" style={{ color: T.accent }}>проверяем</span>?</h2></> })}
  options={[
...
var Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Xato yo'li · API", ru: "Путь ошибки · API" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Bog'lanishni ko'ring", ru: "Посмотрите связь" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Funksiyada <span className="mono" style={{ color: T.accent }}>throw</span> — API'da bu <span className="italic" style={{ color: T.accent }}>nimaga aylanadi</span>?</>, ru: <>В функции <span className="mono" style={{ color: T.accent }}>throw</span> — а <span className="italic" style={{ color: T.accent }}>во что это превращается</span> в API?</> })}</h2></div>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 «▶ POST /order { quantity: 0 }» tugmasini bosing — API javobi o'ngda chiqadi.", ru: "💡 Нажмите «▶ POST /order { quantity: 0 }» — справа появится ответ API." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
        <Zoomable>
        <div className="split">
          <Col>
    ...
    setShow(true);
    setSc((n) => n + 1);
  }}>{show ? tr2({ uz: "✓ Ko'rdingiz", ru: "✓ Просмотрено" }) : "▶ POST /order { quantity: 0 }"}</button>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "API javobi", ru: "ответ API" })}</p>
            {!show ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "So'rovni yuboring ←", ru: "Отправьте запрос ←" })}</p></div> : <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.danger }}>✗ 400 — Bad Request</p><p className="body mono" style={{ margin: 0, color: T.ink, fontSize: 12 }}>"quantity must not be less than 1"</p></div>}
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Natija · bug", ru: "Итог · bug" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Hikoyani ko'ring", ru: "Посмотрите историю" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Faqat <span className="italic" style={{ color: T.accent }}>edge-test</span> tutadigan xato — qanday qutqaradi?</>, ru: <>Ошибка, которую ловит только <span className="italic" style={{ color: T.accent }}>edge-тест</span>, — как он спасает?</> })}</h2></div>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Tugmani ketma-ket bosing — avval faqat happy-path test, keyin edge test qo'shiladi: ikki natijani solishtiring.", ru: "💡 Нажимайте кнопку по очереди — сначала только happy-path тест, потом добавится edge-тест: сравните два результата." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
        <Zoomable>
        <div className="split">
          <Col>
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={done} onClick={go}>{step === 0 ? tr2({ uz: "▶ Faqat happy-path test bilan", ru: "▶ Только с happy-path тестом" }) : step === 1 ? tr2({ uz: "🛡️ Edge testni qo'shish", ru: "🛡️ Добавить edge-тест" }) : tr2({ uz: "✓ Tutildi", ru: "✓ Поймана" })}</button>
            {step >= 1 && <div className={step >= 2 ? "frame-success fade-step" : "frame-warn fade-step"}>
              {step === 1 ? <p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Happy-path testlar <b style={{ color: T.success }}>yashil</b> — lekin manfiy buyurtma xatosi <b style={{ color: T.danger }}>sezilmay ishlab ketdi</b>. Pul yo'qotildi!</>, ru: <>Happy-path тесты <b style={{ color: T.success }}>зелёные</b> — но ошибка с отрицательным заказом <b style={{ color: T.danger }}>проскочила незамеченной</b>. Деньги потеряны!</> })}</p> : <p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Edge test (<span className="mono">toThrow</span>) qo'shildi — endi manfiy buyurtma <b>darhol qizil</b> bo'ladi. Xato mijozgacha yetib bormaydi.</>, ru: <>Edge-тест (<span className="mono">toThrow</span>) добавлен — теперь отрицательный заказ <b>сразу горит красным</b>. Ошибка не дойдёт до клиента.</> })}</p>}
            </div>}
          </Col>
          <Col>
            <p className="flow-label">npm test</p>
            {step === 0 && <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Tugmani bosing ←", ru: "Нажмите кнопку ←" })}</p></div>}
            ...
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Qizil FAIL — bu yaxshi xabar! Test xatoni <b>siz</b> ko'rar oldidan tutdi. Mana edge case'ning kuchi.</>, ru: <>Красный FAIL — это хорошая новость! Тест поймал ошибку раньше, чем её увидели <b>вы</b>. Вот она, сила edge case.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen14 = (props) => <QuestionScreen
  ...
  question={tr2({ uz: <><p className="eyebrow" style={{ color: T.accent }}>To'g'ri javobni tanlang</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Nega faqat <span className="italic" style={{ color: T.accent }}>happy-path</span> yetarli emas?</h2></>, ru: <><p className="eyebrow" style={{ color: T.accent }}>Выберите правильный ответ</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Почему одного <span className="italic" style={{ color: T.accent }}>happy-path</span> недостаточно?</h2></> })}
  options={[
...
var CARD_WHY = {
  ...
  3: { uz: "2 — ishlaydi, lekin eng kichik emas: undan kichigi ham xato tashlamaydi.", ru: "2 — работает, но это не наименьшее: есть значение меньше, которое тоже не бросает ошибку." },
...
var Screen16 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Yakuniy · chegara qiymati", ru: "Финал · граничное значение" })} screen={screen} scrollSignal={sc} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!done} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Chegara chizig'ining aynan qaysi tomonida funksiya <span className="italic" style={{ color: T.accent }}>ishlaydi</span>?</>, ru: <>По какую именно сторону границы функция <span className="italic" style={{ color: T.accent }}>работает</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>Guard shartini eslang: <span className="mono">typeof quantity !== 'number' || quantity &lt;= 0</span> — bu bo'lsa xato. Etalon kartochkasini to'ldiring: <span className="mono">expect(orderTotal(10000, ?)).toBe(10000)</span> — eng kichik <b style={{ color: T.ink }}>TO'G'RI</b> qiymat qaysi?</>, ru: <>Вспомните условие guard: <span className="mono">typeof quantity !== 'number' || quantity &lt;= 0</span> — если оно истинно, будет ошибка. Заполните эталонную карточку: <span className="mono">expect(orderTotal(10000, ?)).toBe(10000)</span> — какое наименьшее <b style={{ color: T.ink }}>ДОПУСТИМОЕ</b> значение?</> })}{oneShot ? tr2({ uz: " Jonli dars — bitta urinish.", ru: " Живой урок — одна попытка." }) : ""}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "order.spec.ts — etalon kartochkasi", ru: "order.spec.ts — эталонная карточка" })}</p>
    ...
    return <button key={i} className={cls} disabled={solved || isMentorLive} onClick={() => pick(i)}><span className="mono">{o}</span></button>;
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "burilish — guard eski testni buzadi", ru: "поворот — guard ломает старый тест" })}</p>
...
var Q_LABELS = { 4: "1 — Edge case", 8: "2 — toThrow", 11: "3 — Happy+edge", 14: { uz: "4 — Qamrov", ru: "4 — Покрытие" }, 16: { uz: "5 — Chegara", ru: "5 — Граница" } };
var QUIZ_MS = 15e3;
var QZ_BG_SHAPES = [
...
var QUIZ_BANK = [
  { q: { uz: "Edge case (chegara holati) nima?", ru: "Что такое edge case (граничный случай)?" }, opts: [
    ...
    { uz: "Faqat har doim xatosiz ishlaydigan, muammosiz oddiy kirish", ru: "Только простой ввод, который всегда работает без ошибок" }
  ], correct: 2 },
  { q: { uz: "orderTotal(10000, 0) himoyasiz funksiyada nima qaytaradi?", ru: "Что вернёт orderTotal(10000, 0) в функции без защиты?" }, opts: [
    ...
    { uz: "0 — bepul buyurtma, lekin xato sezilmay qoladi", ru: "0 — бесплатный заказ, но ошибка останется незамеченной" }
  ], correct: 3 },
  { q: { uz: `Mijoz raqam o'rniga matn ("ikki") yuborsa, himoyasiz funksiya nima qaytaradi?`, ru: "Клиент прислал вместо числа текст («ikki») — что вернёт функция без защиты?" }, opts: [
    ...
    { uz: "10000 — narx hech qanday o'zgarishsiz, o'sha holicha qaytariladi", ru: "10000 — цена вернётся как есть, без изменений" }
  ], correct: 1 },
  { q: { uz: "Guard (himoya) qanday yoziladi?", ru: "Как пишется guard (защита)?" }, opts: [
    ...
    { uz: "Guard umuman yozilmaydi — Jest buni o'zi avtomatik tekshirib qo'yadi", ru: "Guard вообще не пишут — Jest проверит это сам, автоматически" }
  ], correct: 0 },
  { q: { uz: "Funksiya xato tashlashini sinash uchun qaysi yozuv to'g'ri?", ru: "Какая запись правильно проверяет, что функция бросает ошибку?" }, opts: [
    ...
    { uz: "expect(() => orderTotal(10000, 0)).toThrow() — o'rab chaqiradi", ru: "expect(() => orderTotal(10000, 0)).toThrow() — вызов в обёртке" }
  ], correct: 3 },
  { q: { uz: "() => bilan o'ralmasa nima bo'ladi?", ru: "Что будет, если не обернуть в () =>?" }, opts: [
    ...
    { uz: "Jest ogohlantiradi, lekin testni baribir o'tkazib yuboradi", ru: "Jest выдаст предупреждение, но тест всё равно пропустит" }
  ], correct: 2 },
  { q: { uz: "Chegara (boundary) sinovida nimalar tekshiriladi?", ru: "Что проверяют в тесте границы (boundary)?" }, opts: [
    ...
    { uz: "Faqat matn ko'rinishidagi noto'g'ri kirishlar tekshiriladi", ru: "Только неверный ввод в виде текста" }
  ], correct: 0 },
  { q: { uz: "orderTotal funksiyasida throw ishlatilgan. Bu API'da nimaga mos keladi?", ru: "В orderTotal используется throw. Чему это соответствует в API?" }, opts: [
    ...
    { uz: "Hech nimaga mos kelmaydi — bu faqat unit-testga tegishli tushuncha", ru: "Ничему — это понятие относится только к юнит-тестам" }
  ], correct: 1 },
  { q: { uz: "Faqat happy-path testi yozilgan bo'lsa, qanday xavf bor?", ru: "Написаны только happy-path тесты — в чём риск?" }, opts: [
    ...
    { uz: "Xavf umuman yo'q — happy-path testi bu holatda ham yetarli", ru: "Риска нет — happy-path тестов и так вполне достаточно" }
  ], correct: 0 },
  { q: { uz: "AI yozgan testda faqat happy-path bo'lsa, tekshiruvchi nima qilishi kerak?", ru: "AI написал тесты только на happy-path — что должен сделать проверяющий?" }, opts: [
    ...
    { uz: "Faqat console.log qo'shib, natijani ko'z bilan kuzatish", ru: "Просто добавить console.log и следить за результатом глазами" }
  ], correct: 1 },
  { q: { uz: "expect(orderTotal(10000, -5)).toBe(-50000) — bu yozuv edge xatoni tutadimi?", ru: "expect(orderTotal(10000, -5)).toBe(-50000) — поймает ли такая запись edge-ошибку?" }, opts: [
    ...
    { uz: "Ha — toBe tekshiruvchisi (matcher) har doim edge testga tegishli", ru: "Да — проверка (matcher) toBe всегда относится к edge-тестам" },
    { uz: "Yo'q — bu hali himoyasiz funksiyani 'to'g'ri' deb hisoblaydi", ru: "Нет — она считает ещё незащищённую функцию «правильной»" },
    { uz: "Bu umuman test emas — oddiy hisoblash amali, boshqa hech narsa emas", ru: "Это вообще не тест — обычное вычисление, и только" }
  ], correct: 2 },
  { q: { uz: "Guard qo'shilgach, eski expect(orderTotal(10000, 0)).toBe(0) testi nima bo'ladi?", ru: "После добавления guard что станет со старым тестом expect(orderTotal(10000, 0)).toBe(0)?" }, opts: [
    ...
    { uz: "FAIL bo'ladi — endi funksiya 0 o'rniga xato tashlab qo'yadi", ru: "Станет FAIL — функция теперь бросает ошибку вместо 0" }
  ], correct: 3 }
];
...
function QuizArena({ live, onClose, startSolo }) {
  ...
  return <div className="qz-arena">
    ...
    return <div key={i} className={`qz-tile rv ${win ? "win" : "lose"} ${pickedThis ? "picked" : ""}`} style={{ background: QUIZ_COLORS[i] }}>
          ...
          {solo && <button className="qz-btn big" onClick={soloNext}>{lastQ ? tr2({ uz: "🏁 Natijani ko'rish", ru: "🏁 Смотреть результат" }) : tr2({ uz: "Keyingi →", ru: "Дальше →" })}</button>}
        </div>}
  ...
  return <Stage eyebrow={tr2({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        ...
        {!isLive ? <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 16, alignItems: "center" }}>
          ...
          </div> : !loaded2 ? <p className="mono small fade-up" style={{ color: T.ink2 }}>{tr2({ uz: "Natijalar yuklanmoqda…", ru: "Загружаем результаты…" })}</p> : board.length === 0 ? <div className="frame-wait fade-up"><p className="body" style={{ margin: 0 }}>{tr2({ uz: "Bu sessiyaga hali hech kim qo'shilmagan.", ru: "К этой сессии пока никто не подключился." })}</p></div> : <>
            <Confetti />
...
var ScreenEdgePractice = (props) => <ScreenLivePractice
  {...props}
...
var Screen17 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "Edge case va exception'larni o'rgandingiz", ru: "Вы освоили edge case и exception" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Endi kodingiz <span className="italic" style={{ color: T.accent }}>g'alati kirishlardan</span> ham himoyalangan.</>, ru: <>Теперь ваш код защищён и от <span className="italic" style={{ color: T.accent }}>странного ввода</span>.</> })}</h2>{
    /* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */
        ...
        <div className={`qz-cta cs-cta fade-up d2 ${studentLive ? "ready" : ""}`}>
          <CsWordmark stats={false} liveOn={studentLive} disabled={studentWait} onClick={studentWait ? void 0 : openArena} hint={studentWait ? { uz: "⏳ Mentorni kuting", ru: "⏳ Дождитесь ментора" } : void 0} />
        </div>
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>{tr2({ uz: "📝 Uyga vazifa", ru: "📝 Домашнее задание" })}</div><ul>{HOMEWORK.map((h, i) => <li key={i}><b>{tr2(h.b)}</b> <span className="t">{tr2(h.t)}</span></li>)}</ul><p className="hw-note">{tr2({ uz: "🎉 Modul 4b tugadi! Endi kodingizni o'zingiz ham, kompyuter ham tekshiradi — happy path va edge case bilan.", ru: "🎉 Модуль 4b завершён! Теперь ваш код проверяете и вы, и компьютер — с happy path и edge case." })}</p></div>}
...
function EdgeCasesTestLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  return <LangContext.Provider value={lang}>
      <style>{`
        ...
        /* F-0820-290: fayl-nomi YORLIG'I (bosilmaydi) — «tab» nomi tugma taassurotini berardi (4b-01 F-258 pretsedenti) */
...
export {
  EdgeCasesTestLesson as default
};
