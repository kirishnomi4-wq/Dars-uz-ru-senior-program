...
//  Manba:  src/1-Modull/DeployLesson.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/1-Modull/DeployLesson.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/1-Modull/DeployLesson.jsx
...
var LESSON_META = { lessonId: "netlify-deploy-v19", lessonTitle: { uz: "Netlify va deploy", ru: "Netlify и деплой" } };
var HOMEWORK = {
  type: "text",
...
var MiniSite = ({ name = "Aziza" }) => <div style={{ fontFamily: "Georgia, serif" }}>
    <h1 style={{ fontSize: "clamp(18px,2.6vw,24px)", margin: "0 0 6px", color: T.ink }}>{tr2({ uz: "Salom, men", ru: "Привет, я" })} {name}!</h1>
    <p style={{ margin: "0 0 12px", color: T.ink2, fontSize: "clamp(13px,1.7vw,15px)", lineHeight: 1.5 }}>{tr2({ uz: "Bu mening birinchi saytim. Web-dasturlashni endi o'rganyapman.", ru: "Это мой первый сайт. Я только начинаю изучать веб-разработку." })}</p>
    <span style={{ display: "inline-block", background: T.accent, color: "#fff", padding: "7px 15px", borderRadius: 8, fontFamily: "'Manrope', sans-serif", fontWeight: 700, fontSize: 13 }}>{tr2({ uz: "Obuna bo'lish", ru: "Подписаться" })}</span>
  </div>;
var ACHIEVEMENTS = {
...
var ACH_TRIGGERS = { s2: "hostpick", s6: "filefind", s12: "shipit" };
function AchCounter() {
...
var RECAPS = {
  ...
  9: {
    title: { uz: "Netlify — saytning internetdagi joyi", ru: "Netlify — место сайта в интернете" },
    cards: [
      {
        ic: "📁",
        h: { uz: "Netlify nima qiladi?", ru: "Что делает Netlify?" },
      ...
      {
        ic: "💻",
        h: { uz: "Kompyuteringiz va Netlify farqi", ru: "Разница между вашим компьютером и Netlify" },
  ...
  6: {
    title: { uz: "Sayt fayllari va bosh sahifa", ru: "Файлы сайта и главная страница" },
    cards: [
      {
        ic: "📄",
        h: { uz: "Bosh sahifa — index.html", ru: "Главная страница — index.html" },
      ...
      {
        ic: "🗂️",
        h: { uz: "Qaysi fayl nima qiladi", ru: "Какой файл за что отвечает" },
      ...
      {
        ic: "☝️",
        h: { uz: "Nom bitta harf ham farq qilmasin", ru: "Имя не должно отличаться ни на букву" },
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">🙈 {tr2({ uz: "Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "Кто что выбрал и число ✅/❌ скрыты — после «Открыть результат» всё откроется сразу и у вас, и на экранах учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlash tavsiya etiladi.</>, ru: <>⚠️ Только <b>{pct}%</b> верно — класс не понял эту тему. Перед тем как идти дальше, рекомендуем короткое повторение.</> })}</p>
            ...
            {level === "few" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>Javob berganlar kam ({answered} ta) — foiz bo'yicha xulosa chiqarish qiyin. O'zingiz baholang:</>, ru: <>Ответивших мало ({answered}) — делать вывод по проценту сложно. Оцените сами:</> })}</p>
      ...
      {reveal && struggling && <p className="mstats-warn">⚠️ {tr2({ uz: "Ko'pchilik xato qildi — bu mavzu tushunarsiz bo'lgan ko'rinadi. Qayta tushuntirish tavsiya etiladi.", ru: "Большинство ошиблось — похоже, тема осталась непонятной. Рекомендуем объяснить её ещё раз." })}</p>}
...
function MentorWorkStats({ live, screenIdx, taskLabel }) {
  ...
  return <div className="mstats fade-up">
      ...
      {doneN === 0 && <p className="mstats-wait">{tr2({ uz: "O'quvchilar yozib tugatishi bilan shu yerda ✓ belgisi chiqadi…", ru: "Как только ученик закончит писать, здесь появится ✓…" })}</p>}
    </div>;
}
...
var PROJ_KEY = "cc-deploy-project";
var SITE_URL_KEY = "cc-site-url";
var PITCH_KEY = "ccPitch3";
var PAGE_SLOTS = 5;
var MIN_PAGES = 4;
var projRead = () => {
  try {
    return JSON.parse(localStorage.getItem(PROJ_KEY) || "null") || null;
  } catch {
    return null;
  }
};
var projWrite = (patch) => {
  try {
    const cur = projRead() || {};
    localStorage.setItem(PROJ_KEY, JSON.stringify({ ...cur, ...patch }));
  } catch {
  }
};
var demoRead = () => {
...
var demoWrite = (patch) => {
  try {
    localStorage.setItem("ccDemoDay", JSON.stringify({ ...demoRead() || {}, ...patch, savedAt: Date.now() }));
  } catch {
  }
};
var siteUrlRead = () => {
...
var siteUrlWrite = (url) => {
  try {
    localStorage.setItem(SITE_URL_KEY, url);
  } catch {
  }
  try {
    let old = null;
    try {
      old = JSON.parse(localStorage.getItem(PITCH_KEY) || "null");
    } catch {
    }
...
var NetWin = ({ title = "app.netlify.com", minH = 158, children }) => <div className="bp-window">
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const [view, setView] = useState3("me");
  const OPTS = [
  ...
  return <Stage eyebrow={tr2({ uz: "Kirish", ru: "Введение" })} screen={screen} navContent={<NavNext optionalLive disabled={picked === null} label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>Saytingiz faqat kompyuteringizda turibdi. Do'stingizga <span className="italic" style={{ color: T.accent }}>qanday</span> ko'rsatasiz?</>, ru: <>Ваш сайт лежит только на вашем компьютере. <span className="italic" style={{ color: T.accent }}>Как</span> вы покажете его другу?</> })}</h1>
        <Mentor>{tr2({ uz: "Pastdagi uch javobdan bittasini tanlang.", ru: "Выберите один из трёх ответов ниже." })}</Mentor>
        <Split>
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8 }}>
              <button className={`chip ${view === "me" ? "chip-on" : ""}`} onClick={() => setView("me")}>💻 {tr2({ uz: "Mening kompyuterim", ru: "Мой компьютер" })}</button>
              <button className={`chip ${view === "friend" ? "chip-on" : ""}`} onClick={() => setView("friend")}>📱 {tr2({ uz: "Do'stimning telefoni", ru: "Телефон друга" })}</button>
            </div>
            <div className="demo-swap" key={view}>
              {view === "me" ? <Preview minH={158} title="localhost:5500"><MiniSite name="Aziza" /></Preview> : <Preview minH={158} title="???">
                  <div className="dl-shake" style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", gap: 8, padding: "14px 0" }}>
                    ...
                    <p style={{ fontFamily: "Georgia, serif", fontWeight: 700, color: T.ink, margin: 0, fontSize: "clamp(15px,2vw,18px)" }}>{tr2({ uz: "Bu sahifa ochilmadi", ru: "Страница не открылась" })}</p>
                    <p style={{ fontFamily: "'JetBrains Mono', monospace", color: T.ink3, margin: 0, fontSize: 12 }}>{tr2({ uz: "manzil topilmadi", ru: "адрес не найден" })}</p>
                  </div>
                </Preview>}
            </div>
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Sizningcha qaysi biri?", ru: "Как думаете, какой?" })}</p>
    ...
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null} onClick={() => pick(o.id)}>
            ...
            {picked !== null && <p className="hook-ack fade-step">{tr2({ uz: <>Javob — ikkinchisi. Sayt internetga chiqsa, bitta <b>havola</b> yetadi: do'stingiz uni telefonida ochadi. Bugun avval Demo Day saytingizni yasaysiz, keyin shunday havola olasiz.</>, ru: <>Ответ — второй. Когда сайт в интернете, хватает одной <b>ссылки</b>: друг откроет её на телефоне. Сегодня сначала сделаете сайт для Demo Day, потом получите такую ссылку.</> })}</p>}
          </Col>
        </Split>
      </div>
    </Stage>;
...
var Screen1 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const run = () => {
    ...
    const tick = (i) => {
      setStep(i);
      if (i < STEPS.length) timer.current = setTimeout(() => tick(i + 1), 700);
  ...
  return <Stage eyebrow="Netlify" screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Yo'lni ko'rsating", ru: "Покажите путь" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Sayt papkangiz internetda <span className="italic" style={{ color: T.accent }}>qayerda</span> yashaydi?</>, ru: <>Где в интернете <span className="italic" style={{ color: T.accent }}>живёт</span> папка вашего сайта?</> })}</h2></div>
...
var Screen2 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: "Netlify nima qiladi?", ru: "Что делает Netlify?" })}</h2></>}
  options={[
...
var ScreenReja = ({ screen, onNext, onPrev }) => {
  const _gate = useContext2(LiveGateCtx) || {};
  ...
  return <Stage eyebrow={tr2({ uz: "Reja", ru: "План" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(14px,2.4vw,22px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bugun <span className="italic" style={{ color: T.accent }}>nima</span> qilamiz?</>, ru: <><span className="italic" style={{ color: T.accent }}>Что</span> сегодня делаем?</> })}</h2></div>
        <Mentor>{tr2({ uz: "Dars oxirida saytingiz internetda turadi — havolasini istagan odamga yubora olasiz.", ru: "К концу урока ваш сайт будет в интернете — ссылку можно отправить кому угодно." })}</Mentor>
        <div className="rj-row">
          {PLAN.map((p, i) => <React3.Fragment key={i}>
              <div className="rj-card fade-up" style={{ animationDelay: `${0.12 + i * 0.12}s` }}>
                ...
                <span className="rj-h">{tr2(p.h)}</span>
                <span className="rj-s">{tr2(p.s)}</span>
              </div>
              {i < PLAN.length - 1 && <span className="rj-arrow fade-up" style={{ animationDelay: `${0.2 + i * 0.12}s` }} aria-hidden="true">→</span>}
            </React3.Fragment>)}
        </div>
        {isMentorLive && <div className="frame-soft fade-up delay-3">
            <p className="body" style={{ margin: 0, color: T.ink }}>🧑‍🏫 {tr2({ uz: <><b>Faqat mentorga:</b> darsning 1 soati — Demo Day sayti (1-qadam), qolgan 30 daqiqa — Netlify va deploy (2–3-qadam).</>, ru: <><b>Только для ментора:</b> 1 час урока — сайт для Demo Day (шаг 1), оставшиеся 30 минут — Netlify и деплой (шаги 2–3).</> })}</p>
          </div>}
      </div>
    </Stage>;
};
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [muammo, setMuammo] = useState3(demo0.muammo || "");
  const [yechim, setYechim] = useState3(demo0.yechim || "");
  const [stage, setStage] = useState3(hadIdea ? "pages" : "idea");
  ...
  useEffect4(() => {
    if (done && !(storedAnswer && storedAnswer.correct)) onAnswer(screen, { correct: true, picked: filled.length, stage: "artefakt", screenIdx: screen });
  }, [done]);
  const navLabel = stage !== "pages" ? tr2({ uz: "G'oyangizni yozing", ru: "Впишите свою идею" }) : !done ? tr2({ uz: `Yana ${MIN_PAGES - filled.length} ta sahifa nomi`, ru: `Ещё названий страниц: ${MIN_PAGES - filled.length}` }) : tr2({ uz: "Davom etish", ru: "Продолжить" });
  return <Stage eyebrow={tr2({ uz: "Demo Day loyihasi", ru: "Проект Demo Day" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        {stage !== "pages" ? <>
            <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Demo Day saytingiz <span className="italic" style={{ color: T.accent }}>qanday muammoni</span> hal qiladi?</>, ru: <><span className="italic" style={{ color: T.accent }}>Какую проблему</span> решает ваш сайт для Demo Day?</> })}</h2></div>
            <Mentor>{tr2({ uz: "Ikki qatorni to'ldiring: qanday muammo bor va saytingiz uni qanday hal qiladi.", ru: "Заполните две строки: какая есть проблема и как ваш сайт её решает." })}</Mentor>
            <div className={stage === "leaving" ? "idea-out" : ""} style={{ display: "flex", justifyContent: "center" }}>
            <div className="idea-card fade-up delay-1">
              <p className="flow-label">{tr2({ uz: "Muammo", ru: "Проблема" })}</p>
              <label className={`wf ${muammo.trim().length >= 8 ? "filled" : ""} ${nextIdea === "m" ? "next" : ""}`}>
                ...
                <input className="wf-input" autoFocus value={muammo} maxLength={90} onChange={(e) => setMuammo(e.target.value)} placeholder={tr2({ uz: "masalan: to'garak jadvalini uydan ko'rib bo'lmaydi", ru: "например: расписание кружка не посмотреть из дома" })} />
              </label>
              <p className="flow-label">{tr2({ uz: "Yechim — saytingiz", ru: "Решение — ваш сайт" })}</p>
              <label className={`wf ${yechim.trim().length >= 4 ? "filled" : ""} ${nextIdea === "y" ? "next" : ""}`}>
                ...
                <input className="wf-input" value={yechim} maxLength={60} onChange={(e) => setYechim(e.target.value)} placeholder={tr2({ uz: "masalan: to'garak jadvali sayti", ru: "например: сайт с расписанием кружка" })} />
              </label>
              <button className="btn" style={{ alignSelf: "flex-end" }} disabled={!ideaOk || stage === "leaving"} onClick={confirmIdea}>{tr2({ uz: "Tayyor →", ru: "Готово →" })}</button>
            </div>
            </div>
          </> : <>
            <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Saytingiz <span className="italic" style={{ color: T.accent }}>qaysi sahifalardan</span> iborat?</>, ru: <>Из <span className="italic" style={{ color: T.accent }}>каких страниц</span> состоит ваш сайт?</> })}</h2></div>
            <Mentor>{tr2({ uz: "Sahifa nomlarini yozing — kamida to'rttasi. O'ngda saytingiz shu zahoti yig'ilib boradi.", ru: "Напишите названия страниц — минимум четыре. Справа ваш сайт сразу собирается." })}</Mentor>
            <Split>
              <Col>
                <div className="idea-chip fade-up">
                  <span style={{ fontSize: 18 }}>⭐</span>
                  <span className="small" style={{ flex: 1, color: T.ink, fontWeight: 700 }}>{yechim}</span>
                  <button className="pick-redo" onClick={() => setStage("idea")}>↻ {tr2({ uz: "o'zgartirish", ru: "изменить" })}</button>
                </div>
                <div className="pg-list fade-up delay-1">
                  {pages.map((v, i) => <div key={i} className={`pg-row ${v.trim().length > 1 ? "on" : ""}`}>
    ...
    placeholder={i === 0 ? tr2({ uz: "masalan: Bosh sahifa", ru: "например: Главная" }) : i < MIN_PAGES ? tr2({ uz: "Sahifa nomini yozing…", ru: "Напишите название страницы…" }) : tr2({ uz: "Sahifa nomi (ixtiyoriy)…", ru: "Название страницы (не обязательно)…" })}
  />
                      </label>
                    </div>)}
                </div>
                  ...
                  <Preview title="demo-day.netlify.app" minH={230}>
                    <div className="site-mock">
                      <div className="menu-mock">
                        {(filled.length ? filled : [tr2({ uz: "Bosh sahifa", ru: "Главная" }), "…"]).map((p, i) => <span key={i} className={`menu-tab ${i === 0 ? "on" : ""} site-tab-in`} style={{ animationDelay: `${i * 0.06}s` }}>{p.trim()}</span>)}
                      </div>
                      <div className="site-hero">
                        <h3 className="site-h">{yechim}</h3>
                        <p className="site-p">{muammo}</p>
                        <span className="site-btn">{tr2({ uz: "Batafsil", ru: "Подробнее" })}</span>
                      </div>
                    </div>
                  </Preview>
                </div>
...
var ScreenPages = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const saved = projRead() || {};
  const names = Array.isArray(saved.sahifalar) && saved.sahifalar.length ? saved.sahifalar : [tr2({ uz: "Bosh sahifa", ru: "Главная" })];
  const [desc, setDesc] = useState3(() => names.map((_, i) => Array.isArray(saved.mazmun) && saved.mazmun[i] || ""));
  ...
  const setD = (i, v) => {
    ...
    n[i] = v;
    setDesc(n);
    projWrite({ mazmun: n.map((x) => x.trim()) });
  };
  useEffect4(() => {
    if (done && !(storedAnswer && storedAnswer.correct)) onAnswer(screen, { correct: true, picked: okN, stage: "artefakt", screenIdx: screen });
  }, [done]);
  return <Stage eyebrow={tr2({ uz: "Demo Day loyihasi", ru: "Проект Demo Day" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Yozildi: ${okN}/${names.length}`, ru: `Заполнено: ${okN}/${names.length}` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Har sahifada <span className="italic" style={{ color: T.accent }}>nima</span> bo'ladi?</>, ru: <><span className="italic" style={{ color: T.accent }}>Что</span> будет на каждой странице?</> })}</h2></div>
        <Mentor>{tr2({ uz: "Har sahifaga bir qator yozing: unda nima turadi. Qancha aniq yozsangiz, AI shuncha mos sayt quradi.", ru: "Напишите по строке на каждую страницу: что на ней будет. Чем точнее напишете, тем точнее AI соберёт сайт." })}</Mentor>
        <Split>
          <Col>
            <div className="pg-list fade-up delay-1">
              {names.map((nm, i) => <div key={i} className={`pg-row ${desc[i].trim().length >= 5 ? "on" : ""}`}>
                  ...
                  <input className="text-input" value={desc[i]} maxLength={70} onChange={(e) => setD(i, e.target.value)} placeholder={tr2(PAGE_EX[i === 0 ? 0 : 1])} />
                </div>)}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "AI shu rejani oladi", ru: "AI получит этот план" })}</p>
...
var STYLES = [
  {
    id: "minimal",
    ic: "⬜",
    name: { uz: "Minimalist", ru: "Минимализм" },
    sub: { uz: "oq fon, tinch", ru: "белый фон, спокойный" },
  ...
  },
  {
    id: "yorqin",
    ic: "🎨",
    name: { uz: "Yorqin", ru: "Яркий" },
    sub: { uz: "quvnoq ranglar", ru: "весёлые цвета" },
  ...
  },
  {
    id: "qorongi",
    ic: "🌙",
    name: { uz: "Qorong'i", ru: "Тёмный" },
    sub: { uz: "to'q fon, oq matn", ru: "тёмный фон, белый текст" },
    desc: { uz: "to'q fon, oq matn, yashil urg'u rang. Shrift zamonaviy.", ru: "тёмный фон, белый текст, зелёный акцент. Шрифт современный." }
  },
  {
    id: "gradient",
    ic: "🌈",
    name: { uz: "Gradient", ru: "Градиент" },
    sub: { uz: "rang o'tishlari", ru: "переливы цвета" },
...
var buildPrompt = (proj, style) => {
  ...
  return [
    tr2({ uz: "Men Demo Day uchun veb-sayt qurmoqchiman. Menga uning to'liq kodini yozib ber.", ru: "Я хочу сделать веб-сайт для Demo Day. Напиши мне его полный код." }),
    "",
    tr2({ uz: "MUAMMO: " + (demo.muammo || "—"), ru: "ПРОБЛЕМА: " + (demo.muammo || "—") }),
    tr2({ uz: "SAYTIM (YECHIM): " + (demo.yechim || "Mening saytim"), ru: "МОЙ САЙТ (РЕШЕНИЕ): " + (demo.yechim || "Мой сайт") }),
    "",
    tr2({ uz: "SAHIFALAR:", ru: "СТРАНИЦЫ:" }),
    ...pageLines,
    "",
    tr2({ uz: "SHARTLAR:", ru: "УСЛОВИЯ:" }),
    tr2({ uz: "1. Faqat HTML va CSS ishlat. JavaScript ISHLATMA.", ru: "1. Используй только HTML и CSS. JavaScript НЕ используй." }),
    tr2({ uz: "2. Har sahifa alohida fayl bo'lsin: bosh sahifa index.html, qolganlari o'z nomiga mos .html fayllar. Hammasi bitta style.css faylga ulansin.", ru: "2. Каждая страница — отдельный файл: главная index.html, остальные — .html по своим названиям. Все подключены к одному style.css." }),
    tr2({ uz: "3. Har sahifada bir xil menyu (header) tursin va sahifalar bir-biriga havola orqali ulansin.", ru: "3. На каждой странице одинаковое меню (header), страницы связаны ссылками между собой." }),
    tr2({ uz: "4. Semantik teglardan foydalan: header, main, footer.", ru: "4. Используй семантические теги: header, main, footer." }),
    tr2({ uz: "5. Sayt telefonda ham, kompyuterda ham chiroyli ko'rinsin.", ru: "5. Сайт должен красиво выглядеть и на телефоне, и на компьютере." }),
    tr2({ uz: "6. Animatsiya qo'sh: tugma va havolalarda hover-effekt, silliq o'tishlar, sahifa ochilganda matn va rasmlar yumshoq paydo bo'lsin.", ru: "6. Добавь анимацию: hover-эффект на кнопках и ссылках, плавные переходы, мягкое появление текста и картинок при открытии страницы." }),
    tr2({ uz: "7. Rasmlar o'rniga https://picsum.photos/600/400 havolalaridan foydalan.", ru: "7. Вместо картинок используй ссылки https://picsum.photos/600/400." }),
    tr2({ uz: "8. Uslub: " + style.desc.uz, ru: "8. Стиль: " + style.desc.ru }),
    tr2({ uz: "9. Matnlar muammo va yechimim haqida bo'lsin — umumiy namuna matn yozma.", ru: "9. Тексты должны быть про мою проблему и решение — не пиши общий шаблонный текст." }),
    tr2({ uz: "10. Har faylni alohida ko'rsat va tepasiga fayl nomini yoz.", ru: "10. Покажи каждый файл отдельно и напиши сверху его имя." })
  ].join("\n");
};
var Screen4 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const proj = projRead() || {};
  const [style, setStyle] = useState3(proj.uslub || null);
  ...
  useEffect4(() => {
    if (st && !(storedAnswer && storedAnswer.correct)) onAnswer(screen, { correct: true, picked: style, stage: "artefakt", screenIdx: screen });
  }, [style]);
  const copy = () => {
    try {
  ...
  return <Stage eyebrow={tr2({ uz: "Tayyor topshiriq", ru: "Готовое задание" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!st} label={st ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: "Uslubni tanlang", ru: "Выберите стиль" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Sayt kodini AI <span className="italic" style={{ color: T.accent }}>yozib beradi</span></>, ru: <>Код сайта <span className="italic" style={{ color: T.accent }}>напишет</span> AI</> })}</h2></div>
        <Mentor>{tr2({ uz: "Uslubni tanlang — topshiriq matni o'zgaradi. Keyin 📋 tugmasi bilan nusxalab oling.", ru: "Выберите стиль — текст задания изменится. Потом скопируйте его кнопкой 📋." })}</Mentor>
        <Split>
          <Col>
            <div className="prj-grid fade-up delay-1">
              {STYLES.map((s) => <button key={s.id} className={`prj-card ${style === s.id ? "on" : ""}`} onClick={() => choose(s.id)}>
                  <span className="prj-ic">{s.ic}</span>
                  <span className="prj-nm">{tr2(s.name)}</span>
                  <span className="prj-sub">{tr2(s.sub)}</span>
                </button>)}
            </div>
            <p className="small" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Har kim boshqa uslub tanlasa, sinfda bir xil sayt chiqmaydi.", ru: "Если каждый выберет свой стиль, одинаковых сайтов в классе не будет." })}</p>
            <div className="frame-soft"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Bu matn — <b>ish asbobi</b>, koding mashqi emas: uni qo'lda ko'chirmaysiz, nusxalab AI'ga berasiz.</>, ru: <>Этот текст — <b>рабочий инструмент</b>, а не упражнение по кодингу: его не переписывают руками, а копируют и отдают AI.</> })}</p></div>
          </Col>
          <Col>
            {st ? <div className="pr-panel fade-step" key={style}>
                <div className="pr-head">
                  <span className="pr-lbl">📝 {tr2({ uz: "AI uchun topshiriq", ru: "Задание для AI" })}</span>
              ...
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Chapdan uslubni tanlang — topshiriq shu yerda chiqadi", ru: "Выберите стиль слева — задание появится здесь" })}</p></div>}
          </Col>
        </Split>
      </div>
    </Stage>;
...
var Screen5 = (props) => {
  const STEPS = [
    ...
    },
    {
      ...
      res: () => <div className="term"><span className="term-row"><span className="term-out">mening-saytim/</span></span><span className="term-row"><span className="term-cmd">  {tr2({ uz: "(hozircha bo'sh)", ru: "(пока пусто)" })}</span></span></div>
    },
    {
      id: "files",
      nav: { uz: "fayllar", ru: "файлы" },
      ...
      res: () => <div className="term"><span className="term-row"><span className="term-out">mening-saytim/</span></span><span className="term-row"><span className="term-cmd">  index.html</span></span><span className="term-row"><span className="term-cmd">  style.css</span></span><span className="term-row"><span className="term-cmd">  … {tr2({ uz: "(har sahifa uchun .html)", ru: "(.html на каждую страницу)" })}</span></span></div>
    },
    {
      id: "open",
      nav: { uz: "brauzer", ru: "браузер" },
  ...
  return <DoScreen
    ...
    mentor={tr2({ uz: "Qadamlar birma-bir ochiladi. Har birini kompyuteringizda bajaring va «✓ Bajardim»ni bosing.", ru: "Шаги открываются по одному. Выполните каждый на своём компьютере и нажмите «✓ Сделал(а)»." })}
  />;
};
var Screen6 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: "Saytning bosh sahifasi fayli qanday nomlanadi?", ru: "Как называется файл главной страницы сайта?" })}</h2></>}
  options={[
...
var markBtn = (on, col) => ({ border: "none", borderRadius: 8, padding: "6px 12px", fontWeight: 700, fontSize: 14, cursor: "pointer", background: on ? col : T.paper, color: on ? "#fff" : T.ink2, boxShadow: on ? "none" : `inset 0 0 0 1.5px ${T.ink3}`, flexShrink: 0 });
var ScreenImprove = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [marks, setMarks] = useState3(() => storedAnswer && storedAnswer.marks || {});
  const [copied, setCopied] = useState3(false);
  ...
  const text = [
    tr2({ uz: "Sen yozgan saytni tekshirdim. Mana bularni tuzat:", ru: "Я проверил сайт, который ты написал. Исправь вот это:" }),
    ...problems.length ? problems.map((c, i) => `${i + 1}. ${tr2(c.fix)}`) : [tr2({ uz: "1. Hammasi ishlayapti — endi dizaynni jonliroq qil va har sahifaga bittadan chiroyli bo'lim qo'sh.", ru: "1. Всё работает — теперь сделай дизайн живее и добавь на каждую страницу по красивому блоку." })],
    "",
    tr2({ uz: "O'zgargan har faylni to'liq qayta yoz va tepasiga fayl nomini yoz.", ru: "Каждый изменённый файл напиши полностью заново и сверху укажи имя файла." })
  ].join("\n");
  const copy = () => {
    try {
      if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(text);
  ...
  return <Stage eyebrow={tr2({ uz: "Amaliyot · yaxshilash", ru: "Практика · улучшение" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `Tekshiring: ${rated}/${CHECKS.length}`, ru: `Проверьте: ${rated}/${CHECKS.length}` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Saytingizni tekshiring va <span className="italic" style={{ color: T.accent }}>yaxshilang</span></>, ru: <>Проверьте сайт и <span className="italic" style={{ color: T.accent }}>улучшите</span> его</> })}</h2></div>
        <Mentor>{tr2({ uz: "Saytni brauzerda ochib, har bandga ✓ yoki ✗ qo'ying. Keyin tayyor topshiriqni AI'ga bering va yangi kodni fayllaringizga almashtiring.", ru: "Откройте сайт в браузере и поставьте каждому пункту ✓ или ✗. Потом отдайте готовое задание AI и замените код в своих файлах на новый." })}</Mentor>
        <Split>
          <Col>
            <div className="pg-list fade-up delay-1">
              {CHECKS.map((c) => <div key={c.id} className={`pg-row ${marks[c.id] === "ok" ? "on" : ""}`}>
                  <span className="body" style={{ flex: 1, margin: 0, fontSize: 14, color: T.ink }}>{tr2(c.t)}</span>
                  <button style={markBtn(marks[c.id] === "ok", T.success)} onClick={() => setMark(c.id, "ok")} aria-label={tr2({ uz: "Ishlayapti", ru: "Работает" })}>✓</button>
                  <button style={markBtn(marks[c.id] === "bad", T.accent)} onClick={() => setMark(c.id, "bad")} aria-label={tr2({ uz: "Tuzatish kerak", ru: "Нужно исправить" })}>✗</button>
                </div>)}
            </div>
          </Col>
          <Col>
            {done ? <div className="pr-panel fade-step">
                <div className="pr-head">
                  <span className="pr-lbl">📝 {tr2({ uz: "AI uchun yaxshilash topshirig'i", ru: "Задание AI на улучшение" })}</span>
                  <button className={`pr-copy ${copied ? "ok" : ""}`} onClick={copy}>{copied ? tr2({ uz: "✓ Nusxalandi", ru: "✓ Скопировано" }) : tr2({ uz: "📋 Nusxa olish", ru: "📋 Копировать" })}</button>
                </div>
                <pre className="pr-body">{text}</pre>
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Har bandni belgilang — topshiriq shu yerda chiqadi", ru: "Отметьте каждый пункт — задание появится здесь" })}</p></div>}
          </Col>
        </Split>
      </div>
    </Stage>;
...
var Screen8 = (props) => {
  const STEPS = [
    {
      id: "open",
      nav: { uz: "netlify.com", ru: "netlify.com" },
    ...
    },
    {
      ...
      res: () => <NetWin title="app.netlify.com"><span className="nfy-live">✓ {tr2({ uz: "kirdingiz", ru: "вы вошли" })}</span><span className="nfy-row on">Projects</span><span className="nfy-row">{tr2({ uz: "hali sayt yo'q", ru: "сайтов пока нет" })}</span></NetWin>
    }
  ];
  return <DoScreen
    {...props}
    practice
    steps={STEPS}
    eyebrow={tr2({ uz: "Amaliyot · Netlify", ru: "Практика · Netlify" })}
    taskLabel={tr2({ uz: "Netlify'ga kirish", ru: "Вход в Netlify" })}
    title={tr2({ uz: <>Netlify'ga <span className="italic" style={{ color: T.accent }}>GitHub bilan</span> kiring</>, ru: <>Войдите в Netlify <span className="italic" style={{ color: T.accent }}>через GitHub</span></> })}
    mentor={tr2({ uz: "Yangi hisob ochmaysiz — GitHub hisobingiz bilan kirasiz.", ru: "Новый аккаунт заводить не нужно — вы войдёте своим GitHub." })}
  />;
};
var DROP_STEPS = [
  {
    id: "page",
    nav: { uz: "maydon", ru: "зона" },
    ...
    res: () => <NetWin title="app.netlify.com"><span className="nfy-row">Projects</span><span className="nfy-row on">📂 {tr2({ uz: "papkani shu yerga tashlang", ru: "бросьте папку сюда" })}</span></NetWin>
  },
  {
    id: "drag",
    nav: { uz: "sudrash", ru: "перетащить" },
    ...
    res: () => <NetWin title="app.netlify.com"><span className="nfy-row on">📂 mening-saytim</span><span className="nfy-row">⏳ {tr2({ uz: "yuklanmoqda…", ru: "загрузка…" })}</span></NetWin>
  },
  {
    id: "live",
    nav: { uz: "havola", ru: "ссылка" },
    ...
    res: () => <NetWin title="app.netlify.com"><span className="nfy-live">✓ {tr2({ uz: "sayt internetda", ru: "сайт в интернете" })}</span><span className="nfy-link">https://…….netlify.app</span></NetWin>
  }
];
var ScreenDrop = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const _gate = useContext2(LiveGateCtx) || {};
  const isMentorLive = !!(_gate.live && _gate.live.mode === "mentor");
  const [view, setView] = useState3(0);
  const [url, setUrl] = useState3(() => siteUrlRead());
  ...
  const navLabel = allDone || isMentorLive ? tr2({ uz: "Davom etish", ru: "Продолжить" }) : tr2({ uz: `${waitIdx + 1}-qadam: ${tr2(DROP_STEPS[waitIdx].nav)}`, ru: `Шаг ${waitIdx + 1}: ${tr2(DROP_STEPS[waitIdx].nav)}` });
  const cur = DROP_STEPS[Math.min(view, DROP_STEPS.length - 1)];
  return <Stage eyebrow={tr2({ uz: "Amaliyot · Netlify", ru: "Практика · Netlify" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={isMentorLive ? false : !allDone} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Papkani tashlang — sayt <span className="italic" style={{ color: T.accent }}>internetga chiqadi</span></>, ru: <>Бросьте папку — сайт <span className="italic" style={{ color: T.accent }}>выйдет в интернет</span></> })}</h2></div>
        <Mentor>{tr2({ uz: <>Saytni internetga chiqarish <b style={{ color: T.ink }}>deploy</b> deyiladi. Netlify'da buning uchun sayt papkasini sudrab tashlash yetadi.</>, ru: <>Выложить сайт в интернет — это <b style={{ color: T.ink }}>деплой</b>. В Netlify для этого достаточно перетащить папку сайта.</> })}</Mentor>
        <Split>
          <Col>
            {allDone && !isMentorLive ? <>
                <p className="flow-label">{tr2({ uz: "Netlify bergan havolani shu yerga qo'ying", ru: "Вставьте сюда ссылку от Netlify" })}</p>
                ...
                <button className="btn" style={{ alignSelf: "flex-start" }} disabled={!looksOk} onClick={save}>{saved ? tr2({ uz: "✓ Saqlandi", ru: "✓ Сохранено" }) : tr2({ uz: "Saqlash", ru: "Сохранить" })}</button>
                <p className="small" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Bu havola Demo Day darsida kerak bo'ladi.", ru: "Эта ссылка понадобится на уроке Demo Day." })}</p>
              </> : <>
                <p className="flow-label">{tr2({ uz: "Ekraningizda shunday chiqadi", ru: "На вашем экране будет так" })}</p>
                <div className="demo-swap" key={cur.id}>{cur.res()}</div>
              </>}
          </Col>
          <Col>
            <DoSteps screen={screen} storedAnswer={storedAnswer} onAnswer={onAnswer} steps={DROP_STEPS} taskLabel={tr2({ uz: "Saytni Netlify'ga qo'yish", ru: "Размещение сайта на Netlify" })} practice onStep={setView} />
          </Col>
        </Split>
      </div>
    </Stage>;
...
var Screen12 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "Yakuniy savol", ru: "Финальный вопрос" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: "Do'stingiz saytingizni telefonida ochishi uchun unga nima yuborasiz?", ru: "Что вы отправите другу, чтобы он открыл ваш сайт на телефоне?" })}</h2></>}
  options={[
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Uyga vazifa", ru: "Домашнее задание" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr2({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Uyda: saytni tugating va <span className="italic" style={{ color: T.accent }}>havolani yuboring</span></>, ru: <>Дома: доделайте сайт и <span className="italic" style={{ color: T.accent }}>отправьте ссылку</span></> })}</h2></div>
        <Mentor>{tr2({ uz: "Sayt bugun bitmagan bo'lsa ham xavotir olmang: uyda tugatasiz. Oxirida platformaga bitta narsa yuborasiz — havolani.", ru: "Если сайт сегодня не закончен, ничего страшного: доделаете дома. В конце отправляете на платформу одно — ссылку." })}</Mentor>
        <Split>
          <div className="card hw fade-up delay-1">
            <div className="card-lbl" style={{ color: T.accent }}>📝 {tr2({ uz: "Uyga vazifa", ru: "Домашнее задание" })}</div>
            <ul>{TASKS.map((h, i) => <li key={i}><b>{tr2(h.b)}</b> <span className="t">{tr2(h.t)}</span></li>)}</ul>
            <p className="hw-note">{tr2({ uz: "Havolani platformadagi vazifa-savoliga yuborsangiz, vazifa bajarilgan hisoblanadi.", ru: "Задание считается выполненным, когда вы отправите ссылку в задание на платформе." })}</p>
          </div>
          <Col>
            <p className="flow-label">{tr2({ uz: "Yuboriladigan havola", ru: "Ссылка, которую нужно отправить" })}</p>
            <div className="frame">
              <p className="mono" style={{ margin: 0, color: url ? T.success : T.ink3, wordBreak: "break-all", fontSize: 14 }}>{url || tr2({ uz: "havola hali saqlanmagan", ru: "ссылка пока не сохранена" })}</p>
            </div>
            <div className="frame-soft"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Bu havola <b>Demo Day</b> repetitsiyasida ham kerak bo'ladi.</>, ru: <>Эта ссылка понадобится и на репетиции <b>Demo Day</b>.</> })}</p></div>
          </Col>
        </Split>
      </div>
    </Stage>;
};
var DEPLOY_FLASHCARDS = [
...
function Flashcards({ cards }) {
  ...
  return <div className="fc fade-up">
      <div className="fc-top"><span className="fc-pill learn" key={`l-${queue.length}-${swapRef.current}`}>↻ {tr2({ uz: "O'rganilmoqda", ru: "Изучаю" })} · <b>{queue.length}</b></span><span className="fc-pill knew" key={`k-${known}`}>✓ {tr2({ uz: "Bildim", ru: "Знаю" })} · <b>{known}</b></span></div>
...
var ScreenFlashcards = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Takrorlash", ru: "Повторение" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={false} label={tr2({ uz: "Yakunlash →", ru: "К финалу →" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr2({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>Saytingiz endi <span className="italic" style={{ color: T.accent }}>internetda</span>.</>, ru: <>Ваш сайт теперь <span className="italic" style={{ color: T.accent }}>в интернете</span>.</> })}</h2>{
  ...
  />
        ...
        {!isMentorL && hwOpen && <div className="card hw fade-up d4">
            <div className="card-lbl" style={{ color: T.accent }}>🔗 {tr2({ uz: "Sizning havolangiz", ru: "Ваша ссылка" })}</div>
            <p className="mono" style={{ margin: "0 0 10px", color: url ? T.success : T.ink3, wordBreak: "break-all", fontSize: 14 }}>{url || tr2({ uz: "havola hali saqlanmagan", ru: "ссылка пока не сохранена" })}</p>
            <p className="hw-note">{tr2({ uz: "Uyda saytni tugatib, havolani platformadagi vazifa-savoliga yozib yuboring.", ru: "Доделайте сайт дома и отправьте ссылку в задание на платформе." })}</p>
          </div>}
...
var Q_LABELS = { 6: { uz: "Bosh sahifa fayli", ru: "Файл главной страницы" }, 9: { uz: "Netlify vazifasi", ru: "Задача Netlify" }, 12: { uz: "Havola (yakuniy)", ru: "Ссылка (финал)" } };
var Confetti = () => {
...
var INLINE_KEYS = { s2: 0, s6: 2, s12: 3 };
...
function DeployLesson({ lang: langProp, onFinished, liveToken }) {
