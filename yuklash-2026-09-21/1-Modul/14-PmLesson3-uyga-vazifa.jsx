...
//  Manba:  src/1-Modull/PmLesson3.homework.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/1-Modull/PmLesson3.homework.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/1-Modull/PmLesson3.homework.jsx
...
var T = {
  ...
  ink2: "#565073",
  ink3: "#9C97B4",
  // F-0828-02: fon #F2F0FA → #F7F6FC (etalon bilan bir xil)
  paper: "#FFFFFF",
  accent: "#5B3DE6",
  accentSoft: "#EBE5FD",
  accentVivid: "#6E4BFF",
...
var lessonPitchRead = () => {
  try {
    ...
    const texts = {};
    BLOKS.forEach((b) => {
      if (typeof o[b.key] === "string" && o[b.key].trim()) texts[b.key] = o[b.key];
    });
...
var BLOKS = [
  {
    ...
    color: "#E08A2B",
    ic: Ico.hook(15),
    label: { uz: "Birinchi savol", ru: "Первый вопрос" },
  ...
  },
  {
    ...
    color: T.accent,
    ic: Ico.demo(15),
    label: { uz: "Jonli demo", ru: "Живое демо" },
  ...
  },
  {
    ...
    color: "#0E7C86",
    ic: Ico.film(15),
    label: { uz: "Qanday qildim", ru: "Как я это сделал" },
  ...
  },
  {
    ...
    color: T.success,
    ic: Ico.arrow(15),
    label: { uz: "Keyingi qadam", ru: "Следующий шаг" },
...
var BLOK_MIN = 8;
var blokOk = (s) => (s || "").trim().length >= BLOK_MIN;
var BLOK_START = BLOKS.reduce((acc, b, i) => {
  acc.push(i === 0 ? 0 : acc[i - 1] + BLOKS[i - 1].sec);
  return acc;
}, []);
var EARLY_END_SEC = 90;
var WHO = [
...
var ASK_MIN = 10;
var linkOk = (s) => /^(https?:\/\/)?[\w-]+(\.[\w-]+)+(\/\S*)?$/i.test((s || "").trim());
var QUIZ = [
  {
    id: "q1",
...
var PitchView = ({ texts, link, activeIdx, progress, doneIdx }) => <div className="pc">
    {BLOKS.map((b, i) => {
  const t = ((texts || {})[b.key] || "").trim();
  const here = activeIdx === i;
  const done = typeof doneIdx === "number" && i < doneIdx;
  return <div key={b.key} className={`pc-row ${here ? "here" : ""} ${done ? "done" : ""}`} style={{ "--c": T.accent }}>
...
var StageSpeech = ({ data, setData }) => {
  ...
  return <div className="col">
    ...
    return <div key={b.key} className="wrow" style={{ borderBottom: i < BLOKS.length - 1 ? `1px solid ${T.line}` : "none" }}>
...
var speechDone = (d) => BLOKS.every((b) => blokOk((d.texts || {})[b.key]));
var StageRehearse = ({ data, setData, texts, speechReady, goSpeech }) => {
  ...
  const start = () => {
    ...
    iv.current = setInterval(() => {
      const e = Math.min(PITCH_SEC, Math.round((Date.now() - startRef.current) / 1e3));
      setT(e);
      if (e >= PITCH_SEC) {
        clearInterval(iv.current);
        setPhase("ask");
  ...
  const answer = (key) => {
    setData({ ...data, runs: runs + 1, stuck: [...stuck, key] });
    setPhase("idle");
    setT(0);
  };
  if (!speechReady) return <div className="col gate">
  ...
  return <div className="col">
      ...
      <div className="rh fade-up d1">
        ...
        <PitchView texts={texts} activeIdx={activeIdx} progress={progress} doneIdx={phase === "run" ? activeIdx : void 0} />
        {phase === "ask" ? <div className="frame fade-step">
          ...
          </div> : <div className="rh-btns">
            {phase === "run" ? <>
                  <button type="button" className="btn" disabled={t < EARLY_END_SEC} title={t < EARLY_END_SEC ? tr({ uz: `${fmtSec(EARLY_END_SEC)} dan keyin bosiladi`, ru: `Нажимается после ${fmtSec(EARLY_END_SEC)}` }) : void 0} onClick={endEarly}>{tr({ uz: "Nutq tugadi ✓", ru: "Речь закончена ✓" })}</button>
        ...
        {phase === "idle" && runs > 0 && (lastBlok ? <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: <>«{tr(lastBlok.label)}» bo'lagini yana bir marta ayting — ikkinchi urinishda so'zlar o'zi keladi.</>, ru: <>Скажите часть «{tr(lastBlok.label)}» ещё раз — со второй попытки слова придут сами.</> })}</p></div> : <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: "Bir nafasda aytdingiz — Demo Day kuni ham shunday chiqadi.", ru: "Вы сказали на одном дыхании — на Демо-дне будет так же." })}</p></div>)}
      </div>
    </div>;
};
var rehearseDone = (d) => (d.runs || 0) >= 1;
var StageListener = ({ data, setData }) => {
  ...
  return <div className="col">
      ...
      <div className="frame fade-up d1">
        ...
        <div className="chips" role="radiogroup">
          {GOT.map((g, i) => <button key={i} type="button" role="radio" aria-checked={got === i} className={`chip ${got === i ? "on" : ""}`} onClick={() => setData({ ...data, got: i })}>
...
var listenerDone = (d) => typeof d.who === "number" && typeof d.got === "number" && (d.ask || "").trim().length >= ASK_MIN;
var QZ_HOLD_MS = 1500;
var QZ_OUT_MS = 380;
var StageLink = ({ data, setData }) => {
...
var linkDone = (d) => linkOk(d.link) && QUIZ.every((q) => (d.ans || {})[q.id] === q.correct);
var STAGES = [
  ...
  return <div className="col fin">
      ...
      <div className="fin-hero fade-up">
        ...
        <h2 className="title h-title" style={{ margin: "4px 0 2px" }}>
          {hasSpeech ? tr({ uz: <>Demo Day <span className="italic" style={{ color: T.accent }}>nutqingiz</span> tayyor!</>, ru: <>Ваша <span className="italic" style={{ color: T.accent }}>речь</span> к Демо-дню готова!</> }) : tr({ uz: "Uyga vazifa bajarildi!", ru: "Домашнее задание выполнено!" })}
        </h2>
      ...
      {hasSpeech && <div className="fin-site fade-up d1">
...
function PmLesson3Homework({ lang: langProp, onFinished }) {
  ...
  return <div className="hw-root">
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .inp.mono { font-family: 'JetBrains Mono', monospace; font-size: clamp(12.5px,1.4vw,14px); }
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult ? <StageResult data={data} goStage={setStage} onFinishClick={finish} finished={finished} onCelebrated={() => setDataRaw((prev) => prev.celebrated ? prev : { ...prev, celebrated: true })} /> : cur.key === "speech" ? <StageSpeech data={data.speech || {}} setData={setStageData("speech")} /> : cur.key === "rehearse" ? <StageRehearse data={data.rehearse || {}} setData={setStageData("rehearse")} texts={texts} speechReady={speechDone(data.speech || {})} goSpeech={() => setStage(0)} /> : cur.key === "listener" ? <StageListener data={data.listener || {}} setData={setStageData("listener")} /> : <StageLink data={data.link || {}} setData={setStageData("link")} />}
        </div>
      </div>
