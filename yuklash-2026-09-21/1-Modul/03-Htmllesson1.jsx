...
//  Manba:  src/1-Modull/Htmllesson1.jsx
//          src/compilator/HtmlCompiler.jsx
//  Qayta yig'ish:  node scripts/build-lms.mjs src/1-Modull/Htmllesson1.jsx
...
var LESSON_META = { lessonId: "html-01-v17", lessonTitle: { uz: "HTML asoslari", ru: "Основы HTML" } };
var HW_TOKENS = [
...
var CodeBox = ({ children }) => <pre className="code-box">{children}</pre>;
var Tg = ({ children }) => <span style={{ color: CODE.tag }}>{children}</span>;
var At = ({ children }) => <span style={{ color: CODE.attr }}>{children}</span>;
var Sr = ({ children }) => <span style={{ color: CODE.str }}>{children}</span>;
...
var Stage = ({ children, eyebrow, screen, totalScreens = TOTAL_SCREENS, navContent, narrow, mentorStatic }) => {
  ...
  const [mForced, setMForced] = useState4(false);
  ...
  return <MentorCtx.Provider value={{ enabled: collapseOn, collapsed: mCollapsed, setCollapsed, forced: mForced, setForced: setMForced }}>
      <div className="stage">
...
var RECAPS = {
  ...
  4: {
    title: { uz: "HTML va Brauzer", ru: "HTML и браузер" },
    cards: [
      {
        ic: "📄",
        h: { uz: "HTML — sahifaning tili", ru: "HTML — язык страницы" },
        body: {
      ...
      {
        ic: "🎨",
        h: { uz: "Brauzer — kodni o'qib, chizib beradi", ru: "Браузер читает код и рисует страницу" },
        body: {
      ...
      {
        ic: "☝️",
        h: { uz: "Adashtirmang!", ru: "Не путайте!" },
        body: {
  ...
  6: {
    title: { uz: "Skelet: head va body", ru: "Скелет: head и body" },
    cards: [
      {
        ic: "🦴",
        h: { uz: "Har sahifaning skeleti bor", ru: "У каждой страницы есть скелет" },
        body: {
      ...
      {
        ic: "🙈",
        h: { uz: "head — ko'rinmas qism", ru: "head — невидимая часть" },
        body: {
      ...
      {
        ic: "👁️",
        h: { uz: "body — ko'rinadigan qism", ru: "body — видимая часть" },
        body: {
  ...
  8: {
    title: { uz: "Teg ochiladi va yopiladi", ru: "Тег открывается и закрывается" },
    cards: [
      {
        ic: "🤲",
        h: { uz: "Teglar juft ishlaydi", ru: "Теги работают парой" },
        body: {
      ...
      {
        ic: "➗",
        h: { uz: "/ belgisi — yopish belgisi", ru: "Символ / — знак закрытия" },
        body: {
      ...
      {
        ic: "⚠️",
        h: { uz: "Yopilmasa nima bo'ladi?", ru: "Что будет, если не закрыть?" },
        body: {
  ...
  12: {
    title: { uz: "Ro'yxatlar: ul, ol, li", ru: "Списки: ul, ol, li" },
    cards: [
      {
        ic: "🔢",
        h: { uz: "ol — raqamli (tartibli) ro'yxat", ru: "ol — нумерованный (упорядоченный) список" },
        body: {
      ...
      {
        ic: "🔵",
        h: { uz: "ul — nuqtali (tartibsiz) ro'yxat", ru: "ul — маркированный (неупорядоченный) список" },
        body: {
      ...
      {
        ic: "🧩",
        h: { uz: "li — har bir band", ru: "li — каждый пункт" },
        body: {
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">🙈 {tr3({ uz: "Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "Кто что выбрал и число ✅/❌ скрыто — по кнопке «Открыть результат» всё появится одновременно и у вас, и на экранах учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr3({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlash tavsiya etiladi.</>, ru: <>⚠️ Только <b>{pct}%</b> верных — тема осталась непонятной классу. Перед продолжением рекомендуем короткое повторение.</> })}</p>
            ...
            {level === "few" && <>
              <p className="mstats-verdict-t">{tr3({ uz: <>Javob berganlar kam ({answered} ta) — foiz bo'yicha xulosa chiqarish qiyin. O'zingiz baholang:</>, ru: <>Ответивших мало ({answered}) — по проценту судить сложно. Оцените сами:</> })}</p>
      ...
      {reveal && struggling && <p className="mstats-warn">⚠️ {tr3({ uz: "Ko'pchilik xato qildi — bu mavzu tushunarsiz bo'lgan ko'rinadi. Qayta tushuntirish tavsiya etiladi.", ru: "Большинство ошиблись — похоже, тема осталась непонятной. Рекомендуем объяснить ещё раз." })}</p>}
...
function MentorWorkStats({ live, screenIdx, taskLabel }) {
  ...
  return <div className="mstats fade-up">
      <div className="mstats-head">
        <span className="mstats-lbl">✍️ {tr3(taskLabel)}</span>
      ...
      {doneN === 0 && <p className="mstats-wait">{tr3({ uz: "O'quvchilar yozib tugatishi bilan shu yerda ✓ belgisi chiqadi…", ru: "Как только ученики допишут, здесь появится значок ✓…" })}</p>}
    </div>;
}
function MentorPracticeOverlay({ entry, live, onClose }) {
  ...
  if (view === "demo") {
    return <div style={{ position: "fixed", inset: 0, zIndex: 2e3, background: T.bg }}>
        <HtmlCompiler_default lang={__lang2} task={entry.task} starterCode={tr3(entry.starter)} onContinue={() => setView("watch")} onBack={() => setView("watch")} />
      </div>;
  }
  ...
  return <div className="mp-overlay">
      <div className="mp-card">
        <div className="mp-eyebrow">✍️ {tr3({ uz: "Amaliyot · jonli", ru: "Практика · live" })}</div>
        ...
        <div className="mp-flow">
          ...
          <span className="mp-step">2 · {tr3({ uz: "Mentor doskada yozib ko'rsatadi", ru: "Ментор показывает решение на доске" })}</span>
        </div>
        {data.players === null ? <p className="mstats-wait">{tr3({ uz: "Ulanish…", ru: "Подключение…" })}</p> : <div className="mstats" style={{ marginTop: 2 }}>
            <div className="mstats-head">
              <span className="mstats-lbl">👨‍🎓 {tr3({ uz: "Praktikani tugatdi", ru: "Закончили практику" })}</span>
              <span className="mstats-n">{allIn ? tr3({ uz: "✓ Hamma tugatdi!", ru: "✓ Все закончили!" }) : <>{tr3({ uz: "Tugatdi:", ru: "Закончили:" })} <b>{doneN}</b> / {total}</>}</span>
            </div>
            <div className="mstats-prog"><span className={`mstats-prog-fill ${allIn ? "full" : ""}`} style={{ width: `${total ? Math.round(doneN / total * 100) : 0}%` }} /></div>
            ...
            {total === 0 && <p className="mstats-wait">{tr3({ uz: "Hali o'quvchi qo'shilmagan — ular praktikani boshlashi bilan bu yerda ✓ chiqadi…", ru: "Ученики ещё не подключились — как только они начнут практику, здесь появится ✓…" })}</p>}
          </div>}
        <div className="mp-actions">
          <button className="mp-demo" onClick={() => setView("demo")}>🖊 {tr3({ uz: "Doskada yozib ko'rsatish", ru: "Показать решение на доске" })}</button>
          <button className="mp-next" onClick={onClose}>{tr3({ uz: "Keyingi mavzuga", ru: "К следующей теме" })} →</button>
        </div>
        <p className="mp-tip">💡 {tr3({ uz: "Ko'pchilik tugatgach, aynan shu mashqni doskada birga yozing — shunda o'quvchilar o'zini tekshiradi va mavzu mustahkamlanadi.", ru: "Когда закончит большинство, напишите это же упражнение вместе на доске — так ученики проверят себя, а тема закрепится." })}</p>
      </div>
    </div>;
}
var QuestionScreen = ({ screen, scope, eyebrow, question, questionText, options, correctIdx, explainCorrect, explainWrong, audioText, audioOk, audioWrong, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const pick = (i) => {
    ...
    if (live && live.recordAttempt && !fpPractice) live.recordAttempt(screen, SCREEN_META[screen]?.id || `s${screen}`, i, Date.now() - mountTs.current, { question: questionText, options: optTexts, picked: optTexts[i], correct: optTexts[correctIdx], lang: typeof __lang2 !== "undefined" && __lang2 === "ru" ? "ru" : "uz" });
    if (audioText) {
...
var Mentor = ({ children }) => {
  ...
  const expand = (e) => {
    e.stopPropagation();
    if (ctx.forced && ctx.setForced) ctx.setForced(false);
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  const [view, setView] = useState4("site");
  const OPTS = [
  ...
  return <Stage eyebrow={tr3({ uz: "Kirish", ru: "Введение" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={tr3({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr3({ uz: <>Bu sayt nimadan <span className="italic" style={{ color: T.accent }}>yasalgan</span>?</>, ru: <>Из чего <span className="italic" style={{ color: T.accent }}>сделан</span> этот сайт?</> })}</h1>
        <Mentor>{tr3({ uz: <>Har kuni eMaktab, YouTube, Telegram'ni ochasiz, to'g'rimi? Ularning hammasi bitta narsadan yasalgan. <b style={{ color: T.ink }}>"&lt;/&gt; Kod"</b> tugmasini bosib, ichida nima borligini ko'ring.</>, ru: <>Каждый день вы открываете eMaktab, YouTube, Telegram, верно? Все они сделаны из одного и того же. Нажмите кнопку <b style={{ color: T.ink }}>"&lt;/&gt; Код"</b> и посмотрите, что внутри.</> })}</Mentor>
        <Split>
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8 }}>
              <button className={`chip ${view === "site" ? "chip-on" : ""}`} onClick={() => setView("site")}>🌐 {tr3({ uz: "Sayt", ru: "Сайт" })}</button>
              <button className={`chip ${view === "code" ? "chip-on" : ""}`} onClick={() => setView("code")}>{"</>"} {tr3({ uz: "Kod", ru: "Код" })}</button>
            </div>
            <div className="demo-swap" key={view}>
              {view === "site" ? <Preview minH={170} title="maktab.uz">
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingBottom: 10, borderBottom: `1px solid ${T.ink3}40`, marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}><span style={{ width: 22, height: 22, borderRadius: 6, background: T.accent, color: "#fff", display: "inline-flex", alignItems: "center", justifyContent: "center", fontFamily: "'Manrope', sans-serif", fontWeight: 800, fontSize: 13 }}>M</span><b style={{ fontFamily: "'Manrope', sans-serif", color: T.ink, fontSize: 15 }}>{tr3({ uz: "Maktab", ru: "Школа" })}</b></span>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 11, fontFamily: "'Manrope', sans-serif", fontSize: 12, color: T.ink2 }}><span>{tr3({ uz: "Asosiy", ru: "Главная" })}</span><span>{tr3({ uz: "Darslar", ru: "Уроки" })}</span><span style={{ background: T.ink, color: T.bg, padding: "5px 11px", borderRadius: 6, fontWeight: 600 }}>{tr3({ uz: "Kirish", ru: "Войти" })}</span></span>
                  </div>
                  <h1 style={{ fontFamily: "Georgia, serif", fontSize: "clamp(20px,3.2vw,28px)", margin: "0 0 6px", color: T.ink }}>{tr3({ uz: "Xush kelibsiz!", ru: "Добро пожаловать!" })} 👋</h1>
                  <p style={{ fontFamily: "Georgia, serif", margin: "0 0 12px", color: T.ink2, fontSize: "clamp(13px,1.8vw,15px)" }}>{tr3({ uz: "Bilim — bir bosishda.", ru: "Знания — в один клик." })}</p>
                  <span style={{ display: "inline-block", fontFamily: "'Manrope', sans-serif", fontWeight: 700, background: T.accent, color: "#fff", padding: "8px 16px", borderRadius: 8, fontSize: "clamp(12px,1.7vw,14px)" }}>{tr3({ uz: "Boshlash", ru: "Начать" })}</span>
                </Preview> : <>
                  <CodeBox><Tg>{"<header>"}</Tg>{"\n  "}{tr3({ uz: "Maktab · Asosiy · Darslar · Kirish", ru: "Школа · Главная · Уроки · Войти" })}{"\n"}<Tg>{"</header>"}</Tg>{"\n"}<Tg>{"<h1>"}</Tg>{tr3({ uz: "Xush kelibsiz!", ru: "Добро пожаловать!" })} 👋<Tg>{"</h1>"}</Tg>{"\n"}<Tg>{"<p>"}</Tg>{tr3({ uz: "Bilim — bir bosishda.", ru: "Знания — в один клик." })}<Tg>{"</p>"}</Tg>{"\n"}<Tg>{"<button>"}</Tg>{tr3({ uz: "Boshlash", ru: "Начать" })}<Tg>{"</button>"}</Tg></CodeBox>
                  <p className="mono small" style={{ color: T.ink3, marginTop: 6, textAlign: "center" }}>↑ {tr3({ uz: "shu saytning kodi — boshqa hech narsa emas!", ru: "это код того самого сайта — и ничего больше!" })}</p>
                </>}
            </div>
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr3({ uz: "Sizningcha, nima ishlatiladi?", ru: "Как вы думаете, что здесь используется?" })}</p>
    ...
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null} onClick={() => pick(o.id)}>
            ...
            {picked !== null && <p className="hook-ack fade-step">{tr3({ uz: "Yaxshi! Hozir hammasini birga ko'rib chiqamiz.", ru: "Отлично! Сейчас разберём всё вместе." })}</p>}
          </Col>
        </Split>
      </div>
    </Stage>;
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const PreviewBlock = <Col>
      <p className="flow-label">{tr3({ uz: "Natija — dars oxirida shunday bo'ladi", ru: "Результат — в конце урока будет так" })}</p>
      <Preview title="mening-saytim.html" minH={260}>
        <h1 style={{ fontFamily: G, fontSize: "clamp(20px,3vw,26px)", margin: "0 0 8px", color: T.ink }}>{tr3({ uz: "Salom, men Aziza!", ru: "Привет, я Азиза!" })}</h1>
        <p style={{ fontFamily: G, margin: "0 0 14px", color: T.ink2, fontSize: "clamp(13px,1.8vw,15px)", lineHeight: 1.5 }}>{tr3({ uz: "Web-dasturlashni endi o'rganyapman. Bu — mening birinchi saytim.", ru: "Я только начинаю изучать веб-разработку. Это мой первый сайт." })}</p>
        <p style={{ fontFamily: G, fontWeight: 700, margin: "0 0 6px", color: T.ink, fontSize: "clamp(14px,1.9vw,16px)" }}>{tr3({ uz: "Sevimli mashg'ulotlarim:", ru: "Мои любимые занятия:" })}</p>
        <ul style={{ fontFamily: G, color: T.ink, margin: "0 0 14px", paddingLeft: 22, fontSize: "clamp(13px,1.8vw,15px)", lineHeight: 1.6 }}><li>Minecraft</li><li>{tr3({ uz: "Futbol", ru: "Футбол" })}</li><li>{tr3({ uz: "Shaxmat", ru: "Шахматы" })}</li></ul>
        <a style={{ fontFamily: G, color: T.link, textDecoration: "underline", fontSize: "clamp(13px,1.8vw,15px)" }}>{tr3({ uz: "Mening Telegram kanalim", ru: "Мой Telegram-канал" })}</a>
      </Preview>
    </Col>;
  const StepsBlock = <Col>
      <p className="flow-label">{tr3({ uz: "7 qadam", ru: "7 шагов" })}</p>
  ...
  >
            ...
            <span className="step-body">
              <span className="step-text">{tr3(s.text)}</span>
              {s.tag && <span className="step-tag">{s.tag}</span>}
            </span>
          </li>)}
      </ol>
    </Col>;
  return <Stage eyebrow={tr3({ uz: "Reja", ru: "План" })} screen={screen} audioState={audio} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr3({ uz: "Boshlaymiz →", ru: "Начинаем →" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head">
          <h2 className="title h-title fade-up"><span className="italic" style={{ color: T.accent }}>{tr3({ uz: "Bugun haqiqiy sayt yasaymiz!", ru: "Сегодня сделаем настоящий сайт!" })}</span></h2>
        </div>
        <Mentor>{tr3({ uz: <>Ishonasizmi — dars oxirida <b style={{ color: T.ink }}>o'zingizning saytingiz</b> tayyor bo'ladi, xuddi mana shunaqa. Unga <b style={{ color: T.ink }}>7 ta qadamda</b> yetib boramiz.</>, ru: <>Поверите ли — к концу урока у вас будет готов <b style={{ color: T.ink }}>собственный сайт</b>, вот прямо такой. Дойдём до него за <b style={{ color: T.ink }}>7 шагов</b>.</> })}</Mentor>
        {!isNarrow ? <Split>{PreviewBlock}{StepsBlock}</Split> : !showSteps ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>
            {PreviewBlock}
            <button className="btn" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(true)}>📋 {tr3({ uz: "Bugungi 7 qadamni ko'rish", ru: "Посмотреть 7 шагов на сегодня" })}</button>
          </div> : <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>
            <button className="btn-soft" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(false)}>↩ {tr3({ uz: "Natijani ko'rish", ru: "Посмотреть результат" })}</button>
            {StepsBlock}
          </div>}
      </div>
    </Stage>;
...
var rgCellLeft = (i) => `${8 + i * (84 / (RG_N - 1))}%`;
var RG_CONFETTI = [
  { left: "14%", color: "#FF4D26", d: "0s" },
  { left: "26%", color: "#0FA968", d: ".12s" },
  { left: "38%", color: "#019ACB", d: ".05s" },
  { left: "50%", color: "#FF8A3D", d: ".18s" },
  { left: "62%", color: "#FF4D26", d: ".09s" },
  { left: "74%", color: "#0FA968", d: ".22s" },
  { left: "86%", color: "#FFD380", d: ".15s" },
  { left: "20%", color: "#019ACB", d: ".28s" }
];
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [revealed, setRevealed] = useState4(!!storedAnswer);
  const answerRef = useRef4(null);
  const xulosaRef = useRef4(null);
  const [solved, setSolved] = useState4(!!storedAnswer);
  const [program, setProgram] = useState4([]);
  const [pos, setPos] = useState4(RG_START);
  const [phase, setPhase] = useState4("idle");
  const [jumping, setJumping] = useState4(false);
  const [active, setActive] = useState4(-1);
  const botTimer = useRef4(null);
  const running = phase === "running";
  const isCactus = (c) => RG_CACTI.indexOf(c) !== -1;
  useEffect5(() => () => clearTimeout(botTimer.current), []);
  useEffect5(() => {
    if (phase === "win") {
      setSolved(true);
  ...
  const addCmd = (c) => {
    if (running) return;
    setProgram((p) => p.length >= RG_MAX ? p : [...p, c]);
    resetDino();
  };
  const clearProgram = () => {
    if (running) return;
  ...
  const reveal = () => {
    ...
    setTimeout(() => {
      if (answerRef.current) answerRef.current.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 120);
    if (!audio.muted) setTimeout(() => {
      const e = getAudioEngine();
      if (e && !audio.muted) e.pushOneOff(`Mana, dinozavrga buyruq tuzib ko'ring: u o'zicha o'ylamaydi, faqat siz aytgan tartibda yuradi. Kompyuter ham xuddi shunday — biz buyruq beramiz, u darhol bajaradi, to'g'ri-noto'g'riligini esa biz nazorat qilamiz.`);
    }, 300);
  };
  const RG_MSG = {
    running: { cls: "idle", text: tr3({ uz: "🦖 Dinozavr buyruqlaringizni bajaryapti…", ru: "🦖 Динозавр выполняет ваши команды…" }) },
    win: { cls: "ok", text: tr3({ uz: "🎉 Yetib keldi! Buyruqlaringiz to'g'ri ketma-ketlikda edi.", ru: "🎉 Добрался! Ваши команды были в правильном порядке." }) },
    "fail-cactus": { cls: "bad", text: tr3({ uz: "💥 Dinozavr kaktusga urildi! Undan «Sakra» bilan oshish kerak edi. U o'zi o'ylab sakramaydi — aniq buyurasiz.", ru: "💥 Динозавр врезался в кактус! Через него нужно было перепрыгнуть командой «Прыгай». Сам он не догадается — командуйте точно." }) },
    "fail-short": { cls: "bad", text: tr3({ uz: "🤏 Dinozavr 🍖 gacha yetmadi. Yana buyruq qo'shing va qaytadan urinib ko'ring.", ru: "🤏 Динозавр не добрался до 🍖. Добавьте ещё команду и попробуйте снова." }) }
  };
  const msg = RG_MSG[phase];
  return <Stage eyebrow={tr3({ uz: "Hayotdan misol", ru: "Пример из жизни" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!solved} label={solved ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : revealed ? tr3({ uz: "Dinozavrni 🍖 ga yetkazing", ru: "Доведите динозавра до 🍖" }) : tr3({ uz: "Avval o'ylab ko'ring", ru: "Сначала подумайте" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Kompyuter <span className="italic" style={{ color: T.accent }}>o'zicha</span> o'ylay oladimi?</>, ru: <>Умеет ли компьютер думать <span className="italic" style={{ color: T.accent }}>сам</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Kompyuter juda tez ishlaydi, lekin <b style={{ color: T.ink }}>o'zicha hech narsa qila olmaydi</b> — unga har bir ishni o'zimiz buyurishimiz kerak. Qanday qilib? Avval o'zingiz o'ylab ko'ring.</>, ru: <>Компьютер работает очень быстро, но <b style={{ color: T.ink }}>сам ничего сделать не может</b> — каждое действие ему приказываем мы. Как именно? Сначала подумайте сами.</> })}</Mentor>
        <MentorAutoClose when={phase === "win"} />
        {!revealed && <div className="frame frame-col savo fade-up delay-2">
            <p className="eyebrow" style={{ color: T.accent, margin: 0 }}>{tr3({ uz: "Savol", ru: "Вопрос" })}</p>
            <p className="body" style={{ margin: 0, color: T.ink, fontSize: "clamp(15px,1.9vw,18px)", lineHeight: 1.5 }}>{tr3({ uz: <>Kompyuter o'zi hech narsa o'ylab topmaydi. Unga biror ishni <b>qanday buyuramiz</b> — qanday qilib «mana shuni qil» deb tushuntiramiz?</>, ru: <>Компьютер сам ничего не придумывает. <b>Как отдать ему приказ</b> — как объяснить «сделай вот это»?</> })}</p>
            <button className="btn" onClick={reveal} style={{ alignSelf: "flex-start", marginTop: 4 }}>{tr3({ uz: "Dinozavrda sinab ko'ramiz", ru: "Проверим на динозавре" })} →</button>
          </div>}
        {revealed && <>
            <Zoomable>
    ...
    /* Sahna — dinozavr, kaktuslar, manzil */
              ...
              <div className="rg-scene">
                ...
                {phase === "win" && <div className="rg-confetti" aria-hidden="true">
    ...
    /* Buyruq tugmalari — dino ostida */
              ...
              <div className="rg-pad">
                <button className="rg-key rg-key-yur" onClick={() => addCmd("yur")} disabled={running || program.length >= RG_MAX}><span className="rg-key-ic">🐾</span>{tr3(RG_CMDS.yur.label)}</button>
                <button className="rg-key rg-key-jump" onClick={() => addCmd("sakra")} disabled={running || program.length >= RG_MAX}><span className="rg-key-ic">⤴️</span>{tr3(RG_CMDS.sakra.label)}</button>
                <span className="rg-legend">{tr3({ uz: <>🐾 Yur = <b>1 qadam</b> · ⤴️ Sakra = <b>2 qadam</b></>, ru: <>🐾 Иди = <b>1 шаг</b> · ⤴️ Прыгай = <b>2 шага</b></> })}</span>
              </div>
    ...
    /* DASTUR — kod tahririday ko'rinish */
              ...
              <div className="rg-code">
                ...
                <div className="rg-code-body">
                  {program.length === 0 ? <div className="rg-code-empty">{tr3({ uz: "// yuqoridagi tugmalardan buyruq qo'shing…", ru: "// добавьте команды кнопками выше…" })}</div> : program.map((c, i) => <div key={i} className={`rg-line ${active === i ? "now" : ""}`}>
                          <span className="rg-ln">{i + 1}</span>
                          <span className="rg-call"><span className="rg-fn">{c}</span><span className="rg-paren">()</span></span>
                          <span className="rg-cmt">{RG_CMDS[c].icon} {tr3(RG_CMDS[c].note)}</span>
                        </div>)}
                </div>
              </div>
    ...
    /* Boshqaruv — tozalash + ishga tushirish */
              ...
              <div className="rg-run-row">
                <button className="rg-mini" onClick={clearProgram} disabled={running || !program.length}>↺ {tr3({ uz: "Tozalash", ru: "Очистить" })}</button>
                <button className="btn" onClick={runProgram} disabled={running || !program.length} style={{ marginLeft: "auto" }}>{running ? tr3({ uz: "Bajaryapti…", ru: "Выполняет…" }) : tr3({ uz: "▶ Ishga tushirish", ru: "▶ Запустить" })}</button>
              </div>
              </div>
              </div>
...
var Screen3 = ({ screen, onNext, onPrev }) => {
  ...
  const [name, setName] = useState4("Aziz");
  ...
  const codeLines = [
    ...
    <><Tg>{"<p>"}</Tg>{tr3({ uz: "HTML o'rganyapman", ru: "Изучаю HTML" })}<Tg>{"</p>"}</Tg></>,
    <><Tg>{"<button>"}</Tg>{tr3({ uz: "Obuna bo'lish", ru: "Подписаться" })}<Tg>{"</button>"}</Tg></>
  ];
  const runBrowser = () => {
  ...
  return <Stage eyebrow="HTML" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr3({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.5vw,14px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Yozgan so'zingiz qanday <span className="italic" style={{ color: T.accent }}>saytga</span> aylanadi?</>, ru: <>Как написанное слово превращается в <span className="italic" style={{ color: T.accent }}>сайт</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Chrome yoki Safari'ni ochganingizda sayt chiroyli ko'rinadi. Lekin brauzer aslida <b style={{ color: T.ink }}>HTML</b> degan tilni o'qiydi, keyin uni sahifaga aylantiradi. Ismingizni yozing, so'ng <b style={{ color: T.ink }}>«Brauzerni ishga tushirish»</b> tugmasini bosing.</>, ru: <>Когда вы открываете Chrome или Safari, сайт выглядит красиво. Но на самом деле браузер читает язык <b style={{ color: T.ink }}>HTML</b>, а затем превращает его в страницу. Напишите своё имя и нажмите <b style={{ color: T.ink }}>«Запустить браузер»</b>.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col" style={{ gap: 8 }}>
            <div className="fade-up delay-1">
              <p className="eyebrow" style={{ color: T.ink2, margin: "0 0 8px" }}>{tr3({ uz: "Ismingizni yozing", ru: "Напишите своё имя" })}</p>
              <input className="text-input" value={name} onChange={(e) => setName(e.target.value)} maxLength={18} placeholder={tr3({ uz: "Ismingiz", ru: "Ваше имя" })} disabled={running} />
            </div>
            <p className="flow-label">{tr3({ uz: "HTML kod", ru: "HTML-код" })} {isNarrow ? "↓" : "→"} {tr3({ uz: "brauzer o'qiydi", ru: "браузер читает" })}</p>
            <div className="code-box br-code">
              {codeLines.map((ln, i) => <div key={i} className={`br-line ${active === i ? "now" : ""} ${step > i ? "read" : ""}`}>
          ...
          <div className="col" style={{ gap: 7 }}>
            <p className="flow-label">{tr3({ uz: "Brauzer oynasi", ru: "Окно браузера" })}</p>
            <Preview title="profil.html" minH={isNarrow ? 124 : 150}>
              {step === 0 && !running ? <div className="br-empty"><span style={{ fontSize: 24 }}>🌐</span><span>{tr3({ uz: <>Brauzer hali kodni o'qimadi.<br />Pastdagi tugmani bosing.</>, ru: <>Браузер ещё не прочитал код.<br />Нажмите кнопку ниже.</> })}</span></div> : <div className="profile-card">
                    {step >= 1 && <div className="br-el" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
                        <div className="pf-ava">{initial}</div>
                        <h1 className="pf-name">{name || "..."}</h1>
                      </div>}
                    {step >= 2 && <p className="pf-bio br-el">{tr3({ uz: "HTML o'rganyapman", ru: "Изучаю HTML" })}</p>}
                    {step >= 3 && <button className="pf-btn br-el">{tr3({ uz: "Obuna bo'lish", ru: "Подписаться" })}</button>}
                  </div>}
            </Preview>
            <div className="br-controls">
              {built && !running && <button className="rg-mini" onClick={reset}>↺ {tr3({ uz: "Tozalash", ru: "Очистить" })}</button>}
              <button className="btn" onClick={runBrowser} disabled={running} style={{ marginLeft: built && !running ? "auto" : 0 }}>
                {running ? tr3({ uz: "Brauzer o'qiyapti…", ru: "Браузер читает…" }) : built ? tr3({ uz: "↻ Qaytadan qurish", ru: "↻ Построить заново" }) : tr3({ uz: "▶ Brauzerni ishga tushirish", ru: "▶ Запустить браузер" })}
              </button>
            </div>
            {built && <p className="br-status">{tr3({ uz: <>✓ Brauzer kodni <b>yuqoridan pastga</b> o'qib, har bir qatorni sahifaga aylantirdi.</>, ru: <>✓ Браузер прочитал код <b>сверху вниз</b> и превратил каждую строку в часть страницы.</> })}</p>}
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen4 = (props) => <QuestionScreen
  ...
  idx={4}
  scope="module-mikro"
  eyebrow={tr3({ uz: "Mashq · 1-savol", ru: "Упражнение · вопрос 1" })}
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "Kim nima qiladi?", ru: "Кто что делает?" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr3({ uz: "HTML kodini o'qib, sahifani ekranda ko'rsatadigan dastur qaysi?", ru: "Какая программа читает HTML-код и показывает страницу на экране?" })}</h2></>}
  options={[{ uz: "Server", ru: "Сервер" }, "Photoshop", { uz: "Brauzer", ru: "Браузер" }, { uz: "Klaviatura", ru: "Клавиатура" }]}
  correctIdx={2}
  explainCorrect={tr3({ uz: "To'g'ri. Brauzer (Chrome, Safari, Firefox, Edge) HTML kodini o'qib, sahifa qilib ekraningizda ko'rsatadi.", ru: "Верно. Браузер (Chrome, Safari, Firefox, Edge) читает HTML-код и показывает его на экране в виде страницы." })}
  explainWrong={{ 0: tr3({ uz: "Server faqat HTML faylni saqlaydi va jo’natadi. Uni o’qib sahifaga aylantirish — brauzerning ishi.", ru: "Сервер лишь хранит и отправляет HTML-файл. Читать его и превращать в страницу — работа браузера." }), 1: tr3({ uz: "Photoshop — bu rasm muharriri, HTML’ga aloqasi yo’q.", ru: "Photoshop — это редактор изображений, к HTML он отношения не имеет." }), 3: tr3({ uz: "Klaviatura — bu siz yozadigan qurilma. HTML’ni o’qib ko’rsatadigan — brauzer.", ru: "Клавиатура — устройство, на котором вы печатаете. А читает и показывает HTML — браузер." }), default: tr3({ uz: "HTML kodini o’qib, sahifa qilib ko’rsatadigan — brauzer.", ru: "HTML-код читает и показывает как страницу именно браузер." }) }}
/>;
var SKELET_PIECES = [
  ...
  return <div className="dd fade-up">
      <div className="dd-slots">
        {slots.map((sid, i) => <div key={i} ref={(el) => slotRefs.current[i] = el} className={`dd-slot ${sid ? "filled" : ""} ${solved && sid ? "ok" : ""} ${wrong && sid && sid !== order[i] ? "bad" : ""}`}>
            ...
            {sid ? <button className="dd-chip in" onPointerDown={(e) => down(e, sid, i)}>{byId[sid].label}</button> : <span className="dd-hint">{hints ? tr3(hints[i]) : tr3({ uz: "bu yerga joylang", ru: "поместите сюда" })}</span>}
          </div>)}
      </div>
      <div className="dd-pool">
        {pool.length === 0 && !solved && <span className="dd-pool-empty">{tr3({ uz: "Tartib xato — bo'lakni bosib qaytaring va qayta joylang", ru: "Порядок неверный — нажмите на блок, верните его и расставьте заново" })}</span>}
        {pool.map((id) => <button key={id} className="dd-chip" onPointerDown={(e) => down(e, id, "pool")}>{byId[id].label}</button>)}
      </div>
      {solved && <div className="dd-done">✓ {tr3({ uz: "To'g'ri! Skelet aynan shu tartibda.", ru: "Верно! Скелет именно в таком порядке." })}</div>}
      {wrong && !solved && <div className="dd-wrong">⚠️ {tr3({ uz: "Tartib xato — qayta joylang.", ru: "Порядок неверный — расставьте заново." })}</div>}
    </div>;
}
function DebugChallenge({ lines, fixed, explain, onSolved, onWrong }) {
  const bugIdx = lines.findIndex((l) => l.bug);
  const [picked, setPicked] = useState4(-1);
  const [wrongIdx, setWrongIdx] = useState4(-1);
  ...
  return <div className="dbg fade-up">
      <div className="dbg-code">
        {lines.map((l, i) => <div key={i} className={`dbg-line ${solved && i === bugIdx ? "fixed" : ""} ${wrongIdx === i ? "wrong" : ""}`} onClick={() => click(i)}>
            ...
            {solved && i === bugIdx && <span className="dbg-badge">✓ {tr3({ uz: "tuzatildi", ru: "исправлено" })}</span>}
          </div>)}
      </div>
      {!solved ? <p className="dbg-hint">👆 {tr3({ uz: "Xato bor qatorni toping va bosing", ru: "Найдите строку с ошибкой и нажмите на неё" })}</p> : <div className="dbg-ok">✓ {tr3({ uz: "Topdingiz!", ru: "Нашли!" })} {tr3(explain)}</div>}
    </div>;
}
var HTML_FLASHCARDS = [
...
function Flashcards({ cards }) {
  ...
  if (!card) return <div className="fc-done fade-up"><span className="fc-done-emoji">🎉</span><p className="fc-done-h">{tr3({ uz: "Hammasini bilasiz!", ru: "Вы знаете всё!" })}</p><p className="fc-done-s">{total}/{total} {tr3({ uz: "karta yodlandi", ru: "карточек выучено" })}</p><button className="fc-btn ghost" onClick={restart}>↻ {tr3({ uz: "Qaytadan takrorlash", ru: "Повторить заново" })}</button></div>;
  return <div className="fc fade-up">
      <div className="fc-top"><span className="fc-pill learn" key={`l-${queue.length}-${swapRef.current}`}>↻ {tr3({ uz: "O'rganilmoqda", ru: "Учу" })} · <b>{queue.length}</b></span><span className="fc-pill knew" key={`k-${known}`}>✓ {tr3({ uz: "Bildim", ru: "Знаю" })} · <b>{known}</b></span></div>
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const PARTS = {
    doctype: { tag: "<!DOCTYPE html>", word: tr3({ uz: "E'lon", ru: "Объявление" }), role: tr3({ uz: 'Eng boshida turadi va "bu HTML5 sahifa" deb bildiradi.', ru: "Стоит в самом начале и объявляет: «это страница HTML5»." }) },
    html: { tag: "<html>", word: tr3({ uz: "Butun hujjat", ru: "Весь документ" }), role: tr3({ uz: "Butun sahifani o’rab turadi — head ham, body ham shuning ichida.", ru: "Оборачивает всю страницу — и head, и body находятся внутри него." }) },
    head: { tag: "<head>", word: tr3({ uz: "Ko’rinmaydigan qism", ru: "Невидимая часть" }), role: tr3({ uz: "Sahifa sozlamalari: title (nom), til, shriftlar. Xuddi restoran oshxonasi kabi — mehmon ko’rmaydi. title faqat brauzer tabchasida chiqadi, sahifa ichida ko’rinmaydi.", ru: "Настройки страницы: title (имя), язык, шрифты. Как кухня ресторана — гость её не видит. title появляется только на вкладке браузера, внутри страницы его не видно." }) },
    body: { tag: "<body>", word: tr3({ uz: "Ko’rinadigan qism", ru: "Видимая часть" }), role: tr3({ uz: "Ekranda ko’rinadigan hamma narsa shu yerda: sarlavha, matn, rasm, havola. Xuddi restoran zali kabi — mehmon shuni ko’radi.", ru: "Всё, что видно на экране, живёт здесь: заголовок, текст, картинка, ссылка. Как зал ресторана — именно его видит гость." }) }
  };
  const [active, setActive] = useState4(null);
  const [clicked, setClicked] = useState4(/* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={tr3({ uz: "Struktura", ru: "Структура" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : explored ? tr3({ uz: "Skeletni yig'ing", ru: "Соберите скелет" }) : tr3({ uz: `${clicked.size}/4 qismi ko'rilgan`, ru: `Просмотрено частей: ${clicked.size}/4` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Saytning <span className="italic" style={{ color: T.accent }}>ko'rinmaydigan</span> tomoni bormi?</>, ru: <>Есть ли у сайта <span className="italic" style={{ color: T.accent }}>невидимая</span> сторона?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Restoranni tasavvur qiling: <b style={{ color: T.ink }}>zal</b> — mehmonlar o'tiradigan joy, taom va bezaklar ko'rinadi; <b style={{ color: T.ink }}>oshxona</b> — mehmon kirmaydi, lekin taom o'sha yerda tayyorlanadi. Sahifa ham shunaqa: <b style={{ color: T.ink }}>body</b> — ko'rinadigan qism (matn, rasm, tugma), <b style={{ color: T.ink }}>head</b> — ko'rinmaydigan sozlamalar. Chizmadagi 4 ta qismni birma-bir bosib biling.</>, ru: <>Представьте ресторан: <b style={{ color: T.ink }}>зал</b> — здесь сидят гости, видны блюда и декор; <b style={{ color: T.ink }}>кухня</b> — гость туда не заходит, но еда готовится именно там. Страница устроена так же: <b style={{ color: T.ink }}>body</b> — видимая часть (текст, картинки, кнопки), <b style={{ color: T.ink }}>head</b> — невидимые настройки. Нажмите по очереди на 4 части схемы.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            {explored ? <div className="sk-buildbox">
                <p className="eyebrow" style={{ color: T.accent, margin: "0 0 2px" }}>🧲 {tr3({ uz: "Endi o'zingiz yig'ing", ru: "Теперь соберите сами" })}</p>
                <p className="body" style={{ margin: "0 0 10px", color: T.ink2, fontSize: 13.5 }}>{tr3({ uz: "Bo'laklarni to'g'ri tartibda joylang — sudrab yoki bosib.", ru: "Расставьте блоки в правильном порядке — перетаскивая или нажимая." })}</p>
                <DragDropOrder items={SKELET_PIECES} hints={[{ uz: "eng boshida", ru: "в самом начале" }, { uz: "butun sahifa qobig'i", ru: "оболочка всей страницы" }, { uz: "ko'rinmas qism", ru: "невидимая часть" }, { uz: "ko'rinadigan qism", ru: "видимая часть" }]} onSolved={() => setDragDone(true)} onWrong={() => {
    if (achMiss && !done) achMiss.miss(screen);
  }} />
              </div> : <>
            <p className="sk-tapguide fade-up delay-1">{tr3({ uz: <>👆 Chizmadagi <b>4 ta qismni birma-bir bosing</b> — bosilmaganlari yonib turadi</>, ru: <>👆 <b>Нажмите по очереди на 4 части</b> схемы — ненажатые подсвечиваются</> })} <span className="sk-tapcount">{clicked.size}/4</span></p>
            <div className="bskel fade-up delay-2">
              ...
              <div className={fc("html", "bskel-html")} onClick={() => tap("html")}>
                <span className="bskel-htmllabel">{tr3({ uz: "<html> — butun hujjat", ru: "<html> — весь документ" })}</span>
                <div className="bskel-win" onClick={(e) => e.stopPropagation()}>
                  <div className={fc("head", "bskel-tab")} onClick={() => tap("head")}><span className="bskel-dots"><i /><i /><i /></span><span className="bskel-tabpill">{tr3({ uz: "Mening sahifam", ru: "Моя страница" })}</span><span className="bskel-zone">&lt;head&gt;</span></div>
                  <div className={fc("body", "bskel-page")} onClick={() => tap("body")}><p className="bskel-ptitle">{tr3({ uz: "Salom!", ru: "Привет!" })} 👋</p><p className="bskel-ptext">{tr3({ uz: "Bu mening sahifam.", ru: "Это моя страница." })}</p><span className="bskel-zone bskel-zone-b">&lt;body&gt;</span></div>
                </div>
              </div>
            </div>
            </>}
          ...
          <div className="col" style={{ gap: 8 }}>
            <div className="zb-gap" style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
              <div className="flow-label">{tr3({ uz: "HTML kodi", ru: "HTML-код" })}</div>
              <span className="small mono" style={{ color: explored ? T.success : T.ink3 }}>{clicked.size} / 4 {tr3({ uz: "ko'rildi", ru: "просмотрено" })}</span>
            </div>
            <pre className="code-box fade-up delay-2">
              ...
              {"    "}<span className="t-title">&lt;title&gt;{tr3({ uz: "Mening sahifam", ru: "Моя страница" })}&lt;/title&gt;</span>{"\n"}
              {"  "}<span className={ck("head")} onClick={() => tap("head")}><span className="t-tag">&lt;/head&gt;</span></span>{"\n"}
              {"  "}<span className={ck("body")} onClick={() => tap("body")}><span className="t-tag">&lt;body&gt;</span></span>{"\n"}
              {"    "}<span className="t-cm">&lt;!-- {tr3({ uz: "kontent shu yerga", ru: "контент сюда" })} --&gt;</span>{"\n"}
              {"  "}<span className={ck("body")} onClick={() => tap("body")}><span className="t-tag">&lt;/body&gt;</span></span>{"\n"}
              <span className={ck("html")} onClick={() => tap("html")}><span className="t-tag">&lt;/html&gt;</span></span>
            </pre>
            {explored ? <div className="frame-success fade-step"><p className="small mono" style={{ margin: "0 0 4px", fontWeight: 600, color: T.success, textTransform: "uppercase", letterSpacing: "0.08em" }}>✓ {tr3({ uz: "Skeletni o'rgandingiz", ru: "Вы изучили скелет" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Har sahifa shu tartibda: <b>DOCTYPE → html → head (ko'rinmaydi) + body (ko'rinadi)</b>.</>, ru: <>Каждая страница в этом порядке: <b>DOCTYPE → html → head (невидим) + body (виден)</b>.</> })}</p></div> : active ? <div className="sk-info fade-step" key={active}><span className="sk-tagbig"><span className="sk-chip">{PARTS[active].tag}</span><span className="sk-wordbadge">{PARTS[active].word}</span></span><p className="body" style={{ color: T.ink, margin: "11px 0 0" }}>{PARTS[active].role}</p></div> : !isNarrow ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr3({ uz: "Chizma yoki koddan bir qismni bosing", ru: "Нажмите на часть схемы или кода" })}</p></div> : null}
          </div>
        </div>
        </Zoomable>
      </div>
...
var ScreenSkeletTest = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "Mustahkamlash", ru: "Закрепление" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr3({ uz: <>Sahifada <span className="italic" style={{ color: T.accent }}>ko'rinadigan</span> matn qaysi qismga yoziladi?</>, ru: <>В какую часть пишется <span className="italic" style={{ color: T.accent }}>видимый</span> текст страницы?</> })}</h2></>}
  options={["`<head>`", "`<title>`", "`<!DOCTYPE>`", "`<body>`"]}
  correctIdx={3}
  explainCorrect={tr3({ uz: "To'g'ri! `body` — sahifada ko'rinadigan hamma narsa (sarlavha, matn, rasm) shu yerda yoziladi.", ru: "Верно! `body` — всё видимое на странице (заголовок, текст, картинка) пишется здесь." })}
  explainWrong={{
    0: tr3({ uz: "`<head>` — bu ko’rinmaydigan qism: `title` va sozlamalar. Ko’rinadigan matn `body` ichida.", ru: "`<head>` — невидимая часть: `title` и настройки. Видимый текст — внутри `body`." }),
    1: tr3({ uz: "`<title>` — faqat brauzer tabchasidagi nom. To’g’risi — `body`.", ru: "`<title>` — лишь имя на вкладке браузера. Правильный ответ — `body`." }),
    2: tr3({ uz: '`<!DOCTYPE>` — bu "men HTML5 man" degan e’lon, matn joyi emas. To’g’risi — `body`.', ru: "`<!DOCTYPE>` — это объявление «я HTML5», а не место для текста. Правильный ответ — `body`." }),
    default: tr3({ uz: "Ko’rinadigan matn `body` ichiga yoziladi.", ru: "Видимый текст пишется внутрь `body`." })
  }}
/>;
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const PARTS = {
    open: { role: tr3({ uz: "Ochuvchi teg — element shu yerdan boshlanadi.", ru: "Открывающий тег — элемент начинается здесь." }) },
    content: { role: tr3({ uz: "Kontent — ekranda ko'rinadigan matn. Teglar uni yashirmaydi, balki nima ekanini aytadi.", ru: "Контент — видимый на экране текст. Теги его не прячут, а говорят, что это такое." }) },
    close: { role: tr3({ uz: "Yopuvchi teg — / belgili. Element shu yerda tugaydi.", ru: "Закрывающий тег — со знаком /. Элемент заканчивается здесь." }) }
  };
  const [wrapped, setWrapped] = useState4(false);
  const [active, setActive] = useState4(null);
  const [dbgDone, setDbgDone] = useState4(!!storedAnswer);
  const isNarrow = useIsMobile(768);
  const done = dbgDone;
  const achMiss = useContext2(AchMissCtx);
  const tap = (k) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Teg", ru: "Тег" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : wrapped ? tr3({ uz: "🐞 Xatoni toping", ru: "🐞 Найдите ошибку" }) : tr3({ uz: "Avval tegga o'rang", ru: "Сначала оберните в тег" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Brauzer matn qayerda <span className="italic" style={{ color: T.accent }}>tugashini</span> qanday biladi?</>, ru: <>Как браузер узнаёт, где текст <span className="italic" style={{ color: T.accent }}>заканчивается</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Brauzer aqlli emas — matn qayerda <b style={{ color: T.ink }}>boshlanib</b>, qayerda <b style={{ color: T.ink }}>tugashini</b> o'zi bilmaydi. Buni unga teglar aytadi: ochuvchi teg — boshlanishi, yopuvchi teg — tugashi. Xuddi yozuvdagi <b style={{ color: T.ink }}>qo'shtirnoq</b> kabi — ochasiz va albatta yopasiz. Tugmani bosib, <b style={{ color: T.ink }}>"Salom!"</b> so'ziga teg qo'shing.</>, ru: <>Браузер не умён — сам он не знает, где текст <b style={{ color: T.ink }}>начинается</b> и где <b style={{ color: T.ink }}>заканчивается</b>. Это ему говорят теги: открывающий — начало, закрывающий — конец. Совсем как <b style={{ color: T.ink }}>кавычки</b> на письме — открыли и обязательно закрыли. Нажмите кнопку и добавьте тег к слову <b style={{ color: T.ink }}>"Привет!"</b>.</> })}</Mentor>

        <div className={`split${wrapped ? " split3" : ""}`}>{
    /* §34: xato-topish quti uchinchi ustunda */
  }
          <div className="col">
            <div className={`tegbuild-wrap ${wrapped ? "on" : ""} fade-up delay-2`}>
              <div className={`tegbuild ${wrapped ? "on" : ""}`}>
                <div className={ic("open", `tb-chip tb-tag tb-open ${wrapped ? "" : "hide"}`)} onClick={() => tap("open")}>
                  <span className="tb-code">&lt;h1&gt;</span><span className="tb-lbl">{tr3({ uz: "ochuvchi", ru: "открывающий" })}</span>
                </div>
                <div className={ic("content", "tb-chip tb-content")} onClick={() => tap("content")}>
                  <span className="tb-code">{tr3({ uz: "Salom!", ru: "Привет!" })}</span><span className="tb-lbl">{wrapped ? tr3({ uz: "kontent", ru: "контент" }) : tr3({ uz: "oddiy matn", ru: "обычный текст" })}</span>
                </div>
                <div className={ic("close", `tb-chip tb-tag tb-close ${wrapped ? "" : "hide"}`)} onClick={() => tap("close")}>
                  <span className="tb-code">&lt;<span className="tb-slash">/</span>h1&gt;</span><span className="tb-lbl">{tr3({ uz: "yopuvchi", ru: "закрывающий" })}</span>
                </div>
              </div>
              <div className="tb-bracket"><div className="tb-brace" /><span className="tb-brace-lbl">{tr3({ uz: "= bitta element", ru: "= один элемент" })}</span></div>
            </div>
    ...
    setActive(null);
              ...
              {wrapped ? tr3({ uz: "↻ Qaytadan", ru: "↻ Заново" }) : tr3({ uz: "▶ Tegga o’rash", ru: "▶ Обернуть в тег" })}
            </button>

            {wrapped ? active ? <div className="role-line fade-step" key={active}><p className="body" style={{ color: T.ink, margin: 0 }}>{PARTS[active].role}</p></div> : <p className="body" style={{ margin: 0, color: T.ink3, fontSize: 13, fontStyle: "italic" }}>{tr3({ uz: "Yuqoridagi uchta qismdan birini bosib, vazifasini ko'ring.", ru: "Нажмите на одну из трёх частей выше и посмотрите её роль." })}</p> : <div className="hint"><p className="body" style={{ color: T.ink2, margin: 0 }}>{tr3({ uz: <>Hozir "Salom!" — oddiy matn, brauzer u bilan nima qilishni bilmaydi. Tugmani bosing — teglar ikki tomondan kelib uni <b style={{ color: T.ink }}>qamrab oladi</b>.</>, ru: <>Сейчас "Привет!" — обычный текст, браузер не знает, что с ним делать. Нажмите кнопку — теги подойдут с двух сторон и <b style={{ color: T.ink }}>обнимут его</b>.</> })}</p></div>}
          </div>

          {(!isNarrow || wrapped) && <div className="col fade-step">
            <div className="flow-label">{tr3({ uz: "Sahifada qanday ko'rinadi", ru: "Как выглядит на странице" })}</div>
            <Preview title="sahifa.html" minH={92}>{wrapped ? <p className="pv-h1 fade-step">{tr3({ uz: "Salom!", ru: "Привет!" })}</p> : <p className="pv-plain">{tr3({ uz: "Salom!", ru: "Привет!" })}</p>}</Preview>
            {wrapped && <div className="frame-ok fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>✓ Endi brauzer biladi: <span className="mono">&lt;h1&gt;</span> boshladi, <span className="mono">&lt;/h1&gt;</span> tugatdi — orasidagi <b>"Salom!"</b> katta sarlavhaga aylandi.</>, ru: <>✓ Теперь браузер знает: <span className="mono">&lt;h1&gt;</span> начал, <span className="mono">&lt;/h1&gt;</span> закончил — "Привет!" между ними стал большим заголовком.</> })}</p></div>}
          </div>}
          {wrapped && <div className="col fade-step">
          <div className="dbg-box">
            <p className="eyebrow" style={{ color: T.accent, margin: "2px 0 0" }}>🐞 {tr3({ uz: "Endi xatoni toping", ru: "Теперь найдите ошибку" })}</p>
            <p className="body" style={{ margin: "2px 0 8px", color: T.ink2, fontSize: 13.5 }}>{tr3({ uz: <>Bu kodda bitta teg <b style={{ color: T.ink }}>noto'g'ri yopilgan</b>. Xato qatorni bosing.</>, ru: <>В этом коде один тег <b style={{ color: T.ink }}>закрыт неправильно</b>. Нажмите на строку с ошибкой.</> })}</p>
            <DebugChallenge
    lines={[{ text: tr3({ uz: "<h1>Mening saytim<h1>", ru: "<h1>Мой сайт<h1>" }), bug: true }, { text: tr3({ uz: "<p>Xush kelibsiz!</p>", ru: "<p>Добро пожаловать!</p>" }) }]}
    fixed={tr3({ uz: "<h1>Mening saytim</h1>", ru: "<h1>Мой сайт</h1>" })}
    explain={tr3({ uz: "Yopuvchi tegda / belgisi bo'lishi kerak: </h1>", ru: "В закрывающем теге должен быть знак /: </h1>" })}
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [val, setVal] = useState4(typeof storedAnswer?.studentAnswer === "string" ? storedAnswer.studentAnswer : "");
  const correct = normTag(val) === ANSWER;
  const touched = val.trim().length > 0;
  const [missed, setMissed] = useState4(false);
  useEffect5(() => {
    if (!correct && val.includes(">")) setMissed(true);
  }, [val, correct]);
  useEffect5(() => {
    if (correct && storedAnswer === void 0) {
  ...
  return <Stage eyebrow={tr3({ uz: "Yozing", ru: "Напишите" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={isMentorLive ? false : !correct} label={isMentorLive ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : correct ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Yopuvchi tegni yozing", ru: "Напишите закрывающий тег" })} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>o'zingiz</span> yozib ko'ring.</>, ru: <>Теперь попробуйте написать <span className="italic" style={{ color: T.accent }}>сами</span>.</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Har bir o'ralgan tegning <b style={{ color: T.ink }}>yopuvchisi bo'lishi shart</b>. Mana, <span className="mono">&lt;h1&gt;</span> ochildi, lekin yopilmagan. Uni yopuvchi tegini o'zingiz yozing — pastdagi katakka.</>, ru: <>У каждого парного тега <b style={{ color: T.ink }}>обязательно есть закрывающий</b>. Вот <span className="mono">&lt;h1&gt;</span> открылся, но не закрыт. Напишите его закрывающий тег сами — в поле ниже.</> })}</Mentor>
        <div className="split">
          <div className="col"><div className="yz-card fade-up delay-2"><div className="yz-line"><span className="yz-code"><span className="t-tag">&lt;h1&gt;</span>{tr3({ uz: "Salom!", ru: "Привет!" })}</span>{!correct ? <input className="yz-input" value={val} onChange={(e) => setVal(e.target.value)} placeholder={tr3({ uz: "yopuvchi teg…", ru: "закрывающий тег…" })} spellCheck={false} /> : <span className="yz-code yz-done"><span className="t-tag">&lt;/h1&gt;</span></span>}</div>{!correct && <p className="yz-hint">{missed ? tr3({ uz: "Deyarli! Yopuvchi teg / belgisi bilan boshlanadi: </h1>", ru: "Почти! Закрывающий тег начинается со знака /: </h1>" }) : touched ? tr3({ uz: "Davom eting: / dan keyin teg nomi, oxirida > belgisi.", ru: "Продолжайте: после / — имя тега, в конце знак >." }) : tr3({ uz: "Maslahat: avval / yozing, keyin teg nomi va >", ru: "Подсказка: сначала напишите /, потом имя тега и >" })}</p>}{correct && <p className="yz-ok">✓ {tr3({ uz: "To'g'ri! Endi element yopildi:", ru: "Верно! Теперь элемент закрыт:" })} &lt;h1&gt;...&lt;/h1&gt;</p>}</div></div>
          <div className="col"><div className="flow-label">{tr3({ uz: "Natija", ru: "Результат" })}</div><div className="bp-window fade-up delay-2"><div className="bp-bar"><span className="bb-dots"><i /><i /><i /></span><span className="bp-title">sahifa.html</span></div><div className="bp-body" style={{ minHeight: 110, display: "flex", alignItems: "center", justifyContent: "center" }}>{correct ? <p className="pv-h1 fade-step">{tr3({ uz: "Salom!", ru: "Привет!" })}</p> : <p className="yz-placeholder">{tr3({ uz: "Natija shu yerda chiqadi…", ru: "Результат появится здесь…" })}</p>}</div></div></div>
        </div>
        {isMentorLive && <MentorWorkStats live={live} screenIdx={screen} taskLabel={tr3({ uz: "Yopuvchi teg yozish", ru: "Написать закрывающий тег" })} />}
      </div>
    </Stage>;
};
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const LADDER = [
    { n: 1, size: 28, tag: tr3({ uz: "eng katta", ru: "самый большой" }) },
    { n: 2, size: 23, tag: "" },
    { n: 3, size: 19, tag: "" },
    { n: 4, size: 16.5, tag: "" },
    { n: 5, size: 14.5, tag: "" },
    { n: 6, size: 13, tag: tr3({ uz: "eng kichik", ru: "самый маленький" }) }
  ];
  const LADDER_COMPACT = [
    { n: 1, size: 26, tag: tr3({ uz: "eng katta", ru: "самый большой" }) },
    { n: 2, size: 21, tag: "" },
    { ellipsis: true },
    { n: 6, size: 13, tag: tr3({ uz: "eng kichik", ru: "самый маленький" }) }
  ];
  ...
  const G = "Georgia, serif";
  const mctx = useContext2(MentorCtx) || {};
  const isNarrow = useIsMobile(768);
  const pick = (v) => {
    if (done) return;
    setPicked(v);
    ...
    if (!audio.muted) setTimeout(() => {
      const e = getAudioEngine();
      if (e && !audio.muted) e.pushOneOff(`Telegramda eng muhim gap qalin va birinchi turadi. HTML sahifasida esa uni kattaligi ko'rsatadi: h1 eng katta, h6 eng kichik.`);
    }, 300);
  };
  const showAnswer = done && (!isNarrow || revealed);
  const prevCollapsedRef = useRef4(mctx.collapsed);
  useEffect5(() => {
    if (isNarrow && done) {
      if (prevCollapsedRef.current && !mctx.collapsed) setRevealed(false);
      else if (!prevCollapsedRef.current && mctx.collapsed) setRevealed(true);
    }
    prevCollapsedRef.current = mctx.collapsed;
  }, [mctx.collapsed]);
  return <Stage eyebrow={tr3({ uz: "Sarlavhalar", ru: "Заголовки" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr3({ uz: "✍️ Praktika →", ru: "✍️ Практика →" }) : tr3({ uz: "Avval tanlang", ru: "Сначала выберите" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Telegram postida qaysi yozuv <span className="italic" style={{ color: T.accent }}>birinchi</span> ko'zga tashlanadi?</>, ru: <>Какая надпись в посте Telegram бросается в глаза <span className="italic" style={{ color: T.accent }}>первой</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Postni ochganingizda hamma matnni birdan o'qimaysiz — avval <b style={{ color: T.ink }}>qalin, birinchi</b> qatorni ko'rasiz. Sizningcha, qaysi yozuv eng muhim?</>, ru: <>Открыв пост, вы не читаете сразу весь текст — сначала видите <b style={{ color: T.ink }}>жирную первую</b> строку. Как вы думаете, какая надпись самая важная?</> })}</Mentor>
        <div className="split">
          <div className="col">
            <div className={`news-card tg-post frame fade-up delay-1 ${showAnswer ? "tagged" : ""}`}>
              <div className="tgp-head">
                ...
                <div><p className="tgp-name">{tr3({ uz: "Maktab kanali", ru: "Школьный канал" })}</p><p className="tgp-sub">{tr3({ uz: "bugun 09:14", ru: "сегодня 09:14" })}</p></div>
              </div>
              <div className="news-line news-headline">
                <h3 style={{ fontFamily: G, fontWeight: 700, fontSize: "clamp(20px,3vw,26px)", lineHeight: 1.15, color: T.ink, margin: 0 }}>{tr3({ uz: "Maktabimizda robototexnika to'garagi ochildi", ru: "В нашей школе открылся кружок робототехники" })}</h3>
                <span className="tag-badge accent" style={{ transitionDelay: "0.05s" }}>&lt;h1&gt;</span>
              </div>
              <div className="news-line">
                <p style={{ fontFamily: G, fontWeight: 700, fontSize: "clamp(15px,2vw,17px)", color: T.ink, margin: 0 }}>{tr3({ uz: "Kim qatnashishi mumkin?", ru: "Кто может участвовать?" })}</p>
                <span className="tag-badge" style={{ transitionDelay: "0.18s" }}>&lt;h2&gt;</span>
              </div>
              <div className="news-line">
                <p style={{ fontFamily: G, fontSize: "clamp(13px,1.7vw,14.5px)", color: T.ink2, margin: 0, lineHeight: 1.5 }}>{tr3({ uz: "To'garak har shanba soat 10:00 da. Ro'yxatdan o'tish hammaga ochiq.", ru: "Кружок — каждую субботу в 10:00. Запись открыта для всех." })}</p>
                <span className="tag-badge soft" style={{ transitionDelay: "0.31s" }}>&lt;p&gt;</span>
              </div>
              <p className="tgp-foot">👁 <b>412</b> · {tr3({ uz: "Maktab kanali", ru: "Школьный канал" })}</p>
              {showAnswer && <p className="news-hint fade-step">{tr3({ uz: <>↑ Har bir yozuvning o'z "darajasi" bor — eng kattasi <b style={{ color: T.accent }}>&lt;h1&gt;</b>.</>, ru: <>↑ У каждой надписи свой «уровень» — самый большой это <b style={{ color: T.accent }}>&lt;h1&gt;</b>.</> })}</p>}
            </div>
            {showAnswer && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <><b>Sizning loyihangiz:</b> saytingizdagi eng katta sarlavha — bu ismingiz. Demak u <span className="mono">&lt;h1&gt;</span>.</>, ru: <><b>Ваш проект:</b> самый большой заголовок на вашем сайте — это ваше имя. Значит, он будет <span className="mono">&lt;h1&gt;</span>.</> })}</p></div>}
          </div>
          <div className="col">
            {!showAnswer ? <>
                <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr3({ uz: "Sizningcha, qaysi yozuv eng muhim?", ru: "Как вы думаете, какая надпись самая важная?" })}</p>
                {[{ id: "a", label: tr3({ uz: "Eng katta sarlavha (tepadagi)", ru: "Самый большой заголовок (сверху)" }) }, { id: "b", label: tr3({ uz: "«Kim qatnashishi mumkin?»", ru: "«Кто может участвовать?»" }) }].map((o) => <button key={o.id} className="hook-option fade-up delay-3" onClick={() => pick(o.id)}><span className="radio" /><span>{o.label}</span></button>)}
              </> : <div ref={revealRef} className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 12, scrollMarginTop: 16 }}>
                {!isNarrow && <div className="frame-success"><p className="small mono" style={{ margin: "0 0 4px", fontWeight: 600, color: T.success, textTransform: "uppercase", letterSpacing: "0.08em" }}>{picked === "a" ? tr3({ uz: "✓ To'g'ri", ru: "✓ Верно" }) : tr3({ uz: "Mana gap", ru: "Вот в чём суть" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Telegramda eng muhim gap <b>qalin</b> va birinchi turadi. HTML sahifasida esa uni <b>kattaligi</b> ko'rsatadi: <span className="mono">h1</span> eng katta va muhim, keyin <span className="mono">h2</span>, <span className="mono">h3</span>… <span className="mono">h6</span> eng kichik.</>, ru: <>В Telegram самая важная фраза — <b>жирная</b> и первая. На HTML-странице её показывает <b>размер</b>: <span className="mono">h1</span> — самый большой и важный, дальше <span className="mono">h2</span>, <span className="mono">h3</span>… <span className="mono">h6</span> — самый маленький.</> })}</p></div>}
                <p className="flow-label">{isNarrow ? tr3({ uz: "h1 → h6 — sarlavha darajalari", ru: "h1 → h6 — уровни заголовков" }) : tr3({ uz: "h1 dan h6 gacha — darajalar narvoni", ru: "от h1 до h6 — лестница уровней" })}</p>
                <div className="ladder ladder-stair">{(isNarrow ? LADDER_COMPACT : LADDER).map((h, i) => h.ellipsis ? <div key="el" className="hl-row hl-stair" style={{ animationDelay: `${i * 0.1}s`, justifyContent: "center", padding: "2px 0", background: "transparent", boxShadow: "none" }}><span style={{ color: T.ink3, fontSize: 18, letterSpacing: 3 }}>⋮</span></div> : <div key={h.n} className="hl-row hl-stair" style={{ animationDelay: `${i * 0.1}s`, marginLeft: isNarrow ? 0 : `${i * 15}px` }}><span className="hl-bar" style={{ opacity: Math.max(0.18, 1 - i * 0.15) }} /><span className="hl-chip">{`<h${h.n}>`}</span><span className="hl-text" style={{ fontSize: h.size }}>{tr3({ uz: "Sarlavha", ru: "Заголовок" })}</span>{h.tag && <span className="hl-tag">{h.tag}</span>}</div>)}</div>
              </div>}
          </div>
        </div>
      </div>
...
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const labels = { imtihon: tr3({ uz: "imtihon", ru: "контрольная" }), albatta: tr3({ uz: "Albatta", ru: "Обязательно" }) };
  const [fmt, setFmt] = useState4({ imtihon: { b: false, i: false }, albatta: { b: false, i: false } });
  const [sel, setSel] = useState4(null);
  const [usedB, setUsedB] = useState4(false);
  const [usedI, setUsedI] = useState4(false);
  const [sent, setSent] = useState4(false);
  const done = usedB && usedI;
  const applyFmt = (k) => {
    if (!sel) return;
    setFmt((f) => ({ ...f, [sel]: { ...f[sel], [k]: !f[sel][k] } }));
    if (k === "b") setUsedB(true);
    else setUsedI(true);
  };
  useEffect5(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  const chatWord = (id) => <span key={id} className={`tgm-word ${sel === id ? "sel" : ""} ${fmt[id].b ? "bold" : ""} ${fmt[id].i ? "ital" : ""}`} onClick={(e) => {
    e.stopPropagation();
    setSel(id);
  }}>
      {labels[id]}
      {sel === id && <span className="tgm-menu" onClick={(e) => e.stopPropagation()}>
          <button className={`${fmt[id].b ? "on" : ""}${usedB ? "" : " pulse"}`} onClick={() => applyFmt("b")} title={tr3({ uz: "Qalin", ru: "Жирный" })}><b>B</b></button>
          <button className={`${fmt[id].i ? "on" : ""}${usedI ? "" : " pulse"}`} onClick={() => applyFmt("i")} title={tr3({ uz: "Yotiq", ru: "Курсив" })}><i>I</i></button>
        </span>}
    </span>;
  ...
  return <Stage eyebrow={tr3({ uz: "Matn", ru: "Текст" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr3({ uz: "✍️ Praktika →", ru: "✍️ Практика →" }) : tr3({ uz: "B va I ni sinang", ru: "Попробуйте B и I" })} onClick={onNext} /></>}>
    ...
    /* TELEGRAM muhiti */
          ...
          <div className="cmp-side">
            ...
            <div className="tgc-window">
              <div className="tgc-head"><span className="tgc-ava">🧑‍🏫</span><div><p className="tgc-name">{tr3({ uz: "Sinfdosh", ru: "Одноклассник" })}</p><p className="tgc-status">online</p></div></div>
              <div className="tgc-body">
                <div className="tgc-row in"><div className="tgc-bubble in">{tr3({ uz: "Ertaga maktabda muhim narsa bormi?", ru: "Завтра в школе есть что-то важное?" })}</div></div>
                {sent && <div className="tgc-row out"><div className="tgc-bubble out">{tr3({ uz: "Ertaga", ru: "Завтра" })} {renderWord("imtihon")}{tr3({ uz: " bor! ", ru: "! " })}{renderWord("albatta")} {tr3({ uz: "keling.", ru: "приходите." })}<span className="tgc-time">✓✓</span></div></div>}
              </div>
              <div className="tgc-composer" onClick={(e) => e.stopPropagation()}>
                <span className="tgc-cic">😊</span>
                <div className="tgc-input">{tr3({ uz: "Ertaga", ru: "Завтра" })} {chatWord("imtihon")}{tr3({ uz: " bor! ", ru: "! " })}{chatWord("albatta")} {tr3({ uz: "keling.", ru: "приходите." })}</div>
                <button className="tgc-send" onClick={() => setSent(true)} title={tr3({ uz: "Yuborish", ru: "Отправить" })} aria-label={tr3({ uz: "Yuborish", ru: "Отправить" })}><svg viewBox="0 0 24 24" fill="currentColor"><path d="M2.01 21 23 12 2.01 3 2 10l15 2-15 2z" /></svg></button>
              </div>
            </div>
            <p className="tgc-hint">{sel ? tr3({ uz: "👆 Yuqoridagi B yoki I tugmasini bosing", ru: "👆 Нажмите кнопку B или I сверху" }) : tr3({ uz: "👆 xabardagi so'zni bosib belgilang", ru: "👆 нажмите на слово в сообщении, чтобы выделить" })}</p>
          </div>
    ...
    /* VEB-SAYT muhiti */
          ...
          <div className="cmp-side">
            <div className="cmp-hd cmp-hd-html">🌐 {tr3({ uz: "Veb-saytda HTML buni teg bilan qiladi", ru: "На сайте HTML делает это тегом" })}</div>
            <div className="bp-window">
              ...
              <div className="cmp-siteview"><p>{tr3({ uz: "Ertaga", ru: "Завтра" })} {renderWord("imtihon")}{tr3({ uz: " bor! ", ru: "! " })}{renderWord("albatta")} {tr3({ uz: "keling.", ru: "приходите." })}</p></div>
            </div>
            <div className="cmp-codeblock">
              <span className="cmp-codelbl">&lt;/&gt; {tr3({ uz: "HTML kod", ru: "HTML-код" })}</span>
              <pre className="code-box cmp-code"><span className="tg">&lt;p&gt;</span>{tr3({ uz: "Ertaga", ru: "Завтра" })} {codeWord("imtihon")}{tr3({ uz: " bor! ", ru: "! " })}{codeWord("albatta")} {tr3({ uz: "keling.", ru: "приходите." })}<span className="tg">&lt;/p&gt;</span></pre>

              <p className="cmp-leg"><span className="cmp-leg-tag">&lt;strong&gt;</span> — {tr3({ uz: "qalin qiladi", ru: "делает жирным" })} · <span className="cmp-leg-tag">&lt;em&gt;</span> — {tr3({ uz: "yotiq-kursiv qiladi", ru: "делает курсивом" })}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className={`cmp-merge fade-up delay-3 ${done ? "done" : ""}`}>
          {done ? "✓ " : ""}{tr3({ uz: <>Telegramda so'zni belgilab <b>tugma</b> bosasiz; veb-saytda esa o'sha so'z <b>teg</b> ichiga olinadi.</>, ru: <>В Telegram вы выделяете слово и жмёте <b>кнопку</b>; на сайте то же слово берётся в <b>тег</b>.</> })}
        </div>
      </div>
    </Stage>;
};
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const SHOP = [tr3({ uz: "Non", ru: "Хлеб" }), tr3({ uz: "Sut", ru: "Молоко" }), tr3({ uz: "Tuxum", ru: "Яйца" })];
  const RECIPE = [tr3({ uz: "Limon siqing", ru: "Выжмите лимон" }), tr3({ uz: "Shakar soling", ru: "Добавьте сахар" }), tr3({ uz: "Suv quying", ru: "Налейте воду" })];
  const isNarrow = useIsMobile(768);
  ...
  useEffect5(() => {
    ...
    if (!startedRef.current) {
      ...
      timers.push(setTimeout(() => setPhase(1), isNarrow ? 1e3 : 450));
    }
    return () => timers.forEach(clearTimeout);
  }, [isNarrow]);
  const replay = () => setPhase((p) => p + 1);
  const codeList = (tag, items) => <pre className="code-box cmp2-code"><span className="tg">{`<${tag}>`}</span>{"\n"}{items.map((it, i) => <React3.Fragment key={i}>{"  "}<span className="tg">&lt;li&gt;</span>{it}<span className="tg">&lt;/li&gt;</span>{"\n"}</React3.Fragment>)}<span className="tg">{`</${tag}>`}</span></pre>;
  return <Stage eyebrow={tr3({ uz: "Ro'yxatlar", ru: "Списки" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr3({ uz: "✍️ Praktika →", ru: "✍️ Практика →" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Xarid ro'yxati va retsept — <span className="italic" style={{ color: T.accent }}>farqi</span> nimada?</>, ru: <>Список покупок и рецепт — в чём <span className="italic" style={{ color: T.accent }}>разница</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Xarid ro'yxatida tartib <b style={{ color: T.ink }}>muhim emas</b> (non, sut — qaysi avval bo'lsa ham bari bir). Retseptda esa qadamlar <b style={{ color: T.ink }}>aniq tartibda</b>. Shuning uchun HTML'da <b style={{ color: T.ink }}>ikki xil</b> ro'yxat bor — pastdan farqini ko'ring.</>, ru: <>В списке покупок порядок <b style={{ color: T.ink }}>не важен</b> (хлеб, молоко — что первым, без разницы). А в рецепте шаги идут <b style={{ color: T.ink }}>в точном порядке</b>. Поэтому в HTML есть <b style={{ color: T.ink }}>два вида</b> списков — посмотрите разницу ниже.</> })}</Mentor>
    ...
    /* XARID — ul */
          ...
          <div className="cmp2-card fade-up delay-2">
            <div className="cmp2-hd">🛒 {tr3({ uz: "Xarid ro'yxati", ru: "Список покупок" })} <span className="cmp2-chip">&lt;ul&gt;</span></div>
            <span className="cmp2-order no">{tr3({ uz: "tartib muhim EMAS", ru: "порядок НЕ важен" })}</span>
            ...
            <div className="cmp2-bp">
              ...
              <div className="cmp2-site shop">
                <div className="cmp2-top"><span className="cmp2-logo">🛒</span><span className="cmp2-brand">{tr3({ uz: "Mahsulotlar", ru: "Продукты" })}</span><span className="cmp2-nav"><span>{tr3({ uz: "Bosh", ru: "Главная" })}</span><span>{tr3({ uz: "Savat", ru: "Корзина" })}</span></span></div>
                <div className="cmp2-body">
                  <p className="cmp2-site-h">{tr3({ uz: "Bugungi xaridlar", ru: "Покупки на сегодня" })}</p>
            ...
            <div className="cmp2-codewrap">
              <span className="cmp2-pvlbl">&lt;/&gt; {tr3({ uz: "HTML kod", ru: "HTML-код" })}</span>
              {codeList("ul", SHOP)}
            </div>
            </div>
          </div>
    ...
    /* RETSEPT — ol */
          ...
          <div className="cmp2-card fade-up delay-3">
            <div className="cmp2-hd">🥤 {tr3({ uz: "Limonad retsepti", ru: "Рецепт лимонада" })} <span className="cmp2-chip">&lt;ol&gt;</span></div>
            <span className="cmp2-order yes">{tr3({ uz: "tartib MUHIM", ru: "порядок ВАЖЕН" })}</span>
            <div className="cmp2-pair">
            <div className="cmp2-bp">
              ...
              <div className="cmp2-site lemon">
                <div className="cmp2-top"><span className="cmp2-logo">🥤</span><span className="cmp2-brand">{tr3({ uz: "Limonad", ru: "Лимонад" })}</span><span className="cmp2-nav"><span>{tr3({ uz: "Retseptlar", ru: "Рецепты" })}</span></span></div>
                <div className="cmp2-body">
                  <p className="cmp2-site-h">{tr3({ uz: "Limonad retsepti", ru: "Рецепт лимонада" })}</p>
        ...
        <div className="cmp2-replaywrap">
          <button className="cmp2-replay" onClick={replay}>↻ {tr3({ uz: "Qaytadan ko'rsatish", ru: "Показать ещё раз" })}</button>
        </div>

        <div className="cmp2-concl fade-up delay-3">
          <p>{tr3({ uz: <>💡 <b>🛒 ul</b> — belgili (•), tartib muhim emas. <b>🥤 ol</b> — raqamli (1, 2, 3), tartib muhim (limonadda avval shakar, keyin limon bo'lsa — boshqacha chiqadi!). Har bir element — <span className="mono" style={{ color: T.accent }}>&lt;li&gt;</span>.</>, ru: <>💡 <b>🛒 ul</b> — с маркерами (•), порядок не важен. <b>🥤 ol</b> — с номерами (1, 2, 3), порядок важен (если в лимонаде сначала сахар, а потом лимон — получится иначе!). Каждый пункт — <span className="mono" style={{ color: T.accent }}>&lt;li&gt;</span>.</> })}</p>
        </div>
      </div>
    </Stage>;
};
var Screen11 = (props) => <QuestionScreen
  {...props}
  idx={11}
  scope="module-mikro"
  eyebrow={tr3({ uz: "Mashq · 3-savol", ru: "Упражнение · вопрос 3" })}
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "To'g'ri tegni tanlang", ru: "Выберите верный тег" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr3({ uz: "Retsept qadamlarini raqamli tartibda ko'rsatish uchun qaysi tegdan boshlash kerak?", ru: "С какого тега начать, чтобы показать шаги рецепта пронумерованным списком?" })}</h2></>}
  options={["`<li>`", "`<ol>`", "`<ul>`", "`<a>`"]}
  correctIdx={1}
  explainCorrect={tr3({ uz: "To'g'ri. `<ol>` — ordered list, ya'ni raqamli ro'yxat. Tartib muhim bo'lgan joylarda ishlatiladi.", ru: "Верно. `<ol>` — ordered list, то есть нумерованный список. Используется там, где важен порядок." })}
  explainWrong={{ 2: tr3({ uz: "`<ul>` — bu belgili (bullet) ro’yxat. Raqam emas, nuqta chiqaradi.", ru: "`<ul>` — маркированный (bullet) список. Выводит точки, а не номера." }), 0: tr3({ uz: "`<li>` — bu alohida element. Avval uni o’rab oluvchi `<ol>` yoki `<ul>` kerak.", ru: "`<li>` — отдельный пункт. Сначала нужен обёртывающий `<ol>` или `<ul>`." }), 3: tr3({ uz: "`<a>` — bu havola tegi, ro’yxatga aloqasi yo’q.", ru: "`<a>` — тег ссылки, к спискам он не относится." }), default: tr3({ uz: "Raqamli ro’yxat uchun `<ol>` ishlatiladi.", ru: "Для нумерованного списка используется `<ol>`." }) }}
/>;
var Screen12 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const PAGES = {
    bosh: { title: tr3({ uz: "Bosh sahifa", ru: "Главная" }), file: "index.html", url: "sayt.uz", body: tr3({ uz: "Salom! Bu mening birinchi saytim. O'zim haqimda yozaman.", ru: "Привет! Это мой первый сайт. Пишу о себе." }), links: [{ to: "oyinlar", label: tr3({ uz: "O'yinlar", ru: "Игры" }) }, { to: "men", label: tr3({ uz: "Men haqimda", ru: "Обо мне" }) }] },
    oyinlar: { title: tr3({ uz: "O'yinlar", ru: "Игры" }), file: "oyinlar.html", url: "sayt.uz/oyinlar.html", body: tr3({ uz: "Bo'sh vaqtimda Minecraft va futbol o'ynayman.", ru: "В свободное время играю в Minecraft и футбол." }), links: [{ to: "minecraft", label: tr3({ uz: "Minecraft haqida", ru: "Про Minecraft" }) }, { to: "bosh", label: tr3({ uz: "Bosh sahifa", ru: "Главная" }) }] },
    minecraft: { title: "Minecraft", file: "minecraft.html", url: "sayt.uz/minecraft.html", body: tr3({ uz: "Minecraft — menga eng yoqadigan o'yin.", ru: "Minecraft — моя самая любимая игра." }), links: [{ to: "oyinlar", label: tr3({ uz: "O'yinlar", ru: "Игры" }) }, { to: "bosh", label: tr3({ uz: "Bosh sahifa", ru: "Главная" }) }] },
    men: { title: tr3({ uz: "Men haqimda", ru: "Обо мне" }), file: "men.html", url: "sayt.uz/men.html", body: tr3({ uz: "Men CoddyCamp o'quvchisiman. Web-dasturchi bo'lmoqchiman.", ru: "Я ученик CoddyCamp. Хочу стать веб-разработчиком." }), links: [{ to: "bosh", label: tr3({ uz: "Bosh sahifa", ru: "Главная" }) }] }
  };
  const POS = { bosh: [55, 72], oyinlar: [150, 34], minecraft: [216, 90], men: [108, 122] };
  const EDGES = [["bosh", "oyinlar"], ["bosh", "men"], ["oyinlar", "minecraft"]];
  const [page, setPage] = useState4("bosh");
  const [jumped, setJumped] = useState4(false);
  const done = jumped;
  const [revealed, setRevealed] = useState4(false);
  const [travel, setTravel] = useState4(null);
  const travelKey = useRef4(0);
  const travelTimer = useRef4(null);
  const mctx = useContext2(MentorCtx) || {};
  const isNarrow = useIsMobile(768);
  const showResult = !isNarrow || revealed;
  useEffect5(() => () => clearTimeout(travelTimer.current), []);
  const go = (to) => {
    ...
    travelTimer.current = setTimeout(() => {
      ...
      if (!jumped) {
        setJumped(true);
        audio.triggerEvent("link_jumped");
      }
      setRevealed(true);
      if (isNarrow && mctx.setCollapsed) mctx.setCollapsed(true);
      setTravel(null);
  ...
  useEffect5(() => {
    if (isNarrow && mctx.collapsed === false) setRevealed(false);
  }, [mctx.collapsed]);
  return <Stage eyebrow={tr3({ uz: "Havolalar", ru: "Ссылки" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr3({ uz: "✍️ Praktika →", ru: "✍️ Практика →" }) : tr3({ uz: "Havolani bosing", ru: "Нажмите на ссылку" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Bir bosishda boshqa sahifaga qanday <span className="italic" style={{ color: T.accent }}>o'tamiz</span>?</>, ru: <>Как одним кликом <span className="italic" style={{ color: T.accent }}>перейти</span> на другую страницу?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>YouTube'da bitta videoni ko'rib bo'lgach, yonidagi videoga bosib o'tasiz. O'sha bosish — <b style={{ color: T.ink }}>havola</b>. HTML'da havola <span className="mono">{"<a>"}</span> tegi bilan yasaladi: <span className="mono">href</span> ichiga <b style={{ color: T.ink }}>manzil</b> yoziladi, teglar orasiga esa <b style={{ color: T.ink }}>ko'rinadigan matn</b>. Menyuni bosib, sahifalar orasida yuring.</>, ru: <>Досмотрев видео на YouTube, вы кликаете на соседнее. Этот клик — <b style={{ color: T.ink }}>ссылка</b>. В HTML ссылка делается тегом <span className="mono">{"<a>"}</span>: в <span className="mono">href</span> пишется <b style={{ color: T.ink }}>адрес</b>, а между тегами — <b style={{ color: T.ink }}>видимый текст</b>. Кликайте по меню и походите между страницами.</> })}</Mentor>
        <div className="split">
          <div className="col"><div className="flow-label">{tr3({ uz: `Veb — bu "to'r"`, ru: "Веб — это «паутина»" })}</div><div className="web fade-up delay-2"><svg className="web-svg" viewBox="0 0 260 150" preserveAspectRatio="none">{EDGES.map(([a, b], i) => {
    const active = page === a || page === b;
    return <line key={i} x1={POS[a][0]} y1={POS[a][1]} x2={POS[b][0]} y2={POS[b][1]} stroke={active ? T.accent : T.ink3} strokeWidth={active ? 2 : 1.2} strokeDasharray={active ? "0" : "4 3"} opacity={active ? 1 : 0.6} />;
  })}</svg>{Object.keys(PAGES).map((k) => {
    const isCur = page === k;
    const linked = cur.links.some((l) => l.to === k);
    const off = !isCur && !linked;
    return <div key={k} className={`web-node ${isCur ? "on" : ""} ${off ? "web-node-off" : ""}`} onClick={linked && !isCur ? () => go(k) : void 0} style={{ left: `${POS[k][0] / 260 * 100}%`, top: `${POS[k][1] / 150 * 100}%` }}>{PAGES[k].title}</div>;
  })}{travel && <div className="web-travel" key={travel.key} style={{ "--fx": travel.fx, "--fy": travel.fy, "--tx": travel.tx, "--ty": travel.ty }} />}</div><p className="web-cap">{tr3({ uz: <>Har sahifa boshqasiga <b>havola</b> bilan bog'langan. Shu bog'lanishlar <b>"to'r"</b> hosil qiladi — <b>Veb</b> (World Wide Web) shundan nom olgan.</>, ru: <>Каждая страница связана с другой <b>ссылкой</b>. Эти связи образуют <b>«паутину»</b> — отсюда и имя <b>Веб</b> (World Wide Web).</> })}</p></div>
          {showResult && <div className="col">
            {!isNarrow && <div className="bp-window fade-up delay-2"><div className="bp-bar"><span className="bb-dots"><i /><i /><i /></span><span className="bp-url" key={`u-${page}`}><span className="lock">●</span>{cur.url}</span></div><div className="bp-body pg-in" key={`p-${page}`}><div className="site-top"><span className="site-wordmark">{tr3({ uz: "Mening saytim", ru: "Мой сайт" })}</span><span className="site-tag">{tr3({ uz: "o'quvchi · CoddyCamp", ru: "ученик · CoddyCamp" })}</span></div><h1 className="pg-h1">{cur.title}</h1><p className="pg-body">{cur.body}</p><div className="pg-divider" /><p className="pg-linklabel">{tr3({ uz: "Boshqa sahifalar", ru: "Другие страницы" })}</p><div className="pg-links">{cur.links.map((l, i) => <a key={i} className="pg-a" onClick={() => go(l.to)}>{l.label} <span className="arr">→</span></a>)}</div></div></div>}
            <div className="codecard" key={`c-${page}`}><p className="codecard-top"><span className="dotf" />{cur.file} — {tr3({ uz: "havolalar kodi", ru: "код ссылок" })}</p><pre className="codeblock">{cur.links.map((l, i) => <span className="ln" key={i}><span className="tg">&lt;a </span><span className="at">href</span><span className="tx">=</span><span className="st">"{PAGES[l.to].file}"</span><span className="tg">&gt;</span><span className="tx">{l.label}</span><span className="tg">&lt;/a&gt;</span></span>)}</pre><p className="codecap">{tr3({ uz: <>Har bir havola = bitta <span className="mn">&lt;a&gt;</span> teg. U ikki qismdan iborat: <span className="mn">href="…"</span> ichida — <b>manzil</b> (bosilganda qayerga borish), teglar <b>orasida</b> — sahifada <b>ko'rinadigan, bosiladigan yozuv</b>.</>, ru: <>Каждая ссылка = один тег <span className="mn">&lt;a&gt;</span>. Он состоит из двух частей: внутри <span className="mn">href="…"</span> — <b>адрес</b> (куда идти по клику), а <b>между</b> тегами — <b>видимая кликабельная надпись</b>.</> })}</p></div>
          </div>}
        </div>
        {showResult && <div className="frame-success fade-up delay-3" style={{ padding: "9px 15px" }}><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <><b>Sizning loyihangiz:</b> saytingizga sevimli sayt yoki Telegram kanalingizga havola qo'shasiz.</>, ru: <><b>Ваш проект:</b> вы добавите на свой сайт ссылку на любимый сайт или свой Telegram-канал.</> })}</p></div>}
      </div>
    </Stage>;
};
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const MAX = 6;
  const CHIPS = [{ key: "h1", label: tr3({ uz: "Sarlavha", ru: "Заголовок" }), tag: "h1" }, { key: "p", label: tr3({ uz: "Matn", ru: "Текст" }), tag: "p" }, { key: "ul", label: tr3({ uz: "Ro'yxat", ru: "Список" }), tag: "ul" }, { key: "a", label: tr3({ uz: "Havola", ru: "Ссылка" }), tag: "a" }, { key: "img", label: tr3({ uz: "Rasm", ru: "Картинка" }), tag: "img" }];
  const detect = (txt) => {
    const t = (txt || "").toLowerCase();
    if (/sarlavha|ism|title|bosh|заголов|имя/.test(t)) return "h1";
    if (/rasm|surat|img|foto|картин|фото|изображ/.test(t)) return "img";
    if (/ro.?yxat|ruyxat|mashg|list|списо?к|спис/.test(t)) return "ul";
    if (/havola|sayt|link|url|ссылк|сайт/.test(t)) return "a";
    if (/matn|paragraf|haqim|tavsif|yoz|текст|абзац|о себе/.test(t)) return "p";
    return null;
  };
  const elCode = (type) => {
    switch (type) {
      case "h1":
        return <><Tg>{"<h1>"}</Tg>{tr3({ uz: "Mening sahifam", ru: "Моя страница" })}<Tg>{"</h1>"}</Tg></>;
      case "p":
        return <><Tg>{"<p>"}</Tg>{tr3({ uz: "Men HTML o'rganyapman.", ru: "Я изучаю HTML." })}<Tg>{"</p>"}</Tg></>;
      case "ul":
        return <><Tg>{"<ul>"}</Tg>{"\n    "}<Tg>{"<li>"}</Tg>{tr3({ uz: "Futbol", ru: "Футбол" })}<Tg>{"</li>"}</Tg>{"\n    "}<Tg>{"<li>"}</Tg>{tr3({ uz: "Kitob", ru: "Книги" })}<Tg>{"</li>"}</Tg>{"\n  "}<Tg>{"</ul>"}</Tg></>;
      case "a":
        return <><Tg>{"<a "}</Tg><At>href</At>=<Sr>"coddycamp.uz"</Sr><Tg>{">"}</Tg>CoddyCamp<Tg>{"</a>"}</Tg></>;
      case "img":
        return <><Tg>{"<img "}</Tg><At>src</At>=<Sr>"rasm.jpg"</Sr><Tg>{">"}</Tg></>;
      default:
        return null;
    }
  };
  ...
  const elView = (type, i) => {
    switch (type) {
      case "h1":
        return <h1 key={i} style={{ fontFamily: "Georgia, serif", fontSize: "clamp(18px,2.8vw,24px)", margin: "0 0 6px", color: T.ink }}>{tr3({ uz: "Mening sahifam", ru: "Моя страница" })}</h1>;
      case "p":
        return <p key={i} style={{ fontFamily: "Georgia, serif", margin: "0 0 6px", color: T.ink, fontSize: "clamp(13px,1.8vw,15px)" }}>{tr3({ uz: "Men HTML o'rganyapman.", ru: "Я изучаю HTML." })}</p>;
      case "ul":
        return <ul key={i} style={{ fontFamily: "Georgia, serif", color: T.ink, margin: "0 0 6px", paddingLeft: 22, fontSize: "clamp(13px,1.8vw,15px)" }}><li>{tr3({ uz: "Futbol", ru: "Футбол" })}</li><li>{tr3({ uz: "Kitob", ru: "Книги" })}</li></ul>;
      case "a":
        return <a key={i} style={{ fontFamily: "Georgia, serif", color: T.link, textDecoration: "underline", fontSize: "clamp(13px,1.8vw,15px)", display: "inline-block", marginBottom: 6 }}>CoddyCamp</a>;
      case "img":
        return <span key={i} style={{ display: "block", marginBottom: 6 }}><ImgPlaceholder /></span>;
      default:
        return null;
    }
  };
  ...
  const submit = () => {
    ...
    if (!type) {
      setHint(tr3({ uz: "Tushunmadim 🙂 Mana shulardan yozing: sarlavha, matn, ro’yxat, havola, rasm.", ru: "Не понял 🙂 Напишите что-то из этого: заголовок, текст, список, ссылка, картинка." }));
      return;
    }
    generate(type);
    setText("");
  ...
  return <Stage eyebrow={tr3({ uz: "Amaliyot · sahifa quramiz", ru: "Практика · строим страницу" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: `Kamida 3 ta bo’lak (${items.length}/3)`, ru: `Минимум 3 блока (${items.length}/3)` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Buyruq bering — <span className="italic" style={{ color: T.accent }}>kod o'zi yaraladi</span>.</>, ru: <>Дайте команду — <span className="italic" style={{ color: T.accent }}>код появится сам</span>.</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Mana, vaqti keldi. Bugun o'rgangan teglaringizdan o'z sahifangizni yig'asiz. Buyruq yozing yoki tayyor teglardan tanlang — <b style={{ color: T.ink }}>"Yaratish"</b> tugmasini bosing, kod o'zi paydo bo'ladi. Kamida 3 ta bo'lak qo'shing.</>, ru: <>Вот и пришло время. Из выученных сегодня тегов вы соберёте свою страницу. Напишите команду или выберите из готовых тегов — нажмите <b style={{ color: T.ink }}>«Создать»</b>, и код появится сам. Добавьте минимум 3 блока.</> })}</Mentor>
        <div className="split">
          <div className="col">
            <div className="fade-up delay-2"><p className="flow-label" style={{ marginBottom: 7 }}>{tr3({ uz: "Buyruq yozing", ru: "Напишите команду" })}</p><div className="prompt-row"><input className="prompt-input" value={text} placeholder={tr3({ uz: "masalan: rasm qo'sh", ru: "например: добавь картинку" })} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => {
    if (e.key === "Enter") submit();
  }} /><button className="prompt-btn" onClick={submit} disabled={!!pending || items.length >= MAX}>{tr3({ uz: "Yaratish", ru: "Создать" })}</button></div></div>
            <div className="fade-up delay-2"><p className="flow-label" style={{ margin: "2px 0 7px" }}>{tr3({ uz: "yoki tayyor buyruqlardan tanlang", ru: "или выберите из готовых команд" })}</p><div className="chips">{CHIPS.map((c) => <button key={c.key} className="gchip" disabled={items.length >= MAX} onClick={() => {
    setText(tr3({ uz: c.label.toLowerCase() + " qo'sh", ru: "добавь " + c.label.toLowerCase() }));
    setHint("");
  }}>{c.label} <span className="gt">&lt;{c.tag}&gt;</span></button>)}{items.length > 0 && <button className="gchip" onClick={reset}>↺ {tr3({ uz: "Tozalash", ru: "Очистить" })}</button>}</div></div>
            <p className="body fade-up delay-3" style={{ margin: "2px 0 0", color: T.ink3, fontSize: 13 }}>{tr3({ uz: <><b style={{ color: T.ink2 }}>Tez tugatdingizmi?</b> 5 xil teg ishlating yoki yangi teg topib sinab ko'ring.</>, ru: <><b style={{ color: T.ink2 }}>Закончили быстро?</b> Используйте 5 разных тегов или найдите и попробуйте новый.</> })}</p>
            {hint && <p className="hint fade-step">{hint}</p>}
            {done && <div style={{ background: T.successSoft, borderLeft: `4px solid ${T.success}`, borderRadius: 12, padding: "12px 15px" }} className="fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Zo'r! Siz <b>buyruq berib</b> sahifa qurdingiz — va kodni o'qib, nima yozilganini tushunasiz.</>, ru: <>Отлично! Вы построили страницу, <b>отдавая команды</b> — и, читая код, понимаете, что в нём написано.</> })}</p></div>}
          </div>
          <div className="col">
            <div className="flow-label">{tr3({ uz: "Kod", ru: "Код" })}</div>
            <pre className="code-box"><Tg>{"<body>"}</Tg>{"\n"}{items.length === 0 && !pending && <><span className="cm">{tr3({ uz: "  <!-- buyruq bering -->", ru: "  <!-- дайте команду -->" })}</span>{"\n"}</>}{items.map((it, i) => <React3.Fragment key={i}>{"  "}{elCode(it)}{"\n"}</React3.Fragment>)}{pending && <><span className="gen-line">{tr3({ uz: "  yaratilmoqda", ru: "  создаётся" })}</span>{"\n"}</>}<Tg>{"</body>"}</Tg></pre>
            <div className="flow-label">{tr3({ uz: "Sahifa", ru: "Страница" })}</div>
            <div className="bp-window"><div className="bp-bar"><span className="bb-dots"><i /><i /><i /></span><span className="bp-title">mening-sahifam.html</span></div><div className="bp-body">{items.length === 0 && !pending ? <p style={{ color: T.ink3, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif" }}>{tr3({ uz: "Bo'sh sahifa — buyruq bering...", ru: "Пустая страница — дайте команду..." })}</p> : items.map((it, i) => <span key={i} className="el-in" style={{ display: "block" }}>{elView(it, i)}</span>)}</div></div>
          </div>
        </div>
      </div>
    </Stage>;
...
var ScreenFlashcards = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Takrorlash", ru: "Повторение" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={false} label={tr3({ uz: "Yakunlash →", ru: "Завершить →" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  const RECAP = [tr3({ uz: "HTML bilan veb-sahifa yasash", ru: "Делать веб-страницу на HTML" }), tr3({ uz: "Sahifa skeletini qurish (head, body)", ru: "Строить скелет страницы (head, body)" }), tr3({ uz: "Teglarni ochish va yopish", ru: "Открывать и закрывать теги" }), tr3({ uz: "Sarlavha (h1–h6) va matn (strong, em)", ru: "Заголовки (h1–h6) и текст (strong, em)" }), tr3({ uz: "Ro'yxat (ul/ol) va havola (a) qo'shish", ru: "Добавлять списки (ul/ol) и ссылки (a)" })];
  const HOMEWORK2 = [{ b: tr3({ uz: "Sarlavha (h1)", ru: "Заголовок (h1)" }), t: tr3({ uz: "— ismingiz", ru: "— ваше имя" }) }, { b: tr3({ uz: "2–3 paragraf", ru: "2–3 абзаца" }), t: tr3({ uz: "— o'zingiz haqingizda", ru: "— о себе" }) }, { b: tr3({ uz: "Ro'yxat", ru: "Список" }), t: tr3({ uz: "— sevimli mashg'ulotlaringiz", ru: "— ваши любимые занятия" }) }, { b: tr3({ uz: "Havola", ru: "Ссылка" }), t: tr3({ uz: "— sevimli saytingizga", ru: "— на любимый сайт" }) }];
  const correct = SCORED_IDX.filter((i) => answers[i]?.correct).length;
  const total = SCORED_IDX.length;
  const PASSED = (total ? correct / total : 0) >= 0.6;
  return <Stage eyebrow={tr3({ uz: "Tayyor", ru: "Готово" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Keyingi dars →", ru: "Следующий урок →" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr3({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr3({ uz: <>Birinchi <span className="italic" style={{ color: T.accent }}>saytingizni</span> yasadingiz.</>, ru: <>Вы сделали свой первый <span className="italic" style={{ color: T.accent }}>сайт</span>.</> })}</h2>{
  ...
  />
        ...
        <div className="card fade-up d3"><div className="card-lbl" style={{ color: T.success }}><span className="tick" style={{ width: 16, height: 16, borderRadius: "50%", background: T.success, color: "#fff", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 10 }}>✓</span> {tr3({ uz: "Endi siz bilasiz", ru: "Теперь вы умеете" })}</div><ul className="recap">{RECAP.map((r, i) => <li key={i} style={{ animationDelay: `${0.3 + i * 0.07}s` }}><span className="ck">✓</span><span>{r}</span></li>)}</ul></div>
    ...
    /* 🏠 Vazifani boshlash — kompilyator ochiladi (LMS'da onPractice, lokalda overlay).
       Mentor proyektorida KO'RSATILMAYDI: uy ishi shaxsiy (sahna ↔ daftar tamoyili). */
          ...
          {!isMentorL && onHomework && <button className="btn" onClick={onHomework} style={{ marginTop: 14, background: T.accent, boxShadow: "0 8px 22px -6px rgba(255,79,40,0.5)" }}>
              {tr3({ uz: "Vazifani bajarish →", ru: "Выполнить задание →" })}
            </button>}
        </div>}
...
var Q_LABELS = { 4: { uz: "Brauzer roli", ru: "Роль браузера" }, 6: { uz: "Ko'rinadigan matn — body", ru: "Видимый текст — body" }, 8: { uz: "Yopuvchi teg (</h1>)", ru: "Закрывающий тег (</h1>)" }, 12: { uz: "Raqamli ro'yxat — ol", ru: "Нумерованный список — ol" } };
var Confetti = () => {
...
var INLINE_KEYS = { s4: 2, s5b: 3, s7: -1, s11: 1 };
var ScreenPodium = ({ screen, answers, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr3({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Kim <span className="italic" style={{ color: T.accent }}>g'olib</span>?</>, ru: <>Кто <span className="italic" style={{ color: T.accent }}>победитель</span>?</> })}</h2></div>
    ...
    return <div key={rank} className={`pod-col pod-${rank + 1} ${b && live.playerId === b.id ? "me" : ""}`}>
            ...
            {myIdx >= 0 && <p className="pod-my fade-up">{tr3({ uz: <>Siz — <b>{myIdx + 1}-o'rin</b> ({board[myIdx].okCount}/{totalQ} to'g'ri)</>, ru: <>Вы — <b>{myIdx + 1}-е место</b> ({board[myIdx].okCount}/{totalQ} верных)</> })}</p>}
    ...
    return <div key={q} className="qstat-row">
              ...
              {live.mode === "mentor" && <p className="small" style={{ margin: "10px 0 0", color: T.ink2 }}>{tr3({ uz: "⚠️ belgili savollar — sinf qiynalgan mavzular. Qayta tushuntirish tavsiya etiladi.", ru: "Вопросы со значком ⚠️ — темы, где класс споткнулся. Рекомендуем объяснить их ещё раз." })}</p>}
            </div>
          </>}
      </div>
    </Stage>;
...
function QuizArena({ live, onClose, startSolo }) {
  ...
  return <div className="qz-arena">
    ...
    /* ===== YAKUN — PODIUM ===== */
      ...
      {phase === "done" && <div className="qz-view fade-step">
          ...
          {solo ? <div className="qz-solo-res">
              ...
              <p className="qz-sub">{tr3({ uz: `ball · ${soloScore.ok}/${QUIZ_BANK.length} to'g'ri${soloScore.maxStreak >= 2 ? ` · eng uzun streak 🔥x${soloScore.maxStreak}` : ""}`, ru: `баллов · ${soloScore.ok}/${QUIZ_BANK.length} верных${soloScore.maxStreak >= 2 ? ` · лучший стрик 🔥x${soloScore.maxStreak}` : ""}` })}</p>
    ...
    return <div key={rank} className={`qz-pod-col p${rank + 1} ${b && b.id === live.playerId ? "me" : ""}`}>
              ...
              {isStudent && <button className="qz-btn" onClick={startPractice}>↻ {tr3({ uz: "Testni qayta ishlash — mashq (jadvalga yozilmaydi)", ru: "Пройти тест ещё раз — тренировка (в таблицу не пишется)" })}</button>}
            </>}
...
var TASK_HEADINGS = {
  ...
  requirements: [
    { id: "h1", label: { uz: "<h1> — bosh sarlavha", ru: "<h1> — главный заголовок" }, check: checks.text("h1", { uz: "`<h1>` ichiga bosh sarlavha matnini yozing", ru: "Напишите текст главного заголовка внутри `<h1>`" }) },
    { id: "h2", label: { uz: "<h2> — kichik sarlavha", ru: "<h2> — заголовок поменьше" }, check: checks.text("h2", { uz: "`<h2>` ichiga kichik sarlavha yozing", ru: "Напишите заголовок поменьше внутри `<h2>`" }) },
    { id: "h6", label: { uz: "<h6> — eng kichik sarlavha", ru: "<h6> — самый маленький заголовок" }, check: checks.text("h6", { uz: "`<h6>` ichiga eng kichik sarlavha yozing — farqni solishtiring", ru: "Напишите самый маленький заголовок внутри `<h6>` — сравните разницу" }) }
  ]
};
var STARTER_HEADINGS = {
  uz: `<!-- Bu yerga yozing -->
...
var TASK_LINK = {
  ...
  requirements: [
    { id: "href", label: { uz: "<a> — href manzil bilan", ru: "<a> — с адресом в href" }, check: checks.attr("a", "href", { uz: '`<a>` da `href="https://..."` manzilni to\'ldiring', ru: 'Заполните адрес `href="https://..."` у `<a>`' }) },
    { id: "text", label: { uz: "<a> — ko'rinadigan matn", ru: "<a> — видимый текст" }, check: checks.text("a", { uz: "`<a>` ichiga bosiladigan matn yozing (masalan sayt nomi)", ru: "Напишите внутри `<a>` кликабельный текст (например, имя сайта)" }) }
  ]
};
var STARTER_LINK = {
  uz: `<!-- Bu yerga yozing -->
...
var TASK_FINAL = {
  ...
  requirements: [
    { id: "h1", label: { uz: "<h1> sarlavha (matn bilan)", ru: "<h1> заголовок (с текстом)" }, check: checks.text("h1", { uz: "`<h1>` ichiga sarlavha yozing", ru: "Напишите заголовок внутри `<h1>`" }) },
    { id: "p", label: { uz: "<p> — matn (paragraf)", ru: "<p> — текст (абзац)" }, check: checks.text("p", { uz: "`<p>` ichiga bir-ikki gap yozing", ru: "Напишите пару предложений внутри `<p>`" }) },
    { id: "li", label: { uz: "ro'yxatda kamida 2 ta <li>", ru: "в списке минимум 2 <li>" }, check: checks.count("li", 2, { uz: "`<ul>`/`<ol>` ichida kamida 2 ta `<li>` band yozing", ru: "Напишите минимум 2 пункта `<li>` внутри `<ul>`/`<ol>`" }) },
    { id: "a", label: { uz: "<a> havola — href bilan", ru: "<a> ссылка — с href" }, check: checks.attr("a", "href", { uz: '`<a href="...">matn</a>` havola qo\'shing', ru: 'Добавьте ссылку `<a href="...">текст</a>`' }) }
  ]
};
var STARTER_FINAL = {
  uz: `<!-- Bu yerga yozing -->
...
var TASK_HW = {
  ...
  title: { uz: "O'zingiz haqingizda sahifa yasang", ru: "Сделайте страницу о себе" },
  brief: {
    uz: "Darsda saytni mentor bilan birga yig'dingiz — endi o'zingiz haqingizda sahifani yolg'iz yozasiz. Har talab bajarilganda ro'yxatda belgi yashil yonadi — to'rttalasi ham yonsa, vazifa tayyor.",
    ru: "На уроке вы собирали сайт вместе с ментором — теперь напишите страницу о себе сами. Когда требование выполнено, пункт в списке загорается зелёным — горят все четыре, значит задание готово."
  },
  requirements: [
    { id: "h1", label: { uz: "<h1> — ismingiz", ru: "<h1> — ваше имя" }, check: checks.text("h1", { uz: "`<h1>` ichiga ismingizni yozing", ru: "Напишите своё имя внутри `<h1>`" }) },
    { id: "p2", label: { uz: "kamida 2 ta <p> — o'zingiz haqingizda", ru: "минимум 2 <p> — о себе" }, check: checks.count("p", 2, { uz: "O'zingiz haqingizda kamida 2 ta `<p>` paragraf yozing", ru: "Напишите о себе минимум 2 абзаца `<p>`" }) },
    { id: "li2", label: { uz: "ro'yxatda kamida 2 ta <li> — sevimli mashg'ulotlaringiz", ru: "в списке минимум 2 <li> — любимые занятия" }, check: checks.count("li", 2, { uz: "Sevimli mashg'ulotlaringizni `<ul>` ichida kamida 2 ta `<li>` qilib yozing", ru: "Напишите любимые занятия — минимум 2 пункта `<li>` внутри `<ul>`" }) },
    { id: "a", label: { uz: "<a> — sevimli saytingizga havola", ru: "<a> — ссылка на любимый сайт" }, check: checks.attr("a", "href", { uz: 'Sevimli saytingizga `<a href="https://...">nomi</a>` havola qo\'shing', ru: 'Добавьте ссылку на любимый сайт: `<a href="https://...">имя</a>`' }) }
  ]
};
var STARTER_HW = {
  uz: `<!-- Bu yerga yozing -->
...
var PRACTICE_AFTER = {
  ...
  // 2) Havola (a/href)
  14: { task: TASK_FINAL, starter: STARTER_FINAL }
  // 3) Yakuniy (noldan sayt) — oxirgi nazariya-ekranidan (idx 14) keyin
};
var ACHIEVEMENTS = {
...
var ACH_TRIGGERS = { s5: "skelet", s6: "debugger", s7: "firsttag" };
...
function TourGuide({ role, onClose }) {
  ...
  return <div className="tg-root">
      ...
      <div className="tg-call" style={callStyle}>
        <div className="tg-head"><span className="tg-ic">{step.ic}</span><span className="tg-h">{tr3(step.h)}</span><span className="tg-count">{i + 1}/{steps.length}</span></div>
        <p className="tg-body">{tr3(step.body)}</p>
        <div className="tg-nav">
          <button className="tg-skip" onClick={onClose}>{tr3({ uz: "O'tkazib yuborish", ru: "Пропустить" })}</button>
          <div className="tg-navr">
            {i > 0 && <button className="tg-btn ghost" onClick={() => setI(i - 1)} aria-label={tr3({ uz: "Oldingi", ru: "Предыдущий" })}>←</button>}
            <button className="tg-btn go" onClick={next}>{last ? tr3({ uz: "Boshladik!", ru: "Поехали!" }) : tr3({ uz: "Keyingisi →", ru: "Дальше →" })}</button>
          </div>
        </div>
      </div>
    </div>;
...
function HtmlLesson({ lang: langProp, onFinished, onPractice, liveToken }) {
  ...
  const next = () => {
    ...
    const inLiveClass = !!(live && live.mode === "student" && live.status !== "ended");
    if (!inLiveClass) {
      advance();
      return;
    }
  ...
  const [onboard, setOnboard] = useState4(false);
  const onboardShownRef = useRef4(false);
  ...
  useEffect5(() => {
    if (live.mode !== "choosing" && !onboardShownRef.current) {
      ...
      let show = true;
      try {
        if (localStorage.getItem("hcOnboarded_" + onboardRole)) show = false;
      } catch {
      }
      if (show) {
  ...
  const closeOnboard = () => {
    try {
      localStorage.setItem("hcOnboarded_" + onboardRole, "1");
    } catch {
    }
    setOnboard(false);
  };
  ...
  return <LangContext.Provider value={lang}>
      <style>{`
        ...
        /* F-0912-03 · 147-qonun: ⛶ tugmasi turgan burchakda JOY. Tugma o'ng yuqorida
           ...
           tugma yozuvni yopadi (m1-03 da «0 / 4 ko'rildi» ning 31% i ko'rinmasdi). */
        ...
        .split3 { grid-template-columns: minmax(0,1.1fr) minmax(0,1fr) minmax(0,1fr); gap: clamp(14px,2vw,24px); } /* §34 s7: matn-blok · brauzer · xato-topish */
        ...
        .br-status.br-status { margin: 4px 0 0; font-family: 'Manrope'; font-weight: 600; font-size: clamp(12.5px,1.6vw,14px); color: ${T.success}; background: ${T.successSoft}; padding: 9px 13px; border-radius: 11px; animation: fade-step 0.35s ease-out; }
        ...
        .hl-row { display: flex; align-items: center; gap: 13px; padding: 5px 14px; border-radius: 10px; cursor: pointer; transition: all 0.18s; background: ${T.paper}; box-shadow: 0 4px 12px -6px rgba(${T.shadowBase},0.12); }
        ...
        .cmp2-card { display: flex; flex-direction: column; gap: 5px; background: ${T.paper}; border-radius: 13px; padding: 8px 11px; box-shadow: 0 8px 20px -8px rgba(${T.shadowBase},0.16); }
        ...
        .cmp2-body { padding: 7px 10px 9px; }
        ...
        .cmp2-codewrap { margin-top: 4px; padding-top: 6px; border-top: 1.5px dashed ${T.line}; }
        ...
        .rg-grid { display: grid; grid-template-columns: minmax(0,1.45fr) minmax(0,1fr); gap: 14px; align-items: start; } .rg-left, .rg-right { display: flex; flex-direction: column; gap: 12px; min-width: 0; }
        ...
        .rg-msg.rg-msg { margin: 0; font-family: 'Manrope'; font-weight: 600; font-size: clamp(12.5px,1.7vw,14.5px); line-height: 1.45; padding: 10px 14px; border-radius: 11px; animation: fade-step 0.32s ease-out; }
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        ...
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={tr3({ uz: "1-Modul", ru: "Модуль 1" })} /> : <>
    ...
    lang={__lang2}
    task={practice.task}
    starterCode={tr3(practice.starter)}
