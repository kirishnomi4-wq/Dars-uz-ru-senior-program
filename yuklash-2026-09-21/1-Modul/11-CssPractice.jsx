...
//  Manba:  src/1-Modull/CssPractice.jsx
//          src/compilator/HtmlCompiler.jsx
//  Qayta yig'ish:  node scripts/build-lms.mjs src/1-Modull/CssPractice.jsx
...
var LESSON_META = { lessonId: "css-practice-portfolio-v3", lessonTitle: { uz: "CSS Praktika — Portfolioni bezaymiz", ru: "CSS практика — стилизуем портфолио" } };
var HW_TOKENS = [
...
var Pr = ({ children }) => <span style={{ color: CODE.punct }}>{children}</span>;
...
var RECAPS = {
  ...
  4: {
    title: { uz: "Rang: color va background-color", ru: "Цвет: color и background-color" },
    cards: [
      {
        ic: "🎨",
        h: { uz: "color — harflar rangi", ru: "color — цвет букв" },
      ...
      {
        ic: "🖌️",
        h: { uz: "background-color — orqa fon", ru: "background-color — задний фон" },
      ...
      {
        ic: "⚠️",
        h: { uz: "Ikkisini chalkashtirmang", ru: "Не путайте их" },
  ...
  6: {
    title: { uz: "Matn joylashuvi: text-align", ru: "Расположение текста: text-align" },
    cards: [
      {
        ic: "🎯",
        h: { uz: "text-align — matnni joylaydi", ru: "text-align — располагает текст" },
      ...
      {
        ic: "📐",
        h: { uz: "markazga qo'yish", ru: "поставить по центру" },
      ...
      {
        ic: "🚫",
        h: { uz: "bo'shliq emas, joylashuv", ru: "не отступ, а расположение" },
  ...
  8: {
    title: { uz: "Flex: elementlarni qatorga tizish", ru: "Flex: выстроить элементы в ряд" },
    cards: [
      {
        ic: "📚",
        h: { uz: "display: flex — bir qatorga", ru: "display: flex — в один ряд" },
      ...
      {
        ic: "↔️",
        h: { uz: "gap — oralarini ochadi", ru: "gap — раздвигает их" },
      ...
      {
        ic: "☝️",
        h: { uz: "block aksincha tizadi", ru: "block делает наоборот" },
  ...
  11: {
    title: { uz: "Bo'shliq: padding va margin", ru: "Отступы: padding и margin" },
    cards: [
      {
        ic: "📥",
      ...
      {
        ic: "🧩",
        h: { uz: "ichki-tashqini ajrating", ru: "различайте внутри и снаружи" },
  ...
  14: {
    title: { uz: "Flex markaz: justify-content", ru: "Центр во flex: justify-content" },
    cards: [
      {
        ic: "➡️",
        h: { uz: "justify-content — qator bo'ylab", ru: "justify-content — вдоль ряда" },
      ...
      {
        ic: "🎯",
        h: { uz: "center — o'rtaga suradi", ru: "center — сдвигает в середину" },
      ...
      {
        ic: "⬇️",
        h: { uz: "align-items — boshqa o'q", ru: "align-items — другая ось" },
...
function RecapOverlay({ screenIdx, onClose }) {
  ...
  return <div className="rc-overlay">
      <div className="rc-head">
        <span className="rc-tag">📖 {tr3({ uz: "Qayta tushuntirish", ru: "Объясняем ещё раз" })}</span>
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr3({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlash tavsiya etiladi.</>, ru: <>⚠️ Только <b>{pct}%</b> верно — тема осталась непонятой классом. Перед продолжением рекомендуем короткое повторение.</> })}</p>
              {onOpenRecap && <button className="rc-open" onClick={onOpenRecap}>📖 {tr3({ uz: "Qayta tushuntirish", ru: "Объяснить ещё раз" })} — {tr3(RECAPS[screenIdx]?.title)}</button>}
            </>}
            {level === "maybe" && <>
            ...
            {level === "few" && <>
              ...
              {onOpenRecap && <button className="rc-open soft" onClick={onOpenRecap}>📖 {tr3({ uz: "Qayta tushuntirish", ru: "Объяснить ещё раз" })} — {tr3(RECAPS[screenIdx]?.title)}</button>}
            </>}
          </div>;
  })()}
      {waitingPl.length > 0 && answered > 0 && <div className="mstats-waitrow">
      ...
      {reveal && struggling && <p className="mstats-warn">{tr3({ uz: "⚠️ Ko'pchilik xato qildi — bu mavzu tushunarsiz bo'lgan ko'rinadi. Qayta tushuntirish tavsiya etiladi.", ru: "⚠️ Большинство ошиблись — похоже, тема осталась непонятой. Рекомендуем объяснить ещё раз." })}</p>}
    ...
    return <button key={i} className={cls} disabled={solved || isMentorLive} onClick={() => pick(i)} style={{ padding: picked !== null ? "clamp(9px,1.3vw,12px) clamp(15px,2.2vw,20px)" : "clamp(13px,1.9vw,17px) clamp(15px,2.2vw,20px)", fontSize: "clamp(15px,1.85vw,17px)", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <FeedbackBlock show={isMentorLive ? mReveal : picked !== null} isCorrect={isMentorLive ? true : solved && !wrongLocked} neutral={waiting}>
          <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: waiting ? T.blue : isMentorLive || solved && !wrongLocked ? T.success : T.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>
            {isMentorLive ? tr3({ uz: `✓ To'g'ri javob: ${String.fromCharCode(65 + correctIdx)} — ${tr3(options[correctIdx])}`, ru: `✓ Верный ответ: ${String.fromCharCode(65 + correctIdx)} — ${tr3(options[correctIdx])}` }) : waiting ? tr3({ uz: "📨 Javobingiz qabul qilindi", ru: "📨 Ваш ответ принят" }) : wrongLocked ? tr3({ uz: `To'g'ri javob: ${String.fromCharCode(65 + correctIdx)} — ${tr3(options[correctIdx])}`, ru: `Верный ответ: ${String.fromCharCode(65 + correctIdx)} — ${tr3(options[correctIdx])}` }) : solved ? tr3({ uz: "To'g'ri", ru: "Верно" }) : tr3({ uz: "Qaytadan urinib ko'ring", ru: "Попробуйте ещё раз" })}
          </p>
          <p className="body" style={{ margin: 0 }}>
            {isMentorLive ? fmtCode(tr3(explainCorrect)) : waiting ? tr3({ uz: "Hozir to'g'ri javobni bilib olasiz.", ru: "Скоро узнаете правильный ответ." }) : wrongLocked ? fmtCode(tr3(explainWrong[picked] ?? explainWrong.default)) : solved ? fmtCode(tr3(explainCorrect)) : fmtCode(tr3(explainWrong[picked] ?? explainWrong.default))}
          </p>
          {
...
var usePortfolio = (answers) => ({
  ...
  role: (answers?.[3]?.role || "").trim() || tr3({ uz: "Frontend dasturchi", ru: "Фронтенд-разработчик" })
});
var MOD = { page: "pf-page", center: "pf-center", head: "pf-head", nav: "pf-nav", about: "pf-about", list: "pf-list", btn: "pf-btn" };
var StyledSite = ({ name = "Aziza Karimova", role = tr3({ uz: "Frontend dasturchi", ru: "Фронтенд-разработчик" }), parts, on = [], nameColor }) => {
  const has = (p) => parts.includes(p);
  const cls = "pf " + on.map((k) => MOD[k]).filter(Boolean).join(" ");
  return <div className={cls}>
      <div className="pf-inner">
        {has("header") && <header className="pf-header">
            <h1 style={nameColor ? { color: nameColor } : void 0}>{name}</h1>
            <p>{role}</p>
            <nav><a>{tr3({ uz: "Men haqimda", ru: "Обо мне" })}</a><a>{tr3({ uz: "Loyihalar", ru: "Проекты" })}</a><a>{tr3({ uz: "Aloqa", ru: "Контакты" })}</a></nav>
          </header>}
        {has("about") && <section className="pf-sec-about">
            <h2>{tr3({ uz: "Men haqimda", ru: "Обо мне" })}</h2>
            <p>{tr3({ uz: "Salom! Men veb-saytlar yasashni o'rganayotgan dasturchiman.", ru: "Привет! Я разработчик, который учится делать сайты." })}</p>
          </section>}
        {has("projects") && <section className="pf-sec-projects">
        ...
        {has("contact") && <section className="pf-sec-contact">
            <h2>{tr3({ uz: "Aloqa", ru: "Контакты" })}</h2>
            <p>{tr3({ uz: "Men bilan bog'lanish:", ru: "Связаться со мной:" })}</p>
            <a className="pf-email">{tr3({ uz: "Menga yozing ✉", ru: "Напишите мне ✉" })}</a>
          </section>}
        {has("footer") && <footer><p>© 2026 {name}</p></footer>}
      </div>
    </div>;
};
var useStylePick = (correct, storedAnswer, onAnswer, screen) => {
...
var Chips = ({ options, correct, picked, solved, pick }) => <div className="tagpick fade-up delay-3">
    {options.map((o) => {
  const isC = o === correct;
  let cls = "chip";
...
var Screen1 = ({ screen, answers, onNext, onPrev }) => {
  ...
  const scrollDown = (e) => {
    const sc = e.currentTarget.closest(".stage-content");
  ...
  const StepsBlock = <Col>
      ...
      <ol className="roadmap">{STEPS.map((s, i) => <li key={i} className="step-card fade-up" style={{ animationDelay: `${0.08 + i * 0.05}s` }}><span className="step-num">{String(i + 1).padStart(2, "0")}</span><span className="step-body"><span className="step-text">{tr3(s.text)}</span><span className="step-tag">{s.tag}</span></span></li>)}</ol>
    </Col>;
  return <Stage eyebrow={tr3({ uz: "Reja", ru: "План" })} screen={screen} audioState={audio} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Bezashni boshlaymiz →", ru: "Начинаем оформлять →" }} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Portfolioni qanday <span className="italic" style={{ color: T.accent }}>chiroyli</span> qilamiz?</>, ru: <>Как сделать портфолио <span className="italic" style={{ color: T.accent }}>красивым</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Portfolioni <b style={{ color: T.ink }}>6 qadamda</b> bezaymiz. Har qadamda saytingiz chiroyliroq bo'ladi. <b style={{ color: T.ink }}>Ishonasizmi?</b> Dars oxirida o'ngdagi sayt — o'zingizniki!</>, ru: <>Оформим портфолио <b style={{ color: T.ink }}>за 6 шагов</b>. С каждым шагом сайт будет всё красивее. <b style={{ color: T.ink }}>Поверите?</b> К концу урока сайт справа — ваш!</> })}</Mentor>
        {!isNarrow ? <div onClick={scrollDown}><Split>{PreviewBlock}{StepsBlock}</Split></div> : !showSteps ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)", cursor: "pointer" }} onClick={() => stepBtnRef.current && stepBtnRef.current.scrollIntoView({ behavior: "smooth", block: "center" })}>{PreviewBlock}<p className="small" style={{ margin: 0, color: T.ink2, textAlign: "center", fontStyle: "italic" }}>{tr3({ uz: <>👆 Saytni bosing — keyin "6 qadamni ko'rish"</>, ru: <>👆 Нажмите на сайт — затем «Показать 6 шагов»</> })}</p><button ref={stepBtnRef} className="btn" style={{ alignSelf: "flex-start" }} onClick={(e) => {
    e.stopPropagation();
    setShowSteps(true);
  }}>📋 {tr3({ uz: "6 qadamni ko'rish", ru: "Показать 6 шагов" })}</button></div> : <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}><button className="btn-ghost" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(false)}>↩ {tr3({ uz: "Tayyor saytni ko'rish", ru: "Посмотреть готовый сайт" })}</button>{StepsBlock}</div>}
      </div>
    </Stage>;
};
function DragDropOrder({ items, hints, onSolved }) {
  ...
  return <div className="dd fade-up">
      ...
      <div className="dd-pool">
        {pool.length === 0 && !solved && <span className="dd-pool-empty">{tr3({ uz: "Tartib xato — bo'lakni bosib qaytaring va qayta joylang", ru: "Порядок неверный — нажмите на блок, верните его и разместите заново" })}</span>}
        {pool.map((id) => <button key={id} className="dd-chip" onPointerDown={(e) => down(e, id, "pool")}>{byId[id].label}</button>)}
      </div>
      {solved && <div className="dd-done">{tr3({ uz: "✓ To'g'ri! CSS qoidasi aynan shu tartibda yoziladi.", ru: "✓ Верно! Правило CSS пишется именно в таком порядке." })}</div>}
      {wrong && !solved && <div className="dd-wrong">{tr3({ uz: "⚠️ Tartib xato — qayta joylang.", ru: "⚠️ Порядок неверный — разместите заново." })}</div>}
    </div>;
}
var RULE_PIECES = [
  { id: "sel", label: "h1" },
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [seen, setSeen] = useState4(() => new Set(storedAnswer ? ["sel", "prop", "val"] : []));
  const explored = seen.size === 3;
  const [assembled, setAssembled] = useState4(!!storedAnswer);
  const done = assembled;
  const clickPart = (k) => setSeen((s) => {
    if (s.has(k)) return s;
    const n = new Set(s);
    n.add(k);
    return n;
  ...
  const last = [...seen].pop();
  const navLabel = !explored ? tr3({ uz: `Uch qismni bosing (${seen.size}/3)`, ru: `Нажмите три части (${seen.size}/3)` }) : !assembled ? tr3({ uz: "Qoidani yig'ing", ru: "Соберите правило" }) : tr3({ uz: "Birinchi bezakni beramiz →", ru: "Даём первое оформление →" });
  return <Stage eyebrow={tr3({ uz: "CSS qoidasi", ru: "Правило CSS" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split split3">
          ...
          <div className="col">
            <div className="flow-label">{tr3({ uz: "Brauzerda — ismingiz (h1)", ru: "В браузере — ваше имя (h1)" })}</div>
            <Preview title="portfolio.html" minH={120}><StyledSite parts={["header"]} on={[]} nameColor={seen.has("val") || done ? T.accent : void 0} /></Preview>
            {last && !done && <div className="frame-soft fade-step"><p className="small mono" style={{ margin: "0 0 4px", fontWeight: 700, color: T.accent, textTransform: "uppercase", letterSpacing: "0.06em" }}>{tr3(PARTS[last].label)}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr3(PARTS[last].t)}</p></div>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Siz qoidani o'zingiz yig'dingiz: <span className="mono">{"h1 { color: #FF4F28; }"}</span> — "h1 ning rangini to'q sariq qil". Har CSS satri <b>nuqta-vergul</b> (;) bilan tugaydi.</>, ru: <>Вы собрали правило сами: <span className="mono">{"h1 { color: #FF4F28; }"}</span> — «сделай цвет h1 оранжевым». Каждая строка CSS заканчивается <b>точкой с запятой</b> (;).</> })}</p></div>}
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen3 = ({ screen, answers, storedAnswer, onAnswer, onNext, onPrev }) => {
  const pf = usePortfolio(answers);
  ...
  return <Stage eyebrow={tr3({ uz: "Bezak · Fon va shrift", ru: "Оформление · Фон и шрифт" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!solved} label={solved ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "To'g'ri xususiyatni tanlang", ru: "Выберите верное свойство" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Saytimizga qanday <span className="italic" style={{ color: T.accent }}>fon</span> beramiz?</>, ru: <>Какой <span className="italic" style={{ color: T.accent }}>фон</span> дадим нашему сайту?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Butun sahifaga <span className="mono">body</span> orqali fon rangi va shrift beramiz. Fon rangini beradigan xususiyat qaysi? Tanlang — bo'sh joy <span className="slot">?</span> to'ladi.</>, ru: <>Всему сайту через <span className="mono">body</span> дадим цвет фона и шрифт. Какое свойство задаёт цвет фона? Выберите — пустое место <span className="slot">?</span> заполнится.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            ...
            <p className="fld-label">{tr3({ uz: "Fon rangini qaysi xususiyat beradi?", ru: "Какое свойство задаёт цвет фона?" })}</p>
            ...
            <FeedbackBlock show={picked !== null} isCorrect={solved}>
              <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: solved ? T.success : T.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>{tr3(solved ? { uz: "To'g'ri", ru: "Верно" } : { uz: "Yana urinib ko'ring", ru: "Попробуйте ещё раз" })}</p>
          ...
          <div className="col">
            <div className="flow-label">{tr3(solved ? { uz: "CSS bilan", ru: "С CSS" } : { uz: "Hozir — bezaksiz", ru: "Сейчас — без оформления" })}</div>
...
var Screen4 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr3({ uz: "Mashq · 1-savol", ru: "Задание · вопрос 1" })}
  ...
  question={tr3({ uz: <><p className="eyebrow" style={{ color: T.accent }}>To'g'ri javobni tanlang</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Matnning o'zini — <span className="italic" style={{ color: T.accent }}>harflar rangini</span> — o'zgartiradigan xususiyat qaysi?</h2></>, ru: <><p className="eyebrow" style={{ color: T.accent }}>Выберите правильный ответ</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Какое свойство меняет сам текст — <span className="italic" style={{ color: T.accent }}>цвет букв</span>?</h2></> })}
...
var Screen5 = ({ screen, answers, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Bezak · Sarlavhalar", ru: "Оформление · Заголовки" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!solved} label={solved ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "To'g'ri qiymatni tanlang", ru: "Выберите верное значение" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Ismingiz qanday <span className="italic" style={{ color: T.accent }}>ko'zga tashlansin</span>?</>, ru: <>Как ваше имя должно <span className="italic" style={{ color: T.accent }}>бросаться в глаза</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Ismingiz — <span className="mono">h1</span> — eng avval ko'zga tashlanishi kerak. Buning uchun unga yorqin <b style={{ color: T.accent }}>rang</b> beramiz va <b style={{ color: T.ink }}>markazga</b> qo'yamiz. Matnni markazga qo'yadigan qiymat qaysi? Tanlang.</>, ru: <>Ваше имя — <span className="mono">h1</span> — должно бросаться в глаза первым. Для этого дадим ему яркий <b style={{ color: T.accent }}>цвет</b> и поставим <b style={{ color: T.ink }}>по центру</b>. Какое значение ставит текст по центру? Выберите.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            ...
            <p className="fld-label">{tr3({ uz: "Sarlavhani markazga qaysi qiymat qo'yadi?", ru: "Какое значение ставит заголовок по центру?" })}</p>
          ...
          <div className="col">
            <div className="flow-label">{tr3(solved ? { uz: "CSS bilan", ru: "С CSS" } : { uz: "Fon berildi — sarlavha hali oddiy", ru: "Фон задан — заголовок пока обычный" })}</div>
...
var Screen6 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr3({ uz: "Mashq · 2-savol", ru: "Задание · вопрос 2" })}
  ...
  question={tr3({ uz: <><p className="eyebrow" style={{ color: T.accent }}>To'g'ri javobni tanlang</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Matnni <span className="italic" style={{ color: T.accent }}>markazga</span> (yoki chapga/o'ngga) joylashtiradigan xususiyat qaysi?</h2></>, ru: <><p className="eyebrow" style={{ color: T.accent }}>Выберите правильный ответ</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Какое свойство располагает текст <span className="italic" style={{ color: T.accent }}>по центру</span> (или слева/справа)?</h2></> })}
  options={["margin", "padding", "color", "text-align"]}
  correctIdx={3}
      ...
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <div className="col">
            ...
            <p className="fld-label">{tr3({ uz: "Elementlarni bir qatorga qaysi qiymat tizadi?", ru: "Какое значение выстраивает элементы в один ряд?" })}</p>
          ...
          <div className="col">
            <div className="flow-label">{tr3(solved ? { uz: "CSS bilan", ru: "С CSS" } : { uz: "Menyu hali oddiy (ustma-ust)", ru: "Меню пока обычное (друг под другом)" })}</div>
...
/>;
var Screen9 = ({ screen, answers, storedAnswer, onAnswer, onNext, onPrev }) => {
  const pf = usePortfolio(answers);
  ...
  return <Stage eyebrow={tr3({ uz: "Bezak · Men haqimda", ru: "Оформление · Обо мне" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!solved} label={solved ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "To'g'ri xususiyatni tanlang", ru: "Выберите верное свойство" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>"Men haqimda"ni qanday <span className="italic" style={{ color: T.accent }}>joylashtiramiz</span>?</>, ru: <>Как <span className="italic" style={{ color: T.accent }}>расположим</span> раздел «Обо мне»?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>"Men haqimda" matnini <b style={{ color: T.ink }}>markazga</b> qo'yamiz va bo'limga <b style={{ color: T.ink }}>ichki bo'shliq</b> beramiz — qisilib turmasin, nafas olsin. Ichkaridagi bo'shliqni qaysi xususiyat beradi? Tanlang.</>, ru: <>Текст «Обо мне» поставим <b style={{ color: T.ink }}>по центру</b> и дадим разделу <b style={{ color: T.ink }}>внутренний отступ</b> — пусть не жмётся, а дышит. Какое свойство даёт отступ внутри? Выберите.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            ...
            <p className="fld-label">{tr3({ uz: "Bo'lim ichidagi bo'shliqni qaysi xususiyat beradi?", ru: "Какое свойство даёт отступ внутри раздела?" })}</p>
          ...
          <div className="col">
            <div className="flow-label">{tr3(solved ? { uz: "CSS bilan — markazda, bo'shliq bilan", ru: "С CSS — по центру, с отступом" } : { uz: "Bo'lim hali qisilib turibdi", ru: "Раздел пока сжат" })}</div>
...
var Screen10 = ({ screen, answers, storedAnswer, onAnswer, onNext, onPrev }) => {
  const pf = usePortfolio(answers);
      ...
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          <div className="col">
            ...
            <p className="fld-label">{tr3({ uz: "Kartalar orasidagi bo'shliqni qaysi xususiyat beradi?", ru: "Какое свойство даёт отступ между карточками?" })}</p>
          ...
          <div className="col">
            <div className="flow-label">{tr3(solved ? { uz: "CSS bilan — kartalar", ru: "С CSS — карточки" } : { uz: "Ro'yxat hali nuqtali", ru: "Список пока с точками" })}</div>
...
var Screen11 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr3({ uz: "Mashq · 4-savol", ru: "Задание · вопрос 4" })}
  ...
  question={tr3({ uz: <><p className="eyebrow" style={{ color: T.accent }}>To'g'ri javobni tanlang</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Kontent bilan element cheti orasidagi <span className="italic" style={{ color: T.accent }}>ichki bo'shliq</span> qaysi xususiyat?</h2></>, ru: <><p className="eyebrow" style={{ color: T.accent }}>Выберите правильный ответ</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Какое свойство задаёт <span className="italic" style={{ color: T.accent }}>внутренний отступ</span> между контентом и краем элемента?</h2></> })}
  options={["margin", "padding", "gap", "text-align"]}
  correctIdx={1}
...
var Screen12 = ({ screen, answers, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Bezak · Aloqa tugmasi", ru: "Оформление · Кнопка контакта" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!solved} label={solved ? { uz: "Saytni yig'amiz →", ru: "Собираем сайт →" } : { uz: "To'g'ri xususiyatni tanlang", ru: "Выберите верное свойство" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Havolani, tugmaga <span className="italic" style={{ color: T.accent }}>qanday</span> aylantiramiz?</>, ru: <><span className="italic" style={{ color: T.accent }}>Как</span> превратить ссылку в кнопку?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Email havolasiga <b style={{ color: T.accent }}>to'q sariq fon</b> (<span className="mono">background-color</span>) va <span className="mono">padding</span> berib tugma qilamiz. Endi matn ko'rinishi uchun <b style={{ color: T.ink }}>oq</b> bo'lishi kerak — matn rangini qaysi xususiyat beradi?</>, ru: <>Дадим email-ссылке <b style={{ color: T.accent }}>оранжевый фон</b> (<span className="mono">background-color</span>) и <span className="mono">padding</span> — получится кнопка. Теперь текст должен стать <b style={{ color: T.ink }}>белым</b>, чтобы читаться — какое свойство задаёт цвет текста?</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            ...
            <p className="fld-label">{tr3({ uz: "Tugma matnini oq qiladigan xususiyat?", ru: "Какое свойство сделает текст кнопки белым?" })}</p>
          ...
          <div className="col">
            <div className="flow-label">{tr3(solved ? { uz: "CSS bilan — tugma!", ru: "С CSS — кнопка!" } : { uz: "Hozir — oddiy havola", ru: "Сейчас — обычная ссылка" })}</div>
...
var Screen13 = ({ screen, answers, storedAnswer, onAnswer, onNext, onPrev }) => {
  const pf = usePortfolio(answers);
  ...
  const pick2 = (o) => {
    pick(o);
    if (isNarrow) setTimeout(() => {
      if (previewRef.current) previewRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
    }, 140);
  };
  return <Stage eyebrow={tr3({ uz: "Bezak · Markazlash", ru: "Оформление · Центрирование" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!solved} label={solved ? { uz: "Saytni yig'amiz →", ru: "Собираем сайт →" } : { uz: "To'g'ri qiymatni tanlang", ru: "Выберите верное значение" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Butun saytni qanday <span className="italic" style={{ color: T.accent }}>markazga</span> qo'yamiz?</>, ru: <>Как поставить весь сайт <span className="italic" style={{ color: T.accent }}>по центру</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Saytni oq kartaga solib, <span className="mono">max-width</span> bilan kengligini cheklaymiz, so'ng chap-o'ng tashqi bo'shliqni teng bo'lib markazga qo'yamiz. <span className="mono">margin</span> ning qaysi qiymati markazlaydi? Tanlang.</>, ru: <>Поместим сайт в белую карточку, ограничим ширину через <span className="mono">max-width</span>, затем поделим левый и правый внешние отступы поровну — сайт встанет по центру. Какое значение <span className="mono">margin</span> центрирует? Выберите.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            ...
            <p className="fld-label">{tr3({ uz: "margin ning qaysi qiymati markazlaydi?", ru: "Какое значение margin центрирует?" })}</p>
          ...
          <div className="col">
            <div className="flow-label">{tr3(solved ? { uz: "CSS bilan — markazda!", ru: "С CSS — по центру!" } : { uz: "Sayt hali butun enni egallaydi", ru: "Сайт пока занимает всю ширину" })}</div>
...
var Screen14 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr3({ uz: "Mashq · 5-savol", ru: "Задание · вопрос 5" })}
  ...
  question={tr3({ uz: <><p className="eyebrow" style={{ color: T.accent }}>To'g'ri javobni tanlang</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Menyudagi (flex) havolalarni qator bo'ylab <span className="italic" style={{ color: T.accent }}>gorizontal markazga</span> joylash uchun qaysi xususiyat?</h2></>, ru: <><p className="eyebrow" style={{ color: T.accent }}>Выберите правильный ответ</p><h2 className="title h-ask" style={{ marginTop: 8 }}>Какое свойство ставит ссылки меню (flex) в <span className="italic" style={{ color: T.accent }}>горизонтальный центр</span> вдоль ряда?</h2></> })}
  options={["align-items: center", "text-align: center", "gap: 10px", "justify-content: center"]}
  correctIdx={3}
...
var Screen15 = ({ screen, answers, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const LABELS = [
    { k: "page", l: { uz: "Fon va shrift", ru: "Фон и шрифт" } },
    { k: "head", l: { uz: "Sarlavhalar", ru: "Заголовки" } },
    { k: "nav", l: { uz: "Menyu (flex)", ru: "Меню (flex)" } },
    { k: "about", l: { uz: "Men haqimda", ru: "Обо мне" } },
    { k: "list", l: { uz: "Loyiha kartalari", ru: "Карточки проектов" } },
    { k: "btn", l: { uz: "Aloqa tugmasi", ru: "Кнопка контакта" } },
    { k: "center", l: { uz: "Markazlash", ru: "Центрирование" } }
  ];
  const isNarrow = useIsMobile(768);
  const [on, setOn] = useState4(storedAnswer ? ALL : []);
  ...
  const followDown = (last) => {
    requestAnimationFrame(() => {
      ...
      if (last) {
        const sc = endRef.current.closest(".stage-content");
        if (sc) sc.scrollTo({ top: sc.scrollHeight, behavior: "smooth" });
        return;
      }
  ...
  const applyOne = (k) => {
    if (running) return;
    setOn((prev) => prev.includes(k) ? prev : [...prev, k]);
    followDown(false);
  };
  const replay = () => {
    ...
    const tick = (i) => {
      setOn(ALL.slice(0, i));
      const last = i >= ALL.length;
      followDown(last);
      if (i < ALL.length) timer.current = setTimeout(() => tick(i + 1), 430);
      else setRunning(false);
    };
    timer.current = setTimeout(() => tick(1), 250);
  };
  return <Stage eyebrow={tr3({ uz: "Saytni yig'ish", ru: "Сборка сайта" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Yakuniy mashq →", ru: "Финальное задание →" } : tr3({ uz: `O'zingiz bezang (${on.length}/${ALL.length})`, ru: `Оформите сами (${on.length}/${ALL.length})` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Hammasini birlashtirib, <span className="italic" style={{ color: T.accent }}>saytga</span> yig'amizmi?</>, ru: <>Соберём всё вместе в <span className="italic" style={{ color: T.accent }}>сайт</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Endi <b style={{ color: T.ink }}>o'zingiz</b> bezang! Pastdagi bezaklarni birma-bir bosing — har bosishda o'sha qatlam saytga qo'shilib boradi. Hammasi qo'shilsa, portfolioingiz tayyor!</>, ru: <>Теперь оформляйте <b style={{ color: T.ink }}>сами</b>! Нажимайте на оформления ниже по одному — с каждым нажатием этот слой добавляется на сайт. Добавите все — портфолио готово!</> })}</Mentor>
        <Zoomable>
        <div className="split" style={{ alignItems: "stretch" }}>
          <div className="col">
            {!done ? <p className="small" style={{ margin: 0, color: T.ink2, fontStyle: "italic" }}>👇 {tr3({ uz: "Har bir bezakni bosing — o'ngdagi saytga qo'shiladi", ru: "Нажимайте на каждое оформление — оно добавится на сайт справа" })}</p> : <button className="btn" style={{ alignSelf: "flex-start" }} onClick={replay} disabled={running}>{tr3(running ? { uz: "Bezatilmoqda…", ru: "Оформляем…" } : { uz: "↻ Yana jonli ko'rish", ru: "↻ Посмотреть вживую ещё раз" })}</button>}
            <div className="asm-list fade-up delay-2">
              {LABELS.map((b, i) => {
    const applied = on.includes(b.k);
    return <button key={b.k} type="button" className={`asm-row ${applied ? "on" : ""}`} onClick={() => applyOne(b.k)} disabled={applied || running} style={{ border: "none", cursor: applied ? "default" : "pointer", textAlign: "left", width: "100%" }}><span className="asm-ic">{applied ? "✓" : i + 1}</span><span className="small" style={{ fontWeight: 600 }}>{tr3(b.l)}</span></button>;
  })}
            </div>
            {done && <div className="frame-success fade-step" style={{ marginTop: "auto" }}><p className="small mono" style={{ margin: "0 0 4px", fontWeight: 600, color: T.success, textTransform: "uppercase", letterSpacing: "0.08em" }}>✓ {tr3({ uz: "Portfolio tayyor!", ru: "Портфолио готово!" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Bir xil HTML — endi <b>chiroyli, markazlashgan va professional</b>. Siz o'zingiz bezadingiz — mana CSS ning kuchi!</>, ru: <>Тот же HTML — теперь <b>красивый, центрированный и профессиональный</b>. Вы оформили его сами — вот сила CSS!</> })}</p></div>}
          </div>
          <div className="col">
...
var Screen16 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  useEffect5(() => {
    if (valid && !okRef.current) {
      ...
      if (!audio.muted) setTimeout(() => {
        const e = getAudioEngine();
        if (e && !audio.muted) e.pushOneOff("Zo'r! Siz haqiqiy CSS qoidasini o'z qo'lingiz bilan yozdingiz.");
      }, 300);
    }
  }, [valid]);
  return <Stage eyebrow={tr3({ uz: "Yakuniy · amaliy", ru: "Финал · практика" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={!valid} label={valid ? { uz: "Yakunlash →", ru: "Завершить →" } : { uz: "CSS qoidasini yozing", ru: "Напишите правило CSS" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>o'zingiz</span> CSS yozing.</>, ru: <>Теперь напишите CSS <span className="italic" style={{ color: T.accent }}>сами</span>.</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Mashq vaqti! Ismingizga — <span className="mono">h1</span> ga — <b style={{ color: T.ink }}>rang bering</b>. To'liq qoidani <b style={{ color: T.ink }}>o'zingiz qo'lda yozing</b>: <span className="mono">{"h1 { color: tomato; }"}</span>. To'g'ri yozsangiz, pastdagi belgilar yashil bo'lib, ism o'sha rangga bo'yaladi!</>, ru: <>Время практики! Дайте вашему имени — <span className="mono">h1</span> — <b style={{ color: T.ink }}>цвет</b>. Полное правило <b style={{ color: T.ink }}>напишите сами, от руки</b>: <span className="mono">{"h1 { color: tomato; }"}</span>. Напишете верно — метки внизу станут зелёными, а имя окрасится в этот цвет!</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            <input ref={inputRef} className="text-input mono" value={val} placeholder={tr3({ uz: "bu yerga yozing: h1 { color: tomato; }", ru: "пишите здесь: h1 { color: tomato; }" })} onChange={(e) => setVal(e.target.value)} spellCheck={false} autoFocus />
            <p className="small" style={{ margin: 0, color: T.ink2, fontStyle: "italic" }}>✍️ {tr3({ uz: "Klaviaturadan o'zingiz yozing — yordamchi tugmalarsiz.", ru: "Печатайте сами с клавиатуры — без кнопок-помощников." })}</p>
          ...
          <div className="col">
            <div className="flow-label">{tr3({ uz: "Brauzerda — ismingiz", ru: "В браузере — ваше имя" })}</div>
            <Preview title="portfolio.html" minH={130}><StyledSite parts={["header"]} on={["page", "center"]} nameColor={valid && colorVal ? colorVal : void 0} /></Preview>
            {valid && <div className="frame-success fade-step"><p className="small mono" style={{ margin: "0 0 4px", fontWeight: 600, color: T.success, textTransform: "uppercase", letterSpacing: "0.08em" }}>✓ {tr3({ uz: "To'g'ri yozildi", ru: "Написано верно" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: "Siz haqiqiy CSS qoidasini qo'lda yozdingiz — selektor, xususiyat, qiymat va nuqta-vergul bilan!", ru: "Вы написали настоящее правило CSS от руки — с селектором, свойством, значением и точкой с запятой!" })}</p></div>}
          </div>
        </div>
        </Zoomable>
      </div>
...
var FC_VOCAB = /* @__PURE__ */ new Set(["let", "const", "var", "string", "number", "boolean", "true", "false", "null", "undefined", "function", "return", "for", "while", "if", "else", "background-color"]);
var fcIsCode = (s) => {
  ...
  return <div className="fc fade-up">
      ...
      <div className="fc-cardwrap">
        ...
        <div className={`fc-card ${flipped ? "flip" : ""}`} onClick={() => !flipped && !exiting && setFlipped(true)} role="button" tabIndex={0}>
          <div className="fc-face fc-front"><span className="fc-q">{tr3(card.front)}</span><span className="fc-cue">{tr3({ uz: <>Javobni o'ylang 🤔 <span className="fc-tap">bosing</span></>, ru: <>Подумайте над ответом 🤔 <span className="fc-tap">нажмите</span></> })}</span></div>
...
var Screen17 = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Tayyor", ru: "Готово" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Tugatish", ru: "Завершить" })} ✓</button></>}>
    ...
    hint={studentWait ? tr3({ uz: "⏳ Mentorni kuting", ru: "⏳ Ждите ментора" }) : studentSolo ? tr3({ uz: "📖 Testni o'zim ishlash →", ru: "📖 Пройти тест самому →" }) : studentLive ? quizSt === "done" ? tr3({ uz: "🏆 Natijalarni ko'rish →", ru: "🏆 Посмотреть результаты →" }) : tr3({ uz: "🔥 Testga kirish →", ru: "🔥 Войти в тест →" }) : isMentorL ? quizSt === "off" ? tr3({ uz: "⚡ Testni ochish →", ru: "⚡ Открыть тест →" }) : tr3({ uz: "⚡ Davom ettirish →", ru: "⚡ Продолжить →" }) : tr3({ uz: "⚡ Testni ishlash →", ru: "⚡ Пройти тест →" })}
  />
        </div>
        {arena && <QuizArena live={_live || { mode: "self" }} startSolo={arenaSolo} onClose={() => setArena(false)} />}
        <div className="card fade-up d3"><div className="card-lbl" style={{ color: T.success }}><span className="tick" style={{ width: 16, height: 16, borderRadius: "50%", background: T.success, color: "#fff", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 10 }}>✓</span> {tr3({ uz: "Endi siz bezay olasiz", ru: "Теперь вы умеете оформлять" })}</div><ul className="recap">{RECAP.map((r, i) => <li key={i} style={{ animationDelay: `${0.3 + i * 0.06}s` }}><span className="ck">✓</span><span>{tr3(r)}</span></li>)}</ul></div>
  ...
  return <div className="qz-arena">
    ...
    /* QUTQARUV: jonli dars tugadi — o'quvchi osilib qolmaydi, mashq rejimida davom etadi */
      ...
      {classEnded && isStudent && !solo && phase !== "done" && <div className="qz-endnote fade-step">
          <span>⚠️ {tr3({ uz: "Jonli dars yakunlandi — testni o'zingiz davom ettiring:", ru: "Живой урок завершён — продолжайте тест сами:" })}</span>
    ...
    return <div key={i} className={`qz-tile rv ${win ? "win" : "lose"} ${pickedThis ? "picked" : ""}`} style={{ background: QUIZ_COLORS[i] }}>
          ...
          {!isMentor && <div className={`qz-res ${my?.correct ? "good" : "bad"}`}>
              {my?.correct ? <><span className="qz-res-pts">+{myPtsFor(qi)}</span><span className="qz-res-t">{tr3({ uz: "ball", ru: "баллов" })}{streakUpTo(qi) >= 2 ? ` · 🔥 x${streakUpTo(qi)} streak` : ""}</span></> : <span className="qz-res-t">{tr3(my ? { uz: "Adashdingiz — 0 ball. Keyingisida olasiz! 💪", ru: "Ошибка — 0 баллов. Возьмёте на следующем! 💪" } : { uz: "Vaqt tugadi — 0 ball. Tezroq bo'ling! ⏱", ru: "Время вышло — 0 баллов. Быстрее! ⏱" })}</span>}
          ...
          {isMentor && <button className="qz-btn big" onClick={() => lastQ ? ctrl("done", qi) : ctrl("q", qi + 1)}>{tr3(lastQ ? { uz: "🏁 G'oliblarni e'lon qilish", ru: "🏁 Объявить победителей" } : { uz: "Keyingi savol →", ru: "Следующий вопрос →" })}</button>}
          {solo && <button className="qz-btn big" onClick={soloNext}>{tr3(lastQ ? { uz: "🏁 Natijani ko'rish", ru: "🏁 Посмотреть результат" } : { uz: "Keyingi →", ru: "Дальше →" })}</button>}
        </div>}
    ...
    /* ===== YAKUN — PODIUM ===== */
      ...
      {phase === "done" && <div className="qz-view fade-step">
          ...
          {solo ? <div className="qz-solo-res">
              ...
              <p className="qz-sub">{tr3({ uz: "ball", ru: "баллов" })} · {soloScore.ok}/{QUIZ_BANK.length} {tr3({ uz: "to'g'ri", ru: "верно" })}{soloScore.maxStreak >= 2 ? tr3({ uz: ` · eng uzun streak 🔥x${soloScore.maxStreak}`, ru: ` · самая длинная серия 🔥x${soloScore.maxStreak}` }) : ""}</p>
    ...
    return <div key={rank} className={`qz-pod-col p${rank + 1} ${b && b.id === live.playerId ? "me" : ""}`}>
                      ...
                      {b && <span className="qz-pod-pts">{tr3({ uz: `${b.pts} ball`, ru: `${b.pts} баллов` })} · {b.ok}/{QUIZ_BANK.length}</span>}
...
var ACH_TRIGGERS = { s4: "color", s8: "layout", s11: "spacer" };
  ...
  return <div className="mp-overlay">
      <div className="mp-card">
        ...
        {data.players === null ? <p className="mstats-wait">{tr3({ uz: "Ulanish…", ru: "Подключение…" })}</p> : <div className="mstats" style={{ marginTop: 2 }}>
            ...
            {total === 0 && <p className="mstats-wait">{tr3({ uz: "Hali o'quvchi qo'shilmagan — ular praktikani boshlashi bilan bu yerda ✓ chiqadi…", ru: "Пока никто не подключился — как только ученики начнут практику, здесь появятся ✓…" })}</p>}
          </div>}
        <div className="mp-actions">
          <button className="mp-demo" onClick={() => setView("demo")}>🖊 {tr3({ uz: "Doskada yozib ko'rsatish", ru: "Показать, написав на доске" })}</button>
          <button className="mp-next" onClick={onClose}>{tr3({ uz: "Keyingi mavzuga", ru: "К следующей теме" })} →</button>
        </div>
        <p className="mp-tip">💡 {tr3({ uz: "Ko'pchilik tugatgach, aynan shu mashqni doskada birga yozing — shunda o'quvchilar o'zini tekshiradi va mavzu mustahkamlanadi.", ru: "Когда закончит большинство, напишите это же задание на доске вместе — ученики проверят себя, и тема закрепится." })}</p>
      </div>
    </div>;
}
var TASK_HEADER = {
  ...
  requirements: [
    { id: "color", label: { uz: "h1 — color (rang)", ru: "h1 — color (цвет)" }, check: checks.cssProp("h1", "color", { uz: "`h1 { color: #FF4F28; }` yozing — sarlavhaga rang", ru: "напишите `h1 { color: #FF4F28; }` — цвет заголовку" }) },
    { id: "center", label: "h1 — text-align: center", check: checks.cssValue("h1", "text-align", "center", { uz: "`h1 { text-align: center; }` — markazga", ru: "`h1 { text-align: center; }` — по центру" }) }
  ]
};
var TASK_MENU = {
  ...
  requirements: [
    { id: "flex", label: "nav — display: flex", check: checks.cssValue("nav", "display", "flex", { uz: "`nav { display: flex; }` — qatorga tizadi", ru: "`nav { display: flex; }` — выстраивает в ряд" }) },
    { id: "gap", label: { uz: "nav — gap (oralari)", ru: "nav — gap (промежутки)" }, check: checks.cssProp("nav", "gap", { uz: "`nav { gap: 14px; }` qo'shing — havolalar orasi", ru: "добавьте `nav { gap: 14px; }` — промежуток между ссылками" }) }
  ]
};
var TASK_BUTTON = {
  ...
  requirements: [
    { id: "bg", label: { uz: ".btn — background-color (fon)", ru: ".btn — background-color (фон)" }, check: checks.cssProp(".btn", "background-color", { uz: "`.btn { background-color: #FF4F28; }` — fon rangi", ru: "`.btn { background-color: #FF4F28; }` — цвет фона" }) },
    { id: "pad", label: { uz: ".btn — padding (ichki bo'shliq)", ru: ".btn — padding (внутренний отступ)" }, check: checks.cssProp(".btn", "padding", { uz: "`.btn { padding: 10px 20px; }` — ichki bo'shliq", ru: "`.btn { padding: 10px 20px; }` — внутренний отступ" }) }
  ]
};
var TASK_HW = {
...
<a class="btn">Menga yozing</a>`, ru: '<h1>Моё портфолио</h1>\n<p>Напишите здесь фразу о себе.</p>\n<a class="btn">Напишите мне</a>' } },
  ...
  requirements: [
    { id: "bg", label: { uz: "body — background-color (sahifa foni)", ru: "body — background-color (фон страницы)" }, check: checks.cssProp("body", "background-color", { uz: "`body { background-color: ...; }` — o'zingizga yoqqan fon rangi", ru: "Задайте свой фон: `body { background-color: ...; }`" }) },
    { id: "h1c", label: { uz: "h1 — color (sarlavha rangi)", ru: "h1 — color (цвет заголовка)" }, check: checks.cssProp("h1", "color", { uz: "`h1 { color: ...; }` — sarlavhaga rang tanlang", ru: "Выберите цвет заголовка: `h1 { color: ...; }`" }) },
    { id: "btnbg", label: { uz: ".btn — background-color (tugma foni)", ru: ".btn — background-color (фон кнопки)" }, check: checks.cssProp(".btn", "background-color", { uz: "`.btn { background-color: ...; }` — tugma fon rangi", ru: "Фон кнопки: `.btn { background-color: ...; }`" }) },
    { id: "btnc", label: { uz: ".btn — color (tugma matni rangi)", ru: ".btn — color (цвет текста кнопки)" }, check: checks.cssProp(".btn", "color", { uz: "`.btn { color: ...; }` — fon bilan o'qilishi oson bo'lsin", ru: "`.btn { color: ...; }` — чтобы читалось на фоне" }) },
    { id: "btnp", label: { uz: ".btn — padding (tugma ichki bo'shlig'i)", ru: ".btn — padding (внутренний отступ кнопки)" }, check: checks.cssProp(".btn", "padding", { uz: "`.btn { padding: 10px 20px; }` — havola tugmaga aylanadi", ru: "`.btn { padding: 10px 20px; }` — ссылка станет кнопкой" }) }
  ]
};
var HOMEWORK = {
  type: "tex",
...
var ScreenPodium = ({ screen, answers, achievements, onNext, onPrev }) => {
  ...
  const Badges = () => <div className="card ach-coll fade-up d2">
      <div className="card-lbl" style={{ color: T.accent }}>🏅 {tr3({ uz: "Nishonlaringiz", ru: "Ваши награды" })} — {achN}/{achTotal}</div>
      <div className="ach-grid">
        {Object.entries(ACHIEVEMENTS).map(([id, a]) => {
    const got = !!(achievements && achievements.has(id));
    return <div key={id} className={`ach-badge ${got ? "got" : "locked"}`} title={tr3(a.desc)}>
...
var INLINE_KEYS = { s4: 0, s6: 3, s8: 2, s11: 1, s14: 3, s16: -1 };
function CssPractice({ lang: langProp, onFinished, onPractice, liveToken }) {
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        ...
        <AchMissCtx.Provider value={achMissVal}>
          <div className="lesson-root">
            {live.mode === "choosing" ? <LiveGate live={live} title={{ uz: "CSS Praktika", ru: "CSS практика" }} /> : <>
