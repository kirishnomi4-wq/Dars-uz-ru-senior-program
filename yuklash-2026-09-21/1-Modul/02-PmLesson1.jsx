...
//  Qayta yig'ish:  node scripts/build-lms.mjs src/1-Modull/PmLesson1.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/1-Modull/PmLesson1.jsx
import React3, { useState as useState4, useEffect as useEffect5, useRef as useRef4, useCallback as useCallback3, createContext as createContext2, useContext as useContext2, useMemo as useMemo2, useState, useEffect, useLayoutEffect, useRef, useMemo, useCallback, isValidElement } from "react";
    ...
    return <button key={i} className={cls} disabled={solved || isMentorLive} onClick={() => pick(i)} style={{ padding: picked !== null ? "clamp(9px,1.3vw,12px) clamp(15px,2.2vw,20px)" : "clamp(13px,1.9vw,17px) clamp(15px,2.2vw,20px)", fontSize: "clamp(15px,1.85vw,17px)", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <FeedbackBlock show={isMentorLive ? mReveal : picked !== null} isCorrect={isMentorLive ? true : solved && !wrongLocked} neutral={waiting}>
          <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: waiting ? T.blue : isMentorLive || solved && !wrongLocked ? T.success : T.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>
            {isMentorLive ? fmtCode(tr3({ uz: `✓ To'g'ri javob: ${String.fromCharCode(65 + correctIdx)} — ${uzOf(options[correctIdx])}`, ru: `✓ Верный ответ: ${String.fromCharCode(65 + correctIdx)} — ${options[correctIdx] && options[correctIdx].ru || uzOf(options[correctIdx])}` })) : waiting ? tr3({ uz: "📨 Javobingiz qabul qilindi", ru: "📨 Ваш ответ принят" }) : wrongLocked ? fmtCode(tr3({ uz: `To'g'ri javob: ${String.fromCharCode(65 + correctIdx)} — ${uzOf(options[correctIdx])}`, ru: `Верный ответ: ${String.fromCharCode(65 + correctIdx)} — ${options[correctIdx] && options[correctIdx].ru || uzOf(options[correctIdx])}` })) : solved ? tr3({ uz: "To'g'ri", ru: "Верно" }) : tr3({ uz: "Qaytadan urinib ko'ring", ru: "Попробуйте ещё раз" })}
          </p>
...
var MentorNote = ({ children }) => {
  ...
  const live = gate.live;
  if (!live || live.mode !== "mentor") return null;
  return <details className="mnote-d"><summary>{tr3({ uz: "📋 Eslatma — faqat sizga", ru: "📋 Заметка — только для вас" })}</summary><p>{tr3(children)}</p></details>;
};
...
var Mentor = ({ children }) => {
  ...
  return <div className={`mentor fade-up ${enabled ? "mentor-mob" : ""} ${collapsed ? "is-collapsed" : ""}`} onClick={collapsed ? expand : void 0} role={collapsed ? "button" : void 0}>
      ...
      <div className="mentor-col">
        ...
        <div className="mentor-msg body">{tr3(children)}</div>
      </div>
    </div>;
};
...
var SiteMock = ({ logo = "S", color = T.accent, name, headline, sub, rows, cta }) => {
  const nm = tr3(name) || tr3({ uz: "Sayt", ru: "Сайт" }), hl = tr3(headline), sb = tr3(sub), ct = tr3(cta);
  return <div className="pg-in" key={String(nm) + String(hl || "")}>
    <div className="site-header"><span className="site-brand"><span className="site-logo" style={{ background: color }}>{logo}</span><span className="site-name">{nm}</span></span><span className="site-nav"><span>{tr3({ uz: "Asosiy", ru: "Главная" })}</span><span>{tr3({ uz: "Haqida", ru: "О нас" })}</span></span></div>
    ...
    {rows && <div style={{ display: "flex", flexDirection: "column", gap: 8, margin: "0 0 13px" }}>{rows.map((r, i) => <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}><span style={{ width: 38, height: 28, borderRadius: 6, background: T.bg, flexShrink: 0, boxShadow: `inset 0 0 0 1px ${T.ink3}30` }} /><span style={{ fontFamily: G, fontSize: 13.5, color: T.ink }}>{tr3(r)}</span></div>)}</div>}
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={{ uz: "Kirish", ru: "Введение" }} screen={screen} navContent={<NavNext optionalLive turnBusy={picked === null} disabled={picked === null} label={picked === null ? { uz: "Avval javobingizni belgilang", ru: "Сначала отметьте свой ответ" } : { uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr3({ uz: <>Maktab yonidagi lavash do'koni sayt ochsa — unga <span className="italic" style={{ color: T.accent }}>kim kiradi</span>?</>, ru: <>Лавашная у школы открыла сайт — <span className="italic" style={{ color: T.accent }}>кто на него зайдёт</span>?</> })}</h1>
        ...
        <Split>
          <Col>
            <p className="flow-label">{tr3({ uz: "Eshik oldidan kim o'tadi", ru: "Кто проходит мимо двери" })}</p>
            <div className="hk-street fade-up delay-1">
              <span className="hk-flow" aria-hidden="true"><i /><i /><i /></span>
              <span className="hk-shop">{tr3({ uz: "🌯 Lavash do'koni", ru: "🌯 Лавашная" })}</span>
              {PEOPLE.map((o, i) => <div key={i} className={"hk-person" + (i === PEOPLE.length - 1 ? " away" : "")} style={{ animationDelay: 0.15 + i * 0.18 + "s" }}>
                  ...
                  <span className="hk-col"><b>{tr3(o.n)}</b><em>{tr3(o.s)}</em></span>
                </div>)}
            </div>
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr3({ uz: "Sizningcha, sayt kim uchun ochiladi?", ru: "Как вы думаете, для кого открывают сайт?" })}</p>
    ...
    return <button key={o.id} className={"hook-option " + (on ? "on" : "") + (voteTurn ? " turn-ring turn-wave w" + (i + 1) : "")} disabled={picked !== null} onClick={() => pick(o.id)}>
            ...
            {picked !== null && <p className="hook-ack fade-step">{tr3({ uz: "Javobingiz yozildi — sinfda ovozlar turlicha bo'ladi. Kimniki to'g'ri ekanini birozdan keyin birga bilib olamiz.", ru: "Ваш ответ записан — в классе голоса разойдутся. Чуть позже вместе узнаем, кто был прав." })}</p>}
          </Col>
        </Split>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const [step, setStep] = useState4(reduced ? DEMO_CARD.length + 1 : 0);
  ...
  return <Stage eyebrow={{ uz: "Maqsad", ru: "Цель" }} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Boshlaymiz →", ru: "Начнём →" }} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Dars oxirida <span className="italic" style={{ color: T.accent }}>kim uchun</span> sayt qilishni bilib olasiz</>, ru: <>К концу урока вы поймёте, <span className="italic" style={{ color: T.accent }}>для кого</span> делать сайт</> })}</h2></div>
        ...
        <Split>
          <Col>
            <p className="flow-label">{tr3({ uz: "Namuna: lavash do'koni kartasi", ru: "Образец: карточка лавашной" })}</p>
            <div className={"acard fade-up" + (done ? " ready" : "")}>
              {done && <span className="acard-stamp">{tr3({ uz: "KARTA TAYYOR ✓", ru: "КАРТОЧКА ГОТОВА ✓" })}</span>}
              <span className="acard-title">{tr3({ uz: "🌯 Maktab yonidagi lavash do'koni", ru: "🌯 Лавашная рядом со школой" })}</span>
              {DEMO_CARD.map((r, i) => <div key={i} className={"acard-row" + (step > i ? " on" : "")} style={{ borderLeftColor: step > i ? COL[r.c] : "transparent" }}>
                  <span className="mono acard-k" style={{ color: step > i ? COL[r.c] : T.ink3 }}>{tr3(r.k)}</span>
                  <span className="acard-v">{step > i ? tr3(r.v) : <em className="acard-wait">{tr3({ uz: "yozilmoqda…", ru: "пишется…" })}</em>}</span>
                </div>)}
            </div>
            <button className="btn-soft" style={{ alignSelf: "flex-start" }} onClick={play} disabled={!done && !reduced}>{done ? tr3({ uz: "↻ Yana ko'rish", ru: "↻ Посмотреть ещё раз" }) : tr3({ uz: "Yozilmoqda…", ru: "Пишется…" })}</button>
          </Col>
          <Col>
            <p className="flow-label">{tr3({ uz: "Bugungi yo'l", ru: "Сегодняшний путь" })}</p>
            <ol className="roadmap">
              {[{ uz: "Auditoriya nima ekanini bilamiz", ru: "Узнаем, что такое аудитория" }, { uz: "Mashhur saytlar kimga ishlashini ko'ramiz", ru: "Посмотрим, для кого работают известные сайты" }, { uz: "O'z kartangizni yozasiz", ru: "Напишете свою карточку" }, { uz: "Kartani sahifada jonli ko'rsatasiz", ru: "Покажете карточку живьём на странице" }].map((t, i) => <li key={i} className="step-card fade-up" style={{ animationDelay: 0.08 + i * 0.06 + "s" }}>
                  ...
                  <span className="step-body"><span className="step-text">{tr3(t)}</span></span>
                </li>)}
            </ol>
          </Col>
        </Split>
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={{ uz: "Har sayt — o'z odamlari", ru: "У каждого сайта — свои люди" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: seen.size + "/" + KEYS.length + " saytni oching", ru: "Откройте сайты: " + seen.size + "/" + KEYS.length }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Har bir sayt aniq <span className="italic" style={{ color: T.accent }}>kimlar uchun</span> qilingan?</>, ru: <>А каждый сайт сделан <span className="italic" style={{ color: T.accent }}>для кого именно</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr3({ uz: "Saytni tanlang", ru: "Выберите сайт" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {KEYS.map((k) => <button key={k} className={turnCls(lit2, k, pend2.length > 1).trim()} onClick={() => tap(k)} style={{ display: "flex", alignItems: "center", gap: 11, textAlign: "left", cursor: "pointer", border: "none", borderRadius: 13, padding: "13px 15px", background: T.paper, boxShadow: active === k ? "inset 0 0 0 2px " + T.accent + ", 0 8px 20px -7px rgba(91,61,230,0.22)" : "0 6px 16px -8px rgba(" + T.shadowBase + ",0.16)", transition: "all 0.18s" }}>
                  ...
                  <span style={{ fontFamily: "'Manrope',sans-serif", fontWeight: 600, fontSize: 14, color: T.ink }}>{tr3(SITES[k].name)}</span>
            ...
            {cur ? <div className="sk-info fade-step" key={active}>
                <p className="flow-label" style={{ margin: "0 0 4px" }}>{tr3({ uz: "Bu sayt kimlar uchun", ru: "Для кого этот сайт" })}</p>
                <p className="body" style={{ color: T.ink, margin: 0 }}>{tr3(cur.kim)}</p>
                <p className="small" style={{ color: T.ink2, margin: "6px 0 0" }}>{tr3(cur.nega)}</p>
              </div> : <p className="small" style={{ color: T.ink3, fontStyle: "italic", margin: 0 }}>{tr3({ uz: "Saytni bosing — u kimlar uchun qilingani shu yerda ochiladi.", ru: "Нажмите на сайт — здесь откроется, для кого он сделан." })}</p>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Uchchalasida ham javob aniq: sayt <b>aniq odamlar guruhi</b> uchun qilingan. Bu guruh — saytning <b>auditoriyasi</b>.</>, ru: <>Во всех трёх ответ ясен: сайт сделан для <b>конкретной группы людей</b>. Эта группа — <b>аудитория</b> сайта.</> })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{tr3({ uz: "Sayt shu odamlarga shunday ko'rinadi", ru: "Вот каким сайт видят эти люди" })}</p>
            {cur ? <Preview url={cur.url} minH={188}><SiteMock {...cur.mock} /></Preview> : <div className="frame-dash" style={{ minHeight: 188, display: "flex", alignItems: "center", justifyContent: "center" }}><p className="small" style={{ color: T.ink3, fontStyle: "italic", margin: 0 }}>{tr3({ uz: "Chapdan sayt tanlang", ru: "Выберите сайт слева" })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [wide, setWide] = useState4(false);
  ...
  return <Stage eyebrow={{ uz: "Aniq odam — kuch", ru: "Конкретный человек — это сила" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Avval ikkinchi holatni ko'ring", ru: "Сначала посмотрите второе состояние" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Lavash do'koni sayti <span className="italic" style={{ color: T.accent }}>hamma uchun</span> bo'lsa nima bo'ladi?</>, ru: <>Что будет, если сайт лавашной сделать <span className="italic" style={{ color: T.accent }}>для всех</span>?</> })}</h2></div>
        ...
        <div className="s3-scene fade-up delay-1">
          <div className="s3-node">
            ...
            <span className="conn-lbl">{wide ? tr3({ uz: "Hamma", ru: "Все" }) : tr3({ uz: "O'quvchi", ru: "Школьник" })}</span>
            <span className="conn-sub">{wide ? tr3({ uz: "kim uchunligi aniq emas", ru: "непонятно, для кого" }) : tr3({ uz: "tanaffusda lavash oladi", ru: "берёт лаваш на перемене" })}</span>
          </div>
          ...
          <div className={"s3-site " + (wide ? "off" : "")}>
            ...
            <span className={"s3-visit " + (wide ? "zero" : "")} key={wide ? "z" : "v"}>{wide ? tr3({ uz: "👥 Kirgan odam o'zi izlaganini topmadi va chiqib ketdi", ru: "👥 Зашедший не нашёл нужного и ушёл" }) : tr3({ uz: "👥 O'quvchilar har kuni oldindan buyurtma qilib turadi", ru: "👥 Школьники каждый день заказывают заранее" })}</span>
          </div>
        </div>
        <button className={"btn" + (sceneTurn ? " turn-ring" : "")} onClick={toggle} style={{ alignSelf: "flex-start" }}>{wide ? tr3({ uz: "↩ Aniq odamlarga qaytarish", ru: "↩ Вернуть конкретных людей" }) : tr3({ uz: "✂️ Kartani «hamma uchun»ga almashtirish", ru: "✂️ Заменить карточку на «для всех»" })}</button>
        {done && (wide ? <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>«Hamma uchun» yozilgan sayt <b>hech kimga aniq gapirmaydi</b>: kirgan odam o'zi izlaganini topmaydi va saytdan chiqib ketadi.</>, ru: <>Сайт с надписью «для всех» <b>ни с кем не говорит прямо</b>: зашедший не находит нужного и уходит.</> })}</p></div> : <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Aniq odamlar guruhi qaytdi — sayt yana aniq gapirmoqda: <b>nima bor</b>, <b>narxi qancha</b>, <b>qanday buyurtma qilinadi</b>.</>, ru: <>Конкретная группа вернулась — сайт снова говорит прямо: <b>что есть</b>, <b>сколько стоит</b>, <b>как заказать</b>.</> })}</p></div>)}
        </div>
        </Zoomable>
      </div>
    </Stage>;
...
var ScreenKeys = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [i, setI] = useState4(0);
  const [bet, setBet] = useState4(null);
  ...
  return <Stage eyebrow={{ uz: "📊 Haqiqiy misol · Facebook", ru: "📊 Реальный пример · Facebook" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!last} disabled={!last && !isMentor} label={last || isMentor ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Avval voqeani oxirigacha oching", ru: "Сначала откройте историю до конца" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Bugungi eng katta ijtimoiy tarmoq avval <span className="italic" style={{ color: T.accent }}>kimlar uchun</span> ishlagan?</>, ru: <>А для кого сначала работала <span className="italic" style={{ color: T.accent }}>крупнейшая соцсеть</span> наших дней?</> })}</h2></div>
        ...
        <div className="k-slide fade-up" key={i}>
          ...
          {cur.bet ? <div className="k-bet">
              <span className="k-bet-lbl">{tr3({ uz: "🎲 Avval o'zingiz belgilab ko'ring", ru: "🎲 Сначала отметьте свой вариант" })}</span>
              <p className="k-ask">{tr3(cur.ask)}</p>
              <div className="k-chips">
                {cur.opts.map((o, k) => <button key={k} className={"k-chip " + (bet === k ? "on" : "") + (betTurn ? " turn-ring turn-wave w" + (k + 1) : "")} disabled={bet !== null} onClick={() => setBet(k)}>{tr3(o)}</button>)}
              </div>
              {bet !== null && <p className={"k-res " + (bet === cur.right ? "ok" : "")}>
                  {bet === cur.right ? tr3({ uz: "🎯 Topdingiz! ", ru: "🎯 Угадали! " }) : tr3({ uz: "Adashdingiz — asl javob: ", ru: "Не угадали — верный ответ: " })}
                  <b>{tr3(cur.opts[cur.right])}</b>
                </p>}
            </div> : <div className="k-body">
              <span className="k-ic">{cur.ic}</span>
              <p className="k-t">{tr3(cur.t)}</p>
            </div>}
          <div className="k-nav">
            <button className="btn-soft" disabled={i === 0} onClick={() => setI(i - 1)}>{tr3({ uz: "← Oldingi", ru: "← Предыдущий" })}</button>
            <span className="k-dots">{K_SLIDES.map((_, k) => <span key={k} className={"k-dot " + (k === i ? "cur" : k < i ? "fill" : "")} />)}</span>
            {!last && <button className={"btn" + (nextTurn ? " turn-ring" : "")} disabled={betOpen && !isMentor} onClick={() => setI(i + 1)}>{betOpen && !isMentor ? tr3({ uz: "Avval taxminingizni belgilang", ru: "Сначала отметьте свою догадку" }) : tr3({ uz: "Keyingisi →", ru: "Следующий →" })}</button>}
          </div>
        </div>
        {last && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Facebook ham avval <b>juda aniq bir guruh</b> uchun yaratilgan. Sizning auditoriya kartangizdagi «KIM» ham shunday aniq bo'lsin.</>, ru: <>Facebook тоже начинался для <b>очень конкретной группы</b>. Пусть «КТО» в вашей карточке аудитории будет таким же конкретным.</> })}</p></div>}
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [st, setSt] = useState4(() => {
    const saved2 = ((Array.isArray(storedAnswer?.cards) ? storedAnswer.cards : readFullCards()) || []).filter(cardFull).slice(0, 2);
  ...
  return <Stage eyebrow={{ uz: "Mustaqil ish · auditoriya-karta ✍️", ru: "Самостоятельная работа · карточка аудитории ✍️" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext turnBusy={turnBusy} disabled={!done && !isMentor} label={done || isMentor ? { uz: "Davom etish", ru: "Продолжить" } : lockLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>O'z saytingiz uchun <span className="italic" style={{ color: T.accent }}>auditoriya-karta</span> yozing</>, ru: <>Напишите <span className="italic" style={{ color: T.accent }}>карточку аудитории</span> для своего сайта</> })}</h2></div>
    ...
    /* Havodagi qadam-indikator (80a): fonsiz doiralar, yozilgani yashil ✓ */
        ...
        <div className={"jw-steps fade-up" + (turnBusy ? " turn-quiet" : "")} aria-label={tr3({ uz: filledN + "/3 maydon to'ldi", ru: "Заполнено полей: " + filledN + "/3" })}>
          {WFIELDS.map((f, i) => {
    const okF = i === 0 ? okKim : i === 1 ? okMu : okYe;
    const cur = !okF && pendFields[0] === f.k;
    return <React3.Fragment key={f.k}>
                {i > 0 && <span className={"jws-line " + (okF ? "on" : "")} aria-hidden="true" />}
                <span className={"jws " + (okF ? "on" : cur ? "cur" : "")}>
                  <i className="jws-n">{okF ? "✓" : i + 1}</i>
                  <em className="jws-t">{tr3(f.label)}</em>
                </span>
              </React3.Fragment>;
  })}
        </div>
        <div className="split" ref={workRef}>
          <Col>
            {showEditor ? <div className="swed fade-up" key={editing ? "e" + editIdx : "n" + saved.length}>
                {editing && <span className="swed-tag">{tr3({ uz: "✎ Kartani tahrirlash", ru: "✎ Редактировать карточку" })}</span>}
    ...
    return <label key={f.k} className={"smini-f " + f.k + (okF ? " on" : "") + turnCls(litField, f.k, pendFields.length > 1)} style={{ borderLeftColor: okF ? COL[f.color] : void 0 }}>
                        <span style={{ color: COL[f.color] }}>{tr3(f.label)}</span>
                        <input value={draft[f.k]} onChange={(e) => setD({ [f.k]: e.target.value })} onFocus={() => setFieldFocus(true)} onBlur={() => setFieldFocus(false)} placeholder={tr3(f.hint)} />
                      </label>;
  })}
                </div>
                {saveHint && <p className="swed-hint">💡 {tr3(saveHint)}</p>}
                <div className="swed-btns">
                  ...
                  {!full && <span className="swed-cnt">{tr3({ uz: `${filledN}/3 maydon to'ldi`, ru: `Заполнено полей: ${filledN}/3` })}</span>}
                  <button className={"swed-save" + (saveTurn ? " turn-ring" : "")} disabled={!canSave} onClick={saveDraft}>{tr3({ uz: "✓ Saqlash", ru: "✓ Сохранить" })}</button>
                </div>
              </div> : <div className="done-mini fade-step">{tr3({ uz: "✅ Kartangiz saqlandi", ru: "✅ Ваша карточка сохранена" })} <span className="dm-sub">{tr3({ uz: "— tahrirlash uchun ✎ belgisidan foydalaning", ru: "— чтобы поправить, нажмите ✎" })}</span></div>}
            <StudentPracticePulse live={live} screen={screen} />
          </Col>
          <Col>
            <p className="flow-label">{tr3({ uz: "📇 Kartalarim", ru: "📇 Мои карточки" })}</p>
            {saved.length === 0 ? <div className="frame-dash" style={{ padding: "16px 18px" }}><p className="small" style={{ color: T.ink3, fontStyle: "italic", margin: 0 }}>{tr3({ uz: "Kartangiz saqlangach shu yerda ko'rinadi.", ru: "Карточка появится здесь после сохранения." })}</p></div> : <div className="svd full fade-step">
                {saved.map((c, i) => <div key={i} className={"svd-card " + (editIdx === i ? "editing" : "")}>
                    <div className="svd-top">
                      <span className="svd-num">✓ {i + 1}</span>
                      <button className="svd-edit" onClick={() => editCard(i)} aria-label={tr3({ uz: i + 1 + "-kartani tahrirlash", ru: "Редактировать карточку " + (i + 1) })}>{tr3({ uz: "✎ Tahrirlash", ru: "✎ Редактировать" })}</button>
                    </div>
                    <p className="svd-sent"><b style={{ color: SLOT.kim }}>{c.kim}</b> — <b style={{ color: SLOT.muammo }}>{c.muammo}</b>. <b style={{ color: SLOT.yechim }}>{tr3({ uz: "Yechim:", ru: "Решение:" })}</b> {c.yechim}</p>
                  </div>)}
              </div>}
          </Col>
        </div>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={{ uz: "Har xil odam — har xil ehtiyoj", ru: "Разные люди — разные потребности" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: seen.size + "/" + ORDER.length + " odamni ko'ring", ru: "Посмотрите людей: " + seen.size + "/" + ORDER.length }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Bitta saytga kirgan hamma <span className="italic" style={{ color: T.accent }}>bir xil</span> narsani qidiradimi?</>, ru: <>Все, кто зашёл на один сайт, ищут <span className="italic" style={{ color: T.accent }}>одно и то же</span>?</> })}</h2></div>
          ...
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {ORDER.map((k) => <button key={k} className={turnCls(lit8, k, pend8.length > 1).trim()} onClick={() => tap(k)} style={{ display: "flex", alignItems: "center", gap: 12, textAlign: "left", cursor: "pointer", border: "none", borderRadius: 12, padding: "13px 15px", background: T.paper, boxShadow: active === k ? "inset 0 0 0 2px " + T.accent + ", 0 8px 20px -6px rgba(91,61,230,0.22)" : "0 6px 16px -7px rgba(" + T.shadowBase + ",0.16)", transition: "all 0.18s" }}>
                  ...
                  <span style={{ fontFamily: "'Manrope',sans-serif", fontWeight: 600, fontSize: 15, color: T.ink }}>{tr3(USERS[k].name)}</span>
                  {seen.has(k) && <span style={{ marginLeft: "auto", color: T.success, display: "inline-flex" }}>{Ico.check(14)}</span>}
                </button>)}
            </div>
            {active && <div className="sk-info fade-step" key={active + "w"}>
                <p className="flow-label" style={{ margin: "0 0 4px" }}>{USERS[active].ic} {tr3({ uz: <>{tr3(USERS[active].name)} nimani xohlaydi?</>, ru: <>Чего хочет {tr3(USERS[active].name)}?</> })}</p>
                <p className="body" style={{ color: T.ink, margin: 0 }}>{tr3(USERS[active].want)}</p>
              </div>}
            {done && <div className="frame-soft fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Bir saytga turli odamlar kirishi mumkin. Ammo auditoriya kartasida <b>eng muhim guruh</b> tanlanadi.</>, ru: <>На один сайт могут заходить разные люди. Но в карточке аудитории выбирают <b>самую важную группу</b>.</> })}</p></div>}
          </Col>
          <Col>
            <p className="flow-label">{active ? "👀 " + tr3(USERS[active].look) : tr3({ uz: "Sayt — hozircha oddiy ko'rinishda", ru: "Сайт — пока в обычном виде" })}</p>
            <div className="bp-window fade-up delay-1">
              <div className="bp-bar"><span className="bb-dots"><i /><i /><i /></span><span className="bp-url"><span className="lock">●</span>lavash.uz</span></div>
              <div className="bp-body" style={{ minHeight: 168 }}>
                <div className="pg-in" key={active || "none"}>
                  <div className="site-header"><span className="site-brand"><span className="site-logo" style={{ background: "#C97B2D" }}>L</span><span className="site-name">{tr3({ uz: "Lavash do'koni", ru: "Лавашная" })}</span></span><span className="site-nav"><span>{tr3({ uz: "Menyu", ru: "Меню" })}</span><span>{tr3({ uz: "Aloqa", ru: "Контакты" })}</span></span></div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    <div className={"eye-row " + (active === "oquvchi" ? "hot" : "")}>{tr3({ uz: <>📱 Oldindan buyurtma: <b>tayyor bo'lganda xabar keladi</b></>, ru: <>📱 Заказ заранее: <b>придёт сообщение, когда будет готово</b></> })}<span className="eye-cta" style={{ background: active === "oquvchi" ? T.accent : T.ink3 }}>{tr3({ uz: "Buyurtma berish", ru: "Заказать" })}</span></div>
                    <div className={"eye-row " + (active === "sotuvchi" ? "hot" : "")}>{tr3({ uz: <>📋 Bugungi buyurtmalar: <b>7 ta</b> — 3 tasi yangi</>, ru: <>📋 Заказы на сегодня: <b>7</b> — из них 3 новых</> })}</div>
                    <div className={"eye-row " + (active === "ofis" ? "hot" : "")}>{tr3({ uz: <>🔁 Oxirgi buyurtma: <b>bir bosishda takrorlash</b></>, ru: <>🔁 Последний заказ: <b>повторить одним нажатием</b></> })}</div>
                  </div>
                </div>
              </div>
            </div>
            {!active && <p className="small" style={{ color: T.ink3, fontStyle: "italic", margin: 0 }}>{tr3({ uz: "Chapdan odamni tanlang — u qaraydigan qator saytda yonadi.", ru: "Выберите человека слева — строка, на которую он смотрит, подсветится." })}</p>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const gate11 = useContext2(LiveGateCtx) || {};
  ...
  const restoredRef = useRef4(restored);
  const [pick, setPick] = useState4({ kim: g0, muammo: g0, yechim: g0 });
  const [confirmed, setConfirmed] = useState4({ kim: restored, muammo: restored, yechim: restored });
  ...
  const [state, setState] = useState4(restored ? "convert" : "idle");
  const timer = useRef4(null);
  useEffect5(() => () => clearTimeout(timer.current), []);
  const firstRun = useRef4(true);
  useEffect5(() => {
    if (firstRun.current) {
  ...
  return <Stage eyebrow={{ uz: "Kartani yig'ing", ru: "Соберите карточку" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!converted} disabled={!converted && !isMentor11} label={navLabel} onClick={onNext} /></>}>
    ...
    if (isDone) return <div key={`${r.key}-done`} className="g11-donerow el-in" style={{ borderLeftColor: r.color }}>
                  ...
                  <span className="mono g11-done-lbl" style={{ color: r.color }}>{tr3(r.label)}</span>
                  <span className="g11-done-val">{r.key === "kim" && <span aria-hidden="true">{AVA[pick.kim]} </span>}{tr3(GROUPS[pick[r.key]][r.key])}</span>
                  {!converted && <button className="g11-redo" onClick={() => reopen(r.key)} aria-label={tr3({ uz: `${tr3(r.label)}ni o'zgartirish`, ru: `Изменить: ${tr3(r.label)}` })}>↻</button>}
                </div>;
    if (!isActive) return <div key={`${r.key}-lock`} className="g11-lockrow">
                  <span className="g11-num" style={{ background: `${T.ink3}66` }}>{ri + 1}</span>
                  <span>{tr3(r.label)}{tr3({ uz: " — keyingi qadam", ru: " — следующий шаг" })}</span>
                </div>;
    return <div key={`${r.key}-act`} className={`g11-group fade-up ${pick[r.key] === null ? "g11-live" : ""}`}>
                  <p className="g11-glabel" style={{ color: r.color }}><span className="g11-num" style={{ background: r.color }}>{ri + 1}</span>{tr3({ uz: <>{tr3(r.label)}ni tanlang</>, ru: <>Выберите {tr3(r.label)}</> })}</p>
      ...
      >
                          ...
                          <span style={{ flex: 1 }}>{tr3(GROUPS[g][r.key])}</span>
                        </button>;
    })}
                  </div>
                </div>;
          ...
          <Col>
            <p className="flow-label">{tr3({ uz: "Yig'ilayotgan karta", ru: "Собираемая карточка" })}</p>
    ...
    return <div key={k} className={`g11-slot ${g ? "filled" : ""}`} style={g ? { borderLeftColor: META[k].color } : void 0}>
                    <span className="mono g11-slabel" style={{ color: g ? META[k].color : T.ink3 }}>{tr3(META[k].label)}</span>
                    {g ? <span key={g} className="g11-val el-in">
                        <span style={{ color: META[k].color, display: "inline-flex", flexShrink: 0 }}>{META[k].ic}</span>
                        {tr3(GROUPS[g][k])}
                      </span> : <span className="g11-empty">{tr3({ uz: "tanlanmagan…", ru: "не выбрано…" })}</span>}
                  </div>;
  })}
            </div>
            {
    /* 👀 Egasiga ko'rsatish — qahramon-reaksiya, endi har bosqichda (muammo, so'ng yechim) */
            ...
            {(step === "muammo" || step === "yechim" || converted) && <button className={`pm-show-btn ${converted ? "done" : ""}${showTurn ? " turn-ring" : ""}`} disabled={converted || !pick[step] || busy} onClick={showToOwner}>{tr3(btnLabel)}</button>}
            ...
            {state !== "idle" && pick.kim && <div className={`pm-react ${converted ? "ok" : leaving ? "no" : "read"} fade-step`} key={state + pick.kim}>
                ...
                <div className="pm-bubble">
                  <span className="pm-who">{tr3(GROUPS[pick.kim].kim)}</span>
                  <span className={`pm-say ${state === "showing" ? "thinking" : ""}`}>{tr3(BUBBLE[pick.kim][state])}</span>
                  {
    /* «tanildi» bosqich-indikatori — yechimni o'qib chiqish paytini to'lish bilan ko'rsatadi */
            ...
            {converted && <div className="frame-success fade-step pm-match" key={pick.kim}>
                <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: T.success, textTransform: "uppercase", letterSpacing: "0.08em" }}>{tr3({ uz: "Egasiga mos g'oya!", ru: "Идея подошла владельцу!" })}</p>
                <p className="body" style={{ margin: 0, color: T.ink }}><b>{tr3(GROUPS[pick.kim].kim)}</b> — {low(tr3(GROUPS[pick.kim].muammo))}. <span style={{ color: SLOT.yechim, fontWeight: 600 }}>{tr3({ uz: "Yechim:", ru: "Решение:" })}</span> {low(tr3(GROUPS[pick.kim].yechim))}.</p>
              </div>}
            {leaving && <div className="frame-warn fade-step pm-shake" key={"no" + pick.kim + pick.muammo + pick.yechim}>
                <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: AMBER_INK, textTransform: "uppercase", letterSpacing: "0.08em" }}>{tr3({ uz: "Egasiga mos kelmadi", ru: "Владельцу не подошло" })}</p>
                <p className="body" style={{ margin: 0, color: T.ink }}>{state === "reject-p" ? tr3({ uz: <>Bu <b>muammo</b> tanlagan odamniki emas. <b>Muammoni</b> almashtirib, yana ko'rsating.</>, ru: <>Эта <b>проблема</b> не того человека, которого вы выбрали. Поменяйте <b>проблему</b> и покажите снова.</> }) : tr3({ uz: <>Muammosi to'g'ri, lekin <b>yechim</b> uni yechmaydi. <b>Yechimni</b> almashtirib, yana ko'rsating.</>, ru: <>Проблема верная, но <b>решение</b> её не закрывает. Поменяйте <b>решение</b> и покажите снова.</> })}</p>
              </div>}
          </Col>
        </div>
        </Zoomable>
...
var checkKarta = (src) => {
  ...
  KOD_CONDS.forEach((c) => {
    let line = "", mark = tr3(c.mark), ph = tr3(c.ph);
    ...
    res[c.id] = ok;
    if (!ok) res.hints[c.id] = !line ? tr3({ uz: "«" + tr3(c.mark) + "» qatori o'chib ketibdi — «Qaytadan» bilan tayyor sahifani qaytaring.", ru: "Строка «" + tr3(c.mark) + "» пропала — верните готовую страницу кнопкой «Заново»." }) : tr3(c.hint);
  });
  return res;
};
var _kartaSrc = null;
...
var ScreenCoding = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [st, setSt] = useState4(() => {
    ...
    return { code: (typeof storedAnswer?.code === "string" ? storedAnswer.code : null) || saved && saved.code || kodStart(), done: !!(storedAnswer && storedAnswer.solved) || !!(saved && saved.done) };
  });
  const { code, done } = st;
  const [myCard] = useState4(() => readFullCards()[0] || null);
  ...
  return <Stage eyebrow={{ uz: "Koding · 🛠 kompilyator", ru: "Кодинг · 🛠 компилятор" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Avval kompilyatorda almashtiring", ru: "Сначала замените в компиляторе" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Endi kartangizni <span className="italic" style={{ color: T.accent }}>sahifada</span> ko'rsatamiz.</>, ru: <>Теперь покажем вашу карточку <span className="italic" style={{ color: T.accent }}>на странице</span>.</> })}</h2></div>
        ...
        <div className="stq fade-up delay-1" ref={workRef}>
          <div className="stq-code">
            ...
            <code className="stq-code-body">
              <span className="stq-l t">{tr3({ uz: "Kimga: [KIM]", ru: "Кому: [КТО]" })}</span>
              <span className="stq-l m">{tr3({ uz: "Qanday muammoga: [MUAMMO]", ru: "Какая проблема: [ПРОБЛЕМА]" })}</span>
              <span className="stq-l f">{tr3({ uz: "Qanday yechim: [YECHIM]", ru: "Какое решение: [РЕШЕНИЕ]" })}</span>
              <span className="stq-l dim">{tr3({ uz: "← Bu joylarni siz to'ldirasiz", ru: "← Эти места заполняете вы" })}</span>
            </code>
          </div>
          ...
          <div className="stq-page">
            ...
            <div className="stq-mid">
              <span className="stq-tag">{tr3({ uz: "kartangiz", ru: "ваша карточка" })}</span>
              <p className="kdx-line"><b style={{ color: SLOT.kim }}>{tr3({ uz: "Kimga:", ru: "Кому:" })}</b> {myCard ? myCard.kim : tr3({ uz: "tanaffusda lavash oladigan o'quvchilar", ru: "школьники, которые берут лаваш на перемене" })}</p>
              <p className="kdx-line"><b style={{ color: SLOT.muammo }}>{tr3({ uz: "Qanday muammoga:", ru: "Какая проблема:" })}</b> {myCard ? myCard.muammo : tr3({ uz: "navbat uzun — tanaffusga ulgurishmaydi", ru: "очередь длинная — не успевают за перемену" })}</p>
              <p className="kdx-line"><b style={{ color: SLOT.yechim }}>{tr3({ uz: "Qanday yechim:", ru: "Какое решение:" })}</b> {myCard ? myCard.yechim : tr3({ uz: "oldindan buyurtma qilish sahifasi", ru: "страница заказа заранее" })}</p>
            </div>
          </div>
        </div>
    ...
    writeKodingOpen(true);
          ...
          {done && <span className="stq-cta-sub">{tr3({ uz: "Bajarildi — xohlasangiz matnni yana tuzatishingiz mumkin", ru: "Выполнено — при желании текст можно ещё поправить" })}</span>}
          {
    /* 89-qonun: takrorlash-yo'li — FAQAT erkin rejimda va FAQAT bajarilmagan holatda.
       Faqat eshikni ochadi: nishon bermaydi, saqlanmaydi, serverga signal yubormaydi. */
          ...
          {!done && isSelf && <button className="stq-skip" onClick={onNext}>{tr3({ uz: "✓ Bu mashqni sinfda bajarganman — davom etish →", ru: "✓ Это задание я делал в классе — продолжить →" })}</button>}
        </div>
        {done && <div className="done-mini fade-step" style={{ alignSelf: "center" }}>{tr3({ uz: "✅ Ishladi!", ru: "✅ Получилось!" })} <span className="dm-sub">{tr3({ uz: "— sahifada endi sizning kartangiz turibdi", ru: "— теперь на странице стоит ваша карточка" })}</span></div>}
        <StudentPracticePulse live={live} screen={screen} />
  ...
  return <div className="pair-timer">
      {st.running ? <div className="pair-live">
          ...
          <div className="pair-live-txt">
            <span className="pair-now">{tr3({ uz: <>Hozir <span className={"pair-who " + (isA ? "" : "b")}>{isA ? "A" : "B"}</span> gapiradi</>, ru: <>Сейчас говорит <span className={"pair-who " + (isA ? "" : "b")}>{isA ? "A" : "B"}</span></> })}</span>
            <span className="pair-next">{isA ? tr3({ uz: "keyin — B navbati", ru: "потом — очередь B" }) : tr3({ uz: "oxirgi navbat", ru: "последняя очередь" })}</span>
          </div>
        </div> : <p className="pair-now" style={{ margin: 0 }}>{st.done ? tr3({ uz: "✓ Vaqt tugadi — ikkalangiz ham aytib bo'ldingiz. Barakalla!", ru: "✓ Время вышло — вы оба высказались. Молодцы!" }) : tr3({ uz: "Har biringizga 30 soniyadan — avval A, keyin B.", ru: "По 30 секунд каждому — сначала A, потом B." })}</p>}
      <div className="pair-timer-btns">
        {!st.running && <button className={st.done ? "btn-soft" : "pair-start" + (startTurn ? "" : " calm")} onClick={() => setSt({ running: true, left: 60, done: false })}>{st.done ? tr3({ uz: "↻ Yana 1 daqiqa", ru: "↻ Ещё 1 минута" }) : tr3({ uz: "▶ 1 daqiqani boshlash", ru: "▶ Запустить 1 минуту" })}</button>}
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const isMentor = !!(live && live.mode === "mentor");
  const mountTs = useRef4(Date.now());
  const [text, setText] = useState4(() => storedAnswer?.text || (() => {
    try {
  ...
  return <Stage eyebrow={{ uz: "Mustahkamlash · 2 qadam", ru: "Закрепление · 2 шага" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext turnBusy={!written} disabled={!written && !isMentor} label={written || isMentor ? { uz: "Davom etish", ru: "Продолжить" } : pairStage === "done" ? { uz: "② Endi bir qator yozing", ru: "② Теперь напишите одну строку" } : { uz: "① Avval sherigingizga ayting", ru: "① Сначала расскажите напарнику" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Saytingiz kim uchun ekanini <span className="italic" style={{ color: T.accent }}>yoddan</span> ayta olasizmi?</>, ru: <>Сможете <span className="italic" style={{ color: T.accent }}>наизусть</span> сказать, для кого ваш сайт?</> })}</h2></div>
        ...
        <div className="rcp-flow">
          <div className="rcp-step fade-up delay-1">
            <div className="rcp-step-h"><span className="rcp-n">1</span><div><span className="rcp-t">{tr3({ uz: "🗣 Sherigingizga ayting: kimni tanladingiz va nega", ru: "🗣 Расскажите напарнику: кого выбрали и почему" })}</span></div></div>
          ...
          <div className="rcp-step fade-up delay-2">
            ...
            <span className={"turn-wrap" + (inputTurn ? " turn-ring" : "")}>
              <input className="reflect-input" value={text} onChange={(e) => save(e.target.value)} onFocus={() => setReflFocus(true)} onBlur={() => setReflFocus(false)} placeholder={tr3({ uz: "Men ... ni tanladim, chunki ...", ru: "Я выбрал ..., потому что ..." })} maxLength={160} />
            </span>
...
var INLINE_KEYS = { s4: 2, s5b: 1, s9: 2, s12: 2, s15: -1 };
var QUIZ_BANK = [
    ...
    /* ===== YAKUN — PODIUM ===== */
      ...
      {phase === "done" && <div className="qz-view fade-step">
          ...
          {solo ? <div className="qz-solo-res">
              ...
              <p className="qz-sub">{tr3({ uz: `ball · ${soloScore.ok}/${QUIZ_BANK.length} to'g'ri`, ru: `баллов · верно ${soloScore.ok}/${QUIZ_BANK.length}` })}{soloScore.maxStreak >= 2 ? tr3({ uz: ` · eng uzun ketma-ketlik 🔥 ${soloScore.maxStreak} ta`, ru: ` · лучшая серия 🔥 ${soloScore.maxStreak}` }) : ""}</p>
...
var ScreenProject = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [sel, setSel] = useState4(saved ? saved.manba : null);
  const [ownM, setOwnM] = useState4(saved && saved.manba === "ozim" ? saved.muammo : "");
  const [ownY, setOwnY] = useState4(saved && saved.manba === "ozim" ? saved.yechim : "");
  const ownOk = ownM.trim().length >= 8 && ownY.trim().length >= 4;
  const done = sel === "ozim" ? ownOk : !!sel;
  const pickBank = (b) => {
    setSel(b.id);
    demoWrite({ muammo: tr3(b.muammo), yechim: tr3(b.yechim), manba: b.id, holat: "tanlangan" });
  };
  ...
  return <Stage eyebrow={{ uz: "Demo Day loyihasi", ru: "Проект Demo Day" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Demo Day'da <span className="italic" style={{ color: T.accent }}>qaysi muammoni</span> yechasiz?</>, ru: <>Какую проблему вы решите на <span className="italic" style={{ color: T.accent }}>Demo Day</span>?</> })}</h2></div>
    ...
    return <button key={b.id} className={"hook-option " + (on ? "on" : "")} onClick={() => pickBank(b)}>
          ...
          {sel === "ozim" && <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 8, paddingLeft: 6 }}>
              <input className="reflect-input" value={ownM} onChange={(e) => setOwnM(e.target.value)} placeholder={tr3({ uz: "Qaysi muammo? (masalan: seksiya vaqtini hech kim bilmaydi)", ru: "Какая проблема? (например: никто не знает время секции)" })} />
              <input className="reflect-input" value={ownY} onChange={(e) => setOwnY(e.target.value)} placeholder={tr3({ uz: "Sayt nima qiladi? (masalan: vaqtlar jadvali sayti)", ru: "Что сделает сайт? (например: сайт с расписанием)" })} />
            </div>}
        </div>
        {done && <div className="frame-success fade-step" style={{ maxWidth: 640 }}><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Saqlandi. VS Code darsida mentor aynan shu loyihangizni so'raydi.</>, ru: <>Сохранено. На уроке VS Code ментор спросит именно про этот ваш проект.</> })}</p></div>}
      </div>
    </Stage>;
};
var PM_FLASHCARDS = [
...
function Flashcards({ cards }) {
  ...
  if (!card) return <div className="fc-done fade-up"><span className="fc-done-emoji">🎉</span><p className="fc-done-h">{tr3({ uz: "Hammasini bilasiz!", ru: "Вы знаете всё!" })}</p><p className="fc-done-s">{tr3({ uz: `${total}/${total} atama yodlandi`, ru: `Выучено понятий: ${total}/${total}` })}</p><button className="fc-btn ghost" onClick={restart}>{tr3({ uz: "↻ Qaytadan takrorlash", ru: "↻ Повторить заново" })}</button></div>;
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={{ uz: "Tayyor", ru: "Готово" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Yakunlash", ru: "Завершить" })}</button></>}>
    ...
    /* 54-qonun: sarlavha ostidagi h-sub paragrafi YO'Q · 90a: ScoreRing mentorda ko'rinmaydi */
        ...
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">{Ico.check(11)}</span> {tr3({ uz: "Dars tugadi", ru: "Урок окончен" })}</span><h2 className="title h-title fade-up d1">{isMentorL ? tr3({ uz: <>Bugun <span className="italic" style={{ color: T.accent }}>auditoriya-karta</span> tuzishni o'rgandik.</>, ru: <>Сегодня мы научились составлять <span className="italic" style={{ color: T.accent }}>карточку аудитории</span>.</> }) : tr3({ uz: <>Endi siz <span className="italic" style={{ color: T.accent }}>auditoriya-karta</span> tuza olasiz.</>, ru: <>Теперь вы умеете составлять <span className="italic" style={{ color: T.accent }}>карточку аудитории</span>.</> })}</h2></div>{!isMentorL && <ScoreRing correct={correct} total={total} />}</div>
        {
  ...
  />
        ...
        {hwNote && <p className="hw-cta-note fade-step" style={{ textAlign: "center" }}>{tr3({ uz: "Amaliy vazifa — auditoriya-kartangizni 2 ta tanishingizga ko'rsatib, «Siz shunday saytga kirarmidingiz?» deb so'rang. Javoblarini eshitgach, KIM va MUAMMO qatorlarini aniqroq qilib qayta yozing.", ru: "Задание — покажите свою карточку аудитории двум знакомым и спросите: «Вы зашли бы на такой сайт?». Услышав ответы, перепишите строки КТО и ПРОБЛЕМА точнее." })}</p>}
...
function PmLesson1({ lang: langProp, onFinished, liveToken }) {
