...
//  Manba:  src/1-Modull/PmLesson2.homework.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/1-Modull/PmLesson2.homework.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/1-Modull/PmLesson2.homework.jsx
...
var T = {
  ...
  ink2: "#565073",
  ink3: "#9C97B4",
  // F-0828-02: fon #F2F0FA → #F7F6FC (ochroq, foydalanuvchi qarori)
  paper: "#FFFFFF",
  accent: "#5B3DE6",
  accentSoft: "#EBE5FD",
  accentVivid: "#6E4BFF",
...
var packOf = (cat) => PLACES[cat] || PLACES[0];
var PLACE_ICO = ["🍞", "🍲", "✏️"];
var WRITE_FIELDS = [
...
var Preview = ({ url, children, minH }) => <div className="bp-window fade-up">
...
var HwSecView = ({ k, texts, placeName }) => {
  const t = texts && texts[k] || "";
  if (k === "hero") return <div>
...
function DragDropOrder({ items, onSolved, onOrder }) {
  ...
  const place = (id, from, slotIdx) => setSt(({ pool: pool2, slots: slots2 }) => {
    if (typeof from === "number" && from === slotIdx) return { pool: pool2, slots: slots2 };
...
function CustomerRun({ order, correct, texts, placeName, onConvert }) {
  ...
  useEffect(() => {
    if (state === "convert" && onConvert) onConvert();
  }, [state]);
  const send = () => {
    if (!ready) return;
    clearTimeout(timer.current);
...
var StagePlace = ({ data, setData }) => {
  const name = data.name || "";
  const visitor = data.visitor || "";
  const nameOk = name.trim().length >= 3;
  const visitorOk = visitor.trim().length >= 20;
  const curCat = PLACES[data.cat] ? data.cat : 0;
    ...
    /* F-0827-32: Mentor-kartasi → markaziy bir qatorli izoh (2–4-bosqich bilan bitta dizayn-tizim) */
      ...
      <div className="frame fade-up d1">
        ...
        <div className="chips" role="radiogroup">
          {PLACES.map((c, i) => <button key={i} type="button" role="radio" aria-checked={curCat === i} className={`chip ${curCat === i ? "on" : ""}`} onClick={() => setData({ ...data, cat: i })}>
...
var placeDone = (d) => (d.name || "").trim().length >= 3 && (d.visitor || "").trim().length >= 20;
var StageWrite = ({ data, setData, placeName, cat }) => {
  ...
  return <div className="col">
    ...
    return <div key={f.k} className="wrow" style={{ borderBottom: i < WRITE_FIELDS.length - 1 ? `1px solid ${T.line}` : "none" }}>
...
var writeDone = (d) => WRITE_FIELDS.every((f) => {
  const len = ((d.texts || {})[f.k] || "").trim().length;
  return len >= f.min && len <= f.max;
});
var StageOrderRun = ({ data, setData, texts, placeName, writeReady, goWrite }) => {
  ...
  if (!writeReady) return <div className="col gate">
  ...
  return <div className="col">
    ...
    /* Keng ekranda yonma-yon: chapda tartiblash, o'ngda mijoz-sahifasi — bo'sh joy
      ...
      <div className="ogrid">
        <DragDropOrder items={items} onSolved={() => {
    if (!data.solved) setData({ ...data, solved: true });
  }} onOrder={setOrder} />
        <CustomerRun order={order} correct={ORDER} texts={texts} placeName={placeName} onConvert={() => {
    if (!data.converted) setData({ ...data, solved: true, converted: true });
  }} />
      </div>
    </div>;
};
var orderDone = (d) => !!d.converted;
var QZ_HOLD_MS = 1500;
var QZ_OUT_MS = 380;
var StageQuiz = ({ data, setData }) => {
  const ans = data.ans || {};
...
var quizDone = (d) => QUIZ.every((q) => (d.ans || {})[q.id] === q.correct);
var STAGES = [
  ...
  return <div className="col fin">
      ...
      <div className="fin-hero fade-up">
        ...
        <h2 className="title h-title" style={{ margin: "4px 0 2px" }}>
          {placeName ? tr({ uz: <><span className="italic" style={{ color: T.accent }}>{placeName}</span> sayti tayyor!</>, ru: <>Сайт <span className="italic" style={{ color: T.accent }}>{placeName}</span> готов!</> }) : hasSite ? tr({ uz: "Saytingiz tayyor!", ru: "Ваш сайт готов!" }) : tr({ uz: "Uyga vazifa bajarildi!", ru: "Домашнее задание выполнено!" })}
        </h2>
      ...
      {hasSite && <div className="fin-site fade-up d1">
          <Preview url={tr({ uz: "sizning-saytingiz.uz", ru: "vash-sait.uz" })} minH={200}>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {ORDER.map((k, i) => <div key={k} className="fin-sec" style={{ "--fd": `${0.55 + i * 0.2}s`, paddingBottom: 9, borderBottom: i < ORDER.length - 1 ? `1px solid ${T.ink3}22` : "none" }}>
...
function PmLesson2Homework({ lang: langProp, onFinished }) {
    ...
    /* F-0827-02: stepper — «1-bosqich → … → Natija», raqam-doira + bog'lovchi chiziq;
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult ? <StageResult data={data} goStage={setStage} onFinishClick={finish} finished={finished} onCelebrated={() => setDataRaw((prev) => prev.celebrated ? prev : { ...prev, celebrated: true })} /> : cur.key === "place" ? <StagePlace data={data.place || {}} setData={setStageData("place")} /> : cur.key === "write" ? <StageWrite data={data.write || {}} setData={setStageData("write")} placeName={placeName} cat={(data.place || {}).cat} /> : cur.key === "order" ? <StageOrderRun data={data.order || {}} setData={setStageData("order")} texts={(data.write || {}).texts || {}} placeName={placeName} writeReady={writeDone(data.write || {})} goWrite={() => setStage(1)} /> : <StageQuiz data={data.quiz || {}} setData={setStageData("quiz")} />}
        </div>
      </div>
