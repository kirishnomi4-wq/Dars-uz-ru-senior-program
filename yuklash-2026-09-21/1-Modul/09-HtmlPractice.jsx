...
//  Qayta yig'ish:  node scripts/build-lms.mjs src/1-Modull/HtmlPractice.jsx
...
var LESSON_META = { lessonId: "html-practice-portfolio-v2", lessonTitle: { uz: "HTML Praktika — Portfolio sayt", ru: "HTML практика — портфолио" } };
var HW_TOKENS = [
...
var NavNext = ({ disabled, label, onClick, optionalLive }) => {
  const lbl = tr3(label ?? { uz: "Davom etish", ru: "Продолжить" });
...
var usePortfolio = (answers) => ({
  name: (answers?.[3]?.name || "").trim() || "Aziza Karimova",
  role: (answers?.[3]?.role || "").trim() || tr3({ uz: "Frontend dasturchi", ru: "Frontend-разработчик" })
});
var firstName = (name) => (name.trim().split(/\s+/)[0] || "aziza").toLowerCase().replace(/[^a-z]/g, "") || "aziza";
var SiteRender = ({ name, role, parts }) => {
  const has = (p) => parts.includes(p);
  return <div className="raw">
      {has("header") && <header>
          <h1>{name}</h1>
          <p>{role}</p>
          {has("nav") && <nav><a>{tr3({ uz: "Men haqimda", ru: "Обо мне" })}</a><a>{tr3({ uz: "Loyihalar", ru: "Проекты" })}</a><a>{tr3({ uz: "Aloqa", ru: "Контакты" })}</a></nav>}
        </header>}
      {has("header") && (has("about") || has("projects") || has("contact") || has("footer")) && <hr />}
      {has("about") && <section>
          <h2>{tr3({ uz: "Men haqimda", ru: "Обо мне" })}</h2>
          <img className="raw-img" src={PROFILE_IMG} alt={tr3({ uz: "Profil rasmi", ru: "Фото профиля" })} />
          <p>{tr3({ uz: "Salom! Men HTML o'rganayotgan o'quvchiman.", ru: "Привет! Я ученик, изучаю HTML." })}</p>
        </section>}
      {has("projects") && <section>
          <h2>{tr3({ uz: "Loyihalarim", ru: "Мои проекты" })}</h2>
          <ul><li><a>{tr3({ uz: "To-Do ilova", ru: "To-Do приложение" })}</a></li><li><a>{tr3({ uz: "Ob-havo sayti", ru: "Сайт погоды" })}</a></li></ul>
        </section>}
      {has("contact") && <section>
          <h2>{tr3({ uz: "Aloqa", ru: "Контакты" })}</h2>
          <p>{tr3({ uz: "Men bilan bog'lanish uchun:", ru: "Чтобы связаться со мной:" })}</p>
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Praktika · kirish", ru: "Практика · вход" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={{ uz: "Boshlaymiz →", ru: "Начинаем →" }} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr3({ uz: <>O'zingizni dunyoga qanday <span className="italic" style={{ color: T.accent }}>tanishtirasiz</span>?</>, ru: <>Как вы <span className="italic" style={{ color: T.accent }}>представите</span> себя миру?</> })}</h1>
        <Mentor>{tr3({ uz: <>Bugun siz o'zingizning <b style={{ color: T.ink }}>portfolio saytingizni</b> noldan quryapsiz — faqat HTML bilan. Keyingi darsda uni CSS bilan bezaymiz. Avval ayting: portfolio sayt nima uchun kerak?</>, ru: <>Сегодня вы строите свой <b style={{ color: T.ink }}>сайт-портфолио</b> с нуля — только на HTML. На следующем уроке украсим его с помощью CSS. Сначала скажите: зачем нужен сайт-портфолио?</> })}</Mentor>
        <Zoomable>
        <Split>
          <Col>
            <p className="flow-label">{tr3({ uz: "Dars oxirida — sizning saytingiz", ru: "В конце урока — ваш сайт" })}</p>
            <Preview title="portfolio.html" minH={160}>
              {picked !== null ? <div className="fade-step"><SiteRender name="Aziza Karimova" role={tr3({ uz: "Frontend dasturchi", ru: "Frontend-разработчик" })} parts={["header", "nav", "about"]} /></div> : <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center" }}>{tr3({ uz: "Hozircha bo'sh sahifa — uni birga to'ldiramiz…", ru: "Пока страница пустая — заполним её вместе…" })}</p>}
            </Preview>
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr3({ uz: "Sizningcha, portfolio nima uchun kerak?", ru: "Как вы думаете, зачем нужно портфолио?" })}</p>
    ...
    return <button key={o.id} className={`hook-option ${on ? "on" : ""}`} disabled={picked !== null} onClick={() => pick(o.id)}><span className="radio">{on && <span className="radio-dot" />}</span><span>{tr3(o.label)}</span></button>;
            ...
            {picked !== null && <p className="hook-ack fade-step">{tr3({ uz: "Aniq! Portfolio — o'zingizni, ishlaringiz va aloqangizni bir joyda ko'rsatadigan shaxsiy sayt. Bugun shuni 0 dan quramiz.", ru: "Точно! Портфолио — личный сайт, который показывает вас, ваши работы и контакты в одном месте. Сегодня построим его с нуля." })}</p>}
          </Col>
        </Split>
        </Zoomable>
      </div>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const stepBtnRef = useRef4(null);
  const scrollToBtn = () => {
    if (stepBtnRef.current) stepBtnRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
  };
  const PreviewBlock = <Col>
      <p className="flow-label">{tr3({ uz: "Tayyor sayt — dars oxirida shunday bo'ladi", ru: "Готовый сайт — таким он будет в конце урока" })}</p>
      <Preview title="portfolio.html" minH={200} maxH={190}>
        <SiteRender name="Aziza Karimova" role={tr3({ uz: "Frontend dasturchi", ru: "Frontend-разработчик" })} parts={["header", "nav", "about", "projects", "contact", "footer"]} />
      </Preview>
    </Col>;
  const StepsBlock = <Col>
      <p className="flow-label">{tr3({ uz: "5 bo'lim", ru: "5 разделов" })}</p>
      <ol className="roadmap">{STEPS.map((s, i) => <li key={i} className="step-card fade-up" style={{ animationDelay: `${0.08 + i * 0.05}s` }}><span className="step-num">{String(i + 1).padStart(2, "0")}</span><span className="step-body"><span className="step-text">{tr3(s.text)}</span><span className="step-tag">{`<${s.tag.split(" ")[0]}>`}</span></span></li>)}</ol>
    </Col>;
  return <Stage eyebrow={tr3({ uz: "Reja", ru: "План" })} screen={screen} audioState={audio} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Birinchi bo'limga →", ru: "К первому разделу →" }} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Bu sayt sizni qanday <span className="italic" style={{ color: T.accent }}>tanishtiradi</span>?</>, ru: <>Как этот сайт <span className="italic" style={{ color: T.accent }}>представит</span> вас?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Har bir bo'limni birma-bir <b style={{ color: T.ink }}>o'z qo'lingiz bilan</b> quramiz. <b style={{ color: T.ink }}>Ishonasizmi —</b> dars oxirida o'ngdagi saytni o'zingiz qura olasiz. Tayyor bo'lsangiz — boshladik! Sayt ustiga bosing</>, ru: <>Каждый раздел построим по очереди <b style={{ color: T.ink }}>своими руками</b>. <b style={{ color: T.ink }}>Поверите ли —</b> к концу урока вы сами сможете собрать сайт справа. Готовы — поехали! Нажмите на сайт</> })}</Mentor>
        {!isNarrow ? <Split>{PreviewBlock}{StepsBlock}</Split> : !showSteps ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)", cursor: "pointer" }} onClick={scrollToBtn}>{PreviewBlock}<p className="small" style={{ margin: 0, color: T.ink2, textAlign: "center", fontStyle: "italic" }}>{tr3({ uz: `👆 Saytni bosing — keyin "5 bo'limni ko'rish"`, ru: "👆 Нажмите на сайт — потом «Посмотреть 5 разделов»" })}</p><button ref={stepBtnRef} className="btn" style={{ alignSelf: "flex-start" }} onClick={(e) => {
    e.stopPropagation();
    setShowSteps(true);
  }}>📋 {tr3({ uz: "5 bo'limni ko'rish", ru: "Посмотреть 5 разделов" })}</button></div> : <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}><button className="btn-ghost" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(false)}>↩ {tr3({ uz: "Tayyor saytni ko'rish", ru: "Посмотреть готовый сайт" })}</button>{StepsBlock}</div>}
      </div>
    </Stage>;
};
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [shown, setShown] = useState4(!!storedAnswer);
  const [dragDone, setDragDone] = useState4(!!storedAnswer);
  const done = dragDone;
  const achMiss = useContext2(AchMissCtx);
  const onSkelWrong = (slots) => {
    if (!achMiss || done) return;
    if (slots[0] !== "header" || slots[slots.length - 1] !== "footer") achMiss.miss(screen);
  };
  useEffect5(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  return <Stage eyebrow={tr3({ uz: "Skelet", ru: "Скелет" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Birinchi blokni quramiz →", ru: "Строим первый блок →" } : shown ? { uz: "Skeletni yig'ing", ru: "Соберите скелет" } : { uz: "Skeletni ko'ring", ru: "Посмотрите скелет" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Sayt <span className="italic" style={{ color: T.accent }}>nimalardan</span> tuzilgan?</>, ru: <>Из чего <span className="italic" style={{ color: T.accent }}>состоит</span> сайт?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Har qanday sayt — <b style={{ color: T.ink }}>bloklar to'plami</b>, ustma-ust joylashgan. Yuqorida sarlavha, o'rtada bo'limlar, pastda footer. Avval skeletni ko'ring, keyin <b style={{ color: T.ink }}>o'zingiz to'g'ri tartibda yig'ing</b>.</>, ru: <>Любой сайт — <b style={{ color: T.ink }}>набор блоков</b>, лежащих друг на друге. Сверху шапка, посередине разделы, внизу footer. Сначала посмотрите скелет, потом <b style={{ color: T.ink }}>соберите его сами в верном порядке</b>.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            {!shown ? <>
              <div className="skel-stack fade-up delay-2">
                {BLOCKS.map((b, i) => <div key={i} className="skel-block" style={{ transitionDelay: `${i * 0.07}s` }}>
                    <span className="skel-tag">{`<${b.tag}>`}</span>
                    <span className="skel-l">…</span>
                  </div>)}
              </div>
              <button className="btn" style={{ alignSelf: "flex-start" }} onClick={() => setShown(true)}>🧱 {tr3({ uz: "Skeletni ko'rsatish", ru: "Показать скелет" })}</button>
                ...
                <p className="body" style={{ margin: "0 0 10px", color: T.ink2, fontSize: 13.5 }}>{tr3({ uz: "Bo'laklarni to'g'ri tartibda joylang — sudrab yoki bosib. Yuqoridan pastga: header → bo'limlar → footer.", ru: "Расставьте блоки в верном порядке — перетаскивая или нажимая. Сверху вниз: header → разделы → footer." })}</p>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [name, setName] = useState4(typeof storedAnswer?.name === "string" ? storedAnswer.name : "");
  const [role, setRole] = useState4(typeof storedAnswer?.role === "string" ? storedAnswer.role : "");
  const [done, setDone] = useState4(!!storedAnswer);
  const dispName = name.trim() || "Aziza Karimova";
  const dispRole = role.trim() || tr3({ uz: "Frontend dasturchi", ru: "Frontend-разработчик" });
  const add = () => {
    setDone(true);
    onAnswer(screen, { correct: true, name: name.trim(), role: role.trim() });
  };
  return <Stage eyebrow="Build · Header" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Headerni qo'shing", ru: "Добавьте header" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Saytingiz qanday <span className="italic" style={{ color: T.accent }}>boshlanadi</span>?</>, ru: <>С чего <span className="italic" style={{ color: T.accent }}>начинается</span> ваш сайт?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Eng yuqoridagi blok — <b style={{ color: T.ink }}>header</b>. Ismingizni <span className="mono">{"<h1>"}</span> (asosiy sarlavha), kasbingizni <span className="mono">{"<p>"}</span> (oddiy matn) ichiga yozamiz. To'ldiring va qo'shing.</>, ru: <>Самый верхний блок — <b style={{ color: T.ink }}>header</b>. Имя запишем в <span className="mono">{"<h1>"}</span> (главный заголовок), профессию — в <span className="mono">{"<p>"}</span> (обычный текст). Заполните и добавьте.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            <div className="build-in fade-up delay-2">
              <span className="fld-label">{tr3({ uz: "Ismingiz (h1)", ru: "Ваше имя (h1)" })}</span>
              <input className="text-input" value={name} placeholder={tr3({ uz: "masalan: Aziza Karimova", ru: "например: Азиза Каримова" })} onChange={(e) => setName(e.target.value)} />
              <span className="fld-label" style={{ marginTop: 4 }}>{tr3({ uz: "Kasbingiz / yo'nalishingiz (p)", ru: "Ваша профессия / направление (p)" })}</span>
              <input className="text-input" value={role} placeholder={tr3({ uz: "masalan: Frontend dasturchi", ru: "например: Frontend-разработчик" })} onChange={(e) => setRole(e.target.value)} />
            </div>
            <pre className="code-box fade-up delay-2"><Tg>{"<header>"}</Tg>{"\n  "}<Tg>{"<h1>"}</Tg>{dispName}<Tg>{"</h1>"}</Tg>{"\n  "}<Tg>{"<p>"}</Tg>{dispRole}<Tg>{"</p>"}</Tg>{"\n"}<Tg>{"</header>"}</Tg></pre>
            {!done && <button className="btn" style={{ alignSelf: "flex-start" }} onClick={add}>➕ {tr3({ uz: "Headerni qo'shish", ru: "Добавить header" })}</button>}
          </div>
          <div className="col">
            <div className="flow-label">{tr3({ uz: "Brauzerda (hozircha bezaksiz)", ru: "В браузере (пока без оформления)" })}</div>
            <Preview title="portfolio.html" minH={140}>
              {done ? <div className="fade-step"><SiteRender name={dispName} role={dispRole} parts={["header"]} /></div> : <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center" }}>{tr3({ uz: `"Headerni qo'shish" ni bosing — sarlavha shu yerda chiqadi`, ru: "Нажмите «Добавить header» — заголовок появится здесь" })}</p>}
            </Preview>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <><span className="mono">{"<h1>"}</span> — sahifadagi eng katta, eng muhim sarlavha. Bir sahifada bitta <span className="mono">{"<h1>"}</span> bo'ladi.</>, ru: <><span className="mono">{"<h1>"}</span> — самый большой и главный заголовок страницы. На странице бывает один <span className="mono">{"<h1>"}</span>.</> })}</p></div>}
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen4 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr3({ uz: <>Sahifadagi <span className="italic" style={{ color: T.accent }}>eng katta</span>, asosiy sarlavha (ismingiz) qaysi teg bilan yoziladi?</>, ru: <>Каким тегом пишется <span className="italic" style={{ color: T.accent }}>самый большой</span>, главный заголовок страницы (ваше имя)?</> })}</h2></>}
...
/>;
var Screen5 = ({ screen, answers, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const pick = (o) => {
    if (solved) return;
    setPicked(o);
    if (o === CORRECT && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: o });
  };
  return <Stage eyebrow={tr3({ uz: "Build · Navigatsiya", ru: "Build · Навигация" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!solved} label={solved ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "To'g'ri tegni tanlang", ru: "Выберите верный тег" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Bo'limlar orasida qanday <span className="italic" style={{ color: T.accent }}>yuriladi</span>?</>, ru: <>Как <span className="italic" style={{ color: T.accent }}>переходить</span> между разделами?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Header ichiga <b style={{ color: T.ink }}>menyu</b> qo'shamiz — bosilsa bo'limga o'tadigan havolalar. Bosiladigan havola tegi qaysi? Tanlang — bo'sh joy <span className="slot">?</span> to'ladi.</>, ru: <>Внутрь header добавим <b style={{ color: T.ink }}>меню</b> — ссылки, ведущие к разделам. Какой тег у кликабельной ссылки? Выберите — пустое место <span className="slot">?</span> заполнится.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            <pre className="code-box fade-up delay-2"><Tg>{"<nav>"}</Tg>{"\n  "}<span className={`slot ${solved ? "filled" : ""}`}>{solved ? '<a href="#about">' : "<?>"}</span>{tr3({ uz: "Men haqimda", ru: "Обо мне" })}<span className={`slot ${solved ? "filled" : ""}`}>{solved ? "</a>" : "</?>"}</span>{"\n  "}<span style={{ opacity: 0.55 }}>{tr3({ uz: "… (yana 2 ta havola)", ru: "… (ещё 2 ссылки)" })}</span>{"\n"}<Tg>{"</nav>"}</Tg></pre>
            <p className="fld-label">{tr3({ uz: "Bo'sh joyga qaysi teg mos keladi?", ru: "Какой тег подходит в пустое место?" })}</p>
            <div className="tagpick fade-up delay-3">
              {OPTS.map((o) => {
    const isC = o === CORRECT;
    let cls = "chip";
    if (picked === o) cls += isC ? " chip-on" : " chip-wrong";
    return <button key={o} className={cls} disabled={solved} onClick={() => pick(o)}><span className="mono">{o}</span></button>;
  })}
            </div>
            <FeedbackBlock show={picked !== null} isCorrect={solved}>
              <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: solved ? T.success : T.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>{solved ? tr3({ uz: "To'g'ri", ru: "Верно" }) : tr3({ uz: "Yana urinib ko'ring", ru: "Попробуйте ещё раз" })}</p>
              <p className="body" style={{ margin: 0 }}>{solved ? tr3({ uz: <>Aniq! <span className="mono">{"<a>"}</span> (anchor) — bosiladigan havola. <span className="mono">href</span> atributi qayerga o'tishni ko'rsatadi.</>, ru: <>Точно! <span className="mono">{"<a>"}</span> (anchor) — кликабельная ссылка. Атрибут <span className="mono">href</span> указывает, куда переходить.</> }) : tr3({ uz: "Bu teg havola emas. Bosilganda boshqa joyga o'tkazadigan teg — anchor.", ru: "Этот тег — не ссылка. Тег, который при клике ведёт в другое место, — anchor." })}</p>
            </FeedbackBlock>
          </div>
          <div className="col">
            <div className="flow-label">{tr3({ uz: "Brauzerda", ru: "В браузере" })}</div>
            <Preview title="portfolio.html" minH={140}>
              {solved ? <div className="fade-step"><SiteRender name={pf.name} role={pf.role} parts={["header", "nav"]} /></div> : <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center" }}>{tr3({ uz: "To'g'ri tegni tanlang — menyu havolalari paydo bo'ladi", ru: "Выберите верный тег — появятся ссылки меню" })}</p>}
            </Preview>
            {solved && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Havolalar hozir <b>ko'k va tagi chizilgan</b> — bu brauzerning standart ko'rinishi. CSS darsida ularni chiroyli qilamiz.</>, ru: <>Ссылки сейчас <b>синие и подчёркнутые</b> — это стандартный вид браузера. На уроке CSS сделаем их красивыми.</> })}</p></div>}
          </div>
        </div>
        </Zoomable>
      </div>
...
/>;
var Screen7 = ({ screen, answers, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Build · Men haqimda", ru: "Build · Обо мне" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!solved} label={solved ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "To'g'ri atributni tanlang", ru: "Выберите верный атрибут" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>O'zingiz haqingizda qanday <span className="italic" style={{ color: T.accent }}>aytasiz</span>?</>, ru: <>Как вы <span className="italic" style={{ color: T.accent }}>расскажете</span> о себе?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Bu bo'lim <span className="mono">{"<section>"}</span> ichida: sarlavha <span className="mono">{"<h2>"}</span>, rasm <span className="mono">{"<img>"}</span> va matn <span className="mono">{"<p>"}</span>. Rasm fayliga qaysi atribut yo'l ko'rsatadi? Tanlang.</>, ru: <>Этот раздел внутри <span className="mono">{"<section>"}</span>: заголовок <span className="mono">{"<h2>"}</span>, картинка <span className="mono">{"<img>"}</span> и текст <span className="mono">{"<p>"}</span>. Какой атрибут указывает путь к файлу картинки? Выберите.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            <pre className="code-box fade-up delay-2"><Tg>{"<section "}</Tg><At>id</At>=<Sr>"about"</Sr><Tg>{">"}</Tg>{"\n  "}<Tg>{"<h2>"}</Tg>{tr3({ uz: "Men haqimda", ru: "Обо мне" })}<Tg>{"</h2>"}</Tg>{"\n  "}<Tg>{"<img "}</Tg><span className={`slot ${solved ? "filled" : ""}`}>{solved ? "src" : "?"}</span>=<Sr>"rasm.jpg"</Sr> <At>alt</At>=<Sr>{tr3({ uz: '"Mening rasmim"', ru: '"Моё фото"' })}</Sr><Tg>{">"}</Tg>{"\n  "}<Tg>{"<p>"}</Tg>{tr3({ uz: "Salom! Men o'quvchiman.", ru: "Привет! Я ученик." })}<Tg>{"</p>"}</Tg>{"\n"}<Tg>{"</section>"}</Tg></pre>
            <p className="fld-label">{tr3({ uz: "Rasm fayliga qaysi atribut yo'l ko'rsatadi?", ru: "Какой атрибут указывает путь к файлу картинки?" })}</p>
            <div className="tagpick fade-up delay-3">
              {OPTS.map((o) => {
    const isC = o === CORRECT;
    let cls = "chip";
    ...
    return <button key={o} className={cls} disabled={solved} onClick={() => pick(o)}><span className="mono">{o}</span></button>;
            ...
            <FeedbackBlock show={picked !== null} isCorrect={solved}>
              <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: solved ? T.success : T.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>{solved ? tr3({ uz: "To'g'ri", ru: "Верно" }) : tr3({ uz: "Yana urinib ko'ring", ru: "Попробуйте ещё раз" })}</p>
              <p className="body" style={{ margin: 0 }}>{solved ? tr3({ uz: <><span className="mono">src</span> (source) — rasm faylining manzili. <span className="mono">alt</span> esa rasm yuklanmasa, o'rniga chiqadigan matn.</>, ru: <><span className="mono">src</span> (source) — адрес файла картинки. А <span className="mono">alt</span> — текст, который появится вместо картинки, если она не загрузилась.</> }) : tr3({ uz: <>Bu atribut rasm manzilini ko'rsatmaydi. Manzil uchun — <span className="mono">src</span>.</>, ru: <>Этот атрибут не указывает адрес картинки. Для адреса — <span className="mono">src</span>.</> })}</p>
            </FeedbackBlock>
          </div>
          <div className="col">
            <div className="flow-label">{tr3({ uz: "Brauzerda", ru: "В браузере" })}</div>
            <Preview title="portfolio.html" minH={140}>
              {solved ? <div className="fade-step"><SiteRender name={pf.name} role={pf.role} parts={["about"]} /></div> : <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center" }}>{tr3({ uz: "To'g'ri atributni tanlang — bo'lim chiqadi", ru: "Выберите верный атрибут — раздел появится" })}</p>}
            </Preview>
            {solved && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <><span className="mono">{"<img>"}</span> — yopiladigan jufti yo'q (bo'sh teg). U <span className="mono">src</span> (manzil) va <span className="mono">alt</span> (muqobil matn) atributlari bilan ishlaydi.</>, ru: <><span className="mono">{"<img>"}</span> — без закрывающей пары (пустой тег). Он работает с атрибутами <span className="mono">src</span> (адрес) и <span className="mono">alt</span> (альтернативный текст).</> })}</p></div>}
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen8 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr3({ uz: "«Men haqimda» rasmingiz yuklanmasa, uning o'rniga matn ko'rsatadigan (va ko'rish imkoni cheklanganlar uchun muhim) atribut qaysi?", ru: "Если фото из «Обо мне» не загрузится, какой атрибут покажет вместо него текст (важный и для людей с нарушением зрения)?" })}</h2></>}
...
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [added, setAdded] = useState4(Array.isArray(storedAnswer?.added) ? storedAnswer.added : []);
  const done = added.length >= 2;
  useEffect5(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, added });
  }, [done, added]);
  const add = (s) => {
    if (added.includes(s)) return;
    setAdded((a) => [...a, s]);
  };
  return <Stage eyebrow={tr3({ uz: "Build · Loyihalar", ru: "Build · Проекты" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `Yana ${2 - added.length} ta qo'shing`, ru: `Добавьте ещё ${2 - added.length}` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Loyihalaringizni qanday <span className="italic" style={{ color: T.accent }}>ro'yxatlaysiz</span>?</>, ru: <>Как <span className="italic" style={{ color: T.accent }}>оформить списком</span> ваши проекты?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Loyihalar — ro'yxat: <span className="mono">{"<ul>"}</span> butun ro'yxat, ichida har biri <span className="mono">{"<li>"}</span>, va bosish uchun <span className="mono">{"<a>"}</span> havola. Pastdagi loyihalarni bosing — har biri yangi <span className="mono">{"<li>"}</span> bo'lib qo'shiladi.</>, ru: <>Проекты — это список: <span className="mono">{"<ul>"}</span> — весь список, внутри каждый пункт — <span className="mono">{"<li>"}</span>, а для клика — ссылка <span className="mono">{"<a>"}</span>. Нажимайте проекты внизу — каждый добавится новым <span className="mono">{"<li>"}</span>.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            <p className="fld-label">{tr3({ uz: "Loyiha qo'shing (kamida 2 ta)", ru: "Добавьте проект (минимум 2)" })}</p>
            <div className="tagpick fade-up delay-2">
              {POOL.map((p) => {
    const s = tr3(p);
    return <button key={p.uz} className={`chip ${added.includes(s) ? "chip-on" : ""}`} disabled={added.includes(s)} onClick={() => add(s)}>{added.includes(s) ? "✓ " : "+ "}{s}</button>;
  })}
            </div>
            <pre className="code-box fade-up delay-3"><Tg>{"<ul>"}</Tg>{added.length === 0 ? <span style={{ opacity: 0.5 }}>{"\n  "}{tr3({ uz: "… loyiha qo'shing", ru: "… добавьте проект" })}</span> : added.map((s, i) => <React3.Fragment key={i}>{"\n  "}<Tg>{"<li>"}</Tg><Tg>{"<a "}</Tg><At>href</At>=<Sr>"#"</Sr><Tg>{">"}</Tg>{s}<Tg>{"</a>"}</Tg><Tg>{"</li>"}</Tg></React3.Fragment>)}{"\n"}<Tg>{"</ul>"}</Tg></pre>
          </div>
          <div className="col">
            <div className="flow-label">{tr3({ uz: "Brauzerda", ru: "В браузере" })}</div>
            <Preview title="portfolio.html" minH={140}>
              {added.length > 0 ? <div className="raw fade-step"><h2>{tr3({ uz: "Loyihalarim", ru: "Мои проекты" })}</h2><ul>{added.map((s, i) => <li key={i}><a>{s}</a></li>)}</ul></div> : <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center" }}>{tr3({ uz: "Loyiha qo'shing — ro'yxat shu yerda o'sadi", ru: "Добавляйте проекты — список растёт здесь" })}</p>}
            </Preview>
            {done && <div className="frame-success fade-step"><p className="small mono" style={{ margin: "0 0 4px", fontWeight: 600, color: T.success, textTransform: "uppercase", letterSpacing: "0.08em" }}>✓ {tr3({ uz: "Ro'yxat tayyor", ru: "Список готов" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Har bir <span className="mono">{"<li>"}</span> — alohida loyiha, ichidagi <span className="mono">{"<a>"}</span> esa uni ochadigan havola.</>, ru: <>Каждый <span className="mono">{"<li>"}</span> — отдельный проект, а <span className="mono">{"<a>"}</span> внутри — ссылка, которая его открывает.</> })}</p></div>}
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen10 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr3({ uz: "Mashq · 4-savol", ru: "Упражнение · вопрос 4" })}
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr3({ uz: <>Loyihalar ro'yxatida tartib muhim emas. <span className="italic" style={{ color: T.accent }}>Tartibsiz (nuqtali)</span> ro'yxat qaysi teg bilan yaratiladi?</>, ru: <>В списке проектов порядок не важен. Каким тегом создаётся <span className="italic" style={{ color: T.accent }}>маркированный (с точками)</span> список?</> })}</h2></>}
  options={["`<ol>`", "`<ul>`", "`<li>`", "`<dl>`"]}
  correctIdx={1}
...
/>;
var Screen11 = ({ screen, answers, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Build · Aloqa", ru: "Build · Контакты" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!solved} label={solved ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "To'g'ri variantni tanlang", ru: "Выберите верный вариант" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Sizni qanday <span className="italic" style={{ color: T.accent }}>topishadi</span>?</>, ru: <>Как вас <span className="italic" style={{ color: T.accent }}>найдут</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Portfolioning eng muhim qismi — odamlar siz bilan <b style={{ color: T.ink }}>bog'lana olishi</b>. Email havolasini qo'yamiz: bosilganda pochta dasturi ochilishi uchun <span className="mono">href</span> ichida manzil oldiga maxsus belgi qo'yiladi. Qaysi biri?</>, ru: <>Самая важная часть портфолио — чтобы люди могли <b style={{ color: T.ink }}>связаться</b> с вами. Поставим email-ссылку: чтобы по клику открывалась почтовая программа, в <span className="mono">href</span> перед адресом ставится специальная пометка. Какая?</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            <pre className="code-box fade-up delay-2"><Tg>{"<section "}</Tg><At>id</At>=<Sr>"contact"</Sr><Tg>{">"}</Tg>{"\n  "}<Tg>{"<h2>"}</Tg>{tr3({ uz: "Aloqa", ru: "Контакты" })}<Tg>{"</h2>"}</Tg>{"\n  "}<Tg>{"<a "}</Tg><At>href</At>=<Sr>"</Sr><span className={`slot ${solved ? "filled" : ""}`}>{solved ? "mailto:" : "?"}</span><Sr>{firstName(pf.name)}@gmail.com"</Sr><Tg>{">"}</Tg>Email<Tg>{"</a>"}</Tg>{"\n"}<Tg>{"</section>"}</Tg></pre>
            <p className="fld-label">{tr3({ uz: "Email manzili oldiga nima qo'yiladi?", ru: "Что ставится перед email-адресом?" })}</p>
            <div className="tagpick fade-up delay-3">
              {OPTS.map((o) => {
    const isC = o === CORRECT;
    let cls = "chip";
    ...
    return <button key={o} className={cls} disabled={solved} onClick={() => pick(o)}><span className="mono">{o}</span></button>;
            ...
            <FeedbackBlock show={picked !== null} isCorrect={solved}>
              <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: solved ? T.success : T.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>{solved ? tr3({ uz: "To'g'ri", ru: "Верно" }) : tr3({ uz: "Yana urinib ko'ring", ru: "Попробуйте ещё раз" })}</p>
              <p className="body" style={{ margin: 0 }}>{solved ? tr3({ uz: <><span className="mono">mailto:</span> — email havolasi. Bosilsa, pochta dasturi shu manzilga (masalan <span className="mono">{firstName(pf.name)}@gmail.com</span>) xat yozish uchun ochiladi.</>, ru: <><span className="mono">mailto:</span> — email-ссылка. По клику откроется почтовая программа, чтобы написать письмо на этот адрес (например <span className="mono">{firstName(pf.name)}@gmail.com</span>).</> }) : tr3({ uz: <>Bu email uchun emas. Email havolasi <span className="mono">mailto:</span> bilan boshlanadi.</>, ru: <>Это не для email. Email-ссылка начинается с <span className="mono">mailto:</span>.</> })}</p>
            </FeedbackBlock>
          </div>
          <div className="col">
            <div className="flow-label">{tr3({ uz: "Brauzerda", ru: "В браузере" })}</div>
            <Preview title="portfolio.html" minH={140}>
              {solved ? <div className="fade-step"><SiteRender name={pf.name} role={pf.role} parts={["contact"]} /></div> : <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center" }}>{tr3({ uz: "To'g'ri variantni tanlang — aloqa bo'limi chiqadi", ru: "Выберите верный вариант — появится раздел контактов" })}</p>}
            </Preview>
            {solved && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Bu havola <b>gmail.com</b> bilan ham, istalgan pochta bilan ham ishlaydi — <span className="mono">mailto:</span> har qanday email manziliga mos keladi.</>, ru: <>Эта ссылка работает и с <b>gmail.com</b>, и с любой другой почтой — <span className="mono">mailto:</span> подходит к любому email-адресу.</> })}</p></div>}
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen12 = ({ screen, answers, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="Build · Footer" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!solved} label={solved ? { uz: "Saytni yig'amiz →", ru: "Собираем сайт →" } : { uz: "To'g'ri tegni tanlang", ru: "Выберите верный тег" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Sahifa qanday <span className="italic" style={{ color: T.accent }}>yakunlanadi</span>?</>, ru: <>Чем <span className="italic" style={{ color: T.accent }}>завершается</span> страница?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Eng pastki blok — <b style={{ color: T.ink }}>footer</b> ("oyoq" degani). Bu yerda mualliflik <span className="mono">©</span> va yil yoziladi. Pastki qism uchun qaysi teg? Tanlang.</>, ru: <>Самый нижний блок — <b style={{ color: T.ink }}>footer</b> (значит «нога»). Здесь пишут авторство <span className="mono">©</span> и год. Какой тег для нижней части? Выберите.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            ...
            <p className="fld-label">{tr3({ uz: "Sahifaning pastki qismi qaysi teg?", ru: "Какой тег у нижней части страницы?" })}</p>
            <div className="tagpick fade-up delay-3">
              {OPTS.map((o) => {
    const isC = o === CORRECT;
    let cls = "chip";
    ...
    return <button key={o} className={cls} disabled={solved} onClick={() => pick(o)}><span className="mono">{o}</span></button>;
            ...
            <FeedbackBlock show={picked !== null} isCorrect={solved}>
              <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: solved ? T.success : T.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>{solved ? tr3({ uz: "To'g'ri", ru: "Верно" }) : tr3({ uz: "Yana urinib ko'ring", ru: "Попробуйте ещё раз" })}</p>
              <p className="body" style={{ margin: 0 }}>{solved ? tr3({ uz: <><span className="mono">{"<footer>"}</span> — sahifaning eng pastki, yakuniy qismi. Mualliflik, yil va aloqa shu yerga joylashadi.</>, ru: <><span className="mono">{"<footer>"}</span> — самая нижняя, завершающая часть страницы. Авторство, год и контакты живут здесь.</> }) : tr3({ uz: <>Bu teg pastki qism uchun emas. Sahifa "oyog'i" — <span className="mono">{"<footer>"}</span>.</>, ru: <>Этот тег не для нижней части. «Нога» страницы — <span className="mono">{"<footer>"}</span>.</> })}</p>
            </FeedbackBlock>
          </div>
          <div className="col">
            <div className="flow-label">{tr3({ uz: "Brauzerda", ru: "В браузере" })}</div>
            <Preview title="portfolio.html" minH={140}>
              {solved ? <div className="fade-step"><SiteRender name={pf.name} role={pf.role} parts={["footer"]} /></div> : <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center" }}>{tr3({ uz: "To'g'ri tegni tanlang — footer chiqadi", ru: "Выберите верный тег — footer появится" })}</p>}
            </Preview>
            {solved && <div className="frame-success fade-step"><p className="small mono" style={{ margin: "0 0 4px", fontWeight: 600, color: T.success, textTransform: "uppercase", letterSpacing: "0.08em" }}>✓ {tr3({ uz: "Barcha bo'limlar tayyor", ru: "Все разделы готовы" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: "5 blok ham qurildi! Endi hammasini bitta sahifaga yig'amiz.", ru: "Все 5 блоков построены! Теперь соберём всё на одну страницу." })}</p></div>}
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen13 = (props) => <QuestionScreen
  {...props}
  scope="module-mikro"
  eyebrow={tr3({ uz: "Mashq · 5-savol", ru: "Упражнение · вопрос 5" })}
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "To'g'ri javobni tanlang", ru: "Выберите верный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr3({ uz: <>Sahifaning <span className="italic" style={{ color: T.accent }}>eng pastida</span> (mualliflik ©, yil) joylashadigan qism qaysi teg bilan belgilanadi?</>, ru: <>Каким тегом обозначается часть в <span className="italic" style={{ color: T.accent }}>самом низу</span> страницы (авторство ©, год)?</> })}</h2></>}
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="Debugging" screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Xatoni toping", ru: "Найдите ошибку" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>AI kod yozdi — siz uni <span className="italic" style={{ color: T.accent }}>tekshirasiz</span>.</>, ru: <>AI написал код — вы его <span className="italic" style={{ color: T.accent }}>проверяете</span>.</> })}</h2></div>
        <Mentor>{tr3({ uz: <>AI kod yozishda zo'r yordamchi. Lekin <b style={{ color: T.ink }}>odamlar ham, AI ham</b> ba'zan kichik xato qiladi. Shuni topib tuzatish — <b style={{ color: T.ink }}>debugging</b> deyiladi, va bu eng zo'r mahorat. AI yozgan kodda bitta teg yopilmagan — qaysi qatorda? Bosing.</>, ru: <>AI — отличный помощник в написании кода. Но <b style={{ color: T.ink }}>и люди, и AI</b> иногда делают маленькие ошибки. Найти и исправить их — это <b style={{ color: T.ink }}>дебаггинг</b>, и это крутой навык. В коде от AI один тег не закрыт — в какой строке? Нажмите.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <div className="col">
            <div className="ai-card fade-up delay-2">
              <div className="ai-row"><span className="ai-badge">AI</span><span className="ai-bubble">{tr3({ uz: `Mana, "Men haqimda" bo'limi tayyor!`, ru: "Вот, раздел «Обо мне» готов!" })}</span></div>
              <DebugChallenge lines={DBG_LINES} fixed={DBG_FIXED} explain={{ uz: <><span className="mono">{"<p>"}</span> ochilgan edi, lekin <span className="mono">{"</p>"}</span> bilan yopilmagan edi — endi to'g'ri.</>, ru: <><span className="mono">{"<p>"}</span> был открыт, но не закрыт <span className="mono">{"</p>"}</span> — теперь всё верно.</> }} onSolved={() => setDone(true)} onWrong={() => {
    if (achMiss && !done) achMiss.miss(screen);
          ...
          <div className="col">
            {!done ? <div className="hint"><p className="body zb-notch" style={{ margin: 0, color: T.ink2 }}>{tr3({ uz: <>Endi siz teglarni bilasiz — AI yozgan kodni <b style={{ color: T.ink }}>tekshira olasiz</b>. Qaysi teg ochilib, yopilmagan?</>, ru: <>Теперь вы знаете теги — значит, можете <b style={{ color: T.ink }}>проверить</b> код, написанный AI. Какой тег открылся, но не закрылся?</> })}</p></div> : <>
              <div className="flow-label">{tr3({ uz: "Endi sahifa to'g'ri ishlaydi", ru: "Теперь страница работает правильно" })}</div>
              <Preview title="portfolio.html" minH={110}><div className="raw fade-step"><h2>{tr3({ uz: "Men haqimda", ru: "Обо мне" })}</h2><p>{tr3({ uz: "Salom! Men HTML o'rganyapman.", ru: "Привет! Я изучаю HTML." })}</p></div></Preview>
              <div className="takeaway fade-step"><div className="ta-bulb">🛠️</div><p className="ta-h">{tr3({ uz: "Topdingiz va tuzatdingiz — bu debugging!", ru: "Нашли и исправили — это и есть дебаггинг!" })}</p><p className="ta-sub">{tr3({ uz: "AI tez yozadi, siz tekshirib tuzatasiz — zo'r jamoa", ru: "AI быстро пишет, вы проверяете и чините — отличная команда" })}</p></div>
            </>}
          </div>
        </div>
        </Zoomable>
...
var Screen15 = ({ screen, answers, storedAnswer, onAnswer, onNext, onPrev }) => {
  const pf = usePortfolio(answers);
  ...
  const isNarrow = useIsMobile(768);
  const [built, setBuilt] = useState4(storedAnswer ? ALL : []);
  ...
  const assemble = () => {
    ...
    setRunning(true);
    const tick = (i) => {
      setBuilt(ALL.slice(0, i));
      followDown();
      if (i < ALL.length) timer.current = setTimeout(() => tick(i + 1), 460);
      else setRunning(false);
    };
    timer.current = setTimeout(() => tick(1), 250);
  };
  return <Stage eyebrow={tr3({ uz: "Saytni yig'ish", ru: "Сборка сайта" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Yakuniy savol →", ru: "Финальный вопрос →" } : { uz: "Avval saytni yig'ing", ru: "Сначала соберите сайт" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Hammasini bitta <span className="italic" style={{ color: T.accent }}>saytga</span> yig'amizmi?</>, ru: <>Соберём всё в один <span className="italic" style={{ color: T.accent }}>сайт</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Mana eng zo'r qismi! Tugmani bosing — barcha bloklar <b style={{ color: T.ink }}>birma-bir</b> yig'ilib, sizning to'liq portfolio saytingiz paydo bo'ladi. Bezaksiz — bu normal, CSS keyin.</>, ru: <>Вот самая крутая часть! Нажмите кнопку — все блоки соберутся <b style={{ color: T.ink }}>один за другим</b>, и появится ваш полный сайт-портфолио. Без оформления — это нормально, CSS потом.</> })}</Mentor>
        <Zoomable>
        <div className="split" style={{ alignItems: "stretch" }}>
          <div className="col">
            <button className="btn" style={{ alignSelf: "flex-start" }} onClick={assemble} disabled={running}>{running ? tr3({ uz: "Yig'ilmoqda…", ru: "Собирается…" }) : done ? tr3({ uz: "↻ Yana yig'ish", ru: "↻ Собрать ещё раз" }) : tr3({ uz: "▶ Saytni yig'ish", ru: "▶ Собрать сайт" })}</button>
...
var Screen16 = (props) => <QuestionScreen
  {...props}
  scope="final"
  eyebrow={tr3({ uz: "Yakuniy savol", ru: "Финальный вопрос" })}
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr3({ uz: "Butun saytni eslang", ru: "Вспомните весь сайт" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr3({ uz: <>Portfolio sahifangiz <span className="italic" style={{ color: T.accent }}>yuqoridan pastga</span> qanday tartibda joylashadi?</>, ru: <>В каком порядке идёт ваша страница портфолио <span className="italic" style={{ color: T.accent }}>сверху вниз</span>?</> })}</h2></>}
  options={[{ uz: "Footer → Header → Men haqimda → Loyihalar → Aloqa", ru: "Footer → Header → Обо мне → Проекты → Контакты" }, { uz: "Men haqimda → Header → Loyihalar → Aloqa → Footer", ru: "Обо мне → Header → Проекты → Контакты → Footer" }, { uz: "Header → Men haqimda → Loyihalar → Aloqa → Footer", ru: "Header → Обо мне → Проекты → Контакты → Footer" }, { uz: "Header → Loyihalar → Men haqimda → Footer → Aloqa", ru: "Header → Проекты → Обо мне → Footer → Контакты" }]}
  correctIdx={2}
...
/>;
var Screen17 = ({ screen, answers, achievements, onReset, onPrev, onFinish, onHomework }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Tayyor", ru: "Готово" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "CSS bilan bezashni boshlash →", ru: "Начать оформление с CSS →" })}</button></>}>
    ...
    /* 54-qonun (P0 PmUserStory · PmLesson2 qarori): h-sub qatori YO'Q — sarlavha o'zi yetadi. */
        ...
        <div className="card fade-up d3"><div className="card-lbl" style={{ color: T.success }}><span className="tick" style={{ width: 16, height: 16, borderRadius: "50%", background: T.success, color: "#fff", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 10 }}>✓</span> {tr3({ uz: "Endi siz qura olasiz", ru: "Теперь вы умеете строить" })}</div><ul className="recap">{RECAP.map((r, i) => <li key={i} style={{ animationDelay: `${0.3 + i * 0.06}s` }}><span className="ck">✓</span><span>{tr3(r)}</span></li>)}</ul></div>
...
var RECAPS = {
  ...
  4: {
    title: { uz: "Sarlavhalar: h1 va boshqalar", ru: "Заголовки: h1 и другие" },
    cards: [
      {
        ic: "🔤",
        h: { uz: "h1 — eng katta sarlavha", ru: "h1 — самый большой заголовок" },
      ...
      {
        ic: "📉",
        h: { uz: "h2, h3 — kichikroq sarlavhalar", ru: "h2, h3 — заголовки поменьше" },
      ...
      {
        ic: "📰",
        h: { uz: "p — oddiy matn", ru: "p — обычный текст" },
  ...
  6: {
    title: { uz: "Havolalar: a tegi", ru: "Ссылки: тег a" },
    cards: [
      {
        ic: "🔗",
        h: { uz: "a — bosiladigan havola", ru: "a — кликабельная ссылка" },
      ...
      {
        ic: "🧭",
        h: { uz: "href — qayerga olib boradi", ru: "href — куда ведёт" },
      ...
      {
        ic: "📂",
        h: { uz: "nav — menyu idishi", ru: "nav — контейнер меню" },
  ...
  8: {
    title: { uz: "Rasmlar: img, src, alt", ru: "Картинки: img, src, alt" },
    cards: [
      {
        ic: "🖼️",
        h: { uz: "img — rasm qo'yish", ru: "img — вставить картинку" },
      ...
      {
        ic: "📍",
        h: { uz: "src — rasm manzili", ru: "src — адрес картинки" },
      ...
      {
        ic: "💬",
        h: { uz: "alt — o'rniga chiqadigan matn", ru: "alt — текст вместо картинки" },
  ...
  10: {
    ...
    cards: [
      {
        ic: "•",
        h: { uz: "ul — nuqtali ro'yxat", ru: "ul — маркированный список" },
      ...
      {
        ic: "🔢",
        h: { uz: "ol — raqamli ro'yxat", ru: "ol — нумерованный список" },
      ...
      {
        ic: "📄",
        h: { uz: "li — bitta band", ru: "li — один пункт" },
  ...
  13: {
    title: { uz: "Sahifa qismlari: header va footer", ru: "Части страницы: header и footer" },
    cards: [
      {
        ic: "🔝",
        h: { uz: "header — yuqori qism", ru: "header — верхняя часть" },
      ...
      {
        ic: "🔻",
        h: { uz: "footer — pastki qism", ru: "footer — нижняя часть" },
      ...
      {
        ic: "🧱",
        h: { uz: "section — alohida bo'lim", ru: "section — отдельный раздел" },
  ...
  16: {
    title: { uz: "Sahifa tartibi (yuqoridan pastga)", ru: "Порядок страницы (сверху вниз)" },
    cards: [
      {
        ic: "📐",
        h: { uz: "Header — birinchi", ru: "Header — первый" },
      ...
      {
        ic: "📚",
        h: { uz: "Bo'limlar — o'rtada", ru: "Разделы — посередине" },
      ...
      {
        ic: "🏁",
        h: { uz: "Footer — oxirgi", ru: "Footer — последний" },
...
function MentorPracticeOverlay({ entry, live, onClose }) {
  ...
  if (view === "demo") {
    return <div style={{ position: "fixed", inset: 0, zIndex: 2e3, background: T.bg }}>
        <HtmlCompiler_default task={entry.task} starterCode={entry.starter} lang={uiLang} onContinue={() => setView("watch")} onBack={() => setView("watch")} />
      </div>;
  }
...
function DragDropOrder({ items, hints, onSolved, onWrong }) {
  ...
  useEffect5(() => {
    if (wrong) onWrong && onWrong(slots);
  }, [wrong]);
  ...
  return <div className="dd fade-up">
      <div className="dd-slots">
        {slots.map((sid, i) => <div key={i} ref={(el) => slotRefs.current[i] = el} className={`dd-slot ${sid ? "filled" : ""} ${solved && sid ? "ok" : ""} ${wrong && sid && sid !== order[i] ? "bad" : ""}`}>
            ...
            {sid ? <button className="dd-chip in" onPointerDown={(e) => down(e, sid, i)}>{tr3(byId[sid].label)}</button> : <span className="dd-hint">{hints ? tr3(hints[i]) : tr3({ uz: "bu yerga joylang", ru: "поместите сюда" })}</span>}
          </div>)}
      </div>
  ...
  return <div className="dbg fade-up">
      <div className="dbg-code">
        {lines.map((l, i) => <div key={i} className={`dbg-line ${solved && i === bugIdx ? "fixed" : ""} ${wrongIdx === i ? "wrong" : ""}`} onClick={() => click(i)}>
            ...
            <span className="dbg-txt">{solved && i === bugIdx ? tr3(fixed) : tr3(l.text)}</span>
...
var ACH_TRIGGERS = { s2: "built", s14: "debugger" };
...
var ScreenPodium = ({ screen, answers, achievements, onNext, onPrev }) => {
  ...
  const Badges = () => <div className="card ach-coll fade-up d2">
      <div className="card-lbl" style={{ color: T.accent }}>🏅 {tr3({ uz: "Nishonlaringiz", ru: "Ваши значки" })} — {achN}/{achTotal}</div>
      <div className="ach-grid">
        {Object.entries(ACHIEVEMENTS).map(([id, a]) => {
    const got = !!(achievements && achievements.has(id));
    return <div key={id} className={`ach-badge ${got ? "got" : "locked"}`} title={tr3(a.desc)}>
  ...
  return <Stage eyebrow={tr3({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr3({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
    ...
    return <span key={q} className={`pod-dot ${a ? a.correct ? "ok" : "bad" : ""}`} title={tr3(PODIUM_LABELS[q])} />;
  })}</span>
    ...
    return <div key={q} className="qstat-row">
                      <span className="qstat-lbl">{PODIUM_LABELS[q] ? tr3(PODIUM_LABELS[q]) : `#${q}`}{hard && " ⚠️"}</span>
                      <span className="mstats-track"><span className="mstats-fill" style={{ width: `${pct}%`, background: hard ? T.accent : T.success }} /></span>
                      <span className="mono qstat-n">{okN}/{qa.length}</span>
                    </div>;
  })}
              </div>
              {isMentorL && <p className="small" style={{ margin: "10px 0 0", color: T.ink2 }}>{tr3({ uz: "⚠️ belgili savollar — sinf qiynalgan mavzular. Qayta tushuntirish tavsiya etiladi.", ru: "Вопросы со значком ⚠️ — темы, где класс споткнулся. Рекомендуем объяснить их ещё раз." })}</p>}
            </div>
          </>}
      </div>
    </Stage>;
};
var INLINE_KEYS = { s4: 1, s6: 3, s8: 2, s10: 1, s13: 3, s16: 2 };
var QUIZ_MS = 15e3;
...
var TASK_HEADER = {
  ...
  requirements: [
    ...
    { id: "p", label: { uz: "<p> — kasbingiz", ru: "<p> — ваша профессия" }, check: checks.text("p", { uz: "`<p>` ichiga kasbingizni yozing", ru: "Напишите свою профессию внутри `<p>`" }) }
  ]
};
var STARTER_HEADER = `<!-- Bu yerga yozing -->
`;
var TASK_LIST = {
  ...
  requirements: [
    { id: "ul", label: { uz: "<ul> — ro'yxat", ru: "<ul> — список" }, check: checks.has("ul", { uz: "`<ul>` ro'yxat tegini qo'shing", ru: "Добавьте тег списка `<ul>`" }) },
    { id: "li", label: { uz: "kamida 2 ta <li>", ru: "минимум 2 <li>" }, check: checks.count("li", 2, { uz: "`<ul>` ichida kamida 2 ta `<li>` band yozing", ru: "Напишите минимум 2 пункта `<li>` внутри `<ul>`" }) }
  ]
};
var STARTER_LIST = `<!-- Bu yerga yozing -->
`;
var TASK_FINAL = {
  ...
  requirements: [
    { id: "h1", label: { uz: "<h1> — sarlavha (ism)", ru: "<h1> — заголовок (имя)" }, check: checks.text("h1", { uz: "`<h1>` ichiga sarlavha yozing", ru: "Напишите заголовок внутри `<h1>`" }) },
    { id: "h2", label: { uz: "<h2> — bo'lim sarlavhasi", ru: "<h2> — заголовок раздела" }, check: checks.text("h2", { uz: "`<h2>` ichiga bo'lim sarlavhasini yozing", ru: "Напишите заголовок раздела внутри `<h2>`" }) },
    { id: "li", label: { uz: "ro'yxatda kamida 2 ta <li>", ru: "в списке минимум 2 <li>" }, check: checks.count("li", 2, { uz: "`<ul>` ichida kamida 2 ta `<li>` band yozing", ru: "Напишите минимум 2 пункта `<li>` внутри `<ul>`" }) },
...
var STARTER_FINAL = `<!-- Bu yerga yozing -->
`;
var TASK_HW = {
  ...
  requirements: [
    { id: "h1", label: { uz: "<h1> — ismingiz", ru: "<h1> — ваше имя" }, check: checks.text("h1", { uz: "`<h1>` ichiga ismingizni yozing", ru: "Напишите своё имя внутри `<h1>`" }) },
    { id: "p2", label: { uz: "kamida 2 ta <p> — «Men haqimda» o'z matningiz", ru: "минимум 2 <p> — свой текст «Обо мне»" }, check: checks.count("p", 2, { uz: "O'zingiz haqingizda kamida 2 ta `<p>` gap yozing", ru: "Напишите о себе минимум 2 абзаца `<p>`" }) },
    { id: "li3", label: { uz: "loyihalarda kamida 3 ta <li>", ru: "в проектах минимум 3 <li>" }, check: checks.count("li", 3, { uz: "Loyihalar ro'yxatiga yana bitta ish qo'shing — kamida 3 ta `<li>`", ru: "Добавьте в список проектов ещё одну работу — минимум 3 `<li>`" }) },
    { id: "a", label: { uz: "<a> havola — href bilan", ru: "<a> ссылка — с href" }, check: checks.attr("a", "href", { uz: '`<a href="https://...">matn</a>` havola qo\'shing', ru: 'Добавьте ссылку `<a href="https://...">текст</a>`' }) }
  ]
};
var STARTER_HW = `<!-- Bu yerga yozing -->
`;
var HOMEWORK = {
  type: "tex",
  title: TASK_HW.title,
...
var PRACTICE_AFTER = {
  3: { task: TASK_HEADER, starter: STARTER_HEADER },
  // 1) Header (h1/p)
  9: { task: TASK_LIST, starter: STARTER_LIST },
  // 2) Loyihalar ro'yxati (ul/li)
  15: { task: TASK_FINAL, starter: STARTER_FINAL }
  // 3) Yakuniy (noldan portfolio)
};
function HtmlPractice({ lang: langProp, onFinished, onPractice, liveToken }) {
  ...
  return <LangContext.Provider value={lang}>
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        ...
        <AchMissCtx.Provider value={achMissVal}>
          <div className="lesson-root">
            {live.mode === "choosing" ? <LiveGate live={live} title={{ uz: "1-Modul · Praktika", ru: "Модуль 1 · Практика" }} /> : <>
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 2e3, background: T.bg }}>
                <HtmlCompiler_default task={practice.task} starterCode={practice.starter} storageKey={practice.codeKey} lang={lang} onContinue={practice.done} onBack={() => {
      pracClear(LESSON_META.lessonId);
      setPractice(null);
    }} />
              </div>,
