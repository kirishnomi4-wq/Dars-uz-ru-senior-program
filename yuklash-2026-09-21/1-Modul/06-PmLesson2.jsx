...
//  Manba:  src/1-Modull/PmLesson2.jsx
//          src/compilator/HtmlCompiler.jsx
//  Qayta yig'ish:  node scripts/build-lms.mjs src/1-Modull/PmLesson2.jsx
...
var LESSON_META = { lessonId: "pm-m1d6-v1", lessonTitle: { uz: "Struktura — foydalanuvchi uchun qilingan qulaylik", ru: "Структура — удобство для пользователя" } };
var SCREEN_META = [
...
var ACH_TRIGGERS = { s4: "firstwin", s11: "builder", koding: "strategist" };
...
var RECAPS = {
  ...
  4: {
    title: { uz: "Birinchi ekran — birinchi blok", ru: "Первый экран — первый блок" },
    cards: [
      {
        ic: "👀",
        h: { uz: "Birinchi ekran — birinchi taassurot", ru: "Первый экран — первое впечатление" },
      ...
      {
        ic: "📢",
        h: { uz: "Birinchi blok — bir jumlali va'da", ru: "Первый блок — обещание в одной фразе" },
      ...
      {
        ic: "⛔",
        h: { uz: "Tugma birinchi emas", ru: "Кнопка — не первая" },
  ...
  6: {
    title: { uz: "Harakat tugmasi — oxirida", ru: "Кнопка действия — в конце" },
    cards: [
      {
        ic: "🖱️",
        h: { uz: "CTA — harakatga chaqiruvchi tugma", ru: "CTA — кнопка призыва к действию" },
      ...
      {
        ic: "🤝",
        h: { uz: "Avval ishonch — keyin tugma", ru: "Сначала доверие — потом кнопка" },
      ...
      {
        ic: "📍",
        h: { uz: "Shuning uchun — sababdan keyin", ru: "Поэтому — после причины" },
  ...
  9: {
    title: { uz: "Tartib — foydalanuvchi uchun qulaylik", ru: "Порядок — удобство для пользователя" },
    cards: [
      {
        ic: "🧭",
        h: { uz: "Tartib foydalanuvchini yetaklaydi", ru: "Порядок ведёт пользователя" },
      ...
      {
        ic: "🎯",
        h: { uz: "Bezak emas — odam uchun qulaylik", ru: "Не украшение — удобство для человека" },
      ...
      {
        ic: "🔀",
        h: { uz: "Aralash tartib — foydalanuvchi adashadi", ru: "Порядок перепутан — человек теряется" },
  ...
  11: {
    title: { uz: "Sahifa tartibi — hikoya", ru: "Порядок страницы — это история" },
    cards: [
      {
        ic: "📖",
        h: { uz: "Sahifa — bu hikoya", ru: "Страница — это история" },
      ...
      {
        ic: "🧩",
        h: { uz: "Har bo'limning o'z o'rni", ru: "У каждого раздела своё место" },
      ...
      {
        ic: "✅",
        h: { uz: "To'g'ri tartib — konversiya", ru: "Верный порядок — конверсия" },
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {reveal ? <div className="mstats-big">
          ...
          <div className="mstats-chip badc"><span className="mstats-chip-n">{bad}</span><span className="mstats-chip-t">{tr3({ uz: "noto'g'ri ❌", ru: "неверно ❌" })}</span></div>
    ...
    return <div className={`mstats-verdict ${level}`}>
      ...
      {reveal && struggling && <p className="mstats-warn">{tr3({ uz: "⚠️ Ko'pchilik noto'g'ri javob berdi — bu mavzu tushunarsiz bo'lgan ko'rinadi. Qayta tushuntirish tavsiya etiladi.", ru: "⚠️ Большинство ответило неверно — похоже, тема осталась непонятной. Стоит объяснить заново." })}</p>}
    ...
    return <button key={i} className={cls} disabled={solved || isMentorLive} onClick={() => pick(i)} style={{ padding: picked !== null ? "clamp(9px,1.3vw,12px) clamp(15px,2.2vw,20px)" : "clamp(13px,1.9vw,17px) clamp(15px,2.2vw,20px)", fontSize: "clamp(15px,1.85vw,17px)", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <FeedbackBlock show={isMentorLive ? mReveal : picked !== null} isCorrect={isMentorLive ? true : solved && !wrongLocked} neutral={waiting}>
          ...
          <p className="body" style={{ margin: 0 }}>
            {fmtCode(tr3(isMentorLive ? explainCorrect : waiting ? { uz: "Hozir to'g'ri javobni bilib olasiz.", ru: "Сейчас узнаете верный ответ." } : wrongLocked ? explainWrong[picked] ?? explainWrong.default : solved ? explainCorrect : explainWrong[picked] ?? explainWrong.default))}
          </p>
          {
...
var IcoChip = ({ color = T.accent, soft = T.accentSoft, children, size = 46 }) => <span style={{ width: size, height: size, borderRadius: 13, background: soft, color, display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{children}</span>;
...
var ssv = { fill: "none", stroke: "currentColor", strokeWidth: 1.7, strokeLinecap: "round", strokeLinejoin: "round" };
var sIco = {
...
var ORDER = ["hero", "muammo", "yechim", "isbot", "harakat"];
var SecView = ({ k }) => {
  if (k === "hero") return <div><div className="site-header" style={{ marginBottom: 8 }}><span className="site-brand"><span className="site-logo" style={{ background: T.accent }}>O</span><span className="site-name">OLX</span></span><span className="site-nav"><span>{tr3({ uz: "Sotish", ru: "Продать" })}</span><span>{tr3({ uz: "Xarid", ru: "Купить" })}</span></span></div><h3 className="site-h3" style={{ margin: 0 }}>{tr3({ uz: "Ortiqcha buyumingizni soting", ru: "Продайте вещь, которой не пользуетесь" })}</h3></div>;
  if (k === "muammo") return <p style={{ fontFamily: G, fontSize: 13.5, color: T.ink2, margin: 0 }}>{tr3({ uz: "Uyda ishlatilmaydigan narsalar bor — sotish esa qiyin va uzoq.", ru: "Дома лежат вещи без дела — а продать их трудно и долго." })}</p>;
  if (k === "yechim") return <p style={{ fontFamily: G, fontSize: 13.5, color: T.ink, margin: 0 }}>{tr3({ uz: <><b>3 daqiqada</b> e'lon bering — minglab xaridor ko'radi.</>, ru: <>Подайте объявление <b>за 3 минуты</b> — его увидят тысячи покупателей.</> })}</p>;
  if (k === "isbot") return <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>{[{ uz: "Velosiped — 450 000", ru: "Велосипед — 450 000" }, { uz: "Telefon — 1 200 000", ru: "Телефон — 1 200 000" }].map((r, i) => <div key={i} style={{ display: "flex", alignItems: "center", gap: 9 }}><span style={{ width: 34, height: 26, borderRadius: 6, background: T.bg, flexShrink: 0, boxShadow: `inset 0 0 0 1px ${T.ink3}30` }} /><span style={{ fontFamily: G, fontSize: 13, color: T.ink }}>{tr3(r)}</span></div>)}</div>;
  if (k === "harakat") return <span style={{ display: "inline-block", background: T.accent, color: "#fff", fontFamily: "'Manrope',sans-serif", fontWeight: 600, fontSize: 13, padding: "9px 18px", borderRadius: 9 }}>{tr3({ uz: "E'lon berish", ru: "Подать объявление" })}</span>;
  return null;
};
var PagePreview = ({ order, url = "olx.uz", minH = 240 }) => <Preview url={url} minH={minH}>
...
function DragDropOrder({ items, hints, onSolved, onOrder, onWrong }) {
  ...
  const byId = React3.useMemo(() => Object.fromEntries(items.map((x) => [x.id, x])), [items]);
  ...
  const resetAll = () => setSt(({ pool: pool2, slots: slots2 }) => ({ pool: [...pool2, ...slots2.filter(Boolean)], slots: order.map(() => null) }));
  ...
  return <div className="dd fade-up">
      ...
      <div className="dd-pool">
        {pool.map((id) => <button key={id} className={`dd-chip ${fresh ? "hint" : ""}`} onPointerDown={(e) => down(e, id, "pool")}>{tr3(byId[id].label)}</button>)}
      </div>
      {solved && <div className="dd-done">{tr3({ uz: "✓ To'g'ri! Sahifa aynan shu tartibda.", ru: "✓ Верно! Страница идёт именно в таком порядке." })}</div>}
      {wrong && !solved && <div className="dd-wrong">{tr3({ uz: "⚠️ Tartib noto'g'ri.", ru: "⚠️ Порядок неверный." })}<button className="dd-retry" onClick={resetAll}>{tr3({ uz: "↻ Qayta joylash", ru: "↻ Разложить заново" })}</button></div>}
    </div>;
}
function CustomerRun({ order, correct }) {
  const [state, setState] = useState4("idle");
  const [step, setStep] = useState4(-1);
  const [confidence, setConfidence] = useState4(0);
  const [badAt, setBadAt] = useState4(-1);
  ...
  const send = () => {
    ...
    const walk = (i) => {
      ...
      if (order[i] !== correct[i]) {
        setStep(i);
        setBadAt(i);
        timer.current = setTimeout(() => setState("bounce"), 480);
        return;
      }
      setStep(i);
      setConfidence((c) => c + 1);
  ...
  return <div className="cr">
      <p className="flow-label">{tr3({ uz: "Foydalanuvchi — sahifangizni birinchi marta ochib ko'rayotgan odam", ru: "Пользователь — человек, который открыл вашу страницу впервые" })}</p>
      <div className="cr-page">
        {rows.map((k, i) => {
    const here = step === i && (walking || state === "bounce");
    const done = state !== "idle" && step > i;
    const bad = state === "bounce" && badAt === i;
    return <div key={i} className={`cr-row ${here ? "here" : ""} ${done ? "done" : ""} ${bad ? "bad" : ""}`}>
              <div style={{ flex: 1, minWidth: 0 }}>{k ? <SecView k={k} /> : <span className="cr-empty">{tr3({ uz: "— bo'sh —", ru: "— пусто —" })}</span>}</div>
      ...
      <div className="cr-conf">
        <span className="cr-conf-lbl">{tr3({ uz: "Ishonch", ru: "Доверие" })}</span>
...
function Flashcards({ cards }) {
  ...
  if (!card) return <div className="fc-done fade-up"><span className="fc-done-emoji">🎉</span><p className="fc-done-h">{tr3({ uz: "Hammasini bilasiz!", ru: "Вы знаете всё!" })}</p><p className="fc-done-s">{tr3({ uz: `${total}/${total} karta yodlandi`, ru: `Запомнено карточек: ${total}/${total}` })}</p><button className="fc-btn ghost" onClick={restart}>{tr3({ uz: "↻ Qaytadan takrorlash", ru: "↻ Повторить заново" })}</button></div>;
...
var Zoomable = ({ children }) => {
  ...
  return <>
      ...
      <div className={`zoomable ${big ? "zoom-on" : ""}`}>
        <button type="button" className="zoom-btn" onClick={() => setBig((b) => !b)} aria-label={tr3(big ? { uz: "Kichraytirish", ru: "Уменьшить" } : { uz: "Kattalashtirish", ru: "Увеличить" })} title={tr3(big ? { uz: "Kichraytirish", ru: "Уменьшить" } : { uz: "Kattalashtirish", ru: "Увеличить" })}>{big ? "✕" : "⛶"}</button>
        {children}
      </div>
    </>;
};
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  const [v, setV] = useState4("good");
  const [picked, setPicked] = useState4(storedAnswer?.picked ?? null);
  const [seenBad, setSeenBad] = useState4(false);
        ...
        <Split>
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8 }}>
              <button className={`chip ${v === "good" ? "chip-on" : ""}`} onClick={() => setV("good")}>{tr3({ uz: "Tartibli", ru: "По порядку" })}</button>
              <button className={`chip ${v === "bad" ? "chip-on" : ""}${cmpHint ? " turn-ring" : ""}`} onClick={() => {
    setV("bad");
    setSeenBad(true);
  }}>{tr3({ uz: "Aralash", ru: "Вперемешку" })}</button>
            </div>
            <div key={v}><PagePreview order={v === "good" ? goodOrder : badOrder} minH={232} /></div>
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr3({ uz: "Sizningcha, ikkalasining farqi nimada?", ru: "Как по-вашему, в чём между ними разница?" })}</p>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const PreviewBlock = <Col>
      <p className="flow-label">{tr3({ uz: "Asosiy g'oya", ru: "Главная мысль" })}</p>
      <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <Idea ic={sIco.star(22)} c={SEC_AMBER} h={tr3({ uz: "TARTIB — O'YLAB QILINGAN TANLOV", ru: "ПОРЯДОК — ОБДУМАННЫЙ ВЫБОР" })} t={tr3({ uz: "Har bo'limning o'rni foydalanuvchiga qarab hal qilinadi", ru: "Место каждого раздела решают, глядя на пользователя" })} />
        <Idea ic={Ico.arrow(22)} c={T.accent} h={tr3({ uz: "SAHIFA — KICHIK HIKOYA", ru: "СТРАНИЦА — МАЛЕНЬКАЯ ИСТОРИЯ" })} t={tr3({ uz: "U odamni qadam-baqadam kerakli ishgacha yetaklab boradi", ru: "Она шаг за шагом ведёт человека к нужному действию" })} />
      </div>
    </Col>;
  const StepsBlock = <Col><p className="flow-label">{tr3({ uz: "5 qadam", ru: "5 шагов" })}</p><ol className="roadmap">{STEPS.map((s, i) => <li key={i} className="step-card fade-up" style={{ animationDelay: `${0.08 + i * 0.05}s` }}><span className="step-num">{String(i + 1).padStart(2, "0")}</span><span className="step-body"><span className="step-text">{tr3(s.text)}</span>{s.tag && <span className="step-tag">{s.tag}</span>}</span></li>)}</ol></Col>;
  return <Stage eyebrow={tr3({ uz: "Reja", ru: "План" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Boshlaymiz →", ru: "Начинаем →" }} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Saytni <span className="italic" style={{ color: T.accent }}>qaysi tartibda</span> tuzsak tushunarli bo'ladi?</>, ru: <><span className="italic" style={{ color: T.accent }}>В каком порядке</span> собрать сайт, чтобы он был понятным?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Yaxshi sayt bo'limlarni to'g'ri tartibda joylashtiradi — bugun siz ham shu <b style={{ color: T.ink }}>tartibni</b> o'rganasiz. Yo'limiz — 5 qadam: ro'yxatga qarang, so'ng boshlaymiz.</>, ru: <>Хороший сайт расставляет разделы в верном порядке — сегодня вы освоите этот <b style={{ color: T.ink }}>порядок</b>. Наш путь — 5 шагов: посмотрите список, и начинаем.</> })}</Mentor>
        {!isNarrow ? <Zoomable><Split>{PreviewBlock}{StepsBlock}</Split></Zoomable> : !showSteps ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>{PreviewBlock}<button className="btn" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(true)}>{tr3({ uz: "5 qadamni ko'rish", ru: "Посмотреть 5 шагов" })}</button></div> : <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}><button className="btn-soft" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(false)}>{tr3({ uz: "↩ G'oyani ko'rish", ru: "↩ К главной мысли" })}</button>{StepsBlock}</div>}
      </div>
    </Stage>;
};
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [top, setTop] = useState4("hero");
  const [seen, setSeen] = useState4(/* @__PURE__ */ new Set(["hero"]));
  ...
  return <Stage eyebrow={tr3({ uz: "Birinchi ekran", ru: "Первый экран" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Ikkalasini ko'ring", ru: "Посмотрите оба" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Saytga kirgan foydalanuvchi avval <span className="italic" style={{ color: T.accent }}>nimani</span> ko'radi?</>, ru: <><span className="italic" style={{ color: T.accent }}>Что</span> пользователь видит на сайте первым?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Saytga kirgan foydalanuvchi dastlab faqat sahifaning <b style={{ color: T.ink }}>yuqori qismini</b> ko'radi — qolish yoki ketish shu yerda hal bo'ladi. Pastdagi ikki tugmani bosib, farqni o'zingiz ko'ring.</>, ru: <>Зайдя на сайт, пользователь сначала видит только <b style={{ color: T.ink }}>верхнюю часть</b> страницы — остаться или уйти решается именно здесь. Нажмите две кнопки внизу и увидите разницу сами.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", gap: 8 }}>
              <button className={`chip ${top === "hero" ? "chip-on" : ""}${turnCls(lit2, "hero", pend2.length > 1)}`} onClick={() => set("hero")}>{tr3({ uz: "Tepada: birinchi blok", ru: "Наверху: первый блок" })}</button>
              <button className={`chip ${top === "cta" ? "chip-on" : ""}${turnCls(lit2, "cta", pend2.length > 1)}`} onClick={() => set("cta")}>{tr3({ uz: "Tepada: tugma", ru: "Наверху: кнопка" })}</button>
            </div>
            <div key={top}><PagePreview order={order} minH={210} /></div>
          </Col>
          <Col>
            {top === "hero" ? <div className="frame-success fade-step" key="h"><p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: T.success, textTransform: "uppercase", letterSpacing: "0.08em" }}>{tr3({ uz: "Tushunarli", ru: "Понятно" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <><b>Sayt nima taklif qilayotgani</b> birinchi qarashdayoq ko'rinib turibdi — foydalanuvchi qiziqib, saytdan foydalanishda davom etadi.</>, ru: <><b>Что предлагает сайт</b>, видно с первого взгляда — пользователю интересно, и он остаётся.</> })}</p></div> : <div className="frame-warn fade-step" key="c"><p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: AMBER_INK, textTransform: "uppercase", letterSpacing: "0.08em" }}>{tr3({ uz: "Tushunarsiz", ru: "Непонятно" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Tepada <b>tugma</b>, lekin nega bosish kerakligi noma'lum. Foydalanuvchi tushunmay saytga kirmay qo'yadi.</>, ru: <>Наверху <b>кнопка</b>, но зачем на неё жать — неясно. Пользователь не понимает и уходит.</> })}</p></div>}
            {done && <div className="frame-soft fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Shuning uchun eng tepaga <b>birinchi blok (hero)</b> qo'yiladi: katta sarlavha va izoh.</>, ru: <>Поэтому в самый верх ставят <b>первый блок (hero)</b>: крупный заголовок и пояснение.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Real saytlar", ru: "Настоящие сайты" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `${seen.size}/3 ko'ring`, ru: `Посмотрите ${seen.size}/3` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Uchta mashhur saytni bosib, <span className="italic" style={{ color: T.accent }}>tuzilishini</span> solishtiring</>, ru: <>Нажмите на три известных сайта и сравните их <span className="italic" style={{ color: T.accent }}>структуру</span></> })}</h2></div>
        <Mentor>{tr3({ uz: <>Mashhur saytlar ham shu qoidaga amal qiladi. Pastdagi uchta saytni <b style={{ color: T.ink }}>birma-bir bosing</b> — har birida bo'limlar qaysi tartibda turgani ochiladi.</>, ru: <>Известные сайты живут по тому же правилу. Нажмите три сайта внизу <b style={{ color: T.ink }}>по очереди</b> — у каждого откроется порядок разделов.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {Object.keys(SITES).map((k) => <button key={k} className={turnCls(lit3, k, pend3.length > 1).trim()} onClick={() => tap(k)} style={{ display: "flex", alignItems: "center", gap: 11, cursor: "pointer", border: "none", borderRadius: 12, padding: "13px 15px", background: T.paper, boxShadow: active === k ? `inset 0 0 0 2px ${T.accent}, 0 8px 20px -7px rgba(91,61,230,0.22)` : `0 6px 16px -8px rgba(${T.shadowBase},0.16)`, transition: "all 0.18s" }}><span style={{ display: "inline-flex" }}>{SITES[k].ic}</span><span style={{ fontFamily: "'Manrope',sans-serif", fontWeight: 600, fontSize: 15, color: T.ink }}>{tr3(SITES[k].name)}</span>{seen.has(k) && <span style={{ marginLeft: "auto", color: T.success, display: "inline-flex" }}>{Ico.check(14)}</span>}</button>)}
            </div>
          </Col>
          <Col>
            {active ? <div className="sk-info fade-step" key={active}><span className="sk-tagbig"><span style={{ display: "inline-flex" }}>{SITES[active].ic}</span><span className="sk-wordbadge">{tr3(SITES[active].name)}</span></span><div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 12 }}>{SITES[active].rows.map((r, i) => <div key={i} style={{ display: "flex", alignItems: "center", gap: 9 }}><span className="mono" style={{ fontSize: 11, fontWeight: 700, color: T.accent, minWidth: 14 }}>{i + 1}</span><span className="body" style={{ margin: 0, color: T.ink }}>{tr3(r)}</span></div>)}</div></div> : !isNarrow ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr3({ uz: "Bir saytni bosing", ru: "Нажмите на любой сайт" })}</p></div> : null}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Ko'rdingizmi — uchalasida ham <b>eng muhim narsa birinchi blokda</b> turibdi. Tugma esa eng oxirida turibdi — foydalanuvchi nega bosish kerakligini tushungandan keyin chiqadi.</>, ru: <>Видите — у всех трёх <b>самое важное стоит в первом блоке</b>. А кнопка — в самом конце: она появляется, когда человек уже понял, зачем на неё жать.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Bo'lim vazifasi", ru: "Задача раздела" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `${seen.size}/5 bo'limni oching`, ru: `Откройте разделы ${seen.size}/5` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Sahifadagi beshta bo'lim qanday <span className="italic" style={{ color: T.accent }}>vazifalarni</span> bajaradi?</>, ru: <>Какие <span className="italic" style={{ color: T.accent }}>задачи</span> решают пять разделов страницы?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Yaxshi sahifada ishlamaydigan bo'lim bo'lmaydi — hammasining aniq <b style={{ color: T.ink }}>vazifasi</b> bor. Pastdagi beshta bo'limni birma-bir bosing: bosganingizda uning vazifasi ochiladi.</>, ru: <>На хорошей странице нет бесполезных разделов — у каждого есть своя <b style={{ color: T.ink }}>задача</b>. Нажимайте пять разделов внизу по очереди: при нажатии откроется его задача.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {ORDER.map((k) => {
    const s = SECDATA[k];
    return <button key={k} className={turnCls(lit5, k, pend5.length > 1).trim()} onClick={() => tap(k)} style={{ display: "flex", alignItems: "center", gap: 11, textAlign: "left", cursor: "pointer", border: "none", borderRadius: 12, padding: "12px 14px", background: T.paper, color: active === k ? s.color : T.ink2, boxShadow: active === k ? `inset 0 0 0 2px ${s.color}, 0 8px 20px -8px ${s.color}44` : `0 6px 16px -8px rgba(${T.shadowBase},0.16)`, transition: "all 0.18s" }}><span style={{ color: s.color, display: "inline-flex" }}>{s.ic}</span><span style={{ fontFamily: "'Manrope',sans-serif", fontWeight: 600, fontSize: 14, color: T.ink }}>{tr3(s.label)}</span>{seen.has(k) && <span style={{ marginLeft: "auto", color: T.success, display: "inline-flex" }}>{Ico.check(14)}</span>}</button>;
  })}
            </div>
          </Col>
          <Col>
            {active ? <div className="sk-info fade-step" key={active}><span className="sk-tagbig"><span style={{ color: SECDATA[active].color, display: "inline-flex" }}>{SECDATA[active].ic}</span><span className="sk-wordbadge" style={{ color: SECDATA[active].color, background: SECDATA[active].color + "1c" }}>{tr3(SECDATA[active].label)}</span></span><p className="body" style={{ color: T.ink, margin: "12px 0 0" }}>{tr3(SECDATA[active].job)}</p></div> : !isNarrow ? <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr3({ uz: "Bir bo'limni bosing", ru: "Нажмите на любой раздел" })}</p></div> : null}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: <>Endi beshala bo'limning vazifasini bilasiz. Keyingi qadam — ularni <b>to'g'ri tartibda</b> joylashtirishni o'rganish.</>, ru: <>Теперь вы знаете задачу каждого из пяти разделов. Следующий шаг — научиться ставить их <b>в верном порядке</b>.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen5b = (props) => <QuestionScreen
  ...
  options={[{ uz: "Eng tepada, hammadan ham oldin", ru: "В самом верху, раньше всего" }, { uz: "Sahifa o'rtasida, muammodan oldin", ru: "В середине страницы, до проблемы" }, { uz: "Sabab ko'rsatilgandan keyin, oxirroqda", ru: "После того как показана причина, ближе к концу" }, { uz: "Menyuda, boshqa havolalar orasida", ru: "В меню, среди других ссылок" }]}
  correctIdx={2}
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [step, setStep] = useState4(storedAnswer ? ORDER.length : 0);
  ...
  const run = () => {
    ...
    const tick = (i) => {
      setStep(i);
      if (i < ORDER.length) timer.current = setTimeout(() => tick(i + 1), 720);
      else {
        setRunning(false);
      }
    };
  ...
  return <Stage eyebrow={tr3({ uz: "Tartib — hikoya", ru: "Порядок — это история" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Avval kuzating", ru: "Сначала посмотрите" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.4vw,13px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Sahifa qanday qilib <span className="italic" style={{ color: T.accent }}>hikoyaga</span> aylanadi?</>, ru: <>Как страница превращается в <span className="italic" style={{ color: T.accent }}>историю</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Yaxshi sahifa hikoyaga o'xshaydi: bitta bo'limni o'qigan foydalanuvchi <b style={{ color: T.ink }}>keyingisini ham</b> o'qigisi keladi. Pastdagi «Hikoyani qurish» tugmasini bosing — beshta bo'lim ko'z oldingizda birma-bir joylashib boradi.</>, ru: <>Хорошая страница похожа на историю: прочитав один раздел, человек хочет прочитать и <b style={{ color: T.ink }}>следующий</b>. Нажмите кнопку «Собрать историю» — пять разделов встанут на места у вас на глазах.</> })}</Mentor>
        <Zoomable>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [matched, setMatched] = useState4({});
  ...
  return <Stage eyebrow={tr3({ uz: "Amaliyot", ru: "Практика" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `${Object.keys(matched).length}/4 moslang`, ru: `Соедините ${Object.keys(matched).length}/4` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Qaysi bo'lim qaysi ishni bajaradi — <span className="italic" style={{ color: T.accent }}>topa olasizmi</span>?</>, ru: <>Какой раздел какую работу делает — <span className="italic" style={{ color: T.accent }}>найдёте</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Chapdagi bo'limlardan bittasini tanlang, so'ng o'ng tomondan uning <b style={{ color: T.ink }}>vazifasini</b> toping — to'g'ri topsangiz, juftlik yashil rangga o'zgaradi.</>, ru: <>Выберите слева один раздел, затем найдите справа его <b style={{ color: T.ink }}>задачу</b> — если верно, пара станет зелёной.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <p className="flow-label">{tr3({ uz: "Bo'limlar", ru: "Разделы" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {PAIRS.map((id) => {
    const s = SECDATA[id];
    const m = matched[id];
    const on = sel === id;
    return <button key={id} className={turnCls(lit8, id, pend8.length > 1).trim()} onClick={() => pickP(id)} disabled={m} style={cardBtn({ cursor: m ? "default" : "pointer", opacity: m ? 0.5 : 1, background: m ? T.successSoft : T.paper, boxShadow: on ? `inset 0 0 0 2px ${T.accent}, 0 8px 20px -7px rgba(91,61,230,0.22)` : `0 6px 16px -8px rgba(${T.shadowBase},0.16)` })}><span style={{ color: m ? T.success : s.color, display: "inline-flex" }}>{m ? Ico.check(18) : s.ic}</span><span style={{ flex: 1, fontWeight: 600 }}>{tr3(s.label)}</span></button>;
  })}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr3({ uz: "Vazifalar", ru: "Задачи" })}</p>
            <div className="fade-up delay-2" style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {NEEDS.map((n) => {
    const m = matched[n.id];
    const isWrong = wrong === n.id;
    return <button key={n.id} onClick={() => pickN(n.id)} disabled={m || !sel} style={cardBtn({ cursor: m || !sel ? "default" : "pointer", opacity: m ? 0.55 : 1, color: !sel && !m ? T.ink2 : T.ink, background: m ? T.successSoft : isWrong ? T.errSoft : T.paper, boxShadow: m ? "none" : !sel ? `inset 0 0 0 1.5px ${T.line}` : isWrong ? `inset 0 0 0 1.5px ${T.err}66, 0 6px 16px -8px rgba(${T.shadowBase},0.16)` : `inset 0 0 0 1.5px ${T.accent}44, 0 10px 22px -8px rgba(91,61,230,0.28)` })}><span style={{ flex: 1 }}>{tr3(n.text)}</span>{m && <span style={{ color: T.success, display: "inline-flex" }}>{Ico.check(16)}</span>}</button>;
  })}
            </div>
            {wrong && !done && <p className="small" style={{ color: T.err, margin: 0 }}>{tr3({ uz: "Bu boshqa bo'limning vazifasi — chapdagi bo'lim nima qilishini bir eslang.", ru: "Это задача другого раздела — вспомните, что делает раздел слева." })}</p>}
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr3({ uz: "Zo'r, hammasini to'g'ri uladingiz! Ko'rdingizmi — to'rtta bo'lim to'rt xil ish bajaradi. Shuning uchun birining o'rnini boshqasi bosa olmaydi.", ru: "Отлично, вы соединили всё верно! Видите — четыре раздела делают четыре разные работы. Поэтому один не заменит другого." })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [liveArr, setLiveArr] = useState4(() => correct.map(() => null));
  const [dragDone, setDragDone] = useState4(!!storedAnswer);
  const done = dragDone;
  const achMiss = useContext2(AchMissCtx);
  useEffect5(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, arr: correct });
  }, [done]);
  return <Stage eyebrow={tr3({ uz: "Tartiblash", ru: "Расстановка" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Bo'limlarni tartiblang", ru: "Расставьте разделы" }} onClick={onNext} /></>}>
      {
    /* Mentor gapi bilan ish maydoni orasi biroz kengaytirildi — ular ustma-ust tiqilib
      ...
      <div className="screen" style={{ gap: "clamp(14px,2vw,22px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Siz tuzgan sahifadan foydalanuvchi <span className="italic" style={{ color: T.accent }}>oxirigacha</span> o'ta oladimi?</>, ru: <>Дойдёт ли пользователь по вашей странице <span className="italic" style={{ color: T.accent }}>до конца</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Uchta bo'lakni to'g'ri tartibga joylang, so'ng <b style={{ color: T.ink }}>«Sahifani sinab ko'rish»</b> tugmasini bosing — foydalanuvchi sahifangizni tepadan pastgacha o'qib chiqadi.</>, ru: <>Расставьте три части в верном порядке и нажмите <b style={{ color: T.ink }}>«Проверить страницу»</b> — пользователь прочитает вашу страницу сверху донизу.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <p className="flow-label">{tr3({ uz: "Bo'limlar — to'g'ri tartibga joylang", ru: "Разделы — расставьте в верном порядке" })}</p>
            <DragDropOrder items={items} onSolved={() => setDragDone(true)} onOrder={setLiveArr} onWrong={() => {
...
/>;
var Screen14 = ({ screen, onNext, onPrev }) => {
  return <Stage eyebrow={tr3({ uz: "Qoida", ru: "Правило" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Yakuniy ishga →", ru: "К итоговой работе →" }} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Nega ba'zi saytlar <span className="italic" style={{ color: T.accent }}>bir qarashda</span> ishonch uyg'otadi?</>, ru: <>Почему некоторые сайты вызывают доверие <span className="italic" style={{ color: T.accent }}>с первого взгляда</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Ishonch chiroydan emas, <b style={{ color: T.ink }}>tartibdan</b> tug'iladi: to'g'ri tuzilgan sahifa foydalanuvchini muammodan yechimga, so'ng harakatga yetaklaydi. Shuning uchun tartib — <b style={{ color: T.ink }}>foydalanuvchi uchun qilingan qulaylik</b>.</>, ru: <>Доверие рождается не от красоты, а от <b style={{ color: T.ink }}>порядка</b>: верно собранная страница ведёт человека от проблемы к решению, а затем к действию. Поэтому порядок — это <b style={{ color: T.ink }}>удобство, сделанное для пользователя</b>.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <div className="frame fade-up" style={{ display: "flex", alignItems: "center", gap: 16, padding: "clamp(18px,2.6vw,26px)" }}>
              <IcoChip size={54} color={SEC_AMBER} soft={SEC_AMBER + "1c"}>{sIco.star(28)}</IcoChip>
              <div><p style={{ fontFamily: "'Source Serif 4',serif", fontWeight: 600, margin: 0, color: T.ink, fontSize: "clamp(18px,2.4vw,22px)" }}>{tr3({ uz: "Tartib — foydalanuvchi uchun qilingan qulaylik", ru: "Порядок — удобство, сделанное для пользователя" })}</p><p className="body" style={{ margin: "3px 0 0", color: T.ink2 }}>{tr3({ uz: "Har bo'lim o'z o'rnida turibdi — shunda foydalanuvchi adashmay yechimga yetadi.", ru: "Каждый раздел на своём месте — тогда человек без путаницы дойдёт до решения." })}</p></div>
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr3({ uz: "Ishlaydigan tartib", ru: "Порядок, который работает" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {ORDER.map((k, i) => {
    const s = SECDATA[k];
    return <React3.Fragment key={k}><div className="rl-step" style={{ display: "flex", alignItems: "center", gap: 11, background: T.paper, borderRadius: 11, padding: "10px 13px", boxShadow: `0 5px 14px -8px rgba(${T.shadowBase},0.16)`, animationDelay: `${i * 0.9}s` }}><span style={{ color: s.color, display: "inline-flex" }}>{s.ic}</span><span style={{ fontFamily: "'Manrope',sans-serif", fontWeight: 600, color: T.ink, fontSize: 13.5 }}>{tr3(s.label)}</span></div>{i < ORDER.length - 1 && <span className="rl-arrow" style={{ color: T.ink3, textAlign: "center", fontSize: 11, animationDelay: `${i * 0.9 + 0.45}s` }}>↓</span>}</React3.Fragment>;
  })}
            </div>
          </Col>
        </div>
...
var KOD_SECS = ORDER.filter((k) => KOD_WORDS[k]).map((k) => ({ key: k, title: KOD_TITLES[k], word: KOD_WORDS[k] }));
var KOD_CONDS = [
...
function checkStructure(html) {
  ...
  if (miss.length) {
    res.hints.c1 = tr3({ uz: `Yetishmayapti: ${miss.join(", ")}. Sahifa uchta tegdan iborat: <header>, <main>, <footer>.`, ru: `Не хватает: ${miss.join(", ")}. Страница состоит из трёх тегов: <header>, <main>, <footer>.` });
  } else {
    const empty = [];
    if (!txt(header)) empty.push("<header>");
    if (!txt(footer)) empty.push("<footer>");
    if (empty.length) res.hints.c1 = tr3({ uz: `${empty.join(" va ")} ichi bo'sh — tepaga sayt nomini, pastga havola yoki aloqa ma'lumotini yozing.`, ru: `${empty.join(" и ")} — внутри пусто. Наверх напишите название сайта, вниз — ссылку или контакты.` });
    else res.c1 = true;
  }
  const h2s = $$("main h2").filter((el) => txt(el));
  const ps = $$("main p").filter((el) => txt(el));
  if (!main) res.hints.c2 = tr3({ uz: "Avval <main> tegini yozing — bo'limlar shuning ichida yashaydi.", ru: "Сначала напишите тег <main> — разделы живут внутри него." });
  else if (h2s.length < 4) res.hints.c2 = tr3({ uz: `<main> ichida ${h2s.length} ta to'ldirilgan sarlavha bor — 4 ta kerak. Yetmagani uchun <h2>Bo'lim nomi</h2> yozing.`, ru: `Внутри <main> заполненных заголовков: ${h2s.length} — нужно 4. Для недостающих напишите <h2>Название раздела</h2>.` });
  else if (ps.length < 4) res.hints.c2 = tr3({ uz: `Har sarlavha ostiga bir gap izoh kerak: <p>...</p>. Hozir <main> ichida ${ps.length} ta to'ldirilgan izoh bor.`, ru: `Под каждым заголовком нужна фраза-пояснение: <p>...</p>. Сейчас внутри <main> заполненных пояснений: ${ps.length}.` });
  else res.c2 = true;
  if (!header || !main || !footer) {
    res.hints.c3 = tr3({ uz: "Uchala teg yozilgach tartib tekshiriladi — avval <header>, <main>, <footer> ni qo'shing.", ru: "Порядок проверится, когда будут все три тега — добавьте <header>, <main>, <footer>." });
  } else if ($("main header") || $("main footer") || $("header main") || $("footer main")) {
    res.hints.c3 = tr3({ uz: "<header> va <footer> ni <main> dan tashqariga chiqaring — uchtasi yonma-yon turadi, biri ikkinchisining ichida emas.", ru: "Вынесите <header> и <footer> за пределы <main> — все три стоят рядом, а не один внутри другого." });
  } else {
    const isAfter = (a, b) => !!(a.compareDocumentPosition(b) & 4);
    if (!isAfter(header, main)) res.hints.c3 = tr3({ uz: "<main> hozir <header> dan oldin turibdi — sayt nomi eng tepada bo'lsin: avval <header>, keyin <main> yozing.", ru: "Сейчас <main> стоит раньше <header> — название сайта должно быть в самом верху: сначала <header>, потом <main>." });
    else if (!isAfter(main, footer)) res.hints.c3 = tr3({ uz: "<footer> hozir <main> dan oldin turibdi — u eng pastda bo'lsin: <main> dan keyin <footer> yozing.", ru: "Сейчас <footer> стоит раньше <main> — он должен быть в самом низу: сначала <main>, потом <footer>." });
    else res.c3 = true;
  }
  const titles = h2s.map((el) => txt(el).toLowerCase());
  const at = KOD_SECS.map((s) => titles.findIndex((t) => t.includes(String(tr3(s.word)).toLowerCase())));
  const missIdx = at.findIndex((x) => x === -1);
  if (!main || !h2s.length) {
    res.hints.c4 = tr3({ uz: "Avval <main> ichiga bo'lim sarlavhalarini yozing.", ru: "Сначала напишите внутри <main> заголовки разделов." });
  } else if (missIdx !== -1) {
    res.hints.c4 = tr3({ uz: `«${tr3(KOD_SECS[missIdx].title)}» sarlavhasi topilmadi — <main> ichiga <h2>${tr3(KOD_SECS[missIdx].title)}</h2> qo'shing.`, ru: `Заголовок «${tr3(KOD_SECS[missIdx].title)}» не найден — добавьте внутрь <main> строку <h2>${tr3(KOD_SECS[missIdx].title)}</h2>.` });
  } else {
    let bad = -1;
    for (let i = 1; i < at.length; i++) if (at[i] < at[i - 1]) {
      bad = i;
      break;
    }
    if (bad !== -1) res.hints.c4 = tr3({ uz: `«${tr3(KOD_SECS[bad].title)}» hozir ${at[bad] + 1}-o'rinda turibdi — u «${tr3(KOD_SECS[bad - 1].title)}» dan KEYIN kelishi kerak. Shu ikki bo'limni o'rin almashtiring.`, ru: `«${tr3(KOD_SECS[bad].title)}» сейчас на ${at[bad] + 1}-м месте — он должен идти ПОСЛЕ «${tr3(KOD_SECS[bad - 1].title)}». Поменяйте эти два раздела местами.` });
    else res.c4 = true;
  }
  return res;
}
...
var KOD_TASK = {
  ...
  previewUrl: "olx.uz",
  // natija paneli soxta brauzer oynasiga aylanadi
  placeholder: KOD_PLACEHOLDER,
  // xira NAMUNA — yozish boshlansa o'chadi
...
var ScreenCoding = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Yakuniy ish · 🛠 kompilyator", ru: "Итоговая работа · 🛠 компилятор" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Avval kompilyatorda yozing", ru: "Сначала напишите в компиляторе" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Sahifa tuzilishini quradigan <span className="italic" style={{ color: T.accent }}>kod</span> yozamiz</>, ru: <>Напишем <span className="italic" style={{ color: T.accent }}>код</span>, который строит структуру страницы</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Tartibni o'rgandingiz — endi uni haqiqiy kodda qurasiz. Pastdagi <b style={{ color: T.ink }}>«Kompilyatorni ochish»</b> tugmasini bosing: nima yozishni o'sha yerda ko'rsatamiz.</>, ru: <>Порядок вы освоили — теперь соберёте его в настоящем коде. Нажмите кнопку <b style={{ color: T.ink }}>«Открыть компилятор»</b>: что писать, покажем там.</> })}</Mentor>
        ...
        <div className="stq fade-up delay-1" ref={workRef}>
          <div className="stq-code">
            ...
            <code className="stq-code-body">
              ...
              <span className="stq-l dim">   {tr3({ uz: "sayt nomi", ru: "название сайта" })}</span>
              <span className="stq-l t">&lt;/header&gt;</span>
              <span className="stq-l m">&lt;main&gt;</span>
              <span className="stq-l dim">   &lt;h2&gt; + &lt;p&gt; × 4</span>
              <span className="stq-l m">&lt;/main&gt;</span>
              <span className="stq-l f">&lt;footer&gt;</span>
              <span className="stq-l dim">   {tr3({ uz: "havolalar, aloqa", ru: "ссылки, контакты" })}</span>
              <span className="stq-l f">&lt;/footer&gt;</span>
            </code>
          </div>
    ...
    writeKodingOpen(true);
          ...
          {done && <span className="stq-cta-sub">{tr3({ uz: "Bajarildi — xohlasangiz kodni yana sayqallang", ru: "Готово — при желании доработайте код" })}</span>}
          {
    /* 🔓 TAKRORLASH-YO'LI — FAQAT erkin rejimda va FAQAT bajarilmagan holatda.
       ...
       serverga signal yubormaydi, xotiraga saqlanmaydi. Jonli darsda ko'rinmaydi. */
          ...
          {!done && isSelf && <button className="stq-skip" onClick={onNext}>{tr3({ uz: "✓ Bu mashqni sinfda bajarganman — davom etish →", ru: "✓ Это упражнение я делал в классе — продолжить →" })}</button>}
        </div>
        {done && <div className="done-mini fade-step" style={{ alignSelf: "center" }}>{tr3({ uz: "✅ Ishladi!", ru: "✅ Сработало!" })} <span className="dm-sub">{tr3({ uz: "— sahifangiz to'g'ri tuzilishda yig'ildi", ru: "— ваша страница собрана в верной структуре" })}</span></div>}
        {isMentor && <details className="stq-mnote-d"><summary>{tr3({ uz: "👨‍🏫 Eslatma — faqat sizga", ru: "👨‍🏫 Заметка — только для вас" })}</summary><p>{tr3({ uz: "Topshiriqni o'quvchilar bajaradi, siz kuzatasiz. «Davom etish» siz uchun ochiq.", ru: "Задание выполняют ученики, вы наблюдаете. Кнопка «Продолжить» для вас открыта." })}</p></details>}
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Tayyor", ru: "Готово" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr3({ uz: "Yakunlash", ru: "Завершить" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">{Ico.check(11)}</span> {tr3({ uz: "Dars tugadi", ru: "Урок завершён" })}</span><h2 className="title h-title fade-up d1">{tr3({ uz: <>Endi siz <span className="italic" style={{ color: T.accent }}>sahifani</span> qura olasiz.</>, ru: <>Теперь вы умеете собрать <span className="italic" style={{ color: T.accent }}>страницу</span>.</> })}</h2>{
    /* 54-qonun (UserStory qarori bilan bir xil): h-sub qatori YO'Q — sarlavha o'zi yetadi. */
    ...
    hint={studentWait ? tr3({ uz: "⏳ Mentorni kuting", ru: "⏳ Подождите ментора" }) : void 0}
  />
        </div>
        ...
        {hwNote && <p className="hw-cta-note fade-step" style={{ textAlign: "center" }}>{tr3({ uz: "Topshiriq LMS kompilyatorida ochiladi — o'z sahifangizni bugun o'rgangan tartibda bajarasiz.", ru: "Задание откроется в компиляторе LMS — соберёте свою страницу в порядке, который освоили сегодня." })}</p>}
        {
...
var ScreenFlashcards = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Takrorlash", ru: "Повторение" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={false} label={{ uz: "Yakunlashga →", ru: "К завершению →" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
...
var INLINE_KEYS = { s4: 0, s5b: 2, s9: 1, s12: 3 };
  ...
  return <Stage eyebrow={tr3({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        ...
        {!isLive ? <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 16, alignItems: "center" }}>
          ...
          </div> : !loaded2 ? <p className="mono small fade-up" style={{ color: T.ink2 }}>{tr3({ uz: "Natijalar yuklanmoqda…", ru: "Загружаем результаты…" })}</p> : board.length === 0 ? <div className="frame-soft fade-up"><p className="body" style={{ margin: 0 }}>{tr3({ uz: "Bu darsga hali hech kim qo'shilmagan.", ru: "К этому уроку пока никто не подключился." })}</p></div> : <>
            <Confetti />
            {
  ...
  useEffect5(() => {
    ...
    const draw = (tm) => {
      ...
      for (let a = 0; a < em.length; a++) for (let b = a + 1; b < em.length; b++) {
        ...
        if (d < mx) {
          ctx.strokeStyle = "rgba(91,61,230," + 0.055 * (1 - d / mx) + ")";
          ctx.beginPath();
      ...
      for (const p of em) {
        const s = (1.3 + p.z * 2.2) * DPR, tw = 0.16 + p.z * 0.24 + Math.sin(tm / 600 + p.ph) * 0.08;
        ctx.fillStyle = "rgba(245,166,35," + tw + ")";
        ctx.beginPath();
      ...
      for (const t of toks) {
        ...
        ctx.fillStyle = "rgba(20,17,14," + (0.03 + t.z * 0.05) + ")";
  ...
  return <div className="qz-arena">
    ...
    /* ===== LOBBY ===== */
      ...
      {phase === "lobby" && <div className="qz-view fade-step">
          <CsWordmark />
          <p className="qz-sub" style={{ marginTop: -4 }}>{tr3({ uz: "Tezroq to'g'ri bossangiz — ko'proq ball. Ketma-ket to'g'ri javoblar 🔥 bonus beradi!", ru: "Чем быстрее верный ответ, тем больше баллов. Ответы подряд дают 🔥 бонус!" })}</p>
    ...
    /* ===== YAKUN — PODIUM ===== */
      ...
      {phase === "done" && <div className="qz-view fade-step">
          ...
          {solo ? <div className="qz-solo-res">
              ...
              <p className="qz-sub">{tr3({ uz: `ball · ${soloScore.ok}/${QUIZ_BANK.length} to'g'ri`, ru: `баллов · верно ${soloScore.ok}/${QUIZ_BANK.length}` })}{soloScore.maxStreak >= 2 ? tr3({ uz: ` · ketma-ket to'g'ri 🔥x${soloScore.maxStreak}`, ru: ` · подряд верно 🔥x${soloScore.maxStreak}` }) : ""}</p>
              <button className="qz-btn big" onClick={soloReplay}>{tr3({ uz: "↻ Qaytadan yechish", ru: "↻ Пройти заново" })}</button>
            </div> : <>
    ...
    return <div key={rank} className={`qz-pod-col p${rank + 1} ${b && b.id === live.playerId ? "me" : ""}`}>
              ...
              {isStudent && <button className="qz-btn" onClick={startPractice}>{tr3({ uz: "↻ Testni qaytadan yechish — mashq (jadvalga yozilmaydi)", ru: "↻ Пройти тест заново — тренировка (в таблицу не идёт)" })}</button>}
            </>}
          <button className="qz-btn ghost" onClick={closeArena}>{tr3({ uz: "Arenani yopish", ru: "Закрыть арену" })}</button>
        </div>}
    </div>;
}
function PmLesson2({ lang: langProp, onFinished, liveToken }) {
