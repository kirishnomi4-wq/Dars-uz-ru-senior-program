...
//  Manba:  src/1-Modull/PmLesson1.homework.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/1-Modull/PmLesson1.homework.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/1-Modull/PmLesson1.homework.jsx
...
var MUAMMO_MIN = 6;
var YECHIM_MIN = 6;
var KIM_MAVHUM_WORDS = { uz: "hamma|barcha|hammasi|hech kim", ru: "все|всё|вся|всем|всех|любой|каждый|люди" };
var KIM_MAVHUM = new RegExp(`^(${KIM_MAVHUM_WORDS.uz}|${KIM_MAVHUM_WORDS.ru})`, "i");
var kimMavhum = (s) => KIM_MAVHUM.test((s || "").trim());
var kimOk = (s) => {
  const t = (s || "").trim();
  return t.length >= KIM_MIN && !kimMavhum(t);
};
var muammoOk = (s) => (s || "").trim().length >= MUAMMO_MIN;
var yechimOk = (s) => (s || "").trim().length >= YECHIM_MIN;
var cardFull = (c) => !!c && kimOk(c.kim) && muammoOk(c.muammo) && yechimOk(c.yechim);
var LESSON_CARDS_KEY = "pm-m1d2-cards";
var lessonCardRead = () => {
  try {
...
var rowOk = (k, v) => k === "kim" ? kimOk(v) : k === "muammo" ? muammoOk(v) : yechimOk(v);
var WHO = [
...
var CardView = ({ card, newKim, newMuammo, whoList }) => {
  const c = card || {};
  ...
  return <div className="ac">
    ...
    const changed = n && n !== v;
    return <div key={r.k} className="ac-row">
      ...
      {whoList && whoList.length > 0 && <div className="ac-who">
...
var StageCard = ({ card, setCard }) => {
  ...
  return <div className="col">
    ...
    return <div key={r.k} className="wrow" style={{ borderBottom: i < CARD_ROWS.length - 1 ? `1px solid ${T.line}` : "none" }}>
...
var cardDone = (c) => cardFull(c || {});
var StageTalk = ({ data, setData, n }) => {
  ...
  return <div className="col">
      <h2 className="title h-title h-center fade-up">
        {n === 1 ? tr({ uz: <>Kartani <span className="italic" style={{ color: T.accent }}>birinchi</span> tanishingizga ko'rsating</>, ru: <>Покажите карту <span className="italic" style={{ color: T.accent }}>первому</span> знакомому</> }) : tr({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>ikkinchi</span> tanishingizga</>, ru: <>Теперь <span className="italic" style={{ color: T.accent }}>второму</span> знакомому</> })}
      </h2>
      <p className="h-sub fade-up">
        {n === 1 ? tr({ uz: "Kartada yozganingizni aytib bering, keyin so'rang: «Siz shunday saytga kirarmidingiz?»", ru: "Расскажите, что написано в карте, а потом спросите: «Вы зашли бы на такой сайт?»" }) : tr({ uz: "Boshqa odam — boshqa javob. Ikkinchi suhbat kartangizni aniqroq qiladi.", ru: "Другой человек — другой ответ. Второй разговор делает вашу карту точнее." })}
      </p>
      <div className="frame fade-up d1">
        ...
        <div className="chips" role="radiogroup">
          {ANS.map((a, i) => <button key={i} type="button" role="radio" aria-checked={ans === i} className={`chip ${ans === i ? "on" : ""}`} onClick={() => setData({ ...data, ans: i })}>
...
var talkDone = (d) => typeof d.who === "number" && typeof d.ans === "number" && (d.said || "").trim().length >= SAID_MIN;
var QZ_HOLD_MS = 1500;
var QZ_OUT_MS = 380;
var MODES = [
...
var StageSum = ({ data, setData, card }) => {
  ...
  return <div className="col">
      ...
      <div className="frame fade-up d1">
        ...
        <div className="chips" role="radiogroup">
          {MODES.map((m, i) => <button key={i} type="button" role="radio" aria-checked={mode === i} className={`chip ${mode === i ? "on" : ""}`} onClick={() => setData({ ...data, mode: i })}>
    ...
    return <div key={r.k} className="wrow" style={{ paddingTop: i === 0 ? 0 : 10 }}>
  ...
  return <div className="col fin">
      ...
      <div className="fin-hero fade-up">
        ...
        <h2 className="title h-title" style={{ margin: "4px 0 2px" }}>
          {hasCard ? tr({ uz: <>Auditoriya-<span className="italic" style={{ color: T.accent }}>kartangiz</span> tayyor!</>, ru: <>Ваша <span className="italic" style={{ color: T.accent }}>карта аудитории</span> готова!</> }) : tr({ uz: "Uyga vazifa bajarildi!", ru: "Домашнее задание выполнено!" })}
        </h2>
      ...
      {hasCard && <div className="fin-site fade-up d1">
...
function PmLesson1Homework({ lang: langProp, onFinished }) {
  ...
  return <div className="hw-root">
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult ? <StageResult data={data} goStage={setStage} onFinishClick={finish} finished={finished} onCelebrated={() => setDataRaw((prev) => prev.celebrated ? prev : { ...prev, celebrated: true })} /> : cur.key === "card" ? <StageCard card={data.card} setCard={setStageData("card")} /> : cur.key === "talk1" ? <StageTalk data={data.talk1 || {}} setData={setStageData("talk1")} n={1} /> : cur.key === "talk2" ? <StageTalk data={data.talk2 || {}} setData={setStageData("talk2")} n={2} /> : <StageSum data={data.sum || {}} setData={setStageData("sum")} card={data.card} />}
        </div>
      </div>
