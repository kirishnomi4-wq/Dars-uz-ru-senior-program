...
//  Manba:  src/2-Modull/PmLesson5.homework.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/2-Modull/PmLesson5.homework.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/2-Modull/PmLesson5.homework.jsx
...
var itemOk = (s) => (s || "").trim().length >= ITEM_MIN;
var useOk = (s) => {
  const t = (s || "").trim();
  return t.length >= USE_MIN && !isFlat(t);
};
var newOk = (s) => {
  const t = (s || "").trim();
  return t.length >= NEW_MIN && !isFlat(t);
};
...
var LaunchCard = ({ items, uses, extra }) => {
...
var listDone = (items) => [0, 1, 2].every((i) => itemOk((items || [])[i]));
var USE_PH = [
...
var StageUses = ({ data, setData, items }) => {
...
var usesDone = (d) => [0, 1, 2].every((i) => useOk(((d || {}).uses || [])[i]));
var StageScale = ({ data, setData }) => {
  ...
  return <div className="col">
      ...
      <div className="frame fade-up d1">
        ...
        <div className="chips" role="radiogroup">
          {NEED.map((o) => <button key={o.key} type="button" role="radio" aria-checked={need === o.key} className={`chip ${need === o.key ? "on" : ""}`} onClick={() => setData({ ...data, need: o.key })}>
        ...
        <div className="chips" role="radiogroup">
          {COST.map((o) => <button key={o.key} type="button" role="radio" aria-checked={cost === o.key} className={`chip ${cost === o.key ? "on" : ""}`} onClick={() => setData({ ...data, cost: o.key })}>
              <span className="chip-ic" aria-hidden="true">{o.ic}</span>
              <span>{tr(o.label)}</span>
            </button>)}
        </div>
        {b && <div className="frame-success fade-step" style={{ marginTop: 14 }}>
...
var scaleDone = (d) => newOk((d || {}).imkoniyat) && ((d || {}).need === "must" || (d || {}).need === "nice") && ((d || {}).cost === "day" || (d || {}).cost === "week");
var QZ_HOLD_MS = 1500;
var QZ_OUT_MS = 380;
...
function PmLesson5Homework({ lang: langProp, onFinished }) {
  ...
  return <div className="hw-root">
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult ? <StageResult data={data} goStage={setStage} onFinishClick={finish} finished={finished} onCelebrated={() => setDataRaw((prev) => prev.celebrated ? prev : { ...prev, celebrated: true })} /> : cur.key === "list" ? <StageList items={data.list} setItems={setList} /> : cur.key === "uses" ? <StageUses data={data.uses || {}} setData={setStageData("uses")} items={data.list} /> : cur.key === "scale" ? <StageScale data={data.scale || {}} setData={setStageData("scale")} /> : <StageSum data={data.sum || {}} setData={setStageData("sum")} />}
        </div>
      </div>
