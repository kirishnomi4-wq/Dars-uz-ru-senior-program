...
//  Manba:  src/2-Modull/PmLesson6.homework.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/2-Modull/PmLesson6.homework.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/2-Modull/PmLesson6.homework.jsx
...
var hasAnalogy = (t) => {
  ...
  return ANALOGY_WORDS.some((w) => low.includes(w));
};
var KIM_MAVHUM = new RegExp("^(hamma|barcha|hammasi|hech kim|все|всё|вся|всем|всех|любой|каждый|люди)", "i");
...
var fieldOk = (key, v) => fieldErr(key, v) === null;
var pitchDone = (p) => PITCH_FIELDS.every((f) => fieldOk(f.key, (p || {})[f.key]));
...
var PitchCard = ({ pitch, rew, talk }) => {
  ...
  return <div className="ac">
    ...
    return <div key={f.key} className="ac-row">
...
var StagePitch = ({ pitch, setPitch }) => {
  ...
  return <div className="col">
    ...
    return <div key={f.key} className="wrow" style={{ borderBottom: i < PITCH_FIELDS.length - 1 ? `1px solid ${T.line}` : "none" }}>
...
var StageTalk = ({ data, setData }) => {
  ...
  return <div className="col">
      ...
      <div className="frame fade-up d1">
        ...
        <div className="chips" role="radiogroup">
          {GRASP.map((g, i) => <button key={i} type="button" role="radio" aria-checked={grasp === i} className={`chip ${grasp === i ? "on" : ""}`} onClick={() => setData({ ...data, grasp: i })}>
...
var talkDone = (d) => typeof d.who === "number" && typeof d.grasp === "number" && (d.said || "").trim().length >= SAID_MIN;
var MODES = [
...
var StageRewrite = ({ data, setData, pitch, grasp }) => {
  ...
  return <div className="col">
    ...
    return <button key={i} type="button" role="radio" aria-checked={mode === i} disabled={locked} className={`chip ${mode === i ? "on" : ""} ${locked ? "off" : ""}`} onClick={() => setData({ ...data, mode: i })} title={locked ? tr({ uz: "Tinglovchi to'liq tushunishmadi — bitta bo'lakni aniqlashtiring", ru: "Вас поняли не полностью — уточните одну часть" }) : void 0}>
        ...
        {mode === 1 && <div style={{ marginTop: 12 }}>
            ...
            <div className="chips" role="radiogroup">
              {PITCH_FIELDS.map((pf, i) => <button key={pf.key} type="button" role="radio" aria-checked={piece === i} className={`chip ${piece === i ? "on" : ""}`} onClick={() => setData({ ...data, piece: i })}>
                  <span>{tr(pf.short)}</span>
                </button>)}
            </div>
            {f && <div style={{ marginTop: 12 }}>
...
function PmLesson6Homework({ lang: langProp, onFinished }) {
