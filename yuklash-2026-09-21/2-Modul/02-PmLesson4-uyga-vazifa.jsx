...
//  Manba:  src/2-Modull/PmLesson4.homework.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/2-Modull/PmLesson4.homework.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/2-Modull/PmLesson4.homework.jsx
...
var qOk = (s) => (s || "").trim().length >= Q_MIN;
var fOkLesson = (s) => {
  const t = (s || "").trim();
  return t.length >= F_MIN && !isFlat(t) && !isDecor(t);
};
var fOkNew = (s) => {
  const t = (s || "").trim();
  return t.length >= F_MIN && !isFlat(t);
};
var pairFull = (p) => !!p && qOk(p.qiyinchilik) && fOkLesson(p.imkoniyat);
...
var PAIR_FIELDS = [
  {
    ...
    max: 160,
    ic: Ico.problem(15),
    lbl: { uz: "QIYINCHILIK", ru: "ТРУДНОСТЬ" },
...
var StagePairs = ({ pairs, setPairs }) => {
  ...
  return <div className="col">
      ...
      <div className="frame fade-up d1" style={{ padding: "clamp(6px,1.2vw,10px) clamp(14px,2.2vw,20px)" }}>
        {list.map((p, i) => <div key={i} className="wrow" style={{ borderBottom: i < 2 ? `1px solid ${T.line}` : "none" }}>
    ...
    return <div key={f.k} style={{ marginBottom: 8 }}>
...
var pairsDone = (arr) => [0, 1, 2].every((i) => pairFull((arr || [])[i]));
var NEW_FIELDS = [
  {
    k: "kim",
    min: KIM_MIN,
...
var StageNew = ({ data, setData, n, usedBand }) => {
  ...
  return <div className="col">
      <h2 className="title h-title h-center fade-up">
        {n === 1 ? tr({ uz: <>Keraksiz bandga <span className="italic" style={{ color: T.accent }}>egasini</span> toping</>, ru: <>Найдите ненужному пункту <span className="italic" style={{ color: T.accent }}>хозяина</span></> }) : tr({ uz: <>Endi <span className="italic" style={{ color: T.accent }}>ikkinchi</span> band</>, ru: <>Теперь <span className="italic" style={{ color: T.accent }}>второй</span> пункт</> })}
      </h2>
      <p className="h-sub fade-up">
        {n === 1 ? tr({ uz: "Darsda bu bandlarga joy topilmadi. Bittasini tanlang — kimgadir baribir kerak bo'lishi mumkinmi?", ru: "На уроке этим пунктам места не нашлось. Выберите один — вдруг он всё же кому-то нужен?" }) : tr({ uz: "Boshqa bandni oling — boshqa odam, boshqa qiyinchilik.", ru: "Возьмите другой пункт — другой человек, другая трудность." })}
      </p>
    ...
    return <button key={i} type="button" role="radio" aria-checked={band === i} disabled={taken} className={`chip ${band === i ? "on" : ""} ${taken ? "off" : ""}`} onClick={() => setData({ ...data, band: i })} title={taken ? tr({ uz: "Bu band 2-bosqichda olingan", ru: "Этот пункт взят на 2-м этапе" }) : void 0}>
    ...
    return <div key={f.k} className="wrow" style={{ borderBottom: "none", paddingBottom: i === NEW_FIELDS.length - 1 ? 0 : void 0 }}>
...
var newDone = (d) => typeof d.band === "number" && d.band >= 0 && kimOk(d.kim) && qOk(d.qiyinchilik) && fOkNew(d.imkoniyat);
var QZ_HOLD_MS = 1500;
var QZ_OUT_MS = 380;
...
function PmLesson4Homework({ lang: langProp, onFinished }) {
  ...
  return <div className="hw-root">
      ...
      <div className="hw-scroll" ref={scrollRef}>
        <div className="hw-main">
          {isResult ? <StageResult data={data} goStage={setStage} onFinishClick={finish} finished={finished} onCelebrated={() => setDataRaw((prev) => prev.celebrated ? prev : { ...prev, celebrated: true })} /> : cur.key === "pairs" ? <StagePairs pairs={data.pairs} setPairs={setStageData("pairs")} /> : cur.key === "new1" ? <StageNew data={data.new1 || {}} setData={setStageData("new1")} n={1} usedBand={-1} /> : cur.key === "new2" ? <StageNew data={data.new2 || {}} setData={setStageData("new2")} n={2} usedBand={usedBand} /> : <StageSum data={data.sum || {}} setData={setStageData("sum")} />}
        </div>
      </div>
