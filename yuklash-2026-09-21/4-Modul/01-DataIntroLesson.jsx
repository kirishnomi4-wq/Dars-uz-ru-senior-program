...
var INLINE_KEYS = { s4: 0, s5b: 2, s9: 1, s12: 3, s15: 0, s15b: 0, practice: -1 };
var RECAPS = {
  4: {
...
var Screen15 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  useEffect4(() => {
    if (allDone && !storedAnswer) {
      ...
      onAnswer(screen, { stage: "final", screenIdx: screen, question: tr2({ uz: "Ilova ma'lumot sxemasini ulab chizish (3 bog'lanish)", ru: "Соединить схему данных приложения (3 связи)" }), correct: first, firstAttemptCorrect: first, wrongCount: wrongRef.current, solved: true, picked: first ? 0 : 1 });
    }
  }, [allDone]);
...
var ScreenFindFk = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  useEffect4(() => {
    if (allDone && !storedAnswer) {
      const first = wrongRef.current === 0 && !(achMiss && achMiss.missed.has(SCREEN_META[screen].id));
      onAnswer(screen, { stage: "final", screenIdx: screen, question: tr2({ uz: "Bog'lovchi ustunlarni topib, sxemani ulash (maktab jurnali)", ru: "Найти связующие столбцы и соединить схему (школьный журнал)" }), correct: first, firstAttemptCorrect: first, wrongCount: wrongRef.current, solved: true, picked: first ? 0 : 1 });
    }
  }, [allDone]);
  const clickField = (id, col) => {
    setMiss(null);
