...
//  Manba:  src/4-Modull/PmLesson11.jsx
//          src/compilator/HtmlCompiler.jsx
//  Qayta yig'ish:  node scripts/build-lms.mjs src/4-Modull/PmLesson11.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/4-Modull/PmLesson11.jsx
...
var LESSON_META = { lessonId: "pm-m4d2-v1", lessonTitle: { uz: "Ilova nimani eslab qolsin?", ru: "Что приложению запомнить?" } };
var SCREEN_META = [
...
var INLINE_KEYS = { s3: 1, s5: 0, s7: 0, s11: 1, tugma: -1, practice: -1, bolim: -1, koding: -1 };
var RECAPS = {
  3: {
...
var Phone = ({ children, quiet, ping, hint }) => <div className={`pho${quiet ? " quiet" : ""}`}>
    ...
    <span className="pho-app">{tr3({ uz: "🎧 Musiqa", ru: "🎧 Музыка" })}</span>
    <div className="pho-body">
      {ping ? <span key={ping} className="pho-ping" aria-hidden="true" /> : null}
      {quiet && <span className="pho-empty"><i className="pho-empty-ic">🎧</i>{hint || tr3({ uz: "Bo'lim hali qurilmadi", ru: "Раздел ещё не построен" })}</span>}
      {children}
    </div>
...
var HOOK_KEY = "pm-m4d2-hook-choice";
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Kirish · bitta qo'shiq", ru: "Вступление · одна песня" })} screen={screen} navContent={<NavNext optionalLive disabled={picked === null && !isMentor} label={opened ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: "Bittasini tanlang", ru: "Выберите один вариант" })} onClick={onNext} />}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Ilova qo'shiqni <span className="italic" style={{ color: T.accent }}>eslab qolsinmi?</span></>, ru: <>Приложению <span className="italic" style={{ color: T.accent }}>запомнить песню?</span></> })}</h2></div>
        <Mentor>{tr3({ uz: "Sevimli qo'shig'ingizni ilova eslab qolsinmi?", ru: "Пусть приложение запомнит вашу любимую песню?" })}</Mentor>
    ...
    /* IMZO-SAHNA: ikkala tanlovda ham BIR XIL natija ochiladi (104-qonun) */
            ...
            <div className="h0pay fade-step">
              <span className="h0pay-row"><b>{tr3({ uz: "Eslab qolsin", ru: "Пусть запомнит" })}</b><i className="h0pay-arw">→</i><span>{tr3({ uz: "ertaga qo'shiqni darhol topasiz", ru: "завтра найдёте песню сразу" })}</span></span>
              <span className="h0pay-row"><b>{tr3({ uz: "Eslamasin", ru: "Пусть не запоминает" })}</b><i className="h0pay-arw">→</i><span>{tr3({ uz: "ertaga uni yana qidirasiz", ru: "завтра будете искать её снова" })}</span></span>
            </div>
          </>}
        {
    ...
    return <div key={o.k} className={`hvote-row ${picked === i ? "mine" : ""} ${top ? "top" : ""}`}>
        ...
        <MentorNote>{tr3({ uz: "Ovozlar bo'linadi — ikkala tomonni ham himoyalang. «Eslamasin» deganlar ko'p bo'lsa, bu ham dars: saqlashning ikkinchi tomonini bola o'zi sezyapti. Javobni oldindan aytmang.", ru: "Голоса разделятся — защищайте обе стороны. Если многие выбрали «пусть не запоминает», это тоже урок: ребёнок сам чувствует обратную сторону хранения. Не называйте ответ заранее." })}</MentorNote>
      </div>
    </Stage>;
};
var DEMO_MAYDON = [{ uz: "Qo'shiqchi nomi", ru: "Имя исполнителя" }, { uz: "Qo'shiq matni", ru: "Текст песни" }, { uz: "Albom nomi", ru: "Название альбома" }];
var Screen1 = ({ screen, onNext, onPrev }) => <Stage eyebrow={tr3({ uz: "Maqsad", ru: "Цель" })} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={tr3({ uz: "Boshlaymiz →", ru: "Начнём →" })} onClick={onNext} /></>}>
    <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
      <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Dars oxirida ilovangiz uchun <span className="italic" style={{ color: T.accent }}>3 ta maydon</span> yozasiz.</>, ru: <>В конце урока вы напишете <span className="italic" style={{ color: T.accent }}>3 поля</span> для своего приложения.</> })}</h2></div>
      <Mentor>{tr3({ uz: <>Musiqa ilovasi siz haqingizda nimalarni eslab qoladi — mana shu uchtasi <b style={{ color: T.ink }}>maydon</b> deyiladi.</>, ru: <>Что музыкальное приложение запоминает о вас — вот эти три вещи называются <b style={{ color: T.ink }}>полями</b>.</> })}</Mentor>
      <div className="s1demo">
        <span className="s1demo-lbl">{tr3({ uz: "🎧 Musiqa ilovasi eslab qoladi:", ru: "🎧 Музыкальное приложение запоминает:" })}</span>
        <div className="s1demo-list">
          {DEMO_MAYDON.map((m, i) => <span key={i} className="s1row" style={{ "--dd": `${0.4 + i * 0.45}s` }}>
              <span className="s1row-t">{tr3(m)}</span>
              <span className="s1row-ok" style={{ "--dd3": `${0.7 + i * 0.45}s` }}>✅</span>
            </span>)}
        </div>
      </div>
      <div className="takeaway fade-up delay-2"><span className="ta-bulb">🎯</span><p className="ta-h">{tr3({ uz: "Dars oxirida siz ham o'z ilovangiz eslab qoladigan uchta maydonni yozasiz.", ru: "В конце урока вы тоже напишете три поля, которые будет запоминать ваше приложение." })}</p></div>
      <MentorNote>{tr3({ uz: "Ro'yxat yozilib bo'lgunicha gapirmang — vizual o'zi tanishtiradi.", ru: "Не говорите, пока список не допишется, — визуал представит всё сам." })}</MentorNote>
    </div>
  </Stage>;
var S2_CARDS = [
...
var Screen2 = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Muhokama · ikki karta", ru: "Обсуждение · две карточки" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!allSeen && !isMentor} label={allSeen || isMentor ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: `👆 Yana ${qoldi} kartani oching`, ru: `👆 Откройте ещё ${qoldi} карточк${qoldi === 1 ? "у" : "и"}` })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Ilova nimani <span className="italic" style={{ color: T.accent }}>eslab qoladi</span>?</>, ru: <>Что приложение <span className="italic" style={{ color: T.accent }}>запоминает</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: "Ikki kartani solishtiring.", ru: "Сравните две карточки." })}</Mentor>
        ...
        {allSeen && <div className="xul fade-step">
            <span className="xul-h">{tr3({ uz: "Ilova faqat eslab qolgan narsasini keyin yana ko'rsata oladi.", ru: "Приложение может снова показать только то, что запомнило." })}</span>
            <p className="xul-b">{tr3({ uz: <>Eslab qolinmagan narsa faqat o'sha payt ko'rinadi. Ilova eslab qoladigan har bir narsa — <b>maydon</b> deb ataladi.</>, ru: <>Незапомненное видно только в тот момент. Каждая вещь, которую приложение запоминает, называется <b>полем</b>.</> })}</p>
          </div>}
      </div>
    </Stage>;
};
...
var Screen3 = (props) => <QuestionScreen
  {...props}
  eyebrow={tr3({ uz: "Tekshiruv · nima qaytadi", ru: "Проверка · что возвращается" })}
  scope="module-mikro"
  ctaLabel={tr3({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr3({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr3({ uz: "💾 Ilova keyin yana nimani ko'rsata oladi?", ru: "💾 Что приложение сможет показать снова?" })} />}
  questionText={tr3({ uz: "Ilova keyin yana nimani ko'rsata oladi", ru: "Что приложение сможет показать снова" })}
  options={[tr3({ uz: "Bir marta ko'rilgan hamma narsani", ru: "Всё, что один раз увидели" }), tr3({ uz: "Eslab qolgan ma'lumotini", ru: "Данные, которые оно запомнило" }), tr3({ uz: "Foydalanuvchi so'ragan hamma narsani", ru: "Всё, что попросит пользователь" })]}
  correctIdx={1}
  explainCorrect={tr3({ uz: "To'g'ri — eslab qolinmagan narsa faqat o'sha payt ko'rinadi.", ru: "Верно — незапомненное видно только в тот момент." })}
  explainWrong={{
    0: tr3({ uz: "Bir marta ko'rilgani eslab qolinmagan bo'lsa, keyin uni qaytadan chiqarib bo'lmaydi.", ru: "Если увиденное один раз не запомнили, потом его уже не вывести снова." }),
    2: tr3({ uz: "So'rash yetmaydi: ilova faqat eslab qolgan ma'lumotini ko'rsata oladi.", ru: "Просьбы мало: приложение показывает только те данные, которые запомнило." }),
    default: tr3({ uz: "Ilova faqat eslab qolgan ma'lumotini keyin yana ko'rsata oladi.", ru: "Приложение может снова показать только те данные, которые запомнило." })
  }}
/>;
var TIP1_SEC = 40;
var TIP1_TRY = 3;
...
var Screen4 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === "mentor");
  const [on, setOn] = useState5({});
  const [tried, setTried] = useState5(() => storedAnswer?.tried || {});
  const [offSeen, setOffSeen] = useState5(() => !!storedAnswer?.offSeen);
  ...
  useEffect6(() => () => clearTimeout(justT.current), []);
  useEffect6(() => {
  ...
  const navLabel = done || isMentor || rescue ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : qolgan > 0 ? tr3({ uz: `① Yana ${qolgan} tugmani yoqib ko'ring`, ru: `① Включите ещё ${qolgan} ${qolgan === 1 ? "переключатель" : "переключателя"}` }) : tr3({ uz: "② Endi bittasini o'chirib ko'ring", ru: "② Теперь попробуйте один выключить" });
  return <Stage eyebrow={tr3({ uz: "Sinov · ilova nimani eslab qoladi", ru: "Опыт · что запоминает приложение" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor && !rescue} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.3vw,13px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Ilova nimani eslab qolishini <span className="italic" style={{ color: T.accent }}>o'zingiz</span> tanlang.</>, ru: <>Выберите <span className="italic" style={{ color: T.accent }}>сами</span>, что приложению запоминать.</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Faqat <b style={{ color: T.ink }}>kerakli</b> ma'lumotlarni yoqing.</>, ru: <>Включите только <b style={{ color: T.ink }}>нужные</b> данные.</> })}</Mentor>
    ...
    /* 72-qonun: yorliqli idish + diqqat-pulsi; birinchi bosishdan keyin puls tinadi */
            ...
            <div className={`mtg${Object.keys(tried).length > 0 ? " calm" : ""}`}>
              <span className="mtg-lbl">{tr3({ uz: "🎛 Ilova nimani yozib qo'ysin?", ru: "🎛 Что приложению записывать?" })}</span>
              {TUGMALAR.map((m) => <div key={m.id} className={`mtg-row${on[m.id] ? " on" : ""}${just === m.id ? " just" : ""}`}>
                  <button type="button" className="mtg-btn" onClick={() => flip(m.id)} aria-pressed={!!on[m.id]}>
                    <span className="mtg-ic">{m.ic}</span>
                    <span className="mtg-t">{tr3(m.t)}</span>
                    <span className="mtg-sw"><i /></span>
                  </button>
                  {
    /* 106d/71: har bosishdan keyin yonida bitta qator — nimadan qurilishini aytadi */
  }
                  {tried[m.id] && <span className={`mtg-res ${m.bolim ? "ok" : "nil"}`}>{m.bolim ? "✅" : "⬜"} {tr3(m.res)}</span>}
                </div>)}
            </div>
          </Col>
          <Col gap={9}>
    ...
    /* Bo'limlar DOIM turadi, ko'rinishi tugmaga bog'liq: yoqilsa o'sadi, o'chirilsa
       so'nib pasayadi. Ikkala harakat ham bir xil egri bilan — yo'qolish KO'RINADI. */
            ...
            <Phone quiet={bolimlar.length === 0} ping={nilPing}>
              {TUGMALAR.filter((m) => m.bolim).map((m) => <div key={m.id} className={`pho-sec${on[m.id] ? " on" : ""}`} aria-hidden={!on[m.id]}>
                  <span className="pho-sec-h">{tr3(m.bolim)}</span>
        ...
        {tipOn && <p className="bhint fade-step">
            {nextOpen ? tr3({ uz: <>💡 Hali sinalmagan tugma bor: {nextOpen.ic} {tr3(nextOpen.t)} — uni ham yoqib ko'ring.</>, ru: <>💡 Остался непроверенный переключатель: {nextOpen.ic} {tr3(nextOpen.t)} — включите и его.</> }) : tr3({ uz: "💡 Yoqib ko'rdingiz. Endi bittasini o'chirib ham ko'ring — ilovada nima o'zgaradi?", ru: "💡 Включать попробовали. Теперь попробуйте один выключить — что изменится в приложении?" })}
          </p>}
        {rescue && <p className="small fade-step" style={{ margin: 0, color: T.ink3, fontWeight: 600 }}>{tr3({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём позже вместе — «Продолжить» открыто." })}</p>}
        {done && <div className="bdone fade-step">
            <span className="done-mini">{tr3({ uz: <>✅ Uch tugma bittadan bo'lim ochdi, ikkitasi hech narsa ochmadi <span className="dm-sub">— har saqlanadigan maydon ortida bitta bo'lim turadi</span></>, ru: <>✅ Три переключателя открыли по разделу, два не открыли ничего <span className="dm-sub">— за каждым сохраняемым полем стоит один раздел</span></> })}</span>
          </div>}
...
var Screen5 = (props) => <QuestionScreen
  {...props}
  eyebrow={tr3({ uz: "Tekshiruv · foyda bermagan ma'lumot", ru: "Проверка · бесполезные данные" })}
  scope="module-mikro"
  ctaLabel={tr3({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr3({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr3({ uz: "📇 Kontaktlarni saqladik, lekin musiqa ilovasida hech narsa o'zgarmadi. Nega?", ru: "📇 Контакты сохранили, но в музыкальном приложении ничего не изменилось. Почему?" })} />}
  questionText={tr3({ uz: "Kontaktlar saqlandi, lekin ilovada hech narsa o'zgarmadi — nega", ru: "Контакты сохранены, но в приложении ничего не изменилось — почему" })}
  options={[tr3({ uz: "Chunki bu ma'lumotdan foydali bo'lim chiqmaydi", ru: "Потому что из этих данных не получается полезный раздел" }), tr3({ uz: "Chunki ilova buzilgan", ru: "Потому что приложение сломалось" }), tr3({ uz: "Chunki keyinroq o'zi ishlay boshlaydi", ru: "Потому что позже оно заработает само" })]}
  correctIdx={0}
  explainCorrect={tr3({ uz: "To'g'ri — saqlangan ma'lumotdan bo'lim chiqmasa, u bekorga joy egallaydi.", ru: "Верно — если из сохранённых данных не выходит раздел, они зря занимают место." })}
  explainWrong={{
    1: tr3({ uz: "Ilova buzilmagan: kontaktlarni bemalol saqladi. Faqat ulardan musiqa ilovasiga foydali hech narsa chiqmaydi.", ru: "Приложение не сломалось: контакты оно спокойно сохранило. Просто из них для музыкального приложения не выходит ничего полезного." }),
    2: tr3({ uz: "Bo'lim o'zidan paydo bo'lmaydi — uni saqlangan ma'lumot quradi. Kontaktlardan qo'shiq bo'limi chiqmaydi.", ru: "Раздел не появляется сам по себе — его строят сохранённые данные. Из контактов раздел с песнями не выходит." }),
    default: tr3({ uz: "Har foydali bo'lim ortida bitta kerakli ma'lumot turadi — kontaktlardan bunday bo'lim chiqmadi.", ru: "За каждым полезным разделом стоят одни нужные данные — из контактов такого раздела не вышло." })
  }}
/>;
var K6_SLIDES = [
  {
    ...
    body: null,
    predict: {
      ask: { uz: "Ikki odam Netflix'ni ochsa, bosh sahifasi bir xil bo'ladimi?", ru: "Если два человека откроют Netflix, главная у них будет одинаковой?" },
      chips: [
        { ic: "👍", t: { uz: "Ha", ru: "Да" } },
        { ic: "🙅", t: { uz: "Yo'q", ru: "Нет" } },
  ...
  {
    ic: "🏠",
    h: { uz: "Bosh sahifa har kimda o'ziniki", ru: "Главная у каждого своя" },
  ...
  },
  {
    ...
    body: null,
    predict: {
      ask: { uz: "Sizningcha, odamlar ko'radigan kinolarning qanchasi tavsiyadan keladi?", ru: "Как вы думаете, какая часть фильмов, которые смотрят люди, приходит из рекомендаций?" },
      chips: [
  ...
  },
  {
    ic: null,
    big: "80%",
    h: { uz: "2016-yilda Netflix buni ochiq aytdi", ru: "В 2016 году Netflix сказал это открыто" },
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "🎬 Haqiqiy voqea", ru: "🎬 Реальная история" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={betPending && !isMentorK} label={betPending && !isMentorK ? tr3({ uz: "Avval o'zingiz belgilang", ru: "Сначала сделайте свой выбор" }) : last ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: `Keyingi bosqich (${i + 1}/${K6_SLIDES.length})`, ru: `Следующий шаг (${i + 1}/${K6_SLIDES.length})` })} onClick={last ? onNext : () => setI(i + 1)} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Kino olamidan <span className="italic" style={{ color: T.accent }}>mashhur voqea</span></>, ru: <>Известная история <span className="italic" style={{ color: T.accent }}>из мира кино</span></> })}</h2></div>
    ...
    /* 🔴 ETALON 22 (sanoq-mosligi): bashoratli bosqichda ham hisoblagich uzluksiz
       turadi (1·2·3·4·5·6) va har bosqichda AYNAN BITTA joyda ko'rinadi. */
            ...
            <span className="k-slide-eyebrow">{bet === void 0 ? tr3({ uz: "🎲 Avval o'zingiz belgilab ko'ring", ru: "🎲 Сначала попробуйте угадать сами" }) : tr3({ uz: "🎬 Haqiqiy voqea", ru: "🎬 Реальная история" })} · {i + 1} / {K6_SLIDES.length}</span>
    ...
    return <button key={k} className={cls} disabled={locked} onClick={() => setBets((p) => ({ ...p, [i]: k }))}>
        ...
        {showSlide && <div className="k-slide fade-step" key={`s${i}`}>
            {!c.predict && <span className="k-slide-eyebrow">{tr3({ uz: "🎬 Haqiqiy voqea", ru: "🎬 Реальная история" })} · {i + 1} / {K6_SLIDES.length}</span>}
    ...
    /* ETALON 22: ko'prik-bosqichi ham sanoqqa kiradi — zanjir uzilmaydi */
            ...
            <span className="k-slide-eyebrow">{tr3({ uz: "🎬 Haqiqiy voqea", ru: "🎬 Реальная история" })} · {i + 1} / {K6_SLIDES.length}</span>
            <p className="body" style={{ margin: "10px 0 0", color: T.ink }}>{tr3({ uz: <>Netflix ham qaysi ma'lumot muhimligini tanlaydi. <b>Endi shu ishni siz qilasiz.</b></>, ru: <>Netflix тоже выбирает, какие данные важны. <b>Теперь это сделаете вы.</b></> })}</p>
          </div>}
      </div>
    </Stage>;
};
var Screen7 = (props) => <QuestionScreen
  {...props}
  eyebrow={tr3({ uz: "Tekshiruv · bosh sahifa", ru: "Проверка · главная страница" })}
  scope="module-mikro"
  ctaLabel={tr3({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr3({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr3({ uz: "🎬 Netflix bosh sahifasi nimadan yig'iladi?", ru: "🎬 Из чего собирается главная Netflix?" })} />}
  questionText={tr3({ uz: "Netflix bosh sahifasi nimadan yig'iladi", ru: "Из чего собирается главная Netflix" })}
  options={[tr3({ uz: "Har kim ilgari ko'rgan kinolardan", ru: "Из фильмов, которые каждый смотрел раньше" }), tr3({ uz: "Eng ko'p pul ishlagan kinolardan", ru: "Из фильмов, которые заработали больше всего денег" }), tr3({ uz: "Hamma uchun tuzilgan bitta ro'yxatdan", ru: "Из одного списка, составленного для всех" })]}
  correctIdx={0}
  explainCorrect={tr3({ uz: "To'g'ri. Shu sababli ikki odam Netflix'ni ochsa, bosh sahifalari har xil bo'ladi.", ru: "Верно. Поэтому, когда два человека открывают Netflix, главные у них разные." })}
  explainWrong={{
    1: tr3({ uz: "Pul haqida voqeada gap yo'q: bosh sahifa har kimning ilgari ko'rganidan yig'iladi.", ru: "О деньгах в истории ни слова: главная собирается из того, что каждый смотрел раньше." }),
    2: tr3({ uz: "Bitta umumiy ro'yxat bo'lganda ikki odam bir xil bosh sahifani ko'rardi.", ru: "Будь список один на всех, два человека видели бы одинаковую главную." }),
    default: tr3({ uz: "Bosh sahifa har kimning ilgari ko'rgan kinolaridan quriladi — shuning uchun u har kimda har xil.", ru: "Главная строится из фильмов, которые каждый смотрел раньше, — поэтому у всех она разная." })
  }}
/>;
var OUT_KEY = "pm-m4d2-data";
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const live = gate.live;
  const isMentor = !!(live && live.mode === "mentor");
  const [list, setList] = useState5(() => storedAnswer && Array.isArray(storedAnswer.maydonlar) ? storedAnswer.maydonlar : []);
  const [dMaydon, setDMaydon] = useState5("");
  const [dBolim, setDBolim] = useState5("");
  const [edit, setEdit] = useState5(null);
  const [focus, setFocus] = useState5(false);
  const [yordamOpen, setYordamOpen] = useState5(true);
  ...
  useEffect6(() => {
    ...
    try {
      localStorage.setItem(OUT_KEY, JSON.stringify({ maydonlar: list.slice(0, 3), savedAt: Date.now() }));
    } catch {
    }
  ...
  const startEdit = (k) => {
    setEdit(k);
    setDMaydon(list[k].maydon);
    setDBolim(list[k].bolim);
  };
  const nQadam = edit === null ? list.length + 1 : edit + 1;
  const navLabel = done || isMentor ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : list.length === 0 ? tr3({ uz: "① Birinchi maydonni yozing va saqlang", ru: "① Напишите и сохраните первое поле" }) : tr3({ uz: `② Yana ${3 - list.length} maydon yozing`, ru: `② Напишите ещё ${3 - list.length} ${3 - list.length === 1 ? "поле" : "поля"}` });
  return <Stage eyebrow={tr3({ uz: "Mustaqil ish · uch maydon", ru: "Самостоятельная работа · три поля" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Musiqa ilovasiga <span className="italic" style={{ color: T.accent }}>uchta maydon</span> yozing.</>, ru: <>Напишите <span className="italic" style={{ color: T.accent }}>три поля</span> для музыкального приложения.</> })}</h2></div>
        <Mentor>{tr3({ uz: "Har maydonga bitta savol bering: buni saqlasak, ilovada qaysi bo'lim ochiladi?", ru: "Задайте каждому полю один вопрос: если это сохранить, какой раздел откроется в приложении?" })}</Mentor>
        {
    /* 80a: havoda uch doira — yozilgani yashil, joriysi pulsda, kelgusi punktir */
  }
        <div className="stps fade-up">
          {[0, 1, 2].map((k) => <span key={k} className={`stp ${list.length > k ? "done" : (edit === null ? list.length : edit) === k ? "on" : ""}`}><i>{list.length > k ? "✓" : k + 1}</i>{tr3({ uz: <>{k + 1}-maydon{list.length > k ? " tayyor" : ""}</>, ru: <>Поле {k + 1}{list.length > k ? " готово" : ""}</> })}</span>)}
        </div>
        <div className="split">
          <Col gap={9}>
            {
    /* 80b: ekranning yagona kartasi — ikki yozuv-joyi + jonli javob-qatori */
            ...
            {(!done || edit !== null) && <div className="wsp-ed">
                <span className="wsp-ed-h">{tr3({ uz: <>{nQadam}-maydon</>, ru: <>Поле {nQadam}</> })}</span>
                <span className="wsp-q">{tr3({ uz: "Ilova nimani eslab qolsin?", ru: "Что приложению запомнить?" })}</span>
                <input
    className={`reflect-input${inputTurn ? " await" : ""}${uzunM ? " filled" : ""}`}
    value={dMaydon}
    maxLength={90}
    placeholder={tr3({ uz: "Masalan: qo’shiqchi nomi", ru: "Например: имя исполнителя" })}
    onFocus={() => setFocus(true)}
    onBlur={() => setFocus(false)}
    onChange={(e) => setDMaydon(e.target.value)}
  />
                <span className="wsp-q">{tr3({ uz: "Ilovada qaysi bo'lim ochiladi?", ru: "Какой раздел откроется в приложении?" })}</span>
                <input
    className={`reflect-input${bolimBor ? " filled" : ""}`}
    value={dBolim}
    maxLength={90}
    placeholder={tr3({ uz: "Masalan: Qo’shiqchining boshqa qo’shiqlari", ru: "Например: Другие песни исполнителя" })}
  ...
  />
    ...
    /* 106d: ikki tomonlama javob — bloklamaydi, yo'naltiradi */
                ...
                {dMaydon.trim().length > 0 && !uzunM && <p className="sfb ask">{tr3({ uz: "🤔 Juda qisqa qoldi — ilova aynan nimani eslab qolishini aniqroq yozing.", ru: "🤔 Слишком коротко — напишите точнее, что именно приложению запомнить." })}</p>}
                {uzunM && umumiy && <p className="sfb ask">{tr3({ uz: "🤔 Bu hali bo'lim nomi emas. Bundan qaysi bo'lim ochiladi? Masalan: «Kechqurun tinglaganlaringiz».", ru: "🤔 Это ещё не название раздела. Какой раздел из этого откроется? Например: «Слушали вечером»." })}</p>}
                {uzunM && !umumiy && takror && <p className="sfb ask">{tr3({ uz: "🤔 Bu bo'lim yuqorida allaqachon yozilgan — boshqa bo'limni oling.", ru: "🤔 Этот раздел уже записан выше — возьмите другой." })}</p>}
                {canSave && <p className="sfb ok">{tr3({ uz: "✅ Bo'lim nomi aytilgan — bu maydon kerak.", ru: "✅ Раздел назван — это поле нужно." })}</p>}
    ...
    /* 80c: yozilganlar YOZISH PAYTIDA ko'rinmaydi; uchtasi yozilgach ro'yxat ochiladi */
            ...
            {done && edit === null && <div className="wsp-list fade-step">
                <span className="wsp-list-h">{tr3({ uz: "🎧 Musiqa ilovasi eslab qoladigan uchta narsa", ru: "🎧 Три вещи, которые запоминает музыкальное приложение" })}</span>
          ...
          <Col gap={9}>
            <div className="wsp-task">
              ...
              <span className="wsp-task-nom">{tr3({ uz: "Har maydondan bitta foydali bo'lim chiqsin", ru: "Из каждого поля — один полезный раздел" })}</span>
              {
    /* 106c-b: holat ko'rsatkichi */
            ...
            <div className="wsxrow">
              <div className={`wsx ${yordamOpen ? "open" : ""}`}>
                ...
                {yordamOpen && <div className="wsx-body"><p>{tr3({ uz: "O'zingizga savol bering: bu maydonni o'chirsam, ilovada qaysi bo'lim yo'qoladi? Bo'lim nomi topilmasa — o'sha maydon kerak emas.", ru: "Спросите себя: если удалить это поле, какой раздел исчезнет из приложения? Не нашли название раздела — это поле не нужно." })}</p></div>}
              </div>
              <div className={`wsx star ${starOpen ? "open" : ""}`}>
                ...
                {starOpen && <div className="wsx-body"><p>{tr3({ uz: "To'rtinchi maydon yozing — ilova saqlamaydigan narsa va nega saqlamasligi.", ru: "Напишите четвёртое поле — то, что приложение НЕ сохраняет, и почему." })}</p></div>}
              </div>
            </div>
            <StudentPracticePulse live={live} screen={screen} />
        ...
        {done && edit === null && <div className="done-mini fade-step">{tr3({ uz: <>✅ Uch maydoningiz yozildi <span className="dm-sub">— har biridan bittadan bo'lim quriladi</span></>, ru: <>✅ Ваши три поля записаны <span className="dm-sub">— из каждого строится по разделу</span></> })}</div>}
        <MentorNote>{tr3({ uz: "«Foydalanuvchi haqida umumiy ma'lumot» kabi javoblar chiqadi — bu eng foydali xato. Javob-qatori uni tutadi, siz muhokama qiling: bundan qaysi bo'lim quriladi? Bu ishni o'quvchilar bajaradi, siz kuzatasiz; «Davom etish» siz uchun ochiq.", ru: "Появятся ответы вроде «общая информация о пользователе» — это самая полезная ошибка. Строка-ответ её поймает, а вы обсудите: какой раздел из этого строится? Эту работу делают ученики, вы наблюдаете; «Продолжить» для вас открыто." })}</MentorNote>
      </div>
    </Stage>;
};
var YOZUVLAR = [
...
var Screen9 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [built, setBuilt] = useState5(null);
  const [miss, setMiss] = useState5("");
  ...
  const qur = () => {
    ...
    if (ortiqcha.length === 0 && yetmagan.length === 0) {
      setBuilt(sel.slice().sort((a, b) => a - b));
      setMiss("");
      return;
    }
    setMissedOnce(true);
    if (achMiss) achMiss.miss(screen);
    setMiss(ortiqcha.length > 0 ? tr3({ uz: "🤔 Bu yozuv bo'lim nomiga mos kelmaydi — uni yana bir bor o'qing.", ru: "🤔 Эта запись не подходит под название раздела — перечитайте его ещё раз." }) : tr3({ uz: "🤔 Bo'limga mos yana bitta yozuv bor.", ru: "🤔 Есть ещё одна запись, которая подходит разделу." }));
  };
  const keyingi = () => {
    setBuilt(null);
    setSel([]);
  ...
  const navLabel = done || isMentor ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : tr3({ uz: `Yana ${RAUNDLAR.length - ri} bo'limni quring`, ru: `Постройте ещё ${RAUNDLAR.length - ri} ${RAUNDLAR.length - ri === 1 ? "раздел" : "раздела"}` });
  const korsat = built ? built : done ? RAUNDLAR[RAUNDLAR.length - 1].togri : null;
  return <Stage eyebrow={tr3({ uz: "Tekshiruv · bo'limni quring", ru: "Проверка · постройте раздел" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(9px,1.4vw,14px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Bo'limni jadvaldan <span className="italic" style={{ color: T.accent }}>quring</span>.</>, ru: <><span className="italic" style={{ color: T.accent }}>Постройте</span> раздел из таблицы.</> })}</h2></div>
        <Mentor>{tr3({ uz: <>«{tr3(cur.bolim)}» bo'limiga mos qatorlarni belgilang.</>, ru: <>Отметьте строки, которые подходят разделу «{tr3(cur.bolim)}».</> })}</Mentor>
        <div className="split s9">
          <Col gap={9}>
            <div className={`s9ask${korsat ? " ok" : ""}`}>
              <span className="s9ask-n">{tr3({ uz: "Bo'lim", ru: "Раздел" })} {Math.min(ri + 1, RAUNDLAR.length)} / {RAUNDLAR.length}</span>
              <span className="s9ask-t">«{tr3(cur.bolim)}» <i>({tr3(cur.shart)})</i></span>
            </div>
            <div className="tbl">
              <div className="tbl-head">
                <span className="tbl-c c0">#</span>
                <span className="tbl-c">{tr3({ uz: "🎵 Qaysi qo'shiq", ru: "🎵 Какая песня" })}</span>
                <span className="tbl-c">{tr3({ uz: "⏰ Qachon", ru: "⏰ Когда" })}</span>
                <span className="tbl-c">{tr3({ uz: "☑️ Oxirigacha tinglandimi", ru: "☑️ Дослушана до конца" })}</span>
              </div>
              {YOZUVLAR.map((y) => {
    const chosen = sel.includes(y.n);
    const inBuilt = !!(korsat && korsat.includes(y.n));
    return <button key={y.n} type="button" className={`tbl-row${chosen ? " on" : ""}${inBuilt ? " fixed" : ""}`} onClick={() => toggleRow(y.n)} disabled={isMentor || !!built || done}>
                    <span className="tbl-c c0"><i className="tbl-box">{chosen ? "✓" : ""}</i>{y.n}</span>
                    <span className="tbl-c" data-l={tr3({ uz: "🎵 Qaysi qo'shiq", ru: "🎵 Какая песня" })}>{y.qoshiq}</span>
                    <span className="tbl-c mono" data-l={tr3({ uz: "⏰ Qachon", ru: "⏰ Когда" })}>{y.vaqt}</span>
                    <span className="tbl-c" data-l={tr3({ uz: "☑️ Oxirigacha tinglandimi", ru: "☑️ Дослушана до конца" })}><i className={`yn ${y.oxir ? "ok" : "no"}`}>{y.oxir ? tr3({ uz: "ha", ru: "да" }) : tr3({ uz: "yo'q", ru: "нет" })}</i></span>
                  </button>;
  })}
            </div>
            {!built && !done && <button type="button" className="wsp-save" disabled={sel.length === 0 || isMentor} onClick={qur}>{tr3({ uz: "Bo'limni qurish →", ru: "Построить раздел →" })}</button>}
            {built && ri < RAUNDLAR.length - 1 && <button type="button" className="wsp-save" onClick={keyingi}>{tr3({ uz: "Keyingi bo'lim →", ru: "Следующий раздел →" })}</button>}
            {built && ri === RAUNDLAR.length - 1 && !done && <button type="button" className="wsp-save" onClick={keyingi}>{tr3({ uz: "✓ Uch bo'lim qurildi", ru: "✓ Три раздела построены" })}</button>}
            {!done && <AchRule screen={screen} />}
          </Col>
          <Col gap={9}>
            <Phone quiet={!korsat}>
              {korsat && <div className="pho-sec on pop">
                  <span className="pho-sec-h">{tr3(cur.bolim)}</span>
                  <span className="pho-sec-row">{korsat.map((n) => {
    const y = YOZUVLAR.find((z) => z.n === n);
    return <i key={n}>{y.qoshiq} · {y.vaqt}</i>;
  })}</span>
                </div>}
            </Phone>
            <StudentPracticePulse live={live} screen={screen} />
...
var KOD_STARTER = { uz: `const tarix = [
...
var KOD_TASK = {
  ...
  requirements: [
    {
      id: "royxat",
      label: { uz: "Funksiya ro'yxat qaytaradi", ru: "Функция возвращает список" },
      check: checks.evalEquals("Array.isArray(yoqqanlar(tarix))", "true", { uz: "Funksiya ro'yxat (massiv) qaytarsin", ru: "Пусть функция возвращает список (массив)" })
    },
    {
      id: "oxir",
      label: { uz: "Faqat oxirigacha tinglanganlar qoladi", ru: "Остаются только дослушанные до конца" },
      check: checks.custom((x) => /oxirigacha/.test(yoqqanlarTanasi(x.js)) ? true : tr3({ uz: "Funksiya ichida yozuvning oxirigacha maydonini tekshiring", ru: "Внутри функции проверьте поле oxirigacha у записи" }))
    },
    {
      id: "uch",
      label: { uz: "Natijada uch nom chiqadi", ru: "В результате выходят три названия" },
      check: checks.evalEquals("yoqqanlar(tarix).join(',')", "Ohang,Ohang,Shamol", { uz: "Natijada Ohang, Ohang va Shamol chiqishi kerak", ru: "В результате должны выйти Ohang, Ohang и Shamol" })
    }
  ]
};
var ScreenCoding = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const navLabel = done || isMentor ? tr3({ uz: "Davom etish", ru: "Продолжить" }) : !stage2 ? tr3({ uz: "① Maydonni belgilang", ru: "① Отметьте поле" }) : tr3({ uz: "② Kodni yozing", ru: "② Напишите код" });
  return <Stage eyebrow={tr3({ uz: "Koding · 🛠 kod oynasi", ru: "Кодинг · 🛠 окно кода" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive turnBusy={!done} disabled={!done && !isMentor} label={navLabel} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.5vw,15px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Yoqqan qo'shiqlarni ajratadigan <span className="italic" style={{ color: T.accent }}>kod</span> yozamiz.</>, ru: <>Напишем <span className="italic" style={{ color: T.accent }}>код</span>, который отбирает понравившиеся песни.</> })}</h2></div>
        {!stage2 ? <>
            <Mentor>{tr3({ uz: "Kod bitta maydonga qarab ishlaydi. Qaysi maydondan «Sizga yoqadi» bo'limi qurilishini belgilang.", ru: "Код смотрит на одно поле. Отметьте, из какого поля строится раздел «Вам нравится»." })}</Mentor>
            <div className={`cmt hunt${missedOnce ? " calm" : ""}`}>
              <span className="cmt-lbl">{tr3({ uz: "🔎 «Sizga yoqadi» qaysi maydondan quriladi?", ru: "🔎 Из какого поля строится «Вам нравится»?" })}</span>
              <div className="gt-rows">
                {GATE_ITEMS.map((g) => <button key={g.id} type="button" className={`fchoice${miss === g.id ? " miss" : ""}`} onClick={() => pickGate(g)}>
                    {g.ic} {tr3(g.t)}
                  </button>)}
              </div>
              {missedOnce && <p className="cmt-tip">{tr3({ uz: "🤔 Bu maydondan boshqa bo'lim quriladi. Qo'shiq yoqqanini qaysi belgi ko'rsatadi?", ru: "🤔 Из этого поля строится другой раздел. Какой признак показывает, что песня понравилась?" })}</p>}
            </div>
          </> : <>
            <Mentor>{tr3({ uz: "Hozir siz tanladingiz. Endi shu tanlovni kod bajaradi.", ru: "Сейчас выбрали вы. Теперь этот выбор выполнит код." })}</Mentor>
            <div className="cmt-fold fade-step"><span className="cmt-done">{tr3({ uz: "✓ Maydon belgilandi: ☑️ Qo'shiq oxirigacha tinglandimi", ru: "✓ Поле отмечено: ☑️ Дослушана ли песня до конца" })}</span></div>
            <div className="split">
              <Col gap={10}>
                <div className={`kdpanel${done ? " is-done" : ""}`}>
                  <p className="flow-label">{tr3({ uz: "Kod nima qilsin", ru: "Что должен делать код" })}</p>
                  <ul className="kdreq">
                    <li>{tr3({ uz: "Funksiya ro'yxat qaytaradi", ru: "Функция возвращает список" })}</li>
                    <li>{tr3({ uz: "Faqat oxirigacha tinglanganlar qoladi", ru: "Остаются только дослушанные до конца" })}</li>
                    <li>{tr3({ uz: "Natijada uch nom chiqadi", ru: "В результате выходят три названия" })}</li>
                  </ul>
                  <div className={`wsx star ${yordamOpen ? "open" : ""}`}>
                    <button className="wsx-toggle" onClick={() => setYordamOpen((o) => !o)}>{tr3({ uz: "💡 Yordam", ru: "💡 Подсказка" })} {yordamOpen ? "▾" : "▸"}</button>
                    {yordamOpen && <div className="wsx-body">
                      <p>{tr3({ uz: <>Bitta yozuvdan boshlang: uning <code className="qcode">oxirigacha</code> maydoni rostmi? Ishlagach qolganlariga o'ting.</>, ru: <>Начните с одной записи: её поле <code className="qcode">oxirigacha</code> истинно? Заработало — переходите к остальным.</> })}</p>
                      <p>{tr3({ uz: "⭐ Qo'shimcha: ikkinchi funksiya yozing — kechqurun (soat 20 dan keyin) tinglangan qo'shiqlarni qaytarsin.", ru: "⭐ Дополнительно: напишите вторую функцию — пусть возвращает песни, прослушанные вечером (после 20:00)." })}</p>
                    </div>}
                  </div>
                  {done && <div className="done-mini fade-step">{tr3({ uz: <>✅ Uch nom chiqdi <span className="dm-sub">— kod endi yoqqan qo'shiqlarni o'zi ajratadi</span></>, ru: <>✅ Три названия вышли <span className="dm-sub">— теперь код сам отбирает понравившиеся песни</span></> })}</div>}
              ...
              <Col gap={10}>
                <div className="klaunch">
                  <span className="klaunch-lbl">{tr3({ uz: "🎧 Kod oynasi", ru: "🎧 Окно кода" })}</span>
                  <ul className="klaunch-pre">
                    <li><i>📄</i>{tr3({ uz: "4 ta yozuv", ru: "4 записи" })}</li>
                    <li><i>⚙️</i>{tr3({ uz: "1 ta funksiya", ru: "1 функция" })}</li>
                    <li><i>🎯</i>{tr3({ uz: "Natija: 3 ta qo'shiq", ru: "Результат: 3 песни" })}</li>
                  </ul>
...
var REFLECT_KEY = "pm-m4d2-reflection";
...
var ScreenReflection = ({ screen, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr3({ uz: "Mustahkamlash · 2 qadam", ru: "Закрепление · 2 шага" })} screen={screen} navContent={<><NavBack onPrev={onPrev} /><NavNext turnBusy={!written} label={tr3({ uz: "Davom etish", ru: "Продолжить" })} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr3({ uz: <>Uch maydoningizni <span className="italic" style={{ color: T.accent }}>yoddan</span> ayta olasizmi?</>, ru: <>Сможете назвать свои три поля <span className="italic" style={{ color: T.accent }}>по памяти</span>?</> })}</h2></div>
        <Mentor>{tr3({ uz: <>Ekranga qaramasdan javob bering: musiqa ilovasi nimani eslab qoladi va har biridan qaysi bo'lim quriladi? Avval {yakka ? "ovoz chiqarib o'zingizga" : "sherigingizga"} ayting, so'ng shu javobni bir qatorda yozing.</>, ru: <>Ответьте, не глядя на экран: что запоминает музыкальное приложение и какой раздел строится из каждого поля? Сначала скажите {yakka ? "вслух самому себе" : "напарнику"}, потом запишите этот ответ одной строкой.</> })}</Mentor>
        <div className="rcp-flow">
          <div className="rcp-step fade-up delay-1">
            <div className="rcp-step-h"><span className="rcp-n">1</span><div><span className="rcp-t">🗣 {yakka ? tr3({ uz: "Ovoz chiqarib ayting: qaysi uch maydon va qaysi bo'lim", ru: "Скажите вслух: какие три поля и какой раздел" }) : tr3({ uz: "Sherigingizga ayting: qaysi uch maydon va qaysi bo'lim", ru: "Скажите напарнику: какие три поля и какой раздел" })}</span></div></div>
          ...
          <div className="rcp-step fade-up delay-2">
            ...
            <span className={`turn-wrap${inputTurn ? " turn-ring" : ""}`}>
              <input className="reflect-input" value={text} onChange={(e) => save(e.target.value)} onFocus={() => setReflFocus(true)} onBlur={() => setReflFocus(false)} placeholder={tr3({ uz: "Ilova ... ni eslab qoladi, undan ... bo'limi quriladi", ru: "Приложение запоминает ..., из этого строится раздел ..." })} maxLength={160} />
            </span>
            {written && <p className="small" style={{ margin: 0, color: T.success, fontWeight: 700 }}>{tr3({ uz: "✓ Yozildi!", ru: "✓ Записано!" })}</p>}
          </div>
        </div>
        <MentorNote>{tr3({ uz: "Uchdan biri «qaysi bo'lim quriladi» degan savolga javob berolmasa — tugmalar ekranini qayta oching va bittasini birga o'chirib ko'ring.", ru: "Если треть класса не может ответить на вопрос «какой раздел строится» — снова откройте экран с переключателями и выключите один вместе." })}</MentorNote>
      </div>
    </Stage>;
};
...
var ScreenFinalTest = (props) => <QuestionScreen
  ...
  ctaLabel={tr3({ uz: "Javobni tanlang", ru: "Выберите ответ" })}
  revealPrefix={tr3({ uz: "To'g'ri javob", ru: "Верный ответ" })}
  question={<TestQ ask={tr3({ uz: "📋 Qaysi maydonni saqlashga arziydi?", ru: "📋 Какое поле стоит хранить?" })} />}
  questionText={tr3({ uz: "Qaysi maydonni saqlashga arziydi", ru: "Какое поле стоит хранить" })}
  options={[tr3({ uz: "Telefondagi kontaktlar ro'yxati", ru: "Список контактов в телефоне" }), tr3({ uz: "Qo'shiq oxirigacha tinglandimi", ru: "Дослушана ли песня до конца" }), tr3({ uz: "Qo'shiq qayerda tinglandi", ru: "Где прослушана песня" })]}
  correctIdx={1}
  explainCorrect={tr3({ uz: "To'g'ri — bundan «Sizga yoqadi» bo'limi quriladi; qolgan ikkitasi bo'lim bermaydi.", ru: "Верно — из этого строится раздел «Вам нравится»; два других раздела не дают." })}
  explainWrong={{
    0: tr3({ uz: "Kontaktlardan bitta ham bo'lim qurilmaydi — bu maydon bekorga saqlanadi.", ru: "Из контактов не строится ни один раздел — это поле хранится зря." }),
    2: tr3({ uz: "Qayerda tinglanganidan bitta ham bo'lim qurilmaydi — buni beshta tugma sinovida ko'rgansiz.", ru: "Из места прослушивания не строится ни один раздел — вы видели это в опыте с пятью переключателями." }),
    default: tr3({ uz: "Saqlashga arziydigan maydon ortida bitta bo'lim turadi.", ru: "За полем, которое стоит хранить, стоит один раздел." })
  }}
/>;
var HW_KEY = "pm-m4d2-hw-target";
var HW_VARIANT = [
  ...
  return <div className="card hw fade-step" ref={innerRef}>
      ...
      {variant ? <div className="pmtask fade-step">
          ...
          <div className="pmtask-rows">
            <div className="pmtask-row"><span className="pmtask-k">{tr3({ uz: "Nechta", ru: "Сколько" })}</span><span className="pmtask-v"><b>{variant === "qisqa" ? tr3({ uz: "1 ta maydon", ru: "1 поле" }) : tr3({ uz: "3 maydon + 1 qo'shimcha", ru: "3 поля + 1 дополнительное" })}</b></span></div>
...
var ACH_TRIGGERS = { s4: "memoryMaker", s8: "fieldWriter", s9: "sectionBuilder", s10: "dataCoder" };
...
var Q_LABELS = { 3: { uz: "1 — Nima qaytadi", ru: "1 — Что возвращается" }, 5: { uz: "2 — Bo'lim bermagan maydon", ru: "2 — Поле без раздела" }, 7: { uz: "3 — Bosh sahifa", ru: "3 — Главная страница" }, 11: { uz: "4 — Yakuniy savol", ru: "4 — Итоговый вопрос" } };
var QUIZ_MS = 15e3;
var QZ_BG_SHAPES = [
...
function QzFX() {
  ...
  useEffect6(() => {
    ...
    const TOK = tr3({ uz: ["maydon", "bo'lim", "saqlash", "tinglash", "yozuv", "tavsiya", "jadval", "ilova", "✅", "⬜"], ru: ["поле", "раздел", "хранение", "прослушивание", "запись", "рекомендация", "таблица", "приложение", "✅", "⬜"] });
    const em = [], toks = [];
      ...
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="hero">
          <div className="hero-l">
            ...
            <h2 className="title h-title fade-up d1">{tr3({ uz: <>Uchta <span className="italic" style={{ color: T.accent }}>maydoningiz</span> yozildi.</>, ru: <>Ваши три <span className="italic" style={{ color: T.accent }}>поля</span> записаны.</> })}</h2>
          </div>
...
`;
function PmLesson11({ lang: langProp, onFinished, liveToken }) {
