...
//  Manba:  src/4-Modull/BackendCrudPracticeLesson.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs src/4-Modull/BackendCrudPracticeLesson.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// src/4-Modull/BackendCrudPracticeLesson.jsx
...
var T = {
  ...
  amber: "#B45309",
  amberSoft: "#FBEBD8",
  // PUT/UPDATE + REJISSYOR roli (CRUD rang semantikasi)
  shadowBase: "58, 53, 48"
};
...
var LESSON_META = { lessonId: "backend-crud-practice-p1-v18", lessonTitle: { uz: "Praktika: Backend CRUD — AvtoIjara", ru: "Практика: Backend CRUD — AvtoIjara" } };
var HW_TOKENS = [
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
      ...
      {reveal ? <div className="mstats-big">
          <div className="mstats-chip okc"><span className="mstats-chip-n">{ok}</span><span className="mstats-chip-t">{tr2({ uz: "to'g'ri ✅", ru: "правильно ✅" })}</span></div>
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr2({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и число ✅/❌ скрыты — по кнопке «Открыть результат» всё откроется сразу и у вас, и на экранах учеников." })}</p>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlang.</>, ru: <>⚠️ Только <b>{pct}%</b> правильно — эта тема осталась классу непонятной. Перед продолжением рекомендуем короткое повторение.</> })}</p>
            ...
            {level === "maybe" && <>
              <p className="mstats-verdict-t">{tr2({ uz: <>🟡 <b>{pct}%</b> to'g'ri — yomon emas. Xohlasangiz, davom etishdan oldin qisqa takrorlab oling.</>, ru: <>🟡 <b>{pct}%</b> правильно — неплохо. Если хотите, коротко повторите перед продолжением.</> })}</p>
              {onOpenRecap && <button className="rc-open soft" onClick={onOpenRecap}>{tr2({ uz: "📖 Qisqa takrorlash", ru: "📖 Короткое повторение" })}</button>}
            </>}
            {level === "good" && <p className="mstats-verdict-t">{tr2({ uz: <>✅ <b>{pct}%</b> to'g'ri — sinf mavzuni o'zlashtirdi. Bemalol davom eting!</>, ru: <>✅ <b>{pct}%</b> правильно — класс освоил тему. Смело продолжайте!</> })}</p>}
          </div>;
  })()}
      ...
      {reveal && struggling && <p className="mstats-warn">{tr2({ uz: "⚠️ Ko'pchilik xato qildi — bu mavzu tushunarsiz bo'lgan ko'rinadi. Yana bir bor tushuntiring.", ru: "⚠️ Многие ошиблись — похоже, тема осталась непонятной. Рекомендуем объяснить ещё раз." })}</p>}
...
var CARS = [
  { id: 1, nom: "Cobalt", narx: 28e4, yil: 2022, emoji: "🚗", bg: "linear-gradient(135deg,#7EA6F4,#2E4A9E)" },
  { id: 2, nom: "Malibu", narx: 52e4, yil: 2023, emoji: "🚙", bg: "linear-gradient(135deg,#8FD3A8,#2E7A4E)" },
  { id: 3, nom: "Kia K5", narx: 61e4, yil: 2023, emoji: "🚘", bg: "linear-gradient(135deg,#F4B26A,#C9622E)" }
];
var POOL_CARS = [
  { id: 4, nom: "Spark", narx: 19e4, yil: 2021, emoji: "🚐", bg: "linear-gradient(135deg,#F4A6C0,#B5446E)" },
  { id: 5, nom: "Tracker", narx: 45e4, yil: 2024, emoji: "🚓", bg: "linear-gradient(135deg,#B9A8F4,#6A4AC9)" },
  { id: 6, nom: "Onix", narx: 34e4, yil: 2023, emoji: "🚕", bg: "linear-gradient(135deg,#F4D06A,#C99B2E)" }
];
var COLUMNS = [
...
var M_COLOR = { GET: T.blue, POST: T.success, PUT: T.amber, DELETE: T.danger };
var MBadge = ({ m }) => <span className="pm-method" style={{ background: M_COLOR[m] }}>{m}</span>;
...
var RolePassport = ({ answers, active }) => {
  const sig = ROLES.map((r) => answers && answers[r.idx] ? "1" : "0").join("");
  const atMount = useRef3(sig);
  const [press, setPress] = useState3("000");
  const [payoff, setPayoff] = useState3(false);
  useEffect4(() => {
    const fresh = ROLES.map((_, i) => sig[i] === "1" && atMount.current[i] === "0" ? "1" : "0").join("");
    if (!fresh.includes("1")) return;
    setPress(fresh);
    const ts = [setTimeout(() => setPress("000"), 1e3)];
    if (sig === "111") {
  ...
  return <div className="fade-up">
    <div className={`rp ${payoff ? "full" : ""}`} aria-label={tr2({ uz: "Rol pasporti", ru: "Паспорт ролей" })}>
      <span className="rp-lbl">{tr2({ uz: <>Rol<br />pasporti</>, ru: <>Паспорт<br />ролей</> })}</span>
      <div className="rp-stamps">
        {ROLES.map((r, i) => {
    const got = !!(answers && answers[r.idx]);
    const on = active === r.key && !got;
    return <React3.Fragment key={r.key}>
              ...
              <span className={`rp-stamp ${got ? "got" : ""} ${on ? "on" : ""} ${press[i] === "1" ? "press" : ""}`} style={{ "--rc": r.col, "--rs": r.soft }}>
                ...
                <span className="rp-col"><b className="rp-name">{tr2(r.name)}</b><i className="rp-tag">{tr2(r.tag)}</i></span>
...
var CarCard = ({ car, onDelete, flash }) => <div className="rocard el-in" style={{ position: "relative", boxShadow: flash ? `0 0 0 2px ${T.success}, 0 6px 16px -5px rgba(0,0,0,0.2)` : void 0, transition: "all 0.3s" }}>
    ...
    <div className="robody">
      ...
      <div className="rostats"><span>{sp(car.narx)} {tr2({ uz: "so'm/kun", ru: "сум/день" })}</span><span style={{ marginLeft: "auto", color: T.ink3 }}>{car.yil}</span></div>
    </div>
  </div>;
var CardGrid = ({ children, cols = 3 }) => <div style={{ display: "grid", gridTemplateColumns: `repeat(${cols},1fr)`, gap: 8 }}>{children}</div>;
var DbTable = ({ rows, flashId, dimId }) => <div className="db">
    ...
    {rows.length ? rows.map((r) => <div key={r.id} className={`db-row el-in ${flashId === r.id ? "flash" : ""}`} style={{ opacity: dimId === r.id ? 0.35 : 1, textDecoration: dimId === r.id ? "line-through" : "none" }}>
        <span>{r.id}</span><span>{r.nom}</span><span>{sp(r.narx)}</span><span>{r.yil}</span>
      </div>) : <div className="db-empty">{tr2({ uz: "— bo'sh (0 qator) —", ru: "— пусто (0 строк) —" })}</div>}
  </div>;
var Resp = ({ status, text, json }) => <div className="pm-resp el-in">
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={{ uz: "Kirish", ru: "Введение" }} screen={screen} scrollSignal={picked !== null} navContent={<NavNext optionalLive disabled={picked === null} label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} />}>
      <div className="screen">
        <h1 className="title h-title fade-up">{tr2({ uz: <>Saytdagi mashinalar ro'yxatini nega <span className="italic" style={{ color: T.accent }}>o'zgartirib bo'lmaydi</span>?</>, ru: <>Почему список машин на сайте <span className="italic" style={{ color: T.accent }}>нельзя изменить</span>?</> })}</h1>
        ...
        <Split>
          <Col>
            <div className="fade-up delay-1">
              <Win title="localhost:5173 — AvtoIjara">
                <div className="app-hd">
                  ...
                  <button className={`chip ${shakeId === "add" ? "shake" : ""}`} onClick={() => poke("add")} style={{ padding: "6px 12px", fontSize: 12.5 }}>{tr2({ uz: "+ Mashina qo'shish", ru: "+ Добавить машину" })}</button>
                </div>
                <CardGrid cols={3}>
            ...
            <pre className="code-box fade-up delay-2" style={{ padding: "10px 14px", lineHeight: 1.85 }}>
              ...
              <Cm>{tr2({ uz: "// ↑ ro'yxat kod ichida — bazada saqlanmagan", ru: "// ↑ список внутри кода — в базе не хранится" })}</Cm>
            </pre>
            {tried && <p className="small fade-step" style={{ color: T.accent, fontStyle: "italic", margin: 0 }}>{tr2({ uz: "Tugma bor — lekin hech narsa o'zgarmadi. Ro'yxat kod ichida qotib qolgan, orqada server yo'q!", ru: "Кнопка есть — но ничего не изменилось. Список застыл в коде, за ним нет сервера!" })}</p>}
          </Col>
          <Col>
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr2({ uz: "Haqiqiy AvtoIjara ilovasiga faqat ko'rsatish yetarli emas. Yana nima kerak?", ru: "Настоящему приложению AvtoIjara мало просто показывать. Что ещё нужно?" })}</p>
...
var Screen1 = ({ screen, answers, onNext, onPrev }) => {
  ...
  const PreviewBlock = <Col>
      <p className="flow-label">{tr2({ uz: "Dars oxirida — sizning backend'ingiz", ru: "В конце урока — ваш backend" })}</p>
      <div className="pm">
      ...
      <p className="mono small" style={{ color: T.accent, margin: 0 }}>{tr2({ uz: "→ mashinalar endi bazadan keladi — qo'shish · o'qish · o'zgartirish · o'chirish", ru: "→ машины теперь приходят из базы — добавить · прочитать · изменить · удалить" })}</p>
    </Col>;
  ...
  return <Stage eyebrow={{ uz: "Reja", ru: "План" }} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Boshlaymiz →", ru: "Начинаем →" }} onClick={onNext} /></>}>
      <div className="screen">
        <div className="head">
          <h2 className="title h-title fade-up">{tr2({ uz: <>AvtoIjara'ni <span className="italic" style={{ color: T.accent }}>haqiqiy ilovaga</span> qanday aylantiramiz?</>, ru: <>Как превратить AvtoIjara <span className="italic" style={{ color: T.accent }}>в настоящее приложение</span>?</> })}</h2>
        </div>
...
var Screen2 = ({ screen, answers, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [active, setActive] = useState3(null);
  const [seen, setSeen] = useState3(storedAnswer ? new Set(COLUMNS.map((c) => c.key)) : /* @__PURE__ */ new Set());
  ...
  return <Stage eyebrow={{ uz: "1-qadam · ME'MOR", ru: "Шаг 1 · АРХИТЕКТОР" }} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `${seen.size}/${COLUMNS.length} ustun ko'rildi`, ru: `Столбцов просмотрено: ${seen.size}/${COLUMNS.length}` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Mashinalar haqida bazada <span className="italic" style={{ color: T.accent }}>qaysi ma'lumotni</span> saqlaymiz?</>, ru: <>Какие данные о машинах <span className="italic" style={{ color: T.accent }}>будем хранить</span> в базе?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "cars jadvali — ustunlar", ru: "таблица cars — столбцы" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              {COLUMNS.map((c, i) => {
    const on = active === c.key;
    const isSeen = seen.has(c.key);
    return <button key={c.key} className={`vcard ${!isSeen ? "tap-hint" : ""}`} onClick={() => tap(c.key)} style={{ boxShadow: on ? `inset 0 0 0 1.5px ${T.accent}, 0 8px 20px -6px rgba(${T.shadowBase},0.2)` : void 0, animationDelay: `${i * 0.16}s` }}>
                    <span className={`vbadge ${c.key === "id" ? "pk" : ""}`}>{tr2(c.type)}</span>
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "Tayyor sxema — SQL tilida", ru: "Готовая схема — на языке SQL" })}</p>
            <pre className="code-box fade-up delay-1" style={{ lineHeight: 1.9 }}>
              <Jx>{"CREATE TABLE"}</Jx>{" cars ("}{"\n"}
              {COLUMNS.map((c) => <React3.Fragment key={c.key}>
...
var CHAIN = ["Front", "Express", "pool.query", "PostgreSQL", { uz: "javob", ru: "ответ" }];
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [step, setStep] = useState3(storedAnswer ? CHAIN.length : -1);
  ...
  return <Stage eyebrow={{ uz: "2-qadam · ko'prik", ru: "Шаг 2 · мост" }} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `Zanjirni yuring — ${step + 1}/${CHAIN.length}`, ru: `Пройдите цепочку — ${step + 1}/${CHAIN.length}` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Express baza bilan <span className="italic" style={{ color: T.accent }}>qanday</span> gaplashadi?</>, ru: <>Как Express <span className="italic" style={{ color: T.accent }}>разговаривает</span> с базой?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <pre className="code-box fade-up delay-1" style={{ lineHeight: 1.9 }}>
              ...
              <Cm>{tr2({ uz: "// so'rov — ko'prik orqali bazaga:", ru: "// запрос — через мост в базу:" })}</Cm>{"\n"}
            ...
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={done} onClick={advance}>{step < 0 ? tr2({ uz: "▶ So'rovni yuborish", ru: "▶ Отправить запрос" }) : done ? tr2({ uz: "✓ Yetib bordi", ru: "✓ Дошёл" }) : tr2({ uz: "Keyingi qadam →", ru: "Следующий шаг →" })}</button>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "So'rovning yo'li", ru: "Путь запроса" })}</p>
            <div className="chain fade-up delay-1">
              {CHAIN.map((c, i) => {
    const lit = step >= i;
    return <React3.Fragment key={i}>
                    <div className={`chain-node ${lit ? "lit" : ""}`} style={{ background: lit ? T.accent : T.paper, color: lit ? "#fff" : T.ink3, boxShadow: lit ? `0 6px 16px -5px rgba(255,79,40,0.45)` : `0 4px 12px -6px rgba(${T.shadowBase},0.16)` }}>{tr2(c)}</div>
                    {i < CHAIN.length - 1 && <span className={`chain-arr ${step > i ? "flow" : ""}`} style={{ color: step > i ? T.accent : T.ink3 }}>→</span>}
                  </React3.Fragment>;
  })}
            </div>
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Mana ko'prik! Front so'raydi → Express qabul qiladi → <span className="mono">pool.query</span> SQL'ni PostgreSQL'ga olib boradi → javob qaytadi. <span className="mono">pool.query</span> bo'lmasa, server baza bilan gaplasha olmaydi.</>, ru: <>Вот мост! Фронт просит → Express принимает → <span className="mono">pool.query</span> несёт SQL в PostgreSQL → ответ возвращается. Без <span className="mono">pool.query</span> сервер не может говорить с базой.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Zanjirning keyingi bo'g'inini bosing — so'rov Front'dan PostgreSQL'gacha boradi.", ru: "💡 Нажмите следующее звено — запрос идёт от Front до PostgreSQL." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
...
var Screen5 = ({ screen, answers, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={{ uz: "3-qadam · REJISSYOR", ru: "Шаг 3 · РЕЖИССЁР" }} screen={screen} scrollSignal={picked != null} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Aniq promptni tanlang", ru: "Выберите точный промпт" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>AI'ga qaysi buyruq <span className="italic" style={{ color: T.accent }}>to'g'ri kod</span> yozdiradi?</>, ru: <>Какая команда заставит AI написать <span className="italic" style={{ color: T.accent }}>правильный код</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Uch prompt — birini tanlang", ru: "Три промпта — выберите один" })}</p>
            <div className={`prompts fade-up delay-1 ${done ? "is-done" : ""}`}>
              {PROMPTS.map((p) => {
    const on = picked === p.id;
    let cls = "hook-option";
    if (on) cls += p.precise ? " on" : "";
    if (done && !on) cls += " is-dim";
    if (done && on) cls += " is-set";
    return <button key={p.id} className={cls} disabled={done} onClick={() => choose(p.id)} style={on && !p.precise ? { boxShadow: `inset 0 0 0 1.5px ${T.danger}`, background: T.dangerSoft, color: T.danger } : void 0}>
                    <span className="you-badge">{tr2({ uz: "Siz", ru: "Вы" })}</span>
                    <span style={{ flex: 1, textAlign: "left" }}>{tr2(p.text)}</span>
                  </button>;
  })}
            </div>
    ...
    /* F-0916-01 Q4: ikki yashil izoh bitta qutida (147 (e) — takror izoh yo'q); natija-jumlasi o'ng ustundan shu yerga ko'chdi, matni o'zgarmagan */
            ...
            {cur && <div className={cur.precise ? "frame-success fade-step note-s5" : "frame-warn fade-step"}><p className="body" style={{ margin: 0, color: T.ink }}>{tr2(cur.why)}</p>{done && <p className="body" style={{ margin: "8px 0 0", color: T.ink }}>{tr2({ uz: <>Aniq buyruq — to'g'ri kod! <span className="mono">SELECT * FROM cars</span> barcha qatorni oldi, <span className="mono">res.json</span> uni frontga JSON qilib qaytardi.</>, ru: <>Точная команда — правильный код! <span className="mono">SELECT * FROM cars</span> взял все строки, <span className="mono">res.json</span> вернул их фронту в виде JSON.</> })}</p>}</div>}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Natija — Postman bilan sinab ko'ring", ru: "Результат — проверьте через Postman" })}</p>
            <div className="pm pm-s5">
              <div className="pm-bar"><MBadge m="GET" /><span className="pm-url">localhost:3000/api/cars</span><span className="pm-send-static">Send</span></div>
              <div className="pm-body">
                {done ? <><div className="ai-code fade-step" style={{ marginBottom: 8 }}><div className="ai-line ok" style={{ cursor: "default", whiteSpace: "pre-wrap" }}>{"app.get('/api/cars', async (req, res) => {\n  const result = await pool.query('SELECT * FROM cars');\n  res.json(result.rows);\n});"}</div></div><Resp status={200} text="OK" json={'[\n  { "id": 1, "nom": "Cobalt", "narx": 280000, "yil": 2022 },\n  { "id": 2, "nom": "Malibu", "narx": 520000, "yil": 2023 },\n  { "id": 3, "nom": "Kia K5", "narx": 610000, "yil": 2023 }\n]'} /></> : <p style={{ color: T.ink3, fontStyle: "italic", margin: 0, fontFamily: "Georgia, serif", fontSize: 13 }}>{tr2({ uz: "Aniq promptni tanlang — AI to'g'ri endpoint yozadi, GET javobi shu yerda ko'rinadi…", ru: "Выберите точный промпт — AI напишет правильный endpoint, ответ GET появится здесь…" })}</p>}
              </div>
            </div>
          </Col>
        </div>
...
var Screen5b = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Barcha mashinalarni <span className="italic" style={{ color: T.accent }}>qaytarish</span> uchun qaysi juftlik?</>, ru: <>Какая пара нужна, чтобы <span className="italic" style={{ color: T.accent }}>вернуть</span> все машины?</> })}</h2></>}
  options={["POST /api/cars → INSERT INTO cars", "DELETE /api/cars → SELECT * FROM cars", "GET /api/cars → DELETE FROM cars", "GET /api/cars → SELECT * FROM cars"]}
  correctIdx={3}
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [rows, setRows] = useState3(storedAnswer ? [...CARS, POOL_CARS[0]] : CARS);
  const [last, setLast] = useState3(null);
  const added = rows.length - CARS.length;
  const done = added >= 1;
  const remaining = POOL_CARS.filter((p) => !rows.some((r) => r.id === p.id));
  const addOne = (c) => {
    setRows((prev) => prev.some((x) => x.id === c.id) ? prev : [...prev, c]);
    setLast(c.id);
  };
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  ...
  return <Stage eyebrow={{ uz: "Create · qo'shish", ru: "Create · добавление" }} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Bitta mashina POST qiling", ru: "Отправьте POST с одной машиной" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Yangi mashina bazaga <span className="italic" style={{ color: T.accent }}>qanday</span> qo'shiladi?</>, ru: <>Как новая машина <span className="italic" style={{ color: T.accent }}>добавляется</span> в базу?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "Postman — POST so'rovi", ru: "Postman — запрос POST" })}</p>
            <div className="pm">
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "PostgreSQL — cars jadvali", ru: "PostgreSQL — таблица cars" })}</p>
            <DbTable rows={rows} flashId={last} />
            {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Yangi qator paydo bo'ldi! Server <span className="mono">201 Created</span> qaytardi. <span className="mono">$1, $2, $3</span> body'dagi qiymatlar bilan to'ldirildi va bazaga yozildi.</>, ru: <>Появилась новая строка! Сервер вернул <span className="mono">201 Created</span>. <span className="mono">$1, $2, $3</span> заполнились значениями из body и записались в базу.</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [rows, setRows] = useState3(CARS);
  const [didUpd, setDidUpd] = useState3(!!storedAnswer);
  const [didDel, setDidDel] = useState3(!!storedAnswer);
  const [flash, setFlash] = useState3(null);
  const [dim, setDim] = useState3(null);
  const done = didUpd && didDel;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, (didUpd ? 1 : 0) + (didDel ? 1 : 0));
  const lower = (id) => {
    setRows((prev) => prev.map((r) => r.id === id ? { ...r, narx: Math.max(1e5, r.narx - 5e4) } : r));
    setDidUpd(true);
    setFlash(id);
  ...
  return <Stage eyebrow="Update · Delete" screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : didUpd ? { uz: "Endi bittasini o'chiring", ru: "Теперь удалите одну" } : { uz: "Narxni o'zgartiring", ru: "Измените цену" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Bitta mashinani o'zgartirish — <span className="italic" style={{ color: T.accent }}>qaysi birini</span> server qayerdan biladi?</>, ru: <>Изменить одну машину — откуда сервер знает, <span className="italic" style={{ color: T.accent }}>какую именно</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <pre className="code-box fade-up delay-1" style={{ lineHeight: 1.85, padding: "11px 14px" }}>
              <Cm>{tr2({ uz: "// PUT — narxni yangilash", ru: "// PUT — обновить цену" })}</Cm>{"\n"}
              {"app.put("}<St>'/api/cars/:id'</St>{", ...);"}{"\n"}
              {"  UPDATE cars SET narx=$1 "}<At>WHERE id=$2</At>{"\n\n"}
              <Cm>{tr2({ uz: "// DELETE — o'chirish", ru: "// DELETE — удалить" })}</Cm>{"\n"}
              {"app.delete("}<St>'/api/cars/:id'</St>{", ...);"}{"\n"}
              {"  DELETE FROM cars "}<At>WHERE id=$1</At>
            </pre>
            <div className="hint"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr2({ uz: <><span className="mono" style={{ color: T.ink }}>:id</span> manzilda keladi (<span className="mono">req.params.id</span>), <span className="mono">WHERE id = $1</span> bazada aynan o'sha qatorni topadi.</>, ru: <><span className="mono" style={{ color: T.ink }}>:id</span> приходит в адресе (<span className="mono">req.params.id</span>), а <span className="mono">WHERE id = $1</span> находит в базе именно ту строку.</> })}</p></div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "PostgreSQL — cars jadvali", ru: "PostgreSQL — таблица cars" })}</p>
            <div className="db">
              <div className="db-cap">🗄️ <b>cars</b> <span>· {rows.length} {tr2({ uz: "qator", ru: "строк" })}</span></div>
              <div className="db-row db-head"><span>id</span><span>nom</span><span>narx</span><span>{tr2({ uz: "amal", ru: "действие" })}</span></div>
              {rows.map((r) => <div key={r.id} className={`db-row el-in ${flash === r.id ? "flash" : ""}`} style={{ opacity: dim === r.id ? 0.3 : 1 }}>
                  <span>{r.id}</span><span>{r.nom}</span><span>{sp(r.narx)}</span>
                  <span style={{ display: "flex", gap: 5 }}>
                    <button className="db-btn" style={{ color: T.amber }} onClick={() => lower(r.id)} title={tr2({ uz: "PUT — narxni tushir", ru: "PUT — снизить цену" })}>PUT</button>
                    <button className="db-btn" style={{ color: T.danger }} onClick={() => del(r.id)} title={tr2({ uz: "DELETE — o'chir", ru: "DELETE — удалить" })}>DEL</button>
                  </span>
                </div>)}
            </div>
            <div className="fade-up delay-2" style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              <span className="tagpill" style={{ color: didUpd ? T.success : T.ink3 }}>{didUpd ? "✓" : "○"} {tr2({ uz: "PUT — yangiladim", ru: "PUT — обновлено" })}</span>
              <span className="tagpill" style={{ color: didDel ? T.success : T.ink3 }}>{didDel ? "✓" : "○"} {tr2({ uz: "DELETE — o'chirdim", ru: "DELETE — удалено" })}</span>
            </div>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 Ikki amal kerak: narxni tushiring (UPDATE) va bitta qatorni o'chiring (DELETE).", ru: "💡 Нужны два действия: снизить цену (UPDATE) и удалить одну строку (DELETE)." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen8 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Bazaga <span className="italic" style={{ color: T.accent }}>yangi mashina qo'shish</span> uchun?</>, ru: <>Что нужно, чтобы <span className="italic" style={{ color: T.accent }}>добавить в базу новую машину</span>?</> })}</h2></>}
  options={["GET → SELECT", "PUT → UPDATE", "POST → INSERT", "DELETE → DELETE"]}
  correctIdx={2}
...
var Screen9 = ({ screen, answers, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const label = phase === 0 ? tr2({ uz: "1) POST yuborish", ru: "1) Отправить POST" }) : phase === 1 ? tr2({ uz: "2) Serverni o'chirib-yoqish", ru: "2) Перезапустить сервер" }) : phase === 2 ? tr2({ uz: "3) GET — tekshirish", ru: "3) GET — проверить" }) : tr2({ uz: "✓ Tasdiqlandi", ru: "✓ Подтверждено" });
  return <Stage eyebrow={{ uz: "4-qadam · NAZORATCHI", ru: "Шаг 4 · КОНТРОЛЁР" }} screen={screen} scrollSignal={phase > 0} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Uch qadamni bajaring", ru: "Выполните три шага" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Server o'chsa — ma'lumot <span className="italic" style={{ color: T.accent }}>saqlanib qoladimi</span>?</>, ru: <>Если сервер выключится — данные <span className="italic" style={{ color: T.accent }}>сохранятся</span>?</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            ...
            <div className={`code-box ${phase === 2 ? "srv-reboot" : ""}`} style={{ minHeight: 92, padding: "10px 13px" }}>
              ...
              {phase >= 1 && <TLine out={<span style={{ color: CODE.str }}>{tr2({ uz: "✓ POST /api/cars → 201 (Spark qo'shildi)", ru: "✓ POST /api/cars → 201 (Spark добавлен)" })}</span>} />}
              {phase >= 2 && <><TLine out={<span style={{ color: CODE.comment }}>{tr2({ uz: "^C  server to'xtadi…", ru: "^C  сервер остановился…" })}</span>} /><TLine cmd="node server.js" /><TLine out={<span style={{ color: CODE.comment }}>{tr2({ uz: "server qayta yondi :3000", ru: "сервер снова запущен :3000" })}</span>} /></>}
              {phase >= 3 && <TLine out={<span style={{ color: CODE.str }}>{tr2({ uz: "✓ GET /api/cars → 200 (4 qator — Spark joyida!)", ru: "✓ GET /api/cars → 200 (4 строки — Spark на месте!)" })}</span>} />}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "PostgreSQL — cars jadvali", ru: "PostgreSQL — таблица cars" })}</p>
            <DbTable rows={rows} flashId={phase === 1 ? POOL_CARS[0].id : null} />
            {phase >= 1 && phase < 3 && <p className="small fade-step" style={{ color: T.ink2, fontStyle: "italic", margin: 0 }}>{tr2({ uz: <>Ma'lumot serverning xotirasida emas — <b>PostgreSQL</b>'da diskda yotibdi.</>, ru: <>Данные не в памяти сервера — они лежат на диске в <b>PostgreSQL</b>.</> })}</p>}
            {done && <div className="takeaway fade-step"><div className="ta-bulb">💾</div><p className="ta-h">{tr2({ uz: "Ma'lumot saqlanib qoldi!", ru: "Данные сохранились!" })}</p><p className="ta-sub">{tr2({ uz: "Server o'chsa ham — baza yodida. Dars boshidagi muammo hal bo'ldi.", ru: "Даже если сервер выключится — база всё помнит. Проблема из начала урока решена." })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow="Debugging" screen={screen} scrollSignal={found || fixed} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : found ? { uz: "Endi tuzating", ru: "Теперь исправьте" } : { uz: "Xatoni toping", ru: "Найдите ошибку" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Server xato qaytardi — bu bizga <span className="italic" style={{ color: T.accent }}>nimani aytmoqchi</span>?</>, ru: <>Сервер вернул ошибку — что он <span className="italic" style={{ color: T.accent }}>хочет нам сказать</span>?</> })}</h2></div>
        <Mentor>{tr2({ uz: <>POST yuborganda server <span className="mono" style={{ color: T.danger }}>500</span> qaytardi: <i>column "price" does not exist</i>. Bunday xatolar — kod yozishning <b style={{ color: T.ink }}>tabiiy qismi</b>; hatto AI ham adashishi mumkin, shuning uchun biz doim tekshiramiz. Eng yaxshisi: xato matni nima noto'g'ri ekanini <b style={{ color: T.ink }}>o'zi aytib beradi</b> — uni diqqat bilan o'qing. Qaysi qatorda shu nom xato? Bosing.</>, ru: <>При POST сервер вернул <span className="mono" style={{ color: T.danger }}>500</span>: <i>column "price" does not exist</i>. Такие ошибки — <b style={{ color: T.ink }}>естественная часть</b> написания кода; даже AI может ошибиться, поэтому мы всегда проверяем. Лучшее в этом: текст ошибки <b style={{ color: T.ink }}>сам говорит</b>, что не так — прочитайте его внимательно. В какой строке это имя ошибочно? Нажмите.</> })}</Mentor>
        <Zoomable>
        <div className="split">
          <Col>
            <div className="ai-card fade-up delay-1">
              ...
              <div className="ai-code">
                <div className={`ai-line ${picked === "a" ? "ok" : ""} ${!found ? "scan" : ""}`} style={{ animationDelay: "0s" }} onClick={() => {
    if (found) return;
    if (achMiss) achMiss.miss(screen);
    setPicked("a");
  }}><Jx>{"const"}</Jx>{" { nom, narx, yil } = req.body;"}</div>
                {!fixed ? <div className={`ai-line ${found ? "bad" : ""} ${!found ? "scan" : ""}`} style={{ animationDelay: "0.5s" }} onClick={() => {
    if (!found) setPicked("bad");
  }}>{"pool.query('INSERT INTO cars (nom, "}<At>price</At>{", yil)...');"}</div> : <div className="ai-line ok fixed-line">{"pool.query('INSERT INTO cars (nom, "}<At>narx</At>{", yil)...');"}{"  "}<Cm>{"// ✓ narx"}</Cm></div>}
                <div className={`ai-line ${picked === "c" ? "ok" : ""} ${!found ? "scan" : ""}`} style={{ animationDelay: "1s" }} onClick={() => {
    if (found) return;
    if (achMiss) achMiss.miss(screen);
    setPicked("c");
  }}>{`res.json({ status: "qo'shildi" });`}</div>
              </div>
              {!found && <p className="ai-prompt">{tr2({ uz: "Qaysi qator bazadagi ustun nomiga mos emas? Bosing.", ru: "Какая строка не совпадает с именем столбца в базе? Нажмите." })}</p>}
              {found && !fixed && <button className="btn fade-step" style={{ alignSelf: "flex-start" }} onClick={() => setFixed(true)}>{tr2({ uz: "🔧 price → narx ga tuzatish", ru: "🔧 Исправить price → narx" })}</button>}
              {fixed && <p className="ai-prompt" style={{ color: T.success, fontStyle: "normal", fontWeight: 600 }}>{tr2({ uz: "✓ Tuzatildi — ustun nomi sxemaga mos: narx", ru: "✓ Исправлено — имя столбца совпадает со схемой: narx" })}</p>}
            </div>
            {!done && <AchRule screen={screen} />}
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Server javobi", ru: "Ответ сервера" })}</p>
            <div className="pm">
              <div className="pm-bar"><MBadge m="POST" /><span className="pm-url">localhost:3000/api/cars</span></div>
              <div className="pm-body">
                {fixed ? <Resp status={201} text="Created" json={`{ "status": "qo'shildi" }`} /> : <Resp status={500} text="Server Error" json={'{\n  "error": "column \\"price\\" does not exist"\n}'} />}
              </div>
            </div>
            {!found && (picked === "a" || picked === "c") ? <div className="frame-warn fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Bu qator to'g'ri. Yana qarang: xato <i>"price" does not exist"</i> deyapti — kodda qayerda <span className="mono">price</span> yozilgan?</>, ru: <>Эта строка правильная. Посмотрите ещё раз: ошибка говорит <i>"price" does not exist</i> — где в коде написано <span className="mono">price</span>?</> })}</p></div> : null}
            {found && !fixed && <div className="frame-warn fade-step"><p className="note-h" style={{ color: T.accent }}>{tr2({ uz: "✓ Topdingiz!", ru: "✓ Нашли!" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Kodda ustun <span className="mono">price</span> deb yozilgan, biroq sxemada u <span className="mono">narx</span> — shuning uchun baza topa olmadi. Bitta so'zni to'g'rilaymiz. Chapdagi tugma bilan tuzating →</>, ru: <>В коде столбец записан как <span className="mono">price</span>, но в схеме он <span className="mono">narx</span> — поэтому база его не нашла. Исправим одно слово. Нажмите кнопку слева →</> })}</p></div>}
          </Col>
        </div>
        </Zoomable>
      </div>
...
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  const [rows, setRows] = useState3(storedAnswer ? [...CARS, POOL_CARS[1]] : CARS);
  const [didC, setDidC] = useState3(!!storedAnswer);
  const [didR, setDidR] = useState3(!!storedAnswer);
  const [didU, setDidU] = useState3(!!storedAnswer);
  const [didD, setDidD] = useState3(!!storedAnswer);
  const [active, setActive] = useState3(storedAnswer ? "R" : null);
  const [flash, setFlash] = useState3(null);
  const [runs, setRuns] = useState3(0);
  const done = didC && didR && didU && didD;
  const { tip: _tip, rescue: _resc } = useStuckValve(done, (didC ? 1 : 0) + (didR ? 1 : 0) + (didU ? 1 : 0) + (didD ? 1 : 0));
  useEffect4(() => {
    if (done && storedAnswer === void 0) onAnswer(screen, { correct: true, picked: true });
  }, [done]);
  const run = (k) => {
    ...
    setRuns((n) => n + 1);
    if (k === "C") {
      const n = POOL_CARS.find((p) => !rows.some((r) => r.id === p.id));
      if (n) {
        setRows((prev) => [...prev, n]);
        setFlash(n.id);
      }
    ...
    if (k === "U") {
      setRows((prev) => prev.map((r, i) => i === 0 ? { ...r, narx: Math.max(1e5, r.narx - 4e4) } : r));
      setFlash(rows[0]?.id);
      setDidU(true);
    }
    if (k === "D") {
  ...
  return <Stage eyebrow={{ uz: "Amaliyot · to'liq zanjir", ru: "Практика · полная цепочка" }} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !_resc} label={done || _resc ? { uz: "Davom etish", ru: "Продолжить" } : { uz: `To'rt amalni sinang — ${(didC ? 1 : 0) + (didR ? 1 : 0) + (didU ? 1 : 0) + (didD ? 1 : 0)}/4`, ru: `Испытайте четыре действия — ${(didC ? 1 : 0) + (didR ? 1 : 0) + (didU ? 1 : 0) + (didD ? 1 : 0)}/4` }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(8px,1.2vw,12px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Endi o'zingiz — <span className="italic" style={{ color: T.accent }}>to'rt amalni</span> ham yuboring.</>, ru: <>Теперь сами — отправьте <span className="italic" style={{ color: T.accent }}>все четыре действия</span>.</> })}</h2></div>
        ...
        <div className="split">
          <Col>
            <p className="flow-label">{tr2({ uz: "So'rov yuborish", ru: "Отправка запроса" })}</p>
            <div className="fade-up delay-1" style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              {OPS.map((o, i) => {
    const did = { C: didC, R: didR, U: didU, D: didD }[o.key];
    return <button key={o.key} className={`vcard ${!did ? "tap-hint" : ""}`} onClick={() => run(o.key)} style={{ boxShadow: active === o.key ? `inset 0 0 0 1.5px ${T.accent}, 0 8px 20px -6px rgba(${T.shadowBase},0.2)` : void 0, animationDelay: `${i * 0.16}s` }}>
          ...
          <Col>
            <p className="flow-label">{tr2({ uz: "So'rovning yo'li", ru: "Путь запроса" })}</p>
            <div className="chain fade-up delay-1">
              {CHAIN.map((c, i) => <React3.Fragment key={i}>
                  {
    /* ✨ har so'rovda to'lqin zanjirdan o'tadi: key almashadi → animatsiya qayta o'ynaydi, kechikish bosqichma-bosqich */
  }
                  <div key={`${active || "off"}-${runs}`} className={`chain-node ${active ? "lit wave" : ""}`} style={{ background: active ? T.accent : T.paper, color: active ? "#fff" : T.ink3, animationDelay: `${i * 0.09}s` }}>{tr2(c)}</div>
        ...
        {_tip && !done && <p className="bhint fade-step">{tr2({ uz: "💡 C·R·U·D — to'rttasi ham: qo'shish, ko'rish, o'zgartirish, o'chirish.", ru: "💡 C·R·U·D — все четыре: добавить, показать, изменить, удалить." })}</p>}
        {_resc && !done && <p className="bhint calm fade-step">{tr2({ uz: "Qolganini keyinroq birga ko'rib chiqamiz — «Davom etish» ochiq.", ru: "Остальное разберём вместе позже — «Продолжить» открыто." })}</p>}
      </div>
    </Stage>;
};
var Screen12 = (props) => <QuestionScreen
  ...
  question={<><p className="eyebrow" style={{ color: T.accent }}>{tr2({ uz: "To'g'ri javobni tanlang", ru: "Выберите правильный ответ" })}</p><h2 className="title h-ask" style={{ marginTop: 8 }}>{tr2({ uz: <>Express ichida bazaga <span className="italic" style={{ color: T.accent }}>SQL yuborish</span> uchun?</>, ru: <>Что нужно в Express, чтобы <span className="italic" style={{ color: T.accent }}>отправить SQL</span> в базу?</> })}</h2></>}
  options={["pool.query('...')", "res.send(...)", "app.listen(3000, callback)", "console.log(...)"]}
  correctIdx={0}
...
/>;
var Screen13 = ({ screen, answers, onNext, onPrev }) => {
  return <Stage eyebrow={{ uz: "Qoida · xulosa", ru: "Правило · вывод" }} screen={screen} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Oxirgi qadam →", ru: "Последний шаг →" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr2({ uz: <>Mashinalar endi <span className="italic" style={{ color: T.accent }}>qayerda yashaydi</span> — kodda yoki bazada?</>, ru: <>Где теперь <span className="italic" style={{ color: T.accent }}>живут машины</span> — в коде или в базе?</> })}</h2></div>
        <RolePassport answers={answers} />
        ...
        <Split>
          <Col>
            ...
            <div className="db">
              <div className="db-row db-head" style={{ gridTemplateColumns: "1fr 1fr 1.4fr" }}><span>{tr2({ uz: "amal", ru: "действие" })}</span><span>method</span><span>SQL</span></div>
              {OPS.map((o) => <div key={o.key} className="db-row" style={{ gridTemplateColumns: "1fr 1fr 1.4fr" }}>
                  <span>{tr2(o.amal)}</span><span style={{ color: o.color, fontWeight: 700 }}>{o.method}</span><span>{o.sql}</span>
                </div>)}
            </div>
          </Col>
          <Col>
            <p className="flow-label">{tr2({ uz: "Uch rol — har safar shunday", ru: "Три роли — так каждый раз" })}</p>
            <div className="roadmap">
              <div className="step-card"><span className="step-num" style={{ color: T.blue }}>🏗️ 01</span><span className="step-body"><span className="step-text">{tr2({ uz: "ME'MOR", ru: "АРХИТЕКТОР" })}</span><span className="step-tag">{tr2({ uz: "sxemani siz loyihalaysiz", ru: "схему проектируете вы" })}</span></span></div>
              <div className="step-card"><span className="step-num" style={{ color: T.amber }}>🎬 02</span><span className="step-body"><span className="step-text">{tr2({ uz: "REJISSYOR", ru: "РЕЖИССЁР" })}</span><span className="step-tag">{tr2({ uz: "AI'ga aniq prompt", ru: "точный промпт для AI" })}</span></span></div>
              <div className="step-card"><span className="step-num" style={{ color: T.success }}>🔍 03</span><span className="step-body"><span className="step-text">{tr2({ uz: "NAZORATCHI", ru: "КОНТРОЛЁР" })}</span><span className="step-tag">{tr2({ uz: "Postman bilan test", ru: "проверка через Postman" })}</span></span></div>
            </div>
            <div className="frame-success"><p className="body" style={{ margin: 0, color: T.ink }}>{tr2({ uz: <>Ko'prik — <span className="mono">pool.query</span>. Aniq qator — <span className="mono">:id</span> + <span className="mono">WHERE id=$1</span>. Xavfsizlik — qiymatlar <span className="mono">$1, $2</span> o'rinlariga qo'yiladi.</>, ru: <>Мост — <span className="mono">pool.query</span>. Точная строка — <span className="mono">:id</span> + <span className="mono">WHERE id=$1</span>. Безопасность — значения подставляются в места <span className="mono">$1, $2</span>.</> })}</p></div>
          </Col>
        </Split>
        </Zoomable>
      </div>
...
function ScreenLivePractice({ title, task, checklist, screen, storedAnswer, onAnswer, onNext, onPrev, live }) {
  ...
  return <Stage eyebrow={{ uz: "Amaliyot · VS Code", ru: "Практика · VS Code" }} screen={screen} scrollSignal={done} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done && !isMentor} label={done || isMentor ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Avval bajaring", ru: "Сначала выполните" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(12px,2vw,18px)" }}>
...
var ScreenFinalPractice = (props) => <ScreenLivePractice
  {...props}
...
var ACH_TRIGGERS = { s4: "blueprint", s5: "director", s10: "catcher", spf: "finisher" };
...
var Q_LABELS = { 4: { uz: "1 — Sxema (id)", ru: "1 — Схема (id)" }, 6: "2 — READ (GET)", 9: "3 — Create (POST)", 13: { uz: "4 — Ko'prik (pool)", ru: "4 — Мост (pool)" } };
var INLINE_KEYS = { s4: 1, s5b: 3, s8: 2, s12: 0, practice: -1 };
var QUIZ_MS = 15e3;
...
var QZ_BG_SHAPES = [
  ...
  { ch: ":id", l: 70, t: 90, s: 26, c: "rgba(203,173,255,0.12)", d: 18, dl: 2 }
];
var QUIZ_BANK = [
    ...
    return <div key={i} className={`qz-tile rv ${win ? "win" : "lose"} ${pickedThis ? "picked" : ""}`} style={{ background: QUIZ_COLORS[i] }}>
      ...
      {phase === "done" && <div className="qz-view fade-step">
          ...
          {solo ? <div className="qz-solo-res">
              ...
              <p className="qz-sub">{tr2({ uz: "ball", ru: "баллов" })} · {soloScore.ok}/{QUIZ_BANK.length} {tr2({ uz: "to'g'ri", ru: "правильно" })}{soloScore.maxStreak >= 2 ? ` · ${tr2({ uz: "eng uzun streak", ru: "самая длинная серия" })} 🔥x${soloScore.maxStreak}` : ""}</p>
  ...
  return <Stage eyebrow={{ uz: "Natijalar", ru: "Результаты" }} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(14px,2.2vw,20px)" }}>
        ...
        {!isLive ? <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 16, alignItems: "center" }}>
            ...
            <div className="frame-wait" style={{ maxWidth: 480 }}><p className="body" style={{ margin: 0 }}>{tr2({ uz: "Siz mustaqil rejimdasiz. Jonli darsda bu yerda butun guruh reytingi — 🥇🥈🥉 podium chiqadi.", ru: "Вы в самостоятельном режиме. В живом уроке здесь появится рейтинг всей группы — подиум 🥇🥈🥉." })}</p></div>
    ...
    return <div key={rank} className={`pod-col pod-${rank + 1} ${b && live.playerId === b.id ? "me" : ""}`}>
            ...
            {myIdx >= 0 && <p className="pod-my fade-up">{tr2({ uz: "Siz —", ru: "Вы —" })} <b>{myIdx + 1}{tr2({ uz: "-o'rin", ru: "-е место" })}</b> ({board[myIdx].okCount}/{totalQ} {tr2({ uz: "to'g'ri", ru: "правильно" })})</p>}
...
var Screen15 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  return <Stage eyebrow={{ uz: "Tayyor", ru: "Готово" }} screen={screen} navContent={<><NavBack onPrev={onPrev} /><button className="btn-ghost" onClick={onReset} style={{ padding: "clamp(11px,1.6vw,13px) clamp(16px,2.2vw,22px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Qaytadan", ru: "Заново" })}</button><button className="btn-white-accent" onClick={onFinish} style={{ marginLeft: "auto", padding: "clamp(11px,1.6vw,13px) clamp(22px,2.6vw,30px)", fontSize: "clamp(13px,1.5vw,15px)" }}>{tr2({ uz: "Yakunlash ✓", ru: "Завершить ✓" })}</button></>}>
      <div className="screen">
        <div className="hero"><div className="hero-l"><span className="done-chip fade-up"><span className="tick">✓</span> {tr2({ uz: "Birinchi backend'ingiz tayyor", ru: "Ваш первый backend готов" })}</span><h2 className="title h-title fade-up d1">{tr2({ uz: <>AvtoIjara backend'ini <span className="italic" style={{ color: T.accent }}>o'zingiz qurdingiz</span>.</>, ru: <>Вы <span className="italic" style={{ color: T.accent }}>сами собрали</span> backend AvtoIjara.</> })}</h2>{
...
function BackendCrudPracticeLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  return <LangContext.Provider value={lang}>
      <style>{`
        ...
        @media (max-width: 560px) { .prompts.is-done { grid-template-columns: 1fr; } }
        .note-s5 { padding: 9px 14px 8px; } .note-s5 p { font-size: 13px; line-height: 1.4; } /* F-0916-01 Q4: rol-pasporti va izoh ixcham — ru chap ustun 20px ortiq edi */
        .pm-s5 .json { max-height: 72px; overflow-y: auto; scrollbar-width: thin; }
        .pm-s5 .ai-line { font-size: 12px; padding: 5px 8px; }
        ...
        .rp-stamp { display: inline-flex; align-items: center; gap: 8px; padding: 4px 10px; border-radius: 10px; background: ${T.bg}; box-shadow: inset 0 0 0 1px rgba(167,166,162,0.28); opacity: 0.55; transition: opacity 0.25s, background 0.25s, box-shadow 0.25s; }
      ...
      <LiveGateCtx.Provider value={{ locked, live }}>
        <div className="lesson-root">
          {live.mode === "choosing" ? <LiveGate live={live} title={{ uz: "Backend praktikasi", ru: "Бэкенд-практика" }} /> : <>
...
export {
  BackendCrudPracticeLesson as default
};
