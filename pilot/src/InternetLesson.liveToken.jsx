...
// ============================================================
//  PILOT (2026-08-27) — LMS `liveToken` prop'i. FAQAT TASHXIS: token JSX'ga
//  yetib kelganini ko'rsatadi. Payload imzosiz o'qiladi (ko'rsatish uchun),
//  HECH NARSAGA ISHONILMAYDI, hech qayerga yuborilmaydi, console'ga yozilmaydi.
//  Haqiqiy tekshiruv — backend (HS256, iss/aud/kid, jti) — 2-sentabrdan keyin.
// ============================================================
const lmsTokenPeek = (token) => {
  if (typeof token !== 'string' || !token) return { state: 'none' };
  const parts = token.split('.');
  if (parts.length !== 3) return { state: 'bad', why: 'segment' };
  const b64 = (u) => { try { const t = u.replace(/-/g, '+').replace(/_/g, '/'); const bin = atob(t + '==='.slice((t.length + 3) % 4)); return decodeURIComponent(Array.from(bin, c => '%' + c.charCodeAt(0).toString(16).padStart(2, '0')).join('')); } catch { return null; } };
  try {
  ...
  } catch { return { state: 'bad', why: 'json' }; }
};
const _lmsBadgeS = { position: 'fixed', bottom: 10, left: 10, zIndex: 9997, background: '#1F1D1B', color: '#fff', borderRadius: 10, padding: '7px 12px', fontSize: 12, fontFamily: 'monospace', lineHeight: 1.5, maxWidth: 'min(92vw, 520px)', boxShadow: '0 4px 16px rgba(0,0,0,0.3)', opacity: 0.9 };
function LmsTokenBadge({ info }) {
  if (!info) return null;
  const fmtExp = (e) => { if (typeof e !== 'number') return '—'; const d = new Date(e * 1000); return d.toLocaleString('ru-RU', { hour12: false }); };
  ...
  return (
    <div style={{ ..._lmsBadgeS, background: '#1F5E3E' }}>
...
export default function HtmlLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  // PILOT: liveToken avval null, keyin keladi — prop o'zgarishiga reaksiya (LMS §4 talabi)
  const [lmsTok, setLmsTok] = useState(() => lmsTokenPeek(liveToken));
  useEffect(() => { setLmsTok(lmsTokenPeek(liveToken)); }, [liveToken]);
