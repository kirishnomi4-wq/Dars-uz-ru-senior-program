...
//  Manba:  pilot/src/InternetLesson.liveToken.jsx
//  Kompilyator: yo'q (dars uni import qilmaydi)
//  Qayta yig'ish:  node scripts/build-lms.mjs pilot/src/InternetLesson.liveToken.jsx
//  Tahrir MANBAGA kiritiladi, keyin shu buyruq qayta yuriladi.
// ============================================================
// pilot/src/InternetLesson.liveToken.jsx
...
var lmsTokenPeek = (token) => {
  if (typeof token !== "string" || !token) return { state: "none" };
  const parts = token.split(".");
  if (parts.length !== 3) return { state: "bad", why: "segment" };
  const b64 = (u) => {
    try {
      const t = u.replace(/-/g, "+").replace(/_/g, "/");
  ...
  try {
    const h = JSON.parse(b64(parts[0]) || "null"), c = JSON.parse(b64(parts[1]) || "null");
    if (!h || !c) return { state: "bad", why: "json" };
    return { state: "ok", alg: h.alg, typ: h.typ, kid: h.kid, role: c.role, sub: c.sub, name: c.name, gid: c.gid, crm: c.crm_id, iss: c.iss, aud: c.aud, exp: c.exp, jti: c.jti };
  } catch {
    return { state: "bad", why: "json" };
  }
};
var _lmsBadgeS = { position: "fixed", bottom: 10, left: 10, zIndex: 9997, background: "#1F1D1B", color: "#fff", borderRadius: 10, padding: "7px 12px", fontSize: 12, fontFamily: "monospace", lineHeight: 1.5, maxWidth: "min(92vw, 520px)", boxShadow: "0 4px 16px rgba(0,0,0,0.3)", opacity: 0.9 };
function LmsTokenBadge({ info }) {
  if (!info) return null;
  const fmtExp = (e) => {
    if (typeof e !== "number") return "—";
    const d = new Date(e * 1e3);
    return d.toLocaleString("ru-RU", { hour12: false });
  };
  if (info.state === "none") return <div style={_lmsBadgeS}>{tr({ uz: "LMS-token: kelmadi (oddiy rejim)", ru: "LMS-токен: не получен (обычный режим)" })}</div>;
  if (info.state === "bad") return <div style={{ ..._lmsBadgeS, background: "#8B1E1E" }}>{tr({ uz: "LMS-token: keldi, lekin format xato", ru: "LMS-токен: получен, но формат неверный" })} ({info.why})</div>;
  return <div style={{ ..._lmsBadgeS, background: "#1F5E3E" }}>
      <b>{tr({ uz: "LMS-token keldi ✅", ru: "LMS-токен получен ✅" })}</b>{" "}<span style={{ opacity: 0.75 }}>({tr({ uz: "imzo tekshirilmagan — faqat namoyish", ru: "подпись не проверена — только показ" })})</span><br />
      role: <b>{String(info.role ?? "—")}</b> · sub: <b>{String(info.sub ?? "—")}</b> · name: <b>{String(info.name ?? "—")}</b>
      {info.gid != null && <> · gid: <b>{String(info.gid)}</b></>}{info.crm != null && <> · crm_id: <b>{String(info.crm)}</b></>}<br />
      alg: {String(info.alg ?? "—")} · typ: {String(info.typ ?? "—")} · kid: {String(info.kid ?? "—")} · iss: {String(info.iss ?? "—")} · aud: {String(info.aud ?? "—")}<br />
      exp: {fmtExp(info.exp)} · jti: {info.jti ? String(info.jti).slice(0, 8) + "…" : "—"}
    </div>;
}
...
var ouz = (o) => o && typeof o === "object" && !React.isValidElement(o) ? o.uz ?? o.ru ?? "" : o;
    ...
    return <button key={i} className={cls} disabled={solved || isMentorLive} onClick={() => pick(i)} style={{ padding: "clamp(13px,1.9vw,17px) clamp(15px,2.2vw,20px)", fontSize: "clamp(15px,1.85vw,17px)", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <FeedbackBlock show={isMentorLive ? mReveal : picked !== null} isCorrect={isMentorLive ? true : solved && !wrongLocked} neutral={waiting}>
          <p className="small mono" style={{ margin: "0 0 6px", fontWeight: 600, color: waiting ? T.blue : isMentorLive || solved && !wrongLocked ? T.success : T.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>
            {isMentorLive ? fmtCode(`${tr({ uz: "✓ To'g'ri javob", ru: "✓ Правильный ответ" })}: ${String.fromCharCode(65 + correctIdx)} — ${tr(options[correctIdx])}`) : waiting ? tr({ uz: "📨 Javobingiz qabul qilindi", ru: "📨 Ваш ответ принят" }) : wrongLocked ? fmtCode(`${tr({ uz: "To'g'ri javob", ru: "Правильный ответ" })}: ${String.fromCharCode(65 + correctIdx)} — ${tr(options[correctIdx])}`) : solved ? tr({ uz: "To'g'ri", ru: "Правильно" }) : tr({ uz: "Qaytadan urinib ko'ring", ru: "Попробуйте ещё раз" })}
          </p>
          <p className="body" style={{ margin: 0 }}>
            {fmtCode(tr(isMentorLive ? explainCorrect : waiting ? { uz: "Javobingiz yozib olindi. To'g'ri yoki xato ekani mentor «Natijani ochish»ni bosganda hammada birdan ko'rinadi.", ru: "Ваш ответ записан. Правильный он или нет — станет видно всем сразу, когда ментор нажмёт «Открыть результат»." } : wrongLocked ? explainWrong[picked] ?? explainWrong.default : solved ? explainCorrect : explainWrong[picked] ?? explainWrong.default))}
          </p>
          {
    /* Xato qilgan o'quvchi (yoki mustaqil o'quvchi) mavzuni qisqa kartalarda qayta ko'radi.
       Jonli darsda — javob sirini saqlash uchun faqat reveal'dan keyin chiqadi. */
  }
          {hasRecap && !isMentorLive && firstCorrectRef.current === false && (!oneShot || revealed) && <button className="rc-open-mini" onClick={() => setRecapOpen(true)}>{tr({ uz: "📖 Qisqa takrorlash — mavzuni yana bir ko'rish", ru: "📖 Краткое повторение — ещё раз взглянуть на тему" })}</button>}
        </FeedbackBlock>
...
function MentorTestStats({ live, screenIdx, options, correctIdx, reveal, onReveal, onOpenRecap }) {
  ...
  return <div className="mstats fade-up">
    ...
    /* Katta hisoblagichlar. Reveal'gacha ✅/❌ soni ham, variant taqsimoti ham YASHIRIN —
      ...
      {!reveal && answered > 0 && <p className="mstats-hidden">{tr({ uz: "🙈 Kim nimani tanlagani va ✅/❌ soni yashirin — «Natijani ochish» bosilganda sizda ham, o'quvchilar ekranida ham birdan ochiladi.", ru: "🙈 Кто что выбрал и число ✅/❌ скрыто — при нажатии «Открыть результат» откроется сразу и у вас, и на экранах учеников." })}</p>}
    ...
    return <div key={i} className={`mstats-row ${reveal && !isC ? "dimmed" : ""}`}>
              ...
              <span className="mono mstats-count" style={isC ? { color: T.success, fontWeight: 800 } : void 0}>{n > 0 ? `${n} ${tr({ uz: "o'quvchi", ru: "учеников" })} · ${pct}%` : "—"}</span>
            </div>;
  })}
      </div>}
    ...
    return <div className={`mstats-verdict ${level}`}>
            {level === "need" && <>
              <p className="mstats-verdict-t">{tr({ uz: <>⚠️ Faqat <b>{pct}%</b> to'g'ri — bu mavzu sinfga tushunarsiz qolgan. Davom etishdan oldin qisqa takrorlash tavsiya etiladi.</>, ru: <>⚠️ Только <b>{pct}%</b> верно — эта тема осталась классу непонятной. Перед продолжением стоит кратко повторить.</> })}</p>
...
var Screen0 = ({ screen, storedAnswer, onAnswer, onNext }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Kirish", ru: "Вступление" })} screen={screen} audioState={audio} navContent={<NavNext optionalLive disabled={picked === null} label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} />}>
...
var Screen1 = ({ screen, onNext, onPrev }) => {
  ...
  const AnimBlock = <Zoomable>
    <div className="jr fade-up delay-1">
      <div className="jr-ring">
        ...
        <div className="jr-hub"><span className="jr-hub-ic">⏱️</span><span className="jr-hub-t">{tr({ uz: "≈ 1 soniya", ru: "≈ 1 секунда" })}</span></div>
        {JR_STATIONS.map((s) => {
    ...
    return <div key={s.k} className={`jr-st ${on ? "on" : ""} ${delivered ? "delivered" : ""}`} style={jrPos(s.a)}>
              ...
              <span className="jr-l">{tr(s.l)}</span>
            </div>;
  })}
  ...
  const StepsBlock = <Col>
      <p className="flow-label">{tr({ uz: "Dars rejasi — 5 qadam", ru: "План урока — 5 шагов" })}</p>
  ...
  return <Stage eyebrow={tr({ uz: "Reja", ru: "План" })} screen={screen} audioState={audio} mentorStatic navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Boshlaymiz →", ru: "Начнём →" }} onClick={onNext} /></>}>
      <div className="screen">
        ...
        {!isNarrow ? <Split>{AnimBlock}{StepsBlock}</Split> : !showSteps ? <div ref={animScrollRef} className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>
            {AnimBlock}
            <button className="btn-soft" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(true)}>{tr({ uz: "📋 Dars rejasini ko'rish", ru: "📋 Посмотреть план урока" })}</button>
          </div> : <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,16px)" }}>
            <button className="btn-soft" style={{ alignSelf: "flex-start" }} onClick={() => setShowSteps(false)}>{tr({ uz: "↩ Animatsiyani ko'rish", ru: "↩ Посмотреть анимацию" })}</button>
            {StepsBlock}
          </div>}
      </div>
    </Stage>;
...
var Screen2 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Internet", ru: "Интернет" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Ulanishlarni ko'rsating", ru: "Покажите связи" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Internet aslida <span className="italic" style={{ color: T.accent }}>nima</span>?</>, ru: <>Что такое интернет <span className="italic" style={{ color: T.accent }}>на самом деле</span>?</> })}</h2></div>
...
var Screen3 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Brauzer", ru: "Браузер" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Brauzer tanlang", ru: "Выберите браузер" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Saytlarni qaysi <span className="italic" style={{ color: T.accent }}>dastur</span> ochib beradi?</>, ru: <>Какая <span className="italic" style={{ color: T.accent }}>программа</span> открывает сайты?</> })}</h2></div>
        ...
        <div className="split">
          <div className="col">
            <p className="eyebrow fade-up delay-2" style={{ color: T.ink2, margin: 0 }}>{tr({ uz: "Brauzeringizni tanlang", ru: "Выберите свой браузер" })}</p>
    ...
    return <button key={b.k} onClick={() => pick(b.k)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 7, padding: "12px 16px", minWidth: 86, cursor: "pointer", background: T.paper, border: `2px solid ${on ? b.color : "rgba(58,53,48,0.14)"}`, borderRadius: 14, boxShadow: on ? `0 9px 22px -8px ${b.color}88` : "0 2px 8px -4px rgba(58,53,48,0.16)", transform: on ? "translateY(-2px)" : "none", transition: "transform .2s ease, box-shadow .2s ease, border-color .2s ease" }}>
            ...
            <div className="frame-soft fade-up delay-3"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: <>Qaysi brauzer bo'lishidan qat'i nazar — vazifasi bir xil: <b>manzilni olib, saytni keltirib, ekranga chizish</b>.</>, ru: <>Каким бы ни был браузер — задача одна: <b>взять адрес, принести сайт и нарисовать его на экране</b>.</> })}</p></div>
          </div>
          <div className="col">            <div className="flow-label" style={{ display: "flex", alignItems: "center", gap: 8 }}>{done ? <><BrowserLogo k={br} size={18} /><span>{cur.l} {tr({ uz: "youtube.com'ni ochdi", ru: "открыл youtube.com" })}</span></> : tr({ uz: "Sayt shu yerda ochiladi", ru: "Сайт откроется здесь" })}</div>
            <Preview title={done ? `${cur.l} — youtube.com` : "brauzer"} minH={150}>
              {done ? <SiteMock site={{ mock: "youtube", ic: "▶️", name: "YouTube", bar: "#FF0000" }} /> : <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center" }}>{tr({ uz: "Brauzer tanlang — sayt shu yerda ochiladi", ru: "Выберите браузер — сайт откроется здесь" })}</p>}
            </Preview>
          </div>
        </div>
      </div>
...
var Screen5 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Domen", ru: "Домен" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Domenni tanlang", ru: "Выберите домен" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        ...
        <div className="split">
          ...
          <div className="col">
            ...
            {done ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              </div> : <div className="frame-dash"><p className="small" style={{ color: T.ink3, textAlign: "center", fontStyle: "italic", margin: 0 }}>{tr({ uz: "Domenni tanlang — qismlarga ajraladi", ru: "Выберите домен — он разложится на части" })}</p></div>}
          </div>
        </div>
        </Zoomable>
      </div>
...
var Screen6 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "IP manzil", ru: "IP-адрес" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "IP manzilni ko'ring", ru: "Посмотрите IP-адрес" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Kompyuterlar manzilni qanday <span className="italic" style={{ color: T.accent }}>yozadi</span>?</>, ru: <>Как компьютеры <span className="italic" style={{ color: T.accent }}>записывают</span> адрес?</> })}</h2></div>
        ...
        <div className="split">
          <div className="col">
            <div className="frame frame-col fade-up delay-2" style={{ alignItems: "center", gap: 12 }}>
              ...
              {phase === "idle" ? <button className="btn" onClick={reveal}>{tr({ uz: "🔢 IP manzilni ko'rsatish", ru: "🔢 Показать IP-адрес" })}</button> : <span className="ipbox" style={{ opacity: phase === "run" ? 0.72 : 1 }}><span className="ip-type" key={phase === "done" ? "d" : "r"}>{phase === "done" ? "142.250.190.78" : scram}</span></span>}
...
var Screen7 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "DNS", ru: "DNS" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Domenni qidiring", ru: "Найдите домен" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Domen nomidan IP'ni kim <span className="italic" style={{ color: T.accent }}>topadi</span>?</>, ru: <>Кто <span className="italic" style={{ color: T.accent }}>находит</span> IP по доменному имени?</> })}</h2></div>
...
var Screen8 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Server", ru: "Сервер" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "So'rov yuboring", ru: "Отправьте запрос" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Sayt o'zi <span className="italic" style={{ color: T.accent }}>qayerda</span> yashaydi?</>, ru: <>А <span className="italic" style={{ color: T.accent }}>где</span> живёт сам сайт?</> })}</h2></div>
        ...
        <div className="split">
          <div className="col">
            <div className="cs fade-up delay-2">
              <div className="cs-node"><span className="cs-ic">🧑‍💻</span><span className="cs-l">{tr({ uz: <>Siz<br />(brauzer)</>, ru: <>Вы<br />(браузер)</> })}</span></div>
              <div className="cs-wire">
                <div className="cs-track">
                  <span className={`cs-chip cs-chip-req ${step >= 1 ? "sent" : ""} ${step >= 2 ? "gone" : ""}`}>{tr({ uz: "📨 so'rov", ru: "📨 запрос" })}</span>
                  <span className={`cs-chip cs-chip-res ${step >= 2 ? "sent" : ""}`}>{tr({ uz: "📄 sahifa", ru: "📄 страница" })}</span>
                </div>
              </div>
              <div className={`cs-node ${step >= 1 ? "cs-active" : ""} ${step === 1 ? "cs-proc" : ""}`}><span className="cs-ic">🖥️</span><span className="cs-l">{tr({ uz: <>Server<br />(sayt shu yerda)</>, ru: <>Сервер<br />(сайт здесь)</> })}</span></div>
            </div>
            <button className="btn" style={{ alignSelf: "flex-start" }} disabled={step === 1} onClick={send}>{step === 1 ? tr({ uz: "Yuborilmoqda…", ru: "Отправляется…" }) : done ? tr({ uz: "↻ Yana yuborish", ru: "↻ Отправить ещё" }) : tr({ uz: "📨 Serverga so'rov yuborish", ru: "📨 Отправить запрос серверу" })}</button>
          </div>
          <div className="col">            {done ? <div className="fade-step" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <div className="flow-label">{tr({ uz: "🖥️ Server qaytargan sahifa", ru: "🖥️ Страница, которую вернул сервер" })}</div>
              ...
              </div> : <div className="hint"><p className="body" style={{ margin: 0, color: T.ink2 }}>{tr({ uz: "Siz (brauzer) chap tomonda, server o'ngda. So'rov yuborib, javobni kuzating.", ru: "Вы (браузер) слева, сервер справа. Отправьте запрос и проследите за ответом." })}</p></div>}
...
var Screen10 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [step, setStep] = useState(storedAnswer ? PIPE.length - 1 : -1);
  const [playing, setPlaying] = useState(false);
  ...
  return <Stage eyebrow={tr({ uz: "So'rov yo'li", ru: "Путь запроса" })} screen={screen} audioState={audio} mentorCollapse navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Yo'lni ishga tushiring", ru: "Запустите путь" }} onClick={onNext} /></>}>
    ...
    /* Detal-karta — joriy bosqich jonli ko'rsatiladi */
          ...
          <div className="pl-detail">
            ...
            {(step === 2 || step === 3) && <div className="pl-codewrap fade-step">
                <div className="pl-codehead">{step === 2 ? tr({ uz: "📄 Server yuborgan HTML kod", ru: "📄 HTML-код, присланный сервером" }) : tr({ uz: "🌐 Brauzer kodni o'qiyapti…", ru: "🌐 Браузер читает код…" })}</div>
                <pre className={`pl-code ${step === 3 ? "reading" : ""}`}>
                  {PIPE_CODE.map((c, i) => <div key={i} className="pl-codeline">{tr(c)}</div>)}
                  {step === 3 && <span className="pl-scan" />}
                </pre>
              </div>}
            {step === 4 && <div className="pl-render fade-step">
                <div className="pl-codehead">{tr({ uz: "🖼️ Ekranda — tayyor sahifa", ru: "🖼️ На экране — готовая страница" })}</div>
    ...
    /* Har bosqichda Mentor o'sha qadamni tushuntiradi (pauzada — to'liq o'qishga to'xtaydi) */
          ...
          {playing && cur && <div className={`mentor pl-pausebubble fade-step ${paused ? "is-paused" : ""}`} key={step}>
              <div className="mentor-ava" aria-hidden="true"><img src={MENTOR_IMG} alt="" /></div>
              <div className="mentor-col">
                <span className="mentor-name">{paused ? `⏸ ${step + 1}/${PIPE.length} — ${tr({ uz: "o'qing, keyin «Davom etish»", ru: "прочитайте, потом «Продолжить»" })}` : `${tr({ uz: "Mentor", ru: "Ментор" })} · ${step + 1}/${PIPE.length} ${tr({ uz: "qadam", ru: "шаг" })}`}</span>
                <div className="mentor-msg body">{tr(cur.note)}</div>
              </div>
            </div>}

          <div className="pl-controls">
            {playing ? <button className="btn" onClick={togglePause} style={{ alignSelf: "center" }}>{paused ? tr({ uz: "▶ Davom etish", ru: "▶ Продолжить" }) : tr({ uz: "⏸ Pauza", ru: "⏸ Пауза" })}</button> : <button className="btn" onClick={start} style={{ alignSelf: "center" }}>{finished ? tr({ uz: "↻ Yana ko'rsatish", ru: "↻ Показать ещё раз" }) : tr({ uz: "▶ Yo'lni boshlash", ru: "▶ Начать путь" })}</button>}
          </div>
        </div>
        </Zoomable>
        {done && <div ref={doneRef} className="frame-success fade-step"><p className="small mono" style={{ margin: "0 0 4px", fontWeight: 600, color: T.success, textTransform: "uppercase", letterSpacing: "0.08em" }}>{tr({ uz: "✓ Mana butun yo'l", ru: "✓ Вот весь путь" })}</p><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: <>Siz <b>manzil</b> yozasiz → <b>DNS</b> IP topadi → <b>server</b> <b>HTML kodini</b> beradi → <b>brauzer</b> uni o'qib ekranga chizadi. Bularning bari — <b>bir soniyada</b>! Keyingi darsda brauzer HTML'ni qanday o'qishini ochamiz.</>, ru: <>Вы вводите <b>адрес</b> → <b>DNS</b> находит IP → <b>сервер</b> отдаёт <b>HTML-код</b> → <b>браузер</b> читает его и рисует на экране. И всё это — <b>за одну секунду</b>! На следующем уроке разберём, как браузер читает HTML.</> })}</p></div>}
      </div>
    </Stage>;
};
var YT_VIDEO = (title, views, dur) => <div style={{ display: "flex", gap: 10, alignItems: "flex-start", margin: "0 0 11px" }}>
    ...
    <div style={{ minWidth: 0, paddingTop: 1 }}>
      ...
      <div style={{ fontSize: 10.5, color: "#777", fontFamily: "'Manrope', sans-serif" }}>CoddyCamp · {views} {tr({ uz: "ko'rish", ru: "просмотров" })}</div>
    </div>
  </div>;
var Screen11 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [rd, setRd] = useState(storedAnswer ? HTML.length : 0);
  const timer = useRef(null);
  ...
  return <Stage eyebrow={tr({ uz: "Brauzer ichida", ru: "Внутри браузера" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Kodni sahifaga aylantiring", ru: "Превратите код в страницу" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Brauzer kodni qanday <span className="italic" style={{ color: T.accent }}>sahifaga</span> aylantiradi?</>, ru: <>Как браузер превращает код в <span className="italic" style={{ color: T.accent }}>страницу</span>?</> })}</h2></div>
    ...
    /* 12-darsni eslatuvchi "ichiga zoom" strip */
        ...
        <div className="zi fade-up delay-1">
          <div className="zi-mini">
            <div className="zi-node"><span className="zi-node-ic">🖥️</span><span className="zi-node-l">{tr({ uz: "Server", ru: "Сервер" })}</span></div>
            <div className="zi-wire"><span className="zi-pkt">📄</span></div>
            <div className="zi-node focus"><span className="zi-node-ic">🌐</span><span className="zi-node-l">{tr({ uz: "Brauzer", ru: "Браузер" })}</span><span className="zi-lens">🔍</span></div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "clamp(10px,1.6vw,16px)" }}>
          <div className="split" ref={animRef}>
            <div className="col">
              <div className="flow-label">{tr({ uz: "📥 Server qaytargan kod (HTML)", ru: "📥 Код, присланный сервером (HTML)" })}{reading && <span className="dns-status">{tr({ uz: "o'qilmoqda…", ru: "читается…" })}</span>}</div>
              <pre className="code-box fade-up delay-2">{HTML.map((h, i) => {
    const cls = i === active ? "reading" : i < rd ? "read" : phase === "idle" ? "" : "dim";
    return <div key={i} className={`htl-line ${cls}`}><span className="htl-code">{tr(h.code)}</span>{i < rd && <span className="htl-tick">{tr({ uz: "✓ chizildi", ru: "✓ нарисовано" })}</span>}</div>;
  })}</pre>
            </div>
            <div className="col" ref={resultRef}>
              <div className="flow-label">{tr({ uz: "🌐 Brauzer chizgan sahifa", ru: "🌐 Страница, нарисованная браузером" })}</div>
              <Preview title="youtube.com" minH={150}>
                {rd === 0 ? <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center" }}>{tr({ uz: "Tugmani bosing — brauzer kodni o'qib, YouTube sahifasini shu yerda chizadi", ru: "Нажмите кнопку — браузер прочитает код и нарисует страницу YouTube здесь" })}</p> : <div style={{ display: "block" }}>{HTML.slice(0, rd).map((h, i) => <div key={i} className="htl-pop">{tr(h.el)}</div>)}</div>}
              </Preview>
            </div>
          </div>
          <button className="btn" style={{ alignSelf: "center" }} onClick={start} disabled={reading}>{reading ? tr({ uz: "Brauzer o'qiyapti…", ru: "Браузер читает…" }) : done ? tr({ uz: "↻ Qaytadan", ru: "↻ Заново" }) : tr({ uz: "🖥️ Brauzer bilan o'qib chizish", ru: "🖥️ Прочитать и нарисовать браузером" })}</button>
        </div>
        </Zoomable>
        {done && <div className="frame-success fade-step"><p className="body" style={{ margin: 0, color: T.ink }}>{tr({ uz: <>✓ Chapdagi <b>kod</b> = o'ngdagi <b>YouTube sahifasi</b>. Server faqat HTML matn yuboradi, chiroyli sahifani esa <b>brauzer chizadi</b>. Aynan shunday HTML'ni keyingi modulda <b>o'zingiz yozasiz</b>!</>, ru: <>✓ <b>Код</b> слева = <b>страница YouTube</b> справа. Сервер шлёт лишь HTML-текст, а красивую страницу рисует <b>браузер</b>. Точно такой же HTML в следующем модуле вы <b>напишете сами</b>!</> })}</p></div>}
      </div>
    </Stage>;
};
  ...
  if (site.mock === "google") {
    return <div className="fade-step" style={{ textAlign: "center", padding: "18px 6px 14px" }}>
        ...
        <div style={{ display: "flex", gap: 9, justifyContent: "center", marginTop: 16 }}>
          {tr({ uz: ["Google Qidiruv", "Menga omad"], ru: ["Поиск Google", "Мне повезёт"] }).map((l) => <div key={l} style={{ background: "#f8f9fa", border: "1px solid #f1f1f1", borderRadius: 4, padding: "7px 12px", fontSize: 10, color: "#5f6368", fontFamily: "Manrope, sans-serif" }}>{l}</div>)}
        </div>
      </div>;
  }
  ...
  if (site.mock === "wikipedia") {
    return <div className="fade-step">
        <div style={{ display: "flex", alignItems: "center", gap: 9, borderBottom: "1px solid #eaecf0", paddingBottom: 8, marginBottom: 9 }}>
          ...
          <div><div style={{ fontFamily: "Georgia, serif", fontSize: 13, color: "#202122", lineHeight: 1.1 }}>WIKIPEDIA</div><div style={{ fontSize: 8, color: "#72777d" }}>{tr({ uz: "Erkin ensiklopediya", ru: "Свободная энциклопедия" })}</div></div>
        </div>
        <div style={{ fontFamily: "Georgia, serif", fontSize: 14, fontWeight: 700, color: "#000", marginBottom: 8 }}>{tr({ uz: "Maqola", ru: "Статья" })}</div>
...
var Screen13 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Amaliyot · so'rov yuboring", ru: "Практика · отправьте запрос" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "So'rov yuboring", ru: "Отправьте запрос" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>O'zingiz <span className="italic" style={{ color: T.accent }}>so'rov</span> yuboring.</>, ru: <>Отправьте <span className="italic" style={{ color: T.accent }}>запрос</span> сами.</> })}</h2></div>
        ...
        <div className="split">
          <div className="col">
            <div className="urlbar fade-up delay-2"><span className="urlbar-lock">🔒</span><input className="urlbar-input" value={url} placeholder={tr({ uz: "masalan: google.com", ru: "например: google.com" })} onChange={(e) => setUrl(e.target.value)} onKeyDown={(e) => {
    if (e.key === "Enter") send();
  }} /><button className="urlbar-go" onClick={() => send()} disabled={running}>{tr({ uz: "Yubor", ru: "Отправить" })}</button></div>
    ...
    return <div key={s.k} className={`jr-st ${on ? "on" : ""} ${delivered ? "delivered" : ""}`} style={jrPos(s.a)}>
          ...
          <div className="col" ref={resultRef}>
            ...
            <Preview title={loaded || tr({ uz: "sayt", ru: "сайт" })} minH={150}>
              {running ? <p className="mono" style={{ color: T.ink2, textAlign: "center", margin: 0 }}><span className="gen-line">{tr({ uz: "Yuklanmoqda", ru: "Загрузка" })}</span></p> : loaded ? <div className="fade-step"><SiteMock site={site} /><p className="mono small" style={{ color: T.success, margin: "10px 0 0", textAlign: "center" }}>✓ {site?.name || loaded} {tr({ uz: "ochildi", ru: "открылся" })}</p></div> : <p style={{ fontFamily: "Georgia, serif", color: T.ink3, fontStyle: "italic", margin: 0, textAlign: "center" }}>{tr({ uz: "Manzil yuboring — sayt shu yerda ochiladi", ru: "Отправьте адрес — сайт откроется здесь" })}</p>}
            </Preview>
          </div>
        </div>
        </Zoomable>
...
var Screen13b = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const go = (to) => {
    ...
    timer.current = setTimeout(() => {
      ...
      if (node.kind === "dns") {
        setHasIP(true);
        setMsg({ t: `${tr({ uz: "🗂️ DNS telefon kitobidan topdi", ru: "🗂️ DNS нашёл в телефонной книге" })}: ${target.domain} → ${target.ip}`, type: "good" });
      } else if (node.kind === "server") {
        if (!hasIP) return bounce(tr({ uz: "🔒 Qaysi server kerakligini bilmaysiz — avval IP-manzilni oling.", ru: "🔒 Вы не знаете, какой сервер нужен — сначала получите IP-адрес." }));
        if (node.ip === target.ip) {
          setHasPage(true);
          setMatchNode(to);
          setMsg({ t: tr({ uz: "🤝 IP mos keldi! Aynan shu server — sahifa olindi.", ru: "🤝 IP совпал! Это тот самый сервер — страница получена." }), type: "good" });
        } else if (node.name) return bounce(`${tr({ uz: "❌", ru: "❌" })} ${node.ip} — ${tr({ uz: `bu ${node.name}'niki, sizniki emas! (404)`, ru: `это ${node.name}, а не ваш! (404)` })}`);
        else return bounce(`❌ ${node.ip} — ${tr({ uz: "juda o'xshash, lekin boshqa kompyuter! Raqamni oxirigacha solishtiring.", ru: "очень похоже, но другой компьютер! Сравните номер до конца." })}`);
      } else if (node.kind === "screen") {
        if (!hasPage) return bounce(tr({ uz: "🖼️ Hali sahifa yo'q — uni to'g'ri serverdan olib keling.", ru: "🖼️ Страницы ещё нет — принесите её с правильного сервера." }));
      }
      setTraveled((tr2) => Array.from(/* @__PURE__ */ new Set([...tr2, ek(from, to)])));
  ...
  return <Stage eyebrow={tr({ uz: "Sinov · saytni top", ru: "Испытание · найди сайт" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext disabled={phase !== "win"} label={phase === "win" ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Saytni ekranga keltiring", ru: "Выведите сайт на экран" }} onClick={onNext} /></>}>
    ...
    return <button key={k} className={`net-node ${n.kind} ${isCur ? "cur" : ""} ${isVisited ? "visited" : ""} ${reachable ? "reachable" : ""} ${matchNode === k ? "match" : ""} ${shakeNode === k ? "shake" : ""}`} style={{ left: `${n.pos[0]}%`, top: `${n.pos[1]}%` }} onClick={() => go(k)} disabled={!reachable}>
                  ...
                  <span className="net-node-l">{tr(n.l)}</span>
                  {n.kind === "server" ? <span className="net-ip">{n.ip}</span> : <span className="net-node-note">{tr(n.note)}</span>}
                </button>;
  })}
    ...
    /* Xabar / maslahat + boshqaruv */
          ...
          {msg ? <p className={`net-msg ${msg.type}`} key={msg.t}>{msg.t}</p> : phase === "play" && <p className="net-hint">{hasIP ? tr({ uz: "📇 Kartangizdagi IP'ga MOS serverni tanlang — raqamlarni oxirigacha solishtiring!", ru: "📇 Выберите сервер, СОВПАДАЮЩИЙ с IP на вашей карточке — сравните номера до конца!" }) : tr({ uz: "🟢 yonayotgan DNS tugunini bosing", ru: "🟢 нажмите на подсвеченный узел DNS" })}</p>}
        ...
        {phase === "win" && <div ref={winRef} className="dlv-result fade-step">
            <Confetti />
            <p className="dlv-open-lbl">{tr({ uz: <>🎉 Topdingiz! Brauzer <b>{target.name}</b>'ni ekraningizda ochdi</>, ru: <>🎉 Нашли! Браузер открыл <b>{target.name}</b> на вашем экране</> })}</p>
...
var Screen14 = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  const [stage, setStage] = useState(storedAnswer ? "fixed" : "idle");
  ...
  return <Stage eyebrow={tr({ uz: "Debugging", ru: "Отладка" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : stage === "failed" ? { uz: "Endi tuzating", ru: "Теперь исправьте" } : { uz: "Ochib ko'ring", ru: "Попробуйте открыть" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>Sayt ochilmadi — <span className="italic" style={{ color: T.accent }}>nega</span>?</>, ru: <>Сайт не открылся — <span className="italic" style={{ color: T.accent }}>почему</span>?</> })}</h2></div>
...
var ScreenPodium = ({ screen, answers, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Natijalar", ru: "Результаты" })} screen={screen} narrow navContent={<><NavBack onPrev={onPrev} /><NavNext label={{ uz: "Davom etish", ru: "Продолжить" }} onClick={onNext} /></>}>
    ...
    return <div key={q} className="qstat-row">
              ...
              {live.mode === "mentor" && <p className="small" style={{ margin: "10px 0 0", color: T.ink2 }}>{tr({ uz: "⚠️ belgili savollar — sinf qiynalgan mavzular. Qayta tushuntirish tavsiya etiladi.", ru: "⚠️ отмеченные вопросы — темы, с которыми класс справился хуже. Рекомендуется объяснить заново." })}</p>}
            </div>
          </>}
      </div>
    </Stage>;
  ...
  return <div className="qz-arena">
    ...
    /* ===== LOBBY ===== */
      ...
      {phase === "lobby" && <div className="qz-view fade-step">
          <CsWordmark />
          <p className="qz-sub" style={{ marginTop: -4 }}>{tr({ uz: "Tezroq to'g'ri bossangiz — ko'proq ball. Ketma-ket to'g'ri javoblar 🔥 bonus beradi!", ru: "Чем быстрее верно ответите — тем больше баллов. Ответы подряд дают 🔥 бонус!" })}</p>
    ...
    return <button key={i} className={`qz-tile ${my ? pickedThis ? "picked" : "faded" : ""}`} style={{ background: QUIZ_COLORS[i] }} disabled={isMentor || !!my} onClick={() => answer(i)}>
                  ...
                  <span className="qz-opt">{tr(o)}</span>
    ...
    return <div key={i} className={`qz-tile rv ${win ? "win" : "lose"} ${pickedThis ? "picked" : ""}`} style={{ background: QUIZ_COLORS[i] }}>
          ...
          {!isMentor && <div className={`qz-res ${my?.correct ? "good" : "bad"}`}>
              {my?.correct ? <><span className="qz-res-pts">+{myPtsFor(qi)}</span><span className="qz-res-t">{tr({ uz: "ball", ru: "баллов" })}{streakUpTo(qi) >= 2 ? ` · 🔥 x${streakUpTo(qi)} streak` : ""}</span></> : <span className="qz-res-t">{my ? tr({ uz: "Adashdingiz — 0 ball. Keyingisida olasiz! 💪", ru: "Неверно — 0 баллов. В следующий раз получится! 💪" }) : tr({ uz: "Vaqt tugadi — 0 ball. Tezroq bo'ling! ⏱", ru: "Время вышло — 0 баллов. Будьте быстрее! ⏱" })}</span>}
...
var Screen16 = ({ screen, answers, achievements, onReset, onPrev, onFinish }) => {
  ...
  />
        ...
        {hwOpen && <div className="card hw fade-up d4"><div className="card-lbl" style={{ color: T.accent }}>{tr({ uz: "🔎 Uyga vazifa", ru: "🔎 Домашнее задание" })}</div><p className="body" style={{ margin: "0 0 10px", color: T.ink }}>{tr({ uz: "Internetni hayotda kuzating:", ru: "Понаблюдайте за интернетом в жизни:" })}</p><ul>{HOMEWORK2.map((h, i) => <li key={i}><b>{tr(h.b)}</b> <span className="t">{tr(h.t)}</span></li>)}</ul><p className="hw-note">{tr({ uz: "Javoblaringizni platformadagi vazifa-savoliga yozib yuborasiz.", ru: "Ответы отправите в задание на платформе." })}</p></div>}
    ...
    return <div key={id} className={`ach-badge ${got ? "got" : "locked"}`} title={tr(a.desc)}>
      ...
      {showDone && <div className="fin-backdrop" onClick={() => setShowDone(false)}>
          ...
          <div className="fin-card" onClick={(e) => e.stopPropagation()}>
            <button className="fin-x" onClick={() => setShowDone(false)} aria-label={tr({ uz: "Yopish", ru: "Закрыть" })}>✕</button>
            <div className="fin-emoji">🎉</div>
            <h3 className="fin-h">{tr({ uz: "Tabriklaymiz!", ru: "Поздравляем!" })}</h3>
            <p className="fin-sub">{tr({ uz: "Internet darsini muvaffaqiyatli tugatdingiz. Endi sayt sizgacha qanday yetib kelishini bilasiz.", ru: "Вы успешно завершили урок об интернете. Теперь вы знаете, как сайт доходит до вас." })}</p>
            <button className="btn-white-accent" onClick={onFinish} style={{ padding: "12px 32px", fontSize: 15 }}>{tr({ uz: "Tamom", ru: "Готово" })}</button>
          </div>
        </div>}
...
function DragDropOrder({ items, hints, onSolved }) {
  ...
  return <div className={`dd fade-up ${pristine ? "dd-hint-on" : ""}`}>
      ...
      <div className="dd-pool">
        {pool.length === 0 && !solved && <span className="dd-pool-empty">{tr({ uz: "Tartib xato — bo'lakni bosib qaytaring va qayta joylang", ru: "Порядок неверный — верните часть нажатием и разместите заново" })}</span>}
        {pool.map((id) => <button key={id} className="dd-chip" onPointerDown={(e) => down(e, id, "pool")}>{tr(byId[id].label)}</button>)}
      </div>
      {solved && <div className="dd-done">{tr({ uz: "✓ To'g'ri! So'rov aynan shu tartibda yuradi.", ru: "✓ Верно! Запрос идёт именно в таком порядке." })}</div>}
...
var ScreenReqOrder = ({ screen, storedAnswer, onAnswer, onNext, onPrev }) => {
  ...
  return <Stage eyebrow={tr({ uz: "Mustahkamlash", ru: "Закрепление" })} screen={screen} audioState={audio} navContent={<><NavBack onPrev={onPrev} /><NavNext optionalLive disabled={!done} label={done ? { uz: "Davom etish", ru: "Продолжить" } : { uz: "Tartibni yig'ing", ru: "Соберите порядок" }} onClick={onNext} /></>}>
      <div className="screen" style={{ gap: "clamp(10px,1.6vw,16px)" }}>
        <div className="head"><h2 className="title h-title fade-up">{tr({ uz: <>So'rov qanday <span className="italic" style={{ color: T.accent }}>tartibda</span> yuradi?</>, ru: <>В каком <span className="italic" style={{ color: T.accent }}>порядке</span> идёт запрос?</> })}</h2></div>
...
function Flashcards({ cards }) {
  ...
  if (!card) return <div className="fc-done fade-up"><span className="fc-done-emoji">🎉</span><p className="fc-done-h">{tr({ uz: "Hammasini bilasiz!", ru: "Вы знаете всё!" })}</p><p className="fc-done-s">{total}/{total} {tr({ uz: "atama yodlandi", ru: "терминов запомнено" })}</p><button className="fc-btn ghost" onClick={restart}>{tr({ uz: "↻ Qaytadan takrorlash", ru: "↻ Повторить заново" })}</button></div>;
      ...
      <div className="tg-call" style={callStyle}>
        ...
        <div className="tg-nav">
          ...
          <div className="tg-navr">
            ...
            <button className="tg-btn go" onClick={next}>{last ? tr({ uz: "Boshladik!", ru: "Начали!" }) : tr({ uz: "Keyingisi →", ru: "Дальше →" })}</button>
          </div>
        </div>
      </div>
    </div>;
...
function HtmlLesson({ lang: langProp, onFinished, liveToken }) {
  ...
  const [lmsTok, setLmsTok] = useState(() => lmsTokenPeek(liveToken));
  useEffect(() => {
    setLmsTok(lmsTokenPeek(liveToken));
  }, [liveToken]);
  __lang = lang;
