...
Talablar:
1. let ism = "..." — ismingiz, qo'shtirnoq ichida (matn)
2. let yosh = ... — yoshingiz, qo'shtirnoqsiz (raqam)
3. const tugilgan_yil = ... — tug'ilgan yilingiz (bu quti qulflangan: keyin o'zgarmaydi)
4. Yana bitta quti — o'zingiz tanlang (masalan: shahar)
...
2) let bilan raqam-quti bor, qiymati qo'shtirnoqsiz (yosh);
...
- Yetishmagan talabni kod-atamasi bilan ayting (masalan: «yosh qo'shtirnoq ichida qolgan — raqam qo'shtirnoqsiz yoziladi»).
...
Требования:
1. let ism = "..." — ваше имя в кавычках (текст)
2. let yosh = ... — ваш возраст без кавычек (число)
3. const tugilgan_yil = ... — год рождения (эта коробка заперта: потом не меняется)
4. Ещё одна коробка — на ваш выбор (например: город)
...
- Недостающее требование называйте кодовым термином (например: «возраст остался в кавычках — число пишется без кавычек»).
